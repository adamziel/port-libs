...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,343 pass / 0 fail | [3,302 / 2,276 (145.1%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Keep ODF/ODT ship-ready while continuing broader Pandoc input closures | json-native-citation-affix-payloads-85e342e |
...
| --- | --- | --- | ---: | ---: | --- |
| Markdown/CommonMark/GFM | `commonmark`, `commonmark_x`, `gfm`, `markdown`, `markdown_github`, `markdown_mmd`, `markdown_phpextra`, `markdown_strict` | partial | 451 | 1,096 | Generic raw HTML serialization, HTML list item attributes, math attribute round-trip, escaped fenced-Div attributes, fenced Div section-reference boundaries, nested-list fixture round-trip, definition-list term-group handoff, table-cell note placement, and header section Div mapping are covered; complete extension and variant parity. |
...
Current-main counters are reconciled through the JSON/native citation affix payload slice rebased on `85e342e`: 3,343 PHP passes, 0 failures, and 3,302 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax check for `PandocJsonNativeAstTest.php`, focused `PandocJsonNativeAstTest.php` (`1` file, `2195` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `75286` assertions, `0` failures).
...
| JSON/native table, list, citation, and block handoff | Landed citation affix payload, table-caption, note-label, Div Plain, mixed block-container, task-list sidecar, definition-list term-group, and nullary helper payload slices | 252 JSON/native artifacts | 53 JSON/native cases; latest focused JSON/native run 2,195 assertions | Partial | Broader native/json fixture parity, unsupported constructors, and table/citation/metadata round trips. |
| Markdown block/list/note/section boundaries | Landed generic raw HTML serialization, HTML list item attributes, math attributes, nested fenced Div, escaped fenced-Div attributes, fenced Div section-reference boundaries, mixed-content nested-list, definition-list term-group, table-cell note placement, and header section Div slices | 1,096 Markdown-family rows | 451 Markdown-family cases; latest focused NativeReader/MarkdownReader run 7,059 assertions | Partial | Markdown/CommonMark/GFM extension and variant parity. |
...
Verdict: not shippable yet. The native PHP lane now has 53 local passing JSON/native AST evidence cases against 252 upstream native expected artifacts (21.0% evidence ratio). Broader native/json fixture parity, unsupported constructor surfaces, and table/citation/metadata round-trip edges remain open.

| Format focus | Upstream denominator | Local passing evidence | Evidence percent | Remaining critical gap |
| --- | ---: | ---: | ---: | --- |
| JSON/native AST constructors and round trips | 252 | 53 | 21.0% | Broader upstream native/json fixture parity plus unsupported constructor/table/citation/metadata surfaces beyond bounded citation affix sidecars, sidecar reuse, Markdown fixture writer handoff, note label sidecars, task-list sidecars, and nullary helper payload validation. |
...
Implemented follow-up gap: `PandocJsonNativeAstTest.php` now covers citation prefix and suffix inline sidecar payloads on JSON and native readers/writers, preserving unchanged `citationPrefix`/`citationSuffix` payloads while regenerating edited affixes without stale record or inline sidecars. Verification passed `php -l` for `PandocJsonNativeAstTest.php`, focused `PandocJsonNativeAstTest.php` (`1` file, `2195` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `75286` assertions, `0` failures). No Pandoc binary, JSON filters, Cabal/Haskell runners, browser renderer, external validator, online service, live provider test, or live-service provider test was invoked.
...
| Local passing numerator | `lane-status.json` reports 3,343 PHP passes / 0 failures, and `UPSTREAM_TEST_MANIFEST.json` reports 3,302 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,302 / 2,276 = 145.1%; percentages above 100% reflect local PHP slices being more granular than upstream inventory rows. | High coverage, but not global ship-ready. |
...
(`1` file, `2195` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`45` files, `75286` assertions, `0` failures after final rebase onto base `85e342e`).
