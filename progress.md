...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,294 pass / 0 fail | [3,254 / 2,276 (143.0%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Broaden citation reader parity after bounded RIS parsing | citation-ris-parser-59ea77a447 |
...
slice, EPUB3 NCX document metadata provenance slice, XML/HTML/JATS front-matter slice, and RIS citation parser slice.
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json`, focused `CitationCslProcessorTest.php`, and `php tools/run-tests.php lanes/pandoc/tests` passed. | 44 test files, 73,964 assertions, 0 failures. |
...
`php tools/run-tests.php lanes/pandoc/tests/CitationCslProcessorTest.php` (`1` file,
`5308` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`44` files, `73964` assertions, `0` failures on current main `59ea77a447`).
