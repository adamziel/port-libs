...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,342 pass / 0 fail | [3,301 / 2,276 (145.0%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Finish broader CSV/TSV reader parity beyond bounded registry/profile slices | tabular-data-registry-options-f4c55b0 |
...
| HTML/XML/JATS DOM | `html` partial; `xml`, `jats`, `bits` unsupported | mixed | 277 | 29 | JATS/BITS front-matter, back-matter reference identifier review packets, and explicit XML/JATS/BITS direct-reader capability diagnostics are covered; finish HTML5 tree construction and implement full XML/JATS/BITS readers. |
...
Current-main counters are reconciled through the JATS/BITS reference identifier diagnostics slice after tabular data registry option profiles and ODF compact manifest custom attributes: 3,343 PHP passes, 0 failures, and 3,302 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for `XmlHtmlDom.php` and `XmlHtmlDomTest.php`, focused `XmlHtmlDomTest.php` (`1` file, `2031` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `75412` assertions, `0` failures).
...
### XML/JATS/BITS Reference Identifier Diagnostics Update (2026-06-13)

Bounded native PHP XmlHtmlDom coverage advanced by one JATS/BITS back-matter reference identifier slice after the reference text-policy and author/date review packets. `XmlHtmlDom::summarizeJatsFrontMatter()` now exposes safe DOI/PMID/PMCID/ISBN/URI-like identifier summaries, source element/provenance buckets, duplicate and missing identifier diagnostics, and resolved `bibr` xref target identifier linkage while preserving `directReaderParity=false` and blocking full citation text payloads.

Verification passed `php -l` for `XmlHtmlDom.php` and `XmlHtmlDomTest.php`; focused `XmlHtmlDomTest.php` passed (`1` file, `2031` assertions, `0` failures); full `lanes/pandoc/tests` passed (`45` files, `75412` assertions, `0` failures). No Pandoc, XML validators, browser renderers, Node tooling, online services, live provider tests, or live-service provider tests were invoked.
...
| Local passing numerator | `lane-status.json` reports 3,342 PHP passes / 0 failures, and `UPSTREAM_TEST_MANIFEST.json` reports 3,301 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,301 / 2,276 = 145.0%; percentages above 100% reflect local PHP slices being more granular than upstream inventory rows. | High coverage, but not global ship-ready. |
...
status, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/src/PandocFormatRegistry.php lanes/pandoc/tests/PandocFormatRegistryTest.php`,
...
(`1` file, `1425` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`45` files, `75224` assertions, `0` failures after final rebase onto base `f4c55b05f`).
