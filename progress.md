...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,309 pass / 0 fail | [3,269 / 2,276 (143.6%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Markdown fenced Div section references are covered; continue block-structure parity | markdown-fenced-div-section-boundaries-d3ed225156 |
...
| --- | --- | --- | ---: | ---: | --- |
| Markdown/CommonMark/GFM | `commonmark`, `commonmark_x`, `gfm`, `markdown`, `markdown_github`, `markdown_mmd`, `markdown_phpextra`, `markdown_strict` | partial | 444 | 1,096 | Fenced Div section-reference boundaries and nested-list fixture round-trip are covered; complete extension and variant parity. |
...
### Markdown Fenced Div Section Reference Boundary Update (2026-06-13)

Bounded native PHP Markdown writer handoff advanced by one block-structure slice. `MarkdownWriter` now honors `referenceLocation=end_of_section` while rendering fenced Div block collections, so note and reference definitions produced inside a fenced section are flushed before the next nested heading and before the closing fence instead of leaking to the document tail.

Verification passed `php -l` for `MarkdownWriter.php` and `MarkdownReaderTest.php`; focused `MarkdownReaderTest.php` passed (`1` file, `6628` assertions, `0` failures); full `lanes/pandoc/tests` passed (`45` files, `74217` assertions, `0` failures). No Pandoc, Cabal/Haskell runner, browser renderer, TeX engine, Node tooling, office suite, online service, live provider, or external validator was invoked.

Remaining block-structure gaps: broader Markdown/CommonMark/GFM fenced Div and section variant parity, nested section boundaries beyond fenced-section reference placement, header-to-section mapping, and unrelated JSON/native AST/table/citation/metadata edges remain open.
...
| Format focus | Upstream denominator | Local passing evidence | Evidence percent | Remaining critical gap |
| --- | ---: | ---: | ---: | --- |
| Markdown/CommonMark/GFM variants | 1,096 | 444 | 40.5% | Fenced Div section-reference boundaries and nested-list fixture round-trip are covered; complete extension and variant parity. |
...
slice, EPUB3 NCX document metadata provenance slice, XML/HTML/JATS front-matter slice, RIS citation parser slice, JSON/native target tuple sidecar slice, plain writer table caption row slice, PDF/Typst package dependency policy slice, Markdown/WordPress raw HTML boundary slice, JSON/native table caption block writer slice, MediaBag linked-resource handoff slice, Markdown adjacent-list separator slice, Markdown/WordPress footnote label anchor slice, Markdown nested fenced Div slice, ODT configuration package metadata slice, Markdown fixture nested-list round-trip slice, CSV/TSV direct text reader slice, JSON/native note label sidecar slice, and Markdown fenced Div section-reference boundary slice.
...
(`1` file, `6628` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`45` files, `74217` assertions, `0` failures on current main `d3ed225156`).
