...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,372 pass / 0 fail | [3,332 / 2,276 (146.4%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue HTML/XML/JATS tree-construction and direct-reader parity diagnostics | xml-html-template-noscript-boundary-738b8851bc |
...
| HTML/XML/JATS DOM | `html`, `xml`, `jats`, and `bits` partial | partial | 288 | 29 | Object association provenance, image-map association diagnostics, XML/HTML5 template/noscript boundary handling, XML/HTML5 nested table foster-parenting, XML/JATS/BITS direct input registry routing, JATS/BITS relationship diagnostics, inline xref local back-reference and unsupported target diagnostics, title metadata propagation, front-matter, body, table-body, and back-matter reference diagnostics review packets, and DocBook structure and section metadata review packets are covered; finish HTML5 tree construction and full XML/JATS/BITS reader parity. |
...
Current-main counters are reconciled through the XML/HTML5 DOM template/noscript boundary slice rebased on `738b8851bc`: 3,372 PHP passes, 0 failures, and 3,332 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for `XmlHtmlDom.php`, `Html5DomTest.php`, and `XmlHtmlDomTest.php`, focused `Html5DomTest.php` plus `XmlHtmlDomTest.php` (`2` files, `2448` assertions, `0` failures), and full `lanes/pandoc/tests` (`46` files, `76182` assertions, `0` failures).
...
diagnostics slice, and XML/HTML5 DOM template/noscript boundary slice.
...
| Local passing numerator | `lane-status.json` reports 3,372 PHP passes / 0 failures, and `UPSTREAM_TEST_MANIFEST.json` reports 3,332 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,332 / 2,276 = 146.4%; percentages above 100% reflect local PHP slices being more granular than upstream inventory rows. | High coverage, but not global ship-ready. |
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check`, syntax checks for `XmlHtmlDom.php`, `Html5DomTest.php`, and `XmlHtmlDomTest.php`, focused `Html5DomTest.php` plus `XmlHtmlDomTest.php`, and `php tools/run-tests.php lanes/pandoc/tests` passed. | Focused: 2 files, 2,448 assertions, 0 failures. Full: 46 files, 76,182 assertions, 0 failures. |
...
status, `git diff --check -- progress.md PANDOC_STATUS.md porting.html porting-summary.json lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/src/XmlHtmlDom.php lanes/pandoc/tests/Html5DomTest.php lanes/pandoc/tests/XmlHtmlDomTest.php`,
`php -l lanes/pandoc/src/XmlHtmlDom.php`,
`php -l lanes/pandoc/tests/Html5DomTest.php`,
`php -l lanes/pandoc/tests/XmlHtmlDomTest.php`,
`php tools/run-tests.php lanes/pandoc/tests/Html5DomTest.php lanes/pandoc/tests/XmlHtmlDomTest.php`
(`2` files, `2448` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`46` files, `76182` assertions, `0` failures after final rebase onto base `738b8851bc`).
