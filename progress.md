...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,330 pass / 0 fail | [3,289 / 2,276 (144.5%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue non-HTML table body-group semantics after PlainWriter body groups | plain-writer-table-body-groups-0072af8b |
...
Current-main counters are reconciled through the PlainWriter table body-group slice rebased on `0072af8b00`: 3,330 PHP passes, 0 failures, and 3,289 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for the touched PlainWriter files, focused `PlainWriterTest.php` (`1` file, `223` assertions, `0` failures), focused `TableGeometryTest.php` (`1` file, `1904` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74766` assertions, `0` failures).
...
| Local passing numerator | `lane-status.json` reports 3,330 PHP passes / 0 failures, and `UPSTREAM_TEST_MANIFEST.json` reports 3,289 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,289 / 2,276 = 144.5%; percentages above 100% reflect local PHP slices being more granular than upstream inventory rows. | High coverage, but not global ship-ready. |
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/src/PlainWriter.php lanes/pandoc/tests/PlainWriterTest.php`, syntax checks for `PlainWriter.php` and `PlainWriterTest.php`, focused `PlainWriterTest.php`, focused `TableGeometryTest.php`, and `php tools/run-tests.php lanes/pandoc/tests` passed. | 45 test files, 74,766 assertions, 0 failures. |
...
status, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/src/PlainWriter.php lanes/pandoc/tests/PlainWriterTest.php`,
...
(`1` file, `223` assertions, `0` failures),
`php tools/run-tests.php lanes/pandoc/tests/TableGeometryTest.php`
(`1` file, `1904` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`45` files, `74766` assertions, `0` failures on current main `0072af8b00`).
