...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,362 pass / 0 fail | [3,322 / 2,276 (146.0%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue DOCX/OpenXML package-reader parity after note/comment relationship orphan backlink diagnostics | docx-note-relationship-orphan-backlinks-3a32430 |
...
| HTML/XML/JATS DOM | `html` partial; `xml`, `jats`, `bits` unsupported | mixed | 281 | 29 | Object association provenance, JATS/BITS front-matter, body, table-body, and back-matter reference diagnostics review packets, DocBook structure and section metadata review packets, and explicit XML/JATS/BITS direct-reader capability diagnostics are covered; finish HTML5 tree construction and implement full XML/JATS/BITS readers. |
...
| DOCX/OpenXML | `docx` | partial | 95 | 35 | Finish remaining direct WordprocessingML/package reader parity; section-property review metadata, subdocument diagnostics, and note/comment relationship diagnostics, including orphan/backlink reconciliation, are covered. |
...
Current-main counters are reconciled through the DOCX/OpenXML note/comment relationship orphan backlink diagnostics slice rebased on `3a324301`: 3,362 PHP passes, 0 failures, and 3,322 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for `DocxOpenXmlReader.php` and `DocxOpenXmlReaderTest.php`, focused `DocxOpenXmlReaderTest.php` (`1` file, `2522` assertions, `0` failures), and full `lanes/pandoc/tests` (`46` files, `75807` assertions, `0` failures).
...
| DOCX/OpenXML package reader | Landed subdocument and note/comment relationship diagnostics, including orphan/backlink reconciliation | 35 DOCX/OpenXML rows | 95 DOCX/OpenXML cases; latest focused run 2,522 assertions | Partial | Broader WordprocessingML/package reader parity, fields, content controls, revisions, lists, tables, and package edge cases. |
...
### DOCX/OpenXML Note Relationship Orphan Backlink Update (2026-06-13)

Bounded native PHP DOCX/OpenXML package-reader coverage advanced by one note/comment relationship follow-up slice after JATS/BITS table body diagnostics. `DocxOpenXmlReader` now reconciles footnote, endnote, and comment relationship references with per-item backlinks, orphaned relationship IDs, internal/external target collision groups, repeated target suffix provenance, missing target/content-type issue ordering, and preserved existing relationship summary fields plus package diagnostics.

Verification passed `php -l` for `DocxOpenXmlReader.php` and `DocxOpenXmlReaderTest.php`; `jq empty` for `lanes/pandoc/lane-status.json` and `lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`; `git diff --check`; focused `DocxOpenXmlReaderTest.php` passed (`1` file, `2522` assertions, `0` failures); full `lanes/pandoc/tests` passed (`46` files, `75768` assertions, `0` failures). No Pandoc, Word, LibreOffice, office suite, zip/unzip, ZipArchive, browser renderer, Node tooling, online service, live provider, or external validator was invoked.
...
slice, JATS/BITS table body diagnostics slice, and JATS/BITS back-matter
reference diagnostics slice.
...
| Local passing numerator | `lane-status.json` reports 3,361 PHP passes / 0 failures, and `UPSTREAM_TEST_MANIFEST.json` reports 3,321 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,321 / 2,276 = 145.9%; percentages above 100% reflect local PHP slices being more granular than upstream inventory rows. | High coverage, but not global ship-ready. |
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check`, syntax checks for `XmlHtmlDom.php` and `XmlHtmlDomTest.php`, focused `XmlHtmlDomTest.php`, related DOM/table tests, and `php tools/run-tests.php lanes/pandoc/tests` passed. | Focused: 1 file, 2,073 assertions, 0 failures. Related: 4 files, 10,918 assertions, 0 failures. Full: 46 files, 75,768 assertions, 0 failures. |
