...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,322 pass / 0 fail | [3,282 / 2,276 (144.2%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | CSV/TSV format inference diagnostics covered; full reader parity remains partial | csv-tsv-format-inference-76690ec857 |
...
| Tabular/data readers | `csv`, `tsv` | partial | 6 | 2 | Header-option parity, tabular registry option profiles, and auto format inference diagnostics are covered; finish full CSV/TSV reader parity beyond bounded option and inference slices. |
...
Current-main counters are reconciled through `76690ec857`: 3,322 PHP passes, 0 failures, and 3,282 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for the touched CSV/TSV reader and registry files, focused `DelimitedTextReaderTest.php` plus `PandocFormatRegistryTest.php` (`2` files, `1173` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74467` assertions, `0` failures).

| Surface | Evidence state | Upstream denominator | Local passing numerator | Ship verdict | Remaining critical gaps |
| --- | --- | ---: | ---: | --- | --- |
| CSV/TSV direct readers | Landed option and inference diagnostics | 2 CSV command fixtures | 6 local mapped reader/diagnostic cases; latest focused reader/registry run 1,173 assertions | Partial after bounded reader, no-header, registry, and inference slices | Broader CSV/TSV upstream fixture hydration, option behavior, malformed input diagnostics, and table-reader edge cases. |
...
### CSV/TSV Reader Options And Inference Update (2026-06-13)

Verdict: partial, not ship-ready. The native PHP lane now has 6 local mapped CSV/TSV reader and diagnostic cases against the 2 CSV command fixture rows recorded in the accepted static upstream inventory (`test/command/01.csv` and `test/command/3533-rst-csv-tables.csv`).

Implemented gaps: `DelimitedTextReader` parses bounded headed CSV and TSV input into shared table AST nodes, supports `header=false` / no-header imports with ordered diagnostics, pads ragged rows, attaches `delimitedText`, `tableGeometry`, and `formatInference` review packets, and proves Markdown, WordPress table, and Pandoc JSON table export. `readAuto()` and `format=auto` infer `csv` or `tsv` from `extension` / `sourcePath` metadata first, then content row-profile scoring, emitting `delimited-text-format-inferred` or `delimited-text-format-defaulted` diagnostics. `PandocFormatRegistry` now records tabular data option profiles without claiming full reader parity.

Verification passed `php -l` for `DelimitedTextReader.php`, `DelimitedTextReaderTest.php`, `PandocFormatRegistry.php`, and `PandocFormatRegistryTest.php`; focused `DelimitedTextReaderTest.php` plus `PandocFormatRegistryTest.php` passed (`2` files, `1173` assertions, `0` failures); full `lanes/pandoc/tests` passed (`45` files, `74467` assertions, `0` failures). No Pandoc binary, spreadsheet application, browser renderer, Node tooling, online validator, online service, live provider test, or external validator was invoked.

Remaining critical gap: finish full Pandoc CSV/TSV reader parity, including broader upstream fixture hydration, option behavior beyond the bounded no-header slice, malformed input diagnostics, and table-reader edge cases. Overall input-token gaps remain 18 partial input tokens and 31 unsupported input tokens.
...
slice, EPUB3 NCX document metadata provenance slice, XML/HTML/JATS front-matter slice, RIS citation parser slice, JSON/native target tuple sidecar slice, plain writer table caption row slice, PDF/Typst package dependency policy slice, Markdown/WordPress raw HTML boundary slice, JSON/native table caption block writer slice, MediaBag linked-resource handoff slice, Markdown adjacent-list separator slice, Markdown/WordPress footnote label anchor slice, Markdown nested fenced Div slice, ODT configuration package metadata slice, Markdown fixture nested-list round-trip slice, CSV/TSV direct text reader slice, CSV/TSV header-option parity slice, tabular registry option-profile slice, CSV/TSV format inference diagnostics slice, JSON/native note label sidecar slice, LaTeX table-foot longtable writer slice, JSON/native Div Plain block-boundary slice, PDF/Typst package dependency conflict policy slice, JSON/native task-list checkbox sidecar slice, JSON/native nullary helper payload validation slice, LaTeX table body-head writer slice, Markdown fenced Div escaped-attribute slice, and Markdown/WordPress footnote backlink slice.
...
| Local passing numerator | `lane-status.json` reports 3,322 PHP passes / 0 failures, and `UPSTREAM_TEST_MANIFEST.json` reports 3,282 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,282 / 2,276 = 144.2%; percentages above 100% reflect local PHP slices being more granular than upstream inventory rows. | High coverage, but not global ship-ready. |
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/src/DelimitedTextReader.php lanes/pandoc/tests/DelimitedTextReaderTest.php lanes/pandoc/src/PandocFormatRegistry.php lanes/pandoc/tests/PandocFormatRegistryTest.php`, syntax checks for the touched CSV/TSV reader and registry files, focused `DelimitedTextReaderTest.php` plus `PandocFormatRegistryTest.php`, and `php tools/run-tests.php lanes/pandoc/tests` passed. | 45 test files, 74,467 assertions, 0 failures. |
...
status, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/src/DelimitedTextReader.php lanes/pandoc/tests/DelimitedTextReaderTest.php lanes/pandoc/src/PandocFormatRegistry.php lanes/pandoc/tests/PandocFormatRegistryTest.php`,
...
(`2` files, `1173` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`45` files, `74467` assertions, `0` failures on current main `76690ec857`).
...
application, office suite, TeX/Typst engine, browser engine, Node tooling,
online validator, or external validator was invoked.
