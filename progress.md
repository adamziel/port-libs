...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,335 pass / 0 fail | [3,294 / 2,276 (144.7%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Finish broader CSV/TSV option and reader parity beyond headed/no-header table slices | csv-tsv-header-options-48c0f451 |
...
Current-main counters are reconciled through the CSV/TSV header option parity slice rebased on `48c0f4517a`: 3,335 PHP passes, 0 failures, and 3,294 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for the touched CSV/TSV reader and registry files, focused `DelimitedTextReaderTest.php` plus `PandocFormatRegistryTest.php` (`2` files, `1410` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `75039` assertions, `0` failures).
...
| Local passing numerator | `lane-status.json` reports 3,335 PHP passes / 0 failures, and `UPSTREAM_TEST_MANIFEST.json` reports 3,294 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,294 / 2,276 = 144.7%; percentages above 100% reflect local PHP slices being more granular than upstream inventory rows. | High coverage, but not global ship-ready. |
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check`, syntax checks for `DelimitedTextReader.php`, `DelimitedTextReaderTest.php`, `PandocFormatRegistry.php`, and `PandocFormatRegistryTest.php`, focused `DelimitedTextReaderTest.php` plus `PandocFormatRegistryTest.php`, and `php tools/run-tests.php lanes/pandoc/tests` passed. | Focused: 2 files, 1,410 assertions, 0 failures. Full: 45 files, 75,039 assertions, 0 failures. |
...
status, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/notes/csv-tsv-header-options-20260613.md lanes/pandoc/src/DelimitedTextReader.php lanes/pandoc/tests/DelimitedTextReaderTest.php`,
...
(`2` files, `1410` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`45` files, `75039` assertions, `0` failures after final rebase onto base `48c0f4517a`).
