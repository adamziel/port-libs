...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,311 pass / 0 fail | [3,271 / 2,276 (143.7%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue Markdown/CommonMark/GFM fenced Div/section variant parity after header-section mapping | markdown-header-section-divs-d099006810 |
...
| --- | --- | --- | ---: | ---: | --- |
| Markdown/CommonMark/GFM | `commonmark`, `commonmark_x`, `gfm`, `markdown`, `markdown_github`, `markdown_mmd`, `markdown_phpextra`, `markdown_strict` | partial | 444 | 1,096 | Header-to-section Div mapping and nested-list fixture round-trip are covered; complete extension and variant parity. |
...
Branch counters are reconciled on base `d099006810`: 3,311 PHP passes, 0 failures, and 3,271 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This slice was checked with syntax checks for `MarkdownReader.php` and `MarkdownReaderTest.php`, focused `MarkdownReaderTest.php` (`1` file, `6643` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74231` assertions, `0` failures).
...
| Markdown block boundaries | Landed nested fenced Div, mixed-content nested-list, and header-section Div slices | 1,096 Markdown-family rows | 444 Markdown-family cases; latest focused Markdown run 6,643 assertions | Partial | Markdown/CommonMark/GFM extension and variant parity. |
...
| Format focus | Upstream denominator | Local passing evidence | Evidence percent | Remaining critical gap |
| --- | ---: | ---: | ---: | --- |
| Markdown/CommonMark/GFM variants | 1,096 | 444 | 40.5% | Header-to-section Div mapping and nested-list fixture round-trip are covered; complete extension and variant parity. |
...
slice, EPUB3 NCX document metadata provenance slice, XML/HTML/JATS front-matter slice, RIS citation parser slice, JSON/native target tuple sidecar slice, plain writer table caption row slice, PDF/Typst package dependency policy slice, Markdown/WordPress raw HTML boundary slice, JSON/native table caption block writer slice, MediaBag linked-resource handoff slice, Markdown adjacent-list separator slice, Markdown/WordPress footnote label anchor slice, Markdown nested fenced Div slice, ODT configuration package metadata slice, Markdown fixture nested-list round-trip slice, CSV/TSV direct text reader slice, JSON/native note label sidecar slice, LaTeX table-foot longtable writer slice, JSON/native Div Plain block-boundary slice, and Markdown header-to-section Div mapping slice.
...
(`1` file, `6643` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`45` files, `74231` assertions, `0` failures on current main `d099006810`).
