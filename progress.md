...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,337 pass / 0 fail | [3,296 / 2,276 (144.8%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue bounded IPYNB reader diagnostics and broader CSV/TSV, package, and AST parity slices | ipynb-attachment-manifest-diagnostics-52a27eda |
...
| IPYNB/notebook | 2 local cases / skipped | Upstream Pandoc input token intentionally skipped for this phase. | Attachment manifest/collision diagnostics are covered; broader notebook reader/writer parity remains outside this burn-down. |

### Closure-Wave Evidence Snapshot (2026-06-13)

Current-main counters are reconciled through the IPYNB attachment manifest diagnostics slice rebased on `52a27eda2b`: 3,337 PHP passes, 0 failures, and 3,296 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for the touched IPYNB reader and test files, focused `IpynbReaderTest.php` (`1` file, `71` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `75073` assertions, `0` failures).

| Surface | Evidence state | Upstream denominator | Local passing numerator | Ship verdict | Remaining critical gaps |
| --- | --- | ---: | ---: | --- | --- |
| IPYNB/notebook diagnostics | Landed metadata-only attachment manifest and collision diagnostics | skipped for input burn-down | 2 local IPYNB cases; latest focused run 71 assertions | Partial adjacent evidence | Broader notebook media extraction, rich output rendering, and writer parity remain. |
...
| Percent | 3,295 / 2,276 = 144.8%; percentages above 100% reflect local PHP slices being more granular than upstream inventory rows. | High coverage, but not global ship-ready. |
