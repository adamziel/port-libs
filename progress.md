...
| --- | --- | --- | --- | ---: | --- |
| 1 | gitoxide | stopped | targeted object/ref inventory plus pack data slice | 12% | Map OFS_DELTA/REF_DELTA header resolution and delta application for packed blobs. |
...
- Latest root suite: `php tools/run-tests.php` passes 22 test files, 393 assertions, 0 failures.
...
- Gitoxide now has a targeted object/ref static inventory, native tree parsing, native loose direct/symbolic reference parsing/storage, native packed-ref header/reference/peeled lookup parsing, loose+packed reference-store overlay resolution, v2 pack-index parsing/lookup, and pack data non-delta entry decoding. There is still no upstream Cargo runner parity, and delta resolution remains unported. Full Cargo was not executed because `.upstream-cache/gitoxide` is sparse/no-checkout and a runner attempt would require materializing crate source plus building Rust dependencies; full workspace `cargo test` would hydrate/build beyond the current VM cap.
