...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,355 pass / 0 fail | [3,315 / 2,276 (145.7%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue table/native constructor parity after JSON/native span edit sidecar preservation | json-native-table-span-edit-sidecars-eb56c4b |
...
Current-main counters are reconciled through the JSON/native table span edit sidecar slice rebased on `eb56c4b88e`: 3,355 PHP passes, 0 failures, and 3,315 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for `PandocJsonNativeAstTest.php`, focused `PandocJsonNativeAstTest.php` (`1` file, `2253` assertions, `0` failures), and full `lanes/pandoc/tests` (`46` files, `75613` assertions, `0` failures).
...
| JSON/native table, list, raw, and block handoff | Landed table span edit sidecars, raw HTML alias preservation, table-caption, table-attribute, mixed table caption/cell flushing, note-label, Div Plain, mixed block-container, task-list sidecar, definition-list term-group, and nullary helper payload slices | 252 JSON/native artifacts | 56 JSON/native cases; latest focused `PandocJsonNativeAstTest.php` run 2,253 assertions | Partial | Broader native/json fixture parity, unsupported constructors, link/raw payloads around block containers, and table/citation/metadata round trips. |
...
### JSON/Native Table Span Edit Sidecar Update (2026-06-13)

Bounded native PHP JSON/native table-constructor coverage advanced by one span edit sidecar slice after LaTeX labelled note anchor preservation. `PandocJsonNativeAstTest.php` now reads tagged table `Cell`, `Attr`, `RowSpan`, and `ColSpan` payloads through both JSON and native readers, edits the spanning cell attributes plus row/column spans, and verifies `PandocJsonWriter` and `NativeWriter` regenerate the stale edited payloads instead of reusing source sidecars.

The fixture also proves unchanged alignment, row-head, column helper, and neighboring cell sidecars survive the edit. Verification passed `php -l` for `PandocJsonNativeAstTest.php`; focused `PandocJsonNativeAstTest.php` passed (`1` file, `2253` assertions, `0` failures); full `lanes/pandoc/tests` passed (`46` files, `75613` assertions, `0` failures). No Pandoc binary, JSON filters, Cabal/Haskell runners, Node tooling, browser renderer, online service, live provider test, live-service provider test, or external validator was invoked.

JSON/native evidence is now 56 local cases against the 252 accepted static upstream JSON/native artifacts. JSON/native remains partial; broader native/json fixture parity, unsupported constructors, table/citation/metadata round trips, and raw fallback diagnostics remain open.
...
Verdict: not shippable yet. The native PHP lane now has 56 local passing JSON/native AST evidence cases against 252 upstream native expected artifacts (22.2% evidence ratio). Broader native/json fixture parity, unsupported constructor surfaces, and table/citation/metadata/raw round-trip edges remain open.

| Format focus | Upstream denominator | Local passing evidence | Evidence percent | Remaining critical gap |
| --- | ---: | ---: | ---: | --- |
| JSON/native AST constructors and round trips | 252 | 56 | 22.2% | Broader upstream native/json fixture parity plus unsupported constructor/table/citation/metadata/raw surfaces beyond bounded table span edit sidecars, raw HTML aliases, sidecar reuse, Markdown fixture writer handoff, note label sidecars, task-list sidecars, and nullary helper payload validation. |
...
commit history agree on the current shipping call after the JSON/native table
span edit sidecar slice, LaTeX labelled note anchor preservation slice,
JSON/native raw HTML alias preservation slice, PDF/Typst package unsupported-reason slice, JATS/BITS body
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check`, syntax checks for `PandocJsonNativeAstTest.php`, focused `PandocJsonNativeAstTest.php`, and `php tools/run-tests.php lanes/pandoc/tests` passed. | Focused: 1 file, 2,253 assertions, 0 failures. Full: 46 files, 75,613 assertions, 0 failures. |
...
(`1` file, `2253` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`46` files, `75613` assertions, `0` failures after final rebase onto base `eb56c4b88e`).
