...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,298 pass / 0 fail | [3,258 / 2,276 (143.1%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Broaden JSON/native constructor parity after note label sidecars | json-native-note-label-sidecars-b266b6e53c |
...
| JSON/native AST | `json`, `native` | partial | 46 | 252 | Link/Image target tuple sidecars and Note `noteLabel` sidecars are preserved; complete broader JSON/native AST constructor coverage. |
...
slice, EPUB3 NCX document metadata provenance slice, XML/HTML/JATS front-matter slice, RIS citation parser slice, JSON/native target tuple sidecar slice, plain writer table caption row slice, PDF/Typst package dependency policy slice, and JSON/native note label sidecar slice.
...
| Local passing numerator | `lane-status.json` reports 3,298 PHP passes / 0 failures, and `UPSTREAM_TEST_MANIFEST.json` reports 3,258 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,258 / 2,276 = 143.1%; percentages above 100% reflect local PHP slices being more granular than upstream inventory rows. | High coverage, but not global ship-ready. |
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json`, focused `PandocJsonNativeAstTest.php`, and `php tools/run-tests.php lanes/pandoc/tests` passed. | 44 test files, 74,021 assertions, 0 failures. |
...
`php tools/run-tests.php lanes/pandoc/tests/PandocJsonNativeAstTest.php` (`1` file,
`1978` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`44` files, `74021` assertions, `0` failures on current main `b266b6e53c`).
