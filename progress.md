...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,324 pass / 0 fail | [3,283 / 2,276 (144.2%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue notes/endnote placement after table-cell note coverage | markdown-table-cell-note-placement-6c32f56c |
...
| --- | --- | --- | ---: | ---: | --- |
| Markdown/CommonMark/GFM | `commonmark`, `commonmark_x`, `gfm`, `markdown`, `markdown_github`, `markdown_mmd`, `markdown_phpextra`, `markdown_strict` | partial | 447 | 1,096 | Escaped fenced-Div attributes, fenced Div section-reference boundaries, nested-list fixture round-trip, definition-list term-group handoff, and table-cell note placement are covered; complete extension and variant parity. |
...
Current-main counters are reconciled through `6c32f56cfc`: 3,324 PHP passes, 0 failures, and 3,283 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax check for the touched Markdown test file, focused `MarkdownReaderTest.php` (`1` file, `6668` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74604` assertions, `0` failures).
...
| Markdown block/list/note boundaries | Landed nested fenced Div, escaped fenced-Div attributes, fenced Div section-reference boundaries, mixed-content nested-list, definition-list term-group, and table-cell note placement slices | 1,096 Markdown-family rows | 447 Markdown-family cases; latest focused MarkdownReader run 6,668 assertions | Partial | Markdown/CommonMark/GFM extension and variant parity. |
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/tests/MarkdownReaderTest.php`, syntax check for `MarkdownReaderTest.php`, focused `MarkdownReaderTest.php`, and `php tools/run-tests.php lanes/pandoc/tests` passed. | 45 test files, 74,604 assertions, 0 failures. |
...
status, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/tests/MarkdownReaderTest.php`,
`php -l lanes/pandoc/tests/MarkdownReaderTest.php`,
`php tools/run-tests.php lanes/pandoc/tests/MarkdownReaderTest.php`
(`1` file, `6668` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`45` files, `74604` assertions, `0` failures on current main `6c32f56cfc`).
