...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,375 pass / 0 fail | [3,335 / 2,276 (146.5%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue EPUB package reader parity after NCX hierarchy diagnostics | epub-ncx-hierarchy-diagnostics-1f2129da33 |
...
Current-main counters are reconciled through the EPUB NCX hierarchy diagnostics slice rebased on `1f2129da33`: 3,375 PHP passes, 0 failures, and 3,335 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for `EpubPackageReader.php` and `EpubPackageReaderTest.php`, focused `EpubPackageReaderTest.php` (`1` file, `261` assertions, `0` failures), and full `lanes/pandoc/tests` (`46` files, `76366` assertions, `0` failures).
