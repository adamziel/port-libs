...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,433 pass / 0 fail | [3,385 / 2,276 (148.7%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue JSON/native AST parity after mixed metadata container stress coverage | json-native-mixed-metadata-containers |
...
Current-main counters are reconciled through the JSON/native mixed metadata container stress slice rebased on `c023ab6322`: 3,433 PHP passes, 0 failures, and 3,385 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax check for `PandocJsonNativeAstTest.php`, focused `PandocJsonNativeAstTest.php` (`1` file, `3014` assertions, `0` failures), and full `lanes/pandoc/tests` (`46` files, `79751` assertions, `0` failures).
