...
| --- | --- | --- | --- | ---: | --- |
| 1 | gitoxide | stopped | targeted object/ref inventory plus loose ref slice | 8% | Map packed-ref fixture parsing and lookup rules from `gix-ref`, starting with without-header and unsorted fixtures. |
...
- Latest root suite: `php tools/run-tests.php` passes 18 test files, 299 assertions, 0 failures.
...
- Gitoxide now has a targeted object/ref static inventory, native tree parsing, and native loose direct/symbolic reference parsing/storage. There is still no upstream Cargo runner parity, and packed refs remain the next focused gap. Full Cargo was not executed because `.upstream-cache/gitoxide` is sparse/no-checkout and a runner attempt would require materializing crate source plus building Rust dependencies; full workspace `cargo test` would hydrate/build beyond the current VM cap.
