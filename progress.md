...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,319 pass / 0 fail | [3,278 / 2,276 (144.0%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Broaden JSON/native constructor parity after nested metadata payloads | json-native-nested-meta-payloads-4487923aa2 |
...
| JSON/native AST | `json`, `native` | partial | 51 | 252 | Nested metadata payloads, nullary helper payload validation, task-list checkbox sidecars, note label sidecars, and fixture writer handoff are preserved; complete broader JSON/native AST constructor coverage. |
...
Current-main counters are reconciled through `4487923aa2` plus this slice: 3,319 PHP passes, 0 failures, and 3,278 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for the touched JSON/native files, focused `PandocJsonNativeAstTest.php` (`1` file, `2137` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74512` assertions, `0` failures).
...
| JSON/native table, list, block, and metadata handoff | Landed table-caption, note-label, Div Plain, task-list sidecar, nullary helper payload, and nested metadata payload slices | 252 JSON/native artifacts | 51 JSON/native cases; latest focused run 2,137 assertions | Partial | Broader native/json fixture parity, unsupported constructors, and table/citation/metadata round trips. |
...
### JSON/Native Nested Metadata Payload Update (2026-06-13)

Bounded native PHP JSON/native metadata handoff advanced by one nested
constructor sidecar slice. `PandocJsonWriter` and `NativeWriter` now use
reader-indexed `metaConstructorProvenance` paths while recursively emitting
metadata maps and lists, preserving unchanged nested `MetaString` and
`MetaBool` sidecars, including escaped metadata keys, while edited list items
and edited containers regenerate canonical constructors and drop stale sidecars.

Verification passed `php -l` for `PandocJsonWriter.php`, `NativeWriter.php`,
and `PandocJsonNativeAstTest.php`; focused `PandocJsonNativeAstTest.php`
passed (`1` file, `2137` assertions, `0` failures); full `lanes/pandoc/tests`
passed (`45` files, `74512` assertions, `0` failures). No Pandoc binary, JSON
filters, Cabal/Haskell runner, browser renderer, Node tooling, online service,
live provider, or external validator was invoked.
...
Verdict: not shippable yet. The native PHP lane now has 51 local passing JSON/native AST evidence cases against 252 upstream native expected artifacts (20.2% evidence ratio). Broader native/json fixture parity, unsupported constructor surfaces, and table/citation/metadata round-trip edges remain open.

| Format focus | Upstream denominator | Local passing evidence | Evidence percent | Remaining critical gap |
| --- | ---: | ---: | ---: | --- |
| JSON/native AST constructors and round trips | 252 | 51 | 20.2% | Broader upstream native/json fixture parity plus unsupported constructor/table/citation/metadata surfaces beyond bounded sidecar reuse, Markdown fixture writer handoff, note label sidecars, task-list sidecars, nullary helper payload validation, and nested metadata payloads. |
...
Implemented current gap: `PandocJsonWriter` and `NativeWriter` now preserve unchanged nested metadata constructor payloads when edited `MetaMap` and `MetaList` containers are regenerated from reader-indexed `metaConstructorProvenance`. Verification passed `php -l` for `PandocJsonWriter.php`, `NativeWriter.php`, and `PandocJsonNativeAstTest.php`; focused `PandocJsonNativeAstTest.php` (`1` file, `2137` assertions, `0` failures); and full `lanes/pandoc/tests` (`45` files, `74512` assertions, `0` failures). No Pandoc binary, JSON filters, Cabal/Haskell runners, browser renderer, external validator, online service, live provider test, or live-service provider test was invoked.
...
| Local passing numerator | `lane-status.json` reports 3,319 PHP passes / 0 failures, and `UPSTREAM_TEST_MANIFEST.json` reports 3,278 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,278 / 2,276 = 144.0%; percentages above 100% reflect local PHP slices being more granular than upstream inventory rows. | High coverage, but not global ship-ready. |
...
status, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/src/PandocJsonWriter.php lanes/pandoc/src/NativeWriter.php lanes/pandoc/tests/PandocJsonNativeAstTest.php lanes/pandoc/notes/pandoc-json-native-nested-meta-payloads-20260613.md`,
`php -l lanes/pandoc/src/PandocJsonWriter.php`,
`php -l lanes/pandoc/src/NativeWriter.php`,
`php -l lanes/pandoc/tests/PandocJsonNativeAstTest.php`,
`php tools/run-tests.php lanes/pandoc/tests/PandocJsonNativeAstTest.php`
(`1` file, `2137` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`45` files, `74512` assertions, `0` failures on current main `4487923aa2`).
