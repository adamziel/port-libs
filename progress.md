...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,338 pass / 0 fail | [3,297 / 2,276 (144.9%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Finish broader DOCX/OpenXML WordprocessingML reader parity and package edge cases | docx-note-relationship-diagnostics-4fe1d3a |
...
Current-main counters are reconciled through the DOCX note/comment relationship diagnostics slice rebased on `4fe1d3a12e`: 3,338 PHP passes, 0 failures, and 3,297 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for `DocxOpenXmlReader.php` and `DocxOpenXmlReaderTest.php`, focused `DocxOpenXmlReaderTest.php` (`1` file, `2483` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `75121` assertions, `0` failures).
...
| Local passing numerator | `lane-status.json` reports 3,338 PHP passes / 0 failures, and `UPSTREAM_TEST_MANIFEST.json` reports 3,297 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,297 / 2,276 = 144.9%; percentages above 100% reflect local PHP slices being more granular than upstream inventory rows. | High coverage, but not global ship-ready. |
...
status, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/notes/docx-openxml-note-comment-relationship-diagnostics-20260613T0150Z.md lanes/pandoc/src/DocxOpenXmlReader.php lanes/pandoc/tests/DocxOpenXmlReaderTest.php`,
`php -l lanes/pandoc/src/DocxOpenXmlReader.php`,
`php -l lanes/pandoc/tests/DocxOpenXmlReaderTest.php`,
`php tools/run-tests.php lanes/pandoc/tests/DocxOpenXmlReaderTest.php`
(`1` file, `2483` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`45` files, `75121` assertions, `0` failures after final rebase onto base `4fe1d3a12e`).
