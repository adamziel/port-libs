...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,361 pass / 0 fail | [3,320 / 2,276 (145.9%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue package-ingestion parity after ODF selected entry handoff | odf-selected-entry-handoff |
...
| HTML/XML/JATS DOM | `html` partial; `xml`, `jats`, `bits` unsupported | mixed | 280 | 29 | Object association provenance, JATS/BITS front-matter, body, and table-body diagnostics review packets, DocBook structure and section metadata review packets, and explicit XML/JATS/BITS direct-reader capability diagnostics are covered; finish HTML5 tree construction and implement full XML/JATS/BITS readers. |
...
| ODF/ODT/OpenDocument | `odt` | ship-ready | 52 | 20 | 0 critical gaps for native PHP ODT import; selected package entry handoff and compact manifest custom attribute provenance are covered. Continue only non-critical hardening slices as discovered. |
...
Current-main counters are reconciled through the ODF selected entry handoff provenance slice after JATS/BITS table body diagnostics: 3,361 PHP passes, 0 failures, and 3,320 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for `OdfReader.php` and `OdfReaderTest.php`, focused ODF/ODT tests (`5` files, `6015` assertions, `0` failures), and full `lanes/pandoc/tests` (`46` files, `75778` assertions, `0` failures).
...
| ODF/ODT package reader | Landed selected package entry handoff preflight while preserving ship-ready closure | 20 ODF/ODT rows | 52 ODF/ODT cases; latest focused ODF/ODT gate 6,015 assertions | Ship-ready | 0 critical ODF/ODT gaps for native PHP ODT import. |
...
slice, JATS/BITS table body diagnostics slice, and ODF selected entry handoff
provenance slice.
...
| Local passing numerator | `lane-status.json` reports 3,361 PHP passes / 0 failures, and `UPSTREAM_TEST_MANIFEST.json` reports 3,320 mapped upstream cases. | PHP lane remains green. |
...
| Verification | `jq empty lanes/pandoc/lane-status.json`, `git diff --check`, syntax checks for `OdfReader.php` and `OdfReaderTest.php`, focused ODF/ODT tests, and `php tools/run-tests.php lanes/pandoc/tests` passed. | Focused: 5 files, 6,015 assertions, 0 failures. Full: 46 files, 75,778 assertions, 0 failures. |
