...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,337 pass / 0 fail | [3,296 / 2,276 (144.8%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue XML/JATS/BITS direct-reader closure | jats-bits-section-metadata-diagnostics |
...
| HTML/XML/JATS DOM | `html` partial; `xml`, `jats`, `bits` unsupported | mixed | 277 | 29 | JATS/BITS front-matter review packets, section metadata diagnostics, and explicit XML/JATS/BITS direct-reader capability diagnostics are covered; finish HTML5 tree construction and implement full XML/JATS/BITS readers. |
...
Current-branch counters are reconciled through the JATS/BITS section metadata diagnostics slice on top of Typst package source-class policy provenance at `52a27eda2b`: 3,337 PHP passes, 0 failures, and 3,296 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for the touched XML DOM files, focused `XmlHtmlDomTest.php` (`1` file, `1878` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `75080` assertions, `0` failures).
...
| XML/JATS/BITS direct reader capability | Landed explicit unsupported direct-reader capability diagnostics for `xml`, `jats`, and `bits`, plus JATS/BITS section id/label/language/title diagnostics | 29 XML/HTML/JATS/DocBook rows | 277 XML/HTML/JATS/DocBook cases; latest focused XML run 1,878 assertions | Unsupported for full direct readers | Implement full XML/JATS/BITS body, back matter, tables, figures, references, citations, and AST parity. |
...
### JATS/BITS Section Metadata Diagnostics Update (2026-06-13)

Bounded native PHP XML/JATS/BITS review coverage advanced by one section metadata diagnostic slice after shared ZIP/OPC selected-entry role buckets. `XmlHtmlDom::summarizeJatsFrontMatter()` now reports section ids, labels, languages, duplicate ids, missing direct section titles, duplicate direct section titles, and an aggregate direct-reader diagnostic while preserving `directReaderParity=false`.

Verification passed `php -l` for `XmlHtmlDom.php` and `XmlHtmlDomTest.php`; focused `XmlHtmlDomTest.php` passed (`1` file, `1878` assertions, `0` failures); full `lanes/pandoc/tests` passed (`45` files, `75080` assertions, `0` failures). No Pandoc, Cabal/Haskell runner, browser renderer, Node tooling, external XML validator, online service, live provider test, or live-service provider test was invoked.
...
(`1` file, `1878` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`45` files, `75080` assertions, `0` failures).
