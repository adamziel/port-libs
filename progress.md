...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,356 pass / 0 fail | [3,316 / 2,276 (145.7%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue CSV/TSV option and malformed-input parity after row repair provenance | csv-tsv-row-repair-provenance-eb56c4b88e |
...
| Tabular/data readers | `csv`, `tsv` | partial | 11 | 2 | Headed and no-header table option handling, auto format inference diagnostics, relaxed row repair provenance, registry option profiles, extension inference, direction buckets, and source provenance are covered; finish broader CSV/TSV reader parity beyond the bounded table slices. |
...
Current-main counters are reconciled through the CSV/TSV row repair provenance slice rebased on `eb56c4b88e`: 3,356 PHP passes, 0 failures, and 3,316 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for `DelimitedTextReader.php` and `DelimitedTextReaderTest.php`, focused `DelimitedTextReaderTest.php` plus `PandocFormatRegistryTest.php` (`2` files, `1569` assertions, `0` failures), and full `lanes/pandoc/tests` (`46` files, `75617` assertions, `0` failures).

| Surface | Evidence state | Upstream denominator | Local passing numerator | Ship verdict | Remaining critical gaps |
| --- | --- | ---: | ---: | --- | --- |
| CSV/TSV direct readers and registry options | Landed headed/no-header option parity, auto CSV/TSV format inference diagnostics, relaxed row repair provenance, tabular registry extension inference, direction buckets, source provenance, and option profiles | 2 CSV command fixtures plus registry/options harness slices | 11 local CSV/TSV/registry cases; latest focused reader/registry run 1,569 assertions | Partial after bounded reader/profile/inference/repair slices | Broader CSV/TSV option behavior, malformed input diagnostics, multiline-cell behavior, and table-reader edge cases. |
...
### CSV/TSV Row Repair Provenance Update (2026-06-13)

Bounded native PHP CSV/TSV reader coverage advanced by two row repair provenance cases after PDF/Typst package unsupported-reason reporting. `DelimitedTextReader` review packets now expose source row indexes, skipped blank rows, trailing empty field rows, strict row-width mismatch diagnostics, `rowWidthSummary`, and `rowRepairSummary` original/repaired column counts for relaxed padding in CSV and TSV modes.

Verification passed `php -l` for `DelimitedTextReader.php` and `DelimitedTextReaderTest.php`; focused `DelimitedTextReaderTest.php` passed (`1` file, `144` assertions, `0` failures); focused `DelimitedTextReaderTest.php` plus `PandocFormatRegistryTest.php` passed (`2` files, `1569` assertions, `0` failures); full `lanes/pandoc/tests` passed (`46` files, `75617` assertions, `0` failures). No Pandoc binary, spreadsheet application, browser renderer, Node tooling, online validator, online service, live provider test, or external validator was invoked.

CSV/TSV/registry evidence is now 11 local mapped cases against the 2 accepted static upstream CSV command fixture rows. CSV/TSV remains partial; broader upstream fixture hydration, option behavior, malformed input diagnostics, multiline-cell behavior, and table-reader edge cases remain open.
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check`, syntax checks for `LatexWriter.php` and `LatexWriterTest.php`, focused `LatexWriterTest.php`, and `php tools/run-tests.php lanes/pandoc/tests` passed. | Focused: 1 file, 22 assertions, 0 failures. Full: 46 files, 75,549 assertions, 0 failures. |
...
status, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/src/LatexWriter.php lanes/pandoc/tests/LatexWriterTest.php`,
...
(`1` file, `22` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`46` files, `75549` assertions, `0` failures after final rebase onto base `025c076f77`).
