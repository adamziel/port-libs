...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,378 pass / 0 fail | [3,338 / 2,276 (146.7%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue XML/JATS/BITS reader parity after figure metadata diagnostics | xml-jats-bits-figure-metadata-diagnostics-78e0f6c918 |
...
| EPUB/EPUB3 | `epub` | partial | 64 | 9 | Direct manifest suffix diagnostics, skipped spine-entry reporting, XHTML definition-list handoff, XHTML table-section review metadata, compact nav/NCX label provenance, and NCX hierarchy diagnostics are covered; finish broader EPUB package reader parity. |
...
Current-main counters are reconciled through the XML/JATS/BITS figure metadata diagnostics slice rebased on `78e0f6c918`: 3,378 PHP passes, 0 failures, and 3,338 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for `XmlHtmlDom.php` and `XmlHtmlDomTest.php`, focused `XmlHtmlDomTest.php` (`1` file, `2369` assertions, `0` failures), and full `lanes/pandoc/tests` (`46` files, `76451` assertions, `0` failures).
...
| JSON/native table, list, raw, block, metadata, and figure handoff | Landed Figure child block payload preservation, nested metadata payload preservation, raw HTML alias preservation, table-caption, table-attribute, mixed table caption/cell flushing, note-label, Div Plain, mixed block-container, mixed Figure link/raw/code handoff, task-list sidecar, definition-list term-group, and nullary helper payload slices | 252 JSON/native artifacts | 58 JSON/native cases; latest focused `PandocJsonNativeAstTest.php` run 2,293 assertions | Partial | Broader native/json fixture parity, unsupported constructors, and table/citation/metadata round trips. |
...
| HTML/XML/JATS DOM diagnostics | Landed image-map association diagnostics for resolved, missing, duplicate, invalid, and unreferenced map states plus area shape/coords geometry, invalid coordinate diagnostics, and default-area precedence; template/noscript raw-text boundary handling; nested table row-level and row-group foster-parenting; partial direct input registry routing for `xml`, `jats`, and `bits` through `XmlHtmlDom`; unsupported direct-reader parity reason serialization; standalone object association provenance; JATS/BITS relationshipDiagnostics for figure/table/reference xref targets, inline xref local back-reference and unsupported citation-target diagnostics, titleMetadata/subtitleMetadata, section title paths, body roots, section hierarchy, xref resolution, references, figures, figure label/caption/title metadata counts and missing-metadata diagnostics, table-wraps, table body/row/cell metadata, back-matter ref-lists, reference metadata, citation xref targets, resolved/missing citation ids, unreferenced buckets, BITS book-part body metadata, and DocBook section metadata diagnostics | 29 XML/HTML/JATS/DocBook rows | 291 XML/HTML/JATS/DocBook cases; latest focused `XmlHtmlDomTest.php` run 2,369 assertions | Partial after bounded registry and review diagnostics | Implement full HTML5 tree construction and XML/JATS/BITS body/back matter, tables, figures, references, citations, and AST parity. |
...
XML/HTML/JATS/DocBook DOM evidence is now 291 local cases against the 29 accepted static upstream XML/HTML/JATS/DocBook rows. HTML5 tree construction remains partial; broader insertion-mode, malformed content, image-map semantics, and full XML/JATS/BITS direct reader parity remain open.
