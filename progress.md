...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,365 pass / 0 fail | [3,325 / 2,276 (146.1%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue rare text diagnostics or implement native rare text readers | pandoc-rare-text-registry-unsupported-3d3e4b |
...
Current-main counters are reconciled through the rare text-format registry unsupported diagnostics slice rebased on `3d3e4b`: 3,365 PHP passes, 0 failures, and 3,325 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for `PandocFormatRegistry.php` and `PandocFormatRegistryTest.php`, focused `PandocFormatRegistryTest.php` (`1` file, `1595` assertions, `0` failures), and full `lanes/pandoc/tests` (`46` files, `75989` assertions, `0` failures).
...
slice, and rare text-format registry unsupported diagnostics slice.
...
| Local passing numerator | `lane-status.json` reports 3,365 PHP passes / 0 failures, and `UPSTREAM_TEST_MANIFEST.json` reports 3,325 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,325 / 2,276 = 146.1%; percentages above 100% reflect local PHP slices being more granular than upstream inventory rows. | High coverage, but not global ship-ready. |
...
status, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/src/PandocFormatRegistry.php lanes/pandoc/tests/PandocFormatRegistryTest.php lanes/pandoc/notes/pandoc-rare-text-registry-unsupported-20260613.md`,
...
(`1` file, `1595` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`46` files, `75989` assertions, `0` failures after final rebase onto base `3d3e4b`).
