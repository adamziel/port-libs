...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,300 pass / 0 fail | [3,260 / 2,276 (143.2%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Broaden JSON/native constructor parity after task-list sidecars | json-native-task-list-sidecars-5cd9038b4c |
...
| JSON/native AST | `json`, `native` | partial | 46 | 252 | Task-list checkbox sidecars, table caption block writers, and Link/Image target tuple sidecars are preserved; complete broader JSON/native AST constructor coverage. |
...
| Local passing numerator | `lane-status.json` reports 3,300 PHP passes / 0 failures, and `UPSTREAM_TEST_MANIFEST.json` reports 3,260 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,260 / 2,276 = 143.2%; percentages above 100% reflect local PHP slices being more granular than upstream inventory rows. | High coverage, but not global ship-ready. |
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json`, focused `PandocJsonNativeAstTest.php`, and `php tools/run-tests.php lanes/pandoc/tests` passed. | 44 test files, 74,047 assertions, 0 failures. |
...
`1996` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`44` files, `74047` assertions, `0` failures on current main `5cd9038b4c`).
