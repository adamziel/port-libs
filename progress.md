...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,322 pass / 0 fail | [3,281 / 2,276 (144.2%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue bounded XML/HTML5 DOM gaps or EPUB3 package reader parity | xml-html-image-map-association-d1e41a7720 |
...
| HTML/XML/JATS DOM | `html` partial; `xml`, `jats`, `bits` unsupported | mixed | 276 | 29 | JATS/BITS front-matter and image-map association diagnostics are covered; finish HTML5 tree construction and implement full XML/JATS/BITS readers. |
...
Current-branch counters are reconciled on base `d1e41a7720`: 3,322 PHP passes, 0 failures, and 3,281 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for the touched XML/HTML DOM files, focused `XmlHtmlDomTest.php` (`1` file, `1853` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74538` assertions, `0` failures).
...
| XML/HTML5 image-map associations | Landed | 1 mapped XML/HTML DOM slice | 1 focused `XmlHtmlDom` case, 25 assertions | Covered bounded reviewer diagnostics | Full HTML5 tree-construction parity and direct XML/JATS/BITS readers remain open. |
...
### XML/HTML5 Image Map Association Update (2026-06-13)

Bounded native PHP XML/HTML5 DOM reviewer coverage advanced by one image-map association slice. `XmlHtmlDom` now reports resolved, missing, duplicate, invalid, and unreferenced `img usemap`/`map name` states on image and map summaries, including matched area counts plus href/label rollups while preserving deterministic HTML serialization.

Verification passed `php -l` for `XmlHtmlDom.php` and `XmlHtmlDomTest.php`; focused `XmlHtmlDomTest.php` passed (`1` file, `1853` assertions, `0` failures); full `lanes/pandoc/tests` passed (`45` files, `74510` assertions, `0` failures). No Pandoc binary, Cabal/Haskell runner, browser renderer, online sanitizer, external validator, online service, live provider, or live-service provider test was invoked.
...
(`1` file, `1853` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`45` files, `74538` assertions, `0` failures on base `d1e41a7720`).
