...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,359 pass / 0 fail | [3,319 / 2,276 (145.8%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue format closure after MediaBag repair diagnostics | media-bag-repair-conflicts |
...
Current-main counters are reconciled through the MediaBag repair conflict diagnostics slice after IPYNB attachment media diagnostics: 3,359 PHP passes, 0 failures, and 3,319 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for `MediaBag.php` and `MediaBagTest.php`, focused `MediaBagTest.php` (`1` file, `188` assertions, `0` failures), and full `lanes/pandoc/tests` (`46` files, `75684` assertions, `0` failures).
...
| Media linked-resource handoff | Landed linked-resource loading, MIME inference, repair conflict diagnostics, case-folded extraction disambiguation, content-type conflict reporting, and stable canonical/original/repaired path provenance | 3 mapped cross-format resource slices | 3 focused `MediaBag` cases; latest focused run 188 assertions | Covered bounded handoff; not an input-format ship gate | Wider media/resource edge cases outside opt-in linked-resource handoff. |
...
### MediaBag Repair Conflict Diagnostics Update (2026-06-13)

Bounded native PHP MediaBag coverage advanced by one repair conflict diagnostics slice after IPYNB attachment media diagnostics. `MediaBag` now reports conflicting repaired resource candidates, percent-decoded repair conflicts, case-folded extraction path collisions, source extension/content-type disagreement, duplicate linked-resource MIME-group conflicts, and stable canonical/original/repaired path provenance on mapped media nodes.

Verification passed `php -l` for `MediaBag.php` and `MediaBagTest.php`; focused `MediaBagTest.php` passed (`1` file, `188` assertions, `0` failures); full `lanes/pandoc/tests` passed (`46` files, `75684` assertions, `0` failures). No Pandoc binary, Cabal/Haskell runner, browser renderer, Node tooling, online service, live provider test, live-service provider test, or external validator was invoked.

MediaBag linked-resource evidence is now 3 local mapped resource slices. The broader media/resource surface remains partial for duplicate resource provenance beyond this bounded conflict slice, package-local path normalization, external/missing diagnostics, and writer handoff consistency edges.
...
IPYNB attachment media diagnostics slice, and MediaBag repair conflict
diagnostics slice.
...
status, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/src/MediaBag.php lanes/pandoc/tests/MediaBagTest.php`,
...
(`1` file, `188` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`46` files, `75684` assertions, `0` failures).
