...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,381 pass / 0 fail | [3,340 / 2,276 (146.7%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue DocBook reader parity beyond bounded list/table metadata | docbook-list-metadata-d3bb5c6e |
...
| DocBook/list/table geometry | `docbook` | partial | 19 | 16 | DocBook list metadata/diagnostics, structural review, and section metadata diagnostics are covered; finish DocBook XML reader parity, body conversion, inline/block/reference/bibliography mapping, generated AST parity, and broader fixture hydration. |
...
Current-main counters are reconciled through the DocBook list block metadata slice rebased on `d3bb5c6e`: 3,381 PHP passes, 0 failures, and 3,340 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for `MarkdownReader.php` and `MarkdownReaderTest.php`, focused `MarkdownReaderTest.php` (`1` file, `6766` assertions, `0` failures), and full `lanes/pandoc/tests` (`46` files, `76608` assertions, `0` failures).
...
| DocBook structural, section, and list metadata diagnostics | Landed review-only DocBook 4/5 structure packets, root/section title provenance, and bounded `itemizedlist`, `orderedlist`, nested list, `variablelist`, `xml:id`, and unsupported `listitem` child diagnostic handoff | 16 DocBook/table geometry rows | 19 local DocBook/list/table cases; latest focused `MarkdownReaderTest.php` run 6,766 assertions | Partial after bounded review diagnostics and list metadata | Full DocBook body conversion, inline/block/reference/bibliography mapping, generated AST parity, broader fixture hydration, and figure/media/admonition conversion remain open. |
