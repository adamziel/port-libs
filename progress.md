...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,332 pass / 0 fail | [3,291 / 2,276 (144.6%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue XML/JATS/BITS direct reader parity after ref-list diagnostics | jats-bits-ref-list-bibliography-plib-yis1m |
...
| HTML/XML/JATS DOM | `html` partial; `xml`, `jats`, `bits` unsupported | mixed | 277 | 29 | JATS/BITS front-matter packets, ref-list bibliography diagnostics, and explicit XML/JATS/BITS direct-reader capability diagnostics are covered; finish HTML5 tree construction and implement full XML/JATS/BITS readers. |
...
Current-main counters are reconciled through the JATS/BITS ref-list bibliography diagnostic slice rebased on `70d29eb6f8`: 3,332 PHP passes, 0 failures, and 3,291 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for the touched XmlHtmlDom files, focused `XmlHtmlDomTest.php` (`1` file, `1918` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74893` assertions, `0` failures).
...
### XML/JATS/BITS Ref-list Bibliography Diagnostic Update (2026-06-13)

Bounded native PHP XML/JATS/BITS coverage advanced by one ref-list bibliography diagnostic slice after EPUB XHTML table semantics. `XmlHtmlDom::summarizeJatsFrontMatter()` now emits metadata-only `ref-list` and `ref` summaries, resolved/unresolved `xref ref-type="bibr"` inventories, unreferenced and missing-id reference counts, and unresolved bibliography diagnostics while preserving `directReaderParity=false` and avoiding full citation-reader parity claims.

Verification passed `php -l` for `XmlHtmlDom.php` and `XmlHtmlDomTest.php`; focused `XmlHtmlDomTest.php` passed (`1` file, `1918` assertions, `0` failures); full `lanes/pandoc/tests` passed (`45` files, `74893` assertions, `0` failures). No Pandoc, Cabal/Haskell runner, browser renderer, Node tooling, external XML validator, online service, live provider test, or live-service provider test was invoked.
...
(`1` file, `1918` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`45` files, `74893` assertions, `0` failures on current main `70d29eb6f8`).
