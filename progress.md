...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,333 pass / 0 fail | [3,292 / 2,276 (144.6%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue DocBook reader parity beyond bounded list/table metadata | docbook-list-metadata-9ab2f42c |
...
| DocBook/list/table geometry | `docbook` | partial | 17 | 16 | DocBook list metadata/diagnostics and table geometry are covered; finish broader DocBook XML reader parity. |
...
Current-branch counters are reconciled through the DocBook list block metadata slice rebased on `9ab2f42c1e`: 3,333 PHP passes, 0 failures, and 3,292 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for the touched Markdown reader/test files, focused `MarkdownReaderTest.php` (`1` file, `6748` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74863` assertions, `0` failures).
...
| Markdown block/list/note/section boundaries | Landed HTML list item attributes, math attributes, nested fenced Div, escaped fenced-Div attributes, fenced Div section-reference boundaries, mixed-content nested-list, definition-list term-group, table-cell note placement, and header section Div slices | 1,096 Markdown-family rows | 450 Markdown-family cases; latest focused MarkdownReader run 6,748 assertions | Partial | Markdown/CommonMark/GFM extension and variant parity. |
...
| DocBook list metadata | Landed bounded `itemizedlist`, `orderedlist`, nested list, `variablelist`, `xml:id`, and unsupported `listitem` child diagnostic handoff | 16 DocBook/table rows | 17 DocBook cases; latest focused MarkdownReader run 6,748 assertions | Partial | Broader DocBook XML reader parity beyond bounded list metadata and table geometry. |
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/src/MarkdownReader.php lanes/pandoc/tests/MarkdownReaderTest.php`, syntax checks for `MarkdownReader.php` and `MarkdownReaderTest.php`, focused `MarkdownReaderTest.php`, and `php tools/run-tests.php lanes/pandoc/tests` passed. | 45 test files, 74,863 assertions, 0 failures. |
...
status, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/src/MarkdownReader.php lanes/pandoc/tests/MarkdownReaderTest.php`,
`php -l lanes/pandoc/src/MarkdownReader.php`,
`php -l lanes/pandoc/tests/MarkdownReaderTest.php`,
`php tools/run-tests.php lanes/pandoc/tests/MarkdownReaderTest.php`
(`1` file, `6748` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`45` files, `74863` assertions, `0` failures on current main `9ab2f42c1e`).
