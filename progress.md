...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,353 pass / 0 fail | [3,313 / 2,276 (145.6%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue broader media/resource edge coverage after linked-resource collision and normalization diagnostics | media-bag-linked-resource-diagnostics-de537d8 |
...
Current-main counters are reconciled through the MediaBag linked-resource collision and normalization diagnostics slice rebased on `de537d8caa`: 3,353 PHP passes, 0 failures, and 3,313 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for `MediaBag.php` and `MediaBagTest.php`, and focused `MediaBagTest.php` (`1` file, `199` assertions, `0` failures). Full lane verification passed 46 files, 75,542 assertions, 0 failures after final rebase onto base de537d8caa.
...
| Media linked-resource handoff | Landed linked-resource loading, MIME inference, and collision/normalization diagnostics | 3 mapped cross-format resource slices | 3 focused `MediaBag` cases; latest focused run 199 assertions | Covered bounded handoff; not an input-format ship gate | Wider media/resource edge cases outside opt-in linked-resource handoff. |
...
### MediaBag Linked-Resource Diagnostics Update (2026-06-13)

Bounded native PHP MediaBag coverage advanced by one linked-resource collision and normalization diagnostics slice after JATS/BITS body diagnostics. `MediaBag` now records stable canonical-source, source-path, normalized-source-path, MIME-source, and inferred-MIME provenance fields for extracted resources while preserving linked-resource MIME inference behavior.

The focused fixture covers percent-encoded linked-resource path normalization, duplicate linked-resource MIME summaries, declared-vs-inferred MIME disagreement diagnostics, and case-folded extraction path collision disambiguation. Verification passed `php -l` for `MediaBag.php` and `MediaBagTest.php`; focused `MediaBagTest.php` passed (`1` file, `199` assertions, `0` failures). Full lane verification passed 46 files, 75,542 assertions, 0 failures after final rebase onto base de537d8caa. No Pandoc binary, Cabal/Haskell runner, browser renderer, Node tooling, online service, live provider test, or external validator was invoked.

MediaBag linked-resource evidence is now 3 local mapped resource slices. The broader media/resource surface remains partial for external/missing diagnostics and writer handoff consistency edges outside opt-in linked-resource handoff.
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check`, syntax checks for `MediaBag.php` and `MediaBagTest.php`, and focused `MediaBagTest.php` passed. | Focused: 1 file, 199 assertions, 0 failures. Full lane verification passed 46 files, 75,542 assertions, 0 failures after final rebase onto base de537d8caa. |
...
(`1` file, `199` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`46` files, `75542` assertions, `0` failures after final rebase onto base `de537d8caa`).
