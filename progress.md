...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,362 pass / 0 fail | [3,322 / 2,276 (146.0%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue XML/JATS/BITS parity after direct input registry routing | pandoc-xml-direct-input-registry-3a324301 |
...
Current-main counters are reconciled through the XML/JATS/BITS direct input registry slice rebased on `3a324301`: 3,362 PHP passes, 0 failures, and 3,322 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for `PandocFormatRegistry.php`, `XmlHtmlDom.php`, `PandocFormatRegistryTest.php`, and `XmlHtmlDomTest.php`, focused registry/XML tests (`2` files, `3522` assertions, `0` failures), and full `lanes/pandoc/tests` (`46` files, `75791` assertions, `0` failures).
...
| XML/JATS/BITS direct input registry and body diagnostics | Landed partial direct input registry routing for `xml`, `jats`, and `bits` through `XmlHtmlDom`, unsupported direct-reader parity reason serialization, standalone object association provenance, JATS/BITS body roots, section hierarchy, xref resolution, references, figures, table-wraps, table body/row/cell metadata, back-matter ref-lists, reference metadata, citation xref targets, resolved/missing citation ids, unreferenced buckets, BITS book-part body metadata, and DocBook section metadata diagnostics | 29 XML/HTML/JATS/DocBook rows | 282 XML/HTML/JATS/DocBook cases; latest focused registry/XML run 3,522 assertions | Partial after bounded registry and review diagnostics | Implement full XML/JATS/BITS body/back matter, tables, figures, references, citations, and AST parity. |
...
reference diagnostics slice, and XML/JATS/BITS direct input registry slice.
...
| Local passing numerator | `lane-status.json` reports 3,362 PHP passes / 0 failures, and `UPSTREAM_TEST_MANIFEST.json` reports 3,322 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,322 / 2,276 = 146.0%; percentages above 100% reflect local PHP slices being more granular than upstream inventory rows. | High coverage, but not global ship-ready. |
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check`, syntax checks for `PandocFormatRegistry.php`, `XmlHtmlDom.php`, `PandocFormatRegistryTest.php`, and `XmlHtmlDomTest.php`, focused registry/XML tests, and `php tools/run-tests.php lanes/pandoc/tests` passed. | Focused: 2 files, 3,522 assertions, 0 failures. Full: 46 files, 75,791 assertions, 0 failures. |
