...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,313 pass / 0 fail | [3,273 / 2,276 (143.8%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | CSV/TSV header option parity is covered; continue option and malformed-input parity | csv-tsv-header-options-3a4bf7596e |
...
| Tabular/data readers | `csv`, `tsv` | partial | 4 | 2 | Continue broader CSV/TSV option variants, malformed input diagnostics, multiline-cell policy, captions, metadata, and writer parity. |
...
Current branch counters are reconciled on base `3a4bf7596e`: 3,313 PHP passes, 0 failures, and 3,273 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence plus the CSV/TSV header-option slice; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, focused `DelimitedTextReaderTest.php` plus `PandocFormatRegistryTest.php` (`2` files, `1082` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74253` assertions, `0` failures).

| Surface | Evidence state | Upstream denominator | Local passing numerator | Ship verdict | Remaining critical gaps |
| --- | --- | ---: | ---: | --- | --- |
| CSV/TSV direct readers | Landed | 2 CSV command fixtures | 4 local mapped CSV/TSV reader cases; latest focused reader/registry run has 1,082 assertions | Partial after headed and explicit no-header reader slices | Broader CSV/TSV option behavior, malformed input diagnostics, multiline-cell policy, and table-reader edge cases. |
...
Verdict: partial, not ship-ready. The native PHP lane now has 4 local passing CSV/TSV reader cases against the 2 CSV command fixture rows recorded in the accepted static upstream inventory, with explicit no-header body-only imports covered.

Implemented gap: `DelimitedTextReader` now accepts a `header` option for CSV and TSV imports. `header => false` preserves every source row as table body data, emits an empty table head for Pandoc JSON compatibility, generates review-only column labels, and records a `delimited-text-header-disabled` diagnostic; non-boolean `header` options are rejected.

Verification passed `php -l` for `DelimitedTextReader.php`, `DelimitedTextReaderTest.php`, `PandocFormatRegistry.php`, and `PandocFormatRegistryTest.php`; focused `DelimitedTextReaderTest.php` plus `PandocFormatRegistryTest.php` passed (`2` files, `1082` assertions, `0` failures); full `lanes/pandoc/tests` passed (`45` files, `74253` assertions, `0` failures). No Pandoc binary, spreadsheet application, browser renderer, Node tooling, online validator, online service, live provider test, or external validator was invoked.
...
| Local passing numerator | `lane-status.json` reports 3,313 PHP passes / 0 failures, and `UPSTREAM_TEST_MANIFEST.json` reports 3,273 mapped upstream cases. | PHP lane remains green. |
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/src/DelimitedTextReader.php lanes/pandoc/src/PandocFormatRegistry.php lanes/pandoc/tests/DelimitedTextReaderTest.php lanes/pandoc/tests/PandocFormatRegistryTest.php`, syntax checks for the CSV/TSV reader and registry files, focused `DelimitedTextReaderTest.php` plus `PandocFormatRegistryTest.php`, and `php tools/run-tests.php lanes/pandoc/tests` passed. | 45 test files, 74,253 assertions, 0 failures. |
...
status, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/src/DelimitedTextReader.php lanes/pandoc/src/PandocFormatRegistry.php lanes/pandoc/tests/DelimitedTextReaderTest.php lanes/pandoc/tests/PandocFormatRegistryTest.php`,
...
(`2` files, `1082` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`45` files, `74253` assertions, `0` failures on current main `3a4bf7596e`).
