...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,371 pass / 0 fail | [3,331 / 2,276 (146.4%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue JATS/BITS citation/reference diagnostics after inline xref diagnostics | jats-inline-xref-diagnostics-dc0ffc9358 |
...
Current-main counters are reconciled through the JATS/BITS inline xref diagnostics slice rebased on `dc0ffc9358`: 3,371 PHP passes, 0 failures, and 3,331 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for `XmlHtmlDom.php` and `XmlHtmlDomTest.php`, focused `XmlHtmlDomTest.php` (`1` file, `2192` assertions, `0` failures), and full `lanes/pandoc/tests` (`46` files, `76152` assertions, `0` failures).
...
slice, JATS/BITS relationship diagnostics slice, and JATS/BITS inline xref
diagnostics slice.
...
(`1` file, `2192` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`46` files, `76152` assertions, `0` failures after final rebase onto base `dc0ffc9358`).
