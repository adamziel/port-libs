...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,319 pass / 0 fail | [3,278 / 2,276 (144.0%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue XML/JATS/BITS body, back matter, and reader parity after body diagnostics | xml-html-jats-body-diagnostics-4487923aa2 |
...
| HTML/XML/JATS DOM | `html` partial; `xml`, `jats`, `bits` unsupported | mixed | 276 | 29 | JATS/BITS front-matter and bounded body diagnostics are covered; finish HTML5 tree construction and implement full XML/JATS/BITS readers. |
...
Current branch counters are reconciled over `4487923aa2`: 3,319 PHP passes, 0 failures, and 3,278 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for the touched XML DOM files, focused `XmlHtmlDomTest.php` (`1` file, `1868` assertions, `0` failures), adjacent DOM/table tests (`6` files, `6296` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74512` assertions, `0` failures).
...
| XML/JATS body diagnostics | Landed bounded JATS/BITS body diagnostic packets | 29 XML/HTML/JATS/DocBook family rows | 276 XML/HTML/JATS/DocBook evidence cases; latest focused XML DOM run 1,868 assertions | Partial/unsupported mix | Full XML/JATS/BITS direct-reader parity, back matter, table, figure, reference, and citation-reader parity. |
...
| JATS/BITS XML-ish inputs | covered in the 29-case family | bounded JATS/BITS front-matter packets plus body diagnostics | Implement full JATS/BITS body, back matter, tables, figures, references, and citation-reader parity. |
...
Implemented follow-up gap: `XmlHtmlDom::summarizeJatsFrontMatter()` now preserves bounded JATS/BITS body diagnostics while keeping `directReaderParity=false`. Review packets include body root selection, section hierarchy/depth/type metadata, direct and descendant section paragraph counts, resolved and unresolved xref targets, reference labels and back-reference counts, figure labels/captions/graphic hrefs, table-wrap labels/captions/row counts, unreferenced figure/table buckets, and BITS book-part body metadata. Verification passed `php -l` for `XmlHtmlDom.php` and `XmlHtmlDomTest.php`, focused `XmlHtmlDomTest.php` (`1` file, `1868` assertions, `0` failures), adjacent DOM/table tests (`6` files, `6296` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74512` assertions, `0` failures). No Pandoc binary, browser renderer, external XML validator, online service, live provider test, or live-service provider test was invoked.
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/src/XmlHtmlDom.php lanes/pandoc/tests/XmlHtmlDomTest.php lanes/pandoc/notes/xml-html-jats-body-diagnostics-20260613.md`, syntax checks for `XmlHtmlDom.php` and `XmlHtmlDomTest.php`, focused `XmlHtmlDomTest.php`, adjacent DOM/table tests, and `php tools/run-tests.php lanes/pandoc/tests` passed. | 45 test files, 74,512 assertions, 0 failures. |
...
status, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/src/XmlHtmlDom.php lanes/pandoc/tests/XmlHtmlDomTest.php lanes/pandoc/notes/xml-html-jats-body-diagnostics-20260613.md`,
`php -l lanes/pandoc/src/XmlHtmlDom.php`,
`php -l lanes/pandoc/tests/XmlHtmlDomTest.php`,
`php tools/run-tests.php lanes/pandoc/tests/XmlHtmlDomTest.php`
(`1` file, `1868` assertions, `0` failures), `php tools/run-tests.php lanes/pandoc/tests/XmlHtmlDomTest.php lanes/pandoc/tests/Html5DomTest.php lanes/pandoc/tests/XmlHtml5DomTest.php lanes/pandoc/tests/Html5DomFragmentTest.php lanes/pandoc/tests/XmlHtmlDomFragmentTest.php lanes/pandoc/tests/TableGeometryReaderHandoffTest.php`
(`6` files, `6296` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`45` files, `74512` assertions, `0` failures over current main `4487923aa2`).
