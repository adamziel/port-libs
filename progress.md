...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage, active closures | 96.0% | 3,290 pass / 0 fail | [3,250 / 2,276 (142.8%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | No landed stale-open blocker; continue format closures | progress-status-reconciliation-3af5dde50f |
...
commit history agree on the current shipping call.
...
| Local passing numerator | `lane-status.json` reports 3,290 PHP passes / 0 failures, and `UPSTREAM_TEST_MANIFEST.json` reports 3,250 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,250 / 2,276 = 142.8%; percentages above 100% reflect local PHP slices being more granular than upstream inventory rows. | High coverage, but not global ship-ready. |
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json`, and `php tools/run-tests.php lanes/pandoc/tests` passed. | 44 test files, 73,816 assertions, 0 failures. |
...
main `3af5dde50f`). `bd orphans --label lane:pandoc` was used for stale-open
