...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 6,475 pass / 0 fail | [6,465 / 2,276 (284.1%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Markdown writer Plain list-item validation passed after rebase onto current main | pandoc-markdown-writer-plain-list-item-surge |
...
### Markdown Writer Plain List-Item Surge (2026-06-15)

Bounded native PHP Markdown writer coverage advances by 60 upstream-mapped block/list/code compact `Plain` list-item cases after rebase onto current main `87b401356f`. `MarkdownWriter` now emits Pandoc `Plain` blocks on the first list marker line for bullet, task, ordered, default, and numbered-example list items instead of blank marker workarounds; keeps task-list softbreak and second-block continuation indentation at the list marker column; and round-trips nested bullet/ordered/example/code/blockquote handoff shapes from JSON/native AST packets while preserving current auto HTML table fallback, block/list/code marker, inline/link/escape, figure-caption, native-div, URL-normalization, and emoji coverage.

Validation passed after conflict resolution: `php -l` for `MarkdownWriter.php` and `MarkdownWriterPlainListItemSurgeTest.php`; focused `MarkdownWriterPlainListItemSurgeTest.php` (`1` file, `358` assertions, `0` failures); adjacent Markdown block/list/code regression passed (`5` files, `4,537` assertions, `0` failures); `MarkdownReaderTest.php` fixture regression passed (`1` file, `7,019` assertions, `0` failures); full `lanes/pandoc/tests` passed (`77` files, `106,802` assertions, `0` failures); JSON validation; `git diff --check`; and exact conflict-marker scan. No Pandoc binary, cmark/commonmark runner, Cabal/Haskell runner, browser renderer, Node tooling, online service, live provider test, live-service provider test, or external validator is invoked.

Markdown/CommonMark/GFM evidence is now 2,617 local mapped cases against the 1,096 accepted static upstream Markdown rows. The native PHP Markdown reader/writer remains partial pending broader metadata/raw/extension parity.
