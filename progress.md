...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,363 pass / 0 fail | [3,323 / 2,276 (146.0%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue targeted core-blocker hardening after ODF embedded object media-type provenance | odt-embedded-object-media-types-rebased |
...
| HTML/XML/JATS DOM | `html`, `xml`, `jats`, and `bits` partial | partial | 283 | 29 | Object association provenance, image-map association diagnostics, XML/JATS/BITS direct input registry routing, JATS/BITS front-matter, body, table-body, and back-matter reference diagnostics review packets, and DocBook structure and section metadata review packets are covered; finish HTML5 tree construction and full XML/JATS/BITS reader parity. |
| JSON/native AST | `json`, `native` | partial | 57 | 252 | Nested metadata payload preservation, mixed Figure link/raw/code handoff, raw HTML alias preservation, table attribute handoff, mixed block-container and table caption/cell flushing, nullary helper payload validation, task-list checkbox sidecars, note label sidecars, definition-list term handoff, and fixture writer handoff are preserved; complete broader JSON/native AST constructor coverage. |
...
| ODF/ODT/OpenDocument | `odt` | ship-ready | 51 | 20 | 0 critical gaps for native PHP ODT import; compact manifest custom attributes and embedded object contained media-type provenance are covered. Continue only non-critical hardening slices as discovered. |
...
| CSL/BibTeX/BibLaTeX/csljson citations | `bibtex`, `biblatex`, `csljson`, `endnotexml`, `ris` | mixed | 78 | 8 | Citation prefix/suffix affix review metadata and bounded RIS item parsing are covered; EndNote XML, broader RIS coverage, and full reader-registry parity remain. |
...
| DocBook/table geometry | `docbook` | partial | 18 | 16 | DocBook structural review and section metadata diagnostics are covered; finish DocBook XML reader parity, body conversion, inline/block/reference/bibliography mapping, generated AST parity, and broader fixture hydration. |
...
| Tabular/data readers | `csv`, `tsv` | partial | 9 | 2 | Headed and no-header table option handling, auto format inference diagnostics, registry option profiles, extension inference, direction buckets, and source provenance are covered; finish broader CSV/TSV reader parity beyond the bounded table slices. |
...
Current-branch counters are reconciled through the ODF/ODT embedded object contained media-type hardening slice after HTML image-map association diagnostics: 3,363 PHP passes, 0 failures, and 3,323 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for `OdfReader.php`, `OpenDocumentPackage.php`, `OdfReaderTest.php`, and `OpenDocumentPackageTest.php`, focused ODF package tests (`2` files, `5725` assertions, `0` failures), focused ODF/ODT gate (`5` files, `6025` assertions, `0` failures), and full `lanes/pandoc/tests` (`46` files, `75875` assertions, `0` failures).

| Surface | Evidence state | Upstream denominator | Local passing numerator | Ship verdict | Remaining critical gaps |
| --- | --- | ---: | ---: | --- | --- |
| CSV/TSV direct readers and registry options | Landed headed/no-header option parity, auto CSV/TSV format inference diagnostics, tabular registry extension inference, direction buckets, source provenance, and option profiles | 2 CSV command fixtures plus registry/options harness slices | 9 local CSV/TSV/registry cases; latest focused reader/registry run 1,501 assertions | Partial after bounded reader/profile/inference slices | Broader CSV/TSV option behavior, malformed input diagnostics, multiline-cell behavior, and table-reader edge cases. |
| ODF/ODT package reader | Landed compact manifest custom file-entry attributes and embedded object contained media-type provenance while preserving ship-ready closure | 20 ODF/ODT rows | 51 ODF/ODT cases; latest focused ODF/ODT gate 6,025 assertions | Ship-ready | 0 critical ODF/ODT gaps for native PHP ODT import. |
...
| JSON/native table, list, raw, block, metadata, and figure handoff | Landed nested metadata payload preservation, raw HTML alias preservation, table-caption, table-attribute, mixed table caption/cell flushing, note-label, Div Plain, mixed block-container, mixed Figure link/raw/code handoff, task-list sidecar, definition-list term-group, and nullary helper payload slices | 252 JSON/native artifacts | 57 JSON/native cases; latest focused `PandocJsonNativeAstTest.php` run 2,267 assertions | Partial | Broader native/json fixture parity, unsupported constructors, and table/citation/metadata round trips. |
...
| HTML/XML/JATS DOM diagnostics | Landed image-map association diagnostics for resolved, missing, duplicate, invalid, and unreferenced map states; partial direct input registry routing for `xml`, `jats`, and `bits` through `XmlHtmlDom`; unsupported direct-reader parity reason serialization; standalone object association provenance; JATS/BITS body roots, section hierarchy, xref resolution, references, figures, table-wraps, table body/row/cell metadata, back-matter ref-lists, reference metadata, citation xref targets, resolved/missing citation ids, unreferenced buckets, BITS book-part body metadata, and DocBook section metadata diagnostics | 29 XML/HTML/JATS/DocBook rows | 283 XML/HTML/JATS/DocBook cases; latest focused `XmlHtmlDomTest.php` run 2,103 assertions | Partial after bounded registry and review diagnostics | Implement full HTML5 tree construction and XML/JATS/BITS body/back matter, tables, figures, references, citations, and AST parity. |
| DocBook structural and section metadata diagnostics | Landed review-only DocBook 4/5 structure packets plus root/section title provenance, section ids, roles, labels, languages, levels, paragraph counts, child-section counts, missing-title diagnostics, and duplicate-id diagnostics | 16 DocBook/table geometry rows | 18 local DocBook/table geometry cases; latest focused DocBook/XML registry run 4,998 assertions | Partial after bounded review diagnostics | Full DocBook body conversion, inline/block/reference/bibliography mapping, generated AST parity, broader fixture hydration, and figure/media/admonition conversion remain open. |
| IPYNB/notebook reader diagnostics | Landed metadata keys, cell tags, MIME summaries, blocked resource diagnostics, and metadata-only attachment media extraction plans with safe `ipynb-media/` paths plus unsafe-path/collision diagnostics | 1 IPYNB rich-package reader bucket | Focused `IpynbReaderTest.php` run 110 assertions | Covered bounded reader diagnostics | Full Jupyter notebook reader parity, rich output rendering, broader nbformat diagnostics, and native IPYNB writer support remain open. |
...
| CSL citation/bibliography handoff | Landed citation prefix/suffix affix review metadata, RIS parsing, direct CSL alias normalization, BibTeX/BibLaTeX metadata aliases, and bounded CSL rendering diagnostics | 8 CSL/BibTeX/BibLaTeX/csljson/RIS/EndNote XML rows | 78 CSL/BibTeX/BibLaTeX/csljson/RIS cases; latest focused `CitationCslProcessorTest.php` run 5,321 assertions | Partial | EndNote XML, broader RIS tag coverage, bibliography reader-registry parity, and wider CSL handoff diagnostics. |
...
| PDF/Typst dependency policy | Landed package dependency conflict, source-class, and unsupported-reason policy provenance | 17 PDF/Typst boundary rows | 49 PDF/Typst boundary/provenance cases; latest focused run 2,237 assertions | Covered bounded no-engine provenance | Real PDF/Typst output parity remains unsupported without external engines. |
...
### ODT Embedded Object Contained Media-Type Update (2026-06-13)

Bounded native PHP ODF/ODT package-reader hardening advanced the embedded object review path without adding a new mapped upstream case. `OdfReader` and `OpenDocumentPackage` now preserve manifest media-type provenance on contained embedded-object package parts, summarize contained parts by media-type base, count declared, undeclared, and missing-media-type contained parts, and expose embedded object package packets through `OdfReader` metadata while keeping embedded object payload bytes blocked.

Verification passed `php -l` for `OdfReader.php`, `OpenDocumentPackage.php`, `OdfReaderTest.php`, and `OpenDocumentPackageTest.php`; focused `OdfReaderTest.php` plus `OpenDocumentPackageTest.php` passed (`2` files, `5725` assertions, `0` failures); focused ODF/ODT gate passed (`5` files, `6025` assertions, `0` failures); full `lanes/pandoc/tests` passed (`46` files, `75788` assertions, `0` failures). No Pandoc binary, office suite, zip/unzip, ZipArchive, browser renderer, Node tooling, online service, live provider test, or external validator was invoked.

ODF/ODT evidence remains 51 local mapped cases against the 20 accepted static upstream ODF/ODT rows, with 0 remaining critical ODF/ODT gaps. The embedded object package assertion counter moves 77 -> 136, and the native PHP ODF/ODT input ship-ready verdict remains unchanged.
...
| Local passing numerator | 51 current mapped ODF/ODT cases and 6,025 focused assertions across `lanes/pandoc/lane-status.json` plus manifest-carried ODF/ODT counters. |
| Coverage percent | 255.0% by mapped case slices, 1,047.8% by focused assertions. Percentages above 100% reflect local PHP slices being more granular than the upstream inventory rows. |
| Local focused test files | `OdfReaderTest.php`, `OdtReaderTest.php`, `OpenDocumentReaderTest.php`, and `OpenDocumentPackageTest.php` cover 220 focused ODF/ODT test cases. |
| Manifest/package ingestion | Root mimetype validation, `META-INF/manifest.xml`, directory declarations, raw ZIP name/order provenance, URI-encoded paths, declared sizes, custom attributes, media-type parameters, missing/encrypted/unsupported bytes, thumbnails, signatures, Configurations2, scripts, RDF, object replacements, embedded object packages, and contained embedded-object media-type buckets. |
...
| Verification | `jq empty lanes/pandoc/lane-status.json`, `git diff --check -- progress.md lanes/pandoc/lane-status.json`, focused ODF/ODT suite (`5` files, `6,025` assertions, `0` failures), and full `lanes/pandoc/tests` (`46` files, `75,788` assertions, `0` failures). |
...
| Format focus | Upstream denominator | Local passing evidence | Evidence percent | Remaining critical gap |
| --- | ---: | ---: | ---: | --- |
| JSON/native AST constructors and round trips | 252 | 57 | 22.6% | Broader upstream native/json fixture parity plus unsupported constructor/table/citation/metadata/raw surfaces beyond bounded nested metadata payload preservation, mixed Figure handoff, raw HTML aliases, sidecar reuse, Markdown fixture writer handoff, note label sidecars, task-list sidecars, and nullary helper payload validation. |
...
ODF embedded object contained media-type provenance slice,
...
reference diagnostics slice, XML/JATS/BITS direct input registry slice, and
HTML image-map association diagnostics slice.
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check`, syntax checks for `OdfReader.php`, `OpenDocumentPackage.php`, `OdfReaderTest.php`, and `OpenDocumentPackageTest.php`, focused ODF package tests, focused ODF/ODT gate, and full `lanes/pandoc/tests` passed. | Focused package: 2 files, 5,725 assertions, 0 failures. ODF/ODT gate: 5 files, 6,025 assertions, 0 failures. Full: 46 files, 75,875 assertions, 0 failures. |
...
status, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/src/IpynbReader.php lanes/pandoc/tests/IpynbReaderTest.php`,
...
(`1` file, `110` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`46` files, `75664` assertions, `0` failures after final rebase onto base `f70cdddd`).
