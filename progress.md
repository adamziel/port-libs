...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,355 pass / 0 fail | [3,315 / 2,276 (145.7%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue CSV/TSV malformed-input and multiline-cell parity | csv-tsv-control-character-policy-eb56c4b |
...
| Tabular/data readers | `csv`, `tsv` | partial | 10 | 2 | Headed and no-header table option handling, auto format inference diagnostics, registry option profiles, extension inference, direction buckets, source provenance, and bounded control-character policy diagnostics are covered; finish broader CSV/TSV reader parity beyond the bounded table slices. |
...
Current-main counters are reconciled through the CSV/TSV control-character policy diagnostics slice rebased on `eb56c4b88e`: 3,355 PHP passes, 0 failures, and 3,315 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for `DelimitedTextReader.php` and `DelimitedTextReaderTest.php`, focused `DelimitedTextReaderTest.php` plus `PandocFormatRegistryTest.php` (`2` files, `1563` assertions, `0` failures), and full `lanes/pandoc/tests` (`46` files, `75611` assertions, `0` failures).

| Surface | Evidence state | Upstream denominator | Local passing numerator | Ship verdict | Remaining critical gaps |
| --- | --- | ---: | ---: | --- | --- |
| CSV/TSV direct readers and registry options | Landed headed/no-header option parity, auto CSV/TSV format inference diagnostics, tabular registry extension inference, direction buckets, source provenance, option profiles, and bounded control-character policy diagnostics | 2 CSV command fixtures plus registry/options harness slices | 10 local CSV/TSV/registry cases; latest focused reader/registry run 1,563 assertions | Partial after bounded reader/profile/inference/diagnostic slices | Broader CSV/TSV option behavior, malformed input diagnostics, multiline-cell behavior, and table-reader edge cases. |
...
### CSV/TSV Control Character Policy Update (2026-06-13)

Bounded native PHP CSV/TSV reader coverage advanced by one control-character policy diagnostics slice after LaTeX labelled note anchor preservation. `DelimitedTextReader` review packets now report bounded summaries for NUL and non-structural C0/DEL bytes in quoted and unquoted fields, including row/column positions, sourcePath context, byte buckets, and byte/text samples while preserving parsed table text, BOM handling, and row-width behavior.

Verification passed `php -l` for `DelimitedTextReader.php` and `DelimitedTextReaderTest.php`; focused `DelimitedTextReaderTest.php` plus `PandocFormatRegistryTest.php` passed (`2` files, `1563` assertions, `0` failures); full `lanes/pandoc/tests` passed (`46` files, `75611` assertions, `0` failures). No Pandoc binary, spreadsheet application, browser renderer, Node tooling, online service, live provider test, or external validator was invoked.
...
commit history agree on the current shipping call after the CSV/TSV
control-character policy diagnostics slice, LaTeX labelled note anchor
preservation slice, JSON/native raw HTML alias preservation slice, PDF/Typst package unsupported-reason slice, JATS/BITS body
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check`, syntax checks for `DelimitedTextReader.php` and `DelimitedTextReaderTest.php`, focused `DelimitedTextReaderTest.php` plus `PandocFormatRegistryTest.php`, and `php tools/run-tests.php lanes/pandoc/tests` passed. | Focused: 2 files, 1,563 assertions, 0 failures. Full: 46 files, 75,611 assertions, 0 failures. |
...
(`2` files, `1563` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`46` files, `75611` assertions, `0` failures after final rebase onto base `eb56c4b88e`).
`bd orphans --label lane:pandoc` was used for stale-open cleanup, but only
main-ancestor referenced commits were closed. No Pandoc binary, spreadsheet
application, browser engine, Node tooling, or external validator was invoked.
