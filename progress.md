...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,399 pass / 0 fail | [3,355 / 2,276 (147.4%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue IPYNB/notebook parity beyond bounded source diagnostics | ipynb-source-line-ending-diagnostics-2e1f2467 |
...
Current-main counters are reconciled through the IPYNB source line-ending diagnostics slice rebased on `2e1f2467`: 3,399 PHP passes, 0 failures, and 3,355 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for `IpynbReader.php` and `IpynbReaderTest.php`, focused `IpynbReaderTest.php` (`1` file, `506` assertions, `0` failures), and full `lanes/pandoc/tests` (`46` files, `78061` assertions, `0` failures).
