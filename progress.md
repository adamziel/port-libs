...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,322 pass / 0 fail | [3,281 / 2,276 (144.2%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Rare text registry unsupported diagnostics recorded | pandoc-rare-text-registry-unsupported-d1e41a7720 |
...
| --- | --- | --- | ---: | ---: | --- |
| Markdown/CommonMark/GFM | `commonmark`, `commonmark_x`, `gfm`, `markdown`, `markdown_github`, `markdown_mmd`, `markdown_phpextra`, `markdown_strict` | partial | 445 | 1,096 | Escaped fenced-Div attributes, fenced Div section-reference boundaries, and nested-list fixture round-trip are covered; complete extension and variant parity. |
...
Current-main counters are reconciled through `d1e41a7720`: 3,322 PHP passes, 0 failures, and 3,281 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for the touched registry files, focused `PandocFormatRegistryTest.php` (`1` file, `1257` assertions, `0` failures), rare-text filtered registry checks (`4` tests, `151` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74664` assertions, `0` failures).
...
| Markdown block boundaries | Landed nested fenced Div, escaped fenced-Div attributes, fenced Div section-reference boundaries, and mixed-content nested-list slices | 1,096 Markdown-family rows | 445 Markdown-family cases; latest focused MarkdownReader run 6,653 assertions | Partial | Markdown/CommonMark/GFM extension and variant parity. |
...
| Rare text registry diagnostics | Landed explicit unsupported diagnostics for rare text tokens including `org`, `rst`, `textile`, and `muse` | Rare text input/output registry tokens | Rare-text filtered registry checks 151 assertions | Unsupported | Implement native rare text readers/writers before claiming direct parity. |
...
### Text Format Ship-Readiness Update (2026-06-13)

Verdict: not shippable yet. The focused denominator-backed text-format gate is 468 / 1,132 (41.3%) across Markdown/CommonMark/GFM, LaTeX/TeX/math, wiki/roff/rare text readers, and CSV/TSV readers. Plain output has 3 local evidence slices, the wiki/roff/man/text markup ship-gate has an explicit unsupported verdict for 20 input tokens, and rare text registry diagnostics now have one explicit unsupported verdict slice, but reader parity gaps still block the broader text-format family.

| Format focus | Upstream denominator | Local passing evidence | Evidence percent | Remaining critical gap |
| --- | ---: | ---: | ---: | --- |
| Markdown/CommonMark/GFM variants | 1,096 | 445 | 40.6% | Escaped fenced-Div attributes, fenced Div section-reference boundaries, and nested-list fixture round-trip are covered; complete extension and variant parity. |
| LaTeX/TeX/math | 14 | 20 | 142.9% | Local evidence is granular, but native LaTeX reader and math parity remain incomplete. |
| Wiki/roff/rare text readers | 20 | 1 | 5.0% | Text markup reader unsupported ship-gate and rare text unsupported diagnostics are explicit for org/rst/textile/muse and related tokens; native text readers remain unimplemented. |
...
Implemented follow-up gap: `PandocFormatRegistry` now records rare text input/output direction buckets, extension inference, AsciiDoc output aliases, unsupported summaries, and parity counts for `asciidoc`, `djot`, `fb2`, `haddock`, `muse`, `opml`, `org`, `pod`, `rst`, `t2t`, `textile`, and related output-only text tokens. All native rare text reader/writer implementation fields remain empty. Verification passed `php -l` for `PandocFormatRegistry.php` and `PandocFormatRegistryTest.php`, focused `PandocFormatRegistryTest.php` (`1` file, `1257` assertions, `0` failures), rare-text filtered registry checks (`4` tests, `151` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74664` assertions, `0` failures). No Pandoc binary, format-specific CLI, browser renderer, Node tooling, external validator, online service, live provider test, or live-service provider test was invoked.
...
| Local passing numerator | `lane-status.json` reports 3,322 PHP passes / 0 failures, and `UPSTREAM_TEST_MANIFEST.json` reports 3,281 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,281 / 2,276 = 144.2%; percentages above 100% reflect local PHP slices being more granular than upstream inventory rows. | High coverage, but not global ship-ready. |
...
| Remaining critical gaps | 18 input tokens remain partial and 31 remain unsupported across DOCX/OpenXML, EPUB3, shared ZIP/OPC dependencies, JSON/native AST, CSV/TSV, CSL/BibTeX/BibLaTeX/csljson, HTML/XML/JATS DOM, LaTeX/TeX/math, Typst, PPTX/XLSX, and wiki/roff/text readers. Markdown fenced Div section-reference boundaries, inline writer attributes, EPUB direct manifest/spine diagnostics, EPUB XHTML definition-list handoff, text markup unsupported ship-gate accounting, and rare text-format unsupported registry diagnostics are covered. | Full Pandoc input lane remains active. |
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/src/PandocFormatRegistry.php lanes/pandoc/tests/PandocFormatRegistryTest.php lanes/pandoc/notes/pandoc-rare-text-registry-unsupported-20260613.md`, syntax checks for `PandocFormatRegistry.php` and `PandocFormatRegistryTest.php`, focused `PandocFormatRegistryTest.php`, rare-text filtered registry checks, and `php tools/run-tests.php lanes/pandoc/tests` passed. | 45 test files, 74,664 assertions, 0 failures. |
...
(`1` file, `1257` assertions, `0` failures), rare-text filtered registry checks
(`4` tests, `151` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`45` files, `74664` assertions, `0` failures on current main `d1e41a7720`).
