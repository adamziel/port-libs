...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,367 pass / 0 fail | [3,327 / 2,276 (146.2%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue XML/HTML5 DOM tree-construction parity after nested table foster-parenting | xml-html5-dom-table-foster-parenting-8d509ca |
...
Current-main counters are reconciled through the XML/HTML5 DOM nested table foster-parenting slice rebased on `8d509ca`: 3,367 PHP passes, 0 failures, and 3,327 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax check for `XmlHtmlDomTest.php`, focused `XmlHtmlDomTest.php` (`1` file, `2133` assertions, `0` failures), and full `lanes/pandoc/tests` (`46` files, `76019` assertions, `0` failures).
...
title metadata propagation slice, and XML/HTML5 nested table foster-parenting
slice.
...
| Local passing numerator | `lane-status.json` reports 3,367 PHP passes / 0 failures, and `UPSTREAM_TEST_MANIFEST.json` reports 3,327 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,327 / 2,276 = 146.2%; percentages above 100% reflect local PHP slices being more granular than upstream inventory rows. | High coverage, but not global ship-ready. |
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check`, syntax check for `XmlHtmlDomTest.php`, focused `XmlHtmlDomTest.php`, and `php tools/run-tests.php lanes/pandoc/tests` passed. | Focused: 1 file, 2,133 assertions, 0 failures. Full: 46 files, 76,019 assertions, 0 failures. |
...
status, `git diff --check -- progress.md PANDOC_STATUS.md index.html porting.html lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/tests/XmlHtmlDomTest.php lanes/pandoc/notes/xml-html5-dom-table-foster-parenting-current-base-20260613T030852Z.md`,
`php -l lanes/pandoc/tests/XmlHtmlDomTest.php`,
`php tools/run-tests.php lanes/pandoc/tests/XmlHtmlDomTest.php`
(`1` file, `2133` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`46` files, `76019` assertions, `0` failures after final rebase onto base `8d509ca`).
