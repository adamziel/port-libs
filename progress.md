...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,366 pass / 0 fail | [3,326 / 2,276 (146.1%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue XML/JATS/BITS direct reader parity | pandoc-jats-section-title-metadata-follow-up-4251113 |
...
| HTML/XML/JATS DOM | `html`, `xml`, `jats`, and `bits` partial | partial | 284 | 29 | Object association provenance, image-map association diagnostics, XML/JATS/BITS direct input registry routing, JATS/BITS title metadata propagation, front-matter, body, table-body, and back-matter reference diagnostics review packets, and DocBook structure and section metadata review packets are covered; finish HTML5 tree construction and full XML/JATS/BITS reader parity. |
...
Current-main counters are reconciled through the JATS/BITS title metadata follow-up slice rebased on `4251113`: 3,366 PHP passes, 0 failures, and 3,326 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for `XmlHtmlDom.php` and `XmlHtmlDomTest.php`, focused `XmlHtmlDomTest.php` (`1` file, `2113` assertions, `0` failures), DOM family tests (`5` files, `5048` assertions, `0` failures), and full `lanes/pandoc/tests` (`46` files, `75999` assertions, `0` failures).
...
slice, rare text-format registry unsupported diagnostics slice, and JATS/BITS
title metadata propagation slice.
...
| Local passing numerator | `lane-status.json` reports 3,366 PHP passes / 0 failures, and `UPSTREAM_TEST_MANIFEST.json` reports 3,326 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,326 / 2,276 = 146.1%; percentages above 100% reflect local PHP slices being more granular than upstream inventory rows. | High coverage, but not global ship-ready. |
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check`, syntax checks for `XmlHtmlDom.php` and `XmlHtmlDomTest.php`, focused `XmlHtmlDomTest.php`, DOM family tests, and `php tools/run-tests.php lanes/pandoc/tests` passed. | Focused: 1 file, 2,113 assertions, 0 failures. DOM family: 5 files, 5,048 assertions, 0 failures. Full: 46 files, 75,999 assertions, 0 failures. |
...
status, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/src/XmlHtmlDom.php lanes/pandoc/tests/XmlHtmlDomTest.php lanes/pandoc/notes/xml-html5-jats-section-title-metadata-20260613T030207Z.md`,
...
(`5` files, `5048` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`46` files, `75999` assertions, `0` failures after final rebase onto base `4251113`).
