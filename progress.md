...
| --- | --- | --- | --- | ---: | --- |
| 1 | gitoxide | stopped | targeted object/ref inventory plus reference-store overlay slice | 10% | Start a focused pack index/object database slice. |
...
- Latest root suite: `php tools/run-tests.php` passes 20 test files, 345 assertions, 0 failures.
...
- Gitoxide now has a targeted object/ref static inventory, native tree parsing, native loose direct/symbolic reference parsing/storage, native packed-ref header/reference/peeled lookup parsing, and loose+packed reference-store overlay resolution. There is still no upstream Cargo runner parity, and pack indexes/packfiles remain unported. Full Cargo was not executed because `.upstream-cache/gitoxide` is sparse/no-checkout and a runner attempt would require materializing crate source plus building Rust dependencies; full workspace `cargo test` would hydrate/build beyond the current VM cap.
