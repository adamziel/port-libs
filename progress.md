...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,319 pass / 0 fail | [3,278 / 2,276 (144.0%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue citation/bibliography parity after CSL affix review metadata | citation-affix-review-pending |
...
| CSL/BibTeX/BibLaTeX/csljson citations | `bibtex`, `biblatex`, `csljson`, `endnotexml`, `ris` | mixed | 78 | 8 | RIS now has bounded native CSL item parsing and CSL citation affix review metadata; EndNote XML, broader RIS coverage, and full reader-registry parity remain. |
...
Current-main counters are reconciled through `4487923aa2`: 3,319 PHP passes, 0 failures, and 3,278 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for the touched CSL citation files, focused `CitationCslProcessorTest.php` (`1` file, `5321` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74485` assertions, `0` failures).
...
| CSL citation affixes | Landed citation prefix/suffix review metadata for normalized attrs, grouped citation summaries, custom CSL variables, bibliography handoff, and WordPress review output | 8 citation/bibliography format-related rows | 78 citation-family cases; latest focused run 5,321 assertions | Partial | EndNote XML, broader RIS tag coverage, and full reader-registry parity. |
...
### CSL Citation Affix Review Update (2026-06-13)

Bounded native PHP CSL citation handoff advanced by one citation/bibliography slice. `CitationCslProcessor` now preserves citation prefix and suffix review metadata on normalized citation attrs and grouped citation summaries, exposes CSL variables for custom styles, avoids duplicate automatic affix rendering when custom CSL already renders those variables, and carries the review evidence into bibliography and WordPress output.

Verification passed `php -l` for `CitationCslProcessor.php` and `CitationCslProcessorTest.php`; focused `CitationCslProcessorTest.php` passed (`1` file, `5321` assertions, `0` failures); full `lanes/pandoc/tests` passed (`45` files, `74485` assertions, `0` failures). No Pandoc binary, citeproc, BibTeX, Biber, bibliography manager, browser renderer, external validator, online service, live provider test, or live-service provider test was invoked.
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/src/CitationCslProcessor.php lanes/pandoc/tests/CitationCslProcessorTest.php`, syntax checks for the touched CSL citation files, focused `CitationCslProcessorTest.php`, and `php tools/run-tests.php lanes/pandoc/tests` passed. | 45 test files, 74,485 assertions, 0 failures. |
...
(`45` files, `74485` assertions, `0` failures on current main `4487923aa2`).
