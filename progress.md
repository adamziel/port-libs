...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,353 pass / 0 fail | [3,313 / 2,276 (145.6%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Implement native wiki readers or writers only through explicit scoped follow-up beads | pandoc-wiki-output-token-taxonomy-de537d8 |
...
Current-branch counters are reconciled through the wiki output token taxonomy slice rebased on `de537d8c`: 3,353 PHP passes, 0 failures, and 3,313 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for `PandocFormatRegistry.php` and `PandocFormatRegistryTest.php`, focused `PandocFormatRegistryTest.php` (`1` file, `1506` assertions, `0` failures), and full `lanes/pandoc/tests` (`46` files, `75592` assertions, `0` failures).
...
| Wiki output writer taxonomy | Landed explicit unsupported writer taxonomy for `dokuwiki`, `jira`, `mediawiki`, `xwiki`, and `zimwiki` output tokens with stable reason payloads, empty native implementation records, upstream provenance, fixture/template evidence, `externalToolFree=true`, and `directWriterParitySupported=false` | 5 wiki output writer tokens | Focused registry run 1,506 assertions | Unsupported no-writer evidence | Implement native wiki writers only through explicit follow-up work; current taxonomy intentionally registers no writer. |
...
### Wiki Output Token Taxonomy Update (2026-06-13)

Bounded native PHP format-registry coverage advanced by one wiki output taxonomy slice after the ODF/ODT ship-ready evidence guard. `PandocFormatRegistry::wikiOutputTokenTaxonomyPacket()` now records `dokuwiki`, `jira`, `mediawiki`, `xwiki`, and `zimwiki` output tokens with stable `wiki-writer-not-ported` reason payloads, upstream writer provenance, fixture/template evidence, empty native implementation records, `externalToolFree=true`, and `directWriterParitySupported=false` while leaving wiki input extension inference unchanged.

Verification passed `php -l` for `PandocFormatRegistry.php` and `PandocFormatRegistryTest.php`; focused `PandocFormatRegistryTest.php` passed (`1` file, `1506` assertions, `0` failures); full `lanes/pandoc/tests` passed (`46` files, `75592` assertions, `0` failures). No Pandoc binary, wiki renderer, browser renderer, Node tooling, online service, live provider test, or external validator was invoked.

Wiki output taxonomy evidence is now 1 local mapped case covering the 5 accepted wiki output writer tokens. This is explicit unsupported/no-writer evidence; broader wiki reader and writer parity remains unsupported until a scoped native implementation bead lands.
...
boundaries, Pandoc inline attribute writer handoff slice, and wiki output token
taxonomy slice.
...
(`1` file, `1506` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`46` files, `75592` assertions, `0` failures after final rebase onto base `de537d8c`).
