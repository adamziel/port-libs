...
| 8 | syncthing | port-syncthing active | cloned static upstream inventory + scanner block parity slice | 4% | Port protocol vector update/merge/compare semantics and add concurrent WordPress edit conflict fixtures. |
...
- Latest root suite: `php tools/run-tests.php` passes 14 test files, 149 assertions, 0 failures.

## Open Blockers

- Five upstream benchmark manifests are still static seed inventories until upstream repos are cloned or queried and their test suites counted: pandoc, difftastic, rclone, dolt, and esbuild. Quadrable, markerPDF, Gitoxide, LightningCSS, libsqlite, Readability, and Syncthing now have stronger static inventories but not upstream runner parity.
...
- Independent audit on 2026-05-22 found `port-pandoc` and `port-syncthing` active while their manifests remained static seeds and their upstream cache worktrees were dirty/empty; Syncthing is now reconciled with a cloned static inventory, while Pandoc still needs denominator/cache repair before more feature breadth.
...
- Worker sessions: `port-pandoc` and `port-syncthing` are active as of the latest audit; Syncthing has denominator repair and should move next to protocol vector semantics, while Pandoc still needs denominator/cache repair before more implementation breadth.

## Next Best Step

Continue Syncthing with `lib/protocol/vector.go` update/merge/compare semantics and concurrent WordPress edit conflict fixtures, while keeping Pandoc focused on restoring/recounting its upstream denominator before more implementation breadth. Keep dashboard PHP pass/fail values treated as local until they are tied to upstream fixture IDs.
