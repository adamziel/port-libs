...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage, active closures | 96.0% | 3,296 pass / 0 fail | [3,256 / 2,276 (143.1%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue format closures | pandoc-markdown-nested-fenced-div-e5fdecf1ff |
...
| --- | --- | --- | ---: | ---: | --- |
| Markdown/CommonMark/GFM | `commonmark`, `commonmark_x`, `gfm`, `markdown`, `markdown_github`, `markdown_mmd`, `markdown_phpextra`, `markdown_strict` | partial | 440 | 1,096 | Complete extension and variant parity. |
...
Markdown fenced Div boundaries now keep precedence over definition-list marker parsing after a preceding paragraph, and `MarkdownWriter` sizes outer Div fences from all colon runs so nested Divs round-trip. Verification passed `php -l` for `MarkdownReader.php`, `MarkdownWriter.php`, and `MarkdownReaderTest.php`, focused `MarkdownReaderTest.php` (`1` file, `6588` assertions, `0` failures), and full `lanes/pandoc/tests` (`44` files, `74002` assertions, `0` failures) after rebase onto current main `e5fdecf1ff`. No Pandoc binary, Cabal/Haskell runner, browser renderer, external validator, online service, live provider test, or live-service provider test was invoked.
...
slice, EPUB3 NCX document metadata provenance slice, XML/HTML/JATS front-matter slice, RIS citation parser slice, JSON/native target tuple sidecar slice, and Markdown nested fenced Div slice.
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/src/MarkdownReader.php lanes/pandoc/src/MarkdownWriter.php lanes/pandoc/tests/MarkdownReaderTest.php`, focused `MarkdownReaderTest.php`, focused `PandocJsonNativeAstTest.php`, and `php tools/run-tests.php lanes/pandoc/tests` passed. | 44 test files, 74,002 assertions, 0 failures. |
...
`php -l lanes/pandoc/tests/MarkdownReaderTest.php`, `php tools/run-tests.php lanes/pandoc/tests/MarkdownReaderTest.php`
(`1` file, `6588` assertions, `0` failures), `php -l lanes/pandoc/src/PandocJsonReader.php`,
`php -l lanes/pandoc/src/NativeReader.php`,
`php -l lanes/pandoc/src/PandocJsonWriter.php`,
`php -l lanes/pandoc/src/NativeWriter.php`,
`php -l lanes/pandoc/tests/PandocJsonNativeAstTest.php`,
`php tools/run-tests.php lanes/pandoc/tests/PandocJsonNativeAstTest.php` (`1` file,
`1968` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`44` files, `74002` assertions, `0` failures on current main `e5fdecf1ff`).
