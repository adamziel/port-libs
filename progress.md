...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,344 pass / 0 fail | [3,303 / 2,276 (145.1%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue JSON/native table metadata and constructor parity | json-native-table-attribute-writer-handoff-8782aad |
...
Current-main counters are reconciled through the JSON/native table attribute writer handoff slice rebased on `8782aad8c`: 3,344 PHP passes, 0 failures, and 3,303 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for `MarkdownWriter.php`, `LatexWriter.php`, and `NativeReaderTest.php`, focused `DocxReaderTest.php`/`NativeReaderTest.php`/`LatexWriterTest.php`/`TableGeometryTest.php`/`MarkdownReaderTest.php` (`5` files, `14091` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `75253` assertions, `0` failures).
...
attribute writer handoff slice, JSON/native mixed table caption/cell flushing
...
| Local passing numerator | `lane-status.json` reports 3,344 PHP passes / 0 failures, and `UPSTREAM_TEST_MANIFEST.json` reports 3,303 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,303 / 2,276 = 145.1%; percentages above 100% reflect local PHP slices being more granular than upstream inventory rows. | High coverage, but not global ship-ready. |
...
(`5` files, `14091` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`45` files, `75253` assertions, `0` failures after final rebase onto base `8782aad8c`).
