...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 6,305 pass / 0 fail | [6,295 / 2,276 (276.6%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue bounded Markdown/CommonMark/GFM extension and variant parity while keeping phpFail at zero | pandoc-markdown-caption-source-numbered-surge |
...
### Markdown Caption Source Numbered Surge (2026-06-15)

Bounded native PHP Markdown reader coverage advances by 60 upstream-mapped table/figure/caption source marker cases after rebase onto current main `3df651d289`. `MarkdownReader` accepts numbered and abbreviated table and figure caption markers including `Table 1:`, `Table 2a:`, `Tbl. 3:`, `Tab. 4:`, `Figure 1:`, `Fig. 2:`, `Fig 3:`, `Image 4:`, and `Caption 5:` across pipe/simple/grid tables and inline/reference/shortcut figures, and records figure-specific `captionSource` provenance on the current figure-caption path.

Verification passed `php -l` for `MarkdownReader.php` and `MarkdownReaderCaptionSourceNumberedSurgeTest.php`; focused `MarkdownReaderCaptionSourceNumberedSurgeTest.php` passed (`1` file, `871` assertions, `0` failures); full `lanes/pandoc/tests` passed (`75` files, `106,946` assertions, `0` failures). No Pandoc binary, cmark/commonmark runner, Cabal/Haskell runner, browser renderer, Node tooling, online service, live provider test, live-service provider test, or external validator was invoked.

Markdown/CommonMark/GFM evidence is now 2,447 local mapped cases against the 1,096 accepted static upstream Markdown rows. The native PHP Markdown reader remains partial pending broader extension and variant parity.
