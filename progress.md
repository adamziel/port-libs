...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,317 pass / 0 fail | [3,276 / 2,276 (143.9%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Implement first wiki/roff/man text reader after unsupported ship gate | pandoc-text-markup-reader-ship-gate-76690ec857 |
...
Current-main counters are reconciled through `76690ec857`: 3,317 PHP passes, 0 failures, and 3,276 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for the touched registry files, focused `PandocFormatRegistryTest.php` (`1` file, `1106` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74434` assertions, `0` failures).
...
| Local passing numerator | `lane-status.json` reports 3,317 PHP passes / 0 failures, and `UPSTREAM_TEST_MANIFEST.json` reports 3,276 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,276 / 2,276 = 143.9%; percentages above 100% reflect local PHP slices being more granular than upstream inventory rows. | High coverage, but not global ship-ready. |
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/src/PandocFormatRegistry.php lanes/pandoc/tests/PandocFormatRegistryTest.php lanes/pandoc/notes/pandoc-text-markup-reader-ship-gate-current-base-20260613T0014Z.md`, syntax checks for `PandocFormatRegistry.php` and `PandocFormatRegistryTest.php`, focused `PandocFormatRegistryTest.php`, and `php tools/run-tests.php lanes/pandoc/tests` passed. | 45 test files, 74,434 assertions, 0 failures. |
...
status, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/src/PandocFormatRegistry.php lanes/pandoc/tests/PandocFormatRegistryTest.php lanes/pandoc/notes/pandoc-text-markup-reader-ship-gate-current-base-20260613T0014Z.md`,
...
(`1` file, `1106` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`45` files, `74434` assertions, `0` failures on current main `76690ec857`).
