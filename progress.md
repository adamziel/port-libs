...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,397 pass / 0 fail | [3,353 / 2,276 (147.3%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue IPYNB/XML parity beyond bounded output display-order diagnostics | ipynb-output-display-order-f4762ff46 |
...
Current-main counters are reconciled through the IPYNB output display-order diagnostics slice rebased on `f4762ff46`: 3,397 PHP passes, 0 failures, and 3,353 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for `IpynbReader.php` and `IpynbReaderTest.php`, focused `IpynbReaderTest.php` (`1` file, `471` assertions, `0` failures), and full `lanes/pandoc/tests` (`46` files, `77981` assertions, `0` failures).
