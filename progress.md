...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,395 pass / 0 fail | [3,351 / 2,276 (147.2%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue CSL/EndNote/XML parity beyond bounded name-group diagnostics | endnote-xml-name-group-diagnostics-f70869e |
...
Current-main counters are reconciled through the EndNote XML name group diagnostics slice rebased on `f70869e`: 3,395 PHP passes, 0 failures, and 3,351 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for `CitationCslProcessor.php` and `CitationCslProcessorTest.php`, focused `CitationCslProcessorTest.php` (`1` file, `5385` assertions, `0` failures), and full `lanes/pandoc/tests` (`46` files, `77866` assertions, `0` failures).
