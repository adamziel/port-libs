...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,452 pass / 0 fail | [3,400 / 2,276 (149.4%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue DocBook XML reader parity after media role/caption diagnostics | docbook-media-role-caption-diagnostics |
...
Current-main counters are reconciled through the DocBook media role/caption diagnostics slice rebased on `fbdb625e7a`: 3,452 PHP passes, 0 failures, and 3,400 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for `XmlHtmlDom.php` and `XmlHtmlDomTest.php`, focused `XmlHtmlDomTest.php`, and full `lanes/pandoc/tests`.
