...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,337 pass / 0 fail | [3,296 / 2,276 (144.8%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue bounded JSON/native citation payload edges | json-native-citation-note-locator-payload-52a27eda |
...
Current-main counters are reconciled through the JSON/native citation note-locator payload slice rebased on `52a27eda2b`: 3,337 PHP passes, 0 failures, and 3,296 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax check for `PandocJsonNativeAstTest.php`, focused `PandocJsonNativeAstTest.php` (`1` file, `2165` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `75077` assertions, `0` failures).
...
| JSON/native table, list, citation, and block handoff | Landed citation note-locator payload, table-caption, note-label, Div Plain, mixed block-container, task-list sidecar, definition-list term-group, and nullary helper payload slices | 252 JSON/native artifacts | 53 JSON/native cases; latest focused JSON/native run 2,165 assertions | Partial | Broader native/json fixture parity, unsupported constructors, and table/citation/metadata round trips. |
...
### JSON/Native Citation Note-Locator Payload Update (2026-06-13)

Bounded native PHP citation payload coverage advanced by one JSON/native slice after CSV/TSV header option parity. `PandocJsonNativeAstTest` now verifies edited citation note numbers and locator-as-`citationSuffix` regeneration from both JSON and native readers while stale citation record sidecars and source ordinals are dropped.

Verification passed `php -l` for `PandocJsonNativeAstTest.php`; focused `PandocJsonNativeAstTest.php` passed (`1` file, `2165` assertions, `0` failures); full `lanes/pandoc/tests` passed (`45` files, `75071` assertions, `0` failures). No Pandoc, JSON filters, Cabal/Haskell runner, browser renderer, external validator, online service, live provider test, or live-service provider test was invoked.
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/tests/PandocJsonNativeAstTest.php`, syntax check for `PandocJsonNativeAstTest.php`, focused `PandocJsonNativeAstTest.php`, and `php tools/run-tests.php lanes/pandoc/tests` passed. | Focused: 1 file, 2,165 assertions, 0 failures. Full: 45 files, 75,077 assertions, 0 failures. |
...
(`1` file, `2165` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`45` files, `75077` assertions, `0` failures after final rebase onto base `52a27eda2b`).
