...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,373 pass / 0 fail | [3,333 / 2,276 (146.4%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue bounded JSON/native constructor parity diagnostics | json-native-figure-child-block-payloads-1b07a9dc6e |
...
Current-main counters are reconciled through the JSON/native Figure child block payload preservation slice rebased on `1b07a9dc6e`: 3,373 PHP passes, 0 failures, and 3,333 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for `PandocJsonWriter.php` and `PandocJsonNativeAstTest.php`, focused `PandocJsonNativeAstTest.php` (`1` file, `2293` assertions, `0` failures), and full `lanes/pandoc/tests` (`46` files, `76232` assertions, `0` failures).
