...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,354 pass / 0 fail | [3,314 / 2,276 (145.6%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue DocBook XML reader parity beyond review-only bibliography/media diagnostics | docbook-bibliography-media-crosslinks-de537d8 |
...
| DocBook/table geometry | `docbook` | partial | 18 | 16 | DocBook structural review diagnostics and bibliography media crosslink diagnostics are covered; finish DocBook XML reader parity, body conversion, inline/block/reference/bibliography mapping, generated AST parity, and broader fixture hydration. |
...
Current-main counters are reconciled through the DocBook bibliography media crosslink slice rebased on `de537d8caa`: 3,354 PHP passes, 0 failures, and 3,314 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for `XmlHtmlDom.php` and `XmlHtmlDomTest.php`, focused `XmlHtmlDomTest.php` (`1` file, `2011` assertions, `0` failures), and full `lanes/pandoc/tests` (`46` files, `75582` assertions, `0` failures).
...
| DocBook structural and bibliography diagnostics | Landed review-only DocBook 4/5 structure packets plus bibliography contributor/title/year summaries, bibliography-local mediaobject references, media target manifests, and resolved/missing/duplicate bibliography-to-media crosslink diagnostics | 16 DocBook/table geometry rows | 18 local DocBook/table geometry cases; latest focused `XmlHtmlDomTest.php` run 2,011 assertions | Partial after bounded review diagnostics | Full DocBook body conversion, inline/block/reference/bibliography mapping, generated AST parity, broader fixture hydration, and figure/media/admonition conversion remain open. |
...
### DocBook Bibliography Media Crosslink Update (2026-06-13)

Bounded native PHP DocBook/XML coverage advanced by one bibliography media crosslink diagnostics slice after JSON/native raw HTML alias preservation. `XmlHtmlDom::summarizeDocBookStructure()` now reports DocBook ID/media target manifests, bibliography entry contributor/title/year summaries, bibliography-local mediaobject references, and resolved, missing, and duplicate crosslink diagnostics for bibliography references to figure and media targets while keeping `directReaderParity=false`.

Verification passed `php -l` for `XmlHtmlDom.php` and `XmlHtmlDomTest.php`; focused `XmlHtmlDomTest.php` passed (`1` file, `2011` assertions, `0` failures); full `lanes/pandoc/tests` passed (`46` files, `75582` assertions, `0` failures). No Pandoc binary, Cabal/Haskell runner, XML validator, browser renderer, Node tooling, online service, live provider test, or external validator was invoked.

DocBook evidence is now 18 local cases against the 16 accepted static upstream DocBook/table geometry rows. DocBook remains partial; full body conversion, inline/block/reference/bibliography mapping, generated AST parity, broader fixture hydration, and figure/media/admonition conversion remain open.
...
commit history agree on the current shipping call after the DocBook bibliography
media crosslink diagnostics slice, JSON/native raw HTML alias preservation slice, PDF/Typst package unsupported-reason slice, JATS/BITS body
...
| Local passing numerator | `lane-status.json` reports 3,354 PHP passes / 0 failures, and `UPSTREAM_TEST_MANIFEST.json` reports 3,314 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,314 / 2,276 = 145.6%; percentages above 100% reflect local PHP slices being more granular than upstream inventory rows. | High coverage, but not global ship-ready. |
...
(`1` file, `2011` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`46` files, `75582` assertions, `0` failures after final rebase onto base `de537d8caa`).
