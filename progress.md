...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,337 pass / 0 fail | [3,296 / 2,276 (144.8%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | JATS/BITS figure media reference diagnostics preserve metadata-only review packets | jats-bits-figure-media-reference-diagnostics-52a27eda |
...
| HTML/XML/JATS DOM | `html` partial; `xml`, `jats`, `bits` unsupported | mixed | 277 | 29 | JATS/BITS front-matter review packets, figure media reference diagnostics, and explicit XML/JATS/BITS direct-reader capability diagnostics are covered; finish HTML5 tree construction and implement full XML/JATS/BITS readers. |
...
Current-main counters are reconciled through the JATS/BITS figure media reference diagnostics slice rebased on `52a27eda2b`: 3,337 PHP passes, 0 failures, and 3,296 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for the touched XML DOM files, focused `XmlHtmlDomTest.php` (`1` file, `1876` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `75078` assertions, `0` failures).
...
| XML/JATS/BITS direct reader capability | Landed explicit unsupported direct-reader capability diagnostics for `xml`, `jats`, and `bits`, plus metadata-only JATS/BITS figure media reference diagnostics | 29 XML/HTML/JATS/DocBook rows | 277 XML/HTML/JATS/DocBook cases; latest focused XmlHtmlDom run 1,876 assertions | Unsupported for full direct readers | Implement full XML/JATS/BITS body, back matter, tables, figures, references, citations, media extraction, and AST parity. |
...
### JATS/BITS Figure Media Reference Update (2026-06-13)

Bounded native PHP XML/JATS/BITS coverage advanced by one metadata-only figure media reference diagnostic slice after Typst package source-class policy provenance. `XmlHtmlDom::summarizeJatsFrontMatter()` now reports figure label, title, caption text, figure-local xref targets, graphic/media href targets, content-type metadata, missing-target issue codes, unsupported external-reference issue codes, and `payloadBytesExposed=false` while preserving `directReaderParity=false`.

Verification passed `php -l` for `XmlHtmlDom.php` and `XmlHtmlDomTest.php`; focused `XmlHtmlDomTest.php` passed (`1` file, `1876` assertions, `0` failures); full `lanes/pandoc/tests` passed (`45` files, `75078` assertions, `0` failures). No Pandoc, external XML validator, browser renderer, Node tooling, online service, live provider test, or media extraction step was invoked.
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check`, syntax checks for `XmlHtmlDom.php` and `XmlHtmlDomTest.php`, focused `XmlHtmlDomTest.php`, and `php tools/run-tests.php lanes/pandoc/tests` passed. | 45 test files, 75,078 assertions, 0 failures. |
...
(`1` file, `1876` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`45` files, `75078` assertions, `0` failures after final rebase onto base `52a27eda2b`).
