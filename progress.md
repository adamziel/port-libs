...
| [markerPDF](lanes/markerpdf/lane-status.json) | Active | PHP green, upstream gap | 100.0% | 3,622 pass / 0 fail | [764 / 78 (979.5%)](lanes/markerpdf/UPSTREAM_TEST_MANIFEST.json) | 0 | No local benchmark-runner blocker remains for native CI surrogate | pending marker... |
...
### ODT Manifest Control-Byte Package Path Preflight (2026-06-16)

Bounded native PHP ODF/ODT package-reader coverage advances by one package-ingestion preflight case. `OpenDocumentPackage` now rejects URI-decoded ASCII control bytes in manifest `full-path` package references before ZIP member lookup, so encoded `%1F` and `%7F` paths cannot reach package resolution while valid encoded spaces and query/fragment suffix handling remain intact. This stays inside native package preflight and does not invoke Pandoc, office suites, zip/unzip, ZipArchive, browser renderers, external validators, online services, live provider tests, or live-service provider tests.

Verification passed `php -l` for `OpenDocumentPackage.php` and `OpenDocumentPackageTest.php`; focused `OpenDocumentPackageTest.php` passed (`1` file, `1,748` assertions, `0` failures); focused ODF/ODT gate passed (`5` files, `7,062` assertions, `0` failures); full `lanes/pandoc/tests` passed (`203` files, `170,718` assertions, `0` failures); JSON validation; and `git diff --check`. ODF/ODT evidence is now 88 local mapped cases against the 20 accepted static upstream ODF/ODT rows, with 0 remaining critical ODF/ODT gaps.
...
| ODF/ODT/OpenDocument | `odt` | ship-ready | 89 | 20 | 0 critical gaps for native PHP ODT import; compact manifest control-byte package path preflight, compact mimetype local-header placement and extra-field preflight, compact raw ZIP name provenance, compact manifest custom attribute collision provenance, compact custom attribute local-name and namespace prefix collision follow-up coverage, manifest media-family classification including generic octet-stream package-path fallback, preferred-view-mode token diagnostics, aggregate manifest encryption review buckets, rich manifest encryption method summaries across blocked package parts, package inventory role and byte-exposure-policy byte buckets, package signature and layout-cache sidecar metadata-only policies, sidecar package byte-exposure ordering, and handoff ordering across missing package parts plus unsupported-compression byte blocks are covered. Continue only non-critical hardening slices as discovered. |
...
| Shippable format gate | ODF/ODT is ship-ready with 89 local mapped cases / 20 upstream ODF/ODT cases, 445.0%, and 0 critical ODF/ODT gaps. | ODF/ODT can ship under the native PHP/no-external-validator policy. |
