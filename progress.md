...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,319 pass / 0 fail | [3,278 / 2,276 (144.0%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue bounded Markdown raw HTML boundary parity after JSON/native alias handoff | pandoc-json-native-html-raw-aliases-4487923aa2 |
...
| JSON/native AST | `json`, `native` | partial | 51 | 252 | Raw HTML alias handoff, nullary helper payload validation, task-list checkbox sidecars, note label sidecars, and fixture writer handoff are preserved; complete broader JSON/native AST constructor coverage. |
...
Current branch counters are reconciled on base `4487923aa2`: 3,319 PHP passes, 0 failures, and 3,278 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for the touched JSON/native raw HTML alias files, focused `PandocJsonNativeAstTest.php` (`1` file, `2127` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74502` assertions, `0` failures).
...
| JSON/native raw HTML aliases | Landed RawBlock/RawInline html4/html5/xhtml alias hydration and writer round-trip preservation | 252 JSON/native artifacts plus raw HTML boundary slices | 51 JSON/native cases; latest focused run 2,127 assertions | Partial | Broader native/json fixture parity, raw adjacency, disabled raw diagnostics, and direct Markdown generic raw policy. |
...
Bounded native PHP raw HTML boundary handoff advanced by one JSON/native AST alias slice. `PandocJsonReader` and `NativeReader` now hydrate `RawBlock` and `RawInline` formats `html4`, `html5`, and `xhtml` as HTML-family raw nodes while preserving the original format labels through `PandocJsonWriter` and `NativeWriter`. The slice proves Markdown raw HTML emission after JSON/native hydration, WordPress raw HTML block/inline handoff, and unsupported raw format suppression.

Verification passed `php -l` for `PandocJsonReader.php`, `NativeReader.php`, and `PandocJsonNativeAstTest.php`; focused `PandocJsonNativeAstTest.php` passed (`1` file, `2127` assertions, `0` failures); full `lanes/pandoc/tests` passed (`45` files, `74502` assertions, `0` failures). No Pandoc, Cabal/Haskell runner, browser renderer, Node tooling, XML validator, online service, live provider, or external validator was invoked.

Remaining raw-boundary gaps: raw block/inline adjacency, disabled raw fallback diagnostics, direct Markdown generic raw policy, and broader CommonMark/GFM raw extension parity.
...
status JSON to list case counters, raw alias inspection in JSON/native readers,
`git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/src/PandocJsonReader.php lanes/pandoc/src/NativeReader.php lanes/pandoc/tests/PandocJsonNativeAstTest.php`,
`php -l lanes/pandoc/src/PandocJsonReader.php`,
`php -l lanes/pandoc/src/NativeReader.php`,
`php -l lanes/pandoc/tests/PandocJsonNativeAstTest.php`,
`php tools/run-tests.php lanes/pandoc/tests/PandocJsonNativeAstTest.php`
(`1` file, `2127` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`45` files, `74502` assertions, `0` failures on current main `4487923aa2`).
