...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,312 pass / 0 fail | [3,272 / 2,276 (143.8%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Mixed block containers are covered after Typst policy slice | json-native-block-container-mixed-content-3a4bf7596e |
...
| JSON/native AST | `json`, `native` | partial | 50 | 252 | Note label sidecars, Div/Plain block boundaries, fixture writer handoff, and block-container mixed-content flushing are covered; complete broader JSON/native AST constructor coverage. |
...
Current-main counters are reconciled through `3a4bf7596e`: 3,312 PHP passes, 0 failures, and 3,272 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, focused `PandocJsonNativeAstTest.php` (`1` file, `2017` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74256` assertions, `0` failures).
...
| JSON/native table and block handoff | Landed table-caption, note-label, Div Plain, and mixed block-container slices | 252 JSON/native artifacts | 50 JSON/native cases; latest focused run 2,017 assertions | Partial | Broader native/json fixture parity, unsupported constructors, and table/citation/metadata round trips. |
...
| Notes/references | Landed footnote label anchors and JSON/native note-label sidecars | 252 JSON/native artifacts plus notes/reference handoff slices | Focused Markdown/citation and JSON/native evidence; latest JSON/native run 2,017 assertions | Partial | Broader note/reference placement, backlinks, and constructor parity. |
...
Bounded native PHP JSON/native AST round-trip coverage advanced by one mixed-content fixture slice from `lanes/pandoc/fixtures/wordpress-import-markdown.md:blockquotes-divs-footnotes`. `PandocJsonWriter` and `NativeWriter` now flush inline runs into `Plain` blocks around nested block children inside `BlockQuote`, `Div`, and inline footnote `Note` containers, preserving nested `CodeBlock`, `OrderedList`, and `BulletList` payloads instead of encoding inline text as invalid block content.

Verification passed `php -l` for `PandocJsonReader.php`, `NativeReader.php`, `PandocJsonWriter.php`, `NativeWriter.php`, and `PandocJsonNativeAstTest.php`; focused `PandocJsonNativeAstTest.php` passed (`1` file, `2017` assertions, `0` failures); full `lanes/pandoc/tests` passed (`45` files, `74256` assertions, `0` failures). No Pandoc binary, JSON filters, Cabal/Haskell runner, browser renderer, TeX engine, Node tooling, online service, live provider, or external validator was invoked.
...
| Format focus | Upstream denominator | Local passing evidence | Evidence percent | Remaining critical gap |
| --- | ---: | ---: | ---: | --- |
| JSON/native AST constructors and round trips | 252 | 50 | 19.8% | Broader upstream native/json fixture parity plus unsupported constructor/table/citation/metadata surfaces beyond bounded sidecar reuse, Markdown fixture writer handoff, note label sidecars, Div/Plain boundaries, and mixed block-container flushing. |
...
(`1` file, `2017` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`45` files, `74256` assertions, `0` failures on current main `3a4bf7596e`).
