...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,364 pass / 0 fail | [3,324 / 2,276 (146.0%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue EPUB/HTML/XML parity after nav/NCX label provenance | epub-nav-ncx-label-provenance-2b56dcf |
...
Current-main counters are reconciled through the EPUB nav/NCX label provenance slice rebased on `2b56dcf`: 3,364 PHP passes, 0 failures, and 3,324 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for `EpubPackageReader.php` and `EpubPackageReaderTest.php`, focused `EpubPackageReaderTest.php` (`1` file, `235` assertions, `0` failures), and full `lanes/pandoc/tests` (`46` files, `75838` assertions, `0` failures).
...
HTML image-map association diagnostics slice, and EPUB nav/NCX label provenance
slice.
...
| Local passing numerator | `lane-status.json` reports 3,364 PHP passes / 0 failures, and `UPSTREAM_TEST_MANIFEST.json` reports 3,324 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,324 / 2,276 = 146.0%; percentages above 100% reflect local PHP slices being more granular than upstream inventory rows. | High coverage, but not global ship-ready. |
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check`, syntax checks for `EpubPackageReader.php` and `EpubPackageReaderTest.php`, focused `EpubPackageReaderTest.php`, and `php tools/run-tests.php lanes/pandoc/tests` passed. | Focused: 1 file, 235 assertions, 0 failures. Full: 46 files, 75,838 assertions, 0 failures. |
...
(`1` file, `235` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`46` files, `75838` assertions, `0` failures after final rebase onto base `2b56dcf`).
`bd orphans --label lane:pandoc` was used for stale-open cleanup, but only
main-ancestor referenced commits were closed. No Pandoc binary, EPUBCheck,
zip/unzip, office suite, TeX/Typst engine, browser engine, Node tooling, or
