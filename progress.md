...
| --- | --- | --- | --- | ---: | --- |
| 1 | gitoxide | stopped | targeted object/ref inventory plus pack index slice | 11% | Map pack data header and entry decoding enough to read non-delta blob/commit objects by pack-index offset. |
...
- Latest root suite: `php tools/run-tests.php` passes 21 test files, 368 assertions, 0 failures.
...
- Gitoxide now has a targeted object/ref static inventory, native tree parsing, native loose direct/symbolic reference parsing/storage, native packed-ref header/reference/peeled lookup parsing, loose+packed reference-store overlay resolution, and v2 pack-index parsing/lookup. There is still no upstream Cargo runner parity, and pack data entry decoding remains unported. Full Cargo was not executed because `.upstream-cache/gitoxide` is sparse/no-checkout and a runner attempt would require materializing crate source plus building Rust dependencies; full workspace `cargo test` would hydrate/build beyond the current VM cap.
