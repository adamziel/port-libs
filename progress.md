...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,391 pass / 0 fail | [3,347 / 2,276 (147.1%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue EPUB3 package reader parity beyond bounded nav fragment target diagnostics | epub-nav-fragment-targets-4720c27d |
...
Current-main counters are reconciled through the EPUB nav fragment target diagnostics slice rebased on `4720c27d`: 3,391 PHP passes, 0 failures, and 3,347 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for `EpubPackageReader.php` and `EpubPackageReaderTest.php`, focused `EpubPackageReaderTest.php` (`1` file, `407` assertions, `0` failures), and full `lanes/pandoc/tests` (`46` files, `77705` assertions, `0` failures).
