...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,347 pass / 0 fail | [3,306 / 2,276 (145.3%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Finish broader JSON/native AST constructor, table, citation, and metadata parity | json-native-table-colspec-sidecars-fb3af69 |
...
| JSON/native table, list, and block handoff | Landed table-caption, table-attribute, mixed table caption/cell flushing, table-colspec, note-label, Div Plain, mixed block-container, task-list sidecar, definition-list term-group, and nullary helper payload slices | 252 JSON/native artifacts | 55 JSON/native cases; latest focused JSON/native run 2,215 assertions | Partial | Broader native/json fixture parity, unsupported constructors, link/raw payloads around block containers, and table/citation/metadata round trips. |
...
Bounded native PHP JSON/native AST coverage advanced by one table column-specification sidecar slice after MediaBag linked-resource MIME inference. `PandocJsonNativeAstTest.php` now covers `Align*` and `ColWidth*` payload sidecars through safe table rebuilds, verifies unchanged colspec payloads survive JSON and native writers, and verifies an edited numeric column width regenerates only that stale width payload while preserving unchanged alignment and neighboring width sidecars.

Verification passed `php -l` for `PandocJsonNativeAstTest.php`; focused `PandocJsonNativeAstTest.php` passed (`1` file, `2215` assertions, `0` failures); full `lanes/pandoc/tests` passed (`45` files, `75352` assertions, `0` failures); `jq empty` passed for `lanes/pandoc/lane-status.json` and `lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`. No Pandoc binary, JSON filters, Cabal/Haskell runner, Node tooling, browser renderer, online service, live provider test, or external validator was invoked.

JSON/native AST evidence is now 55 local cases against 252 accepted static upstream JSON/native artifacts. JSON/native remains partial, not ship-ready; broader native/json fixture parity, unsupported constructor surfaces, and table/citation/metadata round trips remain open.
...
ColSpec sidecar slice, MediaBag linked-resource MIME inference slice, YAML
metadata alias summary slice, JSON/native table attribute writer handoff slice,
...
(`1` file, `2215` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`45` files, `75352` assertions, `0` failures after final rebase onto base `fb3af697a6`).
