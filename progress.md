...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,396 pass / 0 fail | [3,352 / 2,276 (147.3%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue DocBook/XML parity beyond bounded bibliography/reference diagnostics | docbook-bibliography-reference-diagnostics-38c7c63 |
...
| HTML/XML/JATS DOM | `html`, `xml`, `jats`, and `bits` partial | partial | 296 | 29 | Object association provenance, image-map association and area-geometry diagnostics, XML/HTML5 raw-text boundary diagnostics, XML/HTML5 template/noscript boundary handling, XML/HTML5 nested table foster-parenting, XML/JATS/BITS direct input registry routing, JATS/BITS relationship diagnostics, inline xref local back-reference and unsupported target diagnostics, ref-list bibliography diagnostics, reference identifier diagnostics, title metadata propagation, figure label/caption/title metadata diagnostics, figure caption issue/xref metadata diagnostics, front-matter, body, table-body, and back-matter reference diagnostics review packets, and DocBook bibliography/reference, structure, section metadata, and structural/media review packets are covered; finish HTML5 tree construction and full XML/JATS/BITS reader parity. |
...
| DocBook/list/table geometry | `docbook` | partial | 21 | 16 | DocBook bibliography/reference diagnostics, list metadata/diagnostics, structural review, structural/media packets, and section metadata diagnostics are covered; finish DocBook XML reader parity, body conversion, inline/block/reference/bibliography mapping, actual media/admonition conversion, generated AST parity, and broader fixture hydration. |
...
Current-main counters are reconciled through the DocBook bibliography/reference diagnostics slice rebased on `38c7c63`: 3,396 PHP passes, 0 failures, and 3,352 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for `XmlHtmlDom.php` and `XmlHtmlDomTest.php`, focused `XmlHtmlDomTest.php` (`1` file, `2914` assertions, `0` failures), and full `lanes/pandoc/tests` (`46` files, `77924` assertions, `0` failures).
