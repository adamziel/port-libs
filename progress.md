...
| --- | --- | --- | --- | ---: | --- |
| 1 | gitoxide | stopped | targeted object/ref inventory plus pack delta slice | 13% | Map pack traversal/object-database lookup semantics or run a controlled gix-pack crate no-run probe before broadening packed object coverage. |
...
- Latest root suite: `php tools/run-tests.php` passes 22 test files, 401 assertions, 0 failures.
...
- Gitoxide now has a targeted object/ref static inventory, native tree parsing, native loose direct/symbolic reference parsing/storage, native packed-ref header/reference/peeled lookup parsing, loose+packed reference-store overlay resolution, v2 pack-index parsing/lookup, pack data entry decoding, and OFS_DELTA/REF_DELTA resolution. There is still no upstream Cargo runner parity, and broader pack traversal/object-database lookup semantics remain unported. Full Cargo was not executed because `.upstream-cache/gitoxide` is sparse/no-checkout and a runner attempt would require materializing crate source plus building Rust dependencies; full workspace `cargo test` would hydrate/build beyond the current VM cap.
