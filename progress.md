...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,392 pass / 0 fail | [3,348 / 2,276 (147.1%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue JSON/native AST parity beyond bounded citation affix payloads | json-native-citation-affix-payloads-92673acd |
...
Current-main counters are reconciled through the JSON/native citation prefix/suffix payload slice rebased on `92673acd`: 3,392 PHP passes, 0 failures, and 3,348 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax check for `PandocJsonNativeAstTest.php`, focused `PandocJsonNativeAstTest.php` (`1` file, `2633` assertions, `0` failures), and full `lanes/pandoc/tests` (`46` files, `77767` assertions, `0` failures).
...
| JSON/native table, list, raw, block, metadata, citation, and figure handoff | Landed Figure child block payload preservation, nested metadata payload preservation, citation prefix/suffix payload preservation, raw HTML alias preservation, table-caption, table-attribute, mixed table caption/cell flushing, note-label, Div Plain, mixed block-container, mixed Figure link/raw/code handoff, task-list sidecar, definition-list term-group, and nullary helper payload slices | 252 JSON/native artifacts | 59 JSON/native cases; latest focused `PandocJsonNativeAstTest.php` run 2,633 assertions | Partial | Broader native/json fixture parity, unsupported constructors, and table/citation/metadata round trips. |
