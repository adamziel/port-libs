...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,379 pass / 0 fail | [3,339 / 2,276 (146.7%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue next XML/HTML5 DOM or package-reader blocker after raw-text boundary diagnostics | xml-html5-raw-text-boundary-diagnostics-3c006d786a |
...
Current-main counters are reconciled through the XML/HTML5 raw-text boundary diagnostics slice rebased on `3c006d786a`: 3,379 PHP passes, 0 failures, and 3,339 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for `XmlHtmlDom.php`, `Html5DomFragment.php`, `Html5DomFragmentTest.php`, and `XmlHtmlDomTest.php`, focused `Html5DomFragmentTest.php` plus `XmlHtmlDomTest.php` (`2` files, `4987` assertions, `0` failures), and full `lanes/pandoc/tests` (`46` files, `76484` assertions, `0` failures).
