...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,360 pass / 0 fail | [3,320 / 2,276 (145.9%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue JSON/native constructor parity after link/raw block-container fixtures | json-native-link-raw-containers-620639e7 |
...
Current-main counters are reconciled through the JSON/native mixed link/raw block-container fixture slice on base `620639e7`: 3,360 PHP passes, 0 failures, and 3,320 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax check for `PandocJsonNativeAstTest.php`, focused `PandocJsonNativeAstTest.php` (`1` file, `2339` assertions, `0` failures), and full `lanes/pandoc/tests` (`46` files, `75781` assertions, `0` failures).
...
slice, and JSON/native mixed link/raw block-container fixture slice.
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check`, syntax check for `PandocJsonNativeAstTest.php`, focused `PandocJsonNativeAstTest.php`, and `php tools/run-tests.php lanes/pandoc/tests` passed. | Focused: 1 file, 2,339 assertions, 0 failures. Full: 46 files, 75,781 assertions, 0 failures. |
...
status, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/tests/PandocJsonNativeAstTest.php`,
`php -l lanes/pandoc/tests/PandocJsonNativeAstTest.php`,
`php tools/run-tests.php lanes/pandoc/tests/PandocJsonNativeAstTest.php`
(`1` file, `2339` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`46` files, `75781` assertions, `0` failures on base `620639e7`).
