...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,322 pass / 0 fail | [3,281 / 2,276 (144.2%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue XML/HTML5 DOM tree-construction parity after nested table foster-parenting | xml-html5-dom-table-foster-parenting-6dfacd7bba |
...
| HTML/XML/JATS DOM | `html` partial; `xml`, `jats`, `bits` unsupported | mixed | 276 | 29 | JATS/BITS front-matter review packets and nested table foster-parenting are covered; finish HTML5 tree construction and implement full XML/JATS/BITS readers. |
...
Bounded native PHP XML/HTML5 DOM coverage advanced by one table tree-construction slice. `XmlHtmlDomTest.php` now verifies row-level `<em>` content inside `<tr>` and row-group-level `<span>` content inside `<tbody>` are foster-parented before the table, while valid `<caption>`, `<tbody>`, `<tr>`, and `<td>` structure remains inside the table and the deterministic fragment reaches WordPress raw HTML block output.

Verification passed `php -l` for `XmlHtmlDomTest.php`; focused `XmlHtmlDomTest.php` passed (`1` file, `1848` assertions, `0` failures); full `lanes/pandoc/tests` passed (`45` files, `74533` assertions, `0` failures). No Pandoc, Cabal/Haskell runner, browser renderer, Node tooling, online sanitizer, live provider, or external validator was invoked.

### Closure-Wave Evidence Snapshot (2026-06-13)

Current-main counters are reconciled through local XML/HTML5 DOM table foster-parenting evidence on base `6dfacd7bba`: 3,322 PHP passes, 0 failures, and 3,281 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for the touched XML/HTML DOM test file, focused `XmlHtmlDomTest.php` (`1` file, `1848` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74533` assertions, `0` failures).
...
| XML/HTML5 DOM tree construction | Landed nested row/row-group table foster-parenting | 29 HTML/XML/JATS DOM rows | 276 XML/HTML/JATS DOM evidence cases; latest focused XmlHtmlDom run 1,848 assertions | Partial | Broader HTML5 insertion-mode parity and full XML/JATS/BITS readers. |
...
| Local passing numerator | `lane-status.json` reports 3,321 PHP passes / 0 failures, and `UPSTREAM_TEST_MANIFEST.json` reports 3,280 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,280 / 2,276 = 144.1%; percentages above 100% reflect local PHP slices being more granular than upstream inventory rows. | High coverage, but not global ship-ready. |
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/src/EpubPackageReader.php lanes/pandoc/tests/EpubPackageReaderTest.php lanes/pandoc/notes/pandoc-epub-xhtml-definition-list-handoff-current-base-20260613T0121Z.md`, syntax checks for `EpubPackageReader.php` and `EpubPackageReaderTest.php`, focused `EpubPackageReaderTest.php`, and `php tools/run-tests.php lanes/pandoc/tests` passed. | 45 test files, 74,513 assertions, 0 failures. |
...
status, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/src/EpubPackageReader.php lanes/pandoc/tests/EpubPackageReaderTest.php lanes/pandoc/notes/pandoc-epub-xhtml-definition-list-handoff-current-base-20260613T0121Z.md`,
...
(`1` file, `213` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`45` files, `74513` assertions, `0` failures on current main `6dfacd7bba`).
