...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,332 pass / 0 fail | [3,291 / 2,276 (144.6%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue HTML/XML/JATS DOM parity after image-map area diagnostics | xml-html-image-map-area-geometry-70d29eb6 |
...
| HTML/XML/JATS DOM | `html` partial; `xml`, `jats`, `bits` unsupported | mixed | 277 | 29 | HTML image-map area geometry diagnostics, JATS/BITS front-matter review packets, and explicit XML/JATS/BITS direct-reader capability diagnostics are covered; finish HTML5 tree construction and implement full XML/JATS/BITS readers. |
...
Current-main counters are reconciled through the XML/HTML5 image-map area geometry diagnostics slice rebased on `70d29eb6`: 3,332 PHP passes, 0 failures, and 3,291 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for the touched XML/HTML DOM files, focused `XmlHtmlDomTest.php` (`1` file, `1882` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74857` assertions, `0` failures).
...
| XML/JATS/BITS and HTML5 DOM diagnostics | Landed explicit unsupported direct-reader capability diagnostics for `xml`, `jats`, and `bits`, plus HTML image-map area geometry and usemap association diagnostics | 29 XML/HTML/JATS/DocBook rows | 277 XML/HTML/JATS/DocBook cases; latest focused XmlHtmlDom run 1,882 assertions | Unsupported for full direct readers | Implement full XML/JATS/BITS body, back matter, tables, figures, references, citations, HTML5 tree construction, and AST parity. |
...
Bounded native PHP HTML5 DOM review coverage advanced by one image-map diagnostics slice after EPUB XHTML table semantics. `XmlHtmlDom` now summarizes `<area>` shape/coords geometry, invalid coordinate lists, default-area precedence, and usemap resolved/missing/duplicate/invalid association states in reviewer metadata.

Verification passed `php -l` for `XmlHtmlDom.php` and `XmlHtmlDomTest.php`; focused `XmlHtmlDomTest.php` passed (`1` file, `1882` assertions, `0` failures); full `lanes/pandoc/tests` passed (`45` files, `74857` assertions, `0` failures). No Pandoc, browser renderer, Node tooling, online validator, online service, live provider test, or external validator was invoked.
...
| Local passing numerator | `lane-status.json` reports 3,332 PHP passes / 0 failures, and `UPSTREAM_TEST_MANIFEST.json` reports 3,291 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,291 / 2,276 = 144.6%; percentages above 100% reflect local PHP slices being more granular than upstream inventory rows. | High coverage, but not global ship-ready. |
...
(`1` file, `1882` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`45` files, `74857` assertions, `0` failures on current main `70d29eb6`).
