...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,354 pass / 0 fail | [3,314 / 2,276 (145.6%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue notes/reference parity after LaTeX labelled note anchors | latex-note-anchor-round-trip-025c076 |
...
Current-main counters are reconciled through the IPYNB MIME digest collision policy slice rebased on `eb56c4b88e`: 3,355 PHP passes, 0 failures, and 3,315 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for `IpynbReader.php` and `IpynbReaderTest.php`, focused `IpynbReaderTest.php` (`1` file, `249` assertions, `0` failures), and full `lanes/pandoc/tests` (`46` files, `75712` assertions, `0` failures).
...
| IPYNB/notebook reader diagnostics | Landed metadata keys, cell tags, MIME summaries, stream grouping, MIME bundle digests, blocked resource diagnostics, and notebook-level digest collision policy summaries | 1 IPYNB rich-package reader bucket | Focused `IpynbReaderTest.php` run 249 assertions | Covered bounded reader diagnostics | Full Jupyter notebook reader parity, rich output rendering, attachment/media extraction, broader nbformat diagnostics, and native IPYNB writer support remain open. |
...
### IPYNB MIME Digest Collision Policy Update (2026-06-13)

Bounded native PHP IPYNB reader parity advanced by one digest collision policy slice after LaTeX labelled note anchor preservation. `IpynbReader` now reports notebook-level metadata-only MIME bundle digest collision records, cross-cell/same-output/different-group policy counts, display/execute/error grouping summaries, duplicate-policy counters, and byte-blocked diagnostics while keeping output bytes out of AST attributes and writer handoff.

Verification passed `php -l` for `IpynbReader.php` and `IpynbReaderTest.php`; focused `IpynbReaderTest.php` passed (`1` file, `249` assertions, `0` failures); full `lanes/pandoc/tests` passed (`46` files, `75712` assertions, `0` failures). No Pandoc binary, Jupyter, Python notebook runner, Node tooling, browser renderer, online service, live provider test, live-service provider test, or external validator was invoked.

IPYNB reader evidence is now 5 local mapped rich-package cases against the 1 accepted static upstream IPYNB rich-package bucket. IPYNB remains partial; full Jupyter notebook reader parity, rich output rendering, attachment/media extraction, broader nbformat diagnostics, and native IPYNB writer support remain open.
