...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,394 pass / 0 fail | [3,350 / 2,276 (147.2%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue IPYNB/JATS/JSON parity beyond bounded nbformat diagnostics | ipynb-nbformat-diagnostics-39759b |
...
| HTML/XML/JATS DOM | `html`, `xml`, `jats`, and `bits` partial | partial | 295 | 29 | Object association provenance, image-map association and area-geometry diagnostics, XML/HTML5 raw-text boundary diagnostics, XML/HTML5 template/noscript boundary handling, XML/HTML5 nested table foster-parenting, XML/JATS/BITS direct input registry routing, JATS/BITS relationship diagnostics, inline xref local back-reference and unsupported target diagnostics, ref-list bibliography diagnostics, reference identifier diagnostics, title metadata propagation, figure label/caption/title metadata diagnostics, figure caption issue/xref metadata diagnostics, front-matter, body, table-body, and back-matter reference diagnostics review packets, and DocBook structure, section metadata, and structural/media review packets are covered; finish HTML5 tree construction and full XML/JATS/BITS reader parity. |
...
Current-main counters are reconciled through the IPYNB nbformat diagnostics slice rebased on `39759b`: 3,394 PHP passes, 0 failures, and 3,350 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for `IpynbReader.php`, `PandocFormatRegistry.php`, and `IpynbReaderTest.php`, focused `IpynbReaderTest.php` (`1` file, `414` assertions, `0` failures), and full `lanes/pandoc/tests` (`46` files, `77826` assertions, `0` failures).
...
| IPYNB/notebook reader diagnostics | Landed metadata keys, cell tags, MIME summaries, blocked resource diagnostics, metadata-only attachment media extraction plans with safe `ipynb-media/` paths plus unsafe-path/collision diagnostics, and nbformat major/minor plus cells-array schema diagnostics | 1 IPYNB rich-package reader bucket plus bounded nbformat diagnostics slice | Focused `IpynbReaderTest.php` run 414 assertions | Covered bounded reader and nbformat diagnostics | Full Jupyter notebook reader parity, rich output rendering, broader notebook schema parity, and native IPYNB writer support remain open. |
