...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,355 pass / 0 fail | [3,315 / 2,276 (145.7%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue XML/JATS/BITS and DocBook direct reader parity beyond bounded review diagnostics | xml-html-docbook-media-diagnostics-eb56c4b |
...
| DocBook/table geometry | `docbook` | partial | 18 | 16 | DocBook structural and media role/caption diagnostics are covered; finish DocBook XML reader parity, body conversion, inline/block/reference/bibliography/media mapping, generated AST parity, and broader fixture hydration. |
...
Current-main counters are reconciled through the DocBook media role/caption diagnostics slice rebased on `eb56c4b`: 3,355 PHP passes, 0 failures, and 3,315 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for `XmlHtmlDom.php` and `XmlHtmlDomTest.php`, focused `XmlHtmlDomTest.php` (`1` file, `2019` assertions, `0` failures), and full `lanes/pandoc/tests` (`46` files, `75598` assertions, `0` failures).
...
| JSON/native table, list, raw, and block handoff | Landed raw HTML alias preservation, table-caption, table-attribute, mixed table caption/cell flushing, note-label, Div Plain, mixed block-container, task-list sidecar, definition-list term-group, and nullary helper payload slices | 252 JSON/native artifacts | 55 JSON/native cases; latest focused `PandocJsonNativeAstTest.php` run 2,189 assertions | Partial | Broader native/json fixture parity, unsupported constructors, link/raw payloads around block containers, and table/citation/metadata round trips. |
...
| XML/JATS/BITS direct reader capability and body diagnostics | Landed explicit unsupported direct-reader capability diagnostics for `xml`, `jats`, and `bits`, standalone object association provenance, JATS/BITS body roots, section hierarchy, xref resolution, references, figures, table-wraps, unreferenced buckets, BITS book-part body metadata, and DocBook media diagnostics | 29 XML/HTML/JATS/DocBook rows | 279 XML/HTML/JATS/DocBook cases; latest focused `XmlHtmlDomTest.php` run 2,019 assertions | Unsupported for full direct readers | Implement full XML/JATS/BITS body/back matter, tables, figures, references, citations, and AST parity. |
| DocBook structural and media diagnostics | Landed review-only DocBook 4/5 structure packets plus mediaobject/inlinemediaobject roles, caption/title fallbacks, textobject alternatives, media target manifests, repeated role-target pairs, missing caption/alt diagnostics, and unsupported direct-reader diagnostics | 16 DocBook/table geometry rows | 18 local DocBook/table geometry cases; latest focused `XmlHtmlDomTest.php` run 2,019 assertions | Partial after bounded review diagnostics | Full DocBook body conversion, inline/block/reference/bibliography/media mapping, generated AST parity, broader fixture hydration, and figure/media/admonition conversion remain open. |
...
### DocBook Media Role/Caption Diagnostics Update (2026-06-13)

Bounded native PHP DocBook/XML coverage advanced by one media diagnostics slice after JATS/BITS body diagnostics. `XmlHtmlDom::summarizeDocBookStructure()` now reports `mediaobject` and `inlinemediaobject` roles, caption/title fallbacks, `textobject` alternatives, media target manifests, repeated role-target pairs, and missing caption/alt diagnostics while keeping `directReaderParity=false`.

Verification passed `php -l` for `XmlHtmlDom.php` and `XmlHtmlDomTest.php`; focused `XmlHtmlDomTest.php` passed (`1` file, `2019` assertions, `0` failures); full `lanes/pandoc/tests` passed (`46` files, `75554` assertions, `0` failures). No Pandoc binary, browser renderer, Node tooling, external XML validator, online service, live provider test, or live-service provider test was invoked.

XML/HTML/JATS/DocBook DOM evidence is now 279 local cases against the 29 accepted static upstream XML/HTML/JATS/DocBook rows. DocBook remains partial; full body conversion, inline/block/reference/bibliography/media mapping, generated AST parity, broader fixture hydration, and figure/media/admonition conversion remain open.
...
Verdict: not shippable yet. The native PHP lane now has 55 local passing JSON/native AST evidence cases against 252 upstream native expected artifacts (21.8% evidence ratio). Broader native/json fixture parity, unsupported constructor surfaces, and table/citation/metadata/raw round-trip edges remain open.

| Format focus | Upstream denominator | Local passing evidence | Evidence percent | Remaining critical gap |
| --- | ---: | ---: | ---: | --- |
| JSON/native AST constructors and round trips | 252 | 55 | 21.8% | Broader upstream native/json fixture parity plus unsupported constructor/table/citation/metadata/raw surfaces beyond bounded raw HTML aliases, sidecar reuse, Markdown fixture writer handoff, note label sidecars, task-list sidecars, and nullary helper payload validation. |
...
commit history agree on the current shipping call after the LaTeX labelled note
anchor preservation slice, JSON/native raw HTML alias preservation slice, PDF/Typst package unsupported-reason slice, JATS/BITS body
...
| Local passing numerator | `lane-status.json` reports 3,355 PHP passes / 0 failures, and `UPSTREAM_TEST_MANIFEST.json` reports 3,315 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,315 / 2,276 = 145.7%; percentages above 100% reflect local PHP slices being more granular than upstream inventory rows. | High coverage, but not global ship-ready. |
...
(`1` file, `2019` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`46` files, `75598` assertions, `0` failures after final rebase onto base `eb56c4b`).
