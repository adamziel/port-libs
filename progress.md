...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,293 pass / 0 fail | [3,253 / 2,276 (142.9%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | No local blocker | raw-html-boundary-d7a0bfb42d |
...
slice, EPUB3 NCX document metadata provenance slice, and Markdown/WordPress raw
HTML boundary slice.
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, focused `MarkdownReaderTest.php`, focused `PandocJsonNativeAstTest.php`, and `php tools/run-tests.php lanes/pandoc/tests` passed. | 44 test files, 73,903 assertions, 0 failures. |
...
`6581` assertions, `0` failures),
`php tools/run-tests.php lanes/pandoc/tests/PandocJsonNativeAstTest.php` (`1` file,
`1944` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`44` files, `73903` assertions, `0` failures on current main `d7a0bfb42d`).
