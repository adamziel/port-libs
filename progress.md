...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,371 pass / 0 fail | [3,331 / 2,276 (146.4%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue Pandoc parity after ODF/ODT sidecar blocked-byte provenance | pandoc-odf-sidecar-blocked-byte-provenance-dc0ffc9 |
...
| HTML/XML/JATS DOM | `html`, `xml`, `jats`, and `bits` partial | partial | 286 | 29 | Object association provenance, image-map association diagnostics, XML/HTML5 nested table foster-parenting, XML/JATS/BITS direct input registry routing, JATS/BITS relationship diagnostics, title metadata propagation, front-matter, body, table-body, and back-matter reference diagnostics review packets, and DocBook structure and section metadata review packets are covered; finish HTML5 tree construction and full XML/JATS/BITS reader parity. |
...
| EPUB/EPUB3 | `epub` | partial | 63 | 9 | Direct manifest suffix diagnostics, skipped spine-entry reporting, XHTML definition-list handoff, XHTML table-section review metadata, and compact nav/NCX label provenance are covered; finish broader EPUB package reader parity. |
| ODF/ODT/OpenDocument | `odt` | ship-ready | 52 | 20 | 0 critical gaps for native PHP ODT import; Basic script sidecars, RDF metadata-only sidecars, declared/missing media, unsupported-compression blocks, and package ordering provenance are covered. Continue only non-critical hardening slices as discovered. |
...
Current-main counters are reconciled through the ODF/ODT sidecar blocked-byte provenance slice rebased on `dc0ffc9`: 3,371 PHP passes, 0 failures, and 3,331 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for `OdfReader.php` and `OdfReaderTest.php`, focused `OdfReaderTest.php` (`1` file, `4546` assertions, `0` failures), ODF/ODT readiness gate (`5` files, `6027` assertions, `0` failures), and full `lanes/pandoc/tests` (`46` files, `76182` assertions, `0` failures).
...
| ODF/ODT package reader | Landed sidecar blocked-byte provenance for Basic scripts, RDF metadata-only sidecars, declared media, missing media, unsupported-compression media blocks, and package ordering while preserving ship-ready closure | 20 ODF/ODT rows | 52 ODF/ODT cases; latest focused ODF/ODT gate 6,027 assertions | Ship-ready | 0 critical ODF/ODT gaps for native PHP ODT import. |
...
| HTML/XML/JATS DOM diagnostics | Landed image-map association diagnostics for resolved, missing, duplicate, invalid, and unreferenced map states; nested table row-level and row-group foster-parenting; partial direct input registry routing for `xml`, `jats`, and `bits` through `XmlHtmlDom`; unsupported direct-reader parity reason serialization; standalone object association provenance; JATS/BITS relationshipDiagnostics for figure/table/reference xref targets, titleMetadata/subtitleMetadata, section title paths, body roots, section hierarchy, xref resolution, references, figures, table-wraps, table body/row/cell metadata, back-matter ref-lists, reference metadata, citation xref targets, resolved/missing citation ids, unreferenced buckets, BITS book-part body metadata, and DocBook section metadata diagnostics | 29 XML/HTML/JATS/DocBook rows | 286 XML/HTML/JATS/DocBook cases; latest focused `XmlHtmlDomTest.php` run 2,161 assertions | Partial after bounded registry and review diagnostics | Implement full HTML5 tree construction and XML/JATS/BITS body/back matter, tables, figures, references, citations, and AST parity. |
...
slice, and JATS/BITS relationship diagnostics slice.
...
| Local passing numerator | `lane-status.json` reports 3,371 PHP passes / 0 failures, and `UPSTREAM_TEST_MANIFEST.json` reports 3,331 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,331 / 2,276 = 146.4%; percentages above 100% reflect local PHP slices being more granular than upstream inventory rows. | High coverage, but not global ship-ready. |
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check`, syntax checks for `OdfReader.php` and `OdfReaderTest.php`, focused `OdfReaderTest.php`, the ODF/ODT readiness gate, and `php tools/run-tests.php lanes/pandoc/tests` passed. | Focused: 1 file, 4,546 assertions, 0 failures. ODF/ODT gate: 5 files, 6,027 assertions, 0 failures. Full: 46 files, 76,182 assertions, 0 failures. |
...
status, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/src/OdfReader.php lanes/pandoc/tests/OdfReaderTest.php`,
`php -l lanes/pandoc/src/OdfReader.php`,
`php -l lanes/pandoc/tests/OdfReaderTest.php`,
`php tools/run-tests.php lanes/pandoc/tests/OdfReaderTest.php`
(`1` file, `4546` assertions, `0` failures),
...
(`5` files, `6027` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`46` files, `76182` assertions, `0` failures after final rebase onto base `dc0ffc9`).
