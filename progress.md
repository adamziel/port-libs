...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,315 pass / 0 fail | [3,274 / 2,276 (143.8%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue JSON/native table metadata and constructor parity | json-native-table-attribute-writer-handoff-17c91bad52 |
...
| JSON/native AST | `json`, `native` | partial | 51 | 252 | Table attribute handoff, nullary helper payload validation, task-list checkbox sidecars, note label sidecars, and fixture writer handoff are preserved; complete broader JSON/native AST constructor coverage. |
...
Current-main counters are reconciled through `17c91bad52`: 3,315 PHP passes, 0 failures, and 3,274 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, focused `DocxReaderTest.php`/`NativeReaderTest.php`/`LatexWriterTest.php`/`TableGeometryTest.php`/`MarkdownReaderTest.php` (`5` files, `13995` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74339` assertions, `0` failures).
...
| JSON/native table, list, and block handoff | Landed table-caption, table-attribute, note-label, Div Plain, task-list sidecar, and nullary helper payload slices | 252 JSON/native artifacts | 51 JSON/native cases; latest focused touched-path run 13,995 assertions | Partial | Broader native/json fixture parity, unsupported constructors, and table/citation/metadata round trips. |
...
Implemented highest-impact gap: `MarkdownWriter` now emits safe table caption attributes for explicit ids, classes, data/ARIA keys, language, title, role, and direction metadata while keeping `data-docx-*` review-only layout metadata out of pipe-table Markdown. `LatexWriter` now emits figure/table caption labels from explicit AST identifiers while preserving existing short-caption command output, and the native table metadata test proves the same table id/classes/data source through NativeReader/NativeWriter, Markdown, WordPress, and LaTeX writer paths.

Verification passed `php -l` for `MarkdownWriter.php`, `LatexWriter.php`, and `NativeReaderTest.php`; focused `DocxReaderTest.php`/`NativeReaderTest.php`/`LatexWriterTest.php`/`TableGeometryTest.php`/`MarkdownReaderTest.php` passed (`5` files, `13995` assertions, `0` failures); and full `lanes/pandoc/tests` passed (`45` files, `74339` assertions, `0` failures). No Pandoc binary, office suite, TeX engine, browser renderer, Node tooling, online service, live provider test, or external validator was invoked.
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/src/MarkdownWriter.php lanes/pandoc/src/LatexWriter.php lanes/pandoc/tests/NativeReaderTest.php`, syntax checks for `MarkdownWriter.php`, `LatexWriter.php`, and `NativeReaderTest.php`, focused `DocxReaderTest.php`/`NativeReaderTest.php`/`LatexWriterTest.php`/`TableGeometryTest.php`/`MarkdownReaderTest.php`, and `php tools/run-tests.php lanes/pandoc/tests` passed. | 45 test files, 74,339 assertions, 0 failures. |
...
status, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/src/MarkdownWriter.php lanes/pandoc/src/LatexWriter.php lanes/pandoc/tests/NativeReaderTest.php`,
...
`php tools/run-tests.php lanes/pandoc/tests/DocxReaderTest.php lanes/pandoc/tests/NativeReaderTest.php lanes/pandoc/tests/LatexWriterTest.php lanes/pandoc/tests/TableGeometryTest.php lanes/pandoc/tests/MarkdownReaderTest.php`
(`5` files, `13995` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`45` files, `74339` assertions, `0` failures on current main `17c91bad52`).
