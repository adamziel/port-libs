...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,327 pass / 0 fail | [3,286 / 2,276 (144.4%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue JSON/native fixture parity after mixed block-container flushing | json-native-block-container-mixed-content-7b2c5f3a |
...
Current-main counters are reconciled through the JSON/native block-container mixed-content slice rebased on `7b2c5f3a7c`: 3,327 PHP passes, 0 failures, and 3,286 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for the JSON/native reader and writer files, focused `PandocJsonNativeAstTest.php` (`1` file, `2133` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74728` assertions, `0` failures).
...
| Local passing numerator | `lane-status.json` reports 3,327 PHP passes / 0 failures, and `UPSTREAM_TEST_MANIFEST.json` reports 3,286 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,286 / 2,276 = 144.4%; percentages above 100% reflect local PHP slices being more granular than upstream inventory rows. | High coverage, but not global ship-ready. |
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/src/NativeWriter.php lanes/pandoc/src/PandocJsonWriter.php lanes/pandoc/tests/PandocJsonNativeAstTest.php`, syntax checks for `PandocJsonReader.php`, `NativeReader.php`, `PandocJsonWriter.php`, `NativeWriter.php`, and `PandocJsonNativeAstTest.php`, focused `PandocJsonNativeAstTest.php`, and `php tools/run-tests.php lanes/pandoc/tests` passed. | 45 test files, 74,728 assertions, 0 failures. |
...
status, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/src/NativeWriter.php lanes/pandoc/src/PandocJsonWriter.php lanes/pandoc/tests/PandocJsonNativeAstTest.php`,
...
(`45` files, `74728` assertions, `0` failures on current main `7b2c5f3a7c`).
