...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,322 pass / 0 fail | [3,281 / 2,276 (144.2%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue bounded IPYNB, DOCX/OpenXML, EPUB3, shared ZIP/OPC, JSON/native, citation, XML/HTML5 DOM, or PDF/Typst blocker slices | ipynb-attachment-media-diagnostics-6dfacd7bba |
...
| IPYNB/notebook | 3 / skipped denominator | Upstream Pandoc input token intentionally skipped for this phase. | Native diagnostics now cover notebook/cell metadata, MIME/resource summaries, and metadata-only attachment media extraction plans; full Jupyter reader parity, rich output rendering, actual media extraction, broader nbformat diagnostics, and native writer support remain. |

### Closure-Wave Evidence Snapshot (2026-06-13)

Current branch counters are reconciled through `6dfacd7bba` plus the IPYNB diagnostic slices: 3,322 PHP passes, 0 failures, and 3,281 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for the touched IPYNB files, focused `IpynbReaderTest.php` (`1` file, `110` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74554` assertions, `0` failures).
...
| IPYNB reader diagnostics | Landed metadata/MIME/resource diagnostics and metadata-only attachment media extraction plans | Skipped from the active input denominator | 3 mapped IPYNB diagnostic cases; latest focused run 110 assertions | Partial adjacent evidence | Full Jupyter reader parity, rich output rendering, actual media extraction, broader nbformat diagnostics, and native writer support. |
...
### IPYNB Attachment Media Diagnostics Update (2026-06-13)

Bounded native PHP IPYNB reader diagnostics advanced by one attachment-media slice after carrying the prior notebook/cell metadata, MIME summary, and resource policy coverage. `IpynbReader` now reports notebook-level and cell-level attachment media plans, safe `ipynb-media/` paths, unsafe attachment-name diagnostics, and collision disambiguation while keeping attachment payload bytes out of metadata and rendered output.

Verification passed `php -l` for `IpynbReader.php` and `IpynbReaderTest.php`; focused `IpynbReaderTest.php` passed (`1` file, `110` assertions, `0` failures); full `lanes/pandoc/tests` passed (`45` files, `74554` assertions, `0` failures). No Pandoc, Jupyter, Python notebook runner, browser renderer, Node tooling, online service, live provider, live-service provider, or external validator was invoked.
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/src/IpynbReader.php lanes/pandoc/tests/IpynbReaderTest.php`, syntax checks for `IpynbReader.php` and `IpynbReaderTest.php`, focused `IpynbReaderTest.php`, and `php tools/run-tests.php lanes/pandoc/tests` passed. | 45 test files, 74,554 assertions, 0 failures. |
...
(`45` files, `74554` assertions, `0` failures on current main `6dfacd7bba`).
...
TeX/Typst engine, Jupyter, Python notebook runner, browser engine, Node tooling,
or external validator was invoked.
