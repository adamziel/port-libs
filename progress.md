...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,358 pass / 0 fail | [3,318 / 2,276 (145.8%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue IPYNB and JSON/native parity after attachment media diagnostics | ipynb-attachment-media-f70cddd |
...
Current-main counters are reconciled through the IPYNB attachment media diagnostics slice rebased on `f70cdddd`: 3,358 PHP passes, 0 failures, and 3,318 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for `IpynbReader.php` and `IpynbReaderTest.php`, focused `IpynbReaderTest.php` (`1` file, `110` assertions, `0` failures), and full `lanes/pandoc/tests` (`46` files, `75664` assertions, `0` failures).
...
| Local passing numerator | `lane-status.json` reports 3,358 PHP passes / 0 failures, and `UPSTREAM_TEST_MANIFEST.json` reports 3,318 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,318 / 2,276 = 145.8%; percentages above 100% reflect local PHP slices being more granular than upstream inventory rows. | High coverage, but not global ship-ready. |
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check`, syntax checks for `IpynbReader.php` and `IpynbReaderTest.php`, focused `IpynbReaderTest.php`, and `php tools/run-tests.php lanes/pandoc/tests` passed. | Focused: 1 file, 110 assertions, 0 failures. Full: 46 files, 75,664 assertions, 0 failures. |
