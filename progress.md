...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 5,517 pass / 0 fail | [5,507 / 2,276 (242.0%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue bounded Markdown writer inline/link/escape parity while keeping phpFail at zero | pandoc-markdown-writer-softbreak-space-escape-surge |
...
### Markdown Writer Softbreak-Space Escape Surge (2026-06-15)

Bounded native PHP Markdown writer coverage advanced by 92 upstream-mapped Markdown/CommonMark/GFM inline/link/escape cases after rebase onto current main `e5842793e2`. `MarkdownWriter` now treats `softBreak=space` as an inline space when deciding whether following text needs block-start marker escaping, while hard linebreaks and newline-preserving softbreaks still escape rendered line starts.

Verification passed `php -l` for `MarkdownWriter.php` and `MarkdownWriterSoftBreakEscapeSurgeTest.php`; focused writer inline/link/escape tests passed (`4` files, `574` assertions, `0` failures); full `lanes/pandoc/tests` passed (`68` files, `98,567` assertions, `0` failures). No Pandoc binary, cmark/commonmark runner, browser renderer, Node tooling, online service, live provider test, live-service provider test, or external validator was invoked.

Markdown/CommonMark/GFM evidence is now 1,816 local mapped cases against the 1,096 accepted static upstream Markdown rows. The native PHP Markdown reader/writer remains partial pending broader extension and variant parity.

### Markdown Inline Link Entity Surge (2026-06-15)

Bounded native PHP Markdown reader coverage advanced by 50 upstream-mapped CommonMark/Pandoc reference-label cases on top of the current Markdown block/list, raw-attribute, and table-writer surges after rebase onto current main `77ba889184`. `MarkdownReader` now normalizes backslash escapes and decoded HTML entities in reference labels before matching definitions, so escaped punctuation and entity/literal label pairs resolve consistently while preserving rendered link text, target URLs, titles, and WordPress handoff output.

Verification passed `php -l` for `MarkdownReader.php` and `MarkdownReaderInlineLinkEntitySurgeTest.php`; focused Markdown reader surge tests passed (`2` files, `7,221` assertions, `0` failures); full `lanes/pandoc/tests` passed (`50` files, `91,571` assertions, `0` failures). No Pandoc binary, cmark/commonmark runner, browser renderer, Node tooling, online service, live provider test, live-service provider test, or external validator was invoked.

Markdown/CommonMark/GFM evidence is now 672 local mapped cases against the 1,096 accepted static upstream Markdown rows. The native PHP Markdown reader remains partial pending broader inline/link/entity and extension parity.

### EPUB Duplicate Spine IDRef Update (2026-06-15)

Bounded native PHP EPUB3 compact package validation advanced by one duplicate spine idref case after rebase onto current main `df2df82b11`. `EpubPackage` now reports duplicate OPF spine `itemref` `idref` groups without aborting ingestion, preserving each reading-order occurrence with indexes, itemref IDs, selected package part, media type, and linear flags in package validation and WordPress import handoff.

Verification passed `php -l` for `EpubPackage.php` and `EpubPackageTest.php`; focused `EpubPackageTest.php` passed (`1` file, `3,052` assertions, `0` failures); full `lanes/pandoc/tests` passed (`46` files, `87,426` assertions, `0` failures). No Pandoc binary, EPUBCheck, zip/unzip, ZipArchive, browser renderer, online service, live provider test, live-service provider test, or external validator was invoked.

EPUB evidence is now 83 local mapped cases against the 9 accepted static upstream EPUB rows. The native PHP EPUB input package reader remains partial pending broader structural/content parity.
