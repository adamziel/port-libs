...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,377 pass / 0 fail | [3,337 / 2,276 (146.6%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue next XML/HTML5 DOM or package-reader blocker after image-map area geometry diagnostics | xml-html-image-map-area-geometry-464c59879 |
...
| HTML/XML/JATS DOM | `html`, `xml`, `jats`, and `bits` partial | partial | 290 | 29 | Object association provenance, image-map association and area-geometry diagnostics, XML/HTML5 template/noscript boundary handling, XML/HTML5 nested table foster-parenting, XML/JATS/BITS direct input registry routing, JATS/BITS relationship diagnostics, inline xref local back-reference and unsupported target diagnostics, ref-list bibliography diagnostics, title metadata propagation, front-matter, body, table-body, and back-matter reference diagnostics review packets, and DocBook structure and section metadata review packets are covered; finish HTML5 tree construction and full XML/JATS/BITS reader parity. |
...
Current-main counters are reconciled through the XML/HTML5 image-map area geometry diagnostics slice rebased on `464c59879`: 3,377 PHP passes, 0 failures, and 3,337 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for `XmlHtmlDom.php` and `XmlHtmlDomTest.php`, focused `XmlHtmlDomTest.php` (`1` file, `2328` assertions, `0` failures), and full `lanes/pandoc/tests` (`46` files, `76410` assertions, `0` failures).
...
XML/HTML/JATS/DocBook DOM evidence is now 290 local cases against the 29 accepted static upstream XML/HTML/JATS/DocBook rows. HTML5 tree-construction remains partial; broader insertion-mode, malformed content, image-map semantics, and full XML/JATS/BITS direct reader parity remain open.
