...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,319 pass / 0 fail | [3,278 / 2,276 (144.0%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue notes/reference anchor round-trips after LaTeX note anchor preservation | latex-note-anchor-round-trip-4487923aa2 |
...
| LaTeX/TeX/math | `latex` | partial | 21 | 14 | Labelled LaTeX note anchors are covered; finish LaTeX reader and math conversion parity. |
...
Current-main counters are reconciled through `4487923aa2`: 3,319 PHP passes, 0 failures, and 3,278 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for the touched LaTeX files, focused `LatexWriterTest.php` (`1` file, `21` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74480` assertions, `0` failures).
...
| Notes/references | Landed footnote label anchors, WordPress backlink metadata, LaTeX labelled note hypertargets, and JSON/native note-label sidecars | 252 JSON/native artifacts plus notes/reference handoff slices | Focused Markdown/DOCX run 11,750 assertions; latest LaTeX note-anchor run 21 assertions | Partial | Broader note/reference placement, endnote grouping, anchor round-trips, and constructor parity. |
...
### LaTeX Note Anchor Round-Trip Update (2026-06-13)

Bounded native PHP notes handoff advanced by one LaTeX writer slice. `LatexWriter` now emits de-duped `fn-*` hypertargets for valid source footnote labels inside `\footnote{...}` while generated inline notes stay unlabelled; the cross-format test also preserves Markdown labelled-note output and existing WordPress `fn`/`fnref` anchor plus backlink metadata.

Verification passed `php -l` for `LatexWriter.php` and `LatexWriterTest.php`; focused `LatexWriterTest.php` passed (`1` file, `21` assertions, `0` failures); full `lanes/pandoc/tests` passed (`45` files, `74480` assertions, `0` failures). No Pandoc binary, TeX/PDF engine, browser renderer, Node tooling, office suite, online service, live provider, or external validator was invoked.
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/src/LatexWriter.php lanes/pandoc/tests/LatexWriterTest.php`, syntax checks for `LatexWriter.php` and `LatexWriterTest.php`, focused `LatexWriterTest.php`, and `php tools/run-tests.php lanes/pandoc/tests` passed. | 45 test files, 74,480 assertions, 0 failures. |
...
(`1` file, `21` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`45` files, `74480` assertions, `0` failures on current main `4487923aa2`).
