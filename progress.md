...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,333 pass / 0 fail | [3,292 / 2,276 (144.6%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue XML/JATS/BITS reader parity after figure metadata diagnostics | xml-jats-bits-figure-metadata-diagnostics-9ab2f42c |
...
| HTML/XML/JATS DOM | `html` partial; `xml`, `jats`, `bits` unsupported | mixed | 277 | 29 | JATS/BITS front-matter review packets, figure label/caption/title metadata diagnostics, and explicit XML/JATS/BITS direct-reader capability diagnostics are covered; finish HTML5 tree construction and implement full XML/JATS/BITS readers. |
...
Current-branch counters are reconciled through the XML/JATS/BITS figure metadata diagnostics slice rebased on `9ab2f42c1e`: 3,333 PHP passes, 0 failures, and 3,292 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for the XML DOM source and test files, focused `XmlHtmlDomTest.php` (`1` file, `1901` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74889` assertions, `0` failures).
...
| XML/JATS/BITS direct reader diagnostics | Landed explicit unsupported direct-reader capability diagnostics for `xml`, `jats`, and `bits`, plus metadata-only JATS/BITS figure label/caption/title summaries | 29 XML/HTML/JATS/DocBook rows | 277 XML/HTML/JATS/DocBook cases; latest focused XML run 1,901 assertions | Unsupported for full direct readers | Implement full XML/JATS/BITS body, back matter, tables, figures, references, citations, and AST parity. |
...
### JATS/BITS Figure Metadata Diagnostics Update (2026-06-13)

Bounded native PHP XML/JATS/BITS DOM coverage advanced by one metadata-only figure diagnostics slice after shared ZIP/OPC selected-entry handoff role buckets. `XmlHtmlDom::summarizeJatsFrontMatter()` now emits per-figure summaries for `fig` IDs, direct labels, figure or caption titles, caption text, caption paragraphs, aggregate label/caption/title counts, and explicit missing label/caption/title diagnostics while preserving `directReaderParity=false` and avoiding figure rendering or media extraction.

Verification passed `php -l` for `XmlHtmlDom.php` and `XmlHtmlDomTest.php`; focused `XmlHtmlDomTest.php` passed (`1` file, `1901` assertions, `0` failures); full `lanes/pandoc/tests` passed (`45` files, `74889` assertions, `0` failures). No Pandoc, Cabal/Haskell runner, browser renderer, Node tooling, external XML validator, online service, live provider test, or live-service provider test was invoked.
...
| Local passing numerator | `lane-status.json` reports 3,333 PHP passes / 0 failures, and `UPSTREAM_TEST_MANIFEST.json` reports 3,292 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,292 / 2,276 = 144.6%; percentages above 100% reflect local PHP slices being more granular than upstream inventory rows. | High coverage, but not global ship-ready. |
...
(`1` file, `1901` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`45` files, `74889` assertions, `0` failures on current main `9ab2f42c1e`).
