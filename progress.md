...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,363 pass / 0 fail | [3,323 / 2,276 (146.0%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue HTML/XML/JATS DOM parity after image-map association diagnostics | html-image-map-association-diagnostics-7389d56 |
...
Current-main counters are reconciled through the HTML image-map association diagnostics slice rebased on `7389d563`: 3,363 PHP passes, 0 failures, and 3,323 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for `XmlHtmlDom.php` and `XmlHtmlDomTest.php`, focused `XmlHtmlDomTest.php` (`1` file, `2103` assertions, `0` failures), and full `lanes/pandoc/tests` (`46` files, `75816` assertions, `0` failures).
