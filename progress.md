...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,445 pass / 0 fail | [3,394 / 2,276 (149.1%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue bounded direct-reader parity after ODF/ODT signature sidecar review | odt-package-signature-review |
...
| ODF/ODT/OpenDocument | `odt` | ship-ready | 53 | 20 | 0 critical gaps for native PHP ODT import; rich package signature sidecar review plus compact manifest custom attribute provenance and sidecar package byte-exposure ordering are covered. Continue only non-critical hardening slices as discovered. |
...
Current-main counters are reconciled through the CSV/TSV dialect profile registry packet slice rebased on `9977ade1ee`: 3,439 PHP passes, 0 failures, and 3,389 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for `PandocFormatRegistry.php` and `PandocFormatRegistryTest.php`, focused `PandocFormatRegistryTest.php`, and full `lanes/pandoc/tests`.

| Surface | Evidence state | Upstream denominator | Local passing numerator | Ship verdict | Remaining critical gaps |
| --- | --- | ---: | ---: | --- | --- |
| CSV/TSV direct readers and registry options | Landed headed/no-header option parity, auto CSV/TSV format inference diagnostics, tabular registry extension inference, direction buckets, source provenance, option profiles, dialect profile review packets, explicit-format/extension conflict resolution, and unsupported writer reasons | 2 CSV command fixtures plus registry/options harness slices | 10 local CSV/TSV/registry cases | Partial after bounded reader/profile/inference/registry slices | Broader CSV/TSV option behavior, malformed input diagnostics, multiline-cell behavior, and table-reader edge cases. |
| ODF/ODT package reader | Landed rich package signature sidecar review metadata, compact manifest custom file-entry attributes, and rich package sidecar byte-exposure ordering for Basic scripts, RDF metadata, missing media, and unsupported-compression media while preserving ship-ready closure | 20 ODF/ODT rows | 53 ODF/ODT cases; latest focused ODF/ODT gate 6,169 assertions | Ship-ready | 0 critical ODF/ODT gaps for native PHP ODT import. |
...
| Local passing numerator | 53 current mapped ODF/ODT cases and 6,169 focused assertions across `lanes/pandoc/lane-status.json` plus manifest-carried ODF/ODT counters. |
...
| Local focused test files | `OdfOdtShipReadinessStatusTest.php`, `OdfReaderTest.php`, `OdtReaderTest.php`, `OpenDocumentReaderTest.php`, and `OpenDocumentPackageTest.php` cover 225 focused ODF/ODT test cases. |
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check`, focused ODF/ODT suite (`5` files, `6,169` assertions, `0` failures), and full `lanes/pandoc/tests` (`46` files, `80,299` assertions, `0` failures). |
