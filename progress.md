...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,337 pass / 0 fail | [3,296 / 2,276 (144.8%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Implement the first native text-format reader after roff/manual reason taxonomy | pandoc-roff-manual-unsupported-reasons-52a27eda |
...
Current-main counters are reconciled through the roff/manual unsupported reason taxonomy slice rebased on `52a27eda2b`: 3,337 PHP passes, 0 failures, and 3,296 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for the touched registry files, focused `PandocFormatRegistryTest.php` (`1` file, `1368` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `75064` assertions, `0` failures).
...
| Wiki/roff/text reader ship gate | Landed explicit unsupported verdict, diagnostic packet, and stable roff/manual unsupported reason taxonomy for 20 text markup reader input tokens | 20 wiki/roff/man/text markup tokens | Focused registry run 1,368 assertions | Unsupported | Implement native readers after registry-level ship-gate accounting or explicitly defer them. |
...
### Roff/Manual Unsupported Reason Taxonomy Update (2026-06-13)

Bounded native PHP format-registry coverage advanced by one roff/manual unsupported reason taxonomy slice after Typst package source-class policy provenance. `PandocFormatRegistry::textMarkupReaderShipGate()` now serializes stable `roff-manual-native-reader-not-implemented` reason taxonomy for `man` and `mdoc`, including status, reader parity status, review policy, format counts, and per-format reason packets while preserving `directReaderParity=false` and no native PHP reader implementation.

Verification passed `php -l` for `PandocFormatRegistry.php` and `PandocFormatRegistryTest.php`; focused `PandocFormatRegistryTest.php` passed (`1` file, `1368` assertions, `0` failures); full `lanes/pandoc/tests` passed (`45` files, `75064` assertions, `0` failures). No Pandoc binary, roff processor, online validator, online service, live provider test, or external validator was invoked.

Native `man` and `mdoc` reader parity remains unsupported and not implemented; the next text-format gate is the first real native reader or an explicit product-level deferral.
...
(`1` file, `1368` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`45` files, `75064` assertions, `0` failures after refresh onto current main `52a27eda2b`).
