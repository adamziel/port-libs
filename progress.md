...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,430 pass / 0 fail | [3,384 / 2,276 (148.7%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Finish broader JSON/native AST constructor, table, citation, and metadata parity | odf-sidecar-byte-exposure-ordering |
...
| ODF/ODT/OpenDocument | `odt` | ship-ready | 52 | 20 | 0 critical gaps for native PHP ODT import; compact manifest custom attribute provenance and sidecar package byte-exposure ordering are covered. Continue only non-critical hardening slices as discovered. |
...
Current-main counters are reconciled through the JSON/native table ColSpec sidecar slice rebased on `ade0464da`: 3,400 PHP passes, 0 failures, and 3,356 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax check for `PandocJsonNativeAstTest.php`, focused `PandocJsonNativeAstTest.php` (`1` file, `2689` assertions, `0` failures), and full `lanes/pandoc/tests` (`46` files, `78117` assertions, `0` failures).
...
| ODF/ODT package reader | Landed compact manifest custom file-entry attributes and rich package sidecar byte-exposure ordering for Basic scripts, RDF metadata, missing media, and unsupported-compression media while preserving ship-ready closure | 20 ODF/ODT rows | 52 ODF/ODT cases; latest focused ODF/ODT gate 6,101 assertions | Ship-ready | 0 critical ODF/ODT gaps for native PHP ODT import. |
