...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,322 pass / 0 fail | [3,281 / 2,276 (144.2%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue bounded XML/JATS/BITS diagnostics, EPUB3 XHTML structural parity, or the next targeted input blocker slice | pandoc-jats-back-matter-reference-diagnostics-d1e41a7720 |
...
| HTML/XML/JATS DOM | `html` partial; `xml`, `jats`, `bits` unsupported | mixed | 277 | 29 | JATS/BITS front/body and back-matter reference review packets are covered; finish HTML5 tree construction and implement full XML/JATS/BITS readers. |
...
Current-main counters are reconciled through `d1e41a7720`: 3,322 PHP passes, 0 failures, and 3,281 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for the touched XML DOM files, focused `XmlHtmlDomTest.php` (`1` file, `1861` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74546` assertions, `0` failures).
...
| XML/JATS/BITS DOM | Landed front/body and back-matter reference review diagnostics | 29 XML/HTML/JATS-family rows | 277 XML/HTML/JATS/DocBook DOM cases; latest focused XmlHtmlDom run 1,861 assertions | Partial | Full XML/JATS/BITS direct reader AST mapping, tables, figures, and citation-reader parity. |
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, syntax checks for `XmlHtmlDom.php` and `XmlHtmlDomTest.php`, focused `XmlHtmlDomTest.php`, and `php tools/run-tests.php lanes/pandoc/tests` passed. | Focused XML/HTML DOM: 1 file, 1,861 assertions, 0 failures. Full Pandoc lane: 45 test files, 74,546 assertions, 0 failures. |
...
(`1` file, `1861` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`45` files, `74546` assertions, `0` failures on current main `d1e41a7720`).
