...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,368 pass / 0 fail | [3,328 / 2,276 (146.2%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue XML/JATS/BITS direct reader parity after relationship diagnostics | pandoc-xml-html-jats-relationship-diagnostics-6847e50 |
...
Current-main counters are reconciled through the JATS/BITS relationship diagnostics slice rebased on `6847e50`: 3,368 PHP passes, 0 failures, and 3,328 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for `XmlHtmlDom.php` and `XmlHtmlDomTest.php`, focused `XmlHtmlDomTest.php` (`1` file, `2161` assertions, `0` failures), DOM family tests (`5` files, `5096` assertions, `0` failures), and full `lanes/pandoc/tests` (`46` files, `76047` assertions, `0` failures).
...
| Local passing numerator | `lane-status.json` reports 3,368 PHP passes / 0 failures, and `UPSTREAM_TEST_MANIFEST.json` reports 3,328 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,328 / 2,276 = 146.2%; percentages above 100% reflect local PHP slices being more granular than upstream inventory rows. | High coverage, but not global ship-ready. |
...
status, `git diff --check -- progress.md PANDOC_STATUS.md index.html porting.html lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/src/XmlHtmlDom.php lanes/pandoc/tests/XmlHtmlDomTest.php lanes/pandoc/notes/pandoc-xml-html-jats-relationship-diagnostics-20260613.md`,
...
(`5` files, `5096` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`46` files, `76047` assertions, `0` failures after final rebase onto base `6847e50`).
