...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,359 pass / 0 fail | [3,319 / 2,276 (145.8%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | Continue wiki/roff/text reader parity after registry taxonomy | pandoc-wiki-alias-collision-taxonomy-bcf365 |
...
Current-main counters are reconciled through the Pandoc wiki alias collision taxonomy slice on `bcf36593`: 3,359 PHP passes, 0 failures, and 3,319 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for `PandocFormatRegistry.php` and `PandocFormatRegistryTest.php`, focused `PandocFormatRegistryTest.php` (`1` file, `1497` assertions, `0` failures), and full `lanes/pandoc/tests` (`46` files, `75736` assertions, `0` failures).
...
| Wiki/roff/text reader ship gate | Landed explicit unsupported verdict, diagnostic packet, and wiki alias collision taxonomy for wiki-suffix and `.wiki` MediaWiki/Vimwiki collisions | 20 wiki/roff/man/text markup tokens | Focused registry run 1,497 assertions | Unsupported | Implement native readers after registry-level ship-gate accounting or explicitly defer them. |
...
### Pandoc Wiki Alias Collision Taxonomy Update (2026-06-13)

Bounded native PHP format-registry coverage advanced by one wiki alias collision taxonomy slice after CSL citation affix review metadata. `PandocFormatRegistry` now exposes `wikiAliasCollisionDiagnostics()` and `wikiAliasCollisionReviewPacket()` for the `wiki` token-suffix family collision and the `.wiki` MediaWiki/Vimwiki fixture-extension collision while preserving `.wiki => mediawiki` extension inference.

The diagnostic is no-parser/no-writer taxonomy evidence only: unsupported reader/writer reason payloads are stable, native implementation records stay empty, `externalToolFree=true`, `directReaderParitySupported=false`, and `directWriterParitySupported=false`.

Verification passed `php -l` for `PandocFormatRegistry.php` and `PandocFormatRegistryTest.php`; focused `PandocFormatRegistryTest.php` passed (`1` file, `1497` assertions, `0` failures); full `lanes/pandoc/tests` passed (`46` files, `75634` assertions, `0` failures). No Pandoc binary, wiki converter, browser renderer, Node tooling, online service, live provider test, or external validator was invoked.
...
commit history agree on the current shipping call after the wiki alias collision
taxonomy slice, CSL citation affix review metadata slice, LaTeX labelled note anchor preservation slice, JSON/native raw HTML alias preservation slice, PDF/Typst package unsupported-reason slice, JATS/BITS body
...
and IPYNB attachment media diagnostics slice.
...
status, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/src/PandocFormatRegistry.php lanes/pandoc/tests/PandocFormatRegistryTest.php lanes/pandoc/notes/pandoc-wiki-alias-collision-taxonomy-20260613.md`,
...
(`1` file, `1497` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`46` files, `75736` assertions, `0` failures after final rebase onto base `bcf36593`).
