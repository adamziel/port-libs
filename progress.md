...
| [pandoc](lanes/pandoc/lane-status.json) | Backlog | High coverage | 96.0% | 3,326 pass / 0 fail | [3,285 / 2,276 (144.3%)](lanes/pandoc/UPSTREAM_TEST_MANIFEST.json) | 0 | JATS/BITS inline xref diagnostics covered; continue citation/reference parity | jats-inline-xref-diagnostics-7bc8d592 |
...
| --- | --- | --- | ---: | ---: | --- |
| Markdown/CommonMark/GFM | `commonmark`, `commonmark_x`, `gfm`, `markdown`, `markdown_github`, `markdown_mmd`, `markdown_phpextra`, `markdown_strict` | partial | 448 | 1,096 | Escaped fenced-Div attributes, fenced Div section-reference boundaries, nested-list fixture round-trip, definition-list term-group handoff, table-cell note placement, and header section Div mapping are covered; complete extension and variant parity. |
| HTML/XML/JATS DOM | `html` partial; `xml`, `jats`, `bits` unsupported | mixed | 276 | 29 | JATS/BITS front-matter review packets and inline xref diagnostics are covered; finish HTML5 tree construction and implement full XML/JATS/BITS readers. |
...
Current-main counters are reconciled through `7bc8d59244`: 3,326 PHP passes, 0 failures, and 3,285 mapped upstream cases out of the accepted 2,276-row static upstream inventory. Rows below summarize the recently landed closure-wave evidence; they are factual evidence counters, not global ship-ready claims. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for the touched XML/HTML DOM files, focused `XmlHtmlDomTest.php` (`1` file, `1859` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74652` assertions, `0` failures).
...
| Markdown block/list/note/section boundaries | Landed nested fenced Div, escaped fenced-Div attributes, fenced Div section-reference boundaries, mixed-content nested-list, definition-list term-group, table-cell note placement, and header section Div slices | 1,096 Markdown-family rows | 448 Markdown-family cases; latest focused MarkdownReader run 6,685 assertions | Partial | Markdown/CommonMark/GFM extension and variant parity. |
...
| JATS/BITS XML-ish inputs | covered in the 29-case family | bounded JATS/BITS front-matter review packet plus inline xref local-reference and unsupported-target diagnostics | Implement full JATS/BITS body, back matter, tables, figures, references, and citation-reader parity. |
...
Latest bounded gap: `XmlHtmlDom::summarizeJatsFrontMatter()` now emits safe XML review packets for JATS `article` and BITS `book`/`book-part` front matter, plus inline body/book-body xref diagnostics for local back-matter references and unsupported missing or non-reference targets. Verification passed `php -l` for `XmlHtmlDom.php` and `XmlHtmlDomTest.php`, focused `XmlHtmlDomTest.php` (`1` file, `1859` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74544` assertions, `0` failures). No Pandoc binary, browser renderer, external XML validator, online service, live provider test, or live-service provider test was invoked.
...
| Local passing numerator | `lane-status.json` reports 3,326 PHP passes / 0 failures, and `UPSTREAM_TEST_MANIFEST.json` reports 3,285 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,285 / 2,276 = 144.3%; percentages above 100% reflect local PHP slices being more granular than upstream inventory rows. | High coverage, but not global ship-ready. |
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/src/XmlHtmlDom.php lanes/pandoc/tests/XmlHtmlDomTest.php`, syntax checks for `XmlHtmlDom.php` and `XmlHtmlDomTest.php`, focused `XmlHtmlDomTest.php`, and `php tools/run-tests.php lanes/pandoc/tests` passed. | 45 test files, 74,652 assertions, 0 failures. |
...
(`1` file, `1859` assertions, `0` failures), and `php tools/run-tests.php lanes/pandoc/tests`
(`45` files, `74652` assertions, `0` failures on current main `7bc8d59244`).
