# Independent Audit - 2026-05-23T16:10:00Z
...
the source records behind the dashboard and status claims.
...
Sampled `HEAD`: `21c8222af125` (`Refresh independent audit status`). Recent
...
| --- | ---: | ---: | ---: | ---: | --- |
| difftastic | 604 inspected artifacts | 297 | 297 / 0 | 95% | pending in shared dirty worktree |
| dolt | 613 upstream test files | 574 | 310 / 0 | 95% | not committed |
| esbuild | 2,567 test entry points | 255 | 255 / 0 | 77% | uncommitted |
...
| markerPDF | 289 | 237 | 363 / 0 | 95% | uncommitted |
| pandoc | 2276 inspected artifacts | 752 | 232 / 0 | 98% | pending |
| quadrable | 55 tracked upstream paths | 55 | 152 / 0 | 99% | pending lane batch |
| rclone | 1601 | 527 | 527 / 0 | 98% | pending lane-local changes |
...
1. **Critical - there is still no stable integration snapshot suitable for an accepted root-test baseline.**
   ...
   - Goal requirement at risk: `goal.md` requires capped supervised lane
     work, current owner/session tracking, small committed slices with passing
   ...
   - Evidence: process sampling found active lane/status processes despite
     the stopped table, including `run-tmux-agent` PIDs `2218606`
     (libsqlite), `2222778` (esbuild), `2224907` (syncthing), `2228322`
     (gitoxide), `2230169` (quadrable), `2231694` (lightningcss), `2235650`
     (difftastic), `2237403` (dolt), `2237508` (auditor), `2240765`
     (markerpdf), `2240909` (pandoc), `2241281` (rclone), `2241434`
     (dolt-runner), `2241578` (integrator), plus dashboard updater PID
     `2222131`, team watchdog PID `2347911`, evaluator PID `2424048`, and
     capacity controller PID `2452997`.
   - Evidence: current dirty samples reported `204` tracked dirty rows,
     `2157` all-status rows, and `204 files changed, 84457 insertions(+),
     6542 deletions(-)`.
   - Evidence: the required duplicate-root gate initially matched focused PHP
     harness PID `2238244` (`php tools/run-tests.php lanes/syncthing/tests`);
     owner sampling returned no row because the process exited before `ps`
     could sample it. A later exact gate returned no rows, but the tree was
     still not stable enough for a representative root run.
   - Audit judgment: `php tools/run-tests.php` would not describe one
     accepted snapshot while writers and status publishers are active against
     this broad dirty tree.
...
2. **Critical - `porting.html` and `porting-summary.json` remain stale and fail the dashboard contract.**
   ...
   - Evidence: `porting.html:32` still says generated
     ...
     still identify source snapshot `bda83c6b93d4`, while sampled `HEAD` is
     `21c8222af125`.
   - Evidence: published row counts disagree with current manifests/status
     ...
     manifest/status is `297 / 604`; Dolt is `242 / 613` while current is
     `574 / 613`; Gitoxide is `1432 / 2877` while current is
     `2136 / 2877`; markerPDF is `159 / 78` while current is `237 / 289`;
     rclone is `291 / 327` while current is `527 / 1601`; Readability is
     `1031 / 1984` while current is `1821 / 1984`; Syncthing is
     `235 / 658` while current is `431 / 658`.
...
3. **High - `progress.md` does not describe the active system.**
   - Paths: `progress.md:25`, `progress.md:31` through `progress.md:42`,
     `progress.md:346`, `lanes/*/lane-status.json:4`,
   ...
   - Evidence: the active-lane table still shows all sessions as `stopped`
     ...
     `77%` to `99%` and process sampling shows many lanes running.
   - Evidence: `progress.md:346` already records the same stale-dashboard,
     active-writer, non-comparable-count, and pending-dirty-batch blocker from
     the prior audit, but the top active-lane table and generated dashboard
     remain unreconciled.
...
4. **High - claimed lane progress remains mostly unaccepted dirty-batch work, not small committed slices.**
   ...
   - Evidence: every lane's latest-commit/status field says `pending`,
     ...
     prose. LightningCSS embeds an old audit `HEAD` (`01d192b9`) while still
     saying the lane batch is uncommitted.
...
5. **High - near-complete percentages and "no blocker" language overstate accepted upstream parity.**
   ...
   - Evidence: nine lanes are marked `95%` to `99%`. Those estimates coexist
     ...
     stale dashboard output, skipped full upstream runners, and explicit
     ...
     Haskell runner, Syncthing full upstream Go runner, and Quadrable full
     sync-fuzzer and benchmark runs.
...
6. **High - manifest/status count schemas are still not normalized or comparable.**
   ...
   - Evidence: denominator `total` is numeric in some manifests and prose in
     ...
     upstream files, upstream test functions, copied fixture checks, or local
     behavior checks.
   - Evidence: PHP pass units are not comparable. Gitoxide maps `2136`
     upstream units but reports `3965` PHP passes; LightningCSS maps `1362`
     but reports `1536` PHP passes; Readability maps `1821` but reports `163`
     PHP passes; Syncthing maps `431` but reports `3029` PHP passes.
...
7. **Medium - progress accounting still blends non-native, copied, live-provider-skipped, and plan-only artifacts with native parity.**
   ...
   - Evidence: markerPDF counts supplied-document excerpts, package/lock
     ...
     and `not-executed` runner status while still claiming `95%`.
   - Evidence: Readability counts copied Mozilla fixture/oracle coverage while
     PHP behavior tests are a much smaller unit; rclone records live
     provider/mount parity as explicitly open while still claiming `98%`;
     Gitoxide properly excludes shell-backed filter/askpass execution from
     mapped progress, but still reports `98%` with real process launch and
     full runner parity left open.
...
The initial required gate matched:

```text
2238244 php tools/run-tests.php lanes/syncthing/tests
```

Owner sampling with `ps -o pid=,user=,lstart=,etime=,cmd= -p 2238244`
returned no row because the process exited before sampling. A later exact gate
returned no rows. No root run was started because the stability gate failed:
active lane agents, watchdog/status publishers, dashboard/evaluator/capacity
and integrator loops, pending dirty lane batches, recent audit-only history,
and stale generated dashboards persisted.
