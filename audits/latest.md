...
1. **Critical - the repository is green but still not a reviewable integration checkpoint.**
   ...
   - Evidence: `git status --short` moved repeatedly during this audit and the latest post-commit sample still reports 189 entries. The latest sampled `git diff --stat` reports 52 modified tracked files with 4,427 insertions and 250 deletions, plus many untracked audit/evidence files and untracked lane files such as `lanes/difftastic/src/FileContentDecoder.php`, `lanes/libsqlite/src/SQLiteTrimIndexExpression.php`, `lanes/lightningcss/examples/wordpress-import-supports.php`, and `lanes/rclone/examples/wordpress-provider-id-duplicate-upload-dirs.php`.
...
3. **High - `progress.md` Active Lanes remains stale and conflicts with current lane status files.**
   ...
   - Evidence: `progress.md` still reports Gitoxide `66%`, LightningCSS `14%`, markerPDF `10%`, libsqlite `12%`, Quadrable `8%`, Syncthing `8%`, Dolt `5%`, and esbuild `8%`. Latest sampled lane status files report Gitoxide `87%`, LightningCSS `48%`, markerPDF `43%`, libsqlite `51%`, Quadrable `57%`, Syncthing `47%`, Dolt `34%`, and esbuild `32%`.
...
6. **Medium - high percentages can be overread as upstream parity.**
   ...
   - Evidence: Gitoxide reports `87%` while the full Cargo workspace runner is not executed. Difftastic reports `34%` with no full upstream runner. Dolt reports `34%` while full `go test ./...` and full BATS are not run. markerPDF reports `43%` while the full benchmark runner is not executed. Pandoc reports `49%` while the full Haskell runner is not executed. Syncthing reports `47%` while full `go test ./...` is not executed.
