...
4. **High - root-test evidence remains non-comparable across lane records.**
   ...
   - Evidence: the exact duplicate-root gate returned no active
     `php tools/run-tests.php` process during the final pre-commit audit
     samples. A post-commit handoff check briefly returned focused-lane PID
     `3049224 php tools/run-tests.php lanes/syncthing/tests`, which exited
     before owner sampling; a later handoff gate found active root PID
     `3057590`. The tree was still not stable enough for a trustworthy root
     run because active writer/status loops and `HEAD` movement persisted.
...
Observed result during final pre-commit audit samples: no active exact
`php tools/run-tests.php` process. A post-commit handoff check briefly returned:

```text
3049224 php tools/run-tests.php lanes/syncthing/tests
```

The process exited before owner sampling; `ps -o pid,user,ppid,etime,stat,args
-p 3049224` returned only the header. A later handoff gate found active root
PID `3057590`:

```text
3057590 claude 2989965 00:38 Rs php tools/run-tests.php
```

No duplicate root run was started. The stability gate also failed: active
writer/status loops persisted and `HEAD` moved during the audit.
