# Independent Audit - 2026-05-24T23:08Z

Scope reviewed: `goal.md`, `progress.md`, `porting.html`, `dependency-backlog.json`, every `lanes/*/UPSTREAM_TEST_MANIFEST.json`, every `lanes/*/lane-status.json`, and recent Git history through `b6959f80f8b1`.
...
```text
UTC sample: 2026-05-24T23:08:21Z
HEAD: b6959f80f8b1
recent history: b6959f80 Correct isolated integration root counts; 4cb1c5b Integrate Syncthing folder scan route registry slice; b81017b Integrate Pandoc inline code attribute slice; 958011b Integrate markerPDF literal escape slice; 9a12cb4 Refresh independent audit status; 7f910f2 Integrate libsqlite JSON minmax RHS slice; 0578b4f Refresh independent audit status; e77b7bc Teach integrators to consume isolated marker metadata; fa51cbf Refresh independent audit status; e1ec427 Add esbuild tsconfig runtime fixture; b38a88a Record Readability handoff rejection; cd2c083 Refresh independent audit status
status rows including untracked: 25392
tracked dirty rows: 254
dirty shortstat: 250 files changed, 198362 insertions(+), 26663 deletions(-)
targeted status: all 12 lane manifests and all 12 lane-status files are dirty or live handoff metadata
exact no-argument root process gate: empty at audit sample (`pgrep -af '^php tools/run-tests\.php$'`)
...
1. **Critical - the repository still lacks a stable aggregate acceptance baseline.**
   ...
   - Evidence: `HEAD` is `b6959f80f8b1`, but the checkout has `25392` untracked-inclusive status rows, `254` tracked dirty rows, and `250 files changed` in the dirty shortstat. Every lane manifest/status file is dirty or live handoff metadata, so a root result would describe a mixed pile rather than one reviewable batch.

2. **Critical - root harness execution remains unattributable even when the exact process gate is empty.**
   ...
   - Evidence: `pgrep -af '^php tools/run-tests\.php$'` was empty at the audit sample, but all lane manifests/status files are moving handoff metadata and the dirty tree is not owner-free. Starting `php tools/run-tests.php` now would not prove any single accepted slice is safe.

3. **Critical - accepted commits are still being overwritten in the status surface by newer unaccepted lane metadata.**
   ...
   - Evidence: `porting.html` names accepted source snapshot `6cb369fd15d0`, while live statuses advertise later package-tsconfig, standalone `ins`, indirect PDF stream filter, position-try formatter, tree-offset, PROPFIND, route-registry, and other handoffs that are pending or uncommitted.
...
4. **High - `porting.html` is stale and internally mixed with pending state.**
   ...
   - Evidence: the page says it is a verified snapshot of `6cb369fd15d0`, but rows include newer pending status text such as LightningCSS `2026-05-24T22:55Z`, markerPDF `2026-05-25 00:08 UTC`, `pending`, `not com`, `HEAD 9c`, and average progress `93.3%`. That is not a clean accepted snapshot.
...
5. **High - manifest/status schemas still make progress math unreliable.**
   ...
   - Evidence: the manifests do not expose common top-level denominator/mapped/pass/fail fields; status files leave `upstreamDenominator` and `upstreamMapped` null while reporting `phpPass` as behavior counts, assertion counts, or PASS-line counts depending on lane. The dashboard still shows markerPDF at `176 / 78` mapped and Pandoc at `2276 / 2276` mapped without upstream Haskell runner parity.
...
9. **High - markerPDF still risks counting narrow PDF plumbing and dependency planning as structured-content extraction progress.**
   ...
   - Evidence: live status is narrow indirect stream Filter/DecodeParms handling. Required support rows for PDF text dictionaries, page layout, OCR/layout result, tables, Unicode/charset, and archive/package handling remain inactive and have no dependency-specific pass/fail ledgers.
