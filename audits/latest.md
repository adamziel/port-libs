...
1. **Critical - the visible coordination surfaces are stale against the current manifests and latest root test run.**
   ...
   - Evidence: `porting.html` and `porting-summary.json` were generated at `2026-05-22 15:40:20 UTC` and still show mapped counts `15/5/16/737/18/78/11/19/24/20/89/27`. Current manifests report difftastic `58`, Dolt `74`, esbuild `68`, Gitoxide `1001`, libsqlite `58`, LightningCSS `198`, markerPDF `78`, Pandoc `127`, Quadrable `55`, rclone `98`, Readability `352`, and Syncthing `90`. The latest root run is green with `110 test files, 7961 assertions, 0 failures`. Before this audit update, `progress.md` still recorded the older `109 test files / 7887 assertions` result, and its Active Lanes table still has stale phases/next tasks.
...
2. **High - the worktree is still too broad to be a reviewable integration checkpoint.**
   ...
   - Evidence: `git status --short` still shows modified implementation, test, fixture, manifest, note, status, dashboard, and audit files across multiple lanes, plus untracked fixtures/examples and audit files. Excluding this audit update, `git diff --stat` reports 17 changed tracked files with 2564 insertions and 95 deletions before this audit update.
...
```text
Exit status: 0
110 test files, 7961 assertions, 0 failures
```
