# Independent Audit - 2026-05-25T00:22Z

Scope reviewed: `goal.md`, `progress.md`, `porting.html`, `dependency-backlog.json`, every `lanes/*/UPSTREAM_TEST_MANIFEST.json`, sampled current `lanes/*/lane-status.json`, and recent Git history through `1cc1dee187f1`.
...
```text
UTC sample: 2026-05-25T00:22Z
HEAD: 1cc1dee187f1
recent history: 1cc1dee1 Defer duplicate isolated pandoc quoted inline patch; 3f3bc706 Integrate isolated pandoc quoted inline slice; 93cab519 Refresh independent audit status; 5b0db1ca Refresh independent audit status; 7452fe33 Record isolated queue cleanup; d35bf1db Record remaining isolated queue deferrals; 7b931c65 Record isolated patch queue deferrals; 66c8615c Integrate isolated syncthing watchdog-next-20260524T233852Z slice
status rows including untracked: 26336
tracked dirty rows: 288
dirty shortstat: 288 files changed, 194413 insertions(+), 64378 deletions(-)
...
dependency backlog: 37 rows; statuses blocked=1, candidate=25, deferred=11; 0 active support ports; sampled dependency rows lack dependency-specific denominator/pass-fail fields
...
1. **Critical - aggregate acceptance is still blocked by the moving dirty checkout.**
   ...
   - Evidence: `HEAD` is now `1cc1dee187f1`, but the checkout has `26336` untracked-inclusive status rows, `288` tracked dirty rows, and `288 files changed` in the dirty shortstat. Every lane manifest and sampled lane-status file is dirty/live handoff metadata, so no lane output is accepted by this audit.
...
3. **Critical - dashboard/status alignment is not trustworthy.**
   ...
   - Evidence: `porting.html` says it is generated from source commit `6cb369fd15d0` with average progress `93.3%`, while `HEAD` is `1cc1dee187f1` and rows include pending newer lane text plus `pending` commit fields. This mixes accepted and unaccepted status.
...
4. **High - manifest/status schemas still prevent reliable progress math.**
   ...
   - Evidence: all sampled `lane-status.json` files still have top-level `upstreamDenominator`, `mappedTests`, and `benchmarkSource` as `null`, while manifests carry nested lane-specific denominator shapes. `phpPass` continues to mean assertions, behavior tests, or PASS-line counts depending on lane, making dashboard ratios like markerPDF `176 / 78` and Pandoc `2276 / 2276` misleading.
...
5. **High - support-library rows are visible routing only, not first-class completed ports.**
   ...
   - Evidence: the backlog has 37 rows but no active support port. Sampled rows, including document/PDF/package/XML/Unicode/charset/JSON/YAML/archive rows, have no `upstreamDenominator`, `phpPass`, or `phpFail` values. Candidate/deferred rows should not be counted as implementation progress.
...
9. **Medium - broad dirty lane piles are still not reviewable units.**
   ...
   - Evidence: sampled statuses advertise many interleaved uncommitted slices in the same lane files. Readability reports eighteen interleaved fixture slices; Quadrable reports a long mixed docopt/sync/LMDB/proof stack; Gitoxide says prior stacks cannot be independently isolated from the dirty worktree.
...
I did not run `php tools/run-tests.php`. The required exact process gate was empty at the audit sample, but the checkout is not stable enough for a meaningful audit-owned aggregate run because all lane manifests/status files remain dirty/live and the dashboard is not generated from the current accepted state.
