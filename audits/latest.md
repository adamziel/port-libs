# Independent Audit - 2026-05-23T07:44:20Z
...
through `4f597d6f`, dirty-tree status, active process/test state, and the PHP
shell-out surface.
...
1. **Critical - there is still no stable integration snapshot to audit or
   ...
   - Evidence: `progress.md:25` declares a two-implementation-worker plus
     ...
     dashboard, evaluator, auditor, artifact, verifier, lane-agent, and root
     test activity.
   - Evidence: latest samples report `926` default `git status --short`
     entries, `99` tracked changed entries, and
     `99 files changed, 21732 insertions(+), 561 deletions(-)`.
   - Evidence: the required duplicate-root gate initially returned an active
     exact root harness, `1672240 php tools/run-tests.php`; that process exited
     before owner sampling. A later exact gate returned active root PID
     `1766434 php tools/run-tests.php`, with owner evidence
     `1766434 claude 1604183 00:35 Rs php tools/run-tests.php`.
...
2. **High - root-test state is contradictory across lane records and cannot be
   ...
   - Evidence: Dolt, Readability, Syncthing, Quadrable, and Pandoc record
     aggregate root runs as green around `196` test files and roughly
     `21349`-`21368` assertions. Rclone records a later aggregate root failure:
     `196` test files, `21450` assertions, and `1` unrelated Difftastic
     failure in `lanes/difftastic/tests/TokenDifferTest.php`.
   - Evidence: esbuild still records root publication as pending because a
     duplicate-root gate found active PID `1534970`, while current process
     sampling observed newer active root PID `1766434` owned by `claude`.
...
3. **High - `porting.html` and `porting-summary.json` remain stale and still
   ...
   - Evidence: dashboard rows disagree with current manifests:
     ...
     rclone `291 / 327` versus `368 / 2553`,
...
4. **High - manifest denominator, runner-status, and PHP-count schemas still
   ...
   - Evidence: manifest-level PHP pass/fail fields are inconsistent or absent.
     Dolt has `mapped: 336` while `nativeImplementation.phpBehaviorTests` is
     `221`; markerPDF has `mapped: 180` while `phpBehaviorTests` is `281`;
     rclone has `mapped: 368` and `phpBehaviorTests: 368`; Readability's
     manifest has `mapped: 1230` while `phpBehaviorTests` is `119`, and
     lane-status now says `120` local tests.
   - Evidence: denominator units remain mixed: rclone's manifest denominator is
     `2553` repository files while the stale dashboard still uses `327` Go test
     files; markerPDF uses `239` static behavior/reference units; Difftastic
     mixes Rust test attributes, golden pairs, sample pairs, parser corpus
     files, and parser example files in one denominator.
...
```text
1672240 php tools/run-tests.php
```

That process exited before owner sampling. A later exact duplicate-root gate
returned:

```text
1766434 php tools/run-tests.php
```
...
No root run was started because active root harnesses and active writer/update
loops made the broad dirty worktree non-quiescent.

Latest dirty-tree samples before this update:

```text
git status --short: 926 entries
git status --short --untracked-files=no: 99 entries
git diff --shortstat: 99 files changed, 21732 insertions(+), 561 deletions(-)
```
