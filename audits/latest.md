...
The exact duplicate-root gate returned no rows at the audit samples. A
post-commit handoff gate briefly matched focused PHP harness PID `1996232`
(`php tools/run-tests.php lanes/syncthing/tests lanes/libsqlite/tests
lanes/quadrable/tests`), but it exited before owner sampling and the next exact
gate returned no rows. A later validation gate found active no-argument root
PID `1998816` owned by `claude`:

```text
PID     USER    PPID     ELAPSED STAT COMMAND
1998816 claude  1995671  00:15   Rs   php tools/run-tests.php
```

No duplicate root run was started because the tree failed the stability gate:
active lane agents and status publishers persisted, current lane batches are
uncommitted, the dashboard is stale, and the dirty tree is broad enough that a
root result would not describe one accepted snapshot.
