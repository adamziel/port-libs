...
- Added writer regression coverage for default metadata, nested sections, escaped HTML heading text, Markdown `_note` body attributes, body-only output, OPML writer-to-reader round trip, Pandoc-compatible horizontal-rule markers, tight list items, definition-list markers, empty Div attributes, raw TeX inline nodes, empty link destinations, and link titles with quotes.
...
- Direct `testsuite.native` to `writer.opml` comparison with `columns => 80` still differs only in Markdown serialized inside `_note` attributes. The current measured comparison is `actual_bytes=13982`, `expected_bytes=14075`, Levenshtein `99`, and not byte-for-byte equal. Remaining known gap buckets: paragraph softbreak/wrapping policy, a few loose and nested list blank-line edges, and fenced Div width selection for nested empty-attribute Divs.
...
- Direct upstream writer fixture comparison: structure and metadata align, but byte-for-byte output is not yet equal because of the remaining `_note` Markdown wrapping/list/fenced-Div buckets listed above.
