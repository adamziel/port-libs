...
$usage = static function (): string {
    return <<<'TEXT'
Usage: php tools/pandoc-man-reader-evidence.php [options]
...
This is a denominator/evidence gate for the upstream roff man reader unit suite.
It does not run Cabal/Tasty, execute pandoc, cover mdoc, or claim full roff parity.
TEXT;
};
...
try {
    ...
    if ($json) {
        ...
        fwrite(STDOUT, ManUpstreamReaderEvidence::formatTextReport($report));
    }

    if (
        $requiredTestCount !== null
        && !ManUpstreamReaderEvidence::hasRequiredReaderUnitCaseCount($report, $requiredTestCount)
    ) {
        fwrite(STDERR, "pandoc-man-reader-evidence: reader unit case count did not match {$requiredTestCount}\n");
        exit(1);
    }
    ...
    fwrite(STDERR, 'pandoc-man-reader-evidence: ' . $exception->getMessage() . PHP_EOL);
    fwrite(STDERR, $usage() . PHP_EOL);
    exit(2);
} catch (Throwable $throwable) {
    fwrite(STDERR, 'pandoc-man-reader-evidence: ' . $throwable::class . ': ' . $throwable->getMessage() . PHP_EOL);
    exit(1);
}
