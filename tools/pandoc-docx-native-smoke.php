...
$docxDirectory = getenv('PANDOC_UPSTREAM_DOCX_DIR') ?: getenv('PANDOC_DOCX_NATIVE_SMOKE_DIR') ?: $defaultDocxDirectory;
$limit = 0;
$json = false;
...
foreach (array_slice($argv, 1) as $argument) {
    if ($argument === '--help' || $argument === '-h') {
        fwrite(STDOUT, <<<'TXT'
Usage: php tools/pandoc-docx-native-smoke.php [--upstream-docx-dir=PATH] [--limit=N] [--json]

Compares upstream DOCX packages with same-basename .native expectations when
the upstream cache is present. Missing cache is reported as skipped with exit 0.
