...
try {
    ...
    $report = (new MarkdownUpstreamReaderEvidence($repoRoot, $upstreamRoot, $runnerResultArtifact))->report();
    if ($json) {
    ...
    if ($requireRunnerResultArtifact && !MarkdownUpstreamReaderEvidence::hasRunnerResultArtifactEvidence($report)) {
        fwrite(STDERR, "pandoc-markdown-reader-evidence: runner result artifact evidence is invalid\n");
        exit(1);
    }
