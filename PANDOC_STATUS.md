...
Last reconciled: 2026-06-13 UTC after DOCX/OpenXML note/comment relationship diagnostics on base `d099006810`, following the closure-wave matrix refresh.
...
Current counters after the DOCX/OpenXML note/comment relationship diagnostics slice: 3,312 PHP passes / 0 failures and 3,272 mapped upstream cases. The recent closure wave landed the formerly pending CSV/TSV, MediaBag, Markdown block-boundary, notes/references, LaTeX table-foot, JSON/native Div Plain, PDF/Typst package dependency conflict-policy, and DOCX note/comment relationship diagnostic evidence. This refresh was checked with `jq empty`, `git diff --check`, focused `DocxOpenXmlReaderTest.php` (`1` file, `2413` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74297` assertions, `0` failures).
...
| DOCX note/comment relationships | Footnote, endnote, and comment part-local `.rels` diagnostics now carry counts, target summaries, issue codes, reference suffixes, and per-note/comment relationship IDs; focused `DocxOpenXmlReaderTest.php` passed 2,413 assertions. | DOCX/OpenXML remains partial; broader direct reader parity remains. |
