...
Last reconciled: 2026-06-13 UTC after Typst package namespace policy diagnostics on base `85e342e2d7`, following tabular data registry option profiles.
...
Current `main` is reconciled through the Typst package namespace policy diagnostics slice rebased on `85e342e2d7`: 3,343 PHP passes / 0 failures and 3,302 mapped upstream cases. The recent closure wave landed the formerly pending Typst package namespace policy diagnostics, tabular data registry option profiles, ODF compact manifest custom attributes, DOCX note/comment relationship diagnostics, Markdown generic raw HTML serialization, Typst package dependency/source policy, CSV/TSV direct reader and header option parity, MediaBag, Markdown block-boundary, escaped-attribute, fenced Div section-reference, definition-list term-group, table-cell note placement, header section Div evidence, text markup unsupported diagnostics, shared ZIP selected-entry role buckets, EPUB XHTML table semantics, PlainWriter table body-group boundaries, Markdown HTML list item attribute handoff, Markdown math attribute round-trip, JSON/native block-container mixed-content flushing, XML/JATS/BITS direct reader capability diagnostics, notes/references backlink metadata, DOCX/OpenXML subdocument diagnostics, EPUB direct manifest/spine diagnostics, EPUB XHTML definition-list handoff, inline writer attribute handoff, text markup reader unsupported ship-gate accounting, LaTeX table-foot, JSON/native Div Plain, JSON/native task-list checkbox sidecar, JSON/native nullary helper payload, and LaTeX table body-head row evidence. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for `PdfEngineHandoff.php` and `PdfEngineHandoffTest.php`, focused `PdfEngineHandoffTest.php` (`1` file, `2253` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `75246` assertions, `0` failures).
...
| PDF/Typst dependency policy | Typst package dependency/source policy records sidecar package input counts, metadata-only byte exposure, non-executed network policy, package coordinates, namespace counts, multi-version conflicts, source classes, unsupported reason counts, and duplicate package-coordinate summaries across preview/typst/custom namespaces; focused `PdfEngineHandoffTest.php` passed 2,253 assertions. | Bounded no-engine provenance covered; real PDF/Typst output parity remains unsupported without external engines. |
