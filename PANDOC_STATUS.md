...
Last reconciled: 2026-06-13 UTC after JSON/native citation note-locator payload coverage on base `52a27eda2b`, following Typst package source-class policy provenance.
...
| JSON/native citation, block, and list handoff | Edited citation note numbers and locator-as-`citationSuffix` payloads regenerate stale sidecars; native `Div` adjacent `Plain` block boundaries survive WordPress handoff, and native `definition_term` linebreaks now render through WordPress definition lists; latest focused JSON/native run passed 2,165 assertions. | Bounded citation/block/list handoff slices covered; JSON/native remains partial. |
