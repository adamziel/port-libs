...
Last reconciled: 2026-06-13 UTC after MediaBag linked-resource MIME inference on base `00f244220`, following YAML metadata alias review summaries.
...
| Local passing numerator | `lanes/pandoc/lane-status.json` reports 3,346 PHP passes / 0 failures. `lanes/pandoc/UPSTREAM_TEST_MANIFEST.json` reports 3,305 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,305 / 2,276 mapped upstream inventory rows = 145.2%. Percentages above 100% mean local PHP slices are more granular than upstream inventory rows. | High coverage, not global ship-ready. |
