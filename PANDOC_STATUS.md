...
Last reconciled: 2026-06-13 UTC after Markdown HTML list item attribute handoff on base `3a4bf7596e`, following PDF/Typst package dependency conflict policy.
...
Current branch is reconciled through current main `3a4bf7596e` plus Markdown HTML list item attribute handoff: 3,312 PHP passes / 0 failures and 3,272 mapped upstream cases. The recent closure wave landed the formerly pending CSV/TSV, MediaBag, Markdown block-boundary, notes/references, LaTeX table-foot, JSON/native Div Plain, PDF/Typst package dependency conflict-policy, and Markdown HTML list item attribute evidence. This refresh was checked with `jq empty`, `git diff --check`, focused `MarkdownReaderTest.php` (`1` file, `6638` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74238` assertions, `0` failures).
...
| Markdown HTML list item attributes | HTML `li` attributes now survive into shared `list_item` nodes and safe `id`, `class`, `data-*`, and `title` metadata reaches WordPress list output while unsafe event/style attributes are filtered; focused `MarkdownReaderTest.php` passed 6,638 assertions. | Bounded list-item-id slice covered; broader ordered-list constructor and continuation parity remains partial. |
