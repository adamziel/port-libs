...
Last reconciled: 2026-06-13 UTC after LaTeX note anchor round-trip coverage on base `4487923aa2`, following EPUB direct manifest/spine diagnostics.
...
Current `main` is reconciled through `4487923aa2`: 3,319 PHP passes / 0 failures and 3,278 mapped upstream cases. The recent closure wave landed the formerly pending CSV/TSV, MediaBag, Markdown block-boundary and escaped-attribute, notes/references backlink metadata and LaTeX labelled note anchors, EPUB direct manifest/spine diagnostics, text markup reader unsupported ship-gate accounting, LaTeX table-foot, JSON/native Div Plain, PDF/Typst package dependency conflict-policy, JSON/native task-list checkbox sidecar, JSON/native nullary helper payload, and LaTeX table body-head row evidence. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for the touched LaTeX files, focused `LatexWriterTest.php` (`1` file, `21` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74480` assertions, `0` failures).
...
| Notes/references | Footnote label anchors, WordPress backlink metadata, LaTeX labelled note hypertargets, and JSON/native `Note` label sidecars landed with focused Markdown/DOCX/citation, LaTeX, and JSON/native coverage. | Notes evidence improved; broader references parity remains. |
