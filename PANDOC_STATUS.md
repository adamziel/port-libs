...
Last reconciled: 2026-06-13 UTC after JSON/native task-list checkbox sidecars on base `3a4bf7596e`, following the PDF/Typst package dependency conflict policy.
...
Current `main` is reconciled through `3a4bf7596e`: 3,312 PHP passes / 0 failures and 3,272 mapped upstream cases. The recent closure wave landed the formerly pending CSV/TSV, MediaBag, Markdown block-boundary, notes/references, LaTeX table-foot, JSON/native Div Plain, PDF/Typst package dependency conflict-policy, and JSON/native task-list checkbox sidecar evidence. This refresh was checked with `jq empty`, `git diff --check`, focused `PandocJsonNativeAstTest.php` (`1` file, `2015` assertions, `0` failures), focused `MarkdownReaderTest.php` (`1` file, `6626` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74254` assertions, `0` failures).
