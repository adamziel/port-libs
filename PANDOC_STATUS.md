...
Last reconciled: 2026-06-13 UTC after DocBook structural review diagnostics on base `76690ec857`, following WordPress footnote backlink metadata.
...
| Local passing numerator | `lanes/pandoc/lane-status.json` reports 3,317 PHP passes / 0 failures. `lanes/pandoc/UPSTREAM_TEST_MANIFEST.json` reports 3,276 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,276 / 2,276 mapped upstream inventory rows = 143.9%. Percentages above 100% mean local PHP slices are more granular than upstream inventory rows. | High coverage, not global ship-ready. |
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/src/XmlHtmlDom.php lanes/pandoc/tests/XmlHtmlDomTest.php lanes/pandoc/notes/pandoc-docbook-structure-review-20260613.md`, syntax checks for `XmlHtmlDom.php` and `XmlHtmlDomTest.php`, focused `XmlHtmlDomTest.php`, and `php tools/run-tests.php lanes/pandoc/tests` passed. | 45 test files, 74,405 assertions, 0 failures. |
| External validators | No Pandoc binary, Cabal/Haskell runner, office suite, TeX/Typst engine, browser engine, Node tooling, online service, live provider test, XML validator, or external validator was used for this reconciliation. | Policy satisfied. |

## Closure Wave Snapshot

Current `main` base is reconciled through `76690ec857`: 3,317 PHP passes / 0 failures and 3,276 mapped upstream cases. The recent closure wave landed the formerly pending CSV/TSV, MediaBag, Markdown block-boundary and escaped-attribute, notes/references backlink metadata, LaTeX table-foot, JSON/native Div Plain, PDF/Typst package dependency conflict-policy, JSON/native task-list checkbox sidecar, JSON/native nullary helper payload, LaTeX table body-head row, and DocBook structural review evidence. This refresh was checked with `jq empty`, `git diff --check`, focused `XmlHtmlDomTest.php` (`1` file, `1884` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74405` assertions, `0` failures).
...
| DocBook structural review | `XmlHtmlDom::summarizeDocBookStructure()` now emits DocBook 4/5 review-only packets for metadata, contributors, identifiers, sections, figures, tables, admonitions, bibliography, links, and image refs with explicit unsupported direct-reader diagnostics; focused `XmlHtmlDomTest.php` passed 1,884 assertions. | Bounded diagnostics covered; full DocBook reader parity remains partial. |
