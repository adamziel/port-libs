...
Last reconciled: 2026-06-13 UTC after tabular data registry option profiles on base `f4c55b05f`, following ODF compact manifest custom attributes.
...
| Local passing numerator | `lanes/pandoc/lane-status.json` reports 3,342 PHP passes / 0 failures. `lanes/pandoc/UPSTREAM_TEST_MANIFEST.json` reports 3,301 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,301 / 2,276 mapped upstream inventory rows = 145.0%. Percentages above 100% mean local PHP slices are more granular than upstream inventory rows. | High coverage, not global ship-ready. |
