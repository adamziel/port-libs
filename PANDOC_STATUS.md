...
Last reconciled: 2026-06-13 UTC after YAML metadata alias review summaries on base `d352c6050`, following JSON/native table attribute writer handoff.
...
| Local passing numerator | `lanes/pandoc/lane-status.json` reports 3,345 PHP passes / 0 failures. `lanes/pandoc/UPSTREAM_TEST_MANIFEST.json` reports 3,304 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,304 / 2,276 mapped upstream inventory rows = 145.2%. Percentages above 100% mean local PHP slices are more granular than upstream inventory rows. | High coverage, not global ship-ready. |
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check`, syntax checks for `MarkdownReader.php`, `MarkdownReaderTest.php`, and `YamlMetadataReviewTest.php`, focused `YamlMetadataReviewTest.php` plus `MarkdownReaderTest.php`, and `php tools/run-tests.php lanes/pandoc/tests` passed. | Focused: 2 test files, 6,753 assertions, 0 failures. Full: 45 test files, 75,272 assertions, 0 failures. |
