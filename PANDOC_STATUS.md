...
Last reconciled: 2026-06-13 UTC after JSON/native raw HTML alias preservation on base `4487923aa2`, following EPUB direct manifest/spine diagnostics.
...
Current branch is reconciled on base `4487923aa2`: 3,319 PHP passes / 0 failures and 3,278 mapped upstream cases. The recent closure wave landed the formerly pending CSV/TSV, MediaBag, Markdown block-boundary and escaped-attribute, notes/references backlink metadata, EPUB direct manifest/spine diagnostics, text markup reader unsupported ship-gate accounting, JSON/native raw HTML aliases, LaTeX table-foot, JSON/native Div Plain, PDF/Typst package dependency conflict-policy, JSON/native task-list checkbox sidecar, JSON/native nullary helper payload, and LaTeX table body-head row evidence. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for the touched JSON/native raw HTML alias files, focused `PandocJsonNativeAstTest.php` (`1` file, `2127` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74502` assertions, `0` failures).
...
| JSON/native raw HTML aliases | `RawBlock`/`RawInline` formats `html4`, `html5`, and `xhtml` now hydrate as HTML-family raw nodes, preserve original format labels through JSON/native writers, and reach Markdown plus WordPress raw HTML handoff while unsupported raw remains disabled; focused JSON/native run passed 2,127 assertions. | Bounded alias consistency slice covered; raw adjacency, diagnostics, direct Markdown generic raw policy, and broader CommonMark/GFM raw parity remain. |
