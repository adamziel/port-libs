...
Last reconciled: 2026-06-13 UTC after JSON/native nullary helper payload validation on base `6c77acbadb`.
...
| Local passing numerator | `lanes/pandoc/lane-status.json` reports 3,301 PHP passes / 0 failures. `lanes/pandoc/UPSTREAM_TEST_MANIFEST.json` reports 3,261 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,261 / 2,276 mapped upstream inventory rows = 143.3%. Percentages above 100% mean local PHP slices are more granular than upstream inventory rows. | High coverage, not global ship-ready. |
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json`, focused `PandocJsonNativeAstTest.php`, and `php tools/run-tests.php lanes/pandoc/tests` passed. | 44 test files, 74,116 assertions, 0 failures. |
