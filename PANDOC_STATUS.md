...
Last reconciled: 2026-06-13 UTC after Typst package source-class policy provenance on base `d1cb8cab8d`, following CSV/TSV header option parity.
...
| Local passing numerator | `lanes/pandoc/lane-status.json` reports 3,336 PHP passes / 0 failures. `lanes/pandoc/UPSTREAM_TEST_MANIFEST.json` reports 3,295 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,295 / 2,276 mapped upstream inventory rows = 144.8%. Percentages above 100% mean local PHP slices are more granular than upstream inventory rows. | High coverage, not global ship-ready. |
