...
Last reconciled: 2026-06-13 UTC after JSON/native mixed table caption and table-cell boundary flushing on base `17c91bad52`, following LaTeX table body-head row preservation.
...
Current branch is reconciled over `17c91bad52`: 3,315 PHP passes / 0 failures and 3,274 mapped upstream cases. The recent closure wave landed the formerly pending CSV/TSV, MediaBag, Markdown block-boundary, notes/references, LaTeX table-foot, JSON/native Div Plain, PDF/Typst package dependency conflict-policy, JSON/native task-list checkbox sidecar, JSON/native nullary helper payload, LaTeX table body-head row, and JSON/native mixed table caption/cell boundary evidence. This refresh was checked with `jq empty`, `git diff --check`, focused `PandocJsonNativeAstTest.php` (`1` file, `2123` assertions, `0` failures), focused `MarkdownReaderTest.php` (`1` file, `6626` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74362` assertions, `0` failures).
...
| JSON/native block boundaries | Native `Div` adjacent `Plain` block boundaries now survive WordPress handoff, and mixed table caption/cell inline runs now flush around nested block children; focused JSON/native run passed 2,123 assertions. | Bounded block-boundary slice covered; remaining mixed-content gaps include link/raw payloads around block containers and WordPress invalid JSON/native block encodings. |
...
| JSON/native task lists | `taskChecked` sidecars now preserve unchecked, checked, and nested task-list state through JSON/native readers and writers into Markdown, WordPress, and LaTeX handoff; focused `PandocJsonNativeAstTest.php` passed 2,123 assertions. | Bounded list-semantics sidecar slice covered; broader JSON/native constructor parity remains partial. |
| JSON/native nullary helpers | Stale non-empty nullary helper constructor payloads now regenerate through JSON/native writers while preserving sidecars; focused `PandocJsonNativeAstTest.php` passed 2,123 assertions. | Bounded helper-constructor payload slice covered; broader JSON/native constructor parity remains partial. |
