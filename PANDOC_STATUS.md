...
Last reconciled: 2026-06-13 UTC after XML/HTML5 image-map area geometry diagnostics on base `70d29eb6`, following EPUB XHTML table semantics.
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/src/XmlHtmlDom.php lanes/pandoc/tests/XmlHtmlDomTest.php`, syntax checks for the touched XML/HTML DOM files, focused `XmlHtmlDomTest.php`, and `php tools/run-tests.php lanes/pandoc/tests` passed. | 45 test files, 74,857 assertions, 0 failures. |
...
Current `main` is reconciled through the XML/HTML5 image-map area geometry diagnostics slice rebased on `70d29eb6`: 3,332 PHP passes / 0 failures and 3,291 mapped upstream cases. The recent closure wave landed the formerly pending CSV/TSV, MediaBag, Markdown block-boundary, escaped-attribute, fenced Div section-reference, definition-list term-group, table-cell note placement, header section Div evidence, XML/HTML5 image-map area geometry diagnostics, EPUB XHTML table semantics, PlainWriter table body-group boundaries, Markdown HTML list item attribute handoff, Markdown math attribute round-trip, JSON/native block-container mixed-content flushing, XML/JATS/BITS direct reader capability diagnostics, notes/references backlink metadata, DOCX/OpenXML subdocument diagnostics, EPUB direct manifest/spine diagnostics, EPUB XHTML definition-list handoff, inline writer attribute handoff, text markup reader unsupported ship-gate accounting, LaTeX table-foot, JSON/native Div Plain, PDF/Typst package dependency conflict-policy, JSON/native task-list checkbox sidecar, JSON/native nullary helper payload, and LaTeX table body-head row evidence. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for the touched XML/HTML DOM files, focused `XmlHtmlDomTest.php` (`1` file, `1882` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74857` assertions, `0` failures).
...
| XML/HTML5 image-map diagnostics | `XmlHtmlDomTest.php` now covers `<area>` shape/coords geometry, invalid coordinate diagnostics, default-area precedence, and usemap resolved/missing/duplicate/invalid association states. | Bounded HTML image-map review metadata is covered; broader HTML/XML/JATS DOM parity remains partial. |
