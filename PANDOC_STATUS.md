...
Last reconciled: 2026-06-13 UTC after CSV/TSV header option parity on base `48c0f4517a`, following text markup unsupported diagnostics.
...
| Local passing numerator | `lanes/pandoc/lane-status.json` reports 3,335 PHP passes / 0 failures. `lanes/pandoc/UPSTREAM_TEST_MANIFEST.json` reports 3,294 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,294 / 2,276 mapped upstream inventory rows = 144.7%. Percentages above 100% mean local PHP slices are more granular than upstream inventory rows. | High coverage, not global ship-ready. |
