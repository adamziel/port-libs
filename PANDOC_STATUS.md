...
Last reconciled: 2026-06-13 UTC after JSON/native note label sidecars on base `b266b6e53c`.
...
| Local passing numerator | `lanes/pandoc/lane-status.json` reports 3,298 PHP passes / 0 failures. `lanes/pandoc/UPSTREAM_TEST_MANIFEST.json` reports 3,258 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,258 / 2,276 mapped upstream inventory rows = 143.1%. Percentages above 100% mean local PHP slices are more granular than upstream inventory rows. | High coverage, not global ship-ready. |
