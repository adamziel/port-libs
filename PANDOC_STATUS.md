...
Last reconciled: 2026-06-13 UTC after JSON/native mixed Figure handoff on base `4487923aa2`, following EPUB direct manifest/spine diagnostics.
...
| Remaining critical gaps | 18 input tokens remain partial and 31 remain unsupported across DOCX/OpenXML, EPUB3, shared ZIP/OPC dependencies, JSON/native AST, CSV/TSV, CSL/BibTeX/BibLaTeX/csljson, HTML/XML/JATS DOM, LaTeX/TeX/math, Typst, PPTX/XLSX, and wiki/roff/text readers. EPUB direct manifest/spine diagnostics and JSON/native mixed Figure handoff are covered, and the wiki/roff/man/text markup family has an explicit unsupported ship-gate verdict for all 20 tokens. | Pandoc as a whole is not ship-ready. |
...
Current `main` is reconciled through `4487923aa2`: 3,319 PHP passes / 0 failures and 3,278 mapped upstream cases. The recent closure wave landed the formerly pending CSV/TSV, MediaBag, Markdown block-boundary and escaped-attribute, notes/references backlink metadata, EPUB direct manifest/spine diagnostics, JSON/native mixed Figure handoff, text markup reader unsupported ship-gate accounting, LaTeX table-foot, JSON/native Div Plain, PDF/Typst package dependency conflict-policy, JSON/native task-list checkbox sidecar, JSON/native nullary helper payload, and LaTeX table body-head row evidence. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for the touched WordPress/JSON-native files, focused `PandocJsonNativeAstTest.php` (`1` file, `2135` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74510` assertions, `0` failures).
...
| JSON/native mixed figures | Complex `Figure` children with links, raw inline HTML, nested code blocks, and captions now round-trip through JSON/native packets and WordPress complex figure HTML. | Bounded mixed-content figure slice covered; broader JSON/native constructor parity remains partial. |
...
| JSON/native AST constructor completeness | 9 |
