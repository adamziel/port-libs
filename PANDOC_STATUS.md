...
Last reconciled: 2026-06-13 UTC after IPYNB attachment media diagnostics on base `6dfacd7bba`, following Markdown fenced Div section-reference boundary scoping.
...
| External validators | No Pandoc binary, Cabal/Haskell runner, Jupyter, Python notebook runner, office suite, TeX/Typst engine, browser engine, Node tooling, online service, live provider test, or external validator was used for this reconciliation. | Policy satisfied. |

## Closure Wave Snapshot

Current branch snapshot through `6dfacd7bba` plus the IPYNB slices: 3,322 PHP passes / 0 failures and 3,281 mapped upstream cases. The recent closure wave landed the formerly pending CSV/TSV, MediaBag, Markdown block-boundary, escaped-attribute, and fenced Div section-reference evidence, notes/references backlink metadata, EPUB direct manifest/spine diagnostics, inline writer attribute handoff, text markup reader unsupported ship-gate accounting, LaTeX table-foot, JSON/native Div Plain, PDF/Typst package dependency conflict-policy, JSON/native task-list checkbox sidecar, JSON/native nullary helper payload, LaTeX table body-head row evidence, and IPYNB metadata/attachment diagnostics. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for the touched IPYNB files, focused `IpynbReaderTest.php` (`1` file, `110` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74554` assertions, `0` failures).
...
| IPYNB reader diagnostics | Notebook/cell metadata, MIME/resource summaries, and metadata-only attachment media extraction plans landed in `IpynbReaderTest.php`. | IPYNB evidence improved to 3 mapped cases and 110 assertions; full Jupyter reader parity remains partial and outside this input-format denominator. |
