...
Last reconciled: 2026-06-13 UTC after XML/HTML5 DOM nested table foster-parenting on base `6dfacd7bba`, following EPUB XHTML definition-list handoff.
...
| Local passing numerator | `lanes/pandoc/lane-status.json` reports 3,322 PHP passes / 0 failures. `lanes/pandoc/UPSTREAM_TEST_MANIFEST.json` reports 3,281 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,281 / 2,276 mapped upstream inventory rows = 144.2%. Percentages above 100% mean local PHP slices are more granular than upstream inventory rows. | High coverage, not global ship-ready. |
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/tests/XmlHtmlDomTest.php lanes/pandoc/notes/xml-html5-dom-table-foster-parenting-current-base-20260613T030852Z.md`, syntax check for `XmlHtmlDomTest.php`, focused `XmlHtmlDomTest.php`, and `php tools/run-tests.php lanes/pandoc/tests` passed. | 45 test files, 74,533 assertions, 0 failures. |
| External validators | No Pandoc binary, Cabal/Haskell runner, office suite, TeX/Typst engine, browser engine, Node tooling, online sanitizer, online service, live provider test, or external validator was used for this reconciliation. | Policy satisfied. |

## Closure Wave Snapshot

Current `main` is reconciled through local XML/HTML5 DOM table foster-parenting evidence on base `6dfacd7bba`: 3,322 PHP passes / 0 failures and 3,281 mapped upstream cases. The recent closure wave landed the formerly pending CSV/TSV, MediaBag, Markdown block-boundary, escaped-attribute, and fenced Div section-reference evidence, notes/references backlink metadata, EPUB direct manifest/spine diagnostics, EPUB XHTML definition-list handoff, XML/HTML5 nested table foster-parenting, inline writer attribute handoff, text markup reader unsupported ship-gate accounting, LaTeX table-foot, JSON/native Div Plain, PDF/Typst package dependency conflict-policy, JSON/native task-list checkbox sidecar, JSON/native nullary helper payload, and LaTeX table body-head row evidence. This refresh was checked with `jq empty`, `git diff --check`, syntax check for `XmlHtmlDomTest.php`, focused `XmlHtmlDomTest.php` (`1` file, `1848` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74533` assertions, `0` failures).
...
| EPUB direct package reader | Manifest query/fragment suffix diagnostics, external/missing spine readability reports, and XHTML definition-list handoff landed in `EpubPackageReaderTest.php`. | EPUB evidence improved to 61 local cases; broader EPUB3 package parity remains partial. |
...
| Markdown block boundaries | Nested fenced Div, escaped fenced-Div attributes, fenced Div section-reference boundaries, and fixture nested-list round-trip slices landed with focused `MarkdownReaderTest.php` runs. | Markdown family remains partial after bounded block-structure coverage. |
| XML/HTML5 DOM tree construction | Nested row/row-group table foster-parenting landed in `XmlHtmlDomTest.php`: 1 file, 1,848 assertions, 0 failures. | HTML/XML/JATS DOM remains partial; broader HTML5 insertion-mode parity and full XML/JATS/BITS readers remain. |
