...
Last reconciled: 2026-06-13 UTC after JSON/native nullary helper payload validation on base `1dc7cc07a7`, following task-list checkbox sidecars.
...
Current `main` is reconciled through `1dc7cc07a7`: 3,313 PHP passes / 0 failures and 3,273 mapped upstream cases. The recent closure wave landed the formerly pending CSV/TSV, MediaBag, Markdown block-boundary, notes/references, LaTeX table-foot, JSON/native Div Plain, PDF/Typst package dependency conflict-policy, JSON/native task-list checkbox sidecar, and JSON/native nullary helper payload evidence. This refresh was checked with `jq empty`, `git diff --check`, focused `PandocJsonNativeAstTest.php` (`1` file, `2097` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74336` assertions, `0` failures).
