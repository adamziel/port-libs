...
Last reconciled: 2026-06-13 UTC after definition-list term-group coverage on base `d1e41a7720`, following EPUB XHTML definition-list handoff.
...
Current `main` is reconciled through `d1e41a7720`: 3,322 PHP passes / 0 failures and 3,281 mapped upstream cases. The recent closure wave landed the formerly pending CSV/TSV, MediaBag, Markdown block-boundary, escaped-attribute, fenced Div section-reference, and definition-list term-group evidence, notes/references backlink metadata, EPUB direct manifest/spine diagnostics, EPUB XHTML definition-list handoff, inline writer attribute handoff, text markup reader unsupported ship-gate accounting, LaTeX table-foot, JSON/native Div Plain, PDF/Typst package dependency conflict-policy, JSON/native task-list checkbox sidecar, JSON/native nullary helper payload, and LaTeX table body-head row evidence. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for the touched Markdown/WordPress/list files, focused `MarkdownReaderTest.php` plus `PandocJsonNativeAstTest.php` (`2` files, `8762` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74525` assertions, `0` failures).
