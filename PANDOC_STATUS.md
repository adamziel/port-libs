...
Last reconciled: 2026-06-12 UTC after JSON/native target tuple sidecars on base `b30fb70127`.
...
| Local passing numerator | `lanes/pandoc/lane-status.json` reports 3,295 PHP passes / 0 failures. `lanes/pandoc/UPSTREAM_TEST_MANIFEST.json` reports 3,255 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,255 / 2,276 mapped upstream inventory rows = 143.0%. Percentages above 100% mean local PHP slices are more granular than upstream inventory rows. | High coverage, not global ship-ready. |
