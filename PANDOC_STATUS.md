...
Last reconciled: 2026-06-13 UTC after JSON/native note label sidecars on base `177a271bf9`, following CSV/TSV direct text reader coverage.
...
| Local passing numerator | `lanes/pandoc/lane-status.json` reports 3,308 PHP passes / 0 failures. `lanes/pandoc/UPSTREAM_TEST_MANIFEST.json` reports 3,268 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,268 / 2,276 mapped upstream inventory rows = 143.6%. Percentages above 100% mean local PHP slices are more granular than upstream inventory rows. | High coverage, not global ship-ready. |
