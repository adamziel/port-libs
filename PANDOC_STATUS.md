...
Last reconciled: 2026-06-13 UTC after XML/HTML5 image-map association diagnostics on base `d1e41a7720`, following EPUB XHTML definition-list handoff, Markdown fenced Div section-reference boundary scoping, Pandoc inline attribute writer handoff, and EPUB direct manifest/spine diagnostics.
...
Current branch is reconciled on base `d1e41a7720`: 3,322 PHP passes / 0 failures and 3,281 mapped upstream cases. The recent closure wave landed the formerly pending CSV/TSV, MediaBag, Markdown block-boundary, escaped-attribute, and fenced Div section-reference evidence, notes/references backlink metadata, EPUB direct manifest/spine diagnostics, EPUB XHTML definition-list handoff, inline writer attribute handoff, XML/HTML5 image-map association diagnostics, text markup reader unsupported ship-gate accounting, LaTeX table-foot, JSON/native Div Plain, PDF/Typst package dependency conflict-policy, JSON/native task-list checkbox sidecar, JSON/native nullary helper payload, and LaTeX table body-head row evidence. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for the touched XML/HTML DOM files, focused `XmlHtmlDomTest.php` (`1` file, `1853` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74538` assertions, `0` failures).
...
| XML/HTML5 image-map associations | `XmlHtmlDomTest.php`: 1,853 assertions, 0 failures. | Bounded reviewer diagnostics covered; full HTML5 tree-construction parity remains. |
