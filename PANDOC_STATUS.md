...
Last reconciled: 2026-06-12 UTC after Markdown/WordPress footnote label anchors on base `659cae59e4`.
...
| Local passing numerator | `lanes/pandoc/lane-status.json` reports 3,302 PHP passes / 0 failures. `lanes/pandoc/UPSTREAM_TEST_MANIFEST.json` reports 3,262 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,262 / 2,276 mapped upstream inventory rows = 143.3%. Percentages above 100% mean local PHP slices are more granular than upstream inventory rows. | High coverage, not global ship-ready. |
