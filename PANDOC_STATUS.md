...
Last reconciled: 2026-06-13 UTC after JSON/native table attribute writer handoff on base `17c91bad52`, following LaTeX table body-head row preservation.
...
Current `main` is reconciled through `17c91bad52`: 3,315 PHP passes / 0 failures and 3,274 mapped upstream cases. The recent closure wave landed the formerly pending CSV/TSV, MediaBag, Markdown block-boundary, notes/references, LaTeX table-foot, JSON/native Div Plain, PDF/Typst package dependency conflict-policy, JSON/native task-list checkbox sidecar, JSON/native nullary helper payload, LaTeX table body-head row, and JSON/native table attribute writer handoff evidence. This refresh was checked with `jq empty`, `git diff --check`, focused `DocxReaderTest.php`/`NativeReaderTest.php`/`LatexWriterTest.php`/`TableGeometryTest.php`/`MarkdownReaderTest.php` (`5` files, `13995` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74339` assertions, `0` failures).
...
| JSON/native table attributes | Native table id/class/data-source provenance now survives NativeReader/NativeWriter, Markdown table captions, WordPress table attributes, and LaTeX caption labels; focused touched-path run passed 13,995 assertions. | Bounded table metadata writer handoff covered; broader JSON/native constructor parity remains. |
