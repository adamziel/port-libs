...
Last reconciled: 2026-06-13 UTC after DOCX note/comment relationship diagnostics on base `4fe1d3a12e`, following Markdown generic raw HTML serialization.
...
| Local passing numerator | `lanes/pandoc/lane-status.json` reports 3,338 PHP passes / 0 failures. `lanes/pandoc/UPSTREAM_TEST_MANIFEST.json` reports 3,297 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,297 / 2,276 mapped upstream inventory rows = 144.9%. Percentages above 100% mean local PHP slices are more granular than upstream inventory rows. | High coverage, not global ship-ready. |
