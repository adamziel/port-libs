...
Last reconciled: 2026-06-13 UTC after XML/HTML5 DOM template/noscript boundary handling rebased over `7bc8d59244`, following Markdown header section Div mapping and Markdown table-cell note placement.
...
| Local passing numerator | `lanes/pandoc/lane-status.json` reports 3,326 PHP passes / 0 failures. `lanes/pandoc/UPSTREAM_TEST_MANIFEST.json` reports 3,285 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,285 / 2,276 mapped upstream inventory rows = 144.3%. Percentages above 100% mean local PHP slices are more granular than upstream inventory rows. | High coverage, not global ship-ready. |
...
Current `main` is reconciled through `7bc8d59244`: 3,326 PHP passes / 0 failures and 3,285 mapped upstream cases. The recent closure wave landed the formerly pending CSV/TSV, MediaBag, Markdown block-boundary, escaped-attribute, fenced Div section-reference, definition-list term-group, table-cell note placement, and header section Div evidence, notes/references backlink metadata, DOCX/OpenXML subdocument diagnostics, EPUB direct manifest/spine diagnostics, EPUB XHTML definition-list handoff, inline writer attribute handoff, XML/HTML5 DOM template/noscript boundary handling, text markup reader unsupported ship-gate accounting, LaTeX table-foot, JSON/native Div Plain, PDF/Typst package dependency conflict-policy, JSON/native task-list checkbox sidecar, JSON/native nullary helper payload, and LaTeX table body-head row evidence. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for the touched XML/HTML5 DOM files, focused `Html5DomTest.php` plus `XmlHtmlDomTest.php` (`2` files, `2084` assertions, `0` failures), adjacent DOM family tests (`4` files, `4734` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74651` assertions, `0` failures).
...
| XML/HTML5 DOM template boundaries | Nested template content plus noscript/script raw-text sentinel handling landed in `Html5DomTest.php` and `XmlHtmlDomTest.php`. | Bounded tree-construction boundary slice covered; broader HTML5 parity remains partial. |
