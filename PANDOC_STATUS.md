...
Last reconciled: 2026-06-13 UTC after EPUB nav/NCX label provenance on base `d1e41a7720`, following EPUB XHTML definition-list handoff, Markdown fenced Div section-reference boundary scoping, Pandoc inline attribute writer handoff, and EPUB direct manifest/spine diagnostics.
...
Current `main` is reconciled through `pandoc-epub-nav-ncx-label-provenance-20260613T024437Z` on base `d1e41a7720`: 3,322 PHP passes / 0 failures and 3,281 mapped upstream cases. The recent closure wave landed the formerly pending CSV/TSV, MediaBag, Markdown block-boundary, escaped-attribute, and fenced Div section-reference evidence, notes/references backlink metadata, EPUB direct manifest/spine diagnostics, EPUB XHTML definition-list handoff, inline writer attribute handoff, compact EPUB nav/NCX label provenance, text markup reader unsupported ship-gate accounting, LaTeX table-foot, JSON/native Div Plain, PDF/Typst package dependency conflict-policy, JSON/native task-list checkbox sidecar, JSON/native nullary helper payload, and LaTeX table body-head row evidence. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for the touched EPUB package reader files, focused `EpubPackageReaderTest.php` (`1` file, `235` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74535` assertions, `0` failures).
...
| EPUB direct package reader | Manifest query/fragment suffix diagnostics, external/missing spine readability reports, XHTML definition-list handoff, and compact nav/NCX label provenance landed in `EpubPackageReaderTest.php`. | EPUB evidence improved to 62 local cases; broader EPUB3 package parity remains partial. |
| EPUB3 package reader labels | Compact `EpubPackageReader` now preserves EPUB XHTML nav and NCX navPoint `labelProvenance` for package review. | Bounded label provenance covered; broader EPUB package/content parity remains. |
