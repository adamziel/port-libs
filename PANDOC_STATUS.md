...
Last reconciled: 2026-06-13 UTC after Markdown fenced Div section-reference boundary scoping on base `057a4c4e57`, following Pandoc inline attribute writer handoff.
...
| Local passing numerator | `lanes/pandoc/lane-status.json` reports 3,320 PHP passes / 0 failures. `lanes/pandoc/UPSTREAM_TEST_MANIFEST.json` reports 3,279 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,279 / 2,276 mapped upstream inventory rows = 144.1%. Percentages above 100% mean local PHP slices are more granular than upstream inventory rows. | High coverage, not global ship-ready. |
...
| Remaining critical gaps | 18 input tokens remain partial and 31 remain unsupported across DOCX/OpenXML, EPUB3, shared ZIP/OPC dependencies, JSON/native AST, CSV/TSV, CSL/BibTeX/BibLaTeX/csljson, HTML/XML/JATS DOM, LaTeX/TeX/math, Typst, PPTX/XLSX, and wiki/roff/text readers. Markdown fenced Div section-reference boundaries, inline writer attributes, EPUB direct manifest/spine diagnostics, and text markup unsupported ship-gate accounting are covered. | Pandoc as a whole is not ship-ready. |
