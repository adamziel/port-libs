...
Last reconciled: 2026-06-13 UTC after JSON/native block-container mixed-content flushing on base `7b2c5f3a7c`, following XML/JATS/BITS direct reader capability diagnostics.
...
| Local passing numerator | `lanes/pandoc/lane-status.json` reports 3,327 PHP passes / 0 failures. `lanes/pandoc/UPSTREAM_TEST_MANIFEST.json` reports 3,286 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,286 / 2,276 mapped upstream inventory rows = 144.4%. Percentages above 100% mean local PHP slices are more granular than upstream inventory rows. | High coverage, not global ship-ready. |
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/src/NativeWriter.php lanes/pandoc/src/PandocJsonWriter.php lanes/pandoc/tests/PandocJsonNativeAstTest.php`, syntax checks for the JSON/native reader and writer files, focused `PandocJsonNativeAstTest.php`, and `php tools/run-tests.php lanes/pandoc/tests` passed. | 45 test files, 74,728 assertions, 0 failures. |
...
Current `main` is reconciled through the JSON/native block-container mixed-content slice rebased on `7b2c5f3a7c`: 3,327 PHP passes / 0 failures and 3,286 mapped upstream cases. The recent closure wave landed the formerly pending CSV/TSV, MediaBag, Markdown block-boundary, escaped-attribute, fenced Div section-reference, definition-list term-group, table-cell note placement, and header section Div evidence, JSON/native block-container mixed-content flushing, XML/JATS/BITS direct reader capability diagnostics, notes/references backlink metadata, DOCX/OpenXML subdocument diagnostics, EPUB direct manifest/spine diagnostics, EPUB XHTML definition-list handoff, inline writer attribute handoff, text markup reader unsupported ship-gate accounting, LaTeX table-foot, JSON/native Div Plain, PDF/Typst package dependency conflict-policy, JSON/native task-list checkbox sidecar, JSON/native nullary helper payload, and LaTeX table body-head row evidence. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for the JSON/native reader and writer files, focused `PandocJsonNativeAstTest.php` (`1` file, `2133` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74728` assertions, `0` failures).
