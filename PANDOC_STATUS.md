...
Last reconciled: 2026-06-13 UTC after Markdown header section Div mapping on base `3f0dd072b4`, following Markdown table-cell note placement coverage.
...
| Local passing numerator | `lanes/pandoc/lane-status.json` reports 3,325 PHP passes / 0 failures. `lanes/pandoc/UPSTREAM_TEST_MANIFEST.json` reports 3,284 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,284 / 2,276 mapped upstream inventory rows = 144.3%. Percentages above 100% mean local PHP slices are more granular than upstream inventory rows. | High coverage, not global ship-ready. |
