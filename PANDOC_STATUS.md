...
Last reconciled: 2026-06-13 UTC after shared ZIP/OPC selected-entry handoff role buckets on base `70d29eb6f8`, following EPUB XHTML table semantics.
...
Current `main` is reconciled through the EPUB NCX hierarchy diagnostics slice rebased on `9ab2f42c1e`: 3,334 PHP passes / 0 failures and 3,293 mapped upstream cases. The recent closure wave landed the formerly pending CSV/TSV, MediaBag, Markdown block-boundary, escaped-attribute, fenced Div section-reference, definition-list term-group, table-cell note placement, header section Div evidence, shared ZIP selected-entry role buckets, EPUB XHTML table semantics, EPUB nav/NCX label provenance, EPUB NCX hierarchy diagnostics, PlainWriter table body-group boundaries, Markdown HTML list item attribute handoff, Markdown math attribute round-trip, JSON/native block-container mixed-content flushing, XML/JATS/BITS direct reader capability diagnostics, notes/references backlink metadata, DOCX/OpenXML subdocument diagnostics, EPUB direct manifest/spine diagnostics, EPUB XHTML definition-list handoff, inline writer attribute handoff, text markup reader unsupported ship-gate accounting, LaTeX table-foot, JSON/native Div Plain, PDF/Typst package dependency conflict-policy, JSON/native task-list checkbox sidecar, JSON/native nullary helper payload, and LaTeX table body-head row evidence. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for the touched `EpubPackageReader` files, focused `EpubPackageReaderTest.php` (`1` file, `261` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74879` assertions, `0` failures).
...
| EPUB direct package reader | Manifest query/fragment suffix diagnostics, external/missing spine readability reports, XHTML definition-list handoff, bounded XHTML table-section review metadata, nav/NCX label provenance, and NCX duplicate-target/play-order diagnostics landed in EPUB reader coverage. | EPUB evidence improved to 64 local cases; broader EPUB3 package parity remains partial. |
