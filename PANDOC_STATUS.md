...
Last reconciled: 2026-06-13 UTC after JATS/BITS section metadata diagnostics on current main `52a27eda2b`, following Typst package source-class policy provenance and CSV/TSV header option parity.
...
| Local passing numerator | `lanes/pandoc/lane-status.json` reports 3,337 PHP passes / 0 failures. `lanes/pandoc/UPSTREAM_TEST_MANIFEST.json` reports 3,296 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,296 / 2,276 mapped upstream inventory rows = 144.8%. Percentages above 100% mean local PHP slices are more granular than upstream inventory rows. | High coverage, not global ship-ready. |
...
Current branch is reconciled through the JATS/BITS section metadata diagnostics slice on top of Typst package source-class policy provenance at `52a27eda2b`: 3,337 PHP passes / 0 failures and 3,296 mapped upstream cases. The recent closure wave landed the formerly pending JATS/BITS section metadata diagnostics, Typst package dependency/source policy, CSV/TSV direct reader and header option parity, MediaBag, Markdown block-boundary, escaped-attribute, fenced Div section-reference, definition-list term-group, table-cell note placement, header section Div evidence, text markup unsupported diagnostics, shared ZIP selected-entry role buckets, EPUB XHTML table semantics, PlainWriter table body-group boundaries, Markdown HTML list item attribute handoff, Markdown math attribute round-trip, JSON/native block-container mixed-content flushing, XML/JATS/BITS direct reader capability diagnostics, notes/references backlink metadata, DOCX/OpenXML subdocument diagnostics, EPUB direct manifest/spine diagnostics, EPUB XHTML definition-list handoff, inline writer attribute handoff, text markup reader unsupported ship-gate accounting, LaTeX table-foot, JSON/native Div Plain, JSON/native task-list checkbox sidecar, JSON/native nullary helper payload, and LaTeX table body-head row evidence. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for the touched XML DOM files, focused `XmlHtmlDomTest.php` (`1` file, `1878` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `75080` assertions, `0` failures).
