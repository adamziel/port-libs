...
Last reconciled: 2026-06-13 UTC after rare text-format registry unsupported diagnostics on base `d1e41a7720`, following EPUB XHTML definition-list handoff.
...
| Rare text registry diagnostics | `PandocFormatRegistry` records direction buckets, extension inference, AsciiDoc aliases, unsupported summaries, and parity counts for rare text tokens including `org`, `rst`, `textile`, and `muse`; focused registry checks passed 151 rare-text assertions. | Explicit unsupported verdict covered; native rare text readers and writers remain unimplemented. |
