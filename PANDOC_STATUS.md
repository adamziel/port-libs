...
Last reconciled: 2026-06-12 UTC on main baseline `f4d0e410e4`.
...
| Local passing numerator | `lanes/pandoc/lane-status.json` reports 3,291 PHP passes / 0 failures. `lanes/pandoc/UPSTREAM_TEST_MANIFEST.json` reports 3,251 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,251 / 2,276 mapped upstream inventory rows = 142.8%. Percentages above 100% mean local PHP slices are more granular than upstream inventory rows. | High coverage, not global ship-ready. |
