...
Last reconciled: 2026-06-13 UTC after JATS/BITS figure media reference diagnostics on base `52a27eda2b`, following Typst package source-class policy provenance.
...
| XML/JATS/BITS direct reader capability | `PandocFormatRegistryTest.php` and `XmlHtmlDomTest.php` now make `xml`, `jats`, and `bits` explicit unsupported direct-reader inputs with bounded diagnostic packets and `directReaderParity=false`; JATS/BITS figure packets also preserve label/title/caption/xref metadata and metadata-only graphic/media target diagnostics without payload bytes. | Capability status is explicit; full XML/JATS/BITS readers and media extraction remain unsupported. |
