...
Last reconciled: 2026-06-12 UTC after Markdown nested fenced Div review on base `e5fdecf1ff`.
...
| Local passing numerator | `lanes/pandoc/lane-status.json` reports 3,296 PHP passes / 0 failures. `lanes/pandoc/UPSTREAM_TEST_MANIFEST.json` reports 3,256 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,256 / 2,276 mapped upstream inventory rows = 143.1%. Percentages above 100% mean local PHP slices are more granular than upstream inventory rows. | High coverage, not global ship-ready. |
