...
Last reconciled: 2026-06-13 UTC after CSL citation affix review metadata on base `4487923aa2`, following EPUB direct manifest/spine diagnostics.
...
Current `main` is reconciled through `4487923aa2`: 3,319 PHP passes / 0 failures and 3,278 mapped upstream cases. The recent closure wave landed the formerly pending CSV/TSV, MediaBag, Markdown block-boundary and escaped-attribute, notes/references backlink metadata, EPUB direct manifest/spine diagnostics, text markup reader unsupported ship-gate accounting, CSL citation affix review evidence, LaTeX table-foot, JSON/native Div Plain, PDF/Typst package dependency conflict-policy, JSON/native task-list checkbox sidecar, JSON/native nullary helper payload, and LaTeX table body-head row evidence. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for the touched CSL citation files, focused `CitationCslProcessorTest.php` (`1` file, `5321` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74485` assertions, `0` failures).
...
| CSL citation affixes | Citation prefix/suffix review metadata now survives normalized citation attrs, grouped citation summaries, custom CSL variables, bibliography handoff, and WordPress review output. | Bounded CSL citation affix review covered; broader bibliography reader parity remains. |
