...
Last reconciled: 2026-06-13 UTC after JSON/native table ColSpec sidecar coverage on base `fb3af697a6`, following MediaBag linked-resource MIME inference.
...
| JSON/native table ColSpec sidecars | Table `Align*` and `ColWidth*` sidecars now survive safe JSON/native table rebuilds while edited width payloads regenerate only for the changed column; focused `PandocJsonNativeAstTest.php` passed 2,215 assertions. | Bounded table ColSpec sidecar slice covered; broader JSON/native constructor parity remains partial. |
