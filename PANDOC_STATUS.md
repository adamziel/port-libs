...
Last reconciled: 2026-06-13 UTC after JATS/BITS relationship diagnostics on base `6c32f56cfc`, following DOCX/OpenXML subdocument diagnostics.
...
| Local passing numerator | `lanes/pandoc/lane-status.json` reports 3,324 PHP passes / 0 failures. `lanes/pandoc/UPSTREAM_TEST_MANIFEST.json` reports 3,283 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,283 / 2,276 mapped upstream inventory rows = 144.2%. Percentages above 100% mean local PHP slices are more granular than upstream inventory rows. | High coverage, not global ship-ready. |
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check`, syntax checks for `XmlHtmlDom.php` and `XmlHtmlDomTest.php`, focused `XmlHtmlDomTest.php`, adjacent DOM family tests, and `php tools/run-tests.php lanes/pandoc/tests` passed. | 45 test files, 74,623 assertions, 0 failures. |
...
Current branch is reconciled from `6c32f56cfc`: 3,324 PHP passes / 0 failures and 3,283 mapped upstream cases. The recent closure wave landed the formerly pending CSV/TSV, MediaBag, Markdown block-boundary, escaped-attribute, fenced Div section-reference, and definition-list term-group evidence, notes/references backlink metadata, DOCX/OpenXML subdocument diagnostics, EPUB direct manifest/spine diagnostics, EPUB XHTML definition-list handoff, inline writer attribute handoff, JATS/BITS relationship diagnostics, text markup reader unsupported ship-gate accounting, LaTeX table-foot, JSON/native Div Plain, PDF/Typst package dependency conflict-policy, JSON/native task-list checkbox sidecar, JSON/native nullary helper payload, and LaTeX table body-head row evidence. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for the touched XML DOM files, focused `XmlHtmlDomTest.php` (`1` file, `1856` assertions, `0` failures), adjacent DOM family tests (`6` files, `6284` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74623` assertions, `0` failures).
...
| Markdown/list boundaries | Nested fenced Div, escaped fenced-Div attributes, fenced Div section-reference boundaries, fixture nested-list round-trip, and definition-list term-group slices landed with focused Markdown/JSON-native runs. | Markdown family remains partial after bounded block/list coverage. |
