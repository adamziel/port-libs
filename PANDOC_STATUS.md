...
Last reconciled: 2026-06-13 UTC after Markdown table-cell note placement coverage on base `6c32f56cfc`, following DOCX/OpenXML subdocument diagnostics.
...
Current `main` is reconciled through `6c32f56cfc`: 3,324 PHP passes / 0 failures and 3,283 mapped upstream cases. The recent closure wave landed the formerly pending CSV/TSV, MediaBag, Markdown block-boundary, escaped-attribute, fenced Div section-reference, definition-list term-group, and table-cell note placement evidence, notes/references backlink metadata, DOCX/OpenXML subdocument diagnostics, EPUB direct manifest/spine diagnostics, EPUB XHTML definition-list handoff, inline writer attribute handoff, text markup reader unsupported ship-gate accounting, LaTeX table-foot, JSON/native Div Plain, PDF/Typst package dependency conflict-policy, JSON/native task-list checkbox sidecar, JSON/native nullary helper payload, and LaTeX table body-head row evidence. This refresh was checked with `jq empty`, `git diff --check`, syntax check for the touched Markdown test file, focused `MarkdownReaderTest.php` (`1` file, `6668` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74604` assertions, `0` failures).
...
| Markdown/list/note boundaries | Nested fenced Div, escaped fenced-Div attributes, fenced Div section-reference boundaries, fixture nested-list round-trip, definition-list term-group, and table-cell note placement slices landed with focused Markdown/JSON-native runs. | Markdown family remains partial after bounded block/list/note coverage. |
