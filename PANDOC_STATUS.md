...
Last reconciled: 2026-06-13 UTC after PlainWriter table body-group boundaries on base `0072af8b00`, following Markdown HTML list item attributes.
...
| Local passing numerator | `lanes/pandoc/lane-status.json` reports 3,330 PHP passes / 0 failures. `lanes/pandoc/UPSTREAM_TEST_MANIFEST.json` reports 3,289 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,289 / 2,276 mapped upstream inventory rows = 144.5%. Percentages above 100% mean local PHP slices are more granular than upstream inventory rows. | High coverage, not global ship-ready. |
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/src/PlainWriter.php lanes/pandoc/tests/PlainWriterTest.php`, syntax checks for the touched PlainWriter files, focused `PlainWriterTest.php`, focused `TableGeometryTest.php`, and `php tools/run-tests.php lanes/pandoc/tests` passed. | 45 test files, 74,766 assertions, 0 failures. |
...
Current `main` is reconciled through the PlainWriter table body-group slice rebased on `0072af8b00`: 3,330 PHP passes / 0 failures and 3,289 mapped upstream cases. The recent closure wave landed the formerly pending CSV/TSV, MediaBag, Markdown block-boundary, escaped-attribute, fenced Div section-reference, definition-list term-group, table-cell note placement, header section Div evidence, PlainWriter table body-group boundaries, Markdown HTML list item attribute handoff, Markdown math attribute round-trip, JSON/native block-container mixed-content flushing, XML/JATS/BITS direct reader capability diagnostics, notes/references backlink metadata, DOCX/OpenXML subdocument diagnostics, EPUB direct manifest/spine diagnostics, EPUB XHTML definition-list handoff, inline writer attribute handoff, text markup reader unsupported ship-gate accounting, LaTeX table-foot, JSON/native Div Plain, PDF/Typst package dependency conflict-policy, JSON/native task-list checkbox sidecar, JSON/native nullary helper payload, and LaTeX table body-head row evidence. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for the touched PlainWriter files, focused `PlainWriterTest.php` (`1` file, `223` assertions, `0` failures), focused `TableGeometryTest.php` (`1` file, `1904` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74766` assertions, `0` failures).
