...
Last reconciled: 2026-06-13 UTC after XML/JATS/BITS direct reader capability diagnostics on base `7bc8d59244`, following Markdown header section Div mapping.
...
Current `main` is reconciled through the XML/JATS/BITS direct reader capability slice rebased on `7bc8d59244`: 3,326 PHP passes / 0 failures and 3,285 mapped upstream cases. The recent closure wave landed the formerly pending CSV/TSV, MediaBag, Markdown block-boundary, escaped-attribute, fenced Div section-reference, definition-list term-group, table-cell note placement, and header section Div evidence, XML/JATS/BITS direct reader capability diagnostics, notes/references backlink metadata, DOCX/OpenXML subdocument diagnostics, EPUB direct manifest/spine diagnostics, EPUB XHTML definition-list handoff, inline writer attribute handoff, text markup reader unsupported ship-gate accounting, LaTeX table-foot, JSON/native Div Plain, PDF/Typst package dependency conflict-policy, JSON/native task-list checkbox sidecar, JSON/native nullary helper payload, and LaTeX table body-head row evidence. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for the touched registry/XML files, focused `PandocFormatRegistryTest.php` (`1` file, `1168` assertions, `0` failures), focused `XmlHtmlDomTest.php` (`1` file, `1843` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74698` assertions, `0` failures).
