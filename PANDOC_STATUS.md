...
Last reconciled: 2026-06-13 UTC after JATS/BITS ref-list bibliography diagnostics on base `70d29eb6f8`, following EPUB XHTML table semantics.
...
| Local passing numerator | `lanes/pandoc/lane-status.json` reports 3,332 PHP passes / 0 failures. `lanes/pandoc/UPSTREAM_TEST_MANIFEST.json` reports 3,291 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,291 / 2,276 mapped upstream inventory rows = 144.6%. Percentages above 100% mean local PHP slices are more granular than upstream inventory rows. | High coverage, not global ship-ready. |
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/src/XmlHtmlDom.php lanes/pandoc/tests/XmlHtmlDomTest.php`, syntax checks for the touched XmlHtmlDom files, focused `XmlHtmlDomTest.php`, and `php tools/run-tests.php lanes/pandoc/tests` passed. | 45 test files, 74,893 assertions, 0 failures. |
...
Current `main` is reconciled through the JATS/BITS ref-list bibliography diagnostic slice rebased on `70d29eb6f8`: 3,332 PHP passes / 0 failures and 3,291 mapped upstream cases. The recent closure wave landed the formerly pending CSV/TSV, MediaBag, Markdown block-boundary, escaped-attribute, fenced Div section-reference, definition-list term-group, table-cell note placement, header section Div evidence, JATS/BITS ref-list bibliography diagnostics, EPUB XHTML table semantics, PlainWriter table body-group boundaries, Markdown HTML list item attribute handoff, Markdown math attribute round-trip, JSON/native block-container mixed-content flushing, XML/JATS/BITS direct reader capability diagnostics, notes/references backlink metadata, DOCX/OpenXML subdocument diagnostics, EPUB direct manifest/spine diagnostics, EPUB XHTML definition-list handoff, inline writer attribute handoff, text markup reader unsupported ship-gate accounting, LaTeX table-foot, JSON/native Div Plain, PDF/Typst package dependency conflict-policy, JSON/native task-list checkbox sidecar, JSON/native nullary helper payload, and LaTeX table body-head row evidence. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for the touched XmlHtmlDom files, focused `XmlHtmlDomTest.php` (`1` file, `1918` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74893` assertions, `0` failures).
...
| XML/JATS/BITS direct reader capability | `PandocFormatRegistryTest.php` and `XmlHtmlDomTest.php` now make `xml`, `jats`, and `bits` explicit unsupported direct-reader inputs with bounded diagnostic packets, metadata-only JATS/BITS ref-list bibliography diagnostics, resolved/unresolved `xref ref-type="bibr"` inventories, and `directReaderParity=false`. | Capability status and bounded reference diagnostics are explicit; full XML/JATS/BITS readers remain unsupported. |
