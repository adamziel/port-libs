...
Last reconciled: 2026-06-13 UTC after CSV/TSV direct text reader coverage on base `43dbb735c4`, following Markdown fixture nested-list round-trip.
...
| Local passing numerator | `lanes/pandoc/lane-status.json` reports 3,307 PHP passes / 0 failures. `lanes/pandoc/UPSTREAM_TEST_MANIFEST.json` reports 3,267 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,267 / 2,276 mapped upstream inventory rows = 143.5%. Percentages above 100% mean local PHP slices are more granular than upstream inventory rows. | High coverage, not global ship-ready. |
...
| External validators | No Pandoc binary, spreadsheet application, office suite, TeX/Typst engine, browser engine, Node tooling, online service, live provider test, or external validator was used for this reconciliation. | Policy satisfied. |
