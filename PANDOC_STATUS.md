...
Last reconciled: 2026-06-13 UTC after JSON/native block-container mixed-content review on base `3a4bf7596e`, following Typst package dependency conflict policy.
...
| External validators | No Pandoc binary, JSON filters, Cabal/Haskell runner, office suite, TeX/Typst engine, browser engine, Node tooling, online service, live provider test, or external validator was used for this reconciliation. | Policy satisfied. |

## Closure Wave Snapshot

Current `main` is reconciled through `3a4bf7596e`: 3,312 PHP passes / 0 failures and 3,272 mapped upstream cases. The recent closure wave landed the formerly pending CSV/TSV, MediaBag, Markdown block-boundary, notes/references, LaTeX table-foot, JSON/native Div Plain, PDF/Typst package dependency conflict-policy, and JSON/native block-container evidence. This refresh was checked with `jq empty`, `git diff --check`, focused `PandocJsonNativeAstTest.php` (`1` file, `2017` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74256` assertions, `0` failures).
...
| JSON/native block boundaries | Native `Div` adjacent `Plain` block boundaries survive WordPress handoff, and JSON/native writers flush mixed inline/block container children through valid `Plain` blocks; focused JSON/native run passed 2,017 assertions. | Bounded block-boundary slices covered; JSON/native remains partial. |
