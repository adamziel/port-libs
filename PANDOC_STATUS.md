...
Last reconciled: 2026-06-13 UTC after CSV/TSV format inference diagnostics on base `76690ec857`, including recovered header-option and tabular registry option-profile slices.
...
| Local passing numerator | `lanes/pandoc/lane-status.json` reports 3,322 PHP passes / 0 failures. `lanes/pandoc/UPSTREAM_TEST_MANIFEST.json` reports 3,282 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,282 / 2,276 mapped upstream inventory rows = 144.2%. Percentages above 100% mean local PHP slices are more granular than upstream inventory rows. | High coverage, not global ship-ready. |
...
| External validators | No Pandoc binary, spreadsheet application, Cabal/Haskell runner, office suite, TeX/Typst engine, browser engine, Node tooling, online validator, online service, live provider test, or external validator was used for this reconciliation. | Policy satisfied. |

## Closure Wave Snapshot

Current `main` is reconciled through `76690ec857`: 3,322 PHP passes / 0 failures and 3,282 mapped upstream cases. The recent closure wave landed the formerly pending CSV/TSV direct reader, CSV/TSV header-option parity, tabular registry option-profile, CSV/TSV format inference diagnostics, MediaBag, Markdown block-boundary and escaped-attribute, notes/references backlink metadata, LaTeX table-foot, JSON/native Div Plain, PDF/Typst package dependency conflict-policy, JSON/native task-list checkbox sidecar, JSON/native nullary helper payload, and LaTeX table body-head row evidence. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for the touched CSV/TSV reader and registry files, focused `DelimitedTextReaderTest.php` plus `PandocFormatRegistryTest.php` (`2` files, `1173` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74467` assertions, `0` failures).

| Surface | Landed evidence | Verdict |
| --- | --- | --- |
| CSV/TSV direct readers | `DelimitedTextReaderTest.php` plus format registry focused run: 2 files, 1,173 assertions, 0 failures. CSV/TSV has 6 local mapped reader/diagnostic cases against 2 upstream CSV command rows, covering direct headed tables, no-header option diagnostics, registry option profiles, and auto format inference from extension/sourcePath or content row profiles. | CSV/TSV remains partial; broader upstream fixture hydration, option behavior, malformed input diagnostics, and table-reader edge cases remain. |
