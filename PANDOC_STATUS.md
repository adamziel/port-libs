...
Last reconciled: 2026-06-13 UTC after JATS/BITS body diagnostics on base `4487923aa2`, following EPUB direct manifest/spine diagnostics.
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/src/XmlHtmlDom.php lanes/pandoc/tests/XmlHtmlDomTest.php lanes/pandoc/notes/xml-html-jats-body-diagnostics-20260613.md`, syntax checks for `XmlHtmlDom.php` and `XmlHtmlDomTest.php`, focused `XmlHtmlDomTest.php`, the adjacent DOM/table family, and `php tools/run-tests.php lanes/pandoc/tests` passed. | 45 test files, 74,512 assertions, 0 failures. |
...
Current branch is reconciled over `4487923aa2`: 3,319 PHP passes / 0 failures and 3,278 mapped upstream cases. The recent closure wave landed the formerly pending CSV/TSV, MediaBag, Markdown block-boundary and escaped-attribute, notes/references backlink metadata, EPUB direct manifest/spine diagnostics, XML/JATS body diagnostics, text markup reader unsupported ship-gate accounting, LaTeX table-foot, JSON/native Div Plain, PDF/Typst package dependency conflict-policy, JSON/native task-list checkbox sidecar, JSON/native nullary helper payload, and LaTeX table body-head row evidence. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for the touched XML DOM files, focused `XmlHtmlDomTest.php` (`1` file, `1868` assertions, `0` failures), adjacent DOM/table tests (`6` files, `6296` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74512` assertions, `0` failures).
...
| XML/JATS body diagnostics | Bounded JATS/BITS body packets now include body roots, section hierarchy, xref resolution, reference/figure/table-wrap back-reference counts, unreferenced buckets, and BITS book-part body metadata. | Full XML/JATS/BITS direct-reader parity remains unsupported. |
