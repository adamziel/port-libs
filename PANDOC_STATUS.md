...
Last reconciled: 2026-06-13 UTC after PDF/Typst package dependency conflict policy on base `5cd9038b4c`.
...
| Local passing numerator | `lanes/pandoc/lane-status.json` reports 3,300 PHP passes / 0 failures. `lanes/pandoc/UPSTREAM_TEST_MANIFEST.json` reports 3,260 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,260 / 2,276 mapped upstream inventory rows = 143.2%. Percentages above 100% mean local PHP slices are more granular than upstream inventory rows. | High coverage, not global ship-ready. |
...
Latest bounded slice: PDF/Typst remains not shippable as full output parity. The
package dependency conflict-policy follow-up keeps Typst package dependency bytes
metadata-only, records non-executed network policy, and surfaces multi-version
package conflicts from dependency sidecars without invoking external engines.
