...
Last reconciled: 2026-06-13 UTC after XML/HTML object association provenance on base `fb3af697a`, following MediaBag linked-resource MIME inference.
...
| Local passing numerator | `lanes/pandoc/lane-status.json` reports 3,347 PHP passes / 0 failures. `lanes/pandoc/UPSTREAM_TEST_MANIFEST.json` reports 3,306 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,306 / 2,276 mapped upstream inventory rows = 145.3%. Percentages above 100% mean local PHP slices are more granular than upstream inventory rows. | High coverage, not global ship-ready. |
