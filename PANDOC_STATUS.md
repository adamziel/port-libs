...
Last reconciled: 2026-06-13 UTC after ODF compact manifest custom attributes on base `6ae1fcc83`, following DOCX note/comment relationship diagnostics.
...
| Local passing numerator | `lanes/pandoc/lane-status.json` reports 3,339 PHP passes / 0 failures. `lanes/pandoc/UPSTREAM_TEST_MANIFEST.json` reports 3,298 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,298 / 2,276 mapped upstream inventory rows = 144.9%. Percentages above 100% mean local PHP slices are more granular than upstream inventory rows. | High coverage, not global ship-ready. |
...
Current `main` is reconciled through the ODF compact manifest custom attributes slice rebased on `6ae1fcc83`: 3,339 PHP passes / 0 failures and 3,298 mapped upstream cases. The recent closure wave landed the formerly pending ODF compact manifest custom attributes, DOCX note/comment relationship diagnostics, Markdown generic raw HTML serialization, Typst package dependency/source policy, CSV/TSV direct reader and header option parity, MediaBag, Markdown block-boundary, escaped-attribute, fenced Div section-reference, definition-list term-group, table-cell note placement, header section Div evidence, text markup unsupported diagnostics, shared ZIP selected-entry role buckets, EPUB XHTML table semantics, PlainWriter table body-group boundaries, Markdown HTML list item attribute handoff, Markdown math attribute round-trip, JSON/native block-container mixed-content flushing, XML/JATS/BITS direct reader capability diagnostics, notes/references backlink metadata, DOCX/OpenXML subdocument diagnostics, EPUB direct manifest/spine diagnostics, EPUB XHTML definition-list handoff, inline writer attribute handoff, text markup reader unsupported ship-gate accounting, LaTeX table-foot, JSON/native Div Plain, JSON/native task-list checkbox sidecar, JSON/native nullary helper payload, and LaTeX table body-head row evidence. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for `OpenDocumentPackage.php` and `OpenDocumentPackageTest.php`, focused `OpenDocumentPackageTest.php` (`1` file, `1181` assertions, `0` failures), focused ODF/ODT gate (`4` files, `5942` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `75148` assertions, `0` failures).

| Surface | Landed evidence | Verdict |
| --- | --- | --- |
| CSV/TSV direct readers | `DelimitedTextReaderTest.php` plus format registry focused run: 2 files, 1,410 assertions, 0 failures; 4 local CSV/TSV cases now cover headed and no-header imports. | CSV/TSV remains partial; broader reader parity remains. |
