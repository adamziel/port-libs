...
Last reconciled: 2026-06-13 UTC after XML/JATS/BITS figure metadata diagnostics on base `9ab2f42c1e`, following shared ZIP/OPC selected-entry handoff role buckets.
...
Current branch is reconciled through the XML/JATS/BITS figure metadata diagnostics slice rebased on `9ab2f42c1e`: 3,333 PHP passes / 0 failures and 3,292 mapped upstream cases. The recent closure wave landed the formerly pending CSV/TSV, MediaBag, Markdown block-boundary, escaped-attribute, fenced Div section-reference, definition-list term-group, table-cell note placement, header section Div evidence, JATS/BITS figure label/caption/title diagnostics, shared ZIP selected-entry role buckets, EPUB XHTML table semantics, PlainWriter table body-group boundaries, Markdown HTML list item attribute handoff, Markdown math attribute round-trip, JSON/native block-container mixed-content flushing, XML/JATS/BITS direct reader capability diagnostics, notes/references backlink metadata, DOCX/OpenXML subdocument diagnostics, EPUB direct manifest/spine diagnostics, EPUB XHTML definition-list handoff, inline writer attribute handoff, text markup reader unsupported ship-gate accounting, LaTeX table-foot, JSON/native Div Plain, PDF/Typst package dependency conflict-policy, JSON/native task-list checkbox sidecar, JSON/native nullary helper payload, and LaTeX table body-head row evidence. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for the XML DOM source and test files, focused `XmlHtmlDomTest.php` (`1` file, `1901` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74889` assertions, `0` failures).
...
| XML/JATS/BITS direct reader diagnostics | `XmlHtmlDomTest.php` now covers bounded JATS/BITS figure label, caption, and title metadata summaries, plus the existing explicit unsupported direct-reader diagnostics for `xml`, `jats`, and `bits` with `directReaderParity=false`. | Capability and figure metadata gaps are explicit; full XML/JATS/BITS readers remain unsupported. |
