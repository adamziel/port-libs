...
Last reconciled: 2026-06-13 UTC after Markdown HTML list item attribute handoff on base `6d30f5c54b`, following Markdown math attribute round-trip.
...
| Local passing numerator | `lanes/pandoc/lane-status.json` reports 3,329 PHP passes / 0 failures. `lanes/pandoc/UPSTREAM_TEST_MANIFEST.json` reports 3,288 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,288 / 2,276 mapped upstream inventory rows = 144.5%. Percentages above 100% mean local PHP slices are more granular than upstream inventory rows. | High coverage, not global ship-ready. |
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check -- progress.md PANDOC_STATUS.md lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json lanes/pandoc/src/MarkdownReader.php lanes/pandoc/tests/MarkdownReaderTest.php lanes/pandoc/notes/pandoc-markdown-html-list-item-attrs-20260613.md`, syntax checks for the touched Markdown reader/test files, focused `MarkdownReaderTest.php`, and `php tools/run-tests.php lanes/pandoc/tests` passed. | 45 test files, 74,759 assertions, 0 failures. |
...
Current `main` is reconciled through the Markdown HTML list item attribute slice rebased on `6d30f5c54b`: 3,329 PHP passes / 0 failures and 3,288 mapped upstream cases. The recent closure wave landed the formerly pending CSV/TSV, MediaBag, Markdown block-boundary, escaped-attribute, fenced Div section-reference, definition-list term-group, table-cell note placement, header section Div evidence, Markdown HTML list item attribute handoff, Markdown math attribute round-trip, JSON/native block-container mixed-content flushing, XML/JATS/BITS direct reader capability diagnostics, notes/references backlink metadata, DOCX/OpenXML subdocument diagnostics, EPUB direct manifest/spine diagnostics, EPUB XHTML definition-list handoff, inline writer attribute handoff, text markup reader unsupported ship-gate accounting, LaTeX table-foot, JSON/native Div Plain, PDF/Typst package dependency conflict-policy, JSON/native task-list checkbox sidecar, JSON/native nullary helper payload, and LaTeX table body-head row evidence. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for the touched Markdown reader/test files, focused `MarkdownReaderTest.php` (`1` file, `6716` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `74759` assertions, `0` failures).
