...
Last reconciled: 2026-06-13 UTC after EPUB XHTML definition-list handoff on base `6dfacd7bba`, following Markdown fenced Div section-reference boundary scoping.
...
| Local passing numerator | `lanes/pandoc/lane-status.json` reports 3,321 PHP passes / 0 failures. `lanes/pandoc/UPSTREAM_TEST_MANIFEST.json` reports 3,280 mapped upstream cases. | PHP lane remains green. |
| Percent | 3,280 / 2,276 mapped upstream inventory rows = 144.1%. Percentages above 100% mean local PHP slices are more granular than upstream inventory rows. | High coverage, not global ship-ready. |
