...
Last reconciled: 2026-06-13 UTC after EndNote XML locator/import diagnostics on base `52a27eda2b`, following PDF/Typst package source policy.
...
| Verification | `jq empty lanes/pandoc/lane-status.json lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, `git diff --check`, syntax checks for the touched citation files, focused `CitationCslProcessorTest.php`, and `php tools/run-tests.php lanes/pandoc/tests` passed. | 45 test files, 75,069 assertions, 0 failures. |
...
Current `main` is reconciled through the EndNote XML locator/import diagnostic slice rebased on `52a27eda2b`: 3,337 PHP passes / 0 failures and 3,296 mapped upstream cases. The recent closure wave landed the formerly pending CSV/TSV direct reader and header option parity, MediaBag, Markdown block-boundary, escaped-attribute, fenced Div section-reference, definition-list term-group, table-cell note placement, header section Div evidence, EndNote XML locator/import diagnostics, PDF/Typst package source policy, text markup unsupported diagnostics, shared ZIP selected-entry role buckets, EPUB XHTML table semantics, PlainWriter table body-group boundaries, Markdown HTML list item attribute handoff, Markdown math attribute round-trip, JSON/native block-container mixed-content flushing, XML/JATS/BITS direct reader capability diagnostics, notes/references backlink metadata, DOCX/OpenXML subdocument diagnostics, EPUB direct manifest/spine diagnostics, EPUB XHTML definition-list handoff, inline writer attribute handoff, text markup reader unsupported ship-gate accounting, LaTeX table-foot, JSON/native Div Plain, PDF/Typst package dependency conflict-policy, JSON/native task-list checkbox sidecar, JSON/native nullary helper payload, and LaTeX table body-head row evidence. This refresh was checked with `jq empty`, `git diff --check`, syntax checks for the touched citation files, focused `CitationCslProcessorTest.php` (`1` file, `5332` assertions, `0` failures), and full `lanes/pandoc/tests` (`45` files, `75069` assertions, `0` failures).
...
| CSL/EndNote XML | `CitationCslProcessor::endnoteXmlItems()` maps bounded EndNote XML records into CSL items, preserves attachment/custom-field import diagnostics, and keeps section-locator suffix diagnostics visible through citation rendering. | Citation evidence improved to 78 local cases; full EndNote XML reader parity, broader RIS coverage, and registry-level parity remain partial. |
