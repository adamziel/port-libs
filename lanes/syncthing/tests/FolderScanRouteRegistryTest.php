...
return [
    ...
    'route registry acknowledges retained scan status checkpoints with revision guard' => static function (TestRunner $t): void {
        $mediaRoot = syncthing_folder_scan_route_root();
        $contentRoot = syncthing_folder_scan_route_root();
        try {
            syncthing_folder_scan_route_write($mediaRoot, 'wp-content/uploads/2026/05/hero.jpg', 'abcdefgh');
            ...
            $scheduler->addFolder('wordpress-content', new FolderScanService('wordpress-content', new FileInfoScanner($contentRoot), new FolderScanCheckpointStore()));
            $registry = FolderScanRouteRegistry::forScanApi(new FolderScanApiCoordinator($scheduler));
            ...
            $conflict = $registry->dispatch('POST', '/wp-json/local-first/v1/syncthing/db/scan/status/ack', [
                'folder' => 'wordpress-media',
                'expectedRevision' => 2,
            ], now: 5101);
            $mediaAck = $registry->dispatch('POST', '/syncthing/db/scan/status/ack', [
                'folder' => 'wordpress-media',
                'expectedRevision' => 1,
            ], now: 5101);
            $missingMediaAck = $registry->dispatch('POST', '/syncthing/db/scan/status/ack', [
                'folder' => 'wordpress-media',
            ], now: 5101);
            $statusAfterMediaAck = $registry->dispatch('GET', '/syncthing/db/scan/status', [], now: 5101);
            $allAck = $registry->dispatch('POST', '/syncthing/db/scan/status/ack', [], now: 5102);
            $invalidAllAck = $registry->dispatch('POST', '/syncthing/db/scan/status/ack', [
                'expectedRevision' => 1,
            ], now: 5102);
            $missingFolder = $registry->dispatch('POST', '/syncthing/db/scan/status/ack', [
                'folder' => 'missing-folder',
            ], now: 5102);
            $route = syncthing_folder_scan_route_by_path($registry->routes(), '/syncthing/db/scan/status/ack', 'POST');

            $t->same(FolderScanApiCoordinator::HTTP_CONFLICT, $conflict->statusCode);
            $t->same('checkpoint_revision_conflict', $conflict->body['error']);
            $t->same(FolderScanApiCoordinator::HTTP_OK, $mediaAck->statusCode);
            $t->same('acknowledged', $mediaAck->body['status']);
            $t->same(1, $mediaAck->body['acknowledged']);
            $t->same(1, $mediaAck->body['folders']['wordpress-media']['revision'] ?? null);
            $t->same('missing', $missingMediaAck->body['status']);
            $t->same(null, $statusAfterMediaAck->body['folders']['wordpress-media']['checkpoint'] ?? null);
            $t->same('complete', $statusAfterMediaAck->body['folders']['wordpress-content']['checkpoint']['state'] ?? null);
            $t->same('acknowledged', $allAck->body['status']);
            $t->same(1, $allAck->body['acknowledged']);
            $t->same(['wordpress-content'], array_keys($allAck->body['folders']));
            $t->same(FolderScanApiCoordinator::HTTP_BAD_REQUEST, $invalidAllAck->statusCode);
            $t->same('invalid_request', $invalidAllAck->body['error']);
            $t->same(FolderScanApiCoordinator::HTTP_NOT_FOUND, $missingFolder->statusCode);
            $t->same('folder_missing', $missingFolder->body['error']);
            $t->same('/wp-json/local-first/v1/syncthing/db/scan/status/ack', $route['wordpressRoute']);
            $t->same(true, $route['metadata']['scanStatusAck'] ?? null);
        } finally {
            syncthing_folder_scan_route_rm($mediaRoot);
            syncthing_folder_scan_route_rm($contentRoot);
        }
