...
The `../examples/wordpress-directory-modtime-sync.php` example maps rclone directory timestamp boundaries for WordPress uploads and incremental export directories. It detects a stale destination upload directory timestamp, repairs upload and export directory modtimes without replacing object bytes, and publishes the source upload directory through `lsjson --metadata`.

## Next Task

Map directory metadata copy ordering during sync, especially delayed deepest-first directory timestamp updates after child object writes.
