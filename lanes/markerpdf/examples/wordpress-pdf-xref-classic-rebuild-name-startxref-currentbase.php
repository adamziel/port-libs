...
$currentXmp = $xmpPacket('Current Name Startxref Title');
$nameDecoyXmp = $xmpPacket('Name Startxref Decoy Title');
...
echo '<!-- markerpdf-xref-classic-rebuild-name-startxref-currentbase-smoke ' . htmlspecialchars(json_encode([
    'scenario' => 'wordpress-pdf-xref-classic-rebuild-name-startxref-currentbase',
    'native_boundary' => 'classic xref rebuild ignores /startxref PDF name tokens before text metadata and EmbeddedFiles selection',
    'current_xref_before_name_startxref' => $currentXrefOffset < $nameDecoyXrefOffset,
    'uses_current_classic_trailer_root' => str_contains($plainText, 'Current name-startxref page'),
    'keeps_current_metadata_root' => ($metadata['title'] ?? null) === 'Current Name Startxref Title',
    'keeps_current_info_root' => ($metadata['info']['Title'] ?? null) === 'Current Name Startxref Info',
    'imports_current_attachment' => ($files[0]['name'] ?? null) === 'current-name-startxref.xml',
    'current_attachment_checksum_matches' => ($files[0]['checksum_matches'] ?? null) === true,
    'excludes_name_startxref_page' => !str_contains($plainText, 'Name startxref decoy page'),
    'excludes_name_startxref_metadata' => !str_contains($encodedMetadata, 'Name Startxref Decoy'),
    'excludes_name_startxref_attachment' => !str_contains($encodedFiles, 'decoy-name-startxref'),
