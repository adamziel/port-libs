...
$staleLabels = ['stale-private-88', 'stale-late-99'];

if ($labels !== ['Cover-', 'Body 4', 'App-Z'] || $previewLabels !== $labels) {
    throw new RuntimeException('Expected first usable duplicate PageLabels Nums value before WordPress page-break metadata.');
}

$staleExcluded = count(array_intersect($staleLabels, $labels, $previewLabels)) === 0;
if (!$staleExcluded || ($imagePlan['page_label'] ?? null) !== 'App-Z') {
    throw new RuntimeException('Expected private and later stale duplicate PageLabels Nums arrays to stay excluded.');
}

echo '<!-- markerpdf-page-labels-duplicate-nums-dictionary-key-smoke ' . htmlspecialchars(json_encode([
    'executes_python_or_models' => false,
    'executes_external_pdf_tools' => false,
    'native_boundary' => 'catalog /PageLabels node skips malformed duplicate /Nums values, then keeps first usable /Nums array before stale duplicates',
    'page_labels' => $labels,
    'preview_page_labels' => $previewLabels,
    'selected_preview_page_label' => $imagePlan['page_label'] ?? null,
    'malformed_nums_value_skipped' => $labels === ['Cover-', 'Body 4', 'App-Z'],
    'stale_duplicate_nums_key_rejected' => $staleExcluded,
