...
$content = 'BT /F1 12 Tf 72 720 Td (WordPress duplicate crypt-filter role encrypted text leak) Tj ET';
...
$declarationReview = is_array($metadata['encryption']['crypt_filter_role_declaration_review'] ?? null)
    ? $metadata['encryption']['crypt_filter_role_declaration_review']
    : [];
...
if (($permission['content_extraction_boundary'] ?? null) !== 'blocked_by_duplicate_document_crypt_filter_roles') {
    throw new RuntimeException('Expected duplicate crypt-filter role declarations to fail closed before WordPress import.');
}
if (($declarationReview['duplicate_pdf_names'] ?? []) !== ['StmF']) {
    throw new RuntimeException('Expected duplicate /StmF declarations to be preserved as review metadata.');
}
if (
    !is_string($encoded)
...
echo '<!-- markerpdf-encrypted-duplicate-crypt-filter-role-currentbase-smoke ' . htmlspecialchars(json_encode([
    'scenario' => 'wordpress-pdf-encrypted-duplicate-crypt-filter-role-currentbase',
    'native_boundary' => 'Standard encrypted duplicate crypt-filter role declarations fail closed before WordPress import without decrypting payloads',
    ...
    'duplicate_role_names' => $declarationReview['duplicate_role_names'] ?? [],
    'duplicate_pdf_names' => $declarationReview['duplicate_pdf_names'] ?? [],
    'fail_closed_role_names' => $review['fail_closed_role_names'] ?? [],
    'fail_closed_filter_names' => $review['fail_closed_filter_names'] ?? [],
    'role_declaration_statuses' => $review['role_declaration_statuses'] ?? [],
...
echo "<h2>Encrypted PDF Duplicate Crypt Filter Role Preflight</h2>\n";
echo "<p>Encrypted PDF text remains blocked. Duplicate document stream crypt-filter declarations are preserved as security review metadata and do not authorize native text import.</p>\n";
echo '<!-- markerpdf:encrypted-duplicate-crypt-filter-role-preflight ' . htmlspecialchars(json_encode([
    'permission' => [
    ...
    'crypt_filter_review' => [
        ...
        'fail_closed_filter_names' => $review['fail_closed_filter_names'] ?? [],
        'role_declaration_statuses' => $review['role_declaration_statuses'] ?? [],
        'role_declaration_duplicate_pdf_names' => $review['role_declaration_duplicate_pdf_names'] ?? [],
    ],
    'role_declaration_review' => [
        'duplicate_role_names' => $declarationReview['duplicate_role_names'] ?? [],
        'duplicate_pdf_names' => $declarationReview['duplicate_pdf_names'] ?? [],
        'fail_closed_role_names' => $declarationReview['fail_closed_role_names'] ?? [],
        'fail_closed_pdf_names' => $declarationReview['fail_closed_pdf_names'] ?? [],
    ],
], JSON_UNESCAPED_SLASHES), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . " -->\n";
