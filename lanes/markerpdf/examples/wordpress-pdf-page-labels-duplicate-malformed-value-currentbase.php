...
if ($labels !== ['Cover-', 'Body 4', 'App-Z'] || $previewLabels !== $labels) {
    throw new RuntimeException('Expected first usable duplicate PageLabels value after a malformed duplicate to drive WordPress page-break metadata.');
}

if (in_array('stale-duplicate-99', $labels, true) || in_array('stale-duplicate-99', $previewLabels, true)) {
    throw new RuntimeException('Expected later stale duplicate PageLabels value to stay excluded.');
}

echo '<!-- markerpdf-page-labels-duplicate-malformed-value-smoke ' . htmlspecialchars(json_encode([
    'executes_python_or_models' => false,
    'executes_external_pdf_tools' => false,
    'native_boundary' => 'catalog /PageLabels duplicate /Nums keys skip malformed same-key values before accepting the first usable duplicate section',
    'page_labels' => $labels,
    'preview_page_labels' => $previewLabels,
    'selected_preview_page_label' => $imagePlan['page_label'] ?? null,
    'first_usable_duplicate_value_preserved' => $labels[0] === 'Cover-' && $previewLabels[0] === 'Cover-',
    'stale_later_duplicate_rejected' => !in_array('stale-duplicate-99', $labels, true)
