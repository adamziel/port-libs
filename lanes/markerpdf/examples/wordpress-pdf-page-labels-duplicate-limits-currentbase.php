...
$staleLabels = ['stale-reversed-88', '1', '2'];

if ($labels !== $expected || $previewLabels !== $expected) {
    throw new RuntimeException('Expected duplicate PageLabels /Limits keys to recover the first usable limits range.');
}

if (count(array_intersect($staleLabels, $labels, $previewLabels)) !== 0 || ($imagePlan['page_label'] ?? null) !== 'App-Z') {
    throw new RuntimeException('Expected malformed/reversed duplicate PageLabels /Limits labels to stay excluded.');
}

echo '<!-- markerpdf-page-labels-duplicate-limits-smoke ' . htmlspecialchars(json_encode([
    'executes_python_or_models' => false,
    'executes_external_pdf_tools' => false,
    'native_boundary' => 'catalog /PageLabels duplicate /Limits keys recover the first usable range before WordPress page-break metadata',
    'page_labels' => $labels,
    'preview_page_labels' => $previewLabels,
    'selected_preview_page_label' => $imagePlan['page_label'] ?? null,
    'duplicate_limits_recovered' => $labels === $expected && $previewLabels === $expected,
    'stale_duplicate_limits_labels_excluded' => count(array_intersect($staleLabels, $labels, $previewLabels)) === 0,
