...
$before = 'BT /F1 12 Tf 72 720 Td (Before CCITT DecodeParms tail import) Tj ET';
$after = 'BT /F1 12 Tf 72 680 Td (After CCITT DecodeParms tail import) Tj ET';
$faxPayload = 'BT /F1 12 Tf 72 700 Td (WordPress CCITT DecodeParms tail payload noise) Tj ET';
...
$inlinePayload = "fax bytes EI BT /F1 12 Tf 72 640 Td (Inline WordPress CCITT DecodeParms tail payload noise) Tj ET final";
...
if (
    ...
    || ($decodeParms['decode_parms_operand_detail'] ?? null) !== 'dictionary_with_trailing_operands'
    || ($decodeParms['decode_parms_dictionary_policy'] ?? null) !== 'reject_top_level_decodeparms_dictionary_tail'
    || ($boundary['invalid_decode_parms'] ?? null) !== true
    || ($entry['payload_in_visible_text'] ?? null) !== false
    || ($inlineDecodeParms['decode_parms_operand_detail'] ?? null) !== 'dictionary_with_trailing_operands'
    || ($inlineReview['inline_image_payload_excluded_from_text'] ?? null) !== true
    || str_contains($plainText, 'WordPress CCITT DecodeParms tail payload noise')
    || str_contains(json_encode($review, JSON_UNESCAPED_SLASHES) ?: '', $faxPayload)
    || str_contains(json_encode($inlineReview, JSON_UNESCAPED_SLASHES) ?: '', 'Inline WordPress CCITT DecodeParms tail payload noise')
) {
    throw new RuntimeException('CCITT DecodeParms trailing-operand boundary smoke failed.');
}

$metadata = [
    'source' => 'native-pdf-ccitt-fax-decodeparms-trailing-operand-currentbase',
    ...
    'xobject_decode_parms_policy' => $decodeParms['decode_parms_dictionary_policy'] ?? null,
...
echo '<!-- markerpdf:pdf-ccitt-fax-decodeparms-trailing-operand-currentbase ' . htmlspecialchars(json_encode($metadata, JSON_UNESCAPED_SLASHES), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . " -->\n";
