...
$staleLabels = ['stale-malformed-99', 'stale-malformed-body-100', '1', '2'];

if ($labels !== ['Front iii', 'Body 8', 'App-Z'] || $previewLabels !== $labels || $summaryLabels !== $labels) {
    throw new RuntimeException('Expected valid same-lower PageLabels kid to win after malformed sibling contributes no labels.');
}

if (count(array_intersect($staleLabels, $labels, $previewLabels, $summaryLabels)) !== 0 || ($imagePlan['page_label'] ?? null) !== 'App-Z') {
    throw new RuntimeException('Expected malformed same-lower PageLabels labels and fallback labels to stay excluded.');
}

echo '<!-- markerpdf-page-labels-malformed-same-lower-kid-smoke ' . htmlspecialchars(json_encode([
    'executes_python_or_models' => false,
    'executes_external_pdf_tools' => false,
    'native_boundary' => 'malformed PageLabels same-lower kid nodes do not reserve their limit range unless they contribute usable labels',
    'page_labels' => $labels,
    'preview_page_labels' => $previewLabels,
    'summary_page_labels' => $summaryLabels,
    'selected_preview_page_label' => $imagePlan['page_label'] ?? null,
    'malformed_same_lower_kid_unclaimed' => $labels[0] === 'Front iii' && $labels[1] === 'Body 8',
    'stale_malformed_labels_excluded' => count(array_intersect($staleLabels, $labels, $previewLabels, $summaryLabels)) === 0,
