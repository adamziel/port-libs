...
$content = 'BT /F1 12 Tf 72 720 Td (WordPress composite Filter handler encrypted text leak) Tj ET';
...
if ($plainText !== '') {
    throw new RuntimeException('Expected composite security-handler Filter fixture text to stay blocked.');
}
if (($permission['content_extraction_boundary'] ?? null) !== 'blocked_encrypted_security_handler_malformed') {
    throw new RuntimeException('Expected composite security-handler Filter declarations to fail closed before import.');
}
if (($declaration['status'] ?? null) !== 'malformed_security_handler_filter_entries_review') {
    throw new RuntimeException('Expected composite security-handler Filter operand to be reviewed as malformed.');
}
...
if ($copyAllowed !== null || ($permission['permission_bits_reliable'] ?? true) !== false) {
    throw new RuntimeException('Expected composite security-handler Filter declarations to suppress permission-bit grants.');
}
if (
    !is_string($encoded)
...
echo '<!-- markerpdf-encrypted-composite-filter-handler-currentbase-smoke ' . htmlspecialchars(json_encode([
    'scenario' => 'wordpress-pdf-encrypted-composite-filter-handler-currentbase',
    'native_boundary' => 'composite top-level Encrypt /Filter operand is fail-closed before WordPress import trusts permission bits',
    ...
    'security_handler_malformed_filter_entries' => $permission['security_handler_malformed_filter_entries'] ?? null,
    ...
    'declaration_entry_operand_shapes' => $declaration['entry_operand_shapes'] ?? [],
...
echo '<p>' . htmlspecialchars(
    'Encrypted PDF text remains blocked. Composite security-handler Filter operands are review-only metadata until a real decryption and password-validation path exists.',
    ENT_QUOTES | ENT_SUBSTITUTE,
    'UTF-8'
) . "</p>\n";
echo "<!-- /wp:paragraph -->\n\n";

echo '<!-- markerpdf:encrypted-composite-filter-handler ' . htmlspecialchars(json_encode([
