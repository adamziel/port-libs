...
$buildPdf = static function (string $mappingKind): string {
    if ($mappingKind === 'char') {
        $mappingBlock = "4 begincidchar\n"
            ...
            . "endcidchar\n";
        $cMapName = 'WPDeclaredCountCidCharTrailingOperandSourceWidth-H';
        $baseFont = 'WPDeclaredCountCidCharTrailingOperandSourceWidth';
    } else {
        $mappingBlock = "1 begincidrange\n"
            . "<10> <13> 60 << /Tail true >>\n"
            . "endcidrange\n";
        $cMapName = 'WPDeclaredCountCidRangeTrailingOperandSourceWidth-H';
        $baseFont = 'WPDeclaredCountCidRangeTrailingOperandSourceWidth';
    }
...
$flags = [
    'source' => 'native-pdf-cmap-declared-count-cid-trailing-operand-source-width-currentbase',
    ...
    'cidchar_declared_rows_preserved' => array_column($charSpans, 'bbox') === [[0.0, 0.0, 12.0, 12.0], [12.0, 0.0, 60.0, 12.0]],
    'cidrange_declared_rows_preserved' => array_column($rangeSpans, 'bbox') === [[0.0, 0.0, 12.0, 12.0], [12.0, 0.0, 60.0, 12.0]],
    'trailing_operand_excluded' => !str_contains($charText . $rangeText, 'Tail')
...
if (in_array(false, $behaviorFlags, true)) {
    throw new RuntimeException('Expected CMap declared-count trailing-operand source-width smoke flags to pass: ' . json_encode($flags, JSON_UNESCAPED_SLASHES));
}

echo '<!-- markerpdf:pdf-cmap-declared-count-cid-trailing-operand-source-width-currentbase '
