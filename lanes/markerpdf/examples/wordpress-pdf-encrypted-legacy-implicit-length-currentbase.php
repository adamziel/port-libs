...
$content = 'BT /F1 12 Tf 72 720 Td (Legacy implicit key length WordPress text leak) Tj ET';
...
if ($plainText !== '') {
    throw new RuntimeException('Expected encrypted legacy fixture text to stay blocked.');
}

if (($encryption['key_length_bits'] ?? null) !== 40) {
    throw new RuntimeException('Expected Standard /V 1 implicit key length to resolve to 40 bits.');
}

if (($parameterReview['key_length_present'] ?? null) !== false) {
    throw new RuntimeException('Expected omitted /Length to remain distinguishable from explicit key length.');
}

if (($parameterReview['key_length_status'] ?? null) !== 'standard_security_handler_key_length_implicit_40_bit') {
    throw new RuntimeException('Expected implicit 40-bit Standard key-length status.');
}

if (($permission['policy'] ?? null) !== 'copy_extract_allowed_after_decryption') {
    throw new RuntimeException('Expected copy/extract to remain blocked until decryption despite allowed permission bits.');
}

echo '<!-- markerpdf-encrypted-legacy-implicit-length-currentbase-smoke ' . htmlspecialchars(json_encode([
    'scenario' => 'wordpress-pdf-encrypted-legacy-implicit-length-currentbase',
    'native_boundary' => 'Standard V1 omitted Length is surfaced as implicit 40-bit review metadata before encrypted WordPress import',
...
echo "<!-- wp:heading -->\n";
echo "<h2>Encrypted PDF Legacy Permission Review</h2>\n";
echo "<!-- /wp:heading -->\n\n";

echo "<!-- wp:paragraph -->\n";
echo '<p>' . htmlspecialchars(
    'Encrypted PDF text remains blocked. The security preflight records that this legacy Standard handler omitted /Length and uses the implicit 40-bit key length before any password or decryption step.',
    ENT_QUOTES | ENT_SUBSTITUTE,
    'UTF-8'
) . "</p>\n";
echo "<!-- /wp:paragraph -->\n\n";

echo '<!-- markerpdf:encrypted-legacy-implicit-length ' . htmlspecialchars(json_encode([
    'permission' => [
