...
if (!is_string($objectStream)) {
    throw new RuntimeException('Unable to compress skipped-header object-stream smoke fixture.');
}
...
if (!is_string($compressedXref)) {
    throw new RuntimeException('Unable to compress skipped-header xref-stream smoke fixture.');
}
...
echo '<!-- markerpdf-xref-object-stream-skipped-header-index-currentbase-smoke ' . htmlspecialchars(json_encode([
    'executes_python_or_models' => false,
    'executes_external_pdf_tools' => false,
    'native_boundary' => 'PDF xref-stream type-2 archive indexes remain aligned with original ObjStm header rows',
    'uses_skipped_header_explicit_index_page' => str_contains($plainText, 'Skipped header explicit index page'),
    'uses_direct_guard_page' => str_contains($plainText, 'Current skipped-header guard page'),
    'excluded_skipped_zero_header_decoy' => !str_contains($plainText, 'skipped zero header row decoy'),
    'xref_member_index' => $entry['xref_member_index'] ?? null,
    'actual_member_index' => $entry['actual_member_index'] ?? null,
