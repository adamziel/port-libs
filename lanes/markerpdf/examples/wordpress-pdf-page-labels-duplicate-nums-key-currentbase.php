...
if ($labels !== ['Cover-', 'Body 4', 'App-Z'] || $previewLabels !== $labels) {
    throw new RuntimeException('Expected first valid duplicate PageLabels /Nums key to stay aligned across import and preview metadata.');
}

if (in_array('stale-duplicate-99', $labels, true) || in_array('stale-duplicate-99', $previewLabels, true)) {
    throw new RuntimeException('Expected stale duplicate PageLabels /Nums entry to stay excluded.');
}

echo '<!-- markerpdf-page-labels-duplicate-nums-key-smoke ' . htmlspecialchars(json_encode([
    'executes_python_or_models' => false,
    'executes_external_pdf_tools' => false,
    'native_boundary' => 'catalog /PageLabels duplicate /Nums keys keep the first valid page-index section before stale relabeling',
    'page_labels' => $labels,
    'preview_page_labels' => $previewLabels,
    'selected_preview_page_label' => $imagePlan['page_label'] ?? null,
    'stale_duplicate_nums_key_rejected' => !in_array('stale-duplicate-99', $labels, true)
        && !in_array('stale-duplicate-99', $previewLabels, true),
