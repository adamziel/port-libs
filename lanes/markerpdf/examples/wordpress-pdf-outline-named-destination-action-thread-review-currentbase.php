...
if (($outlineActions[1]['destination_action_target_page_review']['piece_info']['WPThread']['private']['ReviewState'] ?? null) !== 'action-thread-review') {
    throw new RuntimeException('Expected chained action rows to inherit named action target page-review context.');
}
if (($outlineActions[1]['destination_action_target_page_review']['page_associated_files'][0]['filename'] ?? null) !== 'article-review.xml') {
    throw new RuntimeException('Expected chained action rows to inherit named action target associated-file context.');
}
if (str_contains($plainText, 'ArticleAction') || str_contains($plainText, 'ArticleStory') || str_contains($plainText, 'thread-review') || str_contains($plainText, 'hidden thread named action script') || str_contains($plainText, 'wp-outline-review') || str_contains($plainText, 'action-thread-review')) {
...
echo '<!-- markerpdf-outline-named-destination-action-thread-review-currentbase ' . htmlspecialchars(json_encode([
    ...
    'chained_action_target_page_review_state' => $outlineActions[1]['destination_action_target_page_review']['piece_info']['WPThread']['private']['ReviewState'] ?? null,
    'chained_action_target_attachment' => $outlineActions[1]['destination_action_target_page_review']['page_associated_files'][0]['filename'] ?? null,
    ...
    'visible_text_excludes_named_action_operands' => !str_contains($plainText, 'ArticleAction')
        ...
        && !str_contains($plainText, 'wp-outline-review')
        && !str_contains($plainText, 'action-thread-review'),
...
foreach ($outlineActions as $row) {
    ...
    echo '<li data-marker-outline-action-title="' . htmlspecialchars($row['outline_title'], ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8')
        ...
        . '" data-marker-destination-action-review-state="' . htmlspecialchars((string) ($targetReview['piece_info']['WPThread']['private']['ReviewState'] ?? ''), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8')
        . '" data-marker-destination-action-attachment="' . htmlspecialchars((string) ($targetReview['page_associated_files'][0]['filename'] ?? ''), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8')
