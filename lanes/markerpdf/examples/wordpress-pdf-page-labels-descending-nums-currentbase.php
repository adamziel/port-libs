...
if ($labels !== ['Front iv', 'Front v', 'App-Z', 'End-'] || $previewLabels !== $labels) {
    throw new RuntimeException('Expected descending PageLabels /Nums key to be rejected before WordPress page-break metadata.');
}

if (in_array('stale-descending-99', $labels, true) || in_array('stale-descending-99', $previewLabels, true)) {
    throw new RuntimeException('Expected stale descending PageLabels entry to stay excluded.');
}

echo '<!-- markerpdf-page-labels-descending-nums-smoke ' . htmlspecialchars(json_encode([
    'executes_python_or_models' => false,
    'executes_external_pdf_tools' => false,
    'native_boundary' => 'catalog /PageLabels leaf /Nums keys remain monotonically increasing before stale descending relabels are accepted',
    'page_labels' => $labels,
    'preview_page_labels' => $previewLabels,
    'selected_preview_page_label' => $imagePlan['page_label'] ?? null,
    'stale_descending_nums_key_rejected' => !in_array('stale-descending-99', $labels, true)
        && !in_array('stale-descending-99', $previewLabels, true),
