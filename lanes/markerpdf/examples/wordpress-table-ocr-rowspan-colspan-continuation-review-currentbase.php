...
$path = sys_get_temp_dir() . '/markerpdf-table-ocr-span-continuation-' . bin2hex(random_bytes(4)) . '.pdf';
file_put_contents($path, "%PDF-1.4\n% table OCR rowspan colspan continuation fixture\n%%EOF");
...
echo json_encode([
    'scenario' => 'wordpress-table-ocr-rowspan-colspan-continuation-review-currentbase',
    'native_boundary' => 'forced OCR detector cells whose anchors land in an occupied rowspan/colspan grid slot are folded into the covering WordPress review cell',
    'source_truth' => 'tabled recognize_tables applies OCR text before assign_rows_columns, SpanTableCell preserves row_ids/col_ids, and markdown/html formatters consume only first-row/column anchors',
    ...
    'joined_anchor_continuation' => ($review['render_cells'][0]['text'] ?? '') === 'Inventory continued',
    'continuation_count' => $review['render_cells'][0]['continuation_count'] ?? 0,
    ...
    'excluded_stale_pdftext_table_line' => !str_contains($result['text'], 'Stale OCR rowspan table text should be replaced.'),
