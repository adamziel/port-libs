...
$pdfPath = sys_get_temp_dir() . '/markerpdf-wordpress-table-rows-cols-boundary-' . bin2hex(random_bytes(4)) . '.pdf';
file_put_contents($pdfPath, "%PDF-1.4\n% rows_cols table boundary WordPress fixture\n%%EOF");
...
if (($metadata['table_coordinate_space_reviews'][0]['status'] ?? null) !== 'translated_to_table_crop') {
    throw new RuntimeException('Expected rows_cols table geometry to be localized from page-image to table-crop coordinates.');
}
if (($assignedByText['Header']['row_ids'] ?? null) !== [10] || ($assignedByText['Header']['col_ids'] ?? null) !== [20, 21]) {
    throw new RuntimeException('Expected header assignment to preserve upstream rows_cols row and column ids.');
}
if (($renderByText['Header']['colspan'] ?? null) !== 2) {
    throw new RuntimeException('Expected WordPress spanning-grid review to retain rows_cols header colspan.');
}
if (str_contains($result['text'], 'Stale rows-cols table line should be replaced.')) {
    throw new RuntimeException('Expected supplied table Markdown to replace stale pdftext table line.');
}

echo json_encode([
    'scenario' => 'wordpress-table-rows-cols-boundary-currentbase',
    'native_boundary' => 'single-table supplied rows_cols TableResult wrappers are canonicalized before crop-local assignment and WordPress grid review',
    'source_truth' => [
        'upstream' => 'tabled ExtractPageResult stores TableResult row and column bands under rows_cols while cells carry SpanTableCell geometry',
    ...
    'rows_cols_geometry_preserved' => ($assignedByText['Header']['row_ids'] ?? null) === [10]
        && ($assignedByText['Header']['col_ids'] ?? null) === [20, 21]
        && ($renderByText['Header']['colspan'] ?? null) === 2,
    'stale_pdftext_table_line_excluded' => !str_contains($result['text'], 'Stale rows-cols table line should be replaced.'),
