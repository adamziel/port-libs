...
$expectedLabels = ['4', 'Valid-8'];

if ($labels !== $expectedLabels || $previewLabels !== $expectedLabels) {
    throw new RuntimeException('Expected malformed UTF-16 PageLabels prefix to fail closed before WordPress page-break metadata.');
}

if (($imagePlan['page_label'] ?? null) !== 'Valid-8') {
    throw new RuntimeException('Expected preview image metadata to preserve valid UTF-16 PageLabels prefixes.');
}

echo '<!-- markerpdf-page-labels-utf16-malformed-prefix-smoke ' . htmlspecialchars(json_encode([
    'executes_python_or_models' => false,
    'executes_external_pdf_tools' => false,
    'native_boundary' => 'catalog /PageLabels UTF-16 prefix strings validate BOM payloads before WordPress page-break metadata',
    'page_labels' => $labels,
    'preview_page_labels' => $previewLabels,
    'selected_preview_page_label' => $imagePlan['page_label'] ?? null,
    'malformed_utf16_prefix_rejected' => !in_array('AB4', $labels, true)
        && !in_array('AB4', $previewLabels, true),
    'valid_utf16_prefix_preserved' => in_array('Valid-8', $labels, true)
        && in_array('Valid-8', $previewLabels, true),
