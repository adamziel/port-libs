...
echo '<!-- markerpdf-xref-classic-stale-startxref-currentbase-smoke ' . htmlspecialchars(json_encode([
    'native_boundary' => 'stale but valid startxref rebuilds to a later top-level classic xref trailer root before text extraction',
    'uses_latest_classic_trailer_root' => str_contains($plainText, 'Current stale-pointer rebuild page'),
    'repairs_stale_valid_startxref_pointer' => str_contains($plainText, 'Stale startxref pointer repaired'),
    'excludes_stale_valid_startxref_page' => !str_contains($plainText, 'Stale valid startxref page'),
    'excludes_earlier_trailer_root_leak' => !str_contains($plainText, 'Earlier trailer root leak'),
