...
if ($labels !== ['Front iv', 'Front v', 'App-Z']) {
    throw new RuntimeException('Expected native text extraction to resolve indirect PageLabels operands.');
}

if ($previewLabels !== $labels || ($imagePlan['page_label'] ?? null) !== 'App-Z') {
    throw new RuntimeException('Expected MarkerAppPreview labels to match native text extraction.');
}

echo '<!-- markerpdf-page-labels-indirect-operands-boundary-smoke ' . htmlspecialchars(json_encode([
    'executes_python_or_models' => false,
    'executes_external_pdf_tools' => false,
    'native_boundary' => 'catalog /PageLabels indirect style prefix and start operands before WordPress page-break metadata',
    'page_labels' => $labels,
    'preview_page_labels' => $previewLabels,
    'selected_preview_page_label' => $imagePlan['page_label'] ?? null,
    'empty_preview_labels_rejected' => !in_array('', $previewLabels, true),
