...
$charProc = "1000 0 d0\n"
    ...
    . "BT /Fghost 9 Tf (WordPress Type3 CMap CharProc text leak) Tj ET\n";
$pageContent = 'BT /Ft3 24 Tf 72 720 Td <AB01> Tj ET';
$glyphPayload = 'BT /Fghost 9 Tf 0 0 Td (WordPress Type3 CMap Glyph Image Payload Noise) Tj ET';
$unusedPayload = 'BT /Fghost 9 Tf 0 0 Td (WordPress Unused Type3 CMap Image Payload Noise) Tj ET';
...
if (!is_string($glyphCompressed) || !is_string($unusedCompressed)) {
    throw new RuntimeException('Unable to compress Type3 CMap CharProc image smoke payloads.');
}
...
$metadata = [
    'source' => 'native-pdf-image-xobject-type3-charproc-cmap-currentbase',
    'upstream_boundary' => 'markerPDF searchable Type3 text extraction plus image media handoff; CMap-encoded Type3 CharProc Image XObjects remain review-only glyph paint metadata',
    ...
    'type3_resource_path' => $entry['resource_path'] ?? null,
    ...
    'cmap_source_bytes_excluded_from_text' => !str_contains($plainText, 'AB01'),
    'charproc_text_excluded' => !str_contains($plainText, 'WordPress Type3 CMap CharProc text leak'),
    'glyph_payload_excluded_from_text' => !str_contains($plainText, 'WordPress Type3 CMap Glyph Image Payload Noise'),
    'unused_payload_excluded_from_text' => !str_contains($plainText, 'WordPress Unused Type3 CMap Image Payload Noise'),
...
if (
    ...
    || $metadata['uninvoked_image_xobject_count'] !== 0
    || $metadata['glyph_image_invoked'] !== true
    || $metadata['type3_font_resource'] !== 'Ft3'
    || $metadata['type3_glyph_name'] !== 'A'
    || $metadata['type3_resource_path'] !== ['Type3 Ft3', 'A', 'Glyph Image']
    || $metadata['visible_text'] !== 'A'
    || $metadata['cmap_source_bytes_excluded_from_text'] !== true
    ...
    throw new RuntimeException('Type3 CMap CharProc image XObject WordPress smoke failed.');
}

echo '<!-- markerpdf:pdf-image-xobject-type3-charproc-cmap-currentbase '
