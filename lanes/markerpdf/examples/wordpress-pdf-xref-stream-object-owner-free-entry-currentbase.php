...
if (!is_string($objectStream)) {
    throw new RuntimeException('Unable to compress xref-stream free-owner object-stream smoke fixture.');
}
...
if (!is_string($previousCompressed)) {
    throw new RuntimeException('Unable to compress previous xref-stream free-owner smoke fixture.');
}
...
if (!is_string($currentCompressed)) {
    throw new RuntimeException('Unable to compress current xref-stream free-owner smoke fixture.');
}
...
echo '<!-- markerpdf-xref-stream-object-owner-free-entry-currentbase-smoke ' . htmlspecialchars(json_encode([
    'executes_python_or_models' => false,
    'executes_external_pdf_tools' => false,
    'native_boundary' => 'current xref-stream type-0 free rows own object numbers before stale direct scans and previous object-stream members',
    'uses_current_xref_stream_free_owner_page' => str_contains($plainText, 'Current xref stream free owner page'),
    'reports_xref_stream_free_owner' => ($review['xref_stream_free_owner_count'] ?? 0) === 1,
    'suppresses_previous_compressed_owner' => ($freeEntry['owner_policy'] ?? null) === 'xref_stream_free_entry_suppressed_prev_compressed_object',
    'suppresses_scanned_direct_owner' => ($freeEntry['direct_object_suppressed'] ?? false) === true,
    'excluded_stale_direct_page' => !str_contains($plainText, 'Stale direct free-owner page'),
    'excluded_stale_compressed_page' => !str_contains($plainText, 'Stale compressed free-owner page'),
