...
$before = 'BT /F1 12 Tf 72 720 Td (Before ASCIIHex DCT Import) Tj ET';
$after = 'BT /F1 12 Tf 72 680 Td (After ASCIIHex DCT Import) Tj ET';
$fakeObject = 'BT /F1 12 Tf 72 700 Td (WordPress ASCIIHex DCT EOD Leak) Tj ET';
...
if ($fakeTerminatorOffset === false) {
    throw new RuntimeException('DCT ASCIIHex EOD smoke must expose a fake endstream marker.');
}
...
$payloadExcluded = !str_contains($plainText, 'WordPress ASCIIHex DCT EOD Leak')
    && !str_contains($plainText, 'incomplete')
...
if ($lines !== ['Before ASCIIHex DCT Import', 'After ASCIIHex DCT Import'] || !$payloadExcluded || !$recoveredLength) {
    throw new RuntimeException('DCTDecode ASCIIHex early-EOD boundary leaked JPEG payload bytes into WordPress import.');
}

echo '<!-- markerpdf:pdf-dctdecode-asciihex-eod-boundary-currentbase ' . htmlspecialchars(json_encode([
    'source' => 'native-pdf-dctdecode-asciihex-eod-boundary',
    ...
    'prefix_filter_eod_validates_complete_jpeg' => true,
    'fake_asciihex_eod_endstream_ignored' => true,
    'stale_length_recovered_to_review_boundary' => $recoveredLength,
