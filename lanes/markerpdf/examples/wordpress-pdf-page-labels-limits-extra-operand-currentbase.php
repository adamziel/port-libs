...
$staleLabels = ['1', 'App-AA'];

if ($labels !== ['Cover-', 'Body 4', 'App-Z', 'Back-9'] || $previewLabels !== $labels || $summaryLabels !== $labels) {
    throw new RuntimeException('Expected malformed PageLabels /Limits extra operands to be ignored before import metadata.');
}

if (count(array_intersect($staleLabels, $labels, $previewLabels, $summaryLabels)) !== 0 || ($imagePlan['page_label'] ?? null) !== 'Back-9') {
    throw new RuntimeException('Expected malformed /Limits clipping artifacts to stay excluded.');
}

echo '<!-- markerpdf-page-labels-limits-extra-operand-smoke ' . htmlspecialchars(json_encode([
    'executes_python_or_models' => false,
    'executes_external_pdf_tools' => false,
    'native_boundary' => 'catalog /PageLabels /Limits arrays must contain exactly two operands before WordPress page-break metadata',
    'page_labels' => $labels,
    'preview_page_labels' => $previewLabels,
    'summary_page_labels' => $summaryLabels,
    'selected_preview_page_label' => $imagePlan['page_label'] ?? null,
    'malformed_limits_ignored' => $labels[0] === 'Cover-' && $labels[3] === 'Back-9',
    'stale_clipped_labels_excluded' => count(array_intersect($staleLabels, $labels, $previewLabels, $summaryLabels)) === 0,
