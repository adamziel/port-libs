...
if ($labels !== $expected || $previewLabels !== $expected) {
    throw new RuntimeException('Expected touching PageLabels kid Limits to preserve the earlier inclusive endpoint.');
}

if (in_array('stale-touch-77', $labels, true) || in_array('stale-touch-77', $previewLabels, true)) {
    throw new RuntimeException('Expected stale endpoint-overlap PageLabels entry to stay excluded.');
}

echo '<!-- markerpdf-page-labels-touching-kid-limits-smoke ' . htmlspecialchars(json_encode([
    'executes_python_or_models' => false,
    'executes_external_pdf_tools' => false,
    'native_boundary' => 'PageLabels kid /Limits endpoint-crossing overlap is bounded before stale relabeling',
    'page_labels' => $labels,
    'preview_page_labels' => $previewLabels,
    'selected_preview_page_label' => $imagePlan['page_label'] ?? null,
    'touching_endpoint_rejected' => !in_array('stale-touch-77', $labels, true)
        && !in_array('stale-touch-77', $previewLabels, true),
    'earlier_endpoint_continuation_preserved' => ($labels[2] ?? null) === 'Body 9',
