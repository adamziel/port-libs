...
$staleContent = 'BT /F1 12 Tf 72 720 Td (Stale malformed Root smoke page) Tj ET';
$staleXmp = '<?xpacket begin="" id="W5M0MpCehiHzreSzNTczkc9d"?>'
    . '<x:xmpmeta xmlns:x="adobe:ns:meta/">'
    . '<rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#">'
    . '<rdf:Description rdf:about="" xmlns:dc="http://purl.org/dc/elements/1.1/">'
    . '<dc:title><rdf:Alt><rdf:li xml:lang="x-default">Stale Malformed Root Smoke XMP</rdf:li></rdf:Alt></dc:title>'
...
$staleInfoOffset = $addObject(6, 0, '<< /Title (Stale Malformed Root Smoke Info) /Author (Stale Malformed Root Smoke Author) /Producer (Stale Malformed Root Smoke Producer) >>');
$staleMetadataOffset = $addObject(7, 0, '<< /Type /Metadata /Subtype /XML /Length ' . strlen($staleXmp) . " >>\nstream\n{$staleXmp}\nendstream");
$staleNameTreeOffset = $addObject(8, 0, '<< /Names [(stale-malformed-root-smoke.xml) 10 0 R] >>');
$staleFileSpecOffset = $addObject(10, 0, '<< /Type /Filespec /F (stale-malformed-root-smoke.xml) /Desc (Stale malformed Root smoke attachment) /AFRelationship /Source /EF << /F 11 0 R >> >>');
$stalePayload = '<wp-export><post id="stale-malformed-root-smoke"/></wp-export>';
...
$currentInfoOffset = $addObject(6, 0, '<< /Title (Current Malformed Root Smoke Info) /Author (Current Malformed Root Smoke Author) /Producer (Current Malformed Root Smoke Producer) >>');
...
if (!is_string($compressedCurrentRows)) {
    throw new RuntimeException('Unable to compress malformed-Root xref smoke rows.');
}

$currentXrefOffset = strlen($pdf);
$pdf .= "20 0 obj\n"
    . '<< /Type /XRef /Size 21 /Root /CurrentRootCleared /Info 6 0 R /Prev ' . $previousXrefOffset . ' /Index [6 1] /W [1 4 1] /Filter /FlateDecode /Length ' . strlen($compressedCurrentRows) . " >>\n"
...
echo '<!-- markerpdf-xref-prev-chain-malformed-root-currentbase '
    . htmlspecialchars(json_encode([
        'malformed_root_latest_xref_stream_stops_prev_catalog' => $lines === []
            && !isset($metadata['catalog'])
            && !isset($metadata['language']),
        'current_info_selected' => ($metadata['title'] ?? null) === 'Current Malformed Root Smoke Info'
            && ($metadata['info']['Title'] ?? null) === 'Current Malformed Root Smoke Info',
        'stale_prev_text_excluded' => !in_array('Stale malformed Root smoke page', $lines, true),
        'stale_prev_metadata_excluded' => is_string($encodedMetadata)
            && !str_contains($encodedMetadata, 'Stale Malformed Root Smoke')
            && !str_contains($encodedMetadata, 'stale-malformed-root-smoke'),
        'stale_prev_embedded_files_excluded' => $embeddedFiles === []
            && is_string($encodedEmbeddedFiles)
            && !str_contains($encodedEmbeddedFiles, 'stale-malformed-root-smoke'),
        'attachment_summary_empty' => ($attachmentSummary['attachment_count'] ?? null) === 0
            ...
            && !str_contains($encodedAttachmentSummary, 'stale-malformed-root-smoke'),
