...
$before = 'BT /F1 12 Tf 72 720 Td (Before ASCII85 BOM DCT import) Tj ET';
$after = 'BT /F1 12 Tf 72 680 Td (After ASCII85 BOM DCT import) Tj ET';
$fakeObject = 'BT /F1 12 Tf 72 700 Td (WordPress ASCII85 BOM DCT leak) Tj ET';
$jpegPayload = "\xff\xd8\xff\xe0\x00\x10JFIF\0complete!\xff\xd9";
$encodedPayload = "\xef\xbb\xbf" . $ascii85Encode($jpegPayload);
$streamPayload = $encodedPayload
    . "\nendstream\nendobj\n"
    . "9 0 obj\n<< /Length " . strlen($fakeObject) . " >>\nstream\n{$fakeObject}\nendstream\nendobj\n";
$staleTerminatorOffset = strpos($streamPayload, "\nendstream\n");
if ($staleTerminatorOffset === false) {
    throw new RuntimeException('ASCII85 BOM DCT smoke must expose a stale endstream marker.');
}
...
if (
    $plainText !== "Before ASCII85 BOM DCT import\nAfter ASCII85 BOM DCT import"
    || str_contains($plainText, 'WordPress ASCII85 BOM DCT leak')
    ...
    || ($rendererStream['native_prefix_decoded_length'] ?? null) !== strlen($jpegPayload)
    || ($rendererBoundary['review_stream_decoded_from_native_prefix'] ?? false) !== true
    || ($rendererBoundary['source'] ?? null) !== 'dctdecode_jpeg_marker_boundary'
    || ($entry['decoded_with_current_filters'] ?? true) !== false
    || ($entry['native_raster_decode'] ?? true) !== false
) {
    throw new RuntimeException('ASCII85 BOM-prefixed DCT boundary smoke failed.');
}

$metadata = [
    'source' => 'native-pdf-dctdecode-ascii85-bom-prefix-boundary-currentbase',
    ...
    'renderer_native_prefix_decoded_length' => $rendererStream['native_prefix_decoded_length'] ?? null,
    'renderer_boundary_source' => $rendererBoundary['source'] ?? null,
    'renderer_boundary_decoded_from_native_prefix' => $rendererBoundary['review_stream_decoded_from_native_prefix'] ?? null,
    'renderer_stopped_before_filter' => $rendererStream['stopped_before_filter'] ?? null,
    'bom_prefixed_ascii85_member_excluded_from_visible_text' => !str_contains($plainText, 'WordPress ASCII85 BOM DCT leak'),
    'bom_prefixed_ascii85_member_decoded_for_renderer_review' => ($rendererStream['native_prefix_decoded'] ?? false) === true,
...
echo '<!-- markerpdf:pdf-dctdecode-ascii85-bom-prefix-boundary-currentbase ' . htmlspecialchars(json_encode($metadata, JSON_UNESCAPED_SLASHES), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . " -->\n";
foreach (explode("\n", $plainText) as $line) {
    echo "<!-- wp:paragraph -->\n";
