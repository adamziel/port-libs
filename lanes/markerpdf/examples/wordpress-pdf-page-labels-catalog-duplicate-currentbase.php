...
if ($labels !== ['Cover-', 'Body 4', 'App-Z'] || $previewLabels !== $labels) {
    throw new RuntimeException('Expected first usable catalog PageLabels value to stay aligned across import and preview metadata.');
}

if (
    in_array('stale-private-77', $labels, true)
    || in_array('stale-late-99', $labels, true)
    || in_array('stale-late-99', $previewLabels, true)
) {
    throw new RuntimeException('Expected stale duplicate catalog PageLabels values to stay excluded.');
}

echo '<!-- markerpdf-page-labels-catalog-duplicate-smoke ' . htmlspecialchars(json_encode([
    'executes_python_or_models' => false,
    'executes_external_pdf_tools' => false,
    'native_boundary' => 'catalog duplicate /PageLabels keys skip malformed values and keep the first usable number tree before stale duplicates',
    'page_labels' => $labels,
    'preview_page_labels' => $previewLabels,
    'selected_preview_page_label' => $imagePlan['page_label'] ?? null,
    'malformed_catalog_page_labels_skipped' => $labels === ['Cover-', 'Body 4', 'App-Z'],
    'stale_duplicate_catalog_page_labels_rejected' => !in_array('stale-private-77', $labels, true)
        && !in_array('stale-late-99', $labels, true)
        && !in_array('stale-late-99', $previewLabels, true),
