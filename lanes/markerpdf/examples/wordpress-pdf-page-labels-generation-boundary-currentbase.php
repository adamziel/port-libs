...
if ($labels !== ['Cover-', 'Body 4', 'Body 5']) {
    throw new RuntimeException('Expected generation-exact PageLabels dictionaries before WordPress page-break metadata.');
}

if ($previewLabels !== $labels || ($imagePlan['page_label'] ?? null) !== 'Body 5') {
    throw new RuntimeException('Expected MarkerAppPreview labels to stay aligned with native PageLabels extraction.');
}

echo '<!-- markerpdf-page-labels-generation-boundary-smoke ' . htmlspecialchars(json_encode([
    'executes_python_or_models' => false,
    'executes_external_pdf_tools' => false,
    'native_boundary' => 'catalog /PageLabels indirect value dictionaries resolve by exact object generation',
    'page_labels' => $labels,
    'preview_page_labels' => $previewLabels,
    'selected_preview_page_label' => $imagePlan['page_label'] ?? null,
    'stale_high_generation_rejected' => !in_array('stale-high-generation-99', $labels, true)
        && !in_array('stale-high-generation-100', $previewLabels, true),
