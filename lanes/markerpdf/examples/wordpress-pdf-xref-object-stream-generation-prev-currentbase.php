...
$objectStream = static function (array $members, array &$memberIndexes): array {
    ...
    if (!is_string($compressed)) {
        throw new RuntimeException('Unable to compress object-stream generation smoke fixture.');
    }
...
echo 'uses_current_carrier_generation_page=' . (str_contains($plainText, 'Current carrier generation page') ? 'true' : 'false') . "\n";
echo 'skips_prev_type2_member_for_replaced_carrier_generation=' . (!str_contains($plainText, 'Previous selected carrier page') ? 'true' : 'false') . "\n";
echo 'excludes_replacement_generation_carrier_member=' . (!str_contains($plainText, 'Replacement generation carrier leak') ? 'true' : 'false') . "\n";
