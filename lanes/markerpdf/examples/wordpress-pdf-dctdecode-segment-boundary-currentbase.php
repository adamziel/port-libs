...
$before = 'BT /F1 12 Tf 72 720 Td (Before APP DCT Import) Tj ET';
$after = 'BT /F1 12 Tf 72 680 Td (After APP DCT Import) Tj ET';
$fakeObject = 'BT /F1 12 Tf 72 700 Td (WordPress APP Segment DCT Leak) Tj ET';
...
if ($fakeEoiOffset === false) {
    throw new RuntimeException('DCT APP-segment smoke must contain a fake EOI before fake endstream.');
}
$staleLength = $fakeEoiOffset + 2;
...
$payloadExcluded = !str_contains($plainText, 'WordPress APP Segment DCT Leak')
    && !str_contains($plainText, 'Adobe APP data')
...
if ($lines !== ['Before APP DCT Import', 'After APP DCT Import'] || !$payloadExcluded || !$recoveredLength) {
    throw new RuntimeException('DCTDecode APP-segment boundary leaked JPEG payload bytes into WordPress import.');
}

echo '<!-- markerpdf:pdf-dctdecode-segment-boundary-currentbase ' . htmlspecialchars(json_encode([
    'source' => 'native-pdf-dctdecode-segment-boundary',
    ...
    'fake_eoi_inside_app_segment_ignored' => true,
    'stale_length_recovered_to_actual_eoi' => $recoveredLength,
