...
if (!is_string($compressedSample)) {
    throw new RuntimeException('Unable to build inline Decode name operand WordPress fixture.');
}

$dictionary = '/W 1 /H 1 /CS /G /BPC 8 /D [0 /Fl] /F /Fl';
$surplus = 'ZZ EI BT /F1 12 Tf 72 690 Td (Decode Name Operand WordPress Noise) Tj ET rawtail';
...
$payloadExcluded = $lines === ['Before Decode Name Operand Inline', 'After Decode Name Operand Inline']
    && !str_contains($plainText, 'Decode Name Operand WordPress Noise')
    && !str_contains($plainText, 'rawtail')
    && !str_contains($plainText, 'ZZ EI');
$decodeNameOperandPreserved = ($review['inline_image']['canonical_dictionary'] ?? '') ===
    '<< /Width 1 /Height 1 /ColorSpace /DeviceGray /BitsPerComponent 8 /Decode [0 /Fl] /Filter /FlateDecode >>';
$filterAbbreviationExpanded = ($review['image_filters'] ?? []) === ['FlateDecode'];
...
if (
    !$payloadExcluded
    || !$decodeNameOperandPreserved
    || !$filterAbbreviationExpanded
    || !$decodeReviewOnly
    || !$previewFailedClosed
) {
    throw new RuntimeException('Inline Decode name operand boundary did not fail closed before WordPress import.');
}

$metadata = [
    'source' => 'native-pdf-inline-image-decode-name-operand-boundary-currentbase',
    ...
    'decode_name_operand_preserved' => $decodeNameOperandPreserved,
    'filter_abbreviation_expanded' => $filterAbbreviationExpanded,
...
echo '<!-- markerpdf:pdf-inline-image-decode-name-operand-boundary-currentbase '
