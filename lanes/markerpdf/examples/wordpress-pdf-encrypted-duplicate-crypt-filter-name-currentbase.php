...
$content = 'BT /F1 12 Tf 72 720 Td (WordPress duplicate decoded crypt filter name encrypted text leak) Tj ET';
...
if (($permission['content_extraction_boundary'] ?? null) !== 'blocked_by_duplicate_document_crypt_filter_name') {
    throw new RuntimeException('Expected duplicate decoded crypt-filter names to fail closed before WordPress import.');
}
if (($dictionaryReview['duplicate_filter_names'] ?? []) !== ['StdCF']) {
    throw new RuntimeException('Expected escaped duplicate /Std#43F to be reviewed as duplicate decoded StdCF.');
}
if (
    !is_string($encoded)
...
echo '<!-- markerpdf-encrypted-duplicate-crypt-filter-name-currentbase-smoke ' . htmlspecialchars(json_encode([
    'scenario' => 'wordpress-pdf-encrypted-duplicate-crypt-filter-name-currentbase',
    'native_boundary' => 'Standard encrypted duplicate decoded crypt-filter names fail closed before WordPress import even when an escaped alias selects Identity',
    ...
    'dictionary_duplicate_filter_names' => $dictionaryReview['duplicate_filter_names'] ?? [],
...
echo "<h2>Encrypted PDF Duplicate Crypt Filter Name Preflight</h2>\n";
echo "<p>Encrypted PDF text remains blocked. Duplicate decoded crypt-filter names are preserved as ambiguous security metadata and do not authorize native text import.</p>\n";
echo '<!-- markerpdf:encrypted-duplicate-crypt-filter-name-preflight ' . htmlspecialchars(json_encode([
    'permission' => [
    ...
    'crypt_filter_review' => [
        ...
        'crypt_filter_dictionary_duplicate_filter_names' => $review['crypt_filter_dictionary_duplicate_filter_names'] ?? [],
        'fail_closed_role_names' => $review['fail_closed_role_names'] ?? [],
        'fail_closed_filter_names' => $review['fail_closed_filter_names'] ?? [],
    ],
    'dictionary_declaration_review' => [
        ...
        'duplicate_filter_name_entries' => $dictionaryReview['duplicate_filter_name_entries'] ?? null,
        'duplicate_filter_names' => $dictionaryReview['duplicate_filter_names'] ?? [],
