...
if (!is_string($objectStream)) {
    throw new RuntimeException('Unable to compress literal-offset object-stream smoke fixture.');
}
...
if (!is_string($compressedXref)) {
    throw new RuntimeException('Unable to compress literal-offset xref-stream smoke fixture.');
}
...
echo '<!-- markerpdf-xref-object-stream-literal-offset-boundary-currentbase-smoke ' . htmlspecialchars(json_encode([
    'executes_python_or_models' => false,
    'executes_external_pdf_tools' => false,
    'native_boundary' => 'PDF xref-stream type-2 object-stream offsets must start on token boundaries, not inside compressed literal strings',
    'uses_current_literal_offset_guard' => str_contains($plainText, 'Current literal-offset guard page'),
    'excluded_literal_offset_compressed_leak' => !str_contains($plainText, 'Literal-offset compressed leak'),
    'excluded_fake_page_inside_string' => !str_contains($plainText, 'fake page inside string'),
