...
$content = 'BT /F1 12 Tf 72 720 Td (WordPress scalar Encrypt operand text leak) Tj ET';
...
$pdf .= "xref\n0 1\n"
    . $xrefRow(0, 65535, 'f')
    . "trailer\n<< /Size 100 /Root 1 0 R /Encrypt false /Prev {$previousXrefOffset} >>\n"
    . "startxref\n{$currentXrefOffset}\n%%EOF";
...
if ($plainText !== '') {
    throw new RuntimeException('Expected scalar /Encrypt operand content to stay blocked.');
}
if (($report['encrypted'] ?? null) !== true || ($report['import_decision'] ?? null) !== 'block_encrypted_content_review_security_metadata') {
    throw new RuntimeException('Expected scalar non-null /Encrypt operand to fail closed as encrypted.');
}
if (!in_array('encrypt_dictionary_non_dictionary_operand', $report['review_reasons'] ?? [], true)) {
    throw new RuntimeException('Expected scalar /Encrypt operand reason to be visible to WordPress review.');
}
if (($encryption['encrypt_operand_status'] ?? null) !== 'encrypt_dictionary_non_dictionary_operand') {
    throw new RuntimeException('Expected current scalar /Encrypt operand to be classified as non-dictionary.');
}
if (($permission['policy'] ?? null) !== 'permissions_unknown_blocked_without_decryption') {
    throw new RuntimeException('Expected scalar /Encrypt operand permissions to remain unknown and blocked.');
}
if (
    !is_string($encoded)
    ...
    throw new RuntimeException('Expected stale encrypted payload and stale Standard permission grant to stay suppressed.');
}

echo '<!-- markerpdf-encrypted-scalar-encrypt-operand-currentbase-smoke ' . htmlspecialchars(json_encode([
    'scenario' => 'wordpress-pdf-encrypted-scalar-encrypt-operand-currentbase',
    'native_boundary' => 'latest trailer scalar /Encrypt fails closed before stale Prev permission grants',
...
echo "<!-- wp:paragraph -->\n";
echo "<p>Encrypted PDF content remains blocked when the latest trailer declares a scalar non-dictionary encryption operand. WordPress import records sanitized security metadata and does not reuse stale previous permission grants.</p>\n";
echo "<!-- /wp:paragraph -->\n";
