...
if ($labels !== ['Obj-4', 'Tail-'] || $previewLabels !== $labels) {
    throw new RuntimeException('Expected object-stream PageLabels to align import and preview metadata.');
}

if (in_array('1', $previewLabels, true) || in_array('2', $previewLabels, true) || ($imagePlan['page_label'] ?? null) !== 'Tail-') {
    throw new RuntimeException('Expected preview page labels to avoid numeric fallback for compressed PageLabels.');
}

echo '<!-- markerpdf-page-labels-object-stream-smoke ' . htmlspecialchars(json_encode([
    'executes_python_or_models' => false,
    'executes_external_pdf_tools' => false,
    'native_boundary' => 'catalog /PageLabels value dictionary stored as a PDF 1.5 object-stream member stays aligned across import and preview metadata',
    'page_labels' => $labels,
    'preview_page_labels' => $previewLabels,
    'selected_preview_page_label' => $imagePlan['page_label'] ?? null,
    'numeric_preview_fallback_rejected' => !in_array('1', $previewLabels, true)
