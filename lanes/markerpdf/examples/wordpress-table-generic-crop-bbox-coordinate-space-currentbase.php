...
$recognizedTable = [
    ...
    'cells' => [
        ...
        ['bbox' => $normPage([82.0, 250.0, 162.0, 268.0]), 'text' => 'Stale generic row', 'row_ids' => [99], 'col_ids' => [0]],
        ['bbox' => $normPage([360.0, 195.0, 382.0, 215.0]), 'text' => 'Stale generic col', 'row_ids' => [1], 'col_ids' => [99]],
    ],
];
...
$pdfPath = sys_get_temp_dir() . '/markerpdf-wordpress-table-generic-crop-bbox-coordinate-space-' . bin2hex(random_bytes(4)) . '.pdf';
file_put_contents($pdfPath, "%PDF-1.4\n% generic crop bbox coordinate-space WordPress fixture\n%%EOF");
...
$genericCropLocalized = ($directReview['table_bbox_source_coordinate_space'] ?? null) === 'normalized_page_image'
...
$staleExcluded = !in_array('Stale generic row', $assignedTexts, true)
    && !in_array('Stale generic col', $assignedTexts, true)
    && !str_contains($result['text'], 'Stale generic-coordinate table line should be replaced.');

if (!$genericCropLocalized || !$wordpressTableRendered || !$staleExcluded) {
    throw new RuntimeException('Expected generic normalized page-image crop bbox to localize before WordPress table output.');
}

echo json_encode([
    'scenario' => 'wordpress-table-generic-crop-bbox-coordinate-space-currentbase',
    'native_boundary' => 'generic normalized_page_image coordinate_space applies to the top-level table bbox before rows, columns, cells, and OCR conflict geometry are translated to table-crop coordinates',
    'source_truth' => [
        'marker_pdf' => 'marker/tables/table.py crops high-resolution table bboxes before tabled assignment and Markdown formatting',
        'tabled' => 'tabled results.json carries cells, rows, cols, image_bbox, and bbox table geometry used for saved table review',
    ...
    'gutenberg_blocks' => [
        ['blockName' => 'core/heading', 'innerHTML' => '<h1>Generic Coordinate Space Table Boundary</h1>'],
        ...
        ['blockName' => 'core/paragraph', 'innerHTML' => '<p>After generic coordinate-space table.</p>'],
    ],
    ...
    'generic_crop_bbox_localized' => $genericCropLocalized,
    'wordpress_table_rendered' => $wordpressTableRendered,
    'stale_generic_cells_filtered' => $staleExcluded,
