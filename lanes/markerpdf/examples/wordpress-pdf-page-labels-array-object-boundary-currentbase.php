...
if ($labels !== ['1', 'Body 4', 'App-Z'] || $previewLabels !== $labels) {
    throw new RuntimeException('Expected malformed PageLabels array object tails to be ignored across import and preview metadata.');
}

if (in_array('BadArray-99', $labels, true) || in_array('BadArray-99', $previewLabels, true)) {
    throw new RuntimeException('Expected malformed PageLabels array object label to stay excluded.');
}

echo '<!-- markerpdf-page-labels-array-object-boundary ' . htmlspecialchars(json_encode([
    'executes_python_or_models' => false,
    'executes_external_pdf_tools' => false,
    'native_boundary' => 'catalog /PageLabels indirect /Nums arrays must be single array objects; comment-only tails remain PDF whitespace',
    'page_labels' => $labels,
    'preview_page_labels' => $previewLabels,
    'selected_preview_page_label' => $imagePlan['page_label'] ?? null,
    'malformed_array_object_rejected' => !in_array('BadArray-99', $labels, true)
        && !in_array('BadArray-99', $previewLabels, true),
