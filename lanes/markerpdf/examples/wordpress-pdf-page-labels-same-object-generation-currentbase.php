...
if ($labels !== ['Front iv', 'Body 8', 'Body 9']) {
    throw new RuntimeException('Expected same-object PageLabels generation chain in native text extraction.');
}

if ($previewLabels !== $labels || ($imagePlan['page_label'] ?? null) !== 'Body 9') {
    throw new RuntimeException('Expected MarkerAppPreview labels to match same-object generation-chain extraction.');
}

echo '<!-- markerpdf-page-labels-same-object-generation-smoke ' . htmlspecialchars(json_encode([
    'executes_python_or_models' => false,
    'executes_external_pdf_tools' => false,
    'native_boundary' => 'catalog /PageLabels exact-generation references may chain across same object number generations before preview metadata fallback',
    'page_labels' => $labels,
    'preview_page_labels' => $previewLabels,
    'selected_preview_page_label' => $imagePlan['page_label'] ?? null,
    'same_object_generation_chain_imported' => $previewLabels[0] === 'Front iv',
    'numeric_fallback_rejected' => !in_array('1', $previewLabels, true),
    'stale_sibling_generation_excluded' => !in_array('stale-sibling-generation-99', $previewLabels, true),
