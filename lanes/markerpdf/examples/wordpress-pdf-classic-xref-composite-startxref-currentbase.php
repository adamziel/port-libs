...
$decoyContent = 'BT /F1 12 Tf 72 720 Td (Composite startxref decoy page) Tj T* (Composite startxref leak) Tj ET';
...
$decoyPayload = '<wp-export><post id="decoy-composite-startxref"/></wp-export>';
...
$compositeXrefOffset = strlen($pdf) + strlen("[ /Composite ");
$pdf .= "[ /Composite xref\n"
    . "20 12\n";
...
$summary = [
    'source' => 'native-pdf-classic-xref-composite-startxref-boundary',
    'native_boundary' => 'Classic xref rebuild ignores startxref tokens inside top-level PDF composite tokens before WordPress import',
    'current_xref_offset' => $currentXrefOffset,
    'composite_decoy_xref_offset' => $compositeXrefOffset,
    'uses_current_page_text' => str_contains($plainText, 'Current composite startxref page'),
    'skips_composite_startxref_token' => !str_contains($plainText, 'Composite startxref decoy page'),
    'metadata_title_current' => ($metadata['title'] ?? null) === 'Current Composite Startxref Title',
    'embedded_file_current' => ($files[0]['name'] ?? null) === 'current-composite-startxref.xml',
    'excludes_decoy_metadata' => !str_contains($encodedMetadata, 'Composite Startxref Decoy'),
    'excludes_decoy_attachment' => !str_contains($encodedFiles, 'decoy-composite-startxref'),
...
if (
    ...
    || !$summary['skips_composite_startxref_token']
    || !$summary['metadata_title_current']
    || !$summary['embedded_file_current']
) {
    throw new RuntimeException('Expected composite-contained startxref token to be ignored during classic xref rebuild.');
}

echo '<!-- markerpdf-pdf-classic-xref-composite-startxref-currentbase ' . htmlspecialchars(json_encode($summary, JSON_UNESCAPED_SLASHES), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . " -->\n";
