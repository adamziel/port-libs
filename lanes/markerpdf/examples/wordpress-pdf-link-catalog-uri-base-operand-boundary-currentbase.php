...
$summary = [
    'support_component' => 'native-pdf-link-catalog-uri-base-operand-boundary',
    'native_boundary' => 'malformed Catalog /URI /Base operands are ignored before relative Link annotation href resolution',
    ...
    'fragment_href' => $promotedLinks[2]['uri'] ?? null,
    'fragment_resolved_from_base' => $promotedLinks[2]['uri_resolved_from_base'] ?? null,
    'fragment_uri_base_present' => array_key_exists('uri_base', $promotedLinks[2] ?? []),
    'absolute_href' => $promotedLinks[1]['uri'] ?? null,
    ...
    'tainted_catalog_base_promoted' => str_contains($encodedPromotedReview, 'docs.example.com')
        || str_contains($encodedPromotedReview, 'taintedCatalogBaseReview'),
    'unsafe_uri_promoted' => str_contains($encodedPromotedReview, 'javascript:taintedBase'),
    'visible_text_imported' => str_contains($visibleText, 'Relative docs Absolute docs Fragment docs Unsafe docs'),
    'visible_text_excludes_link_metadata' => !str_contains($visibleText, 'articles/import.html')
        ...
        && !str_contains($visibleText, 'taintedCatalogBaseReview')
        && !str_contains($visibleText, 'Relative tainted-base review')
        && !str_contains($visibleText, 'Absolute review')
        && !str_contains($visibleText, 'Fragment tainted-base review'),
...
if (
    ...
    || $summary['tainted_catalog_base_promoted'] !== false
    || $summary['unsafe_uri_promoted'] !== false
    || $summary['visible_text_excludes_link_metadata'] !== true
) {
    throw new RuntimeException('Unexpected markerPDF catalog URI Base operand boundary smoke output.');
}

echo '<!-- markerpdf-pdf-link-catalog-uri-base-operand-boundary-currentbase ' . htmlspecialchars(json_encode($summary, JSON_UNESCAPED_SLASHES), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . " -->\n";
