...
$staleXmp = $xmpPacket('Stale Malformed Hex XRef Title');
$currentXmp = $xmpPacket('Current Malformed Hex XRef Title');
...
echo '<!-- markerpdf-xref-classic-malformed-hex-boundary-currentbase-smoke ' . htmlspecialchars(json_encode([
    'native_boundary' => 'classic xref rebuild recovers after malformed non-hex string openers',
    'previous_xref_offset' => $previousXrefOffset,
    'malformed_hex_opener_offset' => $malformedHexOpenerOffset,
    'current_xref_offset' => $currentXrefOffset,
    'uses_current_page_text' => str_contains($plainText, 'Current malformed-hex xref page'),
    'metadata_title_current' => ($metadata['title'] ?? null) === 'Current Malformed Hex XRef Title',
    'embedded_file_current' => ($files[0]['name'] ?? null) === 'current-malformed-hex-xref.xml',
    'attachment_summary_current' => ($attachmentSummary['filenames'][0] ?? null) === 'current-malformed-hex-xref.xml',
    'rejects_stale_table' => !str_contains($plainText, 'Stale malformed-hex xref page')
        && !str_contains($encodedMetadata, 'Stale Malformed Hex')
        && !str_contains($encodedFiles, 'stale-malformed-hex-xref'),
