...
if (!is_string($objectStream)) {
    throw new RuntimeException('Unable to compress malformed xref-width smoke object stream.');
}
...
if (!is_string($compressedXref)) {
    throw new RuntimeException('Unable to compress malformed xref-width smoke xref stream.');
}
...
echo '<!-- markerpdf-xref-stream-malformed-width-currentbase-smoke ' . htmlspecialchars(json_encode([
    'executes_python_or_models' => false,
    'executes_external_pdf_tools' => false,
    'native_boundary' => 'negative xref-stream /W byte widths are rejected before object-stream and direct fallback text import',
    ...
    'bogus_free_entries_excluded' => $review['xref_stream_free_entry_count'] === 0,
    'visible_text_empty' => $plainText === '' && $lines === [],
    'stale_object_stream_text_excluded' => !str_contains($plainText, 'Object stream malformed W leak'),
    'stale_direct_text_excluded' => !str_contains($plainText, 'Stale malformed W fallback leak'),
