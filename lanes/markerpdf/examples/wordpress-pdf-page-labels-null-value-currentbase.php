...
$leakedLabels = ['Front v', 'Body 9'];

if ($labels !== ['Front iv', '2', 'Body 8', '4'] || $previewLabels !== $labels || $summaryLabels !== $labels) {
    throw new RuntimeException('Expected PageLabels null values to reset to physical page labels across import and preview metadata.');
}

if (count(array_intersect($leakedLabels, $labels, $previewLabels, $summaryLabels)) !== 0 || ($imagePlan['page_label'] ?? null) !== '4') {
    throw new RuntimeException('Expected previous PageLabels ranges to stop at direct and indirect null values.');
}

echo '<!-- markerpdf-page-labels-null-value-smoke ' . htmlspecialchars(json_encode([
    'executes_python_or_models' => false,
    'executes_external_pdf_tools' => false,
    'native_boundary' => 'catalog /PageLabels null values reset labels to physical page numbers before WordPress page-break metadata',
    'page_labels' => $labels,
    'preview_page_labels' => $previewLabels,
    'summary_page_labels' => $summaryLabels,
    'selected_preview_page_label' => $imagePlan['page_label'] ?? null,
    'direct_null_resets_to_physical_label' => $labels[1] === '2',
    'indirect_null_resets_to_physical_label' => $labels[3] === '4',
    'previous_label_ranges_stopped' => count(array_intersect($leakedLabels, $labels, $previewLabels, $summaryLabels)) === 0,
