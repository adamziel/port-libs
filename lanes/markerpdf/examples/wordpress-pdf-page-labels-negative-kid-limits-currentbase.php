...
$staleLabels = ['stale-underflow-77', 'stale-underflow-78', 'stale-app-Z'];

if ($labels !== $expected || $previewLabels !== $expected) {
    throw new RuntimeException('Expected negative PageLabels kid Limits to be rejected before WordPress page-break metadata.');
}

$staleExcluded = count(array_intersect($staleLabels, $labels, $previewLabels)) === 0;
if (!$staleExcluded || ($imagePlan['page_label'] ?? null) !== 'End-') {
    throw new RuntimeException('Expected stale PageLabels underflow range to stay excluded from preview metadata.');
}

echo '<!-- markerpdf-page-labels-negative-kid-limits-smoke ' . htmlspecialchars(json_encode([
    'executes_python_or_models' => false,
    'executes_external_pdf_tools' => false,
    'native_boundary' => 'catalog /PageLabels /Kids with negative /Limits cannot claim physical page indexes before a later valid child',
    'page_labels' => $labels,
    'preview_page_labels' => $previewLabels,
    'selected_preview_page_label' => $imagePlan['page_label'] ?? null,
    'negative_kid_limits_rejected' => !in_array('stale-underflow-77', $labels, true)
        && !in_array('stale-underflow-77', $previewLabels, true),
    'later_valid_kid_preserved' => ($labels[1] ?? null) === 'Front v',
