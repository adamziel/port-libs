...
$content = 'BT /F1 12 Tf 72 720 Td (WordPress duplicate crypt-filter parameter encrypted text leak) Tj ET';
...
if (($permission['content_extraction_boundary'] ?? null) !== 'blocked_by_duplicate_document_crypt_filter_parameters') {
    throw new RuntimeException('Expected duplicate crypt-filter parameters to fail closed before WordPress import.');
}
if (($parameterReview['duplicate_parameter_names'] ?? []) !== ['CFM']) {
    throw new RuntimeException('Expected duplicate /CFM declarations to be preserved as review metadata.');
}
if (
    !is_string($encoded)
...
echo '<!-- markerpdf-encrypted-duplicate-crypt-filter-parameter-currentbase-smoke ' . htmlspecialchars(json_encode([
    'scenario' => 'wordpress-pdf-encrypted-duplicate-crypt-filter-parameter-currentbase',
    'native_boundary' => 'Standard encrypted duplicate crypt-filter CFM declarations fail closed before WordPress import without decrypting payloads',
...
echo "<h2>Encrypted PDF Duplicate Crypt Filter Parameter Preflight</h2>\n";
echo "<p>Encrypted PDF text remains blocked. Duplicate /CFM declarations inside the selected crypt filter are preserved as security review metadata and do not authorize native text import.</p>\n";
echo '<!-- markerpdf:encrypted-duplicate-crypt-filter-parameter-preflight ' . htmlspecialchars(json_encode([
    'permission' => [
    ...
    'crypt_filter_review' => [
        ...
        'fail_closed_filter_names' => $review['fail_closed_filter_names'] ?? [],
        'crypt_filter_parameter_statuses' => $review['crypt_filter_parameter_statuses'] ?? [],
        'crypt_filter_parameter_duplicate_names' => $review['crypt_filter_parameter_duplicate_names'] ?? [],
    ],
