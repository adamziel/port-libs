...
if ($labels !== ['1', 'Body 2'] || $previewLabels !== $labels) {
    throw new RuntimeException('Expected missing-generation PageLabels references to fall back before preview metadata.');
}

if (in_array('stale-missing-generation-99', $labels, true) || in_array('stale-missing-generation-99', $previewLabels, true)) {
    throw new RuntimeException('Expected stale same-object-number PageLabels generation to stay excluded.');
}

echo '<!-- markerpdf-page-labels-missing-generation-smoke ' . htmlspecialchars(json_encode([
    'executes_python_or_models' => false,
    'executes_external_pdf_tools' => false,
    'native_boundary' => 'catalog /PageLabels missing-generation references do not bind stale same-object-number dictionaries',
    'page_labels' => $labels,
    'preview_page_labels' => $previewLabels,
    'selected_preview_page_label' => $imagePlan['page_label'] ?? null,
    'missing_generation_fallback_preserved' => $labels[0] === '1',
    'stale_same_object_generation_rejected' => !in_array('stale-missing-generation-99', $labels, true)
        && !in_array('stale-missing-generation-99', $previewLabels, true),
