...
if ($labels !== ['Leaf-4', 'Leaf-5', 'Leaf-6'] || $previewLabels !== $labels) {
    throw new RuntimeException('Expected PageLabels /Nums leaf to exclude stale same-node /Kids rows.');
}

if (in_array('stale-kid-99', $labels, true) || in_array('stale-kid-99', $previewLabels, true)) {
    throw new RuntimeException('Expected stale PageLabels kid branch to stay out of WordPress page-break metadata.');
}

echo '<!-- markerpdf-page-labels-nums-leaf-kids-smoke ' . htmlspecialchars(json_encode([
    'executes_python_or_models' => false,
    'executes_external_pdf_tools' => false,
    'native_boundary' => 'catalog /PageLabels nodes with usable unbounded /Nums are leaves before stale sibling /Kids rows',
    'page_labels' => $labels,
    'preview_page_labels' => $previewLabels,
    'selected_preview_page_label' => $imagePlan['page_label'] ?? null,
    'nums_leaf_kids_excluded' => !in_array('stale-kid-99', $labels, true)
        && !in_array('stale-kid-99', $previewLabels, true),
