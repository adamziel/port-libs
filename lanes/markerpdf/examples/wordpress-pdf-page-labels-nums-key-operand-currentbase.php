...
if ($labels !== ['Cover-', 'Body 4', 'App-Z'] || $previewLabels !== $labels) {
    throw new RuntimeException('Expected malformed PageLabels Nums key operands to consume their paired value before resync.');
}

if (in_array('Shifted-9', $labels, true) || in_array('Shifted-9', $previewLabels, true)) {
    throw new RuntimeException('Expected shifted PageLabels dictionary to stay excluded from page-break metadata.');
}

echo '<!-- markerpdf-page-labels-nums-key-operand-boundary ' . htmlspecialchars(json_encode([
    'executes_python_or_models' => false,
    'executes_external_pdf_tools' => false,
    'native_boundary' => 'catalog /PageLabels /Nums malformed key-like operands consume their paired value before WordPress page-label resync',
    'page_labels' => $labels,
    'preview_page_labels' => $previewLabels,
    'selected_preview_page_label' => $imagePlan['page_label'] ?? null,
    'shifted_label_rejected' => !in_array('Shifted-9', $labels, true)
        && !in_array('Shifted-9', $previewLabels, true),
