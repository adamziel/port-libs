...
$staleContent = 'BT /F1 12 Tf 72 720 Td (Stale unsupported Root smoke page) Tj ET';
$staleXmp = '<?xpacket begin="" id="W5M0MpCehiHzreSzNTczkc9d"?>'
    . '<x:xmpmeta xmlns:x="adobe:ns:meta/">'
    . '<rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#">'
    . '<rdf:Description rdf:about="" xmlns:dc="http://purl.org/dc/elements/1.1/">'
    . '<dc:title><rdf:Alt><rdf:li xml:lang="x-default">Stale Unsupported Root Smoke XMP</rdf:li></rdf:Alt></dc:title>'
...
$staleInfoOffset = $addObject(6, 0, '<< /Title (Stale Unsupported Root Smoke Info) /Author (Stale Unsupported Root Smoke Author) /Producer (Stale Unsupported Root Smoke Producer) >>');
$staleMetadataOffset = $addObject(7, 0, '<< /Type /Metadata /Subtype /XML /Length ' . strlen($staleXmp) . " >>\nstream\n{$staleXmp}\nendstream");
$staleNameTreeOffset = $addObject(8, 0, '<< /Names [(stale-unsupported-root-smoke.xml) 10 0 R] >>');
$staleFileSpecOffset = $addObject(10, 0, '<< /Type /Filespec /F (stale-unsupported-root-smoke.xml) /Desc (Stale unsupported Root smoke attachment) /AFRelationship /Source /EF << /F 11 0 R >> >>');
$stalePayload = '<wp-export><post id="stale-unsupported-root-smoke"/></wp-export>';
...
$currentInfoOffset = $addObject(6, 0, '<< /Title (Current Unsupported Root Smoke Info) /Author (Current Unsupported Root Smoke Author) /Producer (Current Unsupported Root Smoke Producer) >>');
...
if (!is_string($compressedCurrentRows)) {
    throw new RuntimeException('Unable to compress unsupported-Root-row xref smoke rows.');
}
...
$smoke = [
    'unsupported_current_root_row_stops_prev_catalog' => $lines === []
        && !isset($metadata['catalog'])
        && !isset($metadata['language']),
    'current_info_selected' => ($metadata['title'] ?? null) === 'Current Unsupported Root Smoke Info'
        && ($metadata['info']['Title'] ?? null) === 'Current Unsupported Root Smoke Info',
    'stale_prev_text_excluded' => !in_array('Stale unsupported Root smoke page', $lines, true),
    'stale_prev_metadata_excluded' => is_string($encodedMetadata)
        && !str_contains($encodedMetadata, 'Stale Unsupported Root Smoke')
        && !str_contains($encodedMetadata, 'stale-unsupported-root-smoke'),
    'stale_prev_embedded_files_excluded' => $embeddedFiles === []
        && is_string($encodedEmbeddedFiles)
        && !str_contains($encodedEmbeddedFiles, 'stale-unsupported-root-smoke'),
    'attachment_summary_empty' => ($attachmentSummary['attachment_count'] ?? null) === 0
        ...
        && !str_contains($encodedAttachmentSummary, 'stale-unsupported-root-smoke'),
    'executes_python_or_models' => false,
    'executes_external_pdf_tools' => false,
];
$requiredTrue = $smoke;
unset($requiredTrue['executes_python_or_models'], $requiredTrue['executes_external_pdf_tools']);

if (in_array(false, $requiredTrue, true)) {
    throw new RuntimeException('Unsupported current xref-stream Root row leaked stale Prev catalog data.');
}

echo '<!-- markerpdf-xref-prev-chain-unsupported-root-row-currentbase '
