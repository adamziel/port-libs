...
if ($labels !== ['4', 'Valid-8'] || $previewLabels !== $labels) {
    throw new RuntimeException('Expected PageLabels name prefix scalar to be ignored across import and preview metadata.');
}

if (in_array('NamePrefix4', $labels, true) || in_array('NamePrefix4', $previewLabels, true)) {
    throw new RuntimeException('Expected bare-name PageLabels prefix text to stay excluded.');
}

echo '<!-- markerpdf-page-labels-prefix-name-boundary ' . htmlspecialchars(json_encode([
    'executes_python_or_models' => false,
    'executes_external_pdf_tools' => false,
    'native_boundary' => 'catalog /PageLabels /P prefixes must be PDF text strings, not bare name scalars',
    'page_labels' => $labels,
    'preview_page_labels' => $previewLabels,
    'selected_preview_page_label' => $imagePlan['page_label'] ?? null,
    'name_prefix_scalar_rejected' => !in_array('NamePrefix4', $labels, true)
        && !in_array('NamePrefix4', $previewLabels, true),
