...
if ($labels !== $expected || $previewLabels !== $expected) {
    throw new RuntimeException('Expected malformed PageLabels /Limits dictionaries to be ignored before WordPress page metadata.');
}

if (in_array('1', $labels, true) || in_array('App-AA', $previewLabels, true)) {
    throw new RuntimeException('Expected nested numeric /Limits decoys to stay out of PageLabels bounds.');
}

echo '<!-- markerpdf-page-labels-malformed-limits-smoke ' . htmlspecialchars(json_encode([
    'executes_python_or_models' => false,
    'executes_external_pdf_tools' => false,
    'native_boundary' => 'catalog /PageLabels malformed /Limits dictionary numbers are ignored',
    'page_labels' => $labels,
    'preview_page_labels' => $previewLabels,
    'selected_preview_page_label' => $imagePlan['page_label'] ?? null,
    'malformed_limits_ignored' => $labels === $expected && $previewLabels === $expected,
    'nested_numeric_limits_rejected' => !in_array('1', $labels, true) && !in_array('App-AA', $previewLabels, true),
