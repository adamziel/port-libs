...
if (!is_string($objectStream)) {
    throw new RuntimeException('Unable to compress unsupported-type object-stream smoke fixture.');
}
...
if (!is_string($previousCompressedXref)) {
    throw new RuntimeException('Unable to compress previous unsupported-type xref smoke fixture.');
}
...
if (!is_string($currentCompressedXref)) {
    throw new RuntimeException('Unable to compress current unsupported-type xref smoke fixture.');
}
...
if (str_contains($plainText, 'Stale unsupported-type compressed page')) {
    throw new RuntimeException('Unsupported xref-stream row allowed a stale object-stream member to leak.');
}

if (($review['xref_stream_free_owner_count'] ?? 0) !== 1) {
    throw new RuntimeException('Expected unsupported xref-stream row to be reported as a current null owner.');
}

echo '<!-- markerpdf-xref-stream-unsupported-type-object-stream-currentbase-smoke ' . htmlspecialchars(json_encode([
    'executes_python_or_models' => false,
    'executes_external_pdf_tools' => false,
    'native_boundary' => 'unsupported xref-stream entry type suppresses stale previous object-stream page members before WordPress import',
    'uses_current_guard_page' => str_contains($plainText, 'Current unsupported-type guard page'),
    'reports_current_null_owner' => ($review['xref_stream_free_owner_count'] ?? 0) === 1,
    ...
    'previous_entry_type' => $freeEntry['previous_entry_type'] ?? null,
    'previous_object_stream' => $freeEntry['previous_object_stream'] ?? null,
    'previous_member_index' => $freeEntry['previous_member_index'] ?? null,
    'excluded_stale_compressed_page' => !str_contains($plainText, 'Stale unsupported-type compressed page'),
