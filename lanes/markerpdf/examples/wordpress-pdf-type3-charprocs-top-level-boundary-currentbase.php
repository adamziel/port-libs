...
echo '<!-- markerpdf:pdf-type3-charprocs-top-level-boundary-currentbase ' . htmlspecialchars(json_encode([
    'source' => 'native-pdf-type3-charprocs-top-level-boundary',
    'top_level_charprocs_widths_preserved' => str_contains($plainText, 'WideBlock') && !str_contains($plainText, 'Wide Block'),
    'thin_width_spacing_preserved' => str_contains($plainText, 'Thin Text') && !str_contains($plainText, 'ThinText'),
    'nested_resource_charprocs_ignored' => !str_contains($plainText, 'nested resource charproc decoy leak'),
    'charproc_payload_visible_text_excluded' => !str_contains($plainText, 'top-level charproc text leak'),
