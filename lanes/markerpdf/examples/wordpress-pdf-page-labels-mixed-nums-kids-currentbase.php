...
if ($labels !== ['Cover-', 'Body 8', 'App-Z', 'End-'] || $previewLabels !== $labels) {
    throw new RuntimeException('Expected PageLabels /Kids to stay active after direct /Nums on a malformed intermediate node.');
}

if (in_array('stale-root-99', $labels, true) || in_array('stale-root-99', $previewLabels, true)) {
    throw new RuntimeException('Expected out-of-range direct PageLabels /Nums decoy to stay excluded.');
}

echo '<!-- markerpdf-page-labels-mixed-nums-kids-smoke ' . htmlspecialchars(json_encode([
    'executes_python_or_models' => false,
    'executes_external_pdf_tools' => false,
    'native_boundary' => 'catalog /PageLabels intermediate node merges valid direct /Nums sections before bounded /Kids sections',
    'page_labels' => $labels,
    'preview_page_labels' => $previewLabels,
    'selected_preview_page_label' => $imagePlan['page_label'] ?? null,
    'kids_after_nums_preserved' => $labels[1] === 'Body 8' && $labels[2] === 'App-Z' && $labels[3] === 'End-',
    'stale_root_nums_rejected' => !in_array('stale-root-99', $labels, true)
        && !in_array('stale-root-99', $previewLabels, true),
