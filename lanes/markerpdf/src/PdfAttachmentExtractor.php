...
final class PdfAttachmentExtractor
{
    ...
    private function nameTreeKidsSortedByLimits(array $kids, array $objects, ?array $inheritedLimits): array
    {
        ...
        foreach ($kids as $order => $kid) {
            $child = $this->dict($this->resolveValue($kid, $objects));
            if ($child === null) {
                return $kids;
            }
    ...
    private function pdfObjectEndOffset(string $pdfBytes, int $offset): ?int
    {
        ...
        while ($offset < $length) {
            ...
            if ($char === '<' && ($pdfBytes[$offset + 1] ?? '') !== '<') {
                $end = $this->skipPdfHexStringBoundary($pdfBytes, $offset);
                if ($end !== null) {
                    $offset = $end;
                    continue;
                }
    ...
    private function repairOmittedCurrentUpdateGraphRows(
        ...
        if (
            array_key_exists('XRefStm', $sectionDictionary)
            && $this->refObjectReference($sectionDictionary['Prev'] ?? null) !== null
        ) {
            return $entries;
        }
        ...
        foreach (['Root', 'Info', 'Encrypt'] as $name) {
            $hasValue = array_key_exists($name, $sectionDictionary);
        ...
        while ($pending !== [] && count($seen) < 128) {
            ...
            $valueOffset = 0;
            $value = $this->parseValue(trim($compressedMember['body']), $valueOffset);
            foreach ($this->objectReferencesInParsedValue($value) as $nestedReference) {
                $pending[] = [
                    'reference' => $nestedReference,
                    'source' => 'compressed',
    ...
    private function inheritedTrailerGraphReference(
        ...
        $table = $this->xrefTableSectionAt($pdfBytes, $offset, $definitions);
        if ($table !== null) {
            if (array_key_exists($name, $table['trailer'])) {
                return $this->refObjectReference($table['trailer'][$name]);
            }
            ...
            return $previousOffset === null
                ? null
                : $this->inheritedTrailerGraphReference($pdfBytes, $previousOffset, $name, $definitions, $seenOffsets);
        }
        ...
        if (array_key_exists($name, $stream['dictionary'])) {
            return $this->refObjectReference($stream['dictionary'][$name]);
        }
    ...
    private function currentUpdateObjectStreamMemberEntryForGraphReference(
        ...
        foreach ($entries as $carrierObjectNumber => $carrierEntry) {
            ...
            $objects = [(int) $carrierObjectNumber => $this->parsedObjectFromDefinition($carrierDefinition)];
            $objectStream = $objects[(int) $carrierObjectNumber];
            $dict = $this->dict($objectStream['value']);
            if ($dict === null || $this->nameValue($this->resolveValue($dict['Type'] ?? null, $objects)) !== 'ObjStm') {
                continue;
            }
            ...
            $members = $this->objectStreamHeaderMembers(substr($decoded, 0, $firstOffset), $declaredCount);
            if ($members === [] || ($this->objectStreamMemberObjectNumberCounts($members)[$objectNumber] ?? 0) !== 1) {
                continue;
            }
            ...
            if (($this->objectStreamMemberOffsetCounts($members)[$member['offset']] ?? 0) > 1) {
                continue;
            }
            ...
            $valueOffset = 0;
            $value = $this->parseValue(trim($body), $valueOffset);
            if ($value === null || $this->streamBytesFromBody($body, $valueOffset, $value) !== null) {
                continue;
            }
    ...
    private function previousCompressedEntryUsesUpdatedObjectStream(
        ...
        if ($currentObjectStreamEntry === null) {
            $previousDefinition = $this->xrefEntrySelectedDirectDefinition(
                $objectStreamNumber,
    ...
    private function xrefEntrySelectedDirectDefinition(int $objectNumber, array $entry, array $definitions): ?array
    {
        ...
        return $this->selectedDirectObjectDefinition($definitions[$objectNumber] ?? [], $entry);
    }
    ...
    private function xrefTableKeywordOffsets(string $pdfBytes, array $definitions): array
    {
        ...
        while ($offset < $length) {
            ...
            if (
                ...
                && !$this->xrefTableStartsAfterMalformedHexSelfStartxrefTail($pdfBytes, $definitions, $offset)
            ) {
                $offsets[] = $offset;
                $offset += strlen('xref');
                continue;
    ...
    private function skipPdfHexStringBoundary(string $pdfBytes, int $offset): ?int
    {
        $strictEnd = $this->skipPdfHexStringToken($pdfBytes, $offset);
        if ($strictEnd !== null) {
            return $strictEnd;
        }
