...
final class PdfAcroFormExtractor
{
    ...
    private function fieldReferenceAncestorContext(int $objectNumber, array $objects, array $formDefaults): array
    {
        ...
        while (isset($objects[$candidate]) && !isset($seen[$candidate])) {
            $seen[$candidate] = true;
            $body = $this->completeAcroFormDictionaryObjectBody($candidate, $objects);
            if ($body === null) {
                break;
            }

            $parentObject = $this->lastValidObjectReferenceValueAfterName($body, 'Parent', $objects);
            $parentBody = $parentObject === null ? null : $this->completeAcroFormDictionaryObjectBody($parentObject, $objects);
            if (
                $parentObject === null
            ...
            if (!$this->isFieldDictionaryCandidate($parentBody, $objects) || !$this->fieldParentOwnsChild($parentObject, $candidate, $objects)) {
                break;
            }
        ...
        foreach ($ancestors as $ancestorObject) {
            $body = $this->completeAcroFormDictionaryObjectBody($ancestorObject, $objects);
            if ($body === null) {
                continue;
            }
    ...
    private function fieldsFromObject(
        ...
        if (!$this->isFieldDictionaryCandidate($body, $objects)) {
            return [];
        }
        ...
        foreach ($kidRefs as $kidRef) {
            ...
            $kidBody = $this->completeAcroFormDictionaryObjectBody($kidRef, $objects);
            if ($kidBody === null) {
                continue;
            }

            if ($this->isPureWidget($kidBody, $objects)) {
                $widgetRefs[] = $kidRef;
                continue;
            }

            if ($this->isFieldDictionaryCandidate($kidBody, $objects)) {
    ...
    private function calculationOrderReviewEntry(
        ...
        $body = isset($objects[$objectNumber])
            ...
            : null;
        $isWidget = is_string($body) && $this->isWidget($body, $objects);
    ...
    private function widgetsForField(
        ...
        foreach ($widgetRefs as $widgetRef) {
            ...
            $body = $this->completeAcroFormDictionaryObjectBody($widgetRef, $objects);
            if ($body === null) {
                continue;
            }
    ...
    private function fieldNamesWithPageWidgetParents(array $names, array $objects, array $pageWidgets): array
    {
        foreach (array_keys($pageWidgets) as $widgetObject) {
            ...
            $body = $this->completeAcroFormDictionaryObjectBody($widgetObject, $objects);
            if ($body === null) {
                continue;
            }
    ...
    private function widgetReferencesForField(int $fieldObject, array $widgetRefs, array $objects, array $pageWidgets): array
    {
        ...
        foreach (array_keys($pageWidgets) as $widgetObject) {
            ...
            if (!$this->isWidget($body, $objects) || $this->lastValidObjectReferenceValueAfterName($body, 'Parent', $objects) !== $fieldObject) {
                continue;
            }
    ...
    /**
     * AcroForm field and widget references point at one complete dictionary
     * object. Fail closed on stray top-level operands instead of reviewing the
     * first dictionary and ignoring a malformed sibling.
     *
     * @param array<int, string> $objects
     */
    private function completeAcroFormDictionaryObjectBody(int $objectNumber, array $objects): ?string
    {
        ...
        foreach ($pageObjectNumbers as $pageObjectNumber) {
            ...
            if (str_starts_with($annotsValue, '[')) {
                $rewritten = $this->materializeDirectDictionariesInArrayValue(
                    ...
                    $objects,
                    $nextSyntheticObject,
                    fn (string $dictionary): bool => $this->isWidget($dictionary, $objects)
                );
    ...
    private function materializeDirectWidgetParentFieldDictionaries(array $objects, array $pageObjectNumbers): array
    {
        ...
        foreach ($pageObjectNumbers as $pageObjectNumber) {
            ...
            foreach ($this->annotationObjectReferences($annots, $objects) as $widgetObject) {
                if (!isset($objects[$widgetObject])) {
                    continue;
                }

                $widgetBody = $this->completeAcroFormDictionaryObjectBody($widgetObject, $objects);
                if ($widgetBody === null) {
                    continue;
                }

                if (!$this->isWidget($widgetBody, $objects) || !$this->widgetAnnotationBelongsToPage($widgetBody, $objects, $pageObjectNumber)) {
                    continue;
                }
    ...
    private function materializeDirectDictionariesInArrayBody(
        ...
        $shouldMaterialize ??= fn (string $dictionary): bool => $this->isFieldDictionaryCandidate($dictionary, $objects);
        $rewritten = '';
        $cursor = 0;
        $offset = 0;
        $length = strlen($arrayBody);
    ...
    private function fieldReferencesWithPageWidgetBoundaries(array $fieldRefs, array $objects, array $pageWidgets): array
    {
        ...
        foreach (array_keys($pageWidgets) as $widgetObject) {
            ...
            if (!$this->isWidget($widgetBody, $objects)) {
                continue;
            }
            ...
            } elseif ($this->isFieldWidgetDictionary($widgetBody, $objects)) {
            ...
            $candidateBody = $this->completeAcroFormDictionaryObjectBody($candidate, $objects);
            if ($candidateBody === null) {
                continue;
            }

            if (!$this->isFieldDictionaryCandidate($candidateBody, $objects)) {
                continue;
            }
    ...
    private function rootFieldReferencesFromAcroFormReferences(array $fieldRefs, array $objects): array
    {
        ...
        foreach ($fieldRefs as $fieldRef) {
            ...
            if (isset($objects[$fieldRef])) {
                $body = $this->completeAcroFormDictionaryObjectBody($fieldRef, $objects);
                if ($body === null) {
                    continue;
                }

                if ($this->isPureWidget($body, $objects)) {
                    $parentObject = $this->lastValidObjectReferenceValueAfterName($body, 'Parent', $objects);
    ...
    private function pageWidgetParentFieldCandidate(int $objectNumber, array $objects): ?int
    {
        ...
        return $this->isFieldDictionaryCandidate($body, $objects) ? $objectNumber : null;
    }
    ...
    {
        $parentBody = $this->completeAcroFormDictionaryObjectBody($parentObject, $objects);
        if ($parentBody === null) {
            return false;
        }
    ...
    {
        $childBody = $this->completeAcroFormDictionaryObjectBody($childObject, $objects);
        if ($childBody === null) {
            return false;
        }
    ...
    private function fieldTreeContainsObject(int $rootObject, int $targetObject, array $objects, array $seen = []): bool
    {
        ...
        $body = $this->completeAcroFormDictionaryObjectBody($rootObject, $objects);
        if ($body === null) {
            return false;
        }
    ...
    private function widgetDictionariesEquivalent(int $leftObject, int $rightObject, array $objects): bool
    {
        ...
        $leftBody = $this->completeAcroFormDictionaryObjectBody($leftObject, $objects);
        $rightBody = $this->completeAcroFormDictionaryObjectBody($rightObject, $objects);
        if ($leftBody === null || $rightBody === null) {
            return false;
        }

        if (!$this->isWidget($leftBody, $objects) || !$this->isWidget($rightBody, $objects)) {
            return false;
        }
    ...
    {
        $targetBody = $this->completeAcroFormDictionaryObjectBody($targetObject, $objects);
        if ($targetBody === null) {
            return false;
        }

        if (!$this->isWidget($targetBody, $objects)) {
            return false;
        }
        ...
        while ($offset < $length) {
            ...
            if ($ownsParent && $this->isWidget($dictionary, $objects) && $this->canonicalDictionaryComparisonBody($dictionary) === $targetCanonical) {
                return true;
            }
    ...
    private function isFieldWidgetDictionary(string $body, array $objects = []): bool
    {
        return $this->isWidget($body, $objects)
            && (
    ...
    private function isFieldDictionaryCandidate(string $body, array $objects = []): bool
    {
        if ($this->isNonAcroFormFieldDictionary($body, $objects)) {
            return false;
        }
    ...
    private function isNonAcroFormFieldDictionary(string $body, array $objects = []): bool
    {
        if ($this->isWidget($body, $objects)) {
            return false;
        }

        $subtype = $this->pdfNameValueAfterNameResolvingObjects($body, 'Subtype', $objects);
        ...
        $type = $this->pdfNameValueAfterNameResolvingObjects($body, 'Type', $objects);
        if ($type === null || $type === '') {
            return false;
        }
    ...
    private function pageWidgetMap(array $objects, array $pageObjectNumbers): array
    {
        ...
        foreach ($pageObjectNumbers as $pageIndex => $pageObjectNumber) {
            ...
            foreach ($annotationRefs as $annotationIndex => $annotationRef) {
                ...
                if ($annotationBody === '' || !$this->isWidget($annotationBody, $objects)) {
                    continue;
                }
    ...
    private function pdfObjects(string $pdfBytes): array
    {
        ...
        $selected = $this->xrefSelectedObjects($pdfBytes, $definitions);
        if ($selected !== null) {
    ...
    /**
     * Xref rows identify the current generation for an object number. Prefer
     * that source when available so stale higher-generation direct objects
     * cannot replace the trailer-selected AcroForm catalog.
     *
     ...
     */
    private function xrefSelectedObjects(string $pdfBytes, array $definitions): ?array
    {
        ...
        $seenOffsets = [];
        $entries = $this->xrefEntriesFromOffsetChain($pdfBytes, $offset, $definitions, $seenOffsets);
        if ($entries === []) {
            return null;
        }
     ...
     */
    private function xrefEntriesFromOffsetChain(string $pdfBytes, int $offset, array $definitions, array &$seenOffsets): array
    {
        ...
        if (substr($pdfBytes, $cursor, 4) === 'xref') {
            ...
            $previousEntries = $this->xrefEntriesFromOffsetChain($pdfBytes, $previousOffset, $definitions, $seenOffsets);
        ...
        $xrefStream = $this->xrefStreamDefinitionAtOffset($definitions, $cursor);
        if ($xrefStream === null) {
            return [];
        }

        $currentEntries = $this->xrefStreamEntriesFromDefinition($xrefStream['body']);
        $dictionary = $this->dictionaryObjectBody($xrefStream['body']) ?? $xrefStream['body'];
        $previousOffset = $this->numberValueAfterName($dictionary, 'Prev');
        if ($previousOffset === null) {
            return $currentEntries;
        }
    ...
    {
        foreach ($definitions as $generationEntries) {
            foreach ($generationEntries as $entries) {
                foreach ($entries as $definition) {
                    if ($definition['offset'] !== $offset || $this->pdfNameValueAfterName($definition['body'], 'Type') !== 'XRef') {
                        continue;
                    }
     ...
     * @return array<int, array{offset: int, generation: int, in_use: bool}>
     */
    private function xrefStreamEntriesFromDefinition(string $body): array
    {
        $dictionary = $this->dictionaryObjectBody($body) ?? $body;
        $widths = $this->xrefStreamFieldWidths($dictionary);
        if ($widths === null) {
            return [];
        }
        ...
        $decoded = $this->decodeStreamObject($body, []);
        if ($decoded === null || (strlen($decoded) % $entryWidth) !== 0) {
            return [];
        }

        $ranges = $this->xrefStreamIndexRanges($dictionary, intdiv(strlen($decoded), $entryWidth));
        if ($ranges === []) {
            return [];
        }
        ...
        foreach ($ranges as $range) {
            [$firstObject, $count] = $range;
            for ($index = 0; $index < $count; $index++) {
                if ($rowOffset + $entryWidth > strlen($decoded)) {
                    return $entries;
                }

                $fieldOffset = $rowOffset;
    ...
    {
        $widths = $this->strictIntegerArrayValueAfterName($dictionary, 'W');
    ...
    {
        $indexValues = $this->strictIntegerArrayValueAfterName($dictionary, 'Index');
        if ($indexValues !== null) {
            if ((count($indexValues) % 2) !== 0) {
                return [];
            }
        ...
        $size = $this->numberValueAfterName($dictionary, 'Size');
        if ($size === null || $size < 0) {
            return [];
        }

        return [[0, min($size, $decodedEntryCount)]];
    }

    /**
     * @return list<int>|null
     */
    private function strictIntegerArrayValueAfterName(string $dictionary, string $name): ?array
    {
        ...
        $length = strlen($arrayBody);
        while ($offset < $length) {
            $item = $this->readPdfValueAt($arrayBody, $offset, $endOffset);
            if ($item === null || $endOffset === null) {
                $this->skipWhitespace($arrayBody, $offset);
                if ($offset >= $length) {
                    break;
                }
    ...
    private function xrefStreamFieldValue(string $decoded, int &$offset, int $width): ?int
    {
        if ($width === 0) {
            return 0;
        }

        if ($width < 0 || $width > 8 || $offset + $width > strlen($decoded)) {
            return null;
        }

        $value = 0;
        for ($index = 0; $index < $width; $index++) {
            $value = ($value << 8) | ord($decoded[$offset + $index]);
        }
    ...
    private function currentTrailerRootReference(string $pdfBytes): ?array
    {
        ...
        $seenOffsets = [];
        $definitions = $this->directObjectDefinitionsByOffset($pdfBytes);
        return $this->trailerRootReferenceFromXrefOffset($pdfBytes, (int) $latest[1], $definitions, $seenOffsets);
    }

    /**
     * @return array<int, array<int, list<array{offset: int, body: string}>>>
     */
    private function directObjectDefinitionsByOffset(string $pdfBytes): array
    {
        $definitions = [];
        $offset = 0;
        while (preg_match('/(\d+)\s+(\d+)\s+obj\b/s', $pdfBytes, $match, PREG_OFFSET_CAPTURE, $offset) === 1) {
            ...
            $definitions[(int) $match[1][0]][(int) $match[2][0]][] = [
                'offset' => $objectOffset,
                'body' => substr($pdfBytes, $bodyStart, $bodyEnd - $bodyStart),
            ];
            $offset = $bodyEnd + strlen('endobj');
     ...
     * @return array{object: int, generation: int}|null
     */
    private function trailerRootReferenceFromXrefOffset(string $pdfBytes, int $offset, array $definitions, array &$seenOffsets): ?array
    {
        if ($offset < 0 || isset($seenOffsets[$offset])) {
            return null;
        }
        ...
        if (substr($pdfBytes, $offset, 4) === 'xref') {
            ...
            return $this->trailerRootReferenceFromXrefOffset($pdfBytes, $previousOffset, $definitions, $seenOffsets);
        }

        $xrefStream = $this->xrefStreamDefinitionAtOffset($definitions, $offset);
        if ($xrefStream === null) {
            return null;
        }
        ...
        $root = $this->objectReferenceFromValue($this->valueAfterName($dictionary, 'Root'));
        if ($root !== null) {
            return $root;
        }
