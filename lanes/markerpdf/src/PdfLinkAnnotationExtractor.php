...
final class PdfLinkAnnotationExtractor
{
     ...
     * @return list<array{body: string, object: int|null, generation?: int|null}>
     */
    private function annotationBodiesFromValue(
        string $value,
        array $objects,
        int $pageObjectNumber,
        ?int $pageGeneration,
    ...
    ): array
    {
        ...
        $reference = $this->exactObjectReferenceFromValue($value);
        if ($reference !== null) {
            $objectNumber = $reference['object'];
            $referenceKey = $this->annotationReferenceKey($objectNumber, $reference['generation']);
            ...
            if ($this->exactObjectReferenceFromValue($trimmedObjectBody) !== null) {
                return $this->annotationBodiesFromValue(
                    $trimmedObjectBody,
                    $objects,
                    $pageObjectNumber,
                    $pageGeneration,
     ...
     * @return list<array{body: string, object: int|null, generation?: int|null}>
     */
    private function annotationBodiesFromArray(
        ?string $arrayBody,
        array $objects,
        int $pageObjectNumber,
        ?int $pageGeneration,
    ...
    ): array
    {
        ...
        while ($offset < $length) {
            ...
            if ($this->exactObjectReferenceFromValue($value) !== null) {
                foreach ($this->annotationBodiesFromValue(
                    $value,
                    $objects,
                    $pageObjectNumber,
                    $pageGeneration,
    ...
    private function appendAnnotationBodyIfNew(array &$annotations, array $annotation, array &$emittedAnnotationReferences): void
    {
        ...
        if (is_int($object) && is_int($generation)) {
            $referenceKey = $this->annotationReferenceKey($object, $generation);
     ...
     * @return array{object: int, generation: int}|null
     */
    private function exactObjectReferenceFromValue(string $value): ?array
    {
