...
final class PdfMetadataExtractor
{
    ...
    /**
     * Catalog XMP stream filters may be indirect, but each helper object must
     * resolve to exactly one filter value. Otherwise a body like
     * `/FlateDecode /Crypt ...` would decode with the first name while hiding
     * the trailing operand at the document-metadata trust boundary.
     *
     * @param array<int, string> $objects
     * @return array<string, mixed>
     */
    private function metadataStreamFilterOperandBoundaryReview(string $dictionary, array $objects): array
    {
        $value = $this->dictionaryTopLevelRawValue($dictionary, 'Filter');
        if ($value === null) {
            return [];
        }

        $operands = $this->metadataStreamFilterOperandReviews($value, $objects);
        if ($operands === []) {
            return [];
        }
        ...
        foreach ($operands as $operand) {
            ...
            if (($operand['valid_filter_operand'] ?? null) === false) {
                $invalidCount++;
                $malformedCount++;
            }
        }
        ...
        return [
            ...
            'filter_operand_policy' => $this->metadataStreamFilterOperandPolicy(
                $invalidCount,
                $dictionaryCount,
                $malformedCount,
                $unresolvedCount
     ...
     * @return list<array<string, mixed>>
     */
    private function metadataStreamFilterOperandReviews(
        string $value,
        array $objects,
        array $seenObjects = []
    ): array {
        ...
        if ($reference !== null && $this->pdfValueIsSingleToken($value, $token)) {
            return [$this->metadataStreamIndirectFilterOperandReview($reference, $objects, $seenObjects)];
        }
        ...
        if (str_starts_with($token, '[') && $this->pdfValueIsSingleToken($value, $token)) {
            ...
            $reviews = [];
            foreach ($items as $item) {
                foreach ($this->metadataStreamFilterOperandReviews($item, $objects, $seenObjects) as $review) {
                    $reviews[] = $review;
                }
            }
        ...
        return [$this->metadataStreamDirectFilterOperandReview($value, $objects, $seenObjects)];
    }
     ...
     * @return array<string, mixed>
     */
    private function metadataStreamIndirectFilterOperandReview(
        array $reference,
        array $objects,
        array $seenObjects
    ): array {
        ...
        return $review + $this->metadataStreamFilterOperandBodyReview($body, $objects, $seenObjects);
    }
     ...
     * @return array<string, mixed>
     */
    private function metadataStreamDirectFilterOperandReview(
        string $value,
        array $objects,
        array $seenObjects
    ): array {
        ...
        $review = [
            'name' => 'Filter',
            'kind' => 'direct',
            'value' => $this->metadataStreamFilterOperandValue($token),
            'resolved' => true,
        ];

        return $review + $this->metadataStreamFilterOperandBodyReview($value, $objects, $seenObjects);
    }
     ...
     * @return array<string, mixed>
     */
    private function metadataStreamFilterOperandBodyReview(
        string $body,
        array $objects,
        array $seenObjects
    ): array {
        ...
        $tokenType = $this->metadataStreamFilterOperandTokenType($token);
        $extraOperand = $this->metadataStreamFilterExtraOperand($body);
        $review = [
            'token_type' => $tokenType,
            'valid_filter_operand' => $this->metadataStreamFilterValueIsValid($body, $objects, $seenObjects)
                && $extraOperand === null,
            'dictionary_filter_operand' => $this->metadataStreamFilterValueContainsDictionary(
                $body,
                $objects,
                $seenObjects
            ),
        ...
        if ($tokenType === 'name') {
            $review['filter_name'] = $this->nameValueAt($token, 0);
        }

        if ($extraOperand !== null) {
            $review['valid_filter_operand'] = false;
            $review['extra_filter_operand'] = true;
            $review['extra_filter_operand_type'] = $extraOperand['type'];
            $review['extra_filter_operand_preview'] = $extraOperand['preview'];
            if (($extraOperand['type'] ?? null) === 'name' && isset($extraOperand['name'])) {
                $review['extra_filter_name_operand'] = true;
                $review['extra_filter_name'] = $extraOperand['name'];
            }
        }
     ...
     * @param array<string, true> $seenObjects
     */
    private function metadataStreamFilterValueIsValid(
        string $value,
        array $objects,
        array $seenObjects
    ): bool {
        ...
        if ($reference !== null) {
            ...
            $seenObjects[$objectKey] = true;
            return $this->metadataStreamFilterValueIsValid($body, $objects, $seenObjects);
        }

        if (str_starts_with($token, '[')) {
            foreach ($this->arrayItemsFromValue($token, $objects) as $item) {
                if (!$this->metadataStreamFilterValueIsValid($item, $objects, $seenObjects)) {
                    return false;
                }
            }
     ...
     * @param array<string, true> $seenObjects
     */
    private function metadataStreamFilterValueContainsDictionary(
        string $value,
        array $objects,
        array $seenObjects
    ): bool {
        ...
        if ($reference !== null && $this->pdfValueIsSingleToken($value, $token)) {
            ...
            $seenObjects[$objectKey] = true;
            return $this->metadataStreamFilterValueContainsDictionary($body, $objects, $seenObjects);
        }

        if (str_starts_with($token, '[')) {
            foreach ($this->arrayItemsFromValue($token, $objects) as $item) {
                if ($this->metadataStreamFilterValueContainsDictionary($item, $objects, $seenObjects)) {
                    return true;
                }
            }
        }
     ...
     */
    private function metadataStreamFilterExtraOperand(string $body): ?array
    {
        $token = $this->firstPdfValueToken($body);
        if ($token === '') {
            return null;
        ...
        $after = $this->skipPdfWhitespace($body, $offset + strlen($token));
        if ($after >= strlen($body)) {
            return null;
        }

        $extra = $this->readPdfValueAt($body, $after);
        if ($extra === null || $extra === '') {
            return null;
        }

        $review = [
            'type' => $this->metadataStreamFilterOperandTokenType($extra),
            'preview' => $this->metadataStreamFilterOperandPreview($extra),
        ];

        if ($review['type'] === 'name') {
            $name = $this->nameValueAt($extra, 0);
            if ($name !== null) {
                $review['name'] = $name;
            }
        }
    ...
    private function pdfValueIsSingleToken(string $value, string $token): bool
    {
        $offset = $this->skipPdfWhitespace($value, 0);
        if ($token === '' || substr($value, $offset, strlen($token)) !== $token) {
            return false;
        }

        return $this->skipPdfWhitespace($value, $offset + strlen($token)) >= strlen($value);
    }

    private function metadataStreamFilterOperandTokenType(string $token): string
    {
        $token = $this->trimPdfWhitespaceAndComments($token);
        if ($token === '') {
            return 'empty';
        }
        if ($token === 'null') {
        ...
        if ($this->objectReferenceFromValue($token) !== null && $this->pdfValueIsSingleToken($token, $this->firstPdfValueToken($token))) {
    ...
    private function metadataStreamFilterOperandValue(string $token): mixed
    {
        if ($token === 'null') {
            return null;
        }
        ...
        if (str_starts_with($token, '/')) {
            return $this->nameValueAt($token, 0);
        }

        return $token;
    }

    private function metadataStreamFilterOperandPreview(string $value): string
    {
        $preview = $this->trimPdfWhitespaceAndComments($value);
    ...
    private function metadataStreamFilterOperandPolicy(
        int $invalidCount,
        int $dictionaryCount,
        int $malformedCount,
        int $unresolvedCount
    ...
    private function cryptFilterMetadata(string $dictionary, array $objects, array $encryptionMetadata = []): array
    {
        ...
        for ($offset = 0, $length = strlen($body); $offset < $length;) {
            ...
            if ($parameterDeclarationReview !== []) {
                $metadata['parameter_declaration_review'] = $parameterDeclarationReview;
                $metadata['parameter_declaration_status'] = $parameterDeclarationReview['status'];
    ...
    private function cryptFilterParameterDeclarationReview(string $filterBody, string $filterName, array $objects): array
    {
        ...
        return [
            'source' => 'crypt_filter_parameter_declaration_review',
            'filter_name' => $filterName,
            'duplicate_parameter_names' => $duplicateNames,
            'duplicate_parameter_count' => count($duplicateNames),
            'malformed_parameter_names' => $malformedNames,
    ...
    private function cryptFilterParameterOperandShape(string $value): string
    {
        $trimmed = $this->trimPdfWhitespaceAndComments($value);
        if ($trimmed === '') {
            return 'empty';
    ...
    ): array
    {
        $subfilter = is_string($metadata['subfilter'] ?? null)
            ...
            : ($this->dictionaryNameValue($dictionary, 'SubFilter', $objects)
                ?? $this->dictionaryStringValue($dictionary, 'SubFilter'));
        $legacySubfilterUsesTopLevelRecipients = in_array($subfilter, ['adbe.pkcs7.s3', 'adbe.pkcs7.s4'], true);
        $legacySubfilterSelectionSuppressed = false;
        $roleDeclarations = [];
        ...
        foreach ([
            ...
            $legacyRecipientSelectionSuppressed = $hasRecipients && $legacySubfilterUsesTopLevelRecipients;
            if ($legacyRecipientSelectionSuppressed) {
                $legacySubfilterSelectionSuppressed = true;
            }
            if ($hasRecipients && !$legacyRecipientSelectionSuppressed && !in_array($name, $selectedRecipientNames, true)) {
            ...
            $selectedRows[] = [
                ...
                'auth_event' => is_array($filter) ? ($filter['auth_event'] ?? null) : null,
                'has_recipients' => $hasRecipients,
                'recipient_count' => $hasRecipients && !$legacyRecipientSelectionSuppressed
                    ? (int) ($recipients['recipient_count'] ?? 0)
                    : 0,
                'suppressed_recipient_count' => $legacyRecipientSelectionSuppressed
                    ? (int) ($recipients['recipient_count'] ?? 0)
                    : 0,
                'recipient_bytes' => $hasRecipients && !$legacyRecipientSelectionSuppressed
                    ? (int) ($recipients['recipient_bytes'] ?? 0)
                    : 0,
                'suppressed_recipient_bytes' => $legacyRecipientSelectionSuppressed
                    ? (int) ($recipients['recipient_bytes'] ?? 0)
                    : 0,
                'unresolved_recipient_count' => $hasRecipients && !$legacyRecipientSelectionSuppressed
                    ? (int) ($recipients['unresolved_recipient_count'] ?? 0)
                    : 0,
                'suppressed_unresolved_recipient_count' => $legacyRecipientSelectionSuppressed
                    ? (int) ($recipients['unresolved_recipient_count'] ?? 0)
                    : 0,
                'permission_decode_status' => $hasRecipients
                    ? ($legacyRecipientSelectionSuppressed
                        ? 'legacy_public_key_subfilter_uses_top_level_recipients'
                        : ($recipients['permission_decode_status'] ?? null))
                    : null,
                'recipient_declaration_status' => $hasRecipients ? ($recipients['recipient_declaration_status'] ?? null) : null,
                'recipient_declaration_fail_closed' => $hasRecipients
                    ? (bool) ($recipients['recipient_declaration_fail_closed'] ?? false)
                ...
                'selection_suppressed_by_legacy_subfilter' => $legacyRecipientSelectionSuppressed,
                'selection_suppression_subfilter' => $legacyRecipientSelectionSuppressed ? $subfilter : null,
                'recipient_bytes_exposed' => false,
            ];

            if (!$hasRecipients || $legacyRecipientSelectionSuppressed || isset($countedRecipientFilters[$name])) {
                continue;
            }
        ...
        return [
            ...
            'selection_suppressed_by_legacy_subfilter' => $legacySubfilterSelectionSuppressed,
            'selection_suppression_subfilter' => $legacySubfilterSelectionSuppressed ? $subfilter : null,
    ...
    {
        if ($this->xmpDirectRdfAttributeMembershipValues($container) !== []) {
            return [$container];
        }
    ...
    private function repairOmittedCurrentUpdateGraphRows(
        ...
        if (
            $this->dictionaryTopLevelRawValue($sectionBody, 'XRefStm') !== null
            && $this->objectReferenceFromValue($this->dictionaryTopLevelRawValue($sectionBody, 'Prev')) !== null
        ) {
            return $entries;
        }
    ...
    private function metadataStreamFilterInputHasBoundedEndMarker(string $filter, string $stream): bool
    {
        ...
        $allowComments = !in_array($filter, ['FlateDecode', 'Fl'], true);

        return $this->metadataStreamHasOnlyWhitespaceAfterOffset($stream, $offset, $allowComments);
    }

    private function metadataStreamHasOnlyWhitespaceAfterOffset(
        string $stream,
        int $offset,
