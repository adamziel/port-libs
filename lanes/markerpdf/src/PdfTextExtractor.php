...
final class PdfTextExtractor
{
    ...
    public function extractXrefObjectStreamIndexReview(string $pdfBytes): array
    {
        ...
        $review['malformed_xref_stream_index_entries'] = $this->xrefStreamMalformedIndexEntries(
            $pdfBytes,
            $preliminaryObjects,
            $definitions
        );
    ...
    public function extractCMapStreamFilterLengthOwnerReview(string $pdfBytes): array
    {
        ...
        foreach ($objects as $objectNumber => $body) {
            ...
            $review['entries'][] = [
                ...
                'parser_bounded_cmap_length' => $parserBoundedDecoded === null ? null : strlen($parserBoundedDecoded),
                'parser_excluded_cmap_byte_count' => ($decoded === null || $parserBoundedDecoded === null)
                    ? 0
                    : max(0, strlen($decoded) - strlen($parserBoundedDecoded)),
                'parser_bounded_cmap_bytes_excluded' => $decoded !== null
    ...
    private function pageLabelPageIndexOperand(string $value, array $objects, array $seen = []): ?int
    {
        ...
        $seen[$key] = true;
        return $this->pageLabelPageIndexOperand($body, $objects, $seen);
    }
    ...
    private function pageObjectNumbersFromTree(int $objectNumber, int $generation, array $objects, array $seen = []): array
    {
        ...
        $kids = $this->pageTreeKidObjectReferences($body, $objects);
        if ($kids === []) {
            return [];
        }

        $pages = [];
        foreach ($kids as $childReference) {
    ...
    private function pdfArrayItems(string $arrayBody): array
    {
        ...
        while ($index < $length) {
            ...
            $reference = $this->pdfIndirectReferenceTokenAt($arrayBody, $index);
            if ($reference !== null) {
                $items[] = $reference['token'];
                $index = $reference['endOffset'];
                continue;
            }
     ...
     */
    private function pdfIndirectReferenceTokenAt(string $body, int $offset): ?array
    {
        $length = strlen($body);
        ...
        if ($endOffset < $length && !$this->isBareTokenDelimiter($body[$endOffset])) {
            return null;
        }
    ...
    private function startxrefXrefStreamFilterDecodeFailed(string $pdfBytes, array $objects, array $definitions): bool
    {
        ...
        if ($this->xrefStreamIndexProblem($dictionary, $streamObjects) !== null) {
            return true;
        }
     ...
     * @return list<array<string, mixed>>
     */
    private function xrefStreamMalformedIndexEntries(string $pdfBytes, array $objects, array $definitions): array
    {
        $offset = $this->latestStartxrefOffset($pdfBytes, $definitions);
        if ($offset === null) {
            return [];
     ...
     * @return list<array<string, mixed>>
     */
    private function xrefStreamMalformedIndexEntriesFromOffsetChain(
        string $pdfBytes,
        int $offset,
        array $objects,
        array $definitions,
        ...
        $dictionary = $this->dictionaryObjectBody($streamSection['body']) ?? $streamSection['body'];
        $problem = $this->xrefStreamIndexProblem($dictionary, $streamObjects);
        $entries = [];
        if ($problem !== null) {
            $entries[] = [
                'xref_offset' => $streamSection['definition']['offset'],
                'selected_generation' => $streamSection['definition']['generation'],
                'index_array' => $problem['index_array'],
                'indexes' => $problem['indexes'],
                'malformed_index_indexes' => $problem['malformed_indexes'],
                'owner_policy' => $problem['owner_policy'],
                'rejected_before_row_decode' => true,
                'review_only' => true,
            ];
    ...
    private function skipPdfCompositeTokenAt(string $pdfBytes, int $offset): ?int
    {
        ...
        if (substr($pdfBytes, $offset, 2) === '<<') {
            $dictionaryEnd = $this->pdfDictionaryEndOffset($pdfBytes, $offset);
            return $dictionaryEnd === null ? null : $dictionaryEnd + 1;
        }
    ...
    /**
     * @param array<int, string> $objects
     * @return array{index_array: string|null, indexes: list<int>, malformed_indexes: list<int>, owner_policy: string}|null
     */
    private function xrefStreamIndexProblem(string $dictionary, array $objects): ?array
    {
        $indexOffset = $this->topLevelNameValueOffset($dictionary, 'Index');
        if ($indexOffset === null) {
            return null;
        }
        ...
        if ($indexArray === null) {
            return [
                ...
                'indexes' => [],
                'malformed_indexes' => [],
                'owner_policy' => 'malformed_xref_stream_index_array',
            ];
        }
        ...
        if ($nonIntegerIndexes !== []) {
            return [
                ...
                'indexes' => $indexes,
                'malformed_indexes' => $nonIntegerIndexes,
                'owner_policy' => 'non_integer_xref_stream_index_value',
            ];
        }
        ...
        if ($negativeIndexes !== []) {
            return [
                'index_array' => $indexArray,
                'indexes' => $indexes,
                'malformed_indexes' => $negativeIndexes,
                'owner_policy' => 'negative_xref_stream_index_value',
            ];
        }

        if ((count($indexes) % 2) !== 0) {
            return [
                'index_array' => $indexArray,
                'indexes' => $indexes,
                'malformed_indexes' => [count($indexes) - 1],
                'owner_policy' => 'incomplete_xref_stream_index_pair',
            ];
        }
    ...
    {
        if ($this->xrefStreamIndexProblem($xrefBody, $objects) !== null) {
            return [];
        }
    ...
    private function boundedSingleCMapStreamProgram(string $cmap): string
    {
        $endOffset = $this->cMapEndOperatorOffset($cmap, false);
        if ($endOffset === null) {
            return $cmap;
        }
    ...
    private function cMapEndOperatorOffset(string $cmap, bool $last = true): ?int
    {
        $lastEndOffset = null;
        $length = strlen($cmap);
        for ($index = 0; $index < $length;) {
     ...
     * @param array<int, string> $objects
     */
    private function decodedCMapBodyForParsing(string $objectBody, array $objects, array $seenCMapObjects = []): ?string
    {
        $entry = $this->streamDictionaryAndPayload($objectBody, $objects);
        if ($entry === null) {
            return null;
        ...
        return $this->cMapStreamDictionaryPrelude($entry['dict'], $objects, $seenCMapObjects, true)
            . $this->boundedSingleCMapStreamProgram($decoded);
    }

    /**
     * @param array<int, string> $objects
     */
    private function cMapStreamDictionaryPrelude(
        string $dict,
        array $objects,
        array $seenCMapObjects = [],
        bool $forParsing = false
    ): string
    {
        $lines = [];
        ...
        if ($useCMapName !== null && $useCMapName !== '') {
            ...
            if ($useCMapObjectNumber !== null && isset($objects[$useCMapObjectNumber]) && !isset($seenCMapObjects[$useCMapObjectNumber])) {
                ...
                $baseCMap = $forParsing
                    ? $this->decodedCMapBodyForParsing($objects[$useCMapObjectNumber], $objects, $seenCMapObjects)
                    : $this->decodedCMapBody($objects[$useCMapObjectNumber], $objects, $seenCMapObjects);
            ...
            if ($this->isTextShowingOperator($token)) {
                ...
                if ($operand !== null) {
                    ...
                    $this->appendNativeTextSpan(
                        ...
                        $textRise,
                        $this->styledSpanXStartAfterPendingGap($spans, $pendingStyledSpanGap)
                    );
                    $pendingStyledSpanGap = 0.0;
    ...
    private function appendNativeTextSpan(
        ...
        $fontSize ??= 12.0;
        $xStart = is_float($xStartOverride) && is_finite($xStartOverride) ? $xStartOverride : 0.0;
        if ($xStartOverride === null && $spans !== []) {
     ...
     * @param list<array<string, mixed>> $spans
     */
    private function styledSpanXStartAfterPendingGap(array $spans, float $pendingGap): ?float
    {
        if ($pendingGap < self::POSITIONED_TEXT_WORD_GAP || $spans === []) {
            return null;
        }
     ...
     * @param list<string> $operands
     */
    private function styledTextMoveGapWidth(
        array $operands,
        ?float $currentTextX,
    ...
    private function nativeTextSpanWidth(
        ...
        if ($sourceOperand !== null && $this->mapWritingMode($toUnicodeMap) !== 1) {
            $extentWidth = $this->textOperandHorizontalExtentWidth(
                $sourceOperand,
                $toUnicodeMap,
                $fontSize,
                $characterSpacing,
            ...
            if ($extentWidth !== null && is_finite($extentWidth)) {
                return max(1.0, $extentWidth);
            }
        }
