...
final class PdfEmbeddedFileExtractor
{
    ...
    private function nameTreeNodeLimits(string $nodeBody, array $objects): ?array
    {
        ...
        $lower = $this->stringDetailsFromRaw($items[0], $objects);
        $upper = $this->stringDetailsFromRaw($items[1], $objects);
    ...
    private function nameTreeNamePairsSortedByKey(array $items, array $objects, bool $sortByKey): array
    {
        ...
        for ($index = 0, $count = count($items); $index < $count;) {
            ...
            if (
                $nextIndex < $count
                && $this->stringDetailsFromRaw($items[$nextIndex], $objects) === null
            ) {
                $index = $nextIndex;
                continue;
            }
    ...
    private function nameTreeLimitsMatchAnyPairKey(array $items, array $objects, ?array $limits): bool
    {
        ...
        for ($index = 0, $count = count($items); $index + 1 < $count; $index += 2) {
            $name = $this->stringDetailsFromRaw($items[$index], $objects);
    ...
    private function repairOmittedCurrentUpdateGraphRows(
        ...
        foreach (['Root', 'Info', 'Encrypt'] as $name) {
            $value = $this->dictionaryRawValue($sectionBody, $name);
            $reference = $this->objectReferenceFromValue($value);
            if ($reference !== null) {
                $pending[] = [
                    'reference' => $reference,
    ...
    private function inheritedTrailerGraphReference(
        ...
        if ($tableSection !== null) {
            $value = $this->dictionaryRawValue($tableSection['trailer'], $name);
            if ($value !== null) {
        ...
        $value = $this->dictionaryRawValue($streamSection['body'], $name);
        if ($value !== null) {
            return $this->objectReferenceFromValue($value);
        }
    ...
    private function classicXrefTableHasPreviousOffset(
        ...
        return $this->dictionaryIntegerValue($table['trailer'], 'Prev') === $previousOffset;
    }
     ...
     */
    private function stringDetailsFromRaw(string $value, array $objects): ?array
    {
        $resolved = $this->resolveRawValue($value, $objects) ?? $value;
        $resolved = trim($resolved);
        if ($resolved === '') {
        ...
        if (str_starts_with($resolved, '(')) {
            ...
            $bytes = $this->decodeLiteralEscapes($literal['body']);
        ...
        if (str_starts_with($resolved, '/')) {
            $text = $this->decodePdfName(substr($resolved, 1));
    ...
    private function skipPdfHexStringBoundary(string $value, int $offset): ?int
    {
        $strictEnd = $this->skipPdfHexStringToken($value, $offset);
        if ($strictEnd !== null) {
            return $strictEnd;
        }

        return strpos($value, '>', $offset + 1) === false ? strlen($value) : null;
    }
