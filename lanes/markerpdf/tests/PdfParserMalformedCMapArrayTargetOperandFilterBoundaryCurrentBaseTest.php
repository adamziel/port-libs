...
$parserMalformedCMapArrayTargetOperandFilterBoundaryUtf16beHex = static function (string $ascii): string {
    $hex = '';
...
$parserMalformedCMapArrayTargetOperandFilterBoundaryPdf = static function (
    string $targetOperand,
    string $safeText,
    string $leakingText,
    string $cMapName,
    string $baseFont
) use ($parserMalformedCMapArrayTargetOperandFilterBoundaryUtf16beHex): array {
    $safeHex = $parserMalformedCMapArrayTargetOperandFilterBoundaryUtf16beHex($safeText);
    ...
    $cMap = "/CIDInit /ProcSet findresource begin\n"
        ...
        . "1 beginbfrange\n"
        . "<{$sourceCode}> <{$sourceCode}> [ <"
        . $parserMalformedCMapArrayTargetOperandFilterBoundaryUtf16beHex($leakingText)
        . "> {$targetOperand} ]\n"
        . "endbfrange\n"
        . "endcmap\n"
    ...
    if (!is_string($compressedCMap)) {
        throw new RuntimeException('Unable to compress focused malformed CMap array-target operand fixture.');
    }
    ...
    return [$pdf, $safeText, $leakingText, $cMapName, $targetOperand];
};

$assertParserMalformedCMapArrayTargetOperandFilterBoundary = static function (
    TestRunner $t,
    array $fixture
): void {
    [$pdf, $safeText, $leakingText, $cMapName, $targetOperand] = $fixture;
    ...
    $t->true(!str_contains($plainText, $targetOperand));
...
return [
    'rejects dictionary operands inside filtered ToUnicode bfrange target arrays before current-base text extraction' => static function (
        TestRunner $t
    ) use (
        $parserMalformedCMapArrayTargetOperandFilterBoundaryPdf,
        $assertParserMalformedCMapArrayTargetOperandFilterBoundary
    ): void {
        $assertParserMalformedCMapArrayTargetOperandFilterBoundary(
            $t,
            $parserMalformedCMapArrayTargetOperandFilterBoundaryPdf(
                '<< /Decoy (target arrays require strings) >>',
                'Array Dict Safe Import',
                'Array Dict CMap Leak',
                'ArrayDictTargetOperandBoundary-H',
                'ArrayDictTargetOperandBoundary'
            )
        );
    },
    'rejects name operands inside filtered ToUnicode bfrange target arrays before current-base text extraction' => static function (
        TestRunner $t
    ) use (
        $parserMalformedCMapArrayTargetOperandFilterBoundaryPdf,
        $assertParserMalformedCMapArrayTargetOperandFilterBoundary
        ...
        $assertParserMalformedCMapArrayTargetOperandFilterBoundary(
            ...
            $parserMalformedCMapArrayTargetOperandFilterBoundaryPdf(
                '/DecoyTargetName',
                'Array Name Safe Import',
                'Array Name CMap Leak',
                'ArrayNameTargetOperandBoundary-H',
                'ArrayNameTargetOperandBoundary'
            )
        );
    },
];
