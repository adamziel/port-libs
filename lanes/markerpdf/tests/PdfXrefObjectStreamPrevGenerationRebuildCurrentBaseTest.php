...
$xrefObjectStreamPrevGenerationRebuildCurrentBasePdf = static function (): string {
    ...
    $objectStream = static function (array $members, array &$memberIndexes): array {
        ...
        if (!is_string($compressed)) {
            throw new RuntimeException('Unable to compress object-stream generation rebuild fixture.');
        }
    ...
    if (!is_string($previousCompressedXref)) {
        throw new RuntimeException('Unable to compress previous xref-stream generation rebuild fixture.');
    }
    ...
    if (!is_string($currentCompressedXref)) {
        throw new RuntimeException('Unable to compress current xref-stream generation rebuild fixture.');
    }
...
return [
    'rebuilds current object-stream carrier before stale Prev direct carrier generation' => static function (TestRunner $t) use ($xrefObjectStreamPrevGenerationRebuildCurrentBasePdf): void {
        $extractor = new PdfTextExtractor();
        $pdf = $xrefObjectStreamPrevGenerationRebuildCurrentBasePdf();
        $text = $extractor->extractPlainText($pdf);
        $review = $extractor->extractXrefObjectStreamIndexReview($pdf);
        $entry = $review['entries'][0] ?? [];

        $t->same(['Current rebuilt carrier page', 'Prev carrier generation ignored'], $extractor->extractTextLines($pdf));
        $t->same(['Current rebuilt carrier page', 'Prev carrier generation ignored'], $extractor->extractTextRuns($pdf));
        $t->same("Current rebuilt carrier page\nPrev carrier generation ignored", $text);
        $t->same("Current rebuilt carrier page\nPrev carrier generation ignored\n", $extractor->naiveGetText($pdf));
        $t->same(1, $extractor->extractOutlineMetadata($pdf)['pages']);
        $t->same(['1'], $extractor->extractPageLabels($pdf));
        $t->true(!str_contains($text, 'Stale Prev carrier generation page'));
        $t->true(!str_contains($text, 'stale previous carrier member'));
        $t->true(!str_contains($text, 'current generation rebuilt carrier member'));
