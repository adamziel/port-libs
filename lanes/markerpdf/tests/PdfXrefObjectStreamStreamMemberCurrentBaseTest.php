...
$xrefObjectStreamStreamMemberCurrentBasePdf = static function (): string {
    ...
    if (!is_string($objectStream)) {
        throw new RuntimeException('Unable to compress stream-member object-stream fixture.');
    }
    ...
    if (!is_string($compressedXref)) {
        throw new RuntimeException('Unable to compress stream-member xref-stream fixture.');
    }
...
return [
    'rejects stream-object members inside xref-selected object streams before WordPress text extraction' => static function (TestRunner $t) use ($xrefObjectStreamStreamMemberCurrentBasePdf): void {
        $extractor = new PdfTextExtractor();
        $pdf = $xrefObjectStreamStreamMemberCurrentBasePdf();
        $text = $extractor->extractPlainText($pdf);
        $review = $extractor->extractXrefObjectStreamIndexReview($pdf);
        $entries = array_column($review['entries'], null, 'object_number');

        $t->same(['Current stream-member guard page'], $extractor->extractTextLines($pdf));
        $t->same(['Current stream-member guard page'], $extractor->extractTextRuns($pdf));
        $t->same('Current stream-member guard page', $text);
        $t->same("Current stream-member guard page\n", $extractor->naiveGetText($pdf));
        $t->same(1, $extractor->extractOutlineMetadata($pdf)['pages']);
        $t->same(['1'], $extractor->extractPageLabels($pdf));
        $t->true(!str_contains($text, 'Compressed stream member leak'));
        $t->true(!str_contains($text, 'Stream objects rejected from ObjStm'));
        ...
        $t->same(true, $entries[5]['object_stream_carrier_has_filter'] ?? null);
