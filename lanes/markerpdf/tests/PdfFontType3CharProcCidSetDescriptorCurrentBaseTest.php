...
$fontType3CharProcCidSetDescriptorCurrentBasePdf = static function (): string {
...
return [
    'uses Type3 CharProc widths and descriptor MissingWidth before WordPress text grouping on current base' => static function (TestRunner $t) use ($fontType3CharProcCidSetDescriptorCurrentBasePdf): void {
        $extractor = new PdfTextExtractor();
        $pdf = $fontType3CharProcCidSetDescriptorCurrentBasePdf();
        ...
        $t->true(!str_contains($plainText, 'cidset descriptor payload'));
        $t->same('T3DescriptorBoldSerif_serif_non_symbolic_bold', $firstSpan['font'] ?? null);
        $t->same((1 << 1) | (1 << 5) | (1 << 18), $firstSpan['font_flags'] ?? null);
        $t->same(700.0, $firstSpan['font_weight'] ?? null);
    },
];
