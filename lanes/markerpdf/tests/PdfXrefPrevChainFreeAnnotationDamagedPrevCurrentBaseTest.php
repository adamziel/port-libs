...
$xrefPrevChainFreeAnnotationDamagedPrevCurrentBasePdf = static function (): string {
    $baseContent = 'BT /F1 12 Tf 72 720 Td (Base damaged Prev annotation page) Tj ET';
    $currentContent = 'BT /F1 12 Tf 72 720 Td (Current damaged Prev annotation page) Tj ET';
    ...
    $staleAnnotationOffset = $addObject(7, 0, '<< /Type /Annot /Subtype /Link /Rect [72 700 250 718] /Contents (Stale damaged Prev free annotation) /A << /S /URI /URI (https://stale.example.com/damaged-prev-free-annotation) >> >>');
    ...
    if (!is_string($compressedRows)) {
        throw new RuntimeException('Unable to compress damaged-Prev free annotation xref-stream fixture.');
    }
...
return [
    'repairs damaged xref-stream Prev to suppress stale page annotations before WordPress link promotion' => static function (
        TestRunner $t
    ) use ($xrefPrevChainFreeAnnotationDamagedPrevCurrentBasePdf): void {
        $pdf = $xrefPrevChainFreeAnnotationDamagedPrevCurrentBasePdf();
        $linkExtractor = new PdfLinkAnnotationExtractor();
        $annotationExtractor = new PdfAnnotationExtractor();
        $freeObjects = PdfXrefFreeObjectMap::freeObjectNumbers($pdf);

        $t->true(isset($freeObjects[7]), 'The lightweight free-object map must repair damaged /Prev before link promotion.');
        $t->same([], $linkExtractor->extractPageLinks($pdf), 'The stale annotation freed through the repaired /Prev chain must not become a WordPress link.');
        $t->same([], $annotationExtractor->extractPageAnnotations($pdf), 'The stale annotation freed through the repaired /Prev chain must not become review metadata.');
        ...
        $pages = [[
            ...
            'blocks' => [[
                ...
                'lines' => [[
                    'bbox' => [72.0, 700.0, 250.0, 718.0],
                    'spans' => [[
                        'text' => 'Current damaged Prev annotation page',
                        'bbox' => [72.0, 700.0, 250.0, 718.0],
                        'font' => 'Helvetica',
                    ]],
                ]],
        ...
        $t->true(str_contains($pdf, '/Index [3 2 20 1]'));
        ...
        $t->true(!str_contains($encoded, 'Stale damaged Prev free annotation'));
    },
];
