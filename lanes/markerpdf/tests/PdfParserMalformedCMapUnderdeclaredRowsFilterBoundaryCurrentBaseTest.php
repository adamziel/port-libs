...
$parserMalformedCMapUnderdeclaredRowsFilterUtf16beHex = static function (string $ascii): string {
    $hex = '';
...
$parserMalformedCMapUnderdeclaredRowsFilterPdf = static function (
    string $mappingKind
) use ($parserMalformedCMapUnderdeclaredRowsFilterUtf16beHex): array {
    $safeText = $mappingKind === 'range'
        ? 'Underdeclared Range Safe Import'
        : 'Underdeclared Char Safe Import';
    $leakingText = $mappingKind === 'range'
        ? 'Underdeclared Range CMap Leak'
        : 'Underdeclared Char CMap Leak';
    $safeHex = $parserMalformedCMapUnderdeclaredRowsFilterUtf16beHex($safeText);
    $sourceCode = substr($safeHex, 0, 4);
    $cMapName = $mappingKind === 'range'
        ? 'UnderdeclaredRangeRowsBoundary-H'
        : 'UnderdeclaredCharRowsBoundary-H';
    $baseFont = $mappingKind === 'range'
        ? 'UnderdeclaredRangeRowsBoundary'
        : 'UnderdeclaredCharRowsBoundary';
    $mappingBlock = $mappingKind === 'range'
        ? "2 beginbfrange\n"
            . "<{$sourceCode}> <{$sourceCode}> <" . $parserMalformedCMapUnderdeclaredRowsFilterUtf16beHex($leakingText) . ">\n"
            . "endbfrange\n"
        : "2 beginbfchar\n"
            . "<{$sourceCode}> <" . $parserMalformedCMapUnderdeclaredRowsFilterUtf16beHex($leakingText) . ">\n"
            . "endbfchar\n";
    ...
    if (!is_string($compressedCMap)) {
        throw new RuntimeException('Unable to compress underdeclared CMap row fixture.');
    }
...
$assertMalformedCMapUnderdeclaredRowsFilter = static function (TestRunner $t, array $fixture): void {
...
return [
    'fails closed on filtered ToUnicode bfchar blocks with fewer rows than the declared count' => static function (
        TestRunner $t
    ) use ($parserMalformedCMapUnderdeclaredRowsFilterPdf, $assertMalformedCMapUnderdeclaredRowsFilter): void {
        $assertMalformedCMapUnderdeclaredRowsFilter(
            $t,
            $parserMalformedCMapUnderdeclaredRowsFilterPdf('char')
        );
    },
    'fails closed on filtered ToUnicode bfrange blocks with fewer rows than the declared count' => static function (
        TestRunner $t
    ) use ($parserMalformedCMapUnderdeclaredRowsFilterPdf, $assertMalformedCMapUnderdeclaredRowsFilter): void {
        $assertMalformedCMapUnderdeclaredRowsFilter(
            $t,
            $parserMalformedCMapUnderdeclaredRowsFilterPdf('range')
        );
    },
];
