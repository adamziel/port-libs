...
$cMapMalformedDeclaredCountCidSourceWidthCurrentBasePdf = static function (string $mappingKind): string {
    if ($mappingKind === 'char') {
...
$assertMalformedDeclaredCountCidSourceWidth = static function (TestRunner $t, string $pdf): void {
    ...
    $t->true(!str_contains($plainText, 'BadRow'));
    $t->true(!str_contains($plainText, "\0"));
};

return [
    'consumes malformed declared-count CMap cidchar row slots before source-width fallback on current base' => static function (
        TestRunner $t
    ) use ($cMapMalformedDeclaredCountCidSourceWidthCurrentBasePdf, $assertMalformedDeclaredCountCidSourceWidth): void {
        $assertMalformedDeclaredCountCidSourceWidth($t, $cMapMalformedDeclaredCountCidSourceWidthCurrentBasePdf('char'));
    },

    'consumes malformed declared-count CMap cidrange row slots before source-width fallback on current base' => static function (
        TestRunner $t
    ) use ($cMapMalformedDeclaredCountCidSourceWidthCurrentBasePdf, $assertMalformedDeclaredCountCidSourceWidth): void {
        $assertMalformedDeclaredCountCidSourceWidth($t, $cMapMalformedDeclaredCountCidSourceWidthCurrentBasePdf('range'));
    },
];
