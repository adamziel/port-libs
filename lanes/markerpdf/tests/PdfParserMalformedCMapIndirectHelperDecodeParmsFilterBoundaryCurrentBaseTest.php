...
$parserMalformedCMapIndirectHelperDecodeParmsFilterUtf16beHex = static function (string $ascii): string {
    $hex = '';
...
$parserMalformedCMapIndirectHelperDecodeParmsFilterPdf = static function (
    string $helperBody,
    string $safeText,
    string $leakingText,
    string $cMapName,
    string $baseFont
) use ($parserMalformedCMapIndirectHelperDecodeParmsFilterUtf16beHex): array {
    $safeHex = $parserMalformedCMapIndirectHelperDecodeParmsFilterUtf16beHex($safeText);
    ...
    $cMap = "/CIDInit /ProcSet findresource begin\n"
        ...
        . "endcodespacerange\n"
        . "1 beginbfchar\n"
        . "<{$sourceCode}> <" . $parserMalformedCMapIndirectHelperDecodeParmsFilterUtf16beHex($leakingText) . ">\n"
        . "endbfchar\n"
        . "endcmap\n"
    ...
    if (!is_string($compressedCMap)) {
        throw new RuntimeException('Unable to compress indirect helper DecodeParms CMap fixture.');
    }
    ...
    return [$pdf, $safeText, $leakingText, $cMapName, $helperBody];
};

$assertMalformedCMapIndirectHelperDecodeParmsFilter = static function (
    TestRunner $t,
    array $fixture,
    string $tokenType,
    string $expectedPreview
): void {
    [$pdf, $safeText, $leakingText, $cMapName, $helperBody] = $fixture;
...
return [
    'reports trailing decoder names after keyed DecodeParms in selected indirect scalar CMap Filter helpers' => static function (
        TestRunner $t
    ) use (
        $parserMalformedCMapIndirectHelperDecodeParmsFilterPdf,
        $assertMalformedCMapIndirectHelperDecodeParmsFilter
    ): void {
        $assertMalformedCMapIndirectHelperDecodeParmsFilter(
            $t,
            $parserMalformedCMapIndirectHelperDecodeParmsFilterPdf(
                '/FlateDecode /DecodeParms << /Predictor 1 >> /ASCIIHexDecode',
                'Indirect Helper Keyed Safe Import',
                'Indirect Helper Keyed CMap Leak',
                'IndirectHelperDecodeParmsFilterBoundary-H',
                'IndirectHelperDecodeParmsFilterBoundary'
            ),
            'name',
            '/ASCIIHexDecode'
        );
    },
    'reports trailing decoder names after keyed DecodeParms in selected indirect array CMap Filter helpers' => static function (
        TestRunner $t
    ) use (
        $parserMalformedCMapIndirectHelperDecodeParmsFilterPdf,
        $assertMalformedCMapIndirectHelperDecodeParmsFilter
        ...
        $assertMalformedCMapIndirectHelperDecodeParmsFilter(
            ...
            $parserMalformedCMapIndirectHelperDecodeParmsFilterPdf(
                '[ /FlateDecode ] /DecodeParms << /Predictor 1 >> /ASCIIHexDecode',
                'Indirect Helper Array Keyed Safe Import',
                'Indirect Helper Array Keyed CMap Leak',
                'IndirectHelperArrayDecodeParmsFilterBoundary-H',
                'IndirectHelperArrayDecodeParmsFilterBoundary'
            ),
            'array',
            '/ASCIIHexDecode'
        );
