...
$pageLabelsDuplicateKidsKeyPdf = static function (): string {
    $contents = [
...
return [
    'keeps first usable duplicate PageLabels Kids key before stale malformed arrays' => static function (
        TestRunner $t
    ) use ($pageLabelsDuplicateKidsKeyPdf): void {
        $pdf = $pageLabelsDuplicateKidsKeyPdf();
        ...
        $t->same([
            'Duplicate kids cover imported',
            'Duplicate kids body imported',
            'Duplicate kids appendix imported',
        ], array_column($entries, 'text'));
        foreach (['stale-kids-99', '1', '2', '3'] as $staleLabel) {
