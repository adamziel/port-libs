...
$xrefPrevChainLatestMalformedRowsSuppressPrevPdf = static function () use ($xrefPrevChainIncrementalUpdateCurrentBaseXmp): string {
    $staleContent = 'BT /F1 12 Tf 72 720 Td (Stale wrong-owner row Prev page) Tj ET';
    $currentContent = 'BT /F1 12 Tf 72 720 Td (Current wrong-owner row page) Tj T* (Malformed rows suppress Prev) Tj ET';
    ...
    if (!is_string($staleXmp)) {
        throw new RuntimeException('Unable to compress wrong-owner-row xref Prev chain fixture stream.');
    }
    ...
    $compressedRows = gzcompress($currentRows);
    if (!is_string($compressedRows)) {
        throw new RuntimeException('Unable to compress wrong-owner-row current xref-stream fixture.');
    }
...
$xrefPrevChainLatestRootFreeSuppressPrevPdf = static function () use ($xrefPrevChainIncrementalUpdateCurrentBaseXmp): string {
    ...
    $staleXmp = gzcompress($xrefPrevChainIncrementalUpdateCurrentBaseXmp(
        'Stale Root Free XMP Title',
        'Previous root metadata must not be revived after current free row'
    ));
    if (!is_string($staleXmp)) {
        throw new RuntimeException('Unable to compress latest-root-free xref Prev chain fixture stream.');
    }
    ...
    $staleContentOffset = $addObject(4, 0, "<< /Length " . strlen($staleContent) . " >>\nstream\n{$staleContent}\nendstream");
    $fontOffset = $addObject(5, 0, '<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica /Encoding /WinAnsiEncoding >>');
    $staleInfoOffset = $addObject(6, 0, '<< /Title (Carried Root Free Info Title) /Author (Carried Root Free Author) /Producer (Carried Root Free Producer) >>');
    $staleMetadataOffset = $addObject(7, 0, '<< /Type /Metadata /Subtype /XML /Filter /FlateDecode /Length ' . strlen($staleXmp) . " >>\nstream\n{$staleXmp}\nendstream");
    $staleNameTreeOffset = $addObject(8, 0, '<< /Names [(stale-root-free-prev.xml) 10 0 R] >>');
    $staleFileSpecOffset = $addObject(10, 0, '<< /Type /Filespec /F (stale-root-free-prev.xml) /Desc (Stale root free Prev attachment) /AFRelationship /Source /EF << /F 11 0 R >> >>');
    $staleEmbeddedFileOffset = $addObject(11, 0, '<< /Type /EmbeddedFile /Subtype /text#2Fxml /Length ' . strlen($stalePayload) . " >>\nstream\n{$stalePayload}\nendstream");
    ...
    $compressedRows = gzcompress($currentRows);
    if (!is_string($compressedRows)) {
        throw new RuntimeException('Unable to compress latest-root-free current xref-stream fixture.');
    }
...
$xrefPrevChainDamagedPrevRootFreeSuppressPrevPdf = static function () use ($xrefPrevChainIncrementalUpdateCurrentBaseXmp): string {
    ...
    if (!is_string($staleXmp)) {
        throw new RuntimeException('Unable to compress damaged-root-free xref Prev chain fixture stream.');
    }
    ...
    $compressedRows = gzcompress($currentRows);
    if (!is_string($compressedRows)) {
        throw new RuntimeException('Unable to compress damaged-root-free current xref-stream fixture.');
    }
...
$xrefPrevChainClassicTableDirectPrevRootFreeSuppressPrevPdf = static function () use (
    $xrefPrevChainIncrementalUpdateCurrentBaseXmp
): string {
    $staleContent = 'BT /F1 12 Tf 72 720 Td (Stale classic root free direct Prev page) Tj ET';
    $stalePayload = '<wp-export><post id="stale-classic-root-free-prev"/></wp-export>';
    $staleXmp = gzcompress($xrefPrevChainIncrementalUpdateCurrentBaseXmp(
        'Stale Classic Root Free XMP Title',
        'Classic table direct Prev root metadata must not be revived'
    ));
    if (!is_string($staleXmp)) {
        throw new RuntimeException('Unable to compress classic-root-free xref Prev chain fixture stream.');
    }
    ...
    $staleCatalogOffset = $addObject(1, 0, '<< /Type /Catalog /Pages 2 0 R /Lang (fr-FR) /Metadata 7 0 R /Names << /EmbeddedFiles 8 0 R >> >>');
    $stalePagesOffset = $addObject(2, 0, '<< /Type /Pages /Kids [3 0 R] /Count 1 >>');
    $stalePageOffset = $addObject(3, 0, '<< /Type /Page /Parent 2 0 R /Resources << /Font << /F1 5 0 R >> >> /Contents 4 0 R >>');
    $staleContentOffset = $addObject(4, 0, "<< /Length " . strlen($staleContent) . " >>\nstream\n{$staleContent}\nendstream");
    $fontOffset = $addObject(5, 0, '<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica /Encoding /WinAnsiEncoding >>');
    $staleInfoOffset = $addObject(6, 0, '<< /Title (Carried Classic Root Free Info Title) /Author (Carried Classic Root Free Author) /Producer (Carried Classic Root Free Producer) >>');
    $staleMetadataOffset = $addObject(7, 0, '<< /Type /Metadata /Subtype /XML /Filter /FlateDecode /Length ' . strlen($staleXmp) . " >>\nstream\n{$staleXmp}\nendstream");
    $staleNameTreeOffset = $addObject(8, 0, '<< /Names [(stale-classic-root-free-prev.xml) 10 0 R] >>');
    $staleFileSpecOffset = $addObject(10, 0, '<< /Type /Filespec /F (stale-classic-root-free-prev.xml) /Desc (Stale classic root free Prev attachment) /AFRelationship /Source /EF << /F 11 0 R >> >>');
    $staleEmbeddedFileOffset = $addObject(11, 0, '<< /Type /EmbeddedFile /Subtype /text#2Fxml /Length ' . strlen($stalePayload) . " >>\nstream\n{$stalePayload}\nendstream");
...
$xrefPrevChainLatestSparseTrailerRootPdf = static function () use ($xrefPrevChainIncrementalUpdateCurrentBaseXmp): string {
    ...
    $decoyXmp = gzcompress($xrefPrevChainIncrementalUpdateCurrentBaseXmp(
        'Decoy Sparse Root XMP Title',
        'Unreferenced lower-numbered catalog must not win'
    ));
    $previousXmp = gzcompress($xrefPrevChainIncrementalUpdateCurrentBaseXmp(
        'Previous Sparse Root XMP Title',
        'Previous trailer root metadata must be updated by current rows'
    ));
    $currentXmp = gzcompress($xrefPrevChainIncrementalUpdateCurrentBaseXmp(
        'Current Sparse Root XMP Title',
        'Latest sparse trailer inherits previous Root'
    ));
    if (!is_string($decoyXmp) || !is_string($previousXmp) || !is_string($currentXmp)) {
        throw new RuntimeException('Unable to compress sparse Root xref Prev chain fixture streams.');
    }
    ...
    $compressedCurrentRows = gzcompress($currentRows);
    if (!is_string($compressedCurrentRows)) {
        throw new RuntimeException('Unable to compress sparse Root xref-stream fixture rows.');
    }
...
$xrefPrevChainZeroWidthCurrentOffsetRowsPdf = static function () use ($xrefPrevChainIncrementalUpdateCurrentBaseXmp): string {
    $staleContent = 'BT /F1 12 Tf 72 720 Td (Stale zero-width offset Prev page) Tj ET';
    $currentContent = 'BT /F1 12 Tf 72 720 Td (Current zero-width offset page) Tj T* (Omitted offset fields repaired) Tj ET';
    $stalePayload = '<wp-export><post id="stale-zero-width-offset"/></wp-export>';
    $currentPayload = '<wp-export><post id="current-zero-width-offset"/></wp-export>';
    $staleXmp = gzcompress($xrefPrevChainIncrementalUpdateCurrentBaseXmp(
        'Stale Zero Width Offset XMP Title',
        'Previous zero-width offset metadata must not win'
    ));
    $currentXmp = gzcompress($xrefPrevChainIncrementalUpdateCurrentBaseXmp(
        'Current Zero Width Offset XMP Title',
        'Current type-one rows with omitted offsets selected'
    ));
    if (!is_string($staleXmp) || !is_string($currentXmp)) {
        throw new RuntimeException('Unable to compress zero-width xref offset fixture streams.');
    }
    ...
    $xrefTableRow = static fn (int $offset, int $generation = 0, string $state = 'n'): string => sprintf("%010d %05d %s \n", $offset, $generation, $state);
    $xrefStreamRow = static fn (int $type, int $generation = 0): string => chr($type) . chr($generation);
    ...
    $staleContentOffset = $addObject(4, 0, "<< /Length " . strlen($staleContent) . " >>\nstream\n{$staleContent}\nendstream");
    $fontOffset = $addObject(5, 0, '<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica /Encoding /WinAnsiEncoding >>');
    $staleInfoOffset = $addObject(6, 0, '<< /Title (Stale Zero Width Offset Info Title) /Author (Stale Zero Width Author) /Producer (Stale Zero Width Producer) >>');
    $staleMetadataOffset = $addObject(7, 0, '<< /Type /Metadata /Subtype /XML /Filter /FlateDecode /Length ' . strlen($staleXmp) . " >>\nstream\n{$staleXmp}\nendstream");
    $staleNameTreeOffset = $addObject(8, 0, '<< /Names [(stale-zero-width-offset.xml) 10 0 R] >>');
    $staleFileSpecOffset = $addObject(10, 0, '<< /Type /Filespec /F (stale-zero-width-offset.xml) /Desc (Stale zero-width offset attachment) /AFRelationship /Source /EF << /F 11 0 R >> >>');
    $staleEmbeddedFileOffset = $addObject(11, 0, '<< /Type /EmbeddedFile /Subtype /text#2Fxml /Length ' . strlen($stalePayload) . " >>\nstream\n{$stalePayload}\nendstream");
    ...
    $addObject(3, 0, '<< /Type /Page /Parent 2 0 R /Resources << /Font << /F1 5 0 R >> >> /Contents 4 0 R >>');
    $addObject(4, 0, "<< /Length " . strlen($currentContent) . " >>\nstream\n{$currentContent}\nendstream");
    $addObject(6, 0, '<< /Title (Current Zero Width Offset Info Title) /Author (Current Zero Width Author) /Producer (Current Zero Width Producer) >>');
    $addObject(7, 0, '<< /Type /Metadata /Subtype /XML /Filter /FlateDecode /Length ' . strlen($currentXmp) . " >>\nstream\n{$currentXmp}\nendstream");
    $addObject(8, 0, '<< /Names [(current-zero-width-offset.xml) 10 0 R] >>');
    $addObject(10, 0, '<< /Type /Filespec /F (current-zero-width-offset.xml) /Desc (Current zero-width offset attachment) /AFRelationship /Source /EF << /F 11 0 R >> >>');
    $addObject(11, 0, '<< /Type /EmbeddedFile /Subtype /text#2Fxml /Length ' . strlen($currentPayload) . " >>\nstream\n{$currentPayload}\nendstream");
    ...
    $compressedCurrentRows = gzcompress($currentRows);
    if (!is_string($compressedCurrentRows)) {
        throw new RuntimeException('Unable to compress zero-width offset xref-stream rows.');
    }

    $currentXrefOffset = strlen($pdf);
    $pdf .= "20 0 obj\n"
        . '<< /Type /XRef /Size 21 /Root 1 0 R /Info 6 0 R /Prev ' . $previousXrefOffset . ' /Index [1 8 10 2] /W [1 0 1] /Filter /FlateDecode /Length ' . strlen($compressedCurrentRows) . " >>\n"
        . "stream\n{$compressedCurrentRows}\nendstream\nendobj\n"
        . "startxref\n{$currentXrefOffset}\n%%EOF";
...
return [
    ...
    'inherits previous trailer Root when latest xref-stream omits Root before unreferenced catalog decoys' => static function (
        TestRunner $t
    ) use ($xrefPrevChainLatestSparseTrailerRootPdf): void {
        $pdf = $xrefPrevChainLatestSparseTrailerRootPdf();
        $metadata = (new PdfMetadataExtractor())->extractDocumentMetadata($pdf);
        $extractor = new PdfTextExtractor();
        $files = (new PdfEmbeddedFileExtractor())->extractEmbeddedFiles($pdf);
        $attachmentSummary = (new PdfAttachmentExtractor())->attachmentSummary($pdf);
        ...
        $t->same(['Current sparse Root page', 'Inherited trailer Root selected'], $extractor->extractTextLines($pdf));
        $t->same("Current sparse Root page\nInherited trailer Root selected", $text);
        $t->same(['xmp', 'info', 'catalog'], $metadata['source']);
        $t->same('Current Sparse Root XMP Title', $metadata['title']);
        $t->same('Latest sparse trailer inherits previous Root', $metadata['description']);
        $t->same('Current Sparse Root Info Title', $metadata['info']['Title']);
        $t->same(['Current Sparse Root Author'], $metadata['authors']);
        $t->same('Current Sparse Root Producer', $metadata['producer']);
        $t->same('en-US', $metadata['language']);
        $t->same(1, count($files));
        $t->same('current-sparse-root.xml', $files[0]['filename']);
        $t->same('Current sparse Root attachment', $files[0]['description']);
        $t->same($currentPayload, $files[0]['content']);
        $t->same(1, $attachmentSummary['attachment_count']);
        $t->same(['current-sparse-root.xml'], $attachmentSummary['filenames']);
        $t->same(strlen($currentPayload), $attachmentSummary['total_bytes']);
        $t->same('current-sparse-root.xml', $attachmentSummary['attachments'][0]['filename']);
        $t->same('Current sparse Root attachment', $attachmentSummary['attachments'][0]['description']);
        $t->same(false, $attachmentSummary['executes_python_or_models']);
        $t->same(false, $attachmentSummary['executes_external_pdf_tools']);
        $t->true(str_contains($pdf, "trailer\n<< /Size 21 /Root 12 0 R /Info 6 0 R >>"));
        $t->true(str_contains($pdf, '<< /Type /XRef /Size 31 /Info 6 0 R /Prev '));
        $t->true(is_string($encodedMetadata) && !str_contains($encodedMetadata, 'Decoy Sparse Root'));
        $t->true(is_string($encodedMetadata) && !str_contains($encodedMetadata, 'Previous Sparse Root'));
        $t->true(is_string($encodedFiles) && !str_contains($encodedFiles, 'decoy-sparse-root'));
        $t->true(is_string($encodedFiles) && !str_contains($encodedFiles, 'previous-sparse-root'));
        $t->true(is_string($encodedAttachmentSummary) && !str_contains($encodedAttachmentSummary, 'decoy-sparse-root'));
        $t->true(is_string($encodedAttachmentSummary) && !str_contains($encodedAttachmentSummary, 'previous-sparse-root'));
        $t->true(!str_contains($text, 'Unreferenced sparse Root decoy page'));
        $t->true(!str_contains($text, 'Previous sparse Root page'));
        $t->true(!str_contains($text, "\0"));
    },
    ...
    'suppresses previous metadata and attachments when latest xref-stream rows point at stale wrong owners' => static function (
        TestRunner $t
    ) use ($xrefPrevChainLatestMalformedRowsSuppressPrevPdf): void {
        $pdf = $xrefPrevChainLatestMalformedRowsSuppressPrevPdf();
        $metadata = (new PdfMetadataExtractor())->extractDocumentMetadata($pdf);
        $extractor = new PdfTextExtractor();
        $files = (new PdfEmbeddedFileExtractor())->extractEmbeddedFiles($pdf);
        $attachmentSummary = (new PdfAttachmentExtractor())->attachmentSummary($pdf);
        ...
        $t->same(['Current wrong-owner row page', 'Malformed rows suppress Prev'], $extractor->extractTextLines($pdf));
        $t->same("Current wrong-owner row page\nMalformed rows suppress Prev", $text);
        $t->same(['catalog'], $metadata['source']);
        $t->same([], $metadata['info']);
        $t->same('en-US', $metadata['language']);
        $t->same([], $files);
        ...
        $t->true(str_contains($pdf, '/Prev '));
        $t->true(str_contains($pdf, '/Index [1 4 6 3 10 2]'));
        $t->true(str_contains($pdf, '/Info 6 0 R'));
        $t->true(str_contains($pdf, '/Metadata 7 0 R'));
        $t->true(str_contains($pdf, '/EmbeddedFiles 8 0 R'));
        $t->true(is_string($encodedMetadata) && !str_contains($encodedMetadata, 'Stale Wrong Owner Row'));
        $t->true(is_string($encodedFiles) && !str_contains($encodedFiles, 'stale-wrong-owner-row'));
        $t->true(is_string($encodedSummary) && !str_contains($encodedSummary, 'stale-wrong-owner-row'));
        $t->true(!isset($metadata['title']));
        $t->true(!isset($metadata['authors']));
        $t->true(!isset($metadata['producer']));
        $t->true(!isset($metadata['embedded_files']));
        $t->true(!str_contains($text, 'Stale wrong-owner row Prev page'));
        $t->true(!str_contains($text, "\0"));
    },
    'repairs current type-one xref-stream rows with zero-width offset fields before stale Prev rows' => static function (
        TestRunner $t
    ) use ($xrefPrevChainZeroWidthCurrentOffsetRowsPdf): void {
        $pdf = $xrefPrevChainZeroWidthCurrentOffsetRowsPdf();
        $metadata = (new PdfMetadataExtractor())->extractDocumentMetadata($pdf);
        $extractor = new PdfTextExtractor();
        $files = (new PdfEmbeddedFileExtractor())->extractEmbeddedFiles($pdf);
        $summary = (new PdfAttachmentExtractor())->attachmentSummary($pdf);
        ...
        $t->same(['Current zero-width offset page', 'Omitted offset fields repaired'], $extractor->extractTextLines($pdf));
        $t->same("Current zero-width offset page\nOmitted offset fields repaired", $text);
        $t->same(['xmp', 'info', 'catalog'], $metadata['source']);
        $t->same('Current Zero Width Offset XMP Title', $metadata['title']);
        $t->same('Current type-one rows with omitted offsets selected', $metadata['description']);
        $t->same('Current Zero Width Offset Info Title', $metadata['info']['Title']);
        $t->same(['Current Zero Width Author'], $metadata['authors']);
        $t->same('Current Zero Width Producer', $metadata['producer']);
        $t->same('en-US', $metadata['language']);
        $t->same(1, count($files));
        $t->same('current-zero-width-offset.xml', $files[0]['filename']);
        $t->same('Current zero-width offset attachment', $files[0]['description']);
        $t->same($currentPayload, $files[0]['content']);
        $t->same(1, $summary['attachment_count']);
        $t->same(['current-zero-width-offset.xml'], $summary['filenames']);
        $t->same(strlen($currentPayload), $summary['total_bytes']);
        $t->same('current-zero-width-offset.xml', $summary['attachments'][0]['filename'] ?? null);
        $t->same('Current zero-width offset attachment', $summary['attachments'][0]['description'] ?? null);
        $t->same(false, $summary['executes_python_or_models']);
        $t->same(false, $summary['executes_external_pdf_tools']);
        $t->true(str_contains($pdf, '/W [1 0 1]'));
        $t->true(str_contains($pdf, '/Index [1 8 10 2]'));
        $t->true(is_string($encodedMetadata) && !str_contains($encodedMetadata, 'Stale Zero Width'));
        $t->true(is_string($encodedFiles) && !str_contains($encodedFiles, 'stale-zero-width-offset'));
        $t->true(is_string($encodedSummary) && !str_contains($encodedSummary, 'stale-zero-width-offset'));
        $t->true(!str_contains($text, 'Stale zero-width offset Prev page'));
        $t->true(!str_contains($text, "\0"));
    },
    'blocks previous trailer root fallback when latest xref-stream frees inherited catalog' => static function (
        TestRunner $t
    ) use ($xrefPrevChainLatestRootFreeSuppressPrevPdf): void {
        $pdf = $xrefPrevChainLatestRootFreeSuppressPrevPdf();
        $metadata = (new PdfMetadataExtractor())->extractDocumentMetadata($pdf);
        $extractor = new PdfTextExtractor();
        $files = (new PdfEmbeddedFileExtractor())->extractEmbeddedFiles($pdf);
        $text = $extractor->extractPlainText($pdf);
        ...
        $t->same('Carried Root Free Info Title', $metadata['title']);
        $t->same([], $metadata['xmp']);
        $t->same([], $files);
        $t->true(str_contains($pdf, '/Prev '));
        $t->true(str_contains($pdf, '/Index [1 1]'));
        $t->true(!str_contains(substr($pdf, (int) strrpos($pdf, '/Type /XRef')), '/Root '));
        $t->true(is_string($encodedMetadata) && !str_contains($encodedMetadata, 'Stale Root Free XMP Title'));
        $t->true(is_string($encodedMetadata) && !str_contains($encodedMetadata, 'Previous root metadata must not be revived'));
        $t->true(is_string($encodedFiles) && !str_contains($encodedFiles, 'stale-root-free-prev'));
        $t->true(!isset($metadata['catalog']));
        $t->true(!isset($metadata['language']));
        $t->true(!isset($metadata['embedded_files']));
        $t->true(!str_contains($text, 'Stale inherited root free row page'));
        $t->true(!str_contains($text, "\0"));
    },
    'blocks previous trailer root fallback when latest xref-stream frees inherited catalog through damaged Prev repair' => static function (
        TestRunner $t
    ) use ($xrefPrevChainDamagedPrevRootFreeSuppressPrevPdf): void {
        $pdf = $xrefPrevChainDamagedPrevRootFreeSuppressPrevPdf();
        $metadata = (new PdfMetadataExtractor())->extractDocumentMetadata($pdf);
        $extractor = new PdfTextExtractor();
        $files = (new PdfEmbeddedFileExtractor())->extractEmbeddedFiles($pdf);
        $text = $extractor->extractPlainText($pdf);
        ...
        $t->same('', $text);
        $t->same(['info'], $metadata['source']);
        $t->same('Carried Damaged Root Free Info Title', $metadata['title']);
        $t->same([], $metadata['xmp']);
        $t->same([], $files);
        $t->true(str_contains($pdf, '/Prev '));
        $t->true(str_contains($pdf, '/Index [1 1]'));
        $t->true(!str_contains(substr($pdf, (int) strrpos($pdf, '/Type /XRef')), '/Root '));
        $t->true(is_string($encodedMetadata) && !str_contains($encodedMetadata, 'Stale Damaged Root Free XMP Title'));
        $t->true(is_string($encodedMetadata) && !str_contains($encodedMetadata, 'Damaged Prev root metadata must not be revived'));
        $t->true(is_string($encodedFiles) && !str_contains($encodedFiles, 'stale-damaged-root-free-prev'));
        $t->true(!isset($metadata['catalog']));
        $t->true(!isset($metadata['language']));
        $t->true(!isset($metadata['embedded_files']));
        $t->true(!str_contains($text, 'Stale damaged root free Prev page'));
        $t->true(!str_contains($text, "\0"));
    },
    'blocks previous trailer root fallback when latest classic table frees inherited catalog through direct Prev helper' => static function (
        TestRunner $t
    ) use ($xrefPrevChainClassicTableDirectPrevRootFreeSuppressPrevPdf): void {
        $pdf = $xrefPrevChainClassicTableDirectPrevRootFreeSuppressPrevPdf();
        $metadata = (new PdfMetadataExtractor())->extractDocumentMetadata($pdf);
        $extractor = new PdfTextExtractor();
        $files = (new PdfEmbeddedFileExtractor())->extractEmbeddedFiles($pdf);
        $text = $extractor->extractPlainText($pdf);
        ...
        $t->same('', $text);
        $t->same(['info'], $metadata['source']);
        $t->same('Carried Classic Root Free Info Title', $metadata['title']);
        $t->same([], $metadata['xmp']);
        $t->same([], $files);
        $t->true(str_contains($pdf, '/Prev 30 0 R'));
        $t->true(str_contains($pdf, 'post-xref classic root-free Prev helper decoy'));
        $t->true(str_contains($pdf, "trailer\n<< /Size 31 /Info 6 0 R /Prev 30 0 R >>"));
        $t->true(is_string($encodedMetadata) && !str_contains($encodedMetadata, 'Stale Classic Root Free XMP Title'));
        $t->true(is_string($encodedMetadata) && !str_contains($encodedMetadata, 'Classic table direct Prev root metadata must not be revived'));
        $t->true(is_string($encodedFiles) && !str_contains($encodedFiles, 'stale-classic-root-free-prev'));
        $t->true(!isset($metadata['catalog']));
        $t->true(!isset($metadata['language']));
        $t->true(!isset($metadata['embedded_files']));
