...
$inlineImageBareTailPdf = static function (string $content): string {
    return "%PDF-1.4\n"
...
return [
    'keeps bare-word inline image dictionary tails closed before WordPress text import' => static function (
        TestRunner $t
    ) use ($inlineImageBareTailPdf): void {
        $extractor = new PdfTextExtractor();
        $renderer = new PdfImageRenderer();
        $dictionary = '/W 1 /H 1 BadTail /CS /G /BPC 8 /D [1 0] /F /MalformedPreview';
        $payload = "\x7f EI BT /F1 12 Tf 72 690 Td (Bare Tail Inline Noise) Tj ET rawtail";
        $content = "BT /F1 12 Tf 72 720 Td (Before Bare Tail Inline) Tj ET\n"
            . "BI {$dictionary} ID\n{$payload}\nEI\n"
            . "BT /F1 12 Tf 72 704 Td (After Bare Tail Inline) Tj ET";
        $pdf = $inlineImageBareTailPdf($content);

        $expected = [
            'Before Bare Tail Inline',
            'After Bare Tail Inline',
        ];
        ...
        $t->true(!str_contains($plainText, 'Bare Tail Inline Noise'));
