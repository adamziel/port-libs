...
return [
    'fails closed on duplicate top-level DCTDecode DecodeParms declarations before RGB preview planning' => static function (TestRunner $t): void {
        $renderer = new PdfImageRenderer();
        $extractor = new PdfTextExtractor();
        $before = 'BT /F1 12 Tf 72 720 Td (Before duplicate DCT DecodeParms declaration) Tj ET';
        $after = 'BT /F1 12 Tf 72 680 Td (After duplicate DCT DecodeParms declaration) Tj ET';
        $jpegPayload = "\xff\xd8\xff\xe0\x00\x10JFIF\0duplicate-decodeparms declaration bytes "
            . 'BT /F1 12 Tf 72 700 Td (Duplicate DCT DecodeParms declaration payload leak) Tj ET'
            . "\xff\xd9";
        ...
        $t->contains('duplicate_dctdecode_decodeparms_declaration_fail_closed', implode(',', $plan['notes']));
        ...
        $t->same([
            'duplicate_dctdecode_decodeparms_declaration_fail_closed',
            'render_rgb_preview_from_cmyk',
        ], $colorPlan['notes']);

        $t->same(['Before duplicate DCT DecodeParms declaration', 'After duplicate DCT DecodeParms declaration'], $extractor->extractTextLines($pdf));
        $t->same("Before duplicate DCT DecodeParms declaration\nAfter duplicate DCT DecodeParms declaration", $plainText);
        $t->true(!str_contains($plainText, 'Duplicate DCT DecodeParms declaration payload leak'));
