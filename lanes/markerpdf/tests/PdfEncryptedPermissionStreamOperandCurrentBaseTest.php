...
$streamPermissionWordPdf = static function () use ($hex): array {
    $content = 'BT /F1 12 Tf 72 720 Td (Stream permission encrypted text leak) Tj ET';
    ...
    return [$pdf, $content, $ownerValidation, $userValidation, $streamPayload];
};

return [
    'fails closed when Standard permission word resolves to a stream object' => static function (
        TestRunner $t
    ) use ($streamPermissionWordPdf): void {
        [$pdf, $content, $ownerValidation, $userValidation, $streamPayload] = $streamPermissionWordPdf();
        ...
        $t->same(['encrypted_document', 'encrypted_text_extraction_blocked', 'permission_word_stream_operand'], $report['review_reasons']);
        ...
        $t->same(['permission_word_stream_operand_review'], $declaration['entry_statuses']);
        $t->same(['permission_word_stream_operand_review'], $declaration['malformed_entry_statuses']);
        $t->same('permission_word_stream_operand_review', $declaration['selected_entry_status']);
        $t->same(true, $declaration['selected_entry_resolved']);
        $t->same(false, $declaration['selected_entry_integer']);
        $t->same('dictionary', $declaration['selected_entry_operand_shape']);
        $t->same('indirect_reference', $declaration['selected_entry_raw_operand_shape']);
        $t->same(true, $declaration['selected_entry_stream_container']);
        ...
        $t->same('permission_word_stream_operand_review', $entry['status'] ?? null);
        $t->same('dictionary', $entry['operand_shape'] ?? null);
        ...
        $t->same(true, $entry['stream_container'] ?? null);
        ...
        $t->same(0, $entry['resolved_generation'] ?? null);
