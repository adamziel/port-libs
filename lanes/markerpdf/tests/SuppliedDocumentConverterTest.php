...
return [
    ...
    'normalizes signed numeric page markers before supplied layout and order alignment' => static function (TestRunner $t) use ($pdftextPage): void {
        $path = sys_get_temp_dir() . '/markerpdf-supplied-layout-order-signed-page-marker-' . bin2hex(random_bytes(4)) . '.pdf';
        file_put_contents($path, "%PDF-1.4\n% supplied signed page-marker layout order boundary\n%%EOF");
        try {
            $coverPage = $pdftextPage(950, [
                ['text' => 'Signed keyed cover page artifact.', 'bbox' => [72.0, 80.0, 300.0, 94.0]],
            ]);
            $selectedPage = $pdftextPage(951, [
                ['text' => 'Second signed marker column carries review notes.', 'bbox' => [330.0, 112.0, 560.0, 128.0]],
                ['text' => 'First signed marker column starts the import.', 'bbox' => [72.0, 112.0, 280.0, 128.0]],
            ]);
            $appendixPage = $pdftextPage(952, [
                ['text' => 'Signed keyed appendix page artifact.', 'bbox' => [72.0, 80.0, 320.0, 94.0]],
            ]);
            ...
            $t->same(1, $result['metadata']['order_plan']['order_result_count']);
            $t->same(1, $result['metadata']['order_plan']['assigned_pages']);
            $t->contains('First signed marker column starts the import.', $text);
            $t->contains('Second signed marker column carries review notes.', $text);
            $t->true(strpos($text, 'First signed marker column starts the import.') < strpos($text, 'Second signed marker column carries review notes.'));
            $t->true(!str_contains($text, 'Signed keyed cover page artifact.'));
            $t->true(!str_contains($text, 'Signed keyed appendix page artifact.'));
        } finally {
            unlink($path);
        }
    },
    'prefers exact page markers over one-based page_number collisions in supplied layout and order artifacts' => static function (TestRunner $t) use ($pdftextPage): void {
        ...
        try {
            $coverPage = $pdftextPage(400, [
                ['text' => 'Marker precedence cover page artifact.', 'bbox' => [72.0, 80.0, 300.0, 94.0]],
            ]);
            $selectedPage = $pdftextPage(401, [
                ['text' => 'Second exact supplied marker column carries review notes.', 'bbox' => [330.0, 112.0, 560.0, 128.0]],
                ['text' => 'First exact supplied marker column starts the import.', 'bbox' => [72.0, 112.0, 280.0, 128.0]],
            ]);
            ...
            $result = (new SuppliedDocumentConverter())->convert(
                ...
                [
                    ...
                    'start_page' => 1,
                    'lowres_images' => [
                        ['page_number' => 402, 'image' => 'one-based-collision-layout-render'],
                        ['page' => 401, 'image' => 'exact-page-layout-render'],
                    ],
                    'layout_results' => [
                        [
                            'page_number' => 402,
                    ...
                    'order_images' => [
                        ['page_number' => 402, 'image' => 'one-based-collision-order-render'],
                        ['page' => 401, 'image' => 'exact-page-order-render'],
                    ],
                    'order_results' => [
                        [
                            'page_number' => 402,
            ...
            $t->same(1, $result['metadata']['order_plan']['order_result_count']);
            $t->same(1, $result['metadata']['order_plan']['assigned_pages']);
            $t->contains('First exact supplied marker column starts the import.', $text);
            $t->contains('Second exact supplied marker column carries review notes.', $text);
            $t->true(strpos($text, 'First exact supplied marker column starts the import.') < strpos($text, 'Second exact supplied marker column carries review notes.'));
            $t->true(!str_contains($text, 'Marker precedence cover page artifact.'));
        } finally {
            unlink($path);
        }
    },
    'ignores selected-count keyed layout and order artifacts for unselected pdftext pages' => static function (TestRunner $t) use ($pdftextPage): void {
        ...
        try {
            $coverPage = $pdftextPage(50, [
                ['text' => 'Mismatched keyed cover page artifact.', 'bbox' => [72.0, 80.0, 280.0, 94.0]],
            ]);
            $selectedPage = $pdftextPage(51, [
                ['text' => 'Second selected mismatch column remains source ordered.', 'bbox' => [330.0, 112.0, 560.0, 128.0]],
                ['text' => 'First selected mismatch column lacks matching order.', 'bbox' => [72.0, 112.0, 280.0, 128.0]],
            ]);
            ...
            $result = (new SuppliedDocumentConverter())->convert(
                ...
                [
                    ...
                    'start_page' => 1,
                    'lowres_images' => [
                        ['page' => 50, 'image' => 'mismatched-cover-layout-render'],
                    ],
                    'layout_results' => [[
                        'page' => 50,
                        'image_bbox' => [0.0, 0.0, 612.0, 792.0],
                    ...
                    'order_images' => [
                        ['page' => 50, 'image' => 'mismatched-cover-order-render'],
                    ],
                    'order_results' => [[
                        'page' => 50,
                        'image_bbox' => [0.0, 0.0, 612.0, 792.0],
            ...
            $t->contains('Second selected mismatch column remains source ordered.', $result['text']);
            $t->contains('First selected mismatch column lacks matching order.', $result['text']);
            $t->true(strpos($result['text'], 'Second selected mismatch column remains source ordered.') < strpos($result['text'], 'First selected mismatch column lacks matching order.'));
            $t->true(!str_contains($result['text'], 'Mismatched keyed cover page artifact.'));
        } finally {
            unlink($path);
        }
    },
    ...
    'rejects layout and order artifacts with conflicting source and page identity markers' => static function (TestRunner $t) use ($pdftextPage): void {
        $path = sys_get_temp_dir() . '/markerpdf-supplied-layout-order-conflicting-identity-' . bin2hex(random_bytes(4)) . '.pdf';
        file_put_contents($path, "%PDF-1.4\n% supplied conflicting layout order identity boundary\n%%EOF");
        try {
            $coverPage = $pdftextPage(90, [
                ['text' => 'Conflicting identity cover page artifact.', 'bbox' => [72.0, 80.0, 330.0, 94.0]],
            ]);
            $selectedPage = $pdftextPage(91, [
            ...
            $t->true(!array_key_exists('layout_plan', $result['metadata']));
            $t->true(!array_key_exists('order_plan', $result['metadata']));
            $t->contains('Second conflicting selected column remains source ordered.', $result['text']);
            $t->contains('First conflicting selected column lacks trusted order.', $result['text']);
            $t->true(strpos($result['text'], 'Second conflicting selected column remains source ordered.') < strpos($result['text'], 'First conflicting selected column lacks trusted order.'));
            $t->true(!str_contains($result['text'], 'Conflicting identity cover page artifact.'));
        } finally {
            unlink($path);
        }
    },
