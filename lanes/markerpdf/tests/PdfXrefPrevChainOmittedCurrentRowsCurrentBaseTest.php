...
$xrefPrevChainOmittedCurrentRowsXmp = static function (string $title, string $description): string {
...
$xrefPrevChainOmittedCurrentRowsPdf = static function () use ($xrefPrevChainOmittedCurrentRowsXmp): string {
    ...
    $staleXmp = gzcompress($xrefPrevChainOmittedCurrentRowsXmp(
        'Stale Omitted Row XMP Title',
        'Previous xref rows must not own latest trailer graph'
    ));
    $currentXmp = gzcompress($xrefPrevChainOmittedCurrentRowsXmp(
        'Current Omitted Row XMP Title',
        'Same-generation omitted current rows recovered'
    ));
...
return [
    'repairs same-generation omitted current rows from latest trailer graph before stale Prev rows' => static function (
        TestRunner $t
    ) use ($xrefPrevChainOmittedCurrentRowsPdf): void {
        $pdf = $xrefPrevChainOmittedCurrentRowsPdf();
        ...
        $t->same(['Current omitted-row page', 'Trailer graph repaired'], $extractor->extractTextLines($pdf));
        $t->same("Current omitted-row page\nTrailer graph repaired", $text);
        $t->same(['xmp', 'info', 'catalog'], $metadata['source']);
        $t->same('Current Omitted Row XMP Title', $metadata['title']);
        $t->same('Same-generation omitted current rows recovered', $metadata['description']);
        $t->same('Current Omitted Row Info Title', $metadata['info']['Title']);
        $t->same(['Current Omitted Author'], $metadata['authors']);
        $t->same('Current Omitted Producer', $metadata['producer']);
        $t->same('en-US', $metadata['language']);
        $t->same(1, count($files));
        $t->same('current-omitted-row.xml', $files[0]['filename']);
        $t->same('Current omitted row attachment', $files[0]['description']);
        $t->same($currentPayload, $files[0]['content']);
        $t->same(1, $summary['attachment_count']);
        $t->same(['current-omitted-row.xml'], $summary['filenames']);
        $t->same(strlen($currentPayload), $summary['total_bytes']);
        $t->same(false, $summary['executes_python_or_models']);
        $t->same(false, $summary['executes_external_pdf_tools']);
        $t->true(is_string($encodedMetadata) && !str_contains($encodedMetadata, 'Stale Omitted'));
        $t->true(is_string($encodedFiles) && !str_contains($encodedFiles, 'stale-omitted'));
        $t->true(is_string($encodedSummary) && !str_contains($encodedSummary, 'stale-omitted'));
        $t->true(!str_contains($text, 'Stale omitted-row Prev page'));
        $t->true(str_contains($pdf, '/Index [5 1]'));
