...
$type3CharProcsTextStateBoundaryCurrentBasePdf = static function (): string {
...
return [
    'accepts non-painting Type3 CharProc text-state setup before metrics on current base' => static function (TestRunner $t) use ($type3CharProcsTextStateBoundaryCurrentBasePdf): void {
        $extractor = new PdfTextExtractor();
        $pdf = $type3CharProcsTextStateBoundaryCurrentBasePdf();
        $plainText = $extractor->extractPlainText($pdf);

        $t->same(['WideBlock', 'Thin Text', 'Guard Gap'], $extractor->extractTextLines($pdf));
        $t->same(['Wide', 'Block', 'Thin', 'Text', 'Guard', 'Gap'], $extractor->extractTextRuns($pdf));
        $t->same("WideBlock\nThin Text\nGuard Gap", $plainText);
        $t->same("WideBlock\nThin Text\nGuard Gap\n", $extractor->naiveGetText($pdf));
        $t->true(str_contains($plainText, 'WideBlock'));
        $t->true(str_contains($plainText, 'Thin Text'));
        $t->true(str_contains($plainText, 'Guard Gap'));
        $t->true(!str_contains($plainText, 'Wide Block'));
        $t->true(!str_contains($plainText, 'ThinText'));
        $t->true(!str_contains($plainText, 'GuardGap'));
        $t->true(!str_contains($plainText, 'text-state charproc text leak'));
        $t->true(!str_contains($plainText, 'bad text-state operand'));
    },
];
