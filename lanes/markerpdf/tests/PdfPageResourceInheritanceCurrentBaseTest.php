...
$pageResourceEntryWrapperCurrentBasePdf = static function () use ($pageResourceInheritanceCurrentBaseCMap): string {
    $content = 'BT /Fwrapped 12 Tf 72 720 Td <41> Tj T* /Span /WrappedActual BDC <42> Tj EMC ET q /WrappedForm Do Q';
    $formContent = 'BT /Fwrapped 12 Tf 12 24 Td <43> Tj ET';
    $cmap = $pageResourceInheritanceCurrentBaseCMap([
...
$pageResourceStructPropertyWrapperCurrentBasePdf = static function (): string {
    $content = 'BT /F1 12 Tf '
...
$pageResourceObjectTrailingTokenCurrentBasePdf = static function () use ($pageResourceInheritanceCurrentBaseCMap): string {
    $content = 'BT /F1 12 Tf 72 720 Td <41> Tj ET q /TailForm Do Q';
    $form = 'BT /F1 12 Tf 12 24 Td (Trailing token resource form leak) Tj ET';
    $cmap = $pageResourceInheritanceCurrentBaseCMap([
        '41' => 'Trailing token resource font leak',
    ]);
...
return [
    ...
    'resolves inherited resource entry wrappers before text and form lookup' => static function (TestRunner $t) use ($pageResourceEntryWrapperCurrentBasePdf): void {
        $pdf = $pageResourceEntryWrapperCurrentBasePdf();
        $extractor = new PdfTextExtractor();
        $plainText = $extractor->extractPlainText($pdf);
        $boundary = (new PdfPagePropertyExtractor())->extractPageBoundaryMetadata($pdf);
        $resourceMetadata = $boundary[0]['resources'] ?? [];
    ...
    'uses inherited wrapped Properties entries for StructTree MCID reading order' => static function (TestRunner $t) use ($pageResourceStructPropertyWrapperCurrentBasePdf): void {
        $pdf = $pageResourceStructPropertyWrapperCurrentBasePdf();
        $extractor = new PdfTextExtractor();
        $plainText = $extractor->extractPlainText($pdf);
        $boundary = (new PdfPagePropertyExtractor())->extractPageBoundaryMetadata($pdf);
        $resources = $boundary[0]['resources'] ?? [];
        ...
        $t->same(['F1'], $resources['font_names'] ?? null);
        $t->same(['TitleProp', 'BodyProp'], $resources['properties_names'] ?? null);
        $t->same([0, 1], $boundary[0]['parent_tree']['mcids'] ?? null);
        $t->same(false, str_contains($plainText, 'TitleProp'));
    ...
    'fails closed on indirect page Resources dictionary objects with trailing tokens' => static function (TestRunner $t) use ($pageResourceObjectTrailingTokenCurrentBasePdf): void {
        $pdf = $pageResourceObjectTrailingTokenCurrentBasePdf();
        $extractor = new PdfTextExtractor();
        $plainText = $extractor->extractPlainText($pdf);
        $boundary = (new PdfPagePropertyExtractor())->extractPageBoundaryMetadata($pdf);
        $resources = $boundary[0]['resources'] ?? [];
