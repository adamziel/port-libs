...
$xrefClassicUnterminatedLiteralBoundaryCurrentBasePdf = static function (): array {
    $currentXmp = '<x:xmpmeta xmlns:x="adobe:ns:meta/">'
        . '<rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#">'
        . '<rdf:Description rdf:about="" xmlns:dc="http://purl.org/dc/elements/1.1/">'
        . '<dc:title><rdf:Alt><rdf:li xml:lang="x-default">Current Unterminated Literal XRef Title</rdf:li></rdf:Alt></dc:title>'
        . '</rdf:Description></rdf:RDF></x:xmpmeta>';
    $decoyXmp = '<x:xmpmeta xmlns:x="adobe:ns:meta/">'
        . '<rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#">'
        . '<rdf:Description rdf:about="" xmlns:dc="http://purl.org/dc/elements/1.1/">'
        . '<dc:title><rdf:Alt><rdf:li xml:lang="x-default">Unterminated Literal XRef Decoy Title</rdf:li></rdf:Alt></dc:title>'
        . '</rdf:Description></rdf:RDF></x:xmpmeta>';
    ...
    return [$pdf, $currentPayload, strtolower($currentChecksum), $currentXrefOffset, $unterminatedLiteralXrefOffset];
};

return [
    'skips unterminated literal-string xref decoys before WordPress imports' => static function (
        TestRunner $t
    ) use ($xrefClassicUnterminatedLiteralBoundaryCurrentBasePdf): void {
        [$pdf, $currentPayload, $currentChecksum, $currentXrefOffset, $unterminatedLiteralXrefOffset] = $xrefClassicUnterminatedLiteralBoundaryCurrentBasePdf();
        ...
        $t->true($unterminatedLiteralXrefOffset > $currentXrefOffset);
        $t->same(['Current unterminated literal xref page', 'Unclosed string xref skipped'], $extractor->extractTextLines($pdf));
        $t->same(['Current unterminated literal xref page', 'Unclosed string xref skipped'], $extractor->extractTextRuns($pdf));
        $t->same("Current unterminated literal xref page\nUnclosed string xref skipped", $text);
        $t->same("Current unterminated literal xref page\nUnclosed string xref skipped\n", $extractor->naiveGetText($pdf));
        $t->same(1, $extractor->extractOutlineMetadata($pdf)['pages']);
        $t->same('Current Unterminated Literal XRef Title', $metadata['title']);
        $t->same('Current Unterminated Literal XRef Info', $metadata['info']['Title']);
        $t->same('Current Literal Importer', $metadata['info']['Author']);
        $t->same(1, count($files));
        $t->same('current-unterminated-literal-xref.xml', $files[0]['name']);
        $t->same('current-unterminated-literal-xref.xml', $files[0]['filename']);
        $t->same('Current unterminated literal xref attachment', $files[0]['description']);
        ...
        $t->same(['current-unterminated-literal-xref.xml'], $attachmentSummary['filenames']);
        ...
        $t->true(!str_contains($text, 'Unterminated literal xref decoy page'));
        $t->true(!str_contains($text, 'Unclosed string xref leak'));
        $t->true(!str_contains($encodedMetadata, 'Unterminated Literal XRef Decoy'));
        $t->true(!str_contains($encodedFiles, 'decoy-unterminated-literal-xref'));
        $t->true(!str_contains($encodedAttachmentSummary, 'decoy-unterminated-literal-xref'));
