...
$xrefClassicRebuildTrailerCommentDelimiterFreeMapCurrentBasePdf = static function (): array {
...
return [
    'accepts trailer comment delimiters while rebuilding the free-object map before annotation review' => static function (
        TestRunner $t
    ) use ($xrefClassicRebuildTrailerCommentDelimiterFreeMapCurrentBasePdf): void {
        [$pdf, $previousXrefOffset, $currentXrefOffset] = $xrefClassicRebuildTrailerCommentDelimiterFreeMapCurrentBasePdf();
        ...
        $t->true(str_contains($pdf, "trailer% current trailer comment delimiter\n<< /Size 8"));
        $t->same(['Current trailer-comment free-map page'], $textExtractor->extractTextLines($pdf));
        $t->true(isset($freeObjects[7]), 'The comment-delimited current trailer must keep object 7 free.');
        $t->same(true, $freeObjects[7] ?? null);
        $t->same([], $links, 'The stale trailer-comment annotation URI must remain suppressed.');
        $t->same([], $annotations, 'The stale trailer-comment annotation review metadata must remain suppressed.');
        ...
        $pages = [[
            ...
            'blocks' => [[
                ...
                'lines' => [[
                    'bbox' => [72.0, 700.0, 380.0, 718.0],
                    'spans' => [[
                        'text' => 'Current trailer-comment free-map page',
                        'bbox' => [72.0, 700.0, 380.0, 718.0],
                        'font' => 'Helvetica',
                    ]],
                ]],
        ...
        $t->true(!str_contains($encodedReview, 'stale.example.com/trailer-comment-free-map'));
        $t->true(!str_contains($encodedReview, 'Stale trailer-comment free annotation'));
        $t->true(!str_contains($textExtractor->extractPlainText($pdf), "\0"));
    },
];
