...
$xrefPrevChainActionReviewOmittedRowsPdf = static function (): string {
    ...
    $staleContentOffset = $addObject(4, 0, "<< /Length " . strlen($staleContent) . " >>\nstream\n{$staleContent}\nendstream");
    $fontOffset = $addObject(5, 0, '<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica /Encoding /WinAnsiEncoding >>');
    $staleAnnotOffset = $addObject(7, 0, '<< /Type /Annot /Subtype /Link /Rect [72 700 252 718] /F 4 /Contents (Omitted action row link) /A 8 0 R /AA << /E 9 0 R >> >>');
    $staleActionOffset = $addObject(8, 0, '<< /S /URI /URI (https://example.com/stale-omitted-action-row) >>');
    $staleAdditionalActionOffset = $addObject(9, 0, '<< /S /JavaScript /JS (staleOmittedActionRow()) >>');
    ...
    $currentPageOffset = $addObject(3, 0, '<< /Type /Page /Parent 2 0 R /Resources << /Font << /F1 5 0 R >> >> /Annots [7 0 R] /Contents 4 0 R >>');
    $currentContentOffset = $addObject(4, 0, "<< /Length " . strlen($currentContent) . " >>\nstream\n{$currentContent}\nendstream");
    $currentAnnotOffset = $addObject(7, 0, '<< /Type /Annot /Subtype /Link /Rect [72 700 252 718] /F 4 /Contents (Omitted action row link) /A 8 0 R /AA << /E 9 0 R >> >>');
    ...
    $compressedCurrentRows = gzcompress($currentRows);
    if (!is_string($compressedCurrentRows)) {
        throw new RuntimeException('Unable to compress omitted action-row xref-stream rows.');
    }

    $currentXrefOffset = strlen($pdf);
    $pdf .= "20 0 obj\n"
        . '<< /Type /XRef /Size 21 /Root 1 0 R /Prev ' . $previousXrefOffset . ' /Index [1 5 7 1] /W [1 4 1] /Filter /FlateDecode /Length ' . strlen($compressedCurrentRows) . " >>\n"
        . "stream\n{$compressedCurrentRows}\nendstream\nendobj\n"
        . "startxref\n{$currentXrefOffset}\n%%EOF";
...
return [
    ...
    'repairs omitted current action rows before stale Prev action inheritance' => static function (
        TestRunner $t
    ) use ($xrefPrevChainActionReviewOmittedRowsPdf, $xrefPrevChainActionReviewPages): void {
        $pdf = $xrefPrevChainActionReviewOmittedRowsPdf();

        $annotationPages = (new PdfAnnotationExtractor())->extractPageAnnotations($pdf);
        $t->same(1, count($annotationPages));
        $annotation = $annotationPages[0]['annotations'][0];
        $t->same(7, $annotation['annotation_object']);
        $t->same('https://example.com/current-omitted-action-row', $annotation['actions'][0]['uri']);
        $t->same('mailto:current-omitted-action@example.test', $annotation['additional_actions'][0]['uri']);
        ...
        $t->same(1, count($links[0]['links']));
        $t->same(7, $links[0]['links'][0]['annotation_object']);
        $t->same('https://example.com/current-omitted-action-row', $links[0]['links'][0]['uri']);
        $t->same('mailto:current-omitted-action@example.test', $links[0]['links'][0]['additional_actions'][0]['uri']);

        $linkedPages = $linkExtractor->applyLinksToPages($xrefPrevChainActionReviewPages(), $pdf);
        $span = $linkedPages[0]['blocks'][0]['lines'][0]['spans'][0];
        $t->same('https://example.com/current-omitted-action-row', $span['link_uri']);
        $t->same('mailto:current-omitted-action@example.test', $span['link_additional_actions_review'][0]['uri']);

        $blocks = (new MarkdownPostProcessor())->mergeBlocks((new MarkdownPostProcessor())->mergeSpans($linkedPages));
        $t->same('[Current action docs](https://example.com/current-omitted-action-row)', $blocks[0]['text']);

        $encoded = json_encode([$annotationPages, $links, $linkedPages], JSON_UNESCAPED_SLASHES) ?: '';
        $t->true(str_contains($pdf, '/Index [1 5 7 1]'));
        $t->true(!str_contains($encoded, 'stale-omitted-action-row'));
        $t->true(!str_contains($encoded, 'staleOmittedActionRow'));
    },
