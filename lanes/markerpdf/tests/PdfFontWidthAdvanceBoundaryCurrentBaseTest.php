...
$fontWidthHugeFiniteFontSizeBoundaryCurrentBasePdf = static function (): string {
...
$fontWidthHugeFiniteHorizontalScaleBoundaryCurrentBasePdf = static function (): string {
...
$fontWidthIndirectCidArrayTailAdvanceBoundaryCurrentBasePdf = static function (): string {
    $toUnicode = "/CIDInit /ProcSet findresource begin\n"
        . "12 dict begin\n"
        . "begincmap\n"
        . "1 begincodespacerange\n"
...
$fontWidthIndirectW2ArrayTailAdvanceBoundaryCurrentBasePdf = static function (): string {
    $encodingCMap = "/CIDInit /ProcSet findresource begin\n"
        . "12 dict begin\n"
        . "begincmap\n"
        . "/WMode 1 def\n"
    ...
    $content = 'BT /Fvtail 12 Tf 1 0 0 1 72 720 Tm <0001000200030004> Tj 0 -24 Td <00050006000700080009000A> Tj ET';

    return "%PDF-1.4\n"
        . "1 0 obj\n<< /Type /Page /Resources << /Font << /Fvtail 2 0 R >> >> /Contents 5 0 R >>\nendobj\n"
        . "2 0 obj\n<< /Type /Font /Subtype /Type0 /BaseFont /IndirectVerticalArrayTail /Encoding 3 0 R /DescendantFonts [4 0 R] /ToUnicode 6 0 R >>\nendobj\n"
        . "3 0 obj\n<< /Type /CMap /CMapName /IndirectVerticalArrayTail-V /Length " . strlen($encodingCMap) . " >>\nstream\n{$encodingCMap}\nendstream\nendobj\n"
        . "4 0 obj\n<< /Type /Font /Subtype /CIDFontType2 /BaseFont /IndirectVerticalArrayTail /CIDSystemInfo << /Registry (Adobe) /Ordering (Identity) /Supplement 0 >> /DW2 [880 -1000] /W2 [40 7 0 R 50 55 -250 500 880] >>\nendobj\n"
        . "5 0 obj\n<< /Length " . strlen($content) . " >>\nstream\n{$content}\nendstream\nendobj\n"
        . "6 0 obj\n<< /Length " . strlen($toUnicode) . " >>\nstream\n{$toUnicode}\nendstream\nendobj\n"
        . "7 0 obj\n[-500 500 880 -500 500 880 -500 500 880 -500 500 880] /Tail\nendobj\n%%EOF";
};
...
return [
    ...
    'rejects huge finite Tf font-size operands before current advance overflow on current base' => static function (TestRunner $t) use ($fontWidthHugeFiniteFontSizeBoundaryCurrentBasePdf): void {
        $extractor = new PdfTextExtractor();
        $pdf = $fontWidthHugeFiniteFontSizeBoundaryCurrentBasePdf();
        $plainText = $extractor->extractPlainText($pdf);
        $pages = $extractor->extractStyledTextPages($pdf);
        $line = $pages[0]['blocks'][0]['lines'][0] ?? [];
        $spans = $line['spans'] ?? [];

        $t->same(['ABCD EF'], $extractor->extractTextLines($pdf));
        $t->same(['AB', 'CD', 'EF'], $extractor->extractTextRuns($pdf));
        $t->same('ABCD EF', $plainText);
        $t->same("ABCD EF\n", $extractor->naiveGetText($pdf));
        $t->same(['AB', 'CD', 'EF'], array_column($spans, 'text'));
        $t->same([[0.0, 0.0, 24.0, 12.0], [24.0, 0.0, 48.0, 12.0], [72.0, 0.0, 96.0, 12.0]], array_column($spans, 'bbox'));
    ...
    'rejects overlarge finite Tz horizontal scale before current advance overflow on current base' => static function (TestRunner $t) use ($fontWidthHugeFiniteHorizontalScaleBoundaryCurrentBasePdf): void {
        $extractor = new PdfTextExtractor();
        $pdf = $fontWidthHugeFiniteHorizontalScaleBoundaryCurrentBasePdf();
        $plainText = $extractor->extractPlainText($pdf);
        $pages = $extractor->extractStyledTextPages($pdf);
        $styledLines = [];
        foreach (($pages[0]['blocks'] ?? []) as $block) {
        ...
        $t->true(!str_contains($plainText, 'AB CD'));
        $t->true(!str_contains($plainText, 'EF GH'));
        $t->true(!str_contains($plainText, 'HugeFiniteTzAdvance'));
        $t->true(!str_contains($plainText, 'Ftzhuge'));
        $t->true(!str_contains($plainText, "\0"));
    },
    ...
    'rejects tailed indirect CIDFont W array helpers before current text advance gaps on current base' => static function (TestRunner $t) use ($fontWidthIndirectCidArrayTailAdvanceBoundaryCurrentBasePdf): void {
        $extractor = new PdfTextExtractor();
        $pdf = $fontWidthIndirectCidArrayTailAdvanceBoundaryCurrentBasePdf();
        $plainText = $extractor->extractPlainText($pdf);
        $pages = $extractor->extractStyledTextPages($pdf);
        $line = $pages[0]['blocks'][0]['lines'][0] ?? [];
        $spans = $line['spans'] ?? [];
        ...
        $t->true(array_column($spans, 'bbox') !== [[0.0, 0.0, 48.0, 12.0], [48.0, 0.0, 108.0, 12.0]]);
        $t->true(!str_contains($plainText, 'IndirectCidWidthTail'));
    ...
    'rejects tailed indirect CIDFont W2 metric helpers before vertical styled span bboxes on current base' => static function (TestRunner $t) use ($fontWidthIndirectW2ArrayTailAdvanceBoundaryCurrentBasePdf): void {
        $extractor = new PdfTextExtractor();
        $pdf = $fontWidthIndirectW2ArrayTailAdvanceBoundaryCurrentBasePdf();
        $plainText = $extractor->extractPlainText($pdf);
        $pages = $extractor->extractStyledTextPages($pdf);
        $line = $pages[0]['blocks'][0]['lines'][0] ?? [];
        $spans = $line['spans'] ?? [];
        ...
        $t->true(array_column($spans, 'bbox') !== [[0.0, 0.0, 12.0, 24.0], [12.0, 0.0, 24.0, 18.0]]);
        $t->true(!str_contains($plainText, 'VertImport'));
        $t->true(!str_contains($plainText, 'IndirectVerticalArrayTail'));
        $t->true(!str_contains($plainText, 'Tail'));
        $t->true(!str_contains($plainText, "\0"));
    },
