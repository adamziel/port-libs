...
$xrefStreamPrevHybridOwnerCurrentBasePdf = static function (): string {
...
return [
    'keeps current xref-stream object-stream owner before stale Prev hybrid type-2 rows' => static function (TestRunner $t) use ($xrefStreamPrevHybridOwnerCurrentBasePdf): void {
        $extractor = new PdfTextExtractor();
        $pdf = $xrefStreamPrevHybridOwnerCurrentBasePdf();
        $text = $extractor->extractPlainText($pdf);

        $t->same(['Current xref stream owner page', 'Previous hybrid row skipped'], $extractor->extractTextLines($pdf));
        $t->same(['Current xref stream owner page', 'Previous hybrid row skipped'], $extractor->extractTextRuns($pdf));
        $t->same("Current xref stream owner page\nPrevious hybrid row skipped", $text);
        $t->same("Current xref stream owner page\nPrevious hybrid row skipped\n", $extractor->naiveGetText($pdf));
        $t->same(1, $extractor->extractOutlineMetadata($pdf)['pages']);
        $t->same(['1'], $extractor->extractPageLabels($pdf));
        $t->true(!str_contains($text, 'Stale previous hybrid page'));
        $t->true(!str_contains($text, 'Current replaced object stream leak'));
        $t->true(!str_contains($text, 'current carrier replacement must not satisfy stale Prev row'));
