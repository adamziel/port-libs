...
$outlineDirectNamedDestinationThreadActionReviewPdf = static function (): string {
...
return [
    ...
    'normalizes direct named destination Thread action target context on the primary review row' => static function (TestRunner $t) use ($outlineDirectNamedDestinationThreadActionReviewPdf): void {
        $pdf = $outlineDirectNamedDestinationThreadActionReviewPdf();
        ...
        $t->same([], $metadata['outline'], 'Thread actions stay review-only instead of becoming upstream-style TOC rows.');
        $t->same([], $extractor->getPdfToc($pdf));
        $t->same(['Direct Named Thread'], array_column($metadata['article_threads'], 'title'));

        $actions = $metadata['outline_action_review_actions'];
        $t->same(2, count($actions));
        $t->same(['Thread', 'URI'], array_column($actions, 'action_type'));
        $t->same(['article-thread-review', 'review-uri'], array_column($actions, 'safety'));
        $t->same(['ThreadAction', 'ThreadAction'], array_column($actions, 'destination_action_name'));
        $t->same([false, false], array_column($actions, 'executes_on_import'));
        $t->same([null, true], [$actions[0]['chained'] ?? null, $actions[1]['chained'] ?? null]);

        foreach ($actions as $action) {
            $t->same(1, $action['destination_action_target_page']);
            $t->same('Thread 7', $action['destination_action_target_page_label']);
            ...
            $t->same(['Direct Named Thread'], $action['destination_action_target_article_thread_titles']);
            $t->same([21, 22], array_column($action['destination_action_target_article_beads'], 'bead_object'));
            $t->same('direct-named-thread-review', $action['destination_action_target_page_review']['piece_info']['WPDirectThread']['private']['ReviewState'] ?? null);
            $t->same('direct-thread-review.xml', $action['destination_action_target_page_review']['page_associated_files'][0]['filename'] ?? null);
            $t->same(true, $action['destination_action_target_page_review']['page_associated_files'][0]['checksum_matches'] ?? null);
        }

        $t->same(1, $actions[0]['page']);
        $t->same('Thread 7', $actions[0]['page_label']);
        $t->same(22, $actions[0]['thread_bead_object']);
        $t->same([21, 22], array_column($actions[0]['target_article_beads'], 'bead_object'));
        $t->same('https://example.com/direct-named-thread-review', $actions[1]['uri']);

        $t->contains('Intro direct named thread page remains visible', $plainText);
        $t->contains('Direct named destination thread target remains visible', $plainText);
        $t->true(!str_contains($plainText, 'Direct Named Thread Action'));
        $t->true(!str_contains($plainText, 'ThreadAction'));
        $t->true(!str_contains($plainText, 'Direct Named Thread'));
        $t->true(!str_contains($plainText, 'direct-named-thread-review'));
        $t->true(!str_contains($plainText, 'wp-outline-thread-review'));
        $t->true(!str_contains($plainText, 'direct-thread-review.xml'));
    },
];
