...
$pdfDctDecodeNativePrefixAliasBoundaryCurrentBaseZlibStored = static function (string $bytes): string {
    $length = strlen($bytes);
    if ($length > 65535) {
        throw new RuntimeException('Focused DCTDecode native-prefix alias fixture must fit one deflate stored block.');
    }
...
$pdfDctDecodeNativePrefixAliasBoundaryCurrentBaseFixture = static function () use (
    $pdfDctDecodeNativePrefixAliasBoundaryCurrentBaseZlibStored
): array {
    ...
    $jpegPayload = "\xff\xd8\xff\xe0\x00\x10JFIF\0native prefix alias review bytes "
        . 'BT /F1 12 Tf 72 700 Td (Native-prefix alias DCT payload leak) Tj ET'
        . "\xff\xd9";
    $encodedPayload = $pdfDctDecodeNativePrefixAliasBoundaryCurrentBaseZlibStored($jpegPayload);
...
return [
    'reports canonical native prefix aliases before terminal DCTDecode image review' => static function (
        TestRunner $t
    ) use ($pdfDctDecodeNativePrefixAliasBoundaryCurrentBaseFixture): void {
        $fixture = $pdfDctDecodeNativePrefixAliasBoundaryCurrentBaseFixture();
        ...
        $t->true(!str_contains($plainText, 'Native-prefix alias DCT payload leak'));
        $t->true(!str_contains($plainText, 'native prefix alias review bytes'));
        ...
        $t->same('DCT', $boundary['stopped_before_filter'] ?? null);
