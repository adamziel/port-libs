...
$imageBboxRelativePage = static function (array $lines): array {
    return [
        'page' => 0,
...
$imageBboxRelativeTable = static function (): array {
    return [
...
return [
    'translates image-bbox-relative tabled result geometry into the crop image before assignment' => static function (
        TestRunner $t
    ) use ($imageBboxRelativeTable): void {
        $recognizer = new TableRecognizer();
        $formatted = $recognizer->formatRecognizedTables(
            [$imageBboxRelativeTable()],
            [['width' => 240, 'height' => 80]]
        );
        ...
        $t->same('image_bbox_relative', $review['source_coordinate_spaces']['cells'] ?? null);
        ...
        $t->same('image_bbox_relative', $localized['cells'][0]['source_coordinate_space'] ?? null);
        ...
        $t->true(!in_array('Stale relative row', $assignedTexts, true));
        $t->true(!in_array('Stale relative column', $assignedTexts, true));
    ...
    'surfaces image-bbox-relative table geometry through supplied WordPress conversion metadata' => static function (
        TestRunner $t
    ) use ($imageBboxRelativePage, $imageBboxRelativeTable): void {
        $path = sys_get_temp_dir() . '/markerpdf-table-image-bbox-relative-boundary-' . bin2hex(random_bytes(4)) . '.pdf';
        file_put_contents($path, "%PDF-1.4\n% image-bbox-relative table geometry boundary current-base fixture\n%%EOF");
        try {
            $result = (new SuppliedDocumentConverter())->convert(
                $path,
                [
                    $imageBboxRelativePage([
                        ['text' => 'Image bbox relative table geometry review', 'bbox' => [72.0, 48.0, 520.0, 68.0], 'font' => 'Heading-Bold', 'weight' => 700, 'size' => 18],
                        ['text' => 'Stale image-bbox-relative table line should be replaced.', 'bbox' => [82.0, 176.0, 300.0, 196.0]],
                        ['text' => 'After image bbox relative table.', 'bbox' => [72.0, 276.0, 520.0, 294.0]],
                    ]),
                ],
                [
                    ...
                    'recognized_tables' => [$imageBboxRelativeTable()],
            ...
            $t->contains('# Image Bbox Relative Table Geometry Review', $result['text']);
            $t->contains('| Feature | Status |', $result['text']);
            $t->contains('| Images  | Ready  |', $result['text']);
            $t->contains('After image bbox relative table.', $result['text']);
            $t->true(!str_contains($result['text'], 'Stale image-bbox-relative table line should be replaced.'));
            $t->true(!str_contains($result['text'], 'Stale relative row'));
            $t->true(!str_contains($result['text'], 'Stale relative column'));
            ...
            $t->same('image_bbox_relative', $coordinateReview['source_coordinate_spaces']['cells'] ?? null);
            ...
            $t->same('image_bbox_relative', $gridReview['render_cells'][0]['source_coordinate_space'] ?? null);
