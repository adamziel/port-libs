...
$xrefObjectStreamCommentOffsetCurrentBasePdf = static function (): string {
    ...
    if (!is_string($objectStream)) {
        throw new RuntimeException('Unable to compress comment-offset object-stream fixture.');
    }
    ...
    if (!is_string($compressedXref)) {
        throw new RuntimeException('Unable to compress comment-offset xref-stream fixture.');
    }
...
return [
    'rejects object-stream member offsets that point at PDF comments' => static function (TestRunner $t) use ($xrefObjectStreamCommentOffsetCurrentBasePdf): void {
        $extractor = new PdfTextExtractor();
        $pdf = $xrefObjectStreamCommentOffsetCurrentBasePdf();
        $text = $extractor->extractPlainText($pdf);
        $review = $extractor->extractXrefObjectStreamIndexReview($pdf);
        $entries = array_column($review['entries'], null, 'object_number');
        $entry = $entries[4] ?? [];

        $t->same(['Current comment-offset guard page'], $extractor->extractTextLines($pdf));
        $t->same(['Current comment-offset guard page'], $extractor->extractTextRuns($pdf));
        $t->same('Current comment-offset guard page', $text);
        $t->same("Current comment-offset guard page\n", $extractor->naiveGetText($pdf));
        $t->same(1, $extractor->extractOutlineMetadata($pdf)['pages']);
        $t->same(['1'], $extractor->extractPageLabels($pdf));
        $t->true(!str_contains($text, 'Comment-offset compressed leak'));
        $t->true(!str_contains($text, 'Comment-owned member ignored'));
