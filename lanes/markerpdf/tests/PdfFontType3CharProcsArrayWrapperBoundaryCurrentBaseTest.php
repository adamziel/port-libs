...
$type3CharProcsArrayWrapperPagePdf = static function (): string {
...
$type3CharProcsArrayWrapperFallbackPdf = static function (): string {
    ...
    $charProcs = '<< /A 3 0 R /B 3 0 R /C 3 0 R /D 3 0 R '
        . '/G 3 0 R /H 3 0 R /I 3 0 R /L 3 0 R /O 3 0 R '
        . '/P 3 0 R /S 3 0 R /T 3 0 R /V 3 0 R /Y 3 0 R >>';
...
$type3CharProcsArrayWrapperStreamFallbackPdf = static function (): string {
    $glyphProgram = "650 0 d0\nBT /Fghost 9 Tf (ARRAY WRAPPED CHARPROCS STREAM GLYPH LEAK) Tj ET\n";
    ...
    return "%PDF-1.4\n"
        . "2 0 obj\n<< /Type /Font /Subtype /Type3 /Name /Ft3 /BaseFont /T3ArrayWrapperStreamFallback "
        . "/FontBBox [0 0 1000 700] /FontMatrix [0.001 0 0 0.001 0 0] "
        . "/Encoding /WinAnsiEncoding /CharProcs [21 0 R] >>\nendobj\n"
        . "3 0 obj\n<< /Length " . strlen($glyphProgram) . " >>\nstream\n{$glyphProgram}\nendstream\nendobj\n"
        . "4 0 obj\n<< /Length " . strlen($visibleFallback) . " >>\nstream\n{$visibleFallback}\nendstream\nendobj\n"
...
return [
    'rejects array-wrapped Type3 CharProcs dictionaries before WordPress text grouping on current base' => static function (
        TestRunner $t
    ) use ($type3CharProcsArrayWrapperPagePdf): void {
        $extractor = new PdfTextExtractor();
        $pdf = $type3CharProcsArrayWrapperPagePdf();
        ...
        $t->true(!str_contains($plainText, 'array-wrapped CharProcs text leak'));
    },
    'keeps array-wrapped Type3 CharProcs glyph streams private during fallback extraction on current base' => static function (
        TestRunner $t
    ) use ($type3CharProcsArrayWrapperFallbackPdf): void {
        $extractor = new PdfTextExtractor();
        $pdf = $type3CharProcsArrayWrapperFallbackPdf();
        $plainText = $extractor->extractPlainText($pdf);
        ...
        $t->true(!str_contains($plainText, 'ARRAY WRAPPED CHARPROCS GLYPH LEAK'));
        $t->true(!str_contains($plainText, 'T3ArrayWrapperFallback'));
    },
    'keeps array-wrapped Type3 CharProcs stream dictionaries private during fallback extraction on current base' => static function (
        TestRunner $t
    ) use ($type3CharProcsArrayWrapperStreamFallbackPdf): void {
        $extractor = new PdfTextExtractor();
        $pdf = $type3CharProcsArrayWrapperStreamFallbackPdf();
        $plainText = $extractor->extractPlainText($pdf);
        ...
        $t->same('Visible fallback content', $plainText);
        $t->same("Visible fallback content\n", $extractor->naiveGetText($pdf));
        $t->true(!str_contains($plainText, 'ARRAY WRAPPED CHARPROCS STREAM GLYPH LEAK'));
        $t->true(!str_contains($plainText, 'ARRAY WRAPPED CHARPROCS STREAM PAYLOAD LEAK'));
        $t->true(!str_contains($plainText, 'T3ArrayWrapperStreamFallback'));
    },
];
