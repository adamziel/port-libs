...
$parserMalformedCMapIndirectScalarFilterValueUtf16beHex = static function (string $ascii): string {
    $hex = '';
...
$parserMalformedCMapIndirectScalarFilterValuePdf = static function (
    string $helperBody,
    string $safeText,
    string $leakingText,
    string $cMapName,
    string $baseFont
) use ($parserMalformedCMapIndirectScalarFilterValueUtf16beHex): array {
    $safeHex = $parserMalformedCMapIndirectScalarFilterValueUtf16beHex($safeText);
    ...
    $cMap = "/CIDInit /ProcSet findresource begin\n"
        ...
        . "endcodespacerange\n"
        . "1 beginbfchar\n"
        . "<{$sourceCode}> <" . $parserMalformedCMapIndirectScalarFilterValueUtf16beHex($leakingText) . ">\n"
        . "endbfchar\n"
        . "endcmap\n"
    ...
    if (!is_string($compressedCMap)) {
        throw new RuntimeException('Unable to compress indirect scalar filter-value CMap fixture.');
    }
...
$assertMalformedCMapIndirectScalarFilterValue = static function (
    TestRunner $t,
    array $fixture,
...
return [
    'fails closed and types selected indirect boolean CMap Filter helper values before Length' => static function (
        TestRunner $t
    ) use (
        $parserMalformedCMapIndirectScalarFilterValuePdf,
        $assertMalformedCMapIndirectScalarFilterValue
    ): void {
        $assertMalformedCMapIndirectScalarFilterValue(
            $t,
            $parserMalformedCMapIndirectScalarFilterValuePdf(
                'true',
                'Indirect Boolean Filter Safe Import',
                'Indirect Boolean Filter CMap Leak',
                'IndirectBooleanScalarFilterBoundary-H',
                'IndirectBooleanScalarFilterBoundary'
            ),
            'boolean',
            true
        );
    },
    'fails closed and types selected indirect real-number CMap Filter helper values before Length' => static function (
        TestRunner $t
    ) use (
        $parserMalformedCMapIndirectScalarFilterValuePdf,
        $assertMalformedCMapIndirectScalarFilterValue
        ...
        $assertMalformedCMapIndirectScalarFilterValue(
            ...
            $parserMalformedCMapIndirectScalarFilterValuePdf(
                '1.5',
                'Indirect Number Filter Safe Import',
                'Indirect Number Filter CMap Leak',
                'IndirectNumberScalarFilterBoundary-H',
                'IndirectNumberScalarFilterBoundary'
            ),
            'number',
            1.5
        );
