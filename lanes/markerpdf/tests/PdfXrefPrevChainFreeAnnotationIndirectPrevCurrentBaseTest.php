...
$xrefPrevChainFreeAnnotationIndirectPrevCurrentBasePdf = static function (): string {
    $baseContent = 'BT /F1 12 Tf 72 720 Td (Base indirect Prev annotation page) Tj ET';
    $currentContent = 'BT /F1 12 Tf 72 720 Td (Current indirect Prev annotation page) Tj ET';
    ...
    $staleAnnotationOffset = $addObject(7, 0, '<< /Type /Annot /Subtype /Link /Rect [72 700 250 718] /Contents (Stale indirect Prev free annotation) /A << /S /URI /URI (https://stale.example.com/indirect-prev-free-annotation) >> >>');
    ...
    if (!is_string($compressedRows)) {
        throw new RuntimeException('Unable to compress indirect-Prev free annotation xref-stream fixture.');
    }
...
return [
    'suppresses stale page annotations when latest xref-stream Prev is an indirect numeric helper' => static function (
        TestRunner $t
    ) use ($xrefPrevChainFreeAnnotationIndirectPrevCurrentBasePdf): void {
        $pdf = $xrefPrevChainFreeAnnotationIndirectPrevCurrentBasePdf();
        $linkExtractor = new PdfLinkAnnotationExtractor();
        $annotationExtractor = new PdfAnnotationExtractor();
        $freeObjects = PdfXrefFreeObjectMap::freeObjectNumbers($pdf);

        $t->true(isset($freeObjects[7]), 'The lightweight free-object map must follow indirect /Prev helpers before link promotion.');
        $t->same([], $linkExtractor->extractPageLinks($pdf), 'The stale annotation freed through the indirect /Prev chain must not become a WordPress link.');
        $t->same([], $annotationExtractor->extractPageAnnotations($pdf), 'The stale annotation freed through the indirect /Prev chain must not become review metadata.');
        ...
        $pages = [[
            ...
            'blocks' => [[
                ...
                'lines' => [[
                    'bbox' => [72.0, 700.0, 250.0, 718.0],
                    'spans' => [[
                        'text' => 'Current indirect Prev annotation page',
                        'bbox' => [72.0, 700.0, 250.0, 718.0],
                        'font' => 'Helvetica',
                    ]],
                ]],
        ...
        $t->true(!str_contains($encoded, 'Stale indirect Prev free annotation'));
    },
];
