...
$xrefObjectStreamCarrierConflictRepairCurrentBasePdf = static function (): string {
    $currentContent = 'BT /F1 12 Tf 72 720 Td (Current carrier-conflict guard page) Tj T* (Conflicting carrier stays review-only) Tj ET';
    $compressedContent = 'BT /F1 12 Tf 72 700 Td (Conflicting object-stream carrier leak) Tj T* (Guessed mixed carrier leaked) Tj ET';
    $compressedPage = '<< /Type /Page /Parent 2 0 R /Resources << /Font << /F1 3 0 R >> >> /Contents 5 0 R >>';
    $conflictMetadata = '<wpmeta>not an ObjStm</wpmeta>';

    $header = '4 0';
    $objectStream = gzcompress($header . "\n" . $compressedPage . "\n");
    if (!is_string($objectStream)) {
        throw new RuntimeException('Unable to compress carrier-conflict object stream fixture.');
    }
    ...
    $addObject(6, 0, '<< /Type /Metadata /Subtype /XML /Length ' . strlen($conflictMetadata) . " >>\nstream\n{$conflictMetadata}\nendstream");
    ...
    if (!is_string($compressedXref)) {
        throw new RuntimeException('Unable to compress carrier-conflict xref stream fixture.');
    }
...
return [
    'fails closed when damaged carrier repair sees a same-number non object stream definition' => static function (
        TestRunner $t
    ) use ($xrefObjectStreamCarrierConflictRepairCurrentBasePdf): void {
        $extractor = new PdfTextExtractor();
        $pdf = $xrefObjectStreamCarrierConflictRepairCurrentBasePdf();
        ...
        $expected = [
            'Current carrier-conflict guard page',
            'Conflicting carrier stays review-only',
        ];
        ...
        $t->true(!str_contains($text, 'Conflicting object-stream carrier leak'));
        $t->true(!str_contains($text, 'Guessed mixed carrier leaked'));
        $t->true(!str_contains($text, '<wpmeta>'));
