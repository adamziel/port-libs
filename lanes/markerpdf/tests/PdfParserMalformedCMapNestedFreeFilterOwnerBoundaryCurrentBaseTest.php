...
$parserMalformedCMapNestedFreeFilterOwnerUtf16beHex = static function (string $ascii): string {
    $hex = '';
...
$parserMalformedCMapNestedFreeFilterOwnerPdf = static function () use (
    $parserMalformedCMapNestedFreeFilterOwnerUtf16beHex
): array {
    $safeText = 'Nested Free Safe Import';
    $leakingText = 'Nested Free CMap Leak';
    $safeHex = $parserMalformedCMapNestedFreeFilterOwnerUtf16beHex($safeText);
    $sourceCode = substr($safeHex, 0, 4);
    $cMapName = 'NestedFreeFilterHelperBoundary-H';
    ...
    $cMap = "/CIDInit /ProcSet findresource begin\n"
        ...
        . "endcodespacerange\n"
        . "1 beginbfchar\n"
        . "<{$sourceCode}> <" . $parserMalformedCMapNestedFreeFilterOwnerUtf16beHex($leakingText) . ">\n"
        . "endbfchar\n"
        . "endcmap\n"
    ...
    if (!is_string($compressedCMap)) {
        throw new RuntimeException('Unable to compress nested free-helper CMap fixture.');
    }
    ...
    $addObject(4, 0, '<< /Type /Font /Subtype /Type0 /BaseFont /NestedFreeFilterHelperBoundary /Encoding /Identity-H /ToUnicode 6 0 R >>');
...
return [
    'rejects nested free CMap Filter helper references before text extraction' => static function (
        TestRunner $t
    ) use ($parserMalformedCMapNestedFreeFilterOwnerPdf): void {
        [$pdf, $safeText, $leakingText, $cMapName] = $parserMalformedCMapNestedFreeFilterOwnerPdf();
        ...
        $t->same(1, $review['nested_unresolved_filter_operand_count']);
        ...
        $t->same(1, $entry['nested_unresolved_filter_operand_count'] ?? null);
        ...
        $t->same(0, $filterOperand['selected_generation'] ?? null);
        $t->same('xref_selected_direct_object', $filterOperand['owner_policy'] ?? null);
        $t->same('[ 7 0 R /FlateDecode ]', $filterOperand['value_preview'] ?? null);
        ...
        $t->same(true, $filterOperand['nested_unresolved_filter_operand'] ?? null);
        $t->same(1, $filterOperand['nested_unresolved_filter_operand_count'] ?? null);
    },
];
