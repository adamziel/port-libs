...
$parserMalformedCMapBfcharCodespaceFilterBoundaryUtf16beHex = static function (string $ascii): string {
    $hex = '';
...
$parserMalformedCMapBfcharCodespaceFilterBoundaryPdf = static function () use (
    $parserMalformedCMapBfcharCodespaceFilterBoundaryUtf16beHex
): array {
    $safeText = 'Same Width Safe Import';
    $leakingText = 'Same Width CMap Leak';
    $safeHex = $parserMalformedCMapBfcharCodespaceFilterBoundaryUtf16beHex($safeText);
    $sourceCode = substr($safeHex, 0, 4);
    $cMapName = 'BfcharCodespaceFilterBoundary-H';
    ...
    $cMap = "/CIDInit /ProcSet findresource begin\n"
        ...
        . "endcodespacerange\n"
        . "1 beginbfchar\n"
        . "<{$sourceCode}> <" . $parserMalformedCMapBfcharCodespaceFilterBoundaryUtf16beHex($leakingText) . ">\n"
        . "endbfchar\n"
        . "endcmap\n"
    ...
    if (!is_string($compressedCMap)) {
        throw new RuntimeException('Unable to compress focused bfchar codespace CMap fixture.');
    }
    ...
    $pdf = "%PDF-1.5\n"
        ...
        . "4 0 obj\n<< /Type /Font /Subtype /Type0 /BaseFont /BfcharCodespaceFilterBoundary /Encoding /Identity-H /ToUnicode 6 0 R >>\nendobj\n"
        . "5 0 obj\n<< /Length " . strlen($content) . " >>\nstream\n{$content}\nendstream\nendobj\n"
        . "6 0 obj\n<< /Type /CMap /CMapName /{$cMapName} /Filter /FlateDecode /Length " . strlen($compressedCMap) . " >>\nstream\n{$compressedCMap}\nendstream\nendobj\n"
        . "%%EOF";

    return [$pdf, $safeText, $leakingText, $cMapName, $sourceCode];
};

return [
    'rejects filtered ToUnicode bfchar source keys outside same-width codespace before current-base text extraction' => static function (
        TestRunner $t
    ) use ($parserMalformedCMapBfcharCodespaceFilterBoundaryPdf): void {
        [$pdf, $safeText, $leakingText, $cMapName, $sourceCode] = $parserMalformedCMapBfcharCodespaceFilterBoundaryPdf();
        ...
        $t->true($sourceCode !== '0000' && $sourceCode !== '0001');
