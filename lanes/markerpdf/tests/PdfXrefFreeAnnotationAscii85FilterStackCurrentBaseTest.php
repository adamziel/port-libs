...
$xrefFreeAnnotationAscii85FilterStackCurrentBasePdf = static function (): string {
    ...
    $previousContent = 'BT /F1 12 Tf 72 720 Td (Previous ASCII85 xref free annotation page) Tj ET';
    $currentContent = 'BT /F1 12 Tf 72 720 Td (Current ASCII85 xref free annotation page) Tj ET';
    ...
    $addObject(7, 0, '<< /Type /Annot /Subtype /Link /Rect [72 700 250 718] /Contents (Stale ASCII85 xref free annotation) /A << /S /URI /URI (https://stale.example.com/ascii85-xref-free-annotation) >> >>');
    ...
    if (!is_string($compressedRows)) {
        throw new RuntimeException('Unable to compress ASCII85 filter-stack free-entry xref-stream fixture.');
    }
...
return [
    'suppresses stale free annotations from current xref streams with ASCII85 and Flate filters' => static function (TestRunner $t) use (
        $xrefFreeAnnotationAscii85FilterStackCurrentBasePdf
    ): void {
        $pdf = $xrefFreeAnnotationAscii85FilterStackCurrentBasePdf();
        $linkExtractor = new PdfLinkAnnotationExtractor();
        $annotationExtractor = new PdfAnnotationExtractor();
        $freeObjects = PdfXrefFreeObjectMap::freeObjectNumbers($pdf);

        $t->true(isset($freeObjects[7]), 'The free-object map must decode ASCII85 xref-stream filter stacks before reading free rows.');
        ...
        $pages = [[
            ...
            'blocks' => [[
                ...
                'lines' => [[
                    'bbox' => [72.0, 700.0, 250.0, 718.0],
                    'spans' => [[
                        'text' => 'Current ASCII85 xref free annotation page',
                        'bbox' => [72.0, 700.0, 250.0, 718.0],
                        'font' => 'Helvetica',
                    ]],
                ]],
        ...
        $t->true(str_contains($pdf, '/Filter [ /ASCII85Decode /FlateDecode ]'));
        ...
        $t->true(!str_contains($encodedReview, 'Stale ASCII85 xref free annotation'));
    },
];
