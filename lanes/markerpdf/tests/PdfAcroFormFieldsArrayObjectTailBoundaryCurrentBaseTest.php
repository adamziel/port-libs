...
$tailedIndirectFieldsArrayPdf = static function (): string {
    $pageText = 'BT /F1 12 Tf 72 720 Td (Visible AcroForm tailed Fields array object body) Tj ET';
    ...
    return "%PDF-1.7\n"
        ...
        . "6 0 obj\n<< /FT /Tx /T (malformed.fields.tail.decoy) /TU (Malformed Fields tail label) /TM (malformed-fields-tail-export) /V (Malformed Fields tail decoy value) >>\nendobj\n"
        ...
        . "20 0 obj\n[6 0 R]\n40 0 R\nendobj\n"
        . "40 0 obj\n<< /FT /Tx /T (trailing.fields.tail.decoy) /V (Trailing Fields tail value) >>\nendobj\n"
        . "%%EOF";
};

$tailedIndirectKidsArrayPdf = static function (): string {
    $pageText = 'BT /F1 12 Tf 72 720 Td (Visible AcroForm tailed Kids array object body) Tj ET';
...
return [
    'rejects tailed indirect AcroForm Fields array objects before page-widget field repair' => static function (
        TestRunner $t
    ) use ($tailedIndirectFieldsArrayPdf, $fieldsByName): void {
        $pdf = $tailedIndirectFieldsArrayPdf();
        ...
        foreach ([
            'malformed.fields.tail.decoy',
            'Malformed Fields tail label',
            'malformed-fields-tail-export',
            'Malformed Fields tail decoy value',
            'trailing.fields.tail.decoy',
            'Trailing Fields tail value',
        ] as $decoyText) {
            $t->same(false, str_contains($encoded, $decoyText));
            $t->same(false, str_contains($visibleText, $decoyText));
        }

        $t->same('Visible AcroForm tailed Fields array object body', $visibleText);
    ...
    'rejects tailed indirect AcroForm Kids array objects before terminal field review' => static function (
        TestRunner $t
    ) use ($tailedIndirectKidsArrayPdf, $fieldsByName): void {
        $pdf = $tailedIndirectKidsArrayPdf();
        $form = (new PdfAcroFormExtractor())->extractForm($pdf);
        $fields = $fieldsByName($form['fields']);
        $visibleText = (new PdfTextExtractor())->extractPlainText($pdf);
        $encoded = (string) json_encode($form, JSON_UNESCAPED_SLASHES);
        ...
        $t->same('Visible AcroForm tailed Kids array object body', $visibleText);
