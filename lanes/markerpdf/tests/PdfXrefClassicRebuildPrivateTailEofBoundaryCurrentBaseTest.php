...
$xrefClassicRebuildPrivateTailEofBoundaryCurrentBasePdf = static function (): array {
    ...
    $currentXmp = $xmpPacket('Current Private-Tail EOF Title');
    $tailXmp = $xmpPacket('Rejected Private-Tail EOF Title');
    $decoyXmp = $xmpPacket('Post EOF Private-Tail Decoy Title');
    $currentContent = 'BT /F1 12 Tf 72 720 Td (Current private-tail EOF page) Tj T* (Rejected tail stays bounded) Tj ET';
    $tailContent = 'BT /F1 12 Tf 72 720 Td (Rejected private-tail xref page) Tj T* (Private tail root leak) Tj ET';
    $decoyContent = 'BT /F1 12 Tf 72 720 Td (Post EOF private-tail decoy page) Tj T* (Later EOF root leak) Tj ET';
    ...
    return [$pdf, $currentPayload, strtolower($currentChecksum), $currentXrefOffset, $tailXrefOffset, (int) $tailEofOffset, $decoyXrefOffset];
};

return [
    'bounds rejected numeric private-tail classic startxref before post-EOF xref garbage' => static function (
        TestRunner $t
    ) use ($xrefClassicRebuildPrivateTailEofBoundaryCurrentBasePdf): void {
        [$pdf, $currentPayload, $currentChecksum, $currentXrefOffset, $tailXrefOffset, $tailEofOffset, $decoyXrefOffset] = $xrefClassicRebuildPrivateTailEofBoundaryCurrentBasePdf();
        ...
        $t->true($tailEofOffset > $tailXrefOffset);
        $t->true($decoyXrefOffset > $tailEofOffset);
        $t->true(str_contains($pdf, "startxref\n{$tailXrefOffset} /PrivateTail 20 0 R"));
        $t->same(['Current private-tail EOF page', 'Rejected tail stays bounded'], $textExtractor->extractTextLines($pdf));
        $t->same(['Current private-tail EOF page', 'Rejected tail stays bounded'], $textExtractor->extractTextRuns($pdf));
        $t->same("Current private-tail EOF page\nRejected tail stays bounded", $text);
        $t->same("Current private-tail EOF page\nRejected tail stays bounded\n", $textExtractor->naiveGetText($pdf));
        $t->same(1, $textExtractor->extractOutlineMetadata($pdf)['pages']);
        $t->same('Current Private-Tail EOF Title', $metadata['title']);
        $t->same('Current Private-Tail EOF Info', $metadata['info']['Title']);
        $t->same('Current EOF Importer', $metadata['info']['Author']);
        $t->same(1, count($embeddedFiles));
        $t->same('current-private-tail-eof.xml', $embeddedFiles[0]['name']);
        $t->same('current-private-tail-eof.xml', $embeddedFiles[0]['filename']);
        $t->same('Current private-tail EOF attachment', $embeddedFiles[0]['description']);
        ...
        $t->same(['current-private-tail-eof.xml'], $attachmentSummary['filenames']);
        ...
        $t->same(true, $freeObjects[17] ?? null, 'The last valid current xref table free row must stay selected.');
        $t->true(!str_contains($text, 'Rejected private-tail xref page'));
        $t->true(!str_contains($text, 'Private tail root leak'));
        $t->true(!str_contains($text, 'Post EOF private-tail decoy page'));
        $t->true(!str_contains($text, 'Later EOF root leak'));
        $t->true(!str_contains($encodedReview, 'Rejected Private-Tail'));
        $t->true(!str_contains($encodedReview, 'Post EOF Private-Tail'));
        $t->true(!str_contains($encodedReview, 'tail-private-tail-eof'));
        $t->true(!str_contains($encodedReview, 'decoy-private-tail-eof'));
