...
$xrefObjectStreamNullFilterDecodeParmsCurrentBasePdf = static function (): string {
    $directContent = 'BT /F1 12 Tf 72 720 Td (Direct null-filter guard page) Tj ET';
    $compressedContent = 'BT /F1 12 Tf 72 700 Td (Current null-filter object-stream page) Tj T* (Null DecodeParms slot ignored) Tj ET';
    $compressedPage = '<< /Type /Page /Parent 2 0 R /Resources << /Font << /F1 3 0 R >> >> /Contents 5 0 R >>';

    $header = '4 0';
    $objectStream = gzcompress($header . "\n" . $compressedPage . "\n");
    if (!is_string($objectStream)) {
        throw new RuntimeException('Unable to compress null-filter DecodeParms object-stream fixture.');
    }
    ...
    if (!is_string($compressedXref)) {
        throw new RuntimeException('Unable to compress null-filter DecodeParms xref-stream fixture.');
    }
...
return [
    'ignores DecodeParms entries aligned to null object-stream filter slots before expanding xref members' => static function (
        TestRunner $t
    ) use ($xrefObjectStreamNullFilterDecodeParmsCurrentBasePdf): void {
        $extractor = new PdfTextExtractor();
        $pdf = $xrefObjectStreamNullFilterDecodeParmsCurrentBasePdf();
        ...
        $expected = [
            'Direct null-filter guard page',
            'Current null-filter object-stream page',
            'Null DecodeParms slot ignored',
        ];
