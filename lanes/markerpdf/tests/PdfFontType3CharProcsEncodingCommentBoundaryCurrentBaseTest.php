...
$type3CharProcsEncodingCommentBoundaryCurrentBasePdf = static function (): string {
...
return [
    'ignores PDF comments inside Type3 Encoding Differences before CharProc lookup on current base' => static function (
        TestRunner $t
    ) use ($type3CharProcsEncodingCommentBoundaryCurrentBasePdf): void {
        $extractor = new PdfTextExtractor();
        $pdf = $type3CharProcsEncodingCommentBoundaryCurrentBasePdf();
        ...
        $t->true(!str_contains($plainText, 'encoding comment charproc text leak'));
        $t->true(!str_contains($plainText, 'D.comment'));
        $t->true(!str_contains($plainText, 'S.comment'));
    },
];
