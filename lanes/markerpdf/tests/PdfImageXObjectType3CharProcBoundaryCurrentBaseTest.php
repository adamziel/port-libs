...
$type3CharProcImageXObjectBoundaryPdf = static function (): array {
    $charProc = "1000 0 d0\n"
        . "q 12 0 0 8 1 2 cm /Glyph#20Image Do Q\n"
        . "BT /Fghost 9 Tf (Type3 CharProc visible text leak) Tj ET\n";
    $pageContent = 'BT /Ft3 24 Tf 72 720 Td (A) Tj ET';
    $glyphPayload = 'BT /Fghost 9 Tf 0 0 Td (Type3 Glyph Image Payload Noise) Tj ET';
    $unusedPayload = 'BT /Fghost 9 Tf 0 0 Td (Unused Type3 Image Payload Noise) Tj ET';
    ...
    if (!is_string($glyphCompressed) || !is_string($unusedCompressed)) {
        throw new RuntimeException('Unable to compress Type3 CharProc image XObject fixture payloads.');
    }
...
return [
    'records used Type3 CharProc Image XObject paints as review-only metadata' => static function (TestRunner $t) use ($type3CharProcImageXObjectBoundaryPdf): void {
        [$pdf, $glyphPayload, $unusedPayload] = $type3CharProcImageXObjectBoundaryPdf();
        ...
        $t->true(is_array($entry), 'Type3 CharProc image review row should be present.');
        if (!is_array($entry)) {
            return;
        }
        ...
        $t->same('A', $plainText);
        $t->true(!str_contains($plainText, 'Type3 CharProc visible text leak'));
        $t->true(!str_contains($plainText, 'Type3 Glyph Image Payload Noise'));
        $t->true(!str_contains($plainText, 'Unused Type3 Image Payload Noise'));
