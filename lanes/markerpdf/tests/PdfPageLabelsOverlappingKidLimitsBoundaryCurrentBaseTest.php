...
$pageLabelsOverlappingKidLimitsPdf = static function (): string {
    $contents = [
...
return [
    'rejects overlapping PageLabels kid Limits before stale child relabeling' => static function (
        TestRunner $t
    ) use ($pageLabelsOverlappingKidLimitsPdf): void {
        $pdf = $pageLabelsOverlappingKidLimitsPdf();
        ...
        $t->same([
            'Overlapping kid front imported',
            'Overlapping kid body imported',
            'Overlapping kid appendix imported',
            'Overlapping kid end imported',
        ], array_column($entries, 'text'));
        $t->true(!in_array('stale-overlap-77', $labels, true));
        $t->true(!in_array('stale-overlap-77', $previewLabels, true));
        $t->true(!in_array('2', $labels, true));
