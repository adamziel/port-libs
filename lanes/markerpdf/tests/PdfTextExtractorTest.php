...
$pdfWithIncrementalUpdatedContent = static function (): string {
...
$xrefPrevStreamInvalidOffsetBoundaryPdf = static function (): string {
    $staleContent = 'BT /F1 12 Tf 72 720 Td (Stale invalid offset resurrection) Tj ET';
    $currentContent = 'BT /F1 12 Tf 72 720 Td (Current valid xref offset page) Tj ET';
    ...
    $previousCompressed = gzcompress($previousRows);
    if (!is_string($previousCompressed)) {
        throw new RuntimeException('Unable to compress previous xref-stream boundary fixture.');
    }
    ...
    $currentCatalogOffset = $addObject(1, 1, '<< /Type /Catalog /Pages 2 1 R >>');
    $currentPagesOffset = $addObject(2, 1, '<< /Type /Pages /Kids [3 1 R] /Count 1 >>');
    $currentPageOffset = $addObject(3, 1, '<< /Type /Page /Parent 2 1 R /Resources << /Font << /F1 4 0 R >> >> /Contents [6 1 R 5 1 R] >>');
    $currentContentOffset = $addObject(6, 1, "<< /Length " . strlen($currentContent) . " >>\nstream\n{$currentContent}\nendstream");
    $invalidStaleContentOffset = $staleContentOffset + 7;

    $currentRows = $xrefStreamRows([
        $currentCatalogOffset,
        $currentPagesOffset,
        $currentPageOffset,
        $invalidStaleContentOffset,
        $currentContentOffset,
    ], 0);
    $currentCompressed = gzcompress($currentRows);
    if (!is_string($currentCompressed)) {
    ...
    $currentXrefOffset = strlen($pdf);
    $pdf .= "21 0 obj\n"
        . '<< /Type /XRef /Size 22 /Root 1 1 R /Prev ' . $previousXrefOffset . ' /Index [1 3 5 2] /W [1 4 0] /Filter /FlateDecode /Length ' . strlen($currentCompressed) . " >>\n"
        . "stream\n{$currentCompressed}\nendstream\nendobj\n"
        . "startxref\n{$currentXrefOffset}\n%%EOF";
...
return [
    ...
    'honors declared ToUnicode bfchar and bfrange row counts before WordPress text' => static function (TestRunner $t): void {
        ...
        $t->same("Import B\nWQR", $text);
        $t->same(['Import B', 'WQR'], $extractor->extractTextRuns($pdf));
        $t->true(!str_contains($text, 'Leak'));
        $t->true(!str_contains($text, 'WFG'));
    },
    ...
    'uses decimal CIDFont W widths with ToUnicode CMap boundaries before WordPress text' => static function (TestRunner $t): void {
        $cmap = "/CIDInit /ProcSet findresource begin\n"
            . "12 dict begin\n"
            . "begincmap\n"
            . "1 begincodespacerange\n"
    ...
    'resolves indirect Type0 CIDFont W width differences before WordPress text gaps' => static function (TestRunner $t): void {
        $encodingCMap = "/CIDInit /ProcSet findresource begin\n"
            . "12 dict begin\n"
            . "begincmap\n"
    ...
    'resolves indirect CIDFont W2 vertical metrics before WordPress text advance boundaries' => static function (TestRunner $t): void {
        $cmap = "/CIDInit /ProcSet findresource begin\n"
            . "12 dict begin\n"
            . "begincmap\n"
            . "/WMode 1 def\n"
    ...
    'uses predefined vertical CMap writing mode with CIDSet and ToUnicode source boundaries' => static function (TestRunner $t): void {
        $cmap = "/CIDInit /ProcSet findresource begin\n"
            . "12 dict begin\n"
            . "begincmap\n"
            . "1 begincodespacerange\n"
        ...
        $t->true(is_string($compressedCidSet), 'Vertical CIDSet fixture should compress.');
        $pdf = "%PDF-1.4\n"
            . "1 0 obj\n<< /Type /Page /Resources << /Font << /Fv 2 0 R >> >> /Contents 5 0 R >>\nendobj\n"
    ...
    'uses xref Prev chains to ignore superseded incremental WordPress content streams' => static function (TestRunner $t) use ($pdfWithIncrementalUpdatedContent): void {
        $pdf = $pdfWithIncrementalUpdatedContent();
        $extractor = new PdfTextExtractor();

        $t->same('Published Update', $extractor->extractPlainText($pdf));
        $t->same(['Published Update'], $extractor->extractTextRuns($pdf));
        $t->true(!str_contains($extractor->extractPlainText($pdf), 'Draft'));
        $t->same("Published Update\n", $extractor->naiveGetText($pdf));
    },
    ...
    'rejects invalid explicit xref stream offsets before stale Prev generation fallback' => static function (TestRunner $t) use ($xrefPrevStreamInvalidOffsetBoundaryPdf): void {
        $extractor = new PdfTextExtractor();
        $pdf = $xrefPrevStreamInvalidOffsetBoundaryPdf();
        $text = $extractor->extractPlainText($pdf);

        $t->same(['Current valid xref offset page'], $extractor->extractTextLines($pdf));
        $t->same(['Current valid xref offset page'], $extractor->extractTextRuns($pdf));
        $t->same('Current valid xref offset page', $text);
        $t->same("Current valid xref offset page\n", $extractor->naiveGetText($pdf));
        $t->same(1, $extractor->extractOutlineMetadata($pdf)['pages']);
        $t->same(['1'], $extractor->extractPageLabels($pdf));
        $t->true(!str_contains($text, 'Stale invalid offset resurrection'));
    },
