...
$encryptedPermissionTopLevelBoundaryPdf = static function (string $decoy, string $label): array {
    $content = "BT /F1 12 Tf 72 720 Td ({$label} top-level permission boundary encrypted text leak) Tj ET";
...
return [
    'ignores malformed non-entry tokens before Standard permission word preflight' => static function (
        TestRunner $t
    ) use ($encryptedPermissionTopLevelBoundaryPdf): void {
        $cases = [
            'LITERAL' => '(literal decoy /P -64 should not count)',
            'DICT' => '<< /P -64 /Reason (nested decoy) >>',
            'ARRAY' => '[ /P -64 (array decoy) ]',
            'HEX' => '<2F50202D363420686578206465636F79>',
            'COMMENT' => "% /P -64 comment decoy\n",
        ];

        foreach ($cases as $label => $decoy) {
            [$pdf, $content, $ownerValidation, $userValidation] = $encryptedPermissionTopLevelBoundaryPdf($decoy, $label);
            ...
            $t->same('well_formed_standard_permissions', $reviewEncryption['permission_word_status']);
            $t->same(true, $reviewEncryption['permission_word_well_formed']);
            ...
            $t->true(in_array('copy_or_extract', $permission['allowed'], true));
            $t->true(!in_array('copy_or_extract', $permission['denied'], true));

            $t->same('well_formed_standard_permissions', $handler['status']);
            $t->same(true, $handler['permission_word_well_formed']);
            $t->same(false, $handler['permission_word_duplicate_entries']);
            ...
            $t->same(false, in_array('permission_word_duplicate_entries', $report['review_reasons'], true));
