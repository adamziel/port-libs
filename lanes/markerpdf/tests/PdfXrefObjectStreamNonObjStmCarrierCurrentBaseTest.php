...
$xrefObjectStreamNonObjStmCarrierCurrentBasePdf = static function (): string {
    ...
    if (!is_string($metadataPayload)) {
        throw new RuntimeException('Unable to compress non-ObjStm carrier fixture.');
    }
    ...
    if (!is_string($compressedXref)) {
        throw new RuntimeException('Unable to compress non-ObjStm carrier xref-stream fixture.');
    }
...
return [
    'rejects xref-selected non object-stream carriers with object-stream-shaped bytes' => static function (
        TestRunner $t
    ) use ($xrefObjectStreamNonObjStmCarrierCurrentBasePdf): void {
        $extractor = new PdfTextExtractor();
        $pdf = $xrefObjectStreamNonObjStmCarrierCurrentBasePdf();
        ...
        $expected = [
            'Current non-ObjStm carrier guard page',
            'Metadata stream is not an object stream',
        ];
        ...
        $t->true(!str_contains($text, 'Non object-stream carrier leak'));
        $t->true(!str_contains($text, 'Metadata bytes decoded as compressed objects'));
        $t->true(!str_contains($text, 'metadata carrier must not provide this page'));
        ...
        $t->same(0, $review['invalid_explicit_object_stream_carrier_count']);
        $t->same(4, $entry['object_number'] ?? null);
        $t->same(6, $entry['object_stream'] ?? null);
        $t->same(1, $entry['object_stream_xref_entry_type'] ?? null);
        $t->same(false, $entry['object_stream_carrier_is_objstm'] ?? null);
