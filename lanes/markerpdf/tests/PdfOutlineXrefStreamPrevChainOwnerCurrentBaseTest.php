...
$outlineXrefStreamPrevChainOwnerCurrentBasePdf = static function (): string {
    ...
    if (!is_string($compressedRows)) {
        throw new RuntimeException('Unable to compress outline xref-stream Prev chain rows.');
    }
...
return [
    'uses xref-stream Prev chain owners before post-xref outline decoys' => static function (
        TestRunner $t
    ) use ($outlineXrefStreamPrevChainOwnerCurrentBasePdf): void {
        $pdf = $outlineXrefStreamPrevChainOwnerCurrentBasePdf();
        ...
        $t->same(['Current XRef Prev Chain Start', 'Current XRef Prev Chain Review'], array_column($toc, 'title'));
        ...
        $t->same(['Current XRef Prev Chain Start', 'Current XRef Prev Chain Review'], array_column($navigation['outline'] ?? [], 'title'));
        $t->same(['Current XRef Prev Chain Review', 'Current XRef Prev Chain Review'], array_column($navigation['outline_action_review_actions'] ?? [], 'outline_title'));
        $t->same(['GoTo', 'URI'], array_column($navigation['outline_action_review_actions'] ?? [], 'action_type'));
        $t->same("Current outline Prev chain intro\nCurrent outline Prev chain target", $plainText);
        $t->true(is_string($encodedNavigation) && !str_contains($encodedNavigation, 'Previous XRef Prev Chain Outline'));
        $t->true(is_string($encodedNavigation) && !str_contains($encodedNavigation, 'Post XRef Outline Decoy'));
        $t->true(is_string($encodedNavigation) && !str_contains($encodedNavigation, 'post xref outline decoy action'));
        $t->true(!str_contains($plainText, 'Previous outline Prev chain intro'));
        $t->true(!str_contains($plainText, 'Post xref outline decoy intro'));
        $t->true(!str_contains($plainText, 'Current XRef Prev Chain Start'));
        $t->true(!str_contains($plainText, 'Current XRef Prev Chain Review'));
    },
];
