...
$xrefObjectStreamIncompleteHeaderCurrentBasePdf = static function (): string {
    $directContent = 'BT /F1 12 Tf 72 720 Td (Current incomplete-header guard page) Tj ET';
    $compressedContent = 'BT /F1 12 Tf 72 700 Td (Malformed object stream page leak) Tj T* (Incomplete header member ignored) Tj ET';
    ...
    if (!is_string($objectStream)) {
        throw new RuntimeException('Unable to compress incomplete-header object-stream fixture.');
    }
    ...
    if (!is_string($compressedXref)) {
        throw new RuntimeException('Unable to compress incomplete-header xref-stream fixture.');
    }
...
return [
    'fails closed on incomplete object-stream header pairs before WordPress text extraction' => static function (TestRunner $t) use ($xrefObjectStreamIncompleteHeaderCurrentBasePdf): void {
        $extractor = new PdfTextExtractor();
        $pdf = $xrefObjectStreamIncompleteHeaderCurrentBasePdf();
        $text = $extractor->extractPlainText($pdf);
        $review = $extractor->extractXrefObjectStreamIndexReview($pdf);
        $entries = array_column($review['entries'], null, 'object_number');
        $entry = $entries[4] ?? [];

        $t->same(['Current incomplete-header guard page'], $extractor->extractTextLines($pdf));
        $t->same(['Current incomplete-header guard page'], $extractor->extractTextRuns($pdf));
        $t->same('Current incomplete-header guard page', $text);
        $t->same("Current incomplete-header guard page\n", $extractor->naiveGetText($pdf));
        $t->same(1, $extractor->extractOutlineMetadata($pdf)['pages']);
        $t->same(['1'], $extractor->extractPageLabels($pdf));
        $t->true(!str_contains($text, 'Malformed object stream page leak'));
        $t->true(!str_contains($text, 'Incomplete header member ignored'));
