...
$calculationOrderGenerationBoundaryPdf = static function (): string {
...
return [
    ...
    'rejects stale generation AcroForm calculation order refs while preserving unresolved review targets' => static function (
        TestRunner $t
    ) use ($calculationOrderGenerationBoundaryPdf, $fieldsByName): void {
        $pdf = $calculationOrderGenerationBoundaryPdf();
        $form = (new PdfAcroFormExtractor())->extractForm($pdf);
        $fields = $fieldsByName($form['fields']);
        $orderReview = $form['calculation_order_review'];
        ...
        $t->same(['current.total', 'current.amount'], array_keys($fields));
        $t->same([
            ['object' => 99, 'field_name' => null],
            ['object' => 12, 'field_name' => 'current.amount'],
        ], $form['calculation_order']);

        $t->same([99, 12], array_column($orderReview, 'object'));
        $t->same(['unresolved', 'widget'], array_column($orderReview, 'target_kind'));
        $t->same([null, 10], array_column($orderReview, 'field_object'));
        $t->same([null, 12], array_column($orderReview, 'widget_object'));
        $t->same([null, 'current.amount'], array_column($orderReview, 'field_name'));
        $t->same([true, false], array_column($orderReview, 'unresolved'));
        $t->same([false, true], array_column($orderReview, 'resolved_from_widget'));

        $totalState = $fields['current.total']['calculation_state'];
        $amountState = $fields['current.amount']['calculation_state'];
        $t->same(false, $totalState['in_calculation_order']);
        $t->same(null, $totalState['calculation_order_object']);
        $t->same(true, $amountState['in_calculation_order']);
        $t->same(1, $amountState['calculation_order_index']);
        $t->same(12, $amountState['calculation_order_object']);
        $t->same('widget', $amountState['calculation_order_target_kind']);
        $t->same(10, $amountState['calculation_order_field_object']);
        $t->same(12, $amountState['calculation_order_widget_object']);

        $t->true(str_contains($visibleText, 'Visible calculation order generation boundary body'));
        foreach ([
            'stale.total',
            'stale.amount',
            'stale total value',
            ...
            'decoy.literal',
            'decoy.nested_array',
            'decoy.nested_dictionary',
            'decoy.comment',
        ] as $staleOrDecoy) {
            $t->true(is_string($encoded) && !str_contains($encoded, $staleOrDecoy));
            $t->true(!str_contains($visibleText, $staleOrDecoy));
        }
    },
];
