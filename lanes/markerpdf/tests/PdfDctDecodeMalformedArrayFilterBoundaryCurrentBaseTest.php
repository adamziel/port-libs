...
$pdfDctDecodeMalformedArrayFilterBoundaryCurrentBaseZlibStored = static function (string $bytes): string {
    $length = strlen($bytes);
    if ($length > 65535) {
        throw new RuntimeException('Focused malformed DCT filter-array fixture must fit one deflate stored block.');
    }
...
return [
    'preserves DCTDecode after malformed filter-array operands before renderer and WordPress review' => static function (
        TestRunner $t
    ) use ($pdfDctDecodeMalformedArrayFilterBoundaryCurrentBaseZlibStored): void {
        $extractor = new PdfTextExtractor();
        $renderer = new PdfImageRenderer();
        $before = 'BT /F1 12 Tf 72 720 Td (Before malformed DCT filter array) Tj ET';
        $after = 'BT /F1 12 Tf 72 680 Td (After malformed DCT filter array) Tj ET';
        $jpegPayload = "\xff\xd8\xff\xe0JFIF\0malformed filter array bytes "
            . 'BT /F1 12 Tf 72 700 Td (Malformed DCT filter array payload leak) Tj ET'
            . "\xff\xd9";
        $compressedPayload = $pdfDctDecodeMalformedArrayFilterBoundaryCurrentBaseZlibStored($jpegPayload);
        $filterArray = '[ /FlateDecode BogusFilter /DCTDecode ]';
        $decodeParms = '[ << /Predictor 1 >> null << /ColorTransform 1 >> ]';
        $imageDictionary = '<< /Type /XObject /Subtype /Image /Width 1 /Height 1 /ColorSpace /DeviceRGB /BitsPerComponent 8 /Filter ' . $filterArray . ' /DecodeParms ' . $decodeParms . ' /Length ' . strlen($compressedPayload) . ' >>';
        ...
        $expectedFilters = ['FlateDecode', 'MalformedFilterOperand', 'DCTDecode'];
        $expectedBoundary = [
            'declared_filter' => 'DCTDecode',
            'canonical_filter' => 'DCTDecode',
            'alias_used' => false,
            'non_null_filter_index' => 2,
            'filters_before_dctdecode' => ['FlateDecode', 'MalformedFilterOperand'],
        ...
        $t->same(['Before malformed DCT filter array', 'After malformed DCT filter array'], $extractor->extractTextLines($pdf));
        $t->same("Before malformed DCT filter array\nAfter malformed DCT filter array", $plainText);
        $t->same("Before malformed DCT filter array\nAfter malformed DCT filter array\n", $extractor->naiveGetText($pdf));
        $t->true(!str_contains($plainText, 'Malformed DCT filter array payload leak'));
        $t->true(!str_contains($plainText, 'JFIF'));
        $t->true(!str_contains($plainText, 'BogusFilter'));
        ...
        ], $plan['image_filter_details'][2]['decode_parms'] ?? null);
        ...
        $t->same([
            ...
            'color_transform' => 1,
            'valid_color_transform' => true,
        ], $entry['filter_details'][2]['decode_parms'] ?? null);
        $t->same(false, $entry['native_raster_decode'] ?? null);
        $t->same(false, $entry['decoded_with_current_filters'] ?? null);
        $t->same(false, $entry['payload_in_visible_text'] ?? null);
        $t->true(!str_contains($reviewJson, 'Malformed DCT filter array payload leak'));
