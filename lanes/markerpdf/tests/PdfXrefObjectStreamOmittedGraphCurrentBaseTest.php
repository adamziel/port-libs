...
$xrefObjectStreamOmittedGraphCurrentBasePdf = static function (): string {
    ...
    $objectStream = static function (array $members): array {
        ...
        if (!is_string($compressed)) {
            throw new RuntimeException('Unable to compress omitted graph object stream fixture.');
        }
    ...
    if (!is_string($compressedRows)) {
        throw new RuntimeException('Unable to compress omitted graph xref-stream rows.');
    }
...
return [
    'repairs omitted compressed page-tree graph members before stale Prev rows' => static function (
        TestRunner $t
    ) use ($xrefObjectStreamOmittedGraphCurrentBasePdf): void {
        $extractor = new PdfTextExtractor();
        $pdf = $xrefObjectStreamOmittedGraphCurrentBasePdf();
        $text = $extractor->extractPlainText($pdf);
        $review = $extractor->extractXrefObjectStreamIndexReview($pdf);
        $entries = array_column($review['entries'], null, 'object_number');

        $expected = ['Current omitted graph page', 'Reachable compressed members repaired'];
        ...
        $t->true(!str_contains($text, 'Stale omitted graph page'));
        $t->true(!str_contains($text, 'Previous xref row leaked'));
