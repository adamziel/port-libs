...
$namedDestinationDirectNamesDuplicateDestsBoundaryCurrentBasePdf = static function (): string {
...
$namedDestinationDirectNamesDuplicateDestsBoundaryCurrentBasePages = static function (): array {
    return [[
        'pnum' => 0,
        'blocks' => [[
            'type' => 'Text',
...
return [
    'fails closed on direct catalog Names Dests duplicate keys before standalone destination review' => static function (
        TestRunner $t
    ) use ($namedDestinationDirectNamesDuplicateDestsBoundaryCurrentBasePdf): void {
        $pdf = $namedDestinationDirectNamesDuplicateDestsBoundaryCurrentBasePdf();
    ...
    'keeps direct duplicate Names Dests rows out of link promotion and visible WordPress text' => static function (
        TestRunner $t
    ) use ($namedDestinationDirectNamesDuplicateDestsBoundaryCurrentBasePdf, $namedDestinationDirectNamesDuplicateDestsBoundaryCurrentBasePages): void {
        $pdf = $namedDestinationDirectNamesDuplicateDestsBoundaryCurrentBasePdf();
        ...
        $t->same('https://example.com/direct-names-duplicate-dests', $links[0]['links'][1]['uri']);

        $pages = $linkExtractor->applyLinksToPages($namedDestinationDirectNamesDuplicateDestsBoundaryCurrentBasePages(), $pdf);
        ...
        $t->same('https://example.com/direct-names-duplicate-dests', $spans[3]['link_uri']);

        $blocks = (new MarkdownPostProcessor())->mergeBlocks((new MarkdownPostProcessor())->mergeSpans($pages));
        $t->same('Current tree jump Stale tree jump Legacy jump [Safe URI](https://example.com/direct-names-duplicate-dests)', $blocks[0]['text']);
        ...
        $t->contains('Direct Names duplicate Dests target body', $plainText);
        foreach (['Current Tree', 'Stale Tree', 'FitH', 'XYZ'] as $hidden) {
            $t->same(false, str_contains($encoded, $hidden));
            $t->same(false, str_contains($plainText, $hidden));
        }
        $t->same(false, str_contains($plainText, 'direct-names-duplicate-dests'));
    },
];
