...
$xrefObjectStreamZeroRowTailBoundaryCurrentBasePdf = static function (): string {
    ...
    if (!is_string($objectStream)) {
        throw new RuntimeException('Unable to compress zero-row-tail object-stream fixture.');
    }
    ...
    if (!is_string($compressedXref)) {
        throw new RuntimeException('Unable to compress zero-row-tail xref-stream fixture.');
    }
...
return [
    'uses skipped zero object-number rows as non-selectable object-stream body boundaries' => static function (
        TestRunner $t
    ) use ($xrefObjectStreamZeroRowTailBoundaryCurrentBasePdf): void {
        $extractor = new PdfTextExtractor();
        $pdf = $xrefObjectStreamZeroRowTailBoundaryCurrentBasePdf();
        ...
        $expected = [
            'Current zero-row tail guard page',
            'Current zero-row object-stream page',
            'Zero row tail boundary preserved',
        ];
        ...
        $t->true(!str_contains($text, 'ignored zero-object row tail decoy'));
        ...
        $t->same(0, $review['malformed_object_stream_member_tail_count']);
        ...
        $t->same(2, $entry['object_stream_member_offset_boundary_count'] ?? null);
        $t->same(1, $entry['object_stream_skipped_member_boundary_count'] ?? null);
        $t->same(true, $entry['strict_member_match'] ?? null);
        $t->same(true, $entry['member_offset_token_boundary'] ?? null);
        $t->same(true, $entry['object_stream_member_has_single_value'] ?? null);
        $t->same(false, $entry['malformed_member_tail_rejected'] ?? null);
