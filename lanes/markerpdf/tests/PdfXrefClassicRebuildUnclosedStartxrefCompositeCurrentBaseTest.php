...
$xrefClassicRebuildUnclosedStartxrefCompositeCurrentBasePdf = static function (): array {
    ...
    return [
        ...
        $currentXrefOffset,
        $unclosedCompositeOffset,
        $decoyXrefOffset,
    ];
};

return [
    'skips unclosed composite xref tables even when raw startxref appears before the decoy' => static function (
        TestRunner $t
    ) use ($xrefClassicRebuildUnclosedStartxrefCompositeCurrentBasePdf): void {
        [$pdf, $currentPayload, $currentChecksum, $currentXrefOffset, $unclosedCompositeOffset, $decoyXrefOffset] = $xrefClassicRebuildUnclosedStartxrefCompositeCurrentBasePdf();
        ...
        $t->true($unclosedCompositeOffset > $currentXrefOffset);
        $t->true($decoyXrefOffset > $unclosedCompositeOffset);
        $t->same(['Current unclosed-startxref xref page', 'Raw startxref composite skipped'], $extractor->extractTextLines($pdf));
        $t->same(['Current unclosed-startxref xref page', 'Raw startxref composite skipped'], $extractor->extractTextRuns($pdf));
        $t->same("Current unclosed-startxref xref page\nRaw startxref composite skipped", $text);
        $t->same("Current unclosed-startxref xref page\nRaw startxref composite skipped\n", $extractor->naiveGetText($pdf));
        $t->same(1, $extractor->extractOutlineMetadata($pdf)['pages']);
        $t->same('Current Unclosed Startxref Composite XRef Title', $metadata['title']);
        $t->same('Current Unclosed Startxref Composite XRef Info', $metadata['info']['Title']);
        $t->same('Current Composite Importer', $metadata['info']['Author']);
        $t->same(1, count($files));
        $t->same('current-unclosed-startxref-composite-xref.xml', $files[0]['name']);
        $t->same('current-unclosed-startxref-composite-xref.xml', $files[0]['filename']);
        $t->same('Current unclosed startxref composite attachment', $files[0]['description']);
        ...
        $t->same(['current-unclosed-startxref-composite-xref.xml'], $attachmentSummary['filenames']);
        ...
        $t->true(!str_contains($text, 'Unclosed startxref decoy page'));
        $t->true(!str_contains($text, 'Composite startxref root leak'));
        $t->true(!str_contains($encodedMetadata, 'Unclosed Startxref Composite Decoy'));
        $t->true(!str_contains($encodedFiles, 'decoy-unclosed-startxref-composite'));
        $t->true(!str_contains($encodedAttachmentSummary, 'decoy-unclosed-startxref-composite'));
