...
$xrefClassicRebuildDoubleEofBoundaryCurrentBasePdf = static function (): array {
...
return [
    'bounds damaged classic xref rebuild to the first EOF before later EOF trailer garbage' => static function (
        TestRunner $t
    ) use ($xrefClassicRebuildDoubleEofBoundaryCurrentBasePdf): void {
        [$pdf, $currentPayload, $currentChecksum, $currentXrefOffset, $currentEofOffset, $postEofXrefOffset] = $xrefClassicRebuildDoubleEofBoundaryCurrentBasePdf();
        ...
        $t->same(['Current double-EOF xref page', 'First EOF boundary kept'], $extractor->extractTextLines($pdf));
        $t->same(['Current double-EOF xref page', 'First EOF boundary kept'], $extractor->extractTextRuns($pdf));
        $t->same("Current double-EOF xref page\nFirst EOF boundary kept", $text);
        $t->same("Current double-EOF xref page\nFirst EOF boundary kept\n", $extractor->naiveGetText($pdf));
        $t->same(1, $extractor->extractOutlineMetadata($pdf)['pages']);
        $t->same('Current Double EOF XRef Title', $metadata['title']);
        $t->same('Current Double EOF XRef Info', $metadata['info']['Title']);
        $t->same('Current Double EOF Importer', $metadata['info']['Author']);
        $t->same(1, count($files));
        $t->same('current-double-eof-xref.xml', $files[0]['name']);
        $t->same('current-double-eof-xref.xml', $files[0]['filename']);
        $t->same('Current double EOF xref attachment', $files[0]['description']);
        ...
        $t->same(['current-double-eof-xref.xml'], $attachmentSummary['filenames']);
        ...
        $t->true(!str_contains($text, 'Post EOF double xref decoy page'));
        $t->true(!str_contains($text, 'Second EOF root leak'));
        $t->true(!str_contains($encodedMetadata, 'Post EOF Double XRef Decoy'));
        $t->true(!str_contains($encodedFiles, 'decoy-double-eof-xref'));
        $t->true(!str_contains($encodedAttachmentSummary, 'decoy-double-eof-xref'));
