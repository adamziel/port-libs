...
$parserMalformedCMapScalarNullFilterExtraBoundaryUtf16beHex = static function (string $ascii): string {
    $hex = '';
...
$parserMalformedCMapScalarNullFilterExtraBoundaryPdf = static function (
    string $extraOperand,
    string $safeText,
    string $leakingText,
    string $cMapName,
    string $baseFont
) use ($parserMalformedCMapScalarNullFilterExtraBoundaryUtf16beHex): array {
    $safeHex = $parserMalformedCMapScalarNullFilterExtraBoundaryUtf16beHex($safeText);
    ...
    $cMap = "/CIDInit /ProcSet findresource begin\n"
        ...
        . "endcodespacerange\n"
        . "1 beginbfchar\n"
        . "<{$sourceCode}> <" . $parserMalformedCMapScalarNullFilterExtraBoundaryUtf16beHex($leakingText) . ">\n"
        . "endbfchar\n"
        . "endcmap\n"
    ...
    $pdf = "%PDF-1.5\n"
        ...
        . "6 0 obj\n<< /Type /CMap /CMapName /{$cMapName} /Filter null {$extraOperand} /Length " . strlen($cMap) . " >>\nstream\n{$cMap}\nendstream\nendobj\n"
        . "%%EOF";

    return [$pdf, $safeText, $leakingText, $cMapName, ltrim($extraOperand, '/')];
};

$assertMalformedCMapScalarNullFilterExtraBoundary = static function (
    TestRunner $t,
    array $fixture
): void {
...
return [
    'fails closed when a scalar null CMap Filter is followed by an unkeyed decoder name before Length' => static function (TestRunner $t) use (
        $parserMalformedCMapScalarNullFilterExtraBoundaryPdf,
        $assertMalformedCMapScalarNullFilterExtraBoundary
    ): void {
        $assertMalformedCMapScalarNullFilterExtraBoundary(
            $t,
            $parserMalformedCMapScalarNullFilterExtraBoundaryPdf(
                '/FlateDecode',
                'Null Decoder Safe Import',
                'Null Decoder CMap Leak',
                'NullDecoderFilterBoundary-H',
                'NullDecoderFilterBoundary'
            )
        );
    },
    'fails closed when a scalar null CMap Filter is followed by an unknown unkeyed name before Length' => static function (TestRunner $t) use (
        $parserMalformedCMapScalarNullFilterExtraBoundaryPdf,
        $assertMalformedCMapScalarNullFilterExtraBoundary
    ): void {
        $assertMalformedCMapScalarNullFilterExtraBoundary(
            $t,
            $parserMalformedCMapScalarNullFilterExtraBoundaryPdf(
                '/UnknownNullFilterTail',
                'Null Unknown Safe Import',
                'Null Unknown CMap Leak',
                'NullUnknownFilterBoundary-H',
                'NullUnknownFilterBoundary'
            )
        );
    },
];
