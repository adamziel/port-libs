...
$xrefPrevChainFreeAnnotationCompressedPrevOwnerCurrentBasePdf = static function (): string {
    $baseContent = 'BT /F1 12 Tf 72 720 Td (Base compressed Prev owner annotation page) Tj ET';
    $currentContent = 'BT /F1 12 Tf 72 720 Td (Current compressed Prev owner annotation page) Tj ET';
    ...
    $objectStream = static function (array $members): array {
        ...
        if (!is_string($compressed)) {
            throw new RuntimeException('Unable to compress compressed-Prev owner object stream.');
        }
    ...
    $staleAnnotationOffset = $addObject(7, 0, '<< /Type /Annot /Subtype /Link /Rect [72 700 310 718] /Contents (Stale compressed Prev owner free annotation) /A << /S /URI /URI (https://stale.example.com/compressed-prev-owner-free-annotation) >> >>');
    ...
    if (!is_string($compressedRows)) {
        throw new RuntimeException('Unable to compress compressed-Prev owner xref stream.');
    }
    ...
    if ($directPrevHelperOffset >= $prevHelperCarrierOffset) {
        throw new RuntimeException('Compressed helper owner fixture must place the object-stream helper after the stale direct helper.');
    }
...
return [
    'prefers newer compressed xref-stream Prev helper over stale direct helper for freed annotations' => static function (
        TestRunner $t
    ) use ($xrefPrevChainFreeAnnotationCompressedPrevOwnerCurrentBasePdf): void {
        $pdf = $xrefPrevChainFreeAnnotationCompressedPrevOwnerCurrentBasePdf();
        $linkExtractor = new PdfLinkAnnotationExtractor();
        $annotationExtractor = new PdfAnnotationExtractor();
        $freeObjects = PdfXrefFreeObjectMap::freeObjectNumbers($pdf);

        $t->true(isset($freeObjects[7]), 'The free-object map must prefer the newer compressed /Prev helper over a stale direct helper.');
        $t->same([], $linkExtractor->extractPageLinks($pdf), 'The stale annotation freed through the compressed /Prev helper must not become a WordPress link.');
        $t->same([], $annotationExtractor->extractPageAnnotations($pdf), 'The stale annotation freed through the compressed /Prev helper must not become review metadata.');
        ...
        $pages = [[
            ...
            'blocks' => [[
                ...
                'lines' => [[
                    'bbox' => [72.0, 700.0, 310.0, 718.0],
                    'spans' => [[
                        'text' => 'Current compressed Prev owner annotation page',
                        'bbox' => [72.0, 700.0, 310.0, 718.0],
                        'font' => 'Helvetica',
                    ]],
                ]],
