...
$fontType3ColorGlyphResourceWidthCurrentBasePdf = static function (): string {
...
return [
    'uses named Type3 color glyph CharProc widths while excluding resource payload text on current base' => static function (TestRunner $t) use ($fontType3ColorGlyphResourceWidthCurrentBasePdf): void {
        $extractor = new PdfTextExtractor();
        $pdf = $fontType3ColorGlyphResourceWidthCurrentBasePdf();
        ...
        $t->true(!str_contains($plainText, 'Color glyph resource text leak'));
        $t->same('T3ColorGlyphSerif_symbolic_non_symbolic', $firstSpan['font'] ?? null);
        $t->same((1 << 2) | (1 << 5), $firstSpan['font_flags'] ?? null);
    },
];
