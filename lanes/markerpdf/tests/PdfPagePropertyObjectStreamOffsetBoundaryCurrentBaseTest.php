...
$pagePropertyObjectStreamOffsetBoundaryPdf = static function (): string {
    ...
    if (!is_string($objectStream)) {
        throw new RuntimeException('Unable to compress page-property object stream fixture.');
    }
    ...
    if (!is_string($compressedXref)) {
        throw new RuntimeException('Unable to compress page-property xref stream fixture.');
    }
...
return [
    'rejects page review object-stream offsets that point inside literal-string decoys' => static function (TestRunner $t) use ($pagePropertyObjectStreamOffsetBoundaryPdf): void {
        $pdf = $pagePropertyObjectStreamOffsetBoundaryPdf();
        ...
        $t->same('D:20260607082214Z', $page['piece_info']['WPImport']['last_modified']);
        $t->same('current-object-stream-page-review', $page['piece_info']['WPImport']['private']['BatchId']);
        $t->same(true, $page['piece_info']['WPImport']['private']['NeedsReview']);
        $t->same(['source.xml'], array_column($page['page_associated_files'], 'filename'));
        $t->same(['Source'], array_column($page['page_associated_files'], 'relationship'));
        $t->same(hash('sha256', '<wp-export><post id="page-property-boundary"/></wp-export>'), $page['page_associated_files'][0]['content_sha256']);
        $t->same(false, array_key_exists('content', $page['page_associated_files'][0]));

        $encoded = json_encode($pages, JSON_UNESCAPED_SLASHES);
        $t->contains('Visible page property boundary text', $plainText);
        $t->same(false, str_contains($plainText, 'Offset boundary hidden page leak'));
        $t->same(false, is_string($encoded) && str_contains($encoded, 'offset-decoy-page-review'));
        $t->same(false, is_string($encoded) && str_contains($encoded, 'D:20260607000000Z'));
    },
];
