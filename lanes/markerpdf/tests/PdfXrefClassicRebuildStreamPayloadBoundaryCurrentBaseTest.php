...
$xrefClassicRebuildStreamPayloadBoundaryPdf = static function (): array {
...
return [
    'keeps stream-owned fake classic xref tables out of rebuild before WordPress attachment review' => static function (
        TestRunner $t
    ) use ($xrefClassicRebuildStreamPayloadBoundaryPdf): void {
        [$pdf, $currentPayload, $currentChecksum, $previousXrefOffset, $currentXrefOffset, $streamOwnerOffset] = $xrefClassicRebuildStreamPayloadBoundaryPdf();
        ...
        $encodedReview = json_encode([$attachmentSummary, $embeddedFiles, $freeObjects, $linkedPages], JSON_UNESCAPED_SLASHES) ?: '';

        $t->true($previousXrefOffset > 0);
        $t->true($currentXrefOffset > $previousXrefOffset);
        $t->true($streamOwnerOffset > $currentXrefOffset);
        $t->true(str_contains($pdf, "stream preface endobj\nxref\n"));
        $t->same(['Current stream-owned xref page', 'Stream payload xref ignored'], $textExtractor->extractTextLines($pdf));
        $t->same("Current stream-owned xref page\nStream payload xref ignored", $text);
        $t->same(1, count($embeddedFiles));
        $t->same('current-stream-owned-xref.xml', $embeddedFiles[0]['name']);
        ...
        $t->same(['current-stream-owned-xref.xml'], $attachmentSummary['filenames']);
        ...
        $t->same([], $linkedPages, 'The stale stream-owned fake xref must not resurrect a freed link annotation.');
        $t->true(!str_contains($encodedReview, 'stale-stream-owned-xref'));
        $t->true(!str_contains($encodedReview, 'stale.example.com/stream-owned-xref'));
        $t->true(!str_contains($text, 'Stale stream-owned xref page'));
        $t->true(!str_contains($text, 'Stream payload xref leak'));
