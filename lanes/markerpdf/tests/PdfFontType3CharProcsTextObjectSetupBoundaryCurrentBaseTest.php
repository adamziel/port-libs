...
$type3CharProcsTextObjectSetupBoundaryCurrentBasePdf = static function (): string {
...
return [
    'accepts no-paint Type3 CharProc text objects before metrics on current base' => static function (
        TestRunner $t
    ) use ($type3CharProcsTextObjectSetupBoundaryCurrentBasePdf): void {
        $extractor = new PdfTextExtractor();
        $pdf = $type3CharProcsTextObjectSetupBoundaryCurrentBasePdf();
        $plainText = $extractor->extractPlainText($pdf);

        $t->same(['WideBlock', 'Thin Text', 'Paint Gap', 'Open Gap'], $extractor->extractTextLines($pdf));
        $t->same(['Wide', 'Block', 'Thin', 'Text', 'Paint', 'Gap', 'Open', 'Gap'], $extractor->extractTextRuns($pdf));
        $t->same("WideBlock\nThin Text\nPaint Gap\nOpen Gap", $plainText);
        $t->same("WideBlock\nThin Text\nPaint Gap\nOpen Gap\n", $extractor->naiveGetText($pdf));
        $t->true(str_contains($plainText, 'WideBlock'));
        $t->true(str_contains($plainText, 'Thin Text'));
        $t->true(str_contains($plainText, 'Paint Gap'));
        $t->true(str_contains($plainText, 'Open Gap'));
        $t->true(!str_contains($plainText, 'Wide Block'));
        $t->true(!str_contains($plainText, 'ThinText'));
        $t->true(!str_contains($plainText, 'PaintGap'));
        $t->true(!str_contains($plainText, 'OpenGap'));
        $t->true(!str_contains($plainText, 'text-object setup charproc text leak'));
