...
$type3CharProcsSiblingGenerationBoundaryCurrentBasePdf = static function (): string {
...
return [
    'keeps stale same-object Type3 CharProcs dictionary generations private during fallback extraction on current base' => static function (
        TestRunner $t
    ) use ($type3CharProcsSiblingGenerationBoundaryCurrentBasePdf): void {
        $extractor = new PdfTextExtractor();
        $pdf = $type3CharProcsSiblingGenerationBoundaryCurrentBasePdf();
        ...
        $t->true(!str_contains($plainText, 'CURRENT SIBLING GLYPH LEAK'));
        $t->true(!str_contains($plainText, 'STALE SIBLING GLYPH LEAK'));
        $t->true(!str_contains($plainText, 'T3CharProcsSiblingGeneration'));
    },
];
