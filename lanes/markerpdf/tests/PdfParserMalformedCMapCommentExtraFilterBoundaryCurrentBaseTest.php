...
$parserMalformedCMapCommentExtraFilterBoundaryUtf16beHex = static function (string $ascii): string {
    $hex = '';
...
$parserMalformedCMapCommentExtraFilterBoundaryPdf = static function () use (
    $parserMalformedCMapCommentExtraFilterBoundaryUtf16beHex
): array {
    $safeText = 'Comment Extra Safe Import';
    $leakingText = 'Comment Extra CMap Leak';
    $safeHex = $parserMalformedCMapCommentExtraFilterBoundaryUtf16beHex($safeText);
    $sourceCode = substr($safeHex, 0, 4);
    $cMapName = 'CommentExtraFilterBoundary-H';
    ...
    $cMap = "/CIDInit /ProcSet findresource begin\n"
        ...
        . "endcodespacerange\n"
        . "1 beginbfchar\n"
        . "<{$sourceCode}> <" . $parserMalformedCMapCommentExtraFilterBoundaryUtf16beHex($leakingText) . ">\n"
        . "endbfchar\n"
        . "endcmap\n"
    ...
    if (!is_string($compressedCMap)) {
        throw new RuntimeException('Unable to compress comment-extra CMap filter fixture.');
    }
    ...
    return [
        "%PDF-1.5\n"
            ...
            . "4 0 obj\n<< /Type /Font /Subtype /Type0 /BaseFont /CommentExtraFilterBoundary /Encoding /Identity-H /ToUnicode 6 0 R >>\nendobj\n"
            . "5 0 obj\n<< /Length " . strlen($content) . " >>\nstream\n{$content}\nendstream\nendobj\n"
            . "6 0 obj\n<< /Type /CMap /CMapName /{$cMapName} /Filter /FlateDecode % comment before hidden decoder\n/{$extraFilterName} /Length " . strlen($compressedCMap) . " >>\nstream\n{$compressedCMap}\nendstream\nendobj\n"
            . "%%EOF",
        $safeText,
        $leakingText,
        $cMapName,
...
return [
    'fails closed when a CMap Filter decoder name is hidden after a PDF comment' => static function (
        TestRunner $t
    ) use ($parserMalformedCMapCommentExtraFilterBoundaryPdf): void {
        [$pdf, $safeText, $leakingText, $cMapName, $extraFilterName] = $parserMalformedCMapCommentExtraFilterBoundaryPdf();
        ...
        $t->same(true, $filterOperand['extra_filter_operand_after_comment'] ?? null);
