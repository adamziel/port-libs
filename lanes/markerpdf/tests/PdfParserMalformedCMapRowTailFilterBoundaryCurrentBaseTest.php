...
$parserMalformedCMapRowTailFilterBoundaryUtf16beHex = static function (string $ascii): string {
    $hex = '';
...
$parserMalformedCMapRowTailFilterBoundaryPdf = static function (
    string $cMapRows,
    string $safeText,
    string $leakingText,
    string $cMapName,
    string $baseFont
) use ($parserMalformedCMapRowTailFilterBoundaryUtf16beHex): array {
    $safeHex = $parserMalformedCMapRowTailFilterBoundaryUtf16beHex($safeText);
    $sourceCode = substr($safeHex, 0, 4);
    $cMap = str_replace(
        ['{{SOURCE}}', '{{LEAK_HEX}}', '{{CMAP_NAME}}'],
        [$sourceCode, $parserMalformedCMapRowTailFilterBoundaryUtf16beHex($leakingText), $cMapName],
        $cMapRows
    );
    $compressedCMap = gzcompress($cMap, 0);
    if (!is_string($compressedCMap)) {
        throw new RuntimeException('Unable to compress focused malformed CMap row-tail fixture.');
    }
...
$assertMalformedCMapRowTailFilterBoundary = static function (TestRunner $t, array $fixture): void {
...
return [
    'ignores malformed codespace rows with nested array tails before filtered CMap range replacement' => static function (
        TestRunner $t
    ) use ($parserMalformedCMapRowTailFilterBoundaryPdf, $assertMalformedCMapRowTailFilterBoundary): void {
        ...
        $assertMalformedCMapRowTailFilterBoundary(
            $t,
            $parserMalformedCMapRowTailFilterBoundaryPdf(
                $cMap,
                'Codespace Tail Safe Import',
                'Codespace Tail CMap Leak',
                'CodespaceRowTailBoundary-H',
                'CodespaceRowTailBoundary'
            )
        );
    },
    'ignores malformed bfrange rows with nested array tails before filtered CMap text replacement' => static function (
        TestRunner $t
    ) use ($parserMalformedCMapRowTailFilterBoundaryPdf, $assertMalformedCMapRowTailFilterBoundary): void {
        $cMap = "/CIDInit /ProcSet findresource begin\n"
            . "12 dict begin\n"
        ...
        $assertMalformedCMapRowTailFilterBoundary(
            ...
            $parserMalformedCMapRowTailFilterBoundaryPdf(
                ...
                'BfrangeRowTailBoundary-H',
                'BfrangeRowTailBoundary'
            )
        );
    },
];
