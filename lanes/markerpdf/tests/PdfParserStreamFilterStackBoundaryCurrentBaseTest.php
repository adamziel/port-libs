...
$parserStreamFilterStackBoundaryCurrentBaseNonPdfWhitespacePdf = static function () use (
    $parserStreamFilterStackBoundaryCurrentBaseAscii85
): string {
    $asciiHexLeak = 'BT /F1 12 Tf 72 720 Td (Vertical Tab ASCIIHex Leak) Tj ET';
    $asciiHexEncoded = strtoupper(bin2hex($asciiHexLeak));
    $asciiHexEncoded = substr($asciiHexEncoded, 0, 12) . "\x0b" . substr($asciiHexEncoded, 12) . '>';

    $ascii85Leak = 'BT /F1 12 Tf 72 700 Td (Vertical Tab ASCII85 Leak) Tj ET';
    $ascii85Encoded = $parserStreamFilterStackBoundaryCurrentBaseAscii85($ascii85Leak);
    $ascii85Encoded = substr($ascii85Encoded, 0, 8) . "\x0b" . substr($ascii85Encoded, 8) . '~>';

    $visibleAfter = 'BT /F1 12 Tf 72 680 Td (Visible After Vertical Tab Filter Whitespace) Tj ET';

    return "%PDF-1.4\n"
        . "1 0 obj\n<< /Type /Catalog /Pages 2 0 R >>\nendobj\n"
        . "2 0 obj\n<< /Type /Pages /Kids [3 0 R] /Count 1 >>\nendobj\n"
        . "3 0 obj\n<< /Type /Page /Parent 2 0 R /Resources << /Font << /F1 5 0 R >> >> /Contents [4 0 R 6 0 R 8 0 R] >>\nendobj\n"
        . "4 0 obj\n<< /Filter /ASCIIHexDecode /Length " . strlen($asciiHexEncoded) . " >>\nstream\n{$asciiHexEncoded}\nendstream\nendobj\n"
...
$parserStreamFilterStackBoundaryCurrentBaseEarlyChangeBoundaryPdf = static function () use (
    $parserStreamFilterStackBoundaryCurrentBaseAscii85,
    $parserStreamFilterStackBoundaryCurrentBaseLzwLiteral,
    $parserStreamFilterStackBoundaryCurrentBaseZlibStored
): string {
    $nonLzwLeak = 'BT /F1 12 Tf 72 720 Td (EarlyChange Non LZW Leak) Tj ET';
    $nonLzwEncoded = $parserStreamFilterStackBoundaryCurrentBaseAscii85(
        $parserStreamFilterStackBoundaryCurrentBaseZlibStored($nonLzwLeak)
    ) . '~>';

    $lzwContent = 'BT /F1 12 Tf 72 700 Td (LZW EarlyChange Boundary Imports) Tj ET';
    $lzwEncoded = $parserStreamFilterStackBoundaryCurrentBaseLzwLiteral(
        $parserStreamFilterStackBoundaryCurrentBaseZlibStored($lzwContent)
    );
    $visibleAfter = 'BT /F1 12 Tf 72 680 Td (Visible After EarlyChange Boundary) Tj ET';
    ...
    return "%PDF-1.4\n"
        ...
        . "5 0 obj\n<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica /Encoding /WinAnsiEncoding >>\nendobj\n"
        . "6 0 obj\n<< /Filter [ /LZWDecode /FlateDecode ] /DecodeParms [ << /EarlyChange 0 >> null ] /Length " . strlen($lzwEncoded) . " >>\nstream\n{$lzwEncoded}\nendstream\nendobj\n"
        . "7 0 obj\n<< /Length " . strlen($visibleAfter) . " >>\nstream\n{$visibleAfter}\nendstream\nendobj\n"
        . "%%EOF";
};
...
$parserStreamFilterStackBoundaryCurrentBaseMalformedIndirectLengthPdf = static function () use (
    $parserStreamFilterStackBoundaryCurrentBaseZlibStored
): string {
    $leak = 'BT /F1 12 Tf 72 720 Td (Malformed Indirect Length Leak) Tj ET';
...
$parserStreamFilterStackBoundaryCurrentBaseAscii85OverflowPdf = static function () use (
    $parserStreamFilterStackBoundaryCurrentBaseAscii85
): string {
    $fullTupleLeak = ' BT /F1 12 Tf 72 720 Td (ASCII85 Overflow Full Tuple Leak) Tj ET';
    $fullTupleEncoded = 'uuuuu' . $parserStreamFilterStackBoundaryCurrentBaseAscii85($fullTupleLeak) . '~>';

    $partialTupleLeak = 'BT /F1 12 Tf 72 700 Td (ASCII85 Overflow Partial Tuple Leak) Tj ET';
    while (strlen($partialTupleLeak) % 4 !== 0) {
        $partialTupleLeak .= ' ';
    }
    $partialTupleEncoded = $parserStreamFilterStackBoundaryCurrentBaseAscii85($partialTupleLeak) . 'uu~>';
    $visibleAfter = 'BT /F1 12 Tf 72 680 Td (Visible After ASCII85 Overflow Boundary) Tj ET';
...
return [
    ...
    'rejects non-PDF whitespace inside ASCIIHex and ASCII85 stream filter data' => static function (TestRunner $t) use ($parserStreamFilterStackBoundaryCurrentBaseNonPdfWhitespacePdf): void {
        $extractor = new PdfTextExtractor();
        $pdf = $parserStreamFilterStackBoundaryCurrentBaseNonPdfWhitespacePdf();
        $text = $extractor->extractPlainText($pdf);

        $expected = ['Visible After Vertical Tab Filter Whitespace'];
        $t->same($expected, $extractor->extractTextLines($pdf));
        $t->same($expected, $extractor->extractTextRuns($pdf));
        $t->same('Visible After Vertical Tab Filter Whitespace', $text);
        $t->same("Visible After Vertical Tab Filter Whitespace\n", $extractor->naiveGetText($pdf));
        $t->same(1, $extractor->extractOutlineMetadata($pdf)['pages']);
        $t->same(['1'], $extractor->extractPageLabels($pdf));
    ...
    'rejects non-default LZW-only EarlyChange DecodeParms on non-LZW stream filters' => static function (TestRunner $t) use ($parserStreamFilterStackBoundaryCurrentBaseEarlyChangeBoundaryPdf): void {
        $extractor = new PdfTextExtractor();
        $pdf = $parserStreamFilterStackBoundaryCurrentBaseEarlyChangeBoundaryPdf();
        $text = $extractor->extractPlainText($pdf);

        $expected = [
            'LZW EarlyChange Boundary Imports',
            'Visible After EarlyChange Boundary',
        ];
        $t->same($expected, $extractor->extractTextLines($pdf));
        $t->same($expected, $extractor->extractTextRuns($pdf));
        $t->same(implode("\n", $expected), $text);
        ...
        $t->same(1, $extractor->extractOutlineMetadata($pdf)['pages']);
        $t->same(['1'], $extractor->extractPageLabels($pdf));
        $t->true(!str_contains($text, 'EarlyChange Non LZW Leak'));
        $t->true(!str_contains($text, 'ASCII85Decode'));
        $t->true(!str_contains($text, 'EarlyChange 0'));
        $t->true(!str_contains($text, 'FlateDecode'));
        $t->true(!str_contains($text, "\0"));
    },
    ...
    'rejects malformed indirect Length helper objects before filtered page text import' => static function (TestRunner $t) use ($parserStreamFilterStackBoundaryCurrentBaseMalformedIndirectLengthPdf): void {
        $extractor = new PdfTextExtractor();
        $pdf = $parserStreamFilterStackBoundaryCurrentBaseMalformedIndirectLengthPdf();
        $text = $extractor->extractPlainText($pdf);

        $expected = ['Visible After Malformed Length'];
        $t->same($expected, $extractor->extractTextLines($pdf));
        $t->same($expected, $extractor->extractTextRuns($pdf));
        $t->same('Visible After Malformed Length', $text);
        $t->same("Visible After Malformed Length\n", $extractor->naiveGetText($pdf));
        $t->same(1, $extractor->extractOutlineMetadata($pdf)['pages']);
        $t->same(['1'], $extractor->extractPageLabels($pdf));
    ...
    'rejects ASCII85 overflow tuples before importing page text from stream filter stacks' => static function (TestRunner $t) use ($parserStreamFilterStackBoundaryCurrentBaseAscii85OverflowPdf): void {
        $extractor = new PdfTextExtractor();
        $pdf = $parserStreamFilterStackBoundaryCurrentBaseAscii85OverflowPdf();
        $text = $extractor->extractPlainText($pdf);

        $expected = ['Visible After ASCII85 Overflow Boundary'];
        $t->same($expected, $extractor->extractTextLines($pdf));
        $t->same($expected, $extractor->extractTextRuns($pdf));
        $t->same('Visible After ASCII85 Overflow Boundary', $text);
        $t->same("Visible After ASCII85 Overflow Boundary\n", $extractor->naiveGetText($pdf));
        $t->same(1, $extractor->extractOutlineMetadata($pdf)['pages']);
        $t->same(['1'], $extractor->extractPageLabels($pdf));
        $t->true(!str_contains($text, 'ASCII85 Overflow Full Tuple Leak'));
        $t->true(!str_contains($text, 'ASCII85 Overflow Partial Tuple Leak'));
        $t->true(!str_contains($text, 'ASCII85Decode'));
        $t->true(!str_contains($text, 'uuuuu'));
        $t->true(!str_contains($text, 'uu~>'));
        $t->true(!str_contains($text, "\0"));
    },
];
