...
$parserMalformedCMapDeclaredCountFilterUtf16beHex = static function (string $ascii): string {
    $hex = '';
...
$parserMalformedCMapDeclaredCountFilterPdf = static function (
    string $mappingKind
) use ($parserMalformedCMapDeclaredCountFilterUtf16beHex): array {
    $safeText = $mappingKind === 'range'
        ? 'Declared Range Safe Import'
        : 'Declared Char Safe Import';
    $leakingText = $mappingKind === 'range'
        ? 'Declared Range CMap Leak'
        : 'Declared Char CMap Leak';
    $safeHex = $parserMalformedCMapDeclaredCountFilterUtf16beHex($safeText);
    $sourceCode = substr($safeHex, 0, 4);
    $cMapName = $mappingKind === 'range'
        ? 'DeclaredCountMalformedRange-H'
        : 'DeclaredCountMalformedChar-H';
    $baseFont = $mappingKind === 'range'
        ? 'DeclaredCountMalformedRange'
        : 'DeclaredCountMalformedChar';
    $mappingBlock = $mappingKind === 'range'
        ? "1 beginbfrange\n"
            . "<{$sourceCode}> <{$sourceCode}> << /Malformed (first declared bfrange row is not a mapping) >>\n"
            . "<{$sourceCode}> <{$sourceCode}> <" . $parserMalformedCMapDeclaredCountFilterUtf16beHex($leakingText) . ">\n"
            . "endbfrange\n"
        : "1 beginbfchar\n"
            . "<{$sourceCode}> << /Malformed (first declared bfchar row is not a mapping) >>\n"
            . "<{$sourceCode}> <" . $parserMalformedCMapDeclaredCountFilterUtf16beHex($leakingText) . ">\n"
            . "endbfchar\n";
    ...
    if (!is_string($compressedCMap)) {
        throw new RuntimeException('Unable to compress malformed declared-count CMap fixture.');
    }
...
$assertMalformedCMapDeclaredCountFilter = static function (TestRunner $t, array $fixture): void {
...
return [
    'keeps filtered bfchar declared row counts before malformed-row recovery on current base' => static function (
        TestRunner $t
    ) use ($parserMalformedCMapDeclaredCountFilterPdf, $assertMalformedCMapDeclaredCountFilter): void {
        $assertMalformedCMapDeclaredCountFilter($t, $parserMalformedCMapDeclaredCountFilterPdf('char'));
    },
    'keeps filtered bfrange declared row counts before malformed-row recovery on current base' => static function (
        TestRunner $t
    ) use ($parserMalformedCMapDeclaredCountFilterPdf, $assertMalformedCMapDeclaredCountFilter): void {
        $assertMalformedCMapDeclaredCountFilter($t, $parserMalformedCMapDeclaredCountFilterPdf('range'));
    },
];
