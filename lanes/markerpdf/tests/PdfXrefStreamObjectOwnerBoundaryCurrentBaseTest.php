...
$xrefStreamObjectOwnerBoundaryCurrentBasePdf = static function (): string {
    ...
    if (!is_string($fakeCompressedXref)) {
        throw new RuntimeException('Unable to compress fake xref-stream fixture.');
    }
    ...
    if (!is_string($realCompressedXref)) {
        throw new RuntimeException('Unable to compress real xref-stream fixture.');
    }
...
return [
    'rejects stream-owned xref stream objects before current-base WordPress text extraction' => static function (TestRunner $t) use ($xrefStreamObjectOwnerBoundaryCurrentBasePdf): void {
        $extractor = new PdfTextExtractor();
        $pdf = $xrefStreamObjectOwnerBoundaryCurrentBasePdf();
        $text = $extractor->extractPlainText($pdf);

        $t->same(['Current xref-stream owner page', 'Current base xref stream wins'], $extractor->extractTextLines($pdf));
        $t->same(['Current xref-stream owner page', 'Current base xref stream wins'], $extractor->extractTextRuns($pdf));
        $t->same("Current xref-stream owner page\nCurrent base xref stream wins", $text);
        $t->same("Current xref-stream owner page\nCurrent base xref stream wins\n", $extractor->naiveGetText($pdf));
        $t->same(1, $extractor->extractOutlineMetadata($pdf)['pages']);
        $t->same(['1'], $extractor->extractPageLabels($pdf));
        $t->true(!str_contains($text, 'Stale embedded xref stream page'));
        $t->true(!str_contains($text, 'Stream-owned xref object leak'));
        $t->true(!str_contains($text, 'Owner carrier stream payload'));
