...
$xrefFreeAnnotationFilterStackCurrentBasePdf = static function (): string {
    $asciiHexEncode = static fn (string $bytes): string => strtoupper(bin2hex($bytes)) . '>';
    $previousContent = 'BT /F1 12 Tf 72 720 Td (Previous filter stack free annotation page) Tj ET';
    $currentContent = 'BT /F1 12 Tf 72 720 Td (Current filter stack free annotation page) Tj ET';
    ...
    $addObject(7, 0, '<< /Type /Annot /Subtype /Link /Rect [72 700 250 718] /Contents (Stale filter stack annotation) /A << /S /URI /URI (https://stale.example.com/filter-stack-free-annotation) >> >>');
    ...
    if (!is_string($compressedRows)) {
        throw new RuntimeException('Unable to compress filter-stack free-entry xref-stream fixture.');
    }
...
return [
    'suppresses stale free annotations from current xref streams with ASCIIHex and Flate filters' => static function (TestRunner $t) use (
        $xrefFreeAnnotationFilterStackCurrentBasePdf
    ): void {
        $pdf = $xrefFreeAnnotationFilterStackCurrentBasePdf();
        $linkExtractor = new PdfLinkAnnotationExtractor();
        $annotationExtractor = new PdfAnnotationExtractor();
        $freeObjects = PdfXrefFreeObjectMap::freeObjectNumbers($pdf);

        $t->true(isset($freeObjects[7]), 'The free-object map must decode xref-stream filter stacks before reading free rows.');
        ...
        $pages = [[
            ...
            'blocks' => [[
                ...
                'lines' => [[
                    'bbox' => [72.0, 700.0, 250.0, 718.0],
                    'spans' => [[
                        'text' => 'Current filter stack free annotation page',
                        'bbox' => [72.0, 700.0, 250.0, 718.0],
                        'font' => 'Helvetica',
                    ]],
                ]],
        ...
        $t->true(str_contains($pdf, '/Filter [ /ASCIIHexDecode /FlateDecode ]'));
        ...
        $t->true(!str_contains($encodedReview, 'Stale filter stack annotation'));
    },
];
