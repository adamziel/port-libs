...
$type3CharProcsNestedDictionaryBoundaryCurrentBasePdf = static function (): string {
...
return [
    'uses only top-level entries inside Type3 CharProcs dictionaries before WordPress text grouping on current base' => static function (TestRunner $t) use ($type3CharProcsNestedDictionaryBoundaryCurrentBasePdf): void {
        $extractor = new PdfTextExtractor();
        $pdf = $type3CharProcsNestedDictionaryBoundaryCurrentBasePdf();
        ...
        $t->true(!str_contains($plainText, 'nested dictionary text leak'));
        $t->true(!str_contains($plainText, 'nested dictionary decoy leak'));
    },
];
