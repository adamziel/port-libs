...
$pdfDctDecodeIndirectScalarFilterTailBoundaryFixture = static function (): array {
    $before = 'BT /F1 12 Tf 72 720 Td (Before indirect scalar DCT filter tail) Tj ET';
    $after = 'BT /F1 12 Tf 72 680 Td (After indirect scalar DCT filter tail) Tj ET';
    $fakeObject = 'BT /F1 12 Tf 72 700 Td (Indirect scalar DCT filter tail payload leak) Tj ET';
    $jpegPayload = "\xff\xd8\xff\xe0JFIF\0indirect scalar filter tail bytes\n"
        . "endstream\nendobj\n"
        . "9 0 obj\n<< /Length " . strlen($fakeObject) . " >>\nstream\n{$fakeObject}\nendstream\nendobj\n"
        . "\xff\xd9";
    $fakeTerminatorOffset = strpos($jpegPayload, "\nendstream\n");
    if ($fakeTerminatorOffset === false) {
        throw new RuntimeException('Focused indirect scalar DCT filter-tail fixture must expose a fake endstream marker.');
    }

    $filterObject = '/DCTDecode /Crypt';
    $imageDictionary = '<< /Type /XObject /Subtype /Image /Width 1 /Height 1 /ColorSpace /DeviceRGB /BitsPerComponent 8 /Filter 20 0 R /DecodeParms << /ColorTransform 1 >> /Length ' . $fakeTerminatorOffset . ' >>';
    $pageContent = $before . "\nq 24 0 0 24 72 680 cm /Photo Do Q\n" . $after;
    $rendererDictionary = str_replace('/ColorSpace /DeviceRGB', '/ColorSpace [/ICCBased 30 0 R]', $imageDictionary);

    return [
        'expected_lines' => [
            'Before indirect scalar DCT filter tail',
            'After indirect scalar DCT filter tail',
        ],
...
return [
    'preserves indirect scalar DCTDecode filter names when trailing operands fail closed' => static function (
        TestRunner $t
    ) use ($pdfDctDecodeIndirectScalarFilterTailBoundaryFixture): void {
        $fixture = $pdfDctDecodeIndirectScalarFilterTailBoundaryFixture();
        ...
        $plan = $renderer->imageColorSpaceSoftMaskPlan($fixture['image_dictionary'], [20 => $fixture['filter_object']]);
        ...
        $t->true(!str_contains($plainText, 'Indirect scalar DCT filter tail payload leak'));
        ...
        $t->true(!str_contains($reviewJson, 'Indirect scalar DCT filter tail payload leak'));
