...
$duplicateCryptFilterNamePdf = static function (): array {
    $content = 'BT /F1 12 Tf 72 720 Td (Duplicate decoded crypt filter name encrypted text leak) Tj ET';
    $ownerKey = str_repeat('H', 32);
    $userKey = str_repeat('I', 32);
    ...
    $pdf = "%PDF-1.7\n"
        ...
        . " /CF << /StdCF << /CFM /AESV2 /AuthEvent /DocOpen /Length 16 >> /Std#43F << /CFM /Identity /AuthEvent /DocOpen >> /ClearEmbedded << /CFM /Identity /AuthEvent /EFOpen >> >>"
        . " /StmF /StdCF /StrF /StdCF /EFF /ClearEmbedded >>\nendobj\n"
...
return [
    'fails closed on duplicate decoded crypt-filter names before permission import review' => static function (
        TestRunner $t
    ) use ($duplicateCryptFilterNamePdf): void {
        [$pdf, $content, $ownerKey, $userKey] = $duplicateCryptFilterNamePdf();
        ...
        $t->same(1, $dictionaryReview['declared_entry_count']);
        $t->same(1, $dictionaryReview['resolved_dictionary_entry_count']);
        $t->same(false, $dictionaryReview['duplicate_entries']);
        $t->same(true, $dictionaryReview['duplicate_filter_name_entries']);
        $t->same(['StdCF'], $dictionaryReview['duplicate_filter_names']);
        $t->same(1, $dictionaryReview['duplicate_filter_name_count']);
        ...
        $t->same('duplicate_crypt_filter_name_entries_review', $dictionaryReview['status']);
        $t->same(['crypt_filter_dictionary_entry_resolved'], $dictionaryReview['entry_statuses']);
        $t->same(['StdCF', 'ClearEmbedded'], $dictionaryReview['declared_filter_names']);
        $t->same(0, $dictionaryReview['selected_entry_index']);
        $t->same(['StdCF', 'ClearEmbedded'], $dictionaryReview['selected_filter_names']);
        $t->same(['StdCF'], $dictionaryReview['selected_entry_duplicate_filter_names']);
        $t->same($dictionaryReview, $reportEncryption['crypt_filter_dictionary_declaration_review']);
        $t->same('duplicate_crypt_filter_name_entries_review', $reportEncryption['crypt_filter_dictionary_status']);
        ...
        $t->same('duplicate_crypt_filter_name_entries_fail_closed', $review['text_content_policy']);
        $t->same('duplicate_crypt_filter_name_entries_fail_closed', $review['embedded_file_payload_policy']);
        $t->same(true, $review['crypt_filter_dictionary_fail_closed']);
        $t->same('duplicate_crypt_filter_name_entries_review', $review['crypt_filter_dictionary_declaration_status']);
        $t->same(['StdCF'], $review['crypt_filter_dictionary_duplicate_filter_names']);
        $t->same(1, $review['crypt_filter_dictionary_duplicate_filter_name_count']);
        $t->same(['document_streams', 'document_strings', 'embedded_file_streams'], $review['fail_closed_role_names']);
        $t->same(['StdCF', 'ClearEmbedded'], $review['fail_closed_filter_names']);
        ...
        $t->same('StdCF', $streamRole['filter_name']);
        ...
        $t->same('duplicate_crypt_filter_name_entries_review', $streamRole['crypt_filter_dictionary_declaration_status']);
        $t->same(['StdCF'], $streamRole['crypt_filter_dictionary_duplicate_filter_names']);
        ...
        $t->same('blocked_by_duplicate_document_crypt_filter_name', $permission['content_extraction_boundary']);
        $t->same('duplicate_crypt_filter_name_entries_fail_closed', $permission['crypt_filter_text_policy']);
        ...
        $t->same(['StdCF', 'ClearEmbedded'], $permission['crypt_filter_fail_closed_filter_names']);
