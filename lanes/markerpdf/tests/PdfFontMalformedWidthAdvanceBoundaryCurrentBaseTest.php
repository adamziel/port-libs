...
$fontIndirectWidthArrayTailAdvanceBoundaryCurrentBasePdf = static function (): string {
...
return [
    ...
    'rejects indirect simple-font Widths array helper tail operands before advance grouping on current base' => static function (TestRunner $t) use ($fontIndirectWidthArrayTailAdvanceBoundaryCurrentBasePdf): void {
        $extractor = new PdfTextExtractor();
        $pdf = $fontIndirectWidthArrayTailAdvanceBoundaryCurrentBasePdf();
        $plainText = $extractor->extractPlainText($pdf);
        $pages = $extractor->extractStyledTextPages($pdf);
        $line = $pages[0]['blocks'][0]['lines'][0] ?? [];
        $spans = $line['spans'] ?? [];
        ...
        $t->same('Helvetica_', $spans[0]['font'] ?? null);
        $t->same('Helvetica_', $spans[1]['font'] ?? null);
        $t->true(isset($firstBbox[2]) && abs(((float) $firstBbox[2]) - 8.664) < 0.001, 'Expected indirect Widths helper tail to reject explicit 1000-unit advances.');
        $t->true(isset($secondBbox[0], $firstBbox[2]) && ((float) $secondBbox[0]) - ((float) $firstBbox[2]) > 12.0, 'Expected Base14 fallback to preserve the positioned word gap.');
        $t->true(isset($secondBbox[2], $secondBbox[0]) && ((float) $secondBbox[2]) - ((float) $secondBbox[0]) > 28.0, 'Expected Base14 Helvetica width for Word.');
        $t->true(!str_contains($plainText, 'IllWord'));
        $t->true(!str_contains($plainText, 'BadWidth'));
        $t->true(!str_contains($plainText, "\0"));
