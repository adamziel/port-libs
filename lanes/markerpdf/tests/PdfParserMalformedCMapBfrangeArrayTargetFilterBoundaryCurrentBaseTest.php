...
$parserMalformedCMapBfrangeArrayTargetFilterBoundaryUtf16beHex = static function (string $ascii): string {
    $hex = '';
...
$parserMalformedCMapBfrangeArrayTargetFilterBoundaryPdf = static function () use (
    $parserMalformedCMapBfrangeArrayTargetFilterBoundaryUtf16beHex
): array {
    $safeText = 'Array Target Safe Import';
    $leakingText = 'Array Target CMap Leak';
    $decoyText = 'Array Target Decoy';
    $safeHex = $parserMalformedCMapBfrangeArrayTargetFilterBoundaryUtf16beHex($safeText);
    $sourceCode = substr($safeHex, 0, 4);
    $cMapName = 'BfrangeArrayTargetFilterBoundary-H';
    $cMap = "/CIDInit /ProcSet findresource begin\n"
        ...
        . "1 beginbfrange\n"
        . "<{$sourceCode}> <{$sourceCode}> [ <"
        . $parserMalformedCMapBfrangeArrayTargetFilterBoundaryUtf16beHex($leakingText)
        . "> <"
        . $parserMalformedCMapBfrangeArrayTargetFilterBoundaryUtf16beHex($decoyText)
        . "> ]\n"
        . "endbfrange\n"
        . "endcmap\n"
    ...
    if (!is_string($compressedCMap)) {
        throw new RuntimeException('Unable to compress bfrange array-target CMap fixture.');
    }
    ...
    $pdf = "%PDF-1.5\n"
        ...
        . "4 0 obj\n<< /Type /Font /Subtype /Type0 /BaseFont /BfrangeArrayTargetFilterBoundary /Encoding /Identity-H /ToUnicode 6 0 R >>\nendobj\n"
        . "5 0 obj\n<< /Length " . strlen($content) . " >>\nstream\n{$content}\nendstream\nendobj\n"
        . "6 0 obj\n<< /Type /CMap /CMapName /{$cMapName} /Filter /FlateDecode /Length " . strlen($compressedCMap) . " >>\nstream\n{$compressedCMap}\nendstream\nendobj\n"
        . "%%EOF";

    return [$pdf, $safeText, $leakingText, $decoyText, $cMapName];
};

return [
    'rejects overlong filtered ToUnicode bfrange target arrays before current-base text extraction' => static function (
        TestRunner $t
    ) use ($parserMalformedCMapBfrangeArrayTargetFilterBoundaryPdf): void {
        [$pdf, $safeText, $leakingText, $decoyText, $cMapName] = $parserMalformedCMapBfrangeArrayTargetFilterBoundaryPdf();
        ...
        $t->same([0.0, 0.0, 144.0, 12.0], $spans[0]['bbox'] ?? null);
        $t->same([0.0, 0.0, 144.0, 12.0], $line['bbox'] ?? null);
        $t->true(!str_contains($plainText, $leakingText));
        $t->true(!str_contains($plainText, $decoyText));
