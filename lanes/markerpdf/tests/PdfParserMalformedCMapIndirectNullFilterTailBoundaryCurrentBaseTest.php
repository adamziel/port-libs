...
$parserMalformedCMapIndirectNullFilterTailUtf16beHex = static function (string $ascii): string {
    $hex = '';
...
$parserMalformedCMapIndirectNullFilterTailPdf = static function () use (
    $parserMalformedCMapIndirectNullFilterTailUtf16beHex
): array {
    $safeText = 'Indirect Null Safe Import';
    $leakingText = 'Indirect Null CMap Leak';
    $cMapName = 'IndirectNullFilterTailBoundary-H';
    $extraFilterName = 'FlateDecode';
    $safeHex = $parserMalformedCMapIndirectNullFilterTailUtf16beHex($safeText);
    ...
    $cMap = "/CIDInit /ProcSet findresource begin\n"
        ...
        . "endcodespacerange\n"
        . "1 beginbfchar\n"
        . "<{$sourceCode}> <" . $parserMalformedCMapIndirectNullFilterTailUtf16beHex($leakingText) . ">\n"
        . "endbfchar\n"
        . "endcmap\n"
    ...
    $addObject(4, 0, '<< /Type /Font /Subtype /Type0 /BaseFont /IndirectNullFilterTailBoundary /Encoding /Identity-H /ToUnicode 6 0 R >>');
    $addObject(5, 0, "<< /Length " . strlen($content) . " >>\nstream\n{$content}\nendstream");
    $addObject(6, 0, "<< /Type /CMap /CMapName /{$cMapName} /Filter 7 0 R /Length " . strlen($cMap) . " >>\nstream\n{$cMap}\nendstream");
    $addObject(7, 0, "null /{$extraFilterName}");
...
return [
    'fails closed and types selected indirect null CMap Filter helpers with trailing decoder operands' => static function (
        TestRunner $t
    ) use ($parserMalformedCMapIndirectNullFilterTailPdf): void {
        [$pdf, $safeText, $leakingText, $cMapName, $extraFilterName] = $parserMalformedCMapIndirectNullFilterTailPdf();
        ...
        $t->same('null', $filterOperand['token_type'] ?? null);
        $t->same(null, $filterOperand['value'] ?? null);
        $t->same(false, $filterOperand['valid_filter_operand'] ?? null);
        $t->same(false, $filterOperand['dictionary_filter_operand'] ?? null);
        $t->same('null /' . $extraFilterName, $filterOperand['value_preview'] ?? null);
