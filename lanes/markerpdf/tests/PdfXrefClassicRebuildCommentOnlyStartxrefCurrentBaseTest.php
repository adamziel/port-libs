...
$xrefClassicRebuildCommentOnlyStartxrefCurrentBasePdf = static function (): array {
    $staleXmp = '<x:xmpmeta xmlns:x="adobe:ns:meta/">'
        . '<rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#">'
        . '<rdf:Description rdf:about="" xmlns:dc="http://purl.org/dc/elements/1.1/">'
        . '<dc:title><rdf:Alt><rdf:li xml:lang="x-default">Stale Comment-Only Startxref Title</rdf:li></rdf:Alt></dc:title>'
        . '</rdf:Description></rdf:RDF></x:xmpmeta>';
    $currentXmp = '<x:xmpmeta xmlns:x="adobe:ns:meta/">'
        . '<rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#">'
        . '<rdf:Description rdf:about="" xmlns:dc="http://purl.org/dc/elements/1.1/">'
        . '<dc:title><rdf:Alt><rdf:li xml:lang="x-default">Current Comment-Only Startxref Title</rdf:li></rdf:Alt></dc:title>'
        . '</rdf:Description></rdf:RDF></x:xmpmeta>';
...
return [
    'rebuilds stale classic startxref across later commented startxref markers before WordPress imports' => static function (
        TestRunner $t
    ) use ($xrefClassicRebuildCommentOnlyStartxrefCurrentBasePdf): void {
        [$pdf, $currentPayload, $currentChecksum, $previousXrefOffset, $currentXrefOffset] = $xrefClassicRebuildCommentOnlyStartxrefCurrentBasePdf();
        ...
        $t->same(['Current comment-only startxref page', 'Comment marker ignored for rebuild'], $extractor->extractTextLines($pdf));
        $t->same(['Current comment-only startxref page', 'Comment marker ignored for rebuild'], $extractor->extractTextRuns($pdf));
        $t->same("Current comment-only startxref page\nComment marker ignored for rebuild", $text);
        $t->same("Current comment-only startxref page\nComment marker ignored for rebuild\n", $extractor->naiveGetText($pdf));
        $t->same(1, $extractor->extractOutlineMetadata($pdf)['pages']);
        $t->same('Current Comment-Only Startxref Title', $metadata['title']);
        $t->same('Current Comment-Only Startxref Info', $metadata['info']['Title']);
        $t->same('Current Comment Importer', $metadata['info']['Author']);
        $t->same(1, count($files));
        $t->same('current-comment-only-startxref.xml', $files[0]['name']);
        $t->same('current-comment-only-startxref.xml', $files[0]['filename']);
        $t->same('Current comment-only startxref attachment', $files[0]['description']);
        ...
        $t->same(['current-comment-only-startxref.xml'], $attachmentSummary['filenames']);
        ...
        $t->true(!str_contains($text, 'Stale comment-only startxref page'));
        $t->true(!str_contains($text, 'Comment marker blocked rebuild'));
        $t->true(!str_contains($encodedMetadata, 'Stale Comment-Only Startxref'));
        $t->true(!str_contains($encodedFiles, 'stale-comment-only-startxref'));
        $t->true(!str_contains($encodedAttachmentSummary, 'stale-comment-only-startxref'));
