...
return [
    'blocks encrypted text extraction and quarantines invalid signature byte ranges' => static function (TestRunner $t) use ($encryptedSignedPdf): void {
        ...
        $t->same(['encrypted_document', 'encrypted_text_extraction_blocked', 'copy_or_extract_denied', 'encrypted_signature_byte_range_present', 'signature_byte_range_invalid'], $report['review_reasons']);
        ...
        $encryptedByteRangeReview = $report['encrypted_signature_byte_range_review'];
        $t->same('encrypted_signature_byte_range_review', $encryptedByteRangeReview['source']);
        $t->same(true, $encryptedByteRangeReview['present']);
        $t->same(1, $encryptedByteRangeReview['byte_range_count']);
        $t->same(0, $encryptedByteRangeReview['valid_byte_range_count']);
        $t->same(1, $encryptedByteRangeReview['invalid_byte_range_count']);
        $t->same(['invalid_out_of_bounds'], $encryptedByteRangeReview['byte_range_statuses']);
        $t->same(['approval.signature'], $encryptedByteRangeReview['field_names']);
        $t->same('blocked_by_encryption_and_copy_permission', $encryptedByteRangeReview['content_extraction_boundary']);
        $t->same(true, $encryptedByteRangeReview['byte_range_does_not_grant_import']);
        $t->same('invalid_out_of_bounds', $encryptedByteRangeReview['rows'][0]['byte_range_status']);
        $t->same(true, $encryptedByteRangeReview['rows'][0]['byte_range_does_not_grant_import']);
