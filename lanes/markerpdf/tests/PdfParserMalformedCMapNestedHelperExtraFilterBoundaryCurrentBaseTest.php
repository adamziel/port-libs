...
$parserMalformedCMapNestedHelperExtraFilterUtf16beHex = static function (string $ascii): string {
    $hex = '';
...
$parserMalformedCMapNestedHelperExtraFilterPdf = static function () use (
    $parserMalformedCMapNestedHelperExtraFilterUtf16beHex
): array {
    $safeText = 'Nested Helper Safe Import';
    $leakingText = 'Nested Helper CMap Leak';
    $safeHex = $parserMalformedCMapNestedHelperExtraFilterUtf16beHex($safeText);
    $sourceCode = substr($safeHex, 0, 4);
    $cMapName = 'NestedHelperExtraFilterBoundary-H';
    ...
    $cMap = "/CIDInit /ProcSet findresource begin\n"
        ...
        . "endcodespacerange\n"
        . "1 beginbfchar\n"
        . "<{$sourceCode}> <" . $parserMalformedCMapNestedHelperExtraFilterUtf16beHex($leakingText) . ">\n"
        . "endbfchar\n"
        . "endcmap\n"
    ...
    if (!is_string($compressedCMap)) {
        throw new RuntimeException('Unable to compress nested-helper CMap fixture.');
    }
    ...
    $addObject(4, 0, '<< /Type /Font /Subtype /Type0 /BaseFont /NestedHelperExtraFilterBoundary /Encoding /Identity-H /ToUnicode 6 0 R >>');
    $addObject(5, 0, "<< /Length " . strlen($content) . " >>\nstream\n{$content}\nendstream");
    $addObject(6, 0, "<< /Type /CMap /CMapName /{$cMapName} /Filter 7 0 R /Length " . strlen($compressedCMap) . " >>\nstream\n{$compressedCMap}\nendstream");
    $addObject(7, 0, '8 0 R');
    $addObject(8, 0, '/FlateDecode /' . $extraFilterName);
...
return [
    'reports nested selected CMap Filter helper trailing decoder names before text extraction' => static function (
        TestRunner $t
    ) use ($parserMalformedCMapNestedHelperExtraFilterPdf): void {
        [$pdf, $safeText, $leakingText, $cMapName, $extraFilterName] = $parserMalformedCMapNestedHelperExtraFilterPdf();
