...
$parserMalformedCMapIndirectScalarFilterBoundaryCurrentBasePdf = static function (): string {
    ...
    $cMapName = 'IndirectScalarFilterBoundary-H';
    ...
    if (!is_string($compressedCMap)) {
        throw new RuntimeException('Unable to compress focused indirect scalar CMap filter fixture.');
    }
    ...
    $addObject(4, 0, '<< /Type /Font /Subtype /Type0 /BaseFont /IndirectScalarFilterBoundary /Encoding /Identity-H /ToUnicode 6 0 R >>');
...
return [
    'classifies xref selected indirect scalar CMap Filter helper with an extra decoder-name token before current-base text extraction' => static function (TestRunner $t) use ($parserMalformedCMapIndirectScalarFilterBoundaryCurrentBasePdf): void {
        $extractor = new PdfTextExtractor();
        $pdf = $parserMalformedCMapIndirectScalarFilterBoundaryCurrentBasePdf();
        $text = $extractor->extractPlainText($pdf);
        $review = $extractor->extractCMapStreamFilterLengthOwnerReview($pdf);
        $entry = $review['entries'][0] ?? [];
        $filterOperands = $entry['filter_operands'] ?? [];

        $t->same(['Indirect Scalar Safe Import'], $extractor->extractTextLines($pdf));
        $t->same(['Indirect Scalar Safe Import'], $extractor->extractTextRuns($pdf));
        $t->same('Indirect Scalar Safe Import', $text);
        $t->same("Indirect Scalar Safe Import\n", $extractor->naiveGetText($pdf));
        $t->same(1, $extractor->extractOutlineMetadata($pdf)['pages']);
        $t->same(['1'], $extractor->extractPageLabels($pdf));
        $t->true(!str_contains($text, 'Indirect Scalar CMap Leak'));
        $t->true(!str_contains($text, 'IndirectScalarFilterBoundary-H'));
        ...
        $t->same('IndirectScalarFilterBoundary-H', $entry['cmap_name'] ?? null);
        ...
        $t->same('/FlateDecode /ASCIIHexDecode', $filterOperands[0]['value_preview'] ?? null);
