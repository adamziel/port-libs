...
$xrefObjectStreamOffsetOrderCurrentBasePdf = static function (): string {
    ...
    if (!is_string($objectStream)) {
        throw new RuntimeException('Unable to compress object-stream offset-order fixture.');
    }
    ...
    if (!is_string($compressedXref)) {
        throw new RuntimeException('Unable to compress object-stream offset-order xref fixture.');
    }
...
return [
    'uses object-stream member offsets instead of header order before WordPress text extraction' => static function (TestRunner $t) use ($xrefObjectStreamOffsetOrderCurrentBasePdf): void {
        $extractor = new PdfTextExtractor();
        $pdf = $xrefObjectStreamOffsetOrderCurrentBasePdf();
        $text = $extractor->extractPlainText($pdf);
        $review = $extractor->extractXrefObjectStreamIndexReview($pdf);
        $entries = array_column($review['entries'], null, 'object_number');

        $expected = [
            'First offset-order page',
            'Declared offsets own member bodies',
            'Second offset-order page',
        ];
        ...
        $t->same(0, $entries[4]['actual_member_index'] ?? null);
