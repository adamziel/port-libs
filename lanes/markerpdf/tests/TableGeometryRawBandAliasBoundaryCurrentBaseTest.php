...
function markerpdf_raw_band_alias_boundary_page(array $lines): array
{
    return [
        'page' => 0,
...
function markerpdf_raw_band_alias_boundary_bbox(float $x1, float $y1, float $x2, float $y2): array
{
    return [$x1, $x2, $y1, $y2];
}

function markerpdf_raw_band_alias_boundary_table(): array
{
    return [
        ...
        'row_bboxes' => [
            markerpdf_raw_band_alias_boundary_bbox(72.0, 150.0, 312.0, 182.0),
            markerpdf_raw_band_alias_boundary_bbox(72.0, 190.0, 312.0, 220.0),
            markerpdf_raw_band_alias_boundary_bbox(72.0, 250.0, 312.0, 268.0),
        ],
        'columns' => [
            markerpdf_raw_band_alias_boundary_bbox(72.0, 150.0, 172.0, 230.0),
            markerpdf_raw_band_alias_boundary_bbox(192.0, 150.0, 312.0, 230.0),
            markerpdf_raw_band_alias_boundary_bbox(342.0, 150.0, 362.0, 230.0),
        ],
        'cells' => [
...
return [
    'canonicalizes raw indexed row_bboxes columns aliases before table crop boundary assignment' => static function (
        TestRunner $t
    ): void {
        $recognizer = new TableRecognizer();
        $formatted = $recognizer->formatRecognizedTables(
            [markerpdf_raw_band_alias_boundary_table()],
            [['width' => 240, 'height' => 80]]
        );
        ...
        $t->same([0], $assignedByText['Header']['row_ids'] ?? null);
        $t->same([0, 1], $assignedByText['Header']['col_ids'] ?? null);
        ...
        $t->same(['h-r0-c0'], $renderByText['Ready']['headers'] ?? null);
    },
    'surfaces raw indexed row_bboxes columns aliases through supplied WordPress table conversion' => static function (
        TestRunner $t
    ): void {
        $path = sys_get_temp_dir() . '/markerpdf-table-raw-band-alias-boundary-' . bin2hex(random_bytes(4)) . '.pdf';
        file_put_contents($path, "%PDF-1.4\n% raw band alias table boundary current-base fixture\n%%EOF");
        try {
            $result = (new SuppliedDocumentConverter())->convert(
                $path,
                [
                    markerpdf_raw_band_alias_boundary_page([
                        ['text' => 'Raw band alias table boundary', 'bbox' => [72.0, 48.0, 480.0, 68.0], 'font' => 'Heading-Bold', 'weight' => 700, 'size' => 18],
                        ['text' => 'Stale raw-band table line should be replaced.', 'bbox' => [72.0, 176.0, 380.0, 196.0]],
                        ['text' => 'After raw band alias table.', 'bbox' => [72.0, 276.0, 480.0, 294.0]],
                    ]),
                ],
                [
                    ...
                    'recognized_tables' => [markerpdf_raw_band_alias_boundary_table()],
            ...
            $t->contains('# Raw Band Alias Table Boundary', $result['text']);
            $t->contains('| Header |       |', $result['text']);
            $t->contains('| Images | Ready |', $result['text']);
            $t->contains('After raw band alias table.', $result['text']);
            $t->true(!str_contains($result['text'], 'Stale raw-band table line should be replaced.'));
            $t->true(!str_contains($result['text'], 'Stale raw row'));
            $t->true(!str_contains($result['text'], 'Stale raw column'));
