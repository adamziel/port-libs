...
$pageLabelsKidLowerMismatchPdf = static function (): string {
    $contents = [
...
return [
    'keeps later PageLabels kid when earlier same-lower child starts after its lower Limit' => static function (
        TestRunner $t
    ) use ($pageLabelsKidLowerMismatchPdf): void {
        $pdf = $pageLabelsKidLowerMismatchPdf();
        ...
        $t->same([
            'Lower mismatch cover imported',
            'Lower mismatch body imported',
            'Lower mismatch appendix imported',
        ], array_column($entries, 'text'));
        foreach (['1', '2', 'Body 5', 'App-Y'] as $staleLabel) {
