...
$xrefClassicRebuildPartialTrailingSubsectionBoundaryCurrentBasePdf = static function (): array {
    $staleXmp = '<x:xmpmeta xmlns:x="adobe:ns:meta/">'
        . '<rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#">'
        . '<rdf:Description rdf:about="" xmlns:dc="http://purl.org/dc/elements/1.1/">'
        . '<dc:title><rdf:Alt><rdf:li xml:lang="x-default">Stale Partial-Trailing XRef Title</rdf:li></rdf:Alt></dc:title>'
        . '</rdf:Description></rdf:RDF></x:xmpmeta>';
    $currentXmp = '<x:xmpmeta xmlns:x="adobe:ns:meta/">'
        . '<rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#">'
        . '<rdf:Description rdf:about="" xmlns:dc="http://purl.org/dc/elements/1.1/">'
        . '<dc:title><rdf:Alt><rdf:li xml:lang="x-default">Current Partial-Trailing XRef Title</rdf:li></rdf:Alt></dc:title>'
        . '</rdf:Description></rdf:RDF></x:xmpmeta>';
    ...
    return [$pdf, $currentPayload, strtolower($currentChecksum), $previousXrefOffset, $currentXrefOffset, $offsets[40]];
};
...
$xrefClassicRebuildHeaderGarbageBoundaryCurrentBasePdf = static function (): array {
    $currentXmp = '<x:xmpmeta xmlns:x="adobe:ns:meta/">'
        . '<rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#">'
        . '<rdf:Description rdf:about="" xmlns:dc="http://purl.org/dc/elements/1.1/">'
        . '<dc:title><rdf:Alt><rdf:li xml:lang="x-default">Current Header-Garbage XRef Title</rdf:li></rdf:Alt></dc:title>'
        . '</rdf:Description></rdf:RDF></x:xmpmeta>';
    $decoyXmp = '<x:xmpmeta xmlns:x="adobe:ns:meta/">'
        . '<rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#">'
        . '<rdf:Description rdf:about="" xmlns:dc="http://purl.org/dc/elements/1.1/">'
        . '<dc:title><rdf:Alt><rdf:li xml:lang="x-default">Header Garbage Decoy Title</rdf:li></rdf:Alt></dc:title>'
        . '</rdf:Description></rdf:RDF></x:xmpmeta>';
    ...
    return [$pdf, $currentPayload, strtolower($currentChecksum), $currentXrefOffset, $garbageHeaderXrefOffset];
};
...
return [
    ...
    'preserves completed classic xref subsections before partial malformed trailing subsections' => static function (
        TestRunner $t
    ) use ($xrefClassicRebuildPartialTrailingSubsectionBoundaryCurrentBasePdf): void {
        [$pdf, $currentPayload, $currentChecksum, $previousXrefOffset, $currentXrefOffset, $decoyCatalogOffset] = $xrefClassicRebuildPartialTrailingSubsectionBoundaryCurrentBasePdf();
        $extractor = new PdfTextExtractor();
        $text = $extractor->extractPlainText($pdf);
        $metadata = (new PdfMetadataExtractor())->extractDocumentMetadata($pdf);
        $files = (new PdfEmbeddedFileExtractor())->extractEmbeddedFiles($pdf);
        ...
        $t->true($currentXrefOffset > $previousXrefOffset);
        $t->true($decoyCatalogOffset < $currentXrefOffset);
        $t->same(['Current partial-trailing xref page', 'Partial trailing subsection ignored'], $extractor->extractTextLines($pdf));
        $t->same(['Current partial-trailing xref page', 'Partial trailing subsection ignored'], $extractor->extractTextRuns($pdf));
        $t->same("Current partial-trailing xref page\nPartial trailing subsection ignored", $text);
        $t->same("Current partial-trailing xref page\nPartial trailing subsection ignored\n", $extractor->naiveGetText($pdf));
        $t->same(1, $extractor->extractOutlineMetadata($pdf)['pages']);
        $t->same('Current Partial-Trailing XRef Title', $metadata['title']);
        $t->same('Current Partial-Trailing XRef Info', $metadata['info']['Title']);
        $t->same('Current Partial Importer', $metadata['info']['Author']);
        $t->same(1, count($files));
        $t->same('current-partial-trailing-xref.xml', $files[0]['name']);
        $t->same('current-partial-trailing-xref.xml', $files[0]['filename']);
        $t->same('Current partial-trailing xref attachment', $files[0]['description']);
        $t->same('Source', $files[0]['relationship']);
        $t->same($currentPayload, $files[0]['content']);
        $t->same($currentChecksum, $files[0]['checksum']);
        $t->same(true, $files[0]['checksum_matches']);
        $t->same(1, $attachmentSummary['attachment_count']);
        $t->same(['current-partial-trailing-xref.xml'], $attachmentSummary['filenames']);
        $t->same(strlen($currentPayload), $attachmentSummary['total_bytes']);
        $t->same(false, $attachmentSummary['executes_python_or_models']);
        $t->same(false, $attachmentSummary['executes_external_pdf_tools']);
        $t->true(!str_contains($text, 'Stale partial-trailing xref page'));
        $t->true(!str_contains($text, 'Partial trailing subsection leak'));
        $t->true(!str_contains($encodedMetadata, 'Stale Partial-Trailing'));
        $t->true(!str_contains($encodedFiles, 'stale-partial-trailing-xref'));
        $t->true(!str_contains($encodedAttachmentSummary, 'stale-partial-trailing-xref'));
        $t->true(!str_contains($text, "\0"));
    },
    ...
    'rejects classic xref sections with malformed leading subsection headers before WordPress imports' => static function (
        TestRunner $t
    ) use ($xrefClassicRebuildHeaderGarbageBoundaryCurrentBasePdf): void {
        [$pdf, $currentPayload, $currentChecksum, $currentXrefOffset, $garbageHeaderXrefOffset] = $xrefClassicRebuildHeaderGarbageBoundaryCurrentBasePdf();
        $extractor = new PdfTextExtractor();
        $text = $extractor->extractPlainText($pdf);
        $metadata = (new PdfMetadataExtractor())->extractDocumentMetadata($pdf);
        $files = (new PdfEmbeddedFileExtractor())->extractEmbeddedFiles($pdf);
        ...
        $t->true($currentXrefOffset > 0);
        $t->true($garbageHeaderXrefOffset > $currentXrefOffset);
        $t->same(['Current header-garbage xref page', 'Malformed subsection header skipped'], $extractor->extractTextLines($pdf));
        $t->same(['Current header-garbage xref page', 'Malformed subsection header skipped'], $extractor->extractTextRuns($pdf));
        $t->same("Current header-garbage xref page\nMalformed subsection header skipped", $text);
        $t->same("Current header-garbage xref page\nMalformed subsection header skipped\n", $extractor->naiveGetText($pdf));
        $t->same(1, $extractor->extractOutlineMetadata($pdf)['pages']);
        $t->same('Current Header-Garbage XRef Title', $metadata['title']);
        $t->same('Current Header-Garbage XRef Info', $metadata['info']['Title']);
        $t->same('Current Header Importer', $metadata['info']['Author']);
        $t->same(1, count($files));
        $t->same('current-header-garbage-xref.xml', $files[0]['name']);
        $t->same('current-header-garbage-xref.xml', $files[0]['filename']);
        $t->same('Current header-garbage xref attachment', $files[0]['description']);
        $t->same('Source', $files[0]['relationship']);
        $t->same($currentPayload, $files[0]['content']);
        $t->same($currentChecksum, $files[0]['checksum']);
        $t->same(true, $files[0]['checksum_matches']);
        $t->same(1, $attachmentSummary['attachment_count']);
        $t->same(['current-header-garbage-xref.xml'], $attachmentSummary['filenames']);
        $t->same(strlen($currentPayload), $attachmentSummary['total_bytes']);
        $t->same(false, $attachmentSummary['executes_python_or_models']);
        $t->same(false, $attachmentSummary['executes_external_pdf_tools']);
        $t->true(!str_contains($text, 'Header garbage decoy page'));
        $t->true(!str_contains($text, 'Malformed header root leak'));
