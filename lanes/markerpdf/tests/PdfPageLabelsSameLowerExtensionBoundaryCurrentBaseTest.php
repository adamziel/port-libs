...
$pageLabelsSameLowerExtensionPdf = static function (): string {
    $contents = [
...
return [
    'rejects PageLabels same-lower kid range extension before stale relabeling' => static function (
        TestRunner $t
    ) use ($pageLabelsSameLowerExtensionPdf): void {
        $pdf = $pageLabelsSameLowerExtensionPdf();
        ...
        $t->same(['Front 1', 'Front 2', 'Front 3', 'End-'], $labels);
        $t->same($labels, array_column($entries, 'page_label'));
        $t->same($labels, $previewLabels);
        $t->same([
            'Same lower extension front imported',
            'Same lower extension body imported',
            'Same lower extension continuation imported',
            'Same lower extension end imported',
        ], array_column($entries, 'text'));
        foreach (['stale-same-extend-77', 'stale-same-extend-88'] as $staleLabel) {
            $t->true(!in_array($staleLabel, $labels, true));
            $t->true(!in_array($staleLabel, $previewLabels, true));
        }
        $t->same('Front 3', $summary['pages'][2]['page_label'] ?? null);
