...
if (($argv[1] ?? null) === '--self-test') {
    if ($result !== $expected) {
        fwrite(STDERR, "Unexpected custom at-rule function argument output:\n{$result}\n");
        exit(1);
    }

    if ($events !== $expectedEvents) {
        fwrite(STDERR, "Unexpected custom at-rule function argument events:\n" . json_encode($events) . "\n");
        exit(1);
    }

    $function = $seenPreludeAst['value'][0]['value'] ?? null;
    if (!is_array($function) || ($function['name'] ?? null) !== 'theme') {
        fwrite(STDERR, "Unexpected custom at-rule function AST:\n" . json_encode($seenPreludeAst) . "\n");
        exit(1);
    }
