...
@custom-media --wp\2d wide (min-width: 782px);
@custom-media --wp-theme-token not (min-Theme-Breakpoint: 2);

@import \75rl(./blocks/query-card.css) /* wp block layer, stale layer(old) supports((display:flex)) old alias: (--wp-tablet), ) */ l\61yer(theme.blocks) s\75pports((--wp-wide)) screen and (min-width: 480px /* old floor, ) */) and (--wp\2d wide), print and (--wp-motion);
