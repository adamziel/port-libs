...
CSS;
    ...
    'transition prefixer preserves CSS Regions declarations like upstream property definitions' => static function (TestRunner $t): void {
        $prefixer = new TransitionPrefixer();
        ...
        $preservedPrefixed = '.foo{-webkit-flow-into:article;-ms-flow-into:article;flow-into:article;-webkit-flow-from:article;-ms-flow-from:article;flow-from:article;-webkit-region-fragment:break;-ms-region-fragment:break;region-fragment:break}';
        ...
        $t->same($modern, $prefixer->prefixForTargets($css, ['chrome' => 18, 'ie' => 10]));
        $t->same($modern, $prefixer->prefixForTargets($css, ['chrome' => 14]));
        $t->same($modern, $prefixer->prefixForTargets($css, ['chrome' => 15]));
        $t->same($modern, $prefixer->prefixForTargets($css, ['chrome' => 18]));
        $t->same($modern, $prefixer->prefixForTargets($css, ['chrome' => 19]));
        $t->same($modern, $prefixer->prefixForTargets($css, ['edge' => 11]));
        $t->same($modern, $prefixer->prefixForTargets($css, ['edge' => 12]));
        $t->same($modern, $prefixer->prefixForTargets($css, ['edge' => 18]));
        $t->same($modern, $prefixer->prefixForTargets($css, ['edge' => 19]));
        $t->same($modern, $prefixer->prefixForTargets($css, ['ie' => 9]));
        $t->same($modern, $prefixer->prefixForTargets($css, ['ie' => 10]));
        $t->same($modern, $prefixer->prefixForTargets($css, ['safari' => 6]));
        $t->same($modern, $prefixer->prefixForTargets($css, ['safari' => '6.1']));
        $t->same($modern, $prefixer->prefixForTargets($css, ['safari' => 11]));
        ...
        $t->same($modern, $prefixer->prefixForTargets($css, ['ios_saf' => 11]));
        $t->same($modern, $prefixer->prefixForTargets($css, ['ios_saf' => 12]));
        $t->same($preservedPrefixed, $prefixer->prefixForTargets($stalePrefixed, ['chrome' => 19, 'edge' => 19, 'safari' => 12]));
        $t->same($preservedPrefixed, $prefixer->prefixForTargets($stalePrefixed, ['safari' => 11]));
        $t->same($preservedPrefixed, $prefixer->prefixForTargets($stalePrefixed, ['ie' => 10]));
        $t->same($preservedPrefixed, $prefixer->prefixForTargets($stalePrefixed, ['chrome' => 18, 'ie' => 10]));
    },
...
CSS;
    ...
    'transition prefixer maps upstream media range target fallbacks inside layers' => static function (TestRunner $t): void {
        ...
        $t->same(
            '@layer blocks{@media (min-width:1){.wp-block-query{color:#ff0}}}',
            $prefixer->prefixForTargets('@layer blocks { @media (width >= sign(10px)) { .wp-block-query { color: yellow; } } }', ['firefox' => 60])
        );
        $t->same(
            '@layer blocks{@media (width>=1){.wp-block-query{color:#ff0}}}',
            $prefixer->prefixForTargets('@layer blocks { @media (width >= sign(10px)) { .wp-block-query { color: yellow; } } }', ['firefox' => 64])
        );
        $t->same(
            '@layer blocks{@media (min-width:-1){.wp-block-query{color:#ff0}}}',
            $prefixer->prefixForTargets('@layer blocks { @media (width >= sign(-10px)) { .wp-block-query { color: yellow; } } }', ['firefox' => 60])
        ...
        $t->same(
            '@layer blocks{@media (min-width:2){.wp-block-query{color:#ff0}}}',
            $prefixer->prefixForTargets('@layer blocks { @media (width >= abs(-2)) { .wp-block-query { color: yellow; } } }', ['firefox' => 60])
        );
        $t->same(
        ...
        $t->same(
            '@layer blocks{@media (min-aspect-ratio:1){.wp-block-query{color:#ff0}}}',
            $prefixer->prefixForTargets('@layer blocks { @media (aspect-ratio >= sign(10 / 2)) { .wp-block-query { color: yellow; } } }', ['firefox' => 60])
        );
        $t->same(
    ...
    'transition prefixer maps upstream unknown media range fallbacks inside layers' => static function (TestRunner $t): void {
        ...
        $t->throws(
            InvalidArgumentException::class,
            static fn () => $prefixer->prefixForTargets('@layer blocks { @media (width >= sign(10%)) { .wp-block-query { color: yellow; } } }', ['firefox' => 60])
        );
        $t->throws(
            InvalidArgumentException::class,
            static fn () => $prefixer->prefixForTargets('@layer blocks { @media (width >= sign(10dppx)) { .wp-block-query { color: yellow; } } }', ['firefox' => 60])
        );
        $t->throws(
            InvalidArgumentException::class,
    ...
    'transition prefixer maps upstream import media range tails after layer modifiers' => static function (TestRunner $t): void {
        ...
        $t->same(
            '@import "blocks/query.css" layer(theme.blocks) (min-width:240px);@layer theme.blocks{.wp-block-query{color:#ff0}}',
            $prefixer->prefixForTargets('@import "blocks/query.css" layer(theme.blocks) (width >= 240px); @layer theme.blocks { .wp-block-query { color: yellow; } }', ['firefox' => 60])
        );
        $t->same(
            ...
            $prefixer->prefixForTargets('@import "blocks/query.css" layer(theme.blocks) (100px <= width <= 200px); @layer theme.blocks { .wp-block-query { color: yellow; } }', ['firefox' => 85])
        );
        $t->same(
            '@import "blocks/query.css" layer(theme.blocks) supports(display:grid) (min-width:240px);@layer theme.blocks{.wp-block-query{color:#ff0}}',
        ...
        $t->same(
            '@import "blocks/query.css" layer(theme.blocks) (width>=240px);@layer theme.blocks{.wp-block-query{color:#ff0}}',
            $prefixer->prefixForTargets('@import "blocks/query.css" layer(theme.blocks) (width >= 240px); @layer theme.blocks { .wp-block-query { color: yellow; } }', [
                'firefox' => 60,
                'exclude' => ['MediaRangeSyntax'],
            ])
        );
        $t->same(
            '@import "blocks/query.css" layer(theme.blocks) (100px<=width<=200px);@layer theme.blocks{.wp-block-query{color:#ff0}}',
            $prefixer->prefixForTargets('@import "blocks/query.css" layer(theme.blocks) (100px <= width <= 200px); @layer theme.blocks { .wp-block-query { color: yellow; } }', [
                'firefox' => 85,
                'exclude' => ['MediaIntervalSyntax'],
            ])
        );
