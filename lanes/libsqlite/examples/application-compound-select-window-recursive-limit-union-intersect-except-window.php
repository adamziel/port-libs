...
$payload = [
    'scenario' => 'application-compound-select-window-recursive-limit-current-source-union-intersect-except-window-limit',
    'applicationUse' => 'Copied wp_options preview queries can fence avg() window output and first_value() frame output through UNION distinct, INTERSECT, and EXCEPT after recursive queue LIMIT/OFFSET.',
    'status' => $plan['status'],
...
if (($payload['status'] ?? null) !== 'compound-select-window-recursive-limit-current-source-union-intersect-except-window-limit-ready') {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-union-intersect-except-window-limit self-test failed\n");
    exit(1);
}
if ($payload['operators'] !== ['UNION', 'INTERSECT', 'EXCEPT']) {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-union-intersect-except-window-limit operator guard failed\n");
    exit(1);
}
if ($payload['functions'] !== ['avg', 'first_value']) {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-union-intersect-except-window-limit window guard failed\n");
    exit(1);
}
if ($payload['currentTokenLength'] !== 64 || $payload['nextTokenLength'] !== 64) {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-union-intersect-except-window-limit token guard failed\n");
    exit(1);
}
