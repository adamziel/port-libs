...
$summary = [
    'scenario' => 'application-vfs-uri-shm-filecontrol-regression-current-source-next141',
    'applicationUse' => 'When a copied Application SQLite connection closes its SHM mapping, release that connection read lock before checkpoint/import code reopens the sidecar and probes URI/file-control data-version state.',
...
if (($argv[1] ?? '') === '--self-test') {
    if (
        ...
        fwrite(STDERR, "application-vfs-uri-shm-filecontrol-regression-current-source-next141 self-test failed\n");
        exit(1);
    }

    echo "application-vfs-uri-shm-filecontrol-regression-current-source-next141 self-test passed\n";
    exit(0);
}
