...
echo json_encode([
    'applicationUse' => 'Preview a copied multisite wp_options migration that releases an inner savepoint, then rolls back the outer savepoint so inner UPDATE/DELETE RETURNING rows are discarded and retry reads the restored current source.',
    'status' => $plan['status'],
