...
if (($argv[1] ?? '') === '--self-test') {
    ...
    assert($plan['events'][0]['ack'] === 'shared-cache-next625');
    assert($plan['events'][1]['status'] === 'claimed-reusable-current-source');
    assert($plan['events'][1]['claim'] === 'reader-reuse-next641');
    assert($plan['events'][1]['blocked_reasons'] === []);
    assert($plan['events'][2]['status'] === 'published-reused-current-source');
    assert($plan['events'][2]['reuse_ack'] === 'shared-cache-next625');
    assert($plan['events'][2]['token'] === 'shared-cache-next641');
    assert($plan['events'][2]['published_count'] === 31);
    assert(in_array('vfs-current-source-extended-published-reuse-snapshot-fence', $plan['dependencies'], true));
    ...
    echo "application-vfs-extended-published-reuse-snapshot-fence self-test passed\n";
    return;
}
