...
if ($rows !== $expected) {
    fwrite(STDERR, "wordpress-json-aggregate-filter-window-frame-current-source-next117 self-test failed\n");
    var_export($rows);
    fwrite(STDERR, "\n");
    exit(1);
}

echo "wordpress-json-aggregate-filter-window-frame-current-source-next117 self-test passed\n";
