...
require_once __DIR__ . '/../src/SQLiteWalReaderCheckpointRestartCurrentSourceNext133Plan.php';
...
$plan = SQLiteWalReaderCheckpointRestartCurrentSourceNext133Plan::plan(
    $databasePath,
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    if ($plan['current_reader_preserved_by_source_handle'] !== true || $plan['path_reopen_would_change_current_reader'] !== true) {
        throw new RuntimeException('current WordPress reader was not protected from reopened WAL path bytes');
    }
    ...
    echo "wordpress-wal-reader-checkpoint-restart-current-source-next133 self-test passed\n";
    return;
}
