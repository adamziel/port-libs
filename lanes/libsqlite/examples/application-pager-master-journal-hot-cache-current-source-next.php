...
$summary = [
    'scenario' => 'application-pager-master-journal-hot-cache-current-source-next136',
    'status' => $plan['status'],
    ...
    'applicationUse' => 'Copied Application option repair reads and the next plugin-setting write reuse only cache pages rebased to the current master-journal hot-recovery source; dirty crash cache pages are invalidated before before-images are captured.',
...
if ($summary['status'] !== 'pager-master-journal-hot-cache-current-source-next136'
    ...
    fwrite(STDERR, "application-pager-master-journal-hot-cache-current-source-next136 self-test failed\n");
    exit(1);
}

echo "application-pager-master-journal-hot-cache-current-source-next136 self-test passed\n";
