...
$summary = [
    'scenario' => 'application-pager-master-journal-savepoint-cache-current-source-retry',
    'status' => $plan['status'],
    ...
    'applicationUse' => 'A copied Application SQLite options import recovers through the current master journal, rebases stale pager-cache pages, rolls back a failed savepoint write, and captures retry before-images from the recovered current source instead of crashed cache bytes.',
...
if ($summary['status'] !== 'pager-master-journal-savepoint-cache-current-source-retry'
    ...
    fwrite(STDERR, "application-pager-master-journal-savepoint-cache-current-source-retry self-test failed\n");
    exit(1);
}

echo "application-pager-master-journal-savepoint-cache-current-source-retry self-test passed\n";
