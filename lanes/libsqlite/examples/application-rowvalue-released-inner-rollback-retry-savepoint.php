...
$plan = SQLiteRowValueUpdateDeleteReturningSavepointPlan::executeReleasedInnerRollbackRetry(
    ['wp_options' => $rows],
    [
...
echo json_encode([
    'scenario' => 'application-rowvalue-update-delete-returning-savepoint-current-source-released_inner_retry',
    'applicationUse' => 'Models a copied wp_options import where a released inner savepoint produced UPDATE/DELETE RETURNING rows, then an outer ROLLBACK TO discards those rows and retries from the outer savepoint image.',
    'status' => $plan['status'],
