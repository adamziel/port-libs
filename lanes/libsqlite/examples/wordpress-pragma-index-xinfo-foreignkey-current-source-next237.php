...
$page = SQLitePragmaIndexXinfoForeignKeyCurrentSourceNext237::page(
    $current,
    $next,
...
$summary = [
    'scenario' => 'wordpress-pragma-index-xinfo-foreignkey-current-source-next237',
    'wordpressUse' => 'Copied WordPress import schemas can reject a parent UNIQUE index whose PRAGMA index_xinfo key prefix matches the foreign key but whose extra key columns make it unusable for SQLite FK enforcement.',
...
if (PHP_SAPI === 'cli' && realpath($_SERVER['SCRIPT_FILENAME'] ?? '') === __FILE__) {
    if (
        ...
        fwrite(STDERR, "wordpress-pragma-index-xinfo-foreignkey-current-source-next237 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, json_encode($summary, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) . "\n");
    fwrite(STDOUT, "wordpress-pragma-index-xinfo-foreignkey-current-source-next237 self-test passed\n");
}
