...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "wordpress-select-json-recursive-window-current-next54 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'wordpress-select-json-recursive-window-current-next54',
    'wordpressUse' => 'Copied wp_options navigation JSON and JSONB blobs can be recursively expanded through parser-level json_tree/json_each joins and then traversed with current/next window functions for migration previews without requiring ext/sqlite.',
    'sql' => $sql,
    'rows' => $rows,
