...
$page = SQLitePragmaIndexXinfoForeignKeyCurrentSourceNext::currentNextPageFromCatalog198(
    $currentRecords,
    $tables,
    $nextRecords,
    $tables,
...
if (($argv[1] ?? '') === '--self-test') {
    if (
        ...
        fwrite(STDERR, "application-pragma-index-xinfo-foreignkey-current-source-next198 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "application-pragma-index-xinfo-foreignkey-current-source-next198 self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'application-pragma-index-xinfo-foreignkey-current-source-next198',
    'applicationUse' => 'Copied wp_options import diagnostics must accept a WITHOUT ROWID composite parent primary key even when no separate sqlite_schema index row exists for PRAGMA index_xinfo.',
