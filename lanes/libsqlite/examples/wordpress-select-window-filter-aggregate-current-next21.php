...
$tables = [
    'wp_options' => [
        ...
        ['option_id' => 4, 'option_name' => 'rewrite_rules', 'autoload' => 'yes', 'option_size' => 50],
    ],
];
...
$summary = SQLiteSelectSql::execute(
    "SELECT option_id,
            ...
            sum(option_size) FILTER (WHERE autoload = 'yes')
                OVER (ORDER BY option_id ROWS BETWEEN CURRENT ROW AND 1 FOLLOWING) AS next_autoload_bytes,
            group_concat(option_name) FILTER (WHERE autoload = 'yes')
                OVER (ORDER BY option_id ROWS BETWEEN CURRENT ROW AND 1 FOLLOWING) AS next_autoload_names
       FROM wp_options
      ORDER BY option_id",
    $tables
);
