...
$page = SQLitePragmaIndexXinfoForeignKeyCurrentSourceNext161::currentNextPageFromCatalog(
    $records,
    $currentTables,
    $records,
    $nextTables,
...
if (($argv[1] ?? null) === '--self-test') {
    if ($page['status'] !== 'ok' || $page['current']['foreign_key_violations'] !== 3 || $page['next_counts']['foreign_key_violations'] !== 0 || $page['delta']['cleared'] !== true) {
        fwrite(STDERR, "wordpress-pragma-index-xinfo-foreignkey-current-source-next161 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-index-xinfo-foreignkey-current-source-next161 self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'wordpress-pragma-index-xinfo-foreignkey-current-source-next161',
    'wordpressUse' => 'Admit wp_options foreign_key_check diagnostics when PRAGMA foreign_key_list reports implicit parent primary-key columns for REFERENCES parent declarations.',
    'status' => $page['status'],
