...
$summary = [
    'scenario' => 'wordpress-pragma-index-xinfo-foreignkey-current-source-next359-366',
    'wordpressUse' => 'WordPress import previews can flag CASCADE and RESTRICT foreign-key actions whose child lookup indexes are present but unusable because PRAGMA index_xinfo exposes key-order or collation mismatches.',
...
if (($argv[1] ?? null) === '--self-test') {
    foreach ($cases as $slice => [, , , $status]) {
        if (($summary["next{$slice}_{$status}"] ?? null) !== 1) {
            fwrite(STDERR, "wordpress-pragma-index-xinfo-foreignkey-current-source-next359-366 self-test failed\n");
            exit(1);
        }
    }
    ...
    fwrite(STDOUT, "wordpress-pragma-index-xinfo-foreignkey-current-source-next359-366 self-test passed\n");
    exit(0);
}
