...
$pageSize = 512;
$root = sys_get_temp_dir() . '/port-libsqlite-wordpress-pager-checkpoint-' . bin2hex(random_bytes(4));
...
echo json_encode([
    'scenario' => 'wordpress-pager-checkpoint-atomic-apply',
    'wordpressUse' => 'Apply copied wp_options WAL checkpoint transactions through bounded native PHP file handles only after SQLite lock escalation, rolling back partial file writes if a later sidecar operation fails, without requiring ext/sqlite.',
    'root' => $root,
