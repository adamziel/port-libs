...
$summary = [
    'scenario' => 'application-pragma-index-xinfo-foreignkey-current-source-next307-310',
    'applicationUse' => 'Application import previews can catch FK action combinations that PRAGMA foreign_key_list exposes but only become actionable when table_info and index_xinfo show child nullability, defaults, and lookup coverage.',
...
if (($argv[1] ?? null) === '--self-test') {
    foreach (array_keys($summary) as $key) {
        if (str_starts_with($key, 'next') && $summary[$key] !== 1) {
            fwrite(STDERR, "application-pragma-index-xinfo-foreignkey-current-source-next307-310 self-test failed\n");
            exit(1);
        }
    }
    ...
    fwrite(STDOUT, "application-pragma-index-xinfo-foreignkey-current-source-next307-310 self-test passed\n");
    exit(0);
}
