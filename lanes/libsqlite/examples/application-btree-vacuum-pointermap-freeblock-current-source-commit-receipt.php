...
$plan = SQLiteBTreeVacuumPointerMapFreeblockCurrentSourcePlan::tableLeafCommitReceiptAuditFromDeleteResult(
    $database,
    3,
    [
        'page' => $deletedPage,
...
echo json_encode([
    'scenario' => 'application-btree-vacuum-pointermap-freeblock-current-source-commit-receipt',
    'applicationUse' => 'After deleting an overflow-backed copied wp_options transient and partially vacuuming tail pages, the next writer has commit receipts for only readable pointer-map, freeblock, and replacement overflow pages.',
    ...
    'receipts_match_apply_pages' => $summary['receipts_match_apply_pages'],
    'tail_pages_excluded' => $summary['all_tail_pages_excluded_from_receipts'],
    'receipt_count' => $summary['receipt_row_count'],
...
if (
    ...
    && $summary['all_tail_pages_excluded_from_receipts'] === true
    && $summary['receipts_match_apply_pages'] === true
) {
    echo "application-btree-vacuum-pointermap-freeblock-current-source-commit-receipt self-test passed\n";
}
