...
use PortLibs\LibSqlite\SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNext162Plan;
...
$plan = SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNext162Plan::tableLeafFromDeleteResult(
    $database,
    3,
    [
        'page' => $deletedPage,
...
$summary = [
    'scenario' => 'wordpress-btree-vacuum-pointermap-freeblock-current-source-next162',
    'wordpressUse' => 'Delete a copied wp_options transient, vacuum tail overflow pages, reuse surviving free pages for a replacement overflow chain, then admit only current-source pages that remain inside the next database image for writing.',
...
if (($argv[1] ?? null) === '--self-test') {
    ...
    foreach ($expected as $key => $value) {
        if ($summary[$key] !== $value) {
            fwrite(STDERR, "wordpress-btree-vacuum-pointermap-freeblock-current-source-next162 self-test failed\n");
            exit(1);
        }
    }

    fwrite(STDOUT, "wordpress-btree-vacuum-pointermap-freeblock-current-source-next162 self-test passed\n");
    exit(0);
}
