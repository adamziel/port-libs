...
require_once __DIR__ . '/../src/SQLiteWalSavepointReaderCheckpointCurrentSourceNext117Plan.php';
...
$plan = SQLiteWalSavepointReaderCheckpointCurrentSourceNext117Plan::plan(
    $stack,
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "wordpress-wal-savepoint-reader-checkpoint-current-source-next117 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'wordpress-wal-savepoint-reader-checkpoint-current-source-next117',
    'status' => $plan['status'],
    'staleReaderTailPages' => $plan['stale_reader_tail_pages'],
    'sourceTransitions' => $plan['source_transitions'],
    'wordpressUse' => 'Copied wp_options import retries can ignore a stale reader WAL source after ROLLBACK TO, checkpoint the retained current WAL prefix, and let the next reader use the checkpointed database without ext/sqlite.',
