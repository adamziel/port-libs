...
$plan = SQLiteRowValueUpdateDeleteReturningSavepointPlan::executeAbortRollbackRetrySavepoint(
    ['wp_options' => $rows],
    [
...
$summary = [
    'scenario' => 'application-rowvalue-abort-rollback-retry-savepoint',
    'applicationUse' => 'A copied wp_options import can catch an UPDATE OR ABORT duplicate row-value key statement, keep earlier savepoint UPDATE/DELETE RETURNING effects, then retry cleanup from that current source before release.',
    'status' => $plan['status'],
...
if (
    ...
    fwrite(STDERR, "application-rowvalue-abort-rollback-retry-savepoint self-test failed\n");
    exit(1);
}

echo "application-rowvalue-abort-rollback-retry-savepoint self-test passed\n";
