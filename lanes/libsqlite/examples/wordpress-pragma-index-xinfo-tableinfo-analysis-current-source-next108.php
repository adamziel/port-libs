...
$output = [
    'scenario' => 'wordpress-pragma-index-xinfo-tableinfo-analysis-current-source-next108',
    'wordpressUse' => 'Resume copied WordPress SQLite schema analysis for wp_options temp import tables and attached site metadata only while PRAGMA table_info/table_xinfo/index_xinfo current-source rows are unchanged.',
    'source_id' => $first['source_id'],
...
if (($argv[1] ?? '') === '--self-test') {
    if (
        ...
        fwrite(STDERR, "wordpress-pragma-index-xinfo-tableinfo-analysis-current-source-next108 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-index-xinfo-tableinfo-analysis-current-source-next108 self-test passed\n");
    exit(0);
}
