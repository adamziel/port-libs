...
$page = SQLitePragmaIndexXinfoForeignKeyCurrentSourceNext248::page(
    $current,
    $next,
...
$summary = [
    'scenario' => 'wordpress-pragma-index-xinfo-foreignkey-current-source-next248',
    'wordpressUse' => 'Copied WordPress taxonomy imports can distinguish FK parent keys backed by external UNIQUE indexes from inline UNIQUE constraints before dropping staging indexes.',
...
if (PHP_SAPI === 'cli' && realpath($_SERVER['SCRIPT_FILENAME'] ?? '') === __FILE__) {
    if (
        ...
        fwrite(STDERR, "wordpress-pragma-index-xinfo-foreignkey-current-source-next248 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, json_encode($summary, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) . "\n");
    fwrite(STDOUT, "wordpress-pragma-index-xinfo-foreignkey-current-source-next248 self-test passed\n");
}
