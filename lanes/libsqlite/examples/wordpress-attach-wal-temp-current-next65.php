...
if (($argv[1] ?? '') === '--self-test') {
    if (
        ...
        fwrite(STDERR, "wordpress-attach-wal-temp-current-next65 self-test failed\n");
        exit(1);
    }

    echo "wordpress-attach-wal-temp-current-next65 self-test passed\n";
    exit(0);
}

echo json_encode([
    'scenario' => 'wordpress-attach-wal-temp-current-next65',
    'status' => $plan['status'],
