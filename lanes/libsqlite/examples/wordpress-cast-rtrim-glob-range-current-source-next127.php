...
$plan = SQLiteCastRtrimGlobRangeCurrentSourceNext127Plan::wordpressOptionValuePlan(
    $currentRows,
    $nextRows,
    'TEXT',
    'plugin_cache',
...
$summary = [
    'scenario' => 'wordpress-cast-rtrim-glob-range-current-source-next127',
    'wordpressUse' => 'Copied wp_options option_value scans can use an RTRIM expression range over CAST(option_value AS TEXT) while retaining a binary GLOB residual so space-padded option payloads do not become exact GLOB matches during import diffing.',
    'range' => $plan['range'],
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "wordpress-cast-rtrim-glob-range-current-source-next127 self-test passed\n";
    return;
}
