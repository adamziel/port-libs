...
$payload = [
    'scenario' => 'application-compound-select-window-recursive-limit-current-source-next208',
    'applicationUse' => 'Copied wp_options preview queries can combine recursive dependency queues with rank and dense_rank windows, then fence current-source resume cursors across an EXCEPT membership boundary before next-source rows shift the final LIMIT.',
    'status' => $plan['status'],
...
if (($payload['status'] ?? null) !== 'compound-select-window-recursive-limit-current-source-next208-ready') {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-next208 self-test failed\n");
    exit(1);
}
if ($payload['windowFunctions'] !== ['rank', 'dense_rank']) {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-next208 window dispatch failed\n");
    exit(1);
}
if ($payload['currentTokenLength'] !== 64 || $payload['nextTokenLength'] !== 64) {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-next208 token guard failed\n");
    exit(1);
}
