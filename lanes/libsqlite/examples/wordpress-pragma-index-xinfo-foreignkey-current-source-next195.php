...
$page = SQLitePragmaIndexXinfoForeignKeyCurrentSourceNext195::currentNextPageFromCatalog(
    $currentRecords,
    $tables,
    $nextRecords,
    $tables,
...
if (($argv[1] ?? '') === '--self-test') {
    if (
        ...
        fwrite(STDERR, "wordpress-pragma-index-xinfo-foreignkey-current-source-next195 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-index-xinfo-foreignkey-current-source-next195 self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'wordpress-pragma-index-xinfo-foreignkey-current-source-next195',
    'wordpressUse' => 'Copied wp_options diagnostics must reject a parent UNIQUE index that contains the referenced columns in a different order; PRAGMA index_xinfo proves whether the next schema adds the exact ordered FK parent key.',
