...
$summary = SQLiteTriggerRecursiveViewReturningCurrentSourceNextPlan::executeNext229(
    [
...
if (
    $summary['status_next229'] !== 'trigger-recursive-view-returning-current-source-next229-generation-released'
    || $summary['current_returning_source_plan_next229']['decision'] !== 'publish-next-source-after-current-returning-generation-seal'
    || array_column($summary['visible_returning_payloads_next229'], 'name') !== ['blogdescription_child', 'template_child', 'home', 'next_plugin']
    || $summary['held_next_source_rows_next229'] !== []
) {
    fwrite(STDERR, "wordpress-trigger-recursive-view-returning-current-source-next229 self-test failed\n");
    exit(1);
}

echo "wordpress-trigger-recursive-view-returning-current-source-next229 self-test passed\n";
