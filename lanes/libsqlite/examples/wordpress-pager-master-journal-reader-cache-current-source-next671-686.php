...
$base = [
    'source_id' => 'wordpress-pager-reader-cache-next686',
    'epoch' => 686,
    'format_signature' => hash('sha256', implode('|', [512, 4, 2, 686, 0])),
    'publication_generation' => 686,
    'master_source_digest' => hash('sha256', 'wordpress next686 master source'),
...
$cache = [1 => array_merge($base, ['reader_id' => 'wp-options-reader', 'image' => $recovered[1], 'reader_cache_stmt_vdbe_opcode_trace_branch_handoff_token' => 'stmt-vdbe-opcode-trace-branch-handoff-old'])];
$read = array_merge($base, [
    'reader_id' => 'read-options',
    'page_number' => 1,
    'member_journal_token_digest' => $mapDigest($tokens),
    'member_journal_header_digest' => $mapDigest($headers),
    'reader_cache_stmt_vdbe_opcode_trace_branch_handoff_token' => 'stmt-vdbe-opcode-trace-branch-handoff-old',
]);
$plan = SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::variantNext686(
    $database,
    $master,
    $masterBytes,
    implode('', $before),
...
echo json_encode([
    ...
    'next686_invalidated_pages' => $plan['reader_cache_stmt_vdbe_opcode_trace_branch_handoff_invalidated_cache_page_numbers'],
