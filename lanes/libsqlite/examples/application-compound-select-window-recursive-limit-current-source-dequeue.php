...
$payload = [
    'scenario' => 'application-compound-select-window-recursive-limit-current-source-current-source-dequeue',
    'applicationUse' => 'Copied wp_options preview queries hold next-source compound rows until recursive queue dequeue acknowledgements are sealed for the current source.',
    'status' => $plan['status'],
...
if (($payload['status'] ?? null) !== 'compound-select-window-recursive-limit-current-source-dequeue-ready') {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-current-source-dequeue self-test failed\n");
    exit(1);
}
if ($payload['requiredAckCount'] !== 6) {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-current-source-dequeue dequeue guard failed\n");
    exit(1);
}
if ($payload['dequeueTokenLength'] !== 64) {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-current-source-dequeue token guard failed\n");
    exit(1);
}
if ($payload['nextExposure'] !== 'held-until-current-recursive-dequeue-acks') {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-current-source-dequeue exposure guard failed\n");
    exit(1);
}
