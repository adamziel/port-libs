...
$plan = SQLiteRowValueUpdateDeleteReturningSavepointPlan::executeRollbackToSavepointCurrentSource(
    $tables,
    [
...
$summary = [
    'scenario' => 'application-rowvalue-rollback-to-current-source-next218',
    'applicationUse' => 'Model a copied wp_options import where row-value UPDATE/DELETE RETURNING attempts are rolled back with ROLLBACK TO, suppressing attempted yielded rows and retrying from the savepoint image.',
    'status' => $plan['status'],
...
if (
    ...
    fwrite(STDERR, "application-rowvalue-rollback-to-current-source-next218 self-test failed\n");
    exit(1);
}
