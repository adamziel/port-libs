...
$summary = [
    'scenario' => 'application-rowvalue-in-savepoint-current-source-next181',
    'applicationUse' => 'Copied wp_options imports can use nullable row-value IN/NOT IN cleanup predicates, roll back speculative RETURNING rows after an OR FAIL conflict, and retry from the restored current source without ext/sqlite.',
    'status' => $plan['status'],
...
if (PHP_SAPI === 'cli' && basename(__FILE__) === basename($_SERVER['SCRIPT_FILENAME'] ?? '')) {
    if (
        ...
        fwrite(STDERR, "application-rowvalue-in-savepoint-current-source-next181 self-test failed\n");
        exit(1);
    }

    echo "application-rowvalue-in-savepoint-current-source-next181 self-test passed\n";
}
