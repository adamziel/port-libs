...
if ($databasePath === null) {
    fwrite(STDERR, "Usage: php lanes/libsqlite/examples/wordpress-rtrim-collation-option-lookup.php path/to/wordpress.sqlite [lookup_name] [--inclusive]\n");
    fwrite(STDERR, "Use --self-test to run a small in-memory wp_options fixture.\n");
    exit(1);
}

$database = $databasePath === '--self-test'
    ? SQLiteDatabase::fromBytes(exampleWordPressRtrimCollationFixture())
    : SQLiteDatabase::fromFile($databasePath);

$option = $database->wordpressOptionByIndexedName($lookupName);
$rangeOptions = $database->wordpressOptionsByIndexedNameRange($lookupName, $lookupName, null, $upperInclusive);
...
function exampleWordPressRtrimCollationFixture(): string
{
    $pageSize = 512;
