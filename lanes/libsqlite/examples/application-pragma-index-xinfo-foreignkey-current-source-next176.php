...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "application-pragma-index-xinfo-foreignkey-current-source-next176 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "application-pragma-index-xinfo-foreignkey-current-source-next176 self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'application-pragma-index-xinfo-foreignkey-current-source-next176',
    'applicationUse' => 'Carry named foreign-key constraints through copied wp_options PRAGMA index_xinfo/foreign_key_check diagnostics so migration tools can report the schema name that changed or blocked an import.',
    'status' => $page['status'],
