...
require_once __DIR__ . '/../src/SQLiteRowValueUpdateDeleteReturningSavepointCurrentSourceNext189Plan.php';

use PortLibs\LibSqlite\SQLiteRowValueUpdateDeleteReturningSavepointCurrentSourceNext189Plan;
...
$plan = SQLiteRowValueUpdateDeleteReturningSavepointCurrentSourceNext189Plan::execute(
    ['wp_options' => $rows],
    [
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "wordpress-rowvalue-not-between-savepoint-current-source-next189 self-test passed\n";
    return;
}

echo json_encode([
    'wordpressUse' => 'Model copied wp_options cleanup where row-value NOT BETWEEN updates return full rows, an inner DELETE NOT IN (VALUES ...) stream is rolled back, and the retry reads the current source preserved by the outer savepoint.',
    'status' => $plan['status'],
