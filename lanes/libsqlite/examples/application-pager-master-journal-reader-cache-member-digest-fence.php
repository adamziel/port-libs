...
$summary = [
    'scenario' => 'application-pager-master-journal-reader-cache-member-digest-fence',
    'status' => $plan['status'],
    ...
    'applicationUse' => 'A copied wp_options recovery can discard a reader-cache page whose image still matches but whose cache key was built from stale master-journal membership, then journal the next active_plugins write from the recovered current source.',
...
if ($summary['status'] !== 'pager-master-journal-reader-cache-current-source-next161'
    ...
    fwrite(STDERR, "application-pager-master-journal-reader-cache-member-digest-fence self-test failed\n");
    exit(1);
}

echo "application-pager-master-journal-reader-cache-member-digest-fence self-test passed\n";
