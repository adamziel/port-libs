...
$current = $prepared;
$current['name'] = 'current-wp-options-stat4-handoff-next606621';
...
$plan = SQLitePlannerStat4ExpressionPartialCurrentSourceNext224Plan::materializeNext606621(
    $prepared,
    $current,
    [
...
if (in_array('--self-test', $argv, true)) {
    assert($plan['status'] === 'stat4-expression-partial-current-source-next606-621-prepared');
    assert($plan['stat4Next606621PreparationFence']['preparedSlices'] === range(606, 621));
    assert($plan['stat4Next606621PreparationFence']['handoffWindows'][0]['continuesSlice'] === 590);
    echo "wordpress-sqlplanner-stat4-expression-partial-current-source-next606-621 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'wordpress-sqlplanner-stat4-expression-partial-current-source-next606-621',
    'wordpressUse' => 'Copied wp_options plugin-admin pagination carries the next590-605 current-source STAT4 handoff into next606-621 only when projected current rows still match.',
    'status' => $plan['status'],
    'selectedIndex' => $plan['selectedPlan']['name'] ?? null,
    'preparedSlices' => $plan['stat4Next606621PreparationFence']['preparedSlices'],
    'handoffSignature' => $plan['stat4Next606621PreparationFence']['handoffSignature'],
