...
require_once __DIR__ . '/../src/SQLitePagerCacheSpillWalRecoveryCurrentSourceNext135Plan.php';
...
$plan = SQLitePagerCacheSpillWalRecoveryCurrentSourceNext135Plan::plan(
    $databasePath,
    $databaseBytes,
    $walBytes,
    [
...
$summary = [
    'scenario' => 'wordpress-pager-cache-spill-wal-recovery-current-source-next135',
    'status' => $plan['status'],
    ...
    'wordpressUse' => 'Copied wp_options retry imports in WAL mode must recover only the committed WAL prefix before cache-spill frames are appended, while reader-pinned or corrupt tails prevent premature WAL reset.',
...
if ($summary['status'] !== 'pager_cache_spill_wal_recovery_current_source_next135'
    ...
    fwrite(STDERR, "wordpress-pager-cache-spill-wal-recovery-current-source-next135 self-test failed\n");
    exit(1);
}
