...
if (($argv[1] ?? '') === '--self-test') {
    if ($plan['status'] !== 'ready' || $plan['foreign_keys'][0]['schema'] !== 'temp' || $plan['foreign_keys'][2]['schema'] !== 'archive') {
        fwrite(STDERR, "wordpress-pragma-integrity-autoindex-fk-current-source-next92 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-integrity-autoindex-fk-current-source-next92 self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'copied wp_options PRAGMA integrity autoindex/FK current-source next92',
    'wordpressUse' => 'Preflight copied temp/main/archive WordPress SQLite schemas so autoindex catalog rows and foreign-key parent coverage remain schema-current before import writes.',
    'status' => $plan['status'],
