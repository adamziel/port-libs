...
$summary = [
    'scenario' => 'application-pragma-index-xinfo-foreignkey-current-source-next315-318',
    'applicationUse' => 'Application import previews can flag action-column-specific SET DEFAULT NULL and CASCADE child lookup blockers before replaying PRAGMA foreign_key_list output.',
...
if (($argv[1] ?? null) === '--self-test') {
    foreach (array_keys($summary) as $key) {
        if (str_starts_with($key, 'next') && $summary[$key] !== 1) {
            fwrite(STDERR, "application-pragma-index-xinfo-foreignkey-current-source-next315-318 self-test failed\n");
            exit(1);
        }
    }
    ...
    fwrite(STDOUT, "application-pragma-index-xinfo-foreignkey-current-source-next315-318 self-test passed\n");
    exit(0);
}
