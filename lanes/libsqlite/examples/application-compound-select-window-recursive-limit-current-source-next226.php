...
$payload = [
    'scenario' => 'application-compound-select-window-recursive-limit-current-source-next226',
    'applicationUse' => 'Copied wp_options preview queries can fence aggregate sum/count window output through EXCEPT and INTERSECT after a recursive queue LIMIT/OFFSET before final pagination.',
    'status' => $plan['status'],
...
if (($payload['status'] ?? null) !== 'compound-select-window-recursive-limit-current-source-next226-ready') {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-next226 self-test failed\n");
    exit(1);
}
if ($payload['operators'] !== ['UNION ALL', 'EXCEPT', 'INTERSECT']) {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-next226 operator guard failed\n");
    exit(1);
}
if ($payload['currentTokenLength'] !== 64 || $payload['nextTokenLength'] !== 64) {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-next226 token guard failed\n");
    exit(1);
}
