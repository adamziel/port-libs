...
$reopenPlan = [
    ...
    'source_token' => 'wordpress-import-checkpoint-source-253',
    'next_source_token' => 'wordpress-import-retry-source-253',
    ...
    'database_digest' => $digest('wordpress checkpoint source database next253'),
    'next_database_digest' => $digest('wordpress retry source database next253'),
    'wal_digest' => $digest('wordpress checkpoint source wal next253'),
    'next_wal_digest' => $digest('wordpress retry source wal next253'),
...
$plan = SQLiteWalHotJournalSavepointCheckpointCurrentSourceNextPlan::next253AdmitNextCurrentSource($reopenPlan, [
    ...
    $receipt('fence-wordpress-hot-journal', 'journal', 'fence_hot_journal', ['hot_journal_fenced' => true]),
    $receipt('release-wordpress-checkpoint-savepoint', 'savepoint', 'release_savepoint', ['savepoint_released' => true]),
]);

$summary = [
    'scenario' => 'wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next253',
    'status' => $plan['status'],
    ...
    'wordpressUse' => 'A copied wp_options import can admit retry readers to a new WAL current source only after checkpoint bytes, retry WAL frames, hot-journal fencing, and savepoint release all match the advancing source token.',
...
if (($argv[1] ?? '') === '--self-test') {
    if ($summary['status'] !== 'wal-hot-journal-savepoint-checkpoint-current-source-next253'
        ...
        fwrite(STDERR, "wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next253 self-test failed\n");
        exit(1);
    }

    echo "wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next253 self-test passed\n";
    return;
}
