...
$payload = [
    'scenario' => 'wordpress-compound-select-window-recursive-limit-current-source-next230',
...
if (($payload['status'] ?? null) !== 'compound-select-window-recursive-limit-current-source-next230-ready') {
    fwrite(STDERR, "wordpress-compound-select-window-recursive-limit-current-source-next230 self-test failed\n");
    exit(1);
}
if ($payload['operators'] !== ['UNION', 'INTERSECT', 'EXCEPT']) {
    fwrite(STDERR, "wordpress-compound-select-window-recursive-limit-current-source-next230 operator guard failed\n");
    exit(1);
}
if ($payload['functions'] !== ['avg', 'first_value']) {
    fwrite(STDERR, "wordpress-compound-select-window-recursive-limit-current-source-next230 window guard failed\n");
    exit(1);
}
if ($payload['currentTokenLength'] !== 64 || $payload['nextTokenLength'] !== 64) {
    fwrite(STDERR, "wordpress-compound-select-window-recursive-limit-current-source-next230 token guard failed\n");
    exit(1);
}
