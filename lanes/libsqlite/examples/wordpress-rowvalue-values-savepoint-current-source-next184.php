...
$plan = SQLiteRowValueUpdateDeleteReturningSavepointPlan::executeValuesRetrySavepointBatch(
    ['wp_options' => $rows],
    [
        ...
        "UPDATE wp_options SET (status, option_value) = ('outer184', option_value || ':outer184') WHERE (blog_id, option_name) IN (VALUES (3, 'rewrite_rules')) RETURNING option_id, option_name, status, option_value",
    ],
    [
        "DELETE FROM wp_options WHERE (blog_id, option_name) IN (VALUES (2, '_transient_feed')) RETURNING option_id, blog_id, option_name",
        "UPDATE OR ROLLBACK wp_options SET (blog_id, option_name, status) = (1, 'home', 'duplicate184') WHERE (blog_id, option_name) IN (VALUES (3, 'orphaned_cache')) RETURNING option_id, blog_id, option_name, status",
    ],
    [
        "UPDATE wp_options SET (status, option_value) = ('retry184', option_value || ':retry184') WHERE (blog_id, option_name) IN (VALUES (3, 'rewrite_rules'), (3, 'orphaned_cache')) RETURNING option_id, option_name, status, option_value ORDER BY option_id",
        "DELETE FROM wp_options WHERE (blog_id, option_name) IN (VALUES (1, '_transient_feed'), (1, '_transient_timeout_feed'), (2, '_transient_feed')) RETURNING option_id, blog_id, option_name ORDER BY option_id",
    ],
    [['blog_id', 'option_name']],
...
$summary = [
    'scenario' => 'wordpress-rowvalue-values-savepoint-current-source-next184',
    'wordpressUse' => 'Copied wp_options cleanup can use SQLite row-value IN (VALUES ...) predicates for transient cleanup and retry updates; OR ROLLBACK discards attempted RETURNING rows and retries from the restored transaction image.',
    'status' => $plan['status'],
    ...
    'dependencyClosure' => 'no new support component needed; reuses native row-value UPDATE/DELETE RETURNING execution and transaction/savepoint current-source modeling',
];
...
if (
    ...
    fwrite(STDERR, "wordpress-rowvalue-values-savepoint-current-source-next184 self-test failed\n");
    exit(1);
}

if (PHP_SAPI === 'cli' && realpath($_SERVER['SCRIPT_FILENAME'] ?? '') === __FILE__) {
    echo "wordpress-rowvalue-values-savepoint-current-source-next184 self-test passed\n";
