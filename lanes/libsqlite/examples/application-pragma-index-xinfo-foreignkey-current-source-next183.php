...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "application-pragma-index-xinfo-foreignkey-current-source-next183 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "application-pragma-index-xinfo-foreignkey-current-source-next183 self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'application-pragma-index-xinfo-foreignkey-current-source-next183',
    'applicationUse' => 'Report copied wp_options foreign-key child indexes beside PRAGMA index_xinfo and foreign_key_check rows so imports can distinguish correctness blockers from missing child-key performance indexes.',
    'status' => $page['status'],
