...
$pageCacheDigest = $hash('wordpress next250 clean copied option page cache');
...
$cleanupPlan = [
    ...
    'source_token' => 'wordpress-import-next250',
...
$summary = [
    'scenario' => 'wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next250',
    'wordpressUse' => 'A copied WordPress import clears stale pager cache, readmarks, schema-cookie, and WAL-index state after hot-journal checkpoint cleanup before readers use the new current source.',
    'status' => $plan['status'],
...
if ($summary['status'] !== 'wal-hot-journal-savepoint-checkpoint-current-source-next250'
    ...
    fwrite(STDERR, "wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next250 self-test failed\n");
    exit(1);
}
