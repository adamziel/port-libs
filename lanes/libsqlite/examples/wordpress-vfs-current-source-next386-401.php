...
require_once __DIR__ . '/../src/SQLiteVfsCurrentSourceNext386401Plan.php';
...
$plan = SQLiteVfsCurrentSourceNext386401Plan::run([
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    assert($plan['events'][0]['ack'] === 'shared-cache-next385');
    assert($plan['events'][1]['status'] === 'claimed-reusable-current-source');
    assert($plan['events'][1]['claim'] === 'reader-reuse-next401');
    assert($plan['events'][1]['blocked_reasons'] === []);
    assert($plan['events'][2]['status'] === 'published-reused-current-source');
    assert($plan['events'][2]['reuse_ack'] === 'shared-cache-next385');
    assert($plan['events'][2]['token'] === 'shared-cache-next401');
    assert($plan['events'][2]['published_count'] === 17);
    assert(in_array('vfs-current-source-snapshot-reuse-publish-next386-401', $plan['dependencies'], true));
    assert(in_array('vfs-current-source-snapshot-reuse-publish-next386-401', $plan['dependencies'], true));
    echo "wordpress-vfs-current-source-next386-401 self-test passed\n";
    return;
}
