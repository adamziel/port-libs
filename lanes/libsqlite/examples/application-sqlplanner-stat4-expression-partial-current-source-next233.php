...
$plan = SQLitePlannerStat4ExpressionPartialCurrentSourceNextPlan::materializeCurrentSourceSampleRowGuardFence(
    $prepared,
    $current,
    $where,
...
$summary = [
    'scenario' => 'application-sqlplanner-stat4-expression-partial-current-source-next233',
    'status' => $plan['status'],
    ...
    'applicationUse' => 'Prevents copied wp_options plugin scans from reusing stale sqlite_stat4 expression-index sample rowids after imports or ANALYZE refreshes move option rows.',
];
...
if ($summary['status'] !== 'stat4-expression-partial-current-source-next233-ready'
    ...
    fwrite(STDERR, "application-sqlplanner-stat4-expression-partial-current-source-next233 self-test failed\n");
    exit(1);
}
