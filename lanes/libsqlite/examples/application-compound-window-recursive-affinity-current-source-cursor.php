...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "application-compound-window-recursive-affinity-current-source-cursor self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'application-compound-window-recursive-affinity-current-source-cursor',
    ...
    'applicationUse' => 'Copied wp_options repair/import diagnostics can page through recursive dependency walks while preserving UNION numeric-affinity deduplication, per-arm window evaluation, left-most compound output names, and stale current/next source cursor rejection.',
    'firstPage' => [
