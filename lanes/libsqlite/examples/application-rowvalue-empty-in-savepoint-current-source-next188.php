...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "application-rowvalue-empty-in-savepoint-current-source-next188 self-test passed\n";
    return;
}

echo json_encode([
    'applicationUse' => 'Model copied wp_options cleanup where an empty row-value IN list selects no rows, NOT IN selects all rows, and ROLLBACK TO suppresses attempted RETURNING rows before retry.',
    'status' => $plan['status'],
