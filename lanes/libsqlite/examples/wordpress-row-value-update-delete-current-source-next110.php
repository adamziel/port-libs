...
$payload = [
    'wordpressUse' => 'Preview copied multisite wp_options cleanup/import SQL that relies on SQLite row-value predicates and row-value UPDATE assignments while preserving DELETE old-row RETURNING values and UPDATE new-row RETURNING values without ext/sqlite.',
    ...
    'dependencies' => [
        ...
        'wordpress-wp-options-copy-import-cleanup',
    ],
];
...
if (($argv[1] ?? null) === '--self-test') {
    ...
    echo "wordpress-row-value-update-delete-current-source-next110 self-test passed\n";
    return;
}
