...
if (($argv[1] ?? null) === '--self-test') {
    if ($page['status'] !== 'ok' || $page['current']['quick_check_errors'] !== 4 || $page['current']['foreign_key_violations'] !== 2 || $page['delta']['cleared'] !== true) {
        fwrite(STDERR, "wordpress-pragma-quickcheck-rootpage-foreignkey-current-source-next142 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-quickcheck-rootpage-foreignkey-current-source-next142 self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'wordpress-pragma-quickcheck-rootpage-foreignkey-current-source-next142',
