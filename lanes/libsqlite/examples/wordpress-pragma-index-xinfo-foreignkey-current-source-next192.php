...
$page = SQLitePragmaIndexXinfoForeignKeyCurrentSourceNext192::currentNextPageFromCatalog(
    $records,
    $tables,
    $nextRecords,
    $tables,
...
$summary = [
    'scenario' => 'wordpress-pragma-index-xinfo-foreignkey-current-source-next192',
    'wordpressUse' => 'Copied termmeta imports can block taxonomy FK repair when PRAGMA index_xinfo shows a UNIQUE parent key uses BINARY for a parent column declared COLLATE NOCASE.',
    'status' => $page['status'],
...
if (($argv[1] ?? '') === '--self-test') {
    if (
        ...
        fwrite(STDERR, "wordpress-pragma-index-xinfo-foreignkey-current-source-next192 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-index-xinfo-foreignkey-current-source-next192 self-test passed\n");
    exit(0);
}
