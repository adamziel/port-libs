...
$plan = SQLiteVfsLockByteUriShmCurrentSourceNext97::currentSourceNext139([
...
$payload = [
    'scenario' => 'wordpress-vfs-temp-uri-filecontrol-regression-current-source-next139',
    'wordpressUse' => 'Keep WordPress import scratch/temp SQLite URI file-controls handle-local while the copied live database persists WAL controls through the normal current-source owner.',
    'status' => $plan['status'],
...
if (PHP_SAPI === 'cli' && in_array('--self-test', $argv, true)) {
    ...
    echo "wordpress-vfs-temp-uri-filecontrol-regression-current-source-next139 self-test passed\n";
    return;
}
