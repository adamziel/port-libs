...
$prepared = [
    'name' => 'prepared-application-options-next195',
    'schemaCookie' => 1950,
    'stat4Generation' => 12,
    'rows' => [
...
$current = $prepared;
$current['name'] = 'current-application-options-next195';
...
if (($plan['matchedRowids'] ?? []) !== [40, 30, 20]) {
    throw new RuntimeException('Expected current-source Application plugin rows to survive the changed partial predicate fence');
}

echo json_encode([
    'scenario' => 'application-sqlplanner-stat4-expression-partial-current-source-next195',
    'status' => $plan['status'],
    'selectedSource' => $plan['selectedSource'],
    'partialPredicateChanged' => $plan['partialPredicateFence']['partialPredicateChanged'],
    'matchedRowids' => $plan['matchedRowids'],
    'applicationUse' => 'Copied wp_options plugin diagnostics can safely reuse a current-source STAT4 partial expression index after ANALYZE/schema changes only when the changed partial WHERE predicate is proven by query terms and selected rows.',
