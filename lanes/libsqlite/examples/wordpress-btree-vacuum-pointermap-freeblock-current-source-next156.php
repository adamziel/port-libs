...
require_once __DIR__ . '/../src/SQLiteBTreePointerMapVacuumFreeblockCurrentSourceNext144Plan.php';
require_once __DIR__ . '/../src/SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNext156Plan.php';

use PortLibs\LibSqlite\SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNext156Plan;
...
$plan = SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNext156Plan::tableLeafFromDeleteResult(
    $database,
    3,
    [
        'page' => $deletedPage,
    ...
    4,
    str_repeat('rewritten-wp-option-next156-', 10),
    3,
    true,
);

$summary = [
    'scenario' => 'wordpress-btree-vacuum-pointermap-freeblock-current-source-next156',
    'wordpressUse' => 'Delete an obsolete copied wp_options transient, vacuum tail overflow pages, then reuse only the surviving materialized free page for the replacement overflow chain while preserving auto-vacuum pointer-map ownership.',
    'allocated_overflow_pages' => $plan->allocatedOverflowPages(),
    'surviving_released_overflow_pages_reused' => $plan->survivingReleasedOverflowPagesReused(),
    'truncated_released_overflow_pages_rejected' => $plan->truncatedReleasedOverflowPagesRejected(),
...
if (($argv[1] ?? null) === '--self-test') {
    $expected = [
        'allocated_overflow_pages' => [106],
        'surviving_released_overflow_pages_reused' => [106],
        'truncated_released_overflow_pages_rejected' => [107, 108, 109, 110],
        'final_freelist_page_numbers' => [],
        'updated_page_numbers' => [1, 3, 105, 106],
    ];
    foreach ($expected as $key => $value) {
        if ($summary[$key] !== $value) {
            fwrite(STDERR, "wordpress-btree-vacuum-pointermap-freeblock-current-source-next156 self-test failed\n");
            exit(1);
        }
    }

    fwrite(STDOUT, "wordpress-btree-vacuum-pointermap-freeblock-current-source-next156 self-test passed\n");
    exit(0);
}
