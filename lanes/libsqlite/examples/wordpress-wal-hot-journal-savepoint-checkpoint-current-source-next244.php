...
$pageCacheDigest = $digest('wordpress next244 clean page cache');
$baseline = [
    ...
    'source_token' => 'wordpress-current-source-next244',
    'schema_cookie' => 2440,
...
$summary = [
    ...
    'status' => $plan['status'],
    'wordpressUse' => 'A copied WordPress import retires a recovered hot journal and stale WAL sidecar only after checkpoint bytes, fsync receipts, and reopened wp_options/usermeta readers all match the current source.',
...
if (($argv[1] ?? null) === '--self-test') {
    if ($summary['status'] !== 'wal-hot-journal-savepoint-checkpoint-current-source-next244'
        ...
        fwrite(STDERR, "wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next244 self-test failed\n");
        exit(1);
    }
    echo "wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next244 self-test passed\n";
    exit(0);
}
