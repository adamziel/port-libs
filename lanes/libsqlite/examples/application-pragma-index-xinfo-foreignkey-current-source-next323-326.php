...
$summary = [
    'scenario' => 'application-pragma-index-xinfo-foreignkey-current-source-next323-326',
    'applicationUse' => 'Application import previews can flag RESTRICT and NO ACTION child lookup indexes that are visible through PRAGMA index_xinfo but partial, so FK action probes cannot rely on them for every child row.',
...
if (($argv[1] ?? null) === '--self-test') {
    foreach (array_keys($summary) as $key) {
        if (str_starts_with($key, 'next') && $summary[$key] !== 1) {
            fwrite(STDERR, "application-pragma-index-xinfo-foreignkey-current-source-next323-326 self-test failed\n");
            exit(1);
        }
    }
    ...
    fwrite(STDOUT, "application-pragma-index-xinfo-foreignkey-current-source-next323-326 self-test passed\n");
    exit(0);
}
