...
$page = SQLitePragmaIndexXinfoForeignKeyCurrentSourceNext209::page(
    $current,
    $next,
...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "wordpress-pragma-index-xinfo-foreignkey-current-source-next209 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-index-xinfo-foreignkey-current-source-next209 self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'wordpress-pragma-index-xinfo-foreignkey-current-source-next209',
    'wordpressUse' => 'Before applying copied wp_options taxonomy imports, verify that PRAGMA foreign_key_list omitted-parent-column rows have the same arity as the parent table primary key exposed through PRAGMA table_info/index_xinfo metadata.',
    'status' => $page['status'],
