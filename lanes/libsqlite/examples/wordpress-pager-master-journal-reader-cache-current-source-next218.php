...
$plan = SQLitePagerMasterJournalReaderCacheCurrentSourceNext218Plan::plan(
    $database,
    $master,
    $masterBytes,
    implode('', $before),
...
$summary = [
    'scenario' => 'wordpress-pager-master-journal-reader-cache-current-source-next218',
    'wordpressUse' => 'A copied WordPress database keeps schema/options reader-cache pages only after master-journal cleanup has been durably observed; readers pinned before cleanup reopen before active_plugins is reused.',
    'status' => $plan['status'],
...
if ($summary['status'] !== 'pager-master-journal-reader-cache-current-source-next218'
    ...
    fwrite(STDERR, "wordpress-pager-master-journal-reader-cache-current-source-next218 self-test failed\n");
    exit(1);
}

echo json_encode($summary, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) . "\n";
echo "wordpress-pager-master-journal-reader-cache-current-source-next218 self-test passed\n";
