...
if (($argv[1] ?? '') === '--self-test') {
    if ($plan['status'] !== 'ready' || $plan['foreign_keys'][1]['required_autoindex'] !== 'sqlite_autoindex_wp_option_names_1') {
        fwrite(STDERR, "wordpress-pragma-autoindex-foreignkey-current-next53 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-autoindex-foreignkey-current-next53 self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'copied wp_options pragma autoindex foreign-key current next53',
    'wordpressUse' => 'Preflight copied WordPress SQLite schema metadata so PRAGMA foreign_key_check parent keys are backed by current sqlite_autoindex rows before the next import write.',
    'status' => $plan['status'],
