...
$summary = [
    'scenario' => 'wordpress-wal-checkpoint-reader-restart-snapshot-current-source-next124',
    'wordpressUse' => 'A WordPress import reader keeps an old WAL snapshot while checkpoint restart is possible only after release; the next writer appends to the restarted generation without changing the pinned current source.',
    'status' => $plan['status'],
...
if (in_array('--self-test', $argv, true)) {
    ...
    echo "wordpress-wal-checkpoint-reader-restart-snapshot-current-source-next124 self-test passed\n";
    return;
}
