...
$summary = [
    'scenario' => 'wordpress-rowvalue-savepoint-upsert-current-source-next131',
    'wordpressUse' => 'Preview copied wp_options imports where INSERT ON CONFLICT DO UPDATE uses row-value assignments inside an import savepoint, yielding inserted/updated RETURNING rows while preserving current-source rollback evidence for later unique failures.',
    'status' => $plan['status'],
...
if (PHP_SAPI === 'cli' && realpath($_SERVER['SCRIPT_FILENAME'] ?? '') === __FILE__) {
    ...
    echo "wordpress-rowvalue-savepoint-upsert-current-source-next131 self-test passed\n";
}
