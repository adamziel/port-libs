...
$base = [
    'source_id' => 'application-pager-reader-cache-next558',
    'epoch' => 558,
    'format_signature' => hash('sha256', implode('|', [512, 4, 2, 558, 0])),
    'publication_generation' => 558,
    'master_source_digest' => hash('sha256', 'application next558 master source'),
...
$cache = [1 => array_merge($base, ['reader_id' => 'wp-options-reader', 'image' => $recovered[1], 'reader_cache_stmt_vdbe_multiply_token' => 'stmt-vdbe-multiply-old'])];
$read = array_merge($base, [
    'reader_id' => 'read-options',
    'page_number' => 1,
    'member_journal_token_digest' => $mapDigest($tokens),
    'member_journal_header_digest' => $mapDigest($headers),
    'reader_cache_stmt_vdbe_multiply_token' => 'stmt-vdbe-multiply-old',
]);
$plan = SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::readerCacheStatementFence(558,
    $database,
    $master,
    $masterBytes,
    implode('', $before),
...
echo json_encode([
    ...
    'next558_invalidated_pages' => $plan['reader_cache_stmt_vdbe_multiply_invalidated_cache_page_numbers'],
