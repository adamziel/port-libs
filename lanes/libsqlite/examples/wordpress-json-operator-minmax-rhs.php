...
$matches = [
    'minCache' => $database->wordpressOptionsByIndexedJsonOptionValue('$.cache', 'hit'),
    'maxPluginEnabled' => $database->wordpressOptionsByIndexedJsonOptionValue('$."plugin.enabled"', 'yes'),
    'minSlot' => $database->wordpressOptionsByIndexedJsonOptionValue('$[1]', 'one'),
];

echo json_encode([
    'wordpressUse' => 'Preflight copied wp_options JSON operator indexes whose RHS constants use reduced SQLite min()/max() string or numeric literals without requiring the SQLite extension.',
    ...
    'matches' => array_map(
        static fn (array $options): array => array_map(static fn (SQLiteWordPressOption $option): string => $option->optionName, $options),
        $matches,
    ),
