...
$handoffPlan = [
    ...
    'database_digest' => $hash('application next251 checkpoint database image'),
    'page_cache_digest' => $hash('application next251 clean page cache'),
...
$summary = [
    'scenario' => 'application-wal-hot-journal-savepoint-checkpoint-current-source-next251',
    'applicationUse' => 'A copied Application database import can restart and truncate the WAL sidecar only after durable checkpoint handoff and all reader read-marks release.',
    'status' => $plan['status'],
...
if ($summary['status'] !== 'wal-hot-journal-savepoint-checkpoint-current-source-next251'
    ...
    fwrite(STDERR, "application-wal-hot-journal-savepoint-checkpoint-current-source-next251 self-test failed\n");
    exit(1);
}
