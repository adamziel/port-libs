...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "wordpress-pragma-quickcheck-foreignkey-rootpage-current-source-next self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-quickcheck-foreignkey-rootpage-current-source-next self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'wordpress-pragma-quickcheck-foreignkey-rootpage-current-source-next',
    'wordpressUse' => 'Gate copied wp_options imports on one current-source cursor that combines expression-index metadata, shallow quick_check rootpage blockers, and foreign_key_check rootpage/pointer-map rows.',
    'status' => $page['status'],
