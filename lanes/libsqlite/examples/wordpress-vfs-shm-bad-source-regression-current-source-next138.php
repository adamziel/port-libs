...
$summary = [
    'scenario' => 'wordpress-vfs-shm-bad-source-regression-current-source-next138',
    'status' => $plan['status'],
    ...
    'wordpressUse' => 'Reject malformed VFS/SHM current-source operation arrays before they can create a partial WAL-index state during WordPress SQLite copy/import preflight.',
    'dependencies' => $plan['dependencies'],
];

if (!$badSourceRejected || $summary['staleCurrentSource'] !== true || $summary['currentGeneration'] !== 2) {
    fwrite(STDERR, "wordpress-vfs-shm-bad-source-regression-current-source-next138 self-test failed\n");
    exit(1);
}

if (in_array('--self-test', $argv, true)) {
    echo "wordpress-vfs-shm-bad-source-regression-current-source-next138 self-test passed\n";
    return;
}
