...
$plan = SQLiteWalHotJournalSavepointCheckpointCurrentSourceNextPlan::hotJournalSavepointCheckpointAppendPlan(
    $databasePath,
    $dirtyDatabase,
    $journalBytes,
    $savepoints,
...
$summary = [
    'scenario' => 'application-wal-hot-journal-savepoint-checkpoint-current-source-next153',
    'status' => $plan['status'],
    'reason' => $plan['reason'],
    ...
    'applicationUse' => 'A copied wp_options import can recover a hot rollback journal, roll back the failed plugin-setting savepoint to the current WAL prefix, keep a still-open reader pinned, and only restart the checkpoint generation after that reader is released.',
...
if (PHP_SAPI === 'cli' && in_array('--self-test', $argv, true)) {
    if ($summary['status'] !== 'wal-hot-journal-savepoint-checkpoint-current-source-next153'
        ...
        fwrite(STDERR, "application-wal-hot-journal-savepoint-checkpoint-current-source-next153 self-test failed\n");
        exit(1);
    }

    echo "application-wal-hot-journal-savepoint-checkpoint-current-source-next153 self-test passed\n";
    return;
}
