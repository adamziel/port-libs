...
$readerPlan = [
    ...
    'schema_cookie' => 946,
    'database_digest' => $hash('wordpress next246 checkpoint database image'),
    'page_cache_digest' => $hash('wordpress next246 clean checkpoint page cache'),
    'checkpoint_frame' => 28,
    'dirty_pages' => [1, 2, 5, 9],
...
$summary = [
    'scenario' => 'wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next246',
    'wordpressUse' => 'A copied WordPress import promotes a hot-journal savepoint checkpoint as the current source only after database pages, WAL commit-frame markers, database/WAL/directory syncs, and hot-journal deletion receipts prove a durable VFS handoff.',
    'status' => $plan['status'],
...
if (($argv[1] ?? null) === '--self-test') {
    if ($summary['status'] !== 'wal-hot-journal-savepoint-checkpoint-current-source-next246'
        ...
        fwrite(STDERR, "wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next246 self-test failed\n");
        exit(1);
    }

    echo "wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next246 self-test passed\n";
    exit(0);
}
