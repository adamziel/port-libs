...
$summary = [
    ...
    'wordpressUse' => 'Delete oversized transient option rows and matching index records from a copied wp_options database image, derive overflow chains from current page bytes, and return table/index leaf plus overflow pages to the freelist with auto-vacuum pointer-map rewrites.',
    'table' => [
...
if (in_array('--self-test', $argv, true)) {
    if (
        ...
        fwrite(STDERR, "wordpress-btree-freelist-overflow-rebalance-current-source-next120 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-btree-freelist-overflow-rebalance-current-source-next120 self-test passed\n");
    exit(0);
}
