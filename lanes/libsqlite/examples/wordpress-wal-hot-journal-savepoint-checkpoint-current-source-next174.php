...
$summary = [
    'scenario' => 'wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next174',
    'wordpressUse' => 'A copied WordPress plugin import resumes after a crash between hot-journal recovery, WAL savepoint rollback, and checkpoint publication; file bytes are verified before deleting the hot journal or releasing readers.',
    'status' => $plan['status'],
...
if ($summary['status'] !== 'wal-hot-journal-savepoint-checkpoint-current-source-next174'
    ...
    fwrite(STDERR, "wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next174 self-test failed\n");
    exit(1);
}
