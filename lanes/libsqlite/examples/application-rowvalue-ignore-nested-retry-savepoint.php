...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "application-rowvalue-ignore-nested-retry-savepoint self-test passed\n";
    return;
}

echo json_encode([
    'status' => $plan['status'],
    'applicationUse' => 'Copied wp_options imports can use UPDATE OR IGNORE with row-value assignment inside an inner savepoint, yield no RETURNING row for ignored conflicts, roll back later speculative DELETE RETURNING rows, then retry from the preserved inner current source.',
