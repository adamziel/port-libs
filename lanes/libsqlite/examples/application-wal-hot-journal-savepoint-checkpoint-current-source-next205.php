...
$summary = [
    'scenario' => 'application-wal-hot-journal-savepoint-checkpoint-current-source-next205',
    'applicationUse' => 'After a copied wp_options import recovers a hot rollback journal, releases a savepoint, and publishes a WAL checkpoint, cached page readers are reused only when their current-source token and page image digest match the checkpoint source.',
    'status' => $plan['status'],
...
if (in_array('--self-test', $argv, true)) {
    if (
        ...
        fwrite(STDERR, "application WAL hot-journal checkpoint reader cache next205 smoke failed\n");
        exit(1);
    }
}
