...
$payload = [
    'scenario' => 'wordpress-rowvalue-abort-returning-current-source-next140',
    'wordpressUse' => 'Model a copied wp_options import savepoint where row-value UPDATE RETURNING yields staged rows, a later UPDATE OR ABORT unique conflict backs out only its own statement, and prior RETURNING/current-source rows remain visible for PHP recovery without ext/sqlite.',
    'status' => $plan['status'],
...
if (($argv[1] ?? null) === '--self-test') {
    ...
    echo "wordpress-rowvalue-abort-returning-current-source-next140 self-test passed\n";
    return;
}
