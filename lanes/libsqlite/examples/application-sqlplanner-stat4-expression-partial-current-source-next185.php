...
$prepared = [
    'name' => 'prepared-wp-options-next185',
    'schemaCookie' => 1850,
    'stat4Generation' => 121,
    'rows' => [
...
$current = $prepared;
$current['name'] = 'current-wp-options-next185';
$current['schemaCookie'] = 1858;
$current['stat4Generation'] = 144;
$current['indexes'][0]['rootPage'] = 18588;
...
$plan = SQLitePlannerStat4ExpressionPartialCurrentSourceNextPlan::materializeCurrentSourcePayloadWindowFence(
    $prepared,
    $current,
    [
...
if (in_array('--self-test', $argv, true)) {
    assert($plan['status'] === 'stat4-expression-partial-current-source-next185-ready');
    assert($plan['sampleDeltaFence']['changed'] === true);
    assert($plan['matchedRowids'] === [30, 50, 20, 21]);
    assert(array_column($plan['currentSourceRowProvenance'], 'source') === ['current', 'current', 'current', 'current']);
    echo "application-sqlplanner-stat4-expression-partial-current-source-next185 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'application-sqlplanner-stat4-expression-partial-current-source-next185',
    'applicationUse' => 'Copied wp_options plugin admin pagination can reject stale prepared STAT4 samples and verify the DESC partial lower(option_name) window against current rows after ANALYZE/source changes.',
    'status' => $plan['status'],
    'selectedSource' => $plan['selectedSource'],
    'selectedIndex' => $plan['selectedPlan']['name'] ?? null,
    'sampleDeltaFence' => $plan['sampleDeltaFence'],
    'provenance' => $plan['currentSourceRowProvenance'],
