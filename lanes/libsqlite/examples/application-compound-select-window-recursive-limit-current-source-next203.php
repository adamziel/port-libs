...
$payload = [
    'scenario' => 'application-compound-select-window-recursive-limit-current-source-lagLastValueExcept',
    'applicationUse' => 'Copied wp_options preview queries can run recursive dependency queues, evaluate lag defaults and last_value frames before an EXCEPT tail and final LIMIT, and reject stale current-source resume cursors before next-source rows are admitted.',
    'status' => $plan['status'],
...
if (($payload['status'] ?? null) !== 'compound-select-window-recursive-limit-current-source-lagLastValueExcept-ready') {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-lagLastValueExcept self-test failed\n");
    exit(1);
}
if ($payload['windowFunctions'] !== ['lag', 'last_value']) {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-lagLastValueExcept window dispatch failed\n");
    exit(1);
}
if ($payload['currentTokenLength'] !== 64 || $payload['nextTokenLength'] !== 64) {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-lagLastValueExcept token guard failed\n");
    exit(1);
}
