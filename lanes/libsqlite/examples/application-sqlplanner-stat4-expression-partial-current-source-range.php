...
if (in_array('--self-test', $argv, true)) {
    ...
    echo "application-sqlplanner-stat4-expression-partial-current-source-stat4-current-range self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'application-sqlplanner-stat4-expression-partial-current-source-stat4-current-range',
    'applicationUse' => 'Preview copied wp_options plugin scans after ANALYZE/source changes where narrowed lower(option_name) range bounds prove a partial expression index predicate and select the current STAT4 fence instead of stale prepared rows.',
    'status' => $plan['status'],
