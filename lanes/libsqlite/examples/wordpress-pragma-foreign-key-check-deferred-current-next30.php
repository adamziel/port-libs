...
if (($argv[1] ?? null) === '--self-test') {
    if ($summary['before_repair'][0]['rowid'] !== 11 || $summary['after_repair_violations'] !== 0 || $summary['committed'] !== true) {
        fwrite(STDERR, "wordpress-pragma-foreign-key-check-deferred-current-next30 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-foreign-key-check-deferred-current-next30 self-test passed\n");
    exit(0);
}
