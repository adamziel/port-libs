...
$current = $prepared;
$current['name'] = 'current-wp-options-stat4-partial-page-next231';
$current['schemaCookie'] = 2319;
$current['stat4Generation'] = 321;
$current['indexes'][0]['rootPage'] = 23188;
...
$current['indexes'][0]['stat4ExpressionPayloads'] = array_map($payload, array_slice($current['rows'], 0, 8));

$plan = SQLitePlannerStat4ExpressionPartialCurrentSourceNextPlan::materializeCurrentSourcePageMembershipFence(
    $prepared,
    $current,
    [
...
if (in_array('--self-test', $argv, true)) {
    assert($plan['status'] === 'stat4-expression-partial-current-source-next231-ready');
    assert($plan['currentSourcePageFence']['currentQualifiedRowids'] === [60, 30, 50, 20, 21, 22, 40, 10]);
    assert($plan['currentSourcePageFence']['expectedPageRowids'] === [30, 50, 20, 21, 22]);
    assert($plan['currentSourcePageFence']['selectedPageMatchesCurrentSource'] === true);
    echo "application-sqlplanner-stat4-expression-partial-current-source-next231 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'application-sqlplanner-stat4-expression-partial-current-source-next231',
    'applicationUse' => 'Copied wp_options plugin admin pagination can reject a stale prepared STAT4 partial-expression page when current rows inserted after ANALYZE alter the selected LIMIT/OFFSET window.',
    'status' => $plan['status'],
    'selectedSource' => $plan['selectedSource'],
    'selectedIndex' => $plan['selectedPlan']['name'] ?? null,
    'currentSourcePageFence' => $plan['currentSourcePageFence'],
