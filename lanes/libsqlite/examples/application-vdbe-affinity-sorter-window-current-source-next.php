...
$summary = [
    'scenario' => 'application-vdbe-affinity-sorter-window-current-source-next',
    'applicationUse' => 'Copied wp_options imports can compare current and next VDBE sorter/window sources with SQLite affinity, collation, FILTER, and non-advancing current/next frame summaries before replacing option rows.',
    'status' => $plan['status'],
...
if (($summary['status'] ?? null) !== 'vdbe-affinity-sorter-window-current-source-next-ready'
    ...
    fwrite(STDERR, "application-vdbe-affinity-sorter-window-current-source-next self-test failed\n");
    exit(1);
}
