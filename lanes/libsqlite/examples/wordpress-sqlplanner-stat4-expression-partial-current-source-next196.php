...
use PortLibs\LibSqlite\SQLitePlannerStat4ExpressionPartialCurrentSourceNext196Plan;
...
$prepared = [
    'name' => 'prepared-wp-options-next196',
    'schemaCookie' => 1960,
    'stat4Generation' => 161,
    'rows' => [
...
$current = $prepared;
$current['name'] = 'current-wp-options-next196';
$current['schemaCookie'] = 1969;
$current['stat4Generation'] = 188;
$current['indexes'][0]['rootPage'] = 19688;
...
$plan = SQLitePlannerStat4ExpressionPartialCurrentSourceNext196Plan::materialize(
    $prepared,
    $current,
    [
...
if (in_array('--self-test', $argv, true)) {
    assert($plan['status'] === 'stat4-expression-partial-current-source-next196-ready');
    assert($plan['peerOrderFence']['peerOrderStable'] === true);
    assert($plan['peerOrderFence']['duplicatePeerKeys'] === ['plugin_forms']);
    assert($plan['peerOrderFence']['peers'][2]['rowids'] === [20, 21, 22]);
    echo "wordpress-sqlplanner-stat4-expression-partial-current-source-next196 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'wordpress-sqlplanner-stat4-expression-partial-current-source-next196',
    'wordpressUse' => 'Copied wp_options plugin scans can reuse a current STAT4 partial lower(option_name) covering index only when duplicate mixed-case option names retain stable rowid peer order for LIMIT windows.',
    'status' => $plan['status'],
    'selectedSource' => $plan['selectedSource'],
    'selectedIndex' => $plan['selectedPlan']['name'] ?? null,
    'peerOrderFence' => $plan['peerOrderFence'],
