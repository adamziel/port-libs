...
$summary = [
    'scenario' => 'application-vfs-open-lock-filecontrol-current-source-next82',
    'status' => $plan['status'],
    ...
    'applicationUse' => 'Track copied wp_options database open/reopen current-source xFileControl controls and lock handoff before native SQLite writes run without ext/sqlite.',
    'dependencies' => $plan['dependencies'],
];

if (($summary['reopenedControls']['chunk_size'] ?? null) !== 8192 || ($summary['reopenedControls']['persist_wal'] ?? null) !== true) {
    fwrite(STDERR, "application-vfs-open-lock-filecontrol-current-source-next82 self-test failed\n");
    exit(1);
}
