...
$summary = [
    'scenario' => 'application-pragma-index-xinfo-foreignkey-current-source-next249',
    'applicationUse' => 'Copied Application taxonomy imports can keep generated child foreign-key columns visible by joining PRAGMA foreign_key_list to PRAGMA table_xinfo instead of table_info-only metadata.',
    'status' => $page['status'],
...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "application-pragma-index-xinfo-foreignkey-current-source-next249 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "application-pragma-index-xinfo-foreignkey-current-source-next249 self-test passed\n");
    exit(0);
}
