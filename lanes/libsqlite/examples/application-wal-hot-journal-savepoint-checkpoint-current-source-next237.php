...
$summary = [
    'scenario' => 'application-wal-hot-journal-savepoint-checkpoint-current-source-next237',
    'applicationUse' => 'A copied Application plugin import reuses a durable WAL sidecar after hot-journal/savepoint checkpoint only when the byte boundary, salts, checksum digest, and reader pins match the published current source.',
    'status' => $plan['status'],
...
if (PHP_SAPI === 'cli' && in_array('--self-test', $argv ?? [], true)) {
    if ($summary['status'] !== 'wal-hot-journal-savepoint-checkpoint-current-source-next237'
        ...
        fwrite(STDERR, "application-wal-hot-journal-savepoint-checkpoint-current-source-next237 self-test failed\n");
        exit(1);
    }
    echo "application-wal-hot-journal-savepoint-checkpoint-current-source-next237 self-test passed\n";
    exit(0);
}
