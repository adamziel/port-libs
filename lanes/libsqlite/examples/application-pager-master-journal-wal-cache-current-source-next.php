...
$summary = [
    'scenario' => 'application-pager-master-journal-wal-cache-current-source-next129',
    'applicationUse' => 'After a copied Application SQLite database recovers through a master journal, stale WAL page-cache entries for wp_options pages are refreshed before retry appends or checkpoint writes use them as the current source.',
    'status' => $plan['status'],
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "application-pager-master-journal-wal-cache-current-source-next129 self-test passed\n";
    return;
}
