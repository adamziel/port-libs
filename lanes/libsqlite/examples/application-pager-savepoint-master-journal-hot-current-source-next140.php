...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "application pager savepoint master-journal hot current-source next140 smoke passed\n";
    return;
}

return [
    'applicationUse' => 'A copied Application options import recovers hot rollback-journal pages through the current master journal, rolls back a failed savepoint write to those recovered current-source images, retries option/transient writes, and preserves the master journal until the outer attached transaction commits.',
    'status' => $plan['status'],
