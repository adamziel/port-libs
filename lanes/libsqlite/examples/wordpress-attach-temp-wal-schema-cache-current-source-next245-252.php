...
$schemas = [
    ...
    'analytics' => ['schema_cookie' => 31, 'tables' => ['wp_events', 'wp_eventmeta', 'wp_event_rollup'], 'indexes' => ['wp_events_name', 'wp_eventmeta_key', 'wp_event_rollup_day'], 'file' => '/srv/wp/analytics-next245.sqlite'],
    'audit' => ['schema_cookie' => 17, 'tables' => ['wp_audit_log', 'wp_auditmeta'], 'indexes' => ['wp_audit_log_action', 'wp_auditmeta_key'], 'file' => '/srv/wp/audit-next245.sqlite'],
    'search' => ['schema_cookie' => 9, 'tables' => ['wp_search_cache'], 'indexes' => ['wp_search_cache_key'], 'file' => '/srv/wp/search-next245.sqlite'],
];
...
$plan = SQLiteAttachWalTempSchemaCacheCurrentSourceNext92Plan::currentSourceNext245252($schemas, $statements, [
    ...
    ['op' => 'rename_index', 'schema' => 'audit', 'from' => 'wp_auditmeta_key', 'to' => 'wp_auditmeta_key_next247'],
    ['op' => 'attach', 'schema' => 'queue', 'schema_cookie' => 3, 'tables' => ['wp_job_queue'], 'indexes' => ['wp_job_queue_token'], 'file' => '/srv/wp/queue-next248.sqlite'],
    ...
    ['op' => 'rename_index', 'schema' => 'main', 'from' => 'wp_posts_type_status', 'to' => 'wp_posts_type_status_next252'],
...
if (($argv[1] ?? '') === '--self-test') {
    assert($plan['operation'] === 'attach-wal-temp-schema-cache-current-source-next245-252');
    assert($plan['dependencies'][0] === 'sqlite-attach-temp-wal-schema-cache-current-source-next245');
    assert($plan['dependencies'][7] === 'sqlite-attach-temp-wal-schema-cache-current-source-next252');
    assert(in_array('sqlite-attach-temp-wal-schema-cache-current-source-next244', $plan['dependencies'], true));
    ...
    echo "wordpress-attach-temp-wal-schema-cache-current-source-next245-252 self-test passed\n";
    return;
}
