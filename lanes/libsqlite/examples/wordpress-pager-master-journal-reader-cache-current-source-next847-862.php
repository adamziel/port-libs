...
$pageSize = 512;
$database = '/srv/wp-content/database/wp-next862.sqlite';
...
$formatPage = static function (string $label) use ($pageSize): string {
    ...
    $page = substr_replace($page, pack('N', 862), 60, 4);
...
$variantFields = array_merge($tokenFields, $next639654Fields, $next655670Fields, $next671686Fields, $next687702Fields, $next703718Fields, $next719734Fields, $next735766Fields, $next751766Fields, $next767782Fields, $next783798Fields, $next799814Fields, $next815830Fields, $next831846Fields, $next847862Fields);
$before = [1 => $formatPage('stale schema'), 2 => $page('stale options')];
$recovered = [1 => $formatPage('current schema'), 2 => $page('current options')];
$tokens = [$journal => 'member-main-current-862'];
$headers = [$journal => hash('sha256', 'main header next862')];
$base = [
    'source_id' => 'wordpress-pager-reader-cache-next862',
    'epoch' => 862,
    'format_signature' => hash('sha256', implode('|', [512, 4, 2, 862, 0])),
    'publication_generation' => 862,
    'master_source_digest' => hash('sha256', 'wordpress next862 master source'),
    'recovery_sequence' => 862,
...
foreach ($variantFields as $field) {
    $base[$field] = str_replace('_', '-', preg_replace('/_token$/', '', $field)) . '-current-862';
}
$base['reader_cache_stmt_vdbe_real_affinity_value_token'] = 'reader-cache-stmt-vdbe-affinity-current-862';
$base['reader_cache_stmt_vdbe_real_affinity_value_handoff_token'] = 'reader-cache-stmt-vdbe-affinity-handoff-current-862';
...
$read = array_merge($base, [
    ...
    'reader_cache_stmt_vdbe_return_branch_handoff_token' => 'stmt-vdbe-return-branch-handoff-old',
]);
$plan = SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::currentSourceVdbeVirtualTableBranchHandoff(
    $database,
    $master,
    $masterBytes,
    implode('', $before),
...
echo json_encode([
    ...
    'next862_reopen_reason' => $plan['next_reads'][0]['reader_cache_stmt_vdbe_return_branch_handoff_token_reason'],
