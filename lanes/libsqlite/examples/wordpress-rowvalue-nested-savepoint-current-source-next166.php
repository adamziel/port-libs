...
require_once __DIR__ . '/../src/SQLiteRowValueUpdateDeleteReturningSavepointCurrentSourceNext166Plan.php';

use PortLibs\LibSqlite\SQLiteRowValueUpdateDeleteReturningSavepointCurrentSourceNext166Plan;
...
$plan = SQLiteRowValueUpdateDeleteReturningSavepointCurrentSourceNext166Plan::execute(
    ['wp_options' => $rows],
    [
...
$summary = [
    'scenario' => 'wordpress-rowvalue-nested-savepoint-current-source-next166',
    'wordpressUse' => 'Preview a copied wp_options import cleanup where an inner RELEASE has yielded row-value UPDATE/DELETE RETURNING rows, but a later outer ROLLBACK TO discards those rows and retries from the original option table image.',
    'status' => $plan['status'],
...
if (PHP_SAPI === 'cli' && realpath($_SERVER['SCRIPT_FILENAME'] ?? '') === __FILE__) {
    if ($summary['discarded_returning_count'] !== 8 || $summary['yielded_returning_count'] !== 5) {
        fwrite(STDERR, "wordpress-rowvalue-nested-savepoint-current-source-next166 self-test failed\n");
        exit(1);
    }
    echo "wordpress-rowvalue-nested-savepoint-current-source-next166 self-test passed\n";
}
