...
$root = sys_get_temp_dir() . '/port-libsqlite-wordpress-hot-wal-' . bin2hex(random_bytes(4));
...
$result = [
    'scenario' => 'wordpress-hot-journal-wal-checkpoint-recovery',
    'wordpressUse' => 'Recover a copied wp_options database that has both a hot rollback journal and a WAL sidecar: restore the hot-journal image first, checkpoint the committed WAL prefix against that recovered image, discard corrupt and uncommitted WAL tails, and delete the journal without requiring ext/sqlite.',
