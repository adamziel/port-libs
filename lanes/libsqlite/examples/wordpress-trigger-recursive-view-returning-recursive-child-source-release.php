...
if (PHP_SAPI === 'cli' && basename(__FILE__) === basename($_SERVER['SCRIPT_FILENAME'] ?? '')) {
    ...
    echo "wordpress-trigger-recursive-view-returning-current-source-next163 self-test passed\n";
}

return [
    'scenario' => 'wordpress-trigger-recursive-view-returning-current-source-next163',
    'wordpressUse' => 'Copied wp_options imports through recursive INSTEAD OF view triggers materialize the current source before RETURNING rows are drained, so trigger-generated audit rows do not re-enter the current statement but can seed a released next source.',
