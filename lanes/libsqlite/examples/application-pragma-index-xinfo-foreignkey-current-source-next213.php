...
$summary = [
    'scenario' => 'application-pragma-index-xinfo-foreignkey-current-source-next213',
    'applicationUse' => 'Copied Application postmeta imports can reject FK action repairs that would SET NULL into NOT NULL columns or SET DEFAULT into columns without usable defaults.',
    'status' => $page['status'],
...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "application-pragma-index-xinfo-foreignkey-current-source-next213 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "application-pragma-index-xinfo-foreignkey-current-source-next213 self-test passed\n");
    exit(0);
}
