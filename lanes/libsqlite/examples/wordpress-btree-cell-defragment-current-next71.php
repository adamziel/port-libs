...
$summary = [
    'wordpressScenario' => 'Compact a copied wp_options table leaf after transient cleanup leaves current/next freeblocks and fragmented bytes, preserving row order while rebuilding a contiguous cell content area without ext/sqlite.',
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    if (!$ok) {
        fwrite(STDERR, "wordpress-btree-cell-defragment-current-next71 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-btree-cell-defragment-current-next71 self-test passed\n");
    exit(0);
}
