...
$stack->beginTransaction('wordpress-plugin-import-next121');
...
if (($argv[1] ?? '') === '--self-test') {
    if (
        ...
        fwrite(STDERR, "wordpress-pager-rollback-cache-spill-master-current-source-next121 self-test failed\n");
        exit(1);
    }

    echo "wordpress-pager-rollback-cache-spill-master-current-source-next121 self-test passed\n";
    exit(0);
}

echo json_encode([
    'scenario' => 'wordpress-pager-rollback-cache-spill-master-current-source-next121',
    'wordpressUse' => 'A copied wp_options plugin import crash recovers attached rollback journals through a master journal, then admits only recovered-current-source dirty pages for cache spill before retrying active_plugins writes.',
    'status' => $plan['status'],
