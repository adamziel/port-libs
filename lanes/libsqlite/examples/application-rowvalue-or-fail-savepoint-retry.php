...
$plan = SQLiteRowValueUpdateDeleteReturningSavepointPlan::executeOrFailSavepointRetry(
    ['wp_options' => $rows],
    [$outer],
    [$fail],
    [$retry, $delete],
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    if (
        ...
        fwrite(STDERR, "application-rowvalue-fail-savepoint-current-source-or-fail-savepoint-retry self-test failed\n");
        exit(1);
    }
    echo "application-rowvalue-fail-savepoint-current-source-or-fail-savepoint-retry self-test passed\n";
    exit(0);
}
