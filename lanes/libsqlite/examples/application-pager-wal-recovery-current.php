...
$pageSize = 512;
$root = sys_get_temp_dir() . '/port-libsqlite-application-current-wal-recovery-' . bin2hex(random_bytes(4));
...
if (!is_dir($directory) && !mkdir($directory, 0777, true) && !is_dir($directory)) {
    throw new RuntimeException('Unable to create Application WAL recovery directory');
}
...
$summary = [
    'scenario' => 'application-pager-wal-recovery-current',
    'applicationUse' => 'Recover the current copied Application SQLite database from its local -wal sidecar, preserving committed import pages and discarding an uncommitted crash tail before another request opens the database.',
...
if (in_array('--self-test', $argv, true)) {
    ...
    echo "application-pager-wal-recovery-current self-test passed\n";
    exit(0);
}
