...
$plan = SQLiteBTreeVacuumPointerMapFreeblockCurrentSourcePlan::tableLeafFinalImageAuditFromDeleteResult(
    $database,
    3,
    [
        'page' => $deletedPage,
    ...
    4,
    str_repeat('application-next167-freeblock-audit-', 55),
    3,
    true,
);

$summary = [
    'application_scenario' => 'wp_options transient delete keeps the leaf freeblock stable while vacuumed current-source overflow pages are reused',
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    if ($summary !== $expected) {
        fwrite(STDERR, "application-btree-vacuum-pointermap-freeblock-current-source-next167 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "application-btree-vacuum-pointermap-freeblock-current-source-next167 self-test passed\n");
    exit(0);
}
