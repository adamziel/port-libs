...
use PortLibs\LibSqlite\SQLitePagerMasterJournalReaderCacheCurrentSourceNext256Plan;
...
$plan = SQLitePagerMasterJournalReaderCacheCurrentSourceNext256Plan::plan(
    $database,
    $master,
    $masterBytes,
    implode('', $before),
...
$summary = [
    'scenario' => 'wordpress-pager-master-journal-reader-cache-current-source-next256',
    'wordpressUse' => 'A copied WordPress database reopens schema reader-cache pages unless they were opened against the same recovered master-journal current-source, database header change-counter, and schema-cookie; stale options and active_plugins reads reopen before plugin import resumes.',
    'status' => $plan['status'],
...
if ($summary['status'] !== 'pager-master-journal-reader-cache-current-source-next256'
    ...
    fwrite(STDERR, "wordpress-pager-master-journal-reader-cache-current-source-next256 self-test failed\n");
    exit(1);
}

echo json_encode($summary, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) . "\n";
echo "wordpress-pager-master-journal-reader-cache-current-source-next256 self-test passed\n";
