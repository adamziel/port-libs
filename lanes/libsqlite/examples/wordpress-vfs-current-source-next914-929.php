...
if (($argv[1] ?? '') === '--self-test') {
    ...
    assert($plan['events'][0]['ack'] === 'shared-cache-next913');
    assert($plan['events'][0]['blocked_reasons'] === []);
    assert($plan['events'][1]['status'] === 'claimed-reusable-current-source');
    assert($plan['events'][1]['claim'] === 'reader-reuse-next929');
    assert($plan['events'][1]['blocked_reasons'] === []);
    assert($plan['events'][2]['status'] === 'published-reused-current-source');
    assert($plan['events'][2]['reuse_ack'] === 'shared-cache-next913');
    assert($plan['events'][2]['token'] === 'shared-cache-next929');
    assert($plan['events'][2]['published_count'] === 49);
    assert(in_array('vfs-current-source-snapshot-reuse-publish-next914-929', $plan['dependencies'], true));
    assert(in_array('vfs-current-source-snapshot-reuse-publish-next898-913', $plan['dependencies'], true));
    echo "wordpress-vfs-current-source-next914-929 self-test passed\n";
    return;
}
