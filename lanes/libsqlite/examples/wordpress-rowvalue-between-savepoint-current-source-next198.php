...
require_once __DIR__ . '/../src/SQLiteRowValueUpdateDeleteReturningSavepointCurrentSourceNext188Plan.php';
...
$plan = SQLiteRowValueUpdateDeleteReturningSavepointCurrentSourceNext188Plan::execute(
    ['wp_options' => $rows],
...
$summary = [
    'scenario' => 'wordpress-rowvalue-between-savepoint-current-source-next198',
    'wordpressUse' => 'A copied wp_options import can use scalar BETWEEN and NOT BETWEEN in UPDATE/DELETE RETURNING while ROLLBACK TO suppresses attempted rows and retries from the original current-source savepoint.',
...
if (in_array('--self-test', $argv, true)) {
    if (
        ...
        fwrite(STDERR, "wordpress-rowvalue-between-savepoint-current-source-next198 self-test failed\n");
        exit(1);
    }
    echo "wordpress-rowvalue-between-savepoint-current-source-next198 self-test passed\n";
    exit(0);
}
