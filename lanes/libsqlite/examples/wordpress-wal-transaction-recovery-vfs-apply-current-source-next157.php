...
$root = sys_get_temp_dir() . '/port-libsqlite-wordpress-wal-txn-next157-' . bin2hex(random_bytes(4));
...
if (!is_dir(dirname($databaseLocal)) && !mkdir(dirname($databaseLocal), 0777, true) && !is_dir(dirname($databaseLocal))) {
    throw new RuntimeException('Unable to create WordPress WAL transaction recovery example directory');
}
...
$summary = [
    'wordpressUse' => 'Recover a copied WordPress SQLite database after a crash leaves valid but uncommitted WAL frames: checkpoint only the committed wp_options transaction, truncate the WAL to the committed prefix, and sync both handles before the next importer opens the file.',
...
if (
    ...
    throw new RuntimeException('Unexpected WordPress WAL transaction recovery VFS apply next157 summary');
}
