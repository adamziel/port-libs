...
$payload = [
    'scenario' => 'application-compound-select-window-recursive-limit-current-source-current-page-drain',
    'applicationUse' => 'Copied wp_options preview queries can drain the current limited compound page before exposing staged next-source rows whose window ranks shift the page boundary.',
    'status' => $plan['status'],
...
if (($payload['status'] ?? null) !== 'compound-select-window-recursive-limit-current-source-current-page-drain-ready') {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-current-page-drain self-test failed\n");
    exit(1);
}
if ($payload['requiredAckCount'] !== count($payload['currentLabels'])) {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-current-page-drain drain guard failed\n");
    exit(1);
}
