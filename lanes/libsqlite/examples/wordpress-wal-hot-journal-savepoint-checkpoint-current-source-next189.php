...
$summary = [
    'scenario' => 'wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next189',
    'wordpressUse' => 'A copied WordPress plugin import reopens after hot-journal/checkpoint recovery and admits a retained WAL reader only when its snapshot stops at the last committed retained frame and falls back to the checkpoint database for later pages.',
    'status' => $plan['status'],
...
if ($summary['status'] !== 'wal-hot-journal-savepoint-checkpoint-current-source-next189'
    ...
    fwrite(STDERR, "wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next189 self-test failed\n");
    fwrite(STDERR, json_encode($summary, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) . "\n");
    exit(1);
}

echo "wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next189 self-test passed\n";
