...
$page = SQLitePragmaIndexXinfoForeignKeyCurrentSourceNext228::page(
    $current,
    $next,
...
$summary = [
    'scenario' => 'wordpress-pragma-index-xinfo-foreignkey-current-source-next228',
    'wordpressUse' => 'Copied WordPress import schemas can trust PRAGMA foreign_key_list parent-key validation even when PRAGMA index_xinfo exposes DESC sort order on the matching UNIQUE parent index.',
...
if (PHP_SAPI === 'cli' && realpath($_SERVER['SCRIPT_FILENAME'] ?? '') === __FILE__) {
    if (
        ...
        fwrite(STDERR, "wordpress-pragma-index-xinfo-foreignkey-current-source-next228 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, json_encode($summary, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) . "\n");
    fwrite(STDOUT, "wordpress-pragma-index-xinfo-foreignkey-current-source-next228 self-test passed\n");
}
