...
$summary = [
    'scenario' => 'application-pager-master-journal-reader-cache-current-source-next166',
    'status' => $plan['status'],
    ...
    'applicationUse' => 'A copied wp_options reader can keep unchanged schema/autoload pages after master-journal recovery, while active_plugins and truncated overflow pages are reopened against the next source generation before plugin import continues.',
...
if ($summary['status'] !== 'pager-master-journal-reader-cache-current-source-next166'
    ...
    fwrite(STDERR, "application-pager-master-journal-reader-cache-current-source-next166 self-test failed\n");
    exit(1);
}

echo "application-pager-master-journal-reader-cache-current-source-next166 self-test passed\n";
