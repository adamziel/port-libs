...
$summary = [
    'scenario' => 'application-btree-vacuum-pointermap-freeblock-current-source-next156',
    'applicationUse' => 'Delete an obsolete copied wp_options transient, vacuum tail overflow pages, then reuse only the surviving materialized free page for the replacement overflow chain while preserving auto-vacuum pointer-map ownership.',
...
if (($argv[1] ?? null) === '--self-test') {
    ...
    foreach ($expected as $key => $value) {
        if ($summary[$key] !== $value) {
            fwrite(STDERR, "application-btree-vacuum-pointermap-freeblock-current-source-next156 self-test failed\n");
            exit(1);
        }
    }

    fwrite(STDOUT, "application-btree-vacuum-pointermap-freeblock-current-source-next156 self-test passed\n");
    exit(0);
}
