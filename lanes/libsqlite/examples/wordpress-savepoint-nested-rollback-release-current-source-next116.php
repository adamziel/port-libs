...
$summary = [
    'scenario' => 'wordpress-savepoint-nested-rollback-release-current-source-next116',
    'wordpressUse' => 'Model a copied wp_options import where a nested autoload-index savepoint fails, verifies the current database image, rolls back nested pages, releases the still-active savepoint, and leaves the outer plugin batch ready to retry or commit.',
...
if (PHP_SAPI === 'cli' && in_array('--self-test', $argv, true)) {
    ...
    echo "wordpress-savepoint-nested-rollback-release-current-source-next116 self-test passed\n";
    return;
}
