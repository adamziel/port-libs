...
$current = $prepared;
$current['name'] = 'current-wp-options-stat4-partial-histogram-next234';
$current['schemaCookie'] = 2349;
$current['stat4Generation'] = 432;
$current['indexes'][0]['rootPage'] = 23488;
...
$plan = SQLitePlannerStat4ExpressionPartialCurrentSourceNextPlan::materializeCurrentSourceStat4HistogramFence(
    $prepared,
    $current,
    [
...
if (in_array('--self-test', $argv, true)) {
    assert($plan['status'] === 'stat4-expression-partial-current-source-next234-ready');
    assert($plan['stat4HistogramFence']['qualifiedRowCount'] === 8);
    assert($plan['stat4HistogramFence']['sampleHistogramProofs'][2]['expected'] === ['neq' => 3, 'nlt' => 2, 'ndlt' => 2]);
    echo "application-sqlplanner-stat4-expression-partial-current-source-next234 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'application-sqlplanner-stat4-expression-partial-current-source-next234',
    'applicationUse' => 'Copied wp_options plugin-admin pagination can keep a partial lower(option_name) STAT4 plan only when current neq/nlt/ndlt histogram counts still match the current source.',
    'status' => $plan['status'],
    'selectedIndex' => $plan['selectedPlan']['name'] ?? null,
    'stat4HistogramFence' => $plan['stat4HistogramFence'],
