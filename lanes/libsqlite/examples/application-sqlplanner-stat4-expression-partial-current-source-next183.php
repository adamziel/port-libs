...
$plan = SQLitePlannerStat4ExpressionPartialCurrentSourceNextPlan::materializeCurrentSourcePartialInProbeFence(
    $prepared,
    $current,
    $where,
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    if (($plan['matchedRowids'] ?? []) !== [30, 10, 11, 20]) {
        throw new RuntimeException('Expected Application plugin IN-list rowids to follow current-source probe order');
    }
    echo "application-sqlplanner-stat4-expression-partial-current-source-next183 self-test passed\n";

    return;
}

echo json_encode([
    'scenario' => 'application-sqlplanner-stat4-expression-partial-current-source-next183',
    'status' => $plan['status'],
    'selectedSource' => $plan['selectedSource'],
    'inValues' => $plan['inValues'],
    'matchedRowids' => $plan['matchedRowids'],
    'cursorOpcodes' => array_column($plan['cursorProgram'], 'opcode'),
    'applicationUse' => 'Copied wp_options plugin preload queries can reprepare stale STAT4 partial lower(option_name) IN-list plans, probe the current source once per requested plugin key, deduplicate rowids, and avoid falling back to a table scan without requiring ext/sqlite.',
