...
$numericLike = SQLiteUtf16LikeGlobAffinityCurrentSourceCursor::wordpressOptionValueScan(
    $rows,
    'option_value',
    '1%',
    'LIKE',
...
$unicodeGlob = SQLiteUtf16LikeGlobAffinityCurrentSourceCursor::wordpressOptionValueScan(
    $rows,
    'option_value',
    'plugin_[À-ÿ]*',
    'GLOB',
...
$emojiLike = SQLiteUtf16LikeGlobAffinityCurrentSourceCursor::wordpressOptionValueScan(
    $rows,
    'option_value',
    'plugin_😀%',
    'LIKE',
...
$summary = [
    'scenario' => 'wordpress-utf16-like-glob-affinity-current-source-next87',
    ...
    'wordpressUse' => 'Preview copied wp_options values scanned through SQLite LIKE/GLOB after TEXT affinity coercion and UTF-16 encoding, so numeric settings and Unicode plugin payloads can be filtered without ext/sqlite.',
];
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "wordpress-utf16-like-glob-affinity-current-source-next87 self-test passed\n";
    return;
}
