...
$pageSize = 512;
$sectorSize = 512;
$databasePath = '/wp-content/database/wp-next91.sqlite';
$page = static fn (string $label): string => str_pad($label, $pageSize, '.', STR_PAD_RIGHT);

$clean = [
    1 => $page('wp next91 clean sqlite header'),
    2 => $page('wp next91 clean wp_options root'),
    3 => $page('wp next91 clean active_plugins'),
    4 => $page('wp next91 clean transient'),
];
$dirtyDatabase = $page('wp next91 dirty sqlite header')
    . $page('wp next91 dirty wp_options root')
    . $page('wp next91 dirty active_plugins')
    . $page('wp next91 dirty transient');
$statementBefore = $page('wp next91 statement before active_plugins insert');
$nextBefore = $page('wp next91 retry before plugin option insert');
...
foreach ([
    [1, 0, 'wp next91 retained schema frame'],
    [2, 4, 'wp next91 retained wp_options frame'],
    [3, 0, 'wp next91 failed active_plugins draft'],
    [4, 4, 'wp next91 failed transient commit'],
] as $frame) {
...
$plan = SQLiteWalHotJournalSavepointReplayPlan::statementCurrentSourceNext91(
    ...
    [1, 2, 3, 4],
    [
        1 => $page('wp next91 retained schema frame'),
        2 => $page('wp next91 retained wp_options frame'),
        3 => $page('wp next91 failed active_plugins draft'),
        4 => $page('wp next91 failed transient commit'),
    ],
    true
);
...
if (in_array('--self-test', $argv, true)) {
    if ($summary['status'] !== 'hot_journal_wal_statement_current_source_recovered_next91') {
