...
$summary = [
    'scenario' => 'application-pager-hot-journal-cache-savepoint-current-source-next131',
    'applicationUse' => 'After a copied wp_options import crashes with a hot rollback journal, refresh active savepoint page-cache sources before retrying plugin settings writes so stale dirty pages cannot shadow recovered current-source images.',
    'status' => $plan['status'],
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "application-pager-hot-journal-cache-savepoint-current-source-next131 self-test passed\n";
    return;
}
