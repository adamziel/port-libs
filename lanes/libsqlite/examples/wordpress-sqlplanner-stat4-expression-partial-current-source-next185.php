...
$plan = SQLitePlannerStat4ExpressionPartialCurrentSourceNextPlan::materializeNext185(
    $prepared,
    $current,
    [
...
if (in_array('--self-test', $argv, true)) {
    ...
    echo "wordpress-sqlplanner-stat4-expression-partial-current-source-next185 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'wordpress-sqlplanner-stat4-expression-partial-current-source-next185',
    'wordpressUse' => 'Copied wp_options plugin admin pagination can reject stale prepared STAT4 samples and verify the DESC partial lower(option_name) window against current rows after ANALYZE/source changes.',
    'status' => $plan['status'],
