...
$schemas = [
    ...
    'temp' => ['schema_cookie' => 92, 'tables' => ['wp_options', 'wp_import_stage'], 'indexes' => ['wp_temp_options_name', 'wp_import_stage_token'], 'temp' => true],
    'archive' => ['schema_cookie' => 188, 'tables' => ['wp_comments', 'wp_commentmeta'], 'indexes' => ['wp_comments_post_id', 'wp_commentmeta_key'], 'file' => '/srv/wp/archive-next189.sqlite'],
];
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    assert($plan['schema_cookies_next']['main'] === 191);
    assert($plan['statements']['stage-reader']['schema_transitions'][0]['next_found'] === false);
    assert($plan['statements']['archive-meta-reader']['index_transitions'][0]['next_found'] === false);
    assert(in_array('archive-comments-writer', $plan['write_statements_blocked_before_retry'], true));

    echo "application-attach-temp-wal-schema-cache-current-source-next189-192 self-test passed\n";
    return;
}
