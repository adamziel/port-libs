...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "application-pragma-index-xinfo-foreignkey-current-source-next177 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "application-pragma-index-xinfo-foreignkey-current-source-next177 self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'application-pragma-index-xinfo-foreignkey-current-source-next177',
    'applicationUse' => 'Copied multisite wp_options imports can keep PRAGMA index_xinfo plus foreign_key_check pages tied to named FK constraints and column/table-level origins when schema DDL changes between current and next sources.',
    'status' => $page['status'],
