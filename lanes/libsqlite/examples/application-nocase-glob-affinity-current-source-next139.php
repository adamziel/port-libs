...
$summary = SQLiteNocaseGlobAffinityCurrentSourceNextPlan::optionRowNamePlan(
    $currentRows,
    $nextRows,
    'plugin_*',
);
...
if (PHP_SAPI === 'cli' && in_array('--self-test', $argv, true)) {
    ...
    echo "application-nocase-glob-affinity-current-source-next139 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'application-nocase-glob-affinity-current-source-next139',
    'applicationUse' => 'Copied wp_options scans over a NOCASE option_name index must not reuse the bytewise GLOB prefix range; native PHP falls back to a residual scan so Plugin_Beta is rejected until the next source changes it to plugin_Beta.',
