...
$yieldUpdate = "UPDATE wp_options SET (status, option_value, bytes) = ('yield260', option_value || ':yield260', bytes + 30) WHERE (blog_id, option_name) IN ((2, 'pending_theme'), (3, 'rewrite_rules')) RETURNING option_id, blog_id, option_name, status, bytes ORDER BY option_id";
...
$attemptUpdate = "UPDATE wp_options SET (status, option_value, bytes) = ('attempt260', option_value || ':attempt260', bytes + 5) WHERE (blog_id, option_name) IN ((2, 'pending_theme'), (3, 'rewrite_rules')) RETURNING option_id, blog_id, option_name, status, bytes ORDER BY option_id";
...
$plan = SQLiteRowValueUpdateDeleteReturningWindowCurrentSourceNextPlan::executeFrameBoundaryAdmission(
    ['wp_options' => $rows],
...
if (($plan['boundary_summary_next260']['mixed_boundary_count'] ?? null) !== 2) {
    fwrite(STDERR, "Expected two current-source to next-source RETURNING frame boundaries.\n");
    exit(1);
}
if (($plan['boundary_fence_next260']['current_to_next_boundary_released'] ?? null) !== true) {
    fwrite(STDERR, "Expected current-to-next RETURNING frame boundary to be released.\n");
    exit(1);
}
if (($plan['boundary_summary_next260']['mixed_rowids'] ?? null) !== [3, 9]) {
    fwrite(STDERR, "Expected the final current row and first retry row to own the mixed boundary.\n");
    exit(1);
}

echo "wordpress-rowvalue-returning-window-frame-boundary-admission self-test passed\n";
