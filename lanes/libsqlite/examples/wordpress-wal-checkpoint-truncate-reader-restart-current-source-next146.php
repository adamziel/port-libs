...
$plan = SQLiteWalSavepointCheckpointPlan::readerCheckpointTruncateReaderRestartCurrentSourceNext146(
    $stack,
...
$summary = [
    'scenario' => 'wordpress-wal-checkpoint-truncate-reader-restart-current-source-next146',
    'wordpressUse' => 'A copied WordPress import rolls back a plugin-setting savepoint, waits for the old WAL reader before TRUNCATE, then reopens the reader against the fresh current source before retry writes append a new generation.',
    'status' => $plan['status'],
...
if (PHP_SAPI === 'cli' && basename(__FILE__) === basename($_SERVER['SCRIPT_FILENAME'] ?? '')) {
    if ($summary['status'] !== 'wal-checkpoint-truncate-reader-restart-current-source-next146'
        ...
        fwrite(STDERR, "wordpress-wal-checkpoint-truncate-reader-restart-current-source-next146 self-test failed\n");
        exit(1);
    }

    echo "wordpress-wal-checkpoint-truncate-reader-restart-current-source-next146 self-test passed\n";
}
