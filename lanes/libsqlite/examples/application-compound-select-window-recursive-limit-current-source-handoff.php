...
$out = [
    'scenario' => 'application-compound-select-window-recursive-limit-current-source-current-source-handoff',
    'applicationUse' => 'Copied wp_options preview queries keep next-source plugin rows hidden until the current-source compound/window/recursive LIMIT page high-water row is acknowledged.',
...
if (($out['status'] ?? null) !== 'compound-select-window-recursive-limit-current-source-current-source-handoff-ready') {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-current-source-handoff self-test failed\n");
    exit(1);
}
if (($out['currentHighWaterLabel'] ?? null) !== 'seed:2:3:4' || ($out['nextCandidateLabel'] ?? null) !== 'plugin_prime') {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-current-source-handoff boundary guard failed\n");
    exit(1);
}
