...
$output = [
    'scenario' => 'application-stat4-order-covering-current-source-cursor-tape',
    ...
    'applicationUse' => 'Preview copied wp_options plugin autoload SELECT planning after ANALYZE refresh: STAT4 current-source reprepare keeps ORDER BY on a covering index cursor, avoiding table rowid lookups and temp sorting without ext/sqlite.',
];
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "application-stat4-order-covering-current-source-cursor-tape self-test passed\n";
    exit(0);
}
