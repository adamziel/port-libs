...
$summary = [
    'scenario' => 'application-wal-hot-journal-savepoint-checkpoint-current-source-next217',
    'applicationUse' => 'A copied Application option import only publishes the post-hot-journal checkpoint source after retained readers acknowledge current page images and stale readers carry durable reopen fences.',
    'status' => $plan['status'],
...
if (in_array('--self-test', $argv, true)) {
    if ($summary['status'] !== 'wal-hot-journal-savepoint-checkpoint-current-source-next217'
        ...
        fwrite(STDERR, "application-wal-hot-journal-savepoint-checkpoint-current-source-next217 self-test failed\n");
        exit(1);
    }

    echo "application-wal-hot-journal-savepoint-checkpoint-current-source-next217 self-test passed\n";
    exit(0);
}
