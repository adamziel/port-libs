...
$pageSize = 512;
$sectorSize = 512;
$root = sys_get_temp_dir() . '/sqlite-application-super-master-74-' . bin2hex(random_bytes(4));
...
if (in_array('--self-test', $argv, true)) {
    foreach ([
        ...
        if (!$passed) {
            $removeTree($root);
            throw new RuntimeException('Application pager hot-journal super/master recovery current smoke failed');
        }
    }
}
