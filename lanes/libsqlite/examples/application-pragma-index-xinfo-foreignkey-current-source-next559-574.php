...
$summary = [
    'scenario' => 'application-pragma-index-xinfo-foreignkey-current-source-next559-574',
    'applicationUse' => 'Application import previews can continue paging next-source foreign-key action relationship diagnostics after next543-558 while preserving order, collation, and DESC child lookup mismatch coverage.',
...
if (($argv[1] ?? null) === '--self-test') {
    foreach ($cases as $slice => [, , , $status]) {
        if (($summary["next{$slice}_current_{$status}"] ?? null) !== 0 || ($summary["next{$slice}_next_{$status}"] ?? null) !== 1) {
            fwrite(STDERR, "application-pragma-index-xinfo-foreignkey-current-source-next559-574 self-test failed\n");
            exit(1);
        }
    }
    ...
    fwrite(STDOUT, "application-pragma-index-xinfo-foreignkey-current-source-next559-574 self-test passed\n");
    exit(0);
}
