...
use PortLibs\LibSqlite\SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext219Plan;
...
$plan = SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext219Plan::compare(
    $sql,
    ['wp_options' => $current],
    ['wp_options' => $next],
);
...
$payload = [
    'scenario' => 'wordpress-compound-select-window-recursive-limit-current-source-next219',
    'wordpressUse' => 'Copied wp_options previews can fence percent_rank and cume_dist window rows after recursive queue LIMIT/OFFSET and before EXCEPT plus final LIMIT admits next-source plugin rows.',
    'status' => $plan['status'],
...
if (($payload['status'] ?? null) !== 'compound-select-window-recursive-limit-current-source-next219-ready') {
    fwrite(STDERR, "wordpress-compound-select-window-recursive-limit-current-source-next219 self-test failed\n");
    exit(1);
}
if ($payload['currentTokenLength'] !== 64 || $payload['nextTokenLength'] !== 64) {
    fwrite(STDERR, "wordpress-compound-select-window-recursive-limit-current-source-next219 token guard failed\n");
    exit(1);
}
if (!in_array('plugin_fresh', $payload['nextOnlyRows'], true)) {
    fwrite(STDERR, "wordpress-compound-select-window-recursive-limit-current-source-next219 next-source boundary failed\n");
    exit(1);
}
