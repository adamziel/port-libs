...
$summary = [
    'scenario' => 'wordpress-btree-pointermap-vacuum-freeblock-current-source-next127',
    'wordpress_use' => 'Delete a copied wp_options transient row, defragment its table-leaf freeblock, free obsolete overflow pages, and let incremental auto-vacuum truncate tail pages with pointer-map free-page evidence.',
...
if (PHP_SAPI === 'cli' && in_array('--self-test', $argv, true)) {
    ...
    if (!$ok) {
        fwrite(STDERR, "wordpress-btree-pointermap-vacuum-freeblock-current-source-next127 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-btree-pointermap-vacuum-freeblock-current-source-next127 self-test passed\n");
}
