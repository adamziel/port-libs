...
$plan = SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNextPlan::compareRecursiveOrderCommaLimitNtile($sql, $currentTables, $nextTables);

if (($argv[1] ?? null) === '--self-test') {
    assert($plan['status'] === 'compound-select-window-recursive-limit-current-source-recursive-order-comma-limit-ntile-ready');
    ...
    echo "application-compound-select-window-recursive-limit-current-source-recursive-order-comma-limit-ntile self-test passed\n";
    return;
}
