...
$summary = [
    'scenario' => 'wordpress-wal-checkpoint-savepoint-yield',
    'wordpressUse' => 'Expose copied wp_options import reader visibility before ROLLBACK TO, after savepoint WAL truncation, and after checkpoint reset/truncate without requiring ext/sqlite.',
    'status' => $plan['status'],
...
if (PHP_SAPI === 'cli' && in_array('--self-test', $argv, true)) {
    ...
    fwrite(STDOUT, "wordpress-wal-checkpoint-savepoint-yield self-test passed\n");
    return;
}
