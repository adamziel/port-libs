...
$summary = [
    'scenario' => 'wordpress-pragma-integrity-foreignkey-cursor-current-source-next134',
    'wordpressUse' => 'Resume copied multisite/archive wp_options foreign-key diagnostics against quoted attached schema names without losing current-source cursor identity.',
    'total' => $page['total'],
...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "wordpress-pragma-integrity-foreignkey-cursor-current-source-next134 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-integrity-foreignkey-cursor-current-source-next134 self-test passed\n");
    exit(0);
}
