...
if (
    ...
    fwrite(STDERR, "application-pragma-index-quickcheck-foreignkey-current-source-next self-test failed\n");
    exit(1);
}

fwrite(STDOUT, "application-pragma-index-quickcheck-foreignkey-current-source-next self-test passed\n");
echo json_encode([
    'scenario' => 'copied wp_options PRAGMA index_list quick_check foreign_key_check current-source repair',
    'applicationUse' => 'Resume copied Application option-index analysis only when index catalog metadata, quick_check root blockers, and foreign_key_check rows all match the current source before continuing an import repair.',
