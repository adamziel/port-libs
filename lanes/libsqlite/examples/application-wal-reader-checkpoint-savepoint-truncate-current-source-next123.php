...
$report = [
    'scenario' => 'application-wal-reader-checkpoint-savepoint-truncate-current-source-next123',
    'applicationUse' => 'During a copied wp_options import, a stale reader keeps a pre-rollback WAL source while savepoint rollback keeps only the retained WAL prefix; once the reader releases, truncate checkpoint makes next readers use the checkpointed database and an empty WAL sidecar without requiring ext/sqlite.',
    'status' => $plan['status'],
