...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "wordpress-expression-covering-range-current-source-next146 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'wordpress-expression-covering-range-current-source-next146',
    'status' => $plan['status'],
    ...
    'wordpressUse' => 'Copied wp_options plugin screens can keep a lower(option_name) covering expression range cursor after ANALYZE/schema refresh only when the current covering payload matches the next source fence, avoiding stale prepared bounds and table b-tree lookups.',
