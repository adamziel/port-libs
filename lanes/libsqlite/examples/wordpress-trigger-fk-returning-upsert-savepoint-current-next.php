...
if (($argv[1] ?? null) === '--self-test') {
    if ($plan['status'] !== 'current-returned-then-rolled-back-next-applied' || $plan['current_returning_rows'] === [] || $plan['next_returning_rows'][0]['option_name'] !== 'fixed_plugin') {
        fwrite(STDERR, "wordpress-trigger-fk-returning-upsert-savepoint-current-next75 self-test failed\n");
        exit(1);
    }
    echo "wordpress-trigger-fk-returning-upsert-savepoint-current-next75 self-test passed\n";
    exit(0);
}
