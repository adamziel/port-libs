...
$payload = [
    'scenario' => 'wordpress-compound-select-window-recursive-limit-current-source-window-metric-fence',
...
if (($payload['status'] ?? null) !== 'compound-select-window-recursive-limit-current-source-window-metric-fence-ready') {
    fwrite(STDERR, "wordpress-compound-select-window-recursive-limit-current-source-window-metric-fence self-test failed\n");
    exit(1);
}
if ($payload['requiredMetricAckCount'] !== 6) {
    fwrite(STDERR, "wordpress-compound-select-window-recursive-limit-current-source-window-metric-fence metric ack guard failed\n");
    exit(1);
}
if ($payload['metricFenceTokenLength'] !== 64) {
    fwrite(STDERR, "wordpress-compound-select-window-recursive-limit-current-source-window-metric-fence token guard failed\n");
    exit(1);
}
if ($payload['nextExposure'] !== 'held-until-current-window-metric-acks-match') {
    fwrite(STDERR, "wordpress-compound-select-window-recursive-limit-current-source-window-metric-fence exposure guard failed\n");
    exit(1);
}
