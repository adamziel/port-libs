...
$pageSize = 512;
$database = '/srv/wp-content/database/wp-next526.sqlite';
...
$formatPage = static function (string $label) use ($pageSize): string {
    ...
    $page = substr_replace($page, pack('N', 526), 60, 4);
...
$tokens = [$journal => 'member-main-current-526'];
$headers = [$journal => hash('sha256', 'main header next526')];
$base = [
    'source_id' => 'application-pager-reader-cache-next526',
    'epoch' => 526,
    'format_signature' => hash('sha256', implode('|', [512, 4, 2, 526, 0])),
    'publication_generation' => 526,
    'master_source_digest' => hash('sha256', 'application next526 master source'),
    'recovery_sequence' => 526,
...
foreach ($tokenFields as $field) {
    $base[$field] = str_replace('_', '-', preg_replace('/_token$/', '', $field)) . '-current-526';
}
$cache = [1 => array_merge($base, ['reader_id' => 'wp-options-reader', 'image' => $recovered[1], 'reader_cache_stmt_vdbe_vfilter_token' => 'stmt-vdbe-vfilter-old'])];
$read = array_merge($base, [
    'reader_id' => 'read-options',
    'page_number' => 1,
    'member_journal_token_digest' => $mapDigest($tokens),
    'member_journal_header_digest' => $mapDigest($headers),
    'reader_cache_stmt_vdbe_vfilter_token' => 'stmt-vdbe-vfilter-old',
]);
$plan = SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::readerCacheStatementFence(526,
    $database,
    $master,
    $masterBytes,
    implode('', $before),
...
echo json_encode([
    ...
    'next526_invalidated_pages' => $plan['reader_cache_stmt_vdbe_vfilter_invalidated_cache_page_numbers'],
