...
$schemas = [
    ...
    'temp' => ['schema_cookie' => 152, 'tables' => ['wp_options', 'wp_import_batch', 'wp_rewrite_stage', 'wp_user_stage'], 'indexes' => ['wp_temp_options_name', 'wp_import_batch_token', 'wp_rewrite_stage_name', 'wp_user_stage_login'], 'temp' => true],
    'analytics' => ['schema_cookie' => 39, 'tables' => ['wp_events', 'wp_eventmeta', 'wp_event_rollup'], 'indexes' => ['wp_events_name', 'wp_eventmeta_key', 'wp_event_rollup_day'], 'file' => '/srv/wp/analytics-next253.sqlite'],
    'audit' => ['schema_cookie' => 25, 'tables' => ['wp_audit_log', 'wp_auditmeta'], 'indexes' => ['wp_audit_log_action', 'wp_auditmeta_key'], 'file' => '/srv/wp/audit-next253.sqlite'],
    'search' => ['schema_cookie' => 17, 'tables' => ['wp_search_cache'], 'indexes' => ['wp_search_cache_key'], 'file' => '/srv/wp/search-next253.sqlite'],
];
...
$plan = SQLiteAttachWalTempSchemaCacheCurrentSourceNext92Plan::currentSourceNext253260($schemas, $statements, [
...
if (($argv[1] ?? '') === '--self-test') {
    assert($plan['operation'] === 'attach-wal-temp-schema-cache-current-source-next253-260');
    assert($plan['dependencies'][0] === 'sqlite-attach-temp-wal-schema-cache-current-source-next253');
    assert($plan['dependencies'][7] === 'sqlite-attach-temp-wal-schema-cache-current-source-next260');
    assert(in_array('sqlite-attach-temp-wal-schema-cache-current-source-next252', $plan['dependencies'], true));
    assert($plan['event_count'] === 9);
    assert($plan['changed_schemas'] === ['temp', 'main', 'analytics', 'audit', 'queue', 'search']);
    assert($plan['schema_cookies_next']['main'] === 255);
    assert($plan['schema_cookies_next']['temp'] === 154);
    assert($plan['schema_cookies_next']['analytics'] === 260);
    ...
    echo "wordpress-attach-temp-wal-schema-cache-current-source-next253-260 self-test passed\n";
    return;
}
