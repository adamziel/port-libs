...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "wordpress-pragma-index-integrity-cursor-current-source-next self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-index-integrity-cursor-current-source-next self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'wordpress-pragma-index-integrity-cursor-current-source-next',
    'wordpressUse' => 'Resume copied wp_options PRAGMA index_list/index_xinfo/rootpage integrity pagination only when the table index catalog and database image still match the current source.',
    'status' => $first['status'],
