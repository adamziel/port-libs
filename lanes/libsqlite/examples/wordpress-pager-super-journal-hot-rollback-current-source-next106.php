...
$summary = [
    'scenario' => 'wordpress_pager_super_journal_hot_rollback_current_source_next106',
    'status' => $plan['status'],
    ...
    'wordpressUse' => 'Recover attached copied WordPress SQLite databases after a crashed multi-database plugin import by replaying only current-source hot rollback journals named by the super-journal, ignoring stale snapshots, and deleting the super-journal only after every named participant clears.',
...
if (PHP_SAPI === 'cli' && in_array('--self-test', $argv, true)) {
    ...
    echo "wordpress-pager-super-journal-hot-rollback-current-source-next106 self-test passed\n";
    return;
}
