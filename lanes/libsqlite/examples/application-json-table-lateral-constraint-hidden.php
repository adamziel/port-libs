...
$summary = [
    'scenario' => 'application-json-table-lateral-constraint-hidden',
    'applicationUse' => 'Copied wp_options diagnostics can replan a lateral json_each scan when hidden json/root constraints are sourced from the current option row, preserving left-join NULL extension, host additions, and current-to-next rule expansion without requiring ext/sqlite.',
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "application-json-table-lateral-constraint-hidden self-test passed\n";
    return;
}
