...
if (($argv[1] ?? null) === '--self-test') {
    if ($first['next']['offset'] !== 40 || $second['source_id'] !== $first['source_id'] || $second['rows'][0]['source'] !== 'pointer_map') {
        fwrite(STDERR, "wordpress-pragma-integrity-pointermap-foreignkey-current-source-next83 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-integrity-pointermap-foreignkey-current-source-next83 self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'copied wp_options pragma integrity pointer-map foreign-key current source next83',
    'wordpressUse' => 'Resume copied WordPress SQLite integrity and foreign_key_check pagination only when the database image, pragma SQL, and staged wp_options FK rows still match the original source.',
    'first' => [
        'source_id' => $first['source_id'],
        'count' => $first['count'],
        'total' => $first['total'],
