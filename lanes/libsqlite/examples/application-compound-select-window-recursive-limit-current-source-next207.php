...
$payload = [
    'scenario' => 'application-compound-select-window-recursive-limit-current-source-next207',
    'applicationUse' => 'Copied wp_options preview queries can remove a stale recursive lead() row with EXCEPT before INTERSECT membership and final LIMIT/OFFSET admit staged next-source autoload rows.',
    'status' => $plan['status'],
...
if (($payload['status'] ?? null) !== 'compound-select-window-recursive-limit-current-source-next207-ready') {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-next207 self-test failed\n");
    exit(1);
}
if ($payload['operators'] !== ['UNION ALL', 'EXCEPT', 'INTERSECT']) {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-next207 operator fence failed\n");
    exit(1);
}
if ($payload['currentTokenLength'] !== 64 || $payload['nextTokenLength'] !== 64) {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-next207 token guard failed\n");
    exit(1);
}
