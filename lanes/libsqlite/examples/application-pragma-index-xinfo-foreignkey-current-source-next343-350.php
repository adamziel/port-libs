...
$summary = [
    'scenario' => 'application-pragma-index-xinfo-foreignkey-current-source-next343-350',
    'applicationUse' => 'Application import previews can carry the next335-342 SET NULL and SET DEFAULT PRAGMA index_xinfo action diagnostics forward as current-source next343-350 admission pages.',
...
if (($argv[1] ?? null) === '--self-test') {
    foreach (array_keys($summary) as $key) {
        if (str_starts_with($key, 'next') && $summary[$key] !== 1) {
            fwrite(STDERR, "application-pragma-index-xinfo-foreignkey-current-source-next343-350 self-test failed\n");
            exit(1);
        }
    }
    ...
    fwrite(STDOUT, "application-pragma-index-xinfo-foreignkey-current-source-next343-350 self-test passed\n");
    exit(0);
}
