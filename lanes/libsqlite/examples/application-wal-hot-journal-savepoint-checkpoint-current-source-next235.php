...
$databaseDigest = $hash('application next235 checkpoint database image');
$walDigest = $hash('application next235 restarted wal sidecar');
$readerPlan = [
...
$summary = [
    'scenario' => 'application-wal-hot-journal-savepoint-checkpoint-current-source-next235',
    'applicationUse' => 'A copied Application import keeps reopened wp_options readers on the current source only after the checkpoint database, restarted WAL, deleted hot journal, and containing directory have matching durable publication receipts.',
    'status' => $plan['status'],
...
if (PHP_SAPI === 'cli' && basename(__FILE__) === basename($_SERVER['SCRIPT_FILENAME'] ?? '')) {
    if ($summary['status'] !== 'wal-hot-journal-savepoint-checkpoint-current-source-next235'
        ...
        fwrite(STDERR, "application-wal-hot-journal-savepoint-checkpoint-current-source-next235 self-test failed\n");
        exit(1);
    }

    echo "application-wal-hot-journal-savepoint-checkpoint-current-source-next235 self-test passed\n";
}
