...
$yieldUpdate = "UPDATE wp_options SET (status, option_value, bytes) = ('yield265', option_value || ':yield265', bytes + 30) WHERE (blog_id, option_name) IN ((2, 'pending_theme'), (3, 'rewrite_rules')) RETURNING option_id, blog_id, option_name, status, bytes ORDER BY option_id";
...
$attemptUpdate = "UPDATE wp_options SET (status, option_value, bytes) = ('attempt265', option_value || ':attempt265', bytes + 5) WHERE (blog_id, option_name) IN ((2, 'pending_theme'), (3, 'rewrite_rules')) RETURNING option_id, blog_id, option_name, status, bytes ORDER BY option_id";
...
$retryUpdate = "UPDATE wp_options SET (status, option_value, bytes) = ('retry265', option_value || ':retry265', bytes + 20) WHERE (blog_id, option_name) IN ((2, 'pending_theme'), (3, 'rewrite_rules'), (4, 'plugin_batch')) RETURNING option_id, blog_id, option_name, status, bytes ORDER BY option_id";
...
$plan = SQLiteRowValueUpdateDeleteReturningWindowCurrentSourceNextPlan::executeNext265(
    ['wp_options' => $rows],
...
assert($plan['status'] === 'rowvalue-update-delete-returning-window-current-source-next265');
assert($plan['receipt_ledger_admission_next265']['ledger_count'] === 8);

if (($argv[1] ?? null) === '--self-test') {
    echo "wordpress-rowvalue-returning-window-current-source-next265 self-test passed\n";
    return;
}

return [
    'status' => $plan['status'],
    'ledgerCount' => $plan['receipt_ledger_admission_next265']['ledger_count'],
    'dependencyClosure' => $plan['dependency_closure_next265'],
];
