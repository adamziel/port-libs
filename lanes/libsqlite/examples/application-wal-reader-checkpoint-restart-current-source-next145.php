...
$summary = [
    'scenario' => 'application-wal-reader-checkpoint-restart-current-source-next145',
    'applicationUse' => 'A copied Application options import rolls back a plugin-setting savepoint while an existing reader pins the current WAL source; RESTART checkpoint waits for that reader, then preserves a fresh WAL header for the retry write.',
    'status' => $plan['status'],
...
if (PHP_SAPI === 'cli' && basename(__FILE__) === basename($_SERVER['SCRIPT_FILENAME'] ?? '')) {
    if ($summary['status'] !== 'reader-checkpoint-restart-savepoint-current-source-next145'
        ...
        fwrite(STDERR, "application-wal-reader-checkpoint-restart-current-source-next145 self-test failed\n");
        exit(1);
    }

    echo "application-wal-reader-checkpoint-restart-current-source-next145 self-test passed\n";
}
