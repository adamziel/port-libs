...
if (($argv[1] ?? null) === '--self-test') {
    if ($page['status'] !== 'ok' || $page['current']['foreign_key'] !== 2 || $page['delta']['cleared'] !== true || $page['next_counts']['blockers'] !== 0) {
        fwrite(STDERR, "wordpress-pragma-index-xinfo-foreignkey-current-source-next158 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-index-xinfo-foreignkey-current-source-next158 self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'wordpress-pragma-index-xinfo-foreignkey-current-source-next158',
    'wordpressUse' => 'Resume copied wp_options index metadata and orphan-option foreign_key_check diagnostics from the same current source before admitting a repaired next database image.',
    'status' => $page['status'],
