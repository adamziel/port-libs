...
$summary = [
    ...
    'wordpressUse' => 'Preview copied wp_options imports where an UPSERT conflict on siteurl recursively UPSERTs the current home row, yielding statement and trigger rows without requiring ext/sqlite.',
];

if (($argv[1] ?? null) === '--self-test') {
    if ($summary['changes'] !== 3 || $summary['optionValues'] !== ['https://new.test', 'https://new.test/home', 'enabled']) {
        fwrite(STDERR, "wordpress-recursive-upsert-conflict-yield-current-next30 self-test failed\n");
        exit(1);
    }

    echo "wordpress-recursive-upsert-conflict-yield-current-next30 self-test passed\n";
    exit(0);
}
