...
use PortLibs\LibSqlite\SQLitePagerMasterJournalReaderCacheCurrentSourceNext171Plan;

require_once __DIR__ . '/../src/SQLitePagerMasterJournalReaderCacheCurrentSourceNext171Plan.php';
...
$plan = SQLitePagerMasterJournalReaderCacheCurrentSourceNext171Plan::plan(
    $databasePath,
    $masterPath,
    $masterBytes,
    $pageSize,
...
$summary = [
    'scenario' => 'wordpress-pager-master-journal-reader-cache-current-source-next171',
    'status' => $plan['status'],
    ...
    'wordpressUse' => 'A copied wp_options database can keep a shared schema cache page and refresh a stale options page only when the reader owns the current master-journal recovery/read-lock ticket; active_plugins and rewrite_rules readers opened before recovery are forced back to the current source.',
...
if (
    ...
    fwrite(STDERR, "wordpress-pager-master-journal-reader-cache-current-source-next171 self-test failed\n");
    exit(1);
}

echo "wordpress-pager-master-journal-reader-cache-current-source-next171 self-test passed\n";
