...
$pageCacheDigest = $hash('wordpress next247 clean page cache image');
$readerBaseline = [
...
$summary = [
    'scenario' => 'wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next247',
    'wordpressUse' => 'A copied WordPress import seals post-checkpoint cleanup only after hot-journal unlink, WAL sync, directory sync, savepoint release, and reader-fence receipts all match the reopened-reader current source.',
    'status' => $plan['status'],
...
if (($argv[1] ?? null) === '--self-test') {
    if ($summary['status'] !== 'wal-hot-journal-savepoint-checkpoint-current-source-next247'
        ...
        fwrite(STDERR, "wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next247 self-test failed\n");
        exit(1);
    }

    echo "wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next247 self-test passed\n";
    exit(0);
}
