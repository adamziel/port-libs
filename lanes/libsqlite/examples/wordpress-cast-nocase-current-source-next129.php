...
$summary = [
    'scenario' => 'wordpress-cast-nocase-current-source-next129',
    'wordpressUse' => 'Copied wp_options import scans can keep a NOCASE LIKE prefix range over CAST(option_value AS TEXT) while retaining the LIKE residual and invalidating stale cursors when the next source changes matching option values.',
    'range' => $plan['range'],
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "wordpress-cast-nocase-current-source-next129 self-test passed\n";
    return;
}
