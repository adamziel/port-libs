...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "wordpress-rowvalue-update-delete-returning-savepoint-current-source-next162 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'wordpress-rowvalue-update-delete-returning-savepoint-current-source-next162',
    'status' => $plan['status'],
    ...
    'wordpressUse' => 'Copied wp_options import batches that stream RETURNING rows from row-value UPDATE OR FAIL must roll back partial current-source mutations and discard already-yielded rows when the importer rolls back to its savepoint.',
