...
$output = [
    'scenario' => 'wordpress-planner-stat4-order-covering-current-source',
    ...
    'wordpressUse' => 'Preview copied multisite wp_options plugin scans where ANALYZE refreshes sqlite_stat4 and a stale prepared ORDER BY plan must be rebound to the current covering index before plugin import cleanup queries run.',
];
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "wordpress-planner-stat4-order-covering-current-source self-test passed\n";
    exit(0);
}
