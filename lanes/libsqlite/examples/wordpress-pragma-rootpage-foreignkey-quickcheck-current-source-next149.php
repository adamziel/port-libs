...
if (($argv[1] ?? null) === '--self-test') {
    if ($page['status'] !== 'ok' || $page['current']['quick_check_errors'] !== 2 || $page['current']['foreign_key_violations'] !== 4 || $page['delta']['total'] !== -6) {
        fwrite(STDERR, "wordpress-pragma-rootpage-foreignkey-quickcheck-current-source-next149 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-rootpage-foreignkey-quickcheck-current-source-next149 self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'wordpress-pragma-rootpage-foreignkey-quickcheck-current-source-next149',
    'wordpressUse' => 'Run bounded PRAGMA quick_check(N) rootpage diagnostics before a full foreign_key_check during copied wp_options import repair.',
    'status' => $page['status'],
