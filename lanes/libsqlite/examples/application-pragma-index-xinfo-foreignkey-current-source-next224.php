...
$summary = [
    'scenario' => 'application-pragma-index-xinfo-foreignkey-current-source-next224',
    'applicationUse' => 'Copied multisite option imports can detect parent UNIQUE indexes whose COLLATE terms do not match the parent table declaration before enabling foreign-key checks.',
    'status' => $page['status'],
...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "application-pragma-index-xinfo-foreignkey-current-source-next224 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "application-pragma-index-xinfo-foreignkey-current-source-next224 self-test passed\n");
    exit(0);
}
