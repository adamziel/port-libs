...
$plan = SQLiteRowValueUpdateDeleteReturningSavepointPlan::executeEmptyRowValueInSavepointRetry(
    ['wp_options' => $rows],
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "application-rowvalue-empty-in-savepoint-retry self-test passed\n";
    return;
}

echo json_encode([
    'applicationUse' => 'Copied wp_options import cleanup preserves SQLite empty row-value IN semantics during UPDATE/DELETE RETURNING retries: IN () selects no rows, NOT IN () selects rows including NULL tuples, and ROLLBACK TO suppresses attempted RETURNING rows.',
    'status' => $plan['status'],
