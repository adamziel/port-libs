...
$summary = [
    'scenario' => 'application-pragma-index-xinfo-foreignkey-current-source-next223',
    'applicationUse' => 'Copied Application import schemas can surface custom MATCH names from PRAGMA foreign_key_list before assuming SQLite will enforce alternate MATCH FULL/PARTIAL semantics.',
    'status' => $page['status'],
...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "application-pragma-index-xinfo-foreignkey-current-source-next223 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "application-pragma-index-xinfo-foreignkey-current-source-next223 self-test passed\n");
    exit(0);
}
