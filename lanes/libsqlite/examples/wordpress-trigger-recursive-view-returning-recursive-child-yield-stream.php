...
$summary = SQLiteTriggerRecursiveViewReturningCurrentSourceNextPlan::executeRecursiveChildYieldStream(
    ...
    $nextView,
    $returning,
    ['savepoint' => 'wp_recursive_view_172'],
);
...
if (
    ...
    || array_column($summary['suppressed_returning_rows'], 'name') !== ['home', 'next_plugin', 'home:child', 'home:child:child']
) {
    fwrite(STDERR, "wordpress-trigger-recursive-view-returning-current-source-next172 self-test failed\n");
    exit(1);
}

echo "wordpress-trigger-recursive-view-returning-current-source-next172 self-test passed\n";
