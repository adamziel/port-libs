...
$summary = [
    'scenario' => 'application-wal-hot-journal-reader-checkpoint-current-source-next148',
    'applicationUse' => 'A copied Application SQLite database recovers a hot rollback journal, checks that checkpoint database bytes were built from that recovered source plus current WAL frames, and keeps the current reader pinned until it reopens.',
    'status' => $plan['status'],
...
if (PHP_SAPI === 'cli' && basename(__FILE__) === basename($_SERVER['SCRIPT_FILENAME'] ?? '')) {
    if (($argv[1] ?? '') === '--self-test') {
        ...
        echo 'application-wal-hot-journal-reader-checkpoint-current-source-next148 self-test passed' . PHP_EOL;
        return $summary;
    }
