...
$payload = [
    'scenario' => 'wordpress-pragma-integrity-autoindex-pointermap-current-source-next89',
    'status' => $page['status'],
...
if (($argv[1] ?? null) === '--self-test') {
    if ($payload['status'] !== 'blocked' || $payload['total_rows'] !== 5 || $payload['pointer_map_errors'] !== 2) {
        fwrite(STDERR, "wordpress-pragma-integrity-autoindex-pointermap-current-source-next89 self-test failed\n");
        exit(1);
    }

    echo "wordpress-pragma-integrity-autoindex-pointermap-current-source-next89 self-test passed\n";
    exit(0);
}
