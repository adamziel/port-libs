...
if (in_array('--self-test', $argv, true)) {
    ...
    if (!$ok) {
        fwrite(STDERR, "application-trigger-upsert-returning-savepoint-current-source-next129 self-test failed\n");
        exit(1);
    }

    echo "application-trigger-upsert-returning-savepoint-current-source-next129 self-test passed\n";
    exit(0);
}

echo json_encode([
    'scenario' => 'application-trigger-upsert-returning-savepoint-current-source-next129',
    'status' => $plan['status'],
