...
$summary = [
    'scenario' => 'application-wal-sidecar-retirement',
    'applicationUse' => 'A copied Application plugin import publishes the next current source only after the recovered hot journal is deleted, the restarted WAL generation is durable, SHM read marks are reset, and the sidecar directory entry is synced.',
    'status' => $plan['status'],
...
if (PHP_SAPI === 'cli' && basename(__FILE__) === basename($_SERVER['SCRIPT_FILENAME'] ?? '')) {
    if ($summary['status'] !== 'wal-hot-journal-savepoint-checkpoint-current-source-next221'
        ...
        fwrite(STDERR, "application-wal-sidecar-retirement self-test failed\n");
        exit(1);
    }

    echo "application-wal-sidecar-retirement self-test passed\n";
}
