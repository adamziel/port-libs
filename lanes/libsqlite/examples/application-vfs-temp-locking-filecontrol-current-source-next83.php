...
$summary = [
    'scenario' => 'application-vfs-temp-locking-filecontrol',
    'applicationUse' => 'Route copied wp_options temp statement-journal locks and xFileControl calls through SQLite current-source rules: unqualified controls follow temp after a temp open, schema-qualified main controls still reach the main handle, and non-delete temp handles reuse persisted file-control state after close/reopen without ext/sqlite.',
    'status' => $plan['status'],
...
if (PHP_SAPI === 'cli' && in_array('--self-test', $argv, true)) {
    ...
    echo "application-vfs-temp-locking-filecontrol self-test passed\n";
    return;
}
