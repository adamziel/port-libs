...
if (($argv[1] ?? '') === '--self-test') {
    if ($plan['changes'] !== 6 || count($plan['returning']) !== 6 || $plan['max_depth_seen'] !== 2) {
        fwrite(STDERR, "wordpress-trigger-upsert-returning-recursive-current-next51 self-test failed\n");
        exit(1);
    }

    echo "wordpress-trigger-upsert-returning-recursive-current-next51 self-test passed\n";
    exit(0);
}
