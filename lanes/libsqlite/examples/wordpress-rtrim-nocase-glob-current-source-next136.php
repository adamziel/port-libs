...
$plan = SQLiteRtrimNocaseGlobCurrentSourceNext136Plan::wordpressOptionNameExpressionPlan(
    $current,
    $next,
    'plugin_*',
    'main.wp_options@135',
...
if (($argv[1] ?? null) === '--self-test') {
    ...
    echo "wordpress-rtrim-nocase-glob-current-source-next136 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'wordpress-rtrim-nocase-glob-current-source-next136',
    'wordpressUse' => 'Copied wp_options scans over rtrim(option_name) COLLATE NOCASE can keep a broad encoded index range while applying byte-sensitive GLOB residual checks and invalidating stale cursors when the next source changes bytes, encoding, schema, or matching rowsets.',
