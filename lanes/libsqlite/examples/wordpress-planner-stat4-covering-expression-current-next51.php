...
$summary = [
    'wordpressScenario' => 'Copied wp_options plugin settings can satisfy json_extract(option_value,$.mode) lookup and projected JSON operator expressions from the same STAT4-estimated expression index without decoding the table row.',
...
if (in_array('--self-test', $argv, true)) {
    if (
        ...
        fwrite(STDERR, "wordpress-planner-stat4-covering-expression-current-next51 self-test failed\n");
        exit(1);
    }

    echo "wordpress-planner-stat4-covering-expression-current-next51 self-test passed\n";
    exit(0);
}
