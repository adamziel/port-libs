...
$summary = [
    'scenario' => 'wordpress-window-rowvalue-upsert-current-source-next108',
    'wordpressUse' => 'Copied wp_options imports can apply UPSERT DO UPDATE only when an incoming row-value tuple is newer than the statement current row, then feed accepted RETURNING rows into a bounded window frame for import diagnostics without ext/sqlite.',
...
if (PHP_SAPI === 'cli' && in_array('--self-test', $argv, true)) {
    ...
    echo "wordpress-window-rowvalue-upsert-current-source-next108 self-test passed\n";
    return;
}
