...
$pageSize = 512;
$database = '/srv/wp-content/database/wp-next414.sqlite';
...
$formatPage = static function (string $label) use ($pageSize): string {
    ...
    $page = substr_replace($page, pack('N', 414), 60, 4);
...
$tokenFields = [
    ...
    'reader_cache_stmt_vdbe_reset_token',
];
$before = [1 => $formatPage('stale schema'), 2 => $page('stale options')];
$recovered = [1 => $formatPage('current schema'), 2 => $page('current options')];
$tokens = [$journal => 'member-main-current-414'];
$headers = [$journal => hash('sha256', 'main header next414')];
$base = [
    'source_id' => 'application-pager-reader-cache-next414',
    'epoch' => 414,
    'format_signature' => hash('sha256', implode('|', [512, 4, 2, 414, 0])),
    'publication_generation' => 414,
    'master_source_digest' => hash('sha256', 'application next414 master source'),
    'recovery_sequence' => 414,
...
foreach ($tokenFields as $field) {
    $base[$field] = str_replace('_', '-', preg_replace('/_token$/', '', $field)) . '-current-414';
}
$cache = [1 => array_merge($base, ['reader_id' => 'wp-options-reader', 'image' => $recovered[1], 'reader_cache_stmt_vdbe_reset_token' => 'stmt-vdbe-reset-old'])];
$read = array_merge($base, [
    'reader_id' => 'read-options',
    'page_number' => 1,
    'member_journal_token_digest' => $mapDigest($tokens),
    'member_journal_header_digest' => $mapDigest($headers),
    'reader_cache_stmt_vdbe_reset_token' => 'stmt-vdbe-reset-old',
]);
$plan = SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::readerCacheStatementFence(414,
    $database,
    $master,
    $masterBytes,
    implode('', $before),
...
echo json_encode([
    ...
    'next414_invalidated_pages' => $plan['reader_cache_stmt_vdbe_reset_invalidated_cache_page_numbers'],
