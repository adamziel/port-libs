...
echo json_encode([
    ...
    'wordpressUse' => 'Preview ALTER TABLE wp_options RENAME TO wp_options_imported and ALTER TABLE wp_options RENAME COLUMN option_name TO option_key schema rewrites for copied WordPress database views, triggers, and indexes without requiring ext/sqlite; dependent trigger/view/index SQL follows SQLite-style table and column-reference rewrites while object names and string literals are preserved.',
