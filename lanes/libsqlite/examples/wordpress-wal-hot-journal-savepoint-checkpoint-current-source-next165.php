...
use PortLibs\LibSqlite\SQLiteWalHotJournalSavepointCheckpointCurrentSourceNext165Plan;
...
$plan = SQLiteWalHotJournalSavepointCheckpointCurrentSourceNext165Plan::plan(
...
$summary = [
    'scenario' => 'wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next165',
    'status' => $plan['status'],
    ...
    'wordpressUse' => 'A copied wp_options import recovers a hot rollback journal, rolls back failed savepoint WAL frames, publishes the checkpoint from that current source, and only then resets the WAL generation for the next reader.',
...
if ($summary['status'] !== 'wal-hot-journal-savepoint-checkpoint-current-source-next165'
    ...
    fwrite(STDERR, "wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next165 self-test failed\n");
    exit(1);
}

echo "wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next165 self-test passed\n";
