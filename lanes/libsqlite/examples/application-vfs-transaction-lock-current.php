...
$summary = [
    'scenario' => 'application-vfs-transaction-lock-current',
    'applicationUse' => 'Coordinate copied Application SQLite import transactions with SQLite-style VFS lock transitions: deferred readers take shared locks on first access, BEGIN IMMEDIATE reserves the single writer slot, commit waits for readers before exclusive rollback-journal writes, and locks are released after commit or rollback.',
...
if (in_array('--self-test', $argv, true)) {
    ...
    echo "application-vfs-transaction-lock-current self-test passed\n";
    return;
}
