...
$root = sys_get_temp_dir() . '/port-libsqlite-wordpress-commit-current72-' . bin2hex(random_bytes(4));
...
if (!is_dir(dirname($localDatabase)) && !mkdir(dirname($localDatabase), 0777, true) && !is_dir(dirname($localDatabase))) {
    throw new RuntimeException('Unable to create WordPress savepoint commit-current fixture');
}
...
echo json_encode([
    'scenario' => 'wordpress-pager-savepoint-wal-commit-current-next72',
    'wordpressUse' => 'Copied wp_options import commits the retained WAL prefix after ROLLBACK TO plugin savepoint, discards the failed plugin frame, checkpoints retained pages, and resets the WAL sidecar through bounded native PHP VFS writes.',
