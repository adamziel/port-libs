...
$plan = SQLiteBTreeVacuumPointerMapFreeblockCurrentSourcePlan::tableLeafOverflowChainAuditFromDeleteResult(
    $database,
    3,
    [
        'page' => $deletedPage,
...
$summary = [
    'scenario' => 'application-btree-vacuum-pointermap-freeblock-current-source-next154',
    'application_use' => 'Delete a copied wp_options transient row, verify the current-source overflow next-pointer chain before freeblock/vacuum apply, and preserve only page 106 as the surviving freelist trunk after partial incremental vacuum.',
...
if (PHP_SAPI === 'cli' && in_array('--self-test', $argv, true)) {
    ...
    if (!$ok) {
        fwrite(STDERR, "application-btree-vacuum-pointermap-freeblock-current-source-next154 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "application-btree-vacuum-pointermap-freeblock-current-source-next154 self-test passed\n");
}
