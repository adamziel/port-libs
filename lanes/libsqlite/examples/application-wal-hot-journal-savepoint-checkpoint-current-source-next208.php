...
$summary = [
    'scenario' => 'application-wal-hot-journal-savepoint-checkpoint-current-source-next208',
    'applicationUse' => 'A copied Application plugin import reopens after hot-journal recovery and checkpoint publication, then reuses only reader slots tied to next206 current-source statement consumers.',
    'status' => $plan['status'],
...
if (PHP_SAPI === 'cli' && realpath($_SERVER['SCRIPT_FILENAME'] ?? '') === __FILE__) {
    if ($summary['status'] !== 'wal-hot-journal-savepoint-checkpoint-current-source-next208'
        ...
        fwrite(STDERR, "application-wal-hot-journal-savepoint-checkpoint-current-source-next208 self-test failed\n");
        exit(1);
    }

    echo "application-wal-hot-journal-savepoint-checkpoint-current-source-next208 self-test passed\n";
}
