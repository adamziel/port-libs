...
$summary = [
    'scenario' => 'application-pragma-index-xinfo-foreignkey-current-source-next229',
    'applicationUse' => 'Copied taxonomy option imports can reject a child FK that references only the left prefix of a wider UNIQUE parent index before enabling foreign_key_check.',
    'status' => $page['status'],
...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "application-pragma-index-xinfo-foreignkey-current-source-next229 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "application-pragma-index-xinfo-foreignkey-current-source-next229 self-test passed\n");
    exit(0);
}
