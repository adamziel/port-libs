...
$plan = SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNextPlan::compareNext191($sql, $currentTables, $nextTables);

if (($argv[1] ?? null) === '--self-test') {
    if (($plan['status'] ?? null) !== 'compound-select-window-recursive-limit-current-source-next191-ready') {
        fwrite(STDERR, "unexpected compound/window recursive LIMIT next191 status\n");
        exit(1);
    }
    if (array_column($plan['nextRows'], 'label') !== ['plugin_alpha', 'home', 'theme_mods', 'rewrite_rules', 'home', 'siteurl']) {
        fwrite(STDERR, "unexpected next compound/window recursive LIMIT next191 boundary\n");
        exit(1);
    }
    ...
    echo "wordpress-compound-select-window-recursive-limit-current-source-next191 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'wordpress-compound-select-window-recursive-limit-current-source-next191',
