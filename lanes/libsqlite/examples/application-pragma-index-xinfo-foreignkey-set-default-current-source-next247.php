...
$summary = [
    'scenario' => 'application-pragma-index-xinfo-foreignkey-set-default-current-source-next247',
    'applicationUse' => 'Copied Application option imports can reject SET DEFAULT foreign-key actions that would write NULL into NOT NULL child columns until explicit non-NULL defaults are present.',
    'status' => $page['status'],
...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "application-pragma-index-xinfo-foreignkey-set-default-current-source-next247 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "application-pragma-index-xinfo-foreignkey-set-default-current-source-next247 self-test passed\n");
    exit(0);
}
