...
if (($argv[1] ?? '') === '--self-test') {
    if (
        ...
        fwrite(STDERR, "wordpress-btree-freeblock-pointermap-vacuum-current-source-next93 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-btree-freeblock-pointermap-vacuum-current-source-next93 self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'copied wp_options freeblock coalesce followed by pointer-map boundary vacuum current source next93',
