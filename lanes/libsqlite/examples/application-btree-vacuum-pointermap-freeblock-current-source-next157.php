...
$plan = SQLiteBTreeVacuumPointerMapFreeblockCurrentSourcePlan::tableLeafTransitionFromDeleteResult(
    $database,
    3,
    [
        'page' => $deletedPage,
...
$summary = [
    'scenario' => 'application-btree-vacuum-pointermap-freeblock-current-source-next157',
    'application_use' => 'Delete a copied wp_options transient row, preserve the leaf freeblock, clear the surviving freelist trunk overflow-next pointer, and prove truncated current-source overflow next pointers are not materialized past the auto-vacuum boundary.',
...
if (PHP_SAPI === 'cli' && in_array('--self-test', $argv, true)) {
    ...
    if (!$ok) {
        fwrite(STDERR, "application-btree-vacuum-pointermap-freeblock-current-source-next157 self-test failed\n");
        exit(1);
    }
}
