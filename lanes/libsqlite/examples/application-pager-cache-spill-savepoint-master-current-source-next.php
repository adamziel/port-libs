...
$summary = [
    'scenario' => 'application-pager-cache-spill-savepoint-master-current-source-next141',
    'status' => $plan['status'],
    ...
    'applicationUse' => 'A copied Application SQLite options import recovers through the current master journal, rejects stale pager-cache spill candidates, and only spills savepoint-protected dirty pages whose source still matches the recovered current master-journal source.',
...
if (
    ...
    fwrite(STDERR, "application-pager-cache-spill-savepoint-master-current-source-next141 self-test failed\n");
    exit(1);
}

echo "application-pager-cache-spill-savepoint-master-current-source-next141 self-test passed\n";
