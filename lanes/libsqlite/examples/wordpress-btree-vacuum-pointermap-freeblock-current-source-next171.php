...
use PortLibs\LibSqlite\SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNext171Plan;
...
$plan = SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNext171Plan::tableLeafFromDeleteResult(
    $database,
    3,
    [
        'page' => $deletedPage,
    ...
    4,
    str_repeat('wordpress-next171-source-transition-', 55),
    3,
    true,
);

$summary = [
    'wordpress_scenario' => 'wp_options transient cleanup classifies stable leaf, reused overflow, and rejected truncated current-source pages',
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    if ($summary !== $expected) {
        fwrite(STDERR, "wordpress-btree-vacuum-pointermap-freeblock-current-source-next171 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-btree-vacuum-pointermap-freeblock-current-source-next171 self-test passed\n");
    exit(0);
}
