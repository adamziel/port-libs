...
$summary = [
    'scenario' => 'application-pragma-index-xinfo-foreignkey-current-source-next182',
    'applicationUse' => 'Copied wp_options imports can block PRAGMA foreign_key_check repair pages when a parent UNIQUE index has the right columns but the wrong parent-column collation.',
    'status' => $page['status'],
...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "application-pragma-index-xinfo-foreignkey-current-source-next182 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "application-pragma-index-xinfo-foreignkey-current-source-next182 self-test passed\n");
    exit(0);
}
