...
$summary = [
    'scenario' => 'wordpress-rowvalue-update-returning-conflict-current-source-next137',
    'wordpressUse' => 'Model copied wp_options cleanup where an UPDATE OR REPLACE row-value assignment deletes a later selected transient row before that row can emit RETURNING, while independent selected rows still yield current-source RETURNING images.',
    'status' => $plan['status'],
