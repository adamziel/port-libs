...
$plan = SQLiteRowValueUpdateDeleteReturningSavepointCurrentSourceNext187Plan::execute(
    ['wp_options' => $rows],
    [
...
$summary = [
    'scenario' => 'wordpress-rowvalue-abort-savepoint-current-source-next187',
    'wordpressUse' => 'Copied wp_options cleanup can keep outer rewrite-rule work while an OR ABORT row-value savepoint batch is rolled back, discards attempted RETURNING rows, and retries from the savepoint image.',
    'status' => $plan['status'],
...
if (
    ...
    fwrite(STDERR, "wordpress-rowvalue-abort-savepoint-current-source-next187 self-test failed\n");
    exit(1);
}

if (PHP_SAPI === 'cli' && realpath($_SERVER['SCRIPT_FILENAME'] ?? '') === __FILE__) {
    echo "wordpress-rowvalue-abort-savepoint-current-source-next187 self-test passed\n";
