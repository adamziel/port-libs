...
$receipt = 'master-journal-recovery-receipt:epoch=269:members=main,users:cleanup=deleted:dirsync=ok';
$spill = 'pager-spill-drain:epoch=269:dirty-pages=2:journal-sync=ok:exclusive-lock=held';
$rollbackSource = 'pager-rollback-source:epoch=269:dirty-pages=2:journal-sync=ok:exclusive-lock=held';
$oldSpill = 'pager-rollback-source:epoch=265:dirty-pages=2:journal-sync=pending';
$snapshot = 'reader-snapshot:epoch=269:source=master-current:lease=shared:pages=1,2,3';
$generation = 'pager-reader-cache-generation:epoch=269:master-current:reset=complete';
$source = 'current-source:master-journal:epoch=269:members=2:schema=117';
$base = [
    'source_id' => 'wordpress-pager-rollback-source-smoke',
    'epoch' => 269,
    'format_signature' => hash('sha256', implode('|', [512, 4, 2, 269, 0])),
    'publication_generation' => 269,
    'master_source_digest' => hash('sha256', 'wordpress next269 master source'),
    ...
    'master_journal_file_token' => 'dev=8:ino=2690:size=96:mtime=26900:generation=master-current',
    'master_journal_bytes_digest' => hash('sha256', $masterBytes),
    'database_file_token' => 'dev=8:ino=2699:size=1536:mtime=26999:generation=database-current',
    'master_journal_cleanup_token' => 'master-cleanup:deleted:mtime=26700:dirsync=ok',
    'reader_lease_token' => 'reader-lease:shared-cache:epoch=269:opened-after-master-cleanup',
    'pager_cache_source_token' => 'pager-cache-source:epoch=269:master-journal-recovery=complete',
    'read_transaction_token' => 'read-transaction:epoch=269:schema=117:change-counter=269:master-current',
    'schema_reparse_token' => 'schema-reparse:epoch=269:schema-cookie=117:ddl=master-current',
    'statement_schema_root_token' => 'statement-schema-root:epoch=269:root=1:cookie=117:sql=wp-options-current',
...
$plan = SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::variantNext269(
    $database,
    $master,
    $masterBytes,
    implode('', $before),
...
if (in_array('--self-test', $argv, true)) {
    if ($plan['status'] !== 'pager-master-journal-reader-cache-current-source-next269') {
    ...
    echo "wordpress-pager-master-journal-reader-cache-current-source-next269 self-test passed\n";
    return;
}
