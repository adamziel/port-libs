...
$base = [
    'source_id' => 'wordpress-pager-reader-cache-next590',
    'epoch' => 590,
    'format_signature' => hash('sha256', implode('|', [512, 4, 2, 590, 0])),
    'publication_generation' => 590,
    'master_source_digest' => hash('sha256', 'wordpress next590 master source'),
...
$cache = [1 => array_merge($base, ['reader_id' => 'wp-options-reader', 'image' => $recovered[1], 'reader_cache_stmt_vdbe_transaction_branch_token' => 'stmt-vdbe-transaction-branch-old'])];
$read = array_merge($base, [
    'reader_id' => 'read-options',
    'page_number' => 1,
    'member_journal_token_digest' => $mapDigest($tokens),
    'member_journal_header_digest' => $mapDigest($headers),
    'reader_cache_stmt_vdbe_transaction_branch_token' => 'stmt-vdbe-transaction-branch-old',
]);
$plan = SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::currentSourceVdbeStatementBranchFence(
    $database,
    $master,
    $masterBytes,
    implode('', $before),
...
echo json_encode([
    ...
    'next590_invalidated_pages' => $plan['reader_cache_stmt_vdbe_transaction_branch_invalidated_cache_page_numbers'],
