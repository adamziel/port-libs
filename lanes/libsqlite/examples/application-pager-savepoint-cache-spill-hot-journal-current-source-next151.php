...
if (($argv[1] ?? '') === '--self-test') {
    if (
        ...
        fwrite(STDERR, "application-pager-savepoint-cache-spill-hot-journal-current-source-next151 self-test failed\n");
        exit(1);
    }
    echo "application-pager-savepoint-cache-spill-hot-journal-current-source-next151 self-test passed\n";
    exit(0);
}
...
echo json_encode([
    ...
    'applicationUse' => 'A copied wp_options import can recover a hot rollback journal before cache spill, then spill only dirty savepoint pages whose before-image was recaptured from the recovered current source.',
