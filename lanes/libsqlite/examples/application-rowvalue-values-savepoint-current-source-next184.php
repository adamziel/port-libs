...
$summary = [
    'scenario' => 'application-rowvalue-values-savepoint-current-source-next184',
    'applicationUse' => 'Copied wp_options cleanup can use SQLite row-value IN (VALUES ...) predicates for transient cleanup and retry updates; OR ROLLBACK discards attempted RETURNING rows and retries from the restored transaction image.',
    'status' => $plan['status'],
...
if (
    ...
    fwrite(STDERR, "application-rowvalue-values-savepoint-current-source-next184 self-test failed\n");
    exit(1);
}

if (PHP_SAPI === 'cli' && realpath($_SERVER['SCRIPT_FILENAME'] ?? '') === __FILE__) {
    echo "application-rowvalue-values-savepoint-current-source-next184 self-test passed\n";
