            ...
            'main' => [
                ...
                'reuse_acks' => [
                    [
                        ...
                        'data_version' => 18,
                        'published_count' => 3,
                        'receipt_digest' => hash('sha256', 'publish-next217|shared-cache-next229|shared-cache-next237'),
                    ],
                ],
            ],
        ],
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    assert($plan['events'][0]['claim'] === 'reader-reuse-next245');
    assert($plan['events'][0]['blocked_reasons'] === []);
    assert($plan['events'][1]['status'] === 'published-reused-current-source');
    assert($plan['events'][1]['reuse_ack'] === 'shared-cache-next237');
    assert($plan['events'][1]['published_count'] === 4);
    ...
    echo "application-vfs-current-source-next242-245 self-test passed\n";
    return;
}
