...
$page = SQLitePragmaIndexXinfoForeignKeyCurrentSourceNext175::currentNextPageFromCatalog(
    $records,
    $currentTables,
    $records,
    $nextTables,
...
if (($argv[1] ?? null) === '--self-test') {
    if ($page['status'] !== 'ok' || $page['current']['foreign_key_violations'] !== 3 || $page['next_counts']['foreign_key_violations'] !== 0 || $page['current']['foreign_key_list_rows'] !== 3 || $foreignKeyListRows[1]['from'] !== 'option_name' || $foreignKeyListRows[2]['seq'] !== 1) {
        fwrite(STDERR, "wordpress-pragma-index-xinfo-foreignkey-current-source-next175 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-index-xinfo-foreignkey-current-source-next175 self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'wordpress-pragma-index-xinfo-foreignkey-current-source-next175',
    'wordpressUse' => 'Preserve PRAGMA foreign_key_list id/seq column order while admitting copied multisite wp_options rows after parent tables are repaired.',
    'status' => $page['status'],
