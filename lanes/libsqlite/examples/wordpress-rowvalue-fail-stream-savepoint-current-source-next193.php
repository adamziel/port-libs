...
require_once __DIR__ . '/../src/SQLiteRowValueUpdateDeleteReturningSavepointCurrentSourceNext193Plan.php';

use PortLibs\LibSqlite\SQLiteRowValueUpdateDeleteReturningSavepointCurrentSourceNext193Plan;
...
$plan = SQLiteRowValueUpdateDeleteReturningSavepointCurrentSourceNext193Plan::execute(
    ['wp_options' => $rows],
    [
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "wordpress-rowvalue-fail-stream-savepoint-current-source-next193 self-test passed\n";
    return;
}

echo json_encode([
    'wordpressUse' => 'Model copied wp_options import cleanup where UPDATE OR FAIL yields an early RETURNING row, a later row-value unique conflict triggers ROLLBACK TO, and retry DELETE/UPDATE statements read the restored savepoint source.',
    'status' => $plan['status'],
