...
$summary = [
    'scenario' => 'wordpress-btree-overflow-pointermap-freelist-current-source-next125',
    'wordpress_use' => 'Rewrite a large wp_options transient value so obsolete overflow pages first become freelist leaves, then are immediately reused as the next overflow chain with pointer-map parentage restored.',
...
if (PHP_SAPI === 'cli' && in_array('--self-test', $argv, true)) {
    ...
    if (!$ok) {
        fwrite(STDERR, "wordpress-btree-overflow-pointermap-freelist-current-source-next125 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-btree-overflow-pointermap-freelist-current-source-next125 self-test passed\n");
    exit(0);
}
