...
$summary = [
    'scenario' => 'wordpress-trigger-deferred-view-returning-current-source-next131',
    'wordpressUse' => 'A copied wp_options INSTEAD OF view-trigger import can drain current-source RETURNING rows, materialize the autoloaded-options view, then reject the next source when deferred parent-option references fail.',
    'status' => $plan['status'],
...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "wordpress-trigger-deferred-view-returning-current-source-next131 self-test failed\n");
        exit(1);
    }

    echo "wordpress-trigger-deferred-view-returning-current-source-next131 self-test passed\n";
    exit(0);
}
