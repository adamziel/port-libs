...
$output = [
    'scenario' => 'wordpress-planner-expression-partial-covering-current-source-next100',
    ...
    'wordpressUse' => 'Preview copied wp_options plugin scans where schema/stat4 counters are stable but the current partial covering index root/stat4 payload changed, forcing native PHP plans to reprepare from current source instead of reusing stale prepared coverage.',
];
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "wordpress-planner-expression-partial-covering-current-source-next100 self-test passed\n";
    exit(0);
}
