...
$summary = [
    'scenario' => 'application-pager-master-journal-reader-cache-current-source-next233',
    'applicationUse' => 'A copied Application database keeps schema/options reader-cache pages only when their read-transaction ticket was opened against the recovered master-journal source; stale active_plugins reads reopen before plugin import resumes.',
    'status' => $plan['status'],
...
if ($summary['status'] !== 'pager-master-journal-reader-cache-current-source-next233'
    ...
    fwrite(STDERR, "application-pager-master-journal-reader-cache-current-source-next233 self-test failed\n");
    exit(1);
}

echo json_encode($summary, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) . "\n";
echo "application-pager-master-journal-reader-cache-current-source-next233 self-test passed\n";
