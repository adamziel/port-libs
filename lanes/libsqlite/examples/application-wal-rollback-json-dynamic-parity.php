...
$withScenarios(
    static fn (): array => SQLiteJsonImportRollbackWalPlan::dynamicRollbackDisabledReopenedPrefixCheckpointFollowupScenarios(4),
    static function (array $scenarios) use (&$summary): void {
        $summary['rollbackDisabledReopenedPrefixCheckpointFollowupScenarioCount'] = count($scenarios);
        $summary['rollbackDisabledReopenedPrefixCheckpointFollowupModes'] = array_map(static fn (array $scenario): string => $scenario['checkpoint_mode'], $scenarios);
        $summary['rollbackDisabledReopenedPrefixCheckpointFollowupStartedNewWalHeaders'] = array_map(static fn (array $scenario): bool => $scenario['reopened_checkpoint_followup_started_new_wal_header'], $scenarios);
        $summary['rollbackDisabledReopenedPrefixCheckpointFollowupWalFramesBefore'] = array_map(static fn (array $scenario): int => $scenario['reopened_checkpoint_followup_plan']['wal_frame_count_before'], $scenarios);
        $summary['rollbackDisabledReopenedPrefixCheckpointFollowupWalFramesAfter'] = array_map(static fn (array $scenario): int => $scenario['reopened_checkpoint_followup_plan']['wal_frame_count_after'], $scenarios);
        $summary['rollbackDisabledReopenedPrefixCheckpointFollowupPages'] = array_map(static fn (array $scenario): array => array_column($scenario['reopened_checkpoint_followup_plan']['import_plan']['applied'], 'page_number'), $scenarios);
        $summary['rollbackDisabledReopenedPrefixCheckpointFollowupInsertedKeys'] = array_map(static fn (array $scenario): string => $scenario['reopened_checkpoint_followup_plan']['import_plan']['applied'][1]['key_name'], $scenarios);
        $summary['rollbackDisabledReopenedPrefixCheckpointFollowupReopenedKeysRetained'] = array_map(static fn (array $scenario): bool => $scenario['reopened_checkpoint_followup_reopened_key_retained'], $scenarios);
        $summary['rollbackDisabledReopenedPrefixCheckpointFollowupPreviousRecoveryKeysRetained'] = array_map(static fn (array $scenario): bool => $scenario['reopened_checkpoint_followup_previous_recovery_key_retained'], $scenarios);
        $summary['rollbackDisabledReopenedPrefixCheckpointFollowupRejectedTailKeysRetained'] = array_map(
            static fn (array $scenario): bool => $scenario['reopened_checkpoint_followup_rejected_prior_tail_key_retained'] || $scenario['reopened_checkpoint_followup_rejected_post_recovery_tail_key_retained'],
            $scenarios
        );
    }
);
...
$withScenarios(
    static fn (): array => SQLiteJsonImportRollbackWalPlan::dynamicPostCheckpointTailFailureScenarios(4),
    static function (array $scenarios) use (&$summary): void {
        $summary['postCheckpointTailFailureScenarioCount'] = count($scenarios);
        $summary['postCheckpointTailFailureModes'] = array_map(static fn (array $scenario): string => $scenario['checkpoint_mode'], $scenarios);
        $summary['postCheckpointTailFailureWalFramesBefore'] = array_map(static fn (array $scenario): int => $scenario['post_checkpoint_tail_failure_plan']['wal_frame_count_before'], $scenarios);
        $summary['postCheckpointTailFailureWalFramesAfter'] = array_map(static fn (array $scenario): int => $scenario['post_checkpoint_tail_failure_plan']['wal_frame_count_after'], $scenarios);
        $summary['postCheckpointTailFailurePages'] = array_map(static fn (array $scenario): array => $scenario['post_checkpoint_tail_failure_plan']['rollback_to_savepoint']['restored_page_numbers'], $scenarios);
        $summary['postCheckpointTailFailureFailedStatements'] = array_map(static fn (array $scenario): array => $scenario['post_checkpoint_tail_failure_plan']['failed_statements'], $scenarios);
        $summary['postCheckpointTailFailureInsertedKeys'] = array_map(static fn (array $scenario): string => $scenario['post_checkpoint_tail_failure_plan']['import_plan']['applied'][1]['key_name'], $scenarios);
    }
);

$withScenarios(
    static fn (): array => SQLiteJsonImportRollbackWalPlan::dynamicPostCheckpointTailRecoveryScenarios(4),
    static function (array $scenarios) use (&$summary): void {
        $summary['postCheckpointTailRecoveryScenarioCount'] = count($scenarios);
        $summary['postCheckpointTailRecoveryModes'] = array_map(static fn (array $scenario): string => $scenario['checkpoint_mode'], $scenarios);
        $summary['postCheckpointTailRecoveryWalFramesBefore'] = array_map(static fn (array $scenario): int => $scenario['post_checkpoint_tail_recovery_plan']['wal_frame_count_before'], $scenarios);
        $summary['postCheckpointTailRecoveryWalFramesAfter'] = array_map(static fn (array $scenario): int => $scenario['post_checkpoint_tail_recovery_plan']['wal_frame_count_after'], $scenarios);
        $summary['postCheckpointTailRecoveryPages'] = array_map(static fn (array $scenario): array => array_column($scenario['post_checkpoint_tail_recovery_plan']['import_plan']['applied'], 'page_number'), $scenarios);
        $summary['postCheckpointTailRecoveryInsertedKeys'] = array_map(static fn (array $scenario): string => $scenario['post_checkpoint_tail_recovery_plan']['import_plan']['applied'][1]['key_name'], $scenarios);
        $summary['postCheckpointTailRecoveryRejectedTailKeysRetained'] = array_map(static fn (array $scenario): bool => $scenario['post_checkpoint_tail_inserted_key_retained_after_recovery'], $scenarios);
        $summary['postCheckpointTailRecoveryFollowupKeysRetained'] = array_map(static fn (array $scenario): bool => $scenario['post_checkpoint_followup_inserted_key_retained_after_recovery'], $scenarios);
    }
);

$withScenarios(
    static fn (): array => SQLiteJsonImportRollbackWalPlan::dynamicPostCheckpointTailRecoveryCheckpointScenarios(4),
    static function (array $scenarios) use (&$summary): void {
        $summary['postCheckpointTailRecoveryCheckpointScenarioCount'] = count($scenarios);
        $summary['postCheckpointTailRecoveryCheckpointModes'] = array_map(static fn (array $scenario): string => $scenario['checkpoint_mode'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointActions'] = array_map(static fn (array $scenario): string => $scenario['tail_recovery_released_checkpoint']['wal_action'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointReleasedWalBytes'] = array_map(static fn (array $scenario): int => $scenario['tail_recovery_released_checkpoint']['wal_bytes_length'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointPinnedBusy'] = array_map(static fn (array $scenario): bool => $scenario['tail_recovery_pinned_checkpoint']['busy'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointAppliedPages'] = array_map(static fn (array $scenario): array => $scenario['tail_recovery_applied_page_numbers'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointPagesMaterialized'] = array_map(static fn (array $scenario): bool => $scenario['tail_recovery_checkpointed_pages_match'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupFramePinned'] = array_map(
            static fn (array $scenario): bool => !$scenario['tail_recovery_pinned_followup_page_matches_final_recovery'],
            $scenarios
        );
        $summary['postCheckpointTailRecoveryCheckpointRejectedTailKeysRetained'] = array_map(static fn (array $scenario): bool => $scenario['tail_recovery_rejected_tail_key_retained_after_checkpoint'], $scenarios);
    }
);

$withScenarios(
    static fn (): array => SQLiteJsonImportRollbackWalPlan::dynamicPostCheckpointTailRecoveryCheckpointFollowupScenarios(4),
    static function (array $scenarios) use (&$summary): void {
        $summary['postCheckpointTailRecoveryCheckpointFollowupScenarioCount'] = count($scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupModes'] = array_map(static fn (array $scenario): string => $scenario['checkpoint_mode'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupStartedNewWalHeaders'] = array_map(static fn (array $scenario): bool => $scenario['tail_recovery_checkpoint_followup_started_new_wal_header'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupWalFramesBefore'] = array_map(static fn (array $scenario): int => $scenario['tail_recovery_checkpoint_followup_plan']['wal_frame_count_before'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupWalFramesAfter'] = array_map(static fn (array $scenario): int => $scenario['tail_recovery_checkpoint_followup_plan']['wal_frame_count_after'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupPages'] = array_map(static fn (array $scenario): array => array_column($scenario['tail_recovery_checkpoint_followup_plan']['import_plan']['applied'], 'page_number'), $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupInsertedKeys'] = array_map(static fn (array $scenario): string => $scenario['tail_recovery_checkpoint_followup_plan']['import_plan']['applied'][1]['key_name'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryKeysRetained'] = array_map(static fn (array $scenario): bool => $scenario['tail_recovery_checkpoint_followup_recovery_key_retained'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRejectedTailKeysRetained'] = array_map(static fn (array $scenario): bool => $scenario['tail_recovery_checkpoint_followup_rejected_tail_key_retained'], $scenarios);
    }
);

$withScenarios(
    static fn (): array => SQLiteJsonImportRollbackWalPlan::dynamicPostCheckpointTailRecoveryCheckpointFollowupFailureScenarios(4),
    static function (array $scenarios) use (&$summary): void {
        $summary['postCheckpointTailRecoveryCheckpointFollowupFailureScenarioCount'] = count($scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupFailureModes'] = array_map(static fn (array $scenario): string => $scenario['checkpoint_mode'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupFailureWalFramesBefore'] = array_map(static fn (array $scenario): int => $scenario['tail_recovery_checkpoint_followup_tail_plan']['wal_frame_count_before'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupFailureWalFramesAfter'] = array_map(static fn (array $scenario): int => $scenario['tail_recovery_checkpoint_followup_tail_plan']['wal_frame_count_after'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupFailurePages'] = array_map(static fn (array $scenario): array => $scenario['tail_recovery_checkpoint_followup_tail_plan']['rollback_to_savepoint']['restored_page_numbers'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupFailureFailedStatements'] = array_map(static fn (array $scenario): array => $scenario['tail_recovery_checkpoint_followup_tail_plan']['failed_statements'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupFailureInsertedKeys'] = array_map(static fn (array $scenario): string => $scenario['tail_recovery_checkpoint_followup_tail_plan']['import_plan']['applied'][1]['key_name'], $scenarios);
    }
);

$withScenarios(
    static fn (): array => SQLiteJsonImportRollbackWalPlan::dynamicPostCheckpointTailRecoveryCheckpointFollowupRecoveryScenarios(4),
    static function (array $scenarios) use (&$summary): void {
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryScenarioCount'] = count($scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryModes'] = array_map(static fn (array $scenario): string => $scenario['checkpoint_mode'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryWalFramesBefore'] = array_map(static fn (array $scenario): int => $scenario['tail_recovery_checkpoint_followup_recovery_plan']['wal_frame_count_before'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryWalFramesAfter'] = array_map(static fn (array $scenario): int => $scenario['tail_recovery_checkpoint_followup_recovery_plan']['wal_frame_count_after'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryPages'] = array_map(static fn (array $scenario): array => array_column($scenario['tail_recovery_checkpoint_followup_recovery_plan']['import_plan']['applied'], 'page_number'), $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryInsertedKeys'] = array_map(static fn (array $scenario): string => $scenario['tail_recovery_checkpoint_followup_recovery_plan']['import_plan']['applied'][1]['key_name'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryTailKeysRetained'] = array_map(static fn (array $scenario): bool => $scenario['tail_recovery_checkpoint_followup_tail_inserted_key_retained_after_recovery'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryPriorKeysRetained'] = array_map(static fn (array $scenario): bool => $scenario['tail_recovery_checkpoint_followup_prior_inserted_key_retained_after_recovery'], $scenarios);
    }
);

$withScenarios(
    static fn (): array => SQLiteJsonImportRollbackWalPlan::dynamicPostCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointScenarios(4),
    static function (array $scenarios) use (&$summary): void {
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointScenarioCount'] = count($scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointModes'] = array_map(static fn (array $scenario): string => $scenario['followup_recovery_checkpoint_mode'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointActions'] = array_map(static fn (array $scenario): string => $scenario['tail_recovery_checkpoint_followup_recovery_released_checkpoint']['wal_action'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointReleasedWalBytes'] = array_map(static fn (array $scenario): int => $scenario['tail_recovery_checkpoint_followup_recovery_released_checkpoint']['wal_bytes_length'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointPinnedBusy'] = array_map(static fn (array $scenario): bool => $scenario['tail_recovery_checkpoint_followup_recovery_pinned_checkpoint']['busy'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointAppliedPages'] = array_map(static fn (array $scenario): array => $scenario['followup_recovery_checkpoint_applied_page_numbers'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointPagesMaterialized'] = array_map(static fn (array $scenario): bool => $scenario['followup_recovery_checkpointed_pages_match'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointPriorFramePinned'] = array_map(
            static fn (array $scenario): bool => !$scenario['followup_recovery_pinned_prior_followup_page_matches_final_recovery'],
            $scenarios
        );
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointRejectedTailKeysRetained'] = array_map(static fn (array $scenario): bool => $scenario['followup_recovery_failed_tail_key_retained_after_checkpoint'], $scenarios);
    }
);

$withScenarios(
    static fn (): array => SQLiteJsonImportRollbackWalPlan::dynamicPostCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupScenarios(4),
    static function (array $scenarios) use (&$summary): void {
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupScenarioCount'] = count($scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupModes'] = array_map(static fn (array $scenario): string => $scenario['followup_recovery_checkpoint_mode'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupStartedNewWalHeaders'] = array_map(static fn (array $scenario): bool => $scenario['followup_recovery_checkpoint_followup_started_new_wal_header'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupWalFramesBefore'] = array_map(static fn (array $scenario): int => $scenario['tail_recovery_checkpoint_followup_recovery_checkpoint_followup_plan']['wal_frame_count_before'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupWalFramesAfter'] = array_map(static fn (array $scenario): int => $scenario['tail_recovery_checkpoint_followup_recovery_checkpoint_followup_plan']['wal_frame_count_after'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupPages'] = array_map(static fn (array $scenario): array => array_column($scenario['tail_recovery_checkpoint_followup_recovery_checkpoint_followup_plan']['import_plan']['applied'], 'page_number'), $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupInsertedKeys'] = array_map(static fn (array $scenario): string => $scenario['tail_recovery_checkpoint_followup_recovery_checkpoint_followup_plan']['import_plan']['applied'][1]['key_name'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupRecoveryKeysRetained'] = array_map(static fn (array $scenario): bool => $scenario['followup_recovery_checkpoint_followup_recovery_key_retained'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupPriorKeysRetained'] = array_map(static fn (array $scenario): bool => $scenario['followup_recovery_checkpoint_followup_prior_followup_key_retained'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupRejectedTailKeysRetained'] = array_map(static fn (array $scenario): bool => $scenario['followup_recovery_checkpoint_followup_failed_tail_key_retained'], $scenarios);
    }
);

$withScenarios(
    static fn (): array => SQLiteJsonImportRollbackWalPlan::dynamicPostCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailFailureScenarios(4),
    static function (array $scenarios) use (&$summary): void {
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailFailureScenarioCount'] = count($scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailFailureModes'] = array_map(static fn (array $scenario): string => $scenario['followup_recovery_checkpoint_mode'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailFailureWalFramesBefore'] = array_map(static fn (array $scenario): int => $scenario['tail_recovery_checkpoint_followup_recovery_checkpoint_followup_tail_plan']['wal_frame_count_before'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailFailureWalFramesAfter'] = array_map(static fn (array $scenario): int => $scenario['tail_recovery_checkpoint_followup_recovery_checkpoint_followup_tail_plan']['wal_frame_count_after'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailFailurePages'] = array_map(static fn (array $scenario): array => $scenario['tail_recovery_checkpoint_followup_recovery_checkpoint_followup_tail_plan']['rollback_to_savepoint']['restored_page_numbers'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailFailureFailedStatements'] = array_map(static fn (array $scenario): array => $scenario['tail_recovery_checkpoint_followup_recovery_checkpoint_followup_tail_plan']['failed_statements'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailFailureInsertedKeys'] = array_map(static fn (array $scenario): string => $scenario['tail_recovery_checkpoint_followup_recovery_checkpoint_followup_tail_plan']['import_plan']['applied'][1]['key_name'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailFailureFinalPrefixRestored'] = array_map(static fn (array $scenario): bool => $scenario['followup_recovery_checkpoint_followup_tail_wal_truncated_to_final_followup_prefix'], $scenarios);
    }
);

$withScenarios(
    static fn (): array => SQLiteJsonImportRollbackWalPlan::dynamicPostCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryScenarios(4),
    static function (array $scenarios) use (&$summary): void {
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryScenarioCount'] = count($scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryModes'] = array_map(static fn (array $scenario): string => $scenario['followup_recovery_checkpoint_mode'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryWalFramesBefore'] = array_map(static fn (array $scenario): int => $scenario['tail_recovery_checkpoint_followup_recovery_checkpoint_followup_tail_recovery_plan']['wal_frame_count_before'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryWalFramesAfter'] = array_map(static fn (array $scenario): int => $scenario['tail_recovery_checkpoint_followup_recovery_checkpoint_followup_tail_recovery_plan']['wal_frame_count_after'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryPages'] = array_map(static fn (array $scenario): array => array_column($scenario['tail_recovery_checkpoint_followup_recovery_checkpoint_followup_tail_recovery_plan']['import_plan']['applied'], 'page_number'), $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryInsertedKeys'] = array_map(static fn (array $scenario): string => $scenario['tail_recovery_checkpoint_followup_recovery_checkpoint_followup_tail_recovery_plan']['import_plan']['applied'][1]['key_name'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryTailKeysRetained'] = array_map(static fn (array $scenario): bool => $scenario['followup_recovery_checkpoint_followup_tail_recovery_failed_tail_key_retained'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryFinalKeysRetained'] = array_map(static fn (array $scenario): bool => $scenario['followup_recovery_checkpoint_followup_tail_recovery_final_followup_key_retained'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryPriorKeysRetained'] = array_map(static fn (array $scenario): bool => $scenario['followup_recovery_checkpoint_followup_tail_recovery_prior_recovery_key_retained'], $scenarios);
    }
);
...
$withScenarios(
    static fn (): array => SQLiteJsonImportRollbackWalPlan::dynamicPostCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryCheckpointFollowupScenarios(4),
    static function (array $scenarios) use (&$summary): void {
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryCheckpointFollowupScenarioCount'] = count($scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryCheckpointFollowupModes'] = array_map(static fn (array $scenario): string => $scenario['followup_recovery_checkpoint_followup_tail_recovery_checkpoint_mode'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryCheckpointFollowupStartedNewWalHeaders'] = array_map(static fn (array $scenario): bool => $scenario['followup_recovery_checkpoint_followup_tail_recovery_checkpoint_followup_started_new_wal_header'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryCheckpointFollowupWalFramesBefore'] = array_map(static fn (array $scenario): int => $scenario['tail_recovery_checkpoint_followup_recovery_checkpoint_followup_tail_recovery_checkpoint_followup_plan']['wal_frame_count_before'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryCheckpointFollowupWalFramesAfter'] = array_map(static fn (array $scenario): int => $scenario['tail_recovery_checkpoint_followup_recovery_checkpoint_followup_tail_recovery_checkpoint_followup_plan']['wal_frame_count_after'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryCheckpointFollowupPages'] = array_map(static fn (array $scenario): array => array_column($scenario['tail_recovery_checkpoint_followup_recovery_checkpoint_followup_tail_recovery_checkpoint_followup_plan']['import_plan']['applied'], 'page_number'), $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryCheckpointFollowupInsertedKeys'] = array_map(static fn (array $scenario): string => $scenario['tail_recovery_checkpoint_followup_recovery_checkpoint_followup_tail_recovery_checkpoint_followup_plan']['import_plan']['applied'][1]['key_name'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryCheckpointFollowupTailRecoveryKeysRetained'] = array_map(static fn (array $scenario): bool => $scenario['followup_recovery_checkpoint_followup_tail_recovery_checkpoint_followup_tail_recovery_key_retained'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryCheckpointFollowupFinalKeysRetained'] = array_map(static fn (array $scenario): bool => $scenario['followup_recovery_checkpoint_followup_tail_recovery_checkpoint_followup_final_followup_key_retained'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryCheckpointFollowupRejectedTailKeysRetained'] = array_map(static fn (array $scenario): bool => $scenario['followup_recovery_checkpoint_followup_tail_recovery_checkpoint_followup_failed_tail_key_retained'], $scenarios);
    }
);

$withScenarios(
    static fn (): array => SQLiteJsonImportRollbackWalPlan::dynamicPostCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryCheckpointFollowupTailFailureScenarios(4),
    static function (array $scenarios) use (&$summary): void {
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryCheckpointFollowupTailFailureScenarioCount'] = count($scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryCheckpointFollowupTailFailureModes'] = array_map(static fn (array $scenario): string => $scenario['followup_recovery_checkpoint_followup_tail_recovery_checkpoint_mode'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryCheckpointFollowupTailFailureWalFramesBefore'] = array_map(static fn (array $scenario): int => $scenario['tail_recovery_checkpoint_followup_recovery_checkpoint_followup_tail_recovery_checkpoint_followup_tail_plan']['wal_frame_count_before'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryCheckpointFollowupTailFailureWalFramesAfter'] = array_map(static fn (array $scenario): int => $scenario['tail_recovery_checkpoint_followup_recovery_checkpoint_followup_tail_recovery_checkpoint_followup_tail_plan']['wal_frame_count_after'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryCheckpointFollowupTailFailurePages'] = array_map(static fn (array $scenario): array => $scenario['tail_recovery_checkpoint_followup_recovery_checkpoint_followup_tail_recovery_checkpoint_followup_tail_plan']['rollback_to_savepoint']['restored_page_numbers'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryCheckpointFollowupTailFailureFailedStatements'] = array_map(static fn (array $scenario): array => $scenario['tail_recovery_checkpoint_followup_recovery_checkpoint_followup_tail_recovery_checkpoint_followup_tail_plan']['failed_statements'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryCheckpointFollowupTailFailureInsertedKeys'] = array_map(static fn (array $scenario): string => $scenario['tail_recovery_checkpoint_followup_recovery_checkpoint_followup_tail_recovery_checkpoint_followup_tail_plan']['import_plan']['applied'][1]['key_name'], $scenarios);
        $summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryCheckpointFollowupTailFailureFinalPrefixRestored'] = array_map(static fn (array $scenario): bool => $scenario['followup_recovery_checkpoint_followup_tail_recovery_checkpoint_followup_tail_wal_truncated_to_final_followup_prefix'], $scenarios);
    }
);
...
if (in_array('--self-test', $argv, true)) {
    ...
    assert($summary['rollbackDisabledReopenedPrefixCheckpointFollowupScenarioCount'] === 4);
    ...
    assert($summary['postCheckpointTailFailureScenarioCount'] === 4);
    assert($summary['postCheckpointTailRecoveryScenarioCount'] === 4);
    assert($summary['postCheckpointTailRecoveryCheckpointScenarioCount'] === 4);
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupScenarioCount'] === 4);
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupFailureScenarioCount'] === 4);
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryScenarioCount'] === 4);
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointScenarioCount'] === 4);
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupScenarioCount'] === 4);
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryScenarioCount'] === 4);
    ...
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryCheckpointFollowupScenarioCount'] === 4);
    ...
    assert($summary['rollbackDisabledReopenedPrefixCheckpointFollowupModes'] === ['restart', 'truncate', 'restart', 'truncate']);
    assert($summary['rollbackDisabledReopenedPrefixCheckpointFollowupStartedNewWalHeaders'] === [false, true, false, true]);
    assert($summary['rollbackDisabledReopenedPrefixCheckpointFollowupWalFramesBefore'] === array_fill(0, 4, 0));
    assert($summary['rollbackDisabledReopenedPrefixCheckpointFollowupWalFramesAfter'] === array_fill(0, 4, 3));
    assert($summary['rollbackDisabledReopenedPrefixCheckpointFollowupPages'][0] === [1321, 2521, 2121]);
    assert($summary['rollbackDisabledReopenedPrefixCheckpointFollowupInsertedKeys'][0] === 'disabled_reopened_checkpoint_followup_payload_1');
    assert($summary['rollbackDisabledReopenedPrefixCheckpointFollowupReopenedKeysRetained'] === array_fill(0, 4, true));
    assert($summary['rollbackDisabledReopenedPrefixCheckpointFollowupPreviousRecoveryKeysRetained'] === array_fill(0, 4, true));
    assert($summary['rollbackDisabledReopenedPrefixCheckpointFollowupRejectedTailKeysRetained'] === array_fill(0, 4, false));
    ...
    assert($summary['postCheckpointTailFailureModes'] === ['restart', 'truncate', 'restart', 'truncate']);
    assert($summary['postCheckpointTailFailureWalFramesBefore'] === array_fill(0, 4, 5));
    assert($summary['postCheckpointTailFailureWalFramesAfter'] === array_fill(0, 4, 2));
    assert($summary['postCheckpointTailFailurePages'][0] === [1321, 1931]);
    assert($summary['postCheckpointTailFailureFailedStatements'][0] === ['post_checkpoint_tail_broken_payload_1']);
    assert($summary['postCheckpointTailFailureInsertedKeys'][0] === 'post_checkpoint_tail_payload_1');
    assert($summary['postCheckpointTailRecoveryModes'] === ['restart', 'truncate', 'restart', 'truncate']);
    assert($summary['postCheckpointTailRecoveryWalFramesBefore'] === array_fill(0, 4, 2));
    assert($summary['postCheckpointTailRecoveryWalFramesAfter'] === array_fill(0, 4, 5));
    assert($summary['postCheckpointTailRecoveryPages'][0] === [1321, 1941, 2021]);
    assert($summary['postCheckpointTailRecoveryInsertedKeys'][0] === 'post_checkpoint_recovery_payload_1');
    assert($summary['postCheckpointTailRecoveryRejectedTailKeysRetained'] === array_fill(0, 4, false));
    assert($summary['postCheckpointTailRecoveryFollowupKeysRetained'] === array_fill(0, 4, true));
    assert($summary['postCheckpointTailRecoveryCheckpointModes'] === ['restart', 'truncate', 'restart', 'truncate']);
    assert($summary['postCheckpointTailRecoveryCheckpointActions'] === ['restart_wal', 'truncate_wal', 'restart_wal', 'truncate_wal']);
    assert($summary['postCheckpointTailRecoveryCheckpointReleasedWalBytes'] === [32, 0, 32, 0]);
    assert($summary['postCheckpointTailRecoveryCheckpointPinnedBusy'] === array_fill(0, 4, true));
    assert($summary['postCheckpointTailRecoveryCheckpointAppliedPages'][0] === [1321, 1941, 2021]);
    assert($summary['postCheckpointTailRecoveryCheckpointPagesMaterialized'] === array_fill(0, 4, true));
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupFramePinned'] === array_fill(0, 4, true));
    assert($summary['postCheckpointTailRecoveryCheckpointRejectedTailKeysRetained'] === array_fill(0, 4, false));
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupModes'] === ['restart', 'truncate', 'restart', 'truncate']);
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupStartedNewWalHeaders'] === [false, true, false, true]);
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupWalFramesBefore'] === array_fill(0, 4, 0));
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupWalFramesAfter'] === array_fill(0, 4, 2));
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupPages'][0] === [1321, 2221]);
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupInsertedKeys'][0] === 'tail_recovery_checkpoint_followup_payload_1');
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryKeysRetained'] === array_fill(0, 4, true));
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRejectedTailKeysRetained'] === array_fill(0, 4, false));
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupFailureModes'] === ['restart', 'truncate', 'restart', 'truncate']);
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupFailureWalFramesBefore'] === array_fill(0, 4, 5));
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupFailureWalFramesAfter'] === array_fill(0, 4, 2));
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupFailurePages'][0] === [1321, 2321]);
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupFailureFailedStatements'][0] === ['tail_recovery_checkpoint_followup_tail_broken_payload_1']);
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupFailureInsertedKeys'][0] === 'tail_recovery_checkpoint_followup_tail_payload_1');
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryModes'] === ['restart', 'truncate', 'restart', 'truncate']);
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryWalFramesBefore'] === array_fill(0, 4, 2));
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryWalFramesAfter'] === array_fill(0, 4, 5));
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryPages'][0] === [1321, 2421, 2221]);
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryInsertedKeys'][0] === 'tail_recovery_checkpoint_followup_recovery_payload_1');
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryTailKeysRetained'] === array_fill(0, 4, false));
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryPriorKeysRetained'] === array_fill(0, 4, true));
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointModes'] === ['restart', 'truncate', 'restart', 'truncate']);
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointActions'] === ['restart_wal', 'truncate_wal', 'restart_wal', 'truncate_wal']);
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointReleasedWalBytes'] === [32, 0, 32, 0]);
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointPinnedBusy'] === array_fill(0, 4, true));
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointAppliedPages'][0] === [1321, 2421, 2221]);
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointPagesMaterialized'] === array_fill(0, 4, true));
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointPriorFramePinned'] === array_fill(0, 4, true));
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointRejectedTailKeysRetained'] === array_fill(0, 4, false));
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupModes'] === ['restart', 'truncate', 'restart', 'truncate']);
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupStartedNewWalHeaders'] === [false, true, false, true]);
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupWalFramesBefore'] === array_fill(0, 4, 0));
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupWalFramesAfter'] === array_fill(0, 4, 3));
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupPages'][0] === [1321, 2621, 2421]);
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupInsertedKeys'][0] === 'tail_recovery_checkpoint_followup_recovery_checkpoint_followup_payload_1');
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupRecoveryKeysRetained'] === array_fill(0, 4, true));
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupPriorKeysRetained'] === array_fill(0, 4, true));
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupRejectedTailKeysRetained'] === array_fill(0, 4, false));
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailFailureScenarioCount'] === 4);
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailFailureModes'] === ['restart', 'truncate', 'restart', 'truncate']);
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailFailureWalFramesBefore'] === array_fill(0, 4, 6));
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailFailureWalFramesAfter'] === array_fill(0, 4, 3));
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailFailurePages'][0] === [1321, 2821]);
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailFailureFailedStatements'][0] === ['followup_recovery_checkpoint_followup_tail_bad_insert_1']);
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailFailureInsertedKeys'][0] === 'tail_recovery_checkpoint_followup_recovery_checkpoint_followup_tail_payload_1');
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailFailureFinalPrefixRestored'] === array_fill(0, 4, true));
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryModes'] === ['restart', 'truncate', 'restart', 'truncate']);
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryWalFramesBefore'] === array_fill(0, 4, 3));
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryWalFramesAfter'] === array_fill(0, 4, 6));
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryPages'][0] === [1321, 3021, 2621]);
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryInsertedKeys'][0] === 'tail_recovery_checkpoint_followup_recovery_checkpoint_followup_tail_recovery_payload_1');
    ...
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryCheckpointFollowupModes'] === ['restart', 'truncate', 'restart', 'truncate']);
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryCheckpointFollowupStartedNewWalHeaders'] === [false, true, false, true]);
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryCheckpointFollowupWalFramesBefore'] === array_fill(0, 4, 0));
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryCheckpointFollowupWalFramesAfter'] === array_fill(0, 4, 3));
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryCheckpointFollowupPages'][0] === [1321, 3221, 3021]);
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryCheckpointFollowupInsertedKeys'][0] === 'tail_recovery_checkpoint_followup_recovery_checkpoint_followup_tail_recovery_checkpoint_followup_payload_1');
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryCheckpointFollowupTailRecoveryKeysRetained'] === array_fill(0, 4, true));
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryCheckpointFollowupFinalKeysRetained'] === array_fill(0, 4, true));
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryCheckpointFollowupRejectedTailKeysRetained'] === array_fill(0, 4, false));
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryCheckpointFollowupTailFailureScenarioCount'] === 4);
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryCheckpointFollowupTailFailureModes'] === ['restart', 'truncate', 'restart', 'truncate']);
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryCheckpointFollowupTailFailureWalFramesBefore'] === array_fill(0, 4, 6));
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryCheckpointFollowupTailFailureWalFramesAfter'] === array_fill(0, 4, 3));
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryCheckpointFollowupTailFailurePages'][0] === [1321, 3421]);
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryCheckpointFollowupTailFailureFailedStatements'][0] === ['followup_recovery_checkpoint_followup_tail_recovery_checkpoint_followup_tail_bad_insert_1']);
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryCheckpointFollowupTailFailureInsertedKeys'][0] === 'tail_recovery_checkpoint_followup_recovery_checkpoint_followup_tail_recovery_checkpoint_followup_tail_payload_1');
    assert($summary['postCheckpointTailRecoveryCheckpointFollowupRecoveryCheckpointFollowupTailRecoveryCheckpointFollowupTailFailureFinalPrefixRestored'] === array_fill(0, 4, true));
