...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "application-planner-partial-range-covering-current-source self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'application-planner-partial-range-covering-current-source',
    'status' => $plan['status'],
    'selectedSource' => $plan['selectedSource'],
    'coveredRowids' => array_column($plan['coveredRows'], 'rowid'),
    'filteredRowids' => $plan['partialPredicateFilteredRowids'],
    'detail' => $plan['detail'],
    'applicationUse' => 'Copied wp_options plugin scans can reprepare a stale partial covering range plan against the current source and avoid leaking theme or mu-plugin rows that fall inside the raw option_name range.',
