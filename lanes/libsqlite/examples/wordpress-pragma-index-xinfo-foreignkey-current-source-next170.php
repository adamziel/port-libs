...
$page = SQLitePragmaIndexXinfoForeignKeyCurrentSourceNext170::currentNextPageFromCatalog(
    $currentRecords,
    $currentTables,
    $nextRecords,
    $nextTables,
...
$summary = [
    'scenario' => 'wordpress-pragma-index-xinfo-foreignkey-current-source-next170',
    'wordpressUse' => 'Copied multisite wp_options imports can distinguish immediate FK blockers from deferrable violations that survive until transaction commit while resuming PRAGMA index_xinfo diagnostics.',
    'status' => $page['status'],
...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "wordpress-pragma-index-xinfo-foreignkey-current-source-next170 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-index-xinfo-foreignkey-current-source-next170 self-test passed\n");
    exit(0);
}
