...
$plan = SQLiteBTreeFreelistVacuumPointerMapCurrentSourceNextPlan::currentSourcePointerMapFromDeleteResults(
...
$report = [
    'scenario' => 'application-btree-freelist-vacuum-pointermap-current-source-next148',
    'applicationUse' => 'Replacing an oversized autoloaded option after incremental vacuum crosses an auto-vacuum pointer-map boundary without reusing the pointer-map page as overflow.',
...
if ($report['truncatedPointerMapPages'] !== [311] || $report['allocatedOverflowPages'] !== [310, 309]) {
    fwrite(STDERR, "application-btree-freelist-vacuum-pointermap-current-source-next148 self-test failed\n");
    exit(1);
}
