...
require_once __DIR__ . '/../src/SQLiteVfsCurrentSourceNext370385Plan.php';

use PortLibs\LibSqlite\SQLiteVfsCurrentSourceNext370385Plan;
...
$plan = SQLiteVfsCurrentSourceNext370385Plan::run([
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    assert($plan['events'][0]['ack'] === 'shared-cache-next369');
    assert($plan['events'][1]['status'] === 'claimed-reusable-current-source');
    assert($plan['events'][1]['claim'] === 'reader-reuse-next385');
    assert($plan['events'][1]['blocked_reasons'] === []);
    assert($plan['events'][2]['status'] === 'published-reused-current-source');
    assert($plan['events'][2]['reuse_ack'] === 'shared-cache-next369');
    assert($plan['events'][2]['published_count'] === 16);
    assert(in_array('vfs-current-source-snapshot-reuse-publish-next370-385', $plan['dependencies'], true));
    assert(in_array('vfs-current-source-snapshot-reuse-publish-next370-385', $plan['dependencies'], true));
    echo "wordpress-vfs-current-source-next370-385 self-test passed\n";
    return;
}
