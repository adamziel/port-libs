...
$preparedSource = [
    'name' => 'prepared-wordpress-covering-expression-range-next134',
    'schemaCookie' => 1340,
    'stat4Generation' => 41,
    'indexes' => [[
...
$currentSource['name'] = 'current-wordpress-covering-expression-range-next134';
...
if (($argv[1] ?? '') === '--self-test') {
    assert($plan['status'] === 'covering-expression-range-current-source-next134-ready');
    ...
    echo "wordpress-covering-expression-range-current-source-next134 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'wordpress-covering-expression-range-current-source-next134',
    'status' => $plan['status'],
