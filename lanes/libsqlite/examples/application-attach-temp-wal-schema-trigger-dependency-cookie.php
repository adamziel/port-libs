...
$plan = SQLiteAttachTempWalSchemaTriggerPlan::triggerDependencyCookiePlan(
    $current,
    $next,
...
$output = [
    'scenario' => 'application-attach-temp-wal-schema-trigger-dependency-cookie',
    'applicationUse' => 'Preview copied Application option-import TEMP triggers whose unqualified audit writes move from TEMP to main after schema reload while attached-site audit writes still watch WAL schema cookies, without requiring ext/sqlite.',
    'status' => $plan['status'],
...
if (PHP_SAPI === 'cli' && in_array('--self-test', $argv ?? [], true)) {
    if (
        ...
        fwrite(STDERR, "application attach temp wal schema trigger current-source next95 smoke failed\n");
        exit(1);
    }

    echo "application-attach-temp-wal-schema-trigger-dependency-cookie self-test passed\n";
}
