...
require_once dirname(__DIR__) . '/src/SQLiteTriggerRecursiveViewReturningCurrentSourceNext224Plan.php';
require_once dirname(__DIR__) . '/src/SQLiteTriggerRecursiveViewReturningCurrentSourceNext229Plan.php';
require_once dirname(__DIR__) . '/src/SQLiteTriggerRecursiveViewUpsertCurrentSourceNext233Plan.php';
require_once dirname(__DIR__) . '/src/SQLiteTriggerRecursiveViewUpsertCurrentSourceNext236Plan.php';
...
$summary = SQLiteTriggerRecursiveViewUpsertCurrentSourceNext236Plan::execute(
    [
...
if (
    ...
    || $summary['current_upsert_row_images_next236']['names'] !== ['blogdescription_child', 'template_child']
    || array_column($summary['visible_returning_payloads_next236'], 'name') !== ['blogdescription_child', 'template_child', 'home', 'next_plugin']
    || $summary['held_next_source_rows_next236'] !== []
) {
    fwrite(STDERR, "wordpress-trigger-recursive-view-upsert-current-source-next236 self-test failed\n");
    exit(1);
}

echo "wordpress-trigger-recursive-view-upsert-current-source-next236 self-test passed\n";
