...
$summary = [
    'applicationUse' => 'Copied wp_options imports can keep an active INSTEAD OF view trigger on its current source while a plugin migration drops main/temp/archive trigger sources in the next schema, then force SQLITE_SCHEMA reprepare before the next step.',
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    fwrite(STDOUT, "application-attach-wal-temp-view-trigger-current-source-next101 self-test passed\n");
    return;
}
