...
$summary = [
    'scenario' => 'wordpress-rowvalue-conflict-returning-savepoint-current-source-next138',
    'wordpressUse' => 'Model a copied wp_options import savepoint where row-value UPDATE RETURNING applies SQLite conflict algorithms: OR IGNORE skips conflicting rows without RETURNING output, OR REPLACE deletes the current conflicting row before yielding replacement output, and later cleanup sees that current source.',
    'status' => $plan['status'],
