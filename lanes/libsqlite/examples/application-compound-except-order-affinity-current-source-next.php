...
$result = [
    'scenario' => 'application-compound-except-order-affinity-current-source-next',
    'applicationUse' => 'Copied wp_options migration checks can subtract network-level options with EXCEPT, then apply SQLite tail ORDER BY storage-class and NOCASE rules so changed current-source rows are replayed in deterministic import order without ext/sqlite.',
...
if (PHP_SAPI === 'cli' && basename(__FILE__) === basename($_SERVER['SCRIPT_FILENAME'] ?? '')) {
    ...
    echo "application-compound-except-order-affinity-current-source-next self-test passed\n";
}
