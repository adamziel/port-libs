...
$page = SQLitePragmaIndexXinfoForeignKeyCurrentSourceNext227::page(
    $currentRecords,
    $nextRecords,
...
$summary = [
    'scenario' => 'wordpress-pragma-index-xinfo-foreignkey-current-source-next227',
    'wordpressUse' => 'Copied WordPress postmeta import schemas can identify lookup indexes where FK child columns are present but not leftmost, then confirm the repaired next schema moves those columns to the prefix.',
    'status' => $page['status'],
...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "wordpress-pragma-index-xinfo-foreignkey-current-source-next227 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-index-xinfo-foreignkey-current-source-next227 self-test passed\n");
    exit(0);
}
