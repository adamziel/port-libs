...
$summary = [
    'scenario' => 'application-pragma-index-xinfo-foreignkey-current-source-next287-294',
    'applicationUse' => 'Copied Application plugin schemas can page child-side PRAGMA foreign_key_list/table_info/index_xinfo diagnostics before enabling FK enforcement.',
...
if (($argv[1] ?? null) === '--self-test') {
    foreach (array_keys($summary) as $key) {
        if (str_starts_with($key, 'next') && $summary[$key] !== 1) {
            fwrite(STDERR, "application-pragma-index-xinfo-foreignkey-current-source-next287-294 self-test failed\n");
            exit(1);
        }
    }
    ...
    fwrite(STDOUT, "application-pragma-index-xinfo-foreignkey-current-source-next287-294 self-test passed\n");
    exit(0);
}
