...
if (($argv[1] ?? null) === '--self-test') {
    if ($plan['status'] !== 'rolled-back' || $plan['changes'] !== 0 || count($plan['attempted_yielded']) !== 1 || $plan['rollback_page_numbers'] !== [2, 3]) {
        fwrite(STDERR, "wordpress-trigger-returning-foreignkey-savepoint-current-next54 self-test failed\n");
        exit(1);
    }
    echo "wordpress-trigger-returning-foreignkey-savepoint-current-next54 self-test passed\n";
    exit(0);
}
