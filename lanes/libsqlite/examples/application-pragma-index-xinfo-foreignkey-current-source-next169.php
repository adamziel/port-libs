...
if (($argv[1] ?? null) === '--self-test') {
    if ($page['status'] !== 'ok' || ($page['rows'][3]['deferral_summary'] ?? null) !== 'deferrable_deferred' || ($page['current_source']['deferral_summary']['deferrable_deferred'] ?? null) !== 1) {
        fwrite(STDERR, "application-pragma-index-xinfo-foreignkey-current-source-next169 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "application-pragma-index-xinfo-foreignkey-current-source-next169 self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'application-pragma-index-xinfo-foreignkey-current-source-next169',
    'applicationUse' => 'Classify copied wp_options foreign-key repair rows by immediate, DEFERRABLE INITIALLY IMMEDIATE, and DEFERRABLE INITIALLY DEFERRED DDL before resuming imports.',
    'status' => $page['status'],
