...
require_once __DIR__ . '/../src/SQLiteVfsCurrentSourceNext266273Plan.php';

use PortLibs\LibSqlite\SQLiteVfsCurrentSourceNext266273Plan;

$plan = SQLiteVfsCurrentSourceNext266273Plan::run([
            ...
            'main' => [
                ...
                'reuse_acks' => [
                    [
                        ...
                        'data_version' => 20,
                        'published_count' => 6,
                        'receipt_digest' => hash('sha256', 'publish-next217|shared-cache-next229|shared-cache-next237|shared-cache-next245|shared-cache-next257|shared-cache-next265'),
                    ],
                ],
            ],
        ],
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    assert($plan['events'][0]['claim'] === 'reader-reuse-next273');
    assert($plan['events'][0]['blocked_reasons'] === []);
    assert($plan['events'][1]['status'] === 'published-reused-current-source');
    assert($plan['events'][1]['reuse_ack'] === 'shared-cache-next265');
    assert($plan['events'][1]['published_count'] === 7);
    ...
    echo "wordpress-vfs-current-source-next266-273 self-test passed\n";
    return;
}
