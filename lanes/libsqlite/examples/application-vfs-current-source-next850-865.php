...
if (($argv[1] ?? '') === '--self-test') {
    ...
    assert($plan['events'][0]['ack'] === 'shared-cache-next849');
    assert($plan['events'][0]['blocked_reasons'] === []);
    assert($plan['events'][1]['status'] === 'claimed-reusable-current-source');
    assert($plan['events'][1]['claim'] === 'reader-reuse-next865');
    assert($plan['events'][1]['blocked_reasons'] === []);
    assert($plan['events'][2]['status'] === 'published-reused-current-source');
    assert($plan['events'][2]['reuse_ack'] === 'shared-cache-next849');
    assert($plan['events'][2]['token'] === 'shared-cache-next865');
    assert($plan['events'][2]['published_count'] === 45);
    assert(in_array('vfs-current-source-snapshot-reuse-publish-next850-865', $plan['dependencies'], true));
    assert(in_array('vfs-current-source-snapshot-reuse-publish-next834-849', $plan['dependencies'], true));
    echo "application-vfs-current-source-next850-865 self-test passed\n";
    return;
}
