...
$trigger = SQLiteAttachTempWalSchemaTriggerPlan::currentSourceNext89(
    $catalog,
...
$view = SQLiteAttachWalTempViewCachePlan::currentSourceNext82(
    $catalog,
...
$summary = [
    'scenario' => 'wordpress attach temp wal trigger view cache current source next93',
...
if (($argv[1] ?? '') === '--self-test') {
    if (
        ...
        fwrite(STDERR, "wordpress-attach-temp-wal-trigger-view-cache-current-source-next93 self-test failed\n");
        exit(1);
    }

    echo "wordpress-attach-temp-wal-trigger-view-cache-current-source-next93 self-test passed\n";
    exit(0);
}
