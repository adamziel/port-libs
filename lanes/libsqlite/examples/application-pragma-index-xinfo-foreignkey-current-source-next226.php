...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "application-pragma-index-xinfo-foreignkey-current-source-next226 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "application-pragma-index-xinfo-foreignkey-current-source-next226 self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'application-pragma-index-xinfo-foreignkey-current-source-next226',
    'applicationUse' => 'Copied taxonomy relationship imports must not admit PRAGMA foreign_key_list/index_xinfo repair pages when the child table references parent taxonomy tables that are absent from the current copied catalog.',
