...
$sql = "INSERT INTO wp_options(option_name, option_value, autoload, hits, touched) VALUES "
    ...
    . "('new_plugin','enabled','no',1,'import') "
    ...
    . 'RETURNING option_name AS name, option_value, hits';

$result = SQLiteUpsertReturningSql::execute($sql, ['wp_options' => $options], [['option_name']]);

echo json_encode([
    'applicationUse' => 'Preview INSERT ... ON CONFLICT DO UPDATE WHERE ... RETURNING for copied wp_options import rows without requiring ext/sqlite.',
    'sql' => $sql,
