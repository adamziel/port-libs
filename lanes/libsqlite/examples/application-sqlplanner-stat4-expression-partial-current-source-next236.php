...
$plan = SQLitePlannerStat4ExpressionPartialCurrentSourceNextPlan::materializeCurrentSourceStat4DensityVectorValidation(
    $prepared,
    $current,
    $where,
...
$summary = [
    'scenario' => 'application-sqlplanner-stat4-expression-partial-current-source-next236',
    'status' => $plan['status'],
    ...
    'applicationUse' => 'Prevents copied wp_options plugin scans from reusing sqlite_stat4 expression-index samples whose rowids still resolve but whose neq/nlt/ndlt density vectors are stale after import churn.',
];
...
if ($summary['status'] !== 'stat4-expression-partial-current-source-next236-ready'
    ...
    fwrite(STDERR, "application-sqlplanner-stat4-expression-partial-current-source-next236 self-test failed\n");
    exit(1);
}
