...
$schemas = [
    'main' => ['schema_cookie' => 764, 'tables' => ['wp_options', 'wp_navigation_rule_locale_publish_receipt_next732', 'wp_navigation_rule_locale_publish_final_next764'], 'indexes' => ['wp_options_name', 'wp_navigation_rule_locale_publish_receipt_key_next732', 'wp_navigation_rule_locale_publish_final_key_next764'], 'wal_frames' => [['page' => 1, 'schema_cookie' => 764, 'commit' => true]]],
    'temp' => ['schema_cookie' => 752, 'tables' => ['wp_theme_stage_publish_review_next752', 'wp_theme_stage_publish_notice_next768'], 'indexes' => [], 'temp' => true],
    'archive' => ['schema_cookie' => 750, 'tables' => ['wp_schema_archive_done_next750'], 'indexes' => ['wp_schema_archive_done_key_next750'], 'file' => '/srv/wp/archive-next765.sqlite'],
    'audit' => ['schema_cookie' => 773, 'tables' => ['wp_schema_audit_receipt_next773'], 'indexes' => ['wp_schema_audit_receipt_key_next773'], 'file' => '/srv/wp/audit-next765.sqlite'],
    'handoff' => ['schema_cookie' => 761, 'tables' => ['wp_schema_handoff_receipt_next761'], 'indexes' => ['wp_schema_handoff_receipt_key_next761'], 'file' => '/srv/wp/handoff-next765.sqlite'],
    'publish' => ['schema_cookie' => 739, 'tables' => ['wp_schema_publish_next679', 'wp_schema_publish_done_next706'], 'indexes' => ['wp_schema_publish_key_next686', 'wp_schema_publish_done_key_next722'], 'file' => '/srv/wp/publish-next765.sqlite'],
    'queue' => ['schema_cookie' => 756, 'tables' => ['wp_job_retry_checkpoint_delivered_next688', 'wp_job_retry_checkpoint_archive_next756'], 'indexes' => ['wp_job_retry_checkpoint_delivered_key_next702', 'wp_job_retry_checkpoint_archive_key_next756'], 'file' => '/srv/wp/queue-next765.sqlite'],
    'report' => ['schema_cookie' => 728, 'tables' => ['wp_schema_report_next730', 'wp_schema_report_meta_next694'], 'indexes' => ['wp_schema_report_key_next693', 'wp_schema_report_meta_key_next694'], 'file' => '/srv/wp/report-next765.sqlite'],
];
...
$plan = SQLiteAttachWalTempSchemaCacheCurrentSourceNext92Plan::currentSourceNext765780($schemas, $statements, [
...
if (($argv[1] ?? '') === '--self-test') {
    assert($plan['operation'] === 'attach-wal-temp-schema-cache-current-source-next765-780');
    assert($plan['dependencies'][0] === 'sqlite-attach-temp-wal-schema-cache-current-source-next765');
    assert($plan['dependencies'][15] === 'sqlite-attach-temp-wal-schema-cache-current-source-next780');
    assert(in_array('sqlite-attach-temp-wal-schema-cache-current-source-next764', $plan['dependencies'], true));
    assert($plan['event_count'] === 9);
    assert($plan['schema_cookies_next']['main'] === 780);
    assert($plan['schema_cookies_next']['temp'] === 768);
    assert($plan['schema_cookies_next']['publish'] === 778);
    assert($plan['schema_cookies_next']['review'] === 773);
    ...
    echo "wordpress-attach-temp-wal-schema-cache-current-source-next765-780 self-test passed\n";
    return;
}
