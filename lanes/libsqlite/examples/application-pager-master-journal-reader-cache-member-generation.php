...
$summary = [
    'scenario' => 'application-pager-master-journal-reader-cache-member-generation',
    'status' => $plan['status'],
    ...
    'applicationUse' => 'A copied Application attached-database recovery reuses wp_options reader-cache pages only after the current master-journal member generation is recovered and its rollback journal has been deleted.',
...
if ($summary['status'] !== 'pager-master-journal-reader-cache-current-source-next178'
    ...
    fwrite(STDERR, "application-pager-master-journal-reader-cache-member-generation self-test failed\n");
    exit(1);
}

echo json_encode($summary, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) . "\n";
echo "application-pager-master-journal-reader-cache-member-generation self-test passed\n";
