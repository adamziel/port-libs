...
$payload = [
    'scenario' => 'application-compound-select-window-recursive-limit-current-source-next213',
    'applicationUse' => 'Copied wp_options preview queries can combine recursive dependency queues with min/max window extrema, then fence current-source resume cursors across an INTERSECT membership boundary before next-source rows shift the final LIMIT.',
    'status' => $plan['status'],
...
if (($payload['status'] ?? null) !== 'compound-select-window-recursive-limit-current-source-next213-ready') {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-next213 self-test failed\n");
    exit(1);
}
if (array_values(array_unique($payload['windowFunctions'])) !== ['max', 'min']) {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-next213 window dispatch failed\n");
    exit(1);
}
if ($payload['currentTokenLength'] !== 64 || $payload['nextTokenLength'] !== 64) {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-next213 token guard failed\n");
    exit(1);
}
