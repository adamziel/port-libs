...
$summary = [
    'scenario' => 'wordpress-pragma-index-xinfo-foreignkey-current-source-next375-382',
    'wordpressUse' => 'WordPress import previews can flag descending child lookup indexes before SET NULL, SET DEFAULT, and NO ACTION foreign-key actions are trusted from PRAGMA index_xinfo.',
...
if (($argv[1] ?? null) === '--self-test') {
    foreach ($cases as $slice => [, , $status]) {
        if (($summary["next{$slice}_{$status}"] ?? null) !== 1) {
            fwrite(STDERR, "wordpress-pragma-index-xinfo-foreignkey-current-source-next375-382 self-test failed\n");
            exit(1);
        }
    }
    ...
    fwrite(STDOUT, "wordpress-pragma-index-xinfo-foreignkey-current-source-next375-382 self-test passed\n");
    exit(0);
}
