...
$plan = SQLiteJsonTablePlan::currentSourceRowidHiddenPathNext138(
    'json_tree',
    $current,
    $next,
    'option_value',
...
if (($argv[1] ?? '') === '--self-test') {
    assert(in_array('sqlite-json-table-rowid-hidden-path-current-source-next138', $plan['dependencies'], true));
    ...
    assert(in_array('source-json-changed', $plan['next138ReplanReasons'], true));
    echo "wordpress-json-table-rowid-hidden-path-current-source-next138 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'wordpress-json-table-rowid-hidden-path-current-source-next138',
    ...
    'replanReasons' => $plan['next138ReplanReasons'],
