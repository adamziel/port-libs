...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "wordpress-rowvalue-returning-savepoint-distinct-current-source-next149 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'wordpress-rowvalue-returning-savepoint-distinct-current-source-next149',
    'status' => $plan['status'],
    'savepoint' => $plan['savepoint'],
    'returning' => $plan['yielded_returning'],
    'currentOptionIds' => array_column($plan['current_source_tables']['wp_options'], 'option_id'),
    'wordpressUse' => 'Copied wp_options cleanup/import batches can use real numeric literals in row-value IS DISTINCT FROM and IS NOT DISTINCT FROM predicates while savepoints preserve the current source for rollback.',
