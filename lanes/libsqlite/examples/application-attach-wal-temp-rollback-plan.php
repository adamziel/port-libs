...
$plan = SQLiteAttachWalTempPlan::rollbackPlan($schemas, [
...
if (($argv[1] ?? '') === '--self-test') {
    if (
        ...
        fwrite(STDERR, "application-attach-wal-temp-rollback-plan self-test failed\n");
        exit(1);
    }

    echo "application-attach-wal-temp-rollback-plan self-test passed\n";
    exit(0);
}

echo json_encode([
    'scenario' => 'application-attach-wal-temp-rollback-plan',
    'status' => $plan['status'],
