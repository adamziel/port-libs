...
$result = [
    'scenario' => 'application-pager-savepoint-current-next69',
    'applicationUse' => 'After a copied wp_options plugin import row fails, ROLLBACK TO keeps the plugin batch savepoint open and the next retried option-row savepoint starts beneath that retained current frame with a fresh WAL frame index.',
...
if (($argv[1] ?? '') === '--self-test') {
    if (
        ...
        fwrite(STDERR, "application-pager-savepoint-current-next69 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "application-pager-savepoint-current-next69 self-test passed\n");
    exit(0);
}
