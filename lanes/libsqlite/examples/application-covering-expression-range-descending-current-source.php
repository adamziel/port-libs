...
$preparedSource = [
    'name' => 'prepared-application-covering-expression-range-descending',
    'schemaCookie' => 1340,
    'stat4Generation' => 41,
    'indexes' => [[
...
$currentSource['name'] = 'current-application-covering-expression-range-descending';
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "application-covering-expression-range-current-source-descending self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'application-covering-expression-range-current-source-descending',
    'status' => $plan['status'],
    ...
    'applicationUse' => 'Copied wp_options plugin admin screens can stream lower(option_name) DESC from the current covering expression index after ANALYZE/schema changes, rejecting stale prepared bounds without table b-tree seeks.',
