...
require_once __DIR__ . '/../src/SQLiteVfsCurrentSourceNext610625Plan.php';
...
$plan = SQLiteVfsCurrentSourceNext610625Plan::run([
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    assert($plan['events'][0]['ack'] === 'shared-cache-next609');
    assert($plan['events'][1]['status'] === 'claimed-reusable-current-source');
    assert($plan['events'][1]['claim'] === 'reader-reuse-next625');
    assert($plan['events'][1]['blocked_reasons'] === []);
    assert($plan['events'][2]['status'] === 'published-reused-current-source');
    assert($plan['events'][2]['reuse_ack'] === 'shared-cache-next609');
    assert($plan['events'][2]['token'] === 'shared-cache-next625');
    assert($plan['events'][2]['published_count'] === 31);
    assert(in_array('vfs-current-source-snapshot-reuse-publish-next610-625', $plan['dependencies'], true));
    assert(in_array('vfs-current-source-snapshot-reuse-publish-next594-609', $plan['dependencies'], true));
    assert(in_array('vfs-current-source-snapshot-reuse-publish-next562-577', $plan['dependencies'], true));
    assert(in_array('vfs-current-source-snapshot-reuse-publish-next530-545', $plan['dependencies'], true));
    assert(in_array('vfs-current-source-snapshot-reuse-publish-next514-529', $plan['dependencies'], true));
    echo "wordpress-vfs-current-source-next610-625 self-test passed\n";
    return;
}
