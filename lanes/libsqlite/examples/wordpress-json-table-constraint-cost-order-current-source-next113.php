...
$payload = [
    'scenario' => 'wordpress-json-table-constraint-cost-order-current-source-next113',
    'wordpressUse' => 'Copied wp_options JSON diagnostics can compare current and next json_each() constraint plans, including whether ORDER BY can stream from rowid order or must allocate a sorter, before import tooling commits changed plugin rules without ext/sqlite.',
...
if (($argv[1] ?? null) === '--self-test') {
    ...
    echo "wordpress-json-table-constraint-cost-order-current-source-next113 self-test passed\n";
    return;
}
