...
if (PHP_SAPI === 'cli' && ($argv[1] ?? null) === '--self-test') {
    ...
    echo "application-trigger-upsert-savepoint-returning-current-source-next132 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'application-trigger-upsert-savepoint-returning-current-source-next132',
    'applicationUse' => 'Preview a copied wp_options import where UPSERT RETURNING diagnostics are yielded from the current row image before AFTER-trigger side effects, then a later trigger RAISE(ROLLBACK) restores the import savepoint and clears the next statement RETURNING stream.',
