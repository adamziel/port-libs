...
if ($plan['status'] !== 'spilled' || $plan['next']['spilled_pages'] !== [5] || $plan['next']['dirty_pages'] !== [2, 8, 9]) {
    fwrite(STDERR, "wordpress-pager-dirty-page-cache-spill-current-next71 self-test failed\n");
    exit(1);
}

if (PHP_SAPI === 'cli' && realpath($_SERVER['SCRIPT_FILENAME'] ?? '') === __FILE__) {
    echo "wordpress-pager-dirty-page-cache-spill-current-next71 self-test passed\n";
}

return [
    'scenario' => 'wordpress-pager-dirty-page-cache-spill-current-next71',
    'wordpressUse' => 'Plan SQLite pager dirty-page cache spill for copied wp_options imports where journaled unpinned dirty pages may be written before commit only after journal sync and exclusive-lock promotion.',
    'status' => $plan['status'],
