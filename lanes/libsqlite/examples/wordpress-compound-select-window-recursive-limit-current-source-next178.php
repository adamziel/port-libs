...
$plan = SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNextPlan::compareNext178($sql, $currentTables, $nextTables);

if (in_array('--self-test', $argv, true)) {
    if (($plan['status'] ?? null) !== 'compound-select-window-recursive-limit-current-source-next178-ready') {
        fwrite(STDERR, "unexpected compound next178 status\n");
        exit(1);
    }
    if (array_column($plan['nextRows'], 'label') !== ['rewrite_rules', 'siteurl', 'plugin_alpha', 'seed:2:3', 'seed:2:3:4']) {
        fwrite(STDERR, "unexpected compound next178 next rows\n");
        exit(1);
    }
    if (($plan['distinctUnion']['nextDuplicateLabels'] ?? []) !== []) {
        fwrite(STDERR, "unexpected compound next178 duplicate full-row diagnostic\n");
        exit(1);
    }
    echo "wordpress-compound-select-window-recursive-limit-current-source-next178 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'wordpress-compound-select-window-recursive-limit-current-source-next178',
