...
$finalizerPlan = [
    ...
    'source_token' => 'wordpress-next240-source',
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    assert(in_array('wordpress-import-next-writer-autocheckpoint-after-hot-journal', $plan['dependencies'], true));
    echo "wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next240 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next240',
    'status' => $plan['status'],
    ...
    'wordpressUse' => 'After a copied wp_options import recovers a hot journal, rolls back a WAL savepoint, checkpoints the current source, and finalizes readers, the next writer commit can publish a clean WAL-index/autocheckpoint baseline only when the checkpoint database and page-cache receipts still match.',
