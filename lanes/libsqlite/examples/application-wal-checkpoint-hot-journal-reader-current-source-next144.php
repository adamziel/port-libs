...
$summary = [
    'scenario' => 'application-wal-checkpoint-hot-journal-reader-current-source-next144',
    'applicationUse' => 'After a copied wp_options database recovers a hot rollback journal, checkpoint reset is allowed only for readers pinned to the recovered database source, not dirty pre-recovery bytes with the same WAL header.',
...
if (PHP_SAPI === 'cli' && basename(__FILE__) === basename($_SERVER['SCRIPT_FILENAME'] ?? '')) {
    ...
    if (in_array('--self-test', $_SERVER['argv'] ?? [], true)) {
        echo "application-wal-checkpoint-hot-journal-reader-current-source-next144 self-test passed\n";
        return;
    }
