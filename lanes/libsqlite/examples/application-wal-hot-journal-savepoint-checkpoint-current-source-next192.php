...
$summary = [
    'scenario' => 'application-wal-hot-journal-savepoint-checkpoint-current-source-next192',
    'applicationUse' => 'A copied Application import recovers a hot journal, rolls back a savepoint, checkpoints WAL pages into the database image, and reuses only prepared readers whose observed page digests match the materialized checkpoint pages.',
    'status' => $plan['status'],
...
if ($summary['status'] !== 'wal-hot-journal-savepoint-checkpoint-current-source-next192'
    ...
    fwrite(STDERR, "application-wal-hot-journal-savepoint-checkpoint-current-source-next192 self-test failed\n");
    exit(1);
}
