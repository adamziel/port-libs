...
if (in_array('--self-test', $argv, true)) {
    ...
    echo "application-sqlplanner-stat4-expression-partial-current-or-split-partial-expression self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'application-sqlplanner-stat4-expression-partial-current-or-split-partial-expression',
    'applicationUse' => 'Preview copied wp_options plugin diagnostics whose OR-split lower(option_name) probes can use the current partial STAT4 expression index only when every arm proves blog/autoload partial predicates after ANALYZE/source changes.',
    'status' => $plan['status'],
