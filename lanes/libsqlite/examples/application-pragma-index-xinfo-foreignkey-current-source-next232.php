...
$summary = [
    'scenario' => 'application-pragma-index-xinfo-foreignkey-current-source-next232',
    'applicationUse' => 'Copied Application import schemas can identify child FK action indexes whose columns are present but reversed, then require a leftmost-prefix repair before relying on parent delete/update probes.',
    'status' => $page['status'],
...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "application-pragma-index-xinfo-foreignkey-current-source-next232 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "application-pragma-index-xinfo-foreignkey-current-source-next232 self-test passed\n");
    exit(0);
}
