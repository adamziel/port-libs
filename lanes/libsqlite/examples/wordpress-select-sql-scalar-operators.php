...
$sql = "SELECT option_id, option_name || ':' || coalesce(autoload, 'missing') AS label, bytes + option_id AS weight, bytes % 10 AS bucket FROM wp_options WHERE (bytes + option_id) >= 15 ORDER BY (bytes + option_id) DESC LIMIT 4";
$rows = SQLiteSelectSql::execute($sql, ['wp_options' => $options]);

echo json_encode([
    'wordpressUse' => 'Preview copied wp_options rows from bounded SQLite SELECT text with native scalar arithmetic and concatenation in projection, WHERE, and ORDER BY without requiring ext/sqlite.',
    'sql' => $sql,
