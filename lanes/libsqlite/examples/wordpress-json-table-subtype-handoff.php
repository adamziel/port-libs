...
echo json_encode([
    'scenario' => 'wordpress-json-table-subtype-handoff',
    'inputKind' => $plan['jsonInputKind'],
    'jsonValid' => $plan['jsonValid'],
    'rowCount' => count($rows),
    'matchingFullkeys' => array_column($rows, 'fullkey'),
    'rows' => $rows,
    'wordpressUse' => 'Local-only wp_options diagnostics that pass JSON constructor/subtype results directly into json_tree hidden json constraints, preserving SQLite JSON subtype handoff before copied plugin settings are imported without requiring ext/sqlite.',
