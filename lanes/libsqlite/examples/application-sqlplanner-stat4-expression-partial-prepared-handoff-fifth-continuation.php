...
$plan = SQLitePlannerStat4ExpressionPartialCurrentSourceNextPlan::materializePreparedHandoffWindowContinuation(
    $prepared,
    $current,
    [
...
if (in_array('--self-test', $argv, true)) {
    ...
    echo "application-sqlplanner-stat4-expression-partial-prepared-handoff-fifth-continuation self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'application-sqlplanner-stat4-expression-partial-prepared-handoff-fifth-continuation',
    'applicationUse' => 'Copied wp_options plugin-admin pagination carries the next798-813 current-source STAT4 handoff into next814-829 only when projected current rows still match.',
    'status' => $plan['status'],
