...
$plan = SQLiteRowValueUpdateDeleteReturningWindowCurrentSourceNextPlan::executeRetryCommitWatermark(
    ['wp_options' => $rows],
...
if (($argv[1] ?? null) === '--self-test') {
    echo "application-rowvalue-returning-window-retry-commit-watermark self-test passed\n";
    return;
}
...
return [
    ...
    'applicationUse' => 'Copied wp_options imports can mark next-source retry RETURNING rows durable only after current-source row-value UPDATE/DELETE window chunks and retry cursor commit tokens are both acknowledged.',
