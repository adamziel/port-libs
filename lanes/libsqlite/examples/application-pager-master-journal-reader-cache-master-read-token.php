...
$summary = [
    'scenario' => 'application-pager-master-journal-reader-cache-master-read-token',
    'status' => $plan['status'],
    ...
    'applicationUse' => 'A copied wp_options rollback reader rejects cache pages pinned to an unlinked/recreated master-journal sidecar even when the member list text is unchanged.',
...
if ($summary['status'] !== 'pager-master-journal-reader-cache-current-source-next184'
    ...
    fwrite(STDERR, "application-pager-master-journal-reader-cache-master-read-token self-test failed\n");
    exit(1);
}

echo json_encode($summary, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) . "\n";
echo "application-pager-master-journal-reader-cache-master-read-token self-test passed\n";
