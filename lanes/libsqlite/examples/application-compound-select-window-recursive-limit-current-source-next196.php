...
$payload = [
    'scenario' => 'application-compound-select-window-recursive-limit-current-source-ntileFirstValueUnionDistinct',
    'applicationUse' => 'Copied wp_options preview queries can run recursive dependency queues, evaluate ntile and first_value windows before UNION distinct and final LIMIT, and reject stale current-source resume cursors before next-source rows are admitted.',
    'status' => $plan['status'],
...
if (($payload['status'] ?? null) !== 'compound-select-window-recursive-limit-current-source-ntileFirstValueUnionDistinct-ready') {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-ntileFirstValueUnionDistinct self-test failed\n");
    exit(1);
}
if ($payload['windowFunctions'] !== ['ntile', 'first_value']) {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-ntileFirstValueUnionDistinct window dispatch failed\n");
    exit(1);
}
if ($payload['currentTokenLength'] !== 64 || $payload['nextTokenLength'] !== 64) {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-ntileFirstValueUnionDistinct token guard failed\n");
    exit(1);
}
