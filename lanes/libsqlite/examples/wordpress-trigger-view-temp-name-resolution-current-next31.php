...
if (($argv[1] ?? null) === '--self-test') {
    if ($tempStage['targetSchema'] !== 'temp' || $tempMain['targetSchema'] !== 'main' || $tempStage['status'] !== 'resolved' || $tempMain['status'] !== 'resolved') {
        fwrite(STDERR, "wordpress-trigger-view-temp-name-resolution-current-next31 self-test failed\n");
        exit(1);
    }

    echo "wordpress-trigger-view-temp-name-resolution-current-next31 self-test passed\n";
    exit(0);
}
