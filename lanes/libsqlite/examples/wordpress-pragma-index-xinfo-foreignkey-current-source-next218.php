...
$page = SQLitePragmaIndexXinfoForeignKeyCurrentSourceNext218::page(
    $currentRecords,
    $nextRecords,
...
$summary = [
    'scenario' => 'wordpress-pragma-index-xinfo-foreignkey-current-source-next218',
    'wordpressUse' => 'Copied WordPress import schemas can flag DEFERRABLE foreign keys whose RESTRICT actions still block parent repairs immediately before commit.',
    'status' => $page['status'],
...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "wordpress-pragma-index-xinfo-foreignkey-current-source-next218 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-index-xinfo-foreignkey-current-source-next218 self-test passed\n");
    exit(0);
}
