...
$report = [
    'scenario' => 'wordpress-select-aggregate-current-next56',
    'wordpressUse' => 'Copied wp_options imports can compare current grouped row counts with next distinct option-value counts through parser-level SELECT aggregate text without requiring ext/sqlite.',
...
if (PHP_SAPI === 'cli' && realpath($_SERVER['SCRIPT_FILENAME'] ?? '') === __FILE__) {
    if ($report['autoloadGroups'] !== ['no', 'yes'] || $report['currentRows'] !== [3, 3] || $report['nextDistinctValues'] !== [2, 2]) {
        fwrite(STDERR, "wordpress-select-aggregate-current-next56 self-test failed\n");
        exit(1);
    }
    fwrite(STDOUT, "wordpress-select-aggregate-current-next56 self-test passed\n");
}
