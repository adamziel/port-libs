...
if (($argv[1] ?? '') === '--self-test') {
    ...
    foreach ($checks as $passed) {
        if (!$passed) {
            fwrite(STDERR, "application-pager-savepoint-master-journal-recovery-current-source-next130 self-test failed\n");
            exit(1);
        }
    }

    echo "application-pager-savepoint-master-journal-recovery-current-source-next130 self-test passed\n";
    exit(0);
}
