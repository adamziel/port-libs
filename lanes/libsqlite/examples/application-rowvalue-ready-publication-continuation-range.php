...
$plans = SQLiteRowValueUpdateDeleteReturningWindowCurrentSourceNextPlan::executeReadyPublicationRange(1086, 1101, ...$args);
...
$summary = [
    'status' => 'rowvalue-ready-publication-continuation-range',
    ...
    'applicationUse' => 'Copied wp_options imports validate a row-value UPDATE/DELETE RETURNING window current-source ready-publication continuation range as the direct handoff from next1085_ready.',
];

if (($argv[1] ?? null) === '--self-test') {
    echo "application-rowvalue-ready-publication-continuation-range self-test passed\n";
    return;
}
