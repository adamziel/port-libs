...
$summary = [
    'scenario' => 'application-rowvalue-order-expression-savepoint-current-source-next199',
    'applicationUse' => 'A copied wp_options cleanup can prioritize UPDATE/DELETE targets with row-value ORDER BY expressions, roll back a speculative RETURNING stream, and retry from the restored current source without exposing hidden order keys.',
    'status' => $plan['status'],
    'attemptSelected' => [
...
if (in_array('--self-test', $argv, true)) {
    if (
        ...
        fwrite(STDERR, "application-rowvalue-order-expression-savepoint-current-source-next199 self-test failed\n");
        exit(1);
    }

    echo "application-rowvalue-order-expression-savepoint-current-source-next199 self-test passed\n";
    exit(0);
}
