...
if (($argv[1] ?? '') === '--self-test') {
    if ($plan['status'] !== 'reparse-required' || $plan['generatedAdded'] !== ['option_value_len', 'option_bucket']) {
        fwrite(STDERR, "application-schema-generated-trigger-reparse-current-source self-test failed\n");
        exit(1);
    }
    echo "application-schema-generated-trigger-reparse-current-source self-test passed\n";
    exit(0);
}

echo json_encode([
    'scenario' => 'copied wp_options generated-column trigger reparse current source',
    'applicationUse' => 'Detect when migration DDL adds generated columns used by an existing wp_options audit trigger so the prepared trigger source is reparsed before the next import write.',
    'status' => $plan['status'],
