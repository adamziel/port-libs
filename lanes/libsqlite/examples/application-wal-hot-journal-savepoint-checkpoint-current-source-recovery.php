...
$summary = [
    'scenario' => 'application-wal-hot-journal-savepoint-checkpoint-current-source-next152',
    'applicationUse' => 'A copied Application options import first recovers an interrupted rollback journal, then rolls back a plugin-batch WAL savepoint and checkpoints only the retained current WAL prefix.',
    'status' => $plan['status'],
...
if ($summary['status'] !== 'wal-hot-journal-savepoint-checkpoint-current-source-next152'
    ...
    fwrite(STDERR, "application-wal-hot-journal-savepoint-checkpoint-current-source-next152 self-test failed\n");
    exit(1);
}

echo json_encode($summary, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) . "\n";
echo "application-wal-hot-journal-savepoint-checkpoint-current-source-next152 self-test passed\n";
