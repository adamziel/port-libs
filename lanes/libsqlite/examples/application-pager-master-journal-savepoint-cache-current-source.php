...
if (
    ...
    fwrite(STDERR, "application-pager-master-journal-savepoint-cache-current-source self-test failed\n");
    exit(1);
}

if (PHP_SAPI === 'cli' && realpath($_SERVER['SCRIPT_FILENAME'] ?? '') === __FILE__) {
    echo "application-pager-master-journal-savepoint-cache-current-source self-test passed\n";
}

return [
    'scenario' => 'application-pager-master-journal-savepoint-cache-current-source',
    'applicationUse' => 'Refresh copied Application SQLite pager cache pages after attached-database master-journal recovery and a retry inside a savepoint, so active_plugins and plugin settings reads use recovered current-source bytes instead of stale crashed cache entries.',
    'status' => $plan['status'],
