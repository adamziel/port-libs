...
$pageSize = 512;
$root = sys_get_temp_dir() . '/port-libsqlite-wordpress-vfs-rollback-commit-' . bin2hex(random_bytes(4));
...
echo json_encode([
    'scenario' => 'wordpress-vfs-rollback-commit-apply',
    'wordpressUse' => 'Apply rollback-journal commit ordering for copied wp_options imports through native PHP file handles: journal bytes are written and synced before dirty database pages, database pages are synced before journal deletion, and the directory entry is persisted without ext/sqlite.',
    'root' => $root,
