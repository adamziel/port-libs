...
if (in_array('--self-test', $argv, true)) {
    ...
    echo "application-sqlplanner-stat4-expression-partial-post-analyze-sample-window self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'application-sqlplanner-stat4-expression-partial-post-analyze-sample-window',
    'applicationUse' => 'Preview copied wp_options plugin scans after ANALYZE changes a partial lower(option_name) STAT4 sample window, blocking stale prepared rowids while admitting the new current plugin row without falling back to a table scan.',
    'status' => $plan['status'],
