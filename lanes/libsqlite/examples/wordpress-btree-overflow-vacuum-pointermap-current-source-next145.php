...
use PortLibs\LibSqlite\SQLiteBTreeOverflowVacuumPointerMapCurrentSourceNext145Plan;
...
$plan = SQLiteBTreeOverflowVacuumPointerMapCurrentSourceNext145Plan::fromCurrentSourceOverflowChains(
...
$summary = [
    'scenario' => 'wordpress-btree-overflow-vacuum-pointermap-current-source-next145',
    'wordpressUse' => 'After copied wp_options transient cleanup frees tail overflow pages, incremental vacuum truncates the tail and the next oversized option rewrite reuses only surviving free pages before appending fresh overflow pages with pointer-map ownership.',
...
if (($argv[1] ?? null) === '--self-test') {
    ...
    echo "wordpress-btree-overflow-vacuum-pointermap-current-source-next145 self-test passed\n";
    return;
}
