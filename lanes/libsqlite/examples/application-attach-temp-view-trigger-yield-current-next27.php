...
$catalog->attach('site', '/srv/site.sqlite', [
    ...
    $record('view', 'active_options', 'active_options', 0, 'CREATE VIEW site.active_options(blog_id, option_name, option_value) AS SELECT blog_id, option_name, option_value FROM wp_options', 11),
    $record('trigger', 'site_active_insert_yield', 'active_options', 0, "CREATE TRIGGER site_active_insert_yield INSTEAD OF INSERT ON active_options BEGIN INSERT INTO wp_options(blog_id, option_name, option_value) VALUES(new.blog_id, new.option_name, new.option_value); INSERT INTO site.wp_option_audit(blog_id, label, option_name) VALUES(new.blog_id, 'site-view', new.option_name); END", 12),
]);

$temp = SQLiteAttachTempViewTriggerYieldPlan::yield($catalog, 'temp_active_insert_yield', [
    'option_id' => 501,
    'temp_name' => 'plugin_import_preview',
    'option_value' => '{"dryRun":true}',
]);
$main = SQLiteAttachTempViewTriggerYieldPlan::yield($catalog, 'main.active_options_insert_yield', [
    'option_id' => 7,
    'option_name' => 'siteurl',
    'option_value' => 'https://example.test',
]);
$site = SQLiteAttachTempViewTriggerYieldPlan::yield($catalog, 'site.site_active_insert_yield', [
    'blog_id' => 3,
    'option_name' => 'home',
...
echo json_encode([
    'scenario' => 'application attach temp view trigger yielded operations',
    'applicationUse' => 'Preview copied Application option imports through TEMP, main, and attached INSTEAD OF view triggers, yielding schema-qualified writes with NEW row values before ext/sqlite is available.',
    'schemas' => [
        'temp' => $temp['writesBySchema'],
        'main' => $main['writesBySchema'],
        'site' => $site['writesBySchema'],
    ],
    'firstOperations' => [
        'temp' => $temp['operations'][0],
