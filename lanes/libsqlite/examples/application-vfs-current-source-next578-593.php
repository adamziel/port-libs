...
if (($argv[1] ?? '') === '--self-test') {
    ...
    assert($plan['events'][0]['ack'] === 'shared-cache-next577');
    assert($plan['events'][1]['status'] === 'claimed-reusable-current-source');
    assert($plan['events'][1]['claim'] === 'reader-reuse-next593');
    assert($plan['events'][1]['blocked_reasons'] === []);
    assert($plan['events'][2]['status'] === 'published-reused-current-source');
    assert($plan['events'][2]['reuse_ack'] === 'shared-cache-next577');
    assert($plan['events'][2]['token'] === 'shared-cache-next593');
    assert($plan['events'][2]['published_count'] === 29);
    assert(in_array('vfs-current-source-snapshot-reuse-publish-next578-593', $plan['dependencies'], true));
    ...
    echo "application-vfs-current-source-next578-593 self-test passed\n";
    return;
}
