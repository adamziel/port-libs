...
$plan = SQLiteAttachWalTempSchemaCacheCurrentSourceNext92Plan::currentSourceNext525540($schemas, $statements, [
...
if (($argv[1] ?? '') === '--self-test') {
    assert($plan['operation'] === 'attach-wal-temp-schema-cache-current-source-next525-540');
    assert($plan['dependencies'][0] === 'sqlite-attach-temp-wal-schema-cache-current-source-next525');
    assert($plan['dependencies'][15] === 'sqlite-attach-temp-wal-schema-cache-current-source-next540');
    assert(in_array('sqlite-attach-temp-wal-schema-cache-current-source-next524', $plan['dependencies'], true));
    ...
    echo "wordpress-attach-temp-wal-schema-cache-current-source-next525-540 self-test passed\n";
    return;
}
