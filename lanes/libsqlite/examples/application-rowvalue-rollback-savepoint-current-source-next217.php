...
$plan = SQLiteRowValueUpdateDeleteReturningSavepointPlan::executeRollbackToConflict(
    ['wp_options' => $rows],
    [
...
$summary = [
    'scenario' => 'application-rowvalue-rollback-savepoint-current-source-next217',
    'applicationUse' => 'A copied wp_options import savepoint hits UPDATE OR ROLLBACK on a row-value unique conflict, suppresses the failing RETURNING stream, restores the transaction image, and retries UPDATE/DELETE RETURNING from the restored current source.',
    'status' => $plan['status'],
...
if (
    ...
    fwrite(STDERR, "application-rowvalue-rollback-savepoint-current-source-next217 self-test failed\n");
    exit(1);
}
