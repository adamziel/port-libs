...
$summary = [
    'scenario' => 'application-vfs-shm-lock-filecontrol-current-source-next85',
    'status' => $plan['status'],
    ...
    'applicationUse' => 'Gate copied wp_options WAL-mode xFileControl persistence behind SHM write/checkpoint locks and current-source generations, so stale handles cannot persist controls after another writer advances the WAL-index source without requiring ext/sqlite.',
    'dependencies' => $plan['dependencies'],
];

if (($summary['controls']['persist_wal'] ?? null) !== true || ($summary['controls']['chunk_size'] ?? null) !== 8192) {
    fwrite(STDERR, "application-vfs-shm-lock-filecontrol-current-source-next85 self-test failed\n");
    exit(1);
}
