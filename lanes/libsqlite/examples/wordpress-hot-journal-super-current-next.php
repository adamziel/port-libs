...
$root = sys_get_temp_dir() . '/port-libsqlite-wordpress-hot-super-' . bin2hex(random_bytes(4));
...
echo json_encode([
    'scenario' => 'wordpress-hot-journal-super-current-next',
    'wordpressUse' => 'Recover copied WordPress attached-database images after an interrupted rollback-journal transaction only when the super-journal is present, restoring clean wp_options/wp_sitemeta pages and deleting the attached hot journals as one recovery boundary without ext/sqlite.',
    'root' => $root,
    'status' => $plan['status'],
