...
$plan = SQLiteVfsOpenLockFileControlCurrentSource::currentSourceNext99([
...
$payload = [
    'scenario' => 'wordpress-vfs-open-lock-filecontrol-current-source-next99',
    'wordpressUse' => 'Detect copied wp_options database handles whose current-source snapshot is stale after a sibling writer changes persistent VFS file-control state, without requiring ext/sqlite.',
...
if (PHP_SAPI === 'cli' && in_array('--self-test', $argv, true)) {
    ...
    echo "wordpress-vfs-open-lock-filecontrol-current-source-next99 self-test passed\n";
    return;
}
