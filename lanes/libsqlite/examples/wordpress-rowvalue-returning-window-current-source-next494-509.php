...
for ($next = 494; $next <= 509; $next++) {
    $plans[$next] = SQLiteRowValueUpdateDeleteReturningWindowCurrentSourceNextPlan::executeCurrentSourcePublicationHandoffStep($next, ...$args);
}
...
$summary = [
    ...
    'wordpressUse' => 'Copied wp_options imports validate the next494-509 row-value UPDATE/DELETE RETURNING window current-source continuation after merged next478-493 while preserving independent libsqlite throughput.',
];

if (($argv[1] ?? null) === '--self-test') {
    echo "wordpress-rowvalue-returning-window-current-source-next494-509 self-test passed\n";
    return;
}
