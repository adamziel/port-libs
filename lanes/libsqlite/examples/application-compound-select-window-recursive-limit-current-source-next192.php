...
$payload = [
    'scenario' => 'application-compound-select-window-recursive-limit-current-source-recursiveCurrentGap',
    'applicationUse' => 'Copied wp_options preview queries can combine recursive dependency queues with percent_rank/cume_dist window arms and reject stale current-source resume tokens before staged next-source autoload rows affect the final compound LIMIT.',
    'status' => $plan['status'],
...
if (($payload['status'] ?? null) !== 'compound-select-window-recursive-limit-current-source-recursiveCurrentGap-ready') {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-recursiveCurrentGap self-test failed\n");
    exit(1);
}
if ($payload['distributionFunctions'] !== ['percent_rank', 'cume_dist']) {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-recursiveCurrentGap distribution windows failed\n");
    exit(1);
}
if ($payload['currentTokenLength'] !== 64 || $payload['nextTokenLength'] !== 64) {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-recursiveCurrentGap token guard failed\n");
    exit(1);
}
