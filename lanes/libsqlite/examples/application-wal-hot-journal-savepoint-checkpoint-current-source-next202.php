...
$summary = [
    'scenario' => 'application-wal-hot-journal-savepoint-checkpoint-current-source-next202',
    'applicationUse' => 'A copied Application import reuses wp_options handles after hot-journal recovery, savepoint release, checkpoint publication, WAL sidecar sync, and directory sync while reopening stale readers.',
    'status' => $plan['status'],
...
if ($summary['status'] !== 'wal-hot-journal-savepoint-checkpoint-current-source-next202'
    ...
    fwrite(STDERR, "application-wal-hot-journal-savepoint-checkpoint-current-source-next202 self-test failed\n");
    exit(1);
}
