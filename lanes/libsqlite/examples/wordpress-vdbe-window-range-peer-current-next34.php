...
if (($argv[1] ?? '') === '--self-test') {
    ...
    if (array_column($summary, 'frame_rowids') !== $expectedFrames || array_column($summary, 'autoloaded_bytes') !== $expectedSums) {
        fwrite(STDERR, "wordpress VDBE window RANGE peer current/following smoke failed\n");
        exit(1);
    }

    echo "wordpress VDBE window RANGE peer current/following smoke passed\n";
    exit(0);
}

echo json_encode([
    'scenario' => 'wordpress-vdbe-window-range-peer-current-next34',
    'rows' => $summary,
