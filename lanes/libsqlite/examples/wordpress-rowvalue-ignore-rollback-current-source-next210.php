...
require_once __DIR__ . '/../src/SQLiteRowValueUpdateDeleteReturningSavepointCurrentSourceNext210Plan.php';
...
$plan = SQLiteRowValueUpdateDeleteReturningSavepointCurrentSourceNext210Plan::execute(
    ['wp_options' => $rows],
    [
...
$summary = [
    'scenario' => 'wordpress-rowvalue-ignore-rollback-current-source-next210',
    'wordpressUse' => 'A copied wp_options import savepoint attempts row-value UPDATE OR IGNORE cleanup, suppresses RETURNING for the conflicting siteurl row, rolls back the savepoint, then retries from the original current source.',
    'status' => $plan['status'],
...
if (
    ...
    fwrite(STDERR, "wordpress-rowvalue-ignore-rollback-current-source-next210 self-test failed\n");
    exit(1);
}
