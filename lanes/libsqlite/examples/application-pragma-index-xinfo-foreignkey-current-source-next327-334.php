...
$summary = [
    'scenario' => 'application-pragma-index-xinfo-foreignkey-current-source-next327-334',
    'applicationUse' => 'Application import previews can distinguish child lookup indexes that PRAGMA index_xinfo exposes as partial or expression-based, keeping FK action throughput blocked only for the affected current-source slice.',
...
if (($argv[1] ?? null) === '--self-test') {
    foreach (array_keys($summary) as $key) {
        if (str_starts_with($key, 'next') && $summary[$key] !== 1) {
            fwrite(STDERR, "application-pragma-index-xinfo-foreignkey-current-source-next327-334 self-test failed\n");
            exit(1);
        }
    }
    ...
    fwrite(STDOUT, "application-pragma-index-xinfo-foreignkey-current-source-next327-334 self-test passed\n");
    exit(0);
}
