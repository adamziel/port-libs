...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "application-rowvalue-returning-distinct-savepoint-current-source self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'application-rowvalue-returning-distinct-savepoint-current-source',
    'status' => $plan['status'],
    'savepoint' => $plan['savepoint'],
    'returning' => $plan['yielded_returning'],
    'currentOptionIds' => array_column($plan['current_source_tables']['wp_options'], 'option_id'),
    'applicationUse' => 'Copied wp_options cleanup/import batches can use row-value IS DISTINCT FROM and IS NOT DISTINCT FROM in UPDATE/DELETE RETURNING while a savepoint preserves the current source for rollback.',
