...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "application-stat4-partial-covering-order self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'application-stat4-partial-covering-order',
    'status' => $plan['status'],
    ...
    'applicationUse' => 'Copied wp_options plugin autoload scans can reprepare stale partial-covering STAT4 plans, keep option payload reads on the index cursor, and sort current/next rows by a dashboard freshness column without table lookups.',
