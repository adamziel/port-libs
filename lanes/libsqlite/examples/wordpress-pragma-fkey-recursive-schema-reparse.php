...
if (
    ...
    fwrite(STDERR, "wordpress-pragma-fkey-recursive-schema-reparse self-test failed\n");
    exit(1);
}

if (PHP_SAPI === 'cli' && ($_SERVER['argv'][1] ?? '') === '--self-test') {
    echo "wordpress-pragma-fkey-recursive-schema-reparse self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'wordpress-pragma-fkey-recursive-schema-reparse',
    'wordpressUse' => 'Previews copied WordPress option-dependency schema reparses where PRAGMA foreign_key_list keeps the current cursor stable and the next cursor exposes recursive self-references without ext/sqlite.',
