...
$records = [
    ...
    $record('table', 'wp_options', 'wp_options', 7, 'CREATE TABLE wp_options(option_id INTEGER PRIMARY KEY, blog_id TEXT CONSTRAINT "fk site" REFERENCES wp_sites(blog_id) DEFERRABLE INITIALLY DEFERRED, option_name TEXT, locale TEXT, fallback_name TEXT, CONSTRAINT fk_option_locale FOREIGN KEY(option_name, locale) REFERENCES wp_option_names(name, locale) DEFERRABLE INITIALLY IMMEDIATE, FOREIGN KEY(fallback_name) REFERENCES wp_defaults(default_name) NOT DEFERRABLE)', 4),
...
$nextRecords = [
    ...
    $records[1],
    $records[2],
    $record('table', 'wp_options', 'wp_options', 7, 'CREATE TABLE wp_options(option_id INTEGER PRIMARY KEY, blog_id TEXT CONSTRAINT fk_blog_site REFERENCES wp_sites(blog_id) DEFERRABLE INITIALLY IMMEDIATE, option_name TEXT, locale TEXT, fallback_name TEXT, CONSTRAINT "fk option locale next" FOREIGN KEY(option_name, locale) REFERENCES wp_option_names(name, locale) DEFERRABLE INITIALLY DEFERRED, CONSTRAINT [fk fallback default] FOREIGN KEY(fallback_name) REFERENCES wp_defaults(default_name) NOT DEFERRABLE)', 4),
    $records[4],
    $records[5],
    $records[6],
];
...
$page = SQLitePragmaIndexXinfoForeignKeyCurrentSourceNext::currentNextPageFromCatalog176(
    $records,
    $currentTables,
    $nextRecords,
    $nextTables,
...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "wordpress-pragma-index-xinfo-foreignkey-current-source-next176 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-index-xinfo-foreignkey-current-source-next176 self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'wordpress-pragma-index-xinfo-foreignkey-current-source-next176',
    'wordpressUse' => 'Carry named foreign-key constraints through copied wp_options PRAGMA index_xinfo/foreign_key_check diagnostics so migration tools can report the schema name that changed or blocked an import.',
    'status' => $page['status'],
