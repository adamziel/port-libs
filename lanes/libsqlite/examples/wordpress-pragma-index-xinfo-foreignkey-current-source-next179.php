...
$page = SQLitePragmaIndexXinfoForeignKeyCurrentSourceNext179::currentNextPageFromCatalog(
    $records,
    $currentTables,
    $nextRecords,
    $nextTables,
...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "wordpress-pragma-index-xinfo-foreignkey-current-source-next179 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-index-xinfo-foreignkey-current-source-next179 self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'wordpress-pragma-index-xinfo-foreignkey-current-source-next179',
    'wordpressUse' => 'Carry single-quoted named foreign-key constraints through copied wp_options PRAGMA index_xinfo/foreign_key_check diagnostics so migration tools can report the schema name that changed or blocked an import.',
    'status' => $page['status'],
