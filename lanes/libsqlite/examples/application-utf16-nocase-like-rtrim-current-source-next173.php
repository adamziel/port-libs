...
$summary = SQLiteUtf16NocaseLikeRtrimCurrentSourceNextPlan::keyValueRowKeySourcePlan(
    [
        $row(1, 'Module_Cache  ', 2),
        $row(2, 'module_cache', 3),
        $row(3, "module_cache\t", 2),
        $row(4, 'module_cache' . "\xc2\xa0", 3),
    ],
    [
        $row(1, 'Module_Cache', 3),
        $row(2, 'module_cache   ', 3),
        $row(3, "module_cache\t", 2),
        $row(5, 'MODULE_CACHE_NEW  ', 2),
    ],
    'module!_cache',
    '!',
    'main.app_settings@172',
    'main.app_settings@173',
    172,
    173,
);
...
$payload = [
    ...
    'applicationUse' => 'Application app_settings cursors over UTF-16 rtrim(key_name) COLLATE NOCASE LIKE probes can distinguish byte-only trailing-space changes from semantic rowset changes before resuming a current-source cursor.',
    'dependency_closure' => $summary['dependency_closure'],
];

if (PHP_SAPI === 'cli' && basename(__FILE__) === basename($_SERVER['SCRIPT_FILENAME'] ?? '')) {
    if ($payload['status'] !== 'utf16-nocase-like-rtrim-current-source-nextoneSevenThree'
