...
$target = sys_get_temp_dir() . '/wordpress-vacuum-into-autovacuum-edge.sqlite';
@unlink($target);

$plan = SQLiteVacuumBackupSerializePlan::vacuumInto($database, $target, false, 512, 'incremental');

echo json_encode([
    'scenario' => 'wordpress-vacuum-into-autovacuum-page-edge',
    'wordpressUse' => 'Preview VACUUM INTO for a copied WordPress SQLite database where enabling incremental auto_vacuum creates pointer-map pages at page 2 and the next 512-byte stride edge without ext/sqlite.',
