...
$summary = [
    'scenario' => 'wordpress-wal-checkpoint-savepoint-reader-current-source-next90',
    'wordpressUse' => 'A copied wp_options import rolls back a failed plugin-settings SAVEPOINT while a reader remains pinned; repair tooling can verify the exact current WAL frame source before deciding that the checkpoint must preserve the retained WAL prefix for current and next readers.',
    'status' => $plan['status'],
...
if (in_array('--self-test', $argv, true)) {
    if ($summary['status'] !== 'busy' || $summary['checkpointReason'] !== 'reader_blocks_wal_reset') {
        fwrite(STDERR, "wordpress-wal-checkpoint-savepoint-reader-current-source-next90 status failed\n");
        exit(1);
    }
    if ($summary['walAction'] !== 'preserve_wal' || $summary['retainedSourceFrameCount'] !== 2) {
        fwrite(STDERR, "wordpress-wal-checkpoint-savepoint-reader-current-source-next90 WAL retention failed\n");
        exit(1);
    }
    if ($summary['commitFrameIndexes'] !== [2, 5, 6] || $summary['rolledBackFrameIndexes'] !== [3, 4, 5, 6]) {
        fwrite(STDERR, "wordpress-wal-checkpoint-savepoint-reader-current-source-next90 frame summary failed\n");
        exit(1);
    }
}
