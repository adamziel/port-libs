...
if (($argv[1] ?? null) === '--self-test') {
    if ($page['status'] !== 'ok' || $page['current']['foreign_key_violations'] !== 3 || $page['next_counts']['foreign_key_violations'] !== 0 || ($page['rows'][4]['action_summary'] ?? null) !== 'SET DEFAULT/CASCADE/CUSTOM') {
        fwrite(STDERR, "application-pragma-index-xinfo-foreignkey-current-source-next165 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "application-pragma-index-xinfo-foreignkey-current-source-next165 self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'application-pragma-index-xinfo-foreignkey-current-source-next165',
    'applicationUse' => 'Resume copied multisite wp_options imports only after PRAGMA index_xinfo and foreign_key_check preserve the action/match metadata that decides repair policy.',
    'status' => $page['status'],
