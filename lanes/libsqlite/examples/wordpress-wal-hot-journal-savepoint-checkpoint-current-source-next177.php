...
$plan = SQLiteWalHotJournalSavepointCheckpointCurrentSourceNext177Plan::plan($resume);

$summary = [
    'scenario' => 'wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next177',
    'wordpressUse' => 'A copied WordPress plugin import resumes after a crash between hot-journal recovery and checkpoint publication, rewriting the missing current-source database image before deleting the hot journal.',
    'status' => $plan['status'],
...
if ($summary['status'] !== 'wal-hot-journal-savepoint-checkpoint-current-source-next177'
    ...
    fwrite(STDERR, "wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next177 self-test failed\n");
    exit(1);
}
