...
$summary = [
    'scenario' => 'wordpress-pragma-index-xinfo-foreignkey-current-source-next212',
    'wordpressUse' => 'Copied WordPress postmeta imports can distinguish partial child indexes from full child-action lookup indexes before applying ON DELETE/ON UPDATE foreign-key actions.',
    'status' => $page['status'],
...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "wordpress-pragma-index-xinfo-foreignkey-current-source-next212 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-index-xinfo-foreignkey-current-source-next212 self-test passed\n");
    exit(0);
}
