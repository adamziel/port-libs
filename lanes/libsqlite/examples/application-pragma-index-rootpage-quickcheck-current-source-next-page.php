...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "application-pragma-index-rootpage-quickcheck-current-source-next-page self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "application-pragma-index-rootpage-quickcheck-current-source-next-page self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'application-pragma-index-rootpage-quickcheck-current-source-next-page',
    'applicationUse' => 'Resume copied wp_options expression-index metadata together with bounded PRAGMA quick_check rootpage blockers from the same current database image.',
    'status' => $page['status'],
