...
$payload = [
    'scenario' => 'application-compound-select-window-recursive-limit-current-source-next225',
    'applicationUse' => 'Copied wp_options preview queries can fence lag() default rows and last_value() frame rows after recursive queue LIMIT/OFFSET, INTERSECT, EXCEPT, and final LIMIT admit staged next-source plugin rows.',
    'status' => $plan['status'],
...
if (($payload['status'] ?? null) !== 'compound-select-window-recursive-limit-current-source-next225-ready') {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-next225 self-test failed\n");
    exit(1);
}
if ($payload['currentTokenLength'] !== 64 || $payload['nextTokenLength'] !== 64) {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-next225 token guard failed\n");
    exit(1);
}
if (!in_array('plugin_prime', $payload['nextOnlyRows'], true)) {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-next225 next-source boundary failed\n");
    exit(1);
}
