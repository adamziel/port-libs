...
$summary = [
    'scenario' => 'wordpress-pager-savepoint-master-journal-reader-current-source-next146',
    'status' => $plan['status'],
    ...
    'wordpressUse' => 'A copied wp_options import can reject a pinned reader that predates master-journal recovery and admit the reopened reader after ROLLBACK TO restores the plugin option before-image.',
...
if ($summary['status'] !== 'pager-savepoint-master-journal-reader-current-source-next146'
    ...
    fwrite(STDERR, "wordpress-pager-savepoint-master-journal-reader-current-source-next146 self-test failed\n");
    exit(1);
}

echo "wordpress-pager-savepoint-master-journal-reader-current-source-next146 self-test passed\n";
