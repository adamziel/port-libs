...
use PortLibs\LibSqlite\SQLitePragmaIndexXinfoForeignKeyCurrentSourceNext186;
...
$page = SQLitePragmaIndexXinfoForeignKeyCurrentSourceNext186::currentNextPageFromCatalog(
    $records,
    $tables,
    $nextRecords,
    $tables,
...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "wordpress-pragma-index-xinfo-foreignkey-current-source-next186 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-index-xinfo-foreignkey-current-source-next186 self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'wordpress-pragma-index-xinfo-foreignkey-current-source-next186',
    'wordpressUse' => 'Surface child-key index collation mismatches beside PRAGMA index_xinfo and foreign_key_list rows before a WordPress taxonomy/post import relies on those indexes.',
    'status' => $page['status'],
