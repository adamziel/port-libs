...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "application-wal-checkpoint-hot-journal-truncate-current-source-next138 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'application-wal-checkpoint-hot-journal-truncate-current-source-next138',
    'applicationUse' => 'Recover an interrupted copied wp_options rollback journal before applying a savepoint-truncated WAL checkpoint, so the follow-up TRUNCATE checkpoint writes clean recovered database pages and removes the WAL sidecar only after the pinned reader drains.',
    'status' => $plan['status'],
