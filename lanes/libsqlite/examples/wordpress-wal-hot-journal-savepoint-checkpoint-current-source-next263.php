...
$common = [
    ...
    'database_digest' => $digest('wordpress next263 checkpoint database image'),
    'page_cache_digest' => $digest('wordpress next263 clean page cache'),
...
if (($argv[1] ?? null) === '--self-test') {
    ...
    echo "wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next263 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next263',
    'wordpressUse' => 'A copied WordPress import closes retry readers only after the next262 cache fence proves no hot-journal or stale WAL generation is visible.',
    'status' => $plan['status'],
