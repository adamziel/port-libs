...
$summary = [
    'scenario' => 'application-wal-hot-journal-savepoint-checkpoint-current-source-next219',
    'applicationUse' => 'A copied Application plugin import publishes the checkpoint as the next current source only after hot-journal savepoint scopes are released, page digests are recorded, and stale readers remain fenced for reopen.',
    'status' => $plan['status'],
...
if ($summary['status'] !== 'wal-hot-journal-savepoint-checkpoint-current-source-next219'
    ...
    fwrite(STDERR, "application-wal-hot-journal-savepoint-checkpoint-current-source-next219 self-test failed\n");
    exit(1);
}
