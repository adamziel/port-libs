...
$next221 = [
    ...
    'shm_path' => '/srv/www/wp-content/database/application.sqlite-shm',
    'next_source_token' => [
        'id' => 'application-next222-sidecar-retired-source',
        'epoch' => 222,
...
$ticket = static fn (string $name, string $kind): array => [
    'name' => $name,
    'kind' => $kind,
    'source_id' => 'application-next222-sidecar-retired-source',
    'epoch' => 222,
    'checkpoint_frame' => 222,
    'checkpoint_cookie' => 222222,
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "application-wal-source-ticket self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'application-wal-source-ticket',
    'status' => $plan['status'],
    'sourceTicketSealed' => $plan['source_ticket_sealed'],
    'currentSourceTicketAction' => $plan['current_source_ticket_action'],
    'applicationUse' => 'A copied wp_options import seals the current-source ticket only after database, WAL, journal, and SHM sidecar receipts match the retired checkpoint source.',
