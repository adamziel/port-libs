...
$recoveredDigest = static function (array $pages) use ($pageSize): string {
    ...
    foreach ($pages as $number => $image) {
        if (strlen($image) !== $pageSize) {
            throw new RuntimeException('bad Application page fixture');
        }
...
if ($plan['retained_cache_page_numbers'] !== [1, 3]) {
    throw new RuntimeException('Application retained reader-cache pages changed unexpectedly');
}
if ($plan['refreshed_cache_page_numbers'] !== [2]) {
    throw new RuntimeException('Application alloptions page did not refresh from recovered source');
}
...
echo "application-pager-master-journal-reader-cache-current-source-next202 self-test passed\n";
