...
$plan = SQLiteBTreeVacuumPointerMapFreeblockCurrentSourcePlan::tableLeafSourceBoundaryFromDeleteResult(
    $database,
    3,
    [
        'page' => $deletedPage,
    ...
    4,
    str_repeat('application-next176-current-source-boundary-', 46),
    3,
    true,
);

$summary = [
    'application_scenario' => 'wp_options transient delete/vacuum/rewrite uses current-source freeblock and overflow boundary',
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    if ($summary !== $expected) {
        fwrite(STDERR, "application-btree-vacuum-pointermap-freeblock-current-source-next176 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "application-btree-vacuum-pointermap-freeblock-current-source-next176 self-test passed\n");
    exit(0);
}
