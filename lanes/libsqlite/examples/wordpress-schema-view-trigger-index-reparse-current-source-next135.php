...
$summary = [
    'scenario' => 'wordpress-schema-view-trigger-index-reparse-current-source-next135',
    'wordpressUse' => 'Copied wp_options migration triggers that read generated-column indexes or generated-column views are marked as current-source reparse points before stale import statements resume.',
    'status' => $plan['status'],
...
if (PHP_SAPI === 'cli' && in_array('--self-test', $argv, true)) {
    if (
        ...
        fwrite(STDERR, "wordpress-schema-view-trigger-index-reparse-current-source-next135 self-test failed\n");
        exit(1);
    }

    echo "wordpress-schema-view-trigger-index-reparse-current-source-next135 self-test passed\n";
    exit(0);
}
