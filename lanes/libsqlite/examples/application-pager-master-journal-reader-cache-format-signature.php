...
$summary = [
    'scenario' => 'application-pager-master-journal-reader-cache-format-signature',
    'status' => $plan['status'],
    ...
    'applicationUse' => 'A copied wp_options database refreshes reader-cache pages after master-journal recovery when the page-1 format ticket changes reserved bytes, text encoding, user_version, or application_id.',
...
if ($summary['status'] !== 'pager-master-journal-reader-cache-current-source-next180'
    ...
    fwrite(STDERR, "application-pager-master-journal-reader-cache-format-signature self-test failed\n");
    exit(1);
}

echo "application-pager-master-journal-reader-cache-format-signature self-test passed\n";
