...
$summary = [
    'scenario' => 'wordpress-pager-statement-journal-wal-savepoint-current-source-next112',
    'status' => $plan['status'],
    ...
    'wordpressUse' => 'During a copied wp_options WAL import, a failed statement can be rolled back from its statement journal while the outer savepoint WAL frames remain retained. The retry appends frames after the retained savepoint prefix instead of building on failed statement frames.',
];
...
if (
    ...
    fwrite(STDERR, "wordpress-pager-statement-journal-wal-savepoint-current-source-next112 self-test failed\n");
    exit(1);
}
