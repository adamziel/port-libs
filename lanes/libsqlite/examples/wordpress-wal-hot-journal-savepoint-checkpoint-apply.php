...
$root = sys_get_temp_dir() . '/port-libsqlite-wordpress-savepoint-checkpoint-apply-' . bin2hex(random_bytes(4));
...
if (!is_dir(dirname($localDatabase)) && !mkdir(dirname($localDatabase), 0777, true) && !is_dir(dirname($localDatabase))) {
    throw new RuntimeException('Unable to create WordPress WAL savepoint-checkpoint-apply fixture directory');
}
...
$summary = [
    'scenario' => 'wordpress-wal-hot-journal-savepoint-checkpoint-savepoint-checkpoint-apply',
...
if (($argv[1] ?? '') === '--self-test') {
    if ($summary['status'] !== 'wal-hot-journal-savepoint-checkpoint-savepoint-checkpoint-apply'
        ...
        fwrite(STDERR, "wordpress-wal-hot-journal-savepoint-checkpoint-savepoint-checkpoint-apply self-test failed\n");
        exit(1);
    }
    echo "wordpress-wal-hot-journal-savepoint-checkpoint-savepoint-checkpoint-apply self-test passed\n";
    exit(0);
}
