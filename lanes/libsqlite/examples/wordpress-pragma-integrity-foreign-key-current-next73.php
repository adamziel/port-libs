...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "wordpress-pragma-integrity-foreign-key-current-next73 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-integrity-foreign-key-current-next73 self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'wordpress temp wp_options targeted foreign_key_check current/next pagination',
    'page0' => [
        'status' => $page0['status'],
        'count' => $page0['count'],
        'total' => $page0['total'],
