...
$page = SQLitePragmaIndexXinfoForeignKeyCurrentSourceNext166::currentNextPageFromCatalog(
    $currentRecords,
    $currentTables,
    $nextRecords,
    $nextTables,
...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "wordpress-pragma-index-xinfo-foreignkey-current-source-next166 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-index-xinfo-foreignkey-current-source-next166 self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'wordpress-pragma-index-xinfo-foreignkey-current-source-next166',
    'wordpressUse' => 'Copied multisite wp_options imports can resume PRAGMA index_xinfo plus foreign_key_check only when FK action, MATCH, and deferrability metadata still match the current catalog image.',
    'status' => $page['status'],
