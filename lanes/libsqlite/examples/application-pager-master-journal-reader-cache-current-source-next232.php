...
$summary = [
    'scenario' => 'application-pager-master-journal-reader-cache-current-source-next232',
    'applicationUse' => 'A copied Application main database keeps schema/options reader-cache pages only when the cache row belongs to the main database path namespace; an image-identical active_plugins page from an attached users database reopens.',
    'status' => $plan['status'],
...
if ($summary['status'] !== 'pager-master-journal-reader-cache-current-source-next232'
    ...
    fwrite(STDERR, "application-pager-master-journal-reader-cache-current-source-next232 self-test failed\n");
    exit(1);
}

echo json_encode($summary, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) . "\n";
echo "application-pager-master-journal-reader-cache-current-source-next232 self-test passed\n";
