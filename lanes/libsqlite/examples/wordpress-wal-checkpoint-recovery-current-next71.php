...
$summary = [
    'scenario' => 'wordpress-wal-checkpoint-recovery-current-next71',
    'wordpressUse' => 'Recover copied wp_options checkpoint state when a crash leaves durable database pages beside an old WAL containing a valid uncommitted tail and a corrupt frame; current readers keep their prefix snapshot while the next opener checkpoints only the committed prefix.',
    'status' => $plan['status'],
...
if (PHP_SAPI === 'cli' && in_array('--self-test', $argv, true)) {
    ...
    foreach ($expected as $key => $value) {
        if ($summary[$key] !== $value) {
            fwrite(STDERR, "wordpress-wal-checkpoint-recovery-current-next71 self-test failed for {$key}\n");
            exit(1);
        }
    }

    fwrite(STDOUT, "wordpress-wal-checkpoint-recovery-current-next71 self-test passed\n");
    exit(0);
}
