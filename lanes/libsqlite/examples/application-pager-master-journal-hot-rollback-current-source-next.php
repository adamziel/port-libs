...
$pageSize = 512;
$sectorSize = 512;
$root = sys_get_temp_dir() . '/sqlite-application-master-hot-89-' . bin2hex(random_bytes(4));
...
if (in_array('--self-test', $argv, true)) {
    foreach ([
        ...
        if (!$passed) {
            $removeTree($root);
            throw new RuntimeException('Application pager master-journal hot rollback current-source smoke failed');
        }
    }
}
