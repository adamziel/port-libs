...
$pageSize = 512;
$database = '/srv/wp-content/database/wp-options-schema-lock-fence.sqlite';
$usersDatabase = '/srv/wp-content/database/wp-users-schema-lock-fence.sqlite';
...
$base = [
    'source_id' => 'application-pager-reader-cache-schema-lock-fence',
    'epoch' => 318,
    'format_signature' => hash('sha256', implode('|', [512, 4, 2, 318, 0])),
    'publication_generation' => 318,
    'master_source_digest' => hash('sha256', 'application schema-lock-fence master source'),
...
$plan = SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::currentSourceReaderCacheSchemaLockFence(
    $database,
    $master,
    $masterBytes,
    implode('', $before),
...
if (in_array('--self-test', $argv, true)) {
    if ($plan['status'] !== 'pager-master-journal-reader-cache-current-source-schema-lock-fence') {
        fwrite(STDERR, "unexpected pager reader-cache schema-lock-fence status\n");
        exit(1);
    }
    ...
    echo "application-pager-master-journal-reader-cache-current-source-schema-lock-fence self-test passed\n";
    return;
}
