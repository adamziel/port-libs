...
$plan = SQLiteRowValueUpdateDeleteReturningSavepointPlan::executeConflictRetrySavepointBatch(
    ['wp_options' => $rows],
    [
...
$summary = [
    'scenario' => 'application-rowvalue-fail-rollback-retry-savepoint',
    'applicationUse' => 'A copied wp_options import can yield one UPDATE OR FAIL RETURNING row, hit a later row-value unique conflict, roll back to the import savepoint, and retry UPDATE/DELETE RETURNING against the restored current source.',
    'status' => $plan['status'],
...
if (
    ...
    fwrite(STDERR, "application-rowvalue-fail-rollback-retry-savepoint self-test failed\n");
    exit(1);
}

if (PHP_SAPI === 'cli' && realpath($_SERVER['SCRIPT_FILENAME'] ?? '') === __FILE__) {
    echo "application-rowvalue-fail-rollback-retry-savepoint self-test passed\n";
