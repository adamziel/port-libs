...
$summary = [
    'scenario' => 'application-wal-hot-journal-savepoint-checkpoint-current-source-next154',
    'status' => $plan['status'],
    'statusFamily' => $plan['status_family'],
    'applicationUse' => 'A copied Application plugin import recovers a hot rollback journal, rolls back the failed savepoint tail, then checkpoints only pages visible at the savepoint WAL frame boundary before resetting the current source.',
...
if ($summary['status'] !== 'wal-hot-journal-savepoint-checkpoint-current-source-next154'
    ...
    fwrite(STDERR, "application-wal-hot-journal-savepoint-checkpoint-current-source-next154 self-test failed\n");
    exit(1);
}

echo "application-wal-hot-journal-savepoint-checkpoint-current-source-next154 self-test passed\n";
