...
use PortLibs\LibSqlite\SQLitePragmaIndexXinfoForeignKeyCurrentSourceNext194;
...
$page = SQLitePragmaIndexXinfoForeignKeyCurrentSourceNext194::currentNextPageFromCatalog(
    $currentRecords,
    $currentTables,
    $nextRecords,
    $nextTables,
...
$summary = [
    'scenario' => 'wordpress-pragma-index-xinfo-foreignkey-current-source-next194',
    'wordpressUse' => 'Copied wp_options imports may have an autoload-only partial child index; PRAGMA foreign_key_check should still rely on table data while reporting that the child helper index is diagnostic only.',
    'status' => $page['status'],
...
if (($argv[1] ?? '') === '--self-test') {
    if (
        ...
        fwrite(STDERR, "wordpress-pragma-index-xinfo-foreignkey-current-source-next194 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-index-xinfo-foreignkey-current-source-next194 self-test passed\n");
    exit(0);
}
