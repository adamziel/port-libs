...
$summary = [
    'scenario' => 'application-wal-savepoint-restart-reader-current-source-next103',
    'applicationUse' => 'Model a failed plugin import savepoint whose retained WAL prefix is verified, checkpoint-restarted after the current reader releases, and followed by a retry append visible only to the next reader.',
    'status' => $plan['status'],
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "application-wal-savepoint-restart-reader-current-source-next103 self-test passed\n";
    return;
}
