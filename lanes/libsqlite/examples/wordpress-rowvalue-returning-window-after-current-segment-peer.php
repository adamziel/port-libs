...
$argv = [];
$next261 = require __DIR__ . '/wordpress-rowvalue-returning-window-source-segment-watermark.php';

ob_start();
$argv = [];
require __DIR__ . '/wordpress-rowvalue-returning-window-peer-group-admission.php';
...
assert($statuses[1] === 'wordpress-rowvalue-returning-window-peer-group-admission self-test passed');
...
if ($selfTest) {
    if ($statuses !== [
        ...
        fwrite(STDERR, "wordpress-rowvalue-returning-window-after-current-segment-peer self-test failed\n");
        exit(1);
    }

    echo "wordpress-rowvalue-returning-window-after-current-segment-peer self-test passed\n";
    return;
}
