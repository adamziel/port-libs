...
$currentRows = [
    ...
    ['option_id' => 2, 'option_name_bytes' => "p\x00l\x00u\x00g\x00i\x00n\x00_", 'text_encoding' => 2],
    ['option_id' => 3, 'option_name_bytes' => "\x00p\x00l\x00u\x00g\x00i\x00n\x00_\xd8\x3d", 'text_encoding' => 3],
...
$plan = SQLiteMalformedLikeGlobSourceNextPlan::wordpressOptionNameCurrentNext(
    $currentRows,
    $nextRows,
    'plugin%',
    'LIKE',
    ...
    null,
    false,
    'main.wp_options@cookie96',
    'main.wp_options@cookie97',
);

$output = [
    'scenario' => 'wordpress malformed UTF-16 LIKE range current-source next97',
    ...
    'wordpressUse' => 'Copied wp_options scans can advance a LIKE range over valid UTF-16 option_name index entries while quarantining odd-length and unpaired-surrogate UTF-16 text from the current/next source switch.',
];
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "wordpress-malformed-utf16-like-range-current-source-next97 self-test passed\n";
    exit(0);
}
