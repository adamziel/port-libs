...
require_once __DIR__ . '/../src/SQLiteVfsLockUriTempFileControlCurrentSourceNextPlan.php';

use PortLibs\LibSqlite\SQLiteVfsLockUriTempFileControlCurrentSourceNextPlan;

$plan = SQLiteVfsLockUriTempFileControlCurrentSourceNextPlan::run([
...
$summary = [
    'scenario' => 'application-vfs-lock-uri-temp-filecontrol-current-source-next137',
    'applicationUse' => 'Model a Application import that opens a URI main database and memory/temp staging database, changes the connection temp directory through file-control, reads URI file-control values, and proves temp locks/delete-on-close are current-source scoped.',
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "application-vfs-lock-uri-temp-filecontrol-current-source-next137 self-test passed\n";
    return;
}
