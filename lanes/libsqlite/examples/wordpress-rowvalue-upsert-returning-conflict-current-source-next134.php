...
$summary = [
    'scenario' => 'wordpress-rowvalue-upsert-returning-conflict-current-source-next134',
    'wordpressUse' => 'Preview copied wp_options imports where row-value UPSERT conflict policies decide current-source updates, skipped DO UPDATE WHERE rows, DO NOTHING conflicts, and statement-order RETURNING rows.',
    'status' => $plan['status'],
...
if (PHP_SAPI === 'cli' && realpath($_SERVER['SCRIPT_FILENAME'] ?? '') === __FILE__) {
    ...
    echo "wordpress-rowvalue-upsert-returning-conflict-current-source-next134 self-test passed\n";
}
