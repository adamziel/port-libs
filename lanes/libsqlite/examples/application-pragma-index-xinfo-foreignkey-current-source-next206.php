...
$summary = [
    'scenario' => 'application-pragma-index-xinfo-foreignkey-current-source-next206',
    'applicationUse' => 'Copied wp_options-style term imports can keep INTEGER PRIMARY KEY parent references countable as valid FK parent keys even though PRAGMA index_list has no parent index row for the rowid alias.',
    'status' => $page['status'],
...
if (($argv[1] ?? '') === '--self-test') {
    if (
        ...
        fwrite(STDERR, "application-pragma-index-xinfo-foreignkey-current-source-next206 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "application-pragma-index-xinfo-foreignkey-current-source-next206 self-test passed\n");
    exit(0);
}
