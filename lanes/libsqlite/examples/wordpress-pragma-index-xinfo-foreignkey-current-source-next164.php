...
if (($argv[1] ?? null) === '--self-test') {
    if ($page['status'] !== 'ok' || $page['current']['foreign_key_violations'] !== 3 || $page['next_counts']['foreign_key_violations'] !== 0 || $page['delta']['cleared'] !== true) {
        fwrite(STDERR, "wordpress-pragma-index-xinfo-foreignkey-current-source-next164 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-index-xinfo-foreignkey-current-source-next164 self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'wordpress-pragma-index-xinfo-foreignkey-current-source-next164',
    'wordpressUse' => 'Admit copied wp_options PRAGMA foreign_key_check/index_xinfo diagnostics when schema identifiers and imported row arrays differ only by table or column case.',
    'status' => $page['status'],
