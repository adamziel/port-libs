...
require_once __DIR__ . '/../src/SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNext159Plan.php';

use PortLibs\LibSqlite\SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNext159Plan;
...
$plan = SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNext159Plan::tableLeafFromDeleteResult(
    $database,
    3,
    [
        'page' => $deletedPage,
...
$summary = [
    'scenario' => 'wordpress-btree-vacuum-pointermap-freeblock-current-source-next159',
    'wordpressUse' => 'Delete an obsolete copied wp_options transient, vacuum tail overflow pages, then reuse the surviving freelist leaf and trunk as a multi-page replacement overflow chain with corrected auto-vacuum pointer-map ownership.',
...
if (($argv[1] ?? null) === '--self-test') {
    ...
    foreach ($expected as $key => $value) {
        if ($summary[$key] !== $value) {
            fwrite(STDERR, "wordpress-btree-vacuum-pointermap-freeblock-current-source-next159 self-test failed\n");
            exit(1);
        }
    }

    fwrite(STDOUT, "wordpress-btree-vacuum-pointermap-freeblock-current-source-next159 self-test passed\n");
    exit(0);
}
