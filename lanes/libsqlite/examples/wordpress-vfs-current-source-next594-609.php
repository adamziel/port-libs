...
require_once __DIR__ . '/../src/SQLiteVfsCurrentSourceNext594609Plan.php';
...
$plan = SQLiteVfsCurrentSourceNext594609Plan::run([
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    assert($plan['events'][0]['ack'] === 'shared-cache-next593');
    assert($plan['events'][1]['status'] === 'claimed-reusable-current-source');
    assert($plan['events'][1]['claim'] === 'reader-reuse-next609');
    assert($plan['events'][1]['blocked_reasons'] === []);
    assert($plan['events'][2]['status'] === 'published-reused-current-source');
    assert($plan['events'][2]['reuse_ack'] === 'shared-cache-next593');
    assert($plan['events'][2]['token'] === 'shared-cache-next609');
    assert($plan['events'][2]['published_count'] === 30);
    ...
    echo "wordpress-vfs-current-source-next594-609 self-test passed\n";
    return;
}
