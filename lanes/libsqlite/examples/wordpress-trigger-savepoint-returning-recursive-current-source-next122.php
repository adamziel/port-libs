...
require __DIR__ . '/../src/SQLiteTriggerSavepointReturningRecursiveCurrentSourceNext122Plan.php';

use PortLibs\LibSqlite\SQLiteTriggerSavepointReturningRecursiveCurrentSourceNext122Plan;

$plan = SQLiteTriggerSavepointReturningRecursiveCurrentSourceNext122Plan::execute(
    [
...
if (in_array('--self-test', $argv, true)) {
    if (
        ...
        fwrite(STDERR, "wordpress-trigger-savepoint-returning-recursive-current-source-next122 self-test failed\n");
        exit(1);
    }

    echo "wordpress-trigger-savepoint-returning-recursive-current-source-next122 self-test passed\n";
}

echo json_encode([
    'scenario' => 'wordpress-trigger-savepoint-returning-recursive-current-source-next122',
    'wordpressUse' => 'Preview a copied wp_options import where recursive triggers produce RETURNING diagnostics from current and next sources, then ROLLBACK TO restores the savepoint image while retaining attempted RETURNING evidence for importer diagnostics.',
