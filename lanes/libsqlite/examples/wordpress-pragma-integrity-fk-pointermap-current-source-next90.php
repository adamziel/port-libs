...
$summary = [
    ...
    'wordpressUse' => 'Resume paged PRAGMA integrity_check plus table-valued pragma_foreign_key_check diagnostics for copied WordPress SQLite archives only when database bytes, schema rows, and SQL source are unchanged.',
...
if (in_array('--self-test', $argv, true)) {
    if (
        ...
        fwrite(STDERR, "wordpress-pragma-integrity-fk-pointermap-current-source-next90 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-integrity-fk-pointermap-current-source-next90 self-test passed\n");
    return;
}
