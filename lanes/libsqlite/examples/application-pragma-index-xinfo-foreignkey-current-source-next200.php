...
$summary = [
    'scenario' => 'application-pragma-index-xinfo-foreignkey-current-source-next200',
    'applicationUse' => 'Copied wp_options imports can carry an index on (locale, slug) while FK checks and import lookups need the child-key prefix order (slug, locale); this keeps the wrong-order index visible as diagnostic-only PRAGMA evidence.',
    'status' => $page['status'],
...
if (($argv[1] ?? '') === '--self-test') {
    if (
        ...
        fwrite(STDERR, "application-pragma-index-xinfo-foreignkey-current-source-next200 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "application-pragma-index-xinfo-foreignkey-current-source-next200 self-test passed\n");
    exit(0);
}
