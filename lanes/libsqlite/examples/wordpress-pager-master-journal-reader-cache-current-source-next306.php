...
$pageSize = 512;
$database = '/srv/wp-content/database/wp-options-next306.sqlite';
$usersDatabase = '/srv/wp-content/database/wp-users-next306.sqlite';
...
$headers = [$mainJournal => hash('sha256', 'main header next306'), $usersJournal => hash('sha256', 'users header next306')];
$base = [
    'source_id' => 'wordpress-pager-reader-cache-next306',
    'epoch' => 306,
    'format_signature' => hash('sha256', implode('|', [512, 4, 2, 306, 0])),
    'publication_generation' => 306,
    'master_source_digest' => hash('sha256', 'wordpress next306 master source'),
...
$plan = SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::variantNext306(
    $database,
    $master,
    $masterBytes,
    implode('', $before),
...
if (in_array('--self-test', $argv, true)) {
    if ($plan['status'] !== 'pager-master-journal-reader-cache-current-source-next306') {
        fwrite(STDERR, "unexpected pager reader-cache next306 status\n");
        exit(1);
    }
    ...
    echo "wordpress-pager-master-journal-reader-cache-current-source-next306 self-test passed\n";
    return;
}
