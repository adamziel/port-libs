...
$plan = SQLitePlannerStat4ExpressionPartialCurrentSourceNextPlan::materializeCurrentSourceResidualWhereValidation(
    $prepared,
    $current,
    $terms,
...
$summary = [
    'scenario' => 'application-sqlplanner-stat4-expression-partial-current-source-next241',
    'status' => $plan['status'],
    ...
    'applicationUse' => 'Copied wp_options plugin screens can keep a STAT4 partial expression covering scan only after current-source rowids still satisfy blog/autoload/LIKE residual predicates, avoiding stale payloads after plugin-option churn without requiring ext/sqlite.',
];

if (PHP_SAPI === 'cli' && realpath($_SERVER['SCRIPT_FILENAME'] ?? '') === __FILE__) {
    if ($summary['status'] !== 'stat4-expression-partial-current-source-next241-ready' || $summary['rejectedRowids'] !== []) {
        fwrite(STDERR, "application-sqlplanner-stat4-expression-partial-current-source-next241 self-test failed\n");
        exit(1);
    }
    echo "application-sqlplanner-stat4-expression-partial-current-source-next241 self-test passed\n";
}
