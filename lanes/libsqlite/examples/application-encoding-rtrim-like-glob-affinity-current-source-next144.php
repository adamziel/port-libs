...
$plan = SQLiteEncodingRtrimLikeGlobAffinityCurrentSourceNextPlan::optionRowValuePlan(
    $current,
    $next,
    'option_value',
    'option_pattern',
...
if (($argv[1] ?? null) === '--self-test') {
    ...
    echo "application-encoding-rtrim-like-glob-affinity-current-source-next144 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'application-encoding-rtrim-like-glob-affinity-current-source-next144',
    'applicationUse' => 'Copied wp_options metadata scans can apply SQLite text affinity before dynamic LIKE/GLOB residuals while keeping RTRIM expression keys and invalidating stale current-source cursors when imported option values, patterns, encodings, or malformed text change.',
    'summary' => $plan,
