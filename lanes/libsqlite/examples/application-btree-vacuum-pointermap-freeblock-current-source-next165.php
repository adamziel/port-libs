...
$plan = SQLiteBTreeVacuumPointerMapFreeblockCurrentSourcePlan::tableLeafWritableDiffFromDeleteResult(
    $database,
    3,
    [
        'page' => $deletedPage,
...
$summary = [
    'scenario' => 'application-btree-vacuum-pointermap-freeblock-current-source-next165',
    'applicationUse' => 'Delete a copied wp_options transient, vacuum tail overflow pages, reuse surviving free pages for a replacement overflow chain, and compare current-source page bytes with the next image before admitting writes.',
...
if (($argv[1] ?? null) === '--self-test') {
    ...
    foreach ($expected as $key => $value) {
        if ($summary[$key] !== $value) {
            fwrite(STDERR, "application-btree-vacuum-pointermap-freeblock-current-source-next165 self-test failed\n");
            exit(1);
        }
    }

    fwrite(STDOUT, "application-btree-vacuum-pointermap-freeblock-current-source-next165 self-test passed\n");
    exit(0);
}
