...
$page = SQLitePragmaIndexXinfoForeignKeyCurrentSourceNext220::page(
    $currentRecords,
    $nextRecords,
...
$summary = [
    'scenario' => 'wordpress-pragma-index-xinfo-foreignkey-current-source-next220',
    'wordpressUse' => 'Copied multilingual wp_options imports can detect parent UNIQUE indexes whose PRAGMA index_xinfo collation does not match the referenced parent column declaration before treating foreign-key repair as current-source safe.',
    'status' => $page['status'],
...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "wordpress-pragma-index-xinfo-foreignkey-current-source-next220 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-index-xinfo-foreignkey-current-source-next220 self-test passed\n");
    exit(0);
}
