...
$summary = [
    'scenario' => 'application-pager-master-journal-reader-cache-current-source-next237',
    'status' => $plan['status'],
    ...
    'applicationUse' => 'A copied wp_options import reopens stale reader-cache tickets when master-journal recovery upgrades the schema-format number before reading active_plugins and rewrite_rules pages.',
...
if (in_array('--self-test', $argv, true)) {
    if ($summary['status'] !== 'pager-master-journal-reader-cache-current-source-next237'
        ...
        fwrite(STDERR, "application-pager-master-journal-reader-cache-current-source-next237 self-test failed\n");
        exit(1);
    }
    echo "application-pager-master-journal-reader-cache-current-source-next237 self-test passed\n";
    exit(0);
}
