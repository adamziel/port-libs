...
$summary = [
    'scenario' => 'application-btree-vacuum-pointermap-freeblock-current-source-next160',
    'applicationUse' => 'Delete a copied wp_options transient, vacuum tail overflow pages, then reuse the surviving current-source free pages as a multi-page replacement overflow chain without reusing truncated pages.',
...
if (($argv[1] ?? null) === '--self-test') {
    ...
    foreach ($expected as $key => $value) {
        if ($summary[$key] !== $value) {
            fwrite(STDERR, "application-btree-vacuum-pointermap-freeblock-current-source-next160 self-test failed\n");
            exit(1);
        }
    }

    fwrite(STDOUT, "application-btree-vacuum-pointermap-freeblock-current-source-next160 self-test passed\n");
    exit(0);
}
