...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "application-pragma-index-list-rootpage-integrity-current-source-next self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "application-pragma-index-list-rootpage-integrity-current-source-next self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'application-pragma-index-list-rootpage-integrity-current-source-next',
    'applicationUse' => 'Resume copied wp_options repair only when PRAGMA index_list metadata and sqlite_schema rootpage integrity agree for the table and every listed index in the current database image.',
    'status' => $page['status'],
