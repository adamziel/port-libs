...
if (($argv[1] ?? '') === '--self-test') {
    if ($plan['status'] !== 'committed' || $plan['next']['page_count'] !== 7 || $plan['next']['change_counter'] !== 105) {
        fwrite(STDERR, "wordpress-pager-transaction-current-next56 self-test failed\n");
        exit(1);
    }

    echo "wordpress-pager-transaction-current-next56 self-test passed\n";
    exit(0);
}

echo json_encode([
    'scenario' => 'wordpress-pager-transaction-current-next56',
    'table' => 'wp_options',
    'status' => $plan['status'],
