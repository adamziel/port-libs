...
$range = SQLiteMalformedTextCurrentNextCursor::wordpressOptionNameRange($rows, "plugin_\xc3", "plugin_\xc4", 'NOCASE', ['autoload' => 'yes']);

$report = [
    'scenario' => 'wordpress-malformed-text-current-next70',
    ...
    'wordpressUse' => 'Copied wp_options imports can inspect malformed option_name bytes with SQLite-style BINARY/NOCASE/RTRIM ordering, preserving current/next cursor diagnostics before repair tools rewrite or drop damaged rows.',
];
...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "wordpress-malformed-text-current-next70 self-test failed\n");
        exit(1);
    }
    fwrite(STDOUT, "wordpress-malformed-text-current-next70 self-test passed\n");
    exit(0);
}
