...
require_once __DIR__ . '/../src/SQLiteVfsCurrentSourceNext322337Plan.php';

use PortLibs\LibSqlite\SQLiteVfsCurrentSourceNext322337Plan;

$plan = SQLiteVfsCurrentSourceNext322337Plan::run([
            ...
            'main' => [
                ...
                'reuse_acks' => [
                    [
                        ...
                        'data_version' => 20,
                        'published_count' => 12,
                        'receipt_digest' => hash('sha256', 'publish-next217|shared-cache-next229|shared-cache-next237|shared-cache-next245|shared-cache-next257|shared-cache-next265|shared-cache-next273|shared-cache-next281|shared-cache-next289|shared-cache-next297|shared-cache-next313|shared-cache-next321'),
                    ],
                ],
            ],
        ],
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    assert($plan['events'][0]['ack'] === 'shared-cache-next321');
    assert($plan['events'][1]['status'] === 'claimed-reusable-current-source');
    assert($plan['events'][1]['claim'] === 'reader-reuse-next337');
    assert($plan['events'][1]['blocked_reasons'] === []);
    assert($plan['events'][2]['status'] === 'published-reused-current-source');
    assert($plan['events'][2]['reuse_ack'] === 'shared-cache-next321');
    assert($plan['events'][2]['published_count'] === 13);
    ...
    echo "wordpress-vfs-current-source-next322-337 self-test passed\n";
    return;
}
