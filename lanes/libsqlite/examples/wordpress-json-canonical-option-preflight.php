...
echo json_encode([
    'optionJson' => $report,
    'wordpressUse' => 'Local-only wp_options JSON canonicalization that mirrors SQLite json(X) and jsonb(X) uppercase argument-vector SQL dispatch for copied strict JSON text, JSON5 plugin settings, cast text BLOBs, JSONB option blobs, NULL option values, and malformed settings before import.',
