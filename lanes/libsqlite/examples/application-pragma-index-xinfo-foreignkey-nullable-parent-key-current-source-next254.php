...
if (PHP_SAPI === 'cli' && in_array('--self-test', $argv ?? [], true)) {
    if (
        ...
        fwrite(STDERR, "application pragma nullable parent-key next254 self-test failed\n");
        exit(1);
    }

    echo "application-pragma-index-xinfo-foreignkey-nullable-parent-key-current-source-next254 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'copied Application taxonomy PRAGMA index_xinfo foreign-key nullable parent key current-source next254',
    'applicationUse' => 'Copied taxonomy imports can hold PRAGMA index_xinfo and foreign_key_list on the same current source while detecting nullable parent key columns behind exact UNIQUE parent indexes before admitting the repaired next schema.',
