...
if (($argv[1] ?? '') === '--self-test') {
    ...
    assert($plan['events'][0]['ack'] === 'shared-cache-next817');
    assert($plan['events'][0]['blocked_reasons'] === []);
    assert($plan['events'][1]['status'] === 'claimed-reusable-current-source');
    assert($plan['events'][1]['claim'] === 'reader-reuse-next833');
    assert($plan['events'][1]['blocked_reasons'] === []);
    assert($plan['events'][2]['status'] === 'published-reused-current-source');
    assert($plan['events'][2]['reuse_ack'] === 'shared-cache-next817');
    assert($plan['events'][2]['token'] === 'shared-cache-next833');
    assert($plan['events'][2]['published_count'] === 43);
    assert(in_array('vfs-current-source-snapshot-reuse-publish-next818-833', $plan['dependencies'], true));
    assert(in_array('vfs-current-source-snapshot-reuse-publish-next802-817', $plan['dependencies'], true));
    echo "application-vfs-current-source-next818-833 self-test passed\n";
    return;
}
