...
$plan = SQLitePlannerStat4ExpressionPartialCurrentSourceNextPlan::materializeCurrentSourceInLimitProjectionFence(
    $prepared,
    $current,
    $where,
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    if (($plan['matchedRowids'] ?? []) !== [10, 11, 20]) {
        throw new RuntimeException('Expected LIMIT/OFFSET to window copied Application plugin preload rows');
    }
    ...
    echo "application-sqlplanner-stat4-expression-partial-current-source-next186 self-test passed\n";

    return;
}

echo json_encode([
    'scenario' => 'application-sqlplanner-stat4-expression-partial-current-source-next186',
    'status' => $plan['status'],
    ...
    'applicationUse' => 'Copied wp_options plugin preload queries can reprepare stale STAT4 partial lower(option_name) IN-list plans, apply LIMIT/OFFSET to the current-source probe stream, and return covering option payloads without table lookups.',
