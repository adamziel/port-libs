...
$summary = [
    'scenario' => 'wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next169',
    'status' => $plan['status'],
    ...
    'wordpressUse' => 'A copied wp_options import crash-resumes after current-source checkpoint bytes and retained WAL bytes are synced, keeping the hot journal until reader release is safe and deferring WAL reset until released checkpoint bytes are published.',
...
if ($summary['status'] !== 'wal-hot-journal-savepoint-checkpoint-current-source-next169'
    ...
    fwrite(STDERR, "wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next169 self-test failed\n");
    exit(1);
}

echo "wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next169 self-test passed\n";
