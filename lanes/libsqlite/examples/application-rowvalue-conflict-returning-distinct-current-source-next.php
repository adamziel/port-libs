...
$summary = [
    'scenario' => 'application-rowvalue-conflict-returning-distinct-current-source-next',
    'applicationUse' => 'Model a copied wp_options drift repair where row-value IS DISTINCT FROM selects changed rows, OR IGNORE suppresses conflicting RETURNING rows, OR REPLACE deletes the current conflicting option before RETURNING, clean aligned rows are deleted, and a later OR ABORT leaves the prior current-source prefix intact.',
    'status' => $plan['status'],
