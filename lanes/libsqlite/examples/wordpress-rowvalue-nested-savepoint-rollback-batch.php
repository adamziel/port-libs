...
$plan = SQLiteRowValueUpdateDeleteReturningSavepointCurrentSourceNextPlan::executeNestedSavepointRollbackBatch(
    ['wp_options' => $rows],
...
$summary = [
    'scenario' => 'wordpress-rowvalue-nested-savepoint-rollback-batch',
    'wordpressUse' => 'A copied wp_options import can roll back an inner cleanup savepoint, discard its UPDATE/DELETE RETURNING rows, preserve the outer staged option rows, and continue the next statement from that restored current source.',
    'status' => $plan['status'],
...
if (
    ...
    fwrite(STDERR, "wordpress-rowvalue-nested-savepoint-rollback-batch self-test failed\n");
    exit(1);
}

echo "wordpress-rowvalue-nested-savepoint-rollback-batch self-test passed\n";
