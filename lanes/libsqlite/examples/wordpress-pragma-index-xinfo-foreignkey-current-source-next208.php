...
use PortLibs\LibSqlite\SQLitePragmaIndexXinfoForeignKeyCurrentSourceNext208;
...
$page = SQLitePragmaIndexXinfoForeignKeyCurrentSourceNext208::page(
    $currentRecords,
    $nextRecords,
...
$summary = [
    'scenario' => 'wordpress-pragma-index-xinfo-foreignkey-current-source-next208',
    'wordpressUse' => 'Copied multisite wp_options imports that omit parent-column lists can verify which parent primary-key columns PRAGMA foreign_key_list resolves before applying schema repairs.',
    'status' => $page['status'],
...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "wordpress-pragma-index-xinfo-foreignkey-current-source-next208 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-index-xinfo-foreignkey-current-source-next208 self-test passed\n");
    exit(0);
}
