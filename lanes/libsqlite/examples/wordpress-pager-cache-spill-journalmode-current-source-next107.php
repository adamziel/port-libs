...
if (
    ...
    fwrite(STDERR, "wordpress-pager-cache-spill-journalmode-current-source-next107 self-test failed\n");
    exit(1);
}

if (PHP_SAPI === 'cli' && realpath($_SERVER['SCRIPT_FILENAME'] ?? '') === __FILE__) {
    echo "wordpress-pager-cache-spill-journalmode-current-source-next107 self-test passed\n";
}

return [
    'scenario' => 'wordpress-pager-cache-spill-journalmode-current-source-next107',
    'wordpressUse' => 'Plan copied wp_options import dirty-page cache spill with current SQLite journal-mode routing: WAL spills append frames while rollback-journal modes write database pages only after synced journal evidence and exclusive-lock promotion.',
    'wal' => [
