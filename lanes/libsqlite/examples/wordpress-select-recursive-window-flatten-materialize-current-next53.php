...
if (($argv[1] ?? null) === '--self-test') {
    ...
    echo "wordpress-select-recursive-window-flatten-materialize-current-next53 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'wordpress-select-recursive-window-flatten-materialize-current-next53',
    'wordpressUse' => 'Copied wp_options import queues can keep a recursive CTE materialized while yielding parser-level window current/next sibling diagnostics for each row without requiring ext/sqlite.',
    'rows' => $plan['rows'],
