...
$summary = [
    'scenario' => 'application-pragma-index-xinfo-foreignkey-current-source-next252',
    'applicationUse' => 'Copied Application taxonomy import DDL can catch foreign-key child columns that are absent even from PRAGMA table_xinfo before replaying schema repair.',
    'status' => $page['status'],
...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "application-pragma-index-xinfo-foreignkey-current-source-next252 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "application-pragma-index-xinfo-foreignkey-current-source-next252 self-test passed\n");
    exit(0);
}
