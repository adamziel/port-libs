...
$page = SQLitePragmaIndexXinfoForeignKeyCurrentSourceNext240::page(
    $current,
    $next,
...
$summary = [
    'scenario' => 'wordpress-pragma-index-xinfo-foreignkey-current-source-next240',
    'wordpressUse' => 'Copied WordPress import schemas sometimes use shorthand REFERENCES parent clauses; PRAGMA foreign_key_list reports an empty parent column that must resolve to the parent primary key before schema comparison.',
...
if (PHP_SAPI === 'cli' && realpath($_SERVER['SCRIPT_FILENAME'] ?? '') === __FILE__) {
    if (
        ...
        fwrite(STDERR, "wordpress-pragma-index-xinfo-foreignkey-current-source-next240 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, json_encode($summary, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) . "\n");
    fwrite(STDOUT, "wordpress-pragma-index-xinfo-foreignkey-current-source-next240 self-test passed\n");
}
