...
use PortLibs\LibSqlite\SQLitePagerMasterJournalCacheRecoveryCurrentSourceNext122Plan;
...
$plan = SQLitePagerMasterJournalCacheRecoveryCurrentSourceNext122Plan::currentSourceNext(
    $masterPath,
    $mainPath . "-journal\n",
    $mainPath . "-journal\n" . $sitePath . "-journal\n",
    [
...
$summary = [
    'scenario' => 'wordpress-pager-master-journal-cache-recovery-current-source-next122',
    'status' => $plan['status'],
    ...
    'wordpressUse' => 'Copied WordPress multisite option imports re-read the current master journal before hot rollback and savepoint retry, so stale pager cache membership cannot skip an attached site database journal or seed retry before-images from crashed bytes.',
...
if ($summary['status'] !== 'master_journal_cache_recovery_current_source_next122'
    ...
    fwrite(STDERR, "wordpress-pager-master-journal-cache-recovery-current-source-next122 self-test failed\n");
    exit(1);
}

echo "wordpress-pager-master-journal-cache-recovery-current-source-next122 self-test passed\n";
