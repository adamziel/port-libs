...
$summary = [
    'scenario' => 'wordpress-pragma-index-xinfo-foreignkey-current-source-next267-270',
    'wordpressUse' => 'Copied WordPress-style relational tables can flag action-bearing foreign keys that would force child scans because no child lookup index has the FK columns as a PRAGMA index_xinfo key prefix.',
...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "wordpress-pragma-index-xinfo-foreignkey-current-source-next267-270 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-index-xinfo-foreignkey-current-source-next267-270 self-test passed\n");
    exit(0);
}
