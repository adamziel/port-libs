...
use PortLibs\LibSqlite\SQLitePragmaIndexXinfoForeignKeyCurrentSourceNext234;
...
$page = SQLitePragmaIndexXinfoForeignKeyCurrentSourceNext234::page(
    $current,
    $next,
...
$summary = [
    'scenario' => 'wordpress-pragma-index-xinfo-foreignkey-current-source-next234',
    'wordpressUse' => 'Copied WordPress import schemas can reject an expression-backed UNIQUE parent index exposed by PRAGMA index_xinfo(cid=-2,name=NULL) before trusting PRAGMA foreign_key_list parent-key repair diagnostics.',
...
if (PHP_SAPI === 'cli' && realpath($_SERVER['SCRIPT_FILENAME'] ?? '') === __FILE__) {
    if (
        ...
        fwrite(STDERR, "wordpress-pragma-index-xinfo-foreignkey-current-source-next234 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, json_encode($summary, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) . "\n");
    fwrite(STDOUT, "wordpress-pragma-index-xinfo-foreignkey-current-source-next234 self-test passed\n");
}
