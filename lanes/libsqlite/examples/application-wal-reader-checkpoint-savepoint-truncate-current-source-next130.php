...
if (($argv[1] ?? '') === '--self-test') {
    ...
    if ($plan['wal_sidecar_removed_for_next_open'] !== true || $plan['next_open_uses_checkpoint_database'] !== true) {
        throw new RuntimeException('next Application reader did not reopen from the checkpointed database image');
    }
    ...
    echo "application-wal-reader-checkpoint-savepoint-truncate-current-source-next130 self-test passed\n";
    return;
}
