...
$schemaRows = [
    ...
    ['index', 'wp_options_value_expr', 'wp_options', 5, "CREATE INDEX wp_options_value_expr ON wp_options(json_extract(option_value, '$.plugin'), option_name COLLATE nocase)"],
...
$catalog = new SQLiteAttachedSchemaCatalog([
    ...
    $record('index', 'wp_options_value_expr', 'wp_options', 5, $schemaRows[1][4]),
    $record('index', 'wp_options_autoload', 'wp_options', 6, $schemaRows[2][4]),
]);

$page = SQLitePragmaIntegrityIndexRootpageCurrentSourceNext::page(
    $catalog,
...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        || ($page['rows'][$page['count'] - 1]['page_status'] ?? null) !== 'pointer_map'
    ) {
        fwrite(STDERR, "application-pragma-integrity-index-rootpage-current-source-next self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "application-pragma-integrity-index-rootpage-current-source-next self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'application-pragma-integrity-index-rootpage-current-source-next',
    'applicationUse' => 'Resume copied wp_options expression-index analysis only while PRAGMA index_xinfo metadata and sqlite_schema rootpage integrity still match the same current database image.',
    'status' => $page['status'],
    'source_id' => $page['source_id'],
    'current' => $page['current'],
    'first_row' => $page['rows'][0]['message'] ?? null,
