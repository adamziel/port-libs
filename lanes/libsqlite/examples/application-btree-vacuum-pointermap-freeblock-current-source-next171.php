...
$pages[3] = SQLiteTableLeafPage::assemble([
    ...
    SQLiteTableLeafCell::encode(2, SQLiteRecord::encode([null, '_transient_next171_payload', str_repeat('x', 96)])),
...
$plan = SQLiteBTreeVacuumPointerMapFreeblockCurrentSourcePlan::tableLeafSourceTransitionFromDeleteResult(
    $database,
    3,
    [
        'page' => $deletedPage,
    ...
    4,
    str_repeat('application-next171-source-transition-', 55),
    3,
    true,
);

$summary = [
    'application_scenario' => 'wp_options transient cleanup classifies stable leaf, reused overflow, and rejected truncated current-source pages',
    ...
    'transition_errors' => $plan->sourceTransitionErrors(),
];
...
if (($argv[1] ?? '') === '--self-test') {
    $expected = [
        ...
        'rejected_truncated_pages' => [110],
        'transition_errors' => [],
    ];
    if ($summary !== $expected) {
        fwrite(STDERR, "application-btree-vacuum-pointermap-freeblock-current-source-next171 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "application-btree-vacuum-pointermap-freeblock-current-source-next171 self-test passed\n");
    exit(0);
}
