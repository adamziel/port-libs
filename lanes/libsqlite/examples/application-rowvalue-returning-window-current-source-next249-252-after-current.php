...
$argv = [];
$next249 = require __DIR__ . '/application-rowvalue-chunked-yield-resume-window.php';
ob_start();
$argv = [];
$next250 = require __DIR__ . '/application-rowvalue-returning-window-current-source-next250.php';
$next250Output = ob_get_clean();
$next250Decoded = json_decode((string) $next250Output, true, 512, JSON_THROW_ON_ERROR);
$argv = [];
$next251 = require __DIR__ . '/application-rowvalue-returning-window-current-source-next251.php';
$argv = [];
$next252 = require __DIR__ . '/application-rowvalue-returning-window-current-source-next252.php';
$argv = $outerArgv;
...
$summary = [
    ...
    'applicationUse' => 'Copied wp_options imports can validate the prepared next249-252 current-source row-value UPDATE/DELETE RETURNING handoff as chunked yield windows, EXCLUDE TIES frames, source digest handoff, and high-water window fence coverage.',
];

if ($selfTest) {
    echo "application-rowvalue-returning-window-current-source-next249-252-after-current self-test passed\n";
}
