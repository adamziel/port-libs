...
$databaseDigest = $hash('application next232 checkpoint database');
$previousWalDigest = $hash('application next232 previous wal generation');
$currentWalDigest = $hash('application next232 current wal generation');
$schemaCookie = 23277;
...
$summary = [
    'scenario' => 'application-wal-hot-journal-savepoint-checkpoint-current-source-next232',
    'applicationUse' => 'A copied Application import reopens reader slots after hot-journal recovery and savepoint checkpoint publication, then serves checkpoint pages only after schema-cookie, WAL-salt, generation, digest, and lock receipts match.',
    'status' => $plan['status'],
...
if (PHP_SAPI === 'cli' && basename(__FILE__) === basename($_SERVER['SCRIPT_FILENAME'] ?? '')) {
    if ($summary['status'] !== 'wal-hot-journal-savepoint-checkpoint-current-source-next232'
        ...
        fwrite(STDERR, "application-wal-hot-journal-savepoint-checkpoint-current-source-next232 self-test failed\n");
        exit(1);
    }

    echo "application-wal-hot-journal-savepoint-checkpoint-current-source-next232 self-test passed\n";
}
