...
$plan = SQLiteUtf16NocaseLikeRtrimCurrentSourceNext213Plan::wordpressOptionNameSelfEscapedEscapePlan(
    $currentRows,
    $nextRows,
...
$payload = [
    ...
    'wordpressUse' => 'Copied wp_options scans can bind UTF-16 LIKE patterns where a non-ASCII ESCAPE character escapes itself before an escaped wildcard, preserving the literal prefix for current-source invalidation instead of reusing a stale wildcard range.',
];
...
if (($argv[1] ?? null) === '--self-test') {
    ...
    echo "wordpress-utf16-nocase-like-rtrim-current-source-next213 self-test passed\n";
    return;
}
