...
$page = SQLitePragmaIndexXinfoForeignKeyCurrentSourceNext172::currentNextPageFromCatalog(
    $records,
    $currentTables,
    $records,
    $nextTables,
...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "wordpress-pragma-index-xinfo-foreignkey-current-source-next172 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-index-xinfo-foreignkey-current-source-next172 self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'wordpress-pragma-index-xinfo-foreignkey-current-source-next172',
    'wordpressUse' => 'Copied multisite wp_options imports can resume PRAGMA index_xinfo plus targeted foreign_key_check(table) pages without being blocked by unrelated WordPress tables in the same catalog image.',
    'status' => $page['status'],
