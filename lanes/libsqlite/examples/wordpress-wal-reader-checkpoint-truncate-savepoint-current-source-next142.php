...
$summary = [
    'scenario' => 'wordpress-wal-reader-checkpoint-truncate-savepoint-current-source-next142',
    'wordpressUse' => 'A copied WordPress options import rolls back a plugin-setting savepoint while an existing reader is pinned on the current WAL source; TRUNCATE checkpoint waits for that reader, then starts a fresh WAL generation for the retry write.',
    'status' => $plan['status'],
...
if (PHP_SAPI === 'cli' && basename(__FILE__) === basename($_SERVER['SCRIPT_FILENAME'] ?? '')) {
    if ($summary['status'] !== 'reader-checkpoint-truncate-savepoint-current-source-next142'
        ...
        fwrite(STDERR, "wordpress-wal-reader-checkpoint-truncate-savepoint-current-source-next142 self-test failed\n");
        exit(1);
    }

    echo "wordpress-wal-reader-checkpoint-truncate-savepoint-current-source-next142 self-test passed\n";
}
