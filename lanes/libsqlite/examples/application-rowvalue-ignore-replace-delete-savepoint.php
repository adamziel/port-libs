...
echo json_encode([
    'scenario' => 'application-rowvalue-update-delete-returning-savepoint-current-source-ignore_replace_delete',
    'applicationUse' => 'Model copied wp_options cleanup where OR IGNORE suppresses conflicting RETURNING rows, OR REPLACE deletes the conflicting current row, and a follow-up DELETE RETURNING reads that current source inside the same savepoint.',
    'status' => $plan['status'],
