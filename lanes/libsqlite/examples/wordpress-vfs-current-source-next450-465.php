...
require_once __DIR__ . '/../src/SQLiteVfsCurrentSourceNext450465Plan.php';

use PortLibs\LibSqlite\SQLiteVfsCurrentSourceNext450465Plan;
...
$plan = SQLiteVfsCurrentSourceNext450465Plan::run([
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    assert($plan['events'][0]['ack'] === 'shared-cache-next449');
    assert($plan['events'][1]['status'] === 'claimed-reusable-current-source');
    assert($plan['events'][1]['claim'] === 'reader-reuse-next465');
    assert($plan['events'][1]['blocked_reasons'] === []);
    assert($plan['events'][2]['status'] === 'published-reused-current-source');
    assert($plan['events'][2]['reuse_ack'] === 'shared-cache-next449');
    assert($plan['events'][2]['token'] === 'shared-cache-next465');
    assert($plan['events'][2]['published_count'] === 21);
    ...
    echo "wordpress-vfs-current-source-next450-465 self-test passed\n";
    return;
}
