...
if (PHP_SAPI === 'cli' && in_array('--self-test', $argv, true)) {
    if (
        ...
        fwrite(STDERR, "wordpress-pragma-foreign-key-integrity-pointermap-current-source-next104 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-foreign-key-integrity-pointermap-current-source-next104 self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'wordpress-pragma-foreign-key-integrity-pointermap-current-source-next104',
    'wordpressUse' => 'Page copied WordPress archive foreign_key_check rows together with auto-vacuum pointer-map integrity blockers using a current-source cursor that rejects stale database/schema/catalog resumes without ext/sqlite.',
