...
foreach (glob(dirname(__DIR__) . '/src/SQLiteTriggerRecursiveViewUpsertCurrentSourceNext*Plan.php') ?: [] as $planFile) {
    require_once $planFile;
}
...
$nextView = $view;
$nextView['source'] = 'main@view-cookie-251-next';
$nextView['trigger_source'] = 'main@trigger-cookie-251-next';
$postResetView = $view;
$postResetView['source'] = 'main@view-cookie-251-post-reset';
$postResetView['trigger_source'] = 'main@trigger-cookie-251-post-reset';
$followingView = $view;
$followingView['source'] = 'main@view-cookie-251-following';
$followingView['trigger_source'] = 'main@trigger-cookie-251-following';

$summary = SQLiteTriggerRecursiveViewUpsertCurrentSourceNextPlan::executeCurrentChangeReceipt(
    [
    ...
    [
        ...
        'current_source_cursor_source_close' => 'wp.returning.current.cursor.251',
        'current_source_close_token_source_close' => 'wp.current.source.close.251',
        'current_view_cookie_source_close' => 'main@view-cookie-251-current',
        'current_trigger_cookie_source_close' => 'main@trigger-cookie-251-current',
...
if (
    $summary['status_next251'] !== 'trigger-recursive-view-upsert-current-source-next251-change-counters-released'
    || $summary['current_source_change_counter_plan_next251']['decision'] !== 'publish-next-source-after-current-recursive-view-upsert-change-counters'
    || array_column($summary['visible_returning_payloads_next251'], 'name') !== ['blogdescription_child', 'template_child', 'home', 'next_plugin']
    || $summary['held_next_source_rows_next251'] !== []
) {
    fwrite(STDERR, "wordpress-trigger-recursive-view-upsert-current-source-next251 self-test failed\n");
    exit(1);
}

echo "wordpress-trigger-recursive-view-upsert-current-source-next251 self-test passed\n";
