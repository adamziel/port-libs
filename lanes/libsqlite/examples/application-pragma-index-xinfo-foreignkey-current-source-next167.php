...
if (($argv[1] ?? null) === '--self-test') {
    if ($page['status'] !== 'ok' || $page['current']['foreign_key_violations'] !== 3 || $page['next_counts']['foreign_key_violations'] !== 0 || $page['delta']['foreign_key_action_changed'] !== true) {
        fwrite(STDERR, "application-pragma-index-xinfo-foreignkey-current-source-next167 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "application-pragma-index-xinfo-foreignkey-current-source-next167 self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'application-pragma-index-xinfo-foreignkey-current-source-next167',
    'applicationUse' => 'Carry PRAGMA foreign_key_list action metadata through copied wp_options PRAGMA index_xinfo/foreign_key_check diagnostics so migration tooling can distinguish CASCADE, SET NULL, RESTRICT, and NO ACTION repairs.',
    'status' => $page['status'],
