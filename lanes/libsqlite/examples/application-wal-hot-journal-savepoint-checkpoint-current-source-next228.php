...
$databaseDigest = $digest('application copied options database after durable checkpoint');
...
$summary = [
    'scenario' => 'application-wal-hot-journal-savepoint-checkpoint-current-source-next228',
    'applicationUse' => 'A copied Application plugin import reopens wp_options readers only after the checkpoint source has durable database, WAL reset, hot-journal unlink, SHM lock, and savepoint-release receipts.',
    'status' => $plan['status'],
...
if (($argv[1] ?? null) === '--self-test') {
    if ($summary['status'] !== 'wal-hot-journal-savepoint-checkpoint-current-source-next228'
        ...
        fwrite(STDERR, "application-wal-hot-journal-savepoint-checkpoint-current-source-next228 self-test failed\n");
        exit(1);
    }

    echo "application-wal-hot-journal-savepoint-checkpoint-current-source-next228 self-test passed\n";
    exit(0);
}
