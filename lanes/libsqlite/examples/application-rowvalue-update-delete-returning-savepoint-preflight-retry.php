...
$plan = SQLiteRowValueUpdateDeleteReturningSavepointPlan::executePreflightRetrySavepointBatch(
    ['wp_options' => $rows],
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "application-rowvalue-update-delete-returning-savepoint-preflight-retry self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'application-rowvalue-update-delete-returning-savepoint-preflight-retry',
    'status' => $plan['status'],
    ...
    'applicationUse' => 'Copied wp_options import batches can roll back speculative row-value UPDATE/DELETE RETURNING cleanup, retry from the savepoint image, and only yield RETURNING rows from the released retry source.',
