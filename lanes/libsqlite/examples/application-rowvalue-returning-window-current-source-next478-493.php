...
for ($next = 478; $next <= 493; $next++) {
    $plan = SQLiteRowValueUpdateDeleteReturningWindowCurrentSourceNextPlan::executeWindowCurrentSourceContinuation($next, ...$args);
...
$summary = [
    ...
    'applicationUse' => 'Copied wp_options imports validate the next478-493 row-value UPDATE/DELETE RETURNING window current-source continuation after merged next462-477 while preserving independent libsqlite throughput.',
];

if (($argv[1] ?? null) === '--self-test') {
    echo "application-rowvalue-returning-window-current-source-next478-493 self-test passed\n";
    return;
}
