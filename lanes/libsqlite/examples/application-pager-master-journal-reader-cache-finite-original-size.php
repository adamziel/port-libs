...
$summary = [
    'scenario' => 'application-pager-master-journal-reader-cache-finite-original-size',
    'status' => $plan['status'],
    ...
    'applicationUse' => 'A copied Application database can keep a schema reader-cache page after finite master-journal recovery, but transient option tail pages removed by the rollback journal cannot be served from cache or rewritten without reopening at the truncated current source.',
...
if ($summary['status'] !== 'pager-master-journal-reader-cache-current-source-next185'
    ...
    fwrite(STDERR, "application-pager-master-journal-reader-cache-finite-original-size self-test failed\n");
    exit(1);
}

echo "application-pager-master-journal-reader-cache-finite-original-size self-test passed\n";
