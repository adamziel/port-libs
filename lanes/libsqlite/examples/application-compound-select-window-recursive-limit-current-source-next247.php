...
$payload = [
    'scenario' => 'application-compound-select-window-recursive-limit-current-source-next247',
    'applicationUse' => 'Copied wp_options preview queries can keep recursive LIMIT/OFFSET skipped rows, window metrics, and a yielded next-source cursor sealed until the current compound result page is acknowledged.',
    'status' => $plan['status'],
...
if (($payload['status'] ?? null) !== 'compound-select-window-recursive-limit-current-source-next247-ready') {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-next247 self-test failed\n");
    exit(1);
}
if ($payload['currentSkippedLabels'] !== ['seed', 'seed:2'] || $payload['requiredAckCount'] !== 3) {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-next247 offset seal guard failed\n");
    exit(1);
}
