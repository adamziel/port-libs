...
$schemas = [
    'main' => ['schema_cookie' => 668, 'tables' => ['wp_options', 'wp_navigation_rule_locale_publish_final_next668'], 'indexes' => ['wp_options_name', 'wp_navigation_rule_locale_publish_final_key_next668'], 'wal_frames' => [['page' => 1, 'schema_cookie' => 668, 'commit' => true]]],
    'temp' => ['schema_cookie' => 526, 'tables' => ['wp_theme_stage_publish_retries_next558'], 'indexes' => [], 'temp' => true],
    'archive' => ['schema_cookie' => 575, 'tables' => ['wp_schema_archive_preview_next628', 'wp_schema_archive_receipt_next662'], 'indexes' => ['wp_schema_archive_receipt_key_next662'], 'file' => '/srv/wp/archive-next669.sqlite'],
    'audit' => ['schema_cookie' => 661, 'tables' => ['wp_schema_audit_next660', 'wp_schema_audit_meta_next661'], 'indexes' => ['wp_schema_audit_key_next660', 'wp_schema_audit_meta_key_next661'], 'file' => '/srv/wp/audit-next669.sqlite'],
    'queue' => ['schema_cookie' => 596, 'tables' => ['wp_job_retry_checkpoint_sealed_next623', 'wp_job_retry_checkpoint_preview_next620'], 'indexes' => ['wp_job_retry_checkpoint_preview_job_next655'], 'file' => '/srv/wp/queue-next669.sqlite'],
];
...
$plan = SQLiteAttachWalTempSchemaCacheCurrentSourceNext92Plan::currentSourceNext669684($schemas, $statements, [
...
if (($argv[1] ?? '') === '--self-test') {
    assert($plan['operation'] === 'attach-wal-temp-schema-cache-current-source-next669-684');
    assert($plan['dependencies'][0] === 'sqlite-attach-temp-wal-schema-cache-current-source-next669');
    assert($plan['dependencies'][15] === 'sqlite-attach-temp-wal-schema-cache-current-source-next684');
    assert(in_array('sqlite-attach-temp-wal-schema-cache-current-source-next668', $plan['dependencies'], true));
    assert($plan['event_count'] === 10);
    assert($plan['schema_cookies_next']['main'] === 684);
    assert($plan['schema_cookies_next']['queue'] === 672);
    assert($plan['schema_cookies_next']['audit'] === 662);
    assert($plan['schema_cookies_next']['handoff'] === 676);
    assert($plan['schema_cookies_next']['publish'] === 680);
    assert(in_array('archive-receipt-reader', $plan['active_current_snapshot_statements'], true));
    assert(in_array('queue-preview-writer', $plan['write_statements_blocked_before_retry'], true));
    assert($plan['statements']['archive-receipt-reader']['schema_transitions'][0]['next_schema'] === '__detached__');
    assert($plan['statements']['audit-meta-reader']['schema_transitions'][0]['next_found'] === false);
    ...
    echo "wordpress-attach-temp-wal-schema-cache-current-source-next669-684 self-test passed\n";
    return;
}
