...
$receipt = 'master-journal-recovery-receipt:epoch=270:members=main,users:cleanup=deleted:dirsync=ok';
$spill = 'pager-spill-drain:epoch=270:dirty-pages=2:journal-sync=ok:exclusive-lock=held';
$rollbackSource = 'pager-rollback-source:epoch=270:dirty-pages=2:journal-sync=ok:exclusive-lock=held';
$oldSpill = 'pager-rollback-source:epoch=266:dirty-pages=2:journal-sync=pending';
$snapshot = 'reader-snapshot:epoch=270:source=master-current:lease=shared:pages=1,2,3';
$generation = 'pager-reader-cache-generation:epoch=270:master-current:reset=complete';
$source = 'current-source:master-journal:epoch=270:members=2:schema=117';
$base = [
    'source_id' => 'wordpress-pager-rollback-source-smoke',
    'epoch' => 270,
    'format_signature' => hash('sha256', implode('|', [512, 4, 2, 270, 0])),
    'publication_generation' => 270,
    'master_source_digest' => hash('sha256', 'wordpress next270 master source'),
    ...
    'master_journal_file_token' => 'dev=8:ino=2700:size=96:mtime=27000:generation=master-current',
    'master_journal_bytes_digest' => hash('sha256', $masterBytes),
    'database_file_token' => 'dev=8:ino=2709:size=1536:mtime=27099:generation=database-current',
    'master_journal_cleanup_token' => 'master-cleanup:deleted:mtime=26800:dirsync=ok',
    'reader_lease_token' => 'reader-lease:shared-cache:epoch=270:opened-after-master-cleanup',
    'pager_cache_source_token' => 'pager-cache-source:epoch=270:master-journal-recovery=complete',
    'read_transaction_token' => 'read-transaction:epoch=270:schema=117:change-counter=270:master-current',
    'schema_reparse_token' => 'schema-reparse:epoch=270:schema-cookie=117:ddl=master-current',
    'statement_schema_root_token' => 'statement-schema-root:epoch=270:root=1:cookie=117:sql=wp-options-current',
...
$plan = SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::variantNext270(
    $database,
    $master,
    $masterBytes,
    implode('', $before),
...
if (in_array('--self-test', $argv, true)) {
    if ($plan['status'] !== 'pager-master-journal-reader-cache-current-source-next270') {
    ...
    echo "wordpress-pager-master-journal-reader-cache-current-source-next270 self-test passed\n";
    return;
}
