...
$summary = [
    'scenario' => 'wordpress-pragma-index-xinfo-foreignkey-current-source-next207',
    'wordpressUse' => 'Copied wp_options imports can preflight whether child foreign-key columns have a non-partial PRAGMA index_xinfo prefix before cascade/delete checks scan the staging table.',
    'status' => $page['status'],
...
if (($argv[1] ?? '') === '--self-test') {
    if (
        ...
        fwrite(STDERR, "wordpress-pragma-index-xinfo-foreignkey-current-source-next207 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-index-xinfo-foreignkey-current-source-next207 self-test passed\n");
    exit(0);
}
