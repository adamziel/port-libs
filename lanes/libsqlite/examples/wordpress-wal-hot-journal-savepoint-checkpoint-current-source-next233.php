...
$rootPages = [
    1 => $hash('wordpress next233 schema root'),
    2 => $hash('wordpress next233 wp_options root'),
    5 => $hash('wordpress next233 option_name index root'),
];
$databaseDigest = $hash('wordpress next233 checkpoint database');
$handlePlan = [
    ...
    'source_token' => 'wordpress-next233-current-source',
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    assert(in_array('wordpress-import-checkpoint-statement-reuse-after-hot-journal', $plan['dependencies'], true));
    echo "wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next233 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next233',
    'status' => $plan['status'],
    ...
    'wordpressUse' => 'After copied wp_options WAL checkpoint publication, prepared SELECT statements are reused only when their handles, schema cookie, root-page digests, and read-lock receipts match the new current source.',
