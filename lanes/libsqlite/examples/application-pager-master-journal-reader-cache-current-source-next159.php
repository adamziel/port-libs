...
$summary = [
    'scenario' => 'application-pager-master-journal-reader-cache-current-source-next159',
    'status' => $plan['status'],
    ...
    'applicationUse' => 'A copied wp_options recovery can discard stale pinned reader-cache pages after re-reading the current master-journal member list, then journal the next active_plugins write from the recovered current source.',
...
if ($summary['status'] !== 'pager-master-journal-reader-cache-current-source-next159'
    ...
    fwrite(STDERR, "application-pager-master-journal-reader-cache-current-source-next159 self-test failed\n");
    exit(1);
}

echo "application-pager-master-journal-reader-cache-current-source-next159 self-test passed\n";
