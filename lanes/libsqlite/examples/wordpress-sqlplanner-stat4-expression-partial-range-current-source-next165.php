...
$prepared = [
    'name' => 'prepared-wp-options-stat4-expression-partial-range-next165',
    'schemaCookie' => 1650,
    'stat4Generation' => 31,
    'rows' => [
...
$current = $prepared;
$current['name'] = 'current-wp-options-stat4-expression-partial-range-next165';
...
if (in_array('--self-test', $argv, true)) {
    if (
        ($plan['status'] ?? null) !== 'stat4-expression-partial-range-current-source-next165-ready'
        || ($plan['selectedPlan']['matchedRowids'] ?? []) !== [2, 3, 4]
        || ($plan['partialRangePredicateOperators'] ?? []) !== ['>=', '<']
        || ($plan['selectedSource'] ?? null) !== 'current'
    ) {
        fwrite(STDERR, "wordpress-sqlplanner-stat4-expression-partial-range-current-source-next165 self-test failed\n");
        exit(1);
    }

    echo "wordpress-sqlplanner-stat4-expression-partial-range-current-source-next165 self-test passed\n";
    exit(0);
}

echo json_encode([
    'scenario' => 'wordpress-sqlplanner-stat4-expression-partial-range-current-source-next165',
    'status' => $plan['status'],
