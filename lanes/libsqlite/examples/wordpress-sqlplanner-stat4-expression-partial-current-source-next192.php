...
$plan = SQLitePlannerStat4ExpressionPartialCurrentSourceNextPlan::materializeNext192(
    $prepared,
    $current,
    [
...
if (in_array('--self-test', $argv, true)) {
    ...
    echo "wordpress-sqlplanner-stat4-expression-partial-current-source-next192 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'wordpress-sqlplanner-stat4-expression-partial-current-source-next192',
    'wordpressUse' => 'Copied wp_options plugin admin scans can reuse the current STAT4 partial lower(option_name) index only when option_name, option_value, updated_at, and blog_id are still covered, avoiding table payload reads without ext/sqlite.',
    'status' => $plan['status'],
