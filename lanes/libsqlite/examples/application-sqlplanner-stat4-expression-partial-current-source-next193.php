...
$plan = SQLitePlannerStat4ExpressionPartialCurrentSourceNextPlan::materializeCurrentSourceRowidPartialEstimateFence(
    $prepared,
    $current,
    [
...
if (in_array('--self-test', $argv, true)) {
    ...
    echo "application-sqlplanner-stat4-expression-partial-current-source-next193 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'application-sqlplanner-stat4-expression-partial-current-source-next193',
    'applicationUse' => 'Copied wp_options plugin pagination can reuse a stale prepared STAT4 partial expression plan only after current-source rowid alias constraints are rechecked against rowid/_rowid_/oid semantics.',
    'status' => $plan['status'],
