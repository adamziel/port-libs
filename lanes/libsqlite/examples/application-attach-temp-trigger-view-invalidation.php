...
$output = [
    'scenario' => 'application attach temp trigger view invalidation view cache reprepare108',
    'applicationUse' => 'Preview copied wp_options TEMP view imports where the prepared trigger/view SQL text is unchanged, but a TEMP staging table appears and changes the view source from main.wp_options to temp.wp_options.',
    'status' => $plan['status'],
...
if (($argv[1] ?? '') === '--self-test') {
    if (
        ...
        fwrite(STDERR, "application-attach-temp-trigger-view-invalidation self-test failed\n");
        exit(1);
    }

    echo "application-attach-temp-trigger-view-invalidation self-test passed\n";
    exit(0);
}
