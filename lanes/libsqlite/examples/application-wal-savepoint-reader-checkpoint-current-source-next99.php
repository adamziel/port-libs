...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "application-wal-savepoint-reader-checkpoint-current-source-next99 self-test passed\n";
    return;
}

echo json_encode([
    'status' => $plan['status'],
    'applicationUse' => 'Summarizes copied wp_options WAL savepoint rollback with a pinned current reader and the next reader after checkpoint reset, including exact current-source frame offsets and source transitions without ext/sqlite.',
