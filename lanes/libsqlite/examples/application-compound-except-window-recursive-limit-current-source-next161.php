...
$result = [
    'scenario' => 'application-compound-except-window-recursive-limit-current-source-next161',
    'sqlShape' => 'WITH RECURSIVE queue feeding windowed UNION ALL rows, EXCEPT exclusion, and final ORDER BY/LIMIT/OFFSET',
    'applicationUse' => 'Copied wp_options import previews can remove skipped option rows through an EXCEPT arm after window values are computed, then compare the current and next yield boundary.',
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "application-compound-except-window-recursive-limit-current-source-next161 self-test passed\n";
    exit(0);
}
