...
$page = SQLitePragmaIndexXinfoForeignKeyCurrentSourceNext210::page(
    $current,
    $next,
...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "wordpress-pragma-index-xinfo-foreignkey-current-source-next210 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-index-xinfo-foreignkey-current-source-next210 self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'wordpress-pragma-index-xinfo-foreignkey-current-source-next210',
    'wordpressUse' => 'Before applying copied wp_options staging tables, verify that PRAGMA foreign_key_list SET DEFAULT actions have concrete child-column defaults exposed by PRAGMA table_info beside PRAGMA index_xinfo pagination.',
    'status' => $page['status'],
