...
$statementPlan = [
    ...
    'source_token' => 'wordpress-next236-source',
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    assert(in_array('wordpress-import-checkpoint-finalizer-before-next-writer', $plan['dependencies'], true));
    echo "wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next236 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next236',
    'status' => $plan['status'],
    'nextWriterAllowed' => $plan['next_writer_allowed'],
    'writerAction' => $plan['writer_action'],
    'wordpressUse' => 'A copied wp_options import opens the next WAL writer only after checkpoint-current prepared statements have reached SQLITE_DONE, reset, released reader leases, and produced WAL-hook/autocheckpoint receipts.',
