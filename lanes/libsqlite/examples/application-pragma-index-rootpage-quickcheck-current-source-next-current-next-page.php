...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "application-pragma-index-rootpage-quickcheck-current-source-next-current-next-page self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "application-pragma-index-rootpage-quickcheck-current-source-next-current-next-page self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'application-pragma-index-rootpage-quickcheck-current-source-next-current-next-page',
    'applicationUse' => 'Resume copied wp_options expression-index analysis only after PRAGMA quick_check verifies the next database image repaired the index rootpage pointer-map state.',
