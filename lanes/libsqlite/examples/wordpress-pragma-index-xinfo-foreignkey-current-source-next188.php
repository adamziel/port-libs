...
$page = SQLitePragmaIndexXinfoForeignKeyCurrentSourceNext188::currentNextPageFromCatalog(
    $records,
    $tables,
    $nextRecords,
    $tables,
...
$summary = [
    'scenario' => 'wordpress-pragma-index-xinfo-foreignkey-current-source-next188',
    'wordpressUse' => 'Copied wp_options imports can block a parent-key repair when PRAGMA index_xinfo shows that the only matching UNIQUE parent key is partial and therefore unusable for FK enforcement.',
    'status' => $page['status'],
...
if (($argv[1] ?? '') === '--self-test') {
    if (
        ...
        fwrite(STDERR, "wordpress-pragma-index-xinfo-foreignkey-current-source-next188 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-index-xinfo-foreignkey-current-source-next188 self-test passed\n");
    exit(0);
}
