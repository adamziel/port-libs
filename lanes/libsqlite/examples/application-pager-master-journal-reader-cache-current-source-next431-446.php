...
$base = [
    'source_id' => 'application-pager-reader-cache-next446',
    'epoch' => 446,
    'format_signature' => hash('sha256', implode('|', [512, 4, 2, 446, 0])),
    'publication_generation' => 446,
    'master_source_digest' => hash('sha256', 'application next446 master source'),
...
$cache = [1 => array_merge($base, ['reader_id' => 'wp-options-reader', 'image' => $recovered[1], 'reader_cache_stmt_vdbe_deferred_seek_token' => 'stmt-vdbe-deferred-seek-old'])];
$read = array_merge($base, [
    'reader_id' => 'read-options',
    'page_number' => 1,
    'member_journal_token_digest' => $mapDigest($tokens),
    'member_journal_header_digest' => $mapDigest($headers),
    'reader_cache_stmt_vdbe_deferred_seek_token' => 'stmt-vdbe-deferred-seek-old',
]);
$plan = SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::readerCacheStatementFence(446,
    $database,
    $master,
    $masterBytes,
    implode('', $before),
...
echo json_encode([
    ...
    'next446_invalidated_pages' => $plan['reader_cache_stmt_vdbe_deferred_seek_invalidated_cache_page_numbers'],
