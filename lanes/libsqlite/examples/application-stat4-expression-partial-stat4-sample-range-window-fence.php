...
$plan = SQLitePlannerStat4ExpressionPartialCurrentSourceNextPlan::materializeStat4SampleRangeWindowFence(
    $prepared,
    $current,
    $where,
...
$summary = [
    'scenario' => 'application-stat4-expression-partial-current-source-next226',
    'status' => $plan['status'],
    ...
    'applicationUse' => 'Keeps copied wp_options plugin-option range scans on a current partial expression index when STAT4 samples outside the range churn during import.',
];
...
if ($summary['status'] !== 'stat4-expression-partial-current-source-next226-ready'
    ...
    fwrite(STDERR, "application-stat4-expression-partial-current-source-next226 self-test failed\n");
    exit(1);
}
