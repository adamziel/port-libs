...
$summary = [
    'scenario' => 'wordpress-btree-overflow-pointermap-freeblock-current-source-next142',
    'wordpress_use' => 'Delete a copied wp_options transient row, coalesce the leaf freeblock, release its obsolete overflow pages, and allocate the replacement overflow chain while preserving current-source pointer-map evidence.',
...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "wordpress-btree-overflow-pointermap-freeblock-current-source-next142 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-btree-overflow-pointermap-freeblock-current-source-next142 self-test passed\n");
    exit(0);
}
