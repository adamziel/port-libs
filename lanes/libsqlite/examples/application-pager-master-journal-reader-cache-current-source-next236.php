...
$summary = [
    'scenario' => 'application-pager-master-journal-reader-cache-current-source-next236',
    'applicationUse' => 'A copied Application database keeps schema/options reader-cache pages only when their schema-reparse ticket was opened after master-journal recovery; stale active_plugins schema reads reopen before plugin import resumes.',
    'status' => $plan['status'],
...
if ($summary['status'] !== 'pager-master-journal-reader-cache-current-source-next236'
    ...
    fwrite(STDERR, "application-pager-master-journal-reader-cache-current-source-next236 self-test failed\n");
    exit(1);
}

echo json_encode($summary, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) . "\n";
echo "application-pager-master-journal-reader-cache-current-source-next236 self-test passed\n";
