...
$summary = [
    'scenario' => 'application-pragma-index-xinfo-foreignkey-current-source-next235',
    'applicationUse' => 'Copied taxonomy import schemas may preserve DESC UNIQUE parent indexes; PRAGMA index_xinfo desc metadata should not make valid foreign-key parent keys look unusable.',
    'status' => $page['status'],
...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "application-pragma-index-xinfo-foreignkey-current-source-next235 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "application-pragma-index-xinfo-foreignkey-current-source-next235 self-test passed\n");
    exit(0);
}
