...
$summary = [
    'scenario' => 'wordpress-json-table-indexed-derived-current-next25',
    ...
    'wordpressUse' => 'Copied wp_options JSON settings can be materialized through parser-level json_tree(), indexed by option/attribute, and scanned as current/next rows for import staging without ext/sqlite.',
];

if (($argv[1] ?? null) === '--self-test') {
    if ($summary['derivedRows'] !== 12 || $summary['indexKeys'] !== 6 || $summary['alphaPriorityAtoms'] !== [2, 7]) {
        fwrite(STDERR, "wordpress-json-table-indexed-derived-current-next25 self-test failed\n");
        exit(1);
    }

    echo "wordpress-json-table-indexed-derived-current-next25 self-test passed\n";
    exit(0);
}
