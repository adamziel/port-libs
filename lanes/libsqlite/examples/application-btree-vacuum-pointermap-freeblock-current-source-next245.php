...
$plan = SQLiteBTreeVacuumPointerMapFreeblockCurrentSourcePlan::tableLeafCursorAdmissionFromDeleteResult(
    $database,
    3,
    [
        'page' => $deletedPage,
...
echo json_encode([
    'scenario' => 'application-btree-vacuum-pointermap-freeblock-current-source-next245',
    'applicationUse' => 'After deleting an overflow-backed copied wp_options transient, admit reusable pages through a stable current-source cursor only after pointer-map epochs and leaf receipts are visible.',
    ...
    'cursor_epochs' => $summary['cursor_epochs'],
    'all_pointer_map_epochs_open_before_reuse' => $summary['all_pointer_map_epochs_open_before_reuse'],
    'all_reusable_pages_have_leaf_receipts' => $summary['all_reusable_pages_have_leaf_receipts'],
    'all_cursor_links_valid' => $summary['all_cursor_links_valid'],
...
if (
    ...
    && $summary['cursor_epochs'] === [1, 1, 2, 2, 3, 3, 3]
    && $summary['all_pointer_map_epochs_open_before_reuse'] === true
    && $summary['all_reusable_pages_have_leaf_receipts'] === true
    && $summary['all_cursor_links_valid'] === true
) {
    echo "application-btree-vacuum-pointermap-freeblock-current-source-next245 self-test passed\n";
}
