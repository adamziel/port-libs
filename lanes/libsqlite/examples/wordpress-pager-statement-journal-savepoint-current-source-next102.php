...
$summary = [
    'scenario' => 'wordpress-pager-statement-journal-savepoint-current-source-next102',
    'status' => $plan['status'],
    ...
    'wordpressUse' => 'During a copied wp_options plugin import, a failed statement can be rolled back from its statement journal while the surrounding savepoint remains current, then the retry statement starts from the restored source before RELEASE merges the successful pages.',
];
...
if (
    ...
    fwrite(STDERR, "wordpress-pager-statement-journal-savepoint-current-source-next102 self-test failed\n");
    exit(1);
}
