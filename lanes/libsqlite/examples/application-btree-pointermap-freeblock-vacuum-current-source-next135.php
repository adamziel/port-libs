...
$summary = [
    'scenario' => 'application-btree-pointermap-freeblock-vacuum-current-source-next135',
    'application_use' => 'Delete a copied wp_options transient row, preserve the reusable leaf freeblock, release obsolete overflow pages across a pointer-map page boundary, and partial-vacuum only the tail while page 106 remains a freelist trunk.',
...
if (PHP_SAPI === 'cli' && in_array('--self-test', $argv, true)) {
    ...
    if (!$ok) {
        fwrite(STDERR, "application-btree-pointermap-freeblock-vacuum-current-source-next135 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "application-btree-pointermap-freeblock-vacuum-current-source-next135 self-test passed\n");
}
