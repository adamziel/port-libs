...
use PortLibs\LibSqlite\SQLiteRowValueUpdateDeleteReturningSavepointCurrentSourceNext168Plan;
...
$summary = SQLiteRowValueUpdateDeleteReturningSavepointCurrentSourceNext168Plan::execute(
    $tables,
...
if (PHP_SAPI === 'cli' && basename((string) ($_SERVER['SCRIPT_FILENAME'] ?? '')) === basename(__FILE__)) {
    echo "wordpress-rowvalue-nested-savepoint-current-source-next168 self-test passed\n";
}

return [
    'scenario' => 'wordpress-rowvalue-nested-savepoint-current-source-next168',
    'summary' => [
    ...
    'wordpressUse' => 'Copied wp_options import cleanup can keep an outer savepoint batch, roll back an inner row-value UPDATE/DELETE RETURNING attempt, discard those speculative rows, and retry from the inner savepoint image without losing outer staged rows.',
