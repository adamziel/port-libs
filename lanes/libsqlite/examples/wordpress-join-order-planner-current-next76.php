...
if (($plan['tables'] ?? []) !== ['wp_posts', 'wp_postmeta']) {
    fwrite(STDERR, "wordpress-join-order-planner-current-next76 self-test failed\n");
    exit(1);
}

echo json_encode([
    'scenario' => 'wordpress-join-order-planner-current-next76',
    'wordpressUse' => 'Preview copied wp_posts/wp_postmeta nested-loop order from sqlite_stat1-style evidence before import or archive-generation queries run without ext/sqlite.',
    'tables' => $plan['tables'],
