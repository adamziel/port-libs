...
$pageSize = 512;
$database = '/srv/wp-content/database/wp-options-next302.sqlite';
$usersDatabase = '/srv/wp-content/database/wp-users-next302.sqlite';
...
$base = [
    'source_id' => 'wordpress-pager-reader-cache-next302',
    'epoch' => 302,
    'format_signature' => hash('sha256', implode('|', [512, 4, 2, 302, 0])),
    'publication_generation' => 302,
    'master_source_digest' => hash('sha256', 'wordpress next302 master source'),
...
$plan = SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::variantNext302(
    $database,
    $master,
    $masterBytes,
    implode('', $before),
...
if (in_array('--self-test', $argv, true)) {
    if ($plan['status'] !== 'pager-master-journal-reader-cache-current-source-next302') {
        fwrite(STDERR, "unexpected pager reader-cache next302 status\n");
        exit(1);
    }
    ...
    echo "wordpress-pager-master-journal-reader-cache-current-source-next302 self-test passed\n";
    return;
}
