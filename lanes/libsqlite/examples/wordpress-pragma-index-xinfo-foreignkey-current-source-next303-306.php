...
$summary = [
    'scenario' => 'wordpress-pragma-index-xinfo-foreignkey-current-source-next303-306',
    'wordpressUse' => 'Plugin import preflight can keep FK enforcement throughput high by rejecting child lookup shapes that PRAGMA index_xinfo would make slow or ambiguous.',
...
if (($argv[1] ?? null) === '--self-test') {
    foreach (array_keys($summary) as $key) {
        if (str_starts_with($key, 'next') && $summary[$key] !== 1) {
            fwrite(STDERR, "wordpress-pragma-index-xinfo-foreignkey-current-source-next303-306 self-test failed\n");
            exit(1);
        }
    }
    ...
    fwrite(STDOUT, "wordpress-pragma-index-xinfo-foreignkey-current-source-next303-306 self-test passed\n");
    exit(0);
}
