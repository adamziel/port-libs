...
$sourceId = 'application-next225-master-source';
$publication = 225;
$changeCounter = 84;
$versionValidFor = 84;
...
$summary = [
    'scenario' => 'application-pager-master-journal-reader-cache-current-source-next225',
    'status' => $plan['status'],
    ...
    'applicationUse' => 'A copied Application import that replays an attached rollback member under a master journal reuses schema/root pages only when SQLite page-1 change-counter and version-valid-for tickets still match the recovered current source.',
];
...
if (PHP_SAPI === 'cli' && basename(__FILE__) === basename($_SERVER['SCRIPT_FILENAME'] ?? '')) {
    if ($summary['status'] !== 'pager-master-journal-reader-cache-current-source-next225'
        ...
        fwrite(STDERR, "application-pager-master-journal-reader-cache-current-source-next225 self-test failed\n");
        exit(1);
    }
    echo "application-pager-master-journal-reader-cache-current-source-next225 self-test passed\n";
}
