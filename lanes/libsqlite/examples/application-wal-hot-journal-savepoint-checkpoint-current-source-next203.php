...
$summary = [
    'scenario' => 'application-wal-hot-journal-savepoint-checkpoint-current-source-next203',
    'applicationUse' => 'After a copied Application import recovers a hot journal, rolls back a failed savepoint, checkpoints WAL frames, and publishes the restarted WAL sidecar, wp_options page-cache leases are reused only when their WAL and checkpointed database page digests both match the current source.',
    'status' => $plan['status'],
...
if ($summary['status'] !== 'wal-hot-journal-savepoint-checkpoint-current-source-next203'
    ...
    fwrite(STDERR, "application-wal-hot-journal-savepoint-checkpoint-current-source-next203 self-test failed\n");
    exit(1);
}
