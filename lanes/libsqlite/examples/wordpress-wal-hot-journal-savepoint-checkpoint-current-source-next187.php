...
$summary = [
    ...
    'wordpressUse' => 'A copied WordPress import retires the hot-journal post-apply reader token before admitting a retry WAL checkpoint source.',
    'status' => $plan['status'],
...
if (PHP_SAPI === 'cli' && in_array('--self-test', $argv, true)) {
    if ($summary['status'] !== 'wal-hot-journal-savepoint-checkpoint-current-source-next187'
        ...
        fwrite(STDERR, "wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next187 self-test failed\n");
        exit(1);
    }

    echo "wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next187 self-test passed\n";
    exit(0);
}
