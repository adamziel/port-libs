...
$argv = [];
$next257 = require __DIR__ . '/wordpress-rowvalue-returning-window-current-source-next257.php';
$argv = [];
$next258 = require __DIR__ . '/wordpress-rowvalue-returning-window-current-source-next258.php';

ob_start();
$argv = [];
require __DIR__ . '/wordpress-rowvalue-update-delete-returning-window-current-source-next259.php';
$next259Output = trim((string) ob_get_clean());
$next259 = json_decode($next259Output, true, 512, JSON_THROW_ON_ERROR);

ob_start();
$argv = [];
require __DIR__ . '/wordpress-rowvalue-returning-window-current-source-next260.php';
...
$summary = [
    'status' => 'rowvalue-update-delete-returning-window-current-source-next257-260-after-current',
...
if ($selfTest) {
    if ($statuses !== [
        ...
        fwrite(STDERR, "wordpress-rowvalue-returning-window-current-source-next257-260-after-current self-test failed\n");
        exit(1);
    }

    echo "wordpress-rowvalue-returning-window-current-source-next257-260-after-current self-test passed\n";
    return;
}
