...
$current = [
    'name' => 'current-application-stat4-estimate-next239',
    'schemaCookie' => 2398,
    'stat4Generation' => 398,
    'rows' => [
...
$prepared = [
    'name' => 'prepared-application-stat4-estimate-next239',
    'schemaCookie' => 2390,
    'stat4Generation' => 239,
    'rows' => [
...
if (($plan['stat4PartialEstimateFence']['actualPartialRows'] ?? null) !== 6) {
    throw new RuntimeException('Expected six Application plugin option rows in the current partial index');
}

echo json_encode([
    'scenario' => 'application-sqlplanner-stat4-expression-partial-current-source-next239',
    'status' => $plan['status'],
    ...
    'applicationUse' => 'Copied wp_options plugin scans can reuse a current-source STAT4 expression partial index only when sqlite_stat1/stat4 partial cardinality matches rows admitted by the current partial predicate, preventing stale prepared plans after plugin autoload churn.',
