...
$prepared = [
    'name' => 'prepared-wp-options-next191',
    'schemaCookie' => 1910,
    'stat4Generation' => 121,
    'rows' => [
...
$current = $prepared;
$current['name'] = 'current-wp-options-next191';
$current['schemaCookie'] = 1918;
$current['stat4Generation'] = 144;
$current['indexes'][0]['rootPage'] = 19188;
...
if (in_array('--self-test', $argv, true)) {
    assert($plan['status'] === 'stat4-expression-partial-current-source-next191-ready');
    ...
    echo "application-sqlplanner-stat4-expression-partial-payload-expression-fence self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'application-sqlplanner-stat4-expression-partial-payload-expression-fence',
    'applicationUse' => 'Copied wp_options plugin screens can reuse a partial lower(option_name) STAT4 plan only when the current covering payload still recomputes to each indexed expression key.',
    'status' => $plan['status'],
    'selectedSource' => $plan['selectedSource'],
    'selectedIndex' => $plan['selectedPlan']['name'] ?? null,
    'matchedRowids' => $plan['matchedRowids'],
    'payloadExpressionFence' => $plan['payloadExpressionFence'],
