...
$pageSize = 512;
$database = '/srv/wp-content/database/wp-next478.sqlite';
...
$formatPage = static function (string $label) use ($pageSize): string {
    ...
    $page = substr_replace($page, pack('N', 478), 60, 4);
...
$tokens = [$journal => 'member-main-current-478'];
$headers = [$journal => hash('sha256', 'main header next478')];
$base = [
    'source_id' => 'wordpress-pager-reader-cache-next478',
    'epoch' => 478,
    'format_signature' => hash('sha256', implode('|', [512, 4, 2, 478, 0])),
    'publication_generation' => 478,
    'master_source_digest' => hash('sha256', 'wordpress next478 master source'),
    'recovery_sequence' => 478,
...
foreach ($tokenFields as $field) {
    $base[$field] = str_replace('_', '-', preg_replace('/_token$/', '', $field)) . '-current-478';
}
$cache = [1 => array_merge($base, ['reader_id' => 'wp-options-reader', 'image' => $recovered[1], 'reader_cache_stmt_vdbe_returning_token' => 'stmt-vdbe-returning-old'])];
$read = array_merge($base, [
    'reader_id' => 'read-options',
    'page_number' => 1,
    'member_journal_token_digest' => $mapDigest($tokens),
    'member_journal_header_digest' => $mapDigest($headers),
    'reader_cache_stmt_vdbe_returning_token' => 'stmt-vdbe-returning-old',
]);
$plan = SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::readerCacheStatementFence(478,
    $database,
    $master,
    $masterBytes,
    implode('', $before),
...
echo json_encode([
    ...
    'next478_invalidated_pages' => $plan['reader_cache_stmt_vdbe_returning_invalidated_cache_page_numbers'],
