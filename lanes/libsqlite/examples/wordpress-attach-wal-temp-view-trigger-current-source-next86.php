...
if (in_array('--self-test', $argv, true)) {
    if ($plan['status'] !== 'reprepare-required' || $plan['current']['columns'] === $plan['next']['columns']) {
        fwrite(STDERR, "wordpress-attach-wal-temp-view-trigger-current-source-next86 self-test failed\n");
        exit(1);
    }

    echo "wordpress-attach-wal-temp-view-trigger-current-source-next86 self-test passed\n";
    exit(0);
}

echo json_encode([
    'scenario' => 'wordpress attach WAL temp view trigger current-source next86',
    'wordpressUse' => 'Preview whether a copied WordPress option-import trigger prepared against current view columns must be reprepared after WAL-backed schema DDL changes the next trigger/view source.',
    'status' => $plan['status'],
