...
$databaseDigest = $hash('application next241 checkpoint database image');
$writerPlan = [
...
$receipt = static function (string $name, string $kind, string $path, array $overrides = []) use ($writerPlan, $databaseDigest, $hash): array {
    return array_replace([
        ...
        'commit_digest' => $hash('application next241 committed autoload update frames'),
        'synced' => true,
        'frames_synced' => true,
...
$summary = [
    'scenario' => 'application-wal-hot-journal-savepoint-checkpoint-current-source-next241',
    'status' => $plan['status'],
...
if (in_array('--self-test', $argv, true)) {
    if ($summary['status'] !== 'wal-hot-journal-savepoint-checkpoint-current-source-next241'
        ...
        fwrite(STDERR, "application-wal-hot-journal-savepoint-checkpoint-current-source-next241 self-test failed\n");
        exit(1);
    }

    echo "application-wal-hot-journal-savepoint-checkpoint-current-source-next241 self-test passed\n";
    return;
}
