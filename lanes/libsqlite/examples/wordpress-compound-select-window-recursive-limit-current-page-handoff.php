...
$payload = [
    'scenario' => 'wordpress-compound-select-window-recursive-limit-current-source-current-page-handoff',
    'wordpressUse' => 'Copied wp_options preview queries can hold a next-source dense_rank UNION/EXCEPT page until the current recursive/window limited page has exact row acknowledgements.',
    'status' => $plan['status'],
...
if ($payload['status'] !== 'compound-select-window-recursive-limit-current-source-current-page-handoff-ready') {
    fwrite(STDERR, "wordpress-compound-select-window-recursive-limit-current-source-current-page-handoff self-test failed\n");
    exit(1);
}
if ($payload['requiredAckCount'] !== 3 || $payload['pageTokenLength'] !== 64 || $payload['nextSourceEpochLength'] !== 64) {
    fwrite(STDERR, "wordpress-compound-select-window-recursive-limit-current-source-current-page-handoff handoff guard failed\n");
    exit(1);
}
