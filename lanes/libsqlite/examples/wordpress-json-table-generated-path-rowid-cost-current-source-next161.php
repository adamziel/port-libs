...
$plan = SQLiteJsonTablePlan::currentSourceGeneratedPathRowidCostCurrentSourceNext161(
    'json_tree',
    $currentOption,
    $nextOption,
    'option_value',
...
$payload = [
    'scenario' => 'wordpress-json-table-generated-path-rowid-cost-current-source-next161',
    'wordpressUse' => 'Copied wp_options JSON rule diagnostics can admit generated-path and rowid constraints into a pinned current-source json_tree cursor while leaving visible type checks as residual filters.',
    ...
    'replanReasons' => $plan['next161ReplanReasons'],
...
if (($argv[1] ?? null) === '--self-test') {
    if ($payload['currentIdxStr'] !== 'omit:path:LIKE|omit:id:IN') {
        fwrite(STDERR, "unexpected next161 current idx string\n");
        exit(1);
    }
    if ($payload['currentMatchedSeekRowids'] !== [5, 6]) {
        fwrite(STDERR, "unexpected next161 matched rowids\n");
        exit(1);
    }
    if ($payload['currentOmitColumns'] !== ['path', 'id']) {
        fwrite(STDERR, "unexpected next161 omit columns\n");
        exit(1);
    }
    if (!in_array('json-table-generated-path-rowid-current-source-admission-usage-changed', $payload['replanReasons'], true)) {
        fwrite(STDERR, "missing next161 usage replan reason\n");
        exit(1);
    }

    echo "wordpress-json-table-generated-path-rowid-cost-current-source-next161 self-test passed\n";
    return;
}
