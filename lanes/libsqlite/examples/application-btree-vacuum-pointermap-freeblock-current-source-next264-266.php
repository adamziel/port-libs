...
$makePlan = static function (int $sliceNumber) use ($firstPage, $putPointerMapEntry): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourcePlan {
    ...
    return SQLiteBTreeVacuumPointerMapFreeblockCurrentSourcePlan::tableLeafFreelistSpliceFromDeleteResult(
        ...
        2,
        str_repeat("application-next{$sliceNumber}-current-source-freelist-", 32),
        3,
    );
};
...
foreach ([264, 265, 266] as $sliceNumber) {
    $summaries["next{$sliceNumber}"] = $makePlan($sliceNumber)->freelistSummary();
}

echo json_encode([
    'application_scenario' => 'wp_options transient deletes keep pointer-map freelist splice receipts distinct across current-source next264 through next266',
    'summaries' => $summaries,
], JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) . "\n";

foreach ([264, 265, 266] as $sliceNumber) {
    $summary = $summaries["next{$sliceNumber}"];
    if (
        $summary['status'] !== "btree-vacuum-pointermap-freeblock-current-source-next{$sliceNumber}-ready"
        || $summary['trunk_anchor_pages'] !== [2, 105]
        || $summary['leaf_slot_pages'] !== [3, 106, 107, 108]
        || $summary['all_tail_pages_rejected_from_freelist'] !== true
    ) {
        exit(1);
    }
}

echo "application-btree-vacuum-pointermap-freeblock-current-source-next264-266 self-test passed\n";
