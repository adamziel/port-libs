...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "wordpress-compound-recursive-affinity-current-source-next120 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'wordpress-compound-recursive-affinity-current-source-next120',
    'sql' => $sql,
    'rows' => $rows,
    'wordpressUse' => 'Copied wp_options repair/import queries can combine recursive numeric generators with current option rows while preserving SQLite compound duplicate semantics: integer and real compare equal, text values remain distinct, and left-most recursive output names survive current-source arms.',
], JSON_PRETTY_PRINT) . "\n";
