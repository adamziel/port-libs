...
use PortLibs\LibSqlite\SQLitePlannerStat4ExpressionPartialCurrentSourceNext169Plan;
...
$current = $prepared;
$current['name'] = 'current-wp-options-stat4-expression-partial-cost-next169';
$current['schemaCookie'] = 1698;
$current['stat4Generation'] = 77;
$current['indexes'][0]['rootPage'] = 16981;
...
$plan = SQLitePlannerStat4ExpressionPartialCurrentSourceNext169Plan::materialize(
    $prepared,
    $current,
    [
...
if (in_array('--self-test', $argv, true)) {
    assert($plan['status'] === 'stat4-expression-partial-current-source-next169-ready');
    assert($plan['matchedRowids'] === [10, 20, 40, 30]);
    assert($plan['costFence']['costDelta'] === 35);
    echo "wordpress-sqlplanner-stat4-expression-partial-current-source-next169 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'wordpress-sqlplanner-stat4-expression-partial-current-source-next169',
    'wordpressUse' => 'Preview copied wp_options plugin scans after ANALYZE/source changes where current STAT4 costs keep the partial lower(option_name) expression index ahead of a broader full expression index.',
    'status' => $plan['status'],
    'selectedSource' => $plan['selectedSource'],
    'selectedIndex' => $plan['selectedPlan']['name'] ?? null,
    'partialCost' => $plan['costFence']['selectedPartialCost'],
    'bestFullExpressionCost' => $plan['costFence']['bestFullExpressionCost'],
    'costDelta' => $plan['costFence']['costDelta'],
    'matchedRowids' => $plan['matchedRowids'],
    'rejectedFullExpressionIndexes' => array_column($plan['rejectedFullExpressionIndexes'], 'name'),
