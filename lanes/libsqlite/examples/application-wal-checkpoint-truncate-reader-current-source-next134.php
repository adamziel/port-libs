...
if (($argv[1] ?? '') === '--self-test') {
    ...
    if ($plan['truncate_removed_old_wal_sidecar'] !== true || $plan['next_reader_uses_fresh_wal_generation'] !== true) {
        throw new RuntimeException('next Application reader did not move to the fresh WAL generation');
    }
    ...
    echo "application-wal-checkpoint-truncate-reader-current-source-next134 self-test passed\n";
    return;
}
