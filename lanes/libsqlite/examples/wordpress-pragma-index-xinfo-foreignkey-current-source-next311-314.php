...
$summary = [
    'scenario' => 'wordpress-pragma-index-xinfo-foreignkey-current-source-next311-314',
    'wordpressUse' => 'WordPress import previews can distinguish ON UPDATE from ON DELETE SET NULL and SET DEFAULT relationship blockers before trusting PRAGMA foreign_key_list replay.',
...
if (($argv[1] ?? null) === '--self-test') {
    foreach (array_keys($summary) as $key) {
        if (str_starts_with($key, 'next') && $summary[$key] !== 1) {
            fwrite(STDERR, "wordpress-pragma-index-xinfo-foreignkey-current-source-next311-314 self-test failed\n");
            exit(1);
        }
    }
    ...
    fwrite(STDOUT, "wordpress-pragma-index-xinfo-foreignkey-current-source-next311-314 self-test passed\n");
    exit(0);
}
