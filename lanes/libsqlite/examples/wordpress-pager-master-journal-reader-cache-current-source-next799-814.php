...
$pageSize = 512;
$database = '/srv/wp-content/database/wp-next814.sqlite';
...
$formatPage = static function (string $label) use ($pageSize): string {
    ...
    $page = substr_replace($page, pack('N', 814), 60, 4);
...
$variantFields = array_merge($tokenFields, $next639654Fields, $next655670Fields, $next671686Fields, $next687702Fields, $next703718Fields, $next719734Fields, $next735766Fields, $next751766Fields, $next767782Fields, $next783798Fields, $next799814Fields);
$before = [1 => $formatPage('stale schema'), 2 => $page('stale options')];
$recovered = [1 => $formatPage('current schema'), 2 => $page('current options')];
$tokens = [$journal => 'member-main-current-814'];
$headers = [$journal => hash('sha256', 'main header next814')];
$base = [
    'source_id' => 'wordpress-pager-reader-cache-next814',
    'epoch' => 814,
    'format_signature' => hash('sha256', implode('|', [512, 4, 2, 814, 0])),
    'publication_generation' => 814,
    'master_source_digest' => hash('sha256', 'wordpress next814 master source'),
    'recovery_sequence' => 814,
...
foreach ($variantFields as $field) {
    $base[$field] = str_replace('_', '-', preg_replace('/_token$/', '', $field)) . '-current-814';
}
$base['reader_cache_stmt_vdbe_real_affinity_value_token'] = 'reader-cache-stmt-vdbe-affinity-current-814';
$base['reader_cache_stmt_vdbe_real_affinity_value_handoff_token'] = 'reader-cache-stmt-vdbe-affinity-handoff-current-814';
...
echo json_encode([
    ...
    'next814_reopen_reason' => $plan['next_reads'][0]['reader_cache_stmt_vdbe_affinity_branch_handoff_token_reason'],
