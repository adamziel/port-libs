...
if (
    ...
    fwrite(STDERR, "application-rowvalue-released-inner-rollback-current-source-next174 self-test failed\n");
    exit(1);
}

if (PHP_SAPI === 'cli' && basename((string) ($_SERVER['SCRIPT_FILENAME'] ?? '')) === basename(__FILE__)) {
    echo "application-rowvalue-released-inner-rollback-current-source-next174 self-test passed\n";
}

return [
    'scenario' => 'application-rowvalue-released-inner-rollback-current-source-next174',
    'summary' => [
    ...
    'applicationUse' => 'Copied wp_options import cleanup can release an inner row-value UPDATE/DELETE RETURNING segment into an outer savepoint, roll back the outer batch, discard those released inner rows, and retry from the original current source.',
