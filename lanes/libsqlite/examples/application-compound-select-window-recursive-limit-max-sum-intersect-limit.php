...
$payload = [
    'scenario' => 'application-compound-select-window-recursive-limit-current-source-max-sum-intersect-limit',
    'applicationUse' => 'Copied wp_options previews can fence aggregate window metrics after recursive queue exhaustion, then re-evaluate current versus staged rows through INTERSECT/EXCEPT and final LIMIT boundaries.',
    'status' => $plan['status'],
...
if (($payload['status'] ?? null) !== 'compound-select-window-recursive-limit-current-source-max-sum-intersect-limit-ready') {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-max-sum-intersect-limit self-test failed\n");
    exit(1);
}
if ($payload['currentTokenLength'] !== 64 || $payload['nextTokenLength'] !== 64) {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-max-sum-intersect-limit token guard failed\n");
    exit(1);
}
if (!in_array('plugin_prime', $payload['nextRows'], true)) {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-max-sum-intersect-limit next-source boundary failed\n");
    exit(1);
}
