...
use PortLibs\LibSqlite\SQLitePagerMasterJournalReaderCacheCurrentSourceNext252Plan;
...
$plan = SQLitePagerMasterJournalReaderCacheCurrentSourceNext252Plan::plan(
    $database,
    $master,
    $masterBytes,
    implode('', $before),
...
$summary = [
    'scenario' => 'wordpress-pager-master-journal-reader-cache-current-source-next252',
    'wordpressUse' => 'A copied WordPress database reopens a wp_options reader if its cache ticket was stamped with the previous master-journal member manifest, even when page-owner and schema tickets still match.',
    'status' => $plan['status'],
...
if ($summary['status'] !== 'pager-master-journal-reader-cache-current-source-next252'
    ...
    fwrite(STDERR, "wordpress-pager-master-journal-reader-cache-current-source-next252 self-test failed\n");
    exit(1);
}

echo json_encode($summary, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) . "\n";
echo "wordpress-pager-master-journal-reader-cache-current-source-next252 self-test passed\n";
