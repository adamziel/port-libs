...
$summary = [
    'scenario' => 'application-pragma-index-xinfo-foreignkey-current-source-next225',
    'applicationUse' => 'Copied Application comment imports can compare PRAGMA foreign_key_list action clauses before replaying FK enforcement around post and author slug repairs.',
    'status' => $page['status'],
...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "application-pragma-index-xinfo-foreignkey-current-source-next225 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "application-pragma-index-xinfo-foreignkey-current-source-next225 self-test passed\n");
    exit(0);
}
