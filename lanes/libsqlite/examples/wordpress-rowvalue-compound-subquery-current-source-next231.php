...
$plan = SQLiteRowValueUpdateDeleteReturningSavepointCurrentSourceNextPlan::executeNext231(
...
$summary = [
    'scenario' => 'wordpress-rowvalue-compound-subquery-current-source-next231',
    'status' => $plan['status'],
    ...
    'wordpressUse' => 'A copied WordPress multisite options import can feed row-value UPDATE RETURNING from compound SELECT tuple sources, roll back the first batch to a savepoint, and retry from the restored current source without ext/sqlite.',
];

if (in_array('--self-test', $argv, true)) {
    if (
        $summary['status'] !== 'rowvalue-update-delete-returning-compound-subquery-savepoint-current-source-next231'
        ...
        fwrite(STDERR, "wordpress-rowvalue-compound-subquery-current-source-next231 self-test failed\n");
        exit(1);
    }
    fwrite(STDOUT, "wordpress-rowvalue-compound-subquery-current-source-next231 self-test passed\n");
    exit(0);
}
