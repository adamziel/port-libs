...
$payload = [
    'scenario' => 'application-vfs-filecontrol-persistwal-lock-current-source-next94',
    'applicationUse' => 'Route copied wp_options xFileControl(PERSIST_WAL) through current-source lock rules: unlocked handles cannot flip WAL sidecar persistence, reserved/pending writers can, and the decoded file: URI source preserves the setting across close/reopen without ext/sqlite.',
...
if (PHP_SAPI === 'cli' && in_array('--self-test', $argv, true)) {
    ...
    echo "application-vfs-filecontrol-persistwal-lock-current-source-next94 self-test passed\n";
    return;
}
