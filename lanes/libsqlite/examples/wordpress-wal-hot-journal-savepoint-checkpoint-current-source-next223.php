...
$walDigest = $digest('wordpress next223 reset wal generation');
$writerDigest = $digest('wordpress next223 writer generation');
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    assert(in_array('wordpress-import-checkpoint-current-source-publication', $plan['dependencies'], true));
    echo "wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next223 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next223',
    'status' => $plan['status'],
    'publicationAllowed' => $plan['publication_allowed'],
    'currentSourceAction' => $plan['current_source_action'],
    'wordpressUse' => 'A copied wp_options import advances the visible current source only after database, WAL, hot-journal deletion, and reader-cache reopen receipts all match the checkpoint reset generation.',
