...
if (PHP_SAPI === 'cli' && basename(__FILE__) === basename($_SERVER['SCRIPT_FILENAME'] ?? '')) {
    ...
    echo "application-stat4-expression-partial-current-source-next155 self-test passed\n";
}

return [
    'scenario' => 'application-stat4-expression-partial-current-source-next155',
    'status' => $plan['status'],
    ...
    'applicationUse' => 'Copied wp_options plugin scans can reprepare stale STAT4 partial lower(option_name) plans when ANALYZE/DDL narrows the partial predicate, preserving covering index reads while filtering rows that no longer satisfy the current partial index.',
];
