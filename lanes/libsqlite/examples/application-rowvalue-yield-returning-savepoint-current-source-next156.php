...
$plan = SQLiteRowValueUpdateDeleteReturningSavepointPlan::executeYieldReturningSavepointBatch(
    ['wp_options' => $rows],
    [
...
$summary = [
    'scenario' => 'application-rowvalue-yield-returning-savepoint-current-source-next156',
    'status' => $plan['status'],
    ...
    'applicationUse' => 'A copied wp_options import savepoint can yield UPDATE OR IGNORE rows without yielding ignored conflicts, yield UPDATE OR REPLACE rows after deleting the conflicting option row, then run DELETE RETURNING against the current source produced by those row-value updates.',
...
if (
    ...
    fwrite(STDERR, "application-rowvalue-yield-returning-savepoint-current-source-next156 self-test failed\n");
    exit(1);
}

echo json_encode($summary, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) . PHP_EOL;
echo "application-rowvalue-yield-returning-savepoint-current-source-next156 self-test passed\n";
