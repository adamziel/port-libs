...
$root = sys_get_temp_dir() . '/port-libsqlite-wordpress-next158-' . bin2hex(random_bytes(4));
...
if (!is_dir(dirname($localDatabase)) && !mkdir(dirname($localDatabase), 0777, true) && !is_dir(dirname($localDatabase))) {
    throw new RuntimeException('Unable to create WordPress WAL next158 fixture directory');
}

$page = static fn (string $label): string => str_pad($label, $pageSize, '.', STR_PAD_RIGHT);
$cleanPages = [
    1 => $page('wp next158 clean schema before crashed import'),
    2 => $page('wp next158 clean options before crashed import'),
    3 => $page('wp next158 clean plugin settings before crashed import'),
    4 => $page('wp next158 clean autoload index before crashed import'),
];
$dirtyDatabase = $page('wp next158 dirty schema')
    . $page('wp next158 dirty options')
    . $page('wp next158 dirty plugin settings')
    . $page('wp next158 dirty autoload index');
...
foreach ([
    [2, 0, 'wp next158 wal option draft before savepoint'],
    [3, 4, 'wp next158 wal plugin commit before savepoint'],
    [4, 4, 'wp next158 failed autoload savepoint tail'],
...
$applied = (new SQLiteVfsFileWriter($root))->applyWalHotJournalSavepointCheckpointCurrentSourceNext158(
    $stack,
    'autoload-retry-next158',
    $databasePath,
    [1, 2, 3, 4],
    [[
        'pages' => [
            4 => $page('wp next158 retry autoload committed'),
        ],
...
$summary = [
    'scenario' => 'wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next158',
    'wordpressUse' => 'A copied WordPress options import recovers a hot rollback journal, drops the failed savepoint WAL tail, checkpoints the current source after reader release, and starts a separate retry WAL generation without requiring ext/sqlite.',
    ...
    'databaseHasCommittedPlugin' => str_contains($databaseAfter, 'wp next158 wal plugin commit before savepoint'),
    'databaseExcludesFailedTail' => !str_contains($databaseAfter, 'wp next158 failed autoload savepoint tail'),
    'walHasRetry' => str_contains($walAfter, 'wp next158 retry autoload committed'),
    'dependencyClosure' => $applied['recovery']['dependency_closure'],
];

if (($argv[1] ?? '') === '--self-test') {
    if ($summary['status'] !== 'wal-hot-journal-savepoint-checkpoint-current-source-next158'
        ...
        fwrite(STDERR, "wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next158 self-test failed\n");
        exit(1);
    }
    echo "wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next158 self-test passed\n";
    exit(0);
}
