...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "application-wal-checkpoint-reader-savepoint-current-source-next104 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'application-wal-checkpoint-reader-savepoint-current-source-next104',
    'applicationUse' => 'Copied wp_options import rollback can checkpoint the retained WAL prefix while a current reader originally pinned inside rolled-back savepoint frames is rewound to the current source and a next reader after release uses the checkpointed database, without ext/sqlite.',
    'status' => $plan['status'],
