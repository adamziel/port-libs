...
if (($argv[1] ?? '') === '--self-test') {
    ...
    assert($plan['events'][0]['ack'] === 'shared-cache-next737');
    assert($plan['events'][1]['status'] === 'claimed-reusable-current-source');
    assert($plan['events'][1]['claim'] === 'reader-reuse-next753');
    assert($plan['events'][1]['blocked_reasons'] === []);
    assert($plan['events'][2]['status'] === 'published-reused-current-source');
    assert($plan['events'][2]['reuse_ack'] === 'shared-cache-next737');
    assert($plan['events'][2]['token'] === 'shared-cache-next753');
    assert($plan['events'][2]['published_count'] === 38);
    assert(in_array('vfs-current-source-snapshot-reuse-publish-next738-753', $plan['dependencies'], true));
    assert(in_array('vfs-current-source-snapshot-reuse-publish-next722-737', $plan['dependencies'], true));
    echo "application-vfs-current-source-next738-753 self-test passed\n";
    return;
}
