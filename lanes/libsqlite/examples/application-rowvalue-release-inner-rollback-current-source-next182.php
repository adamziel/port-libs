...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "application-rowvalue-release-inner-rollback-current-source-next182 self-test passed\n";
    return;
}

echo json_encode([
    'status' => $plan['status'],
    'applicationUse' => 'Copied wp_options imports can RELEASE an inner savepoint after UPDATE/DELETE RETURNING work, then ROLLBACK TO the outer savepoint and suppress both the outer and released-inner RETURNING streams before retrying from the outer current source.',
