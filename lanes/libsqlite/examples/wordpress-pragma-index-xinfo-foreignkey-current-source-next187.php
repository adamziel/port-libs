...
use PortLibs\LibSqlite\SQLitePragmaIndexXinfoForeignKeyCurrentSourceNext187;
...
$page = SQLitePragmaIndexXinfoForeignKeyCurrentSourceNext187::currentNextPageFromCatalog(
    $currentRecords,
    $tables,
    $nextRecords,
    $tables,
...
$summary = [
    'scenario' => 'wordpress-pragma-index-xinfo-foreignkey-current-source-next187',
    'wordpressUse' => 'Copied wp_options imports must not treat a partial UNIQUE parent index as satisfying SQLite foreign-key parent-key admission, even when PRAGMA index_xinfo columns match.',
    'status' => $page['status'],
...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "wordpress-pragma-index-xinfo-foreignkey-current-source-next187 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-index-xinfo-foreignkey-current-source-next187 self-test passed\n");
    exit(0);
}
