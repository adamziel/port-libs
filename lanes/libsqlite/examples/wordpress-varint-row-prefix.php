...
if ($rowId < 0 || $payloadLength < 0) {
    fwrite(STDERR, "Usage: php lanes/libsqlite/examples/wordpress-varint-row-prefix.php [rowid] [payload-bytes]\n");
    fwrite(STDERR, "Both values must be non-negative integers for this WordPress wp_options cell-prefix preflight.\n");
    exit(1);
}
...
echo json_encode([
    'wordpressUse' => 'Preflight the varint prefixes for a generated wp_options table-leaf cell before writing or repairing a SQLite database image without the SQLite extension.',
    'rowId' => $rowId,
