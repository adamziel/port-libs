...
echo json_encode([
    'wordpressUse' => 'Plan INSERT OR REPLACE for wp_options where a UNIQUE option_name conflict deletes the old row before inserting the incoming rowid.',
    'plan' => $plan->toArray(),
    'options' => array_map(static fn (SQLiteWordPressOption $option): array => $option->toArray(), $postDatabase->wordpressOptions()),
    'indexRecords' => array_map(static fn (SQLiteIndexCell $cell): array => $cell->record()->values, $postDatabase->indexCells(3)),
    'indexedHomeOption' => $postDatabase->wordpressOptionByIndexedName('home')?->toArray(),
