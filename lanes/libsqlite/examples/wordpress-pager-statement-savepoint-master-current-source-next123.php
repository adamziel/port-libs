...
if (($argv[1] ?? '') === '--self-test') {
    ...
    foreach ($checks as $passed) {
        if (!$passed) {
            fwrite(STDERR, "wordpress-pager-statement-savepoint-master-current-source-next123 self-test failed\n");
            exit(1);
        }
    }

    echo "wordpress-pager-statement-savepoint-master-current-source-next123 self-test passed\n";
    exit(0);
}
