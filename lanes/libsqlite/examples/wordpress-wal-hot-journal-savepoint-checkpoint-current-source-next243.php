...
$databaseDigest = $hash('wordpress next243 checkpoint database image');
$pageCacheDigest = $hash('wordpress next243 reopened clean page cache');
$baselinePlan = [
...
$plan = SQLiteWalHotJournalSavepointCheckpointCurrentSourceNext243Plan::admitReaderSnapshotBaseline($baselinePlan, [
...
$summary = [
    'scenario' => 'wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next243',
    'wordpressUse' => 'A copied WordPress import reopens schema, wp_options, and autoload-index readers after a hot-journal savepoint checkpoint only when their WAL-index readmarks, clean page-cache digest, commit frames, and shared locks match the autocheckpoint baseline.',
    'status' => $plan['status'],
...
if (($argv[1] ?? null) === '--self-test') {
    if ($summary['status'] !== 'wal-hot-journal-savepoint-checkpoint-current-source-next243'
        ...
        fwrite(STDERR, "wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next243 self-test failed\n");
        exit(1);
    }

    echo "wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next243 self-test passed\n";
    exit(0);
}
