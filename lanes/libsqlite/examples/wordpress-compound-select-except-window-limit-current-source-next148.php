...
$plan = SQLiteCompoundSelectExceptWindowLimitCurrentSourceNext148Plan::compare($sql, $currentTables, $nextTables);
$result = [
    ...
    'wordpressUse' => 'Copied wp_options import audits can subtract already-current and stale audit rows after per-arm window ranks are evaluated, then apply the final compound ORDER BY and comma LIMIT before showing the changed option boundary.',
...
if (PHP_SAPI === 'cli' && basename(__FILE__) === basename($_SERVER['SCRIPT_FILENAME'] ?? '')) {
    ...
    echo "wordpress-compound-select-except-window-limit-current-source-next148 self-test passed\n";
}
