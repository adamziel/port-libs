...
use PortLibs\LibSqlite\SQLitePlannerStat4ExpressionPartialCurrentSourceNext189Plan;
...
$prepared = [
    'name' => 'prepared-wp-options-next189',
    'schemaCookie' => 1890,
    'stat4Generation' => 141,
    'rows' => [
...
$current = $prepared;
$current['name'] = 'current-wp-options-next189';
$current['schemaCookie'] = 1899;
$current['stat4Generation'] = 166;
$current['indexes'][0]['rootPage'] = 18988;
...
$plan = SQLitePlannerStat4ExpressionPartialCurrentSourceNext189Plan::materialize(
    $prepared,
    $current,
    [
...
if (in_array('--self-test', $argv, true)) {
    assert($plan['status'] === 'stat4-expression-partial-current-source-next189-ready');
    assert($plan['currentPayloadPartialFence']['ready'] === true);
    assert($plan['currentPayloadPartialFence']['rejectedRowids'] === []);
    assert(array_column($plan['currentPayloadPartialFence']['checks'], 'actualExpressionKey') === ['plugin_seo', 'plugin_mail', 'plugin_forms', 'plugin_forms']);
    echo "wordpress-sqlplanner-stat4-expression-partial-current-source-next189 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'wordpress-sqlplanner-stat4-expression-partial-current-source-next189',
    'wordpressUse' => 'Copied wp_options plugin admin pagination can admit a current-source STAT4 partial expression index only after each row payload still satisfies lower(option_name), autoload, and NOT NULL partial predicates.',
    'status' => $plan['status'],
    'selectedSource' => $plan['selectedSource'],
    'selectedIndex' => $plan['selectedPlan']['name'] ?? null,
    'payloadPartialFence' => $plan['currentPayloadPartialFence'],
