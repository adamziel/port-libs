...
$plan = SQLiteBTreeVacuumPointerMapFreeblockCurrentSourcePlan::tableLeafWriteAdmissionFromDeleteResult(
    $database,
    3,
    [
        'page' => $deletedPage,
    ...
    2,
    str_repeat('next162-multisite-transient-rewrite-', 42),
    3,
    true,
);

$summary = [
    'scenario' => 'application-btree-vacuum-pointermap-freeblock-current-source-next162',
    'applicationUse' => 'Delete a copied wp_options transient, vacuum tail overflow pages, reuse surviving free pages for a replacement overflow chain, then admit only current-source pages that remain inside the next database image for writing.',
    'writable_page_numbers' => $plan->writablePageNumbers(),
    'pointer_map_write_pages' => $plan->pointerMapWritePages(),
    'rejected_truncated_pages' => $plan->rejectedTruncatedPages(),
    'replacement_overflow_pages' => $plan->basePlan->replacementOverflowPages(),
    'replacement_overflow_next_pages' => $plan->basePlan->replacementOverflowNextPages(),
    'replacement_pointer_map_parents' => $plan->basePlan->replacementPointerMapParents(),
];

if (($argv[1] ?? null) === '--self-test') {
    $expected = [
        'writable_page_numbers' => [1, 3, 105, 106, 107, 108],
        'pointer_map_write_pages' => [105],
        'rejected_truncated_pages' => [109, 110],
    ...
    foreach ($expected as $key => $value) {
        if ($summary[$key] !== $value) {
            fwrite(STDERR, "application-btree-vacuum-pointermap-freeblock-current-source-next162 self-test failed\n");
            exit(1);
        }
    }

    fwrite(STDOUT, "application-btree-vacuum-pointermap-freeblock-current-source-next162 self-test passed\n");
    exit(0);
}
