...
if (($argv[1] ?? '') === '--self-test') {
    assert($plan['status'] === 'compound-recursive-affinity-window-current-source-next142-ready');
    ...
    echo "wordpress-compound-recursive-affinity-window-current-source-next142 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'wordpress-compound-recursive-affinity-window-current-source-next142',
    'sqlShape' => 'WITH RECURSIVE ... UNION ... SELECT window(...) FROM cte UNION SELECT window(...) FROM wp_options ORDER BY left-most columns',
    'wordpressUse' => 'Copied wp_options repair/import diagnostics can compare current and next option dependency walks while preserving recursive UNION numeric-affinity deduplication, per-arm window evaluation, and left-most compound output names before committing import changes.',
