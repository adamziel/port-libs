...
$payload = [
    'applicationUse' => 'Preview copied wp_options text through bounded FTS5-style MATCH ranking and snippet diagnostics before a native virtual-table executor is available, without requiring ext/sqlite.',
    'query' => $query,
...
if (($argv[1] ?? '') === '--self-test') {
    if ($payload['selectedOptionIds'] !== [3]) {
        fwrite(STDERR, "application-fts5-option-search self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "application-fts5-option-search self-test passed\n");
    return;
}
