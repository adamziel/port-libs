...
if ($plan['status'] !== 'rolled-back' || $plan['rows'] !== $rows || count($plan['yield_stream']) !== 2 || count($plan['skipped']) !== 1) {
    fwrite(STDERR, "application-trigger-returning-savepoint self-test failed\n");
    exit(1);
}

fwrite(STDOUT, "application-trigger-returning-savepoint self-test passed\n");
fwrite(STDOUT, json_encode([
    'scenario' => 'application-trigger-returning-savepoint',
    'status' => $plan['status'],
    ...
    'applicationUse' => 'Preview wp_options trigger RETURNING diagnostics when RAISE(IGNORE) skips plugin-owned rows and a later trigger RAISE(ROLLBACK) restores the import savepoint.',
