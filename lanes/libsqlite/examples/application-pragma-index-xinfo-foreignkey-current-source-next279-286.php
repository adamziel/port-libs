...
$summary = [
    'scenario' => 'application-pragma-index-xinfo-foreignkey-current-source-next279-286',
    'applicationUse' => 'Application-style plugin schemas can preflight copied foreign keys against PRAGMA foreign_key_list, table_info, index_list, and index_xinfo before activating constraints.',
...
if (($argv[1] ?? null) === '--self-test') {
    ...
    foreach ($expected as $key) {
        if ($summary[$key] !== 1) {
            fwrite(STDERR, "application-pragma-index-xinfo-foreignkey-current-source-next279-286 self-test failed\n");
            exit(1);
        }
    }
    ...
    fwrite(STDOUT, "application-pragma-index-xinfo-foreignkey-current-source-next279-286 self-test passed\n");
    exit(0);
}
