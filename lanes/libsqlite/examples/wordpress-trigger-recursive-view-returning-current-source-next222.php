...
use PortLibs\LibSqlite\SQLiteTriggerRecursiveViewReturningCurrentSourceNext222Plan;
...
$nextView = $view;
$nextView['source'] = 'main@view-cookie-222-next';
$nextView['trigger_source'] = 'main@trigger-cookie-222-next';
$postResetView = $view;
$postResetView['source'] = 'main@view-cookie-222-post-reset';
$postResetView['trigger_source'] = 'main@trigger-cookie-222-post-reset';
$followingView = $view;
$followingView['source'] = 'main@view-cookie-222-following';
$followingView['trigger_source'] = 'main@trigger-cookie-222-following';

$summary = SQLiteTriggerRecursiveViewReturningCurrentSourceNext222Plan::execute(
    [
...
if (
    $summary['status_next222'] !== 'trigger-recursive-view-returning-current-source-next222-source-ticket-released'
    || $summary['current_source_ticket_plan_next222']['decision'] !== 'publish-next-source-after-current-source-ticket'
    || array_column($summary['visible_returning_payloads_next222'], 'name') !== ['blogdescription_child', 'template_child', 'home', 'next_plugin']
    || $summary['held_next_source_rows_next222'] !== []
) {
    fwrite(STDERR, "wordpress-trigger-recursive-view-returning-current-source-next222 self-test failed\n");
    exit(1);
}

echo "wordpress-trigger-recursive-view-returning-current-source-next222 self-test passed\n";
