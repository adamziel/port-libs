...
if (
    ...
    fwrite(STDERR, "wordpress-pager-cache-spill-savepoint-current-source-next137 self-test failed\n");
    exit(1);
}

if (PHP_SAPI === 'cli' && realpath($_SERVER['SCRIPT_FILENAME'] ?? '') === __FILE__) {
    echo "wordpress-pager-cache-spill-savepoint-current-source-next137 self-test passed\n";
}

return [
    'scenario' => 'wordpress-pager-cache-spill-savepoint-current-source-next137',
    'wordpressUse' => 'During a copied wp_options import, dirty cache pages may spill while a plugin-batch savepoint is open only when their current source still matches the database image and the savepoint has a before-image that can undo the spill.',
    'status' => $plan['status'],
