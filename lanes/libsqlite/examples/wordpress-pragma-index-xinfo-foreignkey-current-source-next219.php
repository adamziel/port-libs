...
$page = SQLitePragmaIndexXinfoForeignKeyCurrentSourceNext219::page(
    $current,
    $next,
...
$summary = [
    'scenario' => 'wordpress-pragma-index-xinfo-foreignkey-current-source-next219',
    'wordpressUse' => 'Copied taxonomy/termmeta imports can reject a parent-key repair when PRAGMA index_xinfo shows a UNIQUE parent index with the same columns in the wrong order for SQLite foreign-key parent-key admission.',
...
if (($argv[1] ?? '') === '--self-test') {
    if (
        ...
        fwrite(STDERR, "wordpress-pragma-index-xinfo-foreignkey-current-source-next219 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-index-xinfo-foreignkey-current-source-next219 self-test passed\n");
    exit(0);
}
