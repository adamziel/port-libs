...
$page = SQLitePragmaIndexXinfoForeignKeyCurrentSourceNext::currentNextPageFromCatalog166(
    $currentRecords,
    $currentTables,
    $nextRecords,
    $nextTables,
...
if (($argv[1] ?? null) === '--self-test') {
    if (
        $page['status'] !== 'ok'
        || $page['current']['foreign_key_violations'] !== 4
        || $page['next_counts']['foreign_key_violations'] !== 0
        || $page['current_source']['foreign_key_actions']['on_update']['CASCADE'] !== 1
        || $page['next_source']['foreign_key_actions']['on_update']['RESTRICT'] !== 1
        || $page['current_source']['foreign_key_actions']['initially_deferred'] !== 1
    ) {
        fwrite(STDERR, "application-pragma-index-xinfo-foreignkey-current-source-next166 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "application-pragma-index-xinfo-foreignkey-current-source-next166 self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'application-pragma-index-xinfo-foreignkey-current-source-next166',
    'applicationUse' => 'Copied multisite wp_options imports can resume PRAGMA index_xinfo plus foreign_key_check only when FK action, MATCH, and deferrability metadata still match the current catalog image.',
    'status' => $page['status'],
    ...
    'current_actions' => $page['current_source']['foreign_key_actions'],
    'next_actions' => $page['next_source']['foreign_key_actions'],
