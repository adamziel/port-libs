...
$summary = [
    'scenario' => 'application-rowvalue-rollback-transaction-savepoint',
    'applicationUse' => 'A copied wp_options import that hits UPDATE OR ROLLBACK inside a cleanup savepoint discards outer and savepoint RETURNING rows, restores the transaction image, and retries UPDATE/DELETE RETURNING from the restored current source.',
    'status' => $plan['status'],
...
if (
    ...
    fwrite(STDERR, "application-rowvalue-rollback-transaction-savepoint self-test failed\n");
    exit(1);
}

if (PHP_SAPI === 'cli' && realpath($_SERVER['SCRIPT_FILENAME'] ?? '') === __FILE__) {
    echo "application-rowvalue-rollback-transaction-savepoint self-test passed\n";
