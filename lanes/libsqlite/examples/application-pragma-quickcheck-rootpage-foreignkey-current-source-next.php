...
if (($argv[1] ?? null) === '--self-test') {
    if ($page['status'] !== 'ok' || $page['current']['quick_check_errors'] !== 4 || $page['current']['foreign_key_violations'] !== 2 || $page['delta']['cleared'] !== true) {
        fwrite(STDERR, "application-pragma-quickcheck-rootpage-foreignkey-current-source-next self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "application-pragma-quickcheck-rootpage-foreignkey-current-source-next self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'application-pragma-quickcheck-rootpage-foreignkey-current-source-next',
    'applicationUse' => 'Gate copied wp_options imports on one resumable current-source stream that clears quick_check rootpage diagnostics and foreign_key_check rootpage blockers together.',
    'status' => $page['status'],
