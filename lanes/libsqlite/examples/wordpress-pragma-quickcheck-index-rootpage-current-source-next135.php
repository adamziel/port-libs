...
use PortLibs\LibSqlite\SQLitePragmaQuickcheckIndexRootpageCurrentSourceNext135;
...
$page = SQLitePragmaQuickcheckIndexRootpageCurrentSourceNext135::page(
    $catalog,
...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "wordpress-pragma-quickcheck-index-rootpage-current-source-next135 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-quickcheck-index-rootpage-current-source-next135 self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'wordpress-pragma-quickcheck-index-rootpage-current-source-next135',
    'wordpressUse' => 'Resume copied wp_options expression-index quick_check pages only while index_xinfo metadata and sqlite_schema rootpage state still identify the same database image.',
    'status' => $page['status'],
