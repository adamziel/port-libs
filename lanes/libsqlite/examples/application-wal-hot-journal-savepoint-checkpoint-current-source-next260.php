...
$sourceToken = 'application-import-current-source-next260';
$generation = 260;
$schemaCookie = 4260;
$checkpointFrame = 72;
$commitFrames = [68, 69, 72];
$databaseDigest = $digest('application next260 checkpoint database');
$pageCacheDigest = $digest('application next260 checkpoint cache');
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "application-wal-hot-journal-savepoint-checkpoint-current-source-next260 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'application-wal-hot-journal-savepoint-checkpoint-current-source-next260',
    'status' => $plan['status'],
    ...
    'applicationUse' => 'A copied wp_options plugin import only publishes the post-hot-journal checkpoint as the current source after savepoint WAL prefix, checkpoint sync, and reopened reader tokens all match.',
