...
$databaseDigest = $digest('application next239 copied wp_options checkpoint image');
$finalizerPlan = [
    ...
    'source_token' => 'application-import-next239',
...
$plan = SQLiteWalHotJournalSavepointCheckpointCurrentSourceNextPlan::next239AdmitAtomicCommitBarrier($finalizerPlan, [
    $receipt('application-next239-database', 'database', ['select-wp-options', 'select-active-plugins']),
    $receipt('application-next239-wal', 'wal', ['select-theme-mods']),
    $receipt('application-next239-journal', 'journal', ['select-wp-options']),
    $receipt('application-next239-directory', 'directory', ['select-wp-options', 'select-active-plugins', 'select-theme-mods']),
]);
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    assert(in_array('application-import-atomic-current-source-switch', $plan['dependencies'], true));
    echo "application-wal-hot-journal-savepoint-checkpoint-current-source-next239 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'application-wal-hot-journal-savepoint-checkpoint-current-source-next239',
    'status' => $plan['status'],
    ...
    'applicationUse' => 'A copied wp_options import switches reopened readers to the checkpoint current source only after database, WAL, hot-journal delete, and directory fsync receipts all cover finalized statements under one atomic commit barrier.',
