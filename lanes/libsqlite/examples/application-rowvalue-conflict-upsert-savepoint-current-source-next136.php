...
$summary = [
    'scenario' => 'application-rowvalue-conflict-upsert-savepoint-current-source-next136',
    'applicationUse' => 'Preview copied wp_options imports where an UPSERT row-value assignment moves a composite conflict key, later rows must resolve against that moved current-source key, and a later duplicate composite key rolls the savepoint back to the original source.',
    'status' => $plan['status'],
...
if (PHP_SAPI === 'cli' && realpath($_SERVER['SCRIPT_FILENAME'] ?? '') === __FILE__) {
    ...
    echo "application-rowvalue-conflict-upsert-savepoint-current-source-next136 self-test passed\n";
}
