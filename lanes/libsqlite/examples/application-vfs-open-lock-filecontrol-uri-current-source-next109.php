...
$summary = [
    'scenario' => 'application-vfs-open-lock-filecontrol-uri-current-source-next109',
    'applicationUse' => 'Expose SQLite-compatible URI helper semantics to a custom VFS during copied wp_options import/repair opens, including yes/on booleans, invalid integer fallback to zero, defaults for missing values, and current-source data-version freshness.',
    'status' => $plan['status'],
    'importHandle' => [
...
if (($argv[1] ?? null) === '--self-test') {
    ...
    echo "application-vfs-open-lock-filecontrol-uri-current-source-next109 self-test passed\n";
    return;
}
