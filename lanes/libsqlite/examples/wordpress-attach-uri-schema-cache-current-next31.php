...
$first = $cache->attach(
    ...
    "ATTACH 'file:/srv/wordpress/cache%20copy.sqlite?mode=ro&cache=shared&immutable=1' AS cache_a",
    $loader,
    17,
    17,
);
...
$second = $cache->attach(
    $secondCatalog,
    "ATTACH 'file:/srv/wordpress/cache%20copy.sqlite?mode=ro&cache=shared&immutable=1' AS cache_b",
    $loader,
    17,
    18,
);
...
$summary = [
    ...
    'wordpressUse' => 'Reuse a copied WordPress attached database schema for file: URI opens only while cache=shared and the schema cookie remains current; a changed next schema cookie schedules a reload before the next attach.',
];
...
if (($argv[1] ?? '') === '--self-test') {
    $expected = [
        ...
        'loader_calls' => 1,
        'decoded_file' => '/srv/wordpress/cache copy.sqlite',
        'schema_cookie' => 17,
    ...
    foreach ($expected as $key => $value) {
        if ($summary[$key] !== $value) {
            fwrite(STDERR, "wordpress-attach-uri-schema-cache-current-next31 self-test failed at {$key}\n");
            exit(1);
        }
    }

    echo "wordpress-attach-uri-schema-cache-current-next31 self-test passed\n";
    exit(0);
}
