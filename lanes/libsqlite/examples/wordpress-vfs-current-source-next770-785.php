...
if (($argv[1] ?? '') === '--self-test') {
    ...
    assert($plan['events'][0]['ack'] === 'shared-cache-next769');
    assert($plan['events'][1]['status'] === 'claimed-reusable-current-source');
    assert($plan['events'][1]['claim'] === 'reader-reuse-next785');
    assert($plan['events'][1]['blocked_reasons'] === []);
    assert($plan['events'][2]['status'] === 'published-reused-current-source');
    assert($plan['events'][2]['reuse_ack'] === 'shared-cache-next769');
    assert($plan['events'][2]['token'] === 'shared-cache-next785');
    assert($plan['events'][2]['published_count'] === 40);
    assert(in_array('vfs-current-source-snapshot-reuse-publish-next770-785', $plan['dependencies'], true));
    assert(in_array('vfs-current-source-snapshot-reuse-publish-next754-769', $plan['dependencies'], true));
    echo "wordpress-vfs-current-source-next770-785 self-test passed\n";
    return;
}
