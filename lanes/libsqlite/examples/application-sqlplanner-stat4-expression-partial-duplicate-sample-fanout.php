...
if (in_array('--self-test', $argv, true)) {
    ...
    echo "application-sqlplanner-stat4-expression-partial-duplicate-sample-fanout self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'application-sqlplanner-stat4-expression-partial-duplicate-sample-fanout',
    'applicationUse' => 'Preview copied wp_options plugin scans after ANALYZE keeps one STAT4 sample for duplicate lower(option_name) keys, expanding the current partial expression-index rowid fanout while blocking stale prepared rows.',
    'status' => $plan['status'],
