...
$summary = [
    'scenario' => 'application-pragma-index-xinfo-foreignkey-action-relationship-diagnostics',
    'applicationUse' => 'Application import previews can page staged foreign-key action relationship diagnostics while preserving order, collation, and DESC child lookup mismatch coverage.',
...
if (($argv[1] ?? null) === '--self-test') {
    foreach ($cases as $slice => [, , , $status]) {
        if (($summary["next{$slice}_current_{$status}"] ?? null) !== 0 || ($summary["next{$slice}_next_{$status}"] ?? null) !== 1) {
            fwrite(STDERR, "application-pragma-index-xinfo-foreignkey-action-relationship-diagnostics self-test failed\n");
            exit(1);
        }
    }
    ...
    fwrite(STDOUT, "application-pragma-index-xinfo-foreignkey-action-relationship-diagnostics self-test passed\n");
    exit(0);
}
