...
$plan = SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan::tableLeafFromDeleteResultNext169(
    ...
    2,
    str_repeat('next169-wordpress-transient-gate-', 44),
    3,
    true,
    $staleCurrentPages,
);
...
$summary = [
    'scenario' => 'wordpress-btree-vacuum-pointermap-freeblock-current-source-next169',
    'wordpressUse' => 'Delete a copied wp_options transient, vacuum obsolete overflow tail pages, and reject stale current-source pages before admitting replacement overflow writes.',
...
if (($argv[1] ?? null) === '--self-test') {
    ...
    foreach ($expected as $key => $value) {
        if ($summary[$key] !== $value) {
            fwrite(STDERR, "wordpress-btree-vacuum-pointermap-freeblock-current-source-next169 self-test failed\n");
            exit(1);
        }
    }

    fwrite(STDOUT, "wordpress-btree-vacuum-pointermap-freeblock-current-source-next169 self-test passed\n");
    exit(0);
}
