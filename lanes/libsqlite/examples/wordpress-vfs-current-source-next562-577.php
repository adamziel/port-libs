...
require_once __DIR__ . '/../src/SQLiteVfsCurrentSourceNext562577Plan.php';

use PortLibs\LibSqlite\SQLiteVfsCurrentSourceNext562577Plan;
...
$plan = SQLiteVfsCurrentSourceNext562577Plan::run([
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    assert($plan['events'][0]['ack'] === 'shared-cache-next561');
    assert($plan['events'][1]['status'] === 'claimed-reusable-current-source');
    assert($plan['events'][1]['claim'] === 'reader-reuse-next577');
    assert($plan['events'][1]['blocked_reasons'] === []);
    assert($plan['events'][2]['status'] === 'published-reused-current-source');
    assert($plan['events'][2]['reuse_ack'] === 'shared-cache-next561');
    assert($plan['events'][2]['token'] === 'shared-cache-next577');
    assert($plan['events'][2]['published_count'] === 28);
    assert(in_array('vfs-current-source-snapshot-reuse-publish-next562-577', $plan['dependencies'], true));
    assert(in_array('vfs-current-source-snapshot-reuse-publish-next546-561', $plan['dependencies'], true));
    ...
    echo "wordpress-vfs-current-source-next562-577 self-test passed\n";
    return;
}
