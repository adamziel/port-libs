...
$plan = SQLiteWalSavepointCheckpointPlan::readerCheckpointSavepointReaderRestartCurrentSourceNext151(
    $stack,
...
$summary = [
    'scenario' => 'wordpress-wal-checkpoint-savepoint-reader-restart-current-source-next151',
    'wordpressUse' => 'A copied WordPress options import rolls back a plugin-setting savepoint while an existing reader pins the old WAL frames; after RESTART checkpoint release, a reopened reader binds to the fresh current source before retry writes append.',
    'status' => $plan['status'],
...
if (PHP_SAPI === 'cli' && basename(__FILE__) === basename($_SERVER['SCRIPT_FILENAME'] ?? '')) {
    if ($summary['status'] !== 'wal-checkpoint-savepoint-reader-restart-current-source-next151'
        ...
        fwrite(STDERR, "wordpress-wal-checkpoint-savepoint-reader-restart-current-source-next151 self-test failed\n");
        exit(1);
    }

    echo "wordpress-wal-checkpoint-savepoint-reader-restart-current-source-next151 self-test passed\n";
}
