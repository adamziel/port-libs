...
if (
    ...
    fwrite(STDERR, "application-dml-trigger-returning-conflict-current-source-next106 self-test failed\n");
    exit(1);
}

echo "application-dml-trigger-returning-conflict-current-source-next106 self-test passed\n";
echo json_encode([
    'scenario' => 'application-dml-trigger-returning-conflict-current-source-next106',
    'applicationUse' => 'Preview copied wp_options INSERT OR REPLACE RETURNING where BEFORE triggers retarget incoming rows, current-source conflicts delete old rows before insert, and RETURNING preserves the statement insert image before AFTER-trigger touches.',
