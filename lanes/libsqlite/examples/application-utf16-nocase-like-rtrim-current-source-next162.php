...
require_once __DIR__ . '/../src/SQLiteUtf16NoCaseLikeRtrimPatternCurrentSourceNextPlan.php';
require_once __DIR__ . '/../src/SQLiteUtf16NocaseLikeRtrimCurrentSourceNextPlan.php';
...
$plan = SQLiteUtf16NocaseLikeRtrimCurrentSourceNextPlan::optionRowNameNormalizedPatternPlan(
    $rows,
    $rows,
...
if (($argv[1] ?? null) === '--self-test') {
    ...
    echo "application-utf16-nocase-like-rtrim-current-source-next162 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'application-utf16-nocase-like-rtrim-current-source-next162',
    'applicationUse' => 'Copied wp_options scans can reuse a NOCASE LIKE RTRIM cursor when a prepared UTF-16 pattern is rebound with equivalent decoded text but different byte order.',
