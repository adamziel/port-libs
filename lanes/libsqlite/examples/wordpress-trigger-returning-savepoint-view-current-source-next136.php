...
$summary = [
    'scenario' => 'wordpress-trigger-returning-savepoint-view-current-source-next136',
    'wordpressUse' => 'Copied wp_options imports through an INSTEAD OF view trigger can roll back a failed current-source row to its savepoint while still admitting next-source RETURNING rows from the saved current image, without requiring ext/sqlite.',
    'status' => $plan['status'],
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "wordpress-trigger-returning-savepoint-view-current-source-next136 self-test passed\n";
    return;
}
