...
$published = [
    ...
    'shared-cache-next1121',
    'shared-cache-next1137',
    'shared-cache-next1153',
];
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    assert($plan['events'][0]['ack'] === 'shared-cache-next1153');
    assert($plan['events'][0]['blocked_reasons'] === []);
    assert($plan['events'][1]['status'] === 'claimed-reusable-current-source');
    assert($plan['events'][1]['claim'] === 'reader-reuse-next1169');
    assert($plan['events'][1]['blocked_reasons'] === []);
    assert($plan['events'][2]['status'] === 'published-reused-current-source');
    assert($plan['events'][2]['reuse_ack'] === 'shared-cache-next1153');
    assert($plan['events'][2]['token'] === 'shared-cache-next1169');
    assert($plan['events'][2]['published_count'] === 64);
    assert(in_array('vfs-current-source-snapshot-reuse-publish-next1154-1169', $plan['dependencies'], true));
    assert(in_array('vfs-current-source-snapshot-reuse-publish-next1138-1153', $plan['dependencies'], true));
    echo "application-vfs-current-source-next1154-1169 self-test passed\n";
    return;
}
