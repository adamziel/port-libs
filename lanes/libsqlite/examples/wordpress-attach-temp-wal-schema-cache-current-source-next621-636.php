...
$schemas = [
    ...
    'archive' => ['schema_cookie' => 572, 'tables' => ['wp_schema_archive_retry_next620'], 'indexes' => ['wp_schema_archive_retry_key_next620'], 'file' => '/srv/wp/archive-next621.sqlite'],
    'handoff' => ['schema_cookie' => 585, 'tables' => ['wp_schema_handoff_next582', 'wp_schema_handoff_meta_next621'], 'indexes' => ['wp_schema_handoff_key_next582', 'wp_schema_handoff_meta_key_next621'], 'file' => '/srv/wp/handoff-next621.sqlite'],
    'audit' => ['schema_cookie' => 611, 'tables' => ['wp_schema_audit_next610', 'wp_schema_audit_meta_next611'], 'indexes' => ['wp_schema_audit_key_next610', 'wp_schema_audit_meta_key_next611'], 'file' => '/srv/wp/audit-next621.sqlite'],
    'queue' => ['schema_cookie' => 593, 'tables' => ['wp_job_retry_checkpoint_final_next607', 'wp_job_retry_checkpoint_preview_next620'], 'indexes' => ['wp_job_retry_checkpoint_final_job_next607', 'wp_job_retry_checkpoint_preview_job_next620'], 'file' => '/srv/wp/queue-next621.sqlite'],
];
...
if (($argv[1] ?? '') === '--self-test') {
    assert($plan['operation'] === 'attach-wal-temp-schema-cache-current-source-next621-636');
    assert($plan['dependencies'][0] === 'sqlite-attach-temp-wal-schema-cache-current-source-next621');
    assert($plan['dependencies'][15] === 'sqlite-attach-temp-wal-schema-cache-current-source-next636');
    assert(in_array('sqlite-attach-temp-wal-schema-cache-current-source-next620', $plan['dependencies'], true));
    assert($plan['event_count'] === 9);
    assert($plan['schema_cookies_next']['main'] === 636);
    assert($plan['schema_cookies_next']['queue'] === 594);
    assert($plan['schema_cookies_next']['handoff'] === 586);
    assert($plan['schema_cookies_next']['archive'] === 573);
    assert($plan['schema_cookies_next']['review'] === 627);
    ...
    echo "wordpress-attach-temp-wal-schema-cache-current-source-next621-636 self-test passed\n";
    return;
}
