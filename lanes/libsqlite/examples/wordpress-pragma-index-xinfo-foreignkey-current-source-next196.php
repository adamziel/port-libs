...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "wordpress-pragma-index-xinfo-foreignkey-current-source-next196 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-index-xinfo-foreignkey-current-source-next196 self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'wordpress-pragma-index-xinfo-foreignkey-current-source-next196',
    'wordpressUse' => 'Before applying a copied taxonomy schema reparse, page the current and next PRAGMA index_xinfo plus foreign_key_list rows together so expression-index metadata and composite foreign-key metadata advance from the same catalog snapshot.',
    'status' => $page['status'],
