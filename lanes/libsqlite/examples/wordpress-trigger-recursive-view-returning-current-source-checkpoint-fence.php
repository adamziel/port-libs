...
if (PHP_SAPI === 'cli' && basename(__FILE__) === basename($_SERVER['SCRIPT_FILENAME'] ?? '')) {
    ...
    echo "wordpress-trigger-recursive-view-returning-current-source-next175 self-test passed\n";
}

return [
    'scenario' => 'wordpress-trigger-recursive-view-returning-current-source-next175',
    'wordpressUse' => 'Copied wp_options imports through an INSTEAD OF recursive view trigger can drain current-source RETURNING pages before a savepoint release exposes queued next-source rows, while rollback keeps the next source fenced.',
