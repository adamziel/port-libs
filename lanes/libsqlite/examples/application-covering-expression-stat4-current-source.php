...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "application-covering-expression-stat4-current-source-canonical self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'application-covering-expression-stat4-current-source-canonical',
    'status' => $plan['status'],
    'selectedSource' => $plan['selectedSource'],
    'index' => $plan['selectedPlan']['name'] ?? null,
    'matchedKeys' => $plan['cursorTape']['matchedKeys'],
    'tableLookupElided' => $plan['tableLookupElided'],
    'applicationUse' => 'Preview copied wp_options plugin option scans after ANALYZE refresh: stale prepared expression-index plans reprepare to the current STAT4 source and keep option payload reads on the covering index cursor.',
