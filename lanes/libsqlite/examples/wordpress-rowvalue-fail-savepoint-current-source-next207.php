...
$outer = "UPDATE wp_options SET (status, option_value, bytes) = ('outer207', option_value || ':outer207', bytes + 1) WHERE (blog_id, option_name) IN ((4, 'rewrite_rules')) RETURNING option_id, blog_id, option_name, status, option_value, bytes ORDER BY option_id";
$fail = "UPDATE OR FAIL wp_options SET (option_name, status, option_value, bytes) = ('transient207', 'fail207', option_value || ':fail207', bytes + 5) WHERE (blog_id, option_name) IN (VALUES (2, 'draft_feed'), (1, 'draft_conflict'), (3, 'draft_later')) RETURNING option_id, blog_id, option_name, status, option_value, bytes, (blog_id, option_name) IS (2, 'transient207') AS prefix_tuple_is ORDER BY option_id";
...
$plan = SQLiteRowValueUpdateDeleteReturningSavepointCurrentSourceNext207Plan::execute(
    ['wp_options' => $rows],
    [$outer],
    [$fail],
    [$retry, $delete],
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    if (
        $plan['status'] !== 'rowvalue-update-delete-returning-or-fail-savepoint-current-source-next207'
        ...
        fwrite(STDERR, "wordpress-rowvalue-fail-savepoint-current-source-next207 self-test failed\n");
        exit(1);
    }
    echo "wordpress-rowvalue-fail-savepoint-current-source-next207 self-test passed\n";
    exit(0);
}
