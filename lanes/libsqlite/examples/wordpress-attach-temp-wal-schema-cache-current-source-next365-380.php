...
$plan = SQLiteAttachWalTempSchemaCacheCurrentSourceNext92Plan::currentSourceNext365380($schemas, $statements, [
...
if (($argv[1] ?? '') === '--self-test') {
    assert($plan['operation'] === 'attach-wal-temp-schema-cache-current-source-next365-380');
    assert($plan['dependencies'][0] === 'sqlite-attach-temp-wal-schema-cache-current-source-next365');
    assert($plan['dependencies'][15] === 'sqlite-attach-temp-wal-schema-cache-current-source-next380');
    assert(in_array('sqlite-attach-temp-wal-schema-cache-current-source-next364', $plan['dependencies'], true));
