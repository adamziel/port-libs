...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "wordpress-rowvalue-assignment-savepoint-current-source-next191 self-test passed\n";
    return;
}

echo json_encode([
    'wordpressUse' => 'Model copied wp_options cleanup where row-value predicates are assigned into flag columns, speculative RETURNING rows are rolled back, and retry starts from the restored savepoint image.',
