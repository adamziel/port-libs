...
use PortLibs\LibSqlite\SQLitePlannerStat4ExpressionPartialCurrentSourceNext246Plan;
...
$plan = SQLitePlannerStat4ExpressionPartialCurrentSourceNext246Plan::materialize(
    $prepared,
    $current,
    [
...
if (in_array('--self-test', $argv, true)) {
    ...
    echo "wordpress-sqlplanner-stat4-expression-partial-current-source-next246 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'wordpress-sqlplanner-stat4-expression-partial-current-source-next246',
    'wordpressUse' => 'Copied wp_options plugin scans can reuse a current STAT4 partial expression index only after current sqlite_stat4 neq duplicate counts match the duplicate option_name expression buckets.',
    'status' => $plan['status'],
