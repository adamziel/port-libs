...
$plan = SQLiteJsonTablePlan::currentSourceIndexedHiddenOrderNext122(
    'json_tree',
    $currentOption,
    $nextOption,
    'option_value',
...
$payload = [
    'scenario' => 'wordpress-json-table-indexed-hidden-order-current-source-next122',
    'wordpressUse' => 'Copied wp_options JSON import previews can keep an indexed json_tree() constraint while detecting ORDER BY over hidden json/root source terms when the next option payload changes without changing visible priority rows.',
...
if (($argv[1] ?? null) === '--self-test') {
    ...
    echo "wordpress-json-table-indexed-hidden-order-current-source-next122 self-test passed\n";
    return;
}
