...
$optionValue = $argv[2] ?? 'composite-parent-grown-value';
...
foreach (['r', 's', 't', 'u', 'v', 'w'] as $index => $prefix) {
    $rightLeafCells[] = SQLiteIndexCell::encode(
        SQLiteRecord::encode(['yes', str_repeat($prefix, $nameLength), 200 + $index]),
    );
}
...
$plan = $database->planWordPressOptionInsert(2, $optionName, $optionValue, 'yes');
...
echo json_encode([
    'wordpressUse' => 'Plan a generated wp_options insert that splits a full composite autoload, option_name leaf and grows a full index-interior root, without the SQLite extension.',
    'plan' => $plan->toArray(),
    ...
    'insertedOption' => $postDatabase->wordpressOptionByIndexedAutoloadAndName('yes', $optionName)?->toArray(),
