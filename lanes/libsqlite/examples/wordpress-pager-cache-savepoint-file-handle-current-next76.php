...
$root = sys_get_temp_dir() . '/port-libsqlite-wordpress-pager-cache-savepoint-' . bin2hex(random_bytes(4));
...
try {
    ...
    $summary = [
        'scenario' => 'wordpress-pager-cache-savepoint-file-handle-current-next76',
        'status' => $plan['status'],
        ...
        'wordpressUse' => 'Apply a copied wp_options plugin-settings import through native PHP VFS file handles where the current dirty cache pages are rolled back to a retained savepoint before the next retry captures fresh before-images and appends the follow-up page, without ext/sqlite.',
    ];
    ...
    if (
        ...
        fwrite(STDERR, "wordpress-pager-cache-savepoint-file-handle-current-next76 self-test failed\n");
        exit(1);
    }
