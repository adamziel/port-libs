...
$plan = SQLiteBTreeVacuumPointerMapFreeblockCurrentSourcePlan::tableLeafApplyPageFromDeleteResult(
    $database,
    3,
    [
        'page' => $deletedPage,
...
echo json_encode([
    'scenario' => 'application-btree-vacuum-pointermap-freeblock-current-source-next210',
    'applicationUse' => 'After deleting an overflow-backed copied wp_options transient and vacuuming tail pages, the next writer applies pointer-map current-source pages before payload pages and keeps truncated tails fenced.',
    'status' => $summary['status'],
    'applied_pages' => $summary['applied_pages'],
    'applied_pointer_map_pages' => $summary['applied_pointer_map_pages'],
    'applied_payload_pages' => $summary['applied_payload_pages'],
...
if (
    ...
    && $summary['applied_pages'] === [2, 3, 105, 106, 107, 108]
    && $summary['applied_pointer_map_pages'] === [2, 105]
    && $summary['applied_payload_pages'] === [3, 106, 107, 108]
    ...
    echo "application-btree-vacuum-pointermap-freeblock-current-source-next210 self-test passed\n";
}
