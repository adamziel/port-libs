...
$sectorSize = 512;
$nonce = 0x41424344;
$root = sys_get_temp_dir() . '/port-libsqlite-wordpress-vfs-rollback-' . bin2hex(random_bytes(4));
...
if (!mkdir(dirname($localDatabasePath), 0777, true) && !is_dir(dirname($localDatabasePath))) {
    throw new RuntimeException('Unable to create WordPress rollback VFS smoke directory');
}
...
echo json_encode([
    'scenario' => 'wordpress-vfs-rollback-journal-apply',
    'wordpressUse' => 'Apply accepted hot rollback-journal recovery through bounded native PHP file handles for copied wp_options database repairs, including database page restoration, durable database sync, rollback-journal deletion, and directory persistence diagnostics without ext/sqlite.',
    'root' => $root,
