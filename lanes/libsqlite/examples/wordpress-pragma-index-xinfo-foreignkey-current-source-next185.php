...
$summary = [
    'scenario' => 'wordpress-pragma-index-xinfo-foreignkey-current-source-next185',
    'wordpressUse' => 'Copied wp_options imports can repair concrete foreign_key_check violations while preserving SQLite parity that child rows with NULL foreign-key columns are omitted from PRAGMA foreign_key_check.',
    'status' => $page['status'],
...
if (($argv[1] ?? '') === '--self-test') {
    if (
        ...
        fwrite(STDERR, "wordpress-pragma-index-xinfo-foreignkey-current-source-next185 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-index-xinfo-foreignkey-current-source-next185 self-test passed\n");
    exit(0);
}
