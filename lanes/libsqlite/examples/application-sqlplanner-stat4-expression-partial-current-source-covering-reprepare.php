...
if ($plan['status'] !== 'stat4-expression-partial-current-source-coveringReprepare-ready' || $rowids !== [2, 7, 5, 8]) {
    fwrite(STDERR, "application planner stat4 expression partial current-source coveringReprepare failed\n");
    exit(1);
}

printf(
    "application planner stat4 expression partial current-source coveringReprepare: %s rows=%d rowids=%s names=%s\n",
