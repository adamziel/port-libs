...
if (PHP_SAPI === 'cli' && in_array('--self-test', $argv, true)) {
    if (
        ...
        fwrite(STDERR, "wordpress-foreign-key-deferred-cascade-update-trigger-current-source-next116 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-foreign-key-deferred-cascade-update-trigger-current-source-next116 self-test passed\n");
}

return [
    'scenario' => 'wordpress-foreign-key-deferred-cascade-update-trigger-current-source-next116',
    'wordpressUse' => 'Preview a copied wp_options option_id migration where deferred ON UPDATE CASCADE children and child update triggers are applied at COMMIT against current-source rows inserted after the parent UPDATE statement.',
