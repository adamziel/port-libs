...
if ($plan['status'] !== 'child-rolled-back' || count($plan['next_rows']) !== 2 || $plan['next_rows'][1]['option_value'] !== 'inserted:plugin_abort') {
    fwrite(STDERR, "application-trigger-returning-nested-savepoint self-test failed\n");
    exit(1);
}

fwrite(STDOUT, "application-trigger-returning-nested-savepoint self-test passed\n");
fwrite(STDOUT, json_encode([
    'scenario' => 'application-trigger-returning-nested-savepoint',
    'status' => $plan['status'],
    ...
    'applicationUse' => 'Preview nested wp_options import savepoints where INSERT RETURNING rows survive RELEASE into the outer transaction but a later trigger RAISE(ROLLBACK) suppresses only the rolled-back UPDATE RETURNING rows.',
