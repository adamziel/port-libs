...
$summary = [
    'scenario' => 'wordpress-select-recursive-materialized-json-current-next48',
    ...
    'wordpressUse' => 'Copied wp_options JSON import staging can walk recursive option dependencies, materialize reachable json_tree() rule rows, and scan indexed current/next JSON attributes without ext/sqlite.',
];

if (($argv[1] ?? null) === '--self-test') {
    if ($summary['recursiveOptionIds'] !== [1, 2, 3, 4, 5] || $summary['materializedJsonRows'] !== 30 || $summary['indexKeys'] !== 15) {
        fwrite(STDERR, "wordpress-select-recursive-materialized-json-current-next48 self-test failed\n");
        exit(1);
    }

    echo "wordpress-select-recursive-materialized-json-current-next48 self-test passed\n";
    exit(0);
}
