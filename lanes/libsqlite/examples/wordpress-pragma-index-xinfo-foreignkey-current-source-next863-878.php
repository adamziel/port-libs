...
$summary = [
    'scenario' => 'wordpress-pragma-index-xinfo-foreignkey-current-source-next863-878',
    'wordpressUse' => 'WordPress import previews can page the staged next863-878 foreign-key action relationship diagnostics after next847-862 while preserving order, collation, and DESC child lookup mismatch coverage.',
...
if (($argv[1] ?? null) === '--self-test') {
    foreach ($cases as $slice => [, , , $status]) {
        if (($summary["next{$slice}_current_{$status}"] ?? null) !== 0 || ($summary["next{$slice}_next_{$status}"] ?? null) !== 1) {
            fwrite(STDERR, "wordpress-pragma-index-xinfo-foreignkey-current-source-next863-878 self-test failed\n");
            exit(1);
        }
    }
    ...
    fwrite(STDOUT, "wordpress-pragma-index-xinfo-foreignkey-current-source-next863-878 self-test passed\n");
    exit(0);
}
