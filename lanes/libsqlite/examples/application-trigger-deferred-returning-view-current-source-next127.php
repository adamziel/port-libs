...
$summary = [
    'scenario' => 'application-trigger-deferred-returning-view-current-source-next127',
    'status' => $plan['status'],
    ...
    'applicationUse' => 'Preview a copied wp_options import through an INSTEAD OF view trigger where RETURNING rows are attempted before deferred FK release decides whether current/next source rows are admitted or rolled back, without ext/sqlite.',
];
...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "application-trigger-deferred-returning-view-current-source-next127 self-test failed\n");
        exit(1);
    }

    echo "application-trigger-deferred-returning-view-current-source-next127 self-test passed\n";
    return;
}
