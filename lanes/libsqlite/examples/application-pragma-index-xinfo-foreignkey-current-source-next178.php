...
$summary = [
    'scenario' => 'application-pragma-index-xinfo-foreignkey-current-source-next178',
    'applicationUse' => 'Copied wp_options imports can verify that PRAGMA foreign_key_list parent columns map to PRAGMA index_xinfo key columns while ignoring auxiliary covering columns before resuming FK repair pages.',
    'status' => $page['status'],
...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "application-pragma-index-xinfo-foreignkey-current-source-next178 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "application-pragma-index-xinfo-foreignkey-current-source-next178 self-test passed\n");
    exit(0);
}
