...
$plan = SQLiteJsonTablePlan::currentSourcePathHiddenRowidCostNext126(
    'json_tree',
    $currentOption,
    $nextOption,
    'option_value',
...
$payload = [
    'scenario' => 'wordpress-json-table-path-hidden-rowid-cost-current-source-next126',
    'wordpressUse' => 'Copied wp_options JSON rule diagnostics can combine a path lookup with hidden rowid aliases so json_tree() keeps a one-row lookup cost while current-source cursors remain pinned.',
...
if (($argv[1] ?? null) === '--self-test') {
    ...
    echo "wordpress-json-table-path-hidden-rowid-cost-current-source-next126 self-test passed\n";
    return;
}
