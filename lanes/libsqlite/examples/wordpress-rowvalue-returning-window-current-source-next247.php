...
$retryUpdate = "UPDATE wp_options SET (status, option_value, bytes) = ('retry247', option_value || ':retry247', bytes + 2) WHERE (option_id, option_name) IN (SELECT meta_option_id, meta_value FROM wp_optionmeta WHERE meta_key = 'retry_update') RETURNING option_id, blog_id, option_name, status, option_value, bytes ORDER BY option_id DESC";
...
if (($argv[1] ?? '') === '--self-test') {
    assert($plan['status'] === 'rowvalue-update-delete-returning-window-current-source-next247');
    assert($plan['window_exclude_group_count_next247'] === 9);
    assert($plan['window_exclude_group_replayed_ids_next247'] === [8, 9]);
    assert($plan['window_exclude_group_restart_ids_next247'] === [2, 5, 11, 10]);
    assert($plan['window_exclude_group_discarded_ids_next247'] === [3, 4, 7]);
    assert($plan['window_exclude_group_rows_next247'][6]['exclude_group_frame_rowids_next247'] === [7, 10]);
    assert($plan['window_exclude_group_fence_next247']['exclude_group_count'] === 9);
    assert(strlen($plan['window_exclude_group_fence_next247']['excluded_group_digest']) === 64);

    echo "wordpress-rowvalue-returning-window-current-source-next247 self-test passed\n";
    return;
}

echo json_encode([
    'status' => $plan['status'],
    'excludeGroupCount' => $plan['window_exclude_group_count_next247'],
    'replayed' => $plan['window_exclude_group_replayed_ids_next247'],
    'restartOnly' => $plan['window_exclude_group_restart_ids_next247'],
    'discardedOnly' => $plan['window_exclude_group_discarded_ids_next247'],
    'summary' => $plan['window_exclude_group_summary_next247'],
