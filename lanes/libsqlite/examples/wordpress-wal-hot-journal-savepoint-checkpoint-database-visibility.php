...
$summary = [
    'scenario' => 'wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next155',
    'wordpressUse' => 'A copied WordPress SQLite database recovers a hot rollback journal, rolls a plugin-import savepoint back to an earlier WAL prefix, checkpoints only the retained current-source pages, and requires stale readers to reopen before seeing the checkpoint.',
    'status' => $plan['status'],
...
if (PHP_SAPI === 'cli' && basename(__FILE__) === basename($_SERVER['SCRIPT_FILENAME'] ?? '')) {
    if (($argv[1] ?? '') === '--self-test') {
        ...
        echo 'wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next155 self-test passed' . PHP_EOL;
        return $summary;
    }
