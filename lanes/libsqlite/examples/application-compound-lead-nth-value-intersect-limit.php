...
$payload = [
    'scenario' => 'application-compound-select-window-recursive-limit-current-source-lead-nth-value-intersect-limit',
    'applicationUse' => 'Copied wp_options preview queries can combine recursive dependency queues with lead defaults and nth_value frames, then fence current-source resume cursors across an INTERSECT membership boundary before next-source rows shift the final LIMIT.',
    'status' => $plan['status'],
...
if (($payload['status'] ?? null) !== 'compound-select-window-recursive-limit-current-source-lead-nth-value-intersect-limit-ready') {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-lead-nth-value-intersect-limit self-test failed\n");
    exit(1);
}
if ($payload['windowFunctions'] !== ['lead', 'nth_value']) {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-lead-nth-value-intersect-limit window dispatch failed\n");
    exit(1);
}
if ($payload['currentTokenLength'] !== 64 || $payload['nextTokenLength'] !== 64) {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-lead-nth-value-intersect-limit token guard failed\n");
    exit(1);
}
