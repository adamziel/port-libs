...
$databaseDigest = $hash('application next254 checkpoint database image');
$pageCacheDigest = $hash('application next254 clean page cache image');
$cachePlan = [
...
$summary = [
    'scenario' => 'application-wal-hot-journal-savepoint-checkpoint-current-source-next254',
    'applicationUse' => 'A copied Application options import reuses schema, table, index, and read-transaction leases only after the hot-journal checkpoint cache fence proves every reader, root page, committed WAL frame, and cache receipt belongs to the same current source.',
    'status' => $plan['status'],
...
if (($argv[1] ?? null) === '--self-test') {
    if ($summary['status'] !== 'wal-hot-journal-savepoint-checkpoint-current-source-next254'
        ...
        fwrite(STDERR, "application-wal-hot-journal-savepoint-checkpoint-current-source-next254 self-test failed\n");
        exit(1);
    }

    echo "application-wal-hot-journal-savepoint-checkpoint-current-source-next254 self-test passed\n";
    exit(0);
}
