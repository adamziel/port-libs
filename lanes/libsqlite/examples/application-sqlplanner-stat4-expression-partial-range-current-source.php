...
if (in_array('--self-test', $argv, true)) {
 ...
 fwrite(STDERR, "application-sqlplanner-stat4-expression-partial-range-current-source self-test failed\n");
 exit(1);
 }

 echo "application-sqlplanner-stat4-expression-partial-range-current-source self-test passed\n";
 exit(0);
}

echo json_encode([
 'scenario' => 'application-sqlplanner-stat4-expression-partial-range-current-source',
 'status' => $plan['status'],
 ...
 'applicationUse' => 'Preview copied wp_options import diagnostics where one-sided LOWER(option_name) and updated_at ranges prove a partial expression index from the current STAT4 source after ANALYZE refresh, without requiring ext/sqlite.',
