...
$summary = [
    'scenario' => 'application-pager-master-journal-reader-cache-current-source-next239',
    'applicationUse' => 'A copied Application database keeps schema reader-cache pages only when both schema-reparse and shared-cache generation tickets were opened after master-journal recovery; stale options and active_plugins readers reopen before plugin import resumes.',
    'status' => $plan['status'],
...
if ($summary['status'] !== 'pager-master-journal-reader-cache-current-source-next239'
    ...
    fwrite(STDERR, "application-pager-master-journal-reader-cache-current-source-next239 self-test failed\n");
    exit(1);
}

echo json_encode($summary, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) . "\n";
echo "application-pager-master-journal-reader-cache-current-source-next239 self-test passed\n";
