...
$page = SQLitePragmaIndexXinfoForeignKeyCurrentSourceNext217::page(
    $currentRecords,
    $nextRecords,
...
$summary = [
    'scenario' => 'wordpress-pragma-index-xinfo-foreignkey-current-source-next217',
    'wordpressUse' => 'Copied multisite wp_blog_options imports can detect when a foreign key references only a suffix of a UNIQUE parent index before treating PRAGMA foreign_key_check output as repairable by the current parent catalog.',
    'status' => $page['status'],
...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "wordpress-pragma-index-xinfo-foreignkey-current-source-next217 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-index-xinfo-foreignkey-current-source-next217 self-test passed\n");
    exit(0);
}
