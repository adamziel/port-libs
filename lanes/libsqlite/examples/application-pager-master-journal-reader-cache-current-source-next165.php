...
$summary = [
    'scenario' => 'application-pager-master-journal-reader-cache-current-source-next165',
    'status' => $plan['status'],
    ...
    'applicationUse' => 'A copied wp_options recovery can retain a page-1 reader cache only when the current master-journal membership, change counter, schema cookie, and WAL end-frame still match; active_plugins and autoload pages with stale header tickets reopen from the recovered current source.',
...
if ($summary['status'] !== 'pager-master-journal-reader-cache-current-source-next165'
    ...
    fwrite(STDERR, "application-pager-master-journal-reader-cache-current-source-next165 self-test failed\n");
    exit(1);
}

echo "application-pager-master-journal-reader-cache-current-source-next165 self-test passed\n";
