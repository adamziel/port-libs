...
$summary = [
    'scenario' => 'wordpress-cast-like-glob-affinity-current-source-next133',
    'wordpressUse' => 'Copied wp_options import scans can evaluate CAST(option_value AS ...) through BINARY LIKE/GLOB prefix candidates and residuals while invalidating stale cursors when the next source changes cast text, encoded bytes, or matched rowsets.',
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "wordpress-cast-like-glob-affinity-current-source-next133 self-test passed\n";
    exit(0);
}
