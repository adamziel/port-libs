...
$summary = [
    'scenario' => 'application-pragma-index-xinfo-foreignkey-current-source-next295-302',
    'applicationUse' => 'Application-style schema upgrades can preflight FK relationship metadata that is visible through PRAGMA foreign_key_list, table_info, index_list, and index_xinfo before enabling constraints.',
...
if (($argv[1] ?? null) === '--self-test') {
    foreach (array_keys($summary) as $key) {
        if (str_starts_with($key, 'next') && $summary[$key] !== 1) {
            fwrite(STDERR, "application-pragma-index-xinfo-foreignkey-current-source-next295-302 self-test failed\n");
            exit(1);
        }
    }
    ...
    fwrite(STDOUT, "application-pragma-index-xinfo-foreignkey-current-source-next295-302 self-test passed\n");
    exit(0);
}
