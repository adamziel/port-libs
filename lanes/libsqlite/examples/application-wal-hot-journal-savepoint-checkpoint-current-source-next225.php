...
$summary = [
    'scenario' => 'application-wal-hot-journal-savepoint-checkpoint-current-source-next225',
    'applicationUse' => 'A copied Application wp_options import recovers a hot journal, finalizes plugin savepoints, checkpoints WAL frames, and admits the database header as the current source only after the database header, WAL-index header, and change-counter receipts match the checkpoint cookies.',
    'status' => $plan['status'],
...
if (PHP_SAPI === 'cli' && basename(__FILE__) === basename($_SERVER['SCRIPT_FILENAME'] ?? '')) {
    if (
        ...
        fwrite(STDERR, "application-wal-hot-journal-savepoint-checkpoint-current-source-next225 self-test failed\n");
        exit(1);
    }
