...
$plan = SQLitePlannerStat4ExpressionPartialCurrentSourceNextPlan::materializeCurrentSourceDuplicatePeerFence(
    $prepared,
    $current,
    [
...
if (in_array('--self-test', $argv, true)) {
    ...
    echo "application-sqlplanner-stat4-expression-partial-current-source-next188 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'application-sqlplanner-stat4-expression-partial-current-source-next188',
    'applicationUse' => 'Copied wp_options plugin screens can reuse a partial lower(option_name) STAT4 plan only when duplicate plugin option keys in the current page are rowid-ordered and bracketed by current sqlite_stat4 samples.',
    'status' => $plan['status'],
