...
$walDigest = $digest('wordpress next218 wal before truncate');
$writerDigest = $digest('wordpress next218 writer fence');
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    assert(in_array('wordpress-import-checkpoint-reset-waits-for-current-source-reopen', $plan['dependencies'], true));
    echo "wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next218 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next218',
    'status' => $plan['status'],
    'walAction' => $plan['wal_action'],
    'truncateAllowed' => $plan['truncate_allowed'],
    'wordpressUse' => 'A copied wp_options import can publish a TRUNCATE checkpoint only after hot-journal recovery, savepoint release, current-source reader reopen, and writer sync receipts agree on the checkpointed WAL generation.',
