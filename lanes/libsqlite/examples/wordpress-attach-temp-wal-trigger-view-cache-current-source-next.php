...
$plan = SQLiteAttachTempWalSchemaTriggerPlan::triggerViewCacheCurrentSourceNext(
    $current,
    $next,
    [
...
$summary = [
    'scenario' => 'wordpress-attach-temp-wal-trigger-view-cache-current-source-next97',
    'status' => $plan['status'],
    ...
    'wordpressUse' => 'Copied WordPress SQLite imports can keep active INSTEAD OF triggers on current view definitions until reset while temp schema shadows and WAL page-one schema cookies force the next trigger step to reprepare.',
];
...
if (PHP_SAPI === 'cli' && in_array('--self-test', $argv, true)) {
    ...
    echo "wordpress-attach-temp-wal-trigger-view-cache-current-source-next97 self-test passed\n";
    return;
}
