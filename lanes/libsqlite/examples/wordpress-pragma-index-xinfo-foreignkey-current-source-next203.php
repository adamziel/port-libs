...
$page = SQLitePragmaIndexXinfoForeignKeyCurrentSourceNext203::page(
    $current,
    $next,
...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "wordpress-pragma-index-xinfo-foreignkey-current-source-next203 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-index-xinfo-foreignkey-current-source-next203 self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'wordpress-pragma-index-xinfo-foreignkey-current-source-next203',
    'wordpressUse' => 'Before applying copied taxonomy metadata schema changes, confirm that PRAGMA foreign_key_list parent groups and PRAGMA index_xinfo/index_list agree on usable UNIQUE parent-key coverage in the current and next schema snapshots.',
    'status' => $page['status'],
