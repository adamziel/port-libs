...
$next219 = [
    ...
    'current_source_token' => ['id' => 'wordpress-next220-checkpoint-source', 'epoch' => 219],
...
$receipt = static fn (string $name): array => [
    'name' => $name,
    'source_id' => 'wordpress-next220-checkpoint-source',
    'observed_epoch' => 220,
    'checkpoint_frame' => 220,
    'checkpoint_cookie' => 220220,
    'schema_cookie' => 22017,
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next220 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next220',
    'status' => $plan['status'],
    'readerReopenAllowed' => $plan['reader_reopen_allowed'],
    'readerCacheAction' => $plan['reader_cache_action'],
    'wordpressUse' => 'A copied wp_options import reopens plugin readers only after the checkpoint source, schema cookie, and hot-journal savepoint generation are visible.',
