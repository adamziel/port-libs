...
$databaseDigest = $hash('application next249 checkpoint database image');
$pageCacheDigest = $hash('application next249 clean checkpoint page cache');
$handoffPlan = [
...
$summary = [
    'scenario' => 'application-wal-hot-journal-savepoint-checkpoint-current-source-next249',
    'applicationUse' => 'A copied Application import reopens after a hot-journal savepoint checkpoint and confirms the database digest, clean page cache, WAL commit frames, retired journal, and reader epochs all point at the admitted current source.',
    'status' => $plan['status'],
...
if (($argv[1] ?? null) === '--self-test') {
    if ($summary['status'] !== 'wal-hot-journal-savepoint-checkpoint-current-source-next249'
        ...
        fwrite(STDERR, "application-wal-hot-journal-savepoint-checkpoint-current-source-next249 self-test failed\n");
        exit(1);
    }

    echo "application-wal-hot-journal-savepoint-checkpoint-current-source-next249 self-test passed\n";
    exit(0);
}
