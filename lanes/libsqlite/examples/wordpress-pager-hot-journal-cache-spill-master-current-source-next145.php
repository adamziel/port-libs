...
if (
    ...
    fwrite(STDERR, "wordpress-pager-hot-journal-cache-spill-master-current-source-next145 self-test failed\n");
    exit(1);
}

if (PHP_SAPI === 'cli' && realpath($_SERVER['SCRIPT_FILENAME'] ?? '') === __FILE__) {
    echo "wordpress-pager-hot-journal-cache-spill-master-current-source-next145 self-test passed\n";
}

return [
    'scenario' => 'wordpress-pager-hot-journal-cache-spill-master-current-source-next145',
    'wordpressUse' => 'After copied wp_options recovery through a master hot journal, dirty pager-cache spill is admitted only for pages whose rollback source and current image match the recovered master current source; stale and pinned cache pages wait for a safer retry.',
    'status' => $plan['status'],
