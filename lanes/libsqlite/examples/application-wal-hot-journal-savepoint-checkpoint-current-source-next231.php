...
$walDigest = $digest('application next231 restarted wal sidecar');
$token = ['id' => 'application-import-current-source-next231', 'epoch' => 231];
$admission = [
...
$plan = SQLiteWalHotJournalSavepointCheckpointCurrentSourceNextPlan::next231VerifyWalIndexReopen($published, [[
    'name' => 'application-wal-index-reopen',
...
if (($argv[1] ?? '') === '--self-test') {
    if ($plan['status'] !== 'wal-hot-journal-savepoint-checkpoint-current-source-next231'
        ...
        || !in_array('application-import-wal-index-reopen-current-source', $plan['dependencies'], true)
    ) {
        fwrite(STDERR, "application-wal-hot-journal-savepoint-checkpoint-current-source-next231 self-test failed\n");
        exit(1);
    }

    echo "application-wal-hot-journal-savepoint-checkpoint-current-source-next231 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'application-wal-hot-journal-savepoint-checkpoint-current-source-next231',
    'status' => $plan['status'],
    ...
    'applicationUse' => 'A copied wp_options import reopens the current source only after the WAL-index salts, checksum digest, backfill frame, and reader readmarks match the checkpoint published after hot-journal savepoint recovery.',
