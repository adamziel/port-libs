...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "application-rowvalue-or-fail-savepoint-current-source-next185 self-test passed\n";
    return;
}

echo json_encode([
    'applicationUse' => 'Model copied wp_options cleanup where UPDATE OR FAIL yields partial row-value RETURNING rows inside a savepoint, then ROLLBACK TO discards those rows before a durable retry.',
    'status' => $plan['status'],
