...
$databaseForSlice = static function (int $sliceNumber) use ($makeFirstPage, $putPointerMapEntry): SQLiteDatabase {
    ...
    foreach ([106 => 107, 107 => 108, 108 => 109, 109 => 110, 110 => 0] as $pageNumber => $nextPage) {
        $pages[$pageNumber] = pack('N', $nextPage) . str_repeat(chr(110 + ($pageNumber - 105)), 508);
    }
...
$rows = [];
foreach ([299, 300, 301, 302] as $sliceNumber) {
    $database = $databaseForSlice($sliceNumber);
    $deletedPage = SQLiteTableLeafPage::deleteCellByRowId($database->page(3), 2, secureDelete: true);
    $plan = SQLiteBTreeVacuumPointerMapFreeblockCurrentSourcePlan::tableLeafFreelistSpliceFromDeleteResult(
        $sliceNumber,
        $database,
        3,
        [
    ...
    $rows[] = [
        ...
        'ordered_leaf_slots' => $summary['all_leaf_slots_ordered'],
