...
$current = $prepared;
$current['name'] = 'current-wp-options-plugin-subquery-next106';
...
$summary = [
    'scenario' => 'wordpress-planner-subquery-partial-index-current-source-next106',
    'status' => $plan['status'],
    ...
    'wordpressUse' => 'Copied wp_options plugin scans can reprepare a stale statement and prove a lower(option_name) partial covering index from bounded IN-subquery values before import diagnostics, without ext/sqlite.',
...
if (in_array('--self-test', $argv, true)) {
    ...
    if (($summary['selectedSource'] ?? null) !== 'current' || ($summary['rootPage'] ?? null) !== 10610) {
        fwrite(STDERR, "expected current source root page\n");
        exit(1);
    }
    ...
    echo "wordpress-planner-subquery-partial-index-current-source-next106 self-test passed\n";
    exit(0);
}
