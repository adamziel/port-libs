...
$summary = [
    'scenario' => 'application-rowvalue-or-fail-savepoint-current-source-fail-statement-retry',
    'applicationUse' => 'Copied wp_options repair can keep successful row-value UPDATE RETURNING rows from an UPDATE OR FAIL statement, suppress the conflicting row RETURNING stream, and retry cleanup from that preserved current source.',
    'status' => $plan['status'],
...
if (($argv[1] ?? null) === '--self-test') {
    ...
    echo "application-rowvalue-or-fail-savepoint-current-source-fail-statement-retry self-test passed\n";
    return;
}
