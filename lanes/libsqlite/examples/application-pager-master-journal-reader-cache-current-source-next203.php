...
$currentHeaders = [
    $mainJournal => hash('sha256', 'application-main-current-rollback-header-203'),
    $usersJournal => hash('sha256', 'application-users-current-rollback-header-203'),
];
$formatSignature = hash('sha256', implode('|', [512, 4, 2, 203, 0x57503230]));
$sourceId = 'application-member-order-source';
$masterDigest = hash('sha256', 'application-current-master-source-next203');
...
if (($argv[1] ?? '') === '--self-test') {
    if (
        ...
        fwrite(STDERR, "application-pager-master-journal-reader-cache-current-source-next203 self-test failed\n");
        exit(1);
    }

    echo "application-pager-master-journal-reader-cache-current-source-next203 self-test passed\n";
    exit(0);
}
