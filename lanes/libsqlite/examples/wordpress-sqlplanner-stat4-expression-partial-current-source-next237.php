...
$plan = SQLitePlannerStat4ExpressionPartialCurrentSourceNext237Plan::materialize(
    $prepared,
    $source,
    [
...
printf(
    "wordpress sqlplanner stat4 expression partial current-source next237: %s trailing=%s matched=%s signature=%s\n",
    $plan['status'],
...
return [
    'scenario' => 'wordpress-sqlplanner-stat4-expression-partial-current-source-next237',
    'status' => $plan['status'],
    'matchedTrailingRowids' => $plan['stat4TrailingPayloadFence']['matchedTrailingRowids'],
    'wordpressUse' => 'Copied wp_options plugin preload scans can reuse a current-source partial lower(option_name) covering index only when sqlite_stat4 trailing autoload/blog_id payloads still match current rows after ANALYZE or source refresh.',
];
