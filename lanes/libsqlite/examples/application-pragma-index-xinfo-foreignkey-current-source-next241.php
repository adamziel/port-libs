...
$summary = [
    'scenario' => 'application-pragma-index-xinfo-foreignkey-current-source-next241',
    'applicationUse' => 'Copied Application import schemas can compare raw PRAGMA foreign_key_list NULL parent columns with current-source primary-key resolution before emitting explicit FK DDL.',
    'status' => $page['status'],
...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "application-pragma-index-xinfo-foreignkey-current-source-next241 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "application-pragma-index-xinfo-foreignkey-current-source-next241 self-test passed\n");
    exit(0);
}
