...
$summary = [
    'scenario' => 'application-pager-master-journal-reader-cache-current-source-next247',
    'applicationUse' => 'A copied Application database keeps schema reader-cache pages only when they were opened against the same recovered master-journal current-source and the same pager cache generation; stale options and active_plugins reads reopen before plugin import resumes.',
    'status' => $plan['status'],
...
if ($summary['status'] !== 'pager-master-journal-reader-cache-current-source-next247'
    ...
    fwrite(STDERR, "application-pager-master-journal-reader-cache-current-source-next247 self-test failed\n");
    exit(1);
}

echo json_encode($summary, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) . "\n";
echo "application-pager-master-journal-reader-cache-current-source-next247 self-test passed\n";
