...
$page = SQLitePragmaIndexXinfoForeignKeyCurrentSourceNext244::page(
    $currentRecords,
    $nextRecords,
...
$summary = [
    'scenario' => 'wordpress-pragma-index-xinfo-foreignkey-current-source-next244',
    'wordpressUse' => 'Copied WordPress taxonomy import schemas can avoid accepting a UNIQUE expression index as a foreign-key parent key when PRAGMA index_xinfo reports expression key rows.',
    'status' => $page['status'],
...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "wordpress-pragma-index-xinfo-foreignkey-current-source-next244 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-index-xinfo-foreignkey-current-source-next244 self-test passed\n");
    exit(0);
}
