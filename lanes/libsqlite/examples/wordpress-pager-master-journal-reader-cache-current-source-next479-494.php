...
$base = [
    'source_id' => 'wordpress-pager-reader-cache-next494',
    'epoch' => 494,
    'format_signature' => hash('sha256', implode('|', [512, 4, 2, 494, 0])),
    'publication_generation' => 494,
    'master_source_digest' => hash('sha256', 'wordpress next494 master source'),
...
$cache = [1 => array_merge($base, ['reader_id' => 'wp-options-reader', 'image' => $recovered[1], 'reader_cache_stmt_vdbe_compare_token' => 'stmt-vdbe-compare-old'])];
$read = array_merge($base, [
    'reader_id' => 'read-options',
    'page_number' => 1,
    'member_journal_token_digest' => $mapDigest($tokens),
    'member_journal_header_digest' => $mapDigest($headers),
    'reader_cache_stmt_vdbe_compare_token' => 'stmt-vdbe-compare-old',
]);
$plan = SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::readerCacheStatementFence(494,
    $database,
    $master,
    $masterBytes,
    implode('', $before),
...
echo json_encode([
    ...
    'next494_invalidated_pages' => $plan['reader_cache_stmt_vdbe_compare_invalidated_cache_page_numbers'],
