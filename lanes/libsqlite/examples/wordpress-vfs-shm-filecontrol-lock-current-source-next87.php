...
$plan = SQLiteVfsShmFileControlLockCurrentSourcePlan::currentSourceNext87([
...
$summary = [
    'scenario' => 'wordpress-vfs-shm-filecontrol-lock-current-source-next87',
    'status' => $plan['status'],
    ...
    'wordpressUse' => 'Track copied wp_options WAL-index SHM locks separately from database xFileControl state so checkpoint/import code does not misroute mmap/chunk/persist-WAL controls to the -shm sidecar.',
    'dependencies' => $plan['dependencies'],
];

if (($summary['databaseControls']['mmap_size'] ?? null) !== 65536 || ($summary['shmLocks']['write'] ?? null) !== 'exclusive') {
    fwrite(STDERR, "wordpress-vfs-shm-filecontrol-lock-current-source-next87 self-test failed\n");
    exit(1);
}

if (in_array('--self-test', $argv, true)) {
    echo "wordpress-vfs-shm-filecontrol-lock-current-source-next87 self-test passed\n";
    return;
}
