...
$payload = [
    'scenario' => 'application-compound-select-window-recursive-limit-current-source-next210',
    'applicationUse' => 'Copied wp_options preview queries can combine a recursive queue with row_number and last_value window arms, then fence INTERSECT before EXCEPT and final LIMIT/OFFSET with a current-source cursor token.',
    'status' => $plan['status'],
...
if (($payload['status'] ?? null) !== 'compound-select-window-recursive-limit-current-source-next210-ready') {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-next210 self-test failed\n");
    exit(1);
}
if ($payload['nextOnlyAdmittedLabels'] !== ['plugin_prime', 'theme_mods_next']) {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-next210 next-source fence failed\n");
    exit(1);
}
