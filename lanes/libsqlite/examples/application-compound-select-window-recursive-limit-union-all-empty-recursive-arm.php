...
if (in_array('--self-test', $argv, true)) {
    ...
    echo "application-compound-select-window-recursive-limit-current-source-union-all-empty-recursive-arm self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'application-compound-select-window-recursive-limit-current-source-union-all-empty-recursive-arm',
    'applicationUse' => 'Copied wp_options import previews can keep a recursive arm syntactically present but exhausted by LIMIT 0, while windowed UNION ALL table arms and the final tail LIMIT/OFFSET determine which changed plugin/theme rows appear.',
