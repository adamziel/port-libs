...
$argv = [];
$next257 = require __DIR__ . '/wordpress-rowvalue-returning-window-delete-retry-publication.php';
$argv = [];
$next258 = require __DIR__ . '/wordpress-rowvalue-returning-window-publication-transition-admission.php';

ob_start();
$argv = [];
require __DIR__ . '/wordpress-rowvalue-update-delete-returning-window-current-row-frame-admission.php';
$next259Output = trim((string) ob_get_clean());
$next259 = json_decode($next259Output, true, 512, JSON_THROW_ON_ERROR);

ob_start();
$argv = [];
require __DIR__ . '/wordpress-rowvalue-returning-window-frame-boundary-admission.php';
...
$summary = [
    ...
    'wordpressUse' => 'Copied wp_options imports can validate the prepared next257-260 current-source row-value UPDATE/DELETE RETURNING handoff as tombstone publication, transition-token admission, current-row frame acknowledgement, and mixed-boundary release coverage.',
];
...
if ($selfTest) {
    if ($statuses !== [
        ...
        fwrite(STDERR, "wordpress-rowvalue-returning-window-after-current-publication self-test failed\n");
        exit(1);
    }

    echo "wordpress-rowvalue-returning-window-after-current-publication self-test passed\n";
    return;
}
