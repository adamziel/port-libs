...
$like = SQLiteEncodingLikeGlobRtrimCurrentSourceNextPlan::optionRowNamePlan(
    $current,
    $next,
    'LIKE',
    'plugin\_cache',
...
$glob = SQLiteEncodingLikeGlobRtrimCurrentSourceNextPlan::optionRowNamePlan(
    $current,
    $next,
    'GLOB',
    'plugin_cache*',
...
$summary = [
    'scenario' => 'application-option-name-rtrim-like-glob-current-source-next140',
    ...
    'applicationUse' => 'Copied wp_options option_name scans can reuse an RTRIM expression-index range while still applying LIKE/GLOB residual matching to the original untrimmed decoded text.',
];
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "application-option-name-rtrim-like-glob-current-source-next140 self-test passed\n";
    return;
}
