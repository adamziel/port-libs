...
$summary = [
    'scenario' => 'application-btree-vacuum-pointermap-freeblock-current-source-next158',
    'application_use' => 'Delete a copied wp_options transient row, auto-vacuum the tail overflow pages, then reuse the surviving free page as a new overflow page with fresh pointer-map ownership while proving truncated pages stay unavailable.',
...
if (PHP_SAPI === 'cli' && in_array('--self-test', $argv, true)) {
    ...
    if (!$ok) {
        fwrite(STDERR, "application-btree-vacuum-pointermap-freeblock-current-source-next158 self-test failed\n");
        exit(1);
    }

    echo "application-btree-vacuum-pointermap-freeblock-current-source-next158 self-test passed\n";
}
