...
if (($argv[1] ?? '') === '--self-test') {
    ...
    if (
        ...
        fwrite(STDERR, "wordpress VDBE window FILTER RANGE EXCLUDE CURRENT ROW smoke failed\n");
        exit(1);
    }

    echo "wordpress VDBE window FILTER RANGE EXCLUDE CURRENT ROW smoke passed\n";
    exit(0);
}

echo json_encode([
    'scenario' => 'wordpress-vdbe-window-filter-range-exclude-current-next53',
    'rows' => $summary,
