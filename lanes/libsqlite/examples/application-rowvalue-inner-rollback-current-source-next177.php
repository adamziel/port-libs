...
$summary = SQLiteRowValueUpdateDeleteReturningSavepointPlan::executeInnerRollbackRetrySavepoint(
    $tables,
...
if (
    ...
    fwrite(STDERR, "application-rowvalue-inner-rollback-current-source-next177 self-test failed\n");
    exit(1);
}

if (PHP_SAPI === 'cli' && basename((string) ($_SERVER['SCRIPT_FILENAME'] ?? '')) === basename(__FILE__)) {
    echo "application-rowvalue-inner-rollback-current-source-next177 self-test passed\n";
}

return [
    'scenario' => 'application-rowvalue-inner-rollback-current-source-next177',
    'summary' => [
    ...
    'applicationUse' => 'Copied wp_options imports can keep outer row-value UPDATE RETURNING effects, roll back a speculative inner UPDATE/DELETE RETURNING cleanup to its inner savepoint image, and retry the inner batch before release.',
