...
$plan = SQLiteWalHotJournalSavepointCheckpointCurrentSourceNextPlan::next207Plan(
    [
...
$summary = [
    'scenario' => 'wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next207',
    'wordpressUse' => 'After a copied WordPress import recovers a hot journal, rolls back a failed savepoint, checkpoints WAL pages, and reprepares statements, only write cursors with the current database/WAL digests and exclusive lock token may commit wp_options changes.',
    'status' => $plan['status'],
...
if ($summary['status'] !== 'wal-hot-journal-savepoint-checkpoint-current-source-next207'
    ...
    fwrite(STDERR, "wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next207 self-test failed\n");
    exit(1);
}
