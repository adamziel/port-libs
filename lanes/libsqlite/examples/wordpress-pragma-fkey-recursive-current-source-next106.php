...
if (
    ...
    fwrite(STDERR, "wordpress-pragma-fkey-recursive-current-source-next106 self-test failed\n");
    exit(1);
}

if (PHP_SAPI === 'cli' && ($_SERVER['argv'][1] ?? '') === '--self-test') {
    echo "wordpress-pragma-fkey-recursive-current-source-next106 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'wordpress-pragma-fkey-recursive-current-source-next106',
