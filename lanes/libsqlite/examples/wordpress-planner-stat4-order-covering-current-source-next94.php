...
$plan = SQLiteStat4OrderCoveringCurrentSourceNextPlan::compareNext94(
    $prepared,
    $current,
    $and(
        $point('blog_id', 1),
...
$output = [
    'scenario' => 'wordpress-planner-stat4-order-covering-current-source-next94',
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    if (($output['selectedIndex'] ?? null) !== 'idx_wp_options_blog_autoload_name_value_stat4_next94') {
    ...
    echo "wordpress-planner-stat4-order-covering-current-source-next94 self-test passed\n";
    exit(0);
}
