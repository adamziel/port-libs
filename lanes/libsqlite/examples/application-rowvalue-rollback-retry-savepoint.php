...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "application-rowvalue-rollback-retry-savepoint self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'application-rowvalue-rollback-retry-savepoint',
    ...
    'applicationUse' => 'Copied wp_options import batches using UPDATE OR ROLLBACK with row-value assignments discard speculative RETURNING rows, restore the transaction image, then retry UPDATE/DELETE RETURNING cleanup from the restored current source.',
