...
if (($argv[1] ?? '') === '--self-test') {
    if (
        ...
        fwrite(STDERR, "application-btree-freeblock-pointermap-vacuum-current-source-next self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "application-btree-freeblock-pointermap-vacuum-current-source-next self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'copied wp_options freeblock coalesce followed by pointer-map boundary vacuum current source next',
    'applicationUse' => 'Preview a transient option delete where the leaf page keeps a coalesced reusable freeblock while obsolete tail overflow pages are removed by incremental vacuum across an auto-vacuum pointer-map boundary without ext/sqlite.',
