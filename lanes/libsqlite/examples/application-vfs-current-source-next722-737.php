...
if (($argv[1] ?? '') === '--self-test') {
    ...
    assert($plan['events'][0]['ack'] === 'shared-cache-next721');
    assert($plan['events'][1]['status'] === 'claimed-reusable-current-source');
    assert($plan['events'][1]['claim'] === 'reader-reuse-next737');
    assert($plan['events'][1]['blocked_reasons'] === []);
    assert($plan['events'][2]['status'] === 'published-reused-current-source');
    assert($plan['events'][2]['reuse_ack'] === 'shared-cache-next721');
    assert($plan['events'][2]['token'] === 'shared-cache-next737');
    assert($plan['events'][2]['published_count'] === 37);
    assert(in_array('vfs-current-source-snapshot-reuse-publish-next722-737', $plan['dependencies'], true));
    assert(in_array('vfs-current-source-snapshot-reuse-publish-next706-721', $plan['dependencies'], true));
    echo "application-vfs-current-source-next722-737 self-test passed\n";
    return;
}
