...
$plan = SQLiteBTreeVacuumPointerMapFreeblockCurrentSourcePlan::tableLeafSourceHandoffFromDeleteResult(
    $database,
    3,
    [
        'page' => $deletedPage,
...
$payload = [
    'scenario' => 'application-btree-vacuum-pointermap-freeblock-current-source-next259',
    'applicationUse' => 'After deleting an overflow-backed copied wp_options transient, validate current-source next links so pointer-map pages and the leaf freeblock are visible before reusable overflow payload pages are read.',
    'summary' => $summary,
...
if (PHP_SAPI === 'cli' && realpath($_SERVER['SCRIPT_FILENAME'] ?? '') === __FILE__) {
    ...
    if (!$ok) {
        fwrite(STDERR, "application-btree-vacuum-pointermap-freeblock-current-source-next259 self-test failed\n");
        exit(1);
    }

    echo json_encode($payload, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) . "\n";
    echo "application-btree-vacuum-pointermap-freeblock-current-source-next259 self-test passed\n";
}
