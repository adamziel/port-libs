...
$publication = [
    ...
    'database_digest' => $hash('wordpress checkpoint database'),
    'previous_wal_digest' => $hash('wordpress previous wal before checkpoint reset'),
...
$handle = static function (string $name, array $pages) use ($publication): array {
    return [
        ...
        'wal_digest' => hash('sha256', 'wordpress restarted wal after checkpoint reset'),
        'page_digests' => $pages,
        'lock_receipt' => true,
        'sync_receipt' => true,
    ];
...
$summary = [
    'scenario' => 'wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next229',
    'wordpressUse' => 'A copied WordPress import can serve the checkpointed SQLite source only after reopened readers prove clean page images after hot-journal and savepoint cleanup.',
    'status' => $plan['status'],
...
if ($summary['status'] !== 'wal-hot-journal-savepoint-checkpoint-current-source-next229'
    ...
    fwrite(STDERR, "wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next229 self-test failed\n");
    exit(1);
}
