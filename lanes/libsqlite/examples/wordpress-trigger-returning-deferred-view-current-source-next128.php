...
$summary = [
    'scenario' => 'wordpress-trigger-returning-deferred-view-current-source-next128',
    'wordpressUse' => 'A copied WordPress wp_options import can yield recursive trigger RETURNING rows, materialize an autoloaded-options view from that current source, then check deferred parent-option references before advancing the next source.',
    'status' => $plan['status'],
...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "wordpress-trigger-returning-deferred-view-current-source-next128 self-test failed\n");
        exit(1);
    }

    echo "wordpress-trigger-returning-deferred-view-current-source-next128 self-test passed\n";
    exit(0);
}
