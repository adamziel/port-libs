...
$pageSize = 512;
$database = '/srv/wp-content/database/wp-next430.sqlite';
...
$formatPage = static function (string $label) use ($pageSize): string {
    ...
    $page = substr_replace($page, pack('N', 430), 60, 4);
...
$tokenFields = [
    ...
    'reader_cache_stmt_vdbe_close_cursor_token', 'reader_cache_stmt_vdbe_rewind_token',
];
$before = [1 => $formatPage('stale schema'), 2 => $page('stale options')];
$recovered = [1 => $formatPage('current schema'), 2 => $page('current options')];
$tokens = [$journal => 'member-main-current-430'];
$headers = [$journal => hash('sha256', 'main header next430')];
$base = [
    'source_id' => 'wordpress-pager-reader-cache-next430',
    'epoch' => 430,
    'format_signature' => hash('sha256', implode('|', [512, 4, 2, 430, 0])),
    'publication_generation' => 430,
    'master_source_digest' => hash('sha256', 'wordpress next430 master source'),
    'recovery_sequence' => 430,
...
foreach ($tokenFields as $field) {
    $base[$field] = str_replace('_', '-', preg_replace('/_token$/', '', $field)) . '-current-430';
}
$cache = [1 => array_merge($base, ['reader_id' => 'wp-options-reader', 'image' => $recovered[1], 'reader_cache_stmt_vdbe_rewind_token' => 'stmt-vdbe-rewind-old'])];
$read = array_merge($base, [
    'reader_id' => 'read-options',
    'page_number' => 1,
    'member_journal_token_digest' => $mapDigest($tokens),
    'member_journal_header_digest' => $mapDigest($headers),
    'reader_cache_stmt_vdbe_rewind_token' => 'stmt-vdbe-rewind-old',
]);
$plan = SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::readerCacheStatementFence(430,
    $database,
    $master,
    $masterBytes,
    implode('', $before),
...
echo json_encode([
    ...
    'next430_invalidated_pages' => $plan['reader_cache_stmt_vdbe_rewind_invalidated_cache_page_numbers'],
