...
$plan = SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNextPlan::compareExpressionLimitBoundary($sql, $currentTables, $nextTables);

if (in_array('--self-test', $argv, true)) {
    if (($plan['status'] ?? null) !== 'compound-select-window-recursive-limit-current-source-expression-limit-boundary-ready') {
        fwrite(STDERR, "unexpected compound expression-limit-boundary status\n");
        exit(1);
    }
    ...
    if (array_column($plan['nextRows'], 'label') !== ['siteurl', 'theme_mods_next', 'seed:2:3:4', 'seed:2:3:4:5', 'siteurl', 'plugin_search']) {
        fwrite(STDERR, "unexpected compound expression-limit-boundary next rows\n");
        exit(1);
    }
    echo "wordpress-compound-select-window-recursive-limit-current-source-expression-limit-boundary self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'wordpress-compound-select-window-recursive-limit-current-source-expression-limit-boundary',
