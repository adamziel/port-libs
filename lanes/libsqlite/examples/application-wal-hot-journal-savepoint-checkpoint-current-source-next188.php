...
$summary = [
    'scenario' => 'application-wal-hot-journal-savepoint-checkpoint-current-source-next188',
    'applicationUse' => 'A copied Application import recovers a hot journal and rolls back a savepoint, then keeps only prepared wp_usermeta readers whose WAL generation, schema cookie, and commit-hook counter still match the current source.',
    'status' => $plan['status'],
...
if ($summary['status'] !== 'wal-hot-journal-savepoint-checkpoint-current-source-next188'
    ...
    fwrite(STDERR, "application-wal-hot-journal-savepoint-checkpoint-current-source-next188 self-test failed\n");
    exit(1);
}
