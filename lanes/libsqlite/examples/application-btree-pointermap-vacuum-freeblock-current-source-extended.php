...
$summary = [
    'scenario' => 'application-btree-pointermap-vacuum-freeblock-current-source-next144',
    'application_use' => 'Delete a copied wp_options transient row, keep the materialized leaf freeblock auditable, preserve page 106 as the surviving freelist trunk, and prove tail overflow pages are removed across the auto-vacuum pointer-map boundary.',
...
if (PHP_SAPI === 'cli' && in_array('--self-test', $argv, true)) {
    ...
    if (!$ok) {
        fwrite(STDERR, "application-btree-pointermap-vacuum-freeblock-current-source-next144 self-test failed\n");
        exit(1);
    }
}
