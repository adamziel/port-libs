...
$summary = [
    'scenario' => 'application-pragma-index-xinfo-foreignkey-current-source-next367-374',
    'applicationUse' => 'Application import previews can flag NO ACTION lookup-index order/collation mismatches and CASCADE/RESTRICT descending-key lookup indexes through PRAGMA index_xinfo before foreign-key actions are trusted.',
...
if (($argv[1] ?? null) === '--self-test') {
    foreach ($cases as $slice => [, , , $status]) {
        if (($summary["next{$slice}_{$status}"] ?? null) !== 1) {
            fwrite(STDERR, "application-pragma-index-xinfo-foreignkey-current-source-next367-374 self-test failed\n");
            exit(1);
        }
    }
    ...
    fwrite(STDOUT, "application-pragma-index-xinfo-foreignkey-current-source-next367-374 self-test passed\n");
    exit(0);
}
