...
$root = sys_get_temp_dir() . '/port-libsqlite-wordpress-next117-' . bin2hex(random_bytes(4));
$directory = dirname($local($root, $databasePath));
if (!mkdir($directory, 0777, true) && !is_dir($directory)) {
    throw new RuntimeException('Unable to create WordPress next117 smoke directory');
}
...
$savepoints->beginTransaction('wordpress-import-next117');
...
$result = $writer->applyWalHotJournalStatementCurrentSourceNext117(
    $savepoints,
...
$summary = [
    'scenario' => 'wordpress-wal-statement-hot-rollback-current-source-next117',
    'wordpressUse' => 'Apply hot rollback-journal recovery plus current WAL statement-journal rollback through bounded native PHP VFS file handles for a copied wp_options import, preserving retained WAL frames, deleting the hot journal, and truncating failed statement WAL frames before retry.',
...
if (in_array('--self-test', $argv, true)) {
    if ($summary['status'] !== 'applied' || !$summary['journalDeleted'] || !$summary['databaseHasStatementBefore'] || $summary['databaseHasFailedStatement']) {
        fwrite(STDERR, "wordpress WAL statement hot rollback current-source next117 smoke failed\n");
        exit(1);
    }
    echo "wordpress WAL statement hot rollback current-source next117 self-test passed\n";
    exit(0);
}
