...
for ($step = 366; $step <= 373; $step++) {
    $plan = SQLiteRowValueUpdateDeleteReturningWindowCurrentSourceNextPlan::executeCurrentSourceHandoffContinuationStep($step, ...$args);
...
$summary = [
    ...
    'applicationUse' => 'Copied wp_options imports can validate the next366-373 row-value UPDATE/DELETE RETURNING window current-source continuation after the merged after-ready-publication handoff while preserving independent libsqlite throughput.',
];

if (($argv[1] ?? null) === '--self-test') {
    echo "application-rowvalue-returning-window-current-source-next366-373 self-test passed\n";
    return;
}
