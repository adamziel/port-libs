...
foreach ([106 => 107, 107 => 108, 108 => 109, 109 => 110, 110 => 0] as $pageNumber => $nextPage) {
    $pages[$pageNumber] = pack('N', $nextPage) . str_repeat('Q', 508);
}
...
$plan = SQLiteBTreeVacuumPointerMapFreeblockCurrentSourcePlan::tableLeafCurrentSourceWriteGateFromDeleteResult(
    ...
    2,
    str_repeat('next169-application-transient-gate-', 44),
    3,
    true,
    $staleCurrentPages,
);
...
$summary = [
    'scenario' => 'application-btree-vacuum-pointermap-freeblock-current-source-next169',
    'applicationUse' => 'Delete a copied wp_options transient, vacuum obsolete overflow tail pages, and reject stale current-source pages before admitting replacement overflow writes.',
    'admitted_current_source_pages' => $plan->admittedCurrentSourcePages(),
    'stale_current_source_pages' => $plan->staleCurrentSourcePages(),
    'vacuum_rejected_pages' => $plan->vacuumRejectedPages(),
    'replacement_overflow_pages' => $plan->currentSourceGate()['replacement_overflow_pages'],
    'gate_statuses' => array_column($plan->writeGateRows(), 'gate_status'),
];

if (($argv[1] ?? null) === '--self-test') {
    $expected = [
        'admitted_current_source_pages' => [1, 3, 105, 106, 108],
        'stale_current_source_pages' => [107],
        'vacuum_rejected_pages' => [109, 110],
        'replacement_overflow_pages' => [107, 108, 106],
    ];
    foreach ($expected as $key => $value) {
        if ($summary[$key] !== $value) {
            fwrite(STDERR, "application-btree-vacuum-pointermap-freeblock-current-source-next169 self-test failed\n");
            exit(1);
        }
    }

    fwrite(STDOUT, "application-btree-vacuum-pointermap-freeblock-current-source-next169 self-test passed\n");
    exit(0);
}
