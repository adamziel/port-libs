...
if (($argv[1] ?? '') === '--self-test') {
    if (
        ...
        fwrite(STDERR, "application-trigger-recursive-returning-savepoint-source-comparison self-test failed\n");
        exit(1);
    }

    echo "application-trigger-recursive-returning-savepoint-source-comparison self-test passed\n";
    exit(0);
}
...
echo json_encode([
    ...
    'applicationUse' => 'Model a copied wp_options import savepoint where recursive trigger RETURNING rows are yielded by the failed current source, suppressed after ROLLBACK TO, and the next source restarts from the savepoint image.',
