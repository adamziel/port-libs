...
$summary = [
    'scenario' => 'application-pager-master-journal-reader-cache-member-journal-digest',
    'status' => $plan['status'],
    ...
    'applicationUse' => 'A copied Application database with an attached users database prevents a reader-cache page from surviving when the master-journal member list is unchanged but the attached rollback journal digest changed.',
...
if ($summary['status'] !== 'pager-master-journal-reader-cache-current-source-next189'
    ...
    fwrite(STDERR, "application-pager-master-journal-reader-cache-member-journal-digest self-test failed\n");
    exit(1);
}

echo "application-pager-master-journal-reader-cache-member-journal-digest self-test passed\n";
