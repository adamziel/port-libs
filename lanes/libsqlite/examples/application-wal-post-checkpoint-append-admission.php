...
$summary = [
    'scenario' => 'application-wal-hot-journal-savepoint-checkpoint-post-checkpoint-append-admission',
    'applicationUse' => 'A copied Application options import resumes after hot-journal recovery and checkpoint publication, then admits only current writer WAL frame batches while stale plugin writers reopen before appending.',
    'status' => $plan['status'],
...
if (PHP_SAPI === 'cli' && basename(__FILE__) === basename($_SERVER['SCRIPT_FILENAME'] ?? '')) {
    if (
        $summary['status'] !== 'wal-hot-journal-savepoint-checkpoint-post-checkpoint-append-admission'
        || $summary['acceptedAppendBatches'] !== ['autoload-frame-batch']
        || $summary['blockedAppendBatches'] !== ['stale-plugin-frame-batch']
    ) {
        fwrite(STDERR, "application-wal-hot-journal-savepoint-checkpoint-post-checkpoint-append-admission self-test failed\n");
        exit(1);
    }
