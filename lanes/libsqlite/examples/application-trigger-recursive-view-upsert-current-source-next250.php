...
$nextView = $view;
$nextView['source'] = 'main@view-cookie-250-next';
$nextView['trigger_source'] = 'main@trigger-cookie-250-next';
$postResetView = $view;
$postResetView['source'] = 'main@view-cookie-250-post-reset';
$postResetView['trigger_source'] = 'main@trigger-cookie-250-post-reset';
$followingView = $view;
$followingView['source'] = 'main@view-cookie-250-following';
$followingView['trigger_source'] = 'main@trigger-cookie-250-following';

$summary = SQLiteTriggerRecursiveViewUpsertCurrentSourceNextPlan::executeCurrentRowidProvenanceReceipt(
    [
    ...
    [
        ...
        'current_source_cursor_source_close' => 'wp.returning.current.cursor.250',
        'current_source_close_token_source_close' => 'wp.current.source.close.250',
...
if (
    $summary['status_next250'] !== 'trigger-recursive-view-upsert-current-source-next250-rowid-provenance-released'
    || $summary['current_source_rowid_provenance_plan_next250']['decision'] !== 'publish-next-source-after-current-recursive-view-upsert-rowids'
    || array_column($summary['visible_returning_payloads_next250'], 'name') !== ['blogdescription_child', 'template_child', 'home', 'next_plugin']
    || $summary['held_next_source_rows_next250'] !== []
    || count($summary['required_current_source_rowid_receipts_next250']) !== 2
) {
    fwrite(STDERR, "application-trigger-recursive-view-upsert-current-source-next250 self-test failed\n");
    exit(1);
}

echo "application-trigger-recursive-view-upsert-current-source-next250 self-test passed\n";
