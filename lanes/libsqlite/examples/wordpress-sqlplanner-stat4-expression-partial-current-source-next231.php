...
$plan = SQLitePlannerStat4ExpressionPartialCurrentSourceNext231Plan::materialize(
    $prepared,
    $current,
    [
...
if (in_array('--self-test', $argv, true)) {
    ...
    echo "wordpress-sqlplanner-stat4-expression-partial-current-source-next231 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'wordpress-sqlplanner-stat4-expression-partial-current-source-next231',
    'wordpressUse' => 'Copied wp_options plugin admin pagination can reject a stale prepared STAT4 partial-expression page when current rows inserted after ANALYZE alter the selected LIMIT/OFFSET window.',
    'status' => $plan['status'],
