...
$summary = [
    'scenario' => 'application-rowvalue-distinct-subquery-savepoint-rollback',
    'status' => $plan['status'],
    ...
    'applicationUse' => 'Copied wp_options imports can collapse duplicate staging metadata tuples with SELECT DISTINCT before UPDATE/DELETE RETURNING, roll back the attempted stream to a savepoint image, then retry from the current source without duplicate tuple side effects.',
...
if ($summary['status'] !== 'rowvalue-update-delete-returning-distinct-subquery-savepoint-current-source-next225'
    ...
    fwrite(STDERR, "application-rowvalue-distinct-subquery-savepoint-rollback self-test failed\n");
    exit(1);
}
