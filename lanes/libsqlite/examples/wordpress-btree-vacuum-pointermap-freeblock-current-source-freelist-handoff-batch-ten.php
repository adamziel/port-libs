...
$databaseForSlice = static function (int $sliceNumber) use ($makeFirstPage, $putPointerMapEntry): SQLiteDatabase {
    ...
    $pages[3] = SQLiteTableLeafPage::assemble([
        ...
        SQLiteTableLeafCell::encode(2, SQLiteRecord::encode([null, "_transient_freelist_handoff_batch_ten_{$sliceNumber}", str_repeat('cache:', 42)])),
...
$rows = [];
foreach (range(0, 15) as $sliceNumber) {
    $database = $databaseForSlice($sliceNumber);
    $deletedPage = SQLiteTableLeafPage::deleteCellByRowId($database->page(3), 2, secureDelete: true);
    $method = "tableLeafCurrentSourceFreelistHandoffFromDeleteResult";
    $plan = SQLiteBTreeVacuumPointerMapFreeblockCurrentSourcePlan::{$method}(
        $database,
        3,
        [
            'page' => $deletedPage,
