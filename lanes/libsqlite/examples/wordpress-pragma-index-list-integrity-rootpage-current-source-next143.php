...
use PortLibs\LibSqlite\SQLitePragmaIndexListIntegrityRootpageCurrentSourceNext143;
...
$page = SQLitePragmaIndexListIntegrityRootpageCurrentSourceNext143::currentNextPage(
    $catalog,
    $database(false),
    $catalog,
    $database(true),
...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "wordpress-pragma-index-list-integrity-rootpage-current-source-next143 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-index-list-integrity-rootpage-current-source-next143 self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'wordpress-pragma-index-list-integrity-rootpage-current-source-next143',
    'wordpressUse' => 'Resume copied wp_options index repair only when PRAGMA index_list metadata and repaired sqlite_schema rootpage integrity belong to the same current and next database images.',
    'status' => $page['status'],
