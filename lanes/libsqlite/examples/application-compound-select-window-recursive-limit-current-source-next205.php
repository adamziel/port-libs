...
$payload = [
    'scenario' => 'application-compound-select-window-recursive-limit-current-source-rankDenseRankIntersect',
    'applicationUse' => 'Copied wp_options preview queries can rank recursive dependency rows and dense-rank autoload rows, intersect only materialized window output, and reject stale current-source cursors before next-source rows alter the final LIMIT boundary.',
    'status' => $plan['status'],
...
if (($payload['status'] ?? null) !== 'compound-select-window-recursive-limit-current-source-rankDenseRankIntersect-ready') {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-rankDenseRankIntersect self-test failed\n");
    exit(1);
}
if ($payload['rankingFunctions'] !== ['rank', 'dense_rank']) {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-rankDenseRankIntersect window dispatch failed\n");
    exit(1);
}
if ($payload['currentTokenLength'] !== 64 || $payload['nextTokenLength'] !== 64) {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-rankDenseRankIntersect token guard failed\n");
    exit(1);
}
