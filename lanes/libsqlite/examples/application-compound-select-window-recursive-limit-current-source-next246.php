...
if (($argv[1] ?? '') === '--self-test') {
    assert($plan['status'] === 'compound-select-window-recursive-limit-current-source-next246-ready');
    assert($handoff['recursiveLimitExhausted'] === true);
    assert($handoff['nextOnlyLabels'] === ['plugin_prime']);
    assert($handoff['requiredSourceHandoffAckCount'] === 3);
    echo "application-compound-select-window-recursive-limit-current-source-next246 self-test passed\n";
    return;
}

echo json_encode([
    'status' => $plan['status'],
    'currentRows' => $handoff['currentLabels'],
    'nextRows' => $handoff['nextLabels'],
    'handoff' => $handoff,
