...
$payload = [
    'scenario' => 'application-compound-select-window-recursive-limit-current-source-next212',
    'applicationUse' => 'Copied wp_options preview queries can fence string-window recursive dependency rows before staged next-source option rows cross an EXCEPT and final compound LIMIT boundary.',
    'status' => $plan['status'],
...
if (($payload['status'] ?? null) !== 'compound-select-window-recursive-limit-current-source-next212-ready') {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-next212 self-test failed\n");
    exit(1);
}
if ($payload['currentTokenLength'] !== 64 || $payload['nextTokenLength'] !== 64) {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-next212 token guard failed\n");
    exit(1);
}
