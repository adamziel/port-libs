...
$payload = [
    'scenario' => 'wordpress-compound-select-window-recursive-limit-current-source-recursive-window-promotion-barrier',
...
if ($payload['status'] !== 'compound-select-window-recursive-limit-current-source-recursive-window-promotion-barrier-ready') {
    fwrite(STDERR, "wordpress-compound-select-window-recursive-limit-current-source-recursive-window-promotion-barrier self-test failed\n");
    exit(1);
}
if ($payload['requiredPromotionAckCount'] !== 3 || $payload['barrierTokenLength'] !== 64 || $payload['recursiveTraceTokenLength'] !== 64 || $payload['windowFrameTokenLength'] !== 64) {
    fwrite(STDERR, "wordpress-compound-select-window-recursive-limit-current-source-recursive-window-promotion-barrier barrier guard failed\n");
    exit(1);
}
