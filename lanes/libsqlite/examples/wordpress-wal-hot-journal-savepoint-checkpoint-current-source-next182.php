...
$plan = SQLiteWalHotJournalSavepointCheckpointCurrentSourceNext182Plan::plan(
    $databasePath,
    $databaseBytes,
    $pageSize,
    'plugin-import-next182',
...
$summary = [
    'scenario' => 'wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next182',
    'wordpressUse' => 'A copied WordPress import recovers a hot journal, rolls back a plugin savepoint, checkpoints the retained WAL current source, then keeps only prepared statements whose source token, schema cookie, and root pages are still current.',
    'status' => $plan['status'],
...
if ($summary['status'] !== 'wal-hot-journal-savepoint-checkpoint-current-source-next182'
    ...
    fwrite(STDERR, "wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next182 self-test failed\n");
    exit(1);
}
