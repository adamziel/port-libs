...
if ($databasePath === null || $jsonPath === null || $value === null) {
    fwrite(STDERR, "Usage: php lanes/libsqlite/examples/application-json5-option-value.php path/to/application.sqlite json_path json_scalar [limit]\n");
    fwrite(STDERR, "Reads JSON expression indexes over option_value rows that may contain SQLite JSON5 input such as unquoted keys, single-quoted strings, comments, and trailing commas.\n");
    exit(1);
}
...
echo json_encode([
    ...
    'jsonInput' => 'strict JSON or supported SQLite JSON5',
    'limit' => $limit,
    'options' => $options,
