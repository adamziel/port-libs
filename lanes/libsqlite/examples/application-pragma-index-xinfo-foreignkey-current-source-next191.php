...
if (($argv[1] ?? '') === '--self-test') {
    if (
        ...
        fwrite(STDERR, "application-pragma-index-xinfo-foreignkey-current-source-next191 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "application-pragma-index-xinfo-foreignkey-current-source-next191 self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'application-pragma-index-xinfo-foreignkey-current-source-next191',
    'applicationUse' => 'Copied wp_options diagnostics must reject a parent UNIQUE index that merely prefixes the referenced columns with an extra network/site key; PRAGMA index_xinfo proves whether the next schema adds the exact FK parent key.',
