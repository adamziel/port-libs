...
require __DIR__ . '/../src/SQLiteBTreeOverflowFreeblockPointerMapCurrentSourceNext147Plan.php';
...
$plan = SQLiteBTreeOverflowFreeblockPointerMapCurrentSourceNext147Plan::tableAndIndexFromCurrentSourceDeleteResults(
...
$summary = [
    'scenario' => 'wordpress-btree-overflow-freeblock-pointermap-current-source-next147',
    'wordpress_use' => 'Delete a copied wp_options row and its secondary-index overflow chain, coalesce the table leaf freeblock, release both obsolete chains, and allocate the replacement overflow chain without stale pointer-map parents.',
...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "wordpress-btree-overflow-freeblock-pointermap-current-source-next147 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-btree-overflow-freeblock-pointermap-current-source-next147 self-test passed\n");
    exit(0);
}
