...
$current = $prepared;
$current['name'] = 'current-wp-options-stat4-handoff-next734749';
...
$plan = SQLitePlannerStat4ExpressionPartialCurrentSourceNext224Plan::materializeNext734749(
    $prepared,
    $current,
    [
...
if (in_array('--self-test', $argv, true)) {
    assert($plan['status'] === 'stat4-expression-partial-current-source-next734-749-prepared');
    assert($plan['stat4Next734749PreparationFence']['preparedSlices'] === range(734, 749));
    assert($plan['stat4Next734749PreparationFence']['handoffWindows'][0]['continuesSlice'] === 718);
    echo "wordpress-sqlplanner-stat4-expression-partial-current-source-next734-749 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'wordpress-sqlplanner-stat4-expression-partial-current-source-next734-749',
    'wordpressUse' => 'Copied wp_options plugin-admin pagination carries the next718-733 current-source STAT4 handoff into next734-749 only when projected current rows still match.',
    'status' => $plan['status'],
    'selectedIndex' => $plan['selectedPlan']['name'] ?? null,
    'preparedSlices' => $plan['stat4Next734749PreparationFence']['preparedSlices'],
    'handoffSignature' => $plan['stat4Next734749PreparationFence']['handoffSignature'],
