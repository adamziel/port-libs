...
$summary = [
    'scenario' => 'wordpress-vfs-bad-source-lock-regression-current-source-next140',
    'badSourceBlocked' => $blocked,
    'validHydratedOwners' => $valid['events'][2]['owner_locks']['read0'],
    'validStatus' => $valid['status'],
    'wordpressUse' => 'Reject stale or corrupted hydrated SHM source/lock names before a copied wp_options WAL import reuses current-source lock state.',
    'dependencies' => $valid['dependencies'],
];

if (!$blocked || $summary['validHydratedOwners'] !== ['wp-admin', 'wp-cron']) {
    fwrite(STDERR, "wordpress-vfs-bad-source-lock-regression-current-source-next140 self-test failed\n");
    exit(1);
}

if (in_array('--self-test', $argv, true)) {
    echo "wordpress-vfs-bad-source-lock-regression-current-source-next140 self-test passed\n";
    return;
}
