...
$summary = [
    'scenario' => 'wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next196',
    'wordpressUse' => 'A copied WordPress import recovers a hot journal, rolls back a savepoint, checkpoints WAL pages into the database image, and admits wp_options readers only when the persisted WAL sidecar was restarted for the checkpoint generation.',
    'status' => $plan['status'],
...
if ($summary['status'] !== 'wal-hot-journal-savepoint-checkpoint-current-source-next196'
    ...
    fwrite(STDERR, "wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next196 self-test failed\n");
    exit(1);
}
