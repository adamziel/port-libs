...
$plan = SQLiteVfsOpenLockFileControlCurrentSource::currentSourceNext105([
    ...
    ['op' => 'open', 'filename' => 'file://localhost/srv/www/wp-content/database/wp%20copy.sqlite?mode=rw&cache=private&vfs=unix-dotfile&psow=0&wordpress_role=repair&busy_timeout=100'],
...
$summary = [
    'scenario' => 'wordpress-vfs-open-lock-filecontrol-uri-current-source-next105',
    'wordpressUse' => 'Expose per-handle SQLite file: URI parameters through native xFileControl probes while copied wp_options import and repair handles share the same current-source path, locks, and data-version freshness without requiring ext/sqlite.',
    'status' => $plan['status'],
    'importHandle' => [
...
if (($argv[1] ?? null) === '--self-test') {
    ...
    if ($summary['importHandle']['role'] !== 'import' || $summary['repairHandle']['role'] !== 'repair') {
        throw new RuntimeException('Expected per-handle WordPress URI roles');
    }
    ...
    echo "wordpress-vfs-open-lock-filecontrol-uri-current-source-next105 self-test passed\n";
    return;
}
