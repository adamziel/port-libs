...
$summary = [
    'scenario' => 'application-pragma-index-xinfo-foreignkey-current-source-next181',
    'applicationUse' => 'Copied wp_options imports can block a parent-key repair when PRAGMA index_xinfo exposes a UNIQUE parent index whose key collation differs from the parent table column collation.',
    'status' => $page['status'],
...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "application-pragma-index-xinfo-foreignkey-current-source-next181 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "application-pragma-index-xinfo-foreignkey-current-source-next181 self-test passed\n");
    exit(0);
}
