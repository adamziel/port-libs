...
$plan = SQLiteBTreeVacuumPointerMapFreeblockCurrentSourcePlan::tableLeafCheckpointValidationAuditFromDeleteResult(
    $database,
    3,
    [
        'page' => $deletedPage,
...
echo json_encode([
    'scenario' => 'application-btree-vacuum-pointermap-freeblock-checkpoint-validation',
    'applicationUse' => 'After deleting an overflow-backed copied wp_options transient and vacuuming tail pages, keep current-source next-page cursor receipts chained so pointer-map pages are visible before payload source pages advance.',
    ...
    'source_pages_match_write_pages' => $summary['source_pages_match_write_pages'],
    'all_next_links_valid' => $summary['all_next_links_valid'],
    'all_pointer_maps_visible_before_payload_source' => $summary['all_pointer_maps_visible_before_payload_source'],
...
if (
    ...
    && $summary['source_pages_match_write_pages'] === true
    && $summary['all_next_links_valid'] === true
    && $summary['all_pointer_maps_visible_before_payload_source'] === true
) {
    echo "application-btree-vacuum-pointermap-freeblock-checkpoint-validation self-test passed\n";
}
