...
if (($argv[1] ?? '') === '--self-test') {
    ...
    assert($plan['events'][0]['ack'] === 'shared-cache-next689');
    assert($plan['events'][1]['status'] === 'claimed-reusable-current-source');
    assert($plan['events'][1]['claim'] === 'reader-reuse-next705');
    assert($plan['events'][1]['blocked_reasons'] === []);
    assert($plan['events'][2]['status'] === 'published-reused-current-source');
    assert($plan['events'][2]['reuse_ack'] === 'shared-cache-next689');
    assert($plan['events'][2]['token'] === 'shared-cache-next705');
    assert($plan['events'][2]['published_count'] === 35);
    assert(in_array('vfs-current-source-snapshot-reuse-publish-next690-705', $plan['dependencies'], true));
    assert(in_array('vfs-current-source-snapshot-reuse-publish-next674-689', $plan['dependencies'], true));
    echo "wordpress-vfs-current-source-next690-705 self-test passed\n";
    return;
}
