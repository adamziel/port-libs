...
$plan = SQLiteBTreeVacuumPointerMapFreeblockCurrentSourcePlan::tableLeafReceiptCursorFromDeleteResult(
    ...
    2,
    str_repeat('application-next177-batch-', 50),
    3,
    true,
);

$summary = [
    'application_scenario' => 'wp_options transient cleanup hands only readable current-source pages to the next replay batch while fencing truncated pages',
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    if ($summary !== $expected) {
        fwrite(STDERR, "application-btree-vacuum-pointermap-freeblock-current-source-next177 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "application-btree-vacuum-pointermap-freeblock-current-source-next177 self-test passed\n");
    exit(0);
}
