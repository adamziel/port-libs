...
$summary = [
    'scenario' => 'application-rowvalue-subquery-limit-savepoint-retry',
    'applicationUse' => 'Model copied wp_options cleanup where row-value UPDATE/DELETE RETURNING reads ordered and limited metadata subqueries, rolls the attempt back to a savepoint image, and retries against the restored current source.',
    'status' => $plan['status'],
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "application-rowvalue-subquery-limit-savepoint-retry self-test passed\n";
    return;
}
