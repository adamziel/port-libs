...
use PortLibs\LibSqlite\SQLiteTriggerRecursiveViewReturningCurrentSourceNext182Plan;
...
$summary = SQLiteTriggerRecursiveViewReturningCurrentSourceNext182Plan::execute(
    $rows,
    [
...
if (PHP_SAPI === 'cli' && basename(__FILE__) === basename($_SERVER['SCRIPT_FILENAME'] ?? '')) {
    ...
    echo "wordpress-trigger-recursive-view-returning-current-source-next182 self-test passed\n";
}

return [
    'scenario' => 'wordpress-trigger-recursive-view-returning-current-source-next182',
    'wordpressUse' => 'Copied wp_options imports through an INSTEAD OF recursive view trigger keep RETURNING rows tied to the current view-source and trigger-source generation before queued next-source rows are visible.',
