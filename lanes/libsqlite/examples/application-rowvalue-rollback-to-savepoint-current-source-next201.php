...
$summary = [
    'scenario' => 'application-rowvalue-rollback-to-savepoint-current-source-next201',
    'status' => $plan['status'],
    ...
    'applicationUse' => 'Model a copied wp_options cleanup where a plugin savepoint emits speculative row-value UPDATE/DELETE RETURNING rows, rolls back to the savepoint image, then retries from the restored current source.',
...
if (
    ...
    fwrite(STDERR, "application-rowvalue-rollback-to-savepoint-current-source-next201 self-test failed\n");
    exit(1);
}

echo "application-rowvalue-rollback-to-savepoint-current-source-next201 self-test passed\n";
