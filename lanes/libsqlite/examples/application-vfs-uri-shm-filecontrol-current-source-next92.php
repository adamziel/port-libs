...
$summary = [
    'scenario' => 'application-vfs-uri-shm-filecontrol-current-source-next92',
    'status' => $plan['status'],
    'owner' => $owner,
    ...
    'applicationUse' => 'Canonicalize localhost file: URIs and explicit -wal/-shm sidecar filenames to one decoded database owner so copied wp_options import tools keep xFileControl state on the database handle and SHM byte locks on the SHM handle.',
...
if (
    ...
    fwrite(STDERR, "application-vfs-uri-shm-filecontrol-current-source-next92 self-test failed\n");
    exit(1);
}

if (in_array('--self-test', $argv, true)) {
    echo "application-vfs-uri-shm-filecontrol-current-source-next92 self-test passed\n";
    return;
}
