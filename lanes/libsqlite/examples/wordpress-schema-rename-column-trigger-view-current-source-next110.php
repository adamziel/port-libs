...
$preview = [
    'scenario' => 'wordpress-schema-rename-column-trigger-view-current-source-next110',
    ...
    'wordpressUse' => 'Preview a copied WordPress wp_options ALTER TABLE RENAME COLUMN migration using the current sqlite_schema source: dependent trigger/view/index SQL is rewritten, stale prepared statements are invalidated, unrelated wp_postmeta views remain untouched, and no ext/sqlite dependency is required.',
];

if (($preview['tableColumn'] ?? null) !== 'option_key') {
    fwrite(STDERR, "wordpress-schema-rename-column-trigger-view-current-source-next110 self-test failed\n");
    exit(1);
}
