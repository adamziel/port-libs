...
$pageSize = 512;
$database = '/srv/wp-content/database/wp-options-next318.sqlite';
$usersDatabase = '/srv/wp-content/database/wp-users-next318.sqlite';
...
$headers = [$mainJournal => hash('sha256', 'main header next318'), $usersJournal => hash('sha256', 'users header next318')];
$base = [
    'source_id' => 'wordpress-pager-reader-cache-next318',
    'epoch' => 318,
    'format_signature' => hash('sha256', implode('|', [512, 4, 2, 318, 0])),
    'publication_generation' => 318,
    'master_source_digest' => hash('sha256', 'wordpress next318 master source'),
...
$plan = SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::variantNext318(
    $database,
    $master,
    $masterBytes,
    implode('', $before),
    ...
    $base['reader_cache_data_version_token'],
    $base['reader_cache_schema_lock_token'],
);

if (in_array('--self-test', $argv, true)) {
    if ($plan['status'] !== 'pager-master-journal-reader-cache-current-source-next318') {
        fwrite(STDERR, "unexpected pager reader-cache next318 status\n");
        exit(1);
    }
    ...
    if (($plan['next_reads'][3]['reader_cache_schema_lock_token_reason'] ?? '') !== 'reader_ticket_reader_cache_schema_lock_predates_current_source') {
        fwrite(STDERR, "expected schema lock ticket to reopen\n");
        exit(1);
    }
    echo "wordpress-pager-master-journal-reader-cache-current-source-next318 self-test passed\n";
    return;
}
...
echo json_encode([
    ...
    'current_reader_cache_schema_lock_token' => $plan['current_reader_cache_schema_lock_token'],
