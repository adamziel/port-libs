...
$plan = SQLitePlannerStat4ExpressionPartialCurrentSourceNextPlan::materializePreparedHandoffProjectedContinuation(
    $prepared,
    $current,
    [
...
if (in_array('--self-test', $argv, true)) {
    ...
    echo "application-sqlplanner-stat4-expression-partial-prepared-handoff-second-continuation self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'application-sqlplanner-stat4-expression-partial-prepared-handoff-second-continuation',
    'applicationUse' => 'Copied wp_options plugin-admin pagination carries the next750-765 current-source STAT4 handoff into next766-781 only when projected current rows still match.',
    'status' => $plan['status'],
