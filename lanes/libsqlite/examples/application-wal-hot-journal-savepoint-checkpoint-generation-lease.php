...
$summary = [
    'scenario' => 'application-wal-hot-journal-savepoint-checkpoint-current-source-next204',
    'applicationUse' => 'After a copied Application import recovers a hot journal and checkpoints WAL frames, retained wp_options page-cache leases must also carry the current checkpoint generation, schema cookie, page count, and database digest before reuse.',
    'status' => $plan['status'],
...
if ($summary['status'] !== 'wal-hot-journal-savepoint-checkpoint-current-source-next204'
    ...
    fwrite(STDERR, "application-wal-hot-journal-savepoint-checkpoint-current-source-next204 self-test failed\n");
    exit(1);
}
