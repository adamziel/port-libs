...
if (($argv[1] ?? '') === '--self-test') {
    ...
    if ($plan['current_reader_preserved_by_source_handle'] !== true || $plan['changed_page_numbers'] !== [2, 4, 5, 6]) {
        throw new RuntimeException('current Application reader was not protected across consecutive restart checkpoints');
    }
    ...
    echo "application-wal-reader-restart-checkpoint-current-source-next136 self-test passed\n";
    return;
}
