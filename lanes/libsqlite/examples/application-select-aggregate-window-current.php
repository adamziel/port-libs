...
$report = [
    'scenario' => 'application-select-aggregate-window-current',
    'applicationUse' => 'Copied wp_options diagnostics can run parser-level aggregate window functions with SQLite default frames and total() semantics before imports commit grouped option summaries.',
...
if (PHP_SAPI === 'cli' && realpath($_SERVER['SCRIPT_FILENAME'] ?? '') === __FILE__) {
    if (
        ...
        fwrite(STDERR, "application-select-aggregate-window-current self-test failed\n");
        exit(1);
    }
    fwrite(STDOUT, "application-select-aggregate-window-current self-test passed\n");
}
