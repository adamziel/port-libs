...
require_once __DIR__ . '/../src/SQLiteRowValueUpdateDeleteReturningSavepointCurrentSourceNext220Plan.php';
...
$plan = SQLiteRowValueUpdateDeleteReturningSavepointCurrentSourceNext220Plan::execute(
    ['wp_options' => $rows],
    [
...
$summary = [
    'scenario' => 'wordpress-rowvalue-abort-savepoint-current-source-next220',
    'wordpressUse' => 'A copied wp_options import savepoint hits UPDATE OR ABORT on a row-value unique conflict, suppresses the failing RETURNING stream, preserves prior savepoint changes, and retries UPDATE/DELETE RETURNING from that preserved current source.',
    'status' => $plan['status'],
...
if (
    ...
    fwrite(STDERR, "wordpress-rowvalue-abort-savepoint-current-source-next220 self-test failed\n");
    exit(1);
}
