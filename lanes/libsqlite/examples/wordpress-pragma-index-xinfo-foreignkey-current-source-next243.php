...
$page = SQLitePragmaIndexXinfoForeignKeyCurrentSourceNext243::page(
    $currentRecords,
    $nextRecords,
...
$summary = [
    'scenario' => 'wordpress-pragma-index-xinfo-foreignkey-current-source-next243',
    'wordpressUse' => 'Copied WordPress import schemas can compare PRAGMA foreign_key_list child columns with parent table_info affinities before replaying FK-sensitive rows.',
    'status' => $page['status'],
...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "wordpress-pragma-index-xinfo-foreignkey-current-source-next243 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-index-xinfo-foreignkey-current-source-next243 self-test passed\n");
    exit(0);
}
