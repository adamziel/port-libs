...
if (($argv[1] ?? '') === '--self-test') {
    ...
    assert($plan['events'][0]['ack'] === 'shared-cache-next1025');
    assert($plan['events'][0]['blocked_reasons'] === []);
    assert($plan['events'][1]['status'] === 'claimed-reusable-current-source');
    assert($plan['events'][1]['claim'] === 'reader-reuse-next1041');
    assert($plan['events'][1]['blocked_reasons'] === []);
    assert($plan['events'][2]['status'] === 'published-reused-current-source');
    assert($plan['events'][2]['reuse_ack'] === 'shared-cache-next1025');
    assert($plan['events'][2]['token'] === 'shared-cache-next1041');
    assert($plan['events'][2]['published_count'] === 56);
    assert(in_array('vfs-current-source-snapshot-reuse-publish-next1026-1041', $plan['dependencies'], true));
    assert(in_array('vfs-current-source-snapshot-reuse-publish-next1010-1025', $plan['dependencies'], true));
    echo "application-vfs-current-source-next1026-1041 self-test passed\n";
    return;
}
