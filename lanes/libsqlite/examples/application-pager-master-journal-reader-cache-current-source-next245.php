...
$summary = [
    'scenario' => 'application-pager-master-journal-reader-cache-current-source-next245',
    'applicationUse' => 'A copied Application database keeps a schema reader-cache page only when the prepared statement snapshot and sqlite_schema rootpage map were both opened after master-journal recovery; stale wp_options rootpage and active_plugins statement readers reopen before plugin import resumes.',
    'status' => $plan['status'],
...
if ($summary['status'] !== 'pager-master-journal-reader-cache-current-source-next245'
    ...
    fwrite(STDERR, "application-pager-master-journal-reader-cache-current-source-next245 self-test failed\n");
    exit(1);
}

echo json_encode($summary, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) . "\n";
echo "application-pager-master-journal-reader-cache-current-source-next245 self-test passed\n";
