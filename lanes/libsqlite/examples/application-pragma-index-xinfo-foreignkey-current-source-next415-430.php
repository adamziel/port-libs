...
$summary = [
    'scenario' => 'application-pragma-index-xinfo-foreignkey-current-source-next415-430',
    'applicationUse' => 'Application import previews can continue comparing clean current child lookup indexes against next-source foreign-key action relationships with order, collation, or DESC mismatches.',
...
if (($argv[1] ?? null) === '--self-test') {
    foreach ($cases as $slice => [, , , $status]) {
        if (($summary["next{$slice}_current_{$status}"] ?? null) !== 0 || ($summary["next{$slice}_next_{$status}"] ?? null) !== 1) {
            fwrite(STDERR, "application-pragma-index-xinfo-foreignkey-current-source-next415-430 self-test failed\n");
            exit(1);
        }
    }
    ...
    fwrite(STDOUT, "application-pragma-index-xinfo-foreignkey-current-source-next415-430 self-test passed\n");
    exit(0);
}
