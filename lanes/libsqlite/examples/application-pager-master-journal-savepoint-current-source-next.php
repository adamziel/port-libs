...
$summary = [
    'scenario' => 'application-pager-master-journal-savepoint-current-source-next108',
    'status' => $plan['status'],
    ...
    'applicationUse' => 'Copied Application option imports recover attached rollback journals through the master journal first, then seed active SAVEPOINT before-images from that recovered current source so a retry write can still ROLLBACK TO the savepoint without restoring stale crashed bytes.',
];
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    fwrite(STDOUT, "application-pager-master-journal-savepoint-current-source-next108 self-test passed\n");
    return;
}
