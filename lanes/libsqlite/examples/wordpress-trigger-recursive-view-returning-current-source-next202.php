...
$summary = SQLiteTriggerRecursiveViewReturningCurrentSourceNext202Plan::execute(
    [
...
if (
    ($summary['status_next202'] ?? null) !== 'trigger-recursive-view-returning-current-source-next202-next-source-visible'
    || ($summary['visible_row_count_next202'] ?? null) !== 7
    || ($summary['held_next_row_count_next202'] ?? null) !== 0
) {
    fwrite(STDERR, "wordpress-trigger-recursive-view-returning-current-source-next202 self-test failed\n");
    exit(1);
}

if (PHP_SAPI === 'cli' && basename(__FILE__) === basename($_SERVER['SCRIPT_FILENAME'] ?? '')) {
    echo "wordpress-trigger-recursive-view-returning-current-source-next202 self-test passed\n";
}

return [
    'scenario' => 'wordpress-trigger-recursive-view-returning-current-source-next202',
    'wordpressUse' => 'Copied wp_options imports through recursive INSTEAD OF view triggers keep RETURNING rows pinned to the current view generation and recursive depth acknowledgements before next-source rows become visible.',
    ...
    'summary' => [
        'status' => $summary['status_next202'],
        'visibleRows' => $summary['visible_row_count_next202'],
        'heldNextRows' => $summary['held_next_row_count_next202'],
        'requiredDepths' => $summary['required_current_depths_next202'],
        'acknowledgedDepths' => $summary['acknowledged_current_depths_next202'],
        'boundary' => $summary['yield_boundary_next202'],
    ],
];
