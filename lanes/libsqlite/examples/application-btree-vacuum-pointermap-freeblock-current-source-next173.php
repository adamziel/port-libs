...
$plan = SQLiteBTreeVacuumPointerMapFreeblockCurrentSourcePlan::tableLeafTransitionAuditFromDeleteResult(
    $database,
    3,
    [
        'page' => $deletedPage,
    ...
    4,
    str_repeat('application-next173-current-source-transition-', 40),
    3,
    true,
);

$summary = [
    'application_scenario' => 'wp_options transient delete/vacuum/rewrite rejects stale current-source overflow next-pointers',
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    if ($summary !== $expected) {
        fwrite(STDERR, "application-btree-vacuum-pointermap-freeblock-current-source-next173 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "application-btree-vacuum-pointermap-freeblock-current-source-next173 self-test passed\n");
    exit(0);
}
