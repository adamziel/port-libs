...
if (($argv[1] ?? '') === '--self-test') {
    ...
    if (!$ok) {
        fwrite(STDERR, "wordpress-pragma-foreignkey-integrity-partial-root-current-source-next128 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-foreignkey-integrity-partial-root-current-source-next128 self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'wordpress-pragma-foreignkey-integrity-partial-root-current-source-next128',
    'wordpressUse' => 'Inspect copied wp_options root-page integrity and foreign-key blockers without paging unrelated wp_posts root diagnostics.',
