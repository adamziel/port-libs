...
echo json_encode([
    'scenario' => 'wordpress-wal-multi-transaction-cluster',
    'wordpressUse' => 'Inspect copied wp_options WAL files as committed transaction clusters before checkpointing, so import repair tooling can distinguish current readers that still see WAL frames from the next checkpointed database image without ext/sqlite.',
    'status' => $plan['status'],
