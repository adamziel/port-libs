...
$summary = [
    'scenario' => 'application-pragma-index-xinfo-foreignkey-current-source-next231',
    'applicationUse' => 'Copied taxonomy imports must not treat lower(slug) UNIQUE expression indexes as parent-key coverage for foreign_key_check repair; PRAGMA index_xinfo exposes expression terms with NULL names.',
    'status' => $page['status'],
...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "application-pragma-index-xinfo-foreignkey-current-source-next231 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "application-pragma-index-xinfo-foreignkey-current-source-next231 self-test passed\n");
    exit(0);
}
