...
echo json_encode([
    'scenario' => 'wordpress-json-table-left-join-rowid-current-next77',
    'rows' => $rows,
    'matchedOptions' => array_values(array_filter(array_column($rows, 'option_name'), static fn (string $name): bool => $name !== 'plugin_empty')),
    'wordpressUse' => 'Copied wp_options plugin JSON settings can LEFT JOIN json_each() by rowid/_rowid_/oid aliases while preserving NULL extension for empty option arrays and JSONB-backed option values without requiring ext/sqlite.',
