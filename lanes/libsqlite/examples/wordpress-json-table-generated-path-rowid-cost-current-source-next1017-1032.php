...
$currentOption = [
    ...
    'scan_root' => '$.rules',
    'source_generation' => 'current-10171032',
];
$nextOption = array_replace($currentOption, [
    'option_value' => '{"rules":[{"slug":"seo","priority":3},{"slug":"security","priority":9}],"meta":{"version":2}}',
    'generated_path' => '$.rules[1]',
    'source_generation' => 'next-10171032',
]);

$plan = SQLiteJsonTablePlan::currentSourceGeneratedPathRowidCostCurrentSourceNext1032(
    'json_tree',
    $currentOption,
    $nextOption,
    'option_value',
...
$payload = [
    ...
    'currentCostClass' => $plan['currentGeneratedPathRowidCurrentSourceCostSelection1032']['costClass'],
    'currentEstimatedCost' => $plan['currentGeneratedPathRowidCurrentSourceCostSelection1032']['estimatedCost'],
    'nextCostClass' => $plan['nextGeneratedPathRowidCurrentSourceCostSelection1032']['costClass'],
    'replanReasons' => $plan['next1032ReplanReasons'],
...
if (($argv[1] ?? null) === '--self-test') {
    if (!in_array($payload['dependency'], $plan['dependencies'], true)) {
        fwrite(STDERR, "missing next1032 dependency\n");
        exit(1);
    }
    ...
    if ($payload['nextReaderPolicy'] !== 'reprepare-cost-select-next-json-table-generated-path-rowid-next1032') {
        fwrite(STDERR, "unexpected next1032 reader policy\n");
        exit(1);
    }
