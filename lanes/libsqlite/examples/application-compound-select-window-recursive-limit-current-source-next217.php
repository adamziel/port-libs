...
$payload = [
    'scenario' => 'application-compound-select-window-recursive-limit-current-source-next217',
    'applicationUse' => 'Copied wp_options previews can fence rank/dense_rank window rows after recursive queue exhaustion and before an INTERSECT plus final compound LIMIT boundary admits staged rows.',
    'status' => $plan['status'],
...
if (($payload['status'] ?? null) !== 'compound-select-window-recursive-limit-current-source-next217-ready') {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-next217 self-test failed\n");
    exit(1);
}
if ($payload['currentTokenLength'] !== 64 || $payload['nextTokenLength'] !== 64) {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-next217 token guard failed\n");
    exit(1);
}
if (!in_array('theme_mods_next', $payload['nextRows'], true)) {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-next217 next-source boundary failed\n");
    exit(1);
}
