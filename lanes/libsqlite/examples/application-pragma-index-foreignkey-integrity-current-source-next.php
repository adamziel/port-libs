...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "application-pragma-index-foreignkey-integrity-current-source-next self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "application-pragma-index-foreignkey-integrity-current-source-next self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'application-pragma-index-foreignkey-integrity-current-source-next',
    'applicationUse' => 'Resume copied wp_options PRAGMA index_list/index_xinfo/rootpage checks and foreign_key_check parent-index diagnostics under one current-source cursor.',
    'status' => $first['status'],
