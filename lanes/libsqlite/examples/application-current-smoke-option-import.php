...
$smoke = SQLiteCurrentSmokePlan::optionImport($currentRows, $stagedRows, [
    'fail_on_error' => false,
...
if (in_array('--self-test', $argv, true)) {
    if ($smoke['status'] !== 'partial_errors' || $smoke['applied_count'] !== 3 || $smoke['wp_error_codes'] !== ['sqlite_constraint']) {
        fwrite(STDERR, "application-current-smoke-option-import self-test failed\n");
        exit(1);
    }

    echo "application-current-smoke-option-import self-test passed\n";
    exit(0);
}

echo json_encode([
    'applicationUse' => 'Smoke the current pure-PHP SQLite wp_options import path without ext/sqlite: BEGIN IMMEDIATE admission, per-row yields, WP-style error shaping, and statement-only rollback for a duplicate option_name.',
    'status' => $smoke['status'],
