...
$plan = SQLitePagerMasterJournalReaderCacheCurrentSourceNext241Plan::plan(
    $database,
    $master,
    $masterBytes,
    implode('', $before),
...
$summary = [
    'scenario' => 'wordpress-pager-master-journal-reader-cache-current-source-next241',
    'wordpressUse' => 'A copied WordPress database keeps recovered schema/options reader-cache pages only when the page-1 schema cookie matches the master-journal-recovered DDL state; active_plugins cached before plugin-table DDL reopens before import continues.',
    'status' => $plan['status'],
...
if ($summary['status'] !== 'pager-master-journal-reader-cache-current-source-next241'
    ...
    fwrite(STDERR, "wordpress-pager-master-journal-reader-cache-current-source-next241 self-test failed\n");
    exit(1);
}

echo json_encode($summary, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) . "\n";
echo "wordpress-pager-master-journal-reader-cache-current-source-next241 self-test passed\n";
