...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "application-pragma-index-xinfo-foreignkey-current-source-next211 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "application-pragma-index-xinfo-foreignkey-current-source-next211 self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'application-pragma-index-xinfo-foreignkey-current-source-next211',
    'applicationUse' => 'Before applying copied wp_options import rows, report foreign-key child columns that are nullable and can therefore skip SQLite FK checks when any child-key value is NULL.',
    'status' => $page['status'],
