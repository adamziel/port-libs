...
$base = [
    'source_id' => 'wordpress-pager-reader-cache-next638',
    'epoch' => 638,
    'format_signature' => hash('sha256', implode('|', [512, 4, 2, 638, 0])),
    'publication_generation' => 638,
    'master_source_digest' => hash('sha256', 'wordpress next638 master source'),
...
$cache = [1 => array_merge($base, ['reader_id' => 'wp-options-reader', 'image' => $recovered[1], 'reader_cache_stmt_vdbe_bit_and_handoff_token' => 'stmt-vdbe-bit-and-handoff-old'])];
$read = array_merge($base, [
    'reader_id' => 'read-options',
    'page_number' => 1,
    'member_journal_token_digest' => $mapDigest($tokens),
    'member_journal_header_digest' => $mapDigest($headers),
    'reader_cache_stmt_vdbe_bit_and_handoff_token' => 'stmt-vdbe-bit-and-handoff-old',
]);
$plan = SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::currentSourceVdbeBitAndHandoffFence(
    $database,
    $master,
    $masterBytes,
    implode('', $before),
...
echo json_encode([
    ...
    'next638_invalidated_pages' => $plan['reader_cache_stmt_vdbe_bit_and_handoff_invalidated_cache_page_numbers'],
