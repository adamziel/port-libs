...
$verify = SQLiteWalHotJournalSavepointCheckpointCurrentSourceNext183Plan::verify($apply, $apply['files'], [], 183);

$summary = [
    'scenario' => 'wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next183',
    'wordpressUse' => 'A copied WordPress plugin import restarts after hot-journal/checkpoint resume, verifies the post-apply file map and admits a fresh WAL reader current-source token only after durable directory sync evidence.',
...
if ($summary['status'] !== 'wal-hot-journal-savepoint-checkpoint-current-source-next183'
    ...
    fwrite(STDERR, "wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next183 self-test failed\n");
    exit(1);
}
