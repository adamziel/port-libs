...
$summary = [
    'scenario' => 'application-pragma-index-xinfo-foreignkey-current-source-next335-342',
    'applicationUse' => 'Application import previews can keep SET NULL and SET DEFAULT foreign-key action throughput blocked when PRAGMA index_xinfo shows only partial or expression child lookup indexes.',
...
if (($argv[1] ?? null) === '--self-test') {
    foreach (array_keys($summary) as $key) {
        if (str_starts_with($key, 'next') && $summary[$key] !== 1) {
            fwrite(STDERR, "application-pragma-index-xinfo-foreignkey-current-source-next335-342 self-test failed\n");
            exit(1);
        }
    }
    ...
    fwrite(STDOUT, "application-pragma-index-xinfo-foreignkey-current-source-next335-342 self-test passed\n");
    exit(0);
}
