...
$summary = [
    'scenario' => 'application-pager-statement-journal-savepoint-current-source',
    ...
    'applicationUse' => 'Roll back a failed wp_options statement only when the copied database image still matches the current statement-journal source pages, then open the retry statement journal without ext/sqlite.',
];

if (!$summary['currentSourceVerified'] || $summary['restoredPages'] !== [4, 5] || $summary['nextWalFrame'] !== 4) {
    fwrite(STDERR, "application-pager-statement-journal-savepoint-current-source self-test failed\n");
    exit(1);
}
