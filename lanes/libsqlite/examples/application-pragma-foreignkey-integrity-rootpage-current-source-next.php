...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "application-pragma-foreignkey-integrity-rootpage-current-source-next self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "application-pragma-foreignkey-integrity-rootpage-current-source-next self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'application-pragma-foreignkey-integrity-rootpage-current-source-next',
    'applicationUse' => 'Resume a copied wp_options foreign_key_check and rootpage/pointer-map preflight only after the repaired database image and catalog clear both FK rows and rootpage blockers.',
    'status' => $page['status'],
