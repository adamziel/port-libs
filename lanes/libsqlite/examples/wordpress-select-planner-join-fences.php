...
if (($leftJoinPlan['tables'] ?? []) !== ['wp_posts', 'wp_postmeta'] || ($crossJoinPlan['tables'] ?? []) !== ['wp_options', 'wp_postmeta']) {
    fwrite(STDERR, "wordpress-select-planner-join-fences self-test failed\n");
    exit(1);
}

echo json_encode([
    'scenario' => 'wordpress-select-planner-join-fences',
    'wordpressUse' => 'Copied WordPress import diagnostics can preview SQLite planner join fences for LEFT and CROSS joins without incorrectly moving a selective nullable/source table ahead of the preserved side.',
