...
$summary = [
    'scenario' => 'wordpress-select-materialized-recursive-json-current-next51',
    ...
    'wordpressUse' => 'Copied wp_options JSON imports can preserve recursive CTE current/next boundaries while materializing reachable json_tree() rows, including UNION duplicate skips, without ext/sqlite.',
];
...
if (($argv[1] ?? null) === '--self-test') {
    if ($summary['recursiveOptionIds'] !== [10, 20, 30, 40, 50] || $summary['dedupedRecursiveSkips'] !== 2 || $summary['boundaryCount'] !== 5 || $summary['firstBoundary']['currentJsonRows'] !== 4) {
        fwrite(STDERR, "wordpress-select-materialized-recursive-json-current-next51 self-test failed\n");
        exit(1);
    }

    echo "wordpress-select-materialized-recursive-json-current-next51 self-test passed\n";
    exit(0);
}
