...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "wordpress-pragma-integrity-index-rootpage-current-source-next self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-integrity-index-rootpage-current-source-next self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'wordpress-pragma-integrity-index-rootpage-current-source-next',
    'wordpressUse' => 'Resume copied wp_options expression-index analysis only while PRAGMA index_xinfo metadata and sqlite_schema rootpage integrity still match the same current database image.',
    'status' => $page['status'],
