...
$payload = [
    'scenario' => 'application-compound-select-window-recursive-limit-current-source-final-order-resume',
    'applicationUse' => 'Copied wp_options preview queries can hold next-source compound rows until the current final ORDER BY page ordinals are acknowledged.',
    'status' => $plan['status'],
...
if (($payload['status'] ?? null) !== 'compound-select-window-recursive-limit-current-source-final-order-resume-ready') {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-final-order-resume self-test failed\n");
    exit(1);
}
if ($payload['requiredAckCount'] !== 6) {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-final-order-resume ordinal guard failed\n");
    exit(1);
}
if ($payload['resumeTokenLength'] !== 64) {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-final-order-resume token guard failed\n");
    exit(1);
}
if ($payload['nextExposure'] !== 'held-until-current-final-order-ordinals-acked') {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-final-order-resume exposure guard failed\n");
    exit(1);
}
