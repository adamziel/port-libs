...
if (($argv[1] ?? null) === '--self-test') {
    if ($summary['status'] !== 'blocked' || $summary['current']['integrity_errors'] !== 0 || $summary['current']['foreign_key_violations'] !== 1) {
        fwrite(STDERR, "wordpress-pragma-integrity-freelist-foreignkey-current-next49 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-integrity-freelist-foreignkey-current-next49 self-test passed\n");
    exit(0);
}
