...
$writerPlan = [
    ...
    'source_token' => 'wordpress-next261-source',
    'commit_generation' => 261,
    'checkpoint_frame' => 0,
    'database_digest' => hash('sha256', 'wordpress next261 published database'),
    'page_cache_digest' => hash('sha256', 'wordpress next261 clean cache'),
...
$receipts = [
    array_replace($base, ['name' => 'wordpress-next261-database-image', 'receipt_type' => 'database-image']),
    array_replace($base, ['name' => 'wordpress-next261-wal-frame', 'receipt_type' => 'wal-frame']),
    array_replace($base, ['name' => 'wordpress-next261-shm-index', 'receipt_type' => 'shm-index']),
    array_replace($base, ['name' => 'wordpress-next261-readmark-fence', 'receipt_type' => 'readmark-fence']),
    array_replace($base, ['name' => 'wordpress-next261-savepoint-release', 'receipt_type' => 'savepoint-release']),
...
if (PHP_SAPI === 'cli' && in_array('--self-test', $argv, true)) {
    if ($summary['status'] !== 'wal-hot-journal-savepoint-checkpoint-current-source-next261'
        ...
        fwrite(STDERR, "wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next261 self-test failed\n");
        exit(1);
    }
    echo "wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next261 self-test passed\n";
    return;
}
