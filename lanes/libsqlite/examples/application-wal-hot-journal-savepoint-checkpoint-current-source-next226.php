...
$databasePath = '/srv/www/wp-content/database/application.sqlite';
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    assert(in_array('application-import-wal-reset-file-state-reopen', $plan['dependencies'], true));
    echo "application-wal-hot-journal-savepoint-checkpoint-current-source-next226 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'application-wal-hot-journal-savepoint-checkpoint-current-source-next226',
    'status' => $plan['status'],
    'canPublishReopenedCurrentSource' => $plan['can_publish_reopened_current_source'],
    'receiptCount' => $plan['receipt_count'],
    'applicationUse' => 'A copied wp_options import can reopen readers after hot-journal recovery and savepoint checkpoint reset only when the database, WAL reset bytes, missing hot journal, and sync receipts agree.',
