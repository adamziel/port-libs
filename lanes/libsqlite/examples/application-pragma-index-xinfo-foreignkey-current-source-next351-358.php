...
$summary = [
    'scenario' => 'application-pragma-index-xinfo-foreignkey-current-source-next351-358',
    'applicationUse' => 'Application import previews can flag SET NULL and SET DEFAULT foreign-key actions whose child lookup indexes are present but unusable because PRAGMA index_xinfo exposes key-order or collation mismatches.',
...
if (($argv[1] ?? null) === '--self-test') {
    foreach ($cases as $slice => [, , , $status]) {
        if (($summary["next{$slice}_{$status}"] ?? null) !== 1) {
            fwrite(STDERR, "application-pragma-index-xinfo-foreignkey-current-source-next351-358 self-test failed\n");
            exit(1);
        }
    }
    ...
    fwrite(STDOUT, "application-pragma-index-xinfo-foreignkey-current-source-next351-358 self-test passed\n");
    exit(0);
}
