...
$summary = [
    'scenario' => 'wordpress-pragma-index-xinfo-foreignkey-current-source-next242',
    'wordpressUse' => 'Copied WordPress import schemas can distinguish PRAGMA index_xinfo key=0 rowid auxiliary rows from invalid explicit FOREIGN KEY parent rowid aliases.',
...
if (PHP_SAPI === 'cli' && realpath($_SERVER['SCRIPT_FILENAME'] ?? '') === __FILE__) {
    if (
        ...
        fwrite(STDERR, "wordpress-pragma-index-xinfo-foreignkey-current-source-next242 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, json_encode($summary, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) . "\n");
    fwrite(STDOUT, "wordpress-pragma-index-xinfo-foreignkey-current-source-next242 self-test passed\n");
}
