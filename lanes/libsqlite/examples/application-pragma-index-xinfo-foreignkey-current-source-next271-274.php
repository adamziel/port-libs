...
if (($argv[1] ?? '') === '--self-test') {
    if ($page['operation'] !== 'pragma-index-xinfo-foreignkey-current-source-next271' || $page['current']['foreign_key_child_action_lookup_after_current_next271']['repaired'] !== 1) {
        fwrite(STDERR, "application-pragma-index-xinfo-foreignkey-current-source-next271-274 self-test failed\n");
        exit(1);
    }
    echo "application-pragma-index-xinfo-foreignkey-current-source-next271-274 self-test passed\n";
}

return [
    'status' => 'application-pragma-index-xinfo-foreignkey-current-source-next271-274',
    ...
    'applicationUse' => 'Copied wp_termmeta imports can prove an ON DELETE CASCADE child lookup index was repaired after the current source using PRAGMA foreign_key_list plus PRAGMA index_xinfo.',
];
