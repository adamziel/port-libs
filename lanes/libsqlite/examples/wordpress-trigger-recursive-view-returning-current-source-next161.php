...
require_once __DIR__ . '/../src/SQLiteTriggerRecursiveViewReturningCurrentSourceNext161Plan.php';
...
$plan = SQLiteTriggerRecursiveViewReturningCurrentSourceNext161Plan::execute(
    $rows,
    [
    ...
    $nextView,
    $returning,
    ['key' => 'option_name', 'savepoint' => 'wp_recursive_view_161', 'max_depth' => 2],
);

if (($argv[1] ?? '') === '--self-test') {
    assert($plan['status'] === 'trigger-recursive-view-returning-current-source-pinned-next161');
    ...
    echo "wordpress-trigger-recursive-view-returning-current-source-next161 self-test passed\n";
    return;
}
