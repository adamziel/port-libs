...
$current = $prepared;
$current['name'] = 'current-wp-options-stat4-expression-partial-or-split-partial-expression';
...
$plan = SQLitePlannerStat4ExpressionPartialCurrentSourceNextPlan::materializeCurrentOrSplitPartialExpression(
    $prepared,
    $current,
    $arms,
...
if (in_array('--self-test', $argv, true)) {
    assert($plan['status'] === 'stat4-expression-partial-current-or-split-partial-expression-ready');
    assert($plan['matchedRowids'] === [12, 24, 48, 36]);
    assert($plan['orArmPlans'][1]['matchedStat4Keys'] === ['plugin_forms', 'plugin_mail', 'plugin_seo']);
    echo "wordpress-sqlplanner-stat4-expression-partial-current-or-split-partial-expression self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'wordpress-sqlplanner-stat4-expression-partial-current-or-split-partial-expression',
