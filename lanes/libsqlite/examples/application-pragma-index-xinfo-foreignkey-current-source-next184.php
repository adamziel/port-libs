...
$summary = [
    'scenario' => 'application-pragma-index-xinfo-foreignkey-current-source-next184',
    'applicationUse' => 'Copied wp_options imports can preserve PRAGMA index_xinfo parent-key ASC/DESC metadata in current-source FK diagnostics while a parent UNIQUE index is rebuilt.',
    'status' => $page['status'],
...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "application-pragma-index-xinfo-foreignkey-current-source-next184 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "application-pragma-index-xinfo-foreignkey-current-source-next184 self-test passed\n");
    exit(0);
}
