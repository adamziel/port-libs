...
$pageSize = 512;
$root = sys_get_temp_dir() . '/port-libsqlite-wordpress-vfs-temp-journal-' . bin2hex(random_bytes(4));
...
echo json_encode([
    'scenario' => 'wordpress-vfs-temp-journal-commit',
    'wordpressUse' => 'Apply a copied wp_options temporary b-tree commit through native PHP file handles: the temporary rollback journal is synced before temp database pages, then deleted on commit even when a persistent journal mode was requested, without requiring ext/sqlite.',
    'root' => $root,
