...
$summary = [
    'scenario' => 'application-pager-master-journal-reader-cache-current-source-rebase',
    'status' => $plan['status'],
    ...
    'applicationUse' => 'Copied Application option reads after master-journal recovery keep only reader-cache pages rebased to the current source; stale clean pages are refreshed and pinned stale readers are forced back to recovered database bytes.',
...
if ($summary['status'] !== 'pager-master-journal-reader-cache-current-source-next158'
    ...
    fwrite(STDERR, "application-pager-master-journal-reader-cache-current-source-rebase self-test failed\n");
    exit(1);
}

echo "application-pager-master-journal-reader-cache-current-source-rebase self-test passed\n";
