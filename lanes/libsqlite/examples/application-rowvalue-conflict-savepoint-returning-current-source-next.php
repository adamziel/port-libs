...
$summary = [
    'scenario' => 'application-rowvalue-conflict-savepoint-returning-current-source-next143',
    'applicationUse' => 'Model a copied wp_options import savepoint where earlier row-value UPDATE/DELETE RETURNING rows are yielded, a later OR ABORT row-value conflict forces ROLLBACK TO the savepoint image, and the retry executes against the restored current source rather than the failed attempted source.',
    'status' => $plan['status'],
