...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "application-rowvalue-abort-statement-savepoint self-test passed\n";
    return;
}

echo json_encode([
    'applicationUse' => 'Model copied wp_options import cleanup where UPDATE OR ABORT suppresses the failed RETURNING statement but keeps earlier savepoint changes visible to retry DML.',
