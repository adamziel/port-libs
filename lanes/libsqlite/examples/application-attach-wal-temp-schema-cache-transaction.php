...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "application-attach-wal-temp-schema-cache-transaction self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'copied wp_options attach WAL temp schema-cache current',
    'applicationUse' => 'During plugin import, active copied wp_options readers keep their current snapshot while committed main/network schema writes expire the next prepared-statement cache; rolled-back temp/archive savepoint DDL does not add committed schema-cookie changes.',
    'status' => $plan['status'],
