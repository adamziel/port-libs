...
$base = [
    'source_id' => 'application-pager-checksum-receipt-smoke',
    'epoch' => 257,
    'format_signature' => hash('sha256', implode('|', [512, 4, 2, 257, 0])),
    'publication_generation' => 257,
    'master_source_digest' => hash('sha256', 'application next257 master source'),
...
if (($argv[1] ?? '') === '--self-test') {
    if ($summary['status'] !== 'pager-master-journal-reader-cache-current-source-next257'
        ...
        fwrite(STDERR, "application-pager-master-journal-reader-cache-current-source-next257 self-test failed\n");
        exit(1);
    }

    echo "application-pager-master-journal-reader-cache-current-source-next257 self-test passed\n";
    exit(0);
}
