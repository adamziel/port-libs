...
$plan = SQLiteAttachWalTempSchemaCacheCurrentSourceNext92Plan::currentSourceNext118120($schemas, $statements, $events);

if (($argv[1] ?? '') === '--self-test') {
    assert($plan['operation'] === 'attach-wal-temp-schema-cache-current-source-next118-120');
    ...
    echo "wordpress-attach-temp-wal-schema-cache-current-source-next118-120 self-test passed\n";
    return;
}
