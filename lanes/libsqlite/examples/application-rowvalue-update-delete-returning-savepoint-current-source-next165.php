...
$summary = [
    'scenario' => 'application-rowvalue-update-delete-returning-savepoint-current-source-next165',
    'applicationUse' => 'A copied wp_options import can ignore duplicate row-value key updates without yielding RETURNING rows, keep the savepoint current source active, then continue cleanup DELETE RETURNING and follow-up UPDATE RETURNING statements.',
    'status' => $plan['status'],
...
if (
    ...
    fwrite(STDERR, "application-rowvalue-update-delete-returning-savepoint-current-source-next165 self-test failed\n");
    exit(1);
}

echo "application-rowvalue-update-delete-returning-savepoint-current-source-next165 self-test passed\n";
