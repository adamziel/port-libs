...
if (($argv[1] ?? '') === '--self-test') {
    if (
        ...
        fwrite(STDERR, "application-attach-wal-temp-commit-plan self-test failed\n");
        exit(1);
    }

    echo "application-attach-wal-temp-commit-plan self-test passed\n";
    exit(0);
}

echo json_encode([
    'scenario' => 'application-attach-wal-temp-commit-plan',
    'status' => $plan['status'],
