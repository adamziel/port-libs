...
if (PHP_SAPI === 'cli' && basename(__FILE__) === basename($_SERVER['SCRIPT_FILENAME'] ?? '')) {
    if ($plan['status'] !== 'rolled-back-to-savepoint-after-returning-yield' || $plan['deleted_parent_keys'] !== [1, 2, 3] || $plan['next_parent_keys'] !== [1, 2, 3, 4]) {
        fwrite(STDERR, "wordpress-trigger-returning-recursive-fk-current-source-next124 self-test failed\n");
        exit(1);
    }

    echo "wordpress-trigger-returning-recursive-fk-current-source-next124 self-test passed\n";
}

return [
    'scenario' => 'wordpress-trigger-returning-recursive-fk-current-source-next124',
    'wordpressUse' => 'Preview copied wp_options recursive DELETE triggers whose RETURNING rows are yielded from the current source before FK CASCADE deletes child rows and before a savepoint rollback restores the next source.',
    'status' => $plan['status'],
