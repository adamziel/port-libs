...
$makePlan = static function (int $sliceNumber) use ($firstPage, $putPointerMapEntry): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourcePlan {
    ...
    return SQLiteBTreeVacuumPointerMapFreeblockCurrentSourcePlan::tableLeafFreelistSpliceFromDeleteResult(
        ...
        2,
        str_repeat("wordpress-next{$sliceNumber}-current-source-freelist-", 32),
        3,
    );
};
...
echo json_encode([
    'wordpress_scenario' => 'wp_options transient deletes keep pointer-map freelist splice receipts distinct across current-source next264 through next266',
    'summaries' => $summaries,
...
echo "wordpress-btree-vacuum-pointermap-freeblock-current-source-next264-266 self-test passed\n";
