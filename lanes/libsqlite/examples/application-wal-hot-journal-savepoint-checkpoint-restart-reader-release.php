...
$summary = [
    'scenario' => 'application-wal-hot-journal-savepoint-checkpoint-current-source-restart-reader-release',
    'applicationUse' => 'A copied Application options import finishes hot-journal recovery and savepoint checkpointing, releases current readers, restarts the WAL with a rotated salt, and only then deletes the hot journal.',
    'status' => $plan['status'],
...
if (PHP_SAPI === 'cli' && realpath($_SERVER['SCRIPT_FILENAME'] ?? '') === __FILE__) {
    if ($summary['status'] !== 'wal-hot-journal-savepoint-checkpoint-current-source-restart-reader-release'
        ...
        fwrite(STDERR, "application-wal-hot-journal-savepoint-checkpoint-current-source-restart-reader-release self-test failed\n");
        exit(1);
    }

    echo "application-wal-hot-journal-savepoint-checkpoint-current-source-restart-reader-release self-test passed\n";
}
