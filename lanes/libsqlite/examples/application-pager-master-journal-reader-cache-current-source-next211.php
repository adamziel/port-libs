...
$sourceId = 'application-next211-master-source';
...
$summary = [
    ...
    'applicationUse' => 'A copied Application import with attached usermeta journal replay keeps schema/root reader-cache pages but reopens active_plugins when the attached member journal recovered-page set changes under the same master-journal bytes.',
];
...
if (($argv[1] ?? '') === '--self-test') {
    if ($summary['status'] !== 'pager-master-journal-reader-cache-current-source-next211'
        ...
        fwrite(STDERR, "application-pager-master-journal-reader-cache-current-source-next211 self-test failed\n");
        exit(1);
    }

    echo "application-pager-master-journal-reader-cache-current-source-next211 self-test passed\n";
    exit(0);
}
