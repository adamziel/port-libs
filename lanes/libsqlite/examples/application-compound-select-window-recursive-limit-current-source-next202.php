...
$payload = [
    'scenario' => 'application-compound-select-window-recursive-limit-current-source-ntileFirstValueCursorReplay',
...
if (($payload['status'] ?? null) !== 'compound-select-window-recursive-limit-current-source-ntileFirstValueCursorReplay-ready') {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-ntileFirstValueCursorReplay self-test failed\n");
    exit(1);
}
if ($payload['windowFunctions'] !== ['ntile', 'first_value']) {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-ntileFirstValueCursorReplay window dispatch failed\n");
    exit(1);
}
if ($payload['currentTokenLength'] !== 64 || $payload['nextTokenLength'] !== 64) {
    fwrite(STDERR, "application-compound-select-window-recursive-limit-current-source-ntileFirstValueCursorReplay token guard failed\n");
    exit(1);
}
