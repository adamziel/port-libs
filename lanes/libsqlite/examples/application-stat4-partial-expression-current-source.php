...
if (($argv[1] ?? '') === '--self-test') {
    ...
    echo "application-stat4-partial-expression-current-source self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'application-stat4-partial-expression-current-source',
    'status' => $plan['status'],
    ...
    'applicationUse' => 'Preview copied wp_options plugin-option scans after ANALYZE and row churn: stale partial expression-index plans reprepare to current STAT4 samples, block deleted prepared payload rows, and keep refreshed autoloaded option payloads on the covering cursor.',
