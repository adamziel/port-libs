...
if (($argv[1] ?? null) === '--self-test') {
    ...
    echo "wordpress-planner-stat4-partial-skipscan-order-current-next52 self-test passed\n";
    exit(0);
}

echo json_encode([
    'scenario' => 'wordpress-planner-stat4-partial-skipscan-order-current-next52',
    'status' => $plan['status'],
    ...
    'wordpressUse' => 'Copied wp_options plugin-option scans can use STAT4 current/next evidence for a partial skip-scan index while recognizing suffix-only ORDER BY as a per-prefix block sort, without requiring ext/sqlite.',
