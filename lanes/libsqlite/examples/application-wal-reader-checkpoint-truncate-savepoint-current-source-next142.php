...
$databaseBytes = $page('wp next142 base schema')
    . $page('wp next142 base active_plugins')
    . $page('wp next142 base plugin settings')
    . $page('wp next142 base transient cache');

$prefix = pack('N*', SQLiteWalHeader::MAGIC_BIG_ENDIAN, 3007000, $pageSize, 142, $salt1, $salt2);
$seed = SQLiteWal::checksumPair($prefix, false);
$walBytes = $prefix . pack('N*', $seed[0], $seed[1]);
foreach ([
    [1, 0, 'wp next142 retained schema draft'],
    [2, 4, 'wp next142 retained active_plugins commit'],
    [3, 0, 'wp next142 rolled plugin settings draft'],
    [3, 4, 'wp next142 rolled plugin settings commit'],
    [4, 4, 'wp next142 rolled transient commit'],
...
$makeShm = static function (array $readMarks, array $readLocks, int $backfill, int $attempted, int $mxFrame = 5) use ($pageSize, $salt1, $salt2): string {
    ...
    $header = pack('V*', 3007000, $backfill, 142, $pageSizeField, $mxFrame, 4, 1, 2, $salt1, $salt2, 5, 6);
...
$plan = SQLiteWalSavepointCheckpointPlan::readerCheckpointTruncateSavepointCurrentSourceNext(
    ...
    [[
        'pages' => [
            3 => $page('wp next142 retried plugin settings fresh generation'),
            4 => $page('wp next142 retried transient fresh generation'),
        ],
...
$summary = [
    'scenario' => 'application-wal-reader-checkpoint-truncate-savepoint-current-source-next142',
    'applicationUse' => 'A copied Application options import rolls back a plugin-setting savepoint while an existing reader is pinned on the current WAL source; TRUNCATE checkpoint waits for that reader, then starts a fresh WAL generation for the retry write.',
    'status' => $plan['status'],
...
if (PHP_SAPI === 'cli' && basename(__FILE__) === basename($_SERVER['SCRIPT_FILENAME'] ?? '')) {
    if ($summary['status'] !== 'reader-checkpoint-truncate-savepoint-current-source-next142'
        ...
        fwrite(STDERR, "application-wal-reader-checkpoint-truncate-savepoint-current-source-next142 self-test failed\n");
        exit(1);
    }

    echo "application-wal-reader-checkpoint-truncate-savepoint-current-source-next142 self-test passed\n";
}
