...
$result = [
    'scenario' => 'wordpress-json-aggregate-distinct-order-window-current-source-next90',
    'wordpressUse' => 'Copied wp_options diagnostics can compute parser-level JSON aggregate windows with DISTINCT, FILTER, and multi-term aggregate ORDER BY independently from the window frame order before WordPress imports commit option summaries.',
    'sqlShape' => $sql,
    'windows' => array_map(
...
if (PHP_SAPI === 'cli' && basename($_SERVER['SCRIPT_FILENAME'] ?? '') === basename(__FILE__)) {
    ...
    echo "wordpress-json-aggregate-distinct-order-window-current-source-next90 self-test passed\n";
}
