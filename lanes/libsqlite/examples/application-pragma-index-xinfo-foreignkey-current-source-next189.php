...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "application-pragma-index-xinfo-foreignkey-current-source-next189 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "application-pragma-index-xinfo-foreignkey-current-source-next189 self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'application-pragma-index-xinfo-foreignkey-current-source-next189',
    'applicationUse' => 'During term/meta import preflight, show why partial or expression UNIQUE indexes visible through PRAGMA index_xinfo do not satisfy a foreign-key parent key, and confirm the repaired full UNIQUE index.',
    'status' => $page['status'],
