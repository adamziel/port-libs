...
$summary = [
    'scenario' => 'application-pager-master-journal-reader-cache-current-source-next238',
    'applicationUse' => 'A copied Application database keeps recovered schema/options reader-cache pages only when the sqlite_schema root digest matches the master-journal-recovered DDL state; an active_plugins page cached before plugin-table DDL reopens before import continues.',
    'status' => $plan['status'],
...
if ($summary['status'] !== 'pager-master-journal-reader-cache-current-source-next238'
    ...
    fwrite(STDERR, "application-pager-master-journal-reader-cache-current-source-next238 self-test failed\n");
    exit(1);
}

echo json_encode($summary, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) . "\n";
echo "application-pager-master-journal-reader-cache-current-source-next238 self-test passed\n";
