...
if (($argv[1] ?? null) === '--self-test') {
    ...
    echo "application-planner-covering-range-order-current-source self-test passed\n";
    exit(0);
}

echo json_encode([
    'scenario' => 'application-planner-covering-range-order-current-source',
    ...
    'applicationUse' => 'Copied wp_options plugin option scans can reprepare against the current schema/STAT4 source and stream a covering multicolumn range index in ORDER BY order without table rowid lookup or a temp sort, without requiring ext/sqlite.',
