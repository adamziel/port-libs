...
$root = sys_get_temp_dir() . '/port-libsqlite-wordpress-wal-append-' . bin2hex(random_bytes(4));
$localWal = $root . '/' . $databasePath . '-wal';
if (!is_dir(dirname($localWal)) && !mkdir(dirname($localWal), 0777, true) && !is_dir(dirname($localWal))) {
    throw new RuntimeException('Unable to create WordPress WAL append smoke directory');
}
...
$report = [
    'scenario' => 'wordpress-wal-append-transaction',
    'wordpressUse' => 'Append copied wp_options import pages to a WAL sidecar with SQLite-compatible chained checksums, commit markers, and native PHP VFS persistence, so shared-hosting repair/import tools can stage committed option rows while preserving an uncommitted draft tail without ext/sqlite.',
