...
require_once __DIR__ . '/../src/SQLitePagerMasterJournalReaderCacheCurrentSourceNext179Plan.php';
...
$plan = SQLitePagerMasterJournalReaderCacheCurrentSourceNext179Plan::plan(
    $database,
    $master,
    implode("\n", $members) . "\n",
    $canonicalMap,
...
$summary = [
    'scenario' => 'wordpress-pager-master-journal-reader-cache-current-source-next179',
    'status' => $plan['status'],
    ...
    'wordpressUse' => 'A copied WordPress SQLite database can retain or refresh reader-cache pages when master-journal members are reached through VFS aliases, but only after canonical path resolution proves the same attached rollback journals.',
...
if (
    ...
    fwrite(STDERR, "wordpress-pager-master-journal-reader-cache-current-source-next179 self-test failed\n");
    exit(1);
}

echo "wordpress-pager-master-journal-reader-cache-current-source-next179 self-test passed\n";
