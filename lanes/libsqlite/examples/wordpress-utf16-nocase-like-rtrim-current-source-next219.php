...
use PortLibs\LibSqlite\SQLiteUtf16NocaseLikeRtrimCurrentSourceNext219Plan;
...
$plan = SQLiteUtf16NocaseLikeRtrimCurrentSourceNext219Plan::wordpressOptionNameSupplementaryWildcardPlan(
    $currentRows,
    $nextRows,
);
...
$payload = [
    ...
    'wordpressUse' => 'Copied wp_options scans can keep UTF-16 supplementary-plane option names binary safe: one LIKE underscore wildcard consumes one decoded emoji character, while RTRIM and NOCASE remain SQLite-compatible ASCII operations.',
];
...
if (($argv[1] ?? null) === '--self-test') {
    ...
    echo "wordpress-utf16-nocase-like-rtrim-current-source-next219 self-test passed\n";
    return;
}
