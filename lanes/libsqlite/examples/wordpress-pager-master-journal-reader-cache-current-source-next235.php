...
use PortLibs\LibSqlite\SQLitePagerMasterJournalReaderCacheCurrentSourceNext235Plan;
...
$plan = SQLitePagerMasterJournalReaderCacheCurrentSourceNext235Plan::plan(
    $database,
    $master,
    $masterBytes,
    implode('', $before),
...
$summary = [
    'scenario' => 'wordpress-pager-master-journal-reader-cache-current-source-next235',
    'wordpressUse' => 'A copied WordPress main database keeps recovered schema/options reader-cache pages only when their database change counter matches the master-journal-recovered header; an image-identical active_plugins page from the previous change counter reopens before plugin import continues.',
    'status' => $plan['status'],
...
if ($summary['status'] !== 'pager-master-journal-reader-cache-current-source-next235'
    ...
    fwrite(STDERR, "wordpress-pager-master-journal-reader-cache-current-source-next235 self-test failed\n");
    exit(1);
}

echo json_encode($summary, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) . "\n";
echo "wordpress-pager-master-journal-reader-cache-current-source-next235 self-test passed\n";
