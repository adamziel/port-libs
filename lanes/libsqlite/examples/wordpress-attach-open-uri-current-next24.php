...
$attach = $catalog->executeAttachDetachSql(
    "ATTACH DATABASE 'file:/srv/wordpress/wp-content/database/site%20copy.sqlite?mode=ro&immutable=1&cache=shared' AS site",
    $loader,
);
...
$summary = [
    ...
    'wordpressUse' => 'Attach a copied WordPress SQLite database by file URI, decode the filename for PRAGMA database_list and schema loading, and preserve mode/cache/immutable open metadata without requiring ext/sqlite.',
];

if (($argv[1] ?? '') === '--self-test') {
    $expected = [
        'schema' => 'site',
        'decoded_file' => '/srv/wordpress/wp-content/database/site copy.sqlite',
        'loader_received' => ['/srv/wordpress/wp-content/database/site copy.sqlite', 'site'],
        'open_status' => 'ready',
        'read_only' => true,
        'immutable' => true,
        'cache' => 'shared',
        'database_list_file' => '/srv/wordpress/wp-content/database/site copy.sqlite',
        'site_options_schema' => 'site',
    ];

    foreach ($expected as $key => $value) {
        if ($summary[$key] !== $value) {
            fwrite(STDERR, "wordpress-attach-open-uri-current-next24 self-test failed at {$key}\n");
            exit(1);
        }
    }

    echo "wordpress-attach-open-uri-current-next24 self-test passed\n";
    exit(0);
}
