...
$payload = [
    ...
    'wordpressUse' => 'Copied wp_options imports can prepare ready publication receipts from row-value UPDATE/DELETE RETURNING window candidates before publishing the next source.',
];
...
if (($argv[1] ?? null) === '--self-test') {
    if ($payload['status'] !== 'rowvalue-update-delete-returning-window-ready-publication-metadata'
        ...
        fwrite(STDERR, "wordpress-rowvalue-returning-window-ready-publication-metadata self-test failed\n");
        exit(1);
    }

    echo "wordpress-rowvalue-returning-window-ready-publication-metadata self-test passed\n";
    return;
}
