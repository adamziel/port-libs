...
$resume = SQLiteWalHotJournalSavepointCheckpointCurrentSourceNext174Plan::plan(
    $databasePath,
    $dirtyDatabase,
    $journalBytes,
    $stack,
...
$next177 = SQLiteWalHotJournalSavepointCheckpointCurrentSourceNext177Plan::plan($resume);
$plan = SQLiteWalHotJournalSavepointCheckpointCurrentSourceNext180Plan::apply(
    $next177,
    [
        $journalPath => $journalBytes,
        $walPath => $walPayload,
...
$summary = [
    'scenario' => 'wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next180',
    'wordpressUse' => 'A copied WordPress plugin import resumes after a crash between hot-journal recovery and checkpoint publication, atomically publishing the missing checkpoint database bytes before the hot journal can be retired.',
    'status' => $plan['status'],
...
if ($summary['status'] !== 'wal-hot-journal-savepoint-checkpoint-current-source-next180'
    ...
    fwrite(STDERR, "wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next180 self-test failed\n");
    exit(1);
}
