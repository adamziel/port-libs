...
$summary = [
    'scenario' => 'wordpress-pager-master-journal-statement-recovery-current-next75',
    'status' => $plan['status'],
    ...
    'wordpressUse' => 'Recover copied main and attached WordPress SQLite databases from a master journal, then roll back the failed wp_options statement subjournal and prepare the next retry statement without ext/sqlite.',
];

if ($summary['status'] !== 'recovered' || $summary['statementRollbackPages'] !== [4]) {
    fwrite(STDERR, "wordpress-pager-master-journal-statement-recovery-current-next75 self-test failed\n");
    exit(1);
}

fwrite(STDOUT, "wordpress-pager-master-journal-statement-recovery-current-next75 self-test passed\n");
