...
$plan = SQLiteRowValueReturningSavepointConflictCurrentSourceNextPlan::execute(
    ...
    [['blog_id', 'option_name']],
    'app_settings_conflict_import',
    'option_id',
);

$summary = [
    'scenario' => 'application-rowvalue-returning-savepoint-conflict-current-source-next128',
    'applicationUse' => 'Model a copied wp_options import savepoint where row-value UPDATE RETURNING statements use OR IGNORE and OR REPLACE against the multisite option unique key before transient cleanup, so import tooling can explain yielded rows, ignored rows, replacement deletes, and savepoint state without the SQLite extension.',
    'status' => $plan['status'],
...
if (PHP_SAPI === 'cli' && in_array('--self-test', $argv, true)) {
    ...
    echo "application-rowvalue-returning-savepoint-conflict-current-source-next128 self-test passed\n";
    return;
}
