...
if (($_SERVER['argv'][1] ?? '') === '--self-test') {
    echo "application-planner-stat4-expression-partial-current-source-range-refresh self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'application-planner-stat4-expression-partial-current-source-range-refresh',
    'status' => $plan['status'],
    ...
    'applicationUse' => 'Copied wp_options plugin screens can reprepare stale partial expression-index STAT4 samples after ANALYZE/schema changes, keeping plugin autoload scans on the current-source index without ext/sqlite.',
