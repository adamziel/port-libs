...
$base = [
    'source_id' => 'wordpress-pager-reader-cache-next510',
    'epoch' => 510,
    'format_signature' => hash('sha256', implode('|', [512, 4, 2, 510, 0])),
    'publication_generation' => 510,
    'master_source_digest' => hash('sha256', 'wordpress next510 master source'),
...
$cache = [1 => array_merge($base, ['reader_id' => 'wp-options-reader', 'image' => $recovered[1], 'reader_cache_stmt_vdbe_if_not_open_token' => 'stmt-vdbe-if_not_open-old'])];
$read = array_merge($base, [
    'reader_id' => 'read-options',
    'page_number' => 1,
    'member_journal_token_digest' => $mapDigest($tokens),
    'member_journal_header_digest' => $mapDigest($headers),
    'reader_cache_stmt_vdbe_if_not_open_token' => 'stmt-vdbe-if_not_open-old',
]);
$plan = SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::readerCacheStatementFence(510,
    $database,
    $master,
    $masterBytes,
    implode('', $before),
...
echo json_encode([
    ...
    'next510_invalidated_pages' => $plan['reader_cache_stmt_vdbe_if_not_open_invalidated_cache_page_numbers'],
