...
$current = $prepared;
$current['name'] = 'current-wp-options-stat4-handoff-next830845';
...
$plan = SQLitePlannerStat4ExpressionPartialCurrentSourceNext224Plan::materializeNext830845(
    $prepared,
    $current,
    [
...
if (in_array('--self-test', $argv, true)) {
    assert($plan['status'] === 'stat4-expression-partial-current-source-next830-845-prepared');
    assert($plan['stat4Next830845PreparationFence']['preparedSlices'] === range(830, 845));
    assert($plan['stat4Next830845PreparationFence']['handoffWindows'][0]['continuesSlice'] === 814);
    echo "wordpress-sqlplanner-stat4-expression-partial-current-source-next830-845 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'wordpress-sqlplanner-stat4-expression-partial-current-source-next830-845',
    'wordpressUse' => 'Copied wp_options plugin-admin pagination carries the next814-829 current-source STAT4 handoff into next830-845 only when projected current rows still match.',
    'status' => $plan['status'],
    'selectedIndex' => $plan['selectedPlan']['name'] ?? null,
    'preparedSlices' => $plan['stat4Next830845PreparationFence']['preparedSlices'],
    'handoffSignature' => $plan['stat4Next830845PreparationFence']['handoffSignature'],
