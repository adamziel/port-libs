...
$likePlan = SQLiteLikeGlobCurrentSourceNextPlan::wordpressOptionNameStatement($currentRows, $nextRows, $like, $like);
$globPlan = SQLiteLikeGlobCurrentSourceNextPlan::wordpressOptionNameStatement($currentRows, $nextRows, $glob, $glob);

$summary = [
    'scenario' => 'wordpress-like-escape-glob-candidates-current-source-next147',
    ...
    'wordpressUse' => 'Copied wp_options option_name LIKE ESCAPE and GLOB probes can keep index-range candidate rows separate from residual matches so source-cookie, encoding, and byte changes still reprepare stale cursors.',
];
...
if (in_array('--self-test', $argv, true)) {
    ...
    echo "wordpress-like-escape-glob-candidates-current-source-next147 self-test passed\n";
    return;
}
