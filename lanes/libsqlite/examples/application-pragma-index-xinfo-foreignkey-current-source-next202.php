...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "application-pragma-index-xinfo-foreignkey-current-source-next202 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "application-pragma-index-xinfo-foreignkey-current-source-next202 self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'application-pragma-index-xinfo-foreignkey-current-source-next202',
    'applicationUse' => 'When a copied taxonomy schema is reparsed through SQLite table-valued PRAGMA functions, page index_xinfo and foreign_key_list from the same current-source snapshot before applying the migrated composite term key.',
    'status' => $page['status'],
