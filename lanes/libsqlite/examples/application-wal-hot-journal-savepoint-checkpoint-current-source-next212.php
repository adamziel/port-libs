...
$summary = [
    'scenario' => 'application-wal-hot-journal-savepoint-checkpoint-current-source-next212',
    'applicationUse' => 'A copied Application import recovers a hot journal, rolls back a plugin savepoint, then runs a PASSIVE checkpoint that writes only through the current reader pin and preserves WAL bytes for the still-open wp_options reader.',
    'status' => $plan['status'],
    'busy' => $plan['busy'],
...
if (PHP_SAPI === 'cli' && basename(__FILE__) === basename($_SERVER['SCRIPT_FILENAME'] ?? '')) {
    if (
        ...
        fwrite(STDERR, "application-wal-hot-journal-savepoint-checkpoint-current-source-next212 self-test failed\n");
        exit(1);
    }
