...
$current = $prepared;
$current['name'] = 'current-wp-options-stat4-expression-partial-partial-predicate-delta-equality';
...
$plan = SQLitePlannerStat4ExpressionPartialCurrentSourceNextPlan::materializeCurrentPartialPredicateDeltaEquality(
    $prepared,
    $current,
    [
...
if (($argv[1] ?? null) === '--self-test') {
    assert($plan['status'] === 'stat4-expression-partial-current-partial-predicate-delta-equality-ready');
    ...
    echo "application-sqlplanner-stat4-expression-partial-current-partial-predicate-delta-equality self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'application-sqlplanner-stat4-expression-partial-current-partial-predicate-delta-equality',
    'status' => $plan['status'],
    ...
    'applicationUse' => 'Copied wp_options multisite imports can reprepare a stale partial expression index after ANALYZE and partial WHERE changes, then scan only the current blog equality bucket without leaking network option rows.',
