...
$payload = [
    'scenario' => 'wordpress-select-recursive-cte-materialized-current-next26',
    'wordpressUse' => 'Preview a bounded recursive import ID window over copied wp_options rows while preserving SQLite recursive-term LIMIT/OFFSET semantics for MATERIALIZED CTEs without requiring ext/sqlite.',
...
if (PHP_SAPI === 'cli' && in_array('--self-test', $argv, true)) {
    ...
    echo "wordpress-select-recursive-cte-materialized-current-next26 self-test passed\n";
    return;
}
