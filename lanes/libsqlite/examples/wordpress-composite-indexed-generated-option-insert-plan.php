...
$plan = $database->planWordPressOptionInsert(3, 'home', $optionValue, 'yes');
...
echo json_encode([
    'wordpressUse' => 'Plan a bounded generated wp_options row insert while maintaining a single-leaf autoload, option_name composite index, without the SQLite extension.',
    'plan' => $plan->toArray(),
    'updatedPageNumbers' => array_keys($plan->pageImages()),
    'compositeIndexRecords' => $indexRecords,
    'indexedHomeOption' => $postDatabase->wordpressOptionByIndexedAutoloadAndName('yes', 'HOME')?->toArray(),
    'options' => $options,
