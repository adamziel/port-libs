...
$plan = SQLiteWalHotJournalSavepointCheckpointCurrentSourceNext167Plan::plan(
    $databasePath,
    $databaseBytes,
    $pageSize,
    'plugin-import-next167',
...
$summary = [
    'scenario' => 'wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next167',
    'wordpressUse' => 'A copied WordPress plugin import recovers hot-journal pages, rolls back a failed option savepoint, then verifies the exact current-source fingerprint before retaining any checkpoint readers.',
    'status' => $plan['status'],
...
if ($summary['status'] !== 'wal-hot-journal-savepoint-checkpoint-current-source-next167'
    ...
    fwrite(STDERR, "wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next167 self-test failed\n");
    exit(1);
}
