...
$sectorSize = 512;
$nonce = 0x20260527;
$root = sys_get_temp_dir() . '/port-libsqlite-wordpress-pager-journal-open-' . bin2hex(random_bytes(4));
...
if (!is_dir($directory) && !mkdir($directory, 0777, true) && !is_dir($directory)) {
    throw new RuntimeException('Unable to create WordPress pager journal directory');
}
...
echo json_encode([
    'scenario' => 'wordpress-pager-journal-open-closure',
    'wordpressUse' => 'Open and close an unused rollback-journal transaction for copied wp_options imports through native PHP pager/VFS primitives, while refusing to start a write transaction over a hot rollback journal that must be recovered first.',
    'root' => $root,
