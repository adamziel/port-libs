...
$summary = [
    'scenario' => 'application-pager-master-journal-reader-cache-rollback-journal-source-rebase',
    'status' => $plan['status'],
    ...
    'applicationUse' => 'A copied Application database can reuse a schema reader-cache page when the same attached rollback journals appear in a different master-journal order, while the next active_plugins write still captures the recovered rollback source.',
...
if ($summary['status'] !== 'pager-master-journal-reader-cache-current-source-next174'
    ...
    fwrite(STDERR, "application-pager-master-journal-reader-cache-rollback-journal-source-rebase self-test failed\n");
    exit(1);
}

echo "application-pager-master-journal-reader-cache-rollback-journal-source-rebase self-test passed\n";
