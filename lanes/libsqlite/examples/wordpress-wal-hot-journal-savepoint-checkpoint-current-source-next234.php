...
$walDigest = $digest('wordpress next234 reopened wal');
$admission = [
...
$plan = SQLiteWalHotJournalSavepointCheckpointCurrentSourceNextPlan::next234VerifyDurableHandoff($reopened, [[
    ...
    'schema_cookie' => 1234,
    'wal_digest' => $walDigest,
    'database_digest' => $digest('wordpress checkpointed database'),
    'shm_digest' => $digest('wordpress synced shm'),
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    assert(in_array('wordpress-import-durable-wal-current-source-handoff', $plan['dependencies'], true));
    echo "wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next234 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next234',
    'status' => $plan['status'],
    ...
    'wordpressUse' => 'A copied wp_options import serves the repaired current source only after the checkpointed database, restarted WAL, synced SHM, unlinked hot journal, and containing directory receipts all match the reopened WAL-index source.',
