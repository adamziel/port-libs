...
$basePages = [
    1 => $page('wp next160 clean schema before import crash'),
    2 => $page('wp next160 clean options before checkpoint'),
    3 => $page('wp next160 clean plugin batch before savepoint'),
    4 => $page('wp next160 clean transient before savepoint'),
];
$dirtyDatabaseBytes = $page('wp next160 dirty schema from interrupted rollback')
    . $basePages[2]
    . $page('wp next160 dirty plugin from rolled back savepoint')
    . $page('wp next160 dirty transient from rolled back savepoint');
...
foreach ([
    [1, 0, 'wp next160 committed schema after hot journal'],
    [2, 4, 'wp next160 committed active_plugins option'],
    [3, 0, 'wp next160 plugin draft rolled back'],
    [4, 4, 'wp next160 transient draft discarded'],
...
$savepoints->beginTransaction('wordpress-import-next160');
$savepoints->recordWalFrameWrite(1, 1, false);
$savepoints->recordWalFrameWrite(2, 2, true);
$savepoints->savepoint('plugin_batch_next160');
$savepoints->recordWalFrameWrite(3, 3, false);
$savepoints->recordWalFrameWrite(4, 4, true);

$root = sys_get_temp_dir() . '/port-libsqlite-wp-hot-savepoint-checkpoint-next160-' . bin2hex(random_bytes(4));
...
try {
    $result = (new SQLiteVfsFileWriter($root))->applyHotJournalSavepointCheckpointCurrentSourceNext160(
        $savepoints,
        'plugin_batch_next160',
        $databasePath,
        [1, 2, 3, 4],
        'truncate'
    );
    ...
    echo json_encode([
        ...
        'hasCommittedOption' => str_contains($databaseBytes, 'wp next160 committed active_plugins option'),
        'discardedDraft' => !str_contains($databaseBytes, 'wp next160 plugin draft rolled back'),
