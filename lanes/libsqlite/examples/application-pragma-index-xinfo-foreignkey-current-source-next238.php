...
$summary = [
    'scenario' => 'application-pragma-index-xinfo-foreignkey-current-source-next238',
    'applicationUse' => 'Copied Application import schemas can admit descending UNIQUE parent indexes from PRAGMA index_xinfo(desc=1) as valid foreign-key parent keys instead of requiring a false repair before schema replay.',
...
if (PHP_SAPI === 'cli' && realpath($_SERVER['SCRIPT_FILENAME'] ?? '') === __FILE__) {
    if (
        ...
        fwrite(STDERR, "application-pragma-index-xinfo-foreignkey-current-source-next238 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, json_encode($summary, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) . "\n");
    fwrite(STDOUT, "application-pragma-index-xinfo-foreignkey-current-source-next238 self-test passed\n");
}
