...
$databaseBytes = $page('wordpress sqlite header before failed WAL import') . $baseActivePlugins . $basePluginSettings . $baseAutoload;
...
$savepoints->beginTransaction('wordpress-options-import');
...
$summary = [
    'scenario' => 'wordpress-pager-savepoint-wal-cache-recovery-current-source-next133',
    'status' => $plan['status'],
    ...
    'wordpressUse' => 'A failed plugin option import in WAL mode may leave pager-cache entries sourced from WAL frames that ROLLBACK TO discards. This smoke verifies later reads refresh those cache pages from the retained WAL prefix or base database image before WordPress retries the import.',
