...
$plan = SQLiteBTreeVacuumPointerMapFreeblockCurrentSourcePlan::tableLeafAppendAllocationFromDeleteResult(
    $database,
    3,
    [
        'page' => $deletedPage,
...
$summary = [
    'scenario' => 'application-btree-vacuum-pointermap-freeblock-current-source-next161',
    'applicationUse' => 'Delete an obsolete copied wp_options transient, partially vacuum its overflow tail, then write a larger replacement value that reuses the surviving free page and appends formerly truncated page numbers with fresh auto-vacuum pointer-map ownership.',
...
if (($argv[1] ?? null) === '--self-test') {
    ...
    foreach ($expected as $key => $value) {
        if ($summary[$key] !== $value) {
            fwrite(STDERR, "application-btree-vacuum-pointermap-freeblock-current-source-next161 self-test failed\n");
            exit(1);
        }
    }

    fwrite(STDOUT, "application-btree-vacuum-pointermap-freeblock-current-source-next161 self-test passed\n");
    exit(0);
}
