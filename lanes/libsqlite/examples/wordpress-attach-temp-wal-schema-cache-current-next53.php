...
if (($argv[1] ?? '') === '--self-test') {
    ...
    if (!$ok) {
        fwrite(STDERR, "wordpress-attach-temp-wal-schema-cache-current-next53 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-attach-temp-wal-schema-cache-current-next53 self-test passed\n");
    exit(0);
}
