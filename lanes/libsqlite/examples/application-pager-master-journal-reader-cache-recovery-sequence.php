...
$plan = SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::planRecoverySequenceFence(
    $database,
    $master,
    $masterBytes,
    implode('', $before),
...
$summary = [
    'scenario' => 'application-pager-master-journal-reader-cache-recovery-sequence',
    'status' => $plan['status'],
    ...
    'applicationUse' => 'A copied wp_options database prevents a reader-cache page from a prior recovered-page set from surviving a newer master-journal recovery.',
...
if ($summary['status'] !== 'pager-master-journal-reader-cache-current-source-next186'
    ...
    fwrite(STDERR, "application-pager-master-journal-reader-cache-recovery-sequence self-test failed\n");
    exit(1);
}

echo "application-pager-master-journal-reader-cache-recovery-sequence self-test passed\n";
