...
$summary = [
    'scenario' => 'wordpress-wal-reader-restart-savepoint-checkpoint-current-source-next147',
    'status' => $plan['status'],
    'wordpressUse' => 'A copied WordPress options import rolls back a failed autoload savepoint, runs a restart checkpoint while an existing reader remains pinned to the truncated current WAL source, then appends retry frames to the restarted WAL generation for later readers.',
...
if ($summary['status'] !== 'wal-reader-restart-savepoint-checkpoint-current-source-next147'
    ...
    fwrite(STDERR, "wordpress-wal-reader-restart-savepoint-checkpoint-current-source-next147 self-test failed\n");
    exit(1);
}

echo "wordpress-wal-reader-restart-savepoint-checkpoint-current-source-next147 self-test passed\n";
