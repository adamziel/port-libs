...
$summary = [
    'scenario' => 'application-wal-hot-journal-savepoint-checkpoint-current-source-next209',
    'applicationUse' => 'A copied Application import resumes after hot-journal recovery and checkpoint publication, then admits only writer handles that retain current prepared statements and reopen stale plugin statements before appending new WAL frames.',
    'status' => $plan['status'],
...
if (PHP_SAPI === 'cli' && basename(__FILE__) === basename($_SERVER['SCRIPT_FILENAME'] ?? '')) {
    if (
        ...
        fwrite(STDERR, "application-wal-hot-journal-savepoint-checkpoint-current-source-next209 self-test failed\n");
        exit(1);
    }
