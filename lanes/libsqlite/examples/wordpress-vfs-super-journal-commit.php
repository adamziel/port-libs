...
$pageSize = 512;
$root = sys_get_temp_dir() . '/port-libsqlite-wordpress-super-journal-' . bin2hex(random_bytes(4));
...
echo json_encode([
    'scenario' => 'wordpress-vfs-super-journal-commit',
    'wordpressUse' => 'Apply SQLite super-journal commit ordering for copied WordPress multisite-style attached databases: the master journal lists each rollback journal, all attached journals and database pages are synced, then the super-journal deletion atomically commits the group without ext/sqlite.',
    'root' => $root,
