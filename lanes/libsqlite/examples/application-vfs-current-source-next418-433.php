...
$plan = SQLiteVfsCurrentSourceNextPlan::run([
    'snapshot(reader-ready-next433,shared-cache-next417)',
    'claim(reader-ready-next433,shared-cache-next417,reader-reuse-next433)',
    'publish(reader-ready-next433,reader-reuse-next433,shared-cache-next433)',
], [
    'current' => [
        'current_source' => 'main',
        'sources' => [
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    assert($plan['events'][0]['ack'] === 'shared-cache-next417');
    assert($plan['events'][1]['status'] === 'claimed-reusable-current-source');
    assert($plan['events'][1]['claim'] === 'reader-reuse-next433');
    assert($plan['events'][1]['blocked_reasons'] === []);
    assert($plan['events'][2]['status'] === 'published-reused-current-source');
    assert($plan['events'][2]['reuse_ack'] === 'shared-cache-next417');
    assert($plan['events'][2]['token'] === 'shared-cache-next433');
    assert($plan['events'][2]['published_count'] === 19);
    assert(in_array('vfs-current-source-snapshot-reuse-publish-next418-433', $plan['dependencies'], true));
    assert(in_array('vfs-current-source-snapshot-reuse-publish-next402-417', $plan['dependencies'], true));
    echo "application-vfs-current-source-next418-433 self-test passed\n";
    return;
}
