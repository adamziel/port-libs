...
require_once dirname(__DIR__) . '/src/SQLiteRowValueUpdateDeleteReturningWindowCurrentSourceNext237Plan.php';
...
$rows = [
    ...
    ['option_id' => 8, 'blog_id' => 2, 'option_name' => 'rewrite_rules', 'status' => 'queued', 'bytes' => 9, 'option_value' => 'rules'],
    ['option_id' => 9, 'blog_id' => 3, 'option_name' => 'plugin_batch', 'status' => 'queued', 'bytes' => 11, 'option_value' => 'plugin'],
    ['option_id' => 10, 'blog_id' => 3, 'option_name' => 'theme_mods_twentysixteen', 'status' => 'queued', 'bytes' => 15, 'option_value' => 'mods'],
...
$meta = [
    ...
    ['meta_id' => 4, 'meta_option_id' => 10, 'meta_key' => 'retry_batch', 'meta_value' => 'theme_mods_twentysixteen', 'priority' => 40],
    ['meta_id' => 5, 'meta_option_id' => 11, 'meta_key' => 'retry_batch', 'meta_value' => '_transient_cache', 'priority' => 50],
    ['meta_id' => 6, 'meta_option_id' => 12, 'meta_key' => 'retry_cleanup', 'meta_value' => 'network_plugin', 'priority' => 60],
];

$attempt = "UPDATE wp_options SET (status, option_value, bytes) = ('attempt237', option_value || ':attempt237', bytes + 1) WHERE (option_id, option_name) IN (SELECT meta_option_id, meta_value FROM wp_optionmeta WHERE meta_key = 'retry_batch' ORDER BY priority ASC LIMIT 2) RETURNING option_id, blog_id, option_name, status, option_value, bytes ORDER BY option_id";
$retryUpdate = "UPDATE wp_options SET (status, option_value, bytes) = ('retry237', option_value || ':retry237', bytes + 3) WHERE (option_id, option_name) IN (SELECT meta_option_id, meta_value FROM wp_optionmeta WHERE meta_key = 'retry_batch' ORDER BY priority ASC LIMIT -1) RETURNING option_id, blog_id, option_name, status, option_value, bytes ORDER BY option_id";
$retryDelete = "DELETE FROM wp_options WHERE (option_id, option_name) IN (SELECT meta_option_id, meta_value FROM wp_optionmeta WHERE meta_key = 'retry_cleanup' ORDER BY priority ASC) RETURNING option_id, blog_id, option_name, status, bytes ORDER BY option_id";

$plan = SQLiteRowValueUpdateDeleteReturningWindowCurrentSourceNext237Plan::execute(
...
if (in_array('--self-test', $argv, true)) {
    $ids = array_column($plan['exclude_current_window_rows'], 'option_id');
    ...
    if (($plan['exclude_current_partition_summary']['2']['peer_rowids'] ?? null) !== [[8], [7]]) {
        fwrite(STDERR, "unexpected blog 2 EXCLUDE CURRENT ROW peers\n");
        exit(1);
    }
    ...
    echo "wordpress-rowvalue-returning-window-current-source-next237 self-test passed\n";
    exit(0);
}
...
echo json_encode([
    ...
    'window_rowids' => array_column($plan['exclude_current_window_rows'], 'option_id'),
