...
$summary = [
    'scenario' => 'wordpress-trigger-upsert-returning-view-current-source-next144',
    'wordpressUse' => 'Preview a copied wp_options import routed through an INSTEAD OF view trigger where DO UPDATE WHERE skips suppress RETURNING rows and a held savepoint keeps the next view source out of the visible stream.',
    'status' => $plan['status'],
...
if (in_array('--self-test', $argv, true)) {
    ...
    echo "wordpress-trigger-upsert-returning-view-current-source-next144 self-test passed\n";
    return;
}
