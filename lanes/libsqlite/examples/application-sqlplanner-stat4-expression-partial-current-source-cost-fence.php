...
$plan = SQLitePlannerStat4ExpressionPartialCurrentSourceNextPlan::materializeStat4PartialCostFence(
    $prepared,
    $current,
    [
...
if (in_array('--self-test', $argv, true)) {
    ...
    echo "application-sqlplanner-stat4-expression-partial-current-source-next169 self-test passed\n";
    return;
}

echo json_encode([
    'scenario' => 'application-sqlplanner-stat4-expression-partial-current-source-next169',
    'applicationUse' => 'Preview copied wp_options plugin scans after ANALYZE/source changes where current STAT4 costs keep the partial lower(option_name) expression index ahead of a broader full expression index.',
    'status' => $plan['status'],
