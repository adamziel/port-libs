...
$summary = [
    'scenario' => 'application-pager-master-journal-reader-cache-current-source-next229',
    'applicationUse' => 'A copied Application database keeps schema/options reader-cache pages only when their pager cache source token was minted after master-journal recovery; active_plugins readers from the previous cache source reopen.',
    'status' => $plan['status'],
...
if ($summary['status'] !== 'pager-master-journal-reader-cache-current-source-next229'
    ...
    fwrite(STDERR, "application-pager-master-journal-reader-cache-current-source-next229 self-test failed\n");
    exit(1);
}

echo json_encode($summary, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) . "\n";
echo "application-pager-master-journal-reader-cache-current-source-next229 self-test passed\n";
