...
use PortLibs\LibSqlite\SQLitePragmaIndexXinfoForeignKeyCurrentSourceNext239;
...
$page = SQLitePragmaIndexXinfoForeignKeyCurrentSourceNext239::page(
    $current,
    $next,
...
if (($argv[1] ?? null) === '--self-test') {
    if (
        ...
        fwrite(STDERR, "wordpress-pragma-index-xinfo-foreignkey-current-source-next239 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-index-xinfo-foreignkey-current-source-next239 self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'wordpress-pragma-index-xinfo-foreignkey-current-source-next239',
    'wordpressUse' => 'Copied WordPress term metadata imports can trust PRAGMA index_xinfo key=0 auxiliary rows as storage-only tails, not extra UNIQUE parent-key columns for foreign-key admission.',
