...
$summary = [
    'scenario' => 'application-pager-cache-spill-master-journal-current-source',
    'status' => $plan['status'],
    'spilledPages' => $plan['spilled_page_numbers'],
    'rejectedPages' => $plan['rejected_pages'],
    'cachedMasterStale' => $plan['cached_master_stale'],
    'applicationUse' => 'During copied Application SQLite imports that span attached databases, dirty pager-cache pages may spill only after the current master journal is re-read and each page before-image is proven to match the current master-journal source.',
...
if ($summary['status'] !== 'pager_cache_spill_master_journal_current_source'
    ...
    fwrite(STDERR, "application-pager-cache-spill-master-journal-current-source self-test failed\n");
    exit(1);
}

echo "application-pager-cache-spill-master-journal-current-source self-test passed\n";
