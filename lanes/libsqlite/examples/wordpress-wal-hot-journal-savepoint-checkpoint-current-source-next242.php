...
use PortLibs\LibSqlite\SQLiteWalHotJournalSavepointCheckpointCurrentSourceNext242Plan;

require_once __DIR__ . '/../src/SQLiteWalHotJournalSavepointCheckpointCurrentSourceNext242Plan.php';

$hash = static fn (string $value): string => hash('sha256', $value);
$databaseDigest = $hash('wordpress next242 post checkpoint writer commit');
$admission = [
...
$plan = SQLiteWalHotJournalSavepointCheckpointCurrentSourceNext242Plan::admitCommittedWriter($admission, [
...
$summary = [
    'scenario' => 'wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next242',
    'wordpressUse' => 'After a copied WordPress import restarts WAL from a checkpointed current source, the first writer commit is published only after WAL commit, database backfill, directory sync, and reader-generation receipts agree.',
    'status' => $plan['status'],
...
if (PHP_SAPI === 'cli' && basename(__FILE__) === basename($_SERVER['SCRIPT_FILENAME'] ?? '')) {
    if ($summary['status'] !== 'wal-hot-journal-savepoint-checkpoint-current-source-next242'
        ...
        fwrite(STDERR, "wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next242 self-test failed\n");
        exit(1);
    }

    echo "wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next242 self-test passed\n";
}
