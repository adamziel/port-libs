...
require_once __DIR__ . '/../src/SQLiteUtf16NocaseLikeRtrimCurrentSourceNext176Plan.php';
...
$summary = SQLiteUtf16NocaseLikeRtrimCurrentSourceNext176Plan::wordpressOptionNamePeerYieldPlan(
    [
...
$payload = [
    'scenario' => 'wordpress-utf16-nocase-like-rtrim-current-source-next176',
    ...
    'wordpressUse' => 'Copied wp_options cursors over UTF-16 rtrim(option_name) COLLATE NOCASE LIKE probes preserve SQLite rowid tie ordering when case and trailing-space variants collapse to the same index key.',
    'dependency_closure' => $summary['dependency_closure'],
];

if (PHP_SAPI === 'cli' && basename(__FILE__) === basename($_SERVER['SCRIPT_FILENAME'] ?? '')) {
    if ($payload['status'] !== 'utf16-nocase-like-rtrim-current-source-next176'
        || $payload['nextMatchedRowids'] !== [1, 2, 3, 4, 5]
        || $payload['peerGroupsStraddlingToken']['plugin_cache']['after'] !== [3]
        || $payload['yieldedRowids'] !== [3, 4, 5]
    ) {
        fwrite(STDERR, "wordpress-utf16-nocase-like-rtrim-current-source-next176 self-test failed\n");
        exit(1);
    }
    echo "wordpress-utf16-nocase-like-rtrim-current-source-next176 self-test passed\n";
}
