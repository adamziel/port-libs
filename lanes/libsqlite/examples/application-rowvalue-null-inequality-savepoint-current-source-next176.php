...
$summary = [
    'scenario' => 'application-rowvalue-null-inequality-savepoint-current-source-next176',
    'applicationUse' => 'Copied wp_options cleanup can delete rows that are deterministically not equal to a nullable row-value key while preserving rows whose tuple comparison is still UNKNOWN, then retry from the rollback source after an OR ROLLBACK conflict.',
    'status' => $plan['status'],
...
if (($argv[1] ?? null) === '--self-test') {
    ...
    echo "application-rowvalue-null-inequality-savepoint-current-source-next176 self-test passed\n";
    return;
}
