...
if (($argv[1] ?? null) === '--self-test') {
    if ($page['status'] !== 'ok' || $page['current']['foreign_key_violations'] !== 3 || $page['next_counts']['foreign_key_violations'] !== 0 || $page['delta']['foreign_key_timing_changed'] !== true) {
        fwrite(STDERR, "wordpress-pragma-index-xinfo-foreignkey-current-source-next171 self-test failed\n");
        exit(1);
    }

    fwrite(STDOUT, "wordpress-pragma-index-xinfo-foreignkey-current-source-next171 self-test passed\n");
    exit(0);
}

echo json_encode([
    'scenario' => 'wordpress-pragma-index-xinfo-foreignkey-current-source-next171',
    'wordpressUse' => 'Carry DEFERRABLE and INITIALLY DEFERRED foreign-key timing through copied wp_options PRAGMA index_xinfo/foreign_key_check diagnostics so import tooling can decide whether a violation is statement-immediate or commit-deferred.',
    'status' => $page['status'],
