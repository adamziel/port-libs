...
$source = [
    'name' => 'wp-options-current-next238',
    'schemaCookie' => 2389,
    'stat4Generation' => 438,
    'rows' => $rows,
    'indexes' => [[
...
$prepared = $source;
$prepared['name'] = 'wp-options-prepared-next238';
$prepared['schemaCookie'] = 2380;
$prepared['stat4Generation'] = 238;
$prepared['indexes'][0]['rootPage'] = 23801;
$prepared['indexes'][0]['stat4Samples'] = array_slice($source['indexes'][0]['stat4Samples'], 0, 3);

$plan = SQLitePlannerStat4ExpressionPartialCurrentSourceNextPlan::materializeCurrentSourceCoveringPayloadValidation(
    $prepared,
    $source,
    [
...
if (($argv[1] ?? null) === '--self-test' && $plan['status'] !== 'stat4-expression-partial-current-source-next238-ready') {
    throw new RuntimeException('Expected next238 STAT4 covering payload fence to be ready');
}

printf(
    "application sqlplanner stat4 expression partial current-source next238: %s covered=%d payloads=%d signature=%s\n",
    $plan['status'],
    $plan['stat4CoveringPayloadFence']['coveredRowCount'],
    $plan['stat4CoveringPayloadFence']['payloadRowCount'],
    substr($plan['stat4CoveringPayloadFence']['proofSignature'], 0, 12),
);
