...
$copiedDatabase = SQLiteOpenPlan::forFilename(
    'file:/srv/wordpress/wp-content/database/site%20copy.sqlite?mode=rw&cache=shared&busy_timeout=25',
    true,
    false,
    false,
...
$readOnlyArchive = SQLiteOpenPlan::forFilename(
    'file://localhost/srv/wordpress/archive/site.sqlite?mode=ro&immutable=1&vfs=unix-none',
    true,
    false
);

$newCopy = SQLiteOpenPlan::forFilename(
    'file:/srv/wordpress/wp-content/database/new-copy.sqlite?mode=rwc',
    false,
    true
);

echo json_encode([
    'wordpressUse' => 'Preview native SQLite open admission for copied WordPress databases before a future file-handle/VFS layer touches the filesystem or requires ext/sqlite.',
    'copiedDatabase' => [
