...
if (($argv[1] ?? '') === '--self-test') {
    if (($summary['plugin_row']['plugin_index'] ?? null) !== 1 || ($summary['leaf_row']['atom'] ?? null) !== 'cache') {
        fwrite(STDERR, "wordpress-json-table-rowid-hidden-constraint-current-source-next84 self-test failed\n");
        exit(1);
    }

    echo "wordpress-json-table-rowid-hidden-constraint-current-source-next84 self-test passed\n";
    exit(0);
}
