...
Status: focused PHP behavior growth for LIKE/GLOB prepared cursor reuse across current and next WordPress option-name sources.

Behavior:
- Adds `SQLiteLikeGlobCurrentSourceNextPlan::wordpressOptionNameStatement()` for copied `wp_options.option_name` LIKE/GLOB scans.
...
- `php -l lanes/libsqlite/examples/wordpress-option-name-like-glob-current-source-next88.php`
- `php lanes/libsqlite/examples/wordpress-option-name-like-glob-current-source-next88.php --self-test`
