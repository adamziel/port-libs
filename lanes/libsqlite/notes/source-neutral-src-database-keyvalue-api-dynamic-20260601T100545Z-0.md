...
- `application-composite-indexed-generated-setting-insert-plan.php` now uses `app_settings`, `setting_id`, `key_name`, `key_value`, and `load_policy`.
- `application-index-split-setting-replacement-plan.php` now uses the same generic schema and output labels.
