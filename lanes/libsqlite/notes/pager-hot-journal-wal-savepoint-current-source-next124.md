...
This slice adds `SQLitePagerHotJournalWalSavepointCurrentSourceNext124Plan`. It verifies the parsed rollback journal and WAL match the current source bytes, restores the hot-journal database image, trims WAL recovery to the committed prefix, then performs savepoint WAL byte truncation against that recovered prefix. The current reader keeps retained WAL frames and falls back to hot-recovered database pages for savepoint-discarded frames.
