...
WordPress path: copied `wp_options` import previews can now distinguish a fully yielded recursive current-source RETURNING batch from a stale cursor/watermark before exposing rows from a migrated next view/trigger source.
...
- `php lanes/libsqlite/examples/wordpress-trigger-recursive-view-returning-current-source-next206.php --self-test`
