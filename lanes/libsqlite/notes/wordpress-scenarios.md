...
`examples/wordpress-table-leaf-page-assembly.php` assembles a minimal
two-page SQLite database image containing a `wp_options` table and a `siteurl`
row using only native PHP record, table-leaf cell, and table-leaf page
encoders. The script immediately parses the generated bytes through the native
reader, making it useful for fixture generation and repair preflight workflows
that need to reason about generated SQLite bytes without the PHP SQLite
extension.

## Next Task

Port the next bounded write-side primitive: overflow-page allocation/writing
for encoded table-leaf cells or index leaf cell/page assembly. Keep it paired
with a focused upstream runner and a WordPress fixture that the native reader
can parse back from generated bytes.
