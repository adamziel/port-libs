...
WordPress smoke: `wordpress-trigger-recursive-view-returning-current-source-next192.php` covers a copied `wp_options` recursive import view where next-source rows for `home` and `next_plugin` must drain before a following current import for `blogdescription` and `stylesheet` becomes visible.
...
php -l lanes/libsqlite/examples/wordpress-trigger-recursive-view-returning-current-source-next192.php
No syntax errors detected in lanes/libsqlite/examples/wordpress-trigger-recursive-view-returning-current-source-next192.php
...
php lanes/libsqlite/examples/wordpress-trigger-recursive-view-returning-current-source-next192.php
