...
- Behavior: adds `SQLiteWalHotJournalSavepointCheckpointCurrentSourceNext228Plan`, a post-next224 durability barrier that admits reopened readers only after database sync, WAL reset sync, hot-journal unlink directory sync, SHM lock epoch, savepoint release, and current-source token/generation receipts all match.
