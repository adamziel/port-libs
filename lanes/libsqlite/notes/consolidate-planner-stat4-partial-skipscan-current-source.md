...
Behavior is preserved: the materialization still records per-prefix seek programs, current-source covering payload rows, STAT4 current/next suffix pairs, and current-source fence signatures for Application-style `wp_options` partial indexes on `lower(option_name)`.
...
- `php -l lanes/libsqlite/examples/application-planner-stat4-partial-skipscan-current-source.php`
- `php tools/run-tests.php lanes/libsqlite/tests/SQLitePlannerStat4PartialSkipScanCurrentSourceTest.php`
  ...
  - `65` PASS lines
- `php lanes/libsqlite/examples/application-planner-stat4-partial-skipscan-current-source.php`
