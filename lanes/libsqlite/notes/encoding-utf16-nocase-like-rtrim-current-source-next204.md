...
WordPress path: `wordpress-utf16-nocase-like-rtrim-current-source-next204.php` models copied `wp_options.option_name` prefix scans for localized plugin keys such as `plugin_éclair`. SQLite's default NOCASE LIKE folds ASCII only, so a non-ASCII fixed prefix cannot be admitted to the NOCASE prefix-range cursor. The planner falls back to a decoded full scan, applies the LIKE residual after RTRIM, keeps `é` and `É` distinct, and isolates malformed UTF-16 rows without aborting the valid rowset.
...
- `php tools/run-tests.php lanes/libsqlite/tests/SQLiteUtf16NocaseLikeRtrimCurrentSourceNext204Test.php`
  ...
  - `68` PASS lines
- `php lanes/libsqlite/examples/wordpress-utf16-nocase-like-rtrim-current-source-next204.php --self-test`
  - `wordpress-utf16-nocase-like-rtrim-current-source-next204 self-test passed`
- `php -l lanes/libsqlite/src/SQLiteUtf16NocaseLikeRtrimCurrentSourceNext204Plan.php`
- `php -l lanes/libsqlite/tests/SQLiteUtf16NocaseLikeRtrimCurrentSourceNext204Test.php`
- `php -l lanes/libsqlite/examples/wordpress-utf16-nocase-like-rtrim-current-source-next204.php`
