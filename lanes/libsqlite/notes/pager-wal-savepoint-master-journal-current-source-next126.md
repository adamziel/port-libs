...
This slice adds `SQLitePagerWalSavepointMasterJournalCurrentSourceNext126Plan`. It models the pager edge where a copied WordPress database has stale cached master-journal membership, current VFS master-journal bytes name the database rollback journal, and WAL savepoint replay must reject the cached source before trusting hot-journal recovery, WAL prefix truncation, and the next checkpoint image.
...
php -l lanes/libsqlite/src/SQLitePagerWalSavepointMasterJournalCurrentSourceNext126Plan.php
