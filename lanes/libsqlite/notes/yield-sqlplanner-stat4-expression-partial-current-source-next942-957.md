...
Behavior: extends `SQLitePlannerStat4ExpressionPartialCurrentSourceNextPlan` with `materializeNext942957()`, a direct follow-on to the merged next926-941 preparation fence. The new fence threads the next926-941 handoff signature, rechecks each carried current-source row projection, and prepares slices 942-957 only when the prior projected rows still match the current source.
...
- `application-sqlplanner-stat4-expression-partial-current-source-next942-957.php`
...
- `php -l lanes/libsqlite/examples/application-sqlplanner-stat4-expression-partial-current-source-next942-957.php`
