...
- Behavior: adds `SQLitePagerCacheSpillWalSavepointCurrentSourceNext143Plan` for WAL-mode cache spill admission after `ROLLBACK TO` a savepoint. The plan rolls back to the retained WAL frame prefix, rejects cache pages from discarded WAL tail frames, rejects stale current-source images, requires savepoint before-images, and routes admitted dirty pages to new WAL frames rather than database pages.
