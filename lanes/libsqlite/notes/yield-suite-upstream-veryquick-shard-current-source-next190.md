# Upstream veryquick shard current-source next190
...
`SQLiteUpstreamSuiteEvidence::upstreamVeryquickShardCurrentSourceNext190()`,
...
focused PHP PASS-line output all match the next190 evidence record.

Focused upstream denominator impact: `UPSTREAM_TEST_MANIFEST.json` mapped count
moves from `617` to `618`. The slice records focused current-source veryquick
...
php tools/run-tests.php lanes/libsqlite/tests/SQLiteUpstreamVeryquickShardCurrentSourceNext190Test.php
...
file and exact `82` TestRunner PASS lines. The admitted focused runner-output
delta is exactly `82` PASS lines.
...
conflict work, runner106/jsonvt104 rebase work, accepted batch174 behavior
