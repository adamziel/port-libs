...
- WordPress smoke: `php lanes/libsqlite/examples/wordpress-attach-wal-temp-current-next64.php --self-test` validates copied `wp_options` import behavior where temp shadows main in the current snapshot, WAL DDL removes current temp/main objects, attached archive creates `wp_options`, and reprepare is required.
