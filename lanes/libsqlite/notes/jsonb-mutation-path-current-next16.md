...
- WordPress smoke: `examples/wordpress-jsonb-mutation-current-path.php` applies copied `wp_options.active_plugins` JSONB queue edits with `$.queue[#-1]`, `$.queue[#]`, and nested metadata creation.
...
- Verification:
  ...
  - `php lanes/libsqlite/examples/wordpress-jsonb-mutation-current-path.php` => passed and printed the copied `active_plugins` JSONB queue with pre-last insertion, append insertion, and nested metadata creation.
  - `php -l lanes/libsqlite/src/SQLiteJsonMutation.php && php -l lanes/libsqlite/tests/SQLiteJsonbMutationPathCurrentNext16Test.php && php -l lanes/libsqlite/examples/wordpress-jsonb-mutation-current-path.php` => no syntax errors.
