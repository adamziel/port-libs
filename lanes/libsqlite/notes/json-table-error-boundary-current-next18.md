...
- Application path: copied `wp_options` JSON settings scans can use `json_error_position(option_value) = 0` before `json_each(option_value, '$.phones')`, allowing corrupt option rows to be skipped while valid rows remain queryable.
...
php lanes/libsqlite/examples/application-json-table-error-boundary.php
[
    {
...
php -l lanes/libsqlite/examples/application-json-table-error-boundary.php
