...
- Behavior: `SQLiteWalReaderRestartCheckpointCurrentSourceNext136Plan` proves that a reader pinned to the original WAL byte source keeps resolving pages from that source across two consecutive `RESTART` checkpoints, while fresh path reopens advance through the first and second restarted WAL generations.
- WordPress path: copied `wp_options` imports can explain why a long-lived reader keeps its original `.ht.sqlite-wal` handle even after maintenance replaces the WAL path twice during checkpoint/restart cycles.
...
- Verification:
  ...
  - `php lanes/libsqlite/examples/wordpress-wal-reader-restart-checkpoint-current-source-next136.php --self-test` => self-test passed.
