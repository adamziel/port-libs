...
The affected WAL family also exposed a stale `next175Plan()` generated-key split: production had moved the page-cache seal surface to publish-apply keys while direct tests and the WordPress smoke still consumed the accepted `next175` keys. The canonical implementation now returns both key sets and preserves the legacy `next175` status/metadata aliases.
...
- `php lanes/libsqlite/examples/wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next175.php --self-test`
