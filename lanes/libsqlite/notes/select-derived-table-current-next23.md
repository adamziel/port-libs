...
- Preserves WordPress import staging behavior where copied `wp_options` rows are filtered in a derived table and joined to import metadata without requiring `ext/sqlite`.
...
- Avoids batch21 SELECT window FILTER, nested JSON LEFT JOIN rowid aliases, PRAGMA table-valued rows, recursive trigger savepoint planning, B-tree/WAL/VDBE clusters, expression CASE collation, ATTACH trigger FK resolution, release countability, and WordPress import transaction planning.
