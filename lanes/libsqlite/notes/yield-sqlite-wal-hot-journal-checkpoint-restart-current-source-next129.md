...
- Behavior: after rollback-journal hot recovery, `SQLiteWalHotJournalCheckpointRestartCurrentSourceNext129Plan` distinguishes a pinned current reader that preserves the current WAL bytes from the released `RESTART` checkpoint that writes the checkpointed database image plus a header-only restarted WAL generation.
