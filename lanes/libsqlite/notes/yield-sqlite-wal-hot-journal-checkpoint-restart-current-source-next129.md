...
- Application path: copied `wp_options` import/repair tooling can report why a hot-journal recovery cannot reset `-wal` while a reader is pinned, then verify the next-reader restart generation after the reader releases.
...
- Verification:
  ...
  - `php lanes/libsqlite/examples/application-wal-hot-journal-checkpoint-restart-current-source-next129.php`
