...
- Added `wordpress-wal-savepoint-corpus-next.php` to smoke a failed copied `wp_options` plugin-settings import rollback through native PHP VFS file handles.
...
- This slice keeps the behavior bounded to corpus coverage for pager savepoint WAL rollback semantics and applies the existing native VFS writer path in a WordPress smoke.
