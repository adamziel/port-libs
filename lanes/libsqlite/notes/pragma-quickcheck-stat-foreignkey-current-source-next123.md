...
Adds `SQLitePragmaQuickcheckStatForeignKeyCurrentSourceYield`, a bounded current-source yield for a WordPress import preflight that combines shallow `PRAGMA quick_check`, `sqlite_stat1`/`sqlite_stat4` catalog readiness, and `PRAGMA foreign_key_check` rows behind one stable source cursor.
