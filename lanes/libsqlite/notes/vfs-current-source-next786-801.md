...
This slice extends the consolidated `SQLiteVfsCurrentSourceNext626641Plan` coverage as the direct successor to integrated next770-785. It requires the latest `shared-cache-next785` publish receipt before snapshotting `reader-ready-next801`, records `reader-reuse-next801`, and publishes `shared-cache-next801` only while the handle, path, owner, data version, publish count, receipt digest, and dirty-page state still match the current-source snapshot.
...
- `php -l lanes/libsqlite/examples/wordpress-vfs-current-source-next786-801.php`
