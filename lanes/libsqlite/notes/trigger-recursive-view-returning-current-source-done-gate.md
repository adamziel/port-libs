...
Application path: `application-trigger-recursive-view-returning-current-source-done-gate.php` models a copied `wp_options` import through a recursive view trigger. It proves plugin-created child rows from the current source are fully drained before the next import source can expose `RETURNING` rows.
