...
This slice adds native support for `SELECT DISTINCT` tuple sources inside row-value `IN` subqueries used by UPDATE/DELETE `WHERE` clauses and RETURNING expressions. Duplicate staging rows in copied WordPress option metadata now materialize to one row-value tuple before mutation selection, so savepoint rollback and retry execution do not over-count duplicate source tuples.

WordPress smoke: `wordpress-rowvalue-distinct-subquery-savepoint-current-source-next216.php` covers duplicate `wp_optionmeta` migration rows driving a deduplicated row-value UPDATE plus a deduplicated network DELETE after rollback-to-savepoint.
