...
The WordPress path is copied option import SQL that reads attached multisite
...
- WordPress smoke:
  `php lanes/libsqlite/examples/wordpress-attach-schema-cookie-reprepare-current-source-next100.php | php -r 'json_decode(stream_get_contents(STDIN), true, 512, JSON_THROW_ON_ERROR); echo "json ok\n";'`
  - `json ok`
