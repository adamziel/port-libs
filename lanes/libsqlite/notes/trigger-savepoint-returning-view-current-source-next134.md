...
This slice adds `SQLiteTriggerSavepointReturningViewCurrentSourceNext134Plan`. It models current/next view source tokens for a WordPress `wp_options` import view, yields `RETURNING` rows from the current view trigger before savepoint rollback, restores the pre-savepoint row image after an AFTER trigger raises rollback, suppresses the next-source `RETURNING` stream, and still records attempted next-source rows for reprepare/debug evidence. The release path admits the next view source and its generated `source` column.

WordPress path: `wordpress-trigger-savepoint-returning-view-current-source-next134.php` covers a copied `wp_options` import view where a plugin `INSTEAD OF` trigger exposes `RETURNING` rows but an AFTER guard rolls the savepoint back before the next generated-column view source is visible.
