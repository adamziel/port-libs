...
Application smoke: `application-compound-window-recursive-affinity-current-source-next147.php` models copied `wp_options` dependency-walk import diagnostics where a repair UI pages through current and next recursive compound rows without losing left-most output names or admitting stale cursor state after plugin rows are added.
...
- `php -l lanes/libsqlite/examples/application-compound-window-recursive-affinity-current-source-next147.php`
- `php tools/run-tests.php lanes/libsqlite/tests/SQLiteCompoundWindowRecursiveAffinityCurrentSourceNext147Test.php`
- `php lanes/libsqlite/examples/application-compound-window-recursive-affinity-current-source-next147.php --self-test`
