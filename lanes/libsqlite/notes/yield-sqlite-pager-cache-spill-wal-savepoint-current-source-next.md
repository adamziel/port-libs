...
- Application path: `examples/application-pager-cache-spill-wal-savepoint-current-source-next143.php` models a plugin settings retry where `wp_options` and `active_plugins` pages spill after a savepoint rollback while a discarded autoload-index WAL tail page is kept out of the spill set.
...
- Example evidence: `php lanes/libsqlite/examples/application-pager-cache-spill-wal-savepoint-current-source-next143.php --self-test` passed.
