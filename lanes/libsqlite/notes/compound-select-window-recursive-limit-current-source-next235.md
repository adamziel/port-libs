# compound-select-window-recursive-limit-current-source-next235
...
WordPress path: `wordpress-compound-select-window-recursive-limit-current-source-next235.php`
...
php lanes/libsqlite/examples/wordpress-compound-select-window-recursive-limit-current-source-next235.php
```

Focused result: `1 test files, 468 assertions, 0 failures` with 74 PASS lines.
Example result: JSON self-test emitted status
`compound-select-window-recursive-limit-current-source-next235-ready`.
...
lane-local `SQLiteSelectSql`, accepted next229/next232 compound recursive/window
...
next229 dense-rank UNION/EXCEPT source tokens, next232 page-only current-source
