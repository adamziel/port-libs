...
Behavior: extends `SQLitePlannerStat4ExpressionPartialCurrentSourceNext224Plan` with `materializeNext334349()`, a direct follow-on to the merged next318-333 preparation fence. The new fence threads the prior handoff signature, rechecks each carried current-source row projection, and prepares slices 334-349 only when the prior projected rows still match the current source.
