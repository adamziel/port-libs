...
WordPress path: `wordpress-rowvalue-returning-window-current-source-next257.php` models copied `wp_options` imports where deleted transient rows must be published as current-source tombstones before retry/delete rows from the next source are exposed.
...
- `php -l lanes/libsqlite/examples/wordpress-rowvalue-returning-window-current-source-next257.php`
- `php tools/run-tests.php lanes/libsqlite/tests/SQLiteRowValueUpdateDeleteReturningWindowCurrentSourceNext257Test.php`
- `php lanes/libsqlite/examples/wordpress-rowvalue-returning-window-current-source-next257.php --self-test`
