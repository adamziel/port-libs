...
- WordPress smoke: `examples/wordpress-pager-cache-spill-wal-recovery-current-source-next135.php` models a copied `wp_options` retry import where committed WAL pages become the spill source and an uncommitted transient tail is excluded.
...
- Dependency closure: no new support component needed; this reuses native `SQLiteWal::transactionRecoveryBoundary()` and `SQLitePagerDirtyPageCacheSpillPlan::journalModeCurrentSourceNext107()`.

Verification:

- `php tools/run-tests.php lanes/libsqlite/tests/SQLitePagerCacheSpillWalRecoveryCurrentSourceNext135Test.php`
- `php lanes/libsqlite/examples/wordpress-pager-cache-spill-wal-recovery-current-source-next135.php`
...
- `php -l lanes/libsqlite/examples/wordpress-pager-cache-spill-wal-recovery-current-source-next135.php`
