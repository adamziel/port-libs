...
- `application-sqlplanner-stat4-expression-partial-current-source-next926-941.php` -> `application-sqlplanner-stat4-expression-partial-advanced-prepared-handoff.php`
- `application-sqlplanner-stat4-expression-partial-current-source-next942-957.php` -> `application-sqlplanner-stat4-expression-partial-penultimate-prepared-handoff.php`
- `application-sqlplanner-stat4-expression-partial-current-source-next958-973.php` -> `application-sqlplanner-stat4-expression-partial-terminal-prepared-handoff.php`

The production entrypoints were already canonical descriptive methods:
`materializeAdvancedPreparedHandoff()`, `materializePenultimatePreparedHandoff()`, and `materializeTerminalPreparedHandoff()`. This pass migrates the remaining direct caller filenames and Application scenario names without changing serialized handoff payload fields or asserted behavior data.
...
- All three renamed Application examples passed `--self-test`.
