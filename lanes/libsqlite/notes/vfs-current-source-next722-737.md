...
This slice extends the consolidated `SQLiteVfsCurrentSourceNext626641Plan` coverage as the direct successor to integrated next706-721. It requires the latest `shared-cache-next721` publish receipt before snapshotting `reader-ready-next737`, records `reader-reuse-next737`, and publishes `shared-cache-next737` only while the handle, path, owner, data version, publish count, receipt digest, and dirty-page state still match the current-source snapshot.
