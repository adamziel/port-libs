...
- Covers composite WordPress `wp_options` uniqueness on `(blog_id, option_name)`, `excluded.column` expressions, row-value assignment arity checks, SQL NULL unique-key insertion, and secondary unique failure rollback.

## Evidence

- Focused command: `php tools/run-tests.php lanes/libsqlite/tests/SQLiteRowValueSavepointUpsertCurrentSourceNext131Test.php`
- Example smoke: `php lanes/libsqlite/examples/wordpress-rowvalue-savepoint-upsert-current-source-next131.php`
