# Upstream veryquick shard current-source next225
...
`SQLiteUpstreamSuiteEvidence::upstreamVeryquickShardCurrentSourceNext225()`,
...
focused PHP PASS-line output all match the next225 evidence record.

Focused upstream denominator impact: `UPSTREAM_TEST_MANIFEST.json` mapped count
moves from `625` to `626`. The slice records focused current-source veryquick
...
php -l lanes/libsqlite/tests/SQLiteUpstreamVeryquickShardCurrentSourceNext225Test.php
php tools/run-tests.php lanes/libsqlite/tests/SQLiteUpstreamVeryquickShardCurrentSourceNext225Test.php
...
file and exact `98` TestRunner PASS lines. The admitted focused runner-output
delta is exactly `98` PASS lines.
...
veryquick evidence, exact-shard next148, queued suite217 stale evidence,
runner106/jsonvt104 rebase work, accepted batch107/108 through batch198
