...
- Renamed the direct focused tests and Application examples from numbered `next181` / `next182` / `next183` filenames to descriptive behavior filenames.
...
- `php -l` passed for the changed production class, three focused tests, and three Application examples.
...
- Application examples with `--self-test` passed for union-distinct yield tape, UNION ALL empty recursive arm, and tail window LIMIT boundary.
