# yield-sqlite-wordpress-json-savepoint-schema-current-next52

Status: focused PHP behavior growth for WordPress JSON import batches that carry
`sqlite_schema` DDL changes inside savepoint/WAL current-next planning.
...
- Added `wordpress-json-savepoint-schema-current-next52.php` as a copied
...
```text
php tools/run-tests.php lanes/libsqlite/tests/SQLiteWordPressJsonSavepointSchemaCurrentNext52Test.php
...
- `benchmarkDenominator.mapped`: unchanged; this is a lane-local WordPress
  schema/savepoint current-next behavior surface, not a newly mapped upstream
  Tcl inventory unit.

Non-overlap:

This avoids accepted schema-aware JSON import current-next49 row validation,
