...
- Covers case-sensitive `BINARY` LIKE byte-prefix range scans over malformed Application option names while preserving safe range rejection for default `NOCASE` LIKE when the fixed prefix is not ASCII.
...
- `php lanes/libsqlite/examples/application-option-name-like-glob-malformed-cursor.php`: emits valid JSON with `likeBinaryRowids` `[2,3]`, GLOB parity rowids `[2,3]`, and default NOCASE rejection `nocase_like_prefix_must_be_ascii_for_range`.
...
- `php -l lanes/libsqlite/examples/application-option-name-like-glob-malformed-cursor.php`: no syntax errors.
