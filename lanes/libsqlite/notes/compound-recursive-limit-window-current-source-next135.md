...
after current/next Application option sources are combined.
...
- Adds a Application smoke for import previews where new option dependencies
...
- `php tools/run-tests.php lanes/libsqlite/tests/SQLiteCompoundRecursiveLimitWindowCurrentSourceNext135Test.php`
  ...
  - `53` PASS lines.
- `php lanes/libsqlite/examples/application-compound-recursive-limit-window-current-source-next135.php --self-test`
  - `application-compound-recursive-limit-window-current-source-next135 self-test passed`
...
- No new support component is needed. The slice reuses existing lane-local
  ...
  tail limit execution, window frame evaluation, and Application row-array
  fixtures.
