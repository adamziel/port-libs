...
- Added `SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext218Plan` for a bounded current-source replay fence where a recursive CTE with `LIMIT/OFFSET` feeds a compound `UNION ALL` arm before `INTERSECT`, and a next-source `wp_options` row shifts `row_number()` ranks across the final `LIMIT/OFFSET` page.
...
- WordPress smoke: `examples/wordpress-compound-select-window-recursive-limit-current-source-next218.php`.
