...
- WordPress path: copied `wp_options` rows gain autoloaded plugin/theme options between current and next source snapshots; the derived `INTERSECT` changes the final limited option boundary.
...
- Smoke evidence: `php lanes/libsqlite/examples/wordpress-compound-select-window-recursive-limit-current-source-next175.php` emits the next175 plan JSON.
