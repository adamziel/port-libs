...
- The focused test covers UTF-16LE and UTF-16BE option-name bytes, padded peers, non-breaking space and tab suffixes that must not be RTRIM-trimmed, uppercase case-sensitive GLOB behavior, range upper-bound exclusion, malformed UTF-16 guards, and the WordPress scan helper.
...
- `php -l lanes/libsqlite/examples/wordpress-utf16-rtrim-like-glob-current-source-next90.php`
- `php lanes/libsqlite/examples/wordpress-utf16-rtrim-like-glob-current-source-next90.php --self-test`
...
Dependency closure:
- No new support component is needed. This reuses the existing native UTF-16 decoder, RTRIM/BINARY/NOCASE collation comparison, GLOB matcher, and WordPress option-name scan helper.
