...
- WordPress smoke: copied `wp_options` import through a recursive view trigger freezes current `RETURNING` rows while a plugin migration adds an `import_source` view column in the next schema source.
...
php -l lanes/libsqlite/examples/wordpress-trigger-recursive-view-returning-current-source-next180.php
php tools/run-tests.php lanes/libsqlite/tests/SQLiteTriggerRecursiveViewReturningCurrentSourceNext180Test.php
php lanes/libsqlite/examples/wordpress-trigger-recursive-view-returning-current-source-next180.php --self-test
