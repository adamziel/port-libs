...
- The coverage composes the existing WordPress examples rather than introducing a new execution surface: next245 yield-ticket admission, next246 filtered release receipts, next247 peer-group exclusion, and next248 resumable publication cursors.
- WordPress path: copied `wp_options` imports can verify that retry rows remain behind the current-source yield/window barriers until the prepared next245-248 handoff candidates all report their expected current-source status.
...
- `php -l lanes/libsqlite/examples/wordpress-rowvalue-returning-window-current-source-next245-248-after-current.php`
...
- `php lanes/libsqlite/examples/wordpress-rowvalue-returning-window-current-source-next245-248-after-current.php`
- `git diff --check`
