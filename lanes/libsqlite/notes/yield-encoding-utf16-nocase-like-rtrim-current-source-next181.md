...
- WordPress path: copied `wp_options` scans with plugin option names stored in mixed UTF-8/UTF-16 encodings can resume after yielding `plugin_cache` without duplicating the yielded row or skipping equal RTRIM/NOCASE peers.
...
- Focused verification:
  ...
  - `php lanes/libsqlite/examples/wordpress-utf16-nocase-like-rtrim-current-source-next181.php` -> `wordpress-utf16-nocase-like-rtrim-current-source-next181 self-test passed`.
