...
- WordPress smoke: `examples/wordpress-trigger-recursive-view-returning-current-source-next203.php` exercises copied `wp_options` import rows where current recursive child `RETURNING` payloads are visible before `home`/`next_plugin` next-source rows.
