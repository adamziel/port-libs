...
This slice extends the consolidated `SQLiteVfsCurrentSourceNext626641Plan` coverage as the direct successor to integrated next962-977. It requires the latest `shared-cache-next977` publish receipt before snapshotting `reader-ready-next993`, records `reader-reuse-next993`, and publishes `shared-cache-next993` only while the handle, path, owner, data version, publish count, receipt digest, and dirty-page state still match the current-source snapshot.
