...
- Migrates direct tests and WordPress examples to the canonical class.
- Keeps variant behavior self-contained in the canonical production class. The next250 RTRIM residual variant now uses the clearer entry method `wordpressRtrimLikeResidualSourcePlan()` to avoid colliding with the older next243 `wordpressRtrimLikeResidualPlan()` entry method.
...
- Changed WordPress examples executed with `php` and exited successfully.
