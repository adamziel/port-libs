...
- The WordPress smoke uses copied `wp_options` JSON settings and shows
...
- `php -l lanes/libsqlite/examples/wordpress-json-table-rowid-hidden-constraint-current-source-next99.php`
...
- `php lanes/libsqlite/examples/wordpress-json-table-rowid-hidden-constraint-current-source-next99.php --self-test`
...
- Avoids accepted JSON table cursor/source execution, hidden `json`/`root`
  ...
  hidden rowid source row materialization, and WordPress JSON row projection.
