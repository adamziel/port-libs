# libsqlite suite upstream veryquick shard current-source next390

## Scope

- Removes one focused upstream runner/countability blocker by admitting the
  current-source next390 veryquick shard row.
- Ties the row to launcher Base accepted HEAD
  `164aa6c54dd68feeda944a9b01b5b193eba01b61` and current batch226 source
...
suite rows, and the live B-tree, JSON, VFS, WAL, planner, PRAGMA, ATTACH,
...
```text
$ php tools/run-tests.php lanes/libsqlite/tests/SQLiteUpstreamVeryquickShardCurrentSourceNext390Test.php
