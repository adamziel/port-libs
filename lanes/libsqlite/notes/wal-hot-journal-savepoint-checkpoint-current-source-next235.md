...
WordPress smoke: `wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next235.php` models copied `wp_options` readers staying on the current source only after the checkpoint database image, restarted WAL sidecar, deleted hot journal, and containing directory are durably published.
...
- `php lanes/libsqlite/examples/wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next235.php`
  - `wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next235 self-test passed`
