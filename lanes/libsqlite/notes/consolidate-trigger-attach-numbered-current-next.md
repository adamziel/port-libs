...
ATTACH/WAL/temp, and WordPress import family into stable unsuffixed class names.
The migrated direct tests and WordPress examples continue to exercise the same
...
`rg -n "(Trigger|RowValue|AttachWalTemp|WordPressImport).*Current(Source)?Next[0-9]+|Current(Source)?Next[0-9]+.*(Trigger|RowValue|AttachWalTemp|WordPressImport)|Current(Source)?Next[0-9]+Signal|tagCurrentNext[0-9]+" lanes/libsqlite/src lanes/libsqlite/tests lanes/libsqlite/examples`
...
consolidation that reuses the existing focused PHP tests and WordPress examples.
