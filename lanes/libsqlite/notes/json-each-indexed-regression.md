...
- Consolidation: renamed the dynamic JSON table regression test/note to stable unsuffixed names while preserving the same assertions and stable Application smoke scenario.
...
- Dependency closure: no new support component needed; this reuses the lane-local SELECT parser, dynamic JSON table rows, row-array JOIN executor, JSONB validation, and Application smoke fixtures.
...
No syntax errors detected in lanes/libsqlite/examples/application-select-sql-json-dynamic-join.php
...
prints the stable `application-select-sql-json-dynamic-join` scenario with priority, left-join, and nested-left-join rows.
```
