...
The previous bounded executor keyed GROUPS peers from only the first window `ORDER BY` term. SQLite peer groups use all window `ORDER BY` terms, so copied WordPress option previews such as `ORDER BY bytes, option_name` must split rows that share `bytes` but differ by `option_name`.
...
- `php lanes/libsqlite/examples/wordpress-vdbe-window-frame-groups-current-next29.php`
