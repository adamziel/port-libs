...
This slice adds `SQLiteJsonTablePlan::currentSourceHiddenRowidOrderNext135()`. It composes the accepted hidden-rowid current-source planner with ORDER BY profiling so a prepared `json_tree()` scan can distinguish stable hidden rowid tie-break order from source JSON changes that alter generated order keys, sorted rowids, and reprepare reasons.

WordPress path: `wordpress-json-table-hidden-rowid-order-current-source-next135.php` models copied `wp_options` plugin rule JSON where `ORDER BY atom DESC, rowid` keeps equal priority leaves in hidden rowid order while the next source inserts a new rule with the same priority.

Verification:

```text
php tools/run-tests.php lanes/libsqlite/tests/SQLiteJsonTableHiddenRowidOrderCurrentSourceNext135Test.php
Focused test run: 1 selected test files (root lock skipped)
1 test files, 59 assertions, 0 failures

php lanes/libsqlite/examples/wordpress-json-table-hidden-rowid-order-current-source-next135.php --self-test
wordpress-json-table-hidden-rowid-order-current-source-next135 self-test passed
```
