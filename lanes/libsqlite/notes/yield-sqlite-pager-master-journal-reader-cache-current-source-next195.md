...
- WordPress path: copied `wp_options` plus attached user/meta databases can reuse schema pages, refresh changed active plugin pages, and reject byte-identical alloptions cache rows when the attached rollback journal changed underneath the master journal cleanup.
