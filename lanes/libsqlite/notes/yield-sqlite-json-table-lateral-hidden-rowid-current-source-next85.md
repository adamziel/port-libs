...
- WordPress path: copied `wp_options` plugin settings can join a JSON virtual table through hidden constraints instead of function arguments, preserving left-join null extension for options with empty arrays.
- Verification: run `php tools/run-tests.php lanes/libsqlite/tests/SQLiteJsonTableLateralHiddenRowidCurrentSourceNext85Test.php`, syntax checks for changed PHP files, the WordPress smoke, and `git diff --check -- lanes/libsqlite`.
