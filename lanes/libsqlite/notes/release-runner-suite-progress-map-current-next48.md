...
- Behavior: adds `SQLiteUpstreamSuiteEvidence::releaseRunnerSuiteProgressMapCurrentNext48()` to map current-vs-next suite progress rows by tier, countable status, test totals, advancement, preservation, regression, and open blockers.
- Focused verification: `php tools/run-tests.php lanes/libsqlite/tests/SQLiteReleaseRunnerSuiteProgressMapCurrentNext48Test.php`.
