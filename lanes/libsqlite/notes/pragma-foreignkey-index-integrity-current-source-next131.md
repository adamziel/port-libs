...
- Added `SQLitePragmaForeignKeyIndexIntegrityCurrentSourceNext131`, a current-source cursor wrapper around the existing foreign-key parent-index integrity collector.
...
- Focused verification: `php tools/run-tests.php lanes/libsqlite/tests/SQLitePragmaForeignKeyIndexIntegrityCurrentSourceNext131Test.php` passed with `65` PASS lines, `71` assertions, and `0` failures.
- WordPress smoke: `php lanes/libsqlite/examples/wordpress-pragma-foreignkey-index-integrity-current-source-next131.php` reports the copied `wp_options` parent-key admission blocker and the resumable source id.
