...
- Added `SQLiteJsonTablePlan::currentSourcePathHiddenRowidCostNext126()` for current-source to next-source JSON table planner handoff.
...
- Added focused PHP coverage in `SQLiteJsonTablePathHiddenRowidCostCurrentSourceNext126Test.php`: 56 PASS lines / 56 assertions / 0 failures.
- Added WordPress smoke `wordpress-json-table-path-hidden-rowid-cost-current-source-next126.php` for copied `wp_options` plugin-rule JSON diagnostics without `ext/sqlite`.

## Focused Evidence

```text
$ php tools/run-tests.php lanes/libsqlite/tests/SQLiteJsonTablePathHiddenRowidCostCurrentSourceNext126Test.php
...
```text
$ php lanes/libsqlite/examples/wordpress-json-table-path-hidden-rowid-cost-current-source-next126.php --self-test
wordpress-json-table-path-hidden-rowid-cost-current-source-next126 self-test passed
```
