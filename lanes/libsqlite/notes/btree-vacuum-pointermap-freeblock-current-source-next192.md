...
Application smoke: `application-btree-vacuum-pointermap-freeblock-current-source-next192.php` models deleting an overflow-backed copied `wp_options` transient, vacuuming obsolete overflow tail pages, and admitting the next reader for the committed header, pointer-map, leaf freeblock, and replacement overflow pages.

Verification:

- `php tools/run-tests.php lanes/libsqlite/tests/SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNext192Test.php`
- `php lanes/libsqlite/examples/application-btree-vacuum-pointermap-freeblock-current-source-next192.php`
