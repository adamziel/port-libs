...
Direct tests and Application examples now call the stable descriptive methods. Historical scenario strings and dependency markers are left intact so the existing assertions continue to describe the accepted behavior.
...
- `php lanes/libsqlite/examples/application-rowvalue-rollback-returning-current-source-next146.php --self-test`: pass.
- `php lanes/libsqlite/examples/application-rowvalue-update-delete-returning-savepoint-current-source-next157.php --self-test`: pass.
