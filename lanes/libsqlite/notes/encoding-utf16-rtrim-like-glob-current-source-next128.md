...
Status: focused PHP behavior growth for UTF-16 RTRIM current-source scans when a prepared Application option-name cursor changes from residual `LIKE` to prefix-ranged `GLOB`.
...
Application path: `application-utf16-rtrim-like-glob-current-source-next128.php` models a copied `wp_options.option_name` scan whose prepared predicate switches from escaped `LIKE 'plugin!_cache%' ESCAPE '!'` to `GLOB 'plugin_cache*'` across a current-source transition.
...
- `php -l lanes/libsqlite/examples/application-utf16-rtrim-like-glob-current-source-next128.php`
