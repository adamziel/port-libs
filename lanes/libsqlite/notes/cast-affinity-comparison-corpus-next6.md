...
- WordPress smoke: `php lanes/libsqlite/examples/wordpress-select-sql-cast-affinity.php` reports copied `wp_options` option-value casts for numeric comparison, raw lexical comparison without cast, and text storage-rank comparison without requiring `ext/sqlite`.
