...
- Renames inline VALUES source columns before row production, so copied Application import staging tuples can use semantic names such as `name`, `value`, and `autoload` instead of `column1`, `column2`, and `column3`.
...
```text
php lanes/libsqlite/examples/application-select-sql-values-alias.php
...
This slice avoids accepted VFS sync/apply, process locks, rollback commit, super-journal, WAL checkpoint/savepoint byte truncation, B-tree root collapse/page move/overflow freelist, JSON table cursor/source/hidden/visible constraints, Unicode GLOB, SELECT subquery execution, comma LIMIT, grouped SELECT SQL text, expression ORDER BY, and derived-table materialization. It targets the narrower unhandled SQLite VALUES source alias-list syntax used by inline Application staging SQL.
...
Broaden parser/executor support for VALUES sources in JOIN contexts if needed by later Application import SQL; this patch intentionally keeps join-source behavior out of scope because the current accepted join pipeline has separate constraints.
