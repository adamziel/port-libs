# libsqlite suite upstream veryquick shard current-source next355

## Scope

- Removes one focused upstream runner/countability blocker by admitting the
  current-source next355 veryquick shard row.
...
suite rows, accepted batch109-113 behavior surfaces, and the live B-tree, JSON,
...
- focused test:
  `lanes/libsqlite/tests/SQLiteUpstreamVeryquickShardCurrentSourceNext355Test.php`

## Verification

```text
$ php tools/run-tests.php lanes/libsqlite/tests/SQLiteUpstreamVeryquickShardCurrentSourceNext355Test.php
