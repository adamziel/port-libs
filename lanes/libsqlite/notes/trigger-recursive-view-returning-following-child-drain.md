...
WordPress smoke: `wordpress-trigger-recursive-view-returning-following-child-drain.php` covers copied `wp_options` import rows where `blogdescription` and `template` spawn child option rows through an `INSTEAD OF` recursive view trigger while `stylesheet` does not.
...
- `php -l lanes/libsqlite/examples/wordpress-trigger-recursive-view-returning-following-child-drain.php`
- `php tools/run-tests.php lanes/libsqlite/tests/SQLiteTriggerRecursiveViewReturningFollowingChildDrainTest.php`
- `php lanes/libsqlite/examples/wordpress-trigger-recursive-view-returning-following-child-drain.php`
