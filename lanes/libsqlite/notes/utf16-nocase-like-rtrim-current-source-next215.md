...
Behavior:
- Adds a focused current-source replay plan for `rtrim(option_name) COLLATE NOCASE LIKE ? ESCAPE ?` over copied WordPress `wp_options` rows when decoded UTF-16 text contains embedded NUL bytes.
...
- `php lanes/libsqlite/examples/wordpress-utf16-nocase-like-rtrim-current-source-next215.php`
- Result: `wordpress-utf16-nocase-like-rtrim-current-source-next215 self-test passed`
