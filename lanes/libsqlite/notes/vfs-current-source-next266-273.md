# VFS current-source next266-273

Behavior slice: `vfs-current-source-snapshot-reuse-publish-next266-273`

This prepares the follow-on VFS current-source layer after ready `next258-265`.
The slice keeps the acknowledged reuse-publish receipt from `next258-265`,
...
This does not touch prior `next258-265` files or repeat next206-209
capture/reuse, next214-217 readiness, next218-221 publish fencing, next226-229
lease validation, next234-237 acknowledgement gating, or next258-265 publish
...
- `php -l lanes/libsqlite/examples/application-vfs-current-source-next266-273.php`
...
- `php lanes/libsqlite/examples/application-vfs-current-source-next266-273.php --self-test`
- `git diff --check`
