...
- Cleanup: renamed the internal numbered production helper suffix family to stable `LeadNthValueIntersectLimit` helpers and migrated direct diagnostic strings, test labels, the focused test filename, and the Application smoke filename away from the worker-numbered suffix.
...
- Verification:
  ...
  - `php -l lanes/libsqlite/examples/application-compound-lead-nth-value-intersect-limit.php` passed.
  ...
  - `php lanes/libsqlite/examples/application-compound-lead-nth-value-intersect-limit.php --self-test` passed.
