...
- WordPress path: copied `wp_options` imports can preview which trigger-updated option rows yielded diagnostics, which plugin-owned rows were ignored, and why a later trigger rollback restored the import savepoint.
...
- Verification:
  ...
  - `php lanes/libsqlite/examples/wordpress-trigger-returning-savepoint.php --self-test` -> `wordpress-trigger-returning-savepoint self-test passed`.
  ...
  - `php -l lanes/libsqlite/examples/wordpress-trigger-returning-savepoint.php` -> no syntax errors.
