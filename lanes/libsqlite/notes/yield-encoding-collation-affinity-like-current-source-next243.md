...
- Adds `SQLiteEncodingCollationAffinityLikeCurrentSourceNext243Plan` for WordPress `wp_options.option_value` scans where `LIKE` is evaluated against text-affinity values while an `RTRIM` collation key is available for candidate diagnostics.
...
- Adds `wordpress-encoding-rtrim-like-current-source-next243.php` as a WordPress smoke for current/next option rows with trailing spaces, ASCII `NOCASE`, text affinity, blob/NULL unknown rows, and cursor invalidation reasons.
...
- `php -l lanes/libsqlite/examples/wordpress-encoding-rtrim-like-current-source-next243.php` -> no syntax errors.
...
- `php lanes/libsqlite/examples/wordpress-encoding-rtrim-like-current-source-next243.php --self-test` -> `wordpress-encoding-rtrim-like-current-source-next243 self-test passed`.
