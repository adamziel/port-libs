...
This slice extends the consolidated `SQLiteVfsCurrentSourceNext626641Plan` coverage as the direct successor to integrated next802-817. It requires the latest `shared-cache-next817` publish receipt before snapshotting `reader-ready-next833`, records `reader-reuse-next833`, and publishes `shared-cache-next833` only while the handle, path, owner, data version, publish count, receipt digest, and dirty-page state still match the current-source snapshot.
