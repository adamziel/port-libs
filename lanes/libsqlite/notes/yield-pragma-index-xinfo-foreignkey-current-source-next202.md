...
- Behavior: preserves source identity separately for statement-form `PRAGMA foreign_key_list(...)` and table-valued `pragma_foreign_key_list(...)`, while keeping the same catalog row deltas for copied WordPress taxonomy schema reparses.
- Verification: `php tools/run-tests.php lanes/libsqlite/tests/SQLitePragmaIndexXinfoForeignKeyCurrentSourceNext202Test.php` passed with `1 test files, 85 assertions, 0 failures` and 71 PASS lines.
- Example target: `php lanes/libsqlite/examples/wordpress-pragma-index-xinfo-foreignkey-current-source-next202.php --self-test`.
