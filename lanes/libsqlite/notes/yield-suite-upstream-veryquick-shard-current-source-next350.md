# libsqlite suite upstream veryquick shard current-source next350

## Scope

- Removes one focused upstream runner/countability blocker by admitting the
  current-source next350 veryquick shard row.
...
```text
$ php tools/run-tests.php lanes/libsqlite/tests/SQLiteUpstreamVeryquickShardCurrentSourceNext350Test.php
Focused test run: 1 selected test files (root lock skipped)
...
1 test files, 1397 assertions, 0 failures
```
