...
WordPress smoke: `wordpress-rowvalue-returning-window-current-source-next260.php` models copied `wp_options` yield, rollback, retry, and DELETE RETURNING rows where the mixed frame boundary is owned by option IDs `3` and `9` before next-source rows can be exposed.
...
- `php -l lanes/libsqlite/examples/wordpress-rowvalue-returning-window-current-source-next260.php`
...
- `php lanes/libsqlite/examples/wordpress-rowvalue-returning-window-current-source-next260.php`
  - Result: `wordpress-rowvalue-returning-window-current-source-next260 self-test passed`
