...
- Added `SQLitePragmaIndexIntegrityCursorCurrentSourceNext133` for a table-level current-source cursor over `PRAGMA index_list(table)`, each listed index's `PRAGMA index_xinfo(index)`, and sqlite_schema rootpage integrity rows.
...
```text
php tools/run-tests.php lanes/libsqlite/tests/SQLitePragmaIndexIntegrityCursorCurrentSourceNext133Test.php
Focused test run: 1 selected test files (root lock skipped)
1 test files, 74 assertions, 0 failures
```

```text
php lanes/libsqlite/examples/wordpress-pragma-index-integrity-cursor-current-source-next133.php --self-test
wordpress-pragma-index-integrity-cursor-current-source-next133 self-test passed
```
...
- No new support component is needed. The slice reuses existing native PHP schema catalog cursors, `SQLitePragmaIntegrityIndexRootpageCurrentSourceNext124`, pointer-map/rootpage integrity analysis, and test fixture page assemblers.
