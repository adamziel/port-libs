# Upstream veryquick shard current-source next297
...
`SQLiteUpstreamSuiteEvidence::upstreamVeryquickShardCurrentSourceNext297()`,
which admits one lane-local zero-error guarded veryquick shard row only when
the launcher Base accepted HEAD, accepted batch221 source provenance, concrete
`.test` selections, duplicate-runner gate, removed-blocker classification, and
focused PHP PASS-line output all match the next297 evidence record.
...
php -l lanes/libsqlite/tests/SQLiteUpstreamVeryquickShardCurrentSourceNext297Test.php
php tools/run-tests.php lanes/libsqlite/tests/SQLiteUpstreamVeryquickShardCurrentSourceNext297Test.php
...
batch221 source provenance checks, active-runner gating, and focused
