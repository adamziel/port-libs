...
Behavior: extends `SQLitePlannerStat4ExpressionPartialCurrentSourceNext224Plan` with `materializeNext542557()`, a direct follow-on to the merged next526-541 preparation fence. The new fence threads the next526-541 handoff signature, rechecks each carried current-source row projection, and prepares slices 542-557 only when the prior projected rows still match the current source.
