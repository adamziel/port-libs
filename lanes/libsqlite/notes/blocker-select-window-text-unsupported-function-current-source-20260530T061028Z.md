...
- Change: preserved the unsupported-function guard with `definitely_missing_window(...) OVER (...)` and added focused aggregate window assertions for `sum`, `count(*)`, `avg`, and `max` over Application-style `wp_options` rows.
