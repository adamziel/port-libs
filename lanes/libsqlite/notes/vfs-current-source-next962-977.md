...
This slice extends the consolidated `SQLiteVfsCurrentSourceNext626641Plan` coverage as the direct successor to integrated next946-961. It requires the latest `shared-cache-next961` publish receipt before snapshotting `reader-ready-next977`, records `reader-reuse-next977`, and publishes `shared-cache-next977` only while the handle, path, owner, data version, publish count, receipt digest, and dirty-page state still match the current-source snapshot.
