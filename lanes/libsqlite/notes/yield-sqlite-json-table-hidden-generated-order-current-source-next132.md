...
Behavior slice: adds `SQLiteJsonTablePlan::currentSourceHiddenGeneratedOrderNext132()` for the current-source planner boundary where a `json_tree()` cursor is ordered by hidden `json`/`root` terms and by generated ORDER BY keys extracted from each JSON table row.
...
```sh
php tools/run-tests.php lanes/libsqlite/tests/SQLiteJsonTableHiddenGeneratedOrderCurrentSourceNext132Test.php
...
```sh
php lanes/libsqlite/examples/wordpress-json-table-hidden-generated-order-current-source-next132.php
