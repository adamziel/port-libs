# WordPress bulk import savepoint current-next28

Status: focused PHP corpus growth for copied `wp_options` bulk import batches
inside named savepoints.

This slice adds `SQLiteWordPressBulkImportSavepointPlan`, a bounded row-array
planner for WordPress option imports that release successful savepoint batches
...
```text
php tools/run-tests.php lanes/libsqlite/tests/SQLiteWordPressBulkImportSavepointCurrentNext28Test.php
...
```text
php lanes/libsqlite/examples/wordpress-bulk-import-savepoint-current-next28.php
```
...
native PHP row planning for copied WordPress option import batches.
