# Upstream veryquick shard current-source next295
...
`SQLiteUpstreamSuiteEvidence::upstreamVeryquickShardCurrentSourceNext295()`,
which admits one lane-local zero-error guarded veryquick shard row only when
the launcher Base accepted HEAD, accepted current integration source provenance,
concrete `.test` selections, duplicate-runner gate, removed-blocker
classification, and focused PHP PASS-line output all match the next295 evidence
record.
...
php -l lanes/libsqlite/tests/SQLiteUpstreamVeryquickShardCurrentSourceNext295Test.php
php tools/run-tests.php lanes/libsqlite/tests/SQLiteUpstreamVeryquickShardCurrentSourceNext295Test.php
...
276/277/278/279 veryquick evidence, exact-shard next148, queued
runner106/jsonvt104 rebase work, accepted batch221 behavior surfaces, and live
