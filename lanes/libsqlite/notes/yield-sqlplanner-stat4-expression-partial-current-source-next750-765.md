...
Behavior: extends `SQLitePlannerStat4ExpressionPartialCurrentSourceNext224Plan` with `materializeNext750765()`, a direct follow-on to the merged next734-749 preparation fence. The new fence threads the next734-749 handoff signature, rechecks each carried current-source row projection, and prepares slices 750-765 only when the prior projected rows still match the current source.
