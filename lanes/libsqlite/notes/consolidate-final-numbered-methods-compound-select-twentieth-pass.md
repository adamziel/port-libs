...
- Migrated direct focused tests and Application examples to the descriptive unsuffixed entrypoints.
- Added the missing direct `SQLiteAffinityComparison` include to the recursive-yield Application example so it runs outside the test harness.
...
- `php lanes/libsqlite/examples/application-compound-select-except-window-limit-current-source-next148.php --self-test`
  - Result: self-test passed.
