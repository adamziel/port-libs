...
- Adds a current-source compound SELECT slice for recursive CTE rows unioned with WordPress `wp_options` rows where `rank()` and `dense_rank()` are materialized before a distinct compound arm and a comma-form tail `LIMIT offset,count`.
