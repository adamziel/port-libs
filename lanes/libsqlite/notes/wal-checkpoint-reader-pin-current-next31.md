...
- Adds `wordpress-wal-checkpoint-reader-pin-current-next31.php` for copied `wp_options` WAL diagnostics without requiring ext/sqlite.
...
- `php -l lanes/libsqlite/examples/wordpress-wal-checkpoint-reader-pin-current-next31.php` -> no syntax errors.
...
- `php lanes/libsqlite/examples/wordpress-wal-checkpoint-reader-pin-current-next31.php` -> prints restart checkpoint reason, preserved WAL action, pinned current frame `2`, next reader frame `4`, stable current reader, and latest next-reader visibility.
