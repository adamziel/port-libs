...
- Behavior: adds `SQLiteWalCheckpointReaderHotJournalCurrentSourceNext132Plan`, which validates that a reader-held WAL source still matches the current WAL source after hot rollback-journal recovery before allowing a restart checkpoint reset. If the reader source is stale, the plan restores the hot-journal database image but preserves the current WAL and requires reader reopen before checkpoint reset.
...
- Focused evidence:
  - `php -l lanes/libsqlite/src/SQLiteWalCheckpointReaderHotJournalCurrentSourceNext132Plan.php`
