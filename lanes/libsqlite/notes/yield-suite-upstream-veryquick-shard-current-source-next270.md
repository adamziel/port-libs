# Upstream veryquick shard current-source next270
...
`SQLiteUpstreamSuiteEvidence::upstreamVeryquickShardCurrentSourceNext270()`,
which admits one lane-local zero-error guarded veryquick shard row only when
the launcher Base accepted HEAD, accepted batch217 source provenance, concrete
`.test` selections, duplicate-runner gate, removed-blocker classification, and
focused PHP PASS-line output all match the next270 evidence record.
...
php -l lanes/libsqlite/tests/SQLiteUpstreamVeryquickShardCurrentSourceNext270Test.php
php tools/run-tests.php lanes/libsqlite/tests/SQLiteUpstreamVeryquickShardCurrentSourceNext270Test.php
...
batch217 source provenance checks, active-runner gating, and focused
...
rebase work, accepted batch217 behavior surfaces, and live B-tree/JSON/VFS/WAL/
