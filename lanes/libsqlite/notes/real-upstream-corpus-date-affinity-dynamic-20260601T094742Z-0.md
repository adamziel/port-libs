# real-upstream-corpus-date-affinity-dynamic-20260601T094742Z-0

Added `SQLiteRealUpstreamCorpusDateAffinityDynamicAtof1DecimalRealSuffix5000To5999Test.php` as an additive real upstream date/affinity corpus batch.
...
- Owned range: decimal REAL suffixes `5000..5999` from the upstream recursive `format('18.44674407370955%04d',i+1)` matrix.
...
- Expected `phpPass` movement if accepted alone: `5792118 -> 5793121`
- Mapped denominator movement: none; libsqlite already records `1589 / 1589`.

## Non-Overlap

- Fills the current accepted gap between `atof1.test` `atof-3.2` decimal REAL/mantissa suffixes `0000..4999` and `6000..9999`.
...
- No current handoff candidate owns `atof-3.2` suffixes `5000..5999`; adjacent accepted comments that described this window as pending are historical on the current accepted base.

## Verification

- `php -l lanes/libsqlite/tests/SQLiteRealUpstreamCorpusDateAffinityDynamicAtof1DecimalRealSuffix5000To5999Test.php`
  - `No syntax errors detected`
- `php tools/run-tests.php lanes/libsqlite/tests/SQLiteRealUpstreamCorpusDateAffinityDynamicAtof1DecimalRealSuffix5000To5999Test.php`
