...
Behavior: extends `SQLitePlannerStat4ExpressionPartialCurrentSourceNext224Plan` with `materializeNext926941()`, a direct follow-on to the merged next910-925 preparation fence. The new fence threads the next910-925 handoff signature, rechecks each carried current-source row projection, and prepares slices 926-941 only when the prior projected rows still match the current source.
...
- `wordpress-sqlplanner-stat4-expression-partial-current-source-next926-941.php`
