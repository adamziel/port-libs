...
- WordPress path: copied `wp_options` imports that retry plugin option rows inside a savepoint can explain which recursive trigger RETURNING rows were yielded, which were discarded by rollback, and which rows are visible to the next import source.
...
php -l lanes/libsqlite/examples/wordpress-trigger-recursive-returning-savepoint-current-source-next147.php
No syntax errors detected in lanes/libsqlite/examples/wordpress-trigger-recursive-returning-savepoint-current-source-next147.php
...
php lanes/libsqlite/examples/wordpress-trigger-recursive-returning-savepoint-current-source-next147.php --self-test
wordpress-trigger-recursive-returning-savepoint-current-source-next147 self-test passed
```
