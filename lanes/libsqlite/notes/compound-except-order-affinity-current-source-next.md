...
This slice preserves `SQLiteCompoundExceptOrderAffinityCurrentSourceNextPlan` behavior while removing the generated numbered public and private method names. The direct focused test and WordPress copied `wp_options` smoke were renamed to stable unsuffixed filenames and now call the canonical `compare()` entrypoint.
...
- `php lanes/libsqlite/examples/wordpress-compound-except-order-affinity-current-source-next.php`
