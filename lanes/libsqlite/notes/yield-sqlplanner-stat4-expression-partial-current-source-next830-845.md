...
Behavior: extends `SQLitePlannerStat4ExpressionPartialCurrentSourceNext224Plan` with `materializeNext830845()`, a direct follow-on to the merged next814-829 preparation fence. The new fence threads the next814-829 handoff signature, rechecks each carried current-source row projection, and prepares slices 830-845 only when the prior projected rows still match the current source.
