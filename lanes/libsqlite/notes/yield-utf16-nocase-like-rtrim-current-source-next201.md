...
- Behavior: adds `SQLiteUtf16NocaseLikeRtrimCurrentSourceNext201Plan` for UTF-16 `wp_options` scans using `rtrim(option_name) COLLATE NOCASE LIKE ? ESCAPE ?` when a prepared LIKE pattern is rebound to SQL `NULL`.
...
- WordPress smoke: `examples/wordpress-utf16-nocase-like-rtrim-current-source-next201.php`.
...
- Verification:
  ...
  - `php lanes/libsqlite/examples/wordpress-utf16-nocase-like-rtrim-current-source-next201.php --self-test`: self-test passed.
