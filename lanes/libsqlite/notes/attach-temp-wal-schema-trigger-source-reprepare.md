...
- Application path: copied `wp_options` import triggers are checked across main, temp staging, and attached multisite catalogs, including WAL page-one schema-cookie sources and active trigger reset routing.
...
- Example smoke: `php lanes/libsqlite/examples/application-attach-temp-wal-schema-trigger-source-reprepare.php` emits the current/next reprepare plan for temp-shadowed and schema-qualified Application option triggers.
