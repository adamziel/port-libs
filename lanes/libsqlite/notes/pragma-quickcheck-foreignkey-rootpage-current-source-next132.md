# pragma-quickcheck-foreignkey-rootpage-current-source-next132

Adds `SQLitePragmaQuickcheckForeignKeyRootpageCurrentSourceNext132`, a bounded
...
```sh
php tools/run-tests.php lanes/libsqlite/tests/SQLitePragmaQuickcheckForeignKeyRootpageCurrentSourceNext132Test.php
```

Result: `1 test files, 80 assertions, 0 failures` with 70 focused PASS lines.

```sh
php lanes/libsqlite/examples/wordpress-pragma-quickcheck-foreignkey-rootpage-current-source-next132.php --self-test
```

Result: `wordpress-pragma-quickcheck-foreignkey-rootpage-current-source-next132 self-test passed`.
