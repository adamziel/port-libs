# WordPress JSON Import WAL Savepoint Current Next35

## Scope

- Adds `SQLiteWordPressJsonImportWalSavepointPlan`, a bounded native PHP planner for WordPress `wp_options` JSON import payloads staged through SQLite savepoints while tracking the current WAL frame.
...
- Adds a WordPress smoke example that imports plugin settings from JSON, rolls back a malformed plugin payload, and reports the retained WAL current frame.
...
```text
php tools/run-tests.php lanes/libsqlite/tests/SQLiteWordPressJsonImportWalSavepointCurrentNext35Test.php
