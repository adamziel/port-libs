...
- Application smoke covers deleting an overflow-backed copied `wp_options` transient and verifying that the next writer cursor sees pointer-map pages `2`/`105`, payload pages `3`/`106`/`107`/`108`, and no truncated tail pages.
...
- `php -l lanes/libsqlite/examples/application-btree-vacuum-pointermap-freeblock-current-source-next203.php`
...
- `php lanes/libsqlite/examples/application-btree-vacuum-pointermap-freeblock-current-source-next203.php`
  - `application-btree-vacuum-pointermap-freeblock-current-source-next203 self-test passed`
