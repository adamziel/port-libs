# Upstream Veryquick Shard Prepared Window Alpha

Date: 2026-05-29

This isolated upstream-suite preparation slice does not launch a broad SQLite
`testfixture`, `make test`, `mptest`, `all`, or `release` run. It now uses
`SQLiteUpstreamSuiteEvidence::upstreamVeryquickShardPreparedWindowAlpha()`,
which prepares the alpha window suite evidence only when every shard has
lane-local note provenance, guarded veryquick runner metadata, concrete `.test`
selections, current launcher/source heads, zero-error artifact status,
duplicate-runner clearance, and focused PHP PASS-line output.
...
parity and aggregate alpha-window countability unclaimed. Individual zero-error
...
php tools/run-tests.php lanes/libsqlite/tests/SQLiteUpstreamVeryquickShardPreparedWindowAlphaTest.php
git diff --check
```
...
Non-overlap: this avoids the earlier prepared-window suite evidence,
individual alpha-shard countability, exact-shard baseline evidence,
