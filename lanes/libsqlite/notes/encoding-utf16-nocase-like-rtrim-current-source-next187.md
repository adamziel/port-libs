...
- `wordpress-utf16-nocase-like-rtrim-current-source-next187.php` models copied `wp_options.option_name` UTF-16 rows where a plugin option-name scan is prepared with a dangling escape during a source handoff. The plan forces residual recheck instead of reusing stale prefix candidates as matches.
...
- `php tools/run-tests.php lanes/libsqlite/tests/SQLiteUtf16NocaseLikeRtrimCurrentSourceNext187Test.php`
  ...
  - `54` PASS lines
- `php lanes/libsqlite/examples/wordpress-utf16-nocase-like-rtrim-current-source-next187.php --self-test`
  - `wordpress-utf16-nocase-like-rtrim-current-source-next187 self-test passed`
