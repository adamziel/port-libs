...
WordPress smoke: `wordpress-btree-vacuum-pointermap-freeblock-current-source-next175.php` models a copied `wp_options` database deleting an overflow-backed transient, partially vacuuming the tail, and rewriting replacement overflow pages before admitting the current source for import continuation.
...
- `php lanes/libsqlite/examples/wordpress-btree-vacuum-pointermap-freeblock-current-source-next175.php --self-test`
- Result: `wordpress-btree-vacuum-pointermap-freeblock-current-source-next175 self-test passed`
