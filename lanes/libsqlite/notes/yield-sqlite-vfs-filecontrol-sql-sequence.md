...
- Added size-limit enforcement for `size_hint`, so Application import preallocation is ignored when it would exceed the active file cap.
- Added `SQLiteVfsSqlFileControlSequenceTest.php` with 55 focused PASS cases and 184 assertions.
- Added the Application smoke `application-vfs-filecontrol-sql-sequence.php` for copied `wp_options` import handles applying busy timeout, reserved bytes, mmap, size hint, and data-version controls without ext/sqlite.
...
php -l lanes/libsqlite/examples/application-vfs-filecontrol-sql-sequence.php
No syntax errors detected in lanes/libsqlite/examples/application-vfs-filecontrol-sql-sequence.php
...
php lanes/libsqlite/examples/application-vfs-filecontrol-sql-sequence.php
