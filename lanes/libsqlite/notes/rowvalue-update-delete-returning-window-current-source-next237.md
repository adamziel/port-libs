...
The WordPress smoke models copied `wp_options` import rows where a rolled-back attempt mutates queued options and transient cleanup rows, then the retry returns paired multisite option rows. The window receipt verifies each blog partition sees peer rowids, peer names, and peer byte totals with the current row excluded.
...
- `php lanes/libsqlite/examples/wordpress-rowvalue-returning-window-current-source-next237.php --self-test`
- Result: `wordpress-rowvalue-returning-window-current-source-next237 self-test passed`
