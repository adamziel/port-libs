...
WordPress path: copied `wp_options` imports through an `INSTEAD OF` recursive view trigger can acknowledge current-source UPSERT keys before exposing next-source rows from a changed view/trigger definition. The smoke is `wordpress-trigger-recursive-view-upsert-current-source-next240.php`.
...
- `php -l lanes/libsqlite/examples/wordpress-trigger-recursive-view-upsert-current-source-next240.php`
...
- `php lanes/libsqlite/examples/wordpress-trigger-recursive-view-upsert-current-source-next240.php`
  - `wordpress-trigger-recursive-view-upsert-current-source-next240 self-test passed`
