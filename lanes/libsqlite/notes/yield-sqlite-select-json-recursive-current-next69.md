...
- Covers a WordPress-style `wp_options` recursive navigation graph with text JSON and JSONB option payloads, `json_each()` edge traversal, `json_tree()` rule materialization, duplicate UNION cycle evidence, and derived SELECT filtering/grouping over the materialized source.
...
- `php lanes/libsqlite/examples/wordpress-select-json-recursive-current-next69.php`
