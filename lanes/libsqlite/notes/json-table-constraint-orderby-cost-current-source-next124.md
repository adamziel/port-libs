...
- Added `SQLiteJsonTablePlan::currentSourceConstraintOrderByCostNext124()` for current-source to next-source JSON table planner handoff.
...
- Added focused PHP coverage in `SQLiteJsonTableConstraintOrderByCostCurrentSourceNext124Test.php`: 61 PASS lines / 61 assertions / 0 failures.
- Added WordPress smoke `wordpress-json-table-constraint-orderby-cost-current-source-next124.php` for copied `wp_options` plugin-rule priority JSON diagnostics without `ext/sqlite`.

## Focused Evidence

```text
$ php tools/run-tests.php lanes/libsqlite/tests/SQLiteJsonTableConstraintOrderByCostCurrentSourceNext124Test.php
...
```text
$ php lanes/libsqlite/examples/wordpress-json-table-constraint-orderby-cost-current-source-next124.php --self-test
wordpress-json-table-constraint-orderby-cost-current-source-next124 self-test passed
```
