...
- Added `wordpress-dml-trigger-recursion-corpus.php` for copied `wp_options` import/audit rows where recursive plugin option rows are generated without requiring `ext/sqlite`.
...
- `php -l lanes/libsqlite/examples/wordpress-dml-trigger-recursion-corpus.php`
...
Extend parser/VDBE DML execution after this bounded recursion corpus is integrated, preferably with statement-text trigger body parsing or UPDATE/DELETE trigger recursion over copied WordPress import rows.
