...
This slice adds `SQLiteUtf16CollationAffinityPatternCurrentSourceNextPlan::optionRowValuePlan()`. It decodes UTF-8/UTF-16LE/UTF-16BE pattern and escape bytes, reuses existing option-value affinity matching, and adds BINARY/NOCASE/RTRIM range admission metadata plus current/next UTF-16 range-byte provenance for cursor invalidation.

Application path: `application-utf16-collation-affinity-pattern-current-source-next118.php` models copied `wp_options.option_value` scans for autoload prefixes, literal percent escapes, and numeric option values during a current-source to next-source rebuild.
...
- `php -l lanes/libsqlite/examples/application-utf16-collation-affinity-pattern-current-source-next118.php`
- `php tools/run-tests.php lanes/libsqlite/tests/SQLiteUtf16CollationAffinityPatternCurrentSourceNext118Test.php`
- `php lanes/libsqlite/examples/application-utf16-collation-affinity-pattern-current-source-next118.php --self-test`
