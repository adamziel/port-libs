...
- Behavior: after `ROLLBACK TO` truncates WAL frames for a failed WordPress import savepoint, a restart checkpoint with an active reader preserves that reader on the truncated current WAL source while later writers append retry frames to the restarted WAL header generation.
...
- WordPress smoke: `php lanes/libsqlite/examples/wordpress-wal-reader-restart-savepoint-checkpoint-current-source-next147.php`.
