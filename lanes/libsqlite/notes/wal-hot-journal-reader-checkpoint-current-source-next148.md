...
This slice adds `SQLiteWalHotJournalReaderCheckpointCurrentSourceNext148Plan`, a bounded WAL/pager planner for the checkpoint image produced after rollback-journal hot recovery. Earlier current-source slices validate reader WAL/database source tokens and restarted reader generations. This one validates that the checkpoint database bytes themselves match the hot-recovered database plus current WAL frames before a reset is allowed for a current reader.

Verification:

```sh
php -l lanes/libsqlite/src/SQLiteWalHotJournalReaderCheckpointCurrentSourceNext148Plan.php
php -l lanes/libsqlite/tests/SQLiteWalHotJournalReaderCheckpointCurrentSourceNext148Test.php
php -l lanes/libsqlite/examples/wordpress-wal-hot-journal-reader-checkpoint-current-source-next148.php
php tools/run-tests.php lanes/libsqlite/tests/SQLiteWalHotJournalReaderCheckpointCurrentSourceNext148Test.php
php lanes/libsqlite/examples/wordpress-wal-hot-journal-reader-checkpoint-current-source-next148.php --self-test
git diff --check -- lanes/libsqlite
```

Focused result: `1 test files, 79 assertions, 0 failures` with 79 PASS lines.

Expected dashboard movement: `phpPass` increases by the focused PASS-line count from `SQLiteWalHotJournalReaderCheckpointCurrentSourceNext148Test.php`. Mapped upstream coverage remains conservative because this is current-source WAL/pager behavior over existing hot-journal/checkpoint inventory.
