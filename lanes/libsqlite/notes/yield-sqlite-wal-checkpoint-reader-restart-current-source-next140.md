...
- Application smoke: `examples/application-wal-checkpoint-reader-restart-current-source-next140.php` models copied `wp_options` / cron / transient / rewrite-session pages so the current Application reader can keep its original source while a fresh path reader sees the restarted WAL generation.
- Focused verification:
  ...
  - `php -l lanes/libsqlite/examples/application-wal-checkpoint-reader-restart-current-source-next140.php`
  - `php tools/run-tests.php lanes/libsqlite/tests/SQLiteWalCheckpointReaderRestartCurrentSourceNext140Test.php`
  - `php lanes/libsqlite/examples/application-wal-checkpoint-reader-restart-current-source-next140.php --self-test`
