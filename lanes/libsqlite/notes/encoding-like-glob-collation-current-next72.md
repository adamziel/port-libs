...
- Behavior: adds a bounded `SQLiteGlobCurrentNextCursor` for SQLite-style GLOB index scans over copied `wp_options.option_name` rows. It keeps current/next cursor diagnostics, BINARY/NOCASE/RTRIM index ordering, byte-prefix range bounds, and residual GLOB matching separate so NOCASE index traversal does not make GLOB case-insensitive.
- WordPress smoke: `lanes/libsqlite/examples/wordpress-option-name-glob-current-next.php` reports plugin option-name prefix and Unicode class GLOB rowids without requiring `ext/sqlite`.
...
- Focused evidence:
  - `php tools/run-tests.php lanes/libsqlite/tests/SQLiteGlobCurrentNextCursorCurrentNext72Test.php`
  - `php lanes/libsqlite/examples/wordpress-option-name-glob-current-next.php`
