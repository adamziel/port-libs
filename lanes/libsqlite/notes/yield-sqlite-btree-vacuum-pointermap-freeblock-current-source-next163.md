...
- Behavior: adds `SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan`, which fences a current-source table leaf delete/vacuum flow by tying the deleted leaf freeblock, released overflow source chain, surviving free pages, replacement overflow chain, and final pointer-map parent links into one admission summary.
