...
Behavior slice: adds `SQLiteJsonTablePlan::currentSourcePathGeneratedOrderNext137()` for the current-source planner boundary where a path-constrained `json_tree()` cursor is ordered by generated JSON keys extracted from each filtered row.

Focused verification:

```text
php tools/run-tests.php lanes/libsqlite/tests/SQLiteJsonTablePathGeneratedOrderCurrentSourceNext137Test.php
...
```text
php lanes/libsqlite/examples/wordpress-json-table-path-generated-order-current-source-next137.php --self-test
wordpress-json-table-path-generated-order-current-source-next137 self-test passed
```
