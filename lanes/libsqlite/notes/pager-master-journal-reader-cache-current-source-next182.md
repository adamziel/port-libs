...
WordPress smoke: `wordpress-pager-master-journal-reader-cache-current-source-next182.php` models copied `wp_options` import recovery where a schema reader-cache page survives the checksum-validated master-journal boundary, while a stale `active_plugins` cache row reopens before the next import write.
...
- `php -l lanes/libsqlite/examples/wordpress-pager-master-journal-reader-cache-current-source-next182.php`
- `php lanes/libsqlite/examples/wordpress-pager-master-journal-reader-cache-current-source-next182.php`
