...
- Added `wordpress-transaction-begin-lock-mode.php` to smoke a copied `wp_options` import transaction preflight under `PRAGMA locking_mode=exclusive` and WAL journal mode without requiring ext/sqlite.
...
- `php -l lanes/libsqlite/examples/wordpress-transaction-begin-lock-mode.php`
...
- `php lanes/libsqlite/examples/wordpress-transaction-begin-lock-mode.php` passed and reported planned BEGIN IMMEDIATE WAL preflight.
...
- This slice is bounded to transaction `BEGIN` lock-mode semantics and a WordPress copied-options transaction preflight.
