...
- Added `SQLitePragmaIndexXinfoForeignKeyCurrentSourceNext207` for child-key index prefix diagnostics over `PRAGMA foreign_key_list` groups and `PRAGMA index_xinfo` rows.
...
- Dependency closure: no new support component is needed; this reuses `SQLitePragmaSchemaCatalog`, `SQLitePragmaIndexXinfoForeignKeyCurrentSourceNext159`, and the accepted current-source pagination/resume shape.

Verification recorded by lane worker:

- `php tools/run-tests.php lanes/libsqlite/tests/SQLitePragmaIndexXinfoForeignKeyCurrentSourceNext207Test.php`
