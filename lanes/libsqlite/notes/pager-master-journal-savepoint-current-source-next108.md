...
This slice adds `SQLitePagerMasterJournalSavepointCurrentSourceNext108Plan`. It composes the accepted master-journal current-source hot rollback and savepoint retry path, then records active savepoint before-images from the recovered database image. That proves a WordPress import retry can still `ROLLBACK TO` the savepoint without restoring stale crashed current bytes.
...
```text
php lanes/libsqlite/examples/wordpress-pager-master-journal-savepoint-current-source-next108.php --self-test
wordpress-pager-master-journal-savepoint-current-source-next108 self-test passed
```
