...
This slice adds `SQLiteBTreeOverflowFreelistRootpageCurrentSourceNextPlan`, a bounded current-source transition for the SQLite path where obsolete overflow pages from a deleted Application-sized option payload are released to the freelist, then one of those pages is immediately reused as a new schema root page.
...
- `php -l lanes/libsqlite/examples/application-btree-overflow-freelist-rootpage-current-source-next126.php`
- `php tools/run-tests.php lanes/libsqlite/tests/SQLiteBTreeOverflowFreelistRootpageCurrentSourceNext126Test.php`
- `php lanes/libsqlite/examples/application-btree-overflow-freelist-rootpage-current-source-next126.php`
