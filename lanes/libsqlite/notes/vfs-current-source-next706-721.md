...
This slice extends the consolidated `SQLiteVfsCurrentSourceNext626641Plan` coverage as the direct successor to integrated next690-705. It requires the latest `shared-cache-next705` publish receipt before snapshotting `reader-ready-next721`, records `reader-reuse-next721`, and publishes `shared-cache-next721` only while the handle, path, owner, data version, publish count, receipt digest, and dirty-page state still match the current-source snapshot.
