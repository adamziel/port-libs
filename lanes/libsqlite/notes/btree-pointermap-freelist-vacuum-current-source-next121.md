...
- Behavior: obsolete overflow pages from a deleted WordPress-sized option value are first written into the freelist with auto-vacuum pointer-map entries changed to `free-page`, then the same pages are reused by a follow-up B-tree allocation with pointer-map entries changed to `btree-page` or `root-page`.
...
- WordPress smoke: `php lanes/libsqlite/examples/wordpress-btree-pointermap-freelist-vacuum-current-source-next121.php`.
