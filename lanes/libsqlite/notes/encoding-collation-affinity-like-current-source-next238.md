...
COLLATE BINARY LIKE ?` over WordPress-style `wp_options` rows where REAL values
must be textified before LIKE residual matching.

The slice covers SQLite REAL text-affinity details that matter for copied
WordPress settings: integer-valued REAL values keep a decimal marker
...
- `php lanes/libsqlite/examples/wordpress-encoding-collation-affinity-like-current-source-next238.php`
