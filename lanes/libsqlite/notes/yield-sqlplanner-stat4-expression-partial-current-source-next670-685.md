...
Behavior: extends `SQLitePlannerStat4ExpressionPartialCurrentSourceNext224Plan` with `materializeNext670685()`, a direct follow-on to the merged next654-669 preparation fence. The new fence threads the next654-669 handoff signature, rechecks each carried current-source row projection, and prepares slices 670-685 only when the prior projected rows still match the current source.
