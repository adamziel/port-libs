...
`DELETE`. It records `OLD` and `NEW` values for WordPress `wp_options` style
...
- `php -l lanes/libsqlite/src/SQLiteDmlTriggerCurrentNextPlan.php && php -l lanes/libsqlite/tests/SQLiteDmlTriggerCurrentNextTest.php && php -l lanes/libsqlite/examples/wordpress-dml-trigger-current-next.php`
  - no syntax errors detected in all changed PHP files
- `php lanes/libsqlite/examples/wordpress-dml-trigger-current-next.php`
