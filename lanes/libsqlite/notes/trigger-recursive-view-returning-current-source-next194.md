# trigger-recursive-view-returning-current-source-next194
...
WordPress path: `wordpress-trigger-recursive-view-returning-current-source-next194.php` models a copied `wp_options` import through a recursive view trigger. It proves plugin-created child rows from the current source are fully drained before the next import source can expose `RETURNING` rows.
...
```text
$ php lanes/libsqlite/examples/wordpress-trigger-recursive-view-returning-current-source-next194.php
