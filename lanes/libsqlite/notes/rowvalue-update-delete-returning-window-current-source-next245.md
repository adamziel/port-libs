...
- WordPress path: copied `wp_options` import batches can report current-source migration progress and keep retried rows hidden until all yielded row-value RETURNING window rows are acknowledged.
...
- `php -l lanes/libsqlite/examples/wordpress-rowvalue-returning-window-current-source-next245.php`
  - `No syntax errors detected in lanes/libsqlite/examples/wordpress-rowvalue-returning-window-current-source-next245.php`
...
- `php lanes/libsqlite/examples/wordpress-rowvalue-returning-window-current-source-next245.php`
  - exited `0`
