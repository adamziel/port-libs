...
`ORDER BY` / `LIMIT` / `OFFSET` applied after current/next Application option
...
- Adds a Application smoke for copied `wp_options` import previews where a
...
- No new support component is needed. The slice reuses existing lane-local
  ...
  final result ordering/limit execution, and Application row-array fixtures.
...
- Avoids accepted compound VALUES affinity/order next127, recursive affinity
  ...
  Application option rows are compared under the left compound arm's collation
