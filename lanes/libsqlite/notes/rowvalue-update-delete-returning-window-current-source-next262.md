...
WordPress smoke: `wordpress-rowvalue-returning-window-current-source-next262.php` models copied `wp_options` yield, rollback, retry, and DELETE RETURNING rows where `status='stale'` peers span current row `3` and retry row `4`.
...
- `php -l lanes/libsqlite/examples/wordpress-rowvalue-returning-window-current-source-next262.php`
...
- `php lanes/libsqlite/examples/wordpress-rowvalue-returning-window-current-source-next262.php`
  - Result: `wordpress-rowvalue-returning-window-current-source-next262 self-test passed`
