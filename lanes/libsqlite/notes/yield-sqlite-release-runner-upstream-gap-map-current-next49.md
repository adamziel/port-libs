...
Adds `SQLiteUpstreamSuiteEvidence::releaseRunnerUpstreamGapMapCurrentNext49()` for lane-local release-runner evidence. The helper maps countable current accepted bounded-runner artifacts to explicit next focused runner targets without launching a runner or counting release/all parity.
...
```sh
php tools/run-tests.php lanes/libsqlite/tests/SQLiteReleaseRunnerUpstreamGapMapCurrentNext49Test.php
```
