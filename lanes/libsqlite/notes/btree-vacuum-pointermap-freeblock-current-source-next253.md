...
- Application smoke models copied `wp_options` transient deletion where obsolete overflow pages are vacuumed and the next writer can only reuse freeblock pages after pointer-map apply grouping is visible.
...
- `php -l lanes/libsqlite/examples/application-btree-vacuum-pointermap-freeblock-current-source-next253.php`
...
- `php lanes/libsqlite/examples/application-btree-vacuum-pointermap-freeblock-current-source-next253.php`
- Result: `application-btree-vacuum-pointermap-freeblock-current-source-next253 self-test passed`
