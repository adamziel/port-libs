...
Application smoke: `application-pager-master-journal-hot-cache-current-source-next136.php` covers copied `wp_options` repair behavior where a stale clean options page is refreshed from current hot recovery, a dirty plugin-settings cache page is invalidated, and the next plugin-setting write journals the recovered current-source page.
...
- `php -l lanes/libsqlite/examples/application-pager-master-journal-hot-cache-current-source-next136.php`
- `php tools/run-tests.php lanes/libsqlite/tests/SQLitePagerMasterJournalHotCacheCurrentSourceNext136Test.php`
- `php lanes/libsqlite/examples/application-pager-master-journal-hot-cache-current-source-next136.php`
