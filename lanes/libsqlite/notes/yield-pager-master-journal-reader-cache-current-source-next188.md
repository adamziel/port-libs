...
WordPress smoke: `wordpress-pager-master-journal-reader-cache-current-source-next188.php --self-test` models copied `wp_options` import recovery where the alloptions root can refresh across a NUL-padded master-journal read, while plugin settings and rewrite-rule cache entries are reopened because their member token or dirty state cannot cross the current-source boundary.
...
- `php -l lanes/libsqlite/examples/wordpress-pager-master-journal-reader-cache-current-source-next188.php`
...
- `php lanes/libsqlite/examples/wordpress-pager-master-journal-reader-cache-current-source-next188.php --self-test`
  - `wordpress-pager-master-journal-reader-cache-current-source-next188 self-test passed`
