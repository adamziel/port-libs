...
Adds a bounded current/next planner for WordPress `wp_options` numeric-affinity comparisons. The slice covers numeric text coercion, integer/text/real storage swaps, UTF-16LE/UTF-16BE byte changes, source/schema-cookie invalidation, and text fallback comparison under `NOCASE`/`RTRIM`.
...
- `php lanes/libsqlite/examples/wordpress-encoding-numeric-affinity-current-source-next107.php`
