...
- WordPress smoke: copied `wp_options` import through a recursive view trigger keeps current `RETURNING` rows visible while next-source rows are held behind the current-source cursor resume boundary.
...
php -l lanes/libsqlite/examples/wordpress-trigger-recursive-view-returning-current-source-next177.php
php tools/run-tests.php lanes/libsqlite/tests/SQLiteTriggerRecursiveViewReturningCurrentSourceNext177Test.php
php lanes/libsqlite/examples/wordpress-trigger-recursive-view-returning-current-source-next177.php --self-test
