...
- Added `application-rowvalue-returning-filter-window-current-source-next246.php` smoke for copied `wp_options` import batches that compute filtered RETURNING window receipts across a rolled-back attempt and a released retry.
...
```text
php lanes/libsqlite/examples/application-rowvalue-returning-filter-window-current-source-next246.php
exit 0
```
...
php -l lanes/libsqlite/examples/application-rowvalue-returning-filter-window-current-source-next246.php
No syntax errors detected in lanes/libsqlite/examples/application-rowvalue-returning-filter-window-current-source-next246.php
```
