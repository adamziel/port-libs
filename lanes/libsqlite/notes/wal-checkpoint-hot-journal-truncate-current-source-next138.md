...
This slice adds `SQLiteWalCheckpointHotJournalTruncateCurrentSourceNext138Plan`. It models the current-source boundary where hot rollback-journal recovery must restore the database image before a savepoint rollback truncates the WAL prefix and a follow-up TRUNCATE checkpoint is allowed to remove the WAL sidecar after the pinned reader drains.

WordPress smoke: `wordpress-wal-checkpoint-hot-journal-truncate-current-source-next138.php` covers copied `wp_options` import repair where a failed import leaves dirty database pages, the hot rollback journal restores clean pages, a plugin-batch savepoint discards later WAL frames, and the final TRUNCATE checkpoint exposes database-only pages to the next reader.

Focused verification:

- `php tools/run-tests.php lanes/libsqlite/tests/SQLiteWalCheckpointHotJournalTruncateCurrentSourceNext138Test.php`
- `php -l lanes/libsqlite/src/SQLiteWalCheckpointHotJournalTruncateCurrentSourceNext138Plan.php`
- `php -l lanes/libsqlite/tests/SQLiteWalCheckpointHotJournalTruncateCurrentSourceNext138Test.php`
- `php -l lanes/libsqlite/examples/wordpress-wal-checkpoint-hot-journal-truncate-current-source-next138.php`
- `php lanes/libsqlite/examples/wordpress-wal-checkpoint-hot-journal-truncate-current-source-next138.php --self-test`
