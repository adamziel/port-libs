...
Update: `real-upstream-corpus-date-affinity-dynamic-20260530T165907Z-0`
...
- Extends the same hydrated upstream `date4.test` strftime libc-parity range from `date4-1024` through `date4-1535`, preserving the upstream loop formula `TS = i*86390` and the same Linux libc format list.

Verification:
- `php tools/run-tests.php lanes/libsqlite/tests/SQLiteRealUpstreamDateAffinityDynamicCorpusTest.php`
  - Latest focused run: `1 test files, 1624 assertions, 0 failures`
...
- Additional expected focused PASS-line delta for the `20260530T165907Z` update is `+512` from the non-overlapping `date4-1024` through `date4-1535` range.
...
- The latest extension avoids the already accepted `date4-0` through `date4-1023` range and owns only `date4-1024` through `date4-1535`.
