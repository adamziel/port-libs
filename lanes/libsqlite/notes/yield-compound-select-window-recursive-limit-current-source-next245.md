...
- Behavior: adds `SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNextPlan`, a bounded next-source promotion snapshot for compound SELECT plans that combine recursive CTE LIMIT/OFFSET, window ranking, INTERSECT/EXCEPT membership, and final LIMIT/OFFSET paging.
- Application path: `application-compound-select-window-recursive-limit-current-source-next245.php` models `wp_options` current-source rows where a next-source `plugin_prime` option displaces `rewrite_rules` after the current replay/spillover fences have already been acknowledged.
