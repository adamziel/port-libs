...
Behavior: extends `SQLitePlannerStat4ExpressionPartialCurrentSourceNext224Plan` with `materializeNext846861()`, a direct follow-on to the merged next830-845 preparation fence. The new fence threads the next830-845 handoff signature, rechecks each carried current-source row projection, and prepares slices 846-861 only when the prior projected rows still match the current source.
