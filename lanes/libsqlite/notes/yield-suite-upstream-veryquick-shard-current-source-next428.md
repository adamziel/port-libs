# libsqlite suite upstream veryquick shard current-source next428

## Scope

- Removes one focused upstream runner/countability blocker by admitting the
  current-source next428 veryquick shard row.
...
behavior surfaces, accepted batch228 suite rows, and the live B-tree, JSON,
...
```text
$ php tools/run-tests.php lanes/libsqlite/tests/SQLiteUpstreamVeryquickShardCurrentSourceNext428Test.php
Focused test run: 1 selected test files (root lock skipped)
...
1 test files, 780 assertions, 0 failures
```
