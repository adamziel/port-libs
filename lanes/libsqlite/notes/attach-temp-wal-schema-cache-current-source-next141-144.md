...
- next144 covers `DETACH` of an attached WordPress archive schema, moving qualified statements to `__detached__` and blocking stale write retry before reprepare.
...
php -l lanes/libsqlite/examples/wordpress-attach-temp-wal-schema-cache-current-source-next141-144.php
...
php lanes/libsqlite/examples/wordpress-attach-temp-wal-schema-cache-current-source-next141-144.php --self-test
git diff --check
```
