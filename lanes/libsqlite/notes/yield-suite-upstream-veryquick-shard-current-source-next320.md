# libsqlite suite upstream veryquick shard current-source next320

## Scope

- Removes one focused upstream runner/countability blocker by admitting the
  current-source next320 veryquick shard row.
...
exact-shard next148, runner106/jsonvt104 rebase items, accepted batch107/108
and batch109-113 behavior surfaces, accepted batch222 non-JSON behavior
handoffs, and the live B-tree, JSON, VFS, WAL, planner, PRAGMA, ATTACH,
...
```text
$ php tools/run-tests.php lanes/libsqlite/tests/SQLiteUpstreamVeryquickShardCurrentSourceNext320Test.php
