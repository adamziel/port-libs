...
Adds a reader-reopen fence after next219 checkpoint source publication. The plan admits reopened WordPress readers only when their source token, epoch, checkpoint frame, checkpoint cookie, schema cookie, hot-journal visibility, cache reopen state, and savepoint depth match the published checkpoint source.

Focused verification:

```sh
php tools/run-tests.php lanes/libsqlite/tests/SQLiteWalHotJournalSavepointCheckpointCurrentSourceNext220Test.php
php lanes/libsqlite/examples/wordpress-wal-hot-journal-savepoint-checkpoint-current-source-next220.php --self-test
```
