# Upstream veryquick shard current-source next421
...
`SQLiteUpstreamSuiteEvidence::upstreamVeryquickShardCurrentSourceNext421()`,
...
classification, and focused PHP PASS-line output all match the next421
evidence record.
...
php -l lanes/libsqlite/tests/SQLiteUpstreamVeryquickShardCurrentSourceNext421Test.php
php tools/run-tests.php lanes/libsqlite/tests/SQLiteUpstreamVeryquickShardCurrentSourceNext421Test.php
php -r 'json_decode(file_get_contents("lanes/libsqlite/UPSTREAM_TEST_MANIFEST.json"), true, 512, JSON_THROW_ON_ERROR); json_decode(file_get_contents("lanes/libsqlite/lane-status.json"), true, 512, JSON_THROW_ON_ERROR);'
git diff --check -- lanes/libsqlite
```

Observed focused result before metadata updates:

```text
No syntax errors detected in lanes/libsqlite/src/SQLiteUpstreamSuiteEvidence.php
No syntax errors detected in lanes/libsqlite/tests/SQLiteUpstreamVeryquickShardCurrentSourceNext421Test.php
Focused test run: 1 selected test files (root lock skipped)
73 PASS lines
1 test files, 1012 assertions, 0 failures
```

Expected dashboard movement: `phpPass` increases by the verified focused
TestRunner PASS-line delta `+73`, from `149839` to `149912`, and mapped
coverage moves from `782 / 1589` to `783 / 1589`.
