...
The slice deliberately avoids the accepted parser-level JSON aggregate expression ORDER BY and SQL SELECT paths from batch99. It covers the lower-level aggregate/state/window helper path used by native WordPress option-summary previews.
...
- `php -l lanes/libsqlite/examples/wordpress-json-aggregate-distinct-order-current-source-next102.php`
...
- `php lanes/libsqlite/examples/wordpress-json-aggregate-distinct-order-current-source-next102.php`
