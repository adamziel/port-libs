...
WordPress smoke: `wordpress-pager-master-journal-reader-cache-current-source-next168.php` models copied `wp_options` recovery where the root page is cleanly refreshed, but `active_plugins` and plugin-setting cache pages are rejected because they predate the current master-journal source digest/generation before the next write.
...
- `php lanes/libsqlite/examples/wordpress-pager-master-journal-reader-cache-current-source-next168.php`
  - `wordpress-pager-master-journal-reader-cache-current-source-next168 self-test passed`
...
- `php -l lanes/libsqlite/examples/wordpress-pager-master-journal-reader-cache-current-source-next168.php`
