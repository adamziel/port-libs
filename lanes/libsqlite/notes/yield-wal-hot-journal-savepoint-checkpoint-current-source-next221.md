...
- Behavior: adds `SQLiteWalHotJournalSavepointCheckpointCurrentSourceNext221Plan`, a post-next217 current-source barrier that admits the next WAL source only after hot-journal deletion, restarted WAL generation, SHM read-mark reset, savepoint closure, exclusive-lock receipt, and directory sync receipts all match the checkpoint token.
