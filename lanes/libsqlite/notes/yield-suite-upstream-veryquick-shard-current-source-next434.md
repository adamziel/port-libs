# libsqlite suite upstream veryquick shard current-source next434

## Scope

- Removes one focused upstream runner/countability blocker by admitting the
  current-source next434 veryquick shard row.
...
- focused test:
  `lanes/libsqlite/tests/SQLiteUpstreamVeryquickShardCurrentSourceNext434Test.php`

## Verification

```text
$ php tools/run-tests.php lanes/libsqlite/tests/SQLiteUpstreamVeryquickShardCurrentSourceNext434Test.php
Focused test run: 1 selected test files (root lock skipped)
...
1 test files, 870 assertions, 0 failures
```
