...
- Builds on next181 snapshot rows and creates a current-source cursor contract for a copied WordPress database after deleting a large transient row, partially vacuuming the obsolete overflow tail, and writing replacement overflow pages.
...
- `php tools/run-tests.php lanes/libsqlite/tests/SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNext184Test.php`
  ...
  - `101` PASS lines
- `php lanes/libsqlite/examples/wordpress-btree-vacuum-pointermap-freeblock-current-source-next184.php`
  - self-test passed
