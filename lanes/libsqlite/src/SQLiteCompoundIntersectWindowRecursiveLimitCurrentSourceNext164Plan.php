...
final class SQLiteCompoundIntersectWindowRecursiveLimitCurrentSourceNext164Plan
{
    /**
    ...
    public static function compare(string $sql, array $currentTables, array $nextTables): array
    {
        ...
        return [
            ...
            'intersect' => [
                ...
                'admittedLabels' => self::labels($nextRows),
            ],
            'yieldBoundary' => [
    ...
    private static function assertSupported(string $sql, array $currentPlan, array $nextPlan): void
    {
        ...
        if (!in_array('INTERSECT', self::operators($currentPlan), true)) {
    ...
    {
        return array_values(array_merge(array_diff(self::labels($currentRows), self::labels($nextRows)), array_diff(self::labels($nextRows), self::labels($currentRows))));
    }
