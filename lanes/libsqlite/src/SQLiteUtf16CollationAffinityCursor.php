...
final class SQLiteUtf16CollationAffinityCursor
{
     ...
     * @return list<array{rowid:int,value:mixed,storage:string,encoding:?string,valueBytesHex:?string,comparisonToProbe:?int,payload:array<string,mixed>,position:int}>
     */
    public static function wordpressOptionValueSeek(
        array $rows,
        mixed $probe,
        string $leftAffinity = 'TEXT',
        string $rightAffinity = 'TEXT',
        ...
        foreach ($rows as $row) {
            if (!isset($row['option_id']) || !is_int($row['option_id'])) {
                throw new \InvalidArgumentException('SQLite UTF-16 affinity WordPress seek requires integer option_id');
            }
            if (!array_key_exists('option_value_bytes', $row) && !array_key_exists('option_value', $row)) {
                throw new \InvalidArgumentException('SQLite UTF-16 affinity WordPress seek requires option_value or option_value_bytes');
            }
            $entry = [
                'rowid' => $row['option_id'],
                'payload' => $row,
            ...
            if (array_key_exists('option_value_bytes', $row)) {
                if (!is_string($row['option_value_bytes'])) {
                    throw new \InvalidArgumentException('SQLite UTF-16 affinity WordPress seek requires string option_value_bytes');
                }
                if (!isset($row['text_encoding']) || !is_int($row['text_encoding'])) {
                    throw new \InvalidArgumentException('SQLite UTF-16 affinity WordPress seek requires integer text_encoding');
                }
