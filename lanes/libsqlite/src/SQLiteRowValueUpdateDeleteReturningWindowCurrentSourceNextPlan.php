...
/**
    ...
    private static function readyPublicationBase(
        ...
        if ($targetStep < 800 || $targetStep > 993) {
            throw new \InvalidArgumentException('SQLite row-value window ready-publication base step must be between 800 and 993');
        }

        $base = self::executeNext799($tables, $yieldStatements, $attemptStatements, $retryStatements, $uniqueConstraints, $savepoint, $rowIdColumn);
        for ($step = 800; $step <= $targetStep; $step++) {
            $blockStart = $step - (($step - 798) % 4);
    ...
    public static function executeReadyPublicationContinuation(
        ...
        if ($publicationStep < 800 || $publicationStep > 1181) {
            throw new \InvalidArgumentException('SQLite row-value window ready-publication continuation step must be between 800 and 1181');
        }
