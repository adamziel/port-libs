...
final class SQLiteWordPressSchemaJsonSavepointWalPlan
{
    /**
    ...
    {
        if ($imports === []) {
            throw new \InvalidArgumentException('SQLite WordPress schema JSON savepoint WAL plan requires at least one import');
        }
    ...
    {
        if (!is_array($value)) {
            throw new \InvalidArgumentException('SQLite WordPress schema JSON options require string lists');
        }

        $strings = [];
        foreach ($value as $item) {
            if (!is_string($item) || $item === '') {
                throw new \InvalidArgumentException('SQLite WordPress schema JSON options require non-empty strings');
            }
            $strings[] = $item;
        }
    ...
    private static function savepointName(array $import, int $index): string
    {
        ...
        if ($name === '' || preg_match('/^[A-Za-z_][A-Za-z0-9_]*$/', $name) !== 1) {
            throw new \InvalidArgumentException('SQLite WordPress schema JSON savepoint names must be SQL identifiers');
        }
    ...
    private static function jsonRows(mixed $json, string $path): array
    {
        if (!is_string($json) && !$json instanceof SQLiteBlobValue && !$json instanceof SQLiteJsonSubtypeValue) {
            return self::invalidJson($path, 'SQLite WordPress schema JSON import requires text, JSON subtype, or JSONB blob input');
        }
        ...
        try {
            ...
            if ($valid !== true) {
                return self::invalidJson($path, 'SQLite WordPress schema JSON import source is malformed JSON');
            }

            $extracted = SQLiteJsonExtract::extract($value, $path);
            if ($extracted === null) {
                return self::invalidJson($path, 'SQLite WordPress schema JSON import path did not match any rows');
            }

            $decoded = is_string($extracted) ? json_decode($extracted, true, 1001, JSON_BIGINT_AS_STRING | JSON_THROW_ON_ERROR) : $extracted;
            if (!is_array($decoded)) {
                throw new \InvalidArgumentException('SQLite WordPress schema JSON import path must resolve to an object or array of objects');
            }
            ...
            foreach ($rows as $row) {
                if (!is_array($row)) {
                    throw new \InvalidArgumentException('SQLite WordPress schema JSON import rows must be objects');
                }
                $normalized[] = $row;
            }
    ...
    private static function validateRows(array $rows, array $schema): array
    {
        ...
        return [
            ...
            'error' => $violations === [] ? null : 'SQLite WordPress schema JSON import row failed schema validation',
        ];
    }
    ...
    private static function rowsById(array $rows): array
    {
        ...
        foreach ($rows as $row) {
            ...
            if (!is_int($id)) {
                throw new \InvalidArgumentException('SQLite WordPress schema JSON import rows require integer option_id values');
            }
            $byId[$id] = $row;
        }
        ksort($byId);
