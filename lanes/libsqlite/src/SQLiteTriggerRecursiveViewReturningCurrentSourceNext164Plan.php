...
final class SQLiteTriggerRecursiveViewReturningCurrentSourceNext164Plan
{
    /**
    ...
    public static function execute(
        ...
        $savepoint = self::token((string) ($options['savepoint'] ?? 'wp_recursive_view_returning_164'), 'savepoint');
        ...
        $skipColumn = self::identifier((string) ($options['skip_column'] ?? 'autoload_flag'), 'skip column');
        ...
        $current = self::runViewSource($baseRows, $currentInput, $currentView, $returning, $key, $recursive, $maxDepth, $skipColumn, $skipValue, $conflictAction, 'current');
        $nextAttempt = self::runViewSource($admitNext ? $current['rows'] : $baseRows, $nextInput, $nextView, $returning, $key, $recursive, $maxDepth, $skipColumn, $skipValue, $conflictAction, 'next');
     ...
     */
    private static function runViewSource(array $rows, array $input, array $view, array $returning, string $key, bool $recursive, int $maxDepth, string $skipColumn, mixed $skipValue, string $conflictAction, string $phase): array
    {
        $queue = [];
        ...
        while ($queue !== []) {
            ...
            if ($skip) {
                $envelope = self::returningEnvelope($phase, $view, (int) $item['ordinal'], (int) $item['depth'], $event, [
                    'option_name' => $rowKey,
            ...
            $oldIndex = self::rowIndex($rows, $key, $rowKey);
            $old = $oldIndex === null ? null : $rows[$oldIndex];
            if ($old !== null && $conflictAction === 'ignore') {
                $envelope = self::returningEnvelope($phase, $view, (int) $item['ordinal'], (int) $item['depth'], 'conflict-ignore', [
                    'option_name' => $rowKey,
            ...
            $returningRow = self::returningRow($returning, $incoming, $item['view_row'], $old, $event, (int) $item['ordinal'], (int) $item['depth'], $view['trigger_source']);
     ...
     * @return array<string,mixed>
     */
    private static function returningRow(array $returning, array $new, array $viewRow, ?array $old, string $event, int $ordinal, int $depth, string $source): array
    {
        $out = [];
        foreach (array_values($returning) as $index => $expr) {
            if (is_string($expr)) {
                $alias = str_starts_with($expr, 'new.') ? substr($expr, 4) : $expr;
                $out[$alias] = self::exprValue($expr, $new, $viewRow, $old, $event, $ordinal, $depth, $source);
                continue;
            }
            if (is_array($expr)) {
                $sql = (string) ($expr['expr'] ?? '');
                $alias = (string) ($expr['as'] ?? (str_starts_with($sql, 'new.') ? substr($sql, 4) : $sql));
                $out[self::identifier($alias, 'RETURNING alias')] = self::exprValue($sql, $new, $viewRow, $old, $event, $ordinal, $depth, $source);
                continue;
            }
            if (is_callable($expr)) {
    ...
    private static function exprValue(string $expr, array $new, array $viewRow, ?array $old, string $event, int $ordinal, int $depth, string $source): mixed
    {
        return match ($expr) {
            'event' => $event,
            'ordinal' => $ordinal,
