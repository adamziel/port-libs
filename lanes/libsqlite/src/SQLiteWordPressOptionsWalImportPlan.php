...
final class SQLiteWordPressOptionsWalImportPlan
{
    /**
    ...
    public static function currentNext(
        ...
        if ($databasePath === '') {
            throw new \InvalidArgumentException('SQLite WordPress options WAL import requires a database path');
        }
        if ($importRows === []) {
            throw new \InvalidArgumentException('SQLite WordPress options WAL import requires at least one imported row');
        }
        if ($pageNumbers === []) {
            throw new \InvalidArgumentException('SQLite WordPress options WAL import current/next requires at least one page number');
        }
        if ($firstOptionPageNumber < 2) {
            throw new \InvalidArgumentException('SQLite WordPress options WAL import option pages must start after page one');
        }

        $pageSize = $wal->header->pageSize;
        if ($pageSize < 512) {
            throw new \InvalidArgumentException('SQLite WordPress options WAL import requires a concrete WAL page size');
        }
        ...
        foreach ($pageNumbers as $pageNumber) {
            if (!is_int($pageNumber)) {
                throw new \InvalidArgumentException('SQLite WordPress options WAL import pages must be integers');
            }
        ...
        return [
            'status' => 'planned',
            'reason' => 'wordpress_options_import_wal_commit_current_next_visibility',
            ...
            'dependencies' => array_values(array_unique(array_merge(
                $append['dependencies'],
                ['wordpress-options-wal-import-current-next']
            ))),
        ];
    }
    ...
    private static function normalizeRows(array $rows, bool $requireIds): array
    {
        ...
        foreach ($rows as $index => $row) {
            ...
            if ($name === '') {
                throw new \InvalidArgumentException('SQLite WordPress options WAL import requires option_name');
            }
            $optionId = $row['option_id'] ?? ($index + 1);
            if ($requireIds && (!is_int($optionId) || $optionId < 1)) {
                throw new \InvalidArgumentException('SQLite WordPress options WAL import requires positive current option_id values');
            }
            $normalized[$name] = [
    ...
    private static function rowPage(array $row, int $pageNumber, int $pageSize): string
    {
        ...
        if (!is_string($json)) {
            throw new \RuntimeException('Unable to encode WordPress options WAL import row');
        }
        if (strlen($json) > $pageSize) {
            throw new \InvalidArgumentException('SQLite WordPress options WAL import row page exceeds page size');
        }
    ...
    private static function autoloadIndexPage(array $rows, int $pageNumber, int $pageSize): string
    {
        ...
        if (!is_string($json)) {
            throw new \RuntimeException('Unable to encode WordPress options WAL import autoload page');
        }
        if (strlen($json) > $pageSize) {
            throw new \InvalidArgumentException('SQLite WordPress options WAL import autoload page exceeds page size');
        }
