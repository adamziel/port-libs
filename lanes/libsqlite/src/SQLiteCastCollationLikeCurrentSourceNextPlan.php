    ...
    private static function traceSource(
        ...
        foreach ($rows as $row) {
            ...
            if (!is_int($row['setting_id'])) {
                throw new \InvalidArgumentException('SQLite CAST current-source setting_id must be an integer');
            }
        }

        $quotedTarget = self::castTargetSql($castTarget);
        $castRows = SQLiteSelectSql::execute(
            "SELECT setting_id, key_value, CAST(key_value AS {$quotedTarget}) AS cast_value FROM app_settings ORDER BY setting_id",
            ['app_settings' => $rows],
        );
