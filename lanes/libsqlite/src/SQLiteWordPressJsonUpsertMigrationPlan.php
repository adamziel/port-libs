...
final class SQLiteWordPressJsonUpsertMigrationPlan
{
    /**
    ...
    private static function applyJsonSetValues(string $json, array $excluded, array $jsonSetValues, ?array $current): string
    {
        ...
        if (!is_string($mutated)) {
            throw new \RuntimeException('SQLite WordPress JSON UPSERT migration expected text JSON output');
        }
    ...
    private static function resolveJsonSetValue(mixed $value, array $excluded, ?array $current): mixed
    {
        ...
        throw new \InvalidArgumentException('SQLite WordPress JSON UPSERT migration value expression is unsupported');
    }
    ...
    private static function decodeReturningRow(array $row): array
    {
        ...
        if (!is_array($decoded)) {
            throw new \InvalidArgumentException('SQLite WordPress JSON UPSERT migration option_value must decode to an object or array');
        }
    ...
    private static function requireString(array $row, string $column, string $label): string
    {
        if (!array_key_exists($column, $row) || !is_string($row[$column])) {
            throw new \InvalidArgumentException("SQLite WordPress JSON UPSERT migration {$label} {$column} must be text JSON");
        }
    ...
    private static function requireColumnValue(array $row, mixed $column, string $label): mixed
    {
        if (!is_string($column) || $column === '') {
            throw new \InvalidArgumentException("SQLite WordPress JSON UPSERT migration {$label} column reference is malformed");
        }
        if (!array_key_exists($column, $row)) {
            throw new \InvalidArgumentException("SQLite WordPress JSON UPSERT migration {$label} column {$column} is missing");
        }
    ...
    private static function requireJsonPath(mixed $path): string
    {
        if (!is_string($path) || $path === '') {
            throw new \InvalidArgumentException('SQLite WordPress JSON UPSERT migration JSON path is malformed');
        }
    ...
    {
        if (!is_string($json)) {
            throw new \InvalidArgumentException("SQLite WordPress JSON UPSERT migration {$label} must be text JSON");
        }
    ...
    {
        if ($jsonSetValues === []) {
            throw new \InvalidArgumentException('SQLite WordPress JSON UPSERT migration requires at least one json_set path');
        }

        foreach ($jsonSetValues as $path => $_) {
            if (!is_string($path) || $path === '' || $path[0] !== '$') {
                throw new \InvalidArgumentException('SQLite WordPress JSON UPSERT migration json_set paths must be JSON paths');
            }
        }
    }
}
