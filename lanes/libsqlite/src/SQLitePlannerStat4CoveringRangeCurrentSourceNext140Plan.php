...
final class SQLitePlannerStat4CoveringRangeCurrentSourceNext140Plan
{
    /**
    ...
    public static function materialize(
        ...
        $base = SQLitePlannerCoveringStat4RangeCurrentSourceNext138Plan::materialize(
            $preparedSource,
            $currentSource,
            $predicate,
            $orderBy,
        ...
        $currentBuckets = self::bucketKeys($base);
        $preparedBuckets = self::preparedBucketKeys($preparedSource, $base);
        ...
        return array_replace($base, [
            ...
            'dependencies' => array_values(array_unique(array_merge(
                ...
                [
                    'SQLitePlannerCoveringStat4RangeCurrentSourceNext138Plan',
     ...
     * @return list<string>
     */
    private static function preparedBucketKeys(array $source, array $base): array
    {
     ...
     * @return list<string>
     */
    private static function bucketKeys(array $base): array
    {
    ...
    private static function cursorSteps(array $rows, array $base): array
    {
        ...
        foreach ($rows as $offset => $row) {
            ...
            $sameAsPrevious = is_array($previous) && self::compare($previous['rangeKey'] ?? null, $row['rangeKey'] ?? null) === 0;
            $sameAsNext = is_array($next) && self::compare($next['rangeKey'] ?? null, $row['rangeKey'] ?? null) === 0;
    ...
    private static function stableTieBreak(array $rows): bool
    {
        ...
        foreach ($rows as $row) {
            ...
            $rowid = $row['rowid'] ?? null;
            if ($previousKey !== null && self::compare($previousKey, $key) === 0 && (int) $previousRowid > (int) $rowid) {
                return false;
            }
            $previousKey = $key;
            $previousRowid = $rowid;
