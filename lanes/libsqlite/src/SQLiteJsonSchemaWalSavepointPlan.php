...
final class SQLiteJsonSchemaWalSavepointPlan
{
    /**
    ...
    public static function plan(array $schema, array $steps, array $options = []): array
    {
        ...
        if ($pageSize < 512 || ($pageSize & ($pageSize - 1)) !== 0) {
            throw new \InvalidArgumentException('SQLite Application JSON schema WAL savepoint page size must be a power of two at least 512');
        }
        ...
        $transaction = self::nonEmptyString($options['transaction'] ?? 'application_json_schema_import', 'transaction');
        ...
        if ($database === '' || strlen($database) % $pageSize !== 0) {
            throw new \InvalidArgumentException('SQLite Application JSON schema WAL savepoint database image must be aligned to page size');
        }
        ...
        return [
            ...
            'dependencies' => [
                ...
                'sqlite-application-json-schema-wal-savepoint',
            ],
        ];
    }
    ...
    private static function positiveInt(mixed $value, string $label, bool $allowZero = false): int
    {
        if (!is_int($value) || ($allowZero ? $value < 0 : $value <= 0)) {
            throw new \InvalidArgumentException("SQLite Application JSON schema WAL savepoint {$label} must be " . ($allowZero ? 'non-negative' : 'positive') . ' integer');
        }
    ...
    private static function nonEmptyString(mixed $value, string $label): string
    {
        if (!is_string($value) || $value === '') {
            throw new \InvalidArgumentException("SQLite Application JSON schema WAL savepoint {$label} must be non-empty text");
        }
    ...
    private static function walFrame(mixed $value, int &$nextFrame): int
    {
        if (!is_int($value) || $value < $nextFrame) {
            throw new \InvalidArgumentException('SQLite Application JSON schema WAL savepoint frame indexes must increase');
        }
        $nextFrame = $value + 1;
    ...
    private static function writePage(string $database, int $pageSize, int $pageNumber, string $page): string
    {
        if (strlen($page) !== $pageSize) {
            throw new \InvalidArgumentException('SQLite Application JSON schema WAL savepoint page image has the wrong size');
        }
        $offset = ($pageNumber - 1) * $pageSize;
        if ($offset + $pageSize > strlen($database)) {
