...
final class SQLiteIndexInteriorPage
{
    /**
    ...
    public static function assemble(
        ...
        if ($rightMostPage < 1) {
            throw new \InvalidArgumentException('SQLite index interior right-most page must be positive');
        }

        $usableSize ??= $pageSize;
        if ($usableSize < 480 || $usableSize > $pageSize) {
            throw new \InvalidArgumentException('SQLite index interior usable size is outside the page');
        }
        if ($headerOffset < 0 || $headerOffset + 12 > $usableSize) {
            throw new \InvalidArgumentException('SQLite index interior page header offset is outside the usable page');
        }
        if (count($cells) > 65535) {
            throw new \InvalidArgumentException('SQLite index interior page cannot hold more than 65535 cell pointers');
        }

        $page = $basePage ?? str_repeat("\0", $pageSize);
        if (strlen($page) !== $pageSize) {
            throw new \InvalidArgumentException('SQLite index interior base page length does not match page size');
        }
        ...
        foreach ($cells as $cell) {
            if (!is_string($cell) || $cell === '') {
                throw new \InvalidArgumentException('SQLite index interior page cells must be non-empty byte strings');
            }
            $offset -= strlen($cell);
            if ($offset < $minimumCellOffset) {
                throw new \InvalidArgumentException('SQLite index interior page cells overlap the header or pointer array');
            }
