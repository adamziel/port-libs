    ...
    private static function scanRows(array $rows, string $pattern, string $collation, ?string $escape, bool $caseSensitiveLike, ?array $range): array
    {
        ...
        foreach ($rows as $row) {
            if (!isset($row['setting_id']) || !is_int($row['setting_id'])) {
                throw new \InvalidArgumentException('SQLite NOCASE/RTRIM LIKE current-source rows require integer setting_id');
            }
            if (!array_key_exists('key_name_bytes', $row) || !is_string($row['key_name_bytes'])) {
                throw new \InvalidArgumentException('SQLite NOCASE/RTRIM LIKE current-source rows require key_name_bytes');
            }
