...
final class SQLiteSchemaImportSavepointPlan
{
    /**
    ...
    {
        if ($batches === []) {
            throw new \InvalidArgumentException('SQLite Application schema import savepoint plan requires at least one batch');
        }
        ...
        foreach (array_values($batches) as $batchIndex => $batch) {
            ...
            if (!in_array($onError, ['rollback', 'abort'], true)) {
                throw new \InvalidArgumentException('SQLite Application schema import savepoint on_error must be rollback or abort');
            }
            ...
            try {
                $schemaPlan = SQLiteSchemaBulkImportPlan::plan(
                    (string) $batch['dump'],
        ...
        return [
            ...
            'dependencies' => [
                'sqlite-application-schema-import-savepoint-current',
    ...
    private static function pageSize(mixed $value): int
    {
        ...
        if ($pageSize < 512 || ($pageSize & ($pageSize - 1)) !== 0) {
            throw new \InvalidArgumentException('SQLite Application schema import savepoint page size must be a power of two at least 512');
        }
