...
final class SQLiteSchemaJsonWalImportPlan
{
    ...
    public static function plan(
        ...
        $schemaPlan = SQLiteSchemaBulkImportPlan::plan($schemaSql, $existingObjects, $schemaOptions);
        $jsonPlan = SQLiteJsonImportSavepointPlan::plan($currentRows, $mutations, $jsonOptions);
        ...
        return [
            ...
            'dependencies' => [
                'sqlite-application-schema-bulk-import',
                'sqlite-application-json-import-savepoint',
    ...
    private static function pageSize(mixed $value): int
    {
        if (!is_int($value) && !(is_string($value) && ctype_digit($value))) {
            throw new \InvalidArgumentException('SQLite Application schema JSON WAL import page size must be an integer');
        }
        $pageSize = (int) $value;
        if ($pageSize < 512 || ($pageSize & ($pageSize - 1)) !== 0) {
            throw new \InvalidArgumentException('SQLite Application schema JSON WAL import page size must be a power of two at least 512');
        }
    ...
    private static function databasePath(mixed $value): string
    {
        if (!is_string($value) || $value === '' || str_contains($value, "\0") || !str_starts_with($value, '/')) {
            throw new \InvalidArgumentException('SQLite Application schema JSON WAL import database_path must be an absolute path');
        }
    ...
    {
        if (!is_int($value) && !(is_string($value) && ctype_digit($value))) {
            throw new \InvalidArgumentException("SQLite Application schema JSON WAL import {$label} must be a non-negative integer");
        }
        $integer = (int) $value;
        if ($integer < 0) {
            throw new \InvalidArgumentException("SQLite Application schema JSON WAL import {$label} must be non-negative");
        }
