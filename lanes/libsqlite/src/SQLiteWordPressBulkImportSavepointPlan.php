...
final class SQLiteWordPressBulkImportSavepointPlan
{
    /**
    ...
    {
        if ($batches === []) {
            throw new \InvalidArgumentException('SQLite WordPress bulk import savepoint plan requires at least one batch');
        }
        ...
        $basePlan = SQLiteWordPressImportTransactionPlan::plan($currentRows, [], [
        ...
        foreach (array_values($batches) as $batchIndex => $batch) {
            ...
            if (!in_array($onConflict, ['rollback', 'abort'], true)) {
                throw new \InvalidArgumentException('SQLite WordPress bulk import savepoint conflict action must be rollback or abort');
            }
            ...
            $error = null;
            try {
                $stagePlan = SQLiteWordPressImportTransactionPlan::plan(array_values($visibleRows), $batch['rows'], [
                    'database_path' => $databasePath,
                    'page_size' => $pageSize,
                    'journal_mode' => $journalMode,
                    'sync_mode' => $syncMode,
    ...
    private static function batchName(array $batch, int $index): string
    {
        ...
        if ($name === '' || preg_match('/^[A-Za-z_][A-Za-z0-9_]*$/', $name) !== 1) {
            throw new \InvalidArgumentException('SQLite WordPress bulk import savepoint names must be SQL identifiers');
        }

        if (!isset($batch['rows']) || !is_array($batch['rows'])) {
            throw new \InvalidArgumentException('SQLite WordPress bulk import savepoint batch rows must be a list');
        }
    ...
    private static function rowsById(array $rows): array
    {
        ...
        foreach ($rows as $row) {
            ...
            if (!is_int($id)) {
                throw new \InvalidArgumentException('SQLite WordPress bulk import rows require integer option_id values');
            }
            $byId[$id] = $row;
        }
        ksort($byId);
