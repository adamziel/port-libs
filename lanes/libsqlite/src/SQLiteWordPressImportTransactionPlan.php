...
final class SQLiteWordPressImportTransactionPlan
{
    /**
    ...
    public static function plan(array $currentRows, array $stagedRows, array $options = []): array
    {
        ...
        if ($databasePath === '' || $databasePath[0] !== '/' || str_contains($databasePath, "\0") || str_contains($databasePath, '..')) {
            throw new \InvalidArgumentException('SQLite WordPress import transaction requires a safe absolute database path');
        }
        if ($pageSize < 512 || ($pageSize & ($pageSize - 1)) !== 0) {
            throw new \InvalidArgumentException('SQLite WordPress import transaction page size must be a power of two at least 512');
        }
        if (!in_array($journalMode, ['delete', 'truncate', 'persist'], true)) {
            throw new \InvalidArgumentException('SQLite WordPress import transaction journal mode must be delete, truncate, or persist');
        }
        if (!in_array($syncMode, ['off', 'normal', 'full'], true)) {
            throw new \InvalidArgumentException('SQLite WordPress import transaction sync mode must be off, normal, or full');
        }

        $begin = SQLiteTransactionBeginLockPlan::plan($beginSql, journalMode: 'delete');
        if (!$begin['write_lock_acquired']) {
            throw new \InvalidArgumentException('SQLite WordPress import transaction requires an immediate or exclusive write transaction');
        }
        ...
        foreach ($stagedByKey as $stage) {
            ...
            if ($targetId !== null && isset($finalById[$targetId])) {
                ...
                if ($conflictingId !== null) {
                    ...
                    if (!$replaceConflicts) {
                        throw new \LogicException("SQLite WordPress import unique option_name conflict: {$after['option_name']}");
                    }
            ...
            if ($conflictingId !== null) {
                ...
                if (!$replaceConflicts) {
                    throw new \LogicException("SQLite WordPress import unique option_name conflict: {$stage['option_name']}");
                }
                $deleted[] = $finalById[$conflictingId] + ['reason' => 'replace_conflict'];
                unset($finalById[$conflictingId]);
                $dirtyPages[self::pageForOptionId($conflictingId)] = true;
