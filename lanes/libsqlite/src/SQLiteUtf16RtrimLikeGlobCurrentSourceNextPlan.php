...
final class SQLiteUtf16RtrimLikeGlobCurrentSourceNextPlan
{
     ...
     */
    public static function keyValueRowKeyOperatorSwitchPlan(
        array $currentRows,
        array $nextRows,
    ...
    private static function scanRows(array $rows, string $operator, string $pattern, ?array $range, ?string $escape, bool $caseSensitiveLike): array
    {
        ...
        foreach ($rows as $row) {
            if (!isset($row['setting_id']) || !is_int($row['setting_id'])) {
                throw new \InvalidArgumentException('SQLite UTF-16 RTRIM LIKE/GLOB rows require integer setting_id');
            }
            if (!array_key_exists('key_name_bytes', $row) || !is_string($row['key_name_bytes'])) {
                throw new \InvalidArgumentException('SQLite UTF-16 RTRIM LIKE/GLOB rows require key_name_bytes');
            }
