...
final class SQLiteUpdateDeleteReturningSql
{
    ...
    private static function evaluateLimitScalarFunction(string $function, string $arguments): int|float|string|SQLiteBlobValue|null
    {
        ...
        if ($function === 'concat' && count($parts) < 1) {
            throw new \InvalidArgumentException('SQLite UPDATE/DELETE LIMIT concat() needs at least one argument');
        }
        if ($function === 'concat_ws' && count($parts) < 2) {
            throw new \InvalidArgumentException('SQLite UPDATE/DELETE LIMIT concat_ws() needs at least two arguments');
        }
        ...
        if ($function === 'random' && count($parts) !== 0) {
            throw new \InvalidArgumentException('SQLite UPDATE/DELETE LIMIT random() needs no arguments');
        }
        ...
        if (($function === 'unistr' || $function === 'unistr_quote') && count($parts) !== 1) {
            throw new \InvalidArgumentException("SQLite UPDATE/DELETE LIMIT {$function}() needs one argument");
        }
        ...
        if (in_array($function, ['changes', 'total_changes', 'last_insert_rowid'], true) && count($parts) !== 0) {
            throw new \InvalidArgumentException("SQLite UPDATE/DELETE LIMIT {$function}() needs no arguments");
        }
        ...
        if (in_array($function, ['changes', 'total_changes', 'last_insert_rowid'], true)) {
            return SQLiteConnectionCounters::initial()->sqlFunctionArguments($function, []);
        }
        ...
        if ($function === 'random') {
            $value = SQLiteCoreScalarFunction::sqlFunctionArguments($function, []);
            if (!is_int($value)) {
                throw new \InvalidArgumentException('SQLite UPDATE/DELETE LIMIT random() returned an unsupported value');
            }
        ...
        if ($function === 'unistr' || $function === 'unistr_quote') {
            $value = SQLiteCoreScalarFunction::sqlFunctionArguments($function, $values);
            if (!is_int($value) && !is_float($value) && !is_string($value) && $value !== null) {
                throw new \InvalidArgumentException("SQLite UPDATE/DELETE LIMIT {$function}() returned an unsupported value");
            }
    ...
    private static function limitJsonPathArgument(int|float|string|SQLiteBlobValue|null $value, string $function): ?string
    {
        ...
        if (!is_string($value)) {
            throw new \InvalidArgumentException("SQLite UPDATE/DELETE LIMIT {$function}() path argument must be text or NULL");
        }
