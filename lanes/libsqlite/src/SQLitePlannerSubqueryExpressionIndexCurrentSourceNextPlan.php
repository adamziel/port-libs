...
final class SQLitePlannerSubqueryExpressionIndexCurrentSourceNextPlan
{
    ...
    /**
         ...
         */
        public static function materializeSubqueryExpressionIndex(array $preparedSource, array $currentSource, array $predicate, array $neededColumns = []): array
        {
            $normalized = self::normalizePredicate($predicate);
            $prepared = self::sourcePlan($preparedSource, $normalized, $neededColumns);
            $current = self::sourcePlan($currentSource, $normalized, $neededColumns);
            ...
            return [
                ...
                'cursorTape' => self::cursorTape($selected, $normalized, $stale ? 'current' : 'prepared', $usable, $neededColumns),
                'currentSourceFence' => [
                    'schemaCookie' => $currentCookie,
                    'stat4Generation' => $currentGeneration,
                    'indexSignature' => $currentSignature,
                    'subquerySignature' => self::subquerySignature($normalized),
                ],
                ...
                'dependencies' => [
                    ...
                    'sqlite-subquery-expression-index-current-source',
                ],
                'dependency_closure' => 'no new support component needed; canonical planner composes native expression-index metadata with bounded IN-subquery key materialization',
                'non_overlap' => 'avoids accepted subquery-covering partial indexes and accepted range-cost/expression ORDER BY work by routing subquery-produced expression keys through the current-source expression index fence',
            ];
        }
        ...
        private static function sourcePlan(array $source, array $predicate, array $neededColumns): array
        {
            ...
            foreach ($indexes as $index) {
                ...
                $expression = self::indexExpression($index);
                if ($expression === null) {
                    continue;
                }
                $expressionMatched = self::expressionSignature($expression) === self::expressionSignature($predicate['left']);
                if (!$expressionMatched) {
                    continue;
                }

                $collationMatched = strcasecmp(self::stringValue($expression, 'collation', 'BINARY'), $predicate['subquery']['collation']) === 0;
                $affinityMatched = strtoupper(self::stringValue($expression, 'affinity', 'TEXT')) === $predicate['subquery']['affinity'];
                $partialImplied = self::partialPredicateImplied($index, $predicate);
                if (!$partialImplied) {
                    continue;
                }
                ...
                $estimatedRows = max(1, min(self::nonNegativeInt($index, 'estimatedRows', count($predicate['values']) * 4), count($predicate['values'])));
                $cost = $estimatedRows + ($covering ? 0 : 10) + ($collationMatched ? 0 : 100) + ($affinityMatched ? 0 : 100);
                $candidates[] = [
                    'usable' => true,
                    'name' => self::stringValue($index, 'name', 'expression-index'),
                    'rootPage' => self::nullableInt($index, 'rootPage'),
                    ...
                    'estimatedCost' => $cost,
                    'rangeFence' => self::rangeFence($predicate['typedValues'], (bool) ($expression['descending'] ?? false)),
                    'nextSource' => $covering ? 'expression-index-covering' : 'table-rowid-lookup',
                    'deferredTableLookup' => !$covering,
                    'detail' => 'SEARCH ' . self::stringValue($index, 'name', 'expression-index')
         ...
         * @return array<string,mixed>
         */
        private static function normalizePredicate(array $predicate): array
        {
            if (strtoupper(self::stringValue($predicate, 'operator')) !== 'IN_SUBQUERY') {
            ...
            $keyColumn = self::stringValue($subquery, 'keyColumn', 'key');
            $collation = strtoupper(self::stringValue($subquery, 'collation', 'BINARY'));
            $affinity = strtoupper(self::stringValue($subquery, 'affinity', 'TEXT'));
            ...
            foreach ($rows as $row) {
                ...
                $typed = self::applyAffinity($value, $affinity, $collation);
        ...
        private static function indexExpression(array $index): ?array
        {
        ...
        {
            return strtolower(self::stringValue($expression, 'function'))
                . '(' . strtolower(self::stringValue($expression, 'column')) . ')';
        }
         ...
         * @param array<string,mixed> $predicate
         */
        private static function partialPredicateImplied(array $index, array $predicate): bool
        {
            ...
            if (preg_match("/lower\s*\(\s*([a-zA-Z_][a-zA-Z0-9_]*)\s*\)\s*>=\s*'([^']*)'/i", $partial, $match) === 1) {
                if (self::expressionSignature($predicate['left']) !== 'lower(' . strtolower($match[1]) . ')') {
                    return false;
                }
        ...
        private static function rangeFence(array $typedValues, bool $descending): array
        {
        ...
        private static function applyAffinity(mixed $value, string $affinity, string $collation): array
        {
            $type = 'text';
            if ($affinity === 'INTEGER') {
                if (is_numeric($value)) {
        ...
        private static function subquerySignature(array $normalized): string
        {
        ...
        private static function stringValue(array $value, string $key, string $default = ''): string
        {
        ...
        /** @param array<string,mixed> $value */
        private static function nonNegativeInt(array $value, string $key, int $default = 0): int
        {
            $candidate = $value[$key] ?? $default;
         ...
         * @return list<mixed>
         */
        private static function listValue(array $value, string $key): array
        {
            $candidate = $value[$key] ?? [];
         ...
         * @return list<string>
         */
        private static function stringList(array $value, string $key): array
        {
            $candidate = self::listValue($value, $key);
