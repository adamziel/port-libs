...
final class SQLiteWordPressMultisiteImportSavepointPlan
{
    /**
    ...
    {
        if ($sites === []) {
            throw new \InvalidArgumentException('SQLite WordPress multisite import savepoint plan requires at least one site');
        }
        ...
        if (!is_array($globalBatches)) {
            throw new \InvalidArgumentException('SQLite WordPress multisite global batches must be a list');
        }
        ...
        foreach (array_values($sites) as $siteIndex => $site) {
            ...
            if (isset($sitePlans[$blogId])) {
                throw new \InvalidArgumentException("Duplicate WordPress multisite blog_id {$blogId}");
            }
            ...
            if (!is_array($batches)) {
                throw new \InvalidArgumentException('SQLite WordPress multisite site batches must be a list');
            }
            ...
            try {
                $sitePlan = SQLiteWordPressBulkImportSavepointPlan::plan($site['current_rows'], $prefixedBatches, [
        ...
        if ($globalBatches !== []) {
            ...
            $globalPlan = SQLiteWordPressBulkImportSavepointPlan::plan([], $prefixedGlobal, [
                'database_path' => $databasePath,
                'page_size' => $pageSize,
                'journal_mode' => $journalMode,
                'sync_mode' => $syncMode,
        ...
        return [
            ...
            'dependencies' => [
                'sqlite-wordpress-multisite-import-savepoint-current',
                'sqlite-wordpress-bulk-import-savepoint-current',
    ...
    private static function blogId(array $site): int
    {
        ...
        if (!is_int($blogId) && !(is_string($blogId) && ctype_digit($blogId))) {
            throw new \InvalidArgumentException('SQLite WordPress multisite blog_id must be a positive integer');
        }
        $blogId = (int) $blogId;
        if ($blogId <= 0) {
            throw new \InvalidArgumentException('SQLite WordPress multisite blog_id must be positive');
        }
        if (!isset($site['current_rows']) || !is_array($site['current_rows'])) {
            throw new \InvalidArgumentException('SQLite WordPress multisite current rows must be a list');
        }
