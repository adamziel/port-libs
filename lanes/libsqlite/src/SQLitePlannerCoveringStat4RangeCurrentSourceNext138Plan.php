...
final class SQLitePlannerCoveringStat4RangeCurrentSourceNext138Plan
{
    /**
    ...
    public static function materialize(
        ...
        $base = SQLitePlannerCoveringRangeOrderCurrentSourceNext119Plan::materialize(
            $preparedSource,
            $currentSource,
            $predicate,
            $orderBy,
        ...
        $rows = self::coveredRows($source, $plan, $predicate, $neededColumns);
        $bucketed = self::stat4Buckets($plan, $rows);
        ...
        return array_replace($base, [
            ...
            'dependencies' => array_values(array_unique(array_merge(
                ...
                [
                    'SQLitePlannerCoveringRangeOrderCurrentSourceNext119Plan',
    ...
    private static function coveredRows(array $source, array $plan, array $predicate, array $neededColumns): array
    {
        ...
        usort($covered, static function (array $left, array $right): int {
            $comparison = self::compare($left['rangeKey'] ?? null, $right['rangeKey'] ?? null);
            if ($comparison !== 0) {
                return $comparison;
            }
    ...
    private static function rowSatisfiesPointTerms(array $row, array $predicate): bool
    {
        foreach (self::flattenAndTerms($predicate) as $term) {
            ...
            if (self::compare($row[$column], $term['right'] ?? null) !== 0) {
                return false;
            }
        }
     ...
     * @return list<array<string,mixed>>
     */
    private static function stat4Buckets(array $plan, array $rows): array
    {
