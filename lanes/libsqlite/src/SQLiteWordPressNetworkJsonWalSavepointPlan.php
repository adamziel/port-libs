...
final class SQLiteWordPressNetworkJsonWalSavepointPlan
{
    /**
    ...
    {
        if ($sites === []) {
            throw new \InvalidArgumentException('SQLite WordPress network JSON WAL savepoint plan requires at least one site');
        }
        ...
        if (!is_array($globalImports)) {
            throw new \InvalidArgumentException('SQLite WordPress network JSON WAL global imports must be a list');
        }
        ...
        foreach (array_values($sites) as $siteIndex => $site) {
            ...
            if (isset($sitePlans[$blogId])) {
                throw new \InvalidArgumentException("Duplicate WordPress network blog_id {$blogId}");
            }

            $imports = $site['json_imports'] ?? null;
            if (!is_array($imports)) {
                throw new \InvalidArgumentException('SQLite WordPress network JSON WAL site imports must be a list');
            }
            ...
            try {
                $sitePlan = SQLiteWordPressJsonImportWalSavepointPlan::plan($site['current_rows'], $prefixedImports, [
        ...
        if ($globalImports !== []) {
            ...
            $globalPlan = SQLiteWordPressJsonImportWalSavepointPlan::plan([], $prefixedGlobal, [
                'database_path' => $databasePath,
                'page_size' => $pageSize,
                'journal_mode' => $journalMode,
                'sync_mode' => $syncMode,
    ...
    private static function blogId(array $site): int
    {
        ...
        if (!is_int($blogId) && !(is_string($blogId) && ctype_digit($blogId))) {
            throw new \InvalidArgumentException('SQLite WordPress network JSON WAL blog_id must be a positive integer');
        }
        $blogId = (int) $blogId;
        if ($blogId <= 0) {
        ...
        if (!isset($site['current_rows']) || !is_array($site['current_rows'])) {
            throw new \InvalidArgumentException('SQLite WordPress network JSON WAL current rows must be a list');
        }
