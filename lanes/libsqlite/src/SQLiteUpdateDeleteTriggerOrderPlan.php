...
final class SQLiteUpdateDeleteTriggerOrderPlan
{
    /**
     ...
     * @return array{rows:list<array<string,mixed>>,audit:list<array<string,mixed>>,changes:int,updated:list<array<string,mixed>>,deleted:list<array<string,mixed>>,visited:list<int|string>}
     */
    ...
    private static function selectedIndexes(array $rows, callable $where, array $orderBy, ?int $limit): array
    {
        if ($limit !== null && $limit < 0) {
            throw new \InvalidArgumentException('SQLite trigger order LIMIT cannot be negative');
        }
            ...
            usort($selected, static function (int $left, int $right) use ($rows, $orderBy): int {
                foreach ($orderBy as $term) {
                    ...
                    if (!in_array($direction, ['asc', 'desc'], true)) {
                        throw new \InvalidArgumentException('SQLite trigger order ORDER BY direction is unsupported');
                    }
    ...
    private static function validateTrigger(array $trigger): void
    {
        if (!isset($trigger['name']) || !is_string($trigger['name']) || $trigger['name'] === '') {
            throw new \InvalidArgumentException('SQLite trigger name is required');
        }
        if (!in_array(strtolower((string) ($trigger['timing'] ?? '')), ['before', 'after'], true)) {
            throw new \InvalidArgumentException('SQLite trigger timing is unsupported');
        }
    ...
    {
        if ($assignments === []) {
            throw new \InvalidArgumentException('SQLite trigger order UPDATE assignments cannot be empty');
        }
    ...
    private static function validateIdentifier(string $identifier, string $label): void
    {
        if (preg_match('/^[A-Za-z_][A-Za-z0-9_]*$/', $identifier) !== 1) {
            throw new \InvalidArgumentException("SQLite trigger order {$label} is malformed");
        }
    }
}
