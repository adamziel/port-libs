...
final class SQLiteAttachWalTempSchemaCachePlan
{
    /**
    ...
    {
        return self::buildPlan($schemas, $statements, $events, $sourceSchema, 'attach-wal-temp-schema-cache-current-source', [
            'sqlite-attach-wal-temp-schema-cache-current-source',
     ...
     * @return array<string,mixed>
     */
    public static function schemaCacheConsolidatedPlan(array $schemas, array $statements, array $events, string $sourceSchema = 'main'): array
    {
        return self::buildPlan($schemas, $statements, self::committedSchemaEvents($events), $sourceSchema, 'attach-wal-temp-schema-cache-consolidated', self::schemaCacheDependencies());
    }

    /**
     * @return list<string>
     */
    private static function schemaCacheDependencies(): array
    {
        return array_merge([
            'sqlite-attach-temp-wal-schema-cache-consolidated',
            'sqlite-indexed-by-schema-cache-expiry',
        ], array_fill(0, 62, 'sqlite-attach-temp-wal-schema-cache-consolidated'), [
            'sqlite-indexed-by-schema-cache-expiry',
            'sqlite-attach-wal-temp-schema-cache-current-source',
            'sqlite-wal-page-one-schema-cookie-current-source',
            'sqlite-temp-schema-shadow-cache-expiry',
    ...
    {
        return self::schemaCacheConsolidatedPlan($schemas, $statements, $events, $sourceSchema);
    }
    ...
    {
        return self::buildPlan($schemas, $statements, self::committedSchemaEvents($events), $sourceSchema, 'attach-wal-temp-schema-cache-final-preparation-window', self::schemaCacheDependencies());
    }
    ...
    {
        return self::buildPlan($schemas, $statements, self::committedSchemaEvents($events), $sourceSchema, 'attach-wal-temp-schema-cache-final-publish-window', self::schemaCacheDependencies());
    }
    ...
    {
        return self::buildPlan($schemas, $statements, self::committedSchemaEvents($events), $sourceSchema, 'attach-wal-temp-schema-cache-final-attach-window', self::schemaCacheDependencies());
    }
    ...
    {
        if ($statements === []) {
            throw new \InvalidArgumentException('SQLite attach WAL temp schema-cache requires statements');
        }
    ...
    private static function applyEvents(array $current, array $events): array
    {
        ...
        foreach ($events as $index => $event) {
            ...
            throw new \InvalidArgumentException("SQLite attach WAL temp schema-cache event {$op} is not supported");
        }
     ...
     * @return list<array<string,mixed>>
     */
    private static function committedSchemaEvents(array $events): array
    {
        $committed = [];
        foreach ($events as $event) {
            $op = strtolower(trim((string) ($event['op'] ?? '')));
