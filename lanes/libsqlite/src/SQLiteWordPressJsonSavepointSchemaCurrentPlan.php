...
final class SQLiteWordPressJsonSavepointSchemaCurrentPlan
{
    /**
    ...
    {
        if ($batches === []) {
            throw new \InvalidArgumentException('SQLite WordPress JSON savepoint schema-current plan requires at least one batch');
        }
        ...
        foreach (array_values($batches) as $index => $batch) {
            ...
            $subPlan = SQLiteWordPressSchemaJsonSavepointWalPlan::plan(array_values($visibleRows), [$batch], $importOptions);
    ...
    private static function nonNegativeInt(mixed $value, string $name): int
    {
        if (!is_int($value) || $value < 0) {
            throw new \InvalidArgumentException("SQLite WordPress JSON schema-current {$name} must be a non-negative integer");
        }
    ...
    private static function rowsById(array $rows): array
    {
        ...
        foreach ($rows as $row) {
            ...
            if (!is_int($id)) {
                throw new \InvalidArgumentException('SQLite WordPress JSON schema-current rows require integer option_id values');
            }
            $byId[$id] = $row;
        }
        ksort($byId);
    ...
    {
        if (!is_array($rows)) {
            throw new \InvalidArgumentException('SQLite WordPress JSON schema-current schema rows must be a list');
        }

        $byName = [];
        foreach ($rows as $row) {
            if (!is_array($row)) {
                throw new \InvalidArgumentException('SQLite WordPress JSON schema-current schema rows must be arrays');
            }
    ...
    private static function savepointName(array $batch, int $index): string
    {
        ...
        if ($name === '' || preg_match('/^[A-Za-z_][A-Za-z0-9_]*$/', $name) !== 1) {
            throw new \InvalidArgumentException('SQLite WordPress JSON schema-current savepoint names must be SQL identifiers');
        }
    ...
    private static function applySchemaChanges(array $schemaRows, mixed $changes): array
    {
        ...
        if ($violations !== []) {
            return self::invalidSchemaChange('SQLite WordPress JSON schema-current DDL failed schema validation', $violations);
        }
