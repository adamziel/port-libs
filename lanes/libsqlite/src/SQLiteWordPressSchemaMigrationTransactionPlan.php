...
final class SQLiteWordPressSchemaMigrationTransactionPlan
{
    /**
    ...
    public static function plan(string $tableName, array $columns, array $currentRows, array $options = []): array
    {
        ...
        $databasePath = (string) ($options['database_path'] ?? '/tmp/wordpress.sqlite');
        ...
        if ($databasePath === '' || $databasePath[0] !== '/' || str_contains($databasePath, "\0") || str_contains($databasePath, '..')) {
            throw new \InvalidArgumentException('SQLite WordPress schema migration requires a safe absolute database path');
        }
        if ($pageSize < 512 || ($pageSize & ($pageSize - 1)) !== 0) {
            throw new \InvalidArgumentException('SQLite WordPress schema migration page size must be a power of two at least 512');
        }
        if ($schemaVersion < 0) {
            throw new \InvalidArgumentException('SQLite WordPress schema migration schema_version must be non-negative');
        }
        if ($columns === []) {
            throw new \InvalidArgumentException('SQLite WordPress schema migration requires at least one column');
        }

        $begin = SQLiteTransactionBeginLockPlan::plan($beginSql, journalMode: 'delete');
        if (!$begin['write_lock_acquired']) {
            throw new \InvalidArgumentException('SQLite WordPress schema migration requires an immediate or exclusive write transaction');
        }
        ...
        return [
            ...
            'dependencies' => [
                'sqlite-wordpress-schema-migration-transaction',
    ...
    private static function normalizeIdentifier(string $identifier, string $label): string
    {
        if (!preg_match('/^[A-Za-z_][A-Za-z0-9_]*$/', $identifier)) {
            throw new \InvalidArgumentException("SQLite WordPress schema migration {$label} must be a bounded identifier");
        }
    ...
    private static function normalizeColumn(array $column): array
    {
        ...
        if ($type === '' || !preg_match('/^[A-Z0-9_ ()]+$/', $type)) {
            throw new \InvalidArgumentException('SQLite WordPress schema migration column type must be bounded SQL text');
        }
    ...
    {
        if (!is_array($sqlList)) {
            throw new \InvalidArgumentException("SQLite WordPress schema migration {$label} list must be an array");
        }

        $normalized = [];
        foreach ($sqlList as $sql) {
            if (!is_string($sql) || trim($sql) === '' || str_contains($sql, "\0")) {
                throw new \InvalidArgumentException("SQLite WordPress schema migration {$label} SQL must be non-empty text");
            }
            $normalized[] = trim($sql);
        }
    ...
    private static function normalizeCopyExpressions(mixed $expressions, array $columns): array
    {
        if (!is_array($expressions)) {
            throw new \InvalidArgumentException('SQLite WordPress schema migration copy expressions must be an array');
        }
        ...
        foreach ($expressions as $column => $expression) {
            if (!is_string($column) || !isset($known[$column])) {
                throw new \InvalidArgumentException('SQLite WordPress schema migration copy expression targets a migrated column');
            }
            if (!is_string($expression) || trim($expression) === '' || str_contains($expression, "\0") || str_contains($expression, ';')) {
                throw new \InvalidArgumentException('SQLite WordPress schema migration copy expression must be bounded SQL expression text');
            }
