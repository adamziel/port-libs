...
final class SQLiteCompoundWindowRecursiveAffinityCurrentSourceNextPlan
{
    ...
    /**
         ...
         */
        public static function pageNext147(string $sql, array $currentTables, array $nextTables, int $limit, int $offset = 0, ?array $cursor = null): array
        {
            if ($limit <= 0) {
            ...
            $currentSignature = self::signatureNext147($base['currentSignatures']);
            $nextSignature = self::signatureNext147($base['nextSignatures']);
            if ($cursor !== null) {
                self::validateCursorNext147($cursor, $offset, $currentSignature, $nextSignature);
            }

            $currentRows = self::sliceNext147($base['currentRows'], $limit, $offset);
            $nextRows = self::sliceNext147($base['nextRows'], $limit, $offset);
         ...
         */
        private static function sliceNext147(array $rows, int $limit, int $offset): array
        {
         ...
         */
        private static function signatureNext147(array $rowSignatures): string
        {
         ...
         */
        private static function validateCursorNext147(array $cursor, int $offset, string $currentSignature, string $nextSignature): void
        {
