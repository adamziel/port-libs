...
final class SQLiteRowValueUpdateDeleteReturningSavepointCurrentSourceNext178Plan
{
    /**
    ...
    public static function execute(
        ...
        [$failedCurrent, $savepointExecuted, $savepointReturning, $rollbackReason, $rollbackOrdinal] = self::runSavepointUntilRollback(
            $savepointImage,
            $savepointStatements,
            $uniqueConstraints,
            $rowIdColumn,
        ...
        return [
            ...
            'attempted_changes_before_rollback' => self::changeCount(array_merge($outerExecuted, $savepointExecuted)),
     ...
     */
    private static function runSavepointUntilRollback(array $tables, array $statements, array $uniqueConstraints, string $rowIdColumn): array
    {
        $current = $tables;
        $executed = [];
        $yielded = [];
        ...
        foreach ($statements as $ordinal => $sql) {
            ...
            $current = $result['tables'];
            $executed[] = self::statementSummary('savepoint', $ordinal, $sql, $result, $before, $rowIdColumn, null);
            $yielded[] = [
                'phase' => 'savepoint',
                'ordinal' => $ordinal,
                'action' => $result['action'],
     ...
     * @return array<string,mixed>
     */
    private static function statementSummary(string $phase, int $ordinal, string $sql, array $result, array $before, string $rowIdColumn, ?array $failedConflict): array
    {
        return [
            'phase' => $phase,
            'ordinal' => $ordinal,
