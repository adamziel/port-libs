...
final class SQLitePagerMasterJournalReaderCacheCurrentSourceNext207Plan
{
    /**
    ...
    public static function plan(
        ...
        $base = SQLitePagerMasterJournalReaderCacheCurrentSourceNext206Plan::plan(
            ...
            $pageSize,
            $recoveredPages,
            self::stripDatabaseToken($readerCache),
        ...
        foreach ($base['next_reads'] as &$read) {
            ...
            if (!$ticketCurrent || $pageInvalidated) {
                ...
                $read['source'] = 'master-journal-reader-cache-database-file-token-fence-current-source-next207';
                ...
                $base['operations'][] = [
                    'op' => 'invalidate_reader_cache_database_file_token_after_current_source_next207',
                    'page_number' => $read['page_number'],
                    'reader_id' => $readerId,
    ...
    private static function cacheDatabaseTokens(array $cache): array
    {
        ...
        foreach ($cache as $pageNumber => $entry) {
            ...
            if (!is_int($pageNumber) || $pageNumber < 1 || !is_string($token) || $token === '') {
                throw new \InvalidArgumentException('SQLite pager master-journal reader-cache next207 cache entries require database file tokens');
            }
    ...
    private static function readDatabaseTokens(array $reads): array
    {
        ...
        foreach ($reads as $read) {
            ...
            if ($readerId === '' || !is_string($token) || $token === '') {
                throw new \InvalidArgumentException('SQLite pager master-journal reader-cache next207 reads require reader ids and database file tokens');
            }
