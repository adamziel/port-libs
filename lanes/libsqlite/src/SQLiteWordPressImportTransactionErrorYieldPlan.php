...
final class SQLiteWordPressImportTransactionErrorYieldPlan
{
    /**
    ...
    public static function plan(array $currentRows, array $stagedRows, array $options = []): array
    {
        ...
        if ($statementPrefix === '' || str_contains($statementPrefix, "\0")) {
            throw new \InvalidArgumentException('SQLite WordPress import transaction error yield statement prefix must be non-empty text');
        }

        $begin = SQLiteTransactionBeginLockPlan::plan($beginSql, journalMode: 'delete');
        if (!$begin['write_lock_acquired']) {
            throw new \InvalidArgumentException('SQLite WordPress import transaction error yield requires an immediate or exclusive write transaction');
        }
        ...
        return [
            ...
            'dependencies' => [
                'sqlite-wordpress-import-transaction-error-yield-current-next29',
    ...
    private static function assertSafeDatabasePath(string $databasePath): void
    {
        if ($databasePath === '' || $databasePath[0] !== '/' || str_contains($databasePath, "\0") || str_contains($databasePath, '..')) {
            throw new \InvalidArgumentException('SQLite WordPress import transaction error yield requires a safe absolute database path');
        }
    }
    ...
    private static function assertPageSize(int $pageSize): void
    {
        if ($pageSize < 512 || ($pageSize & ($pageSize - 1)) !== 0) {
            throw new \InvalidArgumentException('SQLite WordPress import transaction error yield page size must be a power of two at least 512');
        }
    }
