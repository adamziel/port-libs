...
final class SQLiteWordPressMultisiteJsonWalImportPlan
{
    /**
    ...
    {
        if ($imports === []) {
            throw new \InvalidArgumentException('SQLite WordPress multisite JSON WAL import requires at least one batch');
        }

        $pageSize = (int) ($options['page_size'] ?? 1024);
        if ($pageSize < 512 || ($pageSize & ($pageSize - 1)) !== 0) {
            throw new \InvalidArgumentException('SQLite WordPress multisite JSON WAL import page size must be a power of two at least 512');
        }

        $databasePath = (string) ($options['database_path'] ?? '/tmp/wp-multisite-json-wal.sqlite');
        if ($databasePath === '') {
            throw new \InvalidArgumentException('SQLite WordPress multisite JSON WAL import requires a database path');
        }
    ...
    private static function normalizeImportRow(array $row, string $scope, array $state): array
    {
        ...
        if (!is_string($name) || $name === '' || str_contains($name, "\0")) {
            throw new \InvalidArgumentException('SQLite WordPress multisite JSON import option_name/meta_key must be non-empty text');
        }
        ...
        if (!is_string($value) && !$value instanceof SQLiteBlobValue && !$value instanceof SQLiteJsonSubtypeValue) {
            throw new \InvalidArgumentException('SQLite WordPress multisite JSON import value must be JSON text, JSON subtype, or JSONB blob');
        }
        if (self::requiresJsonValidation($name) && self::jsonValid($value) !== true) {
            throw new \InvalidArgumentException("SQLite WordPress multisite JSON import value for {$name} is malformed JSON");
        }
        ...
        if (!$network && !in_array($autoload, ['yes', 'no', 'auto', 'on', 'off'], true)) {
            throw new \InvalidArgumentException('SQLite WordPress multisite JSON import autoload must be a WordPress autoload token');
        }
    ...
    private static function batchName(array $import, int $index): string
    {
        ...
        if ($name === '' || preg_match('/^[A-Za-z_][A-Za-z0-9_]*$/', $name) !== 1) {
            throw new \InvalidArgumentException('SQLite WordPress multisite JSON WAL savepoint names must be SQL identifiers');
        }
    ...
    private static function scope(array $row): string
    {
        ...
        if (!in_array($scope, ['blog', 'network'], true)) {
            throw new \InvalidArgumentException('SQLite WordPress multisite JSON WAL import scope must be blog or network');
        }
    ...
    private static function jsonRows(mixed $json, string $path, string $scope, array $import): array
    {
        if (!is_string($json) && !$json instanceof SQLiteBlobValue && !$json instanceof SQLiteJsonSubtypeValue) {
            return self::invalidJson($path, 'SQLite WordPress multisite JSON WAL import requires JSON text, JSON subtype, or JSONB blob');
        }

        try {
            $value = $json instanceof SQLiteJsonSubtypeValue ? $json->json : $json;
            if (self::jsonValid($value) !== true) {
                return self::invalidJson($path, 'SQLite WordPress multisite JSON WAL import source is malformed JSON');
            }
            $extracted = SQLiteJsonExtract::extract($value, $path);
            if ($extracted === null) {
                return self::invalidJson($path, 'SQLite WordPress multisite JSON WAL import path did not match any rows');
            }
            $decoded = is_string($extracted) ? json_decode($extracted, true, 1001, JSON_THROW_ON_ERROR) : $extracted;
            if (!is_array($decoded)) {
                throw new \InvalidArgumentException('SQLite WordPress multisite JSON WAL import path must resolve to rows');
            }
            $rows = array_is_list($decoded) ? $decoded : [$decoded];
            $normalized = [];
            foreach ($rows as $row) {
                if (!is_array($row)) {
                    throw new \InvalidArgumentException('SQLite WordPress multisite JSON WAL import rows must be objects');
                }
    ...
    private static function positiveInt(mixed $value, string $field): int
    {
        if (!is_int($value) && !(is_string($value) && ctype_digit($value))) {
            throw new \InvalidArgumentException("SQLite WordPress multisite JSON import {$field} must be a positive integer");
        }
        $int = (int) $value;
        if ($int <= 0) {
            throw new \InvalidArgumentException("SQLite WordPress multisite JSON import {$field} must be positive");
        }
