...
final class SQLitePagerMasterJournalReaderCacheCurrentSourceNext167Plan
{
    /**
     * @param array<int,string> $currentPages
     * @param array<int,array{image:string,source_id?:string,epoch?:int,reader_id?:string,master_generation?:int,master_deleted?:bool,dirty?:bool,pinned?:bool,shared?:bool}> $readerCache
    ...
    public static function plan(
        ...
        if ($databasePath === '' || $masterJournalPath === '' || $currentSourceId === '') {
            throw new \InvalidArgumentException('SQLite pager master-journal reader-cache next167 requires non-empty paths and source id');
        }
        if (trim($currentMasterJournalBytes) === '') {
            throw new \InvalidArgumentException('SQLite pager master-journal reader-cache next167 requires current master-journal bytes');
        }
        if ($pageSize < 512 || ($pageSize & ($pageSize - 1)) !== 0) {
            throw new \InvalidArgumentException('SQLite pager master-journal reader-cache next167 page size must be a power of two at least 512');
        }
        if ($currentEpoch < 1 || $currentMasterGeneration < 1) {
            throw new \InvalidArgumentException('SQLite pager master-journal reader-cache next167 epoch and generation must be positive');
        }
        ...
        foreach ($readerCache as $pageNumber => $entry) {
            ...
            if ($reason !== null) {
                ...
                $operations[] = [
                    'op' => 'invalidate_reader_cache_master_generation_ticket',
                    'page_number' => $pageNumber,
                    'reader_id' => $entry['reader_id'],
                    'reason' => $reason,
                ];
                ...
                $refreshed[$pageNumber] = [
                    ...
                    'reader_id' => $entry['reader_id'],
                    'shared' => $entry['shared'],
                    'source' => 'reader-cache-refreshed-current-master-generation',
                ];
                $operations[] = [
                    'op' => 'refresh_reader_cache_from_current_master_generation',
                    'page_number' => $pageNumber,
                    'reader_id' => $entry['reader_id'],
                ];
            } else {
                $retained[$pageNumber] = [
                    ...
                    'reader_id' => $entry['reader_id'],
                    'shared' => $entry['shared'],
                    'source' => 'reader-cache-retained-current-master-generation',
                ];
                $operations[] = [
                    'op' => 'retain_reader_cache_current_master_generation',
                    'page_number' => $pageNumber,
                    'reader_id' => $entry['reader_id'],
                ];
            }
            ...
            $rows[] = [
                ...
                'reason' => $reason ?? ($imageMatches ? 'reader_cache_matches_current_master_generation' : 'reader_cache_refreshed_from_current_master_generation'),
        ...
        foreach ($reads as $read) {
            ...
            if (!isset($currentPages[$pageNumber])) {
                throw new \InvalidArgumentException("SQLite pager master-journal reader-cache next167 read page {$pageNumber} is outside current source");
            }
            ...
            $nextReads[] = [
                ...
                'source' => is_array($cache) ? $cache['source'] : 'current-master-generation-source',
            ...
            $operations[] = [
                'op' => is_array($cache) ? 'next_reader_cache_hit_current_master_generation' : 'next_reader_reopen_current_master_generation_page',
                'reader_id' => $read['reader_id'],
                'page_number' => $pageNumber,
            ];
        }
        ...
        return [
            ...
            'source_digest' => hash('sha256', $currentSourceId . '|' . $currentEpoch . '|' . $currentMasterGeneration . '|' . ($masterJournalDeleted ? 'deleted' : 'present') . '|' . implode(',', array_keys($retained)) . '|' . implode(',', array_keys($refreshed)) . '|' . implode(',', array_column($invalidated, 'page_number'))),
            'dependencies' => [
                ...
                'sqlite-master-journal-deleted-generation-reader-ticket',
            ],
        ];
    }
    ...
    private static function normalizePages(array $pages, int $pageSize, string $label): array
    {
        ...
        foreach ($pages as $pageNumber => $image) {
            if (!is_int($pageNumber) || $pageNumber < 1) {
                throw new \InvalidArgumentException("SQLite pager master-journal reader-cache next167 {$label} page numbers must be one-based integers");
            }
            if (!is_string($image) || strlen($image) !== $pageSize) {
                throw new \InvalidArgumentException("SQLite pager master-journal reader-cache next167 {$label} page {$pageNumber} must be page-size bytes");
            }
    ...
    private static function normalizeCache(array $cache, int $pageSize): array
    {
        ...
        foreach ($cache as $pageNumber => $entry) {
            if (!is_int($pageNumber) || $pageNumber < 1) {
                throw new \InvalidArgumentException('SQLite pager master-journal reader-cache next167 cache page numbers must be one-based integers');
            }
            if (!isset($entry['image']) || !is_string($entry['image']) || strlen($entry['image']) !== $pageSize) {
                throw new \InvalidArgumentException("SQLite pager master-journal reader-cache next167 cache page {$pageNumber} must be page-size bytes");
            }
            $sourceId = isset($entry['source_id']) && is_string($entry['source_id']) ? $entry['source_id'] : '';
            $epoch = $entry['epoch'] ?? 0;
            $generation = $entry['master_generation'] ?? 0;
            if ($sourceId === '') {
                throw new \InvalidArgumentException('SQLite pager master-journal reader-cache next167 cache entries require source id');
            }
            if (!is_int($epoch) || $epoch < 1 || !is_int($generation) || $generation < 1) {
                throw new \InvalidArgumentException('SQLite pager master-journal reader-cache next167 cache entries require positive epoch and generation');
            }
    ...
    private static function normalizeReads(array $reads): array
    {
        ...
        foreach ($reads as $index => $read) {
            ...
            if ($readerId === '') {
                throw new \InvalidArgumentException('SQLite pager master-journal reader-cache next167 reads require reader id');
            }
            $pageNumber = $read['page_number'] ?? 0;
            $epoch = $read['epoch'] ?? 0;
            $generation = $read['master_generation'] ?? 0;
            if (!is_int($pageNumber) || $pageNumber < 1) {
                throw new \InvalidArgumentException("SQLite pager master-journal reader-cache next167 read {$index} page number must be one-based");
            }
            if (!isset($read['source_id']) || !is_string($read['source_id']) || $read['source_id'] === '') {
                throw new \InvalidArgumentException('SQLite pager master-journal reader-cache next167 reads require source id');
            }
            if (!is_int($epoch) || $epoch < 1 || !is_int($generation) || $generation < 1) {
                throw new \InvalidArgumentException('SQLite pager master-journal reader-cache next167 reads require positive epoch and generation');
            }
            $normalized[] = [
                'reader_id' => $readerId,
                'page_number' => $pageNumber,
