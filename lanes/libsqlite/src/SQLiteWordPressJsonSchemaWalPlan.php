...
final class SQLiteWordPressJsonSchemaWalPlan
{
    /**
    ...
    public static function currentNext(
        ...
        if ($jsonOptionNames === []) {
            throw new \InvalidArgumentException('WordPress JSON schema WAL planning requires at least one JSON option name');
        }

        $schema = SQLiteWordPressSchemaBulkImportPlan::plan($schemaSql, [], $schemaOptions);
        $schemaNames = array_map('strtolower', $schema['ordered_names']);
        if (!in_array('wp_options', $schemaNames, true)) {
            throw new \InvalidArgumentException('WordPress JSON schema WAL planning requires a wp_options schema object');
        }
        ...
        foreach ($jsonOptionNames as $name) {
            ...
            if ($normalized === '') {
                throw new \InvalidArgumentException('WordPress JSON schema WAL planning requires non-empty JSON option names');
            }
        ...
        foreach ($importRows as $row) {
            ...
            if ($name === '') {
                throw new \InvalidArgumentException('WordPress JSON schema WAL planning requires option_name');
            }
        ...
        if ($acceptedRows === []) {
            throw new \InvalidArgumentException('WordPress JSON schema WAL planning requires at least one accepted import row');
        }

        $walPlan = SQLiteWordPressOptionsWalImportPlan::currentNext(
            $wal,
            $databaseBytes,
            $databasePath,
            $currentRows,
        ...
        return [
            'status' => 'planned',
            'reason' => 'wordpress_json_schema_wal_current_next',
            ...
            'dependencies' => array_values(array_unique(array_merge(
                ...
                ['wordpress-json-schema-wal-current-next']
            ))),
        ];
    }
}
