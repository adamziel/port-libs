...
final class SQLiteWordPressJsonImportRollbackWalPlan
{
    /**
    ...
    public static function plan(array $currentRows, array $mutations, array $options): array
    {
        ...
        if ($pageSize < 512 || ($pageSize & ($pageSize - 1)) !== 0) {
            throw new \InvalidArgumentException('SQLite WordPress JSON import rollback page size must be a power of two at least 512');
        }

        $databaseBytes = $options['database_bytes'] ?? null;
        if (!is_string($databaseBytes) || $databaseBytes === '' || strlen($databaseBytes) % $pageSize !== 0) {
            throw new \InvalidArgumentException('SQLite WordPress JSON import rollback requires a page-aligned database image');
        }
        ...
        if (!is_string($walBytes)) {
            throw new \InvalidArgumentException('SQLite WordPress JSON import rollback WAL bytes must be a string');
        }
        $walState = self::walState($walBytes, $pageSize);

        $importPlan = SQLiteWordPressJsonImportSavepointPlan::plan(
            $currentRows,
            $mutations,
            [
                ...
                'transaction' => $options['transaction'] ?? 'wordpress_json_import',
            ]
        );
        ...
        if ($rollbackToFrame > $walState['frame_count']) {
            throw new \InvalidArgumentException('SQLite WordPress JSON import rollback frame is beyond the WAL byte stream');
        }
    ...
    private static function walState(string $walBytes, int $pageSize): array
    {
        if (strlen($walBytes) < 32) {
            throw new \InvalidArgumentException('SQLite WordPress JSON import rollback WAL bytes require a 32 byte header');
        }

        $frameSize = 24 + $pageSize;
        $frameBytes = strlen($walBytes) - 32;
        if ($frameBytes % $frameSize !== 0) {
            throw new \InvalidArgumentException('SQLite WordPress JSON import rollback WAL bytes have a partial frame tail');
        }
