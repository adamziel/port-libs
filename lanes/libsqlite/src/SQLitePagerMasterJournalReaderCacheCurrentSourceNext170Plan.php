...
final class SQLitePagerMasterJournalReaderCacheCurrentSourceNext170Plan
{
    /**
    ...
    public static function plan(
        ...
        if ($databasePath === '' || $masterJournalPath === '' || $currentSourceId === '') {
            throw new \InvalidArgumentException('SQLite pager master-journal reader-cache next170 requires non-empty paths and source id');
        }
        if (trim($currentMasterJournalBytes) === '') {
            throw new \InvalidArgumentException('SQLite pager master-journal reader-cache next170 requires current master-journal bytes');
        }
        if ($currentRollbackJournalBytes === '') {
            throw new \InvalidArgumentException('SQLite pager master-journal reader-cache next170 requires rollback-journal bytes');
        }
        if ($pageSize < 512 || ($pageSize & ($pageSize - 1)) !== 0) {
            throw new \InvalidArgumentException('SQLite pager master-journal reader-cache next170 page size must be a power of two at least 512');
        }
        if ($databaseBytes === '' || strlen($databaseBytes) % $pageSize !== 0) {
            throw new \InvalidArgumentException('SQLite pager master-journal reader-cache next170 database bytes must be page-size aligned');
        }
        if ($readerCache === []) {
            throw new \InvalidArgumentException('SQLite pager master-journal reader-cache next170 requires reader cache pages');
        }
        if ($readPages === [] && $writePages === []) {
            throw new \InvalidArgumentException('SQLite pager master-journal reader-cache next170 requires read or write pages');
        }
        if ($currentEpoch < 1) {
        ...
        foreach ($readerCache as $pageNumber => $entry) {
            ...
            if ($entry['dirty']) {
                ...
                $reason = 'reader_cache_master_journal_digest_mismatch_next170';
        ...
        foreach ($readPages as $pageNumber) {
            if (!isset($database[$pageNumber])) {
                throw new \InvalidArgumentException("SQLite pager master-journal reader-cache next170 read page {$pageNumber} is outside the database image");
            }
        ...
        foreach ($writePages as $pageNumber => $image) {
            if (!isset($database[$pageNumber])) {
                throw new \InvalidArgumentException("SQLite pager master-journal reader-cache next170 write page {$pageNumber} is outside the database image");
            }
        ...
        return [
            ...
            'dependencies' => [
                ...
                'sqlite-pager-reader-cache-rollback-journal-source-fence-next170',
    ...
    private static function normalizeImages(array $images, int $pageSize, string $label, bool $allowEmpty): array
    {
        if ($images === [] && !$allowEmpty) {
            throw new \InvalidArgumentException("SQLite pager master-journal reader-cache next170 requires {$label} pages");
        }
        $normalized = [];
        foreach ($images as $pageNumber => $image) {
            if (!is_int($pageNumber) || $pageNumber < 1) {
                throw new \InvalidArgumentException("SQLite pager master-journal reader-cache next170 {$label} page numbers must be one-based integers");
            }
            if (!is_string($image) || strlen($image) !== $pageSize) {
                throw new \InvalidArgumentException("SQLite pager master-journal reader-cache next170 {$label} page {$pageNumber} must be page-size bytes");
            }
    ...
    private static function normalizeReaderCache(array $cache, int $pageSize): array
    {
        ...
        foreach ($cache as $pageNumber => $entry) {
            if (!is_int($pageNumber) || $pageNumber < 1) {
                throw new \InvalidArgumentException('SQLite pager master-journal reader-cache next170 cache page numbers must be one-based integers');
            }
            if (!isset($entry['image']) || !is_string($entry['image']) || strlen($entry['image']) !== $pageSize) {
                throw new \InvalidArgumentException("SQLite pager master-journal reader-cache next170 cache page {$pageNumber} must be page-size bytes");
            }
            ...
            if ($sourceId === '' || $masterDigest === '' || $journalDigest === '') {
                throw new \InvalidArgumentException('SQLite pager master-journal reader-cache next170 cache entries require source id and journal digests');
            }
            $epoch = $entry['epoch'] ?? 0;
            ...
            foreach (['epoch' => $epoch, 'journal_page_count' => $journalPageCount, 'journal_initial_page_count' => $initialPageCount] as $name => $value) {
                if (!is_int($value) || $value < 0 || ($name === 'epoch' && $value < 1)) {
                    throw new \InvalidArgumentException("SQLite pager master-journal reader-cache next170 cache {$name} must be a valid integer");
                }
            }
            $journalPageNumbers = $entry['journal_page_numbers'] ?? null;
            if (!is_array($journalPageNumbers)) {
                throw new \InvalidArgumentException('SQLite pager master-journal reader-cache next170 cache journal page numbers must be a list');
            }
            foreach ($journalPageNumbers as $number) {
                if (!is_int($number) || $number < 1) {
                    throw new \InvalidArgumentException('SQLite pager master-journal reader-cache next170 cache journal page numbers must be one-based integers');
                }
            }
    ...
    private static function sourceBytes(array $source, int $pageSize): string
    {
        ...
        foreach ($source as $entry) {
            if (strlen($entry['image']) !== $pageSize) {
                throw new \RuntimeException('SQLite pager master-journal reader-cache next170 final image is not page-size bytes');
            }
            $bytes .= $entry['image'];
        }
