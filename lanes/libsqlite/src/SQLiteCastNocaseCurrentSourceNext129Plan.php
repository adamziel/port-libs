...
final class SQLiteCastNocaseCurrentSourceNext129Plan
{
    /**
    ...
    public static function wordpressOptionValuePlan(
        ...
        $range = $patternPlan['prefix'] === '' ? null : $patternPlan['noCaseRange'];
        ...
        foreach (array_unique(array_merge(array_keys($currentByRowid), array_keys($nextByRowid))) as $rowid) {
            ...
            if ($current === null || $next === null || $current['nocaseKey'] !== $next['nocaseKey']) {
                $changedNocaseKey[] = (int) $rowid;
            }
        ...
        return [
            ...
            'changedNocaseKeyRowids' => $changedNocaseKey,
            ...
            'dependencies' => [
                ...
                'sqlite-nocase-like-prefix-range',
                'sqlite-like-nocase-residual',
                'sqlite-current-source-next129',
            ],
        ];
    }
    ...
    private static function traceRows(array $rows, string $castTarget, string $pattern, ?string $escape, ?array $range): array
    {
        foreach ($rows as $row) {
            if (!is_array($row)) {
                throw new \InvalidArgumentException('SQLite CAST NOCASE current-source rows must be arrays');
            }
            foreach (['option_id', 'option_value'] as $column) {
                if (!array_key_exists($column, $row)) {
                    throw new \InvalidArgumentException("SQLite CAST NOCASE current-source row is missing {$column}");
                }
            }
            if (!is_int($row['option_id'])) {
                throw new \InvalidArgumentException('SQLite CAST NOCASE current-source option_id must be an integer');
            }
        }
        ...
        foreach ($castRows as $row) {
            ...
            $nocaseKey = self::asciiLower($castText);
            $candidate = $range !== null && strcmp($nocaseKey, $range['lowerInclusive']) >= 0
                && ($range['upperBound'] === null || strcmp($nocaseKey, $range['upperBound']) < 0);
            $matched = $candidate && SQLiteDatabase::likeMatches($castText, $pattern, $escape, false);
            $trace[] = [
                'rowid' => $row['option_id'],
                ...
                'nocaseKey' => $nocaseKey,
                'nocaseKeyHex' => strtoupper(bin2hex($nocaseKey)),
                'candidate' => $candidate,
                'matched' => $matched,
            ];
        }
    ...
    private static function castTargetSql(string $target): string
    {
        ...
        if (preg_match('/^[A-Za-z][A-Za-z0-9_ ]*(?:\([0-9 ,]+\))?$/', $target) !== 1) {
            throw new \InvalidArgumentException('SQLite CAST NOCASE current-source target affinity is malformed');
        }
    ...
    private static function textValue(mixed $value): string
    {
        ...
        throw new \InvalidArgumentException('SQLite CAST NOCASE current-source values must be scalar, BLOB, or NULL');
    }
    ...
    private static function storageClass(mixed $value): string
    {
        return match (true) {
            ...
            default => throw new \InvalidArgumentException('SQLite CAST NOCASE current-source values must be scalar, BLOB, or NULL'),
        };
    }

    private static function asciiLower(string $value): string
    {
        return preg_replace_callback('/[A-Z]/', static fn (array $match): string => strtolower($match[0]), $value) ?? $value;
    }
