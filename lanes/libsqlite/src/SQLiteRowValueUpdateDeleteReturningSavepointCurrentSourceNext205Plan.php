...
final class SQLiteRowValueUpdateDeleteReturningSavepointCurrentSourceNext205Plan
{
    /**
    ...
    public static function execute(
        ...
        $releaseToken = self::token((string) ($options['release_token'] ?? 'wp.rowvalue.release.205'), 'release token');
        $expectedReleaseToken = self::token((string) ($options['expected_release_token'] ?? $releaseToken), 'expected release token');
        $nextCursor = self::token((string) ($options['next_cursor'] ?? 'wp.rowvalue.next.cursor.205'), 'next cursor');
        $expectedNextCursor = self::token((string) ($options['expected_next_cursor'] ?? $nextCursor), 'expected next cursor');

        $savepointImage = self::normalizeTables($tables);
        [$releasedCurrent, $savepointExecuted, $savepointReturning] = self::runStatements(
            $savepointImage,
            $savepointStatements,
            $uniqueConstraints,
            $rowIdColumn,
        ...
        [$nextCurrent, $nextExecuted, $nextReturning] = self::runStatements(
            $nextSource,
            $nextStatements,
            $uniqueConstraints,
            $rowIdColumn,
        ...
        $nextReadReleasedRows = $releaseAdmitted && $nextCursorMatches && self::firstStatementSourceMatches($nextExecuted, $releasedCurrent, $rowIdColumn);
        ...
        return [
            ...
            'released_returning_count' => self::returningCount($savepointReturning),
            'next_returning_count' => self::returningCount($nextReturning),
            'released_conflict_delete_count' => self::deletedConflictCount($savepointExecuted),
            'changed_tables_after_release' => self::changedTables($savepointImage, $releasedCurrent),
            'changed_tables_after_next' => self::changedTables($savepointImage, $nextCurrent),
            'row_counts' => self::rowCounts($nextCurrent),
     ...
     */
    private static function firstStatementSourceMatches(array $executed, array $source, string $rowIdColumn): bool
    {
        ...
        return self::rowsByIds($source[$table], $ids, $rowIdColumn) === ($statement['source_rows'] ?? null);
    }
     ...
     * @param list<array<string,mixed>> $executed
     */
    private static function deletedConflictCount(array $executed): int
    {
        $count = 0;
