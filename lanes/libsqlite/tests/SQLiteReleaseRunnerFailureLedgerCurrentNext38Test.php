...
function libsqlite_release_failure38_evidence(): SQLiteUpstreamSuiteEvidence
{
    return SQLiteUpstreamSuiteEvidence::fromManifestPath(__DIR__ . '/../UPSTREAM_TEST_MANIFEST.json');
}

function libsqlite_release_failure38_artifact(
    string $head,
    string $script,
    string $case,
    ...
    return libsqlite_release_failure38_evidence()->boundedRunnerArtifactRecord($audit);
}

function libsqlite_release_failure38_focused(string $head, string $script, string $case): array
{
    ...
    return libsqlite_release_failure38_evidence()->focusedFailureReproGate(
        [
            'script' => $script,
            'case' => $case,
...
return [
    'current next38 preserves matching runtime release failure as blocker evidence' => static function (TestRunner $t) use ($currentHead38, $nextHead38): void {
        $current = [libsqlite_release_failure38_artifact($currentHead38, 'fts5aux.test', 'fts5aux-3.1', 'current')];
        $next = [libsqlite_release_failure38_artifact($nextHead38, 'fts5aux.test', 'fts5aux-3.1', 'next')];
        $focused = libsqlite_release_failure38_focused($nextHead38, 'fts5aux.test', 'fts5aux-3.1');

        $ledger = libsqlite_release_failure38_evidence()->releaseRunnerFailureLedgerCurrentNext38($current, $next, $focused, $currentHead38, $nextHead38);
    ...
    'current next38 reports resolved failure when next accepted head has no failed artifact' => static function (TestRunner $t) use ($currentHead38, $nextHead38): void {
        ...
        $ledger = libsqlite_release_failure38_evidence()->releaseRunnerFailureLedgerCurrentNext38($current, [], $focused, $currentHead38, $nextHead38);
    ...
    'current next38 reports expanded ledger for a new next release failure' => static function (TestRunner $t) use ($currentHead38, $nextHead38): void {
        $current = [libsqlite_release_failure38_artifact($currentHead38, 'fts5aux.test', 'fts5aux-3.1', 'current')];
        $next = [
            libsqlite_release_failure38_artifact($nextHead38, 'fts5aux.test', 'fts5aux-3.1', 'next-a'),
            libsqlite_release_failure38_artifact($nextHead38, 'json101.test', 'json101-9.2', 'next-b'),
        ];
        $focused = libsqlite_release_failure38_focused($nextHead38, 'fts5aux.test', 'fts5aux-3.1');
    ...
    'current next38 blocks stale next failure artifacts' => static function (TestRunner $t) use ($currentHead38, $nextHead38): void {
        $current = [libsqlite_release_failure38_artifact($currentHead38, 'fts5aux.test', 'fts5aux-3.1', 'current')];
        $next = [libsqlite_release_failure38_artifact('stale-head', 'fts5aux.test', 'fts5aux-3.1', 'next-stale')];
        $focused = libsqlite_release_failure38_focused($nextHead38, 'fts5aux.test', 'fts5aux-3.1');
    ...
    'current next38 blocks when focused repro does not cover next failed script' => static function (TestRunner $t) use ($currentHead38, $nextHead38): void {
        $current = [libsqlite_release_failure38_artifact($currentHead38, 'fts5aux.test', 'fts5aux-3.1', 'current')];
        $next = [libsqlite_release_failure38_artifact($nextHead38, 'json101.test', 'json101-9.2', 'next')];
        $focused = libsqlite_release_failure38_focused($nextHead38, 'fts5aux.test', 'fts5aux-3.1');
