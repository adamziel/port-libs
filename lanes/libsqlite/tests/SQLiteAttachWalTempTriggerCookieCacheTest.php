...
$mainWalTriggerRecords = static fn (int $root = 40): array => [
...
$tempWalTriggerRecords = static fn (int $root = 50): array => [
...
$siteWalTriggerRecords = static fn (int $root = 60): array => [
...
foreach ($pathCases89 as $name => [$factory, $path, $expected]) {
    $tests['attach wal temp trigger cookie cache ' . $name] = static function (TestRunner $t) use ($factory, $value89, $path, $expected): void {
...
foreach ($predicateCases89 as $name => $predicate) {
    $tests['attach wal temp trigger cookie cache ' . $name] = static function (TestRunner $t) use ($predicate): void {
