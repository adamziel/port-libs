...
): array => SQLiteEncodingRtrimLikeGlobAffinityCurrentSourceNext144Plan::wordpressOptionValuePlan(
    $current ?? $currentRows,
    $next ?? $nextRows,
    'option_value',
    'option_pattern',
...
$tests['encoding rtrim like glob affinity current source next144 stable identical rows reusable'] = static function (TestRunner $t) use ($plan): void {
    ...
    $result = SQLiteEncodingRtrimLikeGlobAffinityCurrentSourceNext144Plan::wordpressOptionValuePlan($rows, $rows, 'option_value', 'option_pattern', 'LIKE', 'option_escape', 'stable', 'stable', 7, 7, 'UTF-8', 'UTF-8');
...
$tests['encoding rtrim like glob affinity current source next144 rejects missing pattern column'] = static function (TestRunner $t): void {
    ...
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteEncodingRtrimLikeGlobAffinityCurrentSourceNext144Plan::wordpressOptionValuePlan($rows, $rows, 'option_value', 'option_pattern'));
};
...
$tests['encoding rtrim like glob affinity current source next144 records array value as malformed'] = static function (TestRunner $t): void {
    ...
    $result = SQLiteEncodingRtrimLikeGlobAffinityCurrentSourceNext144Plan::wordpressOptionValuePlan($rows, $rows, 'option_value', 'option_pattern');
