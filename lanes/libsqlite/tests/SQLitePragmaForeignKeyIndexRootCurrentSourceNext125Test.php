...
$page = static fn (int $offset = 0, int $limit = 125, ?array $cursor = null, string $indexSql = 'PRAGMA main.index_xinfo(wp_options_name)', string $databaseBytes = null, ?array $schemaRowsForRun = null, SQLiteAttachedSchemaCatalog $runCatalog = null): array => SQLitePragmaForeignKeyIndexRootCurrentSourceNext125::page(
    $runCatalog ?? $catalog,
    $indexSql,
...
$tests['pragma foreignkey index root current source next125 accepts table valued index pragma'] = static function (TestRunner $t) use ($catalog, $cleanDatabase, $schemas): void {
    $result = SQLitePragmaForeignKeyIndexRootCurrentSourceNext125::page(
        $catalog,
...
$tests['pragma foreignkey index root current source next125 rejects negative offset'] = static function (TestRunner $t) use ($catalog, $database, $schemas): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLitePragmaForeignKeyIndexRootCurrentSourceNext125::page($catalog, 'PRAGMA main.index_xinfo(wp_options_name)', $database, $schemas(), 'PRAGMA foreign_key_check', -1));
};
...
$tests['pragma foreignkey index root current source next125 rejects zero limit'] = static function (TestRunner $t) use ($catalog, $database, $schemas): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLitePragmaForeignKeyIndexRootCurrentSourceNext125::page($catalog, 'PRAGMA main.index_xinfo(wp_options_name)', $database, $schemas(), 'PRAGMA foreign_key_check', 0, 0));
};
