...
function libsqlite_suite_next460468_evidence(): SQLiteUpstreamSuiteEvidence
{
    return SQLiteUpstreamSuiteEvidence::fromManifestPath(__DIR__ . '/../UPSTREAM_TEST_MANIFEST.json');
}

function libsqlite_suite_next460468_output(int $next, int $passLines = 96, int $assertions = 96, int $failures = 0): string
{
 ...
 */
function libsqlite_suite_next460468_rows(int $next, int $case = 1): array
{
    ...
    return [
        [
            ...
            'gap_status' => 'removed',
            'removed_blocker' => sprintf('next%d admits one focused veryquick shard row tied to launcher Base accepted HEAD fca16e3d and integration source f276db2c without duplicating accepted next453 through next459 suite evidence', $next),
 ...
 */
function libsqlite_suite_next460468_record(int $next, array $rows, ?string $output = null): array
{
    return libsqlite_suite_next460468_evidence()->upstreamVeryquickShardCurrentSourceShard(
        $next,
        $rows,
        801,
        151655,
        ...
        'lanes/libsqlite/tests/SQLiteUpstreamVeryquickShardCurrentSourceNext460468Test.php',
        $output ?? libsqlite_suite_next460468_output($next),
        sprintf('current-source next%d veryquick-shard admission avoids accepted next453-through-next459 suite evidence, queued runner106/jsonvt104 rebase work, accepted batch228 behavior surfaces, and live B-tree/JSON/VFS/WAL/planner/PRAGMA/ATTACH/window/VDBE work', $next),
        96
    );
}
...
foreach (range(460, 468) as $next) {
    $tests[sprintf('current source next%d admits veryquick shard', $next)] = static function (TestRunner $t) use ($next): void {
        $record = libsqlite_suite_next460468_record($next, libsqlite_suite_next460468_rows($next, $next - 459));
        ...
        $t->same(false, $record['counts_upstream_veryquick_shard_current_source_next459']);
        ...
        $t->contains(sprintf('veryquick-current-source-next%d-%02d.test', $next, $next - 459), implode(',', $record['target_scripts']));
    };
}

$tests['current source next460-468 blocks stale source provenance'] = static function (TestRunner $t): void {
    $rows = libsqlite_suite_next460468_rows(468);
    $rows[0]['implementation_source_head'] = '0000000000000000000000000000000000000000';

    $record = libsqlite_suite_next460468_record(468, $rows);
