...
$plan133136 = static fn (array $events, ?array $statements = null, ?array $schemas = null): array => SQLiteAttachWalTempSchemaCacheCurrentSourceNext92Plan::currentSourceNext133136(
...
$tests['attach temp wal schema cache current source next133 attach supplies qualified future reader'] = static function (TestRunner $t) use ($plan133136): void {
    ...
    $t->same('attach-wal-temp-schema-cache-current-source-next133-136', $result['operation']);
    $t->same('sqlite-attach-temp-wal-schema-cache-current-source-next133', $result['dependencies'][0]);
