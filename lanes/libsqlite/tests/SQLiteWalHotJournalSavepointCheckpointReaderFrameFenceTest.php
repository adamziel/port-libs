...
): array => SQLiteWalHotJournalSavepointCheckpointCurrentSourceNextPlan::savepointCheckpointReaderFrameFencePlan(
    $databasePath,
    $dirtyDatabase,
    $journalBytes,
    $currentWal,
...
$cases = [
    ...
    'status family' => [static fn (): mixed => $ready()['status_family'], 'wal-hot-journal-savepoint-checkpoint-current-source'],
    ...
    'tail checkpoint status family' => [static fn (): mixed => $tailCheckpoint()['status_family'], 'wal-hot-journal-savepoint-checkpoint-current-source-deferred'],
    ...
    'not hot status family' => [static fn (): mixed => $notHot()['status_family'], 'wal-hot-journal-savepoint-checkpoint-current-source-blocked'],
    ...
    'operation alias apply checkpoint' => [static fn (): mixed => in_array('apply_checkpoint_at_savepoint_visible_frame', $ready()['operation_reason_aliases'], true), true],
    'operation alias discard tail' => [static fn (): mixed => in_array('discard_wal_tail_after_savepoint_before_reset', $ready()['operation_reason_aliases'], true), true],
    'operation alias defer' => [static fn (): mixed => in_array('defer_checkpoint_until_savepoint_visible_source_matches', $tailCheckpoint()['operation_reason_aliases'], true), true],
    ...
    'canonical dependency current source' => [static fn (): mixed => in_array('sqlite-wal-hot-journal-savepoint-checkpoint-current-source', $ready()['canonical_dependencies'], true), true],
    'canonical dependency hot journal reader' => [static fn (): mixed => in_array('sqlite-wal-checkpoint-hot-journal-reader-current-source', $ready()['canonical_dependencies'], true), true],
    'canonical dependency savepoint boundary' => [static fn (): mixed => in_array('sqlite-savepoint-visible-wal-frame-boundary', $ready()['canonical_dependencies'], true), true],
...
$throws = [
    ...
    'empty path rejected by base' => static fn () => SQLiteWalHotJournalSavepointCheckpointCurrentSourceNextPlan::savepointCheckpointReaderFrameFencePlan('', $dirtyDatabase, $journalBytes, $currentWal, $walBytes, $hotDatabase, $walBytes, $checkpointDatabase, 'sp', 3, [1], 1),
    'empty dirty database rejected by base' => static fn () => SQLiteWalHotJournalSavepointCheckpointCurrentSourceNextPlan::savepointCheckpointReaderFrameFencePlan($databasePath, '', $journalBytes, $currentWal, $walBytes, $hotDatabase, $walBytes, $checkpointDatabase, 'sp', 3, [1], 1),
    'empty journal rejected by base' => static fn () => SQLiteWalHotJournalSavepointCheckpointCurrentSourceNextPlan::savepointCheckpointReaderFrameFencePlan($databasePath, $dirtyDatabase, '', $currentWal, $walBytes, $hotDatabase, $walBytes, $checkpointDatabase, 'sp', 3, [1], 1),
    'empty wal rejected by base' => static fn () => SQLiteWalHotJournalSavepointCheckpointCurrentSourceNextPlan::savepointCheckpointReaderFrameFencePlan($databasePath, $dirtyDatabase, $journalBytes, $currentWal, '', $hotDatabase, $walBytes, $checkpointDatabase, 'sp', 3, [1], 1),
    'empty reader database rejected by base' => static fn () => SQLiteWalHotJournalSavepointCheckpointCurrentSourceNextPlan::savepointCheckpointReaderFrameFencePlan($databasePath, $dirtyDatabase, $journalBytes, $currentWal, $walBytes, '', $walBytes, $checkpointDatabase, 'sp', 3, [1], 1),
    'empty pages rejected by base' => static fn () => SQLiteWalHotJournalSavepointCheckpointCurrentSourceNextPlan::savepointCheckpointReaderFrameFencePlan($databasePath, $dirtyDatabase, $journalBytes, $currentWal, $walBytes, $hotDatabase, $walBytes, $checkpointDatabase, 'sp', 3, [], 1),
