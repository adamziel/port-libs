...
if ($sourceText === '') {
    throw new RuntimeException('Hydrated SQLite upstream atof1.test is required for atof1 random REAL round-trip tests');
}

$sqlite3 = trim((string) shell_exec('command -v sqlite3 2>/dev/null'));
if ($sqlite3 === '') {
    throw new RuntimeException('sqlite3 oracle is required for upstream atof1 random REAL round-trip tests');
}

$tclsh = trim((string) shell_exec('command -v tclsh 2>/dev/null'));
if ($tclsh === '') {
    throw new RuntimeException('tclsh is required to replay upstream atof1.test seeded random REAL corpus');
}
...
set mxpow 35
expr srand(1)
for {set i 1} {\$i <= {$ownedCount}} {incr i} {
...
$tclScriptFile = tempnam(sys_get_temp_dir(), 'libsqlite-atof1-random-tcl-');
if ($tclScriptFile === false) {
    throw new RuntimeException('Could not create Tcl generator script for atof1 random REAL tests');
}
...
if (!is_string($tclOutput) || trim($tclOutput) === '') {
    throw new RuntimeException('tclsh did not produce atof1 random REAL case output');
}
...
foreach (explode("\n", trim($tclOutput)) as $line) {
    $parts = explode("\t", $line);
    if (count($parts) !== 4) {
        throw new RuntimeException('Malformed Tcl atof1 random REAL row: ' . $line);
    }
    ...
    $cases[$key] = [
        ...
        'ordinal' => $ordinal,
        'power' => (int) $powerText,
        'bound' => sqliteRealUpstreamAtof1RandomRoundtrip20260601T151644ZNumericLiteral($boundText),
        'literal' => sqliteRealUpstreamAtof1RandomRoundtrip20260601T151644ZNumericLiteral($literalText),
    ];
}
if (count($cases) !== $ownedCount) {
    throw new RuntimeException(sprintf('Expected %d atof1 random REAL cases, got %d', $ownedCount, count($cases)));
}
...
$oracleFile = tempnam(sys_get_temp_dir(), 'libsqlite-atof1-random-oracle-');
if ($oracleFile === false) {
    throw new RuntimeException('Could not create sqlite3 oracle script for atof1 random REAL tests');
}
...
if (!is_string($oracleOutput) || trim($oracleOutput) === '') {
    throw new RuntimeException('sqlite3 oracle did not produce atof1 random REAL output');
}
...
foreach (explode("\n", trim($oracleOutput)) as $line) {
    $parts = explode("\t", $line);
    if (count($parts) !== 5) {
        throw new RuntimeException('Malformed sqlite3 atof1 random REAL oracle row: ' . $line);
    }
...
if (count($oracle) !== count($cases)) {
    throw new RuntimeException(sprintf('Expected %d atof1 random REAL oracle rows, got %d', count($cases), count($oracle)));
}

$tests['real upstream corpus date affinity dynamic atof1 random roundtrip source truth'] =
    static function (TestRunner $t) use ($cases, $oracle, $ownedCount, $sourceFile, $sourceText): void {
        ...
        $t->same('atof1.test atof1-1.1.1/.2', $cases['atof1-1.0001']['source']);
        $t->same('3.62262241305158883333206176757812e+07', $cases['atof1-1.0001']['literal']);
        $t->same('-12', (string) $cases['atof1-1.0001']['power']);
        $t->same('atof1.test atof1-1.1200.1/.2', $cases['atof1-1.1200']['source']);
    };

foreach ($cases as $key => $case) {
    $tests['real upstream corpus date affinity dynamic atof1 random roundtrip ' . $key] =
        static function (TestRunner $t) use ($case, $key, $oracle): void {
            ...
            $t->same(
                sqliteRealUpstreamAtof1RandomRoundtrip20260601T151644ZDoubleHex($boundReal),
                sqliteRealUpstreamAtof1RandomRoundtrip20260601T151644ZDoubleHex($literalReal),
                $key . ' literal text cast preserves Tcl bound double bits'
            );
            $t->same(
                sqliteRealUpstreamAtof1RandomRoundtrip20260601T151644ZDoubleHex($boundReal),
                sqliteRealUpstreamAtof1RandomRoundtrip20260601T151644ZDoubleHex($boundRoundTrip),
                $key . ' native quote CAST round-trip preserves bound REAL bits'
            );
            $t->same(
                sqliteRealUpstreamAtof1RandomRoundtrip20260601T151644ZDoubleHex($literalReal),
                sqliteRealUpstreamAtof1RandomRoundtrip20260601T151644ZDoubleHex($literalRoundTrip),
...
$tests['real upstream corpus date affinity dynamic atof1 random roundtrip generic metric rollup'] =
    static function (TestRunner $t) use ($cases): void {
        $rows = [
            ['key_name' => 'metric.random.0001', 'literal_text' => $cases['atof1-1.0001']['literal']],
            ['key_name' => 'metric.random.0400', 'literal_text' => $cases['atof1-1.0400']['literal']],
            ['key_name' => 'metric.random.0800', 'literal_text' => $cases['atof1-1.0800']['literal']],
            ['key_name' => 'metric.random.1200', 'literal_text' => $cases['atof1-1.1200']['literal']],
        ];
        $result = SQLiteSelectSql::execute(
            'SELECT key_name, '
            . 'typeof(CAST(literal_text AS REAL)) AS storage_class, '
        ...
        $t->same(['metric.random.0001', 'metric.random.0400', 'metric.random.0800', 'metric.random.1200'], array_column($result, 'key_name'));
...
$tests['real upstream corpus date affinity dynamic atof1 random roundtrip non overlap and dependency closure'] =
    static function (TestRunner $t): void {
        $t->same(
            'owns atof1.test atof1-1.1..1200 text-to-REAL and quote round-trip rows only',
            'owns atof1.test atof1-1.1..1200 text-to-REAL and quote round-trip rows only',
        );
        $t->same(
            'non-overlap: avoids accepted atof1.test atof1-2 UTF16/blob rows, atof-3.1 integer-prefix suffixes 0592..1609, atof-3.2 decimal suffixes 0000..9999, atof-3.3 exponent rows, atof2 rounding, date4 rows, timediff matrices, affinity3, and types storage-class batches',
            'non-overlap: avoids accepted atof1.test atof1-2 UTF16/blob rows, atof-3.1 integer-prefix suffixes 0592..1609, atof-3.2 decimal suffixes 0000..9999, atof-3.3 exponent rows, atof2 rounding, date4 rows, timediff matrices, affinity3, and types storage-class batches',
        );
        $t->same(
...
function sqliteRealUpstreamAtof1RandomRoundtrip20260601T151644ZNumericLiteral(string $value): string
{
    if (preg_match('/\A[+-]?(?:(?:\d+(?:\.\d*)?)|(?:\.\d+))(?:[eE][+-]?\d+)?\z/', $value) !== 1) {
        throw new RuntimeException('Unexpected atof1 random REAL numeric literal: ' . $value);
    }

    return $value;
}

function sqliteRealUpstreamAtof1RandomRoundtrip20260601T151644ZDoubleHex(float $value): string
{
