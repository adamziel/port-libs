...
function libsqlite_suite_admission74_evidence(): SQLiteUpstreamSuiteEvidence
{
    return SQLiteUpstreamSuiteEvidence::fromManifestPath(__DIR__ . '/../UPSTREAM_TEST_MANIFEST.json');
}

function libsqlite_suite_admission74_output(int $passLines = 74, int $assertions = 296, int $failures = 0): string
{
 ...
 */
function libsqlite_suite_admission74_rows(
    int $case = 1,
    ...
    return [
        [
            ...
            'artifact_path' => 'lanes/libsqlite/notes/suite-release-runner-admission-current-next74.md',
...
function libsqlite_suite_admission74_record(
    array $rows,
    string $launcherBase = '23caf4af795588a2d84150ed1585e33865ff2b76',
    string $nextHead = 'suite-release-runner-admission-rebase-current-next74',
    string $output = null,
    ?int $expected = 74,
    string $snapshot = ''
): array {
    return libsqlite_suite_admission74_evidence()->suiteReleaseRunnerAdmissionCurrentNext74(
        $rows,
        464,
        28200,
        $launcherBase,
        $nextHead,
        'lanes/libsqlite/tests/SQLiteSuiteReleaseRunnerAdmissionCurrentNext74Test.php',
        $output ?? libsqlite_suite_admission74_output(),
...
foreach (range(1, 74) as $case) {
    $tests[sprintf('current next74 admits rebased release runner artifact case %02d', $case)] = static function (TestRunner $t) use ($case): void {
        $record = libsqlite_suite_admission74_record(libsqlite_suite_admission74_rows($case));
...
$tests['current next74 records rebased categories scripts and provenance source'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_admission74_record(libsqlite_suite_admission74_rows(3));
...
$tests['current next74 preserves already countable rebased artifact rows'] = static function (TestRunner $t): void {
    $rows = libsqlite_suite_admission74_rows();
    $rows[0]['current_status'] = 'countable';
    $rows[0]['current_tests'] = 1001;

    $record = libsqlite_suite_admission74_record($rows);
...
$tests['current next74 blocks stale launcher base row'] = static function (TestRunner $t): void {
    $rows = libsqlite_suite_admission74_rows(launcherBase: '0000000000000000000000000000000000000000');
...
$tests['current next74 ignores shared source drift when launcher base matches'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_admission74_record(
        libsqlite_suite_admission74_rows(sharedSource: '5883f5e65ebfd2e9cf8c9acf617a2a818277909c')
    );
...
$tests['current next74 blocks active broad runner snapshots'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_admission74_record(
        libsqlite_suite_admission74_rows(),
...
$tests['current next74 blocks pass line inflation'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_admission74_record(
        libsqlite_suite_admission74_rows(),
        output: libsqlite_suite_admission74_output(passLines: 12, assertions: 296),
        expected: 74
    );
...
$tests['current next74 rejects invalid setup'] = static function (TestRunner $t): void {
    $evidence = libsqlite_suite_admission74_evidence();

    $t->throws(InvalidArgumentException::class, static fn () => $evidence->suiteReleaseRunnerAdmissionCurrentNext74(libsqlite_suite_admission74_rows(), 464, 28200, '', 'next', 'lanes/libsqlite/tests/SQLiteSuiteReleaseRunnerAdmissionCurrentNext74Test.php', libsqlite_suite_admission74_output(), 'non-overlap', 74));
    $t->throws(InvalidArgumentException::class, static fn () => $evidence->suiteReleaseRunnerAdmissionCurrentNext74([], 464, 28200, '23caf4af795588a2d84150ed1585e33865ff2b76', 'next', 'lanes/libsqlite/tests/SQLiteSuiteReleaseRunnerAdmissionCurrentNext74Test.php', libsqlite_suite_admission74_output(), 'non-overlap', 74));
};

$tests['current next74 records dependency closure and next gate'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_admission74_record(libsqlite_suite_admission74_rows());
