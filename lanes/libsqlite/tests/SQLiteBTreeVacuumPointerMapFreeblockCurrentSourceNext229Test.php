...
use PortLibs\LibSqlite\SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNext229Plan;
...
$database229 = static function () use ($makeFirstPage229, $putPointerMapEntry229): SQLiteDatabase {
    ...
    $pages[3] = SQLiteTableLeafPage::assemble([
        ...
        SQLiteTableLeafCell::encode(2, SQLiteRecord::encode([null, '_transient_next229', str_repeat('cache:', 42)])),
...
$plan229 = static function (int $batchSize = 2): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNext229Plan {
    global $database229;

    $database = $database229();
    $deletedPage = SQLiteTableLeafPage::deleteCellByRowId($database->page(3), 2, secureDelete: true);

    return SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNext229Plan::tableLeafFromDeleteResult(
        $database,
        3,
        [
            'page' => $deletedPage,
...
$cases229 = [
    ...
    'current source token length' => static fn (): mixed => strlen($plan229()->resumeSummary()['current_source_next229_token']),
    ...
    'non overlap' => static fn (): mixed => str_contains($plan229()->resumeSummary()['non_overlap'], 'does not repeat next224'),
...
$expected229 = [
    'action label' => 'btree-vacuum-pointermap-freeblock-current-source-next229',
    'summary status' => 'btree-vacuum-pointermap-freeblock-current-source-next229-ready',
    'resume row count' => 7,
    ...
    'dependency closure' => 'no new support component needed; next229 reuses next224 current-source cursor rows and adds resume-window admission receipts only',
    'non overlap' => true,
    'source action' => 'btree-vacuum-pointermap-freeblock-current-source-next224',
    'source row count' => 7,
...
foreach ($cases229 as $name => $callback) {
    $tests['btree vacuum pointermap freeblock current source next229 ' . $name] = static function (TestRunner $t) use ($callback, $expected229, $name): void {
        $t->same($expected229[$name], $callback());
    };
}

foreach (range(1, 80) as $index) {
    $tests['btree vacuum pointermap freeblock current source next229 resume invariant ' . $index] = static function (TestRunner $t) use ($plan229): void {
        $plan = $plan229();
        ...
        $t->same('btree-vacuum-pointermap-freeblock-current-source-next229-ready', $summary['status']);
