...
$rows91 = static fn (SQLiteBTreeOverflowVacuumFreepagePlan $plan): array => $plan->overflowPointerMapFreepageCurrentSourceNext91();

$cases = [
    'action includes current source next91 summary' => static fn (SQLiteBTreeOverflowVacuumFreepagePlan $plan): mixed => $plan->toArray()['action'],
    ...
    'summary embeds next91 rows' => static fn (SQLiteBTreeOverflowVacuumFreepagePlan $plan): mixed => array_column($plan->toArray()['overflow_pointermap_freepage_current_source_next91'], 'page_number'),
...
$expected = [
    'action includes current source next91 summary' => 'btree-overflow-vacuum-freepage-current-next',
    ...
    'summary embeds next91 rows' => [6, 7, 8, 9, 10],
...
foreach ($cases as $name => $callback) {
    $tests['btree overflow pointermap freepage current source next91 ' . $name] = static function (TestRunner $t) use ($fixture91, $callback, $expected, $name): void {
        $t->same($expected[$name], $callback($fixture91(true, 5)));
    };
}

foreach (range(1, 35) as $index) {
    $tests['btree overflow pointermap freepage current source next91 invariant ' . $index] = static function (TestRunner $t) use ($fixture91, $rows91, $index): void {
        $limit = $index % 4;
...
$tests['btree overflow pointermap freepage current source next91 limited allocation keeps positions nullable'] = static function (TestRunner $t) use ($fixture91, $rows91): void {
    $rows = $rows91($fixture91(true, 2));
    $t->same([null, 0, null, null, 1], array_column($rows, 'next_allocation_position'));
};

$tests['btree overflow pointermap freepage current source next91 rejects duplicate chains'] = static function (TestRunner $t) use ($fixture91): void {
    try {
        $plan = $fixture91();
