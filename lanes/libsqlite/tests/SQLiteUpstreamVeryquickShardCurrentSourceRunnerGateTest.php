...
function libsqlite_suite_next157_evidence(): SQLiteUpstreamSuiteEvidence
{
    return SQLiteUpstreamSuiteEvidence::fromManifestPath(__DIR__ . '/../UPSTREAM_TEST_MANIFEST.json');
}

function libsqlite_suite_next157_output(int $passLines = 66, int $assertions = 78, int $failures = 0): string
{
    $lines = ['Focused test run: 1 selected test files (root lock skipped)'];
    for ($i = 1; $i <= $passLines; $i++) {
        $lines[] = sprintf('PASS current source next157 veryquick shard case %02d', $i);
    }
 ...
 */
function libsqlite_suite_next157_rows(
    int $case = 1,
    ...
    string $nextHead = 'suite-upstream-veryquick-shard-current-source-next157'
): array {
    $script = sprintf('veryquick-current-source-next157-%02d.test', $case);

    return [
        [
            'unit' => 'suite-upstream-veryquick-shard-current-source-next157',
            'kind' => 'bounded-upstream-veryquick-shard-runner',
            'gap_id' => 'current-source-next157-veryquick-shard-gap',
            'gap_status' => 'removed',
            'removed_blocker' => 'next157 admits a current-source veryquick shard row tied to launcher Base accepted HEAD 4880a033 and integration source 8a447f44 without reusing stale runner106/jsonvt104 rebase evidence',
            ...
            'artifact_path' => 'lanes/libsqlite/notes/suite-upstream-veryquick-shard-current-source-next157.md',
 ...
 */
function libsqlite_suite_next157_record(
    array $rows,
    string $launcherBase = '4880a03300afb083403cb85638f3d1cb0f0226ad',
    string $dashboardSource = '8a447f445e5d2fd32fc9fd463117f585d1416551',
    string $statusSource = '8a447f445e5d2fd32fc9fd463117f585d1416551',
...
foreach (range(1, 60) as $case) {
    $tests[sprintf('current source next157 admits veryquick shard case %02d', $case)] = static function (TestRunner $t) use ($case): void {
        ...
        $t->same(44700, $record['next_php_pass']);
...
$tests['current source next157 records authoritative launcher and source heads'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next157_record(libsqlite_suite_next157_rows(8));
    ...
    $t->same(['suite-upstream-veryquick-shard-current-source-next157'], $record['artifact_source_heads']);
};

$tests['current source next157 records target scripts and tier counts'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next157_record(libsqlite_suite_next157_rows(11));

    $t->same(44633, $record['tests_total_delta']);
    $t->same(['accepted-batch109-anchor.test', 'testrunner.test', 'veryquick-current-source-next157-11.test'], $record['target_scripts']);
...
$tests['current source next157 preserves already counted row without mapped inflation'] = static function (TestRunner $t): void {
    $rows = libsqlite_suite_next157_rows();
...
$tests['current source next157 blocks stale source provenance'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next157_record(
        libsqlite_suite_next157_rows(
...
$tests['current source next157 blocks unguarded and non local artifacts'] = static function (TestRunner $t): void {
    $rows = libsqlite_suite_next157_rows();
    $rows[0]['artifact_path'] = '/tmp/next157.md';
...
$tests['current source next157 blocks non zero runner artifacts'] = static function (TestRunner $t): void {
    $rows = libsqlite_suite_next157_rows();
    $rows[0]['exit'] = 1;
    $rows[0]['errors'] = 2;
...
$tests['current source next157 blocks missing removed blocker classification'] = static function (TestRunner $t): void {
    $rows = libsqlite_suite_next157_rows();
...
$tests['current source next157 blocks focused php admission mismatch'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next157_record(libsqlite_suite_next157_rows(), output: libsqlite_suite_next157_output(assertions: 77));
...
$tests['current source next157 blocks active broad runner snapshot'] = static function (TestRunner $t): void {
    $snapshot = '1234 1 S 00:10 7.5 ./testfixture ../libsqlite/test/testrunner.tcl --jobs 1 --stop-on-error all';
    $record = libsqlite_suite_next157_record(libsqlite_suite_next157_rows(), snapshot: $snapshot);
