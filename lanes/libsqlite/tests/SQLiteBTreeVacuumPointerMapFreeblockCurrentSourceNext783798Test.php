...
$makeFirstPage783798 = static function (int $pageCount): string {
...
$putPointerMapEntry783798 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database783798 = static function (int $sliceNumber) use ($makeFirstPage783798, $putPointerMapEntry783798): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage783798(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry783798($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan783798 = static function (int $sliceNumber, int $batchSize = 2) use ($database783798): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database783798($sliceNumber);
...
foreach ([783, 784, 785, 786, 787, 788, 789, 790, 791, 792, 793, 794, 795, 796, 797, 798] as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} extends next783-798 handoff receipts"] = static function (TestRunner $t) use ($plan783798, $sliceNumber): void {
        $plan = $plan783798($sliceNumber);
        ...
        $t->same(6, $plan783798($sliceNumber, 3)->currentSourceSummary()['current_source_row_count']);
    };
}
