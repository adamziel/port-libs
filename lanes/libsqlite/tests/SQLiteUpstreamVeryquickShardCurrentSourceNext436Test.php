...
function libsqlite_suite_next436_evidence(): SQLiteUpstreamSuiteEvidence
{
    return SQLiteUpstreamSuiteEvidence::fromManifestPath(__DIR__ . '/../UPSTREAM_TEST_MANIFEST.json');
}

function libsqlite_suite_next436_output(int $passLines = 96, int $assertions = 96, int $failures = 0): string
{
    $lines = ['Focused test run: 1 selected test files (root lock skipped)'];
    for ($i = 1; $i <= $passLines; $i++) {
        $lines[] = sprintf('PASS current source next436 veryquick shard admission case %02d', $i);
    }
 ...
 */
function libsqlite_suite_next436_rows(
    int $case = 1,
    ...
    string $nextHead = 'suite-upstream-veryquick-shard-current-source-next436'
): array {
    $script = sprintf('veryquick-current-source-next436-%02d.test', $case);

    return [
        [
            'unit' => 'suite-upstream-veryquick-shard-current-source-next436',
            'kind' => 'bounded-upstream-veryquick-shard-runner',
            'gap_id' => 'current-source-next436-veryquick-shard-gap',
            'gap_status' => 'removed',
            'removed_blocker' => 'next436 admits one focused veryquick shard row tied to launcher Base accepted HEAD fca16e3d and integration source 8a447f44 without duplicating accepted next155 through next398 suite evidence',
            ...
            'artifact_path' => 'lanes/libsqlite/notes/yield-suite-upstream-veryquick-shard-current-source-next436.md',
 ...
 */
function libsqlite_suite_next436_record(
    array $rows,
    string $launcherBase = 'fca16e3dd1812e6fcb6dc54c4980a5fb898b24ec',
    string $dashboardSource = '8a447f445e5d2fd32fc9fd463117f585d1416551',
    string $statusSource = '8a447f445e5d2fd32fc9fd463117f585d1416551',
    ...
    return libsqlite_suite_next436_evidence()->upstreamVeryquickShardCurrentSourceShard(436,
        $rows,
        801,
        151655,
        $launcherBase,
...
foreach (range(1, 80) as $case) {
    $tests[sprintf('current source next436 admits veryquick shard case %02d', $case)] = static function (TestRunner $t) use ($case): void {
        $record = libsqlite_suite_next436_record(libsqlite_suite_next436_rows($case));

        $t->same('current-source-next436-veryquick-shard-advanced', $record['status']);
        ...
        $t->same(['suite-upstream-veryquick-shard-current-source-next436'], $record['admitted_units']);
        ...
        $t->same(true, $record['counts_upstream_veryquick_shard_current_source_next436']);
...
$tests['current source next436 records authoritative launcher and source heads'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next436_record(libsqlite_suite_next436_rows(8));
    ...
    $t->same(['suite-upstream-veryquick-shard-current-source-next436'], $record['artifact_source_heads']);
};

$tests['current source next436 records target scripts and tier counts'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next436_record(libsqlite_suite_next436_rows(13));

    $t->same(151668, $record['tests_total_delta']);
    $t->same(['accepted-batch228-anchor.test', 'testrunner.test', 'veryquick-current-source-next436-13.test'], $record['target_scripts']);
...
$tests['current source next436 preserves already counted row without mapped inflation'] = static function (TestRunner $t): void {
    $rows = libsqlite_suite_next436_rows();
    $rows[0]['current_countable'] = true;
    $rows[0]['current_tests'] = 144335;

    $record = libsqlite_suite_next436_record($rows);

    $t->same('current-source-next436-veryquick-shard-preserved', $record['status']);
    $t->same(0, $record['mapped_delta']);
    $t->same(801, $record['next_mapped']);
    $t->same(['batch228-current-source-anchor', 'suite-upstream-veryquick-shard-current-source-next436'], $record['preserved_units']);
};

$tests['current source next436 blocks stale source provenance'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next436_record(
        libsqlite_suite_next436_rows(
...
$tests['current source next436 blocks unguarded and non local artifacts'] = static function (TestRunner $t): void {
    $rows = libsqlite_suite_next436_rows();
    $rows[0]['artifact_path'] = '/tmp/next436.md';
...
$tests['current source next436 blocks non zero runner artifacts'] = static function (TestRunner $t): void {
    $rows = libsqlite_suite_next436_rows();
    $rows[0]['exit'] = 1;
    $rows[0]['errors'] = 2;
...
$tests['current source next436 blocks missing removed blocker classification'] = static function (TestRunner $t): void {
    $rows = libsqlite_suite_next436_rows();
...
$tests['current source next436 blocks duplicate broad runner snapshot'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next436_record(
        libsqlite_suite_next436_rows(),
...
$tests['current source next436 blocks focused php admission mismatch'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next436_record(
        libsqlite_suite_next436_rows(),
        output: libsqlite_suite_next436_output(assertions: 83)
    );
...
$tests['current source next436 records exact focused php admission'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next436_record(libsqlite_suite_next436_rows());
...
$tests['current source next436 blocks missing focused runner summary'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next436_record(
        libsqlite_suite_next436_rows(),
...
$tests['current source next436 carries dependency closure and non overlap notes'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next436_record(libsqlite_suite_next436_rows());
...
$tests['current source next436 blocks missing next countability flag'] = static function (TestRunner $t): void {
    $rows = libsqlite_suite_next436_rows();
...
$tests['current source next436 keeps broad release parity unclaimed'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next436_record(libsqlite_suite_next436_rows(21));
...
$tests['current source next436 does not recount prior veryquick shards'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next436_record(libsqlite_suite_next436_rows(22));

    $t->same(false, $record['counts_upstream_veryquick_shard_current_source_next237']);
    $t->same(false, $record['counts_upstream_veryquick_shard_current_source_next212']);
};

$tests['current source next436 records focused shard dependency closure'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next436_record(libsqlite_suite_next436_rows(23));

    $t->contains('current-source next436 veryquick shard admission', $record['dependency_closure']);
