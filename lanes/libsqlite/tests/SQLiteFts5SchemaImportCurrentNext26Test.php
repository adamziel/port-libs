...
$wordpressPostSearch = "CREATE VIRTUAL TABLE wp_posts_fts USING fts5(post_title, post_content, post_excerpt UNINDEXED, content='wp_posts', content_rowid='ID', tokenize='unicode61 remove_diacritics 2 tokenchars ''-_''', prefix='2 3 4', detail=column)";
$wordpressOptionSearch = "CREATE VIRTUAL TABLE temp.wp_option_search USING fts5(option_name, option_value, autoload UNINDEXED, tokenize='porter unicode61', prefix='3,4')";
...
$cases = [
    'post status' => [$wordpressPostSearch, 'status', 'ok'],
    'post module' => [$wordpressPostSearch, 'module', 'fts5'],
    'post default schema' => [$wordpressPostSearch, 'schema', 'main'],
    'post table' => [$wordpressPostSearch, 'table', 'wp_posts_fts'],
    'post qualified name' => [$wordpressPostSearch, 'qualifiedName', 'main.wp_posts_fts'],
    'post column count' => [$wordpressPostSearch, 'columns.count', 3],
    'post first column' => [$wordpressPostSearch, 'columns.0.name', 'post_title'],
    'post second column' => [$wordpressPostSearch, 'columns.1.name', 'post_content'],
    'post unindexed column name' => [$wordpressPostSearch, 'columns.2.name', 'post_excerpt'],
    'post unindexed flag' => [$wordpressPostSearch, 'columns.2.unindexed', true],
    'post indexed columns only' => [$wordpressPostSearch, 'indexedColumns', ['post_title', 'post_content']],
    'post unindexed columns only' => [$wordpressPostSearch, 'unindexedColumns', ['post_excerpt']],
    'post tokenizer name' => [$wordpressPostSearch, 'options.tokenize.name', 'unicode61'],
    'post tokenizer remove diacritics' => [$wordpressPostSearch, 'options.tokenize.removeDiacritics', 2],
    'post tokenizer tokenchars' => [$wordpressPostSearch, 'options.tokenize.tokenchars', '-_'],
    'post prefix lengths' => [$wordpressPostSearch, 'options.prefix', [2, 3, 4]],
    'post content table' => [$wordpressPostSearch, 'options.content', 'wp_posts'],
    'post external content' => [$wordpressPostSearch, 'options.externalContent', true],
    'post content table present' => [$wordpressPostSearch, 'options.contentTablePresent', true],
    'post content rowid' => [$wordpressPostSearch, 'options.contentRowid', 'ID'],
    'post detail mode' => [$wordpressPostSearch, 'options.detail', 'column'],
    'post columnsize default' => [$wordpressPostSearch, 'options.columnsize', 1],
    'post shadow data table' => [$wordpressPostSearch, 'shadowTables.0', 'main.wp_posts_fts_data'],
    'post shadow content table' => [$wordpressPostSearch, 'shadowTables.2', 'main.wp_posts_fts_content'],
    'post shadow config table' => [$wordpressPostSearch, 'shadowTables.4', 'main.wp_posts_fts_config'],
    'post import action rebuild' => [$wordpressPostSearch, 'importActions.3', 'scheduleExternalContentRebuild'],
    'option schema' => [$wordpressOptionSearch, 'schema', 'temp'],
    'option table' => [$wordpressOptionSearch, 'table', 'wp_option_search'],
    'option qualified name' => [$wordpressOptionSearch, 'qualifiedName', 'temp.wp_option_search'],
    'option tokenizer porter' => [$wordpressOptionSearch, 'options.tokenize.name', 'porter'],
    'option tokenizer args' => [$wordpressOptionSearch, 'options.tokenize.args', ['unicode61']],
    'option comma prefix lengths' => [$wordpressOptionSearch, 'options.prefix', [3, 4]],
    'option inline content external false' => [$wordpressOptionSearch, 'options.externalContent', false],
    'option inline content action' => [$wordpressOptionSearch, 'importActions.3', 'copyInlineContentRows'],
