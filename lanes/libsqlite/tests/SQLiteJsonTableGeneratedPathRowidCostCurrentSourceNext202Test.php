...
): array => SQLiteJsonTablePlan::currentSourceGeneratedPathRowidCostCurrentSourceNext202(
    'json_tree',
    $current ?? $current202,
    $next ?? $next202,
    'option_value',
...
$tests = [
    ...
    'bad xnext batch rejected' => function (TestRunner $t) use ($current202): void {
        $t->throws(InvalidArgumentException::class, static fn () => SQLiteJsonTablePlan::currentSourceGeneratedPathRowidCostCurrentSourceNext202(
            'json_tree',
            $current202,
            $current202,
            'option_value',
