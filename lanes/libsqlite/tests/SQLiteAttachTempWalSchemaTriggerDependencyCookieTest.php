...
foreach ($cases95 as $name => [$path, $expected]) {
    $tests['attach temp wal schema trigger dependency cookie ' . $name] = static function (TestRunner $t) use ($plan95, $value95, $path, $expected): void {
...
foreach ($predicateCases95 as $name => $predicate) {
    $tests['attach temp wal schema trigger dependency cookie ' . $name] = static function (TestRunner $t) use ($predicate): void {
