...
$plan = static fn (
    ...
    int $nextCookie = 174,
): array => SQLiteUtf16NocaseLikeRtrimNulCurrentSourceNextPlan::keyValueRowKeyEmbeddedNulPlan(
    $current ?? $currentRows,
    $next ?? $nextRows,
    $pattern,
    $escape,
...
$cases = [
    ...
    'next source' => ['nextSource', 'main.app_settings@174'],
...
$tests['utf16 nocase like rtrim nul current source nextOneSevenFour stable embedded nul false positive is reusable after recheck'] = static function (TestRunner $t) use ($row): void {
    ...
    $result = SQLiteUtf16NocaseLikeRtrimNulCurrentSourceNextPlan::keyValueRowKeyEmbeddedNulPlan(
        $rows,
        $rows,
        'plugin!_cache',
        '!',
