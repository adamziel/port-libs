...
// own 0000..3999, so this shard owns the next non-overlapping 4000..4999
// suffix window.
$firstSuffix = 4000;
$lastSuffix = 4999;
$cases = [];
...
$scriptFile = tempnam(sys_get_temp_dir(), 'libsqlite-atof1-decimal-4000-oracle-');
if ($scriptFile === false) {
...
$tests['real upstream corpus date affinity dynamic atof1 decimal REAL suffix 4000 4999 source truth'] =
    static function (TestRunner $t) use ($cases, $firstSuffix, $lastSuffix, $oracle, $sourceFile, $sourceText): void {
        ...
        $t->same(4000, $firstSuffix);
        $t->same(4999, $lastSuffix);
        $t->same(4000, $cases['suffix-4000']['suffix']);
        $t->same(4999, $cases['suffix-4999']['suffix']);
        $t->same('18.446744073709554000', $cases['suffix-4000']['value']);
        $t->same('18.446744073709554999', $cases['suffix-4999']['value']);
    };

foreach ($cases as $key => $case) {
    $tests['real upstream corpus date affinity dynamic atof1 decimal REAL suffix 4000 4999 ' . $key] =
...
$tests['real upstream corpus date affinity dynamic atof1 decimal REAL suffix 4000 4999 generic application rollup'] =
    static function (TestRunner $t) use ($cases): void {
        $rows = [
            ['key_name' => 'metric-low', 'decimal_metric' => $cases['suffix-4000']['value']],
            ['key_name' => 'metric-mid', 'decimal_metric' => $cases['suffix-4500']['value']],
            ['key_name' => 'metric-high', 'decimal_metric' => $cases['suffix-4999']['value']],
        ];
        $result = SQLiteSelectSql::execute(
            'SELECT key_name, '
...
$tests['real upstream corpus date affinity dynamic atof1 decimal REAL suffix 4000 4999 non overlap and dependency closure'] =
    static function (TestRunner $t): void {
        $t->same(
            'owns atof1.test atof-3.2 decimal REAL suffixes 4000..4999 only',
            'owns atof1.test atof-3.2 decimal REAL suffixes 4000..4999 only',
        );
        $t->same(
            'non-overlap: avoids accepted atof-3.2 decimal REAL/mantissa suffixes 0000..3999, accepted atof-3.1 integer-prefix suffixes 0592..1609, atof2 rounding, date4 rows, date/timediff matrices, and types storage-class batches',
            'non-overlap: avoids accepted atof-3.2 decimal REAL/mantissa suffixes 0000..3999, accepted atof-3.1 integer-prefix suffixes 0592..1609, atof2 rounding, date4 rows, date/timediff matrices, and types storage-class batches',
        );
        $t->same(
