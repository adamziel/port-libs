...
use PortLibs\LibSqlite\SQLitePagerJournalSavepointHotRollbackCurrentSourceNext118Plan;
...
$plan = static fn (array $before = null, array $retry = null, ?string $master = null, bool $reserved = false, bool $requiresSuper = false, ?bool $superExists = null): array => SQLitePagerJournalSavepointHotRollbackCurrentSourceNext118Plan::currentSourceNext(
    $databasePath,
    $databaseBytes,
    $journalBytes,
    $pageSize,
...
$throws = [
    'empty database path rejected' => static fn () => SQLitePagerJournalSavepointHotRollbackCurrentSourceNext118Plan::currentSourceNext('', $databaseBytes, $journalBytes, $pageSize, 'plugin-retry', $beforeImages, $retryWrites),
    'unaligned database rejected' => static fn () => SQLitePagerJournalSavepointHotRollbackCurrentSourceNext118Plan::currentSourceNext($databasePath, 'short', $journalBytes, $pageSize, 'plugin-retry', $beforeImages, $retryWrites),
    'empty journal rejected' => static fn () => SQLitePagerJournalSavepointHotRollbackCurrentSourceNext118Plan::currentSourceNext($databasePath, $databaseBytes, '', $pageSize, 'plugin-retry', $beforeImages, $retryWrites),
    'bad page size rejected' => static fn () => SQLitePagerJournalSavepointHotRollbackCurrentSourceNext118Plan::currentSourceNext($databasePath, $databaseBytes, $journalBytes, 513, 'plugin-retry', $beforeImages, $retryWrites),
    'empty savepoint rejected' => static fn () => SQLitePagerJournalSavepointHotRollbackCurrentSourceNext118Plan::currentSourceNext($databasePath, $databaseBytes, $journalBytes, $pageSize, '', $beforeImages, $retryWrites),
    'empty before images rejected' => static fn () => SQLitePagerJournalSavepointHotRollbackCurrentSourceNext118Plan::currentSourceNext($databasePath, $databaseBytes, $journalBytes, $pageSize, 'plugin-retry', [], $retryWrites),
    'empty retry writes rejected' => static fn () => SQLitePagerJournalSavepointHotRollbackCurrentSourceNext118Plan::currentSourceNext($databasePath, $databaseBytes, $journalBytes, $pageSize, 'plugin-retry', $beforeImages, []),
    'retry without before image rejected' => static fn () => SQLitePagerJournalSavepointHotRollbackCurrentSourceNext118Plan::currentSourceNext($databasePath, $databaseBytes, $journalBytes, $pageSize, 'plugin-retry', [2 => $clean2], $retryWrites),
    'bad before page rejected' => static fn () => SQLitePagerJournalSavepointHotRollbackCurrentSourceNext118Plan::currentSourceNext($databasePath, $databaseBytes, $journalBytes, $pageSize, 'plugin-retry', [0 => $clean2], $retryWrites),
    'short before image rejected' => static fn () => SQLitePagerJournalSavepointHotRollbackCurrentSourceNext118Plan::currentSourceNext($databasePath, $databaseBytes, $journalBytes, $pageSize, 'plugin-retry', [2 => 'short'], $retryWrites),
    'bad retry page rejected' => static fn () => SQLitePagerJournalSavepointHotRollbackCurrentSourceNext118Plan::currentSourceNext($databasePath, $databaseBytes, $journalBytes, $pageSize, 'plugin-retry', $beforeImages, [0 => $retry2]),
    'short retry image rejected' => static fn () => SQLitePagerJournalSavepointHotRollbackCurrentSourceNext118Plan::currentSourceNext($databasePath, $databaseBytes, $journalBytes, $pageSize, 'plugin-retry', $beforeImages, [2 => 'short']),
];
