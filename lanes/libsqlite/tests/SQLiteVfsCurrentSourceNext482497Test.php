...
$plan = static function () use ($readyCurrent): array {
    static $result = null;
    if ($result === null) {
        $result = SQLiteVfsCurrentSourceNext482497Plan::run([
...
return [
    ...
    'vfs current source next482-497 blocks dirty snapshot' => static fn (TestRunner $t) => $t->same(true, in_array('dirty-pages-present', SQLiteVfsCurrentSourceNext482497Plan::run(['snapshot(reader-ready-next497,shared-cache-next481)'], ['current' => $dirtyCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source next482-497 blocks old snapshot ack' => static fn (TestRunner $t) => $t->same(true, in_array('ack-not-latest-publish', SQLiteVfsCurrentSourceNext482497Plan::run(['snapshot(reader-ready-next497,shared-cache-next481)'], ['current' => $oldAckCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source next482-497 blocks publish without claim' => static fn (TestRunner $t) => $t->same(true, in_array('missing-reuse-claim', SQLiteVfsCurrentSourceNext482497Plan::run(['snapshot(reader-ready-next497,shared-cache-next481)', 'publish(reader-ready-next497,reader-reuse-next497,shared-cache-next497)'], ['current' => $readyCurrent])['events'][1]['blocked_reasons'], true)),
    'vfs current source next482-497 blocks stale claim' => static fn (TestRunner $t) => $t->same(true, in_array('stale-reuse-claim', SQLiteVfsCurrentSourceNext482497Plan::run(['publish(reader-ready-next497,reader-reuse-next497,shared-cache-next497)'], ['current' => $staleClaimCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source next482-497 accepts preclaimed publish' => static fn (TestRunner $t) => $t->same('published-reused-current-source', SQLiteVfsCurrentSourceNext482497Plan::run(['publish(reader-ready-next497,reader-reuse-next497,shared-cache-next497)'], ['current' => $claimedCurrent])['events'][0]['status']),
    'vfs current source next482-497 rejects empty operations' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteVfsCurrentSourceNext482497Plan::run([])),
    'vfs current source next482-497 rejects bad claim token' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteVfsCurrentSourceNext482497Plan::run([['op' => 'claim', 'snapshot' => 'reader-ready-next497', 'ack' => 'shared-cache-next481', 'claim' => 'bad claim']], ['current' => $readyCurrent])),
    'vfs current source next482-497 rejects unsupported operation' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteVfsCurrentSourceNext482497Plan::run(['republish(reader-ready-next497,shared-cache-next481)'], ['current' => $readyCurrent])),
