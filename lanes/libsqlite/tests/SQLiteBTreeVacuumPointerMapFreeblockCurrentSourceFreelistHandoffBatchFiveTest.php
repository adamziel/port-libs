...
$makeFirstPage10711086 = static function (int $pageCount): string {
...
$putPointerMapEntry10711086 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database10711086 = static function (int $sliceNumber) use ($makeFirstPage10711086, $putPointerMapEntry10711086): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage10711086(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry10711086($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan10711086 = static function (int $sliceNumber, int $batchSize = 2) use ($database10711086): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database10711086($sliceNumber);
...
foreach (range(1071, 1086) as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source freelist handoff case {$sliceNumber} receipts"] = static function (TestRunner $t) use ($plan10711086, $sliceNumber): void {
        $plan = $plan10711086($sliceNumber);
        ...
        $t->same(6, $plan10711086($sliceNumber, 3)->currentSourceSummary()['current_source_row_count']);
    };
}
