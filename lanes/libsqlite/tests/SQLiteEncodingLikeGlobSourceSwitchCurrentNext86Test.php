...
): array => SQLiteEncodingLikeGlobSourceSwitchPlan::wordpressOptionNameSourceSwitch(
    $currentRows,
    $nextRows,
    $pattern,
    $operator,
...
$tests['encoding like glob source current next86 rejects malformed next source bytes'] = static function (TestRunner $t) use ($currentRows): void {
    ...
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteEncodingLikeGlobSourceSwitchPlan::wordpressOptionNameSourceSwitch($currentRows, $next, 'plugin%'));
};
...
$tests['encoding like glob source current next86 rejects missing current rowid'] = static function (TestRunner $t) use ($nextRows): void {
    ...
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteEncodingLikeGlobSourceSwitchPlan::wordpressOptionNameSourceSwitch($current, $nextRows, 'plugin%'));
};
