...
$cases = [
    'status ok' => ['status', 'ok'],
    'operation name' => ['operation', 'pragma-foreign-key-list-recursive-current-source-next106'],
    ...
    'dependency marker present' => ['dependencies.0', 'sqlite-pragma-foreign-key-list-recursive-current-source-next106'],
];

$tests = [];
foreach ($cases as $name => [$path, $expected]) {
    $tests['pragma foreign key recursive current-source next106 ' . $name] = static function (TestRunner $t) use ($makeCatalog, $nextRecords, $valueAt, $path, $expected): void {
        $plan = $makeCatalog()->foreignKeyListCurrentSourceNext106("pragma_foreign_key_list('wp_option_dependency')", $nextRecords());
        $t->same($expected, $valueAt($plan, $path));
    };
}

$tests['pragma foreign key recursive current-source next106 cursor current stays stable after schema replace'] = static function (TestRunner $t) use ($makeCatalog, $nextRecords): void {
    $plan = $makeCatalog()->foreignKeyListCurrentSourceNext106("pragma_foreign_key_list('wp_option_dependency')", $nextRecords());
...
$tests['pragma foreign key recursive current-source next106 cursor next exposes recursive rows'] = static function (TestRunner $t) use ($makeCatalog, $nextRecords): void {
    $plan = $makeCatalog()->foreignKeyListCurrentSourceNext106("pragma_foreign_key_list('wp_option_dependency')", $nextRecords());
...
$tests['pragma foreign key recursive current-source next106 schema-pinned archive table'] = static function (TestRunner $t) use ($record): void {
    ...
    $plan = $catalog->foreignKeyListCurrentSourceNext106(
...
$tests['pragma foreign key recursive current-source next106 rejects non foreign-key pragma'] = static function (TestRunner $t) use ($makeCatalog, $nextRecords): void {
    $t->throws(InvalidArgumentException::class, static fn () => $makeCatalog()->foreignKeyListCurrentSourceNext106("pragma_table_info('wp_option_dependency')", $nextRecords()));
};
