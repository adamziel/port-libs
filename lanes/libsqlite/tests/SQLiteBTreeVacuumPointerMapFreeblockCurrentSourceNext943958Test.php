...
$makeFirstPage943958 = static function (int $pageCount): string {
...
$putPointerMapEntry943958 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database943958 = static function (int $sliceNumber) use ($makeFirstPage943958, $putPointerMapEntry943958): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage943958(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry943958($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan943958 = static function (int $sliceNumber, int $batchSize = 2) use ($database943958): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database943958($sliceNumber);
...
foreach (range(943, 958) as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} extends next943-958 handoff receipts"] = static function (TestRunner $t) use ($plan943958, $sliceNumber): void {
        $plan = $plan943958($sliceNumber);
        ...
        $t->same(6, $plan943958($sliceNumber, 3)->currentSourceSummary()['current_source_row_count']);
    };
}
