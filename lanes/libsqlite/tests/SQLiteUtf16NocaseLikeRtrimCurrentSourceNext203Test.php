...
$plan203 = static fn (
    ...
    int $nextCookie = 203,
): array => SQLiteUtf16NocaseLikeRtrimCurrentSourceNextPlan::keyValueRowKeyFullScanPlan(
    $current ?? $current203,
...
$cases203 = [
    ...
    'current source' => ['currentSource', 'main.app_settings@202'],
    'next source' => ['nextSource', 'main.app_settings@203'],
...
$tests['utf16 nocase like rtrim current source nextTwoZeroThree stable source still cannot reuse prefix cursor'] = static function (TestRunner $t) use ($row203): void {
    ...
    $result = SQLiteUtf16NocaseLikeRtrimCurrentSourceNextPlan::keyValueRowKeyFullScanPlan(
        $rows,
        $rows,
        '%cache',
        null,
