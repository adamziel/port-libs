...
$plan239 = static fn (
    ...
    int $nextCookie = 239,
): array => SQLiteEncodingCollationAffinityGlobCurrentSourceNextPlan::wordpressOptionValueMalformedGlobPlan(
    $current ?? $current239,
    $next ?? $next239,
    $pattern,
    $currentSource,
...
$tests['encoding collation affinity glob current source next239 valid euro does not match malformed byte range'] = static function (TestRunner $t) use ($current239, $next239): void {
    $plan = SQLiteEncodingCollationAffinityGlobCurrentSourceNextPlan::wordpressOptionValueMalformedGlobPlan($current239, $next239, "plugin_[\xe2-\xe2]*");
...
$tests['encoding collation affinity glob current source next239 continuation byte range matches separate byte'] = static function (TestRunner $t) use ($current239, $next239): void {
    $plan = SQLiteEncodingCollationAffinityGlobCurrentSourceNextPlan::wordpressOptionValueMalformedGlobPlan($current239, $next239, "plugin_[\x80-\x8f]*");
...
$tests['encoding collation affinity glob current source next239 question consumes one malformed byte'] = static function (TestRunner $t): void {
    ...
    $plan = SQLiteEncodingCollationAffinityGlobCurrentSourceNextPlan::wordpressOptionValueMalformedGlobPlan($rows, $rows, 'legacy_?_tail', 'same', 'same', 1, 1);
...
$tests['encoding collation affinity glob current source next239 negated malformed byte class excludes e2'] = static function (TestRunner $t) use ($current239, $next239): void {
    $plan = SQLiteEncodingCollationAffinityGlobCurrentSourceNextPlan::wordpressOptionValueMalformedGlobPlan($current239, $next239, "plugin_[^\xe2]*");
...
$tests['encoding collation affinity glob current source next239 reversed malformed range is literal start only'] = static function (TestRunner $t) use ($current239, $next239): void {
    $plan = SQLiteEncodingCollationAffinityGlobCurrentSourceNextPlan::wordpressOptionValueMalformedGlobPlan($current239, $next239, "plugin_[\xe3-\xe2]*");
...
$tests['encoding collation affinity glob current source next239 numeric affinity participates'] = static function (TestRunner $t): void {
    ...
    $plan = SQLiteEncodingCollationAffinityGlobCurrentSourceNextPlan::wordpressOptionValueMalformedGlobPlan($rows, $rows, '42*', 'same', 'same', 1, 1);
...
$tests['encoding collation affinity glob current source next239 blob and null remain non matches'] = static function (TestRunner $t): void {
    ...
    $plan = SQLiteEncodingCollationAffinityGlobCurrentSourceNextPlan::wordpressOptionValueMalformedGlobPlan($rows, $rows, "plugin_[\xe2-\xe2]*", 'same', 'same', 1, 1);
    $t->same([], $plan['currentRowids']);
...
$tests['encoding collation affinity glob current source next239 rejects missing option value'] = static function (TestRunner $t) use ($next239): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteEncodingCollationAffinityGlobCurrentSourceNextPlan::wordpressOptionValueMalformedGlobPlan([['option_id' => 1]], $next239, "plugin_[\xe2-\xe2]*"));
};
...
$tests['encoding collation affinity glob current source next239 rejects non scalar value'] = static function (TestRunner $t) use ($next239): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteEncodingCollationAffinityGlobCurrentSourceNextPlan::wordpressOptionValueMalformedGlobPlan([['option_id' => 1, 'option_value' => ['x']]], $next239, "plugin_[\xe2-\xe2]*"));
};
