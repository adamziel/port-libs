...
$plan = static fn (): array => SQLitePagerHotJournalCacheSavepointCurrentSourceNext131Plan::currentSourceNext(
    $pageSize,
    'wp_outer_import',
    'plugin_batch',
    'retry_option_write',
...
$tests['pager hot journal cache savepoint current source next131 rejects bad page size'] = static function (TestRunner $t) use ($page): void {
    $t->throws(InvalidArgumentException::class, static fn (): mixed => SQLitePagerHotJournalCacheSavepointCurrentSourceNext131Plan::currentSourceNext(0, 'outer', 'inner', 'stmt', 'a', 'b', [], [1 => $page('hot')], [1 => $page('outer')], [1 => $page('inner')], [1 => $page('next')], [1]));
};
...
$tests['pager hot journal cache savepoint current source next131 rejects empty name'] = static function (TestRunner $t) use ($page): void {
    $t->throws(InvalidArgumentException::class, static fn (): mixed => SQLitePagerHotJournalCacheSavepointCurrentSourceNext131Plan::currentSourceNext(88, '', 'inner', 'stmt', 'a', 'b', [], [1 => $page('hot')], [1 => $page('outer')], [1 => $page('inner')], [1 => $page('next')], [1]));
};
...
$tests['pager hot journal cache savepoint current source next131 rejects duplicate savepoint'] = static function (TestRunner $t) use ($page): void {
    $t->throws(InvalidArgumentException::class, static fn (): mixed => SQLitePagerHotJournalCacheSavepointCurrentSourceNext131Plan::currentSourceNext(88, 's', 's', 'stmt', 'a', 'b', [], [1 => $page('hot')], [1 => $page('outer')], [1 => $page('inner')], [1 => $page('next')], [1]));
};
...
$tests['pager hot journal cache savepoint current source next131 rejects unchanged source'] = static function (TestRunner $t) use ($page): void {
    $t->throws(InvalidArgumentException::class, static fn (): mixed => SQLitePagerHotJournalCacheSavepointCurrentSourceNext131Plan::currentSourceNext(88, 'outer', 'inner', 'stmt', 'a', 'a', [], [1 => $page('hot')], [1 => $page('outer')], [1 => $page('inner')], [1 => $page('next')], [1]));
};
...
$tests['pager hot journal cache savepoint current source next131 rejects zero epoch'] = static function (TestRunner $t) use ($page): void {
    $t->throws(InvalidArgumentException::class, static fn (): mixed => SQLitePagerHotJournalCacheSavepointCurrentSourceNext131Plan::currentSourceNext(88, 'outer', 'inner', 'stmt', 'a', 'b', [], [1 => $page('hot')], [1 => $page('outer')], [1 => $page('inner')], [1 => $page('next')], [1], 0));
};
...
$tests['pager hot journal cache savepoint current source next131 rejects empty pages'] = static function (TestRunner $t) use ($page): void {
    $t->throws(InvalidArgumentException::class, static fn (): mixed => SQLitePagerHotJournalCacheSavepointCurrentSourceNext131Plan::currentSourceNext(88, 'outer', 'inner', 'stmt', 'a', 'b', [], [], [1 => $page('outer')], [1 => $page('inner')], [1 => $page('next')], [1]));
};
...
$tests['pager hot journal cache savepoint current source next131 rejects bad cache page'] = static function (TestRunner $t) use ($page): void {
    $t->throws(InvalidArgumentException::class, static fn (): mixed => SQLitePagerHotJournalCacheSavepointCurrentSourceNext131Plan::currentSourceNext(88, 'outer', 'inner', 'stmt', 'a', 'b', [0 => ['image' => $page('cache')]], [1 => $page('hot')], [1 => $page('outer')], [1 => $page('inner')], [1 => $page('next')], [1]));
};
...
$tests['pager hot journal cache savepoint current source next131 rejects short hot page'] = static function (TestRunner $t) use ($page): void {
    $t->throws(InvalidArgumentException::class, static fn (): mixed => SQLitePagerHotJournalCacheSavepointCurrentSourceNext131Plan::currentSourceNext(88, 'outer', 'inner', 'stmt', 'a', 'b', [], [1 => 'short'], [1 => $page('outer')], [1 => $page('inner')], [1 => $page('next')], [1]));
};
...
$tests['pager hot journal cache savepoint current source next131 rejects bad cursor page'] = static function (TestRunner $t) use ($page): void {
    $t->throws(InvalidArgumentException::class, static fn (): mixed => SQLitePagerHotJournalCacheSavepointCurrentSourceNext131Plan::currentSourceNext(88, 'outer', 'inner', 'stmt', 'a', 'b', [], [1 => $page('hot')], [1 => $page('outer')], [1 => $page('inner')], [1 => $page('next')], [0]));
};
