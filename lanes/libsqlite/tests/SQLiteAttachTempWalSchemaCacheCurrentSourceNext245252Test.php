...
$plan245252 = static fn (array $events, ?array $statements = null, ?array $schemas = null): array => SQLiteAttachWalTempSchemaCacheCurrentSourceNext92Plan::currentSourceNext245252(
    $schemas ?? $schemas245252,
    $statements ?? $statements245252,
    $events,
);

$tests = [];

$tests['attach temp wal schema cache current source next245 main wal commit expires main statements'] = static function (TestRunner $t) use ($plan245252): void {
    $result = $plan245252([
        ['op' => 'wal_commit', 'schema' => 'main', 'schema_cookie' => 238, 'table' => 'wp_postmeta', 'indexes' => ['wp_postmeta_key'], 'commit' => true],
    ]);

    $t->same('attach-wal-temp-schema-cache-current-source-next245-252', $result['operation']);
    $t->same('sqlite-attach-temp-wal-schema-cache-current-source-next245', $result['dependencies'][0]);
    $t->same('sqlite-attach-temp-wal-schema-cache-current-source-next252', $result['dependencies'][7]);
    $t->same('sqlite-attach-temp-wal-schema-cache-current-source-next244', $result['dependencies'][15]);
...
$tests['attach temp wal schema cache current source next246 temp options drop reveals main options source'] = static function (TestRunner $t) use ($plan245252): void {
    $result = $plan245252([
...
$tests['attach temp wal schema cache current source next247 audit index rename blocks audit writer'] = static function (TestRunner $t) use ($plan245252): void {
    $result = $plan245252([
...
$tests['attach temp wal schema cache current source next248 attach queue schema leaves prepared sources stable'] = static function (TestRunner $t) use ($plan245252): void {
    $result = $plan245252([
...
$tests['attach temp wal schema cache current source next249 detach search expires explicit search reader'] = static function (TestRunner $t) use ($plan245252): void {
    $result = $plan245252([
...
$tests['attach temp wal schema cache current source next250 analytics rollup drop leaves main and audit stable'] = static function (TestRunner $t) use ($plan245252): void {
    $result = $plan245252([
...
$tests['attach temp wal schema cache current source next251 uncommitted audit wal frame is ignored'] = static function (TestRunner $t) use ($plan245252): void {
    $result = $plan245252([
...
$tests['attach temp wal schema cache current source next252 combined ddl keeps active readers on current source'] = static function (TestRunner $t) use ($plan245252): void {
    $result = $plan245252([
