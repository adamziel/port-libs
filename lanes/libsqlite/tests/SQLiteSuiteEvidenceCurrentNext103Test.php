...
function libsqlite_suite_evidence103(): SQLiteUpstreamSuiteEvidence
{
    return SQLiteUpstreamSuiteEvidence::fromManifestPath(__DIR__ . '/../UPSTREAM_TEST_MANIFEST.json');
}

function libsqlite_suite_evidence103_output(int $passLines = 12, int $assertions = 112, int $failures = 0): string
{
    $lines = ['Focused test run: 1 selected test files (root lock skipped)'];
    for ($i = 1; $i <= $passLines; $i++) {
        $lines[] = sprintf('PASS current next103 suite evidence case %02d', $i);
    }
 ...
 */
function libsqlite_suite_evidence103_rows(
    ...
    return [
        [
            ...
            'scripts' => [sprintf('pager-current-next103-%02d.test', $case), 'attach3.test', 'wal2.test'],
            'exit' => 0,
            'errors' => 0,
            'current_tests' => 0,
 ...
 */
function libsqlite_suite_evidence103_record(
    array $rows,
    string $head = 'c196709c053869bec78f15d5a1f299d396f8fdb0',
    string $output = null,
    ?int $expected = 12,
    ...
    return libsqlite_suite_evidence103()->suiteEvidenceSliceCurrentNext103(
        $rows,
        489,
        29294,
        $head,
...
return [
    'current next103 counts one bounded suite evidence row after next102' => static function (TestRunner $t): void {
        $record = libsqlite_suite_evidence103_record(libsqlite_suite_evidence103_rows(case: 11));

        $t->same('current-next103-suite-evidence-countable', $record['status']);
        $t->same(true, $record['countable']);
        $t->same(489, $record['current_mapped']);
        $t->same(490, $record['next_mapped']);
        $t->same(1, $record['mapped_delta']);
        $t->same(12, $record['php_pass_delta']);
        $t->same(29306, $record['next_php_pass']);
    ...
    'current next103 records tiers scripts and test delta' => static function (TestRunner $t): void {
        ...
        $t->same([
            'attach3.test',
            'pager-current-next102-10.test',
            'pager-current-next103-07.test',
            'wal2.test',
        ], $record['target_scripts']);
    },
    'current next103 preserves already counted suite evidence' => static function (TestRunner $t): void {
        $rows = libsqlite_suite_evidence103_rows();
        $rows[0]['current_countable'] = true;
        $rows[0]['current_tests'] = $rows[0]['next_tests'];

        $record = libsqlite_suite_evidence103_record($rows);

        $t->same('current-next103-suite-evidence-preserved', $record['status']);
        $t->same(0, $record['mapped_delta']);
        $t->same(489, $record['next_mapped']);
        $t->same([
            'suite-evidence-current-next103-focused-artifact',
            'suite-evidence-current-next103-next102-baseline',
        ], $record['preserved_units']);
    },
    'current next103 blocks stale head and non local artifact' => static function (TestRunner $t): void {
        $rows = libsqlite_suite_evidence103_rows(head: '0000000000000000000000000000000000000000');
    ...
    'current next103 blocks bad runner evidence fields' => static function (TestRunner $t): void {
        $rows = libsqlite_suite_evidence103_rows();
    ...
    'current next103 blocks duplicate regression parity and active runner' => static function (TestRunner $t): void {
        $rows = libsqlite_suite_evidence103_rows();
        $rows[0]['counts_release_parity'] = true;
        $rows[1]['next_countable'] = false;
        $rows[] = $rows[0];

        $record = libsqlite_suite_evidence103_record($rows, snapshot: '123 testfixture ../libsqlite/test/testrunner.tcl --jobs 4 all');
        $evidence = implode('; ', array_column($record['blockers'], 'evidence'));
        ...
        $t->same(['suite-evidence-current-next103-next102-baseline'], $record['regressed_units']);
    },
    'current next103 blocks focused php admission mismatch' => static function (TestRunner $t): void {
        $record = libsqlite_suite_evidence103_record(
            libsqlite_suite_evidence103_rows(),
            output: libsqlite_suite_evidence103_output(passLines: 11),
            expected: 12
        );
    ...
    'current next103 records dependency closure without parity claim' => static function (TestRunner $t): void {
        $record = libsqlite_suite_evidence103_record(libsqlite_suite_evidence103_rows());
    ...
    'current next103 rejects empty row list' => static function (TestRunner $t): void {
        $t->throws(InvalidArgumentException::class, static fn () => libsqlite_suite_evidence103_record([]));
    },
];
