...
$plan637652 = static fn (array $events, ?array $statements = null, ?array $schemas = null): array => SQLiteAttachWalTempSchemaCacheCurrentSourceNext92Plan::currentSourceNext637652(
...
$tests['attach temp wal schema cache current source next637-652 extends next621-636 handoff'] = static function (TestRunner $t) use ($plan637652): void {
    ...
    $t->same('attach-wal-temp-schema-cache-current-source-next637-652', $result['operation']);
    $t->same('sqlite-attach-temp-wal-schema-cache-current-source-next637', $result['dependencies'][0]);
    $t->same('sqlite-attach-temp-wal-schema-cache-current-source-next652', $result['dependencies'][15]);
    $t->same('sqlite-attach-temp-wal-schema-cache-current-source-next621', $result['dependencies'][16]);
    $t->same('sqlite-attach-temp-wal-schema-cache-current-source-next636', $result['dependencies'][31]);
