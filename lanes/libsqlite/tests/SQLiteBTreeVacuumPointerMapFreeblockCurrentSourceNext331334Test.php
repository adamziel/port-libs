...
$makeFirstPage331334 = static function (int $pageCount): string {
...
$putPointerMapEntry331334 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database331334 = static function (int $sliceNumber) use ($makeFirstPage331334, $putPointerMapEntry331334): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage331334(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry331334($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan331334 = static function (int $sliceNumber, int $batchSize = 2) use ($database331334): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database331334($sliceNumber);
...
foreach ([331, 332, 333, 334] as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} keeps final next331-334 freelist splice isolated"] = static function (TestRunner $t) use ($plan331334, $sliceNumber): void {
        $plan = $plan331334($sliceNumber);
        ...
        $t->same(6, $plan331334($sliceNumber, 3)->freelistSummary()['freelist_row_count']);
    };
}
