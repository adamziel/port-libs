...
$pageSize = 512;
$database = '/srv/wp-content/database/wp-next269.sqlite';
$usersDatabase = '/srv/wp-content/database/wp-next269-users.sqlite';
$mainJournal = $database . '-journal';
$usersJournal = $usersDatabase . '-journal';
$master = $database . '-mj';
$sourceId = 'pager-reader-cache-rollback-source-next269';
...
$formatPage = static function (string $label) use ($pageSize): string {
    ...
    $page = substr_replace($page, pack('N', 269), 60, 4);
    $page = substr_replace($page, pack('N', 0x57503269), 68, 4);
...
$before = [
    1 => $formatPage('next269 stale schema before rollback-source fence'),
    2 => $page('next269 stale wp_options root before rollback-source fence'),
    3 => $page('next269 stale active_plugins before rollback-source fence'),
    4 => $page('next269 stale cron before rollback-source fence'),
    5 => $page('next269 stale plugin cache before rollback-source fence'),
    6 => $page('next269 stale autoload index before rollback-source fence'),
];
$recovered = [
    1 => $formatPage('next269 current schema after rollback-source fence'),
    2 => $page('next269 current wp_options root after rollback-source fence'),
    3 => $page('next269 current active_plugins after rollback-source fence'),
    4 => $page('next269 current cron after rollback-source fence'),
    5 => $page('next269 current plugin cache after rollback-source fence'),
    6 => $page('next269 current autoload index after rollback-source fence'),
];
$tokens = [
    $mainJournal => 'dev=8:ino=2691:size=4096:mtime=26901:generation=main-current',
    $usersJournal => 'dev=8:ino=2692:size=1024:mtime=26902:generation=users-current',
];
$headers = [
    $mainJournal => hash('sha256', 'main rollback header next269'),
    $usersJournal => hash('sha256', 'users rollback header next269'),
];
$formatSignature = hash('sha256', implode('|', [512, 4, 2, 269, 0x57503269]));
$masterDigest = hash('sha256', 'next269 master source');
$masterToken = 'dev=8:ino=2690:size=96:mtime=26900:generation=master-current';
$databaseToken = 'dev=8:ino=2699:size=3072:mtime=26999:generation=database-current';
$cleanupToken = 'master-cleanup:deleted:mtime=26700:dirsync=ok';
$readerLeaseToken = 'reader-lease:shared-cache:epoch=269:opened-after-master-cleanup';
$pagerCacheSourceToken = 'pager-cache-source:epoch=269:master-journal-recovery=complete';
$readTransactionToken = 'read-transaction:epoch=269:schema=117:change-counter=269:master-current';
$schemaReparseToken = 'schema-reparse:epoch=269:schema-cookie=117:ddl=master-current';
$statementSchemaRootToken = 'statement-schema-root:epoch=269:root=1:cookie=117:sql=wp-options-current';
$currentSourceProvenanceToken = 'current-source:master-journal:epoch=269:members=2:database-token=2699:schema=117';
$oldCurrentSourceProvenanceToken = 'current-source:master-journal:epoch=265:members=2:database-token=2659:schema=116';
$currentPagerReaderCacheGenerationToken = 'pager-reader-cache-generation:epoch=269:master-current:reset=complete';
$oldPagerReaderCacheGenerationToken = 'pager-reader-cache-generation:epoch=265:before-master-reset';
$currentReaderSnapshotToken = 'reader-snapshot:epoch=269:source=master-current:lease=shared:pages=1,2,3,4,5,6';
$oldReaderSnapshotToken = 'reader-snapshot:epoch=265:source=before-master-current:lease=shared';
$currentRecoveryReceiptToken = 'master-journal-recovery-receipt:epoch=269:members=main,users:cleanup=deleted:dirsync=ok';
$oldRecoveryReceiptToken = 'master-journal-recovery-receipt:epoch=265:members=main,users:cleanup=pending';
$currentPagerSpillDrainToken = 'pager-spill-drain:epoch=269:dirty-pages=2,6:journal-sync=ok:exclusive-lock=held';
$currentRollbackJournalReaderSourceToken = 'pager-rollback-source:epoch=269:dirty-pages=2,6:journal-sync=ok:exclusive-lock=held';
$oldRollbackJournalReaderSourceToken = 'pager-rollback-source:epoch=265:dirty-pages=2,6:journal-sync=pending';
$base = [
    'source_id' => $sourceId,
    'epoch' => 269,
    'format_signature' => $formatSignature,
    'publication_generation' => 269,
    'master_source_digest' => $masterDigest,
    'recovery_sequence' => 269,
...
$plan = static fn (?array $readerCache = null, ?array $nextReads = null, ?string $rollbackSourceToken = null): array => SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::variantNext269(
    $database,
    $master,
    $masterBytes,
    implode('', $before),
...
$cases = [
    'status' => [static fn (): mixed => $plan()['status'], 'pager-master-journal-reader-cache-current-source-next269'],
    ...
    'operation invalidates stale rollback-source cache' => [static fn (): mixed => $opCount($plan(), 'invalidate_reader_cache_rollback_journal_reader_source_current_source_next269'), 1],
    'operation reopens stale rollback-source reader' => [static fn (): mixed => $opCount($plan(), 'reopen_reader_for_rollback_journal_reader_source_current_source_next269'), 1],
    'dependency next269' => [static fn (): mixed => in_array('sqlite-pager-master-journal-reader-cache-current-source-next269', $plan()['dependencies'], true), true],
    ...
    'read stale rollback-source source' => [static fn (): mixed => $readRow('read-2')['source'], 'master-journal-reader-cache-rollback-source-fence-current-source-next269'],
    ...
    'changed current rollback-source invalidates admitted pages' => [static fn (): mixed => $plan(null, null, 'pager-rollback-source:epoch=267:dirty-pages=2,6:journal-sync=ok:exclusive-lock=held')['rollback_journal_reader_source_invalidated_cache_page_numbers'], [1, 2, 6]],
    'changed current rollback-source keeps inherited invalidation' => [static fn (): mixed => in_array(3, $plan(null, null, 'pager-rollback-source:epoch=267:dirty-pages=2,6:journal-sync=ok:exclusive-lock=held')['invalidated_cache_page_numbers'], true), true],
    'changed current rollback-source surfaced' => [static fn (): mixed => $plan(null, null, 'pager-rollback-source:epoch=267:dirty-pages=2,6:journal-sync=ok:exclusive-lock=held')['current_rollback_journal_reader_source_token'], 'pager-rollback-source:epoch=267:dirty-pages=2,6:journal-sync=ok:exclusive-lock=held'],
...
foreach ($cases as $name => [$callback, $expected]) {
    $tests['pager master journal reader cache current source next269 ' . $name] = static function (TestRunner $t) use ($callback, $expected): void {
...
foreach ($throws as $name => $callback) {
    $tests['pager master journal reader cache current source next269 ' . $name] = static function (TestRunner $t) use ($callback): void {
