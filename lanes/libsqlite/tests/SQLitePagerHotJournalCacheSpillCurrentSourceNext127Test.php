...
$plan = static fn (
    ...
    ?string $bytes = null,
    ?int $size = null,
): array => SQLitePagerHotJournalCacheSpillCurrentSourceNext127Plan::plan(
    $databasePath,
    $bytes ?? $databaseBytes,
    $size ?? $pageSize,
    $hotPages ?? $hot,
...
$throws = [
    'rejects empty path' => static fn () => SQLitePagerHotJournalCacheSpillCurrentSourceNext127Plan::plan('', $databaseBytes, $pageSize, $hot, $cache, 8, 3),
    'rejects bad page size' => static fn () => SQLitePagerHotJournalCacheSpillCurrentSourceNext127Plan::plan($databasePath, $databaseBytes, 500, $hot, $cache, 8, 3),
    'rejects empty database' => static fn () => SQLitePagerHotJournalCacheSpillCurrentSourceNext127Plan::plan($databasePath, '', $pageSize, $hot, $cache, 8, 3),
    'rejects unaligned database' => static fn () => SQLitePagerHotJournalCacheSpillCurrentSourceNext127Plan::plan($databasePath, $databaseBytes . 'x', $pageSize, $hot, $cache, 8, 3),
    'rejects empty hot journal' => static fn () => SQLitePagerHotJournalCacheSpillCurrentSourceNext127Plan::plan($databasePath, $databaseBytes, $pageSize, [], $cache, 8, 3),
    'rejects empty cache pages' => static fn () => SQLitePagerHotJournalCacheSpillCurrentSourceNext127Plan::plan($databasePath, $databaseBytes, $pageSize, $hot, [], 8, 3),
