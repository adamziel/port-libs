...
$tests['malformed utf16 like range current source next97 rejects unsupported operator'] = static function (TestRunner $t) use ($currentRows, $nextRows): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteMalformedLikeGlobSourceNextPlan::wordpressOptionNameCurrentNext($currentRows, $nextRows, 'plugin%', 'REGEXP'));
};
...
$tests['malformed utf16 like range current source next97 rejects missing bytes'] = static function (TestRunner $t) use ($nextRows): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteMalformedLikeGlobSourceNextPlan::wordpressOptionNameCurrentNext([['option_id' => 1, 'text_encoding' => 2]], $nextRows, 'plugin%'));
};
