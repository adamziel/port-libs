...
$pageSize = 512;
$databasePath = '/srv/www/wp-content/database/wp-next173.sqlite';
$page = static fn (string $label): string => str_pad($label, $pageSize, '.', STR_PAD_RIGHT);
$databaseBytes = $page('next173 dirty schema after interrupted import')
    . $page('next173 dirty wp_options root after interrupted import')
    . $page('next173 dirty active_plugins after interrupted import');
$journalBytes = 'hot-journal-next173:' . $page('next173 clean wp_options before failed import');
...
$currentWalBytes = $makeWalBytes([
    [1, 0, 'next173 current schema draft'],
    [2, 3, 'next173 current wp_options commit'],
    [3, 3, 'next173 current active_plugins commit'],
], 173, 0x17300101, 0x17300102);
$nextWalBytes = $makeWalBytes([
    [2, 0, 'next173 next wp_options draft'],
    [3, 3, 'next173 next active_plugins commit'],
...
$bootstrap = SQLiteWalHotJournalSavepointCheckpointCurrentSourceNextPlan::next167Plan(
    ...
    $pageSize,
    'wp-batch-next173',
    [2 => $page('next173 hot journal clean wp_options root')],
    [3 => $page('next173 savepoint before active_plugins')],
    $currentWal,
    $currentWalBytes,
    $nextWal,
    $nextWalBytes,
    [1 => ['image' => $page('next173 current schema draft'), 'source_id' => 'bootstrap', 'epoch' => 1]],
    [1, 2, 3],
    [
...
$prepared = SQLiteWalHotJournalSavepointCheckpointCurrentSourceNextPlan::next167Plan(
    ...
    $nextWalBytes,
    [
        1 => ['image' => $page('next173 current schema draft'), 'source_id' => $currentToken['id'], 'epoch' => $currentToken['epoch']],
        2 => ['image' => $page('next173 stale root cache'), 'source_id' => 'old', 'epoch' => 1],
    ],
    [1, 2, 3],
    [
...
$cases = [
    ...
    'operation reason first' => [static fn (): mixed => $ok()['operation_reasons'][0], 'publish_hot_journal_savepoint_checkpoint_database_current_source_next173'],
    'operation reason journal' => [static fn (): mixed => $ok()['operation_reasons'][3], 'delete_hot_journal_after_current_source_match_next173'],
    'operation reason directory' => [static fn (): mixed => $ok()['operation_reasons'][7], 'persist_hot_journal_savepoint_checkpoint_current_source_next173'],
    ...
    'dependency next173 present' => [static fn (): mixed => in_array('sqlite-wal-hot-journal-savepoint-checkpoint-current-source-next173', $ok()['dependencies'], true), true],
    ...
    'stale database status' => [static fn (): mixed => $staleDatabase()['status'], 'wal-hot-journal-savepoint-checkpoint-current-source-blocked-next173'],
    ...
    'pinned status' => [static fn (): mixed => $pinned()['status'], 'wal-hot-journal-savepoint-checkpoint-current-source-blocked-next173'],
...
foreach ($cases as $name => [$callback, $expected]) {
    $tests['wal hot journal savepoint checkpoint current source next173 ' . $name] = static function (TestRunner $t) use ($callback, $expected): void {
...
foreach ($throws as $name => $callback) {
    $tests['wal hot journal savepoint checkpoint current source next173 ' . $name] = static function (TestRunner $t) use ($callback): void {
