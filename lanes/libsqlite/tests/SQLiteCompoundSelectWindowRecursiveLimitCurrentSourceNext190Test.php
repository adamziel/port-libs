...
$currentTables190 = ['wp_options' => $currentOptions190];
$nextTables190 = ['wp_options' => $nextOptions190];
...
$summary190 = static fn (): array => SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNextPlan::compareNext190($sql190, $currentTables190, $nextTables190);
$tests = [];

$tests['compound select window recursive limit next190 status dependencies'] = static function (TestRunner $t) use ($summary190): void {
    $plan = $summary190();
    $t->same('compound-select-window-recursive-limit-current-source-next190-ready', $plan['status']);
    $t->same([
...
$tests['compound select window recursive limit next190 compound expression metadata'] = static function (TestRunner $t) use ($summary190): void {
    $compound = $summary190()['compound'];
...
$tests['compound select window recursive limit next190 recursive expression metadata'] = static function (TestRunner $t) use ($summary190): void {
    $recursive = $summary190()['recursive'];
...
$tests['compound select window recursive limit next190 current rows'] = static function (TestRunner $t) use ($summary190): void {
    $rows = $summary190()['currentRows'];
    $t->same(6, count($rows));
...
$tests['compound select window recursive limit next190 next rows'] = static function (TestRunner $t) use ($summary190): void {
    $rows = $summary190()['nextRows'];
    $t->same(6, count($rows));
...
$tests['compound select window recursive limit next190 windows before compound'] = static function (TestRunner $t) use ($summary190): void {
    $windows = $summary190()['windows'];
...
$tests['compound select window recursive limit next190 expression boundary'] = static function (TestRunner $t) use ($summary190): void {
    $boundary = $summary190()['expressionLimitBoundary'];
...
$tests['compound select window recursive limit next190 limit trace'] = static function (TestRunner $t) use ($summary190): void {
    $trace = $summary190()['limitTrace'];
...
$tests['compound select window recursive limit next190 replan reasons'] = static function (TestRunner $t) use ($summary190): void {
    $reasons = $summary190()['replanReasons'];
    $t->true(in_array('recursive-limit-expression-current-source-next190', $reasons, true));
    $t->true(in_array('compound-tail-limit-expression-current-source-next190', $reasons, true));
    $t->true(in_array('window-values-before-compound-limit-expression-next190', $reasons, true));
    $t->true(in_array('wordpress-option-source-boundary-shifts-expression-limit-next190', $reasons, true));
};

$tests['compound select window recursive limit next190 rejects literal recursive limit'] = static function (TestRunner $t) use ($currentTables190): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNextPlan::compareNext190(
...
$tests['compound select window recursive limit next190 rejects literal final limit'] = static function (TestRunner $t) use ($currentTables190): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNextPlan::compareNext190(
...
foreach (range(1, 58) as $case) {
    $tests['compound select window recursive limit next190 generated expression limit ' . $case] = static function (TestRunner $t) use ($case): void {
        ...
        $plan = SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNextPlan::compareNext190($generatedSql, $tables, $tables);
