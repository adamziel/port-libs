...
$sameRoot88 = static fn (): array => SQLiteJsonTablePlan::currentSourceHiddenConstraintPlannerNext88(
    'json_each',
    $current88,
    $next88,
    'option_value',
...
$conflictingRoot88 = static fn (): array => SQLiteJsonTablePlan::currentSourceHiddenConstraintPlannerNext88(
    'json_each',
    $current88,
    $next88,
    'option_value',
...
$conflictingJson88 = static fn (): array => SQLiteJsonTablePlan::currentSourceHiddenConstraintPlannerNext88(
    'json_each',
    $current88,
    $next88,
    'option_value',
...
$stable88 = static fn (): array => SQLiteJsonTablePlan::currentSourceHiddenConstraintPlannerNext88(
    'json_each',
    $current88,
    $current88,
    'option_value',
...
$jsonbNext88 = static fn (): array => SQLiteJsonTablePlan::currentSourceHiddenConstraintPlannerNext88(
    'json_each',
    $current88,
...
$nullNext88 = static fn (): array => SQLiteJsonTablePlan::currentSourceHiddenConstraintPlannerNext88(
    'json_each',
    $current88,
...
$tests = [
    ...
    'argument tape change remains reported' => static fn (TestRunner $t) => $t->true(in_array('source-argument-tape-changed', $sameRoot88()['next88ReplanReasons'], true)),
    ...
    'hidden residual presence is a next88 reason' => static fn (TestRunner $t) => $t->true(in_array('hidden-residual-constraint-present', $sameRoot88()['next88ReplanReasons'], true)),
    ...
    'rowset change is a next88 reason' => static fn (TestRunner $t) => $t->true(in_array('hidden-residual-rowset-changed', $sameRoot88()['next88ReplanReasons'], true)),
    ...
    'conflicting json rowset change is reported' => static fn (TestRunner $t) => $t->true(in_array('hidden-residual-rowset-changed', $conflictingJson88()['next88ReplanReasons'], true)),
    ...
    'stable duplicate hidden constraints still report residual presence' => static fn (TestRunner $t) => $t->same(['hidden-residual-constraint-present'], $stable88()['next88ReplanReasons']),
    ...
    'jsonb next reports kind changed' => static fn (TestRunner $t) => $t->true(in_array('source-json-kind-changed', $jsonbNext88()['next88ReplanReasons'], true)),
    'jsonb next remains valid' => static fn (TestRunner $t) => $t->same(true, $jsonbNext88()['next']['jsonValid']),
    'jsonb duplicate root residual survives' => static fn (TestRunner $t) => $t->same('root', $jsonbNext88()['nextHiddenResiduals'][0]['column']),
    'sql null next becomes unrunnable' => static fn (TestRunner $t) => $t->same(false, $nullNext88()['next']['runnable']),
    'sql null next has empty rows' => static fn (TestRunner $t) => $t->same([], $nullNext88()['nextRows']),
    'sql null next reports unrunnable' => static fn (TestRunner $t) => $t->true(in_array('next-source-plan-becomes-unrunnable', $nullNext88()['next88ReplanReasons'], true)),
    'missing json column is rejected' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteJsonTablePlan::currentSourceHiddenConstraintPlannerNext88('json_each', [], $next88, 'option_value')),
    'missing root column is rejected' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteJsonTablePlan::currentSourceHiddenConstraintPlannerNext88('json_each', $current88, $next88, 'option_value', [], 'missing_root')),
    'empty json column is rejected' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteJsonTablePlan::currentSourceHiddenConstraintPlannerNext88('json_each', $current88, $next88, '')),
    'empty root column is rejected' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteJsonTablePlan::currentSourceHiddenConstraintPlannerNext88('json_each', $current88, $next88, 'option_value', [], '')),
    'bad function is rejected' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteJsonTablePlan::currentSourceHiddenConstraintPlannerNext88('json_bad', $current88, $next88, 'option_value')),
];

foreach ($tests as $name => $case) {
    $tests['json table hidden constraint planner current source next88 ' . $name] = $case;
    unset($tests[$name]);
}
