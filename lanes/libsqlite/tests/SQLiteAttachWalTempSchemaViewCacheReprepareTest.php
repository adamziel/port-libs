...
$mainViewRecords = static fn (int $root = 40) => [
...
$tempViewRecords = static fn (int $root = 50) => [
...
$siteViewRecords = static fn (int $optionRoot = 60, int $blogRoot = 21) => [
...
foreach ($pathCases82 as $name => [$factory, $path, $expected]) {
    $tests['attach wal temp schema view cache reprepare ' . $name] = static function (TestRunner $t) use ($factory, $value82, $path, $expected): void {
...
foreach ($predicateCases82 as $name => $predicate) {
    $tests['attach wal temp schema view cache reprepare ' . $name] = static function (TestRunner $t) use ($predicate): void {
...
foreach ($errorCases82 as $name => $callback) {
    $tests['attach wal temp schema view cache reprepare ' . $name] = static function (TestRunner $t) use ($callback): void {
