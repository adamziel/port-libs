...
$tests['real upstream corpus date affinity dynamic date4 rows 15300 to 16299 cites upstream loop'] = static function (TestRunner $t) use ($date4Start, $date4Count): void {
...
for ($i = $date4Start; $i < $date4Start + $date4Count; $i++) {
    ...
    $tests[sprintf('real upstream corpus date affinity dynamic date4.test date4-%05d strftime libc format parity', $i)] = static function (TestRunner $t) use ($date4Format, $timestamp, $expectedDate4, $i): void {
...
$tests['real upstream corpus date affinity dynamic date4 rows 15300 to 16299 application audit rollup'] = static function (TestRunner $t) use ($date4Format, $date4Start, $date4Count, $expectedDate4): void {
    $sampleIndexes = [$date4Start, $date4Start + 111, $date4Start + 555, $date4Start + $date4Count - 1];
    $actual = [];
    $expected = [];
