...
use PortLibs\LibSqlite\SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext211Plan;
...
$summary211 = static fn (?array $cursor = null): array => SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext211Plan::compare($sql211, $currentTables211, $nextTables211, $cursor);
$tests = [];
...
$tests['compound select window recursive limit current source next211 rejects missing filter'] = static function (TestRunner $t) use ($currentTables211): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext211Plan::compare(
...
foreach (range(1, 56) as $case) {
    $tests['compound select window recursive limit current source next211 generated filtered boundary ' . $case] = static function (TestRunner $t) use ($case): void {
        ...
        $plan = SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext211Plan::compare($sql, $tables, $nextTables);
        $again = SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext211Plan::compare($sql, $tables, $nextTables, $plan['cursor']);
