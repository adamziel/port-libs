...
$row193 = static fn (int $id, string $name, int|string $encoding): array => [
    'setting_id' => $id,
    'key_name_bytes' => $enc193($name, $encoding),
...
$plan193 = static fn (
    ...
    int $nextCookie = 193,
): array => SQLiteUtf16NocaseLikeRtrimCurrentSourceNextPlan::keyValueRowKeyLimitOffsetPlan(
    $current ?? $current193,
...
$cases193 = [
    ...
    'expression' => ['expression', 'rtrim(key_name) COLLATE NOCASE LIKE ? ESCAPE ? LIMIT ? OFFSET ?'],
    ...
    'current source' => ['currentSource', 'main.app_settings@192'],
    'next source' => ['nextSource', 'main.app_settings@193'],
...
$tests['utf16 nocase like rtrim current source nextOneNineThree stable limit window resumes after window'] = static function (TestRunner $t) use ($row193): void {
    ...
    $result = SQLiteUtf16NocaseLikeRtrimCurrentSourceNextPlan::keyValueRowKeyLimitOffsetPlan(
        $rows,
        $rows,
        'plugin!_cache%',
        '!',
...
$tests['utf16 nocase like rtrim current source nextOneNineThree rejects negative limit'] = static function (TestRunner $t) use ($row193): void {
    ...
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteUtf16NocaseLikeRtrimCurrentSourceNextPlan::keyValueRowKeyLimitOffsetPlan(
        $rows,
        $rows,
        'plugin%',
        null,
