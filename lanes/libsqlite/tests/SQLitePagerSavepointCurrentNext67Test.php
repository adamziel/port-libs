...
$rollback = static fn (): array => SQLitePagerSavepointCurrentNextPlan::rollbackJournalLifecycleCurrentNext67(
    $events,
    ['op' => 'rollback_to', 'name' => 'plugin_batch', 'journal_mode' => 'delete', 'page_size' => 1024, 'database_page_count' => 16]
);
$release = static fn (): array => SQLitePagerSavepointCurrentNextPlan::rollbackJournalLifecycleCurrentNext67(
    $events,
    ['op' => 'release', 'name' => 'single_option', 'journal_mode' => 'persist', 'page_size' => 1024, 'database_page_count' => 16, 'super_journal_participant' => true]
);
$commit = static fn (): array => SQLitePagerSavepointCurrentNextPlan::rollbackJournalLifecycleCurrentNext67(
    $events,
    ['op' => 'commit', 'journal_mode' => 'truncate', 'page_size' => 1024, 'database_page_count' => 16]
);
$rollbackAll = static fn (): array => SQLitePagerSavepointCurrentNextPlan::rollbackJournalLifecycleCurrentNext67(
    $events,
    ['op' => 'rollback', 'journal_mode' => 'persist', 'page_size' => 1024, 'database_page_count' => 16, 'hot_journal' => true]
);
$implicitSavepoint = static fn (): array => SQLitePagerSavepointCurrentNextPlan::rollbackJournalLifecycleCurrentNext67(
    [],
...
$cases = [
    ...
    'rollback dependency current next67' => [static fn (): mixed => in_array('sqlite-pager-savepoint-current-next67', $rollback()['journal_lifecycle']['dependencies'], true), true],
...
foreach ($cases as $name => [$callback, $expected]) {
    $tests['pager savepoint current next67 ' . $name] = static function (TestRunner $t) use ($callback, $expected): void {
        $t->same($expected, $callback());
    };
}

$throws = [
    'rejects empty journal mode' => static fn () => SQLitePagerSavepointCurrentNextPlan::rollbackJournalLifecycleCurrentNext67($events, ['op' => 'commit', 'journal_mode' => '']),
    'rejects unsupported journal mode' => static fn () => SQLitePagerSavepointCurrentNextPlan::rollbackJournalLifecycleCurrentNext67($events, ['op' => 'commit', 'journal_mode' => 'wal']),
    'rejects bad page size' => static fn () => SQLitePagerSavepointCurrentNextPlan::rollbackJournalLifecycleCurrentNext67($events, ['op' => 'commit', 'page_size' => 0]),
    'rejects bad database page count' => static fn () => SQLitePagerSavepointCurrentNextPlan::rollbackJournalLifecycleCurrentNext67($events, ['op' => 'commit', 'database_page_count' => 0]),
    'rejects missing rollback savepoint' => static fn () => SQLitePagerSavepointCurrentNextPlan::rollbackJournalLifecycleCurrentNext67($events, ['op' => 'rollback_to', 'name' => 'missing']),
];

foreach ($throws as $name => $callback) {
    $tests['pager savepoint current next67 ' . $name] = static function (TestRunner $t) use ($callback): void {
