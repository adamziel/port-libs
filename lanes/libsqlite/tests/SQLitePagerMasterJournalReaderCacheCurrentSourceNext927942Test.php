...
$pageSize = 512;
$database = '/srv/wp-content/database/wp-next942.sqlite';
...
$formatPage = static function (string $label) use ($pageSize): string {
    ...
    $page = substr_replace($page, pack('N', 942), 60, 4);
...
$variantFields = array_merge($tokenFields, $next639654Fields, $next655670Fields, $next671686Fields, $next687702Fields, $next703718Fields, $next719734Fields, $next735766Fields, $next751766Fields, $next767782Fields, $next783798Fields, $next799814Fields, $next815830Fields, $next831846Fields, $next847862Fields, $next863878Fields, $next879894Fields, $next895910Fields, $next911926Fields, $next927942Fields);
$before = [1 => $formatPage('stale schema'), 2 => $page('stale options')];
$recovered = [1 => $formatPage('current schema'), 2 => $page('current options')];
$tokens = [$journal => 'member-main-current-942'];
$headers = [$journal => hash('sha256', 'main header next942')];
$base = [
    'source_id' => 'pager-reader-cache-current-source-next942',
    'epoch' => 942,
    'format_signature' => hash('sha256', implode('|', [512, 4, 2, 942, 0])),
    'publication_generation' => 942,
    'master_source_digest' => hash('sha256', 'master-next942'),
    'recovery_sequence' => 942,
...
foreach ($variantFields as $field) {
    $base[$field] = str_replace('_', '-', preg_replace('/_token$/', '', $field)) . '-current-942';
}
$base['reader_cache_stmt_vdbe_real_affinity_value_token'] = 'reader-cache-stmt-vdbe-affinity-current-942';
$base['reader_cache_stmt_vdbe_real_affinity_value_handoff_token'] = 'reader-cache-stmt-vdbe-affinity-handoff-current-942';
...
$plan = static fn (array $cacheExtra = [], array $readExtra = []): array => SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::currentSourceVdbeVirtualTableBranchConditionFence(
    $database,
    $master,
    $masterBytes,
    implode('', $before),
...
$tests['pager master journal reader cache current source next942 publishes current VDBE literal and arithmetic opcode fences'] = static function (TestRunner $t) use ($plan): void {
    $result = $plan();
    $t->same('pager-master-journal-reader-cache-current-source-next942', $result['status']);
    $t->same([1], $result['invalidated_cache_page_numbers']);
    $t->same(['read-options' => false], $result['read_cache_hits']);
    $t->same('reader-cache-stmt-vdbe-null-branch-current-942', $result['current_reader_cache_stmt_vdbe_null_branch_token']);
    $t->same('reader-cache-stmt-vdbe-opcode-trace-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_opcode_trace_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-shift-right-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_shift_right_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-gt-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_gt_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-incr-vacuum-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_incr_vacuum_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-init-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_init_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-goto-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_goto_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-null-row-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_null_row_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-row-value-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_row_value_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-real-affinity-value-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_real_affinity_value_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-cast-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_cast_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-column-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_column_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-affinity-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_affinity_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-cast-affinity-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_cast_affinity_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-compare-collseq-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_compare_collseq_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-once-flag-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_once_flag_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-vbegin-branch-condition-handoff-current-942', $result['current_reader_cache_stmt_vdbe_vbegin_branch_condition_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-else-eq-branch-condition-handoff-current-942', $result['current_reader_cache_stmt_vdbe_else_eq_branch_condition_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-if-open-branch-condition-handoff-current-942', $result['current_reader_cache_stmt_vdbe_if_open_branch_condition_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-transaction-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_transaction_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-vbegin-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_vbegin_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-vcreate-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_vcreate_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-vdestroy-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_vdestroy_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-vopen-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_vopen_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-vfilter-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_vfilter_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-vcolumn-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_vcolumn_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-vnext-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_vnext_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-vrename-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_vrename_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-pagecount-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_pagecount_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-maxpgcnt-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_maxpgcnt_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-cursorhint-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_cursorhint_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-noop-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_noop_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-gosub-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_gosub_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-return-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_return_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-yield-op-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_yield_op_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-halt-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_halt_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-soft-null-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_soft_null_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-boolean-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_boolean_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-zeroblob-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_zeroblob_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-string8-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_string8_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-concat-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_concat_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-add-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_add_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-subtract-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_subtract_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-multiply-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_multiply_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-divide-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_divide_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-remainder-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_remainder_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-bit-and-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_bit_and_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-bit-or-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_bit_or_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-shift-left-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_shift_left_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-shift-right-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_shift_right_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-add-imm-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_add_imm_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-bit-not-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_bit_not_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-affinity-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_affinity_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-cast-affinity-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_cast_affinity_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-permutation-affinity-branch-handoff-current-942', $result['current_reader_cache_stmt_vdbe_permutation_affinity_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-vnext-branch-condition-handoff-current-942', $result['current_reader_cache_stmt_vdbe_vnext_branch_condition_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-vrename-branch-condition-handoff-current-942', $result['current_reader_cache_stmt_vdbe_vrename_branch_condition_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-pagecount-branch-condition-handoff-current-942', $result['current_reader_cache_stmt_vdbe_pagecount_branch_condition_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-opcode-trace-branch-condition-handoff-current-942', $result['current_reader_cache_stmt_vdbe_opcode_trace_branch_condition_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-goto-branch-condition-handoff-current-942', $result['current_reader_cache_stmt_vdbe_goto_branch_condition_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-yield-op-branch-condition-handoff-current-942', $result['current_reader_cache_stmt_vdbe_yield_op_branch_condition_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-blob-branch-condition-handoff-current-942', $result['current_reader_cache_stmt_vdbe_blob_branch_condition_handoff_token']);
    ...
    $t->same(true, in_array('sqlite-pager-master-journal-reader-cache-current-source-next942', $result['dependencies'], true));
};

$tests['pager master journal reader cache current source plan dispatches full width calls to next942'] = static function (TestRunner $t) use ($database, $master, $masterBytes, $before, $pageSize, $recovered, $cacheEntry, $read, $base, $tokens, $headers, $variantFields): void {
...
$tests['pager master journal reader cache current source next942 preserves next934 goto branch condition handoff continuity'] = static function (TestRunner $t) use ($plan, $opCount): void {
...
$tests['pager master journal reader cache current source next942 reopens stale blob branch condition handoff read ticket'] = static function (TestRunner $t) use ($plan, $opCount): void {
    $result = $plan([], ['reader_cache_stmt_vdbe_blob_branch_condition_handoff_token' => 'stmt-vdbe-blob-branch-condition-handoff-old']);
    $t->same(['read-options'], $result['reopen_reader_ids']);
    $t->same(false, $result['next_reads'][0]['cache_hit']);
    $t->same('reader_ticket_reader_cache_stmt_vdbe_blob_branch_condition_handoff_predates_current_source', $result['next_reads'][0]['reader_cache_stmt_vdbe_blob_branch_condition_handoff_token_reason']);
    $t->same(1, $opCount($result, 'reopen_reader_for_reader_cache_stmt_vdbe_blob_branch_condition_handoff_current_source_next942'));
};

$tests['pager master journal reader cache current source next942 missing blob branch condition handoff token rejects'] = static function (TestRunner $t) use ($plan): void {
    $t->throws(Throwable::class, static fn () => $plan(['reader_cache_stmt_vdbe_blob_branch_condition_handoff_token' => null]));
};
