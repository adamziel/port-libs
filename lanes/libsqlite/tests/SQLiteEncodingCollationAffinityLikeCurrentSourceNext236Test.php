...
$tests['encoding collation affinity like current source nextTwoThreeSix multibyte escape quotes wildcard'] = static function (TestRunner $t): void {
    ...
    $plan = SQLiteEncodingCollationAffinityLikeCurrentSourceNextPlan::keyValueRowKeyEscapedLikePlan($rows, $rows, 'wpé_é%é_timeout', 'é', false, 'same', 'same', 1, 1);
...
$tests['encoding collation affinity like current source nextTwoThreeSix blob and null stay non text'] = static function (TestRunner $t): void {
    ...
    $plan = SQLiteEncodingCollationAffinityLikeCurrentSourceNextPlan::keyValueRowKeyEscapedLikePlan($rows, $rows, 'wp!_!%!_timeout%', '!', false, 'same', 'same', 1, 1);
...
$tests['encoding collation affinity like current source nextTwoThreeSix rejects missing option name'] = static function (TestRunner $t) use ($nextTwoThreeSix): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteEncodingCollationAffinityLikeCurrentSourceNextPlan::keyValueRowKeyEscapedLikePlan([['option_id' => 1]], $nextTwoThreeSix, 'wp!_!%', '!'));
};
...
$tests['encoding collation affinity like current source nextTwoThreeSix rejects non scalar option name'] = static function (TestRunner $t) use ($nextTwoThreeSix): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteEncodingCollationAffinityLikeCurrentSourceNextPlan::keyValueRowKeyEscapedLikePlan([['option_id' => 1, 'option_name' => ['wp']]], $nextTwoThreeSix, 'wp!_!%', '!'));
};
...
$tests['encoding collation affinity like current source nextTwoThreeSix rejects multi character escape'] = static function (TestRunner $t) use ($current236, $nextTwoThreeSix): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteEncodingCollationAffinityLikeCurrentSourceNextPlan::keyValueRowKeyEscapedLikePlan($current236, $nextTwoThreeSix, 'wp!_!%', '!!'));
};
