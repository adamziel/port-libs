...
use PortLibs\LibSqlite\SQLitePagerMasterJournalReaderCacheCurrentSourceNext250Plan;

$tests = [];

$pageSize = 512;
$database = '/srv/wp-content/database/wp-next250.sqlite';
$usersDatabase = '/srv/wp-content/database/wp-next250-users.sqlite';
$mainJournal = $database . '-journal';
$usersJournal = $usersDatabase . '-journal';
$master = $database . '-mj';
$sourceId = 'pager-reader-cache-current-source-provenance-next250';
$publication = 250;
$masterDigest = hash('sha256', 'next250-master-source');
$members = [$mainJournal, $usersJournal];
$masterBytes = "{$mainJournal}\n{$usersJournal}\n";
$masterBytesDigest = hash('sha256', $masterBytes);
$masterToken = 'dev=8:ino=2500:size=96:mtime=25000:snapshot=master-current';
$databaseToken = 'dev=8:ino=2509:size=5120:mtime=25099:snapshot=database-current';
$cleanupToken = 'master-cleanup:deleted:mtime=24400:dirsync=ok';
$readerLeaseToken = 'reader-lease:shared-cache:epoch=250:opened-after-master-cleanup';
$pagerCacheSourceToken = 'pager-cache-source:epoch=250:master-journal-recovery=complete';
$readTransactionToken = 'read-transaction:epoch=250:schema=103:change-counter=250:master-current';
$oldReadTransactionToken = 'read-transaction:epoch=242:schema=102:change-counter=242:before-master-current';
$schemaReparseToken = 'schema-reparse:epoch=250:schema-cookie=103:ddl=master-current';
$oldSchemaReparseToken = 'schema-reparse:epoch=242:schema-cookie=102:ddl=before-master-current';
$statementSchemaRootToken = 'statement-schema-root:epoch=250:root=1:cookie=103:sql=wp-options-current';
$oldStatementSchemaRootToken = 'statement-schema-root:epoch=242:root=1:cookie=102:sql=wp-options-prior';
$currentSourceProvenanceToken = 'current-source:master-journal:epoch=250:members=2:database-token=2509:schema=103';
$oldCurrentSourceProvenanceToken = 'current-source:master-journal:epoch=242:members=2:database-token=2508:schema=102';
$currentMasterJournalReaderSnapshotToken = 'pager-reader-cache-snapshot:epoch=250:master-current:reset=complete';
$oldMasterJournalReaderSnapshotToken = 'pager-reader-cache-snapshot:epoch=246:before-master-reset';
$oldPagerCacheSourceToken = 'pager-cache-source:epoch=242:before-master-journal-recovery';
$oldCleanupToken = 'master-cleanup:exists:mtime=25090:dirsync=pending';
$oldDatabaseToken = 'dev=8:ino=2509:size=5120:mtime=25098:snapshot=database-prior';
...
$formatPage = static function (string $label) use ($pageSize): string {
    ...
    $page = substr_replace($page, pack('N', 250), 60, 4);
    $page = substr_replace($page, pack('N', 0x57503250), 68, 4);

    return substr_replace($page, $label, 100, strlen($label));
};
$before = [
    1 => $formatPage('next250 stale schema before source provenance recovery'),
    2 => $page('next250 stale wp_options root before source provenance recovery'),
    3 => $page('next250 stale active_plugins before source provenance recovery'),
    4 => $page('next250 stale usermeta before source provenance recovery'),
    5 => $page('next250 stale rewrite_rules before source provenance recovery'),
    6 => $page('next250 stale cron before source provenance recovery'),
    7 => $page('next250 stale comments before source provenance recovery'),
    8 => $page('next250 stale terms before source provenance recovery'),
    9 => $page('next250 stale options autoload statement before source provenance recovery'),
    10 => $page('next250 stale transient timeout before source provenance recovery'),
];
$recovered = [
    1 => $formatPage('next250 current schema after source provenance recovery'),
    2 => $page('next250 current wp_options root after source provenance recovery'),
    3 => $page('next250 current active_plugins after source provenance recovery'),
    4 => $page('next250 current usermeta after source provenance recovery'),
    6 => $page('next250 current cron after source provenance recovery'),
    9 => $page('next250 current options autoload statement after source provenance recovery'),
    10 => $page('next250 current transient timeout after source provenance recovery'),
];
$formatSignature = hash('sha256', implode('|', [512, 4, 2, 250, 0x57503250]));
...
$tokens = [
    $mainJournal => 'dev=8:ino=2501:size=4096:mtime=25001:snapshot=main-current',
    $usersJournal => 'dev=8:ino=2502:size=1024:mtime=25002:snapshot=users-current',
];
$headers = [
    $mainJournal => hash('sha256', 'main-current-rollback-header-250'),
    $usersJournal => hash('sha256', 'users-current-rollback-header-250'),
];
$oldHeaders = [
    $mainJournal => $headers[$mainJournal],
    $usersJournal => hash('sha256', 'users-old-rollback-header-250'),
];
...
$cacheEntry = static fn (string $label, string $image, array $extra = []): array => array_merge([
    ...
    'master_journal_reader_snapshot_token' => $currentMasterJournalReaderSnapshotToken,
], $extra);
$cache = static fn (): array => [
    1 => $cacheEntry('schema-retained-source-provenance', $recovered[1]),
    2 => $cacheEntry('root-stale-pager-snapshot', $before[2], ['master_journal_reader_snapshot_token' => $oldMasterJournalReaderSnapshotToken]),
    ...
    10 => $cacheEntry('transient-stale-pager-snapshot', $recovered[10], ['master_journal_reader_snapshot_token' => $oldMasterJournalReaderSnapshotToken]),
];
$reads = static fn (
    ...
    static fn (int $pageNumber): array => [
        ...
        'master_journal_reader_snapshot_token' => $readerSnapshot ?? $currentMasterJournalReaderSnapshotToken,
    ],
    range(1, 10),
);
$plan = static fn (
    ...
    ?string $currentReaderSnapshotToken = null,
): array => SQLitePagerMasterJournalReaderCacheCurrentSourceNext250Plan::plan(
    $database,
    $master,
    $masterBytes,
    implode('', $before),
    ...
    $currentReaderSnapshotToken ?? $currentMasterJournalReaderSnapshotToken,
);
...
$cases = [
    'status' => [static fn (): mixed => $plan()['status'], 'pager-master-journal-reader-cache-current-source-next250'],
    'reason' => [static fn (): mixed => $plan()['reason'], 'master_journal_reader_cache_rechecks_master_journal_reader_snapshot_before_reuse'],
    'current source provenance token' => [static fn (): mixed => $plan()['current_source_provenance_token'], $currentSourceProvenanceToken],
    'current master-journal reader snapshot token' => [static fn (): mixed => $plan()['current_master_journal_reader_snapshot_token'], $currentMasterJournalReaderSnapshotToken],
    ...
    'master-journal reader snapshot invalidated pages' => [static fn (): mixed => $plan()['master_journal_reader_snapshot_invalidated_cache_page_numbers'], [2, 10]],
    ...
    'refreshed page invalidated by snapshot' => [static fn (): mixed => $plan()['refreshed_cache_page_numbers'], []],
    ...
    'read hit stale snapshot root' => [static fn (): mixed => $plan()['read_cache_hits']['read-2'], false],
    'read hit stale source' => [static fn (): mixed => $plan()['read_cache_hits']['read-3'], false],
    'read hit stale snapshot transient' => [static fn (): mixed => $plan()['read_cache_hits']['read-10'], false],
    'operation invalidates stale snapshot cache' => [static fn (): mixed => $opCount($plan(), 'invalidate_master_journal_reader_snapshot_after_master_journal_next250'), 2],
    'operation reopens stale snapshot reader' => [static fn (): mixed => $opCount($plan(), 'reopen_reader_for_master_journal_reader_snapshot_after_master_journal_next250'), 2],
    'dependency next250' => [static fn (): mixed => in_array('sqlite-pager-master-journal-reader-cache-current-source-next250', $plan()['dependencies'], true), true],
    'dependency snapshot fence' => [static fn (): mixed => in_array('sqlite-pager-reader-cache-snapshot-fence', $plan()['dependencies'], true), true],
    ...
    'row stale snapshot source admitted' => [static fn (): mixed => $row('root-stale-pager-snapshot')['current_source_provenance_token_admitted'], true],
    'row stale snapshot admitted false' => [static fn (): mixed => $row('root-stale-pager-snapshot')['master_journal_reader_snapshot_token_admitted'], false],
    'row stale snapshot reason' => [static fn (): mixed => $row('root-stale-pager-snapshot')['master_journal_reader_snapshot_token_reason'], 'master_journal_reader_snapshot_predates_master_journal_current_source'],
    'row stale snapshot cache token' => [static fn (): mixed => $row('root-stale-pager-snapshot')['cache_master_journal_reader_snapshot_token'], $oldMasterJournalReaderSnapshotToken],
    'row stale snapshot current token' => [static fn (): mixed => $row('root-stale-pager-snapshot')['current_master_journal_reader_snapshot_token'], $currentMasterJournalReaderSnapshotToken],
    'row stale snapshot mismatch' => [static fn (): mixed => $row('root-stale-pager-snapshot')['master_journal_reader_snapshot_token_matches'], false],
    ...
    'read retained snapshot current' => [static fn (): mixed => $read('read-1')['master_journal_reader_snapshot_token_current'], true],
    'read retained snapshot surfaced' => [static fn (): mixed => $read('read-1')['master_journal_reader_snapshot_token'], $currentMasterJournalReaderSnapshotToken],
    'read retained cache hit' => [static fn (): mixed => $read('read-1')['cache_hit'], true],
    'read stale snapshot cache miss' => [static fn (): mixed => $read('read-2')['cache_hit'], false],
    'read stale snapshot source' => [static fn (): mixed => $read('read-2')['source'], 'master-journal-pager-reader-cache-snapshot-fence-next250'],
    'read stale snapshot reason' => [static fn (): mixed => $read('read-2')['master_journal_reader_snapshot_token_reason'], 'reader_cache_reopened_after_master_journal_reader_snapshot_change'],
    ...
    'stale snapshot ticket cache miss' => [static fn (): mixed => $plan(null, $reads(null, null, null, null, null, $oldMasterJournalReaderSnapshotToken))['read_cache_hits']['read-1'], false],
    'stale snapshot ticket reason' => [static fn (): mixed => $plan(null, $reads(null, null, null, null, null, $oldMasterJournalReaderSnapshotToken))['next_reads'][0]['master_journal_reader_snapshot_token_reason'], 'reader_ticket_master_journal_reader_snapshot_predates_current_source'],
    'stale snapshot ticket reopens all readers' => [static fn (): mixed => $plan(null, $reads(null, null, null, null, null, $oldMasterJournalReaderSnapshotToken))['reopen_reader_ids'], ['read-1', 'read-2', 'read-3', 'read-4', 'read-5', 'read-6', 'read-7', 'read-8', 'read-9', 'read-10']],
    'stale snapshot ticket operation count' => [static fn (): mixed => $opCount($plan(null, $reads(null, null, null, null, null, $oldMasterJournalReaderSnapshotToken)), 'reopen_reader_for_master_journal_reader_snapshot_after_master_journal_next250'), 10],
    ...
    'all fresh no snapshot invalidation' => [static fn (): mixed => $plan([1 => $cacheEntry('single-fresh', $recovered[1])], [['reader_id' => 'fresh-read', 'page_number' => 1] + $reads()[0]])['master_journal_reader_snapshot_invalidated_cache_page_numbers'], []],
    ...
    'changed master-journal reader snapshot invalidates admitted pages' => [static fn (): mixed => $plan(null, null, null, 'pager-reader-cache-snapshot:epoch=248:master-current:reset=second')['master_journal_reader_snapshot_invalidated_cache_page_numbers'], [1, 2, 10]],
    'changed master-journal reader snapshot surfaced' => [static fn (): mixed => $plan(null, null, null, 'pager-reader-cache-snapshot:epoch=248:master-current:reset=second')['current_master_journal_reader_snapshot_token'], 'pager-reader-cache-snapshot:epoch=248:master-current:reset=second'],
    ...
    'provenance token embeds epoch' => [static fn (): mixed => str_contains($currentSourceProvenanceToken, 'epoch=250'), true],
    'provenance token embeds schema' => [static fn (): mixed => str_contains($currentSourceProvenanceToken, 'schema=103'), true],
];

foreach ($cases as $name => [$callback, $expected]) {
    $tests['pager master journal reader cache current source next250 ' . $name] = static function (TestRunner $t) use ($callback, $expected): void {
...
$throws = [
    ...
    'empty master-journal reader snapshot token rejected' => static fn () => $plan(null, null, null, ''),
    ...
    'cache missing master-journal reader snapshot token rejected' => static fn () => $plan([1 => array_diff_key($cacheEntry('missing-snapshot', $recovered[1]), ['master_journal_reader_snapshot_token' => true])]),
    'cache empty master-journal reader snapshot token rejected' => static fn () => $plan([1 => $cacheEntry('empty-snapshot', $recovered[1], ['master_journal_reader_snapshot_token' => ''])]),
    ...
    'read missing master-journal reader snapshot token rejected' => static fn () => $plan(null, [array_diff_key($reads()[0], ['master_journal_reader_snapshot_token' => true])]),
    'read empty master-journal reader snapshot token rejected' => static fn () => $plan(null, [array_merge($reads()[0], ['master_journal_reader_snapshot_token' => ''])]),
    'read missing reader id rejected' => static fn () => $plan(null, [array_diff_key($reads()[0], ['reader_id' => true])]),
];

foreach ($throws as $name => $callback) {
    $tests['pager master journal reader cache current source next250 ' . $name] = static function (TestRunner $t) use ($callback): void {
