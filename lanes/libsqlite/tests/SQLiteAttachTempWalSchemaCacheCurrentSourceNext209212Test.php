...
$schemas209212 = [
    ...
    'archive' => [
        ...
        'file' => '/srv/wp/archive-next209.sqlite',
    ],
    'analytics' => [
        'schema_cookie' => 7,
        'tables' => ['wp_events'],
        'indexes' => ['wp_events_name'],
        'file' => '/srv/wp/analytics-next209.sqlite',
    ],
];
...
$plan209212 = static fn (array $events, ?array $statements = null, ?array $schemas = null): array => SQLiteAttachWalTempSchemaCacheCurrentSourceNext92Plan::currentSourceNext209212(
    $schemas ?? $schemas209212,
    $statements ?? $statements209212,
    $events,
);

$tests = [];

$tests['attach temp wal schema cache current source next209 temp option drop reveals main and keeps explicit main stable'] = static function (TestRunner $t) use ($plan209212): void {
    $result = $plan209212([
        ['op' => 'drop_table', 'schema' => 'temp', 'table' => 'wp_options'],
    ]);

    $t->same('attach-wal-temp-schema-cache-current-source-next209-212', $result['operation']);
    $t->same('sqlite-attach-temp-wal-schema-cache-current-source-next209', $result['dependencies'][0]);
    $t->same('sqlite-attach-temp-wal-schema-cache-current-source-next205', $result['dependencies'][4]);
...
$tests['attach temp wal schema cache current source next210 committed main wal schema cookie expires main readers only'] = static function (TestRunner $t) use ($plan209212): void {
    $result = $plan209212([
        ['op' => 'wal_commit', 'schema' => 'main', 'schema_cookie' => 210, 'table' => 'wp_usermeta', 'commit' => true],
    ]);

    $t->same(['main'], $result['changed_schemas']);
    $t->same(210, $result['schema_cookies_next']['main']);
...
$tests['attach temp wal schema cache current source next211 detach attached schema expires attached writer and reader'] = static function (TestRunner $t) use ($plan209212): void {
    $result = $plan209212([
...
$tests['attach temp wal schema cache current source next212 active temp index rename finishes current source then reprepare'] = static function (TestRunner $t) use ($plan209212): void {
    $result = $plan209212([
