...
$makeFirstPage527542 = static function (int $pageCount): string {
...
$putPointerMapEntry527542 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database527542 = static function (int $sliceNumber) use ($makeFirstPage527542, $putPointerMapEntry527542): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage527542(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry527542($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan527542 = static function (int $sliceNumber, int $batchSize = 2) use ($database527542): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database527542($sliceNumber);
...
foreach ([527, 528, 529, 530, 531, 532, 533, 534, 535, 536, 537, 538, 539, 540, 541, 542] as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} publishes next527-542 handoff receipts"] = static function (TestRunner $t) use ($plan527542, $sliceNumber): void {
        $plan = $plan527542($sliceNumber);
        ...
        $t->same(6, $plan527542($sliceNumber, 3)->currentSourceSummary()['current_source_row_count']);
    };
}
