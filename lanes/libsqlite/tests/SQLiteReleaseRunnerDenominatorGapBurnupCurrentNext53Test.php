...
for ($i = 1; $i <= 52; $i++) {
    $tests['current next53 denominator burnup case ' . $i] = static function (TestRunner $t) use ($i, $currentHead53, $nextHead53, $focusedPath53, $nonOverlap53): void {
        $record = libsqlite_release_burnup53_evidence()->releaseRunnerDenominatorGapBurnupCurrentNext53(
...
$tests['current next53 denominator burnup rejects missing heads and rows'] = static function (TestRunner $t) use ($currentHead53, $nextHead53, $focusedPath53, $nonOverlap53): void {
    ...
    $t->throws(InvalidArgumentException::class, static fn () => $evidence->releaseRunnerDenominatorGapBurnupCurrentNext53([], $currentHead53, $nextHead53, 19277, $focusedPath53, libsqlite_release_burnup53_output(75), $nonOverlap53));
    $t->throws(InvalidArgumentException::class, static fn () => $evidence->releaseRunnerDenominatorGapBurnupCurrentNext53(libsqlite_release_burnup53_rows(1), '', $nextHead53, 19277, $focusedPath53, libsqlite_release_burnup53_output(75), $nonOverlap53));
    $t->throws(InvalidArgumentException::class, static fn () => $evidence->releaseRunnerDenominatorGapBurnupCurrentNext53(libsqlite_release_burnup53_rows(1), $currentHead53, '', 19277, $focusedPath53, libsqlite_release_burnup53_output(75), $nonOverlap53));
};
