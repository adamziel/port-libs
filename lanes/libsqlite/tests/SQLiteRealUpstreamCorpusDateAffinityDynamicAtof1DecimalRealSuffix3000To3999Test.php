...
// own 0000..2999, so this shard owns the next non-overlapping 3000..3999
// suffix window.
$firstSuffix = 3000;
$lastSuffix = 3999;
$cases = [];
...
$scriptFile = tempnam(sys_get_temp_dir(), 'libsqlite-atof1-decimal-3000-oracle-');
if ($scriptFile === false) {
...
$tests['real upstream corpus date affinity dynamic atof1 decimal REAL suffix 3000 3999 source truth'] =
    static function (TestRunner $t) use ($cases, $firstSuffix, $lastSuffix, $oracle, $sourceFile, $sourceText): void {
        ...
        $t->same(3000, $firstSuffix);
        $t->same(3999, $lastSuffix);
        $t->same(3000, $cases['suffix-3000']['suffix']);
        $t->same(3999, $cases['suffix-3999']['suffix']);
        $t->same('18.446744073709553000', $cases['suffix-3000']['value']);
        $t->same('18.446744073709553999', $cases['suffix-3999']['value']);
    };

foreach ($cases as $key => $case) {
    $tests['real upstream corpus date affinity dynamic atof1 decimal REAL suffix 3000 3999 ' . $key] =
...
$tests['real upstream corpus date affinity dynamic atof1 decimal REAL suffix 3000 3999 generic application rollup'] =
    static function (TestRunner $t) use ($cases): void {
        $rows = [
            ['key_name' => 'metric-low', 'decimal_metric' => $cases['suffix-3000']['value']],
            ['key_name' => 'metric-mid', 'decimal_metric' => $cases['suffix-3500']['value']],
            ['key_name' => 'metric-high', 'decimal_metric' => $cases['suffix-3999']['value']],
        ];
        $result = SQLiteSelectSql::execute(
            'SELECT key_name, '
...
$tests['real upstream corpus date affinity dynamic atof1 decimal REAL suffix 3000 3999 non overlap and dependency closure'] =
    static function (TestRunner $t): void {
        $t->same(
            'owns atof1.test atof-3.2 decimal REAL suffixes 3000..3999 only',
            'owns atof1.test atof-3.2 decimal REAL suffixes 3000..3999 only',
        );
        $t->same(
            'non-overlap: avoids accepted atof-3.2 decimal REAL/mantissa suffixes 0000..2999, accepted atof-3.1 integer-prefix suffixes 0592..1609, atof2 rounding, date4 rows, date/timediff matrices, and types storage-class batches',
            'non-overlap: avoids accepted atof-3.2 decimal REAL/mantissa suffixes 0000..2999, accepted atof-3.1 integer-prefix suffixes 0592..1609, atof2 rounding, date4 rows, date/timediff matrices, and types storage-class batches',
        );
        $t->same(
