...
$makeFirstPage431446 = static function (int $pageCount): string {
...
$putPointerMapEntry431446 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database431446 = static function (int $sliceNumber) use ($makeFirstPage431446, $putPointerMapEntry431446): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage431446(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry431446($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan431446 = static function (int $sliceNumber, int $batchSize = 2) use ($database431446): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database431446($sliceNumber);
...
foreach ([431, 432, 433, 434, 435, 436, 437, 438, 439, 440, 441, 442, 443, 444, 445, 446] as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} keeps next431-446 freelist splice ready"] = static function (TestRunner $t) use ($plan431446, $sliceNumber): void {
        $plan = $plan431446($sliceNumber);
        ...
        $t->same(6, $plan431446($sliceNumber, 3)->freelistSummary()['freelist_row_count']);
    };
}
