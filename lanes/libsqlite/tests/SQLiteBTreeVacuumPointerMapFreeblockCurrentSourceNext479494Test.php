...
$makeFirstPage479494 = static function (int $pageCount): string {
...
$putPointerMapEntry479494 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database479494 = static function (int $sliceNumber) use ($makeFirstPage479494, $putPointerMapEntry479494): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage479494(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry479494($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan479494 = static function (int $sliceNumber, int $batchSize = 2) use ($database479494): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database479494($sliceNumber);
...
foreach ([479, 480, 481, 482, 483, 484, 485, 486, 487, 488, 489, 490, 491, 492, 493, 494] as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} publishes next479-494 handoff receipts"] = static function (TestRunner $t) use ($plan479494, $sliceNumber): void {
        $plan = $plan479494($sliceNumber);
        ...
        $t->same(6, $plan479494($sliceNumber, 3)->currentSourceSummary()['current_source_row_count']);
    };
}
