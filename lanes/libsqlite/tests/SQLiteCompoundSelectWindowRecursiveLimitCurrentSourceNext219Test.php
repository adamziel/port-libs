...
$summary219 = static fn (?array $cursor = null): array => SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext219Plan::compare($sql219, $currentTables219, $nextTables219, $cursor);
$tests = [];
...
$tests['compound select window recursive limit current source next219 rejects missing cume dist'] = static function (TestRunner $t) use ($currentTables219): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext219Plan::compare(
...
foreach (range(1, 52) as $case) {
    $tests['compound select window recursive limit current source next219 generated percent cume boundary ' . $case] = static function (TestRunner $t) use ($case): void {
        ...
        $plan = SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext219Plan::compare($sql, $tables, $nextTables);
        $again = SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext219Plan::compare($sql, $tables, $nextTables, $plan['cursor']);
