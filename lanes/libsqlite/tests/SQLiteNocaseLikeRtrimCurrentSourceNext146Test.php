...
use PortLibs\LibSqlite\SQLiteNocaseLikeRtrimCurrentSourceNext146Plan;
...
$plan = static fn (
    ...
    int $nextCookie = 146,
): array => SQLiteNocaseLikeRtrimCurrentSourceNext146Plan::wordpressOptionNamePlan(
    $current ?? $currentRows,
    $next ?? $nextRows,
    $pattern,
    $escape,
...
$cases = [
    ...
    'current source' => ['plugin\_%', '\\', 'currentSource', 'main.wp_options@145'],
    'next source' => ['plugin\_%', '\\', 'nextSource', 'main.wp_options@146'],
...
$tests['nocase like rtrim current source next146 stable rows reusable'] = static function (TestRunner $t) use ($row): void {
    ...
    $plan = SQLiteNocaseLikeRtrimCurrentSourceNext146Plan::wordpressOptionNamePlan($rows, $rows, 'plugin\_%', '\\', 'stable', 'stable', 7, 7);
...
$tests['nocase like rtrim current source next146 stable no-prefix still not reusable'] = static function (TestRunner $t) use ($row): void {
    ...
    $plan = SQLiteNocaseLikeRtrimCurrentSourceNext146Plan::wordpressOptionNamePlan($rows, $rows, '%plugin%', null, 'stable', 'stable', 7, 7);
...
$tests['nocase like rtrim current source next146 rejects missing option id'] = static function (TestRunner $t): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteNocaseLikeRtrimCurrentSourceNext146Plan::wordpressOptionNamePlan([['option_name_bytes' => 'plugin', 'text_encoding' => 1]], [], 'plugin%'));
};

$tests['nocase like rtrim current source next146 rejects missing bytes'] = static function (TestRunner $t): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteNocaseLikeRtrimCurrentSourceNext146Plan::wordpressOptionNamePlan([['option_id' => 1, 'text_encoding' => 1]], [], 'plugin%'));
};

$tests['nocase like rtrim current source next146 rejects missing encoding'] = static function (TestRunner $t): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteNocaseLikeRtrimCurrentSourceNext146Plan::wordpressOptionNamePlan([['option_id' => 1, 'option_name_bytes' => 'plugin']], [], 'plugin%'));
};
...
$tests['nocase like rtrim current source next146 records unsupported encoding as malformed'] = static function (TestRunner $t): void {
    $plan = SQLiteNocaseLikeRtrimCurrentSourceNext146Plan::wordpressOptionNamePlan([['option_id' => 1, 'option_name_bytes' => 'plugin', 'text_encoding' => 9]], [], 'plugin%');
...
$tests['nocase like rtrim current source next146 rejects bad escape'] = static function (TestRunner $t) use ($currentRows): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteNocaseLikeRtrimCurrentSourceNext146Plan::wordpressOptionNamePlan($currentRows, [], 'plugin%', 'xx'));
};
