...
function libsqlite_suite_ledger35_evidence(): SQLiteUpstreamSuiteEvidence
{
    return SQLiteUpstreamSuiteEvidence::fromManifestPath(__DIR__ . '/../UPSTREAM_TEST_MANIFEST.json');
}

function libsqlite_suite_ledger35_root(string $label): string
{
    return sys_get_temp_dir() . '/libsqlite-suite-ledger35-' . $label . '-' . bin2hex(random_bytes(4));
}

function libsqlite_suite_ledger35_cleanup(string $root): void
{
    if (!is_dir($root)) {
        return;
    }
...
function libsqlite_suite_ledger35_php_output(int $assertions = 88, int $failures = 0): string
{
...
function libsqlite_suite_ledger35_write_artifact(
    string $directory,
    string $label,
    string $head,
    int $tests,
...
function libsqlite_suite_ledger35_seed(
    string $root,
    string $acceptedHead,
    int $currentCount,
    int $nextCount,
    ...
    for ($i = 1; $i <= $currentCount; $i++) {
        libsqlite_suite_ledger35_write_artifact($current, sprintf('artifact-%02d', $i), $acceptedHead, 20000 + $i);
    }
    for ($i = 1; $i <= $nextCount; $i++) {
        $label = sprintf('artifact-%02d', $i);
        libsqlite_suite_ledger35_write_artifact($next, $label, $nextHead, 20000 + $i, [
...
$focusedPath35 = 'lanes/libsqlite/tests/SQLiteReleaseRunnerSuiteLedgerCurrentNext35Test.php';
...
foreach ($increaseCases as $case => $config) {
    $tests['current next35 suite ledger classifies count movement ' . $case] = static function (TestRunner $t) use ($case, $config, $acceptedHead35, $focusedPath35, $nonOverlap35): void {
        ...
        $root = libsqlite_suite_ledger35_root($case);

        try {
            [$current, $next] = libsqlite_suite_ledger35_seed($root, $acceptedHead35, (int) $currentCount, (int) $nextCount, $options);
            $record = libsqlite_suite_ledger35_evidence()->releaseRunnerSuiteLedgerCurrentNext35(
                $current,
                $next,
                $acceptedHead35,
                12271,
                $focusedPath35,
                libsqlite_suite_ledger35_php_output(88),
                $nonOverlap35,
            ...
            libsqlite_suite_ledger35_cleanup($root);
        }
    };
}
...
$blockedCases = [
    ...
    'php-failure' => [1, 2, ['focused-php-pass-not-admitted'], [], '', libsqlite_suite_ledger35_php_output(88, 1)],
...
foreach ($blockedCases as $case => $config) {
    $tests['current next35 suite ledger blocks invalid evidence ' . $case] = static function (TestRunner $t) use ($case, $config, $acceptedHead35, $focusedPath35, $nonOverlap35): void {
        ...
        $phpOutput = is_string($config[5] ?? null) ? $config[5] : libsqlite_suite_ledger35_php_output(88);
        $root = libsqlite_suite_ledger35_root($case);
...
for ($i = 1; $i <= 21; $i++) {
    $tests['current next35 suite ledger generated countable case ' . str_pad((string) $i, 2, '0', STR_PAD_LEFT)] = static function (TestRunner $t) use ($i, $acceptedHead35, $focusedPath35, $nonOverlap35): void {
        ...
        $root = libsqlite_suite_ledger35_root('generated-' . $i);

        try {
            [$current, $next] = libsqlite_suite_ledger35_seed($root, $acceptedHead35, $currentCount, $nextCount);
            $record = libsqlite_suite_ledger35_evidence()->releaseRunnerSuiteLedgerCurrentNext35(
                $current,
                $next,
                $acceptedHead35,
                12271,
                $focusedPath35,
                libsqlite_suite_ledger35_php_output(90 + $i),
                $nonOverlap35
            );
