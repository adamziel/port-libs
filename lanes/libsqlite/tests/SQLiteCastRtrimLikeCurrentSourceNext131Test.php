...
$plan = static fn (
    ...
    int $nextCookie = 131,
): array => SQLiteCastRtrimLikeCurrentSourceNextPlan::wordpressOptionValuePlan(
    $current ?? $currentRows,
    $next ?? $nextRows,
    $castTarget,
    $pattern,
...
$tests['cast rtrim like current source next131 stable exact padded peer is reusable'] = static function (TestRunner $t): void {
    ...
    $plan = SQLiteCastRtrimLikeCurrentSourceNextPlan::wordpressOptionValuePlan($rows, $rows, 'TEXT', 'plugin\\_cache', '\\', 'stable', 'stable', 7, 7);
...
$tests['cast rtrim like current source next131 stable leading wildcard keeps no prefix reason'] = static function (TestRunner $t): void {
    ...
    $plan = SQLiteCastRtrimLikeCurrentSourceNextPlan::wordpressOptionValuePlan($rows, $rows, 'TEXT', '%cache', null, 'stable', 'stable', 7, 7);
...
$tests['cast rtrim like current source next131 rejects malformed cast target'] = static function (TestRunner $t) use ($currentRows, $nextRows): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteCastRtrimLikeCurrentSourceNextPlan::wordpressOptionValuePlan($currentRows, $nextRows, 'TEXT); DROP TABLE wp_options; --', 'plugin%'));
};

$tests['cast rtrim like current source next131 rejects missing option id'] = static function (TestRunner $t) use ($nextRows): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteCastRtrimLikeCurrentSourceNextPlan::wordpressOptionValuePlan([['option_value' => 'plugin']], $nextRows, 'TEXT', 'plugin%'));
};

$tests['cast rtrim like current source next131 rejects missing option value'] = static function (TestRunner $t) use ($nextRows): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteCastRtrimLikeCurrentSourceNextPlan::wordpressOptionValuePlan([['option_id' => 1]], $nextRows, 'TEXT', 'plugin%'));
};
...
$tests['cast rtrim like current source next131 rejects non integer option id'] = static function (TestRunner $t) use ($nextRows): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteCastRtrimLikeCurrentSourceNextPlan::wordpressOptionValuePlan([['option_id' => '1', 'option_value' => 'plugin']], $nextRows, 'TEXT', 'plugin%'));
};
...
$tests['cast rtrim like current source next131 rejects multi byte escape'] = static function (TestRunner $t) use ($currentRows, $nextRows): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteCastRtrimLikeCurrentSourceNextPlan::wordpressOptionValuePlan($currentRows, $nextRows, 'TEXT', 'plugin!!_%', '!!'));
};
