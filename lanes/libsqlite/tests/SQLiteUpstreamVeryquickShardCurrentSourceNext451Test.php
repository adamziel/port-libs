...
function libsqlite_suite_next451_evidence(): SQLiteUpstreamSuiteEvidence
{
    return SQLiteUpstreamSuiteEvidence::fromManifestPath(__DIR__ . '/../UPSTREAM_TEST_MANIFEST.json');
}

function libsqlite_suite_next451_output(int $passLines = 96, int $assertions = 96, int $failures = 0): string
{
    $lines = ['Focused test run: 1 selected test files (root lock skipped)'];
    for ($i = 1; $i <= $passLines; $i++) {
        $lines[] = sprintf('PASS current source next451 veryquick shard admission case %02d', $i);
    }
 ...
 */
function libsqlite_suite_next451_rows(
    int $case = 1,
    ...
    string $nextHead = 'suite-upstream-veryquick-shard-current-source-next451'
): array {
    $script = sprintf('veryquick-current-source-next451-%02d.test', $case);

    return [
        [
            'unit' => 'suite-upstream-veryquick-shard-current-source-next451',
            'kind' => 'bounded-upstream-veryquick-shard-runner',
            'gap_id' => 'current-source-next451-veryquick-shard-gap',
            'gap_status' => 'removed',
            'removed_blocker' => 'next451 admits one focused veryquick shard row tied to launcher Base accepted HEAD fca16e3d and accepted batch228 source f276db2c without duplicating accepted next155 through next398 suite evidence',
            ...
            'artifact_path' => 'lanes/libsqlite/notes/yield-suite-upstream-veryquick-shard-current-source-next451.md',
 ...
 */
function libsqlite_suite_next451_record(
    array $rows,
    string $launcherBase = 'fca16e3dd1812e6fcb6dc54c4980a5fb898b24ec',
    string $dashboardSource = 'f276db2cadbe640018aa18d11a7721e7187e05dc',
    string $statusSource = 'f276db2cadbe640018aa18d11a7721e7187e05dc',
    ...
    return libsqlite_suite_next451_evidence()->upstreamVeryquickShardCurrentSourceNext451(
        $rows,
        801,
        151655,
        $launcherBase,
...
foreach (range(1, 80) as $case) {
    $tests[sprintf('current source next451 admits veryquick shard case %02d', $case)] = static function (TestRunner $t) use ($case): void {
        $record = libsqlite_suite_next451_record(libsqlite_suite_next451_rows($case));

        $t->same('current-source-next451-veryquick-shard-advanced', $record['status']);
        ...
        $t->same(['suite-upstream-veryquick-shard-current-source-next451'], $record['admitted_units']);
        $t->same(['batch228-current-source-anchor'], $record['preserved_units']);
        $t->contains(sprintf('veryquick-current-source-next451-%02d.test', $case), implode(',', $record['target_scripts']));
        $t->same(true, $record['counts_upstream_veryquick_shard_current_source_next451']);
...
$tests['current source next451 records authoritative launcher and source heads'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next451_record(libsqlite_suite_next451_rows(8));
    ...
    $t->same(['suite-upstream-veryquick-shard-current-source-next451'], $record['artifact_source_heads']);
};

$tests['current source next451 records target scripts and tier counts'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next451_record(libsqlite_suite_next451_rows(13));

    $t->same(151668, $record['tests_total_delta']);
    $t->same(['accepted-batch228-anchor.test', 'testrunner.test', 'veryquick-current-source-next451-13.test'], $record['target_scripts']);
...
$tests['current source next451 preserves already counted row without mapped inflation'] = static function (TestRunner $t): void {
    $rows = libsqlite_suite_next451_rows();
    $rows[0]['current_countable'] = true;
    $rows[0]['current_tests'] = 151655;

    $record = libsqlite_suite_next451_record($rows);

    $t->same('current-source-next451-veryquick-shard-preserved', $record['status']);
    $t->same(0, $record['mapped_delta']);
    $t->same(801, $record['next_mapped']);
    $t->same(['batch228-current-source-anchor', 'suite-upstream-veryquick-shard-current-source-next451'], $record['preserved_units']);
};

$tests['current source next451 blocks stale source provenance'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next451_record(
        libsqlite_suite_next451_rows(
...
$tests['current source next451 blocks unguarded and non local artifacts'] = static function (TestRunner $t): void {
    $rows = libsqlite_suite_next451_rows();
    $rows[0]['artifact_path'] = '/tmp/next451.md';
...
$tests['current source next451 blocks non zero runner artifacts'] = static function (TestRunner $t): void {
    $rows = libsqlite_suite_next451_rows();
    $rows[0]['exit'] = 1;
    $rows[0]['errors'] = 2;
...
$tests['current source next451 blocks missing removed blocker classification'] = static function (TestRunner $t): void {
    $rows = libsqlite_suite_next451_rows();
...
$tests['current source next451 blocks duplicate broad runner snapshot'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next451_record(
        libsqlite_suite_next451_rows(),
...
$tests['current source next451 blocks focused php admission mismatch'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next451_record(
        libsqlite_suite_next451_rows(),
        output: libsqlite_suite_next451_output(assertions: 83)
    );
...
$tests['current source next451 records exact focused php admission'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next451_record(libsqlite_suite_next451_rows());
...
$tests['current source next451 blocks missing focused runner summary'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next451_record(
        libsqlite_suite_next451_rows(),
...
$tests['current source next451 carries dependency closure and non overlap notes'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next451_record(libsqlite_suite_next451_rows());

    $t->contains('no new support component needed', $record['dependency_closure']);
    $t->contains('343/344/345/346', $record['non_overlap_note']);
    $t->contains('release/all parity remains unclaimed', $record['next_gate']);
};

$tests['current source next451 blocks missing next countability flag'] = static function (TestRunner $t): void {
    $rows = libsqlite_suite_next451_rows();
...
$tests['current source next451 keeps broad release parity unclaimed'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next451_record(libsqlite_suite_next451_rows(21));
...
$tests['current source next451 does not recount prior veryquick shards'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next451_record(libsqlite_suite_next451_rows(22));

    $t->same(false, $record['counts_upstream_veryquick_shard_current_source_next398']);
    $t->same(false, $record['counts_upstream_veryquick_shard_current_source_next202']);
};

$tests['current source next451 records focused shard dependency closure'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next451_record(libsqlite_suite_next451_rows(23));

    $t->contains('current-source next451 veryquick shard admission', $record['dependency_closure']);
