...
$preparedSource126 = static function (array $overrides = []): array {
    return $overrides + [
        'name' => 'prepared-stat4-covering-expression-in-',
        'schemaCookie' => 1260,
        'stat4Generation' => 42,
        'indexes' => [[
...
$currentSource126 = static function () use ($preparedSource126): array {
    $source = $preparedSource126([
        'name' => 'current-stat4-covering-expression-in-',
        'schemaCookie' => 1264,
        'stat4Generation' => 45,
    ]);
...
$plan126 = static fn (
    ...
    ?array $rows = null,
    ?array $needed = null,
): array => SQLitePlannerStat4CoveringExpressionInCurrentSourceNextPlan::materialize(
...
$tests = [
    'planner stat4 covering expression in current source status ready' => static fn (TestRunner $t) => $t->same('stat4-covering-expression-in-current-source-ready', $plan126()['status']),
    'planner stat4 covering expression in current source selects current' => static fn (TestRunner $t) => $t->same('current', $plan126()['selectedSource']),
    'planner stat4 covering expression in current source stale prepared' => static fn (TestRunner $t) => $t->same(true, $plan126()['stalePreparedStatement']),
    'planner stat4 covering expression in current source reprepare required' => static fn (TestRunner $t) => $t->same(true, $plan126()['reprepareRequired']),
    ...
    'planner stat4 covering expression in current source stat4 used' => static fn (TestRunner $t) => $t->same(true, $plan126()['selectedPlan']['stat4Used']),
    'planner stat4 covering expression in current source stat4 matched' => static fn (TestRunner $t) => $t->same(4, $plan126()['selectedPlan']['stat4MatchedSamples']),
    'planner stat4 covering expression in current source covered row count' => static fn (TestRunner $t) => $t->same(5, $plan126()['selectedPlan']['coveredRowCount']),
    'planner stat4 covering expression in current source keys sorted' => static fn (TestRunner $t) => $t->same(['plugin_cache', 'plugin_cache', 'plugin_forms', 'plugin_mail', 'plugin_seo'], $plan126()['cursorTape']['matchedKeys']),
    'planner stat4 covering expression in current source first rowid' => static fn (TestRunner $t) => $t->same(21, $plan126()['currentNextRows'][0]['current']['rowid']),
    'planner stat4 covering expression in current source duplicate key keeps rowid order' => static fn (TestRunner $t) => $t->same(22, $plan126()['currentNextRows'][1]['current']['rowid']),
    ...
    'planner stat4 covering expression in current source seek opcode' => static fn (TestRunner $t) => $t->same('SeekGE', $plan126()['cursorTape']['seekOpcode']),
    'planner stat4 covering expression in current source stop opcode' => static fn (TestRunner $t) => $t->same('IdxGT', $plan126()['cursorTape']['stopOpcode']),
    'planner stat4 covering expression in current source output from index' => static fn (TestRunner $t) => $t->same('index', $plan126()['cursorTape']['outputColumns'][2]['source']),
    'planner stat4 covering expression in current source program first seek' => static fn (TestRunner $t) => $t->same(['opcode' => 'SeekGE', 'source' => 'index', 'key' => 'plugin_cache'], $plan126()['cursorTape']['program'][0]),
    'planner stat4 covering expression in current source program first stop' => static fn (TestRunner $t) => $t->same(['opcode' => 'IdxGT', 'source' => 'index', 'key' => 'plugin_cache'], $plan126()['cursorTape']['program'][1]),
    'planner stat4 covering expression in current source program result covering' => static fn (TestRunner $t) => $t->same('covering-index', $plan126()['cursorTape']['program'][7]['source']),
    'planner stat4 covering expression in current source no rowid dedupe required' => static fn (TestRunner $t) => $t->same(false, $plan126()['cursorTape']['dedupeByRowid']),
    'planner stat4 covering expression in current source fence cookie' => static fn (TestRunner $t) => $t->same(1264, $plan126()['currentSourceFence']['schemaCookie']),
    ...
    'planner stat4 covering expression in current source dependency marker' => static fn (TestRunner $t) => $t->contains('sqlite-planner-stat4-covering-expression-in-current-source', implode(',', $plan126()['dependencies'])),
    'planner stat4 covering expression in current source dependency closure' => static fn (TestRunner $t) => $t->contains('no new support component needed', $plan126()['dependency_closure']),
    'planner stat4 covering expression in current source non overlap' => static fn (TestRunner $t) => $t->contains('multi-seek IN expression probes', $plan126()['non_overlap']),
    'planner stat4 covering expression in current source fresh selects prepared' => static fn (TestRunner $t) => $t->same('prepared', $fresh126()['selectedSource']),
    'planner stat4 covering expression in current source fresh no reprepare' => static fn (TestRunner $t) => $t->same(false, $fresh126()['reprepareRequired']),
    'planner stat4 covering expression in current source fresh root' => static fn (TestRunner $t) => $t->same(12601, $fresh126()['selectedPlan']['rootPage']),
    'planner stat4 covering expression in current source non covering requires next' => static fn (TestRunner $t) => $t->same('requires-next-stage', $nonCovering126()['status']),
    'planner stat4 covering expression in current source non covering deferred seek' => static fn (TestRunner $t) => $t->same('DeferredSeek', $nonCovering126()['deferredTableSeekOpcode']),
    'planner stat4 covering expression in current source no stat4 requires next' => static fn (TestRunner $t) => $t->same('requires-next-stage', $noStat4126()['status']),
    'planner stat4 covering expression in current source no stat4 matched zero' => static fn (TestRunner $t) => $t->same(0, $noStat4126()['selectedPlan']['stat4MatchedSamples']),
    'planner stat4 covering expression in current source missing sample requires next' => static fn (TestRunner $t) => $t->same('requires-next-stage', $missingSample126()['status']),
    'planner stat4 covering expression in current source missing sample rows zero' => static fn (TestRunner $t) => $t->same(0, $missingSample126()['selectedPlan']['coveredRowCount']),
    'planner stat4 covering expression in current source validates source indexes' => static function (TestRunner $t) use ($preparedSource126, $currentSource126, $predicate126, $rows126, $needed126, $lower126): void {
        $bad = $preparedSource126();
        $bad['indexes'] = [];
        $t->throws(InvalidArgumentException::class, static fn () => SQLitePlannerStat4CoveringExpressionInCurrentSourceNextPlan::materialize($bad, $currentSource126(), $predicate126, $rows126(), $needed126, [$lower126]));
    },
    'planner stat4 covering expression in current source validates schema cookie' => static function (TestRunner $t) use ($preparedSource126, $currentSource126, $predicate126, $rows126, $needed126, $lower126): void {
        $bad = $preparedSource126(['schemaCookie' => -1]);
        $t->throws(InvalidArgumentException::class, static fn () => SQLitePlannerStat4CoveringExpressionInCurrentSourceNextPlan::materialize($bad, $currentSource126(), $predicate126, $rows126(), $needed126, [$lower126]));
    },
    'planner stat4 covering expression in current source validates output columns' => static function (TestRunner $t) use ($preparedSource126, $currentSource126, $predicate126, $rows126, $lower126): void {
        $t->throws(InvalidArgumentException::class, static fn () => SQLitePlannerStat4CoveringExpressionInCurrentSourceNextPlan::materialize($preparedSource126(), $currentSource126(), $predicate126, $rows126(), [], [$lower126]));
    },
    'planner stat4 covering expression in current source validates in operator' => static function (TestRunner $t) use ($preparedSource126, $currentSource126, $rows126, $needed126, $lower126): void {
        $bad = ['operator' => '=', 'left' => $lower126, 'right' => 'plugin_cache'];
        $t->throws(InvalidArgumentException::class, static fn () => SQLitePlannerStat4CoveringExpressionInCurrentSourceNextPlan::materialize($preparedSource126(), $currentSource126(), $bad, $rows126(), $needed126, [$lower126]));
    },
    'planner stat4 covering expression in current source validates in values' => static function (TestRunner $t) use ($preparedSource126, $currentSource126, $predicate126, $rows126, $needed126, $lower126): void {
        $bad = $predicate126;
        $bad['values'] = [];
        $t->throws(InvalidArgumentException::class, static fn () => SQLitePlannerStat4CoveringExpressionInCurrentSourceNextPlan::materialize($preparedSource126(), $currentSource126(), $bad, $rows126(), $needed126, [$lower126]));
    },
