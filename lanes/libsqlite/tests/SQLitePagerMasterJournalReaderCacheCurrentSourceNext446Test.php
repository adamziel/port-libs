...
$pageSize = 512;
$database = '/srv/wp-content/database/wp-next446.sqlite';
...
$formatPage = static function (string $label) use ($pageSize): string {
    ...
    $page = substr_replace($page, pack('N', 446), 60, 4);
...
$tokens = [$journal => 'member-main-current-446'];
$headers = [$journal => hash('sha256', 'main header next446')];
$base = [
    'source_id' => 'pager-reader-cache-current-source-next446',
    'epoch' => 446,
    'format_signature' => hash('sha256', implode('|', [512, 4, 2, 446, 0])),
    'publication_generation' => 446,
    'master_source_digest' => hash('sha256', 'master-next446'),
    'recovery_sequence' => 446,
...
foreach ($tokenFields as $field) {
    $base[$field] = str_replace('_', '-', preg_replace('/_token$/', '', $field)) . '-current-446';
}
...
$plan = static fn (array $cacheExtra = [], array $readExtra = []): array => SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::readerCacheStatementFence(446,
    $database,
    $master,
    $masterBytes,
    implode('', $before),
...
$tests['pager master journal reader cache current source next446 admits current VDBE lifecycle fences'] = static function (TestRunner $t) use ($plan): void {
    $result = $plan();
    $t->same('pager-master-journal-reader-cache-current-source-next446', $result['status']);
    $t->same([], $result['invalidated_cache_page_numbers']);
    $t->same(['read-options' => true], $result['read_cache_hits']);
    $t->same('reader-cache-stmt-vdbe-deferred-seek-current-446', $result['current_reader_cache_stmt_vdbe_deferred_seek_token']);
};

$tests['pager master journal reader cache current source next446 invalidates stale deferred-seek cache'] = static function (TestRunner $t) use ($plan, $opCount): void {
    $result = $plan(['reader_cache_stmt_vdbe_deferred_seek_token' => 'stmt-vdbe-deferred-seek-old']);
    $t->same([1], $result['reader_cache_stmt_vdbe_deferred_seek_invalidated_cache_page_numbers']);
    $t->same([1], $result['invalidated_cache_page_numbers']);
    $t->same(1, $opCount($result, 'invalidate_reader_cache_reader_cache_stmt_vdbe_deferred_seek_current_source_next446'));
};

$tests['pager master journal reader cache current source next446 reopens stale deferred-seek read ticket'] = static function (TestRunner $t) use ($plan, $opCount): void {
    $result = $plan([], ['reader_cache_stmt_vdbe_deferred_seek_token' => 'stmt-vdbe-deferred-seek-old']);
    $t->same(['read-options'], $result['reopen_reader_ids']);
    $t->same(false, $result['next_reads'][0]['cache_hit']);
    $t->same('reader_ticket_reader_cache_stmt_vdbe_deferred_seek_predates_current_source', $result['next_reads'][0]['reader_cache_stmt_vdbe_deferred_seek_token_reason']);
    $t->same(1, $opCount($result, 'reopen_reader_for_reader_cache_stmt_vdbe_deferred_seek_current_source_next446'));
};

$tests['pager master journal reader cache current source next446 missing deferred-seek token rejects'] = static function (TestRunner $t) use ($plan): void {
    $t->throws(Throwable::class, static fn () => $plan(['reader_cache_stmt_vdbe_deferred_seek_token' => null]));
};
