...
$makeFirstPage375382 = static function (int $pageCount): string {
...
$putPointerMapEntry375382 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database375382 = static function (int $sliceNumber) use ($makeFirstPage375382, $putPointerMapEntry375382): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage375382(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry375382($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan375382 = static function (int $sliceNumber, int $batchSize = 2) use ($database375382): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database375382($sliceNumber);
...
foreach ([375, 376, 377, 378, 379, 380, 381, 382] as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} keeps next375-382 freelist splice ready"] = static function (TestRunner $t) use ($plan375382, $sliceNumber): void {
        $plan = $plan375382($sliceNumber);
        ...
        $t->same(6, $plan375382($sliceNumber, 3)->freelistSummary()['freelist_row_count']);
    };
}
