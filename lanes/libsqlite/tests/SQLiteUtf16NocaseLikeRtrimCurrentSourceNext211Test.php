...
$row211 = static fn (int $id, string $name, int|string $encoding): array => [
    'setting_id' => $id,
    'key_name_bytes' => $enc211($name, $encoding),
...
$plan211 = static fn (
    ...
    int $nextCookie = 211,
): array => SQLiteUtf16NocaseLikeRtrimCurrentSourceNextPlan::keyValueRowKeySourceRefreshPlan(
    $current ?? $current211,
    $next ?? $nextTwoOneOne,
    $pattern,
    $escape,
...
$cases211 = [
    ...
    'expression' => ['expression', 'rtrim(key_name) COLLATE NOCASE LIKE ? ESCAPE ? /* current-source refresh */'],
    'pattern' => ['pattern', 'plugin!_cache%'],
    'escape' => ['escape', '!'],
    'collation' => ['collation', 'NOCASE'],
    'current source' => ['currentSource', 'main.app_settings@210'],
    'next source' => ['nextSource', 'main.app_settings@211'],
...
$tests['utf16 nocase like rtrim current source nextTwoOneOne byte order only refresh stays reusable'] = static function (TestRunner $t) use ($row211): void {
    ...
    $result = SQLiteUtf16NocaseLikeRtrimCurrentSourceNextPlan::keyValueRowKeySourceRefreshPlan(
        $current,
        $next,
        'plugin!_cache%',
        '!',
...
$tests['utf16 nocase like rtrim current source nextTwoOneOne rejects malformed row shape'] = static function (TestRunner $t): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteUtf16NocaseLikeRtrimCurrentSourceNextPlan::keyValueRowKeySourceRefreshPlan([
