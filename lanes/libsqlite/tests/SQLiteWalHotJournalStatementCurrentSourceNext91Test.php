...
$clean = [
    1 => $page('next91 clean sqlite header before crashed import'),
    2 => $page('next91 clean wp_options root before crashed import'),
    3 => $page('next91 clean active_plugins before statement'),
    4 => $page('next91 clean transient before failed statement'),
];
$dirtyDatabase = $page('next91 dirty sqlite header after crash')
    . $page('next91 dirty wp_options root after crash')
    . $page('next91 dirty active_plugins after failed statement')
    . $page('next91 dirty transient after failed statement');
$statementBefore = $page('next91 statement before active_plugins insert');
$nextBefore = $page('next91 retry before plugin option insert');
...
$walBytes = $makeWalBytes([
    [1, 0, 'next91 retained schema frame'],
    [2, 4, 'next91 retained wp_options root frame'],
    [3, 0, 'next91 failed active_plugins draft'],
    [4, 4, 'next91 failed transient commit'],
]);
...
$plan = static fn (
...
): array => SQLiteWalHotJournalSavepointReplayPlan::statementCurrentSourceNext91(
    ...
    [1, 2, 3, 4],
    $sourcePages ?? [
        1 => $page('next91 retained schema frame'),
        2 => $page('next91 retained wp_options root frame'),
        3 => $page('next91 failed active_plugins draft'),
        4 => $page('next91 failed transient commit'),
    ],
    true,
    $reservedLock,
    $requiresSuper,
...
$cases = [
    'status recovered' => [static fn (): mixed => $plan()['status'], 'hot_journal_wal_statement_current_source_recovered_next91'],
    ...
    'checkpoint bytes include retained root' => [static fn (): mixed => str_contains($plan()['checkpoint_database_bytes'], 'next91 retained wp_options root frame'), true],
    'checkpoint bytes include failed statement page before statement rollback' => [static fn (): mixed => str_contains($plan()['checkpoint_database_bytes'], 'next91 failed active_plugins draft'), true],
    'statement bytes restore statement page' => [static fn (): mixed => str_contains($plan()['statement_database_bytes'], 'next91 statement before active_plugins insert'), true],
    'statement bytes keep retained root' => [static fn (): mixed => str_contains($plan()['statement_database_bytes'], 'next91 retained wp_options root frame'), true],
    'statement bytes exclude failed active plugin draft' => [static fn (): mixed => str_contains($plan()['statement_database_bytes'], 'next91 failed active_plugins draft'), false],
    'statement bytes exclude dirty root crash' => [static fn (): mixed => str_contains($plan()['statement_database_bytes'], 'next91 dirty wp_options root after crash'), false],
    'statement wal prefix length' => [static fn (): mixed => $plan()['statement_wal_bytes_length'], 32 + (2 * (24 + $pageSize))],
    'statement wal keeps frame one' => [static fn (): mixed => str_contains($plan()['statement_wal_bytes'], 'next91 retained schema frame'), true],
    'statement wal keeps frame two' => [static fn (): mixed => str_contains($plan()['statement_wal_bytes'], 'next91 retained wp_options root frame'), true],
    'statement wal excludes frame three' => [static fn (): mixed => str_contains($plan()['statement_wal_bytes'], 'next91 failed active_plugins draft'), false],
    'current source pages' => [static fn (): mixed => $plan()['current_source_pages'], [1, 2, 3, 4]],
    'current source prefix page one' => [static fn (): mixed => str_starts_with($plan()['current_source_prefixes'][1], 'next91 retained schema frame'), true],
    'current source prefix page three' => [static fn (): mixed => str_starts_with($plan()['current_source_prefixes'][3], 'next91 failed active_plugins draft'), true],
    'next source prefix page three' => [static fn (): mixed => str_starts_with($plan()['next_source_prefixes'][3], 'next91 statement before active_plugins insert'), true],
    ...
    'payload statement exists' => [static fn (): mixed => array_key_exists($databasePath . '#statement-current-source-next91', $plan()['payloads']), true],
    'payload statement has before image' => [static fn (): mixed => str_contains($plan()['payloads'][$databasePath . '#statement-current-source-next91'], 'next91 statement before active_plugins insert'), true],
    'payload wal exists' => [static fn (): mixed => array_key_exists($databasePath . '-wal#statement-current-source-next91', $plan()['payloads']), true],
    'payload wal excludes failed transient' => [static fn (): mixed => str_contains($plan()['payloads'][$databasePath . '-wal#statement-current-source-next91'], 'next91 failed transient commit'), false],
    'dependency next91' => [static fn (): mixed => in_array('sqlite-wal-hot-journal-statement-current-source-next91', $plan()['dependencies'], true), true],
    ...
    'blocked status' => [static fn (): mixed => $blocked()['status'], 'hot_journal_wal_statement_current_source_skipped_next91'],
    ...
    'blocked page one wal source verified' => [static fn (): mixed => str_starts_with($blocked()['current_source_prefixes'][1], 'next91 retained schema frame'), true],
];

foreach ($cases as $name => [$callback, $expected]) {
    $tests['wal hot journal statement current source next91 ' . $name] = static function (TestRunner $t) use ($callback, $expected): void {
        $t->same($expected, $callback());
    };
}

$throws = [
    'empty current statement rejected' => static fn () => SQLiteWalHotJournalSavepointReplayPlan::statementCurrentSourceNext91($journal, $dirtyDatabase, $journalBytes, $makeStack(), 'plugin-batch', '', 'next', 5, $nextBefore, $wal, $walBytes, $databasePath, [1], [1 => $clean[1]]),
    'empty next statement rejected' => static fn () => SQLiteWalHotJournalSavepointReplayPlan::statementCurrentSourceNext91($journal, $dirtyDatabase, $journalBytes, $makeStack(), 'plugin-batch', 'insert-active-plugin', '', 5, $nextBefore, $wal, $walBytes, $databasePath, [1], [1 => $clean[1]]),
    'zero next page rejected' => static fn () => SQLiteWalHotJournalSavepointReplayPlan::statementCurrentSourceNext91($journal, $dirtyDatabase, $journalBytes, $makeStack(), 'plugin-batch', 'insert-active-plugin', 'next', 0, $nextBefore, $wal, $walBytes, $databasePath, [1], [1 => $clean[1]]),
    'bad next image rejected' => static fn () => SQLiteWalHotJournalSavepointReplayPlan::statementCurrentSourceNext91($journal, $dirtyDatabase, $journalBytes, $makeStack(), 'plugin-batch', 'insert-active-plugin', 'next', 5, 'short', $wal, $walBytes, $databasePath, [1], [1 => $clean[1]]),
    'empty source pages rejected' => static fn () => SQLiteWalHotJournalSavepointReplayPlan::statementCurrentSourceNext91($journal, $dirtyDatabase, $journalBytes, $makeStack(), 'plugin-batch', 'insert-active-plugin', 'next', 5, $nextBefore, $wal, $walBytes, $databasePath, [1], []),
...
foreach ($throws as $name => $callback) {
    $tests['wal hot journal statement current source next91 ' . $name] = static function (TestRunner $t) use ($callback): void {
