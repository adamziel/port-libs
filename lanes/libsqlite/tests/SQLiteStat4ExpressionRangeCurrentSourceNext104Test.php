...
$plan104 = static fn (?array $prepared = null, ?array $current = null, ?array $predicate = null, ?array $columns = null): array => SQLiteStat4ExpressionRangeCurrentSourceNext104Plan::compare(
    $prepared ?? $source104(),
    $current ?? $current104(),
...
$tests = [
    ...
    'stat4 expression range current source next104 reuses prepared when signatures match' => static function (TestRunner $t) use ($source104): void {
        ...
        $t->same('prepared', SQLiteStat4ExpressionRangeCurrentSourceNext104Plan::compare($source104(), $fresh, $GLOBALS['predicate104'], $GLOBALS['needed104'])['selectedSource']);
    },
    'stat4 expression range current source next104 no reprepare when signatures match' => static function (TestRunner $t) use ($source104): void {
        $fresh = $source104(['name' => 'current-same-stat4']);
        $t->same(false, SQLiteStat4ExpressionRangeCurrentSourceNext104Plan::compare($source104(), $fresh, $GLOBALS['predicate104'], $GLOBALS['needed104'])['reprepareRequired']);
    },
    'stat4 expression range current source next104 schema cookie alone invalidates' => static function (TestRunner $t) use ($source104): void {
        $t->same(true, SQLiteStat4ExpressionRangeCurrentSourceNext104Plan::compare($source104(), $source104(['schemaCookie' => 106]), $GLOBALS['predicate104'], $GLOBALS['needed104'])['schemaCookieChanged']);
    },
    'stat4 expression range current source next104 stat4 generation alone invalidates' => static function (TestRunner $t) use ($source104): void {
        $t->same(true, SQLiteStat4ExpressionRangeCurrentSourceNext104Plan::compare($source104(), $source104(['stat4Generation' => 23]), $GLOBALS['predicate104'], $GLOBALS['needed104'])['stat4GenerationChanged']);
    },
    'stat4 expression range current source next104 projection change invalidates' => static function (TestRunner $t) use ($source104): void {
        $t->same(true, SQLiteStat4ExpressionRangeCurrentSourceNext104Plan::compare($source104(), $source104(['coveringColumns' => ['option_name']]), $GLOBALS['predicate104'], $GLOBALS['needed104'])['projectionChanged']);
    },
    ...
    'stat4 expression range current source next104 validates schema cookie' => static function (TestRunner $t) use ($source104): void {
        $t->throws(InvalidArgumentException::class, static fn () => SQLiteStat4ExpressionRangeCurrentSourceNext104Plan::compare($source104(['schemaCookie' => -1]), $source104(), $GLOBALS['predicate104'], $GLOBALS['needed104']));
    },
    'stat4 expression range current source next104 validates stat4 generation' => static function (TestRunner $t) use ($source104): void {
        $t->throws(InvalidArgumentException::class, static fn () => SQLiteStat4ExpressionRangeCurrentSourceNext104Plan::compare($source104(['stat4Generation' => -1]), $source104(), $GLOBALS['predicate104'], $GLOBALS['needed104']));
    },
    'stat4 expression range current source next104 validates indexes list' => static function (TestRunner $t) use ($source104): void {
        $t->throws(InvalidArgumentException::class, static fn () => SQLiteStat4ExpressionRangeCurrentSourceNext104Plan::compare($source104(['indexes' => ['bad' => []]]), $source104(), $GLOBALS['predicate104'], $GLOBALS['needed104']));
    },
    'stat4 expression range current source next104 validates expression index' => static function (TestRunner $t) use ($source104): void {
        ...
        $t->throws(InvalidArgumentException::class, static fn () => SQLiteStat4ExpressionRangeCurrentSourceNext104Plan::compare($source, $source104(), $GLOBALS['predicate104'], $GLOBALS['needed104']));
    },
