...
): array => SQLiteJsonTablePlan::currentSourceGeneratedPathRowidCostCurrentSourceNext184(
    'json_tree',
    $current ?? $current184,
    $next ?? $next184,
    'option_value',
...
foreach ($tests as $name => $case) {
    $tests['json table generated path rowid cost current source next184 ' . $name] = $case;
    unset($tests[$name]);
}
