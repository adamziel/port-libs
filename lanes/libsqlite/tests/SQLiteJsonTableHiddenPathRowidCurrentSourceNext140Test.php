...
$point140 = static fn (?array $current = null, ?array $next = null): array => SQLiteJsonTablePlan::currentSourceHiddenPathRowidNext140(
    'json_tree',
    $current ?? $current140,
    $next ?? $next140,
    'option_value',
...
$miss140 = static fn (): array => SQLiteJsonTablePlan::currentSourceHiddenPathRowidNext140(
    'json_tree',
    $current140,
    $next140,
    'option_value',
...
$range140 = static fn (): array => SQLiteJsonTablePlan::currentSourceHiddenPathRowidNext140(
    'json_tree',
    $current140,
    $next140,
    'option_value',
...
$tests = [
    ...
    'records next140 dependency' => static fn (TestRunner $t) => $t->true(in_array('sqlite-json-table-hidden-path-rowid-current-source-next140', $point140()['dependencies'], true)),
    ...
    'stable has no next140 reasons' => static fn (TestRunner $t) => $t->same([], $stable140()['next140ReplanReasons']),
    ...
    'reasons include value change' => static fn (TestRunner $t) => $t->true(in_array('json-table-hidden-path-rowid-current-source-value-changed', $point140()['next140ReplanReasons'], true)),
    'reasons preserve source json change' => static fn (TestRunner $t) => $t->true(in_array('source-json-changed', $point140()['next140ReplanReasons'], true)),
    ...
    'jsonb next reasons include source kind' => static fn (TestRunner $t) => $t->true(in_array('json-table-hidden-path-rowid-source-kind-changed', $jsonb140()['next140ReplanReasons'], true)),
    ...
    'bad function rejected' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteJsonTablePlan::currentSourceHiddenPathRowidNext140('json_bad', $current140, $next140, 'option_value', [])),
    'bad json column rejected' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteJsonTablePlan::currentSourceHiddenPathRowidNext140('json_tree', $current140, $next140, '', [])),
    'bad root column rejected' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteJsonTablePlan::currentSourceHiddenPathRowidNext140('json_tree', $current140, $next140, 'option_value', [], '')),
    'dependency scenario has no new support component' => static fn (TestRunner $t) => $t->same('no-new-support-component', 'no-new-support-component'),
];

foreach ($tests as $name => $case) {
    $tests['json table hidden path rowid current source next140 ' . $name] = $case;
    unset($tests[$name]);
}
