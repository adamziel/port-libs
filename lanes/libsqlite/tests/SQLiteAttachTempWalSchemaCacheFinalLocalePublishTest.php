...
$schemasFinalLocalePublish = [
    ...
    'report' => [
        'schema_cookie' => 795,
        'tables' => ['wp_schema_report_report', 'wp_schema_report_meta_report'],
        'indexes' => ['wp_schema_report_key_report', 'wp_schema_report_meta_key_report'],
        'file' => '/srv/wp/report-archive.sqlite',
    ],
];

$statementsFinalLocalePublish = [
...
$planFinalLocalePublish = static fn (array $events, ?array $statements = null, ?array $schemas = null): array => SQLiteAttachWalTempSchemaCachePlan::schemaCacheConsolidatedPlan(
    $schemas ?? $schemasFinalLocalePublish,
    $statements ?? $statementsFinalLocalePublish,
    $events,
);

$tests = [];

$tests['attach temp wal schema cache final locale publish extends predecessor handoff'] = static function (TestRunner $t) use ($planFinalLocalePublish): void {
    $result = $planFinalLocalePublish([
...
$tests['attach temp wal schema cache final locale publish ignores detached scratch handoff'] = static function (TestRunner $t) use ($planFinalLocalePublish): void {
    $result = $planFinalLocalePublish([
        ['op' => 'attach', 'schema' => 'scratch', 'schema_cookie' => 797, 'tables' => ['wp_scratch_archive'], 'indexes' => ['wp_scratch_key_archive'], 'file' => '/srv/wp/scratch-archive.sqlite'],
        ['op' => 'wal_commit', 'schema' => 'scratch', 'schema_cookie' => 798, 'table' => 'wp_scratch_meta_delta', 'indexes' => ['wp_scratch_meta_key_delta'], 'commit' => false],
