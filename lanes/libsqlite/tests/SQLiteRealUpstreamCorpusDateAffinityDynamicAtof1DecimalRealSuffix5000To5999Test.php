...
// recursive query covers decimal suffixes 0000..9999. Current accepted local
// shards own 0000..4999 and 6000..9999, so this shard fills the remaining
// non-overlapping 5000..5999 suffix window.
$firstSuffix = 5000;
$lastSuffix = 5999;
$cases = [];
...
$scriptFile = tempnam(sys_get_temp_dir(), 'libsqlite-atof1-decimal-5000-oracle-');
if ($scriptFile === false) {
...
$tests['real upstream corpus date affinity dynamic atof1 decimal REAL suffix 5000 5999 source truth'] =
    static function (TestRunner $t) use ($cases, $firstSuffix, $lastSuffix, $oracle, $sourceFile, $sourceText): void {
        ...
        $t->same(5000, $firstSuffix);
        $t->same(5999, $lastSuffix);
        $t->same(5000, $cases['suffix-5000']['suffix']);
        $t->same(5999, $cases['suffix-5999']['suffix']);
        $t->same('18.446744073709555000', $cases['suffix-5000']['value']);
        $t->same('18.446744073709555999', $cases['suffix-5999']['value']);
    };

foreach ($cases as $key => $case) {
    $tests['real upstream corpus date affinity dynamic atof1 decimal REAL suffix 5000 5999 ' . $key] =
...
$tests['real upstream corpus date affinity dynamic atof1 decimal REAL suffix 5000 5999 generic application rollup'] =
    static function (TestRunner $t) use ($cases): void {
        $rows = [
            ['key_name' => 'metric-low', 'decimal_metric' => $cases['suffix-5000']['value']],
            ['key_name' => 'metric-mid', 'decimal_metric' => $cases['suffix-5500']['value']],
            ['key_name' => 'metric-high', 'decimal_metric' => $cases['suffix-5999']['value']],
        ];
        $result = SQLiteSelectSql::execute(
            'SELECT key_name, '
...
$tests['real upstream corpus date affinity dynamic atof1 decimal REAL suffix 5000 5999 non overlap and dependency closure'] =
    static function (TestRunner $t): void {
        $t->same(
            'owns atof1.test atof-3.2 decimal REAL suffixes 5000..5999 only',
            'owns atof1.test atof-3.2 decimal REAL suffixes 5000..5999 only',
        );
        $t->same(
            'non-overlap: fills the accepted gap between atof-3.2 decimal REAL suffixes 0000..4999 and 6000..9999; avoids accepted atof-3.1 integer-prefix suffixes 0592..1609, atof-3.3 exponent rows, atof2 rounding, date4 rows, date/timediff matrices, and types storage-class batches',
            'non-overlap: fills the accepted gap between atof-3.2 decimal REAL suffixes 0000..4999 and 6000..9999; avoids accepted atof-3.1 integer-prefix suffixes 0592..1609, atof-3.3 exponent rows, atof2 rounding, date4 rows, date/timediff matrices, and types storage-class batches',
        );
        $t->same(
