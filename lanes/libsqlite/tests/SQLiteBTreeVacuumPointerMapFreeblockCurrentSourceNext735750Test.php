...
$makeFirstPage735750 = static function (int $pageCount): string {
...
$putPointerMapEntry735750 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database735750 = static function (int $sliceNumber) use ($makeFirstPage735750, $putPointerMapEntry735750): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage735750(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry735750($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan735750 = static function (int $sliceNumber, int $batchSize = 2) use ($database735750): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database735750($sliceNumber);
...
foreach ([735, 736, 737, 738, 739, 740, 741, 742, 743, 744, 745, 746, 747, 748, 749, 750] as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} publishes next735-750 follow-on handoff receipts"] = static function (TestRunner $t) use ($plan735750, $sliceNumber): void {
        $plan = $plan735750($sliceNumber);
        ...
        $t->same(6, $plan735750($sliceNumber, 3)->currentSourceSummary()['current_source_row_count']);
    };
}
