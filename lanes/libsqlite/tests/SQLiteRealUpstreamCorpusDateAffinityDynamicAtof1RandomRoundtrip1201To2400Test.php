...
$tclScriptFile = tempnam(sys_get_temp_dir(), 'libsqlite-atof1-random-cont-tcl-');
...
foreach (explode("\n", trim($tclOutput)) as $line) {
    ...
    $cases[$key] = [
        ...
        'ordinal' => $ordinal,
        'power' => (int) $powerText,
        'bound' => sqliteRealUpstreamAtof1RandomRoundtrip1201To2400NumericLiteral($boundText),
        'literal' => sqliteRealUpstreamAtof1RandomRoundtrip1201To2400NumericLiteral($literalText),
    ];
}
...
$oracleFile = tempnam(sys_get_temp_dir(), 'libsqlite-atof1-random-cont-oracle-');
if ($oracleFile === false) {
...
$tests['real upstream corpus date affinity dynamic atof1 random roundtrip 1201 2400 source truth'] =
    static function (TestRunner $t) use ($cases, $oracle, $ownedStart, $ownedEnd, $ownedCount, $sourceFile, $sourceText): void {
        ...
        $t->same(1201, $ownedStart);
        $t->same(2400, $ownedEnd);
        $t->same(1200, $ownedCount);
        $t->same(1200, count($cases));
        $t->same(1200, count($oracle));
        $t->same('atof1.test atof1-1.1201.1/.2', $cases['atof1-1.1201']['source']);
        $t->same('1.94235698401814121938357970975630e-02', $cases['atof1-1.1201']['literal']);
        $t->same('2', (string) $cases['atof1-1.1201']['power']);
        $t->same('atof1.test atof1-1.2400.1/.2', $cases['atof1-1.2400']['source']);
        $t->same('1.89951040856945446710923874888977e-01', $cases['atof1-1.2400']['literal']);
    };

foreach ($cases as $key => $case) {
    $tests['real upstream corpus date affinity dynamic atof1 random roundtrip 1201 2400 ' . $key] =
        static function (TestRunner $t) use ($case, $key, $oracle): void {
            ...
            $t->same(
                sqliteRealUpstreamAtof1RandomRoundtrip1201To2400DoubleHex($boundReal),
                sqliteRealUpstreamAtof1RandomRoundtrip1201To2400DoubleHex($literalReal),
                $key . ' literal text cast preserves Tcl bound double bits'
            );
            $t->same(
                sqliteRealUpstreamAtof1RandomRoundtrip1201To2400DoubleHex($boundReal),
                sqliteRealUpstreamAtof1RandomRoundtrip1201To2400DoubleHex($boundRoundTrip),
                $key . ' native quote CAST round-trip preserves bound REAL bits'
            );
            $t->same(
                sqliteRealUpstreamAtof1RandomRoundtrip1201To2400DoubleHex($literalReal),
                sqliteRealUpstreamAtof1RandomRoundtrip1201To2400DoubleHex($literalRoundTrip),
...
$tests['real upstream corpus date affinity dynamic atof1 random roundtrip 1201 2400 generic metric rollup'] =
    static function (TestRunner $t) use ($cases): void {
        $rows = [
            ['key_name' => 'metric.random.1201', 'literal_text' => $cases['atof1-1.1201']['literal']],
            ['key_name' => 'metric.random.1600', 'literal_text' => $cases['atof1-1.1600']['literal']],
            ['key_name' => 'metric.random.2000', 'literal_text' => $cases['atof1-1.2000']['literal']],
            ['key_name' => 'metric.random.2400', 'literal_text' => $cases['atof1-1.2400']['literal']],
        ];
        $result = SQLiteSelectSql::execute(
            'SELECT key_name, '
            . 'typeof(CAST(literal_text AS REAL)) AS storage_class, '
        ...
        $t->same(['metric.random.1201', 'metric.random.1600', 'metric.random.2000', 'metric.random.2400'], array_column($result, 'key_name'));
...
$tests['real upstream corpus date affinity dynamic atof1 random roundtrip 1201 2400 non overlap and dependency closure'] =
    static function (TestRunner $t): void {
        $t->same(
            'owns atof1.test atof1-1.1201..2400 text-to-REAL and quote round-trip rows only',
            'owns atof1.test atof1-1.1201..2400 text-to-REAL and quote round-trip rows only',
        );
        $t->same(
            'non-overlap: extends the accepted atof1-1.1..1200 random REAL shard and avoids accepted atof1-2 UTF16/blob rows, atof-3.1 integer-prefix suffixes, atof-3.2 decimal suffixes, atof-3.3 exponent rows, atof2 rounding, date4 rows, timediff matrices, affinity2, affinity3, and types storage-class batches',
            'non-overlap: extends the accepted atof1-1.1..1200 random REAL shard and avoids accepted atof1-2 UTF16/blob rows, atof-3.1 integer-prefix suffixes, atof-3.2 decimal suffixes, atof-3.3 exponent rows, atof2 rounding, date4 rows, timediff matrices, affinity2, affinity3, and types storage-class batches',
        );
        $t->same(
...
function sqliteRealUpstreamAtof1RandomRoundtrip1201To2400NumericLiteral(string $value): string
{
...
function sqliteRealUpstreamAtof1RandomRoundtrip1201To2400DoubleHex(float $value): string
{
