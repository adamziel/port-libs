...
$plan234 = static fn (array $options = []): array => SQLiteTriggerRecursiveViewUpsertCurrentSourceNextPlan::executeCurrentSourceUpsertReceipt(
    $rows234,
    $currentInput234,
    $nextInput234,
    $currentView234,
    ...
    $options + [
        ...
        'reset_generation' => 'app-current-reset-234',
        'post_reset_current_source_token' => 'app.current.source.postreset.234',
        'post_reset_cursor' => 'app.returning.postreset.cursor.234',
        ...
        'following_generation' => 'app-following-current-234',
...
$tokenMismatch234 = static fn (): array => $plan234(['auto_ack_current_source_upserts_next234' => true, 'expected_current_source_upsert_token_next234' => 'app.current.source.upsert.stale.234']);
...
$custom234 = static fn (): array => $plan234([
    ...
    'upsert_conflict_columns_next234' => ['key_name', 'load_policy'],
    'current_source_upsert_token_next234' => 'app.current.source.upsert.custom.234',
...
$cases234 = [
    ...
    'savepoint retained' => [static fn (): mixed => $released234()['savepoint'], 'app_recursive_view_234'],
    ...
    'conflict columns default' => [static fn (): mixed => $released234()['upsert_conflict_columns_next234'], ['key_name']],
    'conflict columns custom' => [static fn (): mixed => $custom234()['upsert_conflict_columns_next234'], ['key_name', 'load_policy']],
    'upsert token retained' => [static fn (): mixed => $released234()['current_source_upsert_token_next234'], 'app.current.source.upsert.234'],
    'custom upsert token retained' => [static fn (): mixed => $custom234()['current_source_upsert_token_next234'], 'app.current.source.upsert.custom.234'],
    ...
    'current conflict columns stamped' => [static fn (): mixed => array_values(array_unique(array_map(static fn (array $row): string => implode(',', $row['upsert_conflict_columns_next234']), $released234()['current_source_rows_next234']))), ['key_name']],
    'current token stamped' => [static fn (): mixed => array_values(array_unique(array_column($released234()['current_source_rows_next234'], 'current_source_upsert_token_next234'))), ['app.current.source.upsert.234']],
    ...
    'visible payload names released' => [static fn (): mixed => array_column($released234()['visible_returning_payloads_next234'], 'name'), ['app_summary_child', 'template_child', 'landing_url', 'next_module']],
    'held payload names missing' => [static fn (): mixed => array_column($missing234()['held_next_returning_payloads_next234'], 'name'), ['landing_url', 'next_module']],
