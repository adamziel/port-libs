...
use PortLibs\LibSqlite\SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext209Plan;
...
$summary209 = static fn (?array $cursor = null): array => SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext209Plan::compare($sql209, $currentTables209, $nextTables209, $cursor);
$tests = [];
...
$tests['compound select window recursive limit current source next209 rejects missing count'] = static function (TestRunner $t) use ($currentTables209): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext209Plan::compare(
...
foreach (range(1, 58) as $case) {
    $tests['compound select window recursive limit current source next209 generated aggregate frame boundary ' . $case] = static function (TestRunner $t) use ($case): void {
        ...
        $plan = SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext209Plan::compare($sql, $tables, $nextTables);
        $again = SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext209Plan::compare($sql, $tables, $nextTables, $plan['cursor']);
