...
$makeFirstPage623638 = static function (int $pageCount): string {
...
$putPointerMapEntry623638 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database623638 = static function (int $sliceNumber) use ($makeFirstPage623638, $putPointerMapEntry623638): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage623638(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry623638($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan623638 = static function (int $sliceNumber, int $batchSize = 2) use ($database623638): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourcePlan {
    $database = $database623638($sliceNumber);
    ...
    return SQLiteBTreeVacuumPointerMapFreeblockCurrentSourcePlan::tableLeafCurrentSourceFreelistHandoffFromDeleteResult(
        ...
        2,
        str_repeat("next{$sliceNumber}-current-source-final-handoff-", 36),
        3,
        true,
        $batchSize,
    );
...
foreach ([623, 624, 625, 626, 627, 628, 629, 630, 631, 632, 633, 634, 635, 636, 637, 638] as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} publishes next623-638 handoff receipts"] = static function (TestRunner $t) use ($plan623638, $sliceNumber): void {
        $plan = $plan623638($sliceNumber);
        ...
        $t->same(6, $plan623638($sliceNumber, 3)->currentSourceSummary()['current_source_row_count']);
    };
}
