...
$makeFirstPage399414 = static function (int $pageCount): string {
...
$putPointerMapEntry399414 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database399414 = static function (int $sliceNumber) use ($makeFirstPage399414, $putPointerMapEntry399414): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage399414(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry399414($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan399414 = static function (int $sliceNumber, int $batchSize = 2) use ($database399414): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database399414($sliceNumber);
...
foreach ([399, 400, 401, 402, 403, 404, 405, 406, 407, 408, 409, 410, 411, 412, 413, 414] as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} keeps next399-414 freelist splice ready"] = static function (TestRunner $t) use ($plan399414, $sliceNumber): void {
        $plan = $plan399414($sliceNumber);
        ...
        $t->same(6, $plan399414($sliceNumber, 3)->freelistSummary()['freelist_row_count']);
    };
}
