...
$tests['real upstream corpus date affinity dynamic date2 modifier rows cites source truth'] = static function (TestRunner $t): void {
    $t->same(
        'date2.test date2-500 deterministic datetime(y,m) partial-index modifier table rows',
        'date2.test date2-500 deterministic datetime(y,m) partial-index modifier table rows'
    );
    $t->same(
...
foreach ($modifierFactories as $modifier => $expectedFactory) {
    for ($rowid = 5; $rowid <= 68; $rowid++) {
        ...
        $tests[sprintf('real upstream corpus date affinity dynamic date2.test date2-500 modifier %s row %03d', $label, $rowid)] = static function (TestRunner $t) use ($julianDay, $modifier, $expectedDateTime, $expectedDate, $expectedTime, $rowid): void {
            ...
            $t->same(true, $rowid >= 5 && $rowid <= 68);
        };
    }
}

$tests['real upstream corpus date affinity dynamic date2 modifier rows application retention grid'] = static function (TestRunner $t): void {
    $rows = [
        ['key_name' => 'release.alpha', 'stored_at' => 2457940.5, 'modifier' => '+10 days'],
        ['key_name' => 'release.beta', 'stored_at' => 2457950.5, 'modifier' => '-10 days'],
        ['key_name' => 'release.gamma', 'stored_at' => 2457960.5, 'modifier' => 'start of month'],
        ['key_name' => 'release.delta', 'stored_at' => 2457970.5, 'modifier' => 'weekday 1'],
    ];

    $actual = [];
    foreach ($rows as $row) {
        $actual[$row['key_name']] = SQLiteCoreScalarFunction::sqlFunctionArguments('date', [$row['stored_at'], $row['modifier']]);
    }

    $t->same([
        'release.alpha' => '2017-07-16',
        'release.beta' => '2017-07-06',
        'release.gamma' => '2017-07-01',
        'release.delta' => '2017-08-07',
    ], $actual);
};
