...
use PortLibs\LibSqlite\SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNext220Plan;
...
$database220 = static function () use ($makeFirstPage220, $putPointerMapEntry220): SQLiteDatabase {
    ...
    $pages[3] = SQLiteTableLeafPage::assemble([
        ...
        SQLiteTableLeafCell::encode(2, SQLiteRecord::encode([null, '_transient_next220', str_repeat('cache:', 42)])),
...
$plan220 = static function (int $batchSize = 2): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNext220Plan {
    global $database220;

    $database = $database220();
    $deletedPage = SQLiteTableLeafPage::deleteCellByRowId($database->page(3), 2, secureDelete: true);

    return SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNext220Plan::tableLeafFromDeleteResult(
        $database,
        3,
        [
            'page' => $deletedPage,
        ...
        2,
        str_repeat('next220-current-source-commit-', 50),
        3,
        true,
        $batchSize,
    );
...
$cases220 = [
    ...
    'current source token length' => static fn (): mixed => strlen($plan220()->commitSummary()['current_source_next220_token']),
    ...
    'non overlap' => static fn (): mixed => str_contains($plan220()->commitSummary()['non_overlap'], 'does not repeat next217'),
...
$expected220 = [
    'action label' => 'btree-vacuum-pointermap-freeblock-current-source-next220',
    'summary status' => 'btree-vacuum-pointermap-freeblock-current-source-next220-ready',
    'commit row count' => 7,
    ...
    'dependency closure' => 'no new support component needed; next220 reuses next217 page-write rows, pointer-map-first ordering, duplicate pointer-map rewrite receipts, and fenced-tail guards',
    'non overlap' => true,
    'write action' => 'btree-vacuum-pointermap-freeblock-current-source-next217',
    'write row count' => 7,
...
foreach ($cases220 as $name => $callback) {
    $tests['btree vacuum pointermap freeblock current source next220 ' . $name] = static function (TestRunner $t) use ($callback, $expected220, $name): void {
        $t->same($expected220[$name], $callback());
    };
}

foreach (range(1, 70) as $index) {
    $tests['btree vacuum pointermap freeblock current source next220 commit invariant ' . $index] = static function (TestRunner $t) use ($plan220): void {
        $plan = $plan220();
        ...
        $t->same('btree-vacuum-pointermap-freeblock-current-source-next220-ready', $summary['status']);
