...
$makeFirstPage307310 = static function (int $pageCount): string {
...
$putPointerMapEntry307310 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database307310 = static function (int $sliceNumber) use ($makeFirstPage307310, $putPointerMapEntry307310): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage307310(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry307310($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan307310 = static function (int $sliceNumber, int $batchSize = 2) use ($database307310): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database307310($sliceNumber);
...
foreach ([307, 308, 309, 310] as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} keeps next307-310 splice receipts ordered"] = static function (TestRunner $t) use ($plan307310, $sliceNumber): void {
        $plan = $plan307310($sliceNumber);
        ...
        $t->same(6, $plan307310($sliceNumber, 3)->freelistSummary()['freelist_row_count']);
    };
}
