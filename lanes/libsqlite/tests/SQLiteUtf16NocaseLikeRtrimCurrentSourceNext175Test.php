...
$row175 = static fn (int $id, string $name, int $encoding): array => [
    'setting_id' => $id,
    'key_name_bytes' => $enc175($name, $encoding),
...
$plan175 = static fn (
    ...
    ?string $escape = '!',
): array => SQLiteUtf16NocaseLikeRtrimCurrentSourceNextPlan::keyValueRowKeyTokenFingerprintPlan(
    $current ?? $current175,
...
$tests['utf16 nocase like rtrim current source nextOneSevenFive clean byte token can continue'] = static function (TestRunner $t) use ($row175, $enc175): void {
    ...
    $result = SQLiteUtf16NocaseLikeRtrimCurrentSourceNextPlan::keyValueRowKeyTokenFingerprintPlan($current, $next, 'plugin!_cache%', '!', $token, 'stable', 'stable', 5, 5);
...
$tests['utf16 nocase like rtrim current source nextOneSevenFive source cookie invalidation composes with fingerprint'] = static function (TestRunner $t) use ($row175, $enc175): void {
    ...
    $result = SQLiteUtf16NocaseLikeRtrimCurrentSourceNextPlan::keyValueRowKeyTokenFingerprintPlan($current, $next, 'plugin!_cache%', '!', $token);
...
$tests['utf16 nocase like rtrim current source nextOneSevenFive rejects bad token'] = static function (TestRunner $t) use ($current175, $nextOneSevenFive): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteUtf16NocaseLikeRtrimCurrentSourceNextPlan::keyValueRowKeyTokenFingerprintPlan(
        $current175,
        $nextOneSevenFive,
        'plugin%',
        null,
