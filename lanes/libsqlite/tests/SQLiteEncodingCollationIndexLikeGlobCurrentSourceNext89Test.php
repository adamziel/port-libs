...
$cases = [
    ...
    'like source stable records current source' => ['plugin%', 'LIKE', 'NOCASE', null, false, 'currentSource', 'main.app_settings'],
    'like source stable records next source' => ['plugin%', 'LIKE', 'NOCASE', null, false, 'nextSource', 'main.app_settings'],
    ...
    'same source stable theme reusable' => ['theme%', 'LIKE', 'NOCASE', null, false, 'cursorReusable', true, 'main.app_settings', 'main.app_settings', 21, 21, 3, 3],
    'same source stable theme no invalidation' => ['theme%', 'LIKE', 'NOCASE', null, false, 'cursorInvalidated', false, 'main.app_settings', 'main.app_settings', 21, 21, 3, 3],
    'same source stable theme reasons empty' => ['theme%', 'LIKE', 'NOCASE', null, false, 'invalidationReasons', [], 'main.app_settings', 'main.app_settings', 21, 21, 3, 3],
    'source name change reason' => ['theme%', 'LIKE', 'NOCASE', null, false, 'invalidationReasons.0', 'source-name', 'main.app_settings', 'temp.app_settings', 21, 21, 3, 3],
];
...
foreach ($cases as $name => $case) {
    $tests['encoding collation index like glob current source next89 ' . $name] = static function (TestRunner $t) use ($plan, $case): void {
        ...
        $currentSource = $case[7] ?? 'main.app_settings';
        $nextSource = $case[8] ?? 'main.app_settings';
