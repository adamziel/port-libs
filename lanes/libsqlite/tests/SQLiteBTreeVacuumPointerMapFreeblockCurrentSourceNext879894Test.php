...
$makeFirstPage879894 = static function (int $pageCount): string {
...
$putPointerMapEntry879894 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database879894 = static function (int $sliceNumber) use ($makeFirstPage879894, $putPointerMapEntry879894): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage879894(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry879894($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan879894 = static function (int $sliceNumber, int $batchSize = 2) use ($database879894): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourcePlan {
    $database = $database879894($sliceNumber);
...
foreach ([879, 880, 881, 882, 883, 884, 885, 886, 887, 888, 889, 890, 891, 892, 893, 894] as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} extends next879-894 handoff receipts"] = static function (TestRunner $t) use ($plan879894, $sliceNumber): void {
        $plan = $plan879894($sliceNumber);
        ...
        $t->same(6, $plan879894($sliceNumber, 3)->currentSourceSummary()['current_source_row_count']);
    };
}
