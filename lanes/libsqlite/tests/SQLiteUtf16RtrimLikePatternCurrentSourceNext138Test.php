...
): array => SQLiteUtf16RtrimLikePatternCurrentSourceNext138Plan::wordpressOptionNamePlan(
    $currentRows,
    $nextRows,
...
$tests['utf16 rtrim like pattern current source next138 stable unchanged still records rtrim full scan'] = static function (TestRunner $t) use ($row, $bytes): void {
    ...
    $plan = SQLiteUtf16RtrimLikePatternCurrentSourceNext138Plan::wordpressOptionNamePlan($rows, $rows, $bytes('plugin_cache%', 'UTF-16LE'), 'UTF-16LE', null, null, true, 'stable', 'stable');
...
$tests['utf16 rtrim like pattern current source next138 accepts utf16 alias'] = static function (TestRunner $t) use ($currentRows, $nextRows, $bytes): void {
    $plan = SQLiteUtf16RtrimLikePatternCurrentSourceNext138Plan::wordpressOptionNamePlan($currentRows, $nextRows, $bytes('plugin_cache%', 'UTF-16LE'), 'UTF-16');
...
$tests['utf16 rtrim like pattern current source next138 rejects invalid pattern encoding'] = static function (TestRunner $t) use ($currentRows, $nextRows, $bytes): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteUtf16RtrimLikePatternCurrentSourceNext138Plan::wordpressOptionNamePlan($currentRows, $nextRows, $bytes('plugin%', 'UTF-16LE'), 'UTF-32'));
};
...
$tests['utf16 rtrim like pattern current source next138 rejects malformed pattern'] = static function (TestRunner $t) use ($currentRows, $nextRows): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteUtf16RtrimLikePatternCurrentSourceNext138Plan::wordpressOptionNamePlan($currentRows, $nextRows, "\x00\xd8", 'UTF-16LE'));
};
...
$tests['utf16 rtrim like pattern current source next138 rejects malformed escape'] = static function (TestRunner $t) use ($currentRows, $nextRows, $bytes): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteUtf16RtrimLikePatternCurrentSourceNext138Plan::wordpressOptionNamePlan($currentRows, $nextRows, $bytes('plugin!_%', 'UTF-16LE'), 'UTF-16LE', "\x00\xd8", 'UTF-16LE'));
};
...
$tests['utf16 rtrim like pattern current source next138 rejects multi character escape'] = static function (TestRunner $t) use ($currentRows, $nextRows, $bytes): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteUtf16RtrimLikePatternCurrentSourceNext138Plan::wordpressOptionNamePlan($currentRows, $nextRows, $bytes('plugin!!_%', 'UTF-16LE'), 'UTF-16LE', $bytes('!!', 'UTF-16LE'), 'UTF-16LE'));
};
...
$tests['utf16 rtrim like pattern current source next138 rejects missing option id'] = static function (TestRunner $t) use ($nextRows, $bytes): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteUtf16RtrimLikePatternCurrentSourceNext138Plan::wordpressOptionNamePlan([['option_name_bytes' => 'p', 'text_encoding' => 1]], $nextRows, $bytes('p%', 'UTF-8'), 'UTF-8'));
};
...
$tests['utf16 rtrim like pattern current source next138 rejects missing bytes'] = static function (TestRunner $t) use ($nextRows, $bytes): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteUtf16RtrimLikePatternCurrentSourceNext138Plan::wordpressOptionNamePlan([['option_id' => 1, 'text_encoding' => 1]], $nextRows, $bytes('p%', 'UTF-8'), 'UTF-8'));
};
...
$tests['utf16 rtrim like pattern current source next138 rejects missing encoding'] = static function (TestRunner $t) use ($nextRows, $bytes): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteUtf16RtrimLikePatternCurrentSourceNext138Plan::wordpressOptionNamePlan([['option_id' => 1, 'option_name_bytes' => 'p']], $nextRows, $bytes('p%', 'UTF-8'), 'UTF-8'));
};
