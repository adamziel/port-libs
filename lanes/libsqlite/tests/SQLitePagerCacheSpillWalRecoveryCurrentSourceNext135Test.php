...
use PortLibs\LibSqlite\SQLitePagerCacheSpillWalRecoveryCurrentSourceNext135Plan;
...
$plan = static fn (
    ...
    ?int $limit = 3,
    bool $enabled = true,
): array => SQLitePagerCacheSpillWalRecoveryCurrentSourceNext135Plan::plan(
    $databasePath,
    $databaseBytes,
    $bytes ?? $walBytes,
    $pages ?? $cachePages,
...
$throws = [
    'rejects empty path' => static fn () => SQLitePagerCacheSpillWalRecoveryCurrentSourceNext135Plan::plan('', $databaseBytes, $walBytes, $cachePages, 8, 3),
    'rejects empty database' => static fn () => SQLitePagerCacheSpillWalRecoveryCurrentSourceNext135Plan::plan($databasePath, '', $walBytes, $cachePages, 8, 3),
    'rejects empty wal' => static fn () => SQLitePagerCacheSpillWalRecoveryCurrentSourceNext135Plan::plan($databasePath, $databaseBytes, '', $cachePages, 8, 3),
    'rejects empty cache' => static fn () => SQLitePagerCacheSpillWalRecoveryCurrentSourceNext135Plan::plan($databasePath, $databaseBytes, $walBytes, [], 8, 3),
    'rejects negative reader' => static fn () => SQLitePagerCacheSpillWalRecoveryCurrentSourceNext135Plan::plan($databasePath, $databaseBytes, $walBytes, $cachePages, 8, 3, -1),
    'rejects reader past valid prefix' => static fn () => SQLitePagerCacheSpillWalRecoveryCurrentSourceNext135Plan::plan($databasePath, $databaseBytes, $walBytes, $cachePages, 8, 3, 99),
    'rejects unaligned database' => static fn () => SQLitePagerCacheSpillWalRecoveryCurrentSourceNext135Plan::plan($databasePath, $databaseBytes . 'x', $walBytes, $cachePages, 8, 3),
    ...
    'rejects bad spill threshold' => static fn () => SQLitePagerCacheSpillWalRecoveryCurrentSourceNext135Plan::plan($databasePath, $databaseBytes, $walBytes, $cachePages, 8, 0),
    'rejects bad max spill' => static fn () => SQLitePagerCacheSpillWalRecoveryCurrentSourceNext135Plan::plan($databasePath, $databaseBytes, $walBytes, $cachePages, 8, 3, null, 0),
];
