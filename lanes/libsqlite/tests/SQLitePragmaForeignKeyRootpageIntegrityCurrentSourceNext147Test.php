...
use PortLibs\LibSqlite\SQLitePragmaForeignKeyRootpageIntegrityCurrentSourceNext147;
...
): array => SQLitePragmaForeignKeyRootpageIntegrityCurrentSourceNext147::currentNextPage(
    $currentDatabase,
    $schemas(),
    $catalog,
...
$tests['pragma foreignkey rootpage integrity current source next147 rejects negative offset'] = static function (TestRunner $t) use ($currentDatabase, $schemas, $catalog, $nextDatabase, $cleanSchemas): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLitePragmaForeignKeyRootpageIntegrityCurrentSourceNext147::currentNextPage($currentDatabase, $schemas(), $catalog, $nextDatabase, $cleanSchemas, $catalog, 'PRAGMA foreign_key_check(wp_options)', 'PRAGMA integrity_check', -1));
};
...
$tests['pragma foreignkey rootpage integrity current source next147 rejects zero limit'] = static function (TestRunner $t) use ($currentDatabase, $schemas, $catalog, $nextDatabase, $cleanSchemas): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLitePragmaForeignKeyRootpageIntegrityCurrentSourceNext147::currentNextPage($currentDatabase, $schemas(), $catalog, $nextDatabase, $cleanSchemas, $catalog, 'PRAGMA foreign_key_check(wp_options)', 'PRAGMA integrity_check', 0, 0));
};
