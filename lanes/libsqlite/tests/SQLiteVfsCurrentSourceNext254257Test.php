...
return [
    ...
    'vfs current source next254-257 blocks dirty claim' => static fn (TestRunner $t) => $t->same(true, in_array('dirty-pages-present', SQLiteVfsCurrentSourceNextPlan::run(['claim(reader-ready-next245,shared-cache-next245,reader-reuse-next257)'], ['current' => $dirtyCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source next254-257 blocks stale ack claim' => static fn (TestRunner $t) => $t->same(true, in_array('stale-reuse-ack', SQLiteVfsCurrentSourceNextPlan::run(['claim(reader-ready-next245,shared-cache-next245,reader-reuse-next257)'], ['current' => $staleAckCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source next254-257 blocks publish without claim' => static fn (TestRunner $t) => $t->same(true, in_array('missing-reuse-claim', SQLiteVfsCurrentSourceNextPlan::run(['publish(reader-ready-next245,reader-reuse-next257,shared-cache-next257)'], ['current' => $readyCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source next254-257 blocks stale claim' => static fn (TestRunner $t) => $t->same(true, in_array('stale-reuse-claim', SQLiteVfsCurrentSourceNextPlan::run(['publish(reader-ready-next245,reader-reuse-next257,shared-cache-next257)'], ['current' => $staleClaimCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source next254-257 accepts preclaimed publish' => static fn (TestRunner $t) => $t->same('published-reused-current-source', SQLiteVfsCurrentSourceNextPlan::run(['publish(reader-ready-next245,reader-reuse-next257,shared-cache-next257)'], ['current' => $claimedCurrent])['events'][0]['status']),
    'vfs current source next254-257 rejects empty operations' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteVfsCurrentSourceNextPlan::run([])),
    'vfs current source next254-257 rejects bad claim token' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteVfsCurrentSourceNextPlan::run([['op' => 'claim', 'snapshot' => 'reader-ready-next245', 'ack' => 'shared-cache-next245', 'claim' => 'bad claim']], ['current' => $readyCurrent])),
    'vfs current source next254-257 rejects unsupported operation' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteVfsCurrentSourceNextPlan::run(['republish(reader-ready-next245,shared-cache-next257)'], ['current' => $readyCurrent])),
