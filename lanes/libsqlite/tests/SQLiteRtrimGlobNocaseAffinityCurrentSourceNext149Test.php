...
use PortLibs\LibSqlite\SQLiteRtrimGlobNocaseAffinityCurrentSourceNext149Plan;
...
): array => SQLiteRtrimGlobNocaseAffinityCurrentSourceNext149Plan::wordpressOptionNameValuePlan(
    $current ?? $currentRows,
    $next ?? $nextRows,
    $pattern,
    $minimum,
...
$tests['rtrim glob nocase affinity current source next149 stable rows still keep class residual invalidation'] = static function (TestRunner $t) use ($row): void {
    ...
    $result = SQLiteRtrimGlobNocaseAffinityCurrentSourceNext149Plan::wordpressOptionNameValuePlan($rows, $rows, 'plugin[-_]cache', 9, 12, 'stable', 'stable', 1, 1, 1, 1);
...
$tests['rtrim glob nocase affinity current source next149 range class records unicode ranges'] = static function (TestRunner $t) use ($row): void {
    ...
    $result = SQLiteRtrimGlobNocaseAffinityCurrentSourceNext149Plan::wordpressOptionNameValuePlan($rows, $rows, 'plugin-[a-é]cache', 9, 12, 'stable', 'stable', 1, 1, 1, 1);
...
$tests['rtrim glob nocase affinity current source next149 rejects bad option row shape'] = static function (TestRunner $t) use ($nextRows): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteRtrimGlobNocaseAffinityCurrentSourceNext149Plan::wordpressOptionNameValuePlan([['option_id' => '1', 'option_name_bytes' => 'x', 'option_value_bytes' => '1', 'name_text_encoding' => 1, 'value_text_encoding' => 1]], $nextRows, 'plugin[-_]cache*', 1, 2));
};
