...
$pageSize = 512;
$database = '/srv/wp-content/database/wp-next558.sqlite';
...
$formatPage = static function (string $label) use ($pageSize): string {
    ...
    $page = substr_replace($page, pack('N', 558), 60, 4);
...
$tokens = [$journal => 'member-main-current-558'];
$headers = [$journal => hash('sha256', 'main header next558')];
$base = [
    'source_id' => 'pager-reader-cache-current-source-next558',
    'epoch' => 558,
    'format_signature' => hash('sha256', implode('|', [512, 4, 2, 558, 0])),
    'publication_generation' => 558,
    'master_source_digest' => hash('sha256', 'master-next558'),
    'recovery_sequence' => 558,
...
foreach ($tokenFields as $field) {
    $base[$field] = str_replace('_', '-', preg_replace('/_token$/', '', $field)) . '-current-558';
}
...
$plan = static fn (array $cacheExtra = [], array $readExtra = []): array => SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::readerCacheStatementFence(558,
    $database,
    $master,
    $masterBytes,
    implode('', $before),
...
$tests['pager master journal reader cache current source next558 admits current VDBE opcode literal and arithmetic fences'] = static function (TestRunner $t) use ($plan): void {
    $result = $plan();
    $t->same('pager-master-journal-reader-cache-current-source-next558', $result['status']);
    $t->same([], $result['invalidated_cache_page_numbers']);
    $t->same(['read-options' => true], $result['read_cache_hits']);
    $t->same('reader-cache-stmt-vdbe-multiply-current-558', $result['current_reader_cache_stmt_vdbe_multiply_token']);
};

$tests['pager master journal reader cache current source plan dispatches full width calls to next558'] = static function (TestRunner $t) use ($database, $master, $masterBytes, $before, $pageSize, $recovered, $cacheEntry, $read, $base, $tokens, $headers, $tokenFields): void {
...
$tests['pager master journal reader cache current source next558 invalidates stale multiply cache'] = static function (TestRunner $t) use ($plan, $opCount): void {
    $result = $plan(['reader_cache_stmt_vdbe_multiply_token' => 'stmt-vdbe-multiply-old']);
    $t->same([1], $result['reader_cache_stmt_vdbe_multiply_invalidated_cache_page_numbers']);
    $t->same([1], $result['invalidated_cache_page_numbers']);
    $t->same(1, $opCount($result, 'invalidate_reader_cache_reader_cache_stmt_vdbe_multiply_current_source_next558'));
};

$tests['pager master journal reader cache current source next558 reopens stale multiply read ticket'] = static function (TestRunner $t) use ($plan, $opCount): void {
    $result = $plan([], ['reader_cache_stmt_vdbe_multiply_token' => 'stmt-vdbe-multiply-old']);
    $t->same(['read-options'], $result['reopen_reader_ids']);
    $t->same(false, $result['next_reads'][0]['cache_hit']);
    $t->same('reader_ticket_reader_cache_stmt_vdbe_multiply_predates_current_source', $result['next_reads'][0]['reader_cache_stmt_vdbe_multiply_token_reason']);
    $t->same(1, $opCount($result, 'reopen_reader_for_reader_cache_stmt_vdbe_multiply_current_source_next558'));
};

$tests['pager master journal reader cache current source next558 missing multiply token rejects'] = static function (TestRunner $t) use ($plan): void {
    $t->throws(Throwable::class, static fn () => $plan(['reader_cache_stmt_vdbe_multiply_token' => null]));
};
