...
$tests = [
    ...
    'planner stat4 expression partial current source next318333 cursor appended' => static fn (TestRunner $t) => $t->same('PrepareStat4ExpressionPartialPreparedHandoffRange', $plan318333()['cursorProgram'][array_key_last($plan318333()['cursorProgram'])]['opcode']),
    'planner stat4 expression partial current source next318333 cursor mode' => static fn (TestRunner $t) => $t->same('prepared-handoff-range-current-source-stat4-expression-partial-prep', $plan318333()['cursorProgram'][array_key_last($plan318333()['cursorProgram'])]['mode']),
