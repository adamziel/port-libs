...
return [
    ...
    'vfs current source next242-245 blocks dirty claim' => static fn (TestRunner $t) => $t->same(true, in_array('dirty-pages-present', SQLiteVfsCurrentSourceNextPlan::run(['claim(reader-ready-republished,shared-cache-next237,reader-reuse-next245)'], ['current' => $dirtyCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source next242-245 blocks stale ack claim' => static fn (TestRunner $t) => $t->same(true, in_array('stale-reuse-ack', SQLiteVfsCurrentSourceNextPlan::run(['claim(reader-ready-republished,shared-cache-next237,reader-reuse-next245)'], ['current' => $staleAckCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source next242-245 blocks publish without claim' => static fn (TestRunner $t) => $t->same(true, in_array('missing-reuse-claim', SQLiteVfsCurrentSourceNextPlan::run(['publish(reader-ready-republished,reader-reuse-next245,shared-cache-next245)'], ['current' => $readyCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source next242-245 blocks stale claim' => static fn (TestRunner $t) => $t->same(true, in_array('stale-reuse-claim', SQLiteVfsCurrentSourceNextPlan::run(['publish(reader-ready-republished,reader-reuse-next245,shared-cache-next245)'], ['current' => $staleClaimCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source next242-245 accepts preclaimed publish' => static fn (TestRunner $t) => $t->same('published-reused-current-source', SQLiteVfsCurrentSourceNextPlan::run(['publish(reader-ready-republished,reader-reuse-next245,shared-cache-next245)'], ['current' => $claimedCurrent])['events'][0]['status']),
    'vfs current source next242-245 rejects empty operations' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteVfsCurrentSourceNextPlan::run([])),
    'vfs current source next242-245 rejects bad claim token' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteVfsCurrentSourceNextPlan::run([['op' => 'claim', 'snapshot' => 'reader-ready-republished', 'ack' => 'shared-cache-next237', 'claim' => 'bad claim']], ['current' => $readyCurrent])),
    'vfs current source next242-245 rejects unsupported operation' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteVfsCurrentSourceNextPlan::run(['republish(reader-ready-republished,shared-cache-next245)'], ['current' => $readyCurrent])),
