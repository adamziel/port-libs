...
$outerSql = "UPDATE wp_options SET (option_name, status, option_value, bytes) = (option_name || ':outer_ignore_retry', 'outer', option_value || ':outer_ignore_retry', bytes + 2) WHERE option_id IN (7, 9) RETURNING option_id, blog_id, option_name, status, option_value, bytes, (status, option_name) = ('outer', 'pending_theme:outer_ignore_retry') AS pending_outer ORDER BY option_id";
...
$retryUpdateSql = "UPDATE wp_options SET (option_name, status, option_value, bytes) = (option_name || ':retry_ignore_retry', 'retry', option_value || ':retry_ignore_retry', bytes + 10) WHERE option_id IN (7, 8, 9) RETURNING option_id, blog_id, option_name, status, option_value, bytes, (status, option_name) = ('retry', 'pending_theme:outer_ignore_retry:retry_ignore_retry') AS pending_retry ORDER BY option_id";
...
$cases = [
    ...
    'outer row nine value updated' => [static fn (): mixed => array_column($outerOnly()['tables']['wp_options'], 'option_value', 'option_id')[9], 'rules:outer_ignore_retry'],
    ...
    'ignore keeps outer row nine name' => [static fn (): mixed => array_column($ignoreAfterOuter()['tables']['wp_options'], 'option_name', 'option_id')[9], 'rewrite_rules:outer_ignore_retry'],
    ...
    'plan inner image keeps outer row seven' => [static fn (): mixed => array_column($plan()['inner_savepoint_image_tables']['wp_options'], 'option_name', 'option_id')[7], 'pending_theme:outer_ignore_retry'],
    ...
    'plan rollback restores row seven outer value' => [static fn (): mixed => array_column($plan()['rollback_to_inner_current_source_tables']['wp_options'], 'option_name', 'option_id')[7], 'pending_theme:outer_ignore_retry'],
    ...
    'plan retry update source rows from inner image' => [static fn (): mixed => array_column($plan()['inner_retry_statements'][0]['source_rows'], 'option_name'), ['pending_theme:outer_ignore_retry', 'orphaned_cache', 'rewrite_rules:outer_ignore_retry']],
    ...
    'plan retry row eight value from original cache' => [static fn (): mixed => $plan()['yielded_inner_retry_returning'][0]['rows'][1]['option_value'], 'cache:retry_ignore_retry'],
    'plan retry row nine value includes outer then retry' => [static fn (): mixed => $plan()['yielded_inner_retry_returning'][0]['rows'][2]['option_value'], 'rules:outer_ignore_retry:retry_ignore_retry'],
    ...
    'plan final row seven outer plus retry' => [static fn (): mixed => array_column($plan()['current_source_tables']['wp_options'], 'option_name', 'option_id')[7], 'pending_theme:outer_ignore_retry:retry_ignore_retry'],
    'plan final row eight retry name' => [static fn (): mixed => array_column($plan()['current_source_tables']['wp_options'], 'option_name', 'option_id')[8], 'orphaned_cache:retry_ignore_retry'],
    ...
    'plan dependency outer preserved' => [static fn (): mixed => in_array('sqlite-nested-savepoint-rollback-to-preserves-outer-rowvalue-returning-nested-ignore-retry', $plan()['dependencies'], true), true],
...
foreach ($cases as $name => [$callback, $expected]) {
    $tests['rowvalue update delete returning savepoint current source nested-ignore-retry ' . $name] = static function (TestRunner $t) use ($callback, $expected): void {
