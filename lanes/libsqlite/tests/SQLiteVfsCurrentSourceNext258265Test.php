...
$plan = static function () use ($readyCurrent): array {
    static $result = null;
    if ($result === null) {
        $result = SQLiteVfsCurrentSourceNext258265Plan::run([
...
return [
    ...
    'vfs current source next258-265 blocks dirty claim' => static fn (TestRunner $t) => $t->same(true, in_array('dirty-pages-present', SQLiteVfsCurrentSourceNext258265Plan::run(['claim(reader-ready-next257,shared-cache-next257,reader-reuse-next265)'], ['current' => $dirtyCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source next258-265 blocks stale ack claim' => static fn (TestRunner $t) => $t->same(true, in_array('stale-reuse-ack', SQLiteVfsCurrentSourceNext258265Plan::run(['claim(reader-ready-next257,shared-cache-next257,reader-reuse-next265)'], ['current' => $staleAckCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source next258-265 blocks publish without claim' => static fn (TestRunner $t) => $t->same(true, in_array('missing-reuse-claim', SQLiteVfsCurrentSourceNext258265Plan::run(['publish(reader-ready-next257,reader-reuse-next265,shared-cache-next265)'], ['current' => $readyCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source next258-265 blocks stale claim' => static fn (TestRunner $t) => $t->same(true, in_array('stale-reuse-claim', SQLiteVfsCurrentSourceNext258265Plan::run(['publish(reader-ready-next257,reader-reuse-next265,shared-cache-next265)'], ['current' => $staleClaimCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source next258-265 accepts preclaimed publish' => static fn (TestRunner $t) => $t->same('published-reused-current-source', SQLiteVfsCurrentSourceNext258265Plan::run(['publish(reader-ready-next257,reader-reuse-next265,shared-cache-next265)'], ['current' => $claimedCurrent])['events'][0]['status']),
    'vfs current source next258-265 rejects empty operations' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteVfsCurrentSourceNext258265Plan::run([])),
    'vfs current source next258-265 rejects bad claim token' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteVfsCurrentSourceNext258265Plan::run([['op' => 'claim', 'snapshot' => 'reader-ready-next257', 'ack' => 'shared-cache-next257', 'claim' => 'bad claim']], ['current' => $readyCurrent])),
    'vfs current source next258-265 rejects unsupported operation' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteVfsCurrentSourceNext258265Plan::run(['republish(reader-ready-next257,shared-cache-next265)'], ['current' => $readyCurrent])),
