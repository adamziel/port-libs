...
$plan225 = static fn (
    ...
    int $nextCookie = 225,
): array => SQLiteUtf16NocaseLikeRtrimCurrentSourceNext225Plan::wordpressOptionNameSourceBytePlan(
    $current ?? $current225,
    $next ?? $next225,
    'plugin!_cache%',
    '!',
...
$cases225 = [
    'status' => ['status', 'utf16-nocase-like-rtrim-current-source-next225'],
    'base status' => ['baseStatus', 'utf16-nocase-like-rtrim-current-source-next219'],
    ...
    'dependency source' => ['dependencies.4', 'sqlite-current-source-next225'],
];

foreach ($cases225 as $name => [$path, $expected]) {
    $tests['utf16 nocase like rtrim current source next225 ' . $name] = static function (TestRunner $t) use ($plan225, $valueAt225, $path, $expected): void {
        $t->same($expected, $valueAt225($plan225(), $path));
    };
}

$tests['utf16 nocase like rtrim current source next225 invalidation reason order'] = static function (TestRunner $t) use ($plan225): void {
    $t->same([
        'source-name',
        'schema-cookie',
        'malformed-text',
...
$tests['utf16 nocase like rtrim current source next225 same decoded rows still invalidate on endian rewrite'] = static function (TestRunner $t) use ($row225): void {
    $current = [
        $row225(1, 'plugin_cache', 'UTF-16LE'),
    ...
    $result = SQLiteUtf16NocaseLikeRtrimCurrentSourceNext225Plan::wordpressOptionNameSourceBytePlan(
        $current,
        $next,
        'plugin!_cache%',
        '!',
...
$tests['utf16 nocase like rtrim current source next225 byte identical source can reuse cursor'] = static function (TestRunner $t) use ($row225): void {
    $rows = [
        $row225(1, 'plugin_cache', 'UTF-16LE'),
        $row225(2, 'plugin_cache_alpha', 'UTF-16BE'),
    ];
...
$tests['utf16 nocase like rtrim current source next225 rejects unsupported raw encoding'] = static function (TestRunner $t) use ($bad225): void {
    $rows = [$bad225(1, "p\0", 4)];
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteUtf16NocaseLikeRtrimCurrentSourceNext225Plan::wordpressOptionNameSourceBytePlan($rows, $rows));
};
