...
return [
    ...
    'vfs current source next222-225 blocks prepare without prior publish' => static fn (TestRunner $t) => $t->same('blocked-no-prior-publish', SQLiteVfsCurrentSourceNextPlan::run(['prepare(empty)'], ['current' => $withoutPriorPublish])['status']),
    'vfs current source next222-225 blocks publish without after-ready reuse' => static fn (TestRunner $t) => $t->same('blocked-no-after-ready-reuse', SQLiteVfsCurrentSourceNextPlan::run(['publish(empty)'], ['current' => $current])['status']),
    'vfs current source next222-225 blocks reuse when current dirty' => static fn (TestRunner $t) => $t->same('blocked-dirty', SQLiteVfsCurrentSourceNextPlan::run(['reuse(clean)'], ['current' => $dirtyAfterReady])['status']),
    'vfs current source next222-225 reports missing ready receipt' => static fn (TestRunner $t) => $t->same('missing-ready-receipt', SQLiteVfsCurrentSourceNextPlan::run(['reuse(missing)'], ['current' => $current])['status']),
    'vfs current source next222-225 rejects empty operations' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteVfsCurrentSourceNextPlan::run([])),
    'vfs current source next222-225 rejects bad token' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteVfsCurrentSourceNextPlan::run([['op' => 'prepare', 'token' => 'bad token']], ['current' => $current])),
    'vfs current source next222-225 rejects missing selected source' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteVfsCurrentSourceNextPlan::run(['prepare(no-source)'])),
];
