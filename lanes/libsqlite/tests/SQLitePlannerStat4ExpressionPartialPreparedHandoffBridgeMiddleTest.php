...
$tests = [
    ...
    'planner stat4 expression partial current source next350365 cursor appended' => static fn (TestRunner $t) => $t->same('PrepareStat4ExpressionPartialPreparedHandoffRange', $plan350365()['cursorProgram'][array_key_last($plan350365()['cursorProgram'])]['opcode']),
    'planner stat4 expression partial current source next350365 cursor mode' => static fn (TestRunner $t) => $t->same('prepared-handoff-range-current-source-stat4-expression-partial-prep', $plan350365()['cursorProgram'][array_key_last($plan350365()['cursorProgram'])]['mode']),
