...
$makeFirstPage303306 = static function (int $pageCount): string {
...
$putPointerMapEntry303306 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database303306 = static function (int $sliceNumber) use ($makeFirstPage303306, $putPointerMapEntry303306): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage303306(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry303306($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan303306 = static function (int $sliceNumber, int $batchSize = 2) use ($database303306): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database303306($sliceNumber);
...
foreach ([303, 304, 305, 306] as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} keeps next303-306 freelist splice receipts isolated"] = static function (TestRunner $t) use ($plan303306, $sliceNumber): void {
        $plan = $plan303306($sliceNumber);
        ...
        $t->same(6, $plan303306($sliceNumber, 3)->freelistSummary()['freelist_row_count']);
    };
}
