...
$tests['real upstream corpus date affinity dynamic real date4 continuation cites upstream loop'] = static function (TestRunner $t) use ($date4Start, $date4Count): void {
...
for ($i = $date4Start; $i < $date4Start + $date4Count; $i++) {
    ...
    $tests[sprintf('real upstream corpus date affinity dynamic real date4.test date4-%05d continuation strftime libc format parity', $i)] = static function (TestRunner $t) use ($date4Format, $timestamp, $expectedDate4, $i): void {
...
$tests['real upstream corpus date affinity dynamic real date4 continuation application audit rollup'] = static function (TestRunner $t) use ($date4Format, $date4Start, $date4Count, $expectedDate4): void {
    $sampleIndexes = [$date4Start, $date4Start + 250, $date4Start + 750, $date4Start + $date4Count - 1];
    $actual = [];
    $expected = [];

    foreach ($sampleIndexes as $index) {
        $timestamp = $index * 86390;
        $actual['event-' . $index] = SQLiteCoreScalarFunction::sqlFunctionArguments('strftime', [$date4Format, $timestamp, 'unixepoch']);
        $expected['event-' . $index] = $expectedDate4($timestamp);
    }
