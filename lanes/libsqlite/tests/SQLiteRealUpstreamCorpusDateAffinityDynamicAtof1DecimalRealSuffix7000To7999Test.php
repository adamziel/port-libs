...
// own 0000..4999 and 6000..6999, while a pending ready shard owns 5000..5999,
// so this shard owns the next non-overlapping 7000..7999 suffix window.
$firstSuffix = 7000;
$lastSuffix = 7999;
$cases = [];
...
$scriptFile = tempnam(sys_get_temp_dir(), 'libsqlite-atof1-decimal-7000-oracle-');
if ($scriptFile === false) {
...
$tests['real upstream corpus date affinity dynamic atof1 decimal REAL suffix 7000 7999 source truth'] =
    static function (TestRunner $t) use ($cases, $firstSuffix, $lastSuffix, $oracle, $sourceFile, $sourceText): void {
        ...
        $t->same(7000, $firstSuffix);
        $t->same(7999, $lastSuffix);
        $t->same(7000, $cases['suffix-7000']['suffix']);
        $t->same(7999, $cases['suffix-7999']['suffix']);
        $t->same('18.446744073709557000', $cases['suffix-7000']['value']);
        $t->same('18.446744073709557999', $cases['suffix-7999']['value']);
    };

foreach ($cases as $key => $case) {
    $tests['real upstream corpus date affinity dynamic atof1 decimal REAL suffix 7000 7999 ' . $key] =
...
$tests['real upstream corpus date affinity dynamic atof1 decimal REAL suffix 7000 7999 generic application rollup'] =
    static function (TestRunner $t) use ($cases): void {
        $rows = [
            ['key_name' => 'metric-low', 'decimal_metric' => $cases['suffix-7000']['value']],
            ['key_name' => 'metric-mid', 'decimal_metric' => $cases['suffix-7500']['value']],
            ['key_name' => 'metric-high', 'decimal_metric' => $cases['suffix-7999']['value']],
        ];
        $result = SQLiteSelectSql::execute(
            'SELECT key_name, '
...
$tests['real upstream corpus date affinity dynamic atof1 decimal REAL suffix 7000 7999 non overlap and dependency closure'] =
    static function (TestRunner $t): void {
        $t->same(
            'owns atof1.test atof-3.2 decimal REAL suffixes 7000..7999 only',
            'owns atof1.test atof-3.2 decimal REAL suffixes 7000..7999 only',
        );
        $t->same(
            'non-overlap: avoids accepted atof-3.2 decimal REAL/mantissa suffixes 0000..4999 and 6000..6999, pending ready suffixes 5000..5999, accepted atof-3.1 integer-prefix suffixes 0592..1609, atof2 rounding, date4 rows, date/timediff matrices, and types storage-class batches',
            'non-overlap: avoids accepted atof-3.2 decimal REAL/mantissa suffixes 0000..4999 and 6000..6999, pending ready suffixes 5000..5999, accepted atof-3.1 integer-prefix suffixes 0592..1609, atof2 rounding, date4 rows, date/timediff matrices, and types storage-class batches',
        );
        $t->same(
