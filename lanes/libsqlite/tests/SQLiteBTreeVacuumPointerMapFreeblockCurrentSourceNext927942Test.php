...
$makeFirstPage927942 = static function (int $pageCount): string {
...
$putPointerMapEntry927942 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database927942 = static function (int $sliceNumber) use ($makeFirstPage927942, $putPointerMapEntry927942): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage927942(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry927942($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan927942 = static function (int $sliceNumber, int $batchSize = 2) use ($database927942): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database927942($sliceNumber);
...
foreach (range(927, 942) as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} extends next927-942 handoff receipts"] = static function (TestRunner $t) use ($plan927942, $sliceNumber): void {
        $plan = $plan927942($sliceNumber);
        ...
        $t->same(6, $plan927942($sliceNumber, 3)->currentSourceSummary()['current_source_row_count']);
    };
}
