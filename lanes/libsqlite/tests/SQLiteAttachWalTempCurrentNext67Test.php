...
$cases = [
    ...
    'operation name' => ['operation', 'attach-wal-temp-statement-lifecycle-current-next67'],
    ...
    'dependency slice marker' => ['dependencies.0', 'sqlite-attach-wal-temp-current-next67'],
    'dependency schema cookie expiry' => ['dependencies.1', 'sqlite-schema-cookie-expire-prepared-statements'],
    'dependency wal ddl lifecycle' => ['dependencies.2', 'sqlite-wal-ddl-current-next-statement-lifecycle'],
...
foreach ($cases as $name => [$path, $expected]) {
    $tests['attach wal temp current next67 ' . $name] = static function (TestRunner $t) use ($plan, $value, $path, $expected): void {
...
foreach ($predicateCases as $name => $callback) {
    $tests['attach wal temp current next67 ' . $name] = static function (TestRunner $t) use ($callback): void {
        $t->true($callback());
    };
}
