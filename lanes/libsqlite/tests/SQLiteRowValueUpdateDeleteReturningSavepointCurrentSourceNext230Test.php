...
$preSql230 = "UPDATE wp_options SET (status, option_value, bytes) = ('pre230', option_value || ':pre230', bytes + 1) WHERE (blog_id, option_name) IN (VALUES (1, 'siteurl'), (1, 'home')) RETURNING option_id, blog_id, option_name, status, option_value, bytes ORDER BY option_id";
$innerUpdateSql230 = "UPDATE wp_options SET (status, option_value, bytes) = ('inner230', option_value || ':inner230', bytes + 4) WHERE (option_id, option_name) IN (SELECT meta_option_id, meta_value FROM wp_optionmeta WHERE meta_key = 'migration_batch' ORDER BY meta_id DESC LIMIT 2) RETURNING option_id, blog_id, option_name, status, option_value, (status, option_name) IS ('inner230', 'pending_theme') AS inner_pending ORDER BY option_id";
...
$cases230 = [
    ...
    'pre row one value' => [static fn (): mixed => array_column($preResult230()['tables']['wp_options'], 'option_value', 'option_id')[1], 'https://one.test:pre230'],
    ...
    'retry update starts from original row seven' => [static fn (): mixed => $retryUpdateResult230()['returning'][0]['option_value'], 'theme:retry230'],
    ...
    'plan status' => [static fn (): mixed => $plan230()['status'], 'rowvalue-update-delete-returning-nested-savepoint-current-source-next230'],
    'plan savepoints' => [static fn (): mixed => [$plan230()['outer_savepoint'], $plan230()['inner_savepoint']], ['wp_options_rowvalue_outer_next230', 'wp_options_rowvalue_inner_next230']],
    ...
    'plan pre image row one' => [static fn (): mixed => array_column($plan230()['outer_savepoint_image_tables']['wp_options'], 'status', 'option_id')[1], 'pre230'],
    'plan inner released row seven' => [static fn (): mixed => array_column($plan230()['inner_released_current_source_tables']['wp_options'], 'status', 'option_id')[7], 'inner230'],
    ...
    'plan final row seven retry' => [static fn (): mixed => array_column($plan230()['current_source_tables']['wp_options'], 'option_value', 'option_id')[7], 'theme:retry230'],
    'plan final row eight retry' => [static fn (): mixed => array_column($plan230()['current_source_tables']['wp_options'], 'option_value', 'option_id')[8], 'rules:retry230'],
    ...
    'plan statement phases' => [static fn (): mixed => [array_column($plan230()['inner_statements'], 'phase'), array_column($plan230()['after_release_statements'], 'phase'), array_column($plan230()['retry_statements'], 'phase')], [['inner-before-release-next230', 'inner-before-release-next230'], ['after-inner-release-next230'], ['retry-after-outer-rollback-next230', 'retry-after-outer-rollback-next230']]],
    ...
    'plan dependency nested release' => [static fn (): mixed => in_array('sqlite-nested-savepoint-release-returning-discarded-by-outer-rollback-next230', $plan230()['dependencies'], true), true],
    'plan dependency wordpress current source' => [static fn (): mixed => in_array('wordpress-rowvalue-nested-savepoint-current-source-next230', $plan230()['dependencies'], true), true],
    'plan non overlap mentions next212' => [static fn (): mixed => str_contains($plan230()['non_overlap_next230'], 'next212'), true],
    'plan dependency closure says no new support' => [static fn (): mixed => str_contains($plan230()['dependency_closure_next230'], 'no new support component needed'), true],
...
foreach ($cases230 as $name => [$callback, $expected]) {
    $tests['rowvalue update delete returning savepoint current source next230 ' . $name] = static function (TestRunner $t) use ($callback, $expected): void {
