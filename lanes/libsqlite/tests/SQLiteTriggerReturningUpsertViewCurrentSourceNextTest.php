...
$rows149 = [
    ...
    ['option_id' => 3, 'option_name' => 'blogname', 'option_value' => 'Old Blog', 'autoload' => 'no', 'revision' => 1, 'source' => 'seed'],
];
...
$returning149 = [
    ...
    static fn (array $new, ?array $old, array $incoming, string $event, int $ordinal, string $source): string => $source . ':' . $event . ':' . $ordinal . ':' . ($old['option_name'] ?? 'new') . '>' . $new['option_name'],
];
$currentInput149 = [
    ['import_id' => 11, 'name' => 'siteurl', 'value' => 'https://current.test', 'autoload_flag' => 'yes'],
    ['import_id' => 12, 'name' => 'skip_current', 'value' => 'blocked', 'autoload_flag' => 'no', '_raise_ignore' => true],
...
$nextInput149 = [
    ...
    ['import_id' => 22, 'name' => 'blogname', 'value' => 'blocked next', 'autoload_flag' => 'no', 'origin' => 'next-import', '_raise_ignore' => true],
...
$plan149 = static fn (array $options = [], ?array $currentInput = null, ?array $nextInput = null, ?array $currentView = null, ?array $nextView = null, ?array $returning = null): array => SQLiteTriggerReturningUpsertViewCurrentSourceNextPlan::execute(
    ...
    $options + ['key' => 'option_name', 'savepoint' => 'wp_import_view_149'],
);
...
$cases149 = [
    ...
    'pinned savepoint' => [static fn (): mixed => $pinned149()['savepoint'], 'wp_import_view_149'],
    'pinned key' => [static fn (): mixed => $pinned149()['key'], 'option_name'],
    ...
    'pinned current columns' => [static fn (): mixed => $pinned149()['current_view']['columns'], ['import_id', 'name', 'value', 'autoload_flag']],
    'pinned next columns include origin' => [static fn (): mixed => $pinned149()['next_view']['columns'], ['import_id', 'name', 'value', 'autoload_flag', 'origin']],
    'pinned current mapping name' => [static fn (): mixed => $pinned149()['current_view']['mapping']['name'], 'option_name'],
    ...
    'pinned current returning names' => [static fn (): mixed => array_column(array_column($pinned149()['current_returning_rows'], 'returning'), 'name'), ['siteurl', 'fresh_plugin']],
    ...
    'pinned callable traces' => [static fn (): mixed => array_column(array_column($pinned149()['current_returning_rows'], 'returning'), 'expr7'), ['main@trigger-cookie-149-current:update:0:siteurl>siteurl', 'main@trigger-cookie-149-current:insert:2:new>fresh_plugin']],
    ...
    'pinned current skipped incoming name' => [static fn (): mixed => $pinned149()['current_skipped_rows'][0]['incoming_row']['option_name'], 'skip_current'],
    ...
    'pinned current rows include fresh plugin' => [static fn (): mixed => array_column($pinned149()['current_rows'], 'option_name'), ['siteurl', 'home', 'blogname', 'fresh_plugin']],
    'pinned current siteurl updated' => [static fn (): mixed => $pinned149()['current_rows'][0]['option_value'], 'https://current.test'],
    'pinned after savepoint restores base' => [static fn (): mixed => array_column($pinned149()['after_savepoint'], 'option_name'), ['siteurl', 'home', 'blogname']],
    'pinned after savepoint restores value' => [static fn (): mixed => $pinned149()['after_savepoint'][0]['option_value'], 'https://old.test'],
    'pinned next returning suppressed' => [static fn (): mixed => $pinned149()['next_returning_rows'], []],
    'pinned attempted next returning names' => [static fn (): mixed => array_column(array_column($pinned149()['attempted_next_returning_rows'], 'returning'), 'name'), ['home', 'rewrite_rules']],
    ...
    'pinned attempted next skipped name' => [static fn (): mixed => $pinned149()['attempted_next_skipped_rows'][0]['incoming_row']['option_name'], 'blogname'],
    ...
    'admitted next returning names' => [static fn (): mixed => array_column(array_column($admitted149()['next_returning_rows'], 'returning'), 'name'), ['home', 'rewrite_rules']],
    'admitted final names' => [static fn (): mixed => array_column($admitted149()['after_savepoint'], 'option_name'), ['siteurl', 'home', 'blogname', 'fresh_plugin', 'rewrite_rules']],
    ...
    'admitted home updated by next source' => [static fn (): mixed => $admitted149()['after_savepoint'][1]['option_value'], 'https://next-home.test'],
    'admitted skipped blogname preserved' => [static fn (): mixed => $admitted149()['after_savepoint'][2]['option_value'], 'Old Blog'],
    'admitted next trigger effects visible' => [static fn (): mixed => array_column($admitted149()['next_trigger_effects'], 'option_name'), ['home', 'rewrite_rules']],
    ...
    'custom savepoint accepted' => [static fn (): mixed => $plan149(['savepoint' => 'wp_custom_view_149'])['savepoint'], 'wp_custom_view_149'],
