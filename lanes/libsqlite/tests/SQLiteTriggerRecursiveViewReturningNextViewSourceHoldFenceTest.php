...
$nextView174['where'] = static fn (array $row, string $root, int $depth): bool => $depth <= 3 && str_contains((string) $row['option_name'], '_');
...
$tokenHeld174 = static fn (): array => $run174(['expected_reprepare_token' => 'wp.reprepare.174.expected']);
$noConflict174 = static function () use ($run174, $currentView174): array {
    ...
    return $run174([
        'first_next_generation' => 'wp-import-next-174-first',
        'second_next_generation' => 'wp-import-next-174-second',
        'current_schema_cookie' => 174,
...
$cases174 = [
    ...
    'savepoint retained' => [static fn (): mixed => $conflict174()['savepoint'], 'wp_recursive_view_next174'],
    'cursor retained' => [static fn (): mixed => $conflict174()['cursor'], 'wp_recursive_view_returning_cursor_174'],
    ...
    'conflict keys' => [static fn (): mixed => $conflict174()['conflict_keys'], ['theme_child', 'plugin_child']],
    'watermark current keys' => [static fn (): mixed => $conflict174()['current_source_watermark']['current_keys'], ['theme_root', 'plugin_root', 'theme_child', 'plugin_child']],
    'watermark staged conflict keys' => [static fn (): mixed => $conflict174()['current_source_watermark']['staged_conflict_keys'], ['theme_child', 'plugin_child']],
    ...
    'visible keys conflict names' => [static fn (): mixed => array_map(static fn (string $key): string => substr($key, strrpos($key, ':') + 1), $conflict174()['visible_keys']), ['theme_root', 'plugin_root', 'theme_child', 'plugin_child', 'network_root', 'network_child']],
    'held keys conflict names' => [static fn (): mixed => array_map(static fn (string $key): string => substr($key, strrpos($key, ':') + 1), $conflict174()['held_keys']), ['theme_child', 'plugin_child']],
    'held keys token names' => [static fn (): mixed => array_map(static fn (string $key): string => substr($key, strrpos($key, ':') + 1), $tokenHeld174()['held_keys']), ['theme_child', 'plugin_child', 'network_root', 'network_child']],
    ...
    'network rows not conflicting' => [static fn (): mixed => array_column(array_slice($conflict174()['watermark_steps'], 6), 'conflicts_with_current_key'), [false, false]],
    'network rows visible' => [static fn (): mixed => array_column(array_slice($conflict174()['watermark_steps'], 6), 'visible_after_watermark'), [true, true]],
