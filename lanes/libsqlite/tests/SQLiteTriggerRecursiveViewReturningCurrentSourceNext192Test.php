...
$plan192 = static fn (array $options = []): array => SQLiteTriggerRecursiveViewReturningCurrentSourceNext192Plan::execute(
    $rows192,
    $currentInput192,
    $nextInput192,
    $currentView192,
    ...
    $options + [
        ...
        'current_generation' => 'wp-current-returning-192',
        'next_generation' => 'wp-next-returning-192',
        'page_size' => 3,
        'admit_next_source' => true,
        'current_source_token' => 'wp.current.source.192',
        'drain_ack_token' => 'wp.returning.drain.192',
        ...
        'following_generation' => 'wp-following-current-192',
    ],
);
...
$cursorHeld192 = static fn (): array => $plan192(['close_next_cursor' => 'wp.returning.next.cursor.expected.192']);
$tokenHeld192 = static fn (): array => $plan192(['expected_following_current_source_token' => 'wp.current.source.expected.192']);
$nextHeld192 = static fn (): array => $plan192(['fresh_acknowledged_ordinals' => [0]]);
$custom192 = static fn (): array => $plan192([
    'following_current_source_token' => 'wp.current.source.following.custom.192',
    'expected_following_current_source_token' => 'wp.current.source.following.custom.192',
    'following_cursor' => 'wp.returning.following.custom.cursor.192',
]);
...
$cases192 = [
    ...
    'savepoint retained' => [static fn (): mixed => $admitted192()['savepoint'], 'wp_recursive_view_192'],
    ...
    'next cursor retained' => [static fn (): mixed => $admitted192()['next_cursor_next192'], 'wp.returning.next.cursor.192'],
    'close cursor retained' => [static fn (): mixed => $admitted192()['close_next_cursor_next192'], 'wp.returning.next.cursor.192'],
    ...
    'following token retained' => [static fn (): mixed => $admitted192()['following_current_source_token_next192'], 'wp.current.source.following.192'],
    'expected following token retained' => [static fn (): mixed => $admitted192()['expected_following_current_source_token_next192'], 'wp.current.source.following.192'],
    ...
    'following cursor retained' => [static fn (): mixed => $admitted192()['following_cursor_next192'], 'wp.returning.following.cursor.192'],
    'custom following cursor retained' => [static fn (): mixed => $custom192()['following_cursor_next192'], 'wp.returning.following.custom.cursor.192'],
    ...
    'following payload names' => [static fn (): mixed => array_column($admitted192()['following_current_payloads_next192'], 'name'), ['blogdescription', 'stylesheet']],
    'following payload values' => [static fn (): mixed => array_column($admitted192()['following_current_payloads_next192'], 'value'), ['after-next', 'twentytwentyfive']],
    ...
    'following token stamped' => [static fn (): mixed => array_values(array_unique(array_column($admitted192()['following_current_rows_next192'], 'following_current_source_token_next192'))), ['wp.current.source.following.192']],
    'following cursor stamped' => [static fn (): mixed => array_values(array_unique(array_column($admitted192()['following_current_rows_next192'], 'following_cursor_next192'))), ['wp.returning.following.cursor.192']],
    'following generation stamped' => [static fn (): mixed => array_values(array_unique(array_column($admitted192()['following_current_rows_next192'], 'following_generation_next192'))), ['wp-following-current-192']],
    'following option names stamped' => [static fn (): mixed => array_column($admitted192()['following_current_rows_next192'], 'returning_option_name'), ['blogdescription', 'stylesheet']],
    ...
    'close plan following cursor' => [static fn (): mixed => $admitted192()['cursor_close_plan_next192']['following_cursor'], 'wp.returning.following.cursor.192'],
