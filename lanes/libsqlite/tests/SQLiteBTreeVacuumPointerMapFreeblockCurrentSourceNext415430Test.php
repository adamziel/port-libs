...
$makeFirstPage415430 = static function (int $pageCount): string {
...
$putPointerMapEntry415430 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database415430 = static function (int $sliceNumber) use ($makeFirstPage415430, $putPointerMapEntry415430): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage415430(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry415430($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan415430 = static function (int $sliceNumber, int $batchSize = 2) use ($database415430): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database415430($sliceNumber);
...
foreach ([415, 416, 417, 418, 419, 420, 421, 422, 423, 424, 425, 426, 427, 428, 429, 430] as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} keeps next415-430 freelist splice ready"] = static function (TestRunner $t) use ($plan415430, $sliceNumber): void {
        $plan = $plan415430($sliceNumber);
        ...
        $t->same(6, $plan415430($sliceNumber, 3)->freelistSummary()['freelist_row_count']);
    };
}
