...
$summary218 = static fn (?array $cursor = null): array => SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext218Plan::compare($sql218, $currentTables218, $nextTables218, $cursor);
$tests = [];
...
$tests['compound select window recursive limit current source next218 rejects missing intersect'] = static function (TestRunner $t) use ($currentTables218): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext218Plan::compare(
...
foreach (range(1, 52) as $case) {
    $tests['compound select window recursive limit current source next218 generated rank shift ' . $case] = static function (TestRunner $t) use ($case): void {
        ...
        $plan = SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext218Plan::compare($sql, $tables, $nextTables);
        $again = SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext218Plan::compare($sql, $tables, $nextTables, $plan['cursor']);
