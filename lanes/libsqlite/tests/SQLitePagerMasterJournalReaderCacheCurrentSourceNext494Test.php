...
$pageSize = 512;
$database = '/srv/wp-content/database/wp-next494.sqlite';
...
$formatPage = static function (string $label) use ($pageSize): string {
    ...
    $page = substr_replace($page, pack('N', 494), 60, 4);
...
$tokenFields = [
    ...
    'reader_cache_stmt_vdbe_compare_token',
];
$before = [1 => $formatPage('stale schema'), 2 => $page('stale options')];
$recovered = [1 => $formatPage('current schema'), 2 => $page('current options')];
$tokens = [$journal => 'member-main-current-494'];
$headers = [$journal => hash('sha256', 'main header next494')];
$base = [
    'source_id' => 'pager-reader-cache-current-source-next494',
    'epoch' => 494,
    'format_signature' => hash('sha256', implode('|', [512, 4, 2, 494, 0])),
    'publication_generation' => 494,
    'master_source_digest' => hash('sha256', 'master-next494'),
    'recovery_sequence' => 494,
...
foreach ($tokenFields as $field) {
    $base[$field] = str_replace('_', '-', preg_replace('/_token$/', '', $field)) . '-current-494';
}
...
$plan = static fn (array $cacheExtra = [], array $readExtra = []): array => SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::readerCacheStatementFence(494,
    $database,
    $master,
    $masterBytes,
    implode('', $before),
...
$tests['pager master journal reader cache current source next494 admits current VDBE expression fences'] = static function (TestRunner $t) use ($plan): void {
    $result = $plan();
    $t->same('pager-master-journal-reader-cache-current-source-next494', $result['status']);
    $t->same([], $result['invalidated_cache_page_numbers']);
    $t->same(['read-options' => true], $result['read_cache_hits']);
    $t->same('reader-cache-stmt-vdbe-compare-current-494', $result['current_reader_cache_stmt_vdbe_compare_token']);
};

$tests['pager master journal reader cache current source plan dispatches full width calls to next494'] = static function (TestRunner $t) use ($database, $master, $masterBytes, $before, $pageSize, $recovered, $cacheEntry, $read, $base, $tokens, $headers, $tokenFields): void {
...
$tests['pager master journal reader cache current source next494 invalidates stale compare cache'] = static function (TestRunner $t) use ($plan, $opCount): void {
    $result = $plan(['reader_cache_stmt_vdbe_compare_token' => 'stmt-vdbe-compare-old']);
    $t->same([1], $result['reader_cache_stmt_vdbe_compare_invalidated_cache_page_numbers']);
    $t->same([1], $result['invalidated_cache_page_numbers']);
    $t->same(1, $opCount($result, 'invalidate_reader_cache_reader_cache_stmt_vdbe_compare_current_source_next494'));
};

$tests['pager master journal reader cache current source next494 reopens stale compare read ticket'] = static function (TestRunner $t) use ($plan, $opCount): void {
    $result = $plan([], ['reader_cache_stmt_vdbe_compare_token' => 'stmt-vdbe-compare-old']);
    $t->same(['read-options'], $result['reopen_reader_ids']);
    $t->same(false, $result['next_reads'][0]['cache_hit']);
    $t->same('reader_ticket_reader_cache_stmt_vdbe_compare_predates_current_source', $result['next_reads'][0]['reader_cache_stmt_vdbe_compare_token_reason']);
    $t->same(1, $opCount($result, 'reopen_reader_for_reader_cache_stmt_vdbe_compare_current_source_next494'));
};

$tests['pager master journal reader cache current source next494 missing compare token rejects'] = static function (TestRunner $t) use ($plan): void {
    $t->throws(Throwable::class, static fn () => $plan(['reader_cache_stmt_vdbe_compare_token' => null]));
};
