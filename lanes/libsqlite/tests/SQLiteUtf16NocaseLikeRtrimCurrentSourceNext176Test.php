...
$plan176 = static fn (
    ...
    int $nextCookie = 176,
): array => SQLiteUtf16NocaseLikeRtrimCurrentSourceNextPlan::keyValueRowKeyPeerYieldPlan(
    $current ?? $current176,
...
$cases176 = [
    ...
    'current source' => ['currentSource', 'main.app_settings@175'],
    'next source' => ['nextSource', 'main.app_settings@176'],
...
$tests['utf16 nocase like rtrim current source nextOneSevenSix stable peer rows can continue'] = static function (TestRunner $t) use ($row176): void {
    ...
    $result = SQLiteUtf16NocaseLikeRtrimCurrentSourceNextPlan::keyValueRowKeyPeerYieldPlan($current, $next, 'plugin!_cache%', '!', ['key' => 'plugin_cache', 'rowid' => 2], 2, 'stable', 'stable', 8, 8);
...
$tests['utf16 nocase like rtrim current source nextOneSevenSix yield page can straddle a duplicate peer'] = static function (TestRunner $t) use ($row176): void {
    ...
    $result = SQLiteUtf16NocaseLikeRtrimCurrentSourceNextPlan::keyValueRowKeyPeerYieldPlan($rows, $rows, 'plugin!_cache%', '!', null, 2, 'stable', 'stable', 9, 9);
...
$tests['utf16 nocase like rtrim current source nextOneSevenSix unicode prefix disables range'] = static function (TestRunner $t) use ($row176): void {
    ...
    $result = SQLiteUtf16NocaseLikeRtrimCurrentSourceNextPlan::keyValueRowKeyPeerYieldPlan($rows, $rows, 'éclair%', null, null, 5, 'stable', 'stable', 10, 10);
