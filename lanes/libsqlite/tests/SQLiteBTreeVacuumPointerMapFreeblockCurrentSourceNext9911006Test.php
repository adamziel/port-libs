...
$makeFirstPage9911006 = static function (int $pageCount): string {
...
$putPointerMapEntry9911006 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database9911006 = static function (int $sliceNumber) use ($makeFirstPage9911006, $putPointerMapEntry9911006): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage9911006(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry9911006($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan9911006 = static function (int $sliceNumber, int $batchSize = 2) use ($database9911006): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database9911006($sliceNumber);
...
foreach (range(991, 1006) as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} extends next991-1006 handoff receipts"] = static function (TestRunner $t) use ($plan9911006, $sliceNumber): void {
        $plan = $plan9911006($sliceNumber);
        ...
        $t->same(6, $plan9911006($sliceNumber, 3)->currentSourceSummary()['current_source_row_count']);
    };
}
