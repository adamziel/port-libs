...
$makeFirstPage591606 = static function (int $pageCount): string {
...
$putPointerMapEntry591606 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database591606 = static function (int $sliceNumber) use ($makeFirstPage591606, $putPointerMapEntry591606): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage591606(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry591606($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan591606 = static function (int $sliceNumber, int $batchSize = 2) use ($database591606): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database591606($sliceNumber);
...
foreach ([591, 592, 593, 594, 595, 596, 597, 598, 599, 600, 601, 602, 603, 604, 605, 606] as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} publishes next591-606 handoff receipts"] = static function (TestRunner $t) use ($plan591606, $sliceNumber): void {
        $plan = $plan591606($sliceNumber);
        ...
        $t->same(6, $plan591606($sliceNumber, 3)->currentSourceSummary()['current_source_row_count']);
    };
}
