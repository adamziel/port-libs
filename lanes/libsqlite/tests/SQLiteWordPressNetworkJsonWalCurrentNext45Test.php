...
$plan = static fn (array $imports, array $options = []) => SQLiteWordPressNetworkJsonWalCurrentNextPlan::plan(
    $currentRows(),
    $imports,
...
$tests = [
    ...
    'accepts JSON subtype network payloads' => static function (TestRunner $t) use ($currentRows): void {
        $result = SQLiteWordPressNetworkJsonWalCurrentNextPlan::plan($currentRows(), [
...
$tests['rejects empty imports'] = static function (TestRunner $t) use ($currentRows): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteWordPressNetworkJsonWalCurrentNextPlan::plan($currentRows(), []));
};
