...
$plan446461 = static fn (?array $rows = null, ?array $samples = null): array => SQLitePlannerStat4ExpressionPartialCurrentSourceNextPlan::materializeStat4ExpressionPartialPreparedBridgeCurrentSourceHandoff(
    $prepared446461(),
...
$tests = [
    ...
    'planner stat4 expression partial current source next446461 cursor appended' => static fn (TestRunner $t) => $t->same('PrepareStat4ExpressionPartialPreparedHandoffRange', $plan446461()['cursorProgram'][array_key_last($plan446461()['cursorProgram'])]['opcode']),
    'planner stat4 expression partial current source next446461 cursor mode' => static fn (TestRunner $t) => $t->same('prepared-handoff-range-current-source-stat4-expression-partial-prep', $plan446461()['cursorProgram'][array_key_last($plan446461()['cursorProgram'])]['mode']),
    ...
    'planner stat4 expression partial current source next446461 malformed needed column' => static function (TestRunner $t) use ($prepared446461, $current446461, $terms446461): void {
        $t->throws(InvalidArgumentException::class, static fn () => SQLitePlannerStat4ExpressionPartialCurrentSourceNextPlan::materializeStat4ExpressionPartialPreparedBridgeCurrentSourceHandoff($prepared446461(), $current446461(), $terms446461(), ['option_name', ''], 6));
    },
];
