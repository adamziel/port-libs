...
$plan = static fn (
    ...
    int $nextCookie = 108,
): array => SQLiteEncodingNumericAffinityCurrentSourceNextPlan::wordpressOptionValueComparisonPlan(
    $currentRows,
    $nextRows,
    'option_value',
    $probe,
...
$tests['encoding numeric affinity current source next107 stable source reusable for same bytes'] = static function (TestRunner $t) use ($currentRows): void {
    $stable = SQLiteEncodingNumericAffinityCurrentSourceNextPlan::wordpressOptionValueComparisonPlan(
        $currentRows,
        $currentRows,
        'option_value',
        10,
...
$tests['encoding numeric affinity current source next107 rejects missing column'] = static function (TestRunner $t) use ($nextRows): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteEncodingNumericAffinityCurrentSourceNextPlan::wordpressOptionValueComparisonPlan([['option_id' => 1]], $nextRows, 'option_value', 10));
};
...
$tests['encoding numeric affinity current source next107 rejects unsupported operator'] = static function (TestRunner $t) use ($currentRows, $nextRows): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteEncodingNumericAffinityCurrentSourceNextPlan::wordpressOptionValueComparisonPlan($currentRows, $nextRows, 'option_value', 10, 'LIKE'));
};
...
$tests['encoding numeric affinity current source next107 rejects unsupported collation'] = static function (TestRunner $t) use ($currentRows, $nextRows): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteEncodingNumericAffinityCurrentSourceNextPlan::wordpressOptionValueComparisonPlan($currentRows, $nextRows, 'option_value', 10, '=', 'NUMERIC', 'NONE', 'UNICODE'));
};
