...
$pageSize = 512;
$database = '/srv/wp-content/database/wp-mmap-size-fence.sqlite';
$usersDatabase = '/srv/wp-content/database/wp-mmap-size-fence-users.sqlite';
...
$before = [1 => $formatPage('mmap-size-fence stale schema'), 2 => $page('mmap-size-fence stale options'), 3 => $page('mmap-size-fence stale plugins'), 4 => $page('mmap-size-fence stale users')];
$recovered = [1 => $formatPage('mmap-size-fence current schema'), 2 => $page('mmap-size-fence current options'), 3 => $page('mmap-size-fence current plugins'), 4 => $page('mmap-size-fence current users')];
...
$base = [
    'source_id' => 'pager-reader-cache-mmap-size-fence',
    'epoch' => 302,
    'format_signature' => hash('sha256', implode('|', [512, 4, 2, 302, 0])),
    'publication_generation' => 302,
    'master_source_digest' => hash('sha256', 'master-mmap-size-fence'),
...
$plan = static function (?array $cache = null, ?array $reads = null) use ($database, $master, $masterBytes, $before, $pageSize, $recovered, $base, $tokens, $headers, $cacheEntry, $read): array {
    return SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::currentSourceReaderCacheMmapSizeFence($database, $master, $masterBytes, implode('', $before), $pageSize, $recovered, $cache ?? [
...
$cases = [
    'status' => [static fn (): mixed => $plan()['status'], 'pager-master-journal-reader-cache-current-source-mmap-size-fence'],
    ...
    'next291 transaction-state fence is current' => [static fn (): mixed => $plan()['current_reader_cache_transaction_state_token'], 'transaction-state-current-302'],
    'next292 commit-phase fence is current' => [static fn (): mixed => $plan()['current_reader_cache_commit_phase_token'], 'commit-phase-current-302'],
    'next293 busy-handler fence is current' => [static fn (): mixed => $plan()['current_reader_cache_busy_handler_token'], 'busy-handler-current-302'],
    ...
    'mmap-size-fence mmap-size ticket reopens reader' => [static fn (): mixed => $plan()['next_reads'][3]['reader_cache_mmap_size_token_reason'], 'reader_ticket_reader_cache_mmap_size_predates_current_source'],
    ...
    'dependencies include mmap-size-fence' => [static fn (): mixed => in_array('sqlite-pager-master-journal-reader-cache-current-source-mmap-size-fence', $plan()['dependencies'], true), true],
    ...
    'default plan points at mmap-size-fence' => [static fn (): mixed => $plan()['status'], 'pager-master-journal-reader-cache-current-source-mmap-size-fence'],
];

foreach ($cases as $name => [$callback, $expected]) {
    $tests['pager master journal reader cache mmap size fence ' . $name] = static function (TestRunner $t) use ($callback, $expected): void {
        $t->same($expected, $callback());
    };
}

$tests['pager master journal reader cache mmap size fence missing token rejects'] = static function (TestRunner $t) use ($plan, $cacheEntry, $recovered): void {
    $t->throws(Throwable::class, static fn () => $plan([1 => array_diff_key($cacheEntry('missing', $recovered[1]), ['reader_cache_mmap_size_token' => true])]));
};
