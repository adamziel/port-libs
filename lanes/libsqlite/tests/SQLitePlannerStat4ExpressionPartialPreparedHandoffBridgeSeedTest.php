...
$plan334349 = static fn (?array $rows = null, ?array $samples = null): array => SQLitePlannerStat4ExpressionPartialCurrentSourceNextPlan::materializePreparedHandoffBridgeSeed(
    $prepared334349(),
...
$tests = [
    ...
    'planner stat4 expression partial current source next334349 cursor appended' => static fn (TestRunner $t) => $t->same('PrepareStat4ExpressionPartialPreparedHandoffRange', $plan334349()['cursorProgram'][array_key_last($plan334349()['cursorProgram'])]['opcode']),
    'planner stat4 expression partial current source next334349 cursor mode' => static fn (TestRunner $t) => $t->same('prepared-handoff-range-current-source-stat4-expression-partial-prep', $plan334349()['cursorProgram'][array_key_last($plan334349()['cursorProgram'])]['mode']),
    ...
    'planner stat4 expression partial current source next334349 malformed needed column' => static function (TestRunner $t) use ($prepared334349, $current334349, $terms334349): void {
        $t->throws(InvalidArgumentException::class, static fn () => SQLitePlannerStat4ExpressionPartialCurrentSourceNextPlan::materializePreparedHandoffBridgeSeed($prepared334349(), $current334349(), $terms334349(), ['option_name', ''], 6));
    },
];
