...
$makeFirstPage359366 = static function (int $pageCount): string {
...
$putPointerMapEntry359366 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database359366 = static function (int $sliceNumber) use ($makeFirstPage359366, $putPointerMapEntry359366): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage359366(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry359366($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan359366 = static function (int $sliceNumber, int $batchSize = 2) use ($database359366): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database359366($sliceNumber);
...
foreach ([359, 360, 361, 362, 363, 364, 365, 366] as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} keeps next359-366 freelist splice ready"] = static function (TestRunner $t) use ($plan359366, $sliceNumber): void {
        $plan = $plan359366($sliceNumber);
        ...
        $t->same(6, $plan359366($sliceNumber, 3)->freelistSummary()['freelist_row_count']);
    };
}
