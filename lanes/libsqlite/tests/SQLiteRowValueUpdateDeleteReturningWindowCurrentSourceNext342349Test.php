...
$cases = [
    'combined next342-349 current-source follow-on seal' => static function (TestRunner $t) use ($examplesDir): void {
        $result = require $examplesDir . '/wordpress-rowvalue-returning-window-current-source-next342-349.php';

        $t->same('rowvalue-update-delete-returning-window-current-source-next342-349', $result['status']);
        $t->same([
...
foreach ($cases as $name => $callback) {
    $tests['rowvalue update delete returning window current source next342-349 ' . $name] = $callback;
}
