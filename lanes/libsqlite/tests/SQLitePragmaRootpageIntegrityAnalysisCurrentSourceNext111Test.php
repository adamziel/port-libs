...
$valid = SQLitePragmaRootpageIntegrityAnalysisCurrentSourceNext111::analyze($validDatabase);
...
$duplicate = SQLitePragmaRootpageIntegrityAnalysisCurrentSourceNext111::analyze($duplicateDatabase);
...
$freelist = SQLitePragmaRootpageIntegrityAnalysisCurrentSourceNext111::analyze($freelistDatabase);
...
$wrongType = SQLitePragmaRootpageIntegrityAnalysisCurrentSourceNext111::analyze($wrongTypeDatabase);
...
$pointerMap = SQLitePragmaRootpageIntegrityAnalysisCurrentSourceNext111::analyze($pointerMapDatabase);
...
$largestRoot = SQLitePragmaRootpageIntegrityAnalysisCurrentSourceNext111::analyze($largestRootDatabase);
...
$beyond = SQLitePragmaRootpageIntegrityAnalysisCurrentSourceNext111::analyze($beyondDatabase);
...
foreach (range(1, 28) as $index) {
    $tests['pragma rootpage integrity analysis current source next111 repeated valid wp option case ' . $index] = static function (TestRunner $t) use ($schemaDatabase, $rows, $index): void {
        ...
        $result = SQLitePragmaRootpageIntegrityAnalysisCurrentSourceNext111::analyze($database);
