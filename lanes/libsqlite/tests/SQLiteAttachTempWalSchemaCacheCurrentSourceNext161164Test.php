...
$plan161164 = static fn (array $events, ?array $statements = null, ?array $schemas = null): array => SQLiteAttachWalTempSchemaCacheCurrentSourceNext92Plan::currentSourceNext161164(
...
$tests['attach temp wal schema cache current source next161 temp options shadow wins unqualified reader'] = static function (TestRunner $t) use ($plan161164): void {
    ...
    $t->same('attach-wal-temp-schema-cache-current-source-next161-164', $result['operation']);
    $t->same('sqlite-attach-temp-wal-schema-cache-current-source-next161', $result['dependencies'][0]);
