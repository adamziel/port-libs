...
$plan94 = static fn (array $current = null, array $next = null, array $constraints = null, string $function = 'json_tree'): array => SQLiteJsonTablePlan::currentSourceHiddenRowidPlannerNext94(
    $function,
...
$tests = [
    ...
    'dependency marker is present' => static fn (TestRunner $t) => $t->same(true, in_array('sqlite-json-table-hidden-rowid-source-current-next94', $plan94()['dependencies'], true)),
    ...
    'stable plan still records rowid residual presence' => static fn (TestRunner $t) => $t->same(['hidden-rowid-residual-constraint-present'], $stable94()['next94ReplanReasons']),
    'changed plan records source json change' => static fn (TestRunner $t) => $t->same(true, in_array('source-json-changed', $plan94()['next94ReplanReasons'], true)),
    'changed plan records payload change' => static fn (TestRunner $t) => $t->same(true, in_array('hidden-rowid-source-payload-changed', $plan94()['next94ReplanReasons'], true)),
    ...
    'root shift records root change' => static fn (TestRunner $t) => $t->same(true, in_array('source-root-changed', $rootShift94()['next94ReplanReasons'], true)),
    ...
    'empty json column is rejected' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteJsonTablePlan::currentSourceHiddenRowidPlannerNext94('json_tree', $currentSource94, $nextSource94, '', [])),
    'empty root column is rejected' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteJsonTablePlan::currentSourceHiddenRowidPlannerNext94('json_tree', $currentSource94, $nextSource94, 'option_value', [], '')),
    'missing current json column is rejected' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteJsonTablePlan::currentSourceHiddenRowidPlannerNext94('json_tree', ['scan_root' => '$'], $nextSource94, 'option_value')),
    'missing next root column is rejected' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteJsonTablePlan::currentSourceHiddenRowidPlannerNext94('json_tree', $currentSource94, ['option_value' => '{}'], 'option_value', [], 'scan_root')),
    'bad function is rejected' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteJsonTablePlan::currentSourceHiddenRowidPlannerNext94('json_bad', $currentSource94, $nextSource94, 'option_value')),
];

foreach ($tests as $name => $case) {
    $tests['json table hidden rowid source current next94 ' . $name] = $case;
    unset($tests[$name]);
}
