...
$pageSize = 512;
$database = '/srv/wp-content/database/wp-next270.sqlite';
$usersDatabase = '/srv/wp-content/database/wp-next270-users.sqlite';
$mainJournal = $database . '-journal';
$usersJournal = $usersDatabase . '-journal';
$master = $database . '-mj';
$sourceId = 'pager-reader-cache-rollback-source-next270';
...
$formatPage = static function (string $label) use ($pageSize): string {
    ...
    $page = substr_replace($page, pack('N', 270), 60, 4);
    $page = substr_replace($page, pack('N', 0x57503270), 68, 4);
...
$before = [
    1 => $formatPage('next270 stale schema before rollback-source fence'),
    2 => $page('next270 stale wp_options root before rollback-source fence'),
    3 => $page('next270 stale active_plugins before rollback-source fence'),
    4 => $page('next270 stale cron before rollback-source fence'),
    5 => $page('next270 stale plugin cache before rollback-source fence'),
    6 => $page('next270 stale autoload index before rollback-source fence'),
];
$recovered = [
    1 => $formatPage('next270 current schema after rollback-source fence'),
    2 => $page('next270 current wp_options root after rollback-source fence'),
    3 => $page('next270 current active_plugins after rollback-source fence'),
    4 => $page('next270 current cron after rollback-source fence'),
    5 => $page('next270 current plugin cache after rollback-source fence'),
    6 => $page('next270 current autoload index after rollback-source fence'),
];
$tokens = [
    $mainJournal => 'dev=8:ino=2701:size=4096:mtime=27001:generation=main-current',
    $usersJournal => 'dev=8:ino=2702:size=1024:mtime=27002:generation=users-current',
];
$headers = [
    $mainJournal => hash('sha256', 'main rollback header next270'),
    $usersJournal => hash('sha256', 'users rollback header next270'),
];
$formatSignature = hash('sha256', implode('|', [512, 4, 2, 270, 0x57503270]));
$masterDigest = hash('sha256', 'next270 master source');
$masterToken = 'dev=8:ino=2700:size=96:mtime=27000:generation=master-current';
$databaseToken = 'dev=8:ino=2709:size=3072:mtime=27099:generation=database-current';
$cleanupToken = 'master-cleanup:deleted:mtime=26800:dirsync=ok';
$readerLeaseToken = 'reader-lease:shared-cache:epoch=270:opened-after-master-cleanup';
$pagerCacheSourceToken = 'pager-cache-source:epoch=270:master-journal-recovery=complete';
$readTransactionToken = 'read-transaction:epoch=270:schema=117:change-counter=270:master-current';
$schemaReparseToken = 'schema-reparse:epoch=270:schema-cookie=117:ddl=master-current';
$statementSchemaRootToken = 'statement-schema-root:epoch=270:root=1:cookie=117:sql=wp-options-current';
$currentSourceProvenanceToken = 'current-source:master-journal:epoch=270:members=2:database-token=2709:schema=117';
$oldCurrentSourceProvenanceToken = 'current-source:master-journal:epoch=266:members=2:database-token=2669:schema=116';
$currentPagerReaderCacheGenerationToken = 'pager-reader-cache-generation:epoch=270:master-current:reset=complete';
$oldPagerReaderCacheGenerationToken = 'pager-reader-cache-generation:epoch=266:before-master-reset';
$currentReaderSnapshotToken = 'reader-snapshot:epoch=270:source=master-current:lease=shared:pages=1,2,3,4,5,6';
$oldReaderSnapshotToken = 'reader-snapshot:epoch=266:source=before-master-current:lease=shared';
$currentRecoveryReceiptToken = 'master-journal-recovery-receipt:epoch=270:members=main,users:cleanup=deleted:dirsync=ok';
$oldRecoveryReceiptToken = 'master-journal-recovery-receipt:epoch=266:members=main,users:cleanup=pending';
$currentPagerSpillDrainToken = 'pager-spill-drain:epoch=270:dirty-pages=2,6:journal-sync=ok:exclusive-lock=held';
$currentRollbackJournalReaderSourceToken = 'pager-rollback-source:epoch=270:dirty-pages=2,6:journal-sync=ok:exclusive-lock=held';
$oldRollbackJournalReaderSourceToken = 'pager-rollback-source:epoch=266:dirty-pages=2,6:journal-sync=pending';
$base = [
    'source_id' => $sourceId,
    'epoch' => 270,
    'format_signature' => $formatSignature,
    'publication_generation' => 270,
    'master_source_digest' => $masterDigest,
    'recovery_sequence' => 270,
...
$plan = static fn (?array $readerCache = null, ?array $nextReads = null, ?string $rollbackSourceToken = null): array => SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::variantNext270(
    $database,
    $master,
    $masterBytes,
    implode('', $before),
...
$cases = [
    'status' => [static fn (): mixed => $plan()['status'], 'pager-master-journal-reader-cache-current-source-next270'],
    ...
    'operation invalidates stale rollback-source cache' => [static fn (): mixed => $opCount($plan(), 'invalidate_reader_cache_rollback_journal_reader_source_current_source_next270'), 1],
    'operation reopens stale rollback-source reader' => [static fn (): mixed => $opCount($plan(), 'reopen_reader_for_rollback_journal_reader_source_current_source_next270'), 1],
    'dependency next270' => [static fn (): mixed => in_array('sqlite-pager-master-journal-reader-cache-current-source-next270', $plan()['dependencies'], true), true],
    ...
    'read stale rollback-source source' => [static fn (): mixed => $readRow('read-2')['source'], 'master-journal-reader-cache-rollback-source-fence-current-source-next270'],
    ...
    'changed current rollback-source invalidates admitted pages' => [static fn (): mixed => $plan(null, null, 'pager-rollback-source:epoch=268:dirty-pages=2,6:journal-sync=ok:exclusive-lock=held')['rollback_journal_reader_source_invalidated_cache_page_numbers'], [1, 2, 6]],
    'changed current rollback-source keeps inherited invalidation' => [static fn (): mixed => in_array(3, $plan(null, null, 'pager-rollback-source:epoch=268:dirty-pages=2,6:journal-sync=ok:exclusive-lock=held')['invalidated_cache_page_numbers'], true), true],
    'changed current rollback-source surfaced' => [static fn (): mixed => $plan(null, null, 'pager-rollback-source:epoch=268:dirty-pages=2,6:journal-sync=ok:exclusive-lock=held')['current_rollback_journal_reader_source_token'], 'pager-rollback-source:epoch=268:dirty-pages=2,6:journal-sync=ok:exclusive-lock=held'],
...
foreach ($cases as $name => [$callback, $expected]) {
    $tests['pager master journal reader cache current source next270 ' . $name] = static function (TestRunner $t) use ($callback, $expected): void {
...
foreach ($throws as $name => $callback) {
    $tests['pager master journal reader cache current source next270 ' . $name] = static function (TestRunner $t) use ($callback): void {
