...
$tests['real upstream corpus date affinity dynamic real date4 continuation 20260531 cites upstream loop'] = static function (TestRunner $t) use ($date4Start, $date4Count): void {
...
for ($i = $date4Start; $i < $date4Start + $date4Count; $i++) {
    ...
    $tests[sprintf('real upstream corpus date affinity dynamic real date4.test date4-%05d continuation 20260531 strftime libc format parity', $i)] = static function (TestRunner $t) use ($date4Format, $timestamp, $expectedDate4, $i): void {
...
$tests['real upstream corpus date affinity dynamic real date4 continuation 20260531 application audit rollup'] = static function (TestRunner $t) use ($date4Format, $date4Start, $date4Count, $expectedDate4): void {
