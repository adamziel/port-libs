...
$pageSize = 512;
$database = '/srv/wp-content/database/wp-VDBE virtual table opcode branch handoff.sqlite';
...
$headers = [$journal => hash('sha256', 'main header VDBE virtual table opcode branch handoff')];
$base = [
    'source_id' => 'pager-reader-cache-vdbe-virtual-table-opcode-branch-handoff',
    'epoch' => 686,
    'format_signature' => hash('sha256', implode('|', [512, 4, 2, 686, 0])),
    'publication_generation' => 686,
    'master_source_digest' => hash('sha256', 'master-VDBE virtual table opcode branch handoff'),
...
$plan = static fn (array $cacheExtra = [], array $readExtra = []): array => SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::currentSourceVdbeVirtualTableOpcodeBranchHandoff(
    $database,
    $master,
    $masterBytes,
    implode('', $before),
...
$tests['pager master journal reader cache vdbe virtual table opcode branch handoff admits current VDBE literal and arithmetic opcode fences'] = static function (TestRunner $t) use ($plan): void {
    $result = $plan();
    $t->same('pager-master-journal-reader-cache-vdbe-virtual-table-opcode-branch-handoff', $result['status']);
...
$tests['pager master journal reader cache current source plan dispatches full width calls to VDBE virtual table opcode branch handoff'] = static function (TestRunner $t) use ($database, $master, $masterBytes, $before, $pageSize, $recovered, $cacheEntry, $read, $base, $tokens, $headers, $variantFields): void {
...
$tests['pager master journal reader cache vdbe virtual table opcode branch handoff invalidates stale opcode trace branch cache'] = static function (TestRunner $t) use ($plan, $opCount): void {
    ...
    $t->same(1, $opCount($result, 'invalidate_reader_cache_reader_cache_stmt_vdbe_opcode_trace_branch_handoff_vdbe_virtual_table_opcode_branch_handoff'));
};

$tests['pager master journal reader cache vdbe virtual table opcode branch handoff reopens stale opcode trace branch read ticket'] = static function (TestRunner $t) use ($plan, $opCount): void {
    $result = $plan([], ['reader_cache_stmt_vdbe_opcode_trace_branch_handoff_token' => 'stmt-vdbe-opcode-trace-branch-old']);
    ...
    $t->same(1, $opCount($result, 'reopen_reader_for_reader_cache_stmt_vdbe_opcode_trace_branch_handoff_vdbe_virtual_table_opcode_branch_handoff'));
};

$tests['pager master journal reader cache vdbe virtual table opcode branch handoff missing opcode trace branch token rejects'] = static function (TestRunner $t) use ($plan): void {
