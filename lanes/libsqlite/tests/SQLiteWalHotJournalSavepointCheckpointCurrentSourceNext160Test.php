...
$basePages = [
    1 => $page('next160 clean schema page before crashed import'),
    2 => $page('next160 clean wp_options page before checkpoint'),
    3 => $page('next160 clean plugin page before savepoint'),
    4 => $page('next160 clean transient page before savepoint'),
];
$dirtyDatabaseBytes = $page('next160 dirty schema page from hot rollback journal')
    . $basePages[2]
    . $page('next160 dirty plugin page from rolled back savepoint')
    . $page('next160 dirty transient page from rolled back savepoint');
...
$makeWalBytes = static function () use ($pageSize, $page): string {
    ...
    foreach ([
        [1, 0, 'next160 wal schema committed after hot journal'],
        [2, 4, 'next160 wal wp_options committed after hot journal'],
        [3, 0, 'next160 wal plugin draft rolled back to savepoint'],
        [4, 4, 'next160 wal transient commit discarded by savepoint'],
...
$withFiles = static function (string $databaseBytes, string $journalInput, string $walInput, callable $callback) use ($databasePath): mixed {
    $root = sys_get_temp_dir() . '/port-libsqlite-hot-savepoint-checkpoint-next160-' . bin2hex(random_bytes(4));
...
$apply = static fn (): array => $withFiles(
    ...
    static fn (string $root, string $databaseLocal, string $journalLocal, string $walLocal): array => [
        'applied' => (new SQLiteVfsFileWriter($root))->applyHotJournalSavepointCheckpointCurrentSourceNext160(
            $makeStack(),
...
$skipReserved = static fn (): array => $withFiles(
    ...
    $journalBytes,
    $walBytes,
    static fn (string $root): array => (new SQLiteVfsFileWriter($root))->applyHotJournalSavepointCheckpointCurrentSourceNext160(
        $makeStack(),
        'plugin_batch_next160',
        $databasePath,
        [1],
...
$cases = [
    ...
    'database has committed schema wal page' => [static fn (): mixed => str_contains($apply()['database_bytes'], 'next160 wal schema committed after hot journal'), true],
    'database has committed option wal page' => [static fn (): mixed => str_contains($apply()['database_bytes'], 'next160 wal wp_options committed after hot journal'), true],
    'database restores clean plugin page' => [static fn (): mixed => str_contains($apply()['database_bytes'], 'next160 clean plugin page before savepoint'), true],
    'database excludes dirty schema page' => [static fn (): mixed => str_contains($apply()['database_bytes'], 'next160 dirty schema page from hot rollback journal'), false],
    'database excludes dirty plugin page' => [static fn (): mixed => str_contains($apply()['database_bytes'], 'next160 dirty plugin page from rolled back savepoint'), false],
    'database excludes discarded wal plugin draft' => [static fn (): mixed => str_contains($apply()['database_bytes'], 'next160 wal plugin draft rolled back to savepoint'), false],
    'database excludes discarded wal transient commit' => [static fn (): mixed => str_contains($apply()['database_bytes'], 'next160 wal transient commit discarded by savepoint'), false],
    ...
    'dependency vfs apply next160' => [static fn (): mixed => in_array('sqlite-wal-hot-journal-savepoint-checkpoint-vfs-apply-next160', $apply()['applied']['dependencies'], true), true],
...
foreach ($cases as $name => [$callback, $expected]) {
    $tests['wal hot journal savepoint checkpoint current source next160 ' . $name] = static function (TestRunner $t) use ($callback, $expected): void {
        $t->same($expected, $callback());
    };
}

$tests['wal hot journal savepoint checkpoint current source next160 rejects empty path'] = static function (TestRunner $t): void {
    $t->throws(InvalidArgumentException::class, static fn (): mixed => (new SQLiteVfsFileWriter(sys_get_temp_dir()))->applyHotJournalSavepointCheckpointCurrentSourceNext160(new SQLiteSavepointStack(), 'x', '', [1]));
};

$tests['wal hot journal savepoint checkpoint current source next160 rejects readonly writer'] = static function (TestRunner $t) use ($dirtyDatabaseBytes, $journalBytes, $walBytes, $withFiles, $databasePath, $makeStack): void {
    $withFiles($dirtyDatabaseBytes, $journalBytes, $walBytes, static function (string $root) use ($t, $databasePath, $makeStack): void {
        $t->throws(LogicException::class, static fn (): mixed => (new SQLiteVfsFileWriter($root, readOnly: true))->applyHotJournalSavepointCheckpointCurrentSourceNext160($makeStack(), 'plugin_batch_next160', $databasePath, [1]));
    });
};

$tests['wal hot journal savepoint checkpoint current source next160 rejects missing savepoint'] = static function (TestRunner $t) use ($dirtyDatabaseBytes, $journalBytes, $walBytes, $withFiles, $databasePath, $makeStack): void {
    $withFiles($dirtyDatabaseBytes, $journalBytes, $walBytes, static function (string $root) use ($t, $databasePath, $makeStack): void {
        $t->throws(InvalidArgumentException::class, static fn (): mixed => (new SQLiteVfsFileWriter($root))->applyHotJournalSavepointCheckpointCurrentSourceNext160($makeStack(), 'missing_savepoint', $databasePath, [1]));
    });
};

$tests['wal hot journal savepoint checkpoint current source next160 rejects bad wal checksum'] = static function (TestRunner $t) use ($dirtyDatabaseBytes, $journalBytes, $walBytes, $withFiles, $databasePath, $makeStack): void {
    ...
    $withFiles($dirtyDatabaseBytes, $journalBytes, $badWal, static function (string $root) use ($t, $databasePath, $makeStack): void {
        $t->throws(InvalidArgumentException::class, static fn (): mixed => (new SQLiteVfsFileWriter($root))->applyHotJournalSavepointCheckpointCurrentSourceNext160($makeStack(), 'plugin_batch_next160', $databasePath, [1]));
    });
};

$tests['wal hot journal savepoint checkpoint current source next160 fixture wal is parseable'] = static function (TestRunner $t) use ($wal): void {
