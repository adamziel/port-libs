...
$tests['utf16 rtrim nocase current source nextOneZeroThree stable unchanged eclair source has no reprepare'] = static function (TestRunner $t) use ($plan): void {
    $stable = SQLiteUtf16RtrimNocaseCurrentSourceNextPlan::optionRowNameCurrentNext(
        [
...
$tests['utf16 rtrim nocase current source nextOneZeroThree rejects missing bytes'] = static function (TestRunner $t) use ($nextRows): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteUtf16RtrimNocaseCurrentSourceNextPlan::optionRowNameCurrentNext([['option_id' => 1, 'text_encoding' => 2]], $nextRows, 'plugin_cache'));
};
...
$tests['utf16 rtrim nocase current source nextOneZeroThree rejects missing encoding'] = static function (TestRunner $t) use ($nextRows): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteUtf16RtrimNocaseCurrentSourceNextPlan::optionRowNameCurrentNext([['option_id' => 1, 'option_name_bytes' => 'x']], $nextRows, 'plugin_cache'));
};
...
$tests['utf16 rtrim nocase current source nextOneZeroThree records unsupported encoding as malformed row'] = static function (TestRunner $t) use ($nextRows): void {
    $plan = SQLiteUtf16RtrimNocaseCurrentSourceNextPlan::optionRowNameCurrentNext([['option_id' => 1, 'option_name_bytes' => 'x', 'text_encoding' => 4]], $nextRows, 'plugin_cache');
