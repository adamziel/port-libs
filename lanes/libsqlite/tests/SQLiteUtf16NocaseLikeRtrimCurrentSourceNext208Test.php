...
): array => SQLiteUtf16NocaseLikeRtrimCurrentSourceNextPlan::keyValueRowKeyPreparedEscapePlan(
    $current ?? $current208,
...
$cases208 = [
    ...
    'current source' => ['currentSource', 'main.app_settings@207'],
    'next source' => ['nextSource', 'main.app_settings@208'],
...
$tests['utf16 nocase like rtrim current source nextTwoZeroEight stable escaped parameter can reuse cursor'] = static function (TestRunner $t) use ($row208, $escapeBytes208): void {
    ...
    $result = SQLiteUtf16NocaseLikeRtrimCurrentSourceNextPlan::keyValueRowKeyPreparedEscapePlan(
        $rows,
        $rows,
        'plugin!_cache%',
...
$tests['utf16 nocase like rtrim current source nextTwoZeroEight rejects malformed escape bytes'] = static function (TestRunner $t) use ($current208, $nextTwoZeroEight, $escapeBytes208): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteUtf16NocaseLikeRtrimCurrentSourceNextPlan::keyValueRowKeyPreparedEscapePlan(
        $current208,
        $nextTwoZeroEight,
        'plugin!_cache%',
        $escapeBytes208('!', 'UTF-16LE'),
