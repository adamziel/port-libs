...
$row192 = static fn (int $id, string $name, int|string $encoding): array => [
    'setting_id' => $id,
    'key_name_bytes' => $enc192($name, $encoding),
...
$current192 = [
    $row192(1, 'Module_Cache   ', 'UTF-16LE'),
    $row192(2, 'module_cache', 'UTF-16BE'),
    $row192(3, "module_cache\t", 'UTF-16LE'),
    $row192(4, 'module_cache_alpha', 'UTF-16BE'),
    $row192(5, 'module_cache_new', 'UTF-8'),
    $row192(6, 'module_cache_old', 'UTF-16LE'),
    $row192(7, 'module_caches', 'UTF-16BE'),
    $row192(8, 'layout_cache', 'UTF-16LE'),
    $bad192(9, "\x00\xd8", 2),
];
$nextOneNineTwo = [
    $row192(1, 'module_cache', 'UTF-16BE'),
    $row192(2, 'MODULE_CACHE  ', 'UTF-16LE'),
    $row192(3, "module_cache\t", 'UTF-16BE'),
    $row192(4, 'module_cache_alpha', 'UTF-16LE'),
    $row192(5, 'module_cache', 'UTF-8'),
    $row192(6, 'module_cache_old', 'UTF-16BE'),
    $row192(10, 'Module_Cache   ', 'UTF-16LE'),
    $row192(11, 'module_cache_zip', 'UTF-16BE'),
    $bad192(12, "x\0y", 2),
];
...
$plan192 = static fn (
    ?array $current = null,
    ?array $next = null,
    ?array $token = ['key' => 'module_cache_old', 'rowid' => 6],
...
$cases192 = [
    ...
    'expression' => ['expression', 'rtrim(key_name) COLLATE NOCASE LIKE ? ESCAPE ? /* candidate residual token */'],
    ...
    'token key' => ['resumeToken.key', 'module_cache_old'],
    ...
    'current row three rtrim keeps tab' => ['currentRtrimTexts.3', "module_cache\t"],
    'next row five now exact' => ['nextMatchedTexts.5', 'module_cache'],
    'next row six false positive key' => ['nextNocaseKeys.6', 'module_cache_old'],
...
$tests['utf16 nocase like rtrim current source nextOneNineTwo stable false-positive token can resume after token'] = static function (TestRunner $t) use ($row192): void {
    $rows = [
        $row192(1, 'Module_Cache', 'UTF-16LE'),
        $row192(2, "module_cache\t", 'UTF-16BE'),
        $row192(3, 'module_cache_alpha', 'UTF-16LE'),
        $row192(4, 'module_cache_zip', 'UTF-16BE'),
    ];
...
$tests['utf16 nocase like rtrim current source nextOneNineTwo matched row becoming false positive forces reprepare'] = static function (TestRunner $t) use ($row192): void {
    $current = [$row192(1, 'module_cache', 'UTF-16LE'), $row192(2, 'module_cache_alpha', 'UTF-16LE')];
    $next = [$row192(1, "module_cache\t", 'UTF-16BE'), $row192(2, 'module_cache_alpha', 'UTF-16LE')];
    $result = SQLiteUtf16NocaseLikeRtrimCurrentSourceNextPlan::keyValueRowKeyCandidateTokenPlan(
        $current,
        $next,
        'module!_cache',
...
$tests['utf16 nocase like rtrim current source nextOneNineTwo canonicalizes token key'] = static function (TestRunner $t) use ($row192): void {
    $rows = [$row192(1, 'Module_Cache', 'UTF-16LE'), $row192(2, 'module_cache_zip', 'UTF-16BE')];
    $result = SQLiteUtf16NocaseLikeRtrimCurrentSourceNextPlan::keyValueRowKeyCandidateTokenPlan(
        $rows,
        $rows,
        'module!_cache',
...
$tests['utf16 nocase like rtrim current source nextOneNineTwo missing token blocks resume'] = static function (TestRunner $t) use ($row192): void {
    $rows = [$row192(1, 'module_cache', 'UTF-16LE')];
    $result = SQLiteUtf16NocaseLikeRtrimCurrentSourceNextPlan::keyValueRowKeyCandidateTokenPlan(
        $rows,
        $rows,
        'module!_cache',
