...
$plan231 = static fn (): array => SQLiteRowValueUpdateDeleteReturningSavepointCurrentSourceNextPlan::executeCompoundSubquerySavepointRollback(
    $tables231,
    [$attemptUpdate231, $attemptDelete231],
    [$retryUpdate231, $retryDelete231],
    $unique231,
);
$customPlan231 = static fn (): array => SQLiteRowValueUpdateDeleteReturningSavepointCurrentSourceNextPlan::executeCompoundSubquerySavepointRollback(
    $tables231,
    [$attemptUpdate231],
    [$retryUpdate231],
    $unique231,
    'wp_custom_rowvalue_compound231',
);
...
$cases231 = [
    ...
    'plan status' => [static fn (): mixed => $plan231()['status'], 'rowvalue-update-delete-returning-compound-subquery-savepoint-current-source-next231'],
    'plan savepoint' => [static fn (): mixed => $plan231()['savepoint'], 'wp_options_rowvalue_compound_subquery_next231'],
    'plan compound flag' => [static fn (): mixed => $plan231()['compound_subquery_source_next231'], true],
    'plan operators' => [static fn (): mixed => $plan231()['compound_operators_next231'], ['UNION', 'UNION ALL', 'INTERSECT', 'EXCEPT']],
    ...
    'plan dependency update compound' => [static fn (): mixed => in_array('sqlite-rowvalue-update-returning-compound-select-subquery-next231', $plan231()['dependencies'], true), true],
    'plan dependency delete compound' => [static fn (): mixed => in_array('sqlite-rowvalue-delete-returning-compound-select-subquery-next231', $plan231()['dependencies'], true), true],
    'plan dependency savepoint compound' => [static fn (): mixed => in_array('sqlite-rowvalue-compound-subquery-savepoint-current-source-next231', $plan231()['dependencies'], true), true],
    'plan dependency closure' => [static fn (): mixed => str_contains($plan231()['dependency_closure_next231'], 'no new support component needed'), true],
    'plan non overlap' => [static fn (): mixed => str_contains($plan231()['non_overlap_next231'], 'avoids accepted next226'), true],
    'custom savepoint' => [static fn (): mixed => $customPlan231()['savepoint'], 'wp_custom_rowvalue_compound231'],
    ...
    'malformed empty attempt rejected' => [static fn (): mixed => SQLiteRowValueUpdateDeleteReturningSavepointCurrentSourceNextPlan::executeCompoundSubquerySavepointRollback($tables231, [], [$retryUpdate231], $unique231), InvalidArgumentException::class],
    'malformed empty retry rejected' => [static fn (): mixed => SQLiteRowValueUpdateDeleteReturningSavepointCurrentSourceNextPlan::executeCompoundSubquerySavepointRollback($tables231, [$attemptUpdate231], [], $unique231), InvalidArgumentException::class],
    'malformed empty unique rejected' => [static fn (): mixed => SQLiteRowValueUpdateDeleteReturningSavepointCurrentSourceNextPlan::executeCompoundSubquerySavepointRollback($tables231, [$attemptUpdate231], [$retryUpdate231], []), InvalidArgumentException::class],
    'malformed savepoint rejected' => [static fn (): mixed => SQLiteRowValueUpdateDeleteReturningSavepointCurrentSourceNextPlan::executeCompoundSubquerySavepointRollback($tables231, [$attemptUpdate231], [$retryUpdate231], $unique231, 'bad-name'), InvalidArgumentException::class],
    'malformed row list rejected' => [static fn (): mixed => SQLiteRowValueUpdateDeleteReturningSavepointCurrentSourceNextPlan::executeCompoundSubquerySavepointRollback(['wp_options' => ['bad']], [$attemptUpdate231], [$retryUpdate231], $unique231), InvalidArgumentException::class],
];

$tests = [];
foreach ($cases231 as $name => [$callback, $expected]) {
    $tests['rowvalue update delete returning savepoint current source next231 ' . $name] = static function (TestRunner $t) use ($callback, $expected): void {
