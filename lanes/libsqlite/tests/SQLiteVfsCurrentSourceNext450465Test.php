...
$plan = static function () use ($readyCurrent): array {
    static $result = null;
    if ($result === null) {
        $result = SQLiteVfsCurrentSourceNext450465Plan::run([
...
return [
    ...
    'vfs current source next450-465 blocks dirty snapshot' => static fn (TestRunner $t) => $t->same(true, in_array('dirty-pages-present', SQLiteVfsCurrentSourceNext450465Plan::run(['snapshot(reader-ready-next465,shared-cache-next449)'], ['current' => $dirtyCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source next450-465 blocks old snapshot ack' => static fn (TestRunner $t) => $t->same(true, in_array('ack-not-latest-publish', SQLiteVfsCurrentSourceNext450465Plan::run(['snapshot(reader-ready-next465,shared-cache-next449)'], ['current' => $oldAckCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source next450-465 blocks publish without claim' => static fn (TestRunner $t) => $t->same(true, in_array('missing-reuse-claim', SQLiteVfsCurrentSourceNext450465Plan::run(['snapshot(reader-ready-next465,shared-cache-next449)', 'publish(reader-ready-next465,reader-reuse-next465,shared-cache-next465)'], ['current' => $readyCurrent])['events'][1]['blocked_reasons'], true)),
    'vfs current source next450-465 blocks stale claim' => static fn (TestRunner $t) => $t->same(true, in_array('stale-reuse-claim', SQLiteVfsCurrentSourceNext450465Plan::run(['publish(reader-ready-next465,reader-reuse-next465,shared-cache-next465)'], ['current' => $staleClaimCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source next450-465 accepts preclaimed publish' => static fn (TestRunner $t) => $t->same('published-reused-current-source', SQLiteVfsCurrentSourceNext450465Plan::run(['publish(reader-ready-next465,reader-reuse-next465,shared-cache-next465)'], ['current' => $claimedCurrent])['events'][0]['status']),
    'vfs current source next450-465 rejects empty operations' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteVfsCurrentSourceNext450465Plan::run([])),
    'vfs current source next450-465 rejects bad claim token' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteVfsCurrentSourceNext450465Plan::run([['op' => 'claim', 'snapshot' => 'reader-ready-next465', 'ack' => 'shared-cache-next449', 'claim' => 'bad claim']], ['current' => $readyCurrent])),
    'vfs current source next450-465 rejects unsupported operation' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteVfsCurrentSourceNext450465Plan::run(['republish(reader-ready-next465,shared-cache-next449)'], ['current' => $readyCurrent])),
