...
use PortLibs\LibSqlite\SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext221Plan;
...
$currentTables221 = ['wp_options' => $currentOptions221];
$nextTables221 = ['wp_options' => $nextOptions221];
...
$summary221 = static fn (?array $cursor = null): array => SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext221Plan::compare($sql221, $currentTables221, $nextTables221, $cursor);
$tests = [];
...
$tests['compound select window recursive limit current source next221 status dependencies'] = static function (TestRunner $t) use ($summary221): void {
    $plan = $summary221();
    $t->same('compound-select-window-recursive-limit-current-source-next221-ready', $plan['status']);
    $t->same([
        'sqlite-select-sql-recursive-queue-order-limit-next221',
        'sqlite-select-sql-max-sum-window-next221',
        'sqlite-compound-intersect-current-source-token-fence-next221',
    ], $plan['dependencies']);
    $t->contains('no new support component needed', $plan['dependency_closure']);
};

$tests['compound select window recursive limit current source next221 compound metadata'] = static function (TestRunner $t) use ($summary221): void {
    $compound = $summary221()['compound'];
...
$tests['compound select window recursive limit current source next221 current rows'] = static function (TestRunner $t) use ($summary221): void {
    $rows = $summary221()['currentRows'];
    $t->same(5, count($rows));
...
$tests['compound select window recursive limit current source next221 next source boundary'] = static function (TestRunner $t) use ($summary221): void {
    $plan = $summary221();
...
$tests['compound select window recursive limit current source next221 recursive queue trace'] = static function (TestRunner $t) use ($summary221): void {
    $queue = $summary221()['recursiveQueue'];
    $t->same('q', $queue['name']);
...
$tests['compound select window recursive limit current source next221 aggregate window shape'] = static function (TestRunner $t) use ($summary221): void {
    $windows = $summary221()['windows'];
...
$tests['compound select window recursive limit current source next221 token fence'] = static function (TestRunner $t) use ($summary221): void {
    $first = $summary221();
    $second = $summary221($first['cursor']);
...
$tests['compound select window recursive limit current source next221 rejects stale cursor'] = static function (TestRunner $t) use ($summary221): void {
    $cursor = $summary221()['cursor'];
    $cursor['currentToken'] = str_repeat('f', 64);
    $t->throws(InvalidArgumentException::class, static fn () => $summary221($cursor));
};

$tests['compound select window recursive limit current source next221 limit trace'] = static function (TestRunner $t) use ($summary221): void {
    $trace = $summary221()['limitTrace'];
...
$tests['compound select window recursive limit current source next221 replan reasons'] = static function (TestRunner $t) use ($summary221): void {
    $plan = $summary221();
    $t->contains('max/sum window output', $plan['non_overlap']);
    $t->true(in_array('compound-max-sum-current-source-next221', $plan['replanReasons'], true));
    $t->true(in_array('recursive-queue-exhausted-before-intersect-next221', $plan['replanReasons'], true));
    $t->true(in_array('intersect-window-membership-before-final-limit-next221', $plan['replanReasons'], true));
};

$tests['compound select window recursive limit current source next221 base rows match executor'] = static function (TestRunner $t) use ($sql221, $currentTables221, $summary221): void {
    $t->same(SQLiteSelectSql::execute($sql221, $currentTables221), $summary221()['currentRows']);
};
...
$tests['compound select window recursive limit current source next221 rejects missing sum'] = static function (TestRunner $t) use ($currentTables221): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext221Plan::compare(
...
foreach (range(1, 50) as $case) {
    $tests['compound select window recursive limit current source next221 generated aggregate boundary ' . $case] = static function (TestRunner $t) use ($case): void {
        $finalLimit = 4 + ($case % 3);
        $tables = [
            'wp_options' => [
        ...
        $plan = SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext221Plan::compare($sql, $tables, $nextTables);
        $again = SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext221Plan::compare($sql, $tables, $nextTables, $plan['cursor']);
