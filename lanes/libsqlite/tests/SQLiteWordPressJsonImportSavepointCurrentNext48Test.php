...
$currentRows = static fn (): array => [
    ...
    [
        ...
        'option_value' => '{"colors":{"accent":"blue"}}',
        'autoload' => 'yes',
        'page_number' => 3,
    ],
];
...
$cases = [
    ...
    'insert catalog is marked inserted' => static fn (): mixed => $plan()['applied'][0]['inserted_option'],
    'existing settings update is not marked inserted' => static fn (): mixed => $plan()['applied'][1]['inserted_option'],
    'insert palette is marked inserted' => static fn (): mixed => $plan()['applied'][2]['inserted_option'],
    'inserted catalog option is retained in final rows' => static fn (): mixed => $finalRow($plan(), 'plugin_catalog')['option_id'],
    'inserted catalog autoload defaults from mutation' => static fn (): mixed => $finalRow($plan(), 'plugin_catalog')['autoload'],
    'inserted catalog page is explicit' => static fn (): mixed => $finalRow($plan(), 'plugin_catalog')['page_number'],
    'inserted catalog json subtype array is decoded' => static fn (): mixed => $decodeOption($finalRow($plan(), 'plugin_catalog')['option_value'])['plugins'],
    'existing settings import counter updates' => static fn (): mixed => $decodeOption($finalRow($plan(), 'plugin_settings')['option_value'])['imports'],
    'existing settings enabled value is preserved' => static fn (): mixed => $decodeOption($finalRow($plan(), 'plugin_settings')['option_value'])['enabled'],
    'inserted palette option id is explicit' => static fn (): mixed => $finalRow($plan(), 'theme_palette')['option_id'],
    'inserted palette autoload is explicit' => static fn (): mixed => $finalRow($plan(), 'theme_palette')['autoload'],
    'inserted palette stays jsonb blob' => static fn (): mixed => $finalRow($plan(), 'theme_palette')['option_value'] instanceof SQLiteBlobValue,
    'inserted palette nested subtype name decodes' => static fn (): mixed => $decodeOption($finalRow($plan(), 'theme_palette')['option_value'])['colors']['accent']['name'],
    'inserted palette nested subtype contrast decodes' => static fn (): mixed => $decodeOption($finalRow($plan(), 'theme_palette')['option_value'])['colors']['accent']['contrast'],
    'failed inserted option is not present after statement rollback' => static fn (): mixed => array_column($plan()['final_rows'], 'option_name'),
    'failed statement name is retained' => static fn (): mixed => $plan()['failed'][0]['statement'],
    'failed inserted option name is retained' => static fn (): mixed => $plan()['failed'][0]['option_name'],
    ...
    'implicit inserted option id follows current max id' => static fn (): mixed => $finalRow($plan($currentRows(), [
        ['option_name' => 'implicit_catalog', 'on_missing' => 'insert', 'path' => '$.enabled', 'value' => true],
    ]), 'implicit_catalog')['option_id'],
    'implicit inserted option page follows rowid bucket' => static fn (): mixed => $finalRow($plan($currentRows(), [
        ['option_name' => 'implicit_catalog', 'on_missing' => 'insert', 'path' => '$.enabled', 'value' => true],
    ]), 'implicit_catalog')['page_number'],
    'implicit inserted autoload defaults to no' => static fn (): mixed => $finalRow($plan($currentRows(), [
        ['option_name' => 'implicit_catalog', 'on_missing' => 'insert', 'path' => '$.enabled', 'value' => true],
    ]), 'implicit_catalog')['autoload'],
    'implicit inserted json value mutates from empty object' => static fn (): mixed => $decodeOption($finalRow($plan($currentRows(), [
        ['option_name' => 'implicit_catalog', 'on_missing' => 'insert', 'path' => '$.enabled', 'value' => true],
    ]), 'implicit_catalog')['option_value'])['enabled'],
    'missing option without insert mode still fails' => static fn (): mixed => $plan($currentRows(), [
        ['statement' => 'missing_plain', 'option_name' => 'plain_missing', 'path' => '$.enabled', 'value' => true],
    ])['failed'][0]['statement'],
    'duplicate inserted option id rolls back statement' => static fn (): mixed => $plan($currentRows(), [
        ['statement' => 'duplicate_id', 'option_name' => 'duplicate_catalog', 'on_missing' => 'insert', 'insert_option_id' => 65, 'path' => '$.enabled', 'value' => true],
    ])['failed'][0]['statement'],
    'invalid inserted option id rolls back statement' => static fn (): mixed => $plan($currentRows(), [
        ['statement' => 'bad_id', 'option_name' => 'bad_catalog', 'on_missing' => 'insert', 'insert_option_id' => 0, 'path' => '$.enabled', 'value' => true],
    ])['failed'][0]['statement'],
    'invalid inserted autoload rolls back statement' => static fn (): mixed => $plan($currentRows(), [
        ['statement' => 'bad_autoload', 'option_name' => 'bad_catalog', 'on_missing' => 'insert', 'insert_autoload' => 1, 'path' => '$.enabled', 'value' => true],
    ])['failed'][0]['statement'],
    'invalid inserted page rolls back statement' => static fn (): mixed => $plan($currentRows(), [
        ['statement' => 'bad_page', 'option_name' => 'bad_catalog', 'on_missing' => 'insert', 'page_number' => 0, 'path' => '$.enabled', 'value' => true],
    ])['failed'][0]['statement'],
    'inserted jsonb initial value can be updated as json text function' => static fn (): mixed => $decodeOption($finalRow($plan($currentRows(), [
        ['option_name' => 'jsonb_initial', 'on_missing' => 'insert', 'initial_value' => new SQLiteBlobValue(SQLiteJsonB::encode(['count' => 1])), 'path' => '$.count', 'value' => 2],
    ]), 'jsonb_initial')['option_value'])['count'],
...
$expected = [
    ...
    'inserted catalog option is retained in final rows' => 130,
    'inserted catalog autoload defaults from mutation' => 'no',
    ...
    'inserted palette option id is explicit' => 141,
    'inserted palette autoload is explicit' => 'yes',
    ...
    'failed inserted option is not present after statement rollback' => ['plugin_settings', 'theme_mods_twenty', 'plugin_catalog', 'theme_palette'],
    'failed statement name is retained' => 'insert_broken_catalog',
    'failed inserted option name is retained' => 'broken_catalog',
    ...
    'implicit inserted option id follows current max id' => 66,
    'implicit inserted option page follows rowid bucket' => 3,
    'implicit inserted autoload defaults to no' => 'no',
    'implicit inserted json value mutates from empty object' => true,
    'missing option without insert mode still fails' => 'missing_plain',
    'duplicate inserted option id rolls back statement' => 'duplicate_id',
    'invalid inserted option id rolls back statement' => 'bad_id',
    'invalid inserted autoload rolls back statement' => 'bad_autoload',
