...
$tests = [
    ...
    'planner stat4 expression partial current source next382397 cursor appended' => static fn (TestRunner $t) => $t->same('PrepareStat4ExpressionPartialPreparedHandoffRange', $plan382397()['cursorProgram'][array_key_last($plan382397()['cursorProgram'])]['opcode']),
    'planner stat4 expression partial current source next382397 cursor mode' => static fn (TestRunner $t) => $t->same('prepared-handoff-range-current-source-stat4-expression-partial-prep', $plan382397()['cursorProgram'][array_key_last($plan382397()['cursorProgram'])]['mode']),
