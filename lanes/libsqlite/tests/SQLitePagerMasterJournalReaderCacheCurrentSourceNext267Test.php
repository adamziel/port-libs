...
$pageSize = 512;
$database = '/srv/wp-content/database/wp-next267.sqlite';
$usersDatabase = '/srv/wp-content/database/wp-next267-users.sqlite';
$mainJournal = $database . '-journal';
$usersJournal = $usersDatabase . '-journal';
$master = $database . '-mj';
$sourceId = 'pager-reader-cache-rollback-source-next267';
...
$before = [
    1 => $formatPage('next267 stale schema before rollback-source fence'),
    2 => $page('next267 stale wp_options root before rollback-source fence'),
    3 => $page('next267 stale active_plugins before rollback-source fence'),
    4 => $page('next267 stale cron before rollback-source fence'),
    5 => $page('next267 stale plugin cache before rollback-source fence'),
    6 => $page('next267 stale autoload index before rollback-source fence'),
];
$recovered = [
    1 => $formatPage('next267 current schema after rollback-source fence'),
    2 => $page('next267 current wp_options root after rollback-source fence'),
    3 => $page('next267 current active_plugins after rollback-source fence'),
    4 => $page('next267 current cron after rollback-source fence'),
    5 => $page('next267 current plugin cache after rollback-source fence'),
    6 => $page('next267 current autoload index after rollback-source fence'),
];
$tokens = [
...
$headers = [
    $mainJournal => hash('sha256', 'main rollback header next267'),
    $usersJournal => hash('sha256', 'users rollback header next267'),
];
$formatSignature = hash('sha256', implode('|', [512, 4, 2, 267, 0x57503267]));
$masterDigest = hash('sha256', 'next267 master source');
...
$plan = static fn (?array $readerCache = null, ?array $nextReads = null, ?string $rollbackSourceToken = null): array => SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::variantNext267(
    $database,
    $master,
    $masterBytes,
    implode('', $before),
...
$cases = [
    'status' => [static fn (): mixed => $plan()['status'], 'pager-master-journal-reader-cache-current-source-next267'],
    ...
    'operation invalidates stale rollback-source cache' => [static fn (): mixed => $opCount($plan(), 'invalidate_reader_cache_rollback_journal_reader_source_current_source_next267'), 1],
    'operation reopens stale rollback-source reader' => [static fn (): mixed => $opCount($plan(), 'reopen_reader_for_rollback_journal_reader_source_current_source_next267'), 1],
    'dependency next267' => [static fn (): mixed => in_array('sqlite-pager-master-journal-reader-cache-current-source-next267', $plan()['dependencies'], true), true],
    ...
    'read stale rollback-source source' => [static fn (): mixed => $readRow('read-2')['source'], 'master-journal-reader-cache-rollback-source-fence-current-source-next267'],
...
foreach ($cases as $name => [$callback, $expected]) {
    $tests['pager master journal reader cache current source next267 ' . $name] = static function (TestRunner $t) use ($callback, $expected): void {
...
foreach ($throws as $name => $callback) {
    $tests['pager master journal reader cache current source next267 ' . $name] = static function (TestRunner $t) use ($callback): void {
