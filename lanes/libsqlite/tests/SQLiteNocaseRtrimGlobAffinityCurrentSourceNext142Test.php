...
$plan = static fn (
    ...
    int $nextCookie = 142,
): array => SQLiteNocaseRtrimGlobAffinityCurrentSourceNext142Plan::wordpressOptionNamePlan(
    $current ?? $currentRows,
    $next ?? $nextRows,
    $pattern,
    $affinity,
...
$tests['nocase rtrim glob affinity current source next142 stable binary rows reusable'] = static function (TestRunner $t) use ($row): void {
    ...
    $plan = SQLiteNocaseRtrimGlobAffinityCurrentSourceNext142Plan::wordpressOptionNamePlan($rows, $rows, 'plugin_*', 'TEXT', 'BINARY', 'stable', 'stable', 7, 7);
...
$tests['nocase rtrim glob affinity current source next142 stable nocase records fallback'] = static function (TestRunner $t) use ($row): void {
    $rows = [$row(1, 'plugin_alpha', 'UTF-8'), $row(2, 'Plugin_Beta', 'UTF-16LE')];
    $plan = SQLiteNocaseRtrimGlobAffinityCurrentSourceNext142Plan::wordpressOptionNamePlan($rows, $rows, 'plugin_*', 'TEXT', 'NOCASE', 'stable', 'stable', 7, 7);
...
$tests['nocase rtrim glob affinity current source next142 rejects unsupported collation'] = static function (TestRunner $t) use ($currentRows): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteNocaseRtrimGlobAffinityCurrentSourceNext142Plan::wordpressOptionNamePlan($currentRows, $currentRows, 'plugin_*', 'TEXT', 'UNICODE'));
};

$tests['nocase rtrim glob affinity current source next142 rejects missing option id'] = static function (TestRunner $t): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteNocaseRtrimGlobAffinityCurrentSourceNext142Plan::wordpressOptionNamePlan([['option_name_bytes' => 'plugin', 'text_encoding' => 1]], [], 'plugin_*'));
};

$tests['nocase rtrim glob affinity current source next142 rejects missing bytes'] = static function (TestRunner $t): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteNocaseRtrimGlobAffinityCurrentSourceNext142Plan::wordpressOptionNamePlan([['option_id' => 1, 'text_encoding' => 1]], [], 'plugin_*'));
};

$tests['nocase rtrim glob affinity current source next142 rejects missing encoding'] = static function (TestRunner $t): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteNocaseRtrimGlobAffinityCurrentSourceNext142Plan::wordpressOptionNamePlan([['option_id' => 1, 'option_name_bytes' => 'plugin']], [], 'plugin_*'));
};
...
$tests['nocase rtrim glob affinity current source next142 records unsupported encoding as malformed row'] = static function (TestRunner $t): void {
    $plan = SQLiteNocaseRtrimGlobAffinityCurrentSourceNext142Plan::wordpressOptionNamePlan([['option_id' => 1, 'option_name_bytes' => 'plugin', 'text_encoding' => 9]], [], 'plugin_*');
