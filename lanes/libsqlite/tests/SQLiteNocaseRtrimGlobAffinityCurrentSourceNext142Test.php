...
$cases = [
    ...
    'current source' => ['plugin_*', 'TEXT', 'NOCASE', 'currentSource', 'main.app_settings@141'],
    'next source' => ['plugin_*', 'TEXT', 'NOCASE', 'nextSource', 'main.app_settings@142'],
...
$tests['nocase rtrim glob affinity current source next142 stable binary rows reusable'] = static function (TestRunner $t) use ($row): void {
    ...
    $plan = SQLiteNocaseRtrimGlobAffinityCurrentSourceNextPlan::keyValueRowKeyPlan($rows, $rows, 'plugin_*', 'TEXT', 'BINARY', 'stable', 'stable', 7, 7);
...
$tests['nocase rtrim glob affinity current source next142 stable nocase records fallback'] = static function (TestRunner $t) use ($row): void {
    $rows = [$row(1, 'plugin_alpha', 'UTF-8'), $row(2, 'Plugin_Beta', 'UTF-16LE')];
    $plan = SQLiteNocaseRtrimGlobAffinityCurrentSourceNextPlan::keyValueRowKeyPlan($rows, $rows, 'plugin_*', 'TEXT', 'NOCASE', 'stable', 'stable', 7, 7);
...
$tests['nocase rtrim glob affinity current source next142 rejects unsupported collation'] = static function (TestRunner $t) use ($currentRows): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteNocaseRtrimGlobAffinityCurrentSourceNextPlan::keyValueRowKeyPlan($currentRows, $currentRows, 'plugin_*', 'TEXT', 'UNICODE'));
};

$tests['nocase rtrim glob affinity current source next142 rejects missing option id'] = static function (TestRunner $t): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteNocaseRtrimGlobAffinityCurrentSourceNextPlan::keyValueRowKeyPlan([['option_name_bytes' => 'plugin', 'text_encoding' => 1]], [], 'plugin_*'));
};

$tests['nocase rtrim glob affinity current source next142 rejects missing bytes'] = static function (TestRunner $t): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteNocaseRtrimGlobAffinityCurrentSourceNextPlan::keyValueRowKeyPlan([['option_id' => 1, 'text_encoding' => 1]], [], 'plugin_*'));
};

$tests['nocase rtrim glob affinity current source next142 rejects missing encoding'] = static function (TestRunner $t): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteNocaseRtrimGlobAffinityCurrentSourceNextPlan::keyValueRowKeyPlan([['option_id' => 1, 'option_name_bytes' => 'plugin']], [], 'plugin_*'));
};
...
$tests['nocase rtrim glob affinity current source next142 records unsupported encoding as malformed row'] = static function (TestRunner $t): void {
    $plan = SQLiteNocaseRtrimGlobAffinityCurrentSourceNextPlan::keyValueRowKeyPlan([['option_id' => 1, 'option_name_bytes' => 'plugin', 'text_encoding' => 9]], [], 'plugin_*');
