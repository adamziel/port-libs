...
$makeFirstPage463478 = static function (int $pageCount): string {
...
$putPointerMapEntry463478 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database463478 = static function (int $sliceNumber) use ($makeFirstPage463478, $putPointerMapEntry463478): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage463478(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry463478($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan463478 = static function (int $sliceNumber, int $batchSize = 2) use ($database463478): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourcePlan {
    $database = $database463478($sliceNumber);
...
foreach ([463, 464, 465, 466, 467, 468, 469, 470, 471, 472, 473, 474, 475, 476, 477, 478] as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} publishes next463-478 handoff receipts"] = static function (TestRunner $t) use ($plan463478, $sliceNumber): void {
        $plan = $plan463478($sliceNumber);
        ...
        $t->same(6, $plan463478($sliceNumber, 3)->currentSourceSummary()['current_source_row_count']);
    };
}
