...
$makeFirstPage10391054 = static function (int $pageCount): string {
...
$putPointerMapEntry10391054 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database10391054 = static function (int $sliceNumber) use ($makeFirstPage10391054, $putPointerMapEntry10391054): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage10391054(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry10391054($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan10391054 = static function (int $sliceNumber, int $batchSize = 2) use ($database10391054): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database10391054($sliceNumber);
...
foreach (range(1039, 1054) as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} extends next1039-1054 handoff receipts"] = static function (TestRunner $t) use ($plan10391054, $sliceNumber): void {
        $plan = $plan10391054($sliceNumber);
        ...
        $t->same(6, $plan10391054($sliceNumber, 3)->currentSourceSummary()['current_source_row_count']);
    };
}
