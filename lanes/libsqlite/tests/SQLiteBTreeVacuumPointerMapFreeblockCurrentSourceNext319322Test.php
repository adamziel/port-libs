...
$makeFirstPage319322 = static function (int $pageCount): string {
...
$putPointerMapEntry319322 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database319322 = static function (int $sliceNumber) use ($makeFirstPage319322, $putPointerMapEntry319322): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage319322(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry319322($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan319322 = static function (int $sliceNumber, int $batchSize = 2) use ($database319322): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database319322($sliceNumber);
...
foreach ([319, 320, 321, 322] as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} keeps final next319-322 freelist splice isolated"] = static function (TestRunner $t) use ($plan319322, $sliceNumber): void {
        $plan = $plan319322($sliceNumber);
        ...
        $t->same(6, $plan319322($sliceNumber, 3)->freelistSummary()['freelist_row_count']);
    };
}
