...
$plan = static fn (
    ...
    int $nextCookie = 158,
): array => SQLiteUtf16NocaseLikeRtrimCurrentSourceNext158Plan::wordpressOptionNamePlan(
    $current ?? $currentRows,
    $next ?? $nextRows,
    $pattern,
    $escape,
...
$tests['utf16 nocase like rtrim current source next158 stable identical rows reusable'] = static function (TestRunner $t) use ($enc): void {
    ...
    $result = SQLiteUtf16NocaseLikeRtrimCurrentSourceNext158Plan::wordpressOptionNamePlan($rows, $rows, 'plugin!_%', '!', 'stable', 'stable', 9, 9);
...
$tests['utf16 nocase like rtrim current source next158 rejects missing option bytes'] = static function (TestRunner $t): void {
    ...
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteUtf16NocaseLikeRtrimCurrentSourceNext158Plan::wordpressOptionNamePlan($rows, $rows, 'plugin%'));
};
...
$tests['utf16 nocase like rtrim current source next158 rejects bad encoding id'] = static function (TestRunner $t) use ($enc): void {
    ...
    $result = SQLiteUtf16NocaseLikeRtrimCurrentSourceNext158Plan::wordpressOptionNamePlan($rows, $rows, 'plugin%');
