...
$jsonOperatorText = static fn (mixed $value): mixed => $value instanceof SQLiteJsonSubtypeValue ? $value->json : $value;
...
for ($case = 1; $case <= 1000; $case++) {
    ...
    $tests['real upstream json102 operator RHS object string key vs integer null dynamic ' . str_pad((string) $case, 4, '0', STR_PAD_LEFT)] =
        static function (TestRunner $t) use ($binary, $jsonOperatorText, $objectJson, $objectBlob, $objectStringKey, $slot, $expectedObjectValue, $expectedObjectJsonValue): void {
            ...
            $t->same($expectedObjectJsonValue, $jsonOperatorText(SQLiteSelectExpression::evaluate([], $binary('->', $objectJson, $objectStringKey))), 'json102-1820 text object string RHS returns JSON text');
            ...
            $t->same($expectedObjectJsonValue, $jsonOperatorText(SQLiteSelectExpression::evaluate([], $binary('->', $objectBlob, $objectStringKey))), 'json102-1820 JSONB object string RHS returns JSON text');
    ...
    $tests['real upstream json102 operator RHS array integer index vs string null dynamic ' . str_pad((string) $case, 4, '0', STR_PAD_LEFT)] =
        static function (TestRunner $t) use ($binary, $jsonOperatorText, $arrayJson, $arrayBlob, $arrayStringIndex, $arrayIndex, $expectedArrayValue, $expectedArrayJsonValue): void {
            ...
            $t->same($expectedArrayJsonValue, $jsonOperatorText(SQLiteSelectExpression::evaluate([], $binary('->', $arrayJson, $arrayIndex))), 'json102-1831 text array integer RHS returns JSON text');
            ...
            $t->same($expectedArrayJsonValue, $jsonOperatorText(SQLiteSelectExpression::evaluate([], $binary('->', $arrayBlob, $arrayIndex))), 'json102-1831 JSONB array integer RHS returns JSON text');
        };
}
