...
$plan145148 = static fn (array $events, ?array $statements = null, ?array $schemas = null): array => SQLiteAttachWalTempSchemaCacheCurrentSourceNext92Plan::currentSourceNext145148(
...
$tests['attach temp wal schema cache current source next145 attach report shadows missing unqualified table'] = static function (TestRunner $t) use ($plan145148): void {
    ...
    $t->same('attach-wal-temp-schema-cache-current-source-next145-148', $result['operation']);
    $t->same('sqlite-attach-temp-wal-schema-cache-current-source-next145', $result['dependencies'][0]);
