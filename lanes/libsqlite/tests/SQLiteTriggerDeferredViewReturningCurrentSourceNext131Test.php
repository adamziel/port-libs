...
$fk131 = ['parent_key' => 'option_name', 'child_key' => 'parent_name', 'deferred' => true];
$view131 = [
...
$admitted131 = static fn (): array => $run131(null, [
    ['option_name' => 'theme_mods', 'option_value' => 'new-theme', 'autoload' => 'yes', 'parent_name' => 'siteurl', 'revision' => 4],
    ['option_name' => 'seo_settings', 'option_value' => 'enabled', 'autoload' => 'yes', 'parent_name' => 'siteurl', 'revision' => 1],
]);
...
$cases131 = [
    ...
    'trigger name retained' => [static fn (): mixed => $blocked131()['trigger'], 'wp_options_view_io_update_131'],
    'key retained' => [static fn (): mixed => $blocked131()['key'], 'option_name'],
    ...
    'view name retained' => [static fn (): mixed => $blocked131()['view'], 'wp_autoloaded_options_131'],
    'view columns retained' => [static fn (): mixed => $blocked131()['view_columns'], ['option_name', 'option_value', 'parent_name', 'autoload']],
    'current view rows ordered' => [static fn (): mixed => array_column($blocked131()['current_view_rows'], 'option_name'), ['seo_settings', 'siteurl', 'theme_mods']],
    'current view sees updated value' => [static fn (): mixed => $blocked131()['current_view_rows'][2]['option_value'], 'new-theme'],
    'current view sees inserted option' => [static fn (): mixed => $blocked131()['current_view_rows'][0]['parent_name'], 'siteurl'],
    ...
    'current returning names' => [static fn (): mixed => array_column(array_column($blocked131()['current_returning_rows'], 'returning'), 'option_name'), ['theme_mods', 'seo_settings']],
    ...
    'blocked attempted next names' => [static fn (): mixed => array_column(array_column($blocked131()['attempted_next_returning_rows'], 'returning'), 'option_name'), ['plugin_cache', 'rewrite_rules']],
    ...
    'blocked violation option name' => [static fn (): mixed => $blocked131()['deferred_violations'][0]['option_name'], 'theme_mods'],
    ...
    'blocked final rows restored' => [static fn (): mixed => array_column($blocked131()['final_rows'], 'option_name'), ['siteurl', 'theme_mods', 'plugin_cache']],
    'blocked final value restored' => [static fn (): mixed => $blocked131()['final_rows'][1]['option_value'], 'old-theme'],
    'blocked current rows keep attempted mutation evidence' => [static fn (): mixed => array_column($blocked131()['current_rows'], 'option_name'), ['siteurl', 'theme_mods', 'plugin_cache', 'seo_settings']],
    'blocked current row mutated parent evidence' => [static fn (): mixed => $blocked131()['current_rows'][1]['parent_name'], 'missing-theme-parent'],
    'blocked children from before rows' => [static fn (): mixed => array_column($blocked131()['children'], 'option_name'), ['siteurl', 'theme_mods', 'plugin_cache']],
    ...
    'admitted next returning visible' => [static fn (): mixed => array_column(array_column($admitted131()['next_returning_rows'], 'returning'), 'option_name'), ['plugin_cache', 'rewrite_rules']],
    ...
    'admitted final names include next rows' => [static fn (): mixed => array_column($admitted131()['final_rows'], 'option_name'), ['siteurl', 'theme_mods', 'plugin_cache', 'seo_settings', 'rewrite_rules']],
    'admitted final plugin cache is primed' => [static fn (): mixed => $admitted131()['final_rows'][2]['option_value'], 'primed'],
    ...
    'non rollback final includes next insert' => [static fn (): mixed => array_column($kept131()['final_rows'], 'option_name'), ['siteurl', 'theme_mods', 'plugin_cache', 'seo_settings', 'rewrite_rules']],
    ...
    'bad view where throws' => [static fn (): mixed => $run131(null, null, null, [], ['name' => 'bad_view', 'columns' => ['option_name'], 'where' => 'no']), InvalidArgumentException::class],
    'duplicate keys throw' => [static fn (): mixed => $run131(array_merge($rows131, [['option_name' => 'siteurl']])), InvalidArgumentException::class],
    'mutation missing key throws' => [static fn (): mixed => $run131(null, [['option_value' => 'missing']]), InvalidArgumentException::class],
];
