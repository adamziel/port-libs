...
$tests['vfs filecontrol current next68 exposes initial transaction controls'] = static function (TestRunner $t) use ($makeState): void {
    $sequence = $makeState(['sync_count' => 2, 'commit_phase_two_count' => 1, 'atomic_write_generation' => 3])->currentNext68([]);
    ...
    $t->same(true, in_array('vfs-file-control-current-next68', $sequence['dependencies'], true));
};
...
foreach ($atomicSequences as $name => [$ops, $expected]) {
    $tests['vfs filecontrol current next68 atomic ' . $name] = static function (TestRunner $t) use ($makeState, $ops, $expected): void {
        $sequence = $makeState()->currentNext68($ops);
...
$tests['vfs filecontrol current next68 commit threads generation into next current'] = static function (TestRunner $t) use ($makeState): void {
    $sequence = $makeState()->currentNext68([
        ['op' => 'begin_atomic_write'],
        ['op' => 'commit_atomic_write'],
        ['op' => 'begin_atomic_write'],
    ]);
...
$tests['vfs filecontrol current next68 readonly begin atomic is ignored'] = static function (TestRunner $t): void {
    ...
    $pair = $state->currentNext68([['op' => 'begin_atomic_write']])['pairs'][0];
...
$tests['vfs filecontrol current next68 memory begin atomic is ignored'] = static function (TestRunner $t): void {
...
foreach ($syncCases as $name => [$argument, $flags, $count, $changed]) {
    $tests['vfs filecontrol current next68 ' . $name] = static function (TestRunner $t) use ($makeState, $argument, $flags, $count, $changed): void {
        $pair = $makeState()->currentNext68([['op' => 'sync', 'value' => $argument]])['pairs'][0];
...
$tests['vfs filecontrol current next68 repeated same sync increments count'] = static function (TestRunner $t) use ($makeState): void {
    $sequence = $makeState()->currentNext68([
        ['op' => 'sync', 'value' => 'full'],
        ['op' => 'sync', 'value' => 'full'],
    ]);
...
$tests['vfs filecontrol current next68 readonly sync is ignored'] = static function (TestRunner $t): void {
    ...
    $pair = $state->currentNext68([['op' => 'sync', 'value' => 'full']])['pairs'][0];
...
$tests['vfs filecontrol current next68 memory sync is ignored'] = static function (TestRunner $t): void {
    $state = SQLiteVfsFileControlState::fromCapabilityPlan(SQLiteVfsCapabilityPlan::forFilename('file::memory:?mode=memory', false, true));
    $pair = $state->currentNext68([['op' => 'sync', 'value' => 'full']])['pairs'][0];
...
foreach ($phaseTwoCases as $name => $op) {
    $tests['vfs filecontrol current next68 ' . $name] = static function (TestRunner $t) use ($makeState, $op): void {
        $sequence = $makeState()->currentNext68([
            ['op' => $op],
            ['op' => $op],
        ]);
...
$tests['vfs filecontrol current next68 readonly commit phasetwo ignored'] = static function (TestRunner $t): void {
    $state = SQLiteVfsFileControlState::fromCapabilityPlan(SQLiteVfsCapabilityPlan::forFilename('file:/srv/www/wp-content/database/archive.sqlite?mode=ro', true, false));
    $pair = $state->currentNext68([['op' => 'commit_phasetwo']])['pairs'][0];
...
foreach ($writeHints as $name => [$argument, $expected, $changed]) {
    $tests['vfs filecontrol current next68 ' . $name] = static function (TestRunner $t) use ($makeState, $argument, $expected, $changed): void {
        $pair = $makeState()->currentNext68([['op' => 'write_hint', 'value' => $argument]])['pairs'][0];
...
$tests['vfs filecontrol current next68 write hint threads into next current'] = static function (TestRunner $t) use ($makeState): void {
    $sequence = $makeState()->currentNext68([
...
$tests['vfs filecontrol current next68 readonly write hint ignored'] = static function (TestRunner $t): void {
    $state = SQLiteVfsFileControlState::fromCapabilityPlan(SQLiteVfsCapabilityPlan::forFilename('file:/srv/www/wp-content/database/archive.sqlite?mode=ro', true, false));
    $pair = $state->currentNext68([['op' => 'write_hint', 'value' => 4096]])['pairs'][0];
...
foreach ($overwritePages as $name => [$page, $expected, $changed]) {
    $tests['vfs filecontrol current next68 ' . $name] = static function (TestRunner $t) use ($makeState, $page, $expected, $changed): void {
        $pair = $makeState()->currentNext68([['op' => 'overwrite', 'value' => $page]])['pairs'][0];
...
$tests['vfs filecontrol current next68 overwrite pages sort and deduplicate'] = static function (TestRunner $t) use ($makeState): void {
    $sequence = $makeState()->currentNext68([
...
$tests['vfs filecontrol current next68 readonly overwrite ignored'] = static function (TestRunner $t): void {
    $state = SQLiteVfsFileControlState::fromCapabilityPlan(SQLiteVfsCapabilityPlan::forFilename('file:/srv/www/wp-content/database/archive.sqlite?mode=ro', true, false));
    $pair = $state->currentNext68([['op' => 'overwrite', 'value' => 3]])['pairs'][0];
...
$tests['vfs filecontrol current next68 mixed pager commit sequence'] = static function (TestRunner $t) use ($makeState): void {
    $sequence = $makeState()->currentNext68([
...
$tests['vfs filecontrol current next68 mixed pager rollback sequence'] = static function (TestRunner $t) use ($makeState): void {
    $sequence = $makeState()->currentNext68([
        ['op' => 'begin_atomic_write'],
...
$tests['vfs filecontrol current next68 keeps existing current next64 controls'] = static function (TestRunner $t) use ($makeState): void {
    $sequence = $makeState(['size_limit' => 1024, 'reserve_bytes' => 4, 'lock_timeout' => 25])->currentNext68([
        ['op' => 'begin_atomic_write'],
    ]);
...
$tests['vfs filecontrol current next68 normalizes dash ops'] = static function (TestRunner $t) use ($makeState): void {
    $sequence = $makeState()->currentNext68([
...
$tests['vfs filecontrol current next68 counts notfound without state change'] = static function (TestRunner $t) use ($makeState): void {
    $sequence = $makeState()->currentNext68([
...
$errorCases = [
    'rejects invalid sync scalar' => static fn (SQLiteVfsFileControlState $state) => $state->currentNext68([['op' => 'sync', 'value' => 7]]),
    'rejects invalid sync flag' => static fn (SQLiteVfsFileControlState $state) => $state->currentNext68([['op' => 'sync', 'value' => 'invalid']]),
    'rejects negative write hint' => static fn (SQLiteVfsFileControlState $state) => $state->currentNext68([['op' => 'write_hint', 'value' => -1]]),
    'rejects zero overwrite page' => static fn (SQLiteVfsFileControlState $state) => $state->currentNext68([['op' => 'overwrite', 'value' => 0]]),
    'rejects negative overwrite page' => static fn (SQLiteVfsFileControlState $state) => $state->currentNext68([['op' => 'overwrite', 'value' => -4]]),
    'rejects non integer overwrite page' => static fn (SQLiteVfsFileControlState $state) => $state->currentNext68([['op' => 'overwrite', 'value' => '4']]),
    'rejects malformed current next68 item' => static fn (SQLiteVfsFileControlState $state) => $state->currentNext68([['value' => 1]]),
];

foreach ($errorCases as $name => $callback) {
    $tests['vfs filecontrol current next68 ' . $name] = static function (TestRunner $t) use ($makeState, $callback): void {
...
foreach ($constructorErrors as $name => $controls) {
    $tests['vfs filecontrol current next68 ' . $name] = static function (TestRunner $t) use ($makeState, $controls): void {
        $t->throws(InvalidArgumentException::class, static fn () => $makeState($controls));
    };
}

$tests['vfs filecontrol current next68 constructor accepts initial write hint'] = static function (TestRunner $t) use ($makeState): void {
    $snapshot = $makeState(['write_hint_bytes' => 8192])->snapshot();

    $t->same(8192, $snapshot['controls']['write_hint_bytes']);
};

$tests['vfs filecontrol current next68 constructor accepts initial counters'] = static function (TestRunner $t) use ($makeState): void {
...
$tests['vfs filecontrol current next68 wordpress rollback journal import summary'] = static function (TestRunner $t) use ($makeState): void {
    $sequence = $makeState(['size_limit' => 8388608])->currentNext68([
...
$tests['vfs filecontrol current next68 dependencies stay lane local'] = static function (TestRunner $t) use ($makeState): void {
    $dependencies = $makeState()->currentNext68([['op' => 'begin_atomic_write']])['dependencies'];

    $t->same(true, in_array('vfs-file-control-state', $dependencies, true));
    $t->same(true, in_array('vfs-xfilecontrol', $dependencies, true));
    $t->same(true, in_array('vfs-file-control-current-next68', $dependencies, true));
};
