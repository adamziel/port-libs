...
): array => SQLitePragmaForeignKeyIntegrityRootpageCurrentSourceNext140Plan::page(
    $currentDatabase,
    $schemas(),
    $currentCatalog,
...
$tests['pragma foreignkey integrity rootpage current source next140 rejects bad foreign key sql'] = static function (TestRunner $t) use ($currentDatabase, $nextDatabase, $schemas, $cleanSchemas, $currentCatalog, $nextCatalog): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLitePragmaForeignKeyIntegrityRootpageCurrentSourceNext140Plan::page(
        $currentDatabase,
        $schemas(),
        $currentCatalog,
        $nextDatabase,
