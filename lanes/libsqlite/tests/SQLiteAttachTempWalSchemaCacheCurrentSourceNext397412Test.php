...
$schemas397412 = [
    'main' => [
        ...
        'indexes' => ['wp_options_name', 'wp_posts_type_status_next293', 'wp_postmeta_key', 'wp_users_login_next308', 'wp_global_styles_slug', 'wp_navigation_menus_slug', 'wp_navigation_locations_menu', 'wp_navigation_aliases_slug'],
        'wal_frames' => [
            ['page' => 1, 'schema_cookie' => 382, 'commit' => true],
        ],
    ],
    'temp' => [
        'schema_cookie' => 396,
        'tables' => ['wp_theme_stage', 'wp_theme_stage_preview', 'wp_theme_stage_publish', 'wp_theme_stage_archive'],
        'indexes' => ['wp_theme_stage_stylesheet_next350', 'wp_theme_stage_preview_key_next366', 'wp_theme_stage_publish_key_next382', 'wp_theme_stage_archive_key'],
        'temp' => true,
    ],
    'analytics' => [
        'schema_cookie' => 394,
        'tables' => ['wp_eventmeta', 'wp_event_archive', 'wp_event_rollup_next362', 'wp_event_capacity_next394'],
        'indexes' => ['wp_eventmeta_key_next313', 'wp_event_capacity_day_next394'],
        'file' => '/srv/wp/analytics-next397.sqlite',
    ],
    'media' => [
        'schema_cookie' => 340,
        'tables' => ['wp_media', 'wp_media_metadata_next387'],
        'indexes' => ['wp_media_mime', 'wp_media_meta_key'],
        'file' => '/srv/wp/media-next397.sqlite',
    ],
    'queue' => [
        'schema_cookie' => 389,
        'tables' => ['wp_job_queue', 'wp_job_claims', 'wp_job_history', 'wp_job_deadletter', 'wp_job_retries', 'wp_job_retry_notes', 'wp_job_retry_audit'],
        'indexes' => ['wp_job_queue_token', 'wp_job_claims_token_next323', 'wp_job_history_finished_next339', 'wp_job_deadletter_reason_next360', 'wp_job_retries_job_next376', 'wp_job_retry_notes_job_next392', 'wp_job_retry_audit_job'],
        'file' => '/srv/wp/queue-next397.sqlite',
    ],
    'campaign' => [
        'schema_cookie' => 347,
        'tables' => [],
        'indexes' => ['wp_campaigns_slug'],
        'file' => '/srv/wp/campaign-next397.sqlite',
    ],
];
...
$plan397412 = static fn (array $events, ?array $statements = null, ?array $schemas = null): array => SQLiteAttachWalTempSchemaCacheCurrentSourceNext92Plan::currentSourceNext397412(
...
$tests['attach temp wal schema cache current source next397-412 extends next381-396'] = static function (TestRunner $t) use ($plan397412): void {
    ...
    $t->same('attach-wal-temp-schema-cache-current-source-next397-412', $result['operation']);
    $t->same('sqlite-attach-temp-wal-schema-cache-current-source-next397', $result['dependencies'][0]);
    $t->same('sqlite-attach-temp-wal-schema-cache-current-source-next412', $result['dependencies'][15]);
    $t->same('sqlite-attach-temp-wal-schema-cache-current-source-next396', $result['dependencies'][31]);
    $t->same(15, $result['event_count']);
    $t->same(['temp', 'main', 'analytics', 'campaign', 'media', 'queue'], $result['changed_schemas']);
    $t->same(398, $result['schema_cookies_next']['main']);
    $t->same(412, $result['schema_cookies_next']['temp']);
    $t->same(410, $result['schema_cookies_next']['analytics']);
    $t->same(341, $result['schema_cookies_next']['media']);
    $t->same(391, $result['schema_cookies_next']['queue']);
    $t->same(404, $result['schema_cookies_next']['campaign']);
    $t->same(['temp', 'main', 'analytics', 'campaign', 'media', 'queue'], $result['search_order_next']);
    $t->same(['alias-reader', 'archive-reader', 'queue-audit-reader'], $result['active_current_snapshot_statements']);
    ...
    $t->same(false, $result['statements']['media-meta-writer']['index_transitions'][0]['next_found']);
