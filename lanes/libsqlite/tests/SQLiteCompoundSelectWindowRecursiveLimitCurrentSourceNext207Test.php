...
$summary207 = static fn (?array $cursor = null): array => SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNextPlan::compareExceptBeforeIntersectWindowLimit($sql207, $currentTables207, $nextTables207, $cursor);
$tests = [];
...
$tests['compound select window recursive limit current source next207 rejects missing except'] = static function (TestRunner $t) use ($currentTables207): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNextPlan::compareExceptBeforeIntersectWindowLimit(
...
foreach (range(1, 58) as $case) {
    $tests['compound select window recursive limit current source next207 generated lead intersect boundary ' . $case] = static function (TestRunner $t) use ($case): void {
        ...
        $plan = SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNextPlan::compareExceptBeforeIntersectWindowLimit($sql, $tables, $nextTables);
        $again = SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNextPlan::compareExceptBeforeIntersectWindowLimit($sql, $tables, $nextTables, $plan['cursor']);
