...
$pageSize = 512;
$database = '/srv/wp-content/database/wp-foreign-key-fence.sqlite';
$usersDatabase = '/srv/wp-content/database/wp-foreign-key-fence-users.sqlite';
...
$before = [1 => $formatPage('foreign-key-fence stale schema'), 2 => $page('foreign-key-fence stale options'), 3 => $page('foreign-key-fence stale plugins'), 4 => $page('foreign-key-fence stale users')];
$recovered = [1 => $formatPage('foreign-key-fence current schema'), 2 => $page('foreign-key-fence current options'), 3 => $page('foreign-key-fence current plugins'), 4 => $page('foreign-key-fence current users')];
...
$base = [
    'source_id' => 'pager-reader-cache-foreign-key-fence',
    'epoch' => 306,
    'format_signature' => hash('sha256', implode('|', [512, 4, 2, 306, 0])),
    'publication_generation' => 306,
    'master_source_digest' => hash('sha256', 'master-foreign-key-fence'),
...
$plan = static function (?array $cache = null, ?array $reads = null) use ($database, $master, $masterBytes, $before, $pageSize, $recovered, $base, $tokens, $headers, $cacheEntry, $read): array {
    return SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::currentSourceReaderCacheForeignKeyFence($database, $master, $masterBytes, implode('', $before), $pageSize, $recovered, $cache ?? [
...
$cases = [
    'status' => [static fn (): mixed => $plan()['status'], 'pager-master-journal-reader-cache-current-source-foreign-key-fence'],
    ...
    'next291 transaction-state fence is current' => [static fn (): mixed => $plan()['current_reader_cache_transaction_state_token'], 'transaction-state-current-306'],
    'next292 commit-phase fence is current' => [static fn (): mixed => $plan()['current_reader_cache_commit_phase_token'], 'commit-phase-current-306'],
    'next293 busy-handler fence is current' => [static fn (): mixed => $plan()['current_reader_cache_busy_handler_token'], 'busy-handler-current-306'],
    ...
    'foreign-key-fence foreign-key ticket reopens reader' => [static fn (): mixed => $plan()['next_reads'][3]['reader_cache_foreign_key_token_reason'], 'reader_ticket_reader_cache_foreign_key_predates_current_source'],
    ...
    'dependencies include foreign-key-fence' => [static fn (): mixed => in_array('sqlite-pager-master-journal-reader-cache-current-source-foreign-key-fence', $plan()['dependencies'], true), true],
    ...
    'default plan points at foreign-key-fence' => [static fn (): mixed => $plan()['status'], 'pager-master-journal-reader-cache-current-source-foreign-key-fence'],
];

foreach ($cases as $name => [$callback, $expected]) {
    $tests['pager master journal reader cache foreign key fence ' . $name] = static function (TestRunner $t) use ($callback, $expected): void {
        $t->same($expected, $callback());
    };
}

$tests['pager master journal reader cache foreign key fence missing token rejects'] = static function (TestRunner $t) use ($plan, $cacheEntry, $recovered): void {
    $t->throws(Throwable::class, static fn () => $plan([1 => array_diff_key($cacheEntry('missing', $recovered[1]), ['reader_cache_foreign_key_token' => true])]));
};
