...
function libsqlite_suite_evidence78(): SQLiteUpstreamSuiteEvidence
{
    return SQLiteUpstreamSuiteEvidence::fromManifestPath(__DIR__ . '/../UPSTREAM_TEST_MANIFEST.json');
}

function libsqlite_suite_evidence78_output(int $passLines = 12, int $assertions = 104, int $failures = 0): string
{
    $lines = ['Focused test run: 1 selected test files (root lock skipped)'];
    for ($i = 1; $i <= $passLines; $i++) {
        $lines[] = sprintf('PASS current next78 suite evidence case %02d', $i);
    }
 ...
 */
function libsqlite_suite_evidence78_rows(
    string $head = 'c196709c053869bec78f15d5a1f299d396f8fdb0',
    int $case = 1
): array {
    return [
        [
            'unit' => 'suite-evidence-current-next78-focused-artifact',
            'tier' => 'focused',
            ...
            'scripts' => [sprintf('wal-current-next78-%02d.test', $case), 'attach3.test', 'wal2.test'],
            'exit' => 0,
            'errors' => 0,
            'current_tests' => 0,
            'next_tests' => 780 + $case,
            'evidence' => 'current-next78 records the next suite evidence slice after next77 without claiming release/all parity',
        ],
        [
            'unit' => 'suite-evidence-current-next78-next77-baseline',
            'tier' => 'focused',
            'repository_head' => $head,
            ...
            'runner_command' => './testfixture ../libsqlite/test/testrunner.tcl --jobs 1 --stop-on-error veryquick attach3.test wal2.test',
            'scripts' => ['attach-current-next77-07.test', 'attach3.test', 'wal2.test'],
            'exit' => 0,
            'errors' => 0,
            'current_tests' => 707,
            'next_tests' => 707,
            'evidence' => 'accepted current-next77 suite evidence remains preserved for current-next78',
        ],
    ];
}
 ...
 */
function libsqlite_suite_evidence78_record(
    array $rows,
    string $head = 'c196709c053869bec78f15d5a1f299d396f8fdb0',
    string $output = null,
    ?int $expected = 12,
...
return [
    'current next78 counts one bounded suite evidence row after next77' => static function (TestRunner $t): void {
        $record = libsqlite_suite_evidence78_record(libsqlite_suite_evidence78_rows(case: 8));

        $t->same('current-next78-suite-evidence-countable', $record['status']);
        $t->same(true, $record['countable']);
        $t->same(466, $record['current_mapped']);
        $t->same(467, $record['next_mapped']);
        $t->same(1, $record['mapped_delta']);
        $t->same(12, $record['php_pass_delta']);
        $t->same(29030, $record['next_php_pass']);
        $t->same(['suite-evidence-current-next78-focused-artifact'], $record['advanced_units']);
        $t->same(['suite-evidence-current-next78-next77-baseline'], $record['preserved_units']);
        $t->same(false, $record['counts_suite_evidence_current_next77']);
        $t->same(true, $record['counts_suite_evidence_current_next78']);
        $t->same(false, $record['counts_release_parity']);
    },
    'current next78 records tiers scripts and test delta' => static function (TestRunner $t): void {
        $record = libsqlite_suite_evidence78_record(libsqlite_suite_evidence78_rows(case: 4));

        $t->same(['focused' => 2], $record['tiers']);
        $t->same(1, $record['tier_count']);
        $t->same(784, $record['tests_total_delta']);
        $t->same([
            'attach-current-next77-07.test',
            'attach3.test',
            'wal-current-next78-04.test',
            'wal2.test',
        ], $record['target_scripts']);
    },
    'current next78 preserves already counted suite evidence' => static function (TestRunner $t): void {
        ...
        $t->same('current-next78-suite-evidence-preserved', $record['status']);
        $t->same(0, $record['mapped_delta']);
        $t->same(466, $record['next_mapped']);
        $t->same([
            'suite-evidence-current-next78-focused-artifact',
    ...
    'current next78 blocks bad runner evidence fields' => static function (TestRunner $t): void {
        $rows = libsqlite_suite_evidence78_rows();
    ...
    'current next78 blocks duplicate regression parity and active runner' => static function (TestRunner $t): void {
        $rows = libsqlite_suite_evidence78_rows();
        $rows[0]['counts_release_parity'] = true;
        $rows[1]['next_countable'] = false;
        $rows[] = $rows[0];

        $record = libsqlite_suite_evidence78_record($rows, snapshot: '123 testfixture ../libsqlite/test/testrunner.tcl --jobs 4 all');
        $evidence = implode('; ', array_column($record['blockers'], 'evidence'));
        ...
        $t->same(['suite-evidence-current-next78-next77-baseline'], $record['regressed_units']);
    },
    'current next78 blocks focused php admission mismatch' => static function (TestRunner $t): void {
        $record = libsqlite_suite_evidence78_record(
            libsqlite_suite_evidence78_rows(),
            output: libsqlite_suite_evidence78_output(passLines: 11),
            expected: 12
        );
    ...
    'current next78 records dependency closure without parity claim' => static function (TestRunner $t): void {
        $record = libsqlite_suite_evidence78_record(libsqlite_suite_evidence78_rows());

        $t->contains('current-next78 suite evidence', $record['dependency_closure']);
