...
use PortLibs\LibSqlite\SQLiteAttachWalTempTransactionCurrentNext69Plan;
...
$plan = static fn (?string $outcome = null, ?array $overrideSchemas = null, ?array $overrideOperations = null): array => SQLiteAttachWalTempTransactionCurrentNext69Plan::plan(
...
$predicateCases = [
    ...
    'wal schema cookie wins when no committed frame exists' => static function () use ($schemas, $operations): bool {
        ...
        return SQLiteAttachWalTempTransactionCurrentNext69Plan::plan($copy, [$operations[0]])['schemas']['main']['current_cookie'] === 44;
    },
    'uncommitted wal frame does not become current cookie' => static function () use ($schemas, $operations): bool {
        ...
        return SQLiteAttachWalTempTransactionCurrentNext69Plan::plan($copy, [$operations[3]])['schemas']['archive']['current_cookie'] === 10;
    },
    ...
    'quoted schema names normalize before lookup' => static function () use ($schemas): bool {
        $result = SQLiteAttachWalTempTransactionCurrentNext69Plan::plan($schemas, [
    ...
    'empty operations rejected' => static function () use ($schemas): bool {
        try {
            SQLiteAttachWalTempTransactionCurrentNext69Plan::plan($schemas, []);
            return false;
    ...
    'unknown outcome rejected' => static function () use ($schemas, $operations): bool {
        try {
            SQLiteAttachWalTempTransactionCurrentNext69Plan::plan($schemas, $operations, 'abort');
            return false;
        } catch (InvalidArgumentException) {
            return true;
        }
    ...
    'non integer schema cookie rejected' => static function () use ($schemas, $operations): bool {
        ...
        try {
            SQLiteAttachWalTempTransactionCurrentNext69Plan::plan($copy, [$operations[0]]);
            return false;
        } catch (InvalidArgumentException) {
            return true;
        }
