...
$plan108 = static fn (?array $prepared = null, ?array $states = null): array => SQLiteAttachTempWalSchemaTriggerPlan::triggerViewDependencyInvalidationPlan(
    $current108(),
    $next108(),
...
$cases108 = [
    'operation marker' => ['operation', 'attach-temp-trigger-view-dependency-invalidation'],
    'dependency marker first' => ['dependencies.0', 'sqlite-attach-temp-trigger-view-dependency-invalidation'],
...
foreach ($cases108 as $name => [$path, $expected]) {
    $tests['attach temp trigger view invalidation view cache reprepare108 ' . $name] = static function (TestRunner $t) use ($plan108, $value108, $path, $expected): void {
...
foreach ($predicates108 as $name => $predicate) {
    $tests['attach temp trigger view invalidation view cache reprepare108 ' . $name] = static function (TestRunner $t) use ($predicate): void {
        $t->same(true, $predicate());
    };
}

$tests['attach temp trigger view invalidation view cache reprepare108 rejects empty prepared list'] = static function (TestRunner $t) use ($current108, $next108): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteAttachTempWalSchemaTriggerPlan::triggerViewDependencyInvalidationPlan($current108(), $next108(), []));
};

$tests['attach temp trigger view invalidation view cache reprepare108 rejects missing trigger'] = static function (TestRunner $t) use ($current108, $next108): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteAttachTempWalSchemaTriggerPlan::triggerViewDependencyInvalidationPlan($current108(), $next108(), [['name' => 'missing']]));
};
