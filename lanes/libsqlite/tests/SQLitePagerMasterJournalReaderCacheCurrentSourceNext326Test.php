...
$pageSize = 512;
$database = '/srv/wp-content/database/wp-next326.sqlite';
$usersDatabase = '/srv/wp-content/database/wp-next326-users.sqlite';
...
$before = [1 => $formatPage('next326 stale schema'), 2 => $page('next326 stale options'), 3 => $page('next326 stale plugins'), 4 => $page('next326 stale users')];
$recovered = [1 => $formatPage('next326 current schema'), 2 => $page('next326 current options'), 3 => $page('next326 current plugins'), 4 => $page('next326 current users')];
$tokens = [$mainJournal => 'member-main-current-326', $usersJournal => 'member-users-current-326'];
$headers = [$mainJournal => hash('sha256', 'main header next326'), $usersJournal => hash('sha256', 'users header next326')];
$base = [
    'source_id' => 'pager-reader-cache-current-source-next326',
    'epoch' => 326,
    'format_signature' => hash('sha256', implode('|', [512, 4, 2, 326, 0])),
    'publication_generation' => 326,
    'master_source_digest' => hash('sha256', 'master-next326'),
...
$plan = static function (?array $cache = null, ?array $reads = null) use ($database, $master, $masterBytes, $before, $pageSize, $recovered, $base, $tokens, $headers, $cacheEntry, $read): array {
    return SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::variantNext326($database, $master, $masterBytes, implode('', $before), $pageSize, $recovered, $cache ?? [
...
$cases = [
    'status' => [static fn (): mixed => $plan()['status'], 'pager-master-journal-reader-cache-current-source-next326'],
    ...
    'dependencies include next326' => [static fn (): mixed => in_array('sqlite-pager-master-journal-reader-cache-current-source-next326', $plan()['dependencies'], true), true],
    ...
    'default plan points at next326' => [static fn (): mixed => $plan()['status'], 'pager-master-journal-reader-cache-current-source-next326'],
];

foreach ($cases as $name => [$callback, $expected]) {
    $tests['pager master journal reader cache current source next326 ' . $name] = static function (TestRunner $t) use ($callback, $expected): void {
        $t->same($expected, $callback());
    };
}

$tests['pager master journal reader cache current source next326 missing token rejects'] = static function (TestRunner $t) use ($plan, $cacheEntry, $recovered): void {
