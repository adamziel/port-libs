...
): array => SQLiteJsonTablePlan::currentSourceGeneratedHiddenRowidCostNext142(
    'json_tree',
    $current ?? $current142,
    $next ?? $next142,
    'option_value',
...
$tests = [
    ...
    'records next142 dependency' => static fn (TestRunner $t) => $t->true(in_array('sqlite-json-table-generated-hidden-rowid-cost-current-source-next142', $plan142()['dependencies'], true)),
    ...
    'stable has no next142 reasons' => static fn (TestRunner $t) => $t->same([], $stable142()['next142ReplanReasons']),
    ...
    'reasons do not invent generated count change' => static fn (TestRunner $t) => $t->same(false, in_array('json-table-generated-hidden-rowid-generated-count-changed', $plan142()['next142ReplanReasons'], true)),
    'reasons do not invent rowid tape change' => static fn (TestRunner $t) => $t->same(false, in_array('json-table-generated-hidden-rowid-tape-changed', $plan142()['next142ReplanReasons'], true)),
    'reasons preserve source json change' => static fn (TestRunner $t) => $t->true(in_array('source-json-changed', $plan142()['next142ReplanReasons'], true)),
    ...
    'jsonb next records source kind change' => static fn (TestRunner $t) => $t->true(in_array('source-json-kind-changed', $jsonb142()['next142ReplanReasons'], true)),
...
foreach ($tests as $name => $case) {
    $tests['json table generated hidden rowid cost current source next142 ' . $name] = $case;
    unset($tests[$name]);
}
