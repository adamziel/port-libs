...
foreach ($cases90 as $name => [$path, $expected]) {
    $tests['attach temp wal schema trigger source reprepare ' . $name] = static function (TestRunner $t) use ($plan90, $value90, $path, $expected): void {
...
foreach ($predicateCases90 as $name => $predicate) {
    $tests['attach temp wal schema trigger source reprepare ' . $name] = static function (TestRunner $t) use ($predicate): void {
...
foreach ($errorCases90 as $name => $callback) {
    $tests['attach temp wal schema trigger source reprepare ' . $name] = static function (TestRunner $t) use ($callback): void {
