...
$implicitLike234 = static fn (): array => SQLiteBlobLikeGlobAffinityCurrentSourceNext234Plan::wordpressOptionValuePlan(
    $currentRows234,
    $nextRows234,
    'plugin:%',
    'LIKE',
...
$castLike234 = static fn (): array => SQLiteBlobLikeGlobAffinityCurrentSourceNext234Plan::wordpressOptionValuePlan(
    $currentRows234,
    $nextRows234,
    'plugin:%',
    'LIKE',
...
$implicitGlob234 = static fn (): array => SQLiteBlobLikeGlobAffinityCurrentSourceNext234Plan::wordpressOptionValuePlan(
    $currentRows234,
    $nextRows234,
    'plugin:*',
    'GLOB',
...
$castGlob234 = static fn (): array => SQLiteBlobLikeGlobAffinityCurrentSourceNext234Plan::wordpressOptionValuePlan(
    $currentRows234,
    $nextRows234,
    'plugin:*',
    'GLOB',
...
$tests['blob like glob affinity current source next234 stable implicit scan is reusable'] = static function (TestRunner $t): void {
    ...
    $plan = SQLiteBlobLikeGlobAffinityCurrentSourceNext234Plan::wordpressOptionValuePlan($rows, $rows, 'plugin:%', 'LIKE', 'BINARY', null, false, false, 'stable', 'stable', 7, 7);
...
$tests['blob like glob affinity current source next234 stable cast scan admits blob'] = static function (TestRunner $t): void {
    ...
    $plan = SQLiteBlobLikeGlobAffinityCurrentSourceNext234Plan::wordpressOptionValuePlan($rows, $rows, 'plugin:%', 'LIKE', 'BINARY', null, false, true, 'stable', 'stable', 7, 7);
    $t->same(true, $plan['cursorReusable']);
...
$tests['blob like glob affinity current source next234 rejects bad operator'] = static function (TestRunner $t) use ($currentRows234): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteBlobLikeGlobAffinityCurrentSourceNext234Plan::wordpressOptionValuePlan($currentRows234, [], '%', 'REGEXP'));
};
...
$tests['blob like glob affinity current source next234 rejects glob escape'] = static function (TestRunner $t) use ($currentRows234): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteBlobLikeGlobAffinityCurrentSourceNext234Plan::wordpressOptionValuePlan($currentRows234, [], '*', 'GLOB', 'BINARY', '\\'));
};
...
$tests['blob like glob affinity current source next234 rejects bad collation'] = static function (TestRunner $t) use ($currentRows234): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteBlobLikeGlobAffinityCurrentSourceNext234Plan::wordpressOptionValuePlan($currentRows234, [], '%', 'LIKE', 'UNICODE'));
};

$tests['blob like glob affinity current source next234 rejects missing rowid'] = static function (TestRunner $t): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteBlobLikeGlobAffinityCurrentSourceNext234Plan::wordpressOptionValuePlan([['option_value' => 'plugin']], [], '%'));
};

$tests['blob like glob affinity current source next234 rejects missing value'] = static function (TestRunner $t): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteBlobLikeGlobAffinityCurrentSourceNext234Plan::wordpressOptionValuePlan([['option_id' => 1]], [], '%'));
};
...
$tests['blob like glob affinity current source next234 nonscalar value becomes malformed row'] = static function (TestRunner $t): void {
    $plan = SQLiteBlobLikeGlobAffinityCurrentSourceNext234Plan::wordpressOptionValuePlan([['option_id' => 1, 'option_value' => []]], [], '%');
