...
$nextView162['where'] = static fn (array $row, string $root, int $depth): bool => $depth <= 3 && str_contains((string) $row['key_name'], '_');
...
$run162 = static fn (array $options = [], ?array $firstRoots = null, ?array $secondRoots = null): array => SQLiteTriggerRecursiveViewReturningCurrentSourceNextPlan::executeStagedReturningSourceQueue(
    $rows162,
    [['root_name' => 'base_url']],
...
$emptySecond162 = static fn (): array => $run162([], null, [['root_name' => 'group_child']]);

$cases162 = [
    'held status' => [static fn (): mixed => $held162()['status'], 'trigger-recursive-view-returning-current-source-queue-held-next162'],
    'savepoint retained' => [static fn (): mixed => $held162()['savepoint'], 'app_recursive_view_next162'],
    'current generation retained' => [static fn (): mixed => $held162()['current_generation'], 'app-import-current-162'],
    'staged generations retained' => [static fn (): mixed => $held162()['staged_generations'], ['app-import-next-162-a', 'app-import-next-162-b']],
    'visible generation remains current while held' => [static fn (): mixed => $held162()['visible_generation'], 'app-import-current-162'],
    ...
    'first queue generation' => [static fn (): mixed => $held162()['next_source_queue'][0]['generation'], 'app-import-next-162-a'],
    'second queue generation' => [static fn (): mixed => $held162()['next_source_queue'][1]['generation'], 'app-import-next-162-b'],
    'first queue roots retained' => [static fn (): mixed => $held162()['next_source_queue'][0]['roots'], [['root_name' => 'theme_root']]],
    'second queue roots retained' => [static fn (): mixed => $held162()['next_source_queue'][1]['roots'], [['root_name' => 'module_root']]],
    ...
    'current first stage rows' => [static fn (): mixed => array_column($held162()['first_stage']['current_recursive_rows'], 'key_name'), ['theme_root', 'module_root', 'theme_child', 'module_child']],
    'current first stage depths' => [static fn (): mixed => array_column($held162()['first_stage']['current_recursive_rows'], '_depth'), [1, 1, 2, 2]],
    'first next attempted rows' => [static fn (): mixed => array_column($held162()['first_stage']['attempted_next_recursive_rows'], 'key_name'), ['theme_child']],
    'second next attempted rows' => [static fn (): mixed => array_column($held162()['second_stage']['attempted_next_recursive_rows'], 'key_name'), ['module_child', 'group_root', 'group_child']],
    ...
    'after savepoint restores base held' => [static fn (): mixed => array_column($held162()['after_savepoint'], 'key_name'), array_column($rows162, 'key_name')],
    'held visible keys current generation only' => [static fn (): mixed => array_values(array_unique(array_map(static fn (string $key): string => explode(':', $key, 2)[0], $held162()['returning_visibility']['visible']))), ['app-import-current-162']],
    'held suppressed keys both next generations' => [static fn (): mixed => array_values(array_unique(array_map(static fn (string $key): string => explode(':', $key, 2)[0], $held162()['returning_visibility']['suppressed']))), ['app-import-next-162-a', 'app-import-next-162-b']],
    ...
    'first release visible generation' => [static fn (): mixed => $firstRelease162()['visible_generation'], 'app-import-next-162-a'],
    ...
    'first release after savepoint includes first audit' => [static fn (): mixed => array_slice(array_column($firstRelease162()['after_savepoint'], 'key_name'), -1), ['audit:next:theme_root:theme_child']],
    ...
    'all release visible generation' => [static fn (): mixed => $allRelease162()['visible_generation'], 'app-import-next-162-b'],
    ...
    'all release after savepoint audit tail' => [static fn (): mixed => array_slice(array_column($allRelease162()['after_savepoint'], 'key_name'), -3), ['audit:next:module_root:module_child', 'audit:next:module_root:group_root', 'audit:next:module_root:group_child']],
    'all release visible generations in order' => [static fn (): mixed => array_values(array_unique(array_map(static fn (string $key): string => explode(':', $key, 2)[0], $allRelease162()['returning_visibility']['visible']))), ['app-import-current-162', 'app-import-next-162-a', 'app-import-next-162-b']],
    ...
    'limited current rows' => [static fn (): mixed => array_column($limited162()['first_stage']['current_recursive_rows'], 'key_name'), ['theme_root', 'module_root']],
    'limited second attempted rows' => [static fn (): mixed => array_column($limited162()['second_stage']['attempted_next_recursive_rows'], 'key_name'), ['module_child']],
