...
$makeFirstPage671686 = static function (int $pageCount): string {
...
$putPointerMapEntry671686 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database671686 = static function (int $sliceNumber) use ($makeFirstPage671686, $putPointerMapEntry671686): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage671686(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry671686($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan671686 = static function (int $sliceNumber, int $batchSize = 2) use ($database671686): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourcePlan {
    $database = $database671686($sliceNumber);
...
foreach ([671, 672, 673, 674, 675, 676, 677, 678, 679, 680, 681, 682, 683, 684, 685, 686] as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} publishes next671-686 follow-on handoff receipts"] = static function (TestRunner $t) use ($plan671686, $sliceNumber): void {
        $plan = $plan671686($sliceNumber);
        ...
        $t->same(6, $plan671686($sliceNumber, 3)->currentSourceSummary()['current_source_row_count']);
    };
}
