...
$plan138 = static function (
    ...
    string $pageType = "\x0d",
) use ($database138, $currentChains138, $deleteResults138, $payload138): SQLiteBTreePointerMapOverflowFreeblockCurrentSourceNext138Plan {
    return SQLiteBTreePointerMapOverflowFreeblockCurrentSourceNext138Plan::fromCurrentSourceDeleteResults(
...
$cases138 = [
    ...
    'final leaf fragment status' => static fn (): mixed => $afterHeader138()->freeblockCurrentNextFragmentReport($plan138()->databaseAfterAllocation->page(3))['status'],
    'final leaf current next fragments' => static fn (): mixed => $afterHeader138()->freeblockCurrentNextFragmentReport($plan138()->databaseAfterAllocation->page(3))['current_next_fragment_bytes'],
    ...
    'empty payload rejected' => static fn () => $throwsMessage138(static fn () => SQLiteBTreePointerMapOverflowFreeblockCurrentSourceNext138Plan::fromCurrentSourceDeleteResults($database138(), 3, $currentChains138(), $deleteResults138(), 3, '')),
    'bad parent rejected' => static fn () => $throwsMessage138(static fn () => SQLiteBTreePointerMapOverflowFreeblockCurrentSourceNext138Plan::fromCurrentSourceDeleteResults($database138(), 3, $currentChains138(), $deleteResults138(), 1, 'abc')),
    'trailing current chain rejected' => static function () use ($database138, $currentChains138, $deleteResults138, $payload138, $throwsMessage138): mixed {
        ...
        return $throwsMessage138(static fn () => SQLiteBTreePointerMapOverflowFreeblockCurrentSourceNext138Plan::fromCurrentSourceDeleteResults($database138(), 3, $chains, $deleteResults138(), 3, $payload138));
    },
];
