...
$makeFirstPage311314 = static function (int $pageCount): string {
...
$putPointerMapEntry311314 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database311314 = static function (int $sliceNumber) use ($makeFirstPage311314, $putPointerMapEntry311314): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage311314(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry311314($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan311314 = static function (int $sliceNumber, int $batchSize = 2) use ($database311314): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database311314($sliceNumber);
...
foreach ([311, 312, 313, 314] as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} keeps final next311-314 freelist splice isolated"] = static function (TestRunner $t) use ($plan311314, $sliceNumber): void {
        $plan = $plan311314($sliceNumber);
        ...
        $t->same(6, $plan311314($sliceNumber, 3)->freelistSummary()['freelist_row_count']);
    };
}
