...
$plan669684 = static fn (array $events, ?array $statements = null, ?array $schemas = null): array => SQLiteAttachWalTempSchemaCacheCurrentSourceNext92Plan::currentSourceNext669684(
...
$tests['attach temp wal schema cache current source next669-684 extends next653-668 handoff'] = static function (TestRunner $t) use ($plan669684): void {
    ...
    $t->same('attach-wal-temp-schema-cache-current-source-next669-684', $result['operation']);
    $t->same('sqlite-attach-temp-wal-schema-cache-current-source-next669', $result['dependencies'][0]);
    $t->same('sqlite-attach-temp-wal-schema-cache-current-source-next684', $result['dependencies'][15]);
    $t->same('sqlite-attach-temp-wal-schema-cache-current-source-next653', $result['dependencies'][16]);
    $t->same('sqlite-attach-temp-wal-schema-cache-current-source-next668', $result['dependencies'][31]);
