...
$makeFirstPage607622 = static function (int $pageCount): string {
...
$putPointerMapEntry607622 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database607622 = static function (int $sliceNumber) use ($makeFirstPage607622, $putPointerMapEntry607622): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage607622(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry607622($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan607622 = static function (int $sliceNumber, int $batchSize = 2) use ($database607622): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database607622($sliceNumber);
...
foreach ([607, 608, 609, 610, 611, 612, 613, 614, 615, 616, 617, 618, 619, 620, 621, 622] as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} publishes next607-622 handoff receipts"] = static function (TestRunner $t) use ($plan607622, $sliceNumber): void {
        $plan = $plan607622($sliceNumber);
        ...
        $t->same(6, $plan607622($sliceNumber, 3)->currentSourceSummary()['current_source_row_count']);
    };
}
