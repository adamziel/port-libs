...
use PortLibs\LibSqlite\SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNext224Plan;
...
$database224 = static function () use ($makeFirstPage224, $putPointerMapEntry224): SQLiteDatabase {
    ...
    $pages[3] = SQLiteTableLeafPage::assemble([
        ...
        SQLiteTableLeafCell::encode(2, SQLiteRecord::encode([null, '_transient_next224', str_repeat('cache:', 42)])),
...
$plan224 = static function (int $batchSize = 2): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNext224Plan {
    global $database224;

    $database = $database224();
    $deletedPage = SQLiteTableLeafPage::deleteCellByRowId($database->page(3), 2, secureDelete: true);

    return SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNext224Plan::tableLeafFromDeleteResult(
        $database,
        3,
        [
            'page' => $deletedPage,
        ...
        2,
        str_repeat('next224-current-source-cursor-', 50),
        3,
        true,
        $batchSize,
    );
...
$cases224 = [
    ...
    'current source token length' => static fn (): mixed => strlen($plan224()->sourceSummary()['current_source_next224_token']),
    ...
    'non overlap' => static fn (): mixed => str_contains($plan224()->sourceSummary()['non_overlap'], 'does not repeat next218'),
...
$expected224 = [
    'action label' => 'btree-vacuum-pointermap-freeblock-current-source-next224',
    'summary status' => 'btree-vacuum-pointermap-freeblock-current-source-next224-ready',
    'source row count' => 7,
    ...
    'dependency closure' => 'no new support component needed; next224 reuses next218 write receipts and adds current-source next-page cursor sequencing only',
    'non overlap' => true,
...
foreach ($cases224 as $name => $callback) {
    $tests['btree vacuum pointermap freeblock current source next224 ' . $name] = static function (TestRunner $t) use ($callback, $expected224, $name): void {
        $t->same($expected224[$name], $callback());
    };
}

foreach (range(1, 75) as $index) {
    $tests['btree vacuum pointermap freeblock current source next224 cursor invariant ' . $index] = static function (TestRunner $t) use ($plan224): void {
        $plan = $plan224();
        ...
        $t->same('btree-vacuum-pointermap-freeblock-current-source-next224-ready', $summary['status']);
