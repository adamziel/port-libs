...
): array => SQLiteJsonTablePlan::currentSourceGeneratedPathRowidCostCurrentSourceNext195(
    'json_tree',
    $current ?? $current195,
    $next ?? $next195,
    'option_value',
...
foreach ($tests as $name => $case) {
    $tests['json table generated path rowid cost current source next195 ' . $name] = $case;
    unset($tests[$name]);
}
