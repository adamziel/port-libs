...
$makeFirstPage323326 = static function (int $pageCount): string {
...
$putPointerMapEntry323326 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database323326 = static function (int $sliceNumber) use ($makeFirstPage323326, $putPointerMapEntry323326): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage323326(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry323326($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan323326 = static function (int $sliceNumber, int $batchSize = 2) use ($database323326): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database323326($sliceNumber);
...
foreach ([323, 324, 325, 326] as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} keeps final next323-326 freelist splice isolated"] = static function (TestRunner $t) use ($plan323326, $sliceNumber): void {
        $plan = $plan323326($sliceNumber);
        ...
        $t->same(6, $plan323326($sliceNumber, 3)->freelistSummary()['freelist_row_count']);
    };
}
