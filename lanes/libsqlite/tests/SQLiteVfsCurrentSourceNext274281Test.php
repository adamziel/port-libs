...
use PortLibs\LibSqlite\SQLiteVfsCurrentSourceNext274281Plan;
...
$plan = static function () use ($readyCurrent): array {
    static $result = null;
    if ($result === null) {
        $result = SQLiteVfsCurrentSourceNext274281Plan::run([
...
return [
    ...
    'vfs current source next274-281 blocks dirty claim' => static fn (TestRunner $t) => $t->same(true, in_array('dirty-pages-present', SQLiteVfsCurrentSourceNext274281Plan::run(['claim(reader-ready-next273,shared-cache-next273,reader-reuse-next281)'], ['current' => $dirtyCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source next274-281 blocks stale ack claim' => static fn (TestRunner $t) => $t->same(true, in_array('stale-reuse-ack', SQLiteVfsCurrentSourceNext274281Plan::run(['claim(reader-ready-next273,shared-cache-next273,reader-reuse-next281)'], ['current' => $staleAckCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source next274-281 blocks publish without claim' => static fn (TestRunner $t) => $t->same(true, in_array('missing-reuse-claim', SQLiteVfsCurrentSourceNext274281Plan::run(['publish(reader-ready-next273,reader-reuse-next281,shared-cache-next281)'], ['current' => $readyCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source next274-281 blocks stale claim' => static fn (TestRunner $t) => $t->same(true, in_array('stale-reuse-claim', SQLiteVfsCurrentSourceNext274281Plan::run(['publish(reader-ready-next273,reader-reuse-next281,shared-cache-next281)'], ['current' => $staleClaimCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source next274-281 accepts preclaimed publish' => static fn (TestRunner $t) => $t->same('published-reused-current-source', SQLiteVfsCurrentSourceNext274281Plan::run(['publish(reader-ready-next273,reader-reuse-next281,shared-cache-next281)'], ['current' => $claimedCurrent])['events'][0]['status']),
    'vfs current source next274-281 rejects empty operations' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteVfsCurrentSourceNext274281Plan::run([])),
    'vfs current source next274-281 rejects bad claim token' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteVfsCurrentSourceNext274281Plan::run([['op' => 'claim', 'snapshot' => 'reader-ready-next273', 'ack' => 'shared-cache-next273', 'claim' => 'bad claim']], ['current' => $readyCurrent])),
    'vfs current source next274-281 rejects unsupported operation' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteVfsCurrentSourceNext274281Plan::run(['republish(reader-ready-next273,shared-cache-next273)'], ['current' => $readyCurrent])),
