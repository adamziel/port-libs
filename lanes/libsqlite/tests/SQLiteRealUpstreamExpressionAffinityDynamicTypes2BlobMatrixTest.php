...
$oracle = static function (string $sql) use ($sqlite3): array {
    ...
    if ($sqlite3 === '') {
        throw new RuntimeException('sqlite3 oracle is required for real upstream expression affinity dynamic types2 blob matrix tests');
    }
...
// types2-4.*. The companion INTEGER/NUMERIC/TEXT matrix already covers the
// affinity columns. This shard owns the no-affinity `o XBLOBY` column so BLOB
// declaration semantics cannot silently collapse into TEXT or NUMERIC affinity.
foreach ($operators as $operator) {
    foreach ($literalPool as $literal) {
        ++$caseCount;
        $where = "o {$operator} {$literal}";
        $tests["real upstream expression affinity dynamic types2 blob matrix o {$operator} {$literal}"] = static function (TestRunner $t) use ($portRowids, $oracleRowids, $where): void {
            $t->same($oracleRowids($where), $portRowids($where), $where);
        };
    }
}

$tests['real upstream expression affinity dynamic types2 blob matrix owns 1248 rowset cases'] = static function (TestRunner $t) use ($operators, $literalPool, $caseCount): void {
    $t->same(8, count($operators));
    $t->same(156, count($literalPool));
    $t->same(1248, $caseCount);
    $t->same('types2.test: types2-2.* through types2-4.* no-affinity XBLOBY column comparisons', 'types2.test: types2-2.* through types2-4.* no-affinity XBLOBY column comparisons');
};
