...
use PortLibs\LibSqlite\SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext200Plan;
...
$summary200 = static fn (?array $cursor = null): array => SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext200Plan::compare($sql200, $currentTables200, $nextTables200, $cursor);
$tests = [];

$tests['compound select window recursive limit next200 status dependencies'] = static function (TestRunner $t) use ($summary200): void {
    $plan = $summary200();
    $t->same('compound-select-window-recursive-limit-current-source-next200-ready', $plan['status']);
    $t->same([
        'sqlite-select-sql-recursive-order-limit-offset-next200',
        'sqlite-window-rank-last-value-compound-next200',
        'sqlite-compound-union-distinct-except-final-limit-next200',
    ], $plan['dependencies']);
};

$tests['compound select window recursive limit next200 compound metadata'] = static function (TestRunner $t) use ($summary200): void {
...
$tests['compound select window recursive limit next200 recursive order trace'] = static function (TestRunner $t) use ($summary200): void {
...
$tests['compound select window recursive limit next200 window metadata'] = static function (TestRunner $t) use ($summary200): void {
...
$tests['compound select window recursive limit next200 current boundary'] = static function (TestRunner $t) use ($summary200): void {
...
$tests['compound select window recursive limit next200 next boundary shifts'] = static function (TestRunner $t) use ($summary200): void {
    $boundary = $summary200()['distinctExceptBoundary'];
...
$tests['compound select window recursive limit next200 cursor accepts current token'] = static function (TestRunner $t) use ($summary200): void {
    $first = $summary200();
...
$tests['compound select window recursive limit next200 rejects stale cursor'] = static function (TestRunner $t) use ($summary200): void {
...
$tests['compound select window recursive limit next200 replan reasons'] = static function (TestRunner $t) use ($summary200): void {
    $reasons = $summary200()['replanReasons'];
    $t->true(in_array('compound-union-distinct-except-window-recursive-limit-next200', $reasons, true));
    $t->true(in_array('recursive-order-limit-offset-before-window-next200', $reasons, true));
    $t->true(in_array('rank-last-value-before-compound-distinct-next200', $reasons, true));
    $t->true(in_array('wordpress-option-preview-distinct-except-boundary-next200', $reasons, true));
};

$tests['compound select window recursive limit next200 dependency closure and non overlap'] = static function (TestRunner $t) use ($summary200): void {
    $plan = $summary200();
    $t->contains('no new support component needed', $plan['dependency_closure']);
    $t->contains('avoids accepted next194', $plan['non_overlap']);
    $t->contains('UNION distinct plus EXCEPT', $plan['non_overlap']);
};

$tests['compound select window recursive limit next200 rejects missing union distinct'] = static function (TestRunner $t) use ($currentTables200): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext200Plan::compare(
...
$tests['compound select window recursive limit next200 rejects missing last value'] = static function (TestRunner $t) use ($currentTables200): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext200Plan::compare(
...
$tests['compound select window recursive limit next200 rejects missing recursive order'] = static function (TestRunner $t) use ($currentTables200): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext200Plan::compare(
...
foreach (range(1, 54) as $case) {
    $tests['compound select window recursive limit next200 generated union except fence ' . $case] = static function (TestRunner $t) use ($case): void {
        $tables = [
            'wp_options' => [
        ...
        $plan = SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext200Plan::compare($generatedSql, $tables, $nextTables);
        $again = SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext200Plan::compare($generatedSql, $tables, $nextTables, $plan['cursor']);
