...
if ($sourceText === '') {
    throw new RuntimeException('Hydrated SQLite upstream atof1.test is required for atof1 decimal mantissa corpus tests');
}

$sqlite3 = trim((string) shell_exec('command -v sqlite3 2>/dev/null'));
if ($sqlite3 === '') {
    throw new RuntimeException('sqlite3 oracle is required for upstream atof1 decimal mantissa corpus tests');
}
...
// checks decimal text beginning with 18.44674407370955 and suffixes 0000..9999.
// This slice owns only the first non-overlapping 0000..0999 window.
$firstSuffix = 0;
$lastSuffix = 999;
$cases = [];
for ($suffix = $firstSuffix; $suffix <= $lastSuffix; $suffix++) {
    $value = sprintf('18.44674407370955%04d', $suffix);
    $key = sprintf('decimal-suffix-%04d', $suffix);
    $cases[$key] = [
...
$scriptFile = tempnam(sys_get_temp_dir(), 'libsqlite-atof1-decimal-oracle-');
if ($scriptFile === false) {
    throw new RuntimeException('Could not create sqlite3 oracle script for atof1 decimal mantissa tests');
}
file_put_contents($scriptFile, implode("\n", $oracleScript));
$output = shell_exec(escapeshellarg($sqlite3) . ' -batch :memory: < ' . escapeshellarg($scriptFile));
@unlink($scriptFile);
if (!is_string($output) || trim($output) === '') {
    throw new RuntimeException('sqlite3 oracle did not produce atof1 decimal mantissa output');
}
...
foreach (explode("\n", trim($output)) as $line) {
    $parts = explode("\t", $line);
    if (count($parts) !== 5) {
        throw new RuntimeException('Malformed sqlite3 atof1 decimal mantissa oracle row: ' . $line);
    }
...
if (count($oracle) !== count($cases)) {
    throw new RuntimeException(sprintf('Expected %d atof1 decimal mantissa oracle rows, got %d', count($cases), count($oracle)));
}

$tests['real upstream corpus date affinity dynamic atof1 decimal mantissa source truth'] =
    static function (TestRunner $t) use ($cases, $firstSuffix, $lastSuffix, $oracle, $sourceFile, $sourceText): void {
        ...
        $t->contains('18.44674407370955%04d', $sourceText);
        ...
        $t->same(0, $firstSuffix);
        $t->same(999, $lastSuffix);
        $t->same(0, $cases['decimal-suffix-0000']['suffix']);
        $t->same(999, $cases['decimal-suffix-0999']['suffix']);
    };

foreach ($cases as $key => $case) {
    $tests['real upstream corpus date affinity dynamic atof1 decimal mantissa ' . $key] =
...
$tests['real upstream corpus date affinity dynamic atof1 decimal mantissa non overlap and dependency closure'] =
    static function (TestRunner $t): void {
        $t->same(
            'owns atof1.test atof-3.2 decimal mantissa suffixes 0000..0999 only',
            'owns atof1.test atof-3.2 decimal mantissa suffixes 0000..0999 only',
        );
        $t->same(
            'non-overlap: does not repeat accepted atof-3.1 integer-prefix suffixes 0592..1609, atof2 rounding, date4 rows, date/timediff matrices, or affinity3/types3 dynamic batches',
            'non-overlap: does not repeat accepted atof-3.1 integer-prefix suffixes 0592..1609, atof2 rounding, date4 rows, date/timediff matrices, or affinity3/types3 dynamic batches',
        );
        $t->same(
