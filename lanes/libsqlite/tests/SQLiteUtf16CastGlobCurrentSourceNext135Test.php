...
$plan = static fn (
    ...
    int $nextCookie = 135,
): array => SQLiteUtf16CastGlobCurrentSourceNextPlan::wordpressOptionValuePlan(
    $current ?? $currentRows,
    $next ?? $nextRows,
    $pattern,
    $currentSource,
...
$tests['utf16 cast glob current source nextOneThreeFive stable rows are reusable'] = static function (TestRunner $t) use ($enc): void {
    ...
    $plan = SQLiteUtf16CastGlobCurrentSourceNextPlan::wordpressOptionValuePlan($rows, $rows, 'plugin_*', 'stable', 'stable', 7, 7);
