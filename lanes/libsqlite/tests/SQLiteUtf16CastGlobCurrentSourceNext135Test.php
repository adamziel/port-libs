...
$plan = static fn (
    ...
    int $nextCookie = 135,
): array => SQLiteUtf16CastGlobCurrentSourceNext135Plan::wordpressOptionValuePlan(
    $current ?? $currentRows,
    $next ?? $nextRows,
    $pattern,
    $currentSource,
...
$tests['utf16 cast glob current source next135 stable rows are reusable'] = static function (TestRunner $t) use ($enc): void {
    ...
    $plan = SQLiteUtf16CastGlobCurrentSourceNext135Plan::wordpressOptionValuePlan($rows, $rows, 'plugin_*', 'stable', 'stable', 7, 7);
...
$tests['utf16 cast glob current source next135 rejects missing option id'] = static function (TestRunner $t) use ($nextRows): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteUtf16CastGlobCurrentSourceNext135Plan::wordpressOptionValuePlan([['option_value_bytes' => 'p', 'text_encoding' => 1]], $nextRows, 'p*'));
};

$tests['utf16 cast glob current source next135 rejects missing value bytes'] = static function (TestRunner $t) use ($nextRows): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteUtf16CastGlobCurrentSourceNext135Plan::wordpressOptionValuePlan([['option_id' => 1, 'text_encoding' => 1]], $nextRows, 'p*'));
};

$tests['utf16 cast glob current source next135 rejects missing encoding'] = static function (TestRunner $t) use ($nextRows): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteUtf16CastGlobCurrentSourceNext135Plan::wordpressOptionValuePlan([['option_id' => 1, 'option_value_bytes' => 'p']], $nextRows, 'p*'));
};
...
$tests['utf16 cast glob current source next135 rejects unsupported storage class'] = static function (TestRunner $t): void {
    ...
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteUtf16CastGlobCurrentSourceNext135Plan::wordpressOptionValuePlan($rows, $rows, 'p*'));
};
