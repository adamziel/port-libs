...
$plan = static fn (
    ...
    int $nextCookie = 129,
): array => SQLiteCastNocaseCurrentSourceNextPlan::wordpressOptionValuePlan(
    $current ?? $currentRows,
    $next ?? $nextRows,
    $castTarget,
    $pattern,
...
$tests['cast nocase current source next129 stable sources are reusable'] = static function (TestRunner $t): void {
    ...
    $plan = SQLiteCastNocaseCurrentSourceNextPlan::wordpressOptionValuePlan($rows, $rows, 'TEXT', 'plugin\\_cache%', '\\', 'stable', 'stable', 7, 7);
...
$tests['cast nocase current source next129 leading wildcard has no range'] = static function (TestRunner $t) use ($currentRows): void {
    $plan = SQLiteCastNocaseCurrentSourceNextPlan::wordpressOptionValuePlan($currentRows, $currentRows, 'TEXT', '%cache%', null, 'stable', 'stable', 7, 7);
...
$tests['cast nocase current source next129 rejects malformed cast target'] = static function (TestRunner $t) use ($currentRows): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteCastNocaseCurrentSourceNextPlan::wordpressOptionValuePlan($currentRows, $currentRows, 'TEXT); DROP TABLE wp_options; --', 'plugin%'));
};

$tests['cast nocase current source next129 rejects missing option id'] = static function (TestRunner $t): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteCastNocaseCurrentSourceNextPlan::wordpressOptionValuePlan([['option_value' => 'plugin']], [], 'TEXT', 'plugin%'));
};

$tests['cast nocase current source next129 rejects missing option value'] = static function (TestRunner $t): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteCastNocaseCurrentSourceNextPlan::wordpressOptionValuePlan([['option_id' => 1]], [], 'TEXT', 'plugin%'));
};

$tests['cast nocase current source next129 rejects non integer option id'] = static function (TestRunner $t): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteCastNocaseCurrentSourceNextPlan::wordpressOptionValuePlan([['option_id' => '1', 'option_value' => 'plugin']], [], 'TEXT', 'plugin%'));
};

$tests['cast nocase current source next129 rejects multi byte escape'] = static function (TestRunner $t) use ($currentRows): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteCastNocaseCurrentSourceNextPlan::wordpressOptionValuePlan($currentRows, $currentRows, 'TEXT', 'plugin!!_%', '!!'));
};
