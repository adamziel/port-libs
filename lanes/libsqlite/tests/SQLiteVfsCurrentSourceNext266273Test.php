...
use PortLibs\LibSqlite\SQLiteVfsCurrentSourceNext266273Plan;
...
$plan = static function () use ($readyCurrent): array {
    static $result = null;
    if ($result === null) {
        $result = SQLiteVfsCurrentSourceNext266273Plan::run([
...
return [
    ...
    'vfs current source next266-273 blocks dirty claim' => static fn (TestRunner $t) => $t->same(true, in_array('dirty-pages-present', SQLiteVfsCurrentSourceNext266273Plan::run(['claim(reader-ready-next265,shared-cache-next265,reader-reuse-next273)'], ['current' => $dirtyCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source next266-273 blocks stale ack claim' => static fn (TestRunner $t) => $t->same(true, in_array('stale-reuse-ack', SQLiteVfsCurrentSourceNext266273Plan::run(['claim(reader-ready-next265,shared-cache-next265,reader-reuse-next273)'], ['current' => $staleAckCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source next266-273 blocks publish without claim' => static fn (TestRunner $t) => $t->same(true, in_array('missing-reuse-claim', SQLiteVfsCurrentSourceNext266273Plan::run(['publish(reader-ready-next265,reader-reuse-next273,shared-cache-next273)'], ['current' => $readyCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source next266-273 blocks stale claim' => static fn (TestRunner $t) => $t->same(true, in_array('stale-reuse-claim', SQLiteVfsCurrentSourceNext266273Plan::run(['publish(reader-ready-next265,reader-reuse-next273,shared-cache-next273)'], ['current' => $staleClaimCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source next266-273 accepts preclaimed publish' => static fn (TestRunner $t) => $t->same('published-reused-current-source', SQLiteVfsCurrentSourceNext266273Plan::run(['publish(reader-ready-next265,reader-reuse-next273,shared-cache-next273)'], ['current' => $claimedCurrent])['events'][0]['status']),
    'vfs current source next266-273 rejects empty operations' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteVfsCurrentSourceNext266273Plan::run([])),
    'vfs current source next266-273 rejects bad claim token' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteVfsCurrentSourceNext266273Plan::run([['op' => 'claim', 'snapshot' => 'reader-ready-next265', 'ack' => 'shared-cache-next265', 'claim' => 'bad claim']], ['current' => $readyCurrent])),
    'vfs current source next266-273 rejects unsupported operation' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteVfsCurrentSourceNext266273Plan::run(['republish(reader-ready-next265,shared-cache-next265)'], ['current' => $readyCurrent])),
