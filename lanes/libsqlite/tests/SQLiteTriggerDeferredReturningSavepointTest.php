...
$foreignKey119 = ['parent_key' => 'post_id', 'child_key' => 'post_id', 'on_update' => 'cascade', 'deferred' => true];
$returning119 = [
    'old.post_id',
...
$cases119 = [
    'savepoint name is preserved' => [static fn (): mixed => $rolled119()['savepoint'], 'wp_import_batch'],
    ...
    'returning old ids survive rollback evidence' => [static fn (): mixed => array_column($rolled119()['returning_rows'], 'post_id'), [10, 20, 30]],
    'returning new ids survive rollback evidence' => [static fn (): mixed => array_column($rolled119()['returning_rows'], 'new_post_id'), [110, 120, 130]],
    ...
    'yielded rows carry savepoint name' => [static fn (): mixed => array_column($rolled119()['yielded'], 'savepoint'), ['wp_import_batch', 'wp_import_batch', 'wp_import_batch']],
    'after statement parent ids reflect recursive updates' => [static fn (): mixed => array_column($rolled119()['after_statement']['parent'], 'post_id'), [110, 120, 130]],
    'after statement child ids reflect cascades and audit orphan' => [static fn (): mixed => array_column($rolled119()['after_statement']['child'], 'post_id'), [110, 120, 130, 20]],
    ...
    'after savepoint restores parent ids' => [static fn (): mixed => array_column($rolled119()['after_savepoint']['parent'], 'post_id'), [10, 20, 30]],
    'after savepoint restores parent titles' => [static fn (): mixed => array_column($rolled119()['after_savepoint']['parent'], 'post_title'), ['Imported parent', 'Imported child', 'Imported leaf']],
    'after savepoint restores child ids' => [static fn (): mixed => array_column($rolled119()['after_savepoint']['child'], 'post_id'), [10, 20, 30]],
    'after savepoint removes trigger inserted audit child' => [static fn (): mixed => array_column($rolled119()['after_savepoint']['child'], 'meta_id'), [1, 2, 3]],
    ...
    'before parent image remains original' => [static fn (): mixed => array_column($rolled119()['before']['parent'], 'post_id'), [10, 20, 30]],
    'before child image remains original' => [static fn (): mixed => array_column($rolled119()['before']['child'], 'post_id'), [10, 20, 30]],
    ...
    'release path keeps updated parent ids' => [static fn (): mixed => array_column($released119()['after_savepoint']['parent'], 'post_id'), [110, 120, 130]],
    'release path keeps cascaded child ids' => [static fn (): mixed => array_column($released119()['after_savepoint']['child'], 'post_id'), [110, 120, 130, 20]],
    ...
    'non recursive savepoint statement updates only first parent' => [static fn (): mixed => array_column($nonRecursive119()['after_statement']['parent'], 'post_id'), [110, 20, 30]],
    'non recursive savepoint restores all parents' => [static fn (): mixed => array_column($nonRecursive119()['after_savepoint']['parent'], 'post_id'), [10, 20, 30]],
