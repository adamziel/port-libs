...
if ($sqlite3 === '') {
    throw new RuntimeException('sqlite3 oracle is required for real upstream expression affinity dynamic real expression tests');
}
...
$scriptFile = tempnam(sys_get_temp_dir(), 'libsqlite-real-expr-oracle-');
if ($scriptFile === false) {
...
if (!is_string($output) || trim($output) === '') {
    throw new RuntimeException('sqlite3 oracle did not produce real expression affinity output');
}
...
if (count($oracle) !== count($cases)) {
    throw new RuntimeException(sprintf('Expected %d sqlite3 real expression oracle rows, got %d', count($cases), count($oracle)));
}

foreach ($cases as $key => $case) {
    $tests['real upstream expression affinity dynamic real expr cast.test expr.test types3.test ' . $case['name']] = static function (TestRunner $t) use ($case, $key, $oracle): void {
        $expression = $case['expression'];
...
$tests['real upstream expression affinity dynamic real expr owns exactly 4224 dynamic cases'] = static function (TestRunner $t) use ($cases, $literals, $targets, $rightExpressions, $operators): void {
    $t->same(16, count($literals));
    $t->same(4, count($targets));
    $t->same(6, count($rightExpressions));
    $t->same(11, count($operators));
    $t->same(4224, count($cases));
    $t->same(
        'cast.test cast-1/cast-2 target conversion, expr.test arithmetic/comparison/null propagation, types3.test storage-class observation',
        'cast.test cast-1/cast-2 target conversion, expr.test arithmetic/comparison/null propagation, types3.test storage-class observation',
    );
};
