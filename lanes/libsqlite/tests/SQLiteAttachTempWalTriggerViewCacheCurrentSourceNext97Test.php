...
$plan97 = static fn (?array $prepared = null, ?array $states = null): array => SQLiteAttachTempWalSchemaTriggerPlan::triggerViewCacheCurrentSourceNext97(
    $current97(),
    $next97(),
    $prepared ?? $prepared97,
    $states ?? $states97(),
...
$tests['attach temp wal trigger view cache current source next97 rejects empty prepared list'] = static function (TestRunner $t) use ($current97, $next97): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteAttachTempWalSchemaTriggerPlan::triggerViewCacheCurrentSourceNext97($current97(), $next97(), []));
};
...
$tests['attach temp wal trigger view cache current source next97 rejects table trigger without view cache only when missing trigger'] = static function (TestRunner $t) use ($current97, $next97): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteAttachTempWalSchemaTriggerPlan::triggerViewCacheCurrentSourceNext97($current97(), $next97(), [['name' => 'missing_view_trigger']]));
};
