...
$master = $database . '-mj';
$sourceId = 'pager-reader-cache-current-source-provenance-next253';
$publication = 253;
$masterDigest = hash('sha256', 'next253-master-source');
...
$databaseToken = 'dev=8:ino=2539:size=5120:mtime=25399:snapshot=database-current';
...
$oldCleanupToken = 'master-cleanup:exists:mtime=25390:dirsync=pending';
$oldDatabaseToken = 'dev=8:ino=2539:size=5120:mtime=25398:snapshot=database-prior';
...
$before = [
    1 => $formatPage('next253 stale schema before source provenance recovery'),
    2 => $page('next253 stale wp_options root before source provenance recovery'),
    3 => $page('next253 stale active_plugins before source provenance recovery'),
    4 => $page('next253 stale usermeta before source provenance recovery'),
    5 => $page('next253 stale rewrite_rules before source provenance recovery'),
    6 => $page('next253 stale cron before source provenance recovery'),
    7 => $page('next253 stale comments before source provenance recovery'),
    8 => $page('next253 stale terms before source provenance recovery'),
    9 => $page('next253 stale options autoload statement before source provenance recovery'),
    10 => $page('next253 stale transient timeout before source provenance recovery'),
];
$recovered = [
    1 => $formatPage('next253 current schema after source provenance recovery'),
    2 => $page('next253 current wp_options root after source provenance recovery'),
    3 => $page('next253 current active_plugins after source provenance recovery'),
    4 => $page('next253 current usermeta after source provenance recovery'),
    6 => $page('next253 current cron after source provenance recovery'),
    9 => $page('next253 current options autoload statement after source provenance recovery'),
    10 => $page('next253 current transient timeout after source provenance recovery'),
];
...
$headers = [
    $mainJournal => hash('sha256', 'main-current-rollback-header-253'),
    $usersJournal => hash('sha256', 'users-current-rollback-header-253'),
];
$oldHeaders = [
    $mainJournal => $headers[$mainJournal],
    $usersJournal => hash('sha256', 'users-old-rollback-header-253'),
];
...
): array => SQLitePagerMasterJournalReaderCacheCurrentSourceNext253Plan::plan(
    $database,
    $master,
    $masterBytes,
    implode('', $before),
...
$cases = [
    'status' => [static fn (): mixed => $plan()['status'], 'pager-master-journal-reader-cache-current-source-next253'],
    'reason' => [static fn (): mixed => $plan()['reason'], 'master_journal_reader_cache_rechecks_database_header_change_counter_before_reuse'],
    ...
    'dependency next253' => [static fn (): mixed => in_array('sqlite-pager-master-journal-reader-cache-current-source-next253', $plan()['dependencies'], true), true],
    ...
    'provenance token embeds epoch' => [static fn (): mixed => str_contains($currentSourceProvenanceToken, 'epoch=253'), true],
    'provenance token embeds schema' => [static fn (): mixed => str_contains($currentSourceProvenanceToken, 'schema=103'), true],
];

foreach ($cases as $name => [$callback, $expected]) {
    $tests['pager master journal reader cache current source next253 ' . $name] = static function (TestRunner $t) use ($callback, $expected): void {
...
foreach ($throws as $name => $callback) {
    $tests['pager master journal reader cache current source next253 ' . $name] = static function (TestRunner $t) use ($callback): void {
