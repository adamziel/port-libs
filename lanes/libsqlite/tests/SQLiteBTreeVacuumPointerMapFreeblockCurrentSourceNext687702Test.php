...
$makeFirstPage687702 = static function (int $pageCount): string {
...
$putPointerMapEntry687702 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database687702 = static function (int $sliceNumber) use ($makeFirstPage687702, $putPointerMapEntry687702): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage687702(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry687702($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan687702 = static function (int $sliceNumber, int $batchSize = 2) use ($database687702): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database687702($sliceNumber);
...
foreach ([687, 688, 689, 690, 691, 692, 693, 694, 695, 696, 697, 698, 699, 700, 701, 702] as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} publishes next687-702 follow-on handoff receipts"] = static function (TestRunner $t) use ($plan687702, $sliceNumber): void {
        $plan = $plan687702($sliceNumber);
        ...
        $t->same(6, $plan687702($sliceNumber, 3)->currentSourceSummary()['current_source_row_count']);
    };
}
