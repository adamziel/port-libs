...
$base = [
    'source_id' => 'pager-reader-cache-current-source-next430',
    'epoch' => 430,
    'format_signature' => hash('sha256', implode('|', [512, 4, 2, 430, 0])),
    'publication_generation' => 430,
    'master_source_digest' => hash('sha256', 'master-next430'),
...
$plan = static fn (array $cacheExtra = [], array $readExtra = []): array => SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::readerCacheStatementFence(430,
    $database,
    $master,
    $masterBytes,
    implode('', $before),
...
$tests['pager master journal reader cache current source next430 admits current VDBE lifecycle fences'] = static function (TestRunner $t) use ($plan): void {
    $result = $plan();
    $t->same('pager-master-journal-reader-cache-current-source-next430', $result['status']);
    $t->same([], $result['invalidated_cache_page_numbers']);
    $t->same(['read-options' => true], $result['read_cache_hits']);
    $t->same('reader-cache-stmt-vdbe-rewind-current-430', $result['current_reader_cache_stmt_vdbe_rewind_token']);
};

$tests['pager master journal reader cache current source next430 invalidates stale rewind cache'] = static function (TestRunner $t) use ($plan, $opCount): void {
    $result = $plan(['reader_cache_stmt_vdbe_rewind_token' => 'stmt-vdbe-rewind-old']);
    $t->same([1], $result['reader_cache_stmt_vdbe_rewind_invalidated_cache_page_numbers']);
    $t->same([1], $result['invalidated_cache_page_numbers']);
    $t->same(1, $opCount($result, 'invalidate_reader_cache_reader_cache_stmt_vdbe_rewind_current_source_next430'));
};

$tests['pager master journal reader cache current source next430 reopens stale rewind read ticket'] = static function (TestRunner $t) use ($plan, $opCount): void {
    $result = $plan([], ['reader_cache_stmt_vdbe_rewind_token' => 'stmt-vdbe-rewind-old']);
    $t->same(['read-options'], $result['reopen_reader_ids']);
    $t->same(false, $result['next_reads'][0]['cache_hit']);
    $t->same('reader_ticket_reader_cache_stmt_vdbe_rewind_predates_current_source', $result['next_reads'][0]['reader_cache_stmt_vdbe_rewind_token_reason']);
    $t->same(1, $opCount($result, 'reopen_reader_for_reader_cache_stmt_vdbe_rewind_current_source_next430'));
};

$tests['pager master journal reader cache current source next430 missing rewind token rejects'] = static function (TestRunner $t) use ($plan): void {
    $t->throws(Throwable::class, static fn () => $plan(['reader_cache_stmt_vdbe_rewind_token' => null]));
};
