...
$tests['compound recursive affinity window current source next129 status and compound shape'] = static function (TestRunner $t) use ($summary): void {
    $plan = $summary();
    $t->same('compound-recursive-affinity-window-current-source-next129-ready', $plan['status']);
...
$tests['compound recursive affinity window current source next129 current rows preserve affinity representatives'] = static function (TestRunner $t) use ($summary): void {
...
$tests['compound recursive affinity window current source next129 next rows include next source'] = static function (TestRunner $t) use ($summary): void {
...
$tests['compound recursive affinity window current source next129 recursive trace records numeric duplicate skip'] = static function (TestRunner $t) use ($summary): void {
...
$tests['compound recursive affinity window current source next129 diagnostics identify windows and affinity'] = static function (TestRunner $t) use ($summary): void {
    $plan = $summary();
...
$tests['compound recursive affinity window current source next129 changed signatures name next plugin'] = static function (TestRunner $t) use ($summary): void {
    $plan = $summary();
...
foreach (range(1, 28) as $stop) {
    $tests['compound recursive affinity window current source next129 generated recursive stop ' . $stop] = static function (TestRunner $t) use ($stop): void {
        $tables = [
            'wp_options' => [
