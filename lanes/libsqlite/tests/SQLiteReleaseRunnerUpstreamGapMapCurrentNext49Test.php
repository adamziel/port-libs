...
function libsqlite_gap49_evidence(): SQLiteUpstreamSuiteEvidence
{
    return SQLiteUpstreamSuiteEvidence::fromManifestPath(__DIR__ . '/../UPSTREAM_TEST_MANIFEST.json');
}

function libsqlite_gap49_artifact(
    string $head,
    string $label,
    string $testset,
    array $patterns,
    ...
    return libsqlite_gap49_evidence()->boundedRunnerArtifactRecord($audit);
}

function libsqlite_gap49_focused_output(int $assertions = 64, int $failures = 0, int $files = 1): string
{
...
$focusedPath49 = 'lanes/libsqlite/tests/SQLiteReleaseRunnerUpstreamGapMapCurrentNext49Test.php';
...
return [
    'current next49 maps current release artifact to ready focused next targets' => static function (TestRunner $t) use ($currentHead49, $nextHead49, $focusedPath49, $nonOverlap49): void {
        $artifacts = [
            libsqlite_gap49_artifact($currentHead49, 'current-release', 'release', ['fts5aux.test'], 4096),
            libsqlite_gap49_artifact($currentHead49, 'current-json', 'veryquick', ['json101.test'], 48),
        ];
        $targets = [
        ...
        $record = libsqlite_gap49_evidence()->releaseRunnerUpstreamGapMapCurrentNext49(
            $artifacts,
            $targets,
            $currentHead49,
            $nextHead49,
            17920,
            $focusedPath49,
            libsqlite_gap49_focused_output(64),
            $nonOverlap49
        );
    ...
    'current next49 marks targets already covered by current focused artifacts' => static function (TestRunner $t) use ($currentHead49, $nextHead49, $focusedPath49, $nonOverlap49): void {
        $artifacts = [
            libsqlite_gap49_artifact($currentHead49, 'current-json', 'veryquick', ['json101.test', 'json102.test'], 54),
        ];
        $targets = [
        ...
        $record = libsqlite_gap49_evidence()->releaseRunnerUpstreamGapMapCurrentNext49(
            ...
            17920,
            $focusedPath49,
            libsqlite_gap49_focused_output(59),
            $nonOverlap49
        );
    ...
    'current next49 blocks release target without release tier baseline' => static function (TestRunner $t) use ($currentHead49, $nextHead49, $focusedPath49, $nonOverlap49): void {
        $artifacts = [
            libsqlite_gap49_artifact($currentHead49, 'current-focused', 'veryquick', ['json101.test'], 21),
        ];
        $targets = [
            'full-release-map' => [
                'testset' => 'release',
        ...
        $record = libsqlite_gap49_evidence()->releaseRunnerUpstreamGapMapCurrentNext49(
            ...
            17920,
            $focusedPath49,
            libsqlite_gap49_focused_output(55),
            $nonOverlap49
        );
    ...
    'current next49 blocks non hydrated next target scripts' => static function (TestRunner $t) use ($currentHead49, $nextHead49, $focusedPath49, $nonOverlap49): void {
        ...
        $record = libsqlite_gap49_evidence()->releaseRunnerUpstreamGapMapCurrentNext49(
            ...
            17920,
            $focusedPath49,
            libsqlite_gap49_focused_output(52),
            $nonOverlap49
        );
    ...
    'current next49 blocks duplicate broad runner snapshot' => static function (TestRunner $t) use ($currentHead49, $nextHead49, $focusedPath49, $nonOverlap49): void {
        ...
        $record = libsqlite_gap49_evidence()->releaseRunnerUpstreamGapMapCurrentNext49(
            ...
            17920,
            $focusedPath49,
            libsqlite_gap49_focused_output(51),
            $nonOverlap49,
    ...
    'current next49 blocks stale or failed current artifacts globally' => static function (TestRunner $t) use ($currentHead49, $nextHead49, $focusedPath49, $nonOverlap49): void {
        $artifacts = [
            libsqlite_gap49_artifact('stale-head', 'stale-release', 'release', ['fts5aux.test'], 4096),
        ];
        $targets = [
            'json-target' => [
                'testset' => 'veryquick',
        ...
        $record = libsqlite_gap49_evidence()->releaseRunnerUpstreamGapMapCurrentNext49(
            ...
            17920,
            $focusedPath49,
            libsqlite_gap49_focused_output(50),
            $nonOverlap49
        );
    ...
    'current next49 reports partial script coverage for mixed target' => static function (TestRunner $t) use ($currentHead49, $nextHead49, $focusedPath49, $nonOverlap49): void {
        ...
        $record = libsqlite_gap49_evidence()->releaseRunnerUpstreamGapMapCurrentNext49(
            ...
            17920,
            $focusedPath49,
            libsqlite_gap49_focused_output(49),
            $nonOverlap49
        );
    ...
    'current next49 validates required inputs' => static function (TestRunner $t) use ($currentHead49, $nextHead49, $focusedPath49, $nonOverlap49): void {
        $evidence = libsqlite_gap49_evidence();
        $artifact = libsqlite_gap49_artifact($currentHead49, 'current-release', 'release', ['fts5aux.test'], 4096);
        $targets = ['json' => ['testset' => 'veryquick', 'scripts' => ['json101.test']]];

        $t->throws(InvalidArgumentException::class, static fn () => $evidence->releaseRunnerUpstreamGapMapCurrentNext49([$artifact], $targets, '', $nextHead49, 1, $focusedPath49, libsqlite_gap49_focused_output(), $nonOverlap49));
        $t->throws(InvalidArgumentException::class, static fn () => $evidence->releaseRunnerUpstreamGapMapCurrentNext49([$artifact], $targets, $currentHead49, '', 1, $focusedPath49, libsqlite_gap49_focused_output(), $nonOverlap49));
        $t->throws(InvalidArgumentException::class, static fn () => $evidence->releaseRunnerUpstreamGapMapCurrentNext49([$artifact], [], $currentHead49, $nextHead49, 1, $focusedPath49, libsqlite_gap49_focused_output(), $nonOverlap49));
        $t->throws(InvalidArgumentException::class, static fn () => $evidence->releaseRunnerUpstreamGapMapCurrentNext49([$artifact], $targets, $currentHead49, $nextHead49, -1, $focusedPath49, libsqlite_gap49_focused_output(), $nonOverlap49));
    },
];
