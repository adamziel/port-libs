...
$nextRows144 = [
    ...
    ['import_id' => 22, 'name' => 'blogname', 'value' => 'Preview Only', 'autoload_flag' => 'no', 'origin' => 'preview-only'],
...
$plan144 = static fn (array $options = [], ?array $currentRows = null, ?array $nextRows = null, ?array $currentView = null, ?array $nextView = null, ?array $returning = null): array => SQLiteTriggerUpsertReturningViewCurrentSourceNextPlan::execute(
    ...
    $options + ['key' => 'option_name', 'savepoint' => 'wp_import_view_144', 'trigger' => 'wp_options_view_io_upsert_144'],
);
...
$cases144 = [
    ...
    'retained savepoint' => [static fn (): mixed => $retained144()['savepoint'], 'wp_import_view_144'],
    'retained trigger' => [static fn (): mixed => $retained144()['trigger'], 'wp_options_view_io_upsert_144'],
    'retained key' => [static fn (): mixed => $retained144()['key'], 'option_name'],
    ...
    'retained current columns' => [static fn (): mixed => $retained144()['current_view']['columns'], ['import_id', 'name', 'value', 'autoload_flag']],
    'retained next columns include origin' => [static fn (): mixed => $retained144()['next_view']['columns'], ['import_id', 'name', 'value', 'autoload_flag', 'origin']],
    'retained current mapping option name' => [static fn (): mixed => $retained144()['current_view']['mapping']['name'], 'option_name'],
    ...
    'retained current skipped row name' => [static fn (): mixed => $retained144()['current_skipped_rows'][0]['incoming_row']['option_name'], 'home'],
    'retained current skipped current row preserved' => [static fn (): mixed => $retained144()['current_skipped_rows'][0]['current_row']['option_value'], 'https://home.test'],
    ...
    'retained current rows include attempted fresh plugin' => [static fn (): mixed => array_column($retained144()['current_rows'], 'option_name'), ['siteurl', 'home', 'blogname', 'fresh_plugin']],
    'retained current siteurl attempted value' => [static fn (): mixed => $retained144()['current_rows'][0]['option_value'], 'https://current.test'],
    'retained current home skipped value' => [static fn (): mixed => $retained144()['current_rows'][1]['option_value'], 'https://home.test'],
    'retained after savepoint restores base names' => [static fn (): mixed => array_column($retained144()['after_savepoint'], 'option_name'), ['siteurl', 'home', 'blogname']],
    'retained after savepoint restores siteurl value' => [static fn (): mixed => $retained144()['after_savepoint'][0]['option_value'], 'https://old.test'],
    ...
    'retained attempted next skipped name' => [static fn (): mixed => $retained144()['attempted_next_skipped_rows'][0]['incoming_row']['option_name'], 'blogname'],
    ...
    'released final names' => [static fn (): mixed => array_column($released144()['after_savepoint'], 'option_name'), ['siteurl', 'home', 'blogname', 'fresh_plugin', 'rewrite_rules']],
    ...
    'released final home updated by next' => [static fn (): mixed => $released144()['after_savepoint'][1]['option_value'], 'https://next-home.test'],
    'released blogname skipped by next source where' => [static fn (): mixed => $released144()['after_savepoint'][2]['option_value'], 'Old Blog'],
    ...
    'custom savepoint accepted' => [static fn (): mixed => $plan144(['savepoint' => 'wp_custom_view_144'])['savepoint'], 'wp_custom_view_144'],
