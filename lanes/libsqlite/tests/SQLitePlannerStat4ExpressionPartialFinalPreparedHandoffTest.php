...
$tests = [
    ...
    'planner stat4 expression partial current source finalPreparedHandoff inherits terminalPreparedHandoff' => static fn (TestRunner $t) => $t->same(true, $planfinalPreparedHandoff()['selectedPlan']['terminalPreparedHandoffPrepared']),
    ...
    'planner stat4 expression partial current source finalPreparedHandoff canonical fence alias' => static fn (TestRunner $t) => $t->same($planfinalPreparedHandoff()['stat4FinalPreparedHandoffPreparationFence'], $planfinalPreparedHandoff()['stat4FinalPreparedHandoffFence']),
    ...
    'planner stat4 expression partial current source finalPreparedHandoff canonical selected key' => static fn (TestRunner $t) => $t->same('stat4FinalPreparedHandoffFence', $planfinalPreparedHandoff()['selectedPlan']['finalPreparedHandoffCanonicalFenceKey']),
    'planner stat4 expression partial current source finalPreparedHandoff canonical stat4 key' => static fn (TestRunner $t) => $t->same('stat4FinalPreparedHandoffFence', $planfinalPreparedHandoff()['stat4Fence']['finalPreparedHandoffCanonicalFenceKey']),
    'planner stat4 expression partial current source finalPreparedHandoff terminal canonical selected key' => static fn (TestRunner $t) => $t->same('stat4TerminalPreparedHandoffFence', $planfinalPreparedHandoff()['selectedPlan']['terminalPreparedHandoffCanonicalFenceKey']),
    'planner stat4 expression partial current source finalPreparedHandoff terminal canonical stat4 key' => static fn (TestRunner $t) => $t->same('stat4TerminalPreparedHandoffFence', $planfinalPreparedHandoff()['stat4Fence']['terminalPreparedHandoffCanonicalFenceKey']),
    'planner stat4 expression partial current source finalPreparedHandoff terminal canonical fence alias' => static fn (TestRunner $t) => $t->same($planfinalPreparedHandoff()['stat4TerminalPreparedHandoffPreparationFence'], $planfinalPreparedHandoff()['stat4TerminalPreparedHandoffFence']),
    'planner stat4 expression partial current source finalPreparedHandoff omits legacy numbered terminal alias' => static fn (TestRunner $t) => $t->same(false, array_key_exists('stat4Next958973PreparationFence', $planfinalPreparedHandoff())),
    'planner stat4 expression partial current source finalPreparedHandoff omits legacy numbered terminal selected flag' => static fn (TestRunner $t) => $t->same(false, array_key_exists('next958973Prepared', $planfinalPreparedHandoff()['selectedPlan'])),
    ...
    'planner stat4 expression partial current source finalPreparedHandoff preserves finalPreparedHandoff fence' => static fn (TestRunner $t) => $t->same(range(958, 973), $planfinalPreparedHandoff()['stat4TerminalPreparedHandoffPreparationFence']['preparedSlices']),
    ...
    'planner stat4 expression partial current source finalPreparedHandoff omits dynamic legacy cursor opcode' => static fn (TestRunner $t) => $t->same(false, array_key_exists('legacyOpcode', $planfinalPreparedHandoff()['cursorProgram'][array_key_last($planfinalPreparedHandoff()['cursorProgram'])])),
    'planner stat4 expression partial current source finalPreparedHandoff omits dynamic legacy cursor mode' => static fn (TestRunner $t) => $t->same(false, array_key_exists('legacyMode', $planfinalPreparedHandoff()['cursorProgram'][array_key_last($planfinalPreparedHandoff()['cursorProgram'])])),
