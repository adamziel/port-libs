...
$plan = static fn (
    ...
    int $nextCookie = 133,
): array => SQLiteCastLikeGlobAffinityCurrentSourceNextPlan::optionRowValuePlan(
    $current ?? $currentRows,
    $next ?? $nextRows,
    $castTarget,
    $pattern,
...
$tests['cast like glob affinity current source next133 stable sources are reusable'] = static function (TestRunner $t): void {
    ...
    $plan = SQLiteCastLikeGlobAffinityCurrentSourceNextPlan::optionRowValuePlan($rows, $rows, 'TEXT', 'plugin:%', 'LIKE', null, 'stable', 'stable', 7, 7);
...
$tests['cast like glob affinity current source next133 stable leading wildcard keeps no prefix reason'] = static function (TestRunner $t): void {
    ...
    $plan = SQLiteCastLikeGlobAffinityCurrentSourceNextPlan::optionRowValuePlan($rows, $rows, 'TEXT', '%alpha', 'LIKE', null, 'stable', 'stable', 7, 7);
...
$tests['cast like glob affinity current source next133 stable glob leading class keeps no prefix reason'] = static function (TestRunner $t): void {
    $rows = [['option_id' => 1, 'option_value' => 'plugin:alpha']];
    $plan = SQLiteCastLikeGlobAffinityCurrentSourceNextPlan::optionRowValuePlan($rows, $rows, 'TEXT', '[Pp]lugin:*', 'GLOB', null, 'stable', 'stable', 7, 7);
    $t->same(null, $plan['range']);
    $t->same([], $plan['currentRowids']);
    $t->same(['no-prefix-range'], $plan['invalidationReasons']);
};
...
$tests['cast like glob affinity current source next133 rejects malformed cast target'] = static function (TestRunner $t) use ($currentRows): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteCastLikeGlobAffinityCurrentSourceNextPlan::optionRowValuePlan($currentRows, $currentRows, 'TEXT); DROP TABLE wp_options; --', 'plugin:%'));
};
...
$tests['cast like glob affinity current source next133 rejects unsupported operator'] = static function (TestRunner $t) use ($currentRows): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteCastLikeGlobAffinityCurrentSourceNextPlan::optionRowValuePlan($currentRows, $currentRows, 'TEXT', 'plugin:%', 'REGEXP'));
};
...
$tests['cast like glob affinity current source next133 rejects glob escape'] = static function (TestRunner $t) use ($currentRows): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteCastLikeGlobAffinityCurrentSourceNextPlan::optionRowValuePlan($currentRows, $currentRows, 'TEXT', 'plugin:*', 'GLOB', '!'));
};

$tests['cast like glob affinity current source next133 rejects missing option id'] = static function (TestRunner $t): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteCastLikeGlobAffinityCurrentSourceNextPlan::optionRowValuePlan([['option_value' => 'plugin']], [], 'TEXT', 'plugin:%'));
};

$tests['cast like glob affinity current source next133 rejects missing option value'] = static function (TestRunner $t): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteCastLikeGlobAffinityCurrentSourceNextPlan::optionRowValuePlan([['option_id' => 1]], [], 'TEXT', 'plugin:%'));
};

$tests['cast like glob affinity current source next133 rejects non integer option id'] = static function (TestRunner $t): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteCastLikeGlobAffinityCurrentSourceNextPlan::optionRowValuePlan([['option_id' => '1', 'option_value' => 'plugin']], [], 'TEXT', 'plugin:%'));
};
...
$tests['cast like glob affinity current source next133 rejects multi byte escape'] = static function (TestRunner $t) use ($currentRows): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteCastLikeGlobAffinityCurrentSourceNextPlan::optionRowValuePlan($currentRows, $currentRows, 'TEXT', 'plugin!!:%', 'LIKE', '!!'));
};
