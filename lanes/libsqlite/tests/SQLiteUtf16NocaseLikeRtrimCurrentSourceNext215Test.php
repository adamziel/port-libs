...
$current215 = [
    $row215(1, 'Module_Cache', 'UTF-16LE'),
    $row215(2, "module_cache\0shadow", 'UTF-16BE'),
    $row215(3, 'module_cache ', 'UTF-16LE'),
    $row215(4, "module_cache\0Shadow", 'UTF-8'),
    $row215(5, 'module_cache_extra', 'UTF-16BE'),
    $row215(6, 'module_case', 'UTF-16LE'),
    $row215(7, "module_cache\0", 'UTF-16LE'),
    $bad215(8, "\x00\xd8", 2),
];
$nextTwoOneFive = [
    $row215(1, 'Module_Cache', 'UTF-16BE'),
    $row215(2, "module_cache\0shadow", 'UTF-16LE'),
    $row215(3, 'module_cache', 'UTF-16BE'),
    $row215(4, "module_cache\0Shadow", 'UTF-8'),
    $row215(5, 'module_cache_extra', 'UTF-16LE'),
    $row215(7, "module_cache\0", 'UTF-16BE'),
    $row215(9, "module_cache\0later", 'UTF-16LE'),
    $row215(10, 'MODULE_CACHE_NEW', 'UTF-16BE'),
    $bad215(11, "\x00\xd8", 2),
];
...
$plan215 = static fn (
    ?array $current = null,
    ?array $next = null,
    ?array $token = ['key' => "module_cache\0shadow", 'rowid' => 4],
...
$cases215 = [
    ...
    'current nul text' => ['currentRtrimTexts.2', "module_cache\0shadow"],
    'next nul text' => ['nextRtrimTexts.9', "module_cache\0later"],
    'current nul key hex' => ['currentNocaseKeysHex.2', '6d6f64756c655f636163686500736861646f77'],
    'next nul key hex' => ['nextNocaseKeysHex.9', '6d6f64756c655f6361636865006c61746572'],
    'current truncated key' => ['currentTruncatedNocaseKeys.2', 'module_cache'],
    'next truncated key' => ['nextTruncatedNocaseKeys.9', 'module_cache'],
    'token key' => ['resumeToken.key', "module_cache\0shadow"],
...
$tests['utf16 nocase like rtrim current source nextTwoOneFive stable embedded nul token can resume by full key'] = static function (TestRunner $t) use ($row215): void {
    $rows = [
        $row215(1, 'module_cache', 'UTF-16LE'),
        $row215(2, "module_cache\0shadow", 'UTF-16BE'),
        $row215(3, 'module_cache_extra', 'UTF-16LE'),
    ];
...
$tests['utf16 nocase like rtrim current source nextTwoOneFive canonicalizes resume token before comparing'] = static function (TestRunner $t) use ($row215): void {
    $rows = [
        $row215(1, "Module_Cache\0Shadow", 'UTF-16LE'),
        $row215(2, 'module_cache_zip', 'UTF-16BE'),
    ];
    $result = SQLiteUtf16NocaseLikeRtrimCurrentSourceNextPlan::keyValueRowKeyEmbeddedNulTokenPlan(
        $rows,
        $rows,
        'module!_cache%',
        '!',
        ['key' => "MODULE_CACHE\0SHADOW  ", 'rowid' => 1],
        'stable',
        'stable',
        215,
        215,
    ...
    $t->same("module_cache\0shadow", $result['resumeToken']['key']);
...
$tests['utf16 nocase like rtrim current source nextTwoOneFive null token replays full next range'] = static function (TestRunner $t) use ($row215): void {
    $rows = [
        $row215(1, 'module_cache', 'UTF-16LE'),
        $row215(2, "module_cache\0later", 'UTF-16BE'),
        $row215(3, 'layout_cache', 'UTF-8'),
    ];
    $result = SQLiteUtf16NocaseLikeRtrimCurrentSourceNextPlan::keyValueRowKeyEmbeddedNulTokenPlan(
        $rows,
        $rows,
