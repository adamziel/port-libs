...
$plan99 = static fn (array $constraints = null): array => SQLiteJsonTablePlan::currentSourceRowidHiddenConstraintPlannerNext99(
    'json_tree',
    $current99,
    $next99,
    'option_value',
...
$tests = [
    ...
    'dependency marker is appended' => static fn (TestRunner $t) => $t->true(in_array('sqlite-json-table-rowid-hidden-constraint-current-source-next99', $plan99()['dependencies'], true)),
    ...
    'next99 reasons include next94 reasons' => static fn (TestRunner $t) => $t->true(in_array('hidden-rowid-residual-constraint-present', $plan99()['next99ReplanReasons'], true)),
    'next99 reasons include source json changed' => static fn (TestRunner $t) => $t->true(in_array('source-json-changed', $plan99()['next99ReplanReasons'], true)),
    'next99 reasons do not invent alias change for stable alias' => static fn (TestRunner $t) => $t->same(false, in_array('hidden-rowid-alias-provenance-changed', $plan99()['next99ReplanReasons'], true)),
    ...
    'bad function is still rejected' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteJsonTablePlan::currentSourceRowidHiddenConstraintPlannerNext99('json_bad', $current99, $next99, 'option_value')),
    'missing current json column is still rejected' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteJsonTablePlan::currentSourceRowidHiddenConstraintPlannerNext99('json_tree', [], $next99, 'option_value')),
];

foreach ($tests as $name => $case) {
    $tests['json table rowid hidden constraint current source next99 ' . $name] = $case;
    unset($tests[$name]);
}
