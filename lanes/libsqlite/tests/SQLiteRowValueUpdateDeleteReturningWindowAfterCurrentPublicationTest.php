...
$cases = [
    'next257 tombstone publication candidate' => static function (TestRunner $t) use ($examplesDir): void {
        $result = require $examplesDir . '/application-rowvalue-returning-window-delete-retry-publication.php';
    ...
    'next258 transition token admission candidate' => static function (TestRunner $t) use ($examplesDir): void {
        $result = require $examplesDir . '/application-rowvalue-returning-window-publication-transition-admission.php';
    ...
    'next259 current row frame acknowledgement candidate' => static function (TestRunner $t) use ($examplesDir): void {
        ob_start();
        $argv = [];
        require $examplesDir . '/application-rowvalue-update-delete-returning-window-current-row-frame-admission.php';
    ...
    'next260 mixed boundary release candidate' => static function (TestRunner $t) use ($examplesDir): void {
        ob_start();
        $result = require $examplesDir . '/application-rowvalue-returning-window-frame-boundary-admission.php';
        $output = trim((string) ob_get_clean());

        $t->same(1, $result);
        $t->same('application-rowvalue-returning-window-frame-boundary-admission self-test passed', $output);
    },
    'combined after-current handoff' => static function (TestRunner $t) use ($examplesDir): void {
        $result = require $examplesDir . '/application-rowvalue-returning-window-after-current-publication.php';
        ...
        $t->same('application-rowvalue-returning-window-frame-boundary-admission self-test passed', $result['next260SelfTestOutput']);
    },
];
