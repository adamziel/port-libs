...
$pageSize = 512;
$sectorSize = 512;
$databasePath = '/srv/www/wp-content/database/.ht.sqlite-next122';
...
$clean1 = $page('next122 clean schema before interrupted import');
$clean2 = $page('next122 clean wp_options before interrupted import');
$clean3 = $page('next122 clean autoload index before interrupted import');
$clean4 = $page('next122 clean transients before interrupted import');
$dirty1 = $page('next122 dirty schema from interrupted import');
$dirty2 = $page('next122 dirty wp_options from interrupted import');
$dirty3 = $page('next122 dirty autoload index from interrupted import');
$dirty4 = $page('next122 dirty transients from interrupted import');
...
$walBytes = $makeWal([
    [2, 4, 'next122 wal committed siteurl after hot recovery'],
    [3, 0, 'next122 wal draft plugin autoload'],
    [4, 4, 'next122 wal committed transient cleanup'],
    [2, 4, 'next122 wal committed option retry tail'],
]);
...
$cases = [
    'status' => [static fn (): mixed => $restart()['status'], 'wal-checkpoint-hot-journal-reader-current-source-next122'],
    ...
    'row one hot label' => [static fn (): mixed => $restart()['rows'][0]['hot_current_label'], 'next122 clean schema before interrupted import'],
    'row one dirty label' => [static fn (): mixed => $restart()['rows'][0]['dirty_label'], 'next122 dirty schema from interrupted import'],
    'row two reader label' => [static fn (): mixed => $restart()['rows'][1]['reader_label'], 'next122 wal committed siteurl after hot recovery'],
    'row three reader label' => [static fn (): mixed => $restart()['rows'][2]['reader_label'], 'next122 clean autoload index before interrupted import'],
    'row four released label' => [static fn (): mixed => $restart()['rows'][3]['released_next_label'], 'next122 wal committed transient cleanup'],
    ...
    'dependency next122' => [static fn (): mixed => in_array('sqlite-wal-checkpoint-hot-journal-reader-current-source-next122', $restart()['dependencies'], true), true],
    ...
    'reserved blocked status' => [static fn (): mixed => $reserved()['status'], 'wal-checkpoint-hot-journal-reader-current-source-blocked-next122'],
    ...
    'present super status' => [static fn (): mixed => $presentSuper()['status'], 'wal-checkpoint-hot-journal-reader-current-source-next122'],
];

foreach ($cases as $name => [$actual, $expected]) {
    $tests['wal checkpoint hot journal reader current source next122 ' . $name] = static function (TestRunner $t) use ($actual, $expected): void {
...
foreach ($throws as $name => $callback) {
    $tests['wal checkpoint hot journal reader current source next122 ' . $name] = static function (TestRunner $t) use ($callback): void {
