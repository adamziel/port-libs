...
$oldDatabaseDigest = $digest('wp next210 old database pages');
$oldWalDigest = $digest('wp next210 old wal source');
$oldConsumerDigest = $digest('wp next210 old consumer source');
$hotJournalDigest = $digest('wp next210 stale hot journal');
$pageDigests = [
...
$writerPlan = [
    ...
    'database_path' => '/srv/www/wp-content/database/wp-next210.sqlite',
    'journal_path' => '/srv/www/wp-content/database/wp-next210.sqlite-journal',
    'wal_path' => '/srv/www/wp-content/database/wp-next210.sqlite-wal',
    'page_size' => 512,
...
$batches = [
    $batch(),
    $batch(['name' => 'cron-frame-batch', 'writer_name' => 'wp-cron-option-update', 'page_digests' => [7 => $digest('cron root after next210 update')]]),
...
$cases = [
    ...
    'database path' => [static fn (): mixed => $plan()['database_path'], '/srv/www/wp-content/database/wp-next210.sqlite'],
    'journal path' => [static fn (): mixed => $plan()['journal_path'], '/srv/www/wp-content/database/wp-next210.sqlite-journal'],
    'wal path' => [static fn (): mixed => $plan()['wal_path'], '/srv/www/wp-content/database/wp-next210.sqlite-wal'],
    ...
    'operation verify present' => [static fn (): mixed => in_array('verify_post_checkpoint_wal_append_current_source_next210', $plan()['operation_names'], true), true],
    'accept operation count' => [static fn (): mixed => count(array_filter($plan()['operation_names'], static fn (string $name): bool => $name === 'accept_post_checkpoint_wal_append_current_source_next210')), 2],
    'block operation count' => [static fn (): mixed => count(array_filter($plan()['operation_names'], static fn (string $name): bool => $name === 'block_post_checkpoint_wal_append_current_source_next210')), 12],
    'append digest length' => [static fn (): mixed => strlen($plan()['append_digest']), 64],
    'dependency next210' => [static fn (): mixed => in_array('sqlite-wal-hot-journal-savepoint-checkpoint-current-source-next210', $plan()['dependencies'], true), true],
    ...
    'blocked status' => [static fn (): mixed => $blocked()['status'], 'wal-hot-journal-savepoint-checkpoint-current-source-blocked-next210'],
    'blocked guard from base' => [static fn (): mixed => $blocked()['blocked_guard_names'], ['next209_writer_generation_fence']],
    'all accepted blocked by missing mix' => [static fn (): mixed => $allAccepted()['status'], 'wal-hot-journal-savepoint-checkpoint-current-source-blocked-next210'],
    'all accepted guard' => [static fn (): mixed => $allAccepted()['blocked_guard_names'], ['post_checkpoint_append_mix']],
    'commit before first keeps mixed fence ready' => [static fn (): mixed => $badCommit()['status'], 'wal-hot-journal-savepoint-checkpoint-current-source-next210'],
...
foreach ($cases as $name => [$callback, $expected]) {
    $tests['wal hot journal savepoint checkpoint current source next210 ' . $name] = static function (TestRunner $t) use ($callback, $expected): void {
...
foreach ($throws as $name => $callback) {
    $tests['wal hot journal savepoint checkpoint current source next210 ' . $name] = static function (TestRunner $t) use ($callback): void {
