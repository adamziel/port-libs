...
$makeFirstPage719734 = static function (int $pageCount): string {
...
$putPointerMapEntry719734 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database719734 = static function (int $sliceNumber) use ($makeFirstPage719734, $putPointerMapEntry719734): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage719734(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry719734($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan719734 = static function (int $sliceNumber, int $batchSize = 2) use ($database719734): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database719734($sliceNumber);
...
foreach ([719, 720, 721, 722, 723, 724, 725, 726, 727, 728, 729, 730, 731, 732, 733, 734] as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} publishes next719-734 follow-on handoff receipts"] = static function (TestRunner $t) use ($plan719734, $sliceNumber): void {
        $plan = $plan719734($sliceNumber);
        ...
        $t->same(6, $plan719734($sliceNumber, 3)->currentSourceSummary()['current_source_row_count']);
    };
}
