...
$pageSize = 512;
$database = '/srv/wp-content/database/wp-next590.sqlite';
...
$formatPage = static function (string $label) use ($pageSize): string {
    ...
    $page = substr_replace($page, pack('N', 590), 60, 4);
...
$tokens = [$journal => 'member-main-current-590'];
$headers = [$journal => hash('sha256', 'main header next590')];
$base = [
    'source_id' => 'pager-reader-cache-current-source-next590',
    'epoch' => 590,
    'format_signature' => hash('sha256', implode('|', [512, 4, 2, 590, 0])),
    'publication_generation' => 590,
    'master_source_digest' => hash('sha256', 'master-next590'),
    'recovery_sequence' => 590,
...
foreach ($tokenFields as $field) {
    $base[$field] = str_replace('_', '-', preg_replace('/_token$/', '', $field)) . '-current-590';
}
...
$plan = static fn (array $cacheExtra = [], array $readExtra = []): array => SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::currentSourceVdbeStatementBranchFence(
    $database,
    $master,
    $masterBytes,
    implode('', $before),
...
$tests['pager master journal reader cache current source next590 admits current VDBE opcode literal and arithmetic and branch fences'] = static function (TestRunner $t) use ($plan): void {
    $result = $plan();
    $t->same('pager-master-journal-reader-cache-current-source-next590', $result['status']);
    $t->same([], $result['invalidated_cache_page_numbers']);
    $t->same(['read-options' => true], $result['read_cache_hits']);
    $t->same('reader-cache-stmt-vdbe-transaction-branch-current-590', $result['current_reader_cache_stmt_vdbe_transaction_branch_token']);
};

$tests['pager master journal reader cache current source plan dispatches full width calls to next590'] = static function (TestRunner $t) use ($database, $master, $masterBytes, $before, $pageSize, $recovered, $cacheEntry, $read, $base, $tokens, $headers, $tokenFields): void {
...
$tests['pager master journal reader cache current source next590 invalidates stale transaction_branch cache'] = static function (TestRunner $t) use ($plan, $opCount): void {
    $result = $plan(['reader_cache_stmt_vdbe_transaction_branch_token' => 'stmt-vdbe-transaction-branch-old']);
    $t->same([1], $result['reader_cache_stmt_vdbe_transaction_branch_invalidated_cache_page_numbers']);
    $t->same([1], $result['invalidated_cache_page_numbers']);
    $t->same(1, $opCount($result, 'invalidate_reader_cache_reader_cache_stmt_vdbe_transaction_branch_current_source_next590'));
};

$tests['pager master journal reader cache current source next590 reopens stale transaction_branch read ticket'] = static function (TestRunner $t) use ($plan, $opCount): void {
    $result = $plan([], ['reader_cache_stmt_vdbe_transaction_branch_token' => 'stmt-vdbe-transaction-branch-old']);
    $t->same(['read-options'], $result['reopen_reader_ids']);
    $t->same(false, $result['next_reads'][0]['cache_hit']);
    $t->same('reader_ticket_reader_cache_stmt_vdbe_transaction_branch_predates_current_source', $result['next_reads'][0]['reader_cache_stmt_vdbe_transaction_branch_token_reason']);
    $t->same(1, $opCount($result, 'reopen_reader_for_reader_cache_stmt_vdbe_transaction_branch_current_source_next590'));
};

$tests['pager master journal reader cache current source next590 missing transaction_branch token rejects'] = static function (TestRunner $t) use ($plan): void {
    $t->throws(Throwable::class, static fn () => $plan(['reader_cache_stmt_vdbe_transaction_branch_token' => null]));
};
