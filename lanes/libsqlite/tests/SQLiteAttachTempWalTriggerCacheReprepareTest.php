...
$plan = static fn (array $updates = [], array $triggers = ['options_after_update', 'temp_options_after_update', 'site.site_options_after_update'], ?array $schemaStates = null, string $source = 'main'): array => SQLiteAttachWalTempViewCachePlan::triggerProgramCacheRepreparePlan(
    $catalog(),
    $triggers,
    $updates,
    $schemaStates ?? $states(),
...
$pathCases = [
    ...
    'operation names slice' => [[], 'operation', 'attach-temp-wal-trigger-cache-reprepare'],
...
foreach ($pathCases as $name => $case) {
    $tests['attach temp wal trigger cache reprepare ' . $name] = static function (TestRunner $t) use ($plan, $value, $case): void {
...
foreach ($predicateCases as $name => $predicate) {
    $tests['attach temp wal trigger cache reprepare ' . $name] = static function (TestRunner $t) use ($predicate): void {
