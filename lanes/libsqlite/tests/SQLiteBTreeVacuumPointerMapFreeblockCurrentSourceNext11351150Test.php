...
$makeFirstPage11351150 = static function (int $pageCount): string {
...
$putPointerMapEntry11351150 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database11351150 = static function (int $sliceNumber) use ($makeFirstPage11351150, $putPointerMapEntry11351150): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage11351150(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry11351150($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan11351150 = static function (int $sliceNumber, int $batchSize = 2) use ($database11351150): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database11351150($sliceNumber);
...
foreach (range(1135, 1150) as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} continues next1135-1150 handoff receipts"] = static function (TestRunner $t) use ($plan11351150, $sliceNumber): void {
        $plan = $plan11351150($sliceNumber);
        ...
        $t->same(6, $plan11351150($sliceNumber, 3)->currentSourceSummary()['current_source_row_count']);
    };
}
