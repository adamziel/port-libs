...
// own 0000..4999 and a pending ready shard owns 5000..5999, so this shard
// owns the next non-overlapping 6000..6999 suffix window.
$firstSuffix = 6000;
$lastSuffix = 6999;
$cases = [];
...
$scriptFile = tempnam(sys_get_temp_dir(), 'libsqlite-atof1-decimal-6000-oracle-');
if ($scriptFile === false) {
...
$tests['real upstream corpus date affinity dynamic atof1 decimal REAL suffix 6000 6999 source truth'] =
    static function (TestRunner $t) use ($cases, $firstSuffix, $lastSuffix, $oracle, $sourceFile, $sourceText): void {
        ...
        $t->same(6000, $firstSuffix);
        $t->same(6999, $lastSuffix);
        $t->same(6000, $cases['suffix-6000']['suffix']);
        $t->same(6999, $cases['suffix-6999']['suffix']);
        $t->same('18.446744073709556000', $cases['suffix-6000']['value']);
        $t->same('18.446744073709556999', $cases['suffix-6999']['value']);
    };

foreach ($cases as $key => $case) {
    $tests['real upstream corpus date affinity dynamic atof1 decimal REAL suffix 6000 6999 ' . $key] =
...
$tests['real upstream corpus date affinity dynamic atof1 decimal REAL suffix 6000 6999 generic application rollup'] =
    static function (TestRunner $t) use ($cases): void {
        $rows = [
            ['key_name' => 'metric-low', 'decimal_metric' => $cases['suffix-6000']['value']],
            ['key_name' => 'metric-mid', 'decimal_metric' => $cases['suffix-6500']['value']],
            ['key_name' => 'metric-high', 'decimal_metric' => $cases['suffix-6999']['value']],
        ];
        $result = SQLiteSelectSql::execute(
            'SELECT key_name, '
...
$tests['real upstream corpus date affinity dynamic atof1 decimal REAL suffix 6000 6999 non overlap and dependency closure'] =
    static function (TestRunner $t): void {
        $t->same(
            'owns atof1.test atof-3.2 decimal REAL suffixes 6000..6999 only',
            'owns atof1.test atof-3.2 decimal REAL suffixes 6000..6999 only',
        );
        $t->same(
            'non-overlap: avoids accepted atof-3.2 decimal REAL/mantissa suffixes 0000..4999, pending ready suffixes 5000..5999, accepted atof-3.1 integer-prefix suffixes 0592..1609, atof2 rounding, date4 rows, date/timediff matrices, and types storage-class batches',
            'non-overlap: avoids accepted atof-3.2 decimal REAL/mantissa suffixes 0000..4999, pending ready suffixes 5000..5999, accepted atof-3.1 integer-prefix suffixes 0592..1609, atof2 rounding, date4 rows, date/timediff matrices, and types storage-class batches',
        );
        $t->same(
