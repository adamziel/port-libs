...
$currentRows = [
    ...
    [
        ...
        'key_value' => '{"modules":["search"],"source":"current"}',
        'load_policy' => 'yes',
...
$skipPlan = static fn (): array => SQLiteJsonUpsertMigrationPlan::execute(
    ...
    [
        [
            'setting_id' => 30,
            'key_name' => 'module_registry',
            'key_value' => '{"modules":["stale-import"],"source":"stale"}',
            'load_policy' => 'no',
...
$cases = [
    ...
    'returning rows preserve source order' => [static fn (): mixed => array_column($plan()['returning_rows'], 'key_name'), ['display_banner', 'route_rules', 'theme_palette']],
    'updated rows are conflict rows only' => [static fn (): mixed => array_column($plan()['updated_rows'], 'key_name'), ['display_banner', 'theme_palette']],
    ...
    'after keeps original order and appends insert' => [static fn (): mixed => array_column($plan()['after'], 'key_name'), ['display_banner', 'theme_palette', 'module_registry', 'route_rules']],
    'before keeps original JSON unchanged' => [static fn (): mixed => $plan()['before'][0]['key_value'], $currentRows[0]['key_value']],
    'display load policy updates from excluded row' => [static fn (): mixed => $plan()['after'][0]['load_policy'], 'no'],
    'palette load policy updates from excluded row' => [static fn (): mixed => $plan()['after'][1]['load_policy'], 'yes'],
    'insert load_policy keeps incoming row' => [static fn (): mixed => $plan()['after'][3]['load_policy'], 'yes'],
    'display generation increments max current excluded' => [static fn (): mixed => $plan()['after'][0]['migration_generation'], 8],
    'palette generation increments max current excluded' => [static fn (): mixed => $plan()['after'][1]['migration_generation'], 9],
    ...
    'display migrated flag is set' => [static fn (): mixed => $plan()['decoded_returning'][0]['decoded_key_value']['migrated'], 1],
    'palette migrated flag is set' => [static fn (): mixed => $plan()['decoded_returning'][2]['decoded_key_value']['migrated'], 1],
    'insert migrated flag is set before insert' => [static fn (): mixed => $plan()['decoded_returning'][1]['decoded_key_value']['migrated'], 1],
    'display source comes from excluded JSON' => [static fn (): mixed => $plan()['decoded_returning'][0]['decoded_key_value']['source'], 'import'],
    'palette source comes from excluded JSON' => [static fn (): mixed => $plan()['decoded_returning'][2]['decoded_key_value']['source'], 'import'],
    'insert source remains import JSON' => [static fn (): mixed => $plan()['decoded_returning'][1]['decoded_key_value']['source'], 'import'],
    'display previous source comes from current row' => [static fn (): mixed => $plan()['decoded_returning'][0]['decoded_key_value']['previous_source'], 'current'],
    'palette previous source comes from current row' => [static fn (): mixed => $plan()['decoded_returning'][2]['decoded_key_value']['previous_source'], 'current'],
    ...
    'display incoming version comes from excluded JSON' => [static fn (): mixed => $plan()['decoded_returning'][0]['decoded_key_value']['incoming_version'], 5],
    'palette incoming version comes from excluded JSON' => [static fn (): mixed => $plan()['decoded_returning'][2]['decoded_key_value']['incoming_version'], 6],
    ...
    'display previous generation comes from current column' => [static fn (): mixed => $plan()['decoded_returning'][0]['decoded_key_value']['previous_generation'], 2],
    'palette previous generation comes from current column' => [static fn (): mixed => $plan()['decoded_returning'][2]['decoded_key_value']['previous_generation'], 4],
    'insert previous generation is null' => [static fn (): mixed => $plan()['decoded_returning'][1]['decoded_key_value']['previous_generation'], null],
    'display load policy json mirrors excluded column' => [static fn (): mixed => $plan()['decoded_returning'][0]['decoded_key_value']['load_policy_after'], 'no'],
    'palette load policy json mirrors excluded column' => [static fn (): mixed => $plan()['decoded_returning'][2]['decoded_key_value']['load_policy_after'], 'yes'],
    ...
    'display nested JSON literal is preserved as object' => [static fn (): mixed => $plan()['decoded_returning'][0]['decoded_key_value']['app_import'], ['tool' => 'data-liberation', 'batch' => 27]],
    ...
    'display existing nested data remains from current row' => [static fn (): mixed => $plan()['decoded_returning'][0]['decoded_key_value']['widgets']['hero']['title'], 'Old'],
    'palette existing nested data remains from current row' => [static fn (): mixed => $plan()['decoded_returning'][2]['decoded_key_value']['mods']['color'], 'blue'],
    ...
    'display setting id remains current on update' => [static fn (): mixed => $plan()['returning_rows'][0]['setting_id'], 1],
    'palette setting id remains current on update' => [static fn (): mixed => $plan()['returning_rows'][2]['setting_id'], 2],
    ...
    'stale conflict is skipped by WHERE' => [static fn (): mixed => array_column($skipPlan()['skipped_rows'], 'key_name'), ['module_registry']],
    'stale conflict makes no changes' => [static fn (): mixed => $skipPlan()['changes'], 0],
    'stale conflict leaves module registry unchanged' => [static fn (): mixed => $skipPlan()['after'][2]['key_value'], $currentRows[2]['key_value']],
    ...
    'bad current JSON rejected on update' => [static fn (): mixed => SQLiteJsonUpsertMigrationPlan::execute([['key_name' => 'display_banner', 'key_value' => '{']], [$incomingRows[0]], $jsonSetValues), Throwable::class],
