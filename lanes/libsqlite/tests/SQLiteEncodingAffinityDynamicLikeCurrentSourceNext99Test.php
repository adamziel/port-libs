...
): array => SQLiteEncodingAffinityLikeCurrentSourceNextPlan::wordpressOptionValueDynamicPatternPlan(
    $currentRows,
    $nextRows,
    'option_value',
    'like_pattern',
...
$cases = [
    ...
    'records current source' => ['currentSource', 'main.wp_options'],
    'records next source' => ['nextSource', 'main.wp_options'],
...
$tests['encoding affinity dynamic like current source next99 stable cursor reusable'] = static function (TestRunner $t) use ($currentRows): void {
    $stable = SQLiteEncodingAffinityLikeCurrentSourceNextPlan::wordpressOptionValueDynamicPatternPlan(
        $currentRows,
        $currentRows,
        'option_value',
        'like_pattern',
...
$tests['encoding affinity dynamic like current source next99 source switch invalidates first'] = static function (TestRunner $t) use ($currentRows): void {
    $switched = SQLiteEncodingAffinityLikeCurrentSourceNextPlan::wordpressOptionValueDynamicPatternPlan(
        $currentRows,
        $currentRows,
        'option_value',
        'like_pattern',
...
$tests['encoding affinity dynamic like current source next99 case sensitive like excludes plugin alpha'] = static function (TestRunner $t) use ($currentRows, $nextRows): void {
    $caseSensitive = SQLiteEncodingAffinityLikeCurrentSourceNextPlan::wordpressOptionValueDynamicPatternPlan($currentRows, $nextRows, 'option_value', 'like_pattern', 'like_escape', true);
...
$tests['encoding affinity dynamic like current source next99 rejects missing pattern column'] = static function (TestRunner $t) use ($nextRows): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteEncodingAffinityLikeCurrentSourceNextPlan::wordpressOptionValueDynamicPatternPlan([['option_id' => 1, 'option_value' => 'x']], $nextRows, 'option_value', 'like_pattern'));
};
...
$tests['encoding affinity dynamic like current source next99 rejects malformed pattern'] = static function (TestRunner $t) use ($nextRows): void {
    ...
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteEncodingAffinityLikeCurrentSourceNextPlan::wordpressOptionValueDynamicPatternPlan($current, $nextRows, 'option_value', 'like_pattern'));
};
...
$tests['encoding affinity dynamic like current source next99 rejects multi char escape after affinity'] = static function (TestRunner $t) use ($nextRows): void {
    ...
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteEncodingAffinityLikeCurrentSourceNextPlan::wordpressOptionValueDynamicPatternPlan($current, $nextRows, 'option_value', 'like_pattern', 'like_escape'));
};
