...
$tests = [
    ...
    'planner stat4 expression partial current source prepared handoff resume window inherits next846861' => static fn (TestRunner $t) => $t->same(true, $planpreparedHandoffResumeWindow()['selectedPlan']['next846861Prepared']),
    ...
    'planner stat4 expression partial current source prepared handoff resume window prior signature threaded' => static fn (TestRunner $t) => $t->same($planpreparedHandoffResumeWindow()['stat4Next846861PreparationFence']['handoffSignature'], $planpreparedHandoffResumeWindow()['selectedPlan']['preparedHandoffResumeWindowPriorHandoffSignature']),
    'planner stat4 expression partial current source prepared handoff resume window preserves next846861 fence' => static fn (TestRunner $t) => $t->same(range(846, 861), $planpreparedHandoffResumeWindow()['stat4Next846861PreparationFence']['preparedSlices']),
    ...
    'planner stat4 expression partial current source prepared handoff resume window non overlap' => static fn (TestRunner $t) => $t->contains('next846-861 handoff windows', $planpreparedHandoffResumeWindow()['non_overlap']),
