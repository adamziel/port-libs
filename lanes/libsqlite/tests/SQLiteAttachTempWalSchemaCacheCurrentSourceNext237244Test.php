...
$plan237244 = static fn (array $events, ?array $statements = null, ?array $schemas = null): array => SQLiteAttachWalTempSchemaCacheCurrentSourceNext92Plan::currentSourceNext237244(
    $schemas ?? $schemas237244,
    $statements ?? $statements237244,
    $events,
);

$tests = [];

$tests['attach temp wal schema cache current source next237 main wal commit expires main statements'] = static function (TestRunner $t) use ($plan237244): void {
    $result = $plan237244([
        ['op' => 'wal_commit', 'schema' => 'main', 'schema_cookie' => 238, 'table' => 'wp_postmeta', 'indexes' => ['wp_postmeta_key'], 'commit' => true],
    ]);

    $t->same('attach-wal-temp-schema-cache-current-source-next237-244', $result['operation']);
    $t->same('sqlite-attach-temp-wal-schema-cache-current-source-next237', $result['dependencies'][0]);
    $t->same('sqlite-attach-temp-wal-schema-cache-current-source-next244', $result['dependencies'][7]);
    $t->same('sqlite-attach-temp-wal-schema-cache-current-source-next236', $result['dependencies'][15]);
    $t->same(['main'], $result['changed_schemas']);
    $t->same(238, $result['schema_cookies_next']['main']);
...
$tests['attach temp wal schema cache current source next238 temp options drop reveals main options source'] = static function (TestRunner $t) use ($plan237244): void {
    $result = $plan237244([
...
$tests['attach temp wal schema cache current source next239 audit index rename blocks audit writer'] = static function (TestRunner $t) use ($plan237244): void {
    $result = $plan237244([
...
$tests['attach temp wal schema cache current source next240 attach queue schema leaves prepared sources stable'] = static function (TestRunner $t) use ($plan237244): void {
    $result = $plan237244([
...
$tests['attach temp wal schema cache current source next241 detach search expires explicit search reader'] = static function (TestRunner $t) use ($plan237244): void {
    $result = $plan237244([
...
$tests['attach temp wal schema cache current source next242 analytics rollup drop leaves main and audit stable'] = static function (TestRunner $t) use ($plan237244): void {
    $result = $plan237244([
...
$tests['attach temp wal schema cache current source next243 uncommitted audit wal frame is ignored'] = static function (TestRunner $t) use ($plan237244): void {
    $result = $plan237244([
    ...
    $t->same([], $result['expired_statements']);
    $t->same(17, $result['schema_cookies_next']['audit']);
};

$tests['attach temp wal schema cache current source next244 combined ddl keeps active readers on current source'] = static function (TestRunner $t) use ($plan237244): void {
    $result = $plan237244([
