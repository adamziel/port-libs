...
$plan = static function () use ($readyCurrent): array {
    static $result = null;
    if ($result === null) {
        $result = SQLiteVfsCurrentSourceNext530545Plan::run([
...
return [
    ...
    'vfs current source next530-545 blocks dirty snapshot' => static fn (TestRunner $t) => $t->same(true, in_array('dirty-pages-present', SQLiteVfsCurrentSourceNext530545Plan::run(['snapshot(reader-ready-next545,shared-cache-next529)'], ['current' => $dirtyCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source next530-545 blocks old snapshot ack' => static fn (TestRunner $t) => $t->same(true, in_array('ack-not-latest-publish', SQLiteVfsCurrentSourceNext530545Plan::run(['snapshot(reader-ready-next545,shared-cache-next529)'], ['current' => $oldAckCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source next530-545 blocks publish without claim' => static fn (TestRunner $t) => $t->same(true, in_array('missing-reuse-claim', SQLiteVfsCurrentSourceNext530545Plan::run(['snapshot(reader-ready-next545,shared-cache-next529)', 'publish(reader-ready-next545,reader-reuse-next545,shared-cache-next545)'], ['current' => $readyCurrent])['events'][1]['blocked_reasons'], true)),
    'vfs current source next530-545 blocks stale claim' => static fn (TestRunner $t) => $t->same(true, in_array('stale-reuse-claim', SQLiteVfsCurrentSourceNext530545Plan::run(['publish(reader-ready-next545,reader-reuse-next545,shared-cache-next545)'], ['current' => $staleClaimCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source next530-545 accepts preclaimed publish' => static fn (TestRunner $t) => $t->same('published-reused-current-source', SQLiteVfsCurrentSourceNext530545Plan::run(['publish(reader-ready-next545,reader-reuse-next545,shared-cache-next545)'], ['current' => $claimedCurrent])['events'][0]['status']),
    'vfs current source next530-545 rejects empty operations' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteVfsCurrentSourceNext530545Plan::run([])),
    'vfs current source next530-545 rejects bad claim token' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteVfsCurrentSourceNext530545Plan::run([['op' => 'claim', 'snapshot' => 'reader-ready-next545', 'ack' => 'shared-cache-next529', 'claim' => 'bad claim']], ['current' => $readyCurrent])),
    'vfs current source next530-545 rejects unsupported operation' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteVfsCurrentSourceNext530545Plan::run(['republish(reader-ready-next545,shared-cache-next545)'], ['current' => $readyCurrent])),
