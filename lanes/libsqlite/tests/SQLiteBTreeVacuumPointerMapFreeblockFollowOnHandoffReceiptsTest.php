...
$databaseForHandoff = static function (int $sliceNumber) use ($makeFirstPage, $putPointerMapEntry): SQLiteDatabase {
...
$planForHandoff = static function (int $sliceNumber, int $batchSize = 2) use ($databaseForHandoff): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $databaseForHandoff($sliceNumber);
...
foreach ([687, 688, 689, 690, 691, 692, 693, 694, 695, 696, 697, 698, 699, 700, 701, 702] as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock publishes stable follow-on handoff receipt {$sliceNumber}"] = static function (TestRunner $t) use ($planForHandoff, $sliceNumber): void {
        $plan = $planForHandoff($sliceNumber);
        ...
        $t->same(6, $planForHandoff($sliceNumber, 3)->currentSourceSummary()['current_source_row_count']);
    };
}
