...
use PortLibs\LibSqlite\SQLitePagerMasterJournalHotRollbackCurrentSourceNext89Plan;
...
$plan = static fn (?string $bytes = null, array $input = null): array => SQLitePagerMasterJournalHotRollbackCurrentSourceNext89Plan::currentSourceNext(
    $masterPath,
    $bytes,
    $input ?? $databases,
    $pageSize
...
$cases = [
    ...
    'reserved lock blocks main' => static function () use ($databases, $masterBytes, $masterPath, $pageSize, $mainPath): mixed {
        ...
        return SQLitePagerMasterJournalHotRollbackCurrentSourceNext89Plan::currentSourceNext($masterPath, $masterBytes, $copy, $pageSize)['hot_journals'][$mainPath . '-journal']['reason'];
    },
    ...
    'empty master path rejected' => static function () use ($masterBytes, $databases, $pageSize): mixed {
        try {
            SQLitePagerMasterJournalHotRollbackCurrentSourceNext89Plan::currentSourceNext('', $masterBytes, $databases, $pageSize);
    ...
    'empty database list rejected' => static function () use ($masterPath, $masterBytes, $pageSize): mixed {
        try {
            SQLitePagerMasterJournalHotRollbackCurrentSourceNext89Plan::currentSourceNext($masterPath, $masterBytes, [], $pageSize);
        } catch (InvalidArgumentException) {
            return 'rejected';
        }
        return 'accepted';
    ...
    'bad page size rejected' => static function () use ($masterPath, $masterBytes, $databases): mixed {
        try {
            SQLitePagerMasterJournalHotRollbackCurrentSourceNext89Plan::currentSourceNext($masterPath, $masterBytes, $databases, 513);
        } catch (InvalidArgumentException) {
            return 'rejected';
        }
        return 'accepted';
    ...
    'duplicate database rejected' => static function () use ($masterPath, $masterBytes, $databases, $pageSize): mixed {
        try {
            SQLitePagerMasterJournalHotRollbackCurrentSourceNext89Plan::currentSourceNext($masterPath, $masterBytes, [$databases[0], $databases[0]], $pageSize);
        } catch (InvalidArgumentException) {
            return 'rejected';
        }
        return 'accepted';
    ...
    'read only rejected' => static function () use ($masterPath, $masterBytes, $databases, $pageSize): mixed {
        try {
            SQLitePagerMasterJournalHotRollbackCurrentSourceNext89Plan::currentSourceNext($masterPath, $masterBytes, $databases, $pageSize, true);
    ...
    'unaligned current source rejected' => static function () use ($masterPath, $masterBytes, $databases, $pageSize): mixed {
        ...
        try {
            SQLitePagerMasterJournalHotRollbackCurrentSourceNext89Plan::currentSourceNext($masterPath, $masterBytes, $copy, $pageSize);
        } catch (InvalidArgumentException) {
            return 'rejected';
        }
        return 'accepted';
