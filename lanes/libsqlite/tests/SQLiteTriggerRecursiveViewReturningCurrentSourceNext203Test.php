...
$plan203 = static fn (array $options = []): array => SQLiteTriggerRecursiveViewReturningCurrentSourceNextPlan::executeCurrentGenerationReceipt(
    $rows203,
    $currentInput203,
    $nextInput203,
    $currentView203,
    ...
    $options + [
        ...
        'current_generation' => 'app-current-returning-203',
        'next_generation' => 'app-next-returning-203',
        'page_size' => 3,
        'admit_next_source' => true,
        'current_source_token' => 'app.current.source.203',
        'drain_ack_token' => 'app.returning.drain.203',
        ...
        'following_generation' => 'app-following-current-203',
        'recursive_child_acknowledged_ordinals' => [0, 1],
        'recursive_child_source_token' => 'app.current.source.recursive.child.203',
        'recursive_child_cursor' => 'app.returning.recursive.child.cursor.203',
        'recursive_child_generation' => 'app-recursive-child-current-203',
        'current_generation_next203' => 'app.current.recursive.returning.generation.203',
        'expected_current_generation_next203' => 'app.current.recursive.returning.generation.203',
        'current_handoff_cursor_next203' => 'app.returning.current.handoff.cursor.203',
        'current_generation_commit_marker_next203' => 'app.current.recursive.returning.commit.203',
    ],
);
...
$generationHeld203 = static fn (): array => $plan203(['auto_ack_current_generation_receipts_next203' => true, 'expected_current_generation_next203' => 'app.current.recursive.returning.generation.stale.203']);
...
$custom203 = static fn (): array => $plan203([
    ...
    'current_generation_next203' => 'app.current.recursive.returning.generation.custom.203',
    'expected_current_generation_next203' => 'app.current.recursive.returning.generation.custom.203',
    'current_handoff_cursor_next203' => 'app.returning.current.handoff.cursor.custom.203',
    'current_generation_commit_marker_next203' => 'app.current.recursive.returning.commit.custom.203',
]);
...
$cases203 = [
    ...
    'base following child drain visible' => [static fn (): mixed => $released203()['base']['status_next196'], 'trigger-recursive-view-returning-following-child-drain-next-source-visible'],
    'base held keeps following child drain waiting' => [static fn (): mixed => $baseHeld203()['base']['status_next196'], 'trigger-recursive-view-returning-following-child-drain-awaiting-recursive-child-acks'],
    'savepoint retained' => [static fn (): mixed => $released203()['savepoint'], 'app_recursive_view_203'],
    ...
    'current generation retained' => [static fn (): mixed => $released203()['current_generation_next203'], 'app.current.recursive.returning.generation.203'],
    'expected generation retained' => [static fn (): mixed => $released203()['expected_current_generation_next203'], 'app.current.recursive.returning.generation.203'],
    ...
    'custom generation retained' => [static fn (): mixed => $custom203()['current_generation_next203'], 'app.current.recursive.returning.generation.custom.203'],
    'handoff cursor retained' => [static fn (): mixed => $released203()['current_handoff_cursor_next203'], 'app.returning.current.handoff.cursor.203'],
    'custom handoff cursor retained' => [static fn (): mixed => $custom203()['current_handoff_cursor_next203'], 'app.returning.current.handoff.cursor.custom.203'],
    'commit marker retained' => [static fn (): mixed => $released203()['current_generation_commit_marker_next203'], 'app.current.recursive.returning.commit.203'],
    'custom commit marker retained' => [static fn (): mixed => $custom203()['current_generation_commit_marker_next203'], 'app.current.recursive.returning.commit.custom.203'],
    ...
    'current generation stamped' => [static fn (): mixed => array_values(array_unique(array_column($released203()['current_generation_rows_next203'], 'current_generation_next203'))), ['app.current.recursive.returning.generation.203']],
    'next generation stamped' => [static fn (): mixed => array_values(array_unique(array_column($released203()['attempted_next_generation_rows_next203'], 'current_generation_next203'))), ['app.current.recursive.returning.generation.203']],
    'current cursor stamped' => [static fn (): mixed => array_values(array_unique(array_column($released203()['current_generation_rows_next203'], 'current_handoff_cursor_next203'))), ['app.returning.current.handoff.cursor.203']],
    'next cursor stamped' => [static fn (): mixed => array_values(array_unique(array_column($released203()['attempted_next_generation_rows_next203'], 'current_handoff_cursor_next203'))), ['app.returning.current.handoff.cursor.203']],
    'visible payload names released' => [static fn (): mixed => array_column($released203()['visible_returning_payloads_next203'], 'name'), ['app_summary_child', 'template_child', 'landing_url', 'next_module']],
    'held payload names missing' => [static fn (): mixed => array_column($missing203()['held_next_returning_payloads_next203'], 'name'), ['landing_url', 'next_module']],
