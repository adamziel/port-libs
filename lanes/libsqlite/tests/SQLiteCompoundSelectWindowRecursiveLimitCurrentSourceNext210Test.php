...
$summary210 = static fn (?array $cursor = null): array => SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext210Plan::compare($sql210, $currentTables210, $nextTables210, $cursor);
$tests = [];
...
$tests['compound select window recursive limit current source next210 rejects missing last value'] = static function (TestRunner $t) use ($currentTables210): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext210Plan::compare(
...
foreach (range(1, 58) as $case) {
    $tests['compound select window recursive limit current source next210 generated intersect except value boundary ' . $case] = static function (TestRunner $t) use ($case): void {
        ...
        $plan = SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext210Plan::compare($sql, $tables, $nextTables);
        $again = SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext210Plan::compare($sql, $tables, $nextTables, $plan['cursor']);
