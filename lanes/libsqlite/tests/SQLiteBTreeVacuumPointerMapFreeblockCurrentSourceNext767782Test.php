...
$makeFirstPage767782 = static function (int $pageCount): string {
...
$putPointerMapEntry767782 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database767782 = static function (int $sliceNumber) use ($makeFirstPage767782, $putPointerMapEntry767782): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage767782(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry767782($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan767782 = static function (int $sliceNumber, int $batchSize = 2) use ($database767782): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database767782($sliceNumber);
...
foreach ([767, 768, 769, 770, 771, 772, 773, 774, 775, 776, 777, 778, 779, 780, 781, 782] as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} extends next767-782 handoff receipts"] = static function (TestRunner $t) use ($plan767782, $sliceNumber): void {
        $plan = $plan767782($sliceNumber);
        ...
        $t->same(6, $plan767782($sliceNumber, 3)->currentSourceSummary()['current_source_row_count']);
    };
}
