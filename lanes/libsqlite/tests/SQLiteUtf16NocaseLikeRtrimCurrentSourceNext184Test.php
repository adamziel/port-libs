...
$row184 = static fn (int $id, string $name, int|string $encoding): array => [
    'setting_id' => $id,
    'key_name_bytes' => $enc184($name, $encoding),
...
$plan184 = static fn (
    ...
    int $nextCookie = 184,
): array => SQLiteUtf16NocaseLikeRtrimCurrentSourceNextPlan::keyValueRowKeyEscapedPeerReplayPlan(
    $current ?? $rows184,
    $next ?? $rows184,
    $pattern,
    $escape,
...
$tests['utf16 nocase like rtrim current source nextOneEightFour escaped percent blocks wildcard token'] = static function (TestRunner $t) use ($rows184, $enc184, $token184): void {
    ...
    $result = SQLiteUtf16NocaseLikeRtrimCurrentSourceNextPlan::keyValueRowKeyEscapedPeerReplayPlan($rows184, $rows184, 'plugin!_!%cache%', '!', $bad, 'stable', 'stable', 184, 184);
...
$tests['utf16 nocase like rtrim current source nextOneEightFour token trailing spaces match after rtrim'] = static function (TestRunner $t) use ($rows184, $enc184, $token184): void {
    ...
    $result = SQLiteUtf16NocaseLikeRtrimCurrentSourceNextPlan::keyValueRowKeyEscapedPeerReplayPlan($rows184, $rows184, 'plugin!_!%cache', '!', $token, 'stable', 'stable', 184, 184);
...
$tests['utf16 nocase like rtrim current source nextOneEightFour malformed token bytes are rejected before unsafe continuation'] = static function (TestRunner $t) use ($rows184, $token184): void {
    ...
    $bad['keyEncoding'] = 2;
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteUtf16NocaseLikeRtrimCurrentSourceNextPlan::keyValueRowKeyEscapedPeerReplayPlan($rows184, $rows184, 'plugin!_!%cache%', '!', $bad, 'stable', 'stable', 184, 184));
};
