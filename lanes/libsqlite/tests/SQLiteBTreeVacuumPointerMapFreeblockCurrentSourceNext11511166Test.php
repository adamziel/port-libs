...
$makeFirstPage11511166 = static function (int $pageCount): string {
...
$putPointerMapEntry11511166 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database11511166 = static function (int $sliceNumber) use ($makeFirstPage11511166, $putPointerMapEntry11511166): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage11511166(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry11511166($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan11511166 = static function (int $sliceNumber, int $batchSize = 2) use ($database11511166): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database11511166($sliceNumber);
...
foreach (range(1151, 1166) as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} continues next1151-1166 handoff receipts"] = static function (TestRunner $t) use ($plan11511166, $sliceNumber): void {
        $plan = $plan11511166($sliceNumber);
        ...
        $t->same(6, $plan11511166($sliceNumber, 3)->currentSourceSummary()['current_source_row_count']);
    };
}
