...
$pageSize = 512;
$database = '/srv/wp-content/database/wp-next846.sqlite';
...
$formatPage = static function (string $label) use ($pageSize): string {
    ...
    $page = substr_replace($page, pack('N', 846), 60, 4);
...
$variantFields = array_merge($tokenFields, $next639654Fields, $next655670Fields, $next671686Fields, $next687702Fields, $next703718Fields, $next719734Fields, $next735766Fields, $next751766Fields, $next767782Fields, $next783798Fields, $next799814Fields, $next815830Fields, $next831846Fields);
$before = [1 => $formatPage('stale schema'), 2 => $page('stale options')];
$recovered = [1 => $formatPage('current schema'), 2 => $page('current options')];
$tokens = [$journal => 'member-main-current-846'];
$headers = [$journal => hash('sha256', 'main header next846')];
$base = [
    'source_id' => 'pager-reader-cache-current-source-next846',
    'epoch' => 846,
    'format_signature' => hash('sha256', implode('|', [512, 4, 2, 846, 0])),
    'publication_generation' => 846,
    'master_source_digest' => hash('sha256', 'master-next846'),
    'recovery_sequence' => 846,
...
foreach ($variantFields as $field) {
    $base[$field] = str_replace('_', '-', preg_replace('/_token$/', '', $field)) . '-current-846';
}
$base['reader_cache_stmt_vdbe_real_affinity_value_token'] = 'reader-cache-stmt-vdbe-affinity-current-846';
$base['reader_cache_stmt_vdbe_real_affinity_value_handoff_token'] = 'reader-cache-stmt-vdbe-affinity-handoff-current-846';
...
$plan = static fn (array $cacheExtra = [], array $readExtra = []): array => SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::currentSourceVdbeTransactionBranchHandoff(
    $database,
    $master,
    $masterBytes,
    implode('', $before),
...
$tests['pager master journal reader cache current source next846 publishes current VDBE literal and arithmetic opcode fences'] = static function (TestRunner $t) use ($plan): void {
    $result = $plan();
    $t->same('pager-master-journal-reader-cache-current-source-next846', $result['status']);
    $t->same([1], $result['invalidated_cache_page_numbers']);
    $t->same(['read-options' => false], $result['read_cache_hits']);
    $t->same('reader-cache-stmt-vdbe-null-branch-current-846', $result['current_reader_cache_stmt_vdbe_null_branch_token']);
    $t->same('reader-cache-stmt-vdbe-opcode-trace-branch-handoff-current-846', $result['current_reader_cache_stmt_vdbe_opcode_trace_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-shift-right-branch-handoff-current-846', $result['current_reader_cache_stmt_vdbe_shift_right_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-gt-branch-handoff-current-846', $result['current_reader_cache_stmt_vdbe_gt_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-incr-vacuum-branch-handoff-current-846', $result['current_reader_cache_stmt_vdbe_incr_vacuum_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-init-branch-handoff-current-846', $result['current_reader_cache_stmt_vdbe_init_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-goto-branch-handoff-current-846', $result['current_reader_cache_stmt_vdbe_goto_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-null-row-branch-handoff-current-846', $result['current_reader_cache_stmt_vdbe_null_row_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-row-value-branch-handoff-current-846', $result['current_reader_cache_stmt_vdbe_row_value_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-real-affinity-value-branch-handoff-current-846', $result['current_reader_cache_stmt_vdbe_real_affinity_value_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-cast-branch-handoff-current-846', $result['current_reader_cache_stmt_vdbe_cast_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-column-branch-handoff-current-846', $result['current_reader_cache_stmt_vdbe_column_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-affinity-branch-handoff-current-846', $result['current_reader_cache_stmt_vdbe_affinity_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-cast-affinity-branch-handoff-current-846', $result['current_reader_cache_stmt_vdbe_cast_affinity_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-compare-collseq-branch-handoff-current-846', $result['current_reader_cache_stmt_vdbe_compare_collseq_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-once-flag-branch-handoff-current-846', $result['current_reader_cache_stmt_vdbe_once_flag_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-ge-branch-condition-handoff-current-846', $result['current_reader_cache_stmt_vdbe_ge_branch_condition_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-else-eq-branch-condition-handoff-current-846', $result['current_reader_cache_stmt_vdbe_else_eq_branch_condition_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-if-open-branch-condition-handoff-current-846', $result['current_reader_cache_stmt_vdbe_if_open_branch_condition_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-transaction-branch-handoff-current-846', $result['current_reader_cache_stmt_vdbe_transaction_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-vbegin-branch-handoff-current-846', $result['current_reader_cache_stmt_vdbe_vbegin_branch_handoff_token']);
...
$tests['pager master journal reader cache current source plan dispatches full width calls to next846'] = static function (TestRunner $t) use ($database, $master, $masterBytes, $before, $pageSize, $recovered, $cacheEntry, $read, $base, $tokens, $headers, $variantFields): void {
...
$tests['pager master journal reader cache current source next846 preserves next830 ge branch condition handoff continuity'] = static function (TestRunner $t) use ($plan, $opCount): void {
...
$tests['pager master journal reader cache current source next846 reopens stale vbegin branch handoff read ticket'] = static function (TestRunner $t) use ($plan, $opCount): void {
    $result = $plan([], ['reader_cache_stmt_vdbe_vbegin_branch_handoff_token' => 'stmt-vdbe-vbegin-branch-handoff-old']);
    $t->same(['read-options'], $result['reopen_reader_ids']);
    $t->same(false, $result['next_reads'][0]['cache_hit']);
    $t->same('reader_ticket_reader_cache_stmt_vdbe_vbegin_branch_handoff_predates_current_source', $result['next_reads'][0]['reader_cache_stmt_vdbe_vbegin_branch_handoff_token_reason']);
    $t->same(1, $opCount($result, 'reopen_reader_for_reader_cache_stmt_vdbe_vbegin_branch_handoff_current_source_next846'));
};

$tests['pager master journal reader cache current source next846 missing vbegin branch handoff token rejects'] = static function (TestRunner $t) use ($plan): void {
    $t->throws(Throwable::class, static fn () => $plan(['reader_cache_stmt_vdbe_vbegin_branch_handoff_token' => null]));
};
