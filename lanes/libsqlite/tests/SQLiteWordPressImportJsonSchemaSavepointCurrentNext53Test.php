...
use PortLibs\LibSqlite\SQLiteWordPressImportJsonSchemaSavepointCurrentNext53Plan;
...
$plan = static fn (array $imports, array $options = []) => SQLiteWordPressImportJsonSchemaSavepointCurrentNext53Plan::plan(
    $currentRows(),
    $imports,
...
$tests = [
    ...
    'accepts JSONB schema import sources' => static function (TestRunner $t) use ($currentRows): void {
        ...
        $result = SQLiteWordPressImportJsonSchemaSavepointCurrentNext53Plan::plan($currentRows(), [
    ...
    'rejects empty import lists' => static function (TestRunner $t) use ($currentRows): void {
        $t->throws(InvalidArgumentException::class, static fn () => SQLiteWordPressImportJsonSchemaSavepointCurrentNext53Plan::plan($currentRows(), []));
    },
];
...
foreach ([64 => 2, 65 => 3, 128 => 3, 129 => 4, 192 => 4, 193 => 5] as $optionId => $pageNumber) {
    $tests["explicit schema option {$optionId} maps to WAL page {$pageNumber}"] = static function (TestRunner $t) use ($currentRows, $jsonRows, $optionId, $pageNumber): void {
        $rows = $currentRows();
        $result = SQLiteWordPressImportJsonSchemaSavepointCurrentNext53Plan::plan($rows, [
