...
$makeFirstPage751766 = static function (int $pageCount): string {
...
$putPointerMapEntry751766 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database751766 = static function (int $sliceNumber) use ($makeFirstPage751766, $putPointerMapEntry751766): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage751766(110);
    ...
    $pages[3] = SQLiteTableLeafPage::assemble([
        ...
        SQLiteTableLeafCell::encode(3, SQLiteRecord::encode([null, 'active_plugins', str_repeat('plugin:', 12)])),
    ]);
    ...
    foreach ([
        ...
        $putPointerMapEntry751766($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan751766 = static function (int $sliceNumber, int $batchSize = 2) use ($database751766): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourcePlan {
    $database = $database751766($sliceNumber);
    $deletedPage = SQLiteTableLeafPage::deleteCellByRowId($database->page(3), 2, secureDelete: true);

    return SQLiteBTreeVacuumPointerMapFreeblockCurrentSourcePlan::tableLeafCurrentSourceFreelistHandoffFromDeleteResult(
        $database,
        3,
        [
            'page' => $deletedPage,
...
foreach ([751, 752, 753, 754, 755, 756, 757, 758, 759, 760, 761, 762, 763, 764, 765, 766] as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} extends next751-766 handoff receipts"] = static function (TestRunner $t) use ($plan751766, $sliceNumber): void {
        $plan = $plan751766($sliceNumber);
        ...
        $t->same(6, $plan751766($sliceNumber, 3)->currentSourceSummary()['current_source_row_count']);
    };
}
