...
return [
    ...
    'vfs wal shm lock byte current source reader holder stored' => static fn (TestRunner $t) => $t->same(['wp-reader' => 'shared'], $writer()['events'][0]['next']['selected']['holders']),
    'vfs wal shm lock byte current source reader shm acquired' => static fn (TestRunner $t) => $t->same('acquired', $writer()['events'][1]['status']),
    'vfs wal shm lock byte current source reader shm stored' => static fn (TestRunner $t) => $t->same(['wp-reader' => 'shared'], $writer()['events'][1]['next']['selected']['shm_locks']['read1']),
    ...
    'vfs wal shm lock byte current source pending keeps import' => static fn (TestRunner $t) => $t->same('pending', $writer()['events'][4]['next']['selected']['holders']['wp-import']),
    ...
    'vfs wal shm lock byte current source exclusive blocker names reader' => static fn (TestRunner $t) => $t->same(['wp-reader:shared'], $writer()['events'][5]['blocking']),
    ...
    'vfs wal shm lock byte current source yield releases main holder' => static fn (TestRunner $t) => $t->same(false, array_key_exists('wp-reader', $writer()['events'][6]['next']['selected']['holders'])),
    ...
    'vfs wal shm lock byte current source replay current reader present' => static fn (TestRunner $t) => $t->same(['wp-reader' => 'shared'], $replay()['current']['selected']['holders']),
    ...
    'vfs wal shm lock byte current source replay reader gone' => static fn (TestRunner $t) => $t->same(false, array_key_exists('wp-reader', $replay()['next']['selected']['holders'])),
    'vfs wal shm lock byte current source replay import exclusive' => static fn (TestRunner $t) => $t->same('exclusive', $replay()['next']['selected']['holders']['wp-import']),
    ...
    'vfs wal shm lock byte current source nolock recover holder' => static fn (TestRunner $t) => $t->same(['wp-repair' => 'exclusive'], $nolock()['next']['selected']['shm_locks']['recover']),

    'vfs wal shm lock byte current source multi source count' => static fn (TestRunner $t) => $t->same(2, $multi()['next']['source_count']),
    'vfs wal shm lock byte current source multi selected path' => static fn (TestRunner $t) => $t->same('/srv/www/wp-content/database/site-meta.sqlite', $multi()['next']['selected_path']),
    'vfs wal shm lock byte current source multi main holder isolated' => static fn (TestRunner $t) => $t->same(['wp-main' => 'shared'], $multi()['next']['sources'][$path]['holders']),
    'vfs wal shm lock byte current source multi meta holder isolated' => static fn (TestRunner $t) => $t->same(['wp-meta' => 'shared'], $multi()['next']['selected']['holders']),
    'vfs wal shm lock byte current source multi meta shm lock' => static fn (TestRunner $t) => $t->same(['wp-meta' => 'shared'], $multi()['next']['selected']['shm_locks']['read0']),
    ...
    'vfs wal shm lock byte current source rejects bad level' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => $run([], [['op' => 'lock', 'level' => 'bogus', 'connection' => 'wp']])),
    'vfs wal shm lock byte current source rejects bad shm lock' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => $run([], [['op' => 'shm', 'lock' => 'read9', 'mode' => 'shared', 'connection' => 'wp']])),
    'vfs wal shm lock byte current source rejects bad shm mode' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => $run([], [['op' => 'shm', 'lock' => 'read0', 'mode' => 'bad', 'connection' => 'wp']])),
    'vfs wal shm lock byte current source rejects bad slot' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => $run([], [['op' => 'lock', 'level' => 'shared', 'connection' => 'wp', 'shared_slot' => 510]])),
];
