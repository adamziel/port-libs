...
): array => SQLiteJsonTablePlan::currentSourceHiddenGeneratedCostNext148(
    'json_tree',
    $current ?? $current148,
    $next ?? $next148,
    'option_value',
...
$tests = [
    ...
    'records next148 dependency' => static fn (TestRunner $t) => $t->true(in_array('sqlite-json-table-hidden-generated-cost-current-source-next148', $plan148()['dependencies'], true)),
    'preserves next143 dependency' => static fn (TestRunner $t) => $t->true(in_array('sqlite-json-table-hidden-path-generated-current-source-next143', $plan148()['dependencies'], true)),
    ...
    'stable has no next148 replan reasons' => static fn (TestRunner $t) => $t->same([], $stable148()['next148ReplanReasons']),
    ...
    'reasons include values change' => static fn (TestRunner $t) => $t->true(in_array('json-table-hidden-generated-cost-values-changed', $plan148()['next148ReplanReasons'], true)),
    'reasons include row estimate change' => static fn (TestRunner $t) => $t->true(in_array('json-table-hidden-generated-cost-row-estimate-changed', $plan148()['next148ReplanReasons'], true)),
    'reasons include estimate change' => static fn (TestRunner $t) => $t->true(in_array('json-table-hidden-generated-cost-estimate-changed', $plan148()['next148ReplanReasons'], true)),
    'reasons include fingerprint change' => static fn (TestRunner $t) => $t->true(in_array('json-table-hidden-generated-cost-fingerprint-changed', $plan148()['next148ReplanReasons'], true)),
    'reasons preserve next143 value change' => static fn (TestRunner $t) => $t->true(in_array('json-table-hidden-path-generated-value-changed', $plan148()['next148ReplanReasons'], true)),
    'jsonb next source kind changes' => static fn (TestRunner $t) => $t->same('jsonb', $jsonb148()['nextHiddenGeneratedCost']['sourceKind']),
    'jsonb reason includes source kind' => static fn (TestRunner $t) => $t->true(in_array('json-table-hidden-generated-cost-source-kind-changed', $jsonb148()['next148ReplanReasons'], true)),
    'missing generated constraints rejected' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteJsonTablePlan::currentSourceHiddenGeneratedCostNext148('json_tree', $current148, $next148, 'option_value', $constraints148, 'scan_root', [], [])),
...
foreach ($tests as $name => $case) {
    $tests['json table hidden generated cost current source next148 ' . $name] = $case;
    unset($tests[$name]);
}
