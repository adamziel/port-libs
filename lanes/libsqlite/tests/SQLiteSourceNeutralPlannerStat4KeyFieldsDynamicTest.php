...
$domainMatches = static function (array $methods) use ($methodSource): array {
    $matches = [];
    $pattern = '/wp_|wp_options|blog_id|blogId|option_name|optionName|option_value|optionValue|autoload|Autoload|plugin_|theme_mods|siteurl/';
...
return [
    ...
    'planner stat4 sample row guard like helper uses generic wildcard' => static fn (TestRunner $t) => $t->same(true, $callPrivate('likePrefixForSampleRowGuardFence', 'moduleB_forms', 'module_%')),
    'planner stat4 density vector like helper uses generic wildcard' => static fn (TestRunner $t) => $t->same(true, $callPrivate('likePrefixCurrentSourceStat4DensityVectorValidation', 'moduleC_forms', 'module_%')),
    'planner stat4 histogram like helper uses generic wildcard' => static fn (TestRunner $t) => $t->same(true, $callPrivate('likePrefixCurrentSourceHistogramValidation', 'moduleD_forms', 'module_%')),
    'planner stat4 sample anchor like helper uses generic wildcard' => static fn (TestRunner $t) => $t->same(true, $callPrivate('likePrefixStat4SampleAnchor', 'moduleE_forms', 'module_%')),
    'planner stat4 duplicate peer like helper uses generic wildcard' => static fn (TestRunner $t) => $t->same(true, $callPrivate('duplicatePeerLikePrefix', 'moduleF_forms', 'module_%')),
    'planner stat4 scan direction like helper uses generic wildcard' => static fn (TestRunner $t) => $t->same(true, $callPrivate('currentScanDirectionLikePrefix', 'moduleG_forms', 'module_%')),
