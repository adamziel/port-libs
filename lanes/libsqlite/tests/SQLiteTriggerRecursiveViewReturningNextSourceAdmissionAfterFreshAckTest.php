...
$plan189 = static fn (array $options = []): array => SQLiteTriggerRecursiveViewReturningCurrentSourceNextPlan::executeNextSourceAdmissionAfterFreshAck(
    ...
    $options + [
        ...
        'current_generation' => 'app-current-returning-189',
        'next_generation' => 'app-next-returning-189',
        'page_size' => 3,
        'admit_next_source' => true,
        'current_source_token' => 'app.current.source.189',
        'drain_ack_token' => 'app.returning.drain.189',
        'rollback_token' => 'app.rollback.current.189',
...
$tokenHeld189 = static fn (): array => $plan189(['expected_next_source_token' => 'app.next.source.expected.189']);
$generationHeld189 = static fn (): array => $plan189(['expected_reset_generation' => 'app-current-reset-expected-189']);
$postResetHeld189 = static fn (): array => $plan189(['reuse_stale_returning_cursor' => true]);
$custom189 = static fn (): array => $plan189(['next_source_token' => 'app.next.source.custom.189', 'expected_next_source_token' => 'app.next.source.custom.189', 'next_cursor' => 'app.returning.next.custom.cursor.189']);
...
$cases189 = [
    ...
    'savepoint retained' => [static fn (): mixed => $admitted189()['savepoint'], 'app_recursive_view_189'],
    ...
    'next token retained' => [static fn (): mixed => $admitted189()['next_source_token_next189'], 'app.next.source.189'],
    'expected next token retained' => [static fn (): mixed => $admitted189()['expected_next_source_token_next189'], 'app.next.source.189'],
    ...
    'reset generation retained' => [static fn (): mixed => $admitted189()['expected_reset_generation_next189'], 'app-current-reset-189'],
    ...
    'next cursor retained' => [static fn (): mixed => $admitted189()['next_cursor_next189'], 'app.returning.next.cursor.189'],
    'custom cursor retained' => [static fn (): mixed => $custom189()['next_cursor_next189'], 'app.returning.next.custom.cursor.189'],
    ...
    'next payload names' => [static fn (): mixed => array_column($admitted189()['next_source_payloads_next189'], 'name'), ['landing_url', 'next_module']],
    ...
    'next token stamped' => [static fn (): mixed => array_values(array_unique(array_column($admitted189()['next_source_rows_next189'], 'next_source_token_next189'))), ['app.next.source.189']],
    'next cursor stamped' => [static fn (): mixed => array_values(array_unique(array_column($admitted189()['next_source_rows_next189'], 'next_cursor_next189'))), ['app.returning.next.cursor.189']],
    'next generation stamped' => [static fn (): mixed => array_values(array_unique(array_column($admitted189()['next_source_rows_next189'], 'next_generation_next189'))), ['app-next-returning-189']],
    'next setting keys stamped' => [static fn (): mixed => array_column($admitted189()['next_source_rows_next189'], 'returning_key_name'), ['landing_url', 'next_module']],
    ...
    'handoff next cursor' => [static fn (): mixed => $admitted189()['handoff_plan_next189']['next_cursor'], 'app.returning.next.cursor.189'],
