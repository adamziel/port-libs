...
use PortLibs\LibSqlite\SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext169Plan;
...
$summary169 = static fn (): array => SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext169Plan::compare($sql169, $currentTables169, $nextTables169);
$tests = [];

$tests['compound select window recursive limit current source next169 status dependencies'] = static function (TestRunner $t) use ($summary169): void {
    $plan = $summary169();
    $t->same('compound-select-window-recursive-limit-current-source-next169-ready', $plan['status']);
    $t->same([
...
$tests['compound select window recursive limit current source next169 compound metadata'] = static function (TestRunner $t) use ($summary169): void {
...
$tests['compound select window recursive limit current source next169 current rows'] = static function (TestRunner $t) use ($summary169): void {
...
$tests['compound select window recursive limit current source next169 next rows'] = static function (TestRunner $t) use ($summary169): void {
...
$tests['compound select window recursive limit current source next169 recursive comma limit trace'] = static function (TestRunner $t) use ($summary169): void {
...
$tests['compound select window recursive limit current source next169 window metadata'] = static function (TestRunner $t) use ($summary169): void {
...
$tests['compound select window recursive limit current source next169 prelimit trace'] = static function (TestRunner $t) use ($summary169): void {
    $plan = $summary169();
...
$tests['compound select window recursive limit current source next169 bucket delta'] = static function (TestRunner $t) use ($summary169): void {
...
$tests['compound select window recursive limit current source next169 boundary and reasons'] = static function (TestRunner $t) use ($summary169): void {
    $plan = $summary169();
...
$tests['compound select window recursive limit current source next169 rejects missing comma limit'] = static function (TestRunner $t) use ($currentTables169): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext169Plan::compare(
...
$tests['compound select window recursive limit current source next169 rejects missing ntile'] = static function (TestRunner $t) use ($currentTables169): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext169Plan::compare(
...
$tests['compound select window recursive limit current source next169 rejects non compound'] = static function (TestRunner $t) use ($currentTables169): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext169Plan::compare(
...
foreach (range(1, 58) as $case) {
    $tests['compound select window recursive limit current source next169 generated ntile comma limit boundary ' . $case] = static function (TestRunner $t) use ($case): void {
        $tables = [
            'wp_options' => [
