...
$plan97 = static fn (?array $prepared = null, ?array $states = null): array => SQLiteAttachTempWalSchemaTriggerPlan::triggerViewCacheRepreparePlan(
    $current97(),
    $next97(),
    $prepared ?? $prepared97,
    $states ?? $states97(),
...
foreach ($cases97 as $name => [$path, $expected]) {
    $tests['attach temp wal trigger view cache reprepare97 ' . $name] = static function (TestRunner $t) use ($plan97, $value97, $path, $expected): void {
        $t->same($expected, $value97($plan97(), $path));
    };
}

$tests['attach temp wal trigger view cache reprepare97 stable-only view trigger is reusable'] = static function (TestRunner $t) use ($plan97): void {
...
$tests['attach temp wal trigger view cache reprepare97 active flag controls current source retention'] = static function (TestRunner $t) use ($plan97): void {
...
$tests['attach temp wal trigger view cache reprepare97 explicit wal cookie overrides frame'] = static function (TestRunner $t) use ($plan97, $states97): void {
    $states = $states97();
    $states['site']['wal_schema_cookie'] = 44;
    $t->same(44, $plan97(null, $states)['schema_cookies_next']['site']);
};

$tests['attach temp wal trigger view cache reprepare97 rejects empty prepared list'] = static function (TestRunner $t) use ($current97, $next97): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteAttachTempWalSchemaTriggerPlan::triggerViewCacheRepreparePlan($current97(), $next97(), []));
};

$tests['attach temp wal trigger view cache reprepare97 rejects table trigger without view cache only when missing trigger'] = static function (TestRunner $t) use ($current97, $next97): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteAttachTempWalSchemaTriggerPlan::triggerViewCacheRepreparePlan($current97(), $next97(), [['name' => 'missing_view_trigger']]));
};
