...
$siteTriggerViewRecords = static fn (int $root): array => [
...
$tempTriggerViewRecords = static fn (int $root): array => [
...
$triggerPlan93 = static fn (): array => SQLiteAttachTempWalSchemaTriggerPlan::walTriggerCookieCachePlan(
    ...
    ['site' => $siteTriggerViewRecords(50)],
    [
...
$viewPlan93 = static fn (): array => SQLiteAttachWalTempViewCachePlan::preparedViewCacheRepreparePlan(
    ...
    ['temp' => $tempTriggerViewRecords(70)],
    [
...
foreach ($cases93 as $name => [$factory, $path, $expected]) {
    $tests['attach temp wal trigger view cache reprepare ' . $name] = static function (TestRunner $t) use ($factory, $value93, $path, $expected): void {
        $t->same($expected, $value93($factory(), $path));
    };
}

$tests['attach temp wal trigger view cache reprepare rejects empty bracket source'] = static function (TestRunner $t) use ($catalog93): void {
...
$tests['attach temp wal trigger view cache reprepare rejects empty bracket view source'] = static function (TestRunner $t) use ($catalog93): void {
