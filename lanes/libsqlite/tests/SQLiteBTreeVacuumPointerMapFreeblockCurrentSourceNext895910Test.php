...
$makeFirstPage895910 = static function (int $pageCount): string {
...
$putPointerMapEntry895910 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database895910 = static function (int $sliceNumber) use ($makeFirstPage895910, $putPointerMapEntry895910): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage895910(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry895910($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan895910 = static function (int $sliceNumber, int $batchSize = 2) use ($database895910): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database895910($sliceNumber);
...
foreach ([895, 896, 897, 898, 899, 900, 901, 902, 903, 904, 905, 906, 907, 908, 909, 910] as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} extends next895-910 handoff receipts"] = static function (TestRunner $t) use ($plan895910, $sliceNumber): void {
        $plan = $plan895910($sliceNumber);
        ...
        $t->same(6, $plan895910($sliceNumber, 3)->currentSourceSummary()['current_source_row_count']);
    };
}
