...
$plan207 = static fn (
    ...
    int $offset = 0,
): array => SQLiteJsonTablePlan::currentSourceGeneratedPathRowidCostCurrentSourceNext207(
    'json_tree',
    $current ?? $current207,
    $next ?? $next207,
    'option_value',
...
$tests = [
    ...
    'current ordered rowids inherited' => static fn (TestRunner $t) => $t->same([8, 7, 6, 5], $plan207()['currentGeneratedPathRowidAliasLimit207']['orderedRowids']),
    ...
    'current bounded rowids top two' => static fn (TestRunner $t) => $t->same([8, 7], $plan207()['currentGeneratedPathRowidAliasLimit207']['boundedRowids']),
    'current remaining after limit' => static fn (TestRunner $t) => $t->same([6, 5], $plan207()['currentGeneratedPathRowidAliasLimit207']['remainingRowidsAfterLimit']),
    ...
    'current limit tape first rowid' => static fn (TestRunner $t) => $t->same(8, $plan207()['currentGeneratedPathRowidAliasLimit207']['limitTape'][0]['rowid']),
    ...
    'offset skipped rowids' => static fn (TestRunner $t) => $t->same([8], $offset207()['currentGeneratedPathRowidAliasLimit207']['skippedRowids']),
    'offset bounded rowids' => static fn (TestRunner $t) => $t->same([7, 6], $offset207()['currentGeneratedPathRowidAliasLimit207']['boundedRowids']),
    'offset remaining rowids' => static fn (TestRunner $t) => $t->same([5], $offset207()['currentGeneratedPathRowidAliasLimit207']['remainingRowidsAfterLimit']),
    ...
    'ascending offset bounded rowids' => static fn (TestRunner $t) => $t->same([8, 9], $ascending207()['currentGeneratedPathRowidAliasLimit207']['boundedRowids']),
    'ascending offset skipped rowid' => static fn (TestRunner $t) => $t->same([7], $ascending207()['currentGeneratedPathRowidAliasLimit207']['skippedRowids']),
    ...
    'no limit bounded all rowids' => static fn (TestRunner $t) => $t->same([8, 7, 6, 5], $noLimit207()['currentGeneratedPathRowidAliasLimit207']['boundedRowids']),
    ...
    'bad function rejected' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteJsonTablePlan::currentSourceGeneratedPathRowidCostCurrentSourceNext207('json_bad', $current207, $current207, 'option_value', 'generated_path')),
    'dependency closure' => static fn (TestRunner $t) => $t->same('no-new-support-component', 'no-new-support-component'),
];

foreach ($tests as $name => $case) {
    $tests['json table generated path rowid cost current source next207 ' . $name] = $case;
    unset($tests[$name]);
}
