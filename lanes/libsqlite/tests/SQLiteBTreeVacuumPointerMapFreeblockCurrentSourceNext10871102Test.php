...
$makeFirstPage10871102 = static function (int $pageCount): string {
...
$putPointerMapEntry10871102 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database10871102 = static function (int $sliceNumber) use ($makeFirstPage10871102, $putPointerMapEntry10871102): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage10871102(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry10871102($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan10871102 = static function (int $sliceNumber, int $batchSize = 2) use ($database10871102): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database10871102($sliceNumber);
...
foreach (range(1087, 1102) as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} continues next1087-1102 handoff receipts"] = static function (TestRunner $t) use ($plan10871102, $sliceNumber): void {
        $plan = $plan10871102($sliceNumber);
        ...
        $t->same(6, $plan10871102($sliceNumber, 3)->currentSourceSummary()['current_source_row_count']);
    };
}
