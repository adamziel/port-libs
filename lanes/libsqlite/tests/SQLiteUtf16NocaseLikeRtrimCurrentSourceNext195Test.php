...
$plan195 = static fn (
    ...
    int $nextCookie = 195,
): array => SQLiteUtf16NocaseLikeRtrimCurrentSourceNextPlan::keyValueRowKeyEscapedLiteralTailPlan(
    $current ?? $current195,
...
$cases195 = [
    ...
    'escape' => ['escape', '!'],
    'current source' => ['currentSource', 'main.app_settings@194'],
    'next source' => ['nextSource', 'main.app_settings@195'],
...
$tests['utf16 nocase like rtrim current source nextOneNineFive stable exact literal cursor is reusable'] = static function (TestRunner $t) use ($row195): void {
    ...
    $result = SQLiteUtf16NocaseLikeRtrimCurrentSourceNextPlan::keyValueRowKeyEscapedLiteralTailPlan(
        $rows,
        $rows,
        'plugin!_!%!_cache',
        '!',
...
$tests['utf16 nocase like rtrim current source nextOneNineFive demoted exact row forces residual restart'] = static function (TestRunner $t) use ($row195): void {
    ...
    $result = SQLiteUtf16NocaseLikeRtrimCurrentSourceNextPlan::keyValueRowKeyEscapedLiteralTailPlan($current, $next, 'plugin!_!%!_cache', '!', 'same', 'same', 1, 1);
    $t->same([1, 2], $result['currentMatchedRowids']);
