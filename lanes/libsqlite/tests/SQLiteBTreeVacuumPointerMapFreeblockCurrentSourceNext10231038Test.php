...
$makeFirstPage10231038 = static function (int $pageCount): string {
...
$putPointerMapEntry10231038 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database10231038 = static function (int $sliceNumber) use ($makeFirstPage10231038, $putPointerMapEntry10231038): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage10231038(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry10231038($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan10231038 = static function (int $sliceNumber, int $batchSize = 2) use ($database10231038): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database10231038($sliceNumber);
...
foreach (range(1023, 1038) as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} extends next1023-1038 handoff receipts"] = static function (TestRunner $t) use ($plan10231038, $sliceNumber): void {
        $plan = $plan10231038($sliceNumber);
        ...
        $t->same(6, $plan10231038($sliceNumber, 3)->currentSourceSummary()['current_source_row_count']);
    };
}
