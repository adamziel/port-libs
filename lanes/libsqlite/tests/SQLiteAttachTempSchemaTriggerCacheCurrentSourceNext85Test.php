...
$mainNext85 = static fn (int $root = 40): array => [
...
$tempNext85 = static fn (int $root = 50): array => [
...
$siteNext85 = static fn (int $root = 60): array => [
...
$archiveNext85 = static fn (int $root = 70): array => [
...
foreach ($pathCases85 as $name => [$factory, $path, $expected]) {
    $tests['attach temp schema trigger cache current source next85 ' . $name] = static function (TestRunner $t) use ($factory, $value85, $path, $expected): void {
...
foreach ($predicateCases85 as $name => $predicate) {
    $tests['attach temp schema trigger cache current source next85 ' . $name] = static function (TestRunner $t) use ($predicate): void {
...
foreach ($errorCases85 as $name => $callback) {
    $tests['attach temp schema trigger cache current source next85 ' . $name] = static function (TestRunner $t) use ($callback): void {
