...
use PortLibs\LibSqlite\SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext237Plan;
...
$summary237 = static fn (?array $cursor = null): array => SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext237Plan::compare($sql237, $currentTables237, $nextTables237, $cursor);
$tests = [];
...
$tests['compound select window recursive limit current source next237 rejects missing intersect'] = static function (TestRunner $t) use ($currentTables237): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext237Plan::compare(
...
foreach (range(1, 58) as $case) {
    $tests['compound select window recursive limit current source next237 generated dequeue fence ' . $case] = static function (TestRunner $t) use ($case): void {
        ...
        $plan = SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext237Plan::compare($sql, $tables, $nextTables);
        $cursor = $plan['cursor'];
        $cursor['acknowledgedCurrentDequeueAcksNext237'] = $plan['currentSourceDequeueNext237']['requiredCurrentDequeueAcks'];
        $again = SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext237Plan::compare($sql, $tables, $nextTables, $cursor);
