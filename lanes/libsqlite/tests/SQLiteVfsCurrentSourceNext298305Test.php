...
$plan = static function () use ($readyCurrent): array {
    static $result = null;
    if ($result === null) {
        $result = SQLiteVfsCurrentSourceNext298305Plan::run([
...
return [
    ...
    'vfs current source next298-305 blocks dirty snapshot' => static fn (TestRunner $t) => $t->same(true, in_array('dirty-pages-present', SQLiteVfsCurrentSourceNext298305Plan::run(['snapshot(reader-ready-next305,shared-cache-next297)'], ['current' => $dirtyCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source next298-305 blocks old snapshot ack' => static fn (TestRunner $t) => $t->same(true, in_array('ack-not-latest-publish', SQLiteVfsCurrentSourceNext298305Plan::run(['snapshot(reader-ready-next305,shared-cache-next297)'], ['current' => $oldAckCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source next298-305 blocks publish without claim' => static fn (TestRunner $t) => $t->same(true, in_array('missing-reuse-claim', SQLiteVfsCurrentSourceNext298305Plan::run(['snapshot(reader-ready-next305,shared-cache-next297)', 'publish(reader-ready-next305,reader-reuse-next305,shared-cache-next305)'], ['current' => $readyCurrent])['events'][1]['blocked_reasons'], true)),
    'vfs current source next298-305 blocks stale claim' => static fn (TestRunner $t) => $t->same(true, in_array('stale-reuse-claim', SQLiteVfsCurrentSourceNext298305Plan::run(['publish(reader-ready-next305,reader-reuse-next305,shared-cache-next305)'], ['current' => $staleClaimCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source next298-305 accepts preclaimed publish' => static fn (TestRunner $t) => $t->same('published-reused-current-source', SQLiteVfsCurrentSourceNext298305Plan::run(['publish(reader-ready-next305,reader-reuse-next305,shared-cache-next305)'], ['current' => $claimedCurrent])['events'][0]['status']),
    'vfs current source next298-305 rejects empty operations' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteVfsCurrentSourceNext298305Plan::run([])),
    'vfs current source next298-305 rejects bad claim token' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteVfsCurrentSourceNext298305Plan::run([['op' => 'claim', 'snapshot' => 'reader-ready-next305', 'ack' => 'shared-cache-next297', 'claim' => 'bad claim']], ['current' => $readyCurrent])),
    'vfs current source next298-305 rejects unsupported operation' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteVfsCurrentSourceNext298305Plan::run(['republish(reader-ready-next305,shared-cache-next297)'], ['current' => $readyCurrent])),
