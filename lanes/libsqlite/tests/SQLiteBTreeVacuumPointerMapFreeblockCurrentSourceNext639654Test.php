...
$makeFirstPage639654 = static function (int $pageCount): string {
...
$putPointerMapEntry639654 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database639654 = static function (int $sliceNumber) use ($makeFirstPage639654, $putPointerMapEntry639654): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage639654(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry639654($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan639654 = static function (int $sliceNumber, int $batchSize = 2) use ($database639654): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database639654($sliceNumber);
...
foreach ([639, 640, 641, 642, 643, 644, 645, 646, 647, 648, 649, 650, 651, 652, 653, 654] as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} publishes next639-654 handoff receipts"] = static function (TestRunner $t) use ($plan639654, $sliceNumber): void {
        $plan = $plan639654($sliceNumber);
        ...
        $t->same(6, $plan639654($sliceNumber, 3)->currentSourceSummary()['current_source_row_count']);
    };
}
