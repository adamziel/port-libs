...
// own 0000..4999 and 6000..8999, while a historical pending shard owns
// 5000..5999, so this shard owns the final non-overlapping 9000..9999 suffix
// window.
$firstSuffix = 9000;
$lastSuffix = 9999;
$cases = [];
...
$scriptFile = tempnam(sys_get_temp_dir(), 'libsqlite-atof1-decimal-9000-oracle-');
if ($scriptFile === false) {
...
$tests['real upstream corpus date affinity dynamic atof1 decimal REAL suffix 9000 9999 source truth'] =
    static function (TestRunner $t) use ($cases, $firstSuffix, $lastSuffix, $oracle, $sourceFile, $sourceText): void {
        ...
        $t->same(9000, $firstSuffix);
        $t->same(9999, $lastSuffix);
        $t->same(9000, $cases['suffix-9000']['suffix']);
        $t->same(9999, $cases['suffix-9999']['suffix']);
        $t->same('18.446744073709559000', $cases['suffix-9000']['value']);
        $t->same('18.446744073709559999', $cases['suffix-9999']['value']);
    };

foreach ($cases as $key => $case) {
    $tests['real upstream corpus date affinity dynamic atof1 decimal REAL suffix 9000 9999 ' . $key] =
...
$tests['real upstream corpus date affinity dynamic atof1 decimal REAL suffix 9000 9999 generic application rollup'] =
    static function (TestRunner $t) use ($cases): void {
        $rows = [
            ['key_name' => 'metric-low', 'decimal_metric' => $cases['suffix-9000']['value']],
            ['key_name' => 'metric-mid', 'decimal_metric' => $cases['suffix-9500']['value']],
            ['key_name' => 'metric-high', 'decimal_metric' => $cases['suffix-9999']['value']],
        ];
        $result = SQLiteSelectSql::execute(
            'SELECT key_name, '
...
$tests['real upstream corpus date affinity dynamic atof1 decimal REAL suffix 9000 9999 non overlap and dependency closure'] =
    static function (TestRunner $t): void {
        $t->same(
            'owns atof1.test atof-3.2 decimal REAL suffixes 9000..9999 only',
            'owns atof1.test atof-3.2 decimal REAL suffixes 9000..9999 only',
        );
        $t->same(
            'non-overlap: avoids accepted atof-3.2 decimal REAL/mantissa suffixes 0000..4999 and 6000..8999, historical pending suffixes 5000..5999, accepted atof-3.1 integer-prefix suffixes 0592..1609, atof-3.3 exponent rows, atof2 rounding, date4 rows, date/timediff matrices, and types storage-class batches',
            'non-overlap: avoids accepted atof-3.2 decimal REAL/mantissa suffixes 0000..4999 and 6000..8999, historical pending suffixes 5000..5999, accepted atof-3.1 integer-prefix suffixes 0592..1609, atof-3.3 exponent rows, atof2 rounding, date4 rows, date/timediff matrices, and types storage-class batches',
        );
        $t->same(
