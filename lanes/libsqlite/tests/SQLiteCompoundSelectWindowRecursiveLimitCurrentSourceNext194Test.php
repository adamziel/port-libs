...
use PortLibs\LibSqlite\SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext194Plan;
...
$summary194 = static fn (?array $cursor = null): array => SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext194Plan::compare($sql194, $currentTables194, $nextTables194, $cursor);
$tests = [];

$tests['compound select window recursive limit next194 status dependencies'] = static function (TestRunner $t) use ($summary194): void {
    $plan = $summary194();
    $t->same('compound-select-window-recursive-limit-current-source-next194-ready', $plan['status']);
    $t->same([
        'sqlite-select-sql-compound-intersect-except-next194',
        'sqlite-window-before-compound-membership-next194',
        'sqlite-recursive-limit-current-source-next194',
    ], $plan['dependencies']);
};

$tests['compound select window recursive limit next194 compound metadata'] = static function (TestRunner $t) use ($summary194): void {
...
$tests['compound select window recursive limit next194 recursive trace'] = static function (TestRunner $t) use ($summary194): void {
...
$tests['compound select window recursive limit next194 windows'] = static function (TestRunner $t) use ($summary194): void {
...
$tests['compound select window recursive limit next194 current membership'] = static function (TestRunner $t) use ($summary194): void {
...
$tests['compound select window recursive limit next194 next membership shifts'] = static function (TestRunner $t) use ($summary194): void {
    $boundary = $summary194()['membershipBoundary'];
...
$tests['compound select window recursive limit next194 cursor accepts current token'] = static function (TestRunner $t) use ($summary194): void {
    $first = $summary194();
...
$tests['compound select window recursive limit next194 rejects stale cursor'] = static function (TestRunner $t) use ($summary194): void {
...
$tests['compound select window recursive limit next194 replan reasons'] = static function (TestRunner $t) use ($summary194): void {
    $reasons = $summary194()['replanReasons'];
    $t->true(in_array('compound-intersect-except-window-recursive-limit-next194', $reasons, true));
    $t->true(in_array('window-values-before-compound-membership-next194', $reasons, true));
    $t->true(in_array('recursive-limit-current-source-token-next194', $reasons, true));
    $t->true(in_array('wordpress-option-preview-stale-membership-fence-next194', $reasons, true));
};

$tests['compound select window recursive limit next194 dependency closure and non overlap'] = static function (TestRunner $t) use ($summary194): void {
    $plan = $summary194();
...
$tests['compound select window recursive limit next194 rejects missing intersect'] = static function (TestRunner $t) use ($currentTables194): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext194Plan::compare(
...
$tests['compound select window recursive limit next194 rejects missing except'] = static function (TestRunner $t) use ($currentTables194): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext194Plan::compare(
...
$tests['compound select window recursive limit next194 rejects missing final limit'] = static function (TestRunner $t) use ($currentTables194): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext194Plan::compare(
...
foreach (range(1, 54) as $case) {
    $tests['compound select window recursive limit next194 generated intersect except fence ' . $case] = static function (TestRunner $t) use ($case): void {
        $tables = [
            'wp_options' => [
        ...
        $plan = SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext194Plan::compare($generatedSql, $tables, $nextTables);
        $again = SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext194Plan::compare($generatedSql, $tables, $nextTables, $plan['cursor']);
