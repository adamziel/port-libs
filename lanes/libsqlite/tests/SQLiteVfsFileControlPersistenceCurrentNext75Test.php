...
foreach ($durableCases as $name => [$op, $value, $expected]) {
    $tests['vfs filecontrol persistence current next75 persists durable ' . $name] = static function (TestRunner $t) use ($makeRoot, $op, $value, $expected): void {
        $root = $makeRoot(str_replace(' ', '-', $op));
        $persistence = new SQLiteVfsFileControlPersistence($root);
        $result = $persistence->currentNext75(
        ...
        $t->same(true, in_array('vfs-file-control-persistence-current-next75', $result['dependencies'], true));
    };
}
...
foreach ($ephemeralCases as $name => [$op, $value, $expected]) {
    $tests['vfs filecontrol persistence current next75 drops ephemeral ' . $name] = static function (TestRunner $t) use ($makeRoot, $op, $value, $expected): void {
        $root = $makeRoot(str_replace(' ', '-', $op));
        $persistence = new SQLiteVfsFileControlPersistence($root);
        $result = $persistence->currentNext75(
...
foreach ($sequenceCases as $name => [$controls, $expectedPersisted]) {
    $tests['vfs filecontrol persistence current next75 sequence ' . $name] = static function (TestRunner $t) use ($makeRoot, $controls, $expectedPersisted): void {
...
$tests['vfs filecontrol persistence current next75 rehydrates second current from sidecar'] = static function (TestRunner $t) use ($makeRoot): void {
...
$tests['vfs filecontrol persistence current next75 readonly open can rehydrate but ignores writes'] = static function (TestRunner $t) use ($makeRoot): void {
...
$tests['vfs filecontrol persistence current next75 nolock open blocks lock before persistence'] = static function (TestRunner $t) use ($makeRoot): void {
...
$invalidCases = [
    ...
    'empty controls' => static fn (): array => (new SQLiteVfsFileControlPersistence(sys_get_temp_dir()))->currentNext75('file:/tmp/site.sqlite?mode=rwc', false, true, []),
];

foreach ($invalidCases as $name => $callback) {
    $tests['vfs filecontrol persistence current next75 rejects ' . $name] = static function (TestRunner $t) use ($callback): void {
        $t->throws(InvalidArgumentException::class, $callback);
    };
}

$tests['vfs filecontrol persistence current next75 rejects unopenable current handle'] = static function (TestRunner $t): void {
    $persistence = new SQLiteVfsFileControlPersistence(sys_get_temp_dir());

    $t->throws(RuntimeException::class, static fn () => $persistence->currentNext75('file:/tmp/missing.sqlite?mode=rw', false, false, ['chunk_size' => 4096]));
};
...
foreach ($roundTripCases as $name => [$left, $right, $same]) {
    $tests['vfs filecontrol persistence current next75 sidecar ' . $name] = static function (TestRunner $t) use ($left, $right, $same): void {
        $persistence = new SQLiteVfsFileControlPersistence(sys_get_temp_dir());
