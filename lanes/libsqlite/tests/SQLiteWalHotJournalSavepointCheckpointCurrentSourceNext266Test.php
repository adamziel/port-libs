...
$plan = static fn (?array $inputBase = null, ?array $receipts = null): array =>
    SQLiteWalHotJournalSavepointCheckpointCurrentSourceNextPlan::next266AfterCurrentCheckpoint($inputBase ?? $base, $receipts ?? [$receipt]);
...
$tests['wal hot journal savepoint checkpoint current source next266 rejects wrong base'] = static function (TestRunner $t) use ($plan, $base): void {
    $t->throws(Throwable::class, static fn () => $plan(array_replace($base, ['status' => 'wal-hot-journal-savepoint-checkpoint-current-source-next264'])));
};
