...
$makeFirstPage559574 = static function (int $pageCount): string {
...
$putPointerMapEntry559574 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database559574 = static function (int $sliceNumber) use ($makeFirstPage559574, $putPointerMapEntry559574): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage559574(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry559574($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan559574 = static function (int $sliceNumber, int $batchSize = 2) use ($database559574): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourcePlan {
    $database = $database559574($sliceNumber);
...
foreach ([559, 560, 561, 562, 563, 564, 565, 566, 567, 568, 569, 570, 571, 572, 573, 574] as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} publishes next559-574 handoff receipts"] = static function (TestRunner $t) use ($plan559574, $sliceNumber): void {
        $plan = $plan559574($sliceNumber);
        ...
        $t->same(6, $plan559574($sliceNumber, 3)->currentSourceSummary()['current_source_row_count']);
    };
}
