...
$plan = static fn (
    ...
    ?array $current = null,
    ?array $next = null,
    string $currentSource = 'main.wp_options@138',
    string $nextSource = 'main.wp_options@139',
    int $currentSchemaCookie = 138,
    int $nextSchemaCookie = 139,
): array => SQLiteNocaseGlobAffinityCurrentSourceNext139Plan::wordpressOptionNamePlan(
    $current ?? $currentRows,
    $next ?? $nextRows,
    $pattern,
    $affinity,
...
$cases = [
    ...
    'current source recorded' => ['plugin_*', 'TEXT', 'NOCASE', 'currentSource', 'main.wp_options@138'],
    'next source recorded' => ['plugin_*', 'TEXT', 'NOCASE', 'nextSource', 'main.wp_options@139'],
...
$tests['nocase glob affinity current source next139 stable binary source reusable'] = static function (TestRunner $t): void {
    ...
    $plan = SQLiteNocaseGlobAffinityCurrentSourceNext139Plan::wordpressOptionNamePlan($rows, $rows, 'plugin_*', 'TEXT', 'BINARY', 'stable', 'stable', 9, 9);
...
$tests['nocase glob affinity current source next139 stable nocase still records range rejection'] = static function (TestRunner $t): void {
    ...
    $plan = SQLiteNocaseGlobAffinityCurrentSourceNext139Plan::wordpressOptionNamePlan($rows, $rows, 'plugin_*', 'TEXT', 'NOCASE', 'stable', 'stable', 9, 9);
...
$tests['nocase glob affinity current source next139 rejects unsupported collation'] = static function (TestRunner $t) use ($currentRows): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteNocaseGlobAffinityCurrentSourceNext139Plan::wordpressOptionNamePlan($currentRows, $currentRows, 'plugin_*', 'TEXT', 'UNICODE'));
};

$tests['nocase glob affinity current source next139 rejects missing option name'] = static function (TestRunner $t): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteNocaseGlobAffinityCurrentSourceNext139Plan::wordpressOptionNamePlan([['option_id' => 1]], [], 'plugin_*'));
};

$tests['nocase glob affinity current source next139 rejects non integer option id'] = static function (TestRunner $t): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteNocaseGlobAffinityCurrentSourceNext139Plan::wordpressOptionNamePlan([['option_id' => '1', 'option_name' => 'plugin_alpha']], [], 'plugin_*'));
};

$tests['nocase glob affinity current source next139 rejects blob option name'] = static function (TestRunner $t): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteNocaseGlobAffinityCurrentSourceNext139Plan::wordpressOptionNamePlan([['option_id' => 1, 'option_name' => new SQLiteBlobValue('plugin_alpha')]], [], 'plugin_*'));
};

$tests['nocase glob affinity current source next139 rejects null option name'] = static function (TestRunner $t): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteNocaseGlobAffinityCurrentSourceNext139Plan::wordpressOptionNamePlan([['option_id' => 1, 'option_name' => null]], [], 'plugin_*'));
};

$tests['nocase glob affinity current source next139 rejects malformed utf8'] = static function (TestRunner $t): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteNocaseGlobAffinityCurrentSourceNext139Plan::wordpressOptionNamePlan([['option_id' => 1, 'option_name' => "plugin_\xc3"]], [], 'plugin_*'));
};
