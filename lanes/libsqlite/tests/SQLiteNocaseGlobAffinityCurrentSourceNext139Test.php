...
): array => SQLiteNocaseGlobAffinityCurrentSourceNextPlan::wordpressOptionNamePlan(
    $current ?? $currentRows,
    $next ?? $nextRows,
    $pattern,
    $affinity,
...
$tests['nocase glob affinity current source next139 stable binary source reusable'] = static function (TestRunner $t): void {
    ...
    $plan = SQLiteNocaseGlobAffinityCurrentSourceNextPlan::wordpressOptionNamePlan($rows, $rows, 'plugin_*', 'TEXT', 'BINARY', 'stable', 'stable', 9, 9);
...
$tests['nocase glob affinity current source next139 stable nocase still records range rejection'] = static function (TestRunner $t): void {
    ...
    $plan = SQLiteNocaseGlobAffinityCurrentSourceNextPlan::wordpressOptionNamePlan($rows, $rows, 'plugin_*', 'TEXT', 'NOCASE', 'stable', 'stable', 9, 9);
...
$tests['nocase glob affinity current source next139 rejects unsupported collation'] = static function (TestRunner $t) use ($currentRows): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteNocaseGlobAffinityCurrentSourceNextPlan::wordpressOptionNamePlan($currentRows, $currentRows, 'plugin_*', 'TEXT', 'UNICODE'));
};

$tests['nocase glob affinity current source next139 rejects missing option name'] = static function (TestRunner $t): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteNocaseGlobAffinityCurrentSourceNextPlan::wordpressOptionNamePlan([['option_id' => 1]], [], 'plugin_*'));
};

$tests['nocase glob affinity current source next139 rejects non integer option id'] = static function (TestRunner $t): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteNocaseGlobAffinityCurrentSourceNextPlan::wordpressOptionNamePlan([['option_id' => '1', 'option_name' => 'plugin_alpha']], [], 'plugin_*'));
};

$tests['nocase glob affinity current source next139 rejects blob option name'] = static function (TestRunner $t): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteNocaseGlobAffinityCurrentSourceNextPlan::wordpressOptionNamePlan([['option_id' => 1, 'option_name' => new SQLiteBlobValue('plugin_alpha')]], [], 'plugin_*'));
};

$tests['nocase glob affinity current source next139 rejects null option name'] = static function (TestRunner $t): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteNocaseGlobAffinityCurrentSourceNextPlan::wordpressOptionNamePlan([['option_id' => 1, 'option_name' => null]], [], 'plugin_*'));
};

$tests['nocase glob affinity current source next139 rejects malformed utf8'] = static function (TestRunner $t): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteNocaseGlobAffinityCurrentSourceNextPlan::wordpressOptionNamePlan([['option_id' => 1, 'option_name' => "plugin_\xc3"]], [], 'plugin_*'));
};
