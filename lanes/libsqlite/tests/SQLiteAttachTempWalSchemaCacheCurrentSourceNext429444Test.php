...
$schemas429444 = [
    'main' => [
        'schema_cookie' => 414,
        'tables' => ['wp_options', 'wp_posts', 'wp_postmeta', 'wp_users', 'wp_global_styles', 'wp_navigation_menus', 'wp_navigation_redirects', 'wp_navigation_rules_next413'],
        'indexes' => ['wp_options_name', 'wp_posts_type_status_next293', 'wp_postmeta_key', 'wp_users_login_next308', 'wp_global_styles_slug', 'wp_navigation_menus_slug', 'wp_navigation_redirects_slug', 'wp_navigation_rules_slug_next413'],
        'wal_frames' => [
            ['page' => 1, 'schema_cookie' => 414, 'commit' => true],
        ],
    ],
    'temp' => [
        'schema_cookie' => 428,
        'tables' => ['wp_theme_stage', 'wp_theme_stage_preview', 'wp_theme_stage_publish', 'wp_theme_stage_archive', 'wp_theme_stage_diff', 'wp_theme_stage_publish_queue'],
        'indexes' => ['wp_theme_stage_stylesheet_next350', 'wp_theme_stage_preview_key_next366', 'wp_theme_stage_publish_key_next382', 'wp_theme_stage_archive_key_next398', 'wp_theme_stage_diff_key_next414', 'wp_theme_stage_publish_queue_key'],
        'temp' => true,
    ],
    'analytics' => [
        'schema_cookie' => 426,
        'tables' => ['wp_eventmeta', 'wp_event_archive', 'wp_event_rollup_next362', 'wp_event_capacity_next426'],
        'indexes' => ['wp_eventmeta_key_next313', 'wp_event_capacity_day_next426'],
        'file' => '/srv/wp/analytics-next429.sqlite',
    ],
    'media' => [
        'schema_cookie' => 342,
        'tables' => ['wp_media', 'wp_media_metadata_next387'],
        'indexes' => [],
        'file' => '/srv/wp/media-next429.sqlite',
    ],
    'queue' => [
        'schema_cookie' => 393,
        'tables' => ['wp_job_queue', 'wp_job_claims', 'wp_job_history', 'wp_job_deadletter', 'wp_job_retries', 'wp_job_retry_notes', 'wp_job_retry_audit_next419'],
        'indexes' => ['wp_job_queue_token', 'wp_job_claims_token_next323', 'wp_job_history_finished_next339', 'wp_job_deadletter_reason_next360', 'wp_job_retries_job_next376', 'wp_job_retry_notes_job_next392', 'wp_job_retry_audit_job_next424'],
        'file' => '/srv/wp/queue-next429.sqlite',
    ],
    'campaign' => [
        'schema_cookie' => 420,
        'tables' => ['wp_campaign_archive_next404', 'wp_campaign_rollup_next420'],
        'indexes' => ['wp_campaigns_slug', 'wp_campaign_archive_slug_next404', 'wp_campaign_rollup_slug_next420'],
        'file' => '/srv/wp/campaign-next429.sqlite',
    ],
];
...
$plan429444 = static fn (array $events, ?array $statements = null, ?array $schemas = null): array => SQLiteAttachWalTempSchemaCacheCurrentSourceNext92Plan::currentSourceNext429444(
    $schemas ?? $schemas429444,
    $statements ?? $statements429444,
    $events,
);

$tests = [];

$tests['attach temp wal schema cache current source next429-444 extends next413-428'] = static function (TestRunner $t) use ($plan429444): void {
    $result = $plan429444([
    ...
    $t->same('attach-wal-temp-schema-cache-current-source-next429-444', $result['operation']);
    $t->same('sqlite-attach-temp-wal-schema-cache-current-source-next429', $result['dependencies'][0]);
    $t->same('sqlite-attach-temp-wal-schema-cache-current-source-next444', $result['dependencies'][15]);
    $t->same('sqlite-attach-temp-wal-schema-cache-current-source-next428', $result['dependencies'][31]);
    $t->same(15, $result['event_count']);
    $t->same(['temp', 'main', 'analytics', 'campaign', 'media', 'queue'], $result['changed_schemas']);
    $t->same(430, $result['schema_cookies_next']['main']);
    $t->same(444, $result['schema_cookies_next']['temp']);
    $t->same(442, $result['schema_cookies_next']['analytics']);
    $t->same(343, $result['schema_cookies_next']['media']);
    $t->same(395, $result['schema_cookies_next']['queue']);
    $t->same(436, $result['schema_cookies_next']['campaign']);
    $t->same(['temp', 'main', 'analytics', 'campaign', 'media', 'queue'], $result['search_order_next']);
    $t->same(['rules-reader', 'publish-queue-reader', 'queue-audit-reader'], $result['active_current_snapshot_statements']);
    $t->same([], $result['write_statements_blocked_before_retry']);
    $t->same(false, $result['statements']['publish-queue-reader']['index_transitions'][0]['next_found']);
    $t->same(false, $result['statements']['capacity-reader']['schema_transitions'][0]['next_found']);
...
$tests['attach temp wal schema cache current source next429-444 ignores duplicate uncommitted scratch churn'] = static function (TestRunner $t) use ($plan429444): void {
    $result = $plan429444([
        ['op' => 'attach', 'schema' => 'scratch', 'schema_cookie' => 17, 'tables' => ['wp_scratch'], 'indexes' => ['wp_scratch_key'], 'file' => '/srv/wp/scratch-next429.sqlite'],
        ['op' => 'detach', 'schema' => 'scratch'],
        ['op' => 'wal_commit', 'schema' => 'main', 'schema_cookie' => 431, 'table' => 'wp_pending_next429', 'indexes' => ['wp_pending_next429_key'], 'commit' => false],
        ['op' => 'wal_commit', 'schema' => 'main', 'schema_cookie' => 431, 'table' => 'wp_pending_next429', 'indexes' => ['wp_pending_next429_key'], 'commit' => false],
    ]);
