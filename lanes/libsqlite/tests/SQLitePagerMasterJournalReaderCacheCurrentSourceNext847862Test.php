...
$base = [
    'source_id' => 'pager-reader-cache-current-source-next862',
    'epoch' => 862,
    'format_signature' => hash('sha256', implode('|', [512, 4, 2, 862, 0])),
    'publication_generation' => 862,
    'master_source_digest' => hash('sha256', 'master-next862'),
...
$plan = static fn (array $cacheExtra = [], array $readExtra = []): array => SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::currentSourceVdbeVirtualTableBranchHandoff(
    $database,
    $master,
    $masterBytes,
    implode('', $before),
...
$tests['pager master journal reader cache current source next862 publishes current VDBE literal and arithmetic opcode fences'] = static function (TestRunner $t) use ($plan): void {
    $result = $plan();
    $t->same('pager-master-journal-reader-cache-current-source-next862', $result['status']);
    $t->same([1], $result['invalidated_cache_page_numbers']);
    $t->same(['read-options' => false], $result['read_cache_hits']);
    $t->same('reader-cache-stmt-vdbe-null-branch-current-862', $result['current_reader_cache_stmt_vdbe_null_branch_token']);
    $t->same('reader-cache-stmt-vdbe-opcode-trace-branch-handoff-current-862', $result['current_reader_cache_stmt_vdbe_opcode_trace_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-shift-right-branch-handoff-current-862', $result['current_reader_cache_stmt_vdbe_shift_right_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-gt-branch-handoff-current-862', $result['current_reader_cache_stmt_vdbe_gt_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-incr-vacuum-branch-handoff-current-862', $result['current_reader_cache_stmt_vdbe_incr_vacuum_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-init-branch-handoff-current-862', $result['current_reader_cache_stmt_vdbe_init_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-goto-branch-handoff-current-862', $result['current_reader_cache_stmt_vdbe_goto_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-null-row-branch-handoff-current-862', $result['current_reader_cache_stmt_vdbe_null_row_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-row-value-branch-handoff-current-862', $result['current_reader_cache_stmt_vdbe_row_value_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-real-affinity-value-branch-handoff-current-862', $result['current_reader_cache_stmt_vdbe_real_affinity_value_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-cast-branch-handoff-current-862', $result['current_reader_cache_stmt_vdbe_cast_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-column-branch-handoff-current-862', $result['current_reader_cache_stmt_vdbe_column_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-affinity-branch-handoff-current-862', $result['current_reader_cache_stmt_vdbe_affinity_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-cast-affinity-branch-handoff-current-862', $result['current_reader_cache_stmt_vdbe_cast_affinity_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-compare-collseq-branch-handoff-current-862', $result['current_reader_cache_stmt_vdbe_compare_collseq_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-once-flag-branch-handoff-current-862', $result['current_reader_cache_stmt_vdbe_once_flag_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-ge-branch-condition-handoff-current-862', $result['current_reader_cache_stmt_vdbe_ge_branch_condition_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-else-eq-branch-condition-handoff-current-862', $result['current_reader_cache_stmt_vdbe_else_eq_branch_condition_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-if-open-branch-condition-handoff-current-862', $result['current_reader_cache_stmt_vdbe_if_open_branch_condition_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-transaction-branch-handoff-current-862', $result['current_reader_cache_stmt_vdbe_transaction_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-vbegin-branch-handoff-current-862', $result['current_reader_cache_stmt_vdbe_vbegin_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-vcreate-branch-handoff-current-862', $result['current_reader_cache_stmt_vdbe_vcreate_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-vdestroy-branch-handoff-current-862', $result['current_reader_cache_stmt_vdbe_vdestroy_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-vopen-branch-handoff-current-862', $result['current_reader_cache_stmt_vdbe_vopen_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-vfilter-branch-handoff-current-862', $result['current_reader_cache_stmt_vdbe_vfilter_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-vcolumn-branch-handoff-current-862', $result['current_reader_cache_stmt_vdbe_vcolumn_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-vnext-branch-handoff-current-862', $result['current_reader_cache_stmt_vdbe_vnext_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-vrename-branch-handoff-current-862', $result['current_reader_cache_stmt_vdbe_vrename_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-pagecount-branch-handoff-current-862', $result['current_reader_cache_stmt_vdbe_pagecount_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-maxpgcnt-branch-handoff-current-862', $result['current_reader_cache_stmt_vdbe_maxpgcnt_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-cursorhint-branch-handoff-current-862', $result['current_reader_cache_stmt_vdbe_cursorhint_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-noop-branch-handoff-current-862', $result['current_reader_cache_stmt_vdbe_noop_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-gosub-branch-handoff-current-862', $result['current_reader_cache_stmt_vdbe_gosub_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-return-branch-handoff-current-862', $result['current_reader_cache_stmt_vdbe_return_branch_handoff_token']);
    ...
    $t->same(true, in_array('sqlite-pager-master-journal-reader-cache-current-source-next862', $result['dependencies'], true));
};

$tests['pager master journal reader cache current source plan dispatches full width calls to next862'] = static function (TestRunner $t) use ($database, $master, $masterBytes, $before, $pageSize, $recovered, $cacheEntry, $read, $base, $tokens, $headers, $variantFields): void {
...
$tests['pager master journal reader cache current source next862 preserves next830 ge branch condition handoff continuity'] = static function (TestRunner $t) use ($plan, $opCount): void {
...
$tests['pager master journal reader cache current source next862 reopens stale return branch handoff read ticket'] = static function (TestRunner $t) use ($plan, $opCount): void {
    $result = $plan([], ['reader_cache_stmt_vdbe_return_branch_handoff_token' => 'stmt-vdbe-return-branch-handoff-old']);
    $t->same(['read-options'], $result['reopen_reader_ids']);
    $t->same(false, $result['next_reads'][0]['cache_hit']);
    $t->same('reader_ticket_reader_cache_stmt_vdbe_return_branch_handoff_predates_current_source', $result['next_reads'][0]['reader_cache_stmt_vdbe_return_branch_handoff_token_reason']);
    $t->same(1, $opCount($result, 'reopen_reader_for_reader_cache_stmt_vdbe_return_branch_handoff_current_source_next862'));
};

$tests['pager master journal reader cache current source next862 missing return branch handoff token rejects'] = static function (TestRunner $t) use ($plan): void {
    $t->throws(Throwable::class, static fn () => $plan(['reader_cache_stmt_vdbe_return_branch_handoff_token' => null]));
};
