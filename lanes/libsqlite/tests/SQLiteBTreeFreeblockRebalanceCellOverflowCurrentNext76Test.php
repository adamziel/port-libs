...
use PortLibs\LibSqlite\SQLiteBTreeFreeblockRebalanceCellOverflowCurrentNext76Plan;
...
$fixture = static function (string $replacementPayload = '') use ($makeFirstPage, $payload): array {
    $pageSize = 512;
    $largePayload = SQLiteRecord::encode([null, '_transient_rebalance_delete_10', str_repeat('delete-current-next76:', 62), 'no']);
    ...
    $replacementPayload = $replacementPayload !== '' ? $replacementPayload : str_repeat('replacement-current-next76:', 42);
    ...
    $plan = SQLiteBTreeFreeblockRebalanceCellOverflowCurrentNext76Plan::tableDeleteRebalanceThenReplaceOverflow(
        $database,
        3,
        4,
        5,
...
$expected = [
    'action label' => 'btree-freeblock-rebalance-cell-overflow-current-next76',
    'deleted rowid' => 10,
    ...
    'third chain terminal payload bytes' => 118,
...
foreach ($cases as $name => $case) {
    $tests['btree freeblock rebalance cell overflow current next76 ' . $name] = static function (TestRunner $t) use ($fixture, $case, $expected, $name): void {
        $t->same($expected[$name], $case($fixture()));
    };
}

$tests['btree freeblock rebalance cell overflow current next76 reads replacement payload back'] = static function (TestRunner $t) use ($fixture): void {
...
$tests['btree freeblock rebalance cell overflow current next76 rejects empty replacement payload'] = static function (TestRunner $t) use ($fixture): void {
    [$database] = $fixture();
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteBTreeFreeblockRebalanceCellOverflowCurrentNext76Plan::tableDeleteRebalanceThenReplaceOverflow(
        $database,
        3,
        4,
        5,
...
$tests['btree freeblock rebalance cell overflow current next76 rejects absent row'] = static function (TestRunner $t) use ($fixture): void {
    [$database] = $fixture();
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteBTreeFreeblockRebalanceCellOverflowCurrentNext76Plan::tableDeleteRebalanceThenReplaceOverflow(
        $database,
        3,
        ...
        999,
        static fn (): array => [],
        str_repeat('replacement-current-next76:', 42),
    ));
};

$tests['btree freeblock rebalance cell overflow current next76 rejects no append when replacement needs extra page'] = static function (TestRunner $t) use ($fixture): void {
    [$database] = $fixture();
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteBTreeFreeblockRebalanceCellOverflowCurrentNext76Plan::tableDeleteRebalanceThenReplaceOverflow(
        $database,
        3,
        ...
        10,
        static fn (): array => [7, 8],
        str_repeat('replacement-current-next76:', 90),
        true,
        false,
    ));
};
