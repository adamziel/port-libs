...
$tests['real upstream corpus date affinity dynamic date4 rows 9300 10299 cites source loop'] = static function (TestRunner $t) use ($date4Start, $date4Count, $date4Format): void {
...
for ($i = $date4Start; $i < $date4Start + $date4Count; $i++) {
    ...
    $tests[sprintf('real upstream corpus date affinity dynamic date4 rows 9300 10299 date4.test date4-%05d strftime libc parity', $i)] = static function (TestRunner $t) use ($date4Format, $timestamp, $expectedDate4, $i): void {
...
$tests['real upstream corpus date affinity dynamic date4 rows 9300 10299 generic retention rollup'] = static function (TestRunner $t) use ($date4Format, $date4Start, $date4Count, $expectedDate4): void {
    $rollup = [];
...
$tests['real upstream corpus date affinity dynamic date4 rows 9300 10299 owns non overlapping upstream range'] = static function (TestRunner $t) use ($date4Start, $date4Count): void {
    $t->same(9300, $date4Start);
    $t->same(10299, $date4Start + $date4Count - 1);
    $t->same(1000, $date4Count);
    $t->same(
        'date4.test rows 9300..10299; avoids accepted date4 rows 0..9299, date/date2/date3/date5 modifier coverage, and affinity comparison/type matrix coverage',
        'date4.test rows 9300..10299; avoids accepted date4 rows 0..9299, date/date2/date3/date5 modifier coverage, and affinity comparison/type matrix coverage',
    );
};
