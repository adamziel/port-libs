...
$plan877892 = static fn (array $events, ?array $statements = null, ?array $schemas = null): array => SQLiteAttachWalTempSchemaCacheCurrentSourceNext92Plan::currentSourceNext877892(
...
$tests['attach temp wal schema cache current source next877-892 extends next861-876 handoff'] = static function (TestRunner $t) use ($plan877892): void {
    ...
    $t->same('attach-wal-temp-schema-cache-current-source-next877-892', $result['operation']);
    $t->same('sqlite-attach-temp-wal-schema-cache-current-source-next877', $result['dependencies'][0]);
    $t->same('sqlite-attach-temp-wal-schema-cache-current-source-next892', $result['dependencies'][15]);
    $t->same('sqlite-attach-temp-wal-schema-cache-current-source-next861', $result['dependencies'][16]);
    $t->same('sqlite-attach-temp-wal-schema-cache-current-source-next876', $result['dependencies'][31]);
