...
$rows108 = static fn (SQLiteBTreeFreelistVacuumReuseCurrentSourceNextPlan $plan): array => $plan->incrementalVacuumReuseRows();

$cases108 = [
    'summary includes incremental vacuum reuse rows' => static fn (): mixed => array_column($fixture108()->toArray()['incremental_vacuum_reuse_rows'], 'page_number'),
    ...
    'incremental vacuum reuse row count covers every released overflow page' => static fn (): mixed => count($rows108($fixture108())),
    'incremental vacuum reuse row keys are stable' => static fn (): mixed => array_keys($rows108($fixture108())[0]),
];

$expected108 = [
    'summary includes incremental vacuum reuse rows' => [306, 307, 308, 309, 310],
    ...
    'summary action remains accepted reuse planner' => 'btree-freelist-pointermap-vacuum-reuse',
    'incremental vacuum reuse row count covers every released overflow page' => 5,
    'incremental vacuum reuse row keys are stable' => [
        'page_number',
        'source',
        'source_position',
...
foreach ($cases108 as $name => $callback) {
    $tests['btree incremental vacuum current source incremental vacuum reuse ' . $name] = static function (TestRunner $t) use ($callback, $expected108, $name): void {
        $t->same($expected108[$name], $callback());
    };
}

foreach (range(1, 30) as $index) {
    $tests['btree incremental vacuum current source incremental vacuum reuse invariant ' . $index] = static function (TestRunner $t) use ($fixture108, $rows108, $index): void {
