...
$cases246 = [
    'status' => ['status', 'encoding-collation-affinity-like-current-source-next246'],
    ...
    'current error malformed' => ['currentErrors.10', 'SQLite dynamic ESCAPE LIKE next246 option_value text is malformed UTF-8'],
    'next error malformed' => ['nextErrors.10', 'SQLite dynamic ESCAPE LIKE next246 option_value text is malformed UTF-8'],
    ...
    'dependency source' => ['dependencies.3', 'sqlite-current-source-next246'],
];

foreach ($cases246 as $name => [$path, $expected]) {
    $tests['encoding collation affinity like current source next246 ' . $name] = static function (TestRunner $t) use ($plan246, $valueAt246, $path, $expected): void {
        $t->same($expected, $valueAt246($plan246(), $path));
    };
}

$tests['encoding collation affinity like current source next246 stable cursor reusable'] = static function (TestRunner $t) use ($current246, $plan246): void {
...
$tests['encoding collation affinity like current source next246 numeric escape is text affinity'] = static function (TestRunner $t) use ($plan246): void {
    $rows = [
...
$tests['encoding collation affinity like current source next246 boolean escape can quote percent'] = static function (TestRunner $t) use ($plan246): void {
    $rows = [
...
$tests['encoding collation affinity like current source next246 null escape makes all rows unknown'] = static function (TestRunner $t) use ($plan246): void {
    $rows = [
...
$tests['encoding collation affinity like current source next246 blob escape reports malformed escape'] = static function (TestRunner $t) use ($plan246, $current246): void {
    ...
    $t->same('SQLite dynamic ESCAPE LIKE next246 current ESCAPE is BLOB, not text', $plan['currentEscape']['error']);
    $t->same(true, in_array('escape-malformed', $plan['invalidationReasons'], true));
};

$tests['encoding collation affinity like current source next246 multi character escape reports malformed escape'] = static function (TestRunner $t) use ($plan246, $current246): void {
    ...
    $t->same('SQLite dynamic ESCAPE LIKE next246 current ESCAPE must be one SQLite character after affinity', $plan['currentEscape']['error']);
    $t->same(true, in_array('escape-malformed', $plan['invalidationReasons'], true));
};

$tests['encoding collation affinity like current source next246 multibyte escape is one sqlite character'] = static function (TestRunner $t) use ($plan246): void {
    $rows = [
        ['option_id' => 1, 'option_value' => 'plugin_%enabled'],
...
$tests['encoding collation affinity like current source next246 case sensitive excludes uppercase'] = static function (TestRunner $t) use ($plan246): void {
...
$tests['encoding collation affinity like current source next246 binary collation sort key preserves case'] = static function (TestRunner $t) use ($plan246): void {
...
$tests['encoding collation affinity like current source next246 rejects invalid collation'] = static function (TestRunner $t) use ($plan246): void {
    $t->throws(InvalidArgumentException::class, static fn () => $plan246(collation: 'UNICODE'));
};

$tests['encoding collation affinity like current source next246 rejects missing option value'] = static function (TestRunner $t) use ($plan246): void {
    $t->throws(InvalidArgumentException::class, static fn () => $plan246(current: [['option_id' => 1]], next: []));
};

$tests['encoding collation affinity like current source next246 rejects nonscalar option value'] = static function (TestRunner $t) use ($plan246): void {
    $plan = $plan246(current: [['option_id' => 1, 'option_value' => ['bad']]], next: []);
    $t->same([1], $plan['currentMalformedRowids']);
    $t->same('SQLite dynamic ESCAPE LIKE next246 option_value must be scalar text-affinity input', $plan['currentErrors'][1]);
};
