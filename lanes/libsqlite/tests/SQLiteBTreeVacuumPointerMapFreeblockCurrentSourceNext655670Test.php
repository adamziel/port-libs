...
$makeFirstPage655670 = static function (int $pageCount): string {
...
$putPointerMapEntry655670 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database655670 = static function (int $sliceNumber) use ($makeFirstPage655670, $putPointerMapEntry655670): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage655670(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry655670($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan655670 = static function (int $sliceNumber, int $batchSize = 2) use ($database655670): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database655670($sliceNumber);
...
foreach ([655, 656, 657, 658, 659, 660, 661, 662, 663, 664, 665, 666, 667, 668, 669, 670] as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} publishes next655-670 follow-on handoff receipts"] = static function (TestRunner $t) use ($plan655670, $sliceNumber): void {
        $plan = $plan655670($sliceNumber);
        ...
        $t->same(6, $plan655670($sliceNumber, 3)->currentSourceSummary()['current_source_row_count']);
    };
}
