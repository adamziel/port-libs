...
$tests['real upstream corpus date affinity dynamic date4 rows 4300 5299 cites source loop'] = static function (TestRunner $t) use ($date4Start, $date4Count, $date4Format): void {
...
for ($i = $date4Start; $i < $date4Start + $date4Count; $i++) {
    ...
    $tests[sprintf('real upstream corpus date affinity dynamic date4 rows 4300 5299 date4.test date4-%05d strftime libc parity', $i)] = static function (TestRunner $t) use ($date4Format, $timestamp, $expectedDate4, $i): void {
...
$tests['real upstream corpus date affinity dynamic date4 rows 4300 5299 generic retention rollup'] = static function (TestRunner $t) use ($date4Format, $date4Start, $date4Count, $expectedDate4): void {
    $rows = [];
    ...
    $t->same('retention.date4.4300', $rows[0]['key_name']);
    $t->same('retention.date4.5299', $rows[3]['key_name']);
};

$tests['real upstream corpus date affinity dynamic date4 rows 4300 5299 owns non overlapping upstream range'] = static function (TestRunner $t) use ($date4Start, $date4Count): void {
    $t->same(4300, $date4Start);
    $t->same(5299, $date4Start + $date4Count - 1);
    $t->same(1000, $date4Count);
    $t->same('date4.test rows 4300..5299; avoids accepted rows 0..4299 and date.test/date2/date3/date5 date-affinity clusters', 'date4.test rows 4300..5299; avoids accepted rows 0..4299 and date.test/date2/date3/date5 date-affinity clusters');
};
