...
): array => SQLiteJsonTablePlan::currentSourceGeneratedPathRowidCostCurrentSourceNext190(
    'json_tree',
    $current ?? $current190,
    $next ?? $next190,
    'option_value',
...
foreach ($tests as $name => $case) {
    $tests['json table generated path rowid cost current source next190 ' . $name] = $case;
    unset($tests[$name]);
}
