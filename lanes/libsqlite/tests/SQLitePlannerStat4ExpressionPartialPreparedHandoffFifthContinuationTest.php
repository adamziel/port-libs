...
$plan814829 = static fn (?array $rows = null, ?array $samples = null): array => SQLitePlannerStat4ExpressionPartialCurrentSourceNextPlan::materializePreparedHandoffWindowContinuation(
    $prepared814829(),
...
$tests = [
    ...
    'planner stat4 expression partial current source next814829 cursor appended' => static fn (TestRunner $t) => $t->same('PrepareStat4ExpressionPartialPreparedHandoffRange', $plan814829()['cursorProgram'][array_key_last($plan814829()['cursorProgram'])]['opcode']),
    'planner stat4 expression partial current source next814829 cursor mode' => static fn (TestRunner $t) => $t->same('prepared-handoff-range-current-source-stat4-expression-partial-prep', $plan814829()['cursorProgram'][array_key_last($plan814829()['cursorProgram'])]['mode']),
    ...
    'planner stat4 expression partial current source next814829 malformed needed column' => static function (TestRunner $t) use ($prepared814829, $current814829, $terms814829): void {
        $t->throws(InvalidArgumentException::class, static fn () => SQLitePlannerStat4ExpressionPartialCurrentSourceNextPlan::materializePreparedHandoffWindowContinuation($prepared814829(), $current814829(), $terms814829(), ['option_name', ''], 6));
    },
];
