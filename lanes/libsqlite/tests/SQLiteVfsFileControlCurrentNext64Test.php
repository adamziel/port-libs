...
$tests['vfs filecontrol current next64 exposes initial 64 bit controls'] = static function (TestRunner $t) use ($makeState): void {
    ...
    $t->same(true, in_array('vfs-file-control-current-next64', $sequence['dependencies'], true));
};
...
foreach ($operations as $name => [$op, $value, $initial, $expected, $changed]) {
    $tests['vfs filecontrol current next64 ' . $name] = static function (TestRunner $t) use ($makeState, $op, $value, $initial, $expected, $changed): void {
        $initialControls = [];
        if ($initial !== null) {
            $initialControls[$op] = $initial;
        }
        $state = $makeState($initialControls);
        $sequence = $state->currentNext64([['op' => $op, 'value' => $value]]);
...
$tests['vfs filecontrol current next64 preserves previous size limit in pair'] = static function (TestRunner $t) use ($makeState): void {
    $state = $makeState(['size_limit' => 4096]);
    $pair = $state->currentNext64(['size_limit' => 8192])['pairs'][0];
...
$tests['vfs filecontrol current next64 preserves previous reserve bytes in pair'] = static function (TestRunner $t) use ($makeState): void {
    $state = $makeState(['reserve_bytes' => 4]);
    $pair = $state->currentNext64(['reserve_bytes' => 16])['pairs'][0];
...
$tests['vfs filecontrol current next64 preserves previous lock timeout in pair'] = static function (TestRunner $t) use ($makeState): void {
    $state = $makeState(['lock_timeout' => 75]);
    $pair = $state->currentNext64(['lock_timeout' => 150])['pairs'][0];
...
$tests['vfs filecontrol current next64 leaves data version read only'] = static function (TestRunner $t) use ($makeState): void {
    $state = $makeState(['data_version' => 44]);
    $pair = $state->currentNext64(['data_version' => null])['pairs'][0];
...
$tests['vfs filecontrol current next64 reports same path has not moved'] = static function (TestRunner $t) use ($makeState): void {
    $state = $makeState();
    $pair = $state->currentNext64(['has_moved' => '/srv/www/wp-content/database/.ht.sqlite'])['pairs'][0];
...
$tests['vfs filecontrol current next64 reports renamed path moved'] = static function (TestRunner $t) use ($makeState): void {
    $state = $makeState();
    $pair = $state->currentNext64(['has_moved' => '/srv/www/wp-content/database/.ht-renamed.sqlite'])['pairs'][0];
...
$tests['vfs filecontrol current next64 has moved can return current flag'] = static function (TestRunner $t) use ($makeState): void {
    $state = $makeState();
    $sequence = $state->currentNext64([
...
$tests['vfs filecontrol current next64 temp filename is deterministic'] = static function (TestRunner $t) use ($makeState): void {
    $state = $makeState();
    $sequence = $state->currentNext64([
...
$tests['vfs filecontrol current next64 temp filename defaults to sqlite suffix'] = static function (TestRunner $t) use ($makeState): void {
    $state = $makeState();
    $value = $state->currentNext64(['tempfilename' => null])['pairs'][0]['result']['value'];

    $t->same(true, str_starts_with($value, '/srv/www/wp-content/database/etilqs_'));
    $t->same('.sqlite', substr($value, -7));
};

$tests['vfs filecontrol current next64 sequence reports ordinals'] = static function (TestRunner $t) use ($makeState): void {
    $state = $makeState();
    $sequence = $state->currentNext64([
        'size_limit' => 1024,
        'reserve_bytes' => 2,
...
$tests['vfs filecontrol current next64 sequence threads size limit into next current'] = static function (TestRunner $t) use ($makeState): void {
...
$tests['vfs filecontrol current next64 sequence threads reserve into next current'] = static function (TestRunner $t) use ($makeState): void {
...
$tests['vfs filecontrol current next64 sequence threads lock timeout into next current'] = static function (TestRunner $t) use ($makeState): void {
...
$tests['vfs filecontrol current next64 applies mixed integer batch controls'] = static function (TestRunner $t) use ($makeState): void {
    $state = $makeState();
    $sequence = $state->currentNext64([
...
$tests['vfs filecontrol current next64 counts notfound without applying'] = static function (TestRunner $t) use ($makeState): void {
    $state = $makeState(['size_limit' => 7000]);
    $sequence = $state->currentNext64(['not_a_control' => 1]);
...
$tests['vfs filecontrol current next64 apply many counts new controls'] = static function (TestRunner $t) use ($makeState): void {
...
$tests['vfs filecontrol current next64 readonly reserve is ignored'] = static function (TestRunner $t): void {
    ...
    $pair = $state->currentNext64(['reserve_bytes' => 8])['pairs'][0];
...
$tests['vfs filecontrol current next64 memory reserve is ignored'] = static function (TestRunner $t): void {
...
$tests['vfs filecontrol current next64 nolock lock timeout is ignored'] = static function (TestRunner $t): void {
    ...
    $pair = $state->currentNext64(['lock_timeout' => 500])['pairs'][0];
...
$tests['vfs filecontrol current next64 memory lock timeout is ignored'] = static function (TestRunner $t): void {
    $state = SQLiteVfsFileControlState::fromCapabilityPlan(SQLiteVfsCapabilityPlan::forFilename('file::memory:?mode=memory', false, true));
    $pair = $state->currentNext64(['lock_timeout' => 500])['pairs'][0];
...
$tests['vfs filecontrol current next64 keeps mmap and chunk in pair snapshots'] = static function (TestRunner $t) use ($makeState): void {
    $state = $makeState(['chunk_size' => 16384, 'mmap_size' => 32768]);
    $pair = $state->currentNext64(['size_limit' => 123456])['pairs'][0];
...
$tests['vfs filecontrol current next64 keeps boolean controls in pair snapshots'] = static function (TestRunner $t) use ($makeState): void {
    $state = $makeState(['persist_wal' => true, 'powersafe_overwrite' => true]);
    $pair = $state->currentNext64(['reserve_bytes' => 4])['pairs'][0];
...
$tests['vfs filecontrol current next64 supports dash normalized size limit'] = static function (TestRunner $t) use ($makeState): void {
    $state = $makeState();
    $pair = $state->currentNext64(['size-limit' => 512])['pairs'][0];

    $t->same('size_limit', $pair['op']);
    $t->same(512, $pair['next']['size_limit']);
};

$tests['vfs filecontrol current next64 supports dash normalized reserve bytes'] = static function (TestRunner $t) use ($makeState): void {
    $state = $makeState();
    $pair = $state->currentNext64(['reserve-bytes' => 5])['pairs'][0];

    $t->same('reserve_bytes', $pair['op']);
    $t->same(5, $pair['next']['reserve_bytes']);
};

$tests['vfs filecontrol current next64 supports dash normalized lock timeout'] = static function (TestRunner $t) use ($makeState): void {
    $state = $makeState();
    $pair = $state->currentNext64(['lock-timeout' => 15])['pairs'][0];

    $t->same('lock_timeout', $pair['op']);
    $t->same(15, $pair['next']['lock_timeout']);
};

$tests['vfs filecontrol current next64 rejects negative size limit'] = static function (TestRunner $t) use ($makeState): void {
    $t->throws(InvalidArgumentException::class, static fn () => $makeState()->currentNext64(['size_limit' => -2]));
};

$tests['vfs filecontrol current next64 rejects negative reserve bytes'] = static function (TestRunner $t) use ($makeState): void {
    $t->throws(InvalidArgumentException::class, static fn () => $makeState()->currentNext64(['reserve_bytes' => -2]));
};

$tests['vfs filecontrol current next64 rejects oversized reserve bytes'] = static function (TestRunner $t) use ($makeState): void {
    $t->throws(InvalidArgumentException::class, static fn () => $makeState()->currentNext64(['reserve_bytes' => 256]));
};

$tests['vfs filecontrol current next64 rejects negative lock timeout'] = static function (TestRunner $t) use ($makeState): void {
    $t->throws(InvalidArgumentException::class, static fn () => $makeState()->currentNext64(['lock_timeout' => -1]));
};

$tests['vfs filecontrol current next64 rejects non integer lock timeout'] = static function (TestRunner $t) use ($makeState): void {
    $t->throws(InvalidArgumentException::class, static fn () => $makeState()->currentNext64(['lock_timeout' => '100']));
};

$tests['vfs filecontrol current next64 rejects malformed sequence item'] = static function (TestRunner $t) use ($makeState): void {
    $t->throws(InvalidArgumentException::class, static fn () => $makeState()->currentNext64([['value' => 1]]));
};

$tests['vfs filecontrol current next64 rejects empty has moved comparison'] = static function (TestRunner $t) use ($makeState): void {
    $t->throws(InvalidArgumentException::class, static fn () => $makeState()->currentNext64(['has_moved' => '']));
};

$tests['vfs filecontrol current next64 rejects temp filename slash'] = static function (TestRunner $t) use ($makeState): void {
    $t->throws(InvalidArgumentException::class, static fn () => $makeState()->currentNext64(['tempfilename' => '../bad']));
};

$tests['vfs filecontrol current next64 rejects temp filename nul'] = static function (TestRunner $t) use ($makeState): void {
    $t->throws(InvalidArgumentException::class, static fn () => $makeState()->currentNext64(['tempfilename' => "bad\0name"]));
};

$tests['vfs filecontrol current next64 constructor rejects invalid reserve bytes'] = static function (TestRunner $t) use ($makeState): void {
    $t->throws(InvalidArgumentException::class, static fn () => $makeState(['reserve_bytes' => 300]));
};

$tests['vfs filecontrol current next64 constructor rejects invalid data version'] = static function (TestRunner $t) use ($makeState): void {
    $t->throws(InvalidArgumentException::class, static fn () => $makeState(['data_version' => 0]));
};

$tests['vfs filecontrol current next64 constructor rejects invalid lock timeout'] = static function (TestRunner $t) use ($makeState): void {
    $t->throws(InvalidArgumentException::class, static fn () => $makeState(['lock_timeout' => -1]));
};

$tests['vfs filecontrol current next64 constructor accepts zero size limit'] = static function (TestRunner $t) use ($makeState): void {
    $state = $makeState(['size_limit' => 0]);

    $t->same(0, $state->snapshot()['controls']['size_limit']);
};

$tests['vfs filecontrol current next64 snapshot includes new controls'] = static function (TestRunner $t) use ($makeState): void {
...
$tests['vfs filecontrol current next64 wordpress import sequence summary'] = static function (TestRunner $t) use ($makeState): void {
...
$tests['vfs filecontrol current next64 wordpress readonly archive sequence'] = static function (TestRunner $t): void {
    $state = SQLiteVfsFileControlState::fromCapabilityPlan(SQLiteVfsCapabilityPlan::forFilename('file:/srv/www/wp-content/database/archive.sqlite?mode=ro&immutable=1', true, false));
    $sequence = $state->currentNext64([
...
$tests['vfs filecontrol current next64 dependencies stay lane local'] = static function (TestRunner $t) use ($makeState): void {
    $dependencies = $makeState()->currentNext64(['size_limit' => 1])['dependencies'];

    $t->same(true, in_array('vfs-file-control-state', $dependencies, true));
    $t->same(true, in_array('vfs-xfilecontrol', $dependencies, true));
    $t->same(true, in_array('vfs-file-control-current-next64', $dependencies, true));
};
