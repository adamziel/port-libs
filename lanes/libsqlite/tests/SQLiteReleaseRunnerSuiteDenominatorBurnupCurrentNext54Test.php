...
function libsqlite_release_denominator54_evidence(): SQLiteUpstreamSuiteEvidence
{
    return SQLiteUpstreamSuiteEvidence::fromManifestPath(__DIR__ . '/../UPSTREAM_TEST_MANIFEST.json');
}

function libsqlite_release_denominator54_output(int $assertions = 70, int $failures = 0): string
{
    return "Focused test run: 1 selected test files (root lock skipped)\n"
        . "1 test files, {$assertions} assertions, {$failures} failures\n";
}

function libsqlite_release_denominator54_rows(int $case): array
{
    $rows = [
        [
...
$focusedPath54 = 'lanes/libsqlite/tests/SQLiteReleaseRunnerSuiteDenominatorBurnupCurrentNext54Test.php';
...
for ($i = 1; $i <= 54; $i++) {
    $tests['current next54 suite denominator burnup maps case ' . $i] = static function (TestRunner $t) use ($i, $currentHead54, $nextHead54, $focusedPath54, $nonOverlap54): void {
        $record = libsqlite_release_denominator54_evidence()->releaseRunnerSuiteDenominatorBurnupCurrentNext54(
            libsqlite_release_denominator54_rows($i),
            $currentHead54,
            $nextHead54,
            19277,
            $focusedPath54,
            libsqlite_release_denominator54_output(70),
            $nonOverlap54
        );
...
$tests['current next54 reports open next denominator gaps'] = static function (TestRunner $t) use ($currentHead54, $nextHead54, $focusedPath54, $nonOverlap54): void {
    $record = libsqlite_release_denominator54_evidence()->releaseRunnerSuiteDenominatorBurnupCurrentNext54(
        libsqlite_release_denominator54_rows(1),
        $currentHead54,
        $nextHead54,
        19277,
        $focusedPath54,
...
$tests['current next54 blocks regressed denominator units'] = static function (TestRunner $t) use ($currentHead54, $nextHead54, $focusedPath54, $nonOverlap54): void {
    $record = libsqlite_release_denominator54_evidence()->releaseRunnerSuiteDenominatorBurnupCurrentNext54(
        libsqlite_release_denominator54_rows(11),
        $currentHead54,
        $nextHead54,
        19277,
        $focusedPath54,
...
$tests['current next54 rejects missing heads'] = static function (TestRunner $t) use ($focusedPath54, $nonOverlap54): void {
    $t->throws(
        ...
        static fn () => libsqlite_release_denominator54_evidence()->releaseRunnerSuiteDenominatorBurnupCurrentNext54(
