...
$plan = static fn (array $options = []): array => SQLiteTriggerRecursiveViewUpsertCurrentSourceNextPlan::executeCurrentTargetReasonReceipt(
    ...
    $options + [
        ...
        'reset_generation' => 'app-current-reset-245',
        'post_reset_current_source_token' => 'app.current.source.postreset.245',
        'post_reset_cursor' => 'app.returning.postreset.cursor.245',
        ...
        'following_generation' => 'app-following-current-245',
...
$targetHeld = static fn (): array => $plan(['auto_ack_current_source_upsert_targets_next245' => true, 'expected_current_source_upsert_conflict_target_next245' => ['setting_id']]);
$excludedHeld = static fn (): array => $plan(['auto_ack_current_source_upsert_targets_next245' => true, 'expected_current_source_upsert_excluded_columns_next245' => ['key_value']]);
$tokenHeld = static fn (): array => $plan(['auto_ack_current_source_upsert_targets_next245' => true, 'expected_current_source_upsert_target_token_next245' => 'app.current.source.upsert.target.stale.245']);
...
$cases = [
    ...
    'savepoint retained' => [static fn (): mixed => $released()['savepoint'], 'app_recursive_view_245'],
    ...
    'target token retained' => [static fn (): mixed => $released()['current_source_upsert_target_token_next245'], 'app.current.source.upsert.target.245'],
    ...
    'conflict target retained' => [static fn (): mixed => $released()['current_source_upsert_conflict_target_next245'], ['key_name']],
    ...
    'excluded columns retained' => [static fn (): mixed => $released()['current_source_upsert_excluded_columns_next245'], ['key_value', 'load_policy']],
    ...
    'target stamped on current' => [static fn (): mixed => array_values(array_unique(array_map(static fn (array $row): string => implode(',', $row['current_source_upsert_conflict_target_next245']), $released()['current_source_rows_next245']))), ['key_name']],
    'excluded stamped on current' => [static fn (): mixed => array_values(array_unique(array_map(static fn (array $row): string => implode(',', $row['current_source_upsert_excluded_columns_next245']), $released()['current_source_rows_next245']))), ['key_value,load_policy']],
    'visible payload names released' => [static fn (): mixed => array_column($released()['visible_returning_payloads_next245'], 'name'), ['app_summary_child', 'template_child', 'landing_url', 'next_module']],
    'held payload names missing' => [static fn (): mixed => array_column($missing()['held_next_returning_payloads_next245'], 'name'), ['landing_url', 'next_module']],
