...
return [
    'vfs current source extended published reuse snapshot fence dependency marker' => static fn (TestRunner $t) => $t->same(true, in_array('vfs-current-source-extended-published-reuse-snapshot-fence', $plan()['dependencies'], true)),
    'vfs current source extended published reuse snapshot fence records next610-625 prerequisite' => static fn (TestRunner $t) => $t->same(true, in_array('vfs-current-source-snapshot-reuse-publish-next610-625', $plan()['dependencies'], true)),
    'vfs current source extended published reuse snapshot fence records next562-577 prerequisite' => static fn (TestRunner $t) => $t->same(true, in_array('vfs-current-source-snapshot-reuse-publish-next562-577', $plan()['dependencies'], true)),
    'vfs current source extended published reuse snapshot fence records next530-545 prerequisite' => static fn (TestRunner $t) => $t->same(true, in_array('vfs-current-source-snapshot-reuse-publish-next530-545', $plan()['dependencies'], true)),
    'vfs current source extended published reuse snapshot fence records next514-529 prerequisite' => static fn (TestRunner $t) => $t->same(true, in_array('vfs-current-source-snapshot-reuse-publish-next514-529', $plan()['dependencies'], true)),
    'vfs current source extended published reuse snapshot fence records next498-513 prerequisite' => static fn (TestRunner $t) => $t->same(true, in_array('vfs-current-source-snapshot-reuse-publish-next498-513', $plan()['dependencies'], true)),
    'vfs current source extended published reuse snapshot fence records next482-497 prerequisite' => static fn (TestRunner $t) => $t->same(true, in_array('vfs-current-source-snapshot-reuse-publish-next482-497', $plan()['dependencies'], true)),
    'vfs current source extended published reuse snapshot fence records next450-465 prerequisite' => static fn (TestRunner $t) => $t->same(true, in_array('vfs-current-source-snapshot-reuse-publish-next450-465', $plan()['dependencies'], true)),
    'vfs current source extended published reuse snapshot fence records next434-449 prerequisite' => static fn (TestRunner $t) => $t->same(true, in_array('vfs-current-source-snapshot-reuse-publish-next434-449', $plan()['dependencies'], true)),
    'vfs current source extended published reuse snapshot fence snapshots fresh current source' => static fn (TestRunner $t) => $t->same('snapshotted-current-source', $plan()['events'][0]['status']),
    'vfs current source extended published reuse snapshot fence snapshot records next625 ack' => static fn (TestRunner $t) => $t->same('shared-cache-next625', $plan()['events'][0]['ack']),
    'vfs current source extended published reuse snapshot fence claims reusable snapshot' => static fn (TestRunner $t) => $t->same('claimed-reusable-current-source', $plan()['events'][1]['status']),
    'vfs current source extended published reuse snapshot fence records claim token' => static fn (TestRunner $t) => $t->same('reader-reuse-next641', $plan()['events'][1]['claim']),
    'vfs current source extended published reuse snapshot fence claim has no blockers' => static fn (TestRunner $t) => $t->same([], $plan()['events'][1]['blocked_reasons']),
    'vfs current source extended published reuse snapshot fence publishes after claim' => static fn (TestRunner $t) => $t->same('published-reused-current-source', $plan()['events'][2]['status']),
    'vfs current source extended published reuse snapshot fence publish uses claim' => static fn (TestRunner $t) => $t->same('reader-reuse-next641', $plan()['events'][2]['claim']),
    'vfs current source extended published reuse snapshot fence publish preserves ack' => static fn (TestRunner $t) => $t->same('shared-cache-next625', $plan()['events'][2]['reuse_ack']),
    'vfs current source extended published reuse snapshot fence publish count advances' => static fn (TestRunner $t) => $t->same(31, $plan()['events'][2]['published_count']),
    'vfs current source extended published reuse snapshot fence blocks dirty snapshot' => static fn (TestRunner $t) => $t->same(true, in_array('dirty-pages-present', SQLiteVfsCurrentSourceNextPlan::run(['snapshot(reader-ready-next641,shared-cache-next625)'], ['current' => $dirtyCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source extended published reuse snapshot fence blocks old snapshot ack' => static fn (TestRunner $t) => $t->same(true, in_array('ack-not-latest-publish', SQLiteVfsCurrentSourceNextPlan::run(['snapshot(reader-ready-next641,shared-cache-next625)'], ['current' => $oldAckCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source extended published reuse snapshot fence blocks publish without claim' => static fn (TestRunner $t) => $t->same(true, in_array('missing-reuse-claim', SQLiteVfsCurrentSourceNextPlan::run(['snapshot(reader-ready-next641,shared-cache-next625)', 'publish(reader-ready-next641,reader-reuse-next641,shared-cache-next641)'], ['current' => $readyCurrent])['events'][1]['blocked_reasons'], true)),
    'vfs current source extended published reuse snapshot fence blocks stale claim' => static fn (TestRunner $t) => $t->same(true, in_array('stale-reuse-claim', SQLiteVfsCurrentSourceNextPlan::run(['publish(reader-ready-next641,reader-reuse-next641,shared-cache-next641)'], ['current' => $staleClaimCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source extended published reuse snapshot fence accepts preclaimed publish' => static fn (TestRunner $t) => $t->same('published-reused-current-source', SQLiteVfsCurrentSourceNextPlan::run(['publish(reader-ready-next641,reader-reuse-next641,shared-cache-next641)'], ['current' => $claimedCurrent])['events'][0]['status']),
    'vfs current source extended published reuse snapshot fence rejects empty operations' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteVfsCurrentSourceNextPlan::run([])),
    'vfs current source extended published reuse snapshot fence rejects bad claim token' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteVfsCurrentSourceNextPlan::run([['op' => 'claim', 'snapshot' => 'reader-ready-next641', 'ack' => 'shared-cache-next625', 'claim' => 'bad claim']], ['current' => $readyCurrent])),
    'vfs current source extended published reuse snapshot fence rejects unsupported operation' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteVfsCurrentSourceNextPlan::run(['republish(reader-ready-next641,shared-cache-next641)'], ['current' => $readyCurrent])),
    'vfs current source extended published reuse snapshot fence notes prior window non-overlap' => static fn (TestRunner $t) => $t->same(true, str_contains($plan()['non_overlap'], 'merged next610-625')),
    ...
    'vfs current source next642-657 records canonical prerequisite' => static fn (TestRunner $t) => $t->same(true, in_array('vfs-current-source-extended-published-reuse-snapshot-fence', $next642657Plan()['dependencies'], true)),
    ...
    'vfs current source next642-657 notes non-overlap handoff' => static fn (TestRunner $t) => $t->same(true, str_contains($next642657Plan()['non_overlap'], 'next642-657 follows the integrated extended published-reuse snapshot fence handoff')),
