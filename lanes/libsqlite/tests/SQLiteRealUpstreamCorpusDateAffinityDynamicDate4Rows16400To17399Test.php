...
$expectedDate4 = static function (int $timestamp): string {
    ...
    return implode(',', [
        ...
        $instant->format('N'),
        $instant->format('w'),
        sqliteRealUpstreamDate4Rows16400To17399WeekNumber($instant, 1),
        $instant->format('Y'),
        '%',
        strtolower($instant->format('A')),
        $instant->format('A'),
        sqliteRealUpstreamDate4Rows16400To17399WeekNumber($instant, 0),
        $instant->format('W'),
        $instant->format('o'),
...
$tests['real upstream corpus date affinity dynamic date4 rows 16400 17399 cites source loop'] = static function (TestRunner $t) use ($date4Start, $date4Count, $date4Format): void {
...
for ($i = $date4Start; $i < $date4Start + $date4Count; $i++) {
    ...
    $tests[sprintf('real upstream corpus date affinity dynamic date4 rows 16400 17399 date4.test date4-%05d strftime libc parity', $i)] = static function (TestRunner $t) use ($date4Format, $timestamp, $expected, $i): void {
...
$tests['real upstream corpus date affinity dynamic date4 rows 16400 17399 generic retention rollup'] = static function (TestRunner $t) use ($date4Format, $date4Start, $date4Count, $expectedDate4): void {
    $rows = [];
    ...
    $t->same('retention.date4.16400', $rows[0]['key_name']);
    $t->same('retention.date4.17399', $rows[3]['key_name']);
};

$tests['real upstream corpus date affinity dynamic date4 rows 16400 17399 owns non overlapping upstream range'] = static function (TestRunner $t) use ($date4Start, $date4Count): void {
    $t->same(16400, $date4Start);
    $t->same(17399, $date4Start + $date4Count - 1);
    $t->same(1000, $date4Count);
    $t->same(
        'date4.test rows 16400..17399; avoids accepted date4 rows 0..16399, date/date2/date3/date5 modifier coverage, and affinity comparison/type matrix coverage',
        'date4.test rows 16400..17399; avoids accepted date4 rows 0..16399, date/date2/date3/date5 modifier coverage, and affinity comparison/type matrix coverage'
    );
};

$tests['real upstream corpus date affinity dynamic date4 rows 16400 17399 dependency closure'] = static function (TestRunner $t): void {
...
function sqliteRealUpstreamDate4Rows16400To17399WeekNumber(DateTimeImmutable $instant, int $firstWeekday): string
{
