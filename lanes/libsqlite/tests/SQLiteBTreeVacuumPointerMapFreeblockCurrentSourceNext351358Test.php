...
$makeFirstPage351358 = static function (int $pageCount): string {
...
$putPointerMapEntry351358 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database351358 = static function (int $sliceNumber) use ($makeFirstPage351358, $putPointerMapEntry351358): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage351358(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry351358($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan351358 = static function (int $sliceNumber, int $batchSize = 2) use ($database351358): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database351358($sliceNumber);
...
foreach ([351, 352, 353, 354, 355, 356, 357, 358] as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} keeps next351-358 freelist splice ready"] = static function (TestRunner $t) use ($plan351358, $sliceNumber): void {
        $plan = $plan351358($sliceNumber);
        ...
        $t->same(6, $plan351358($sliceNumber, 3)->freelistSummary()['freelist_row_count']);
    };
}
