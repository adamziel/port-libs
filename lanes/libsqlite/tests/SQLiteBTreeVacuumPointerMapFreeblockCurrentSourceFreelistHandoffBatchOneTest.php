...
$makeFirstPage10071022 = static function (int $pageCount): string {
...
$putPointerMapEntry10071022 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database10071022 = static function (int $sliceNumber) use ($makeFirstPage10071022, $putPointerMapEntry10071022): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage10071022(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry10071022($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan10071022 = static function (int $sliceNumber, int $batchSize = 2) use ($database10071022): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database10071022($sliceNumber);
...
foreach (range(1007, 1022) as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source freelist handoff case {$sliceNumber} receipts"] = static function (TestRunner $t) use ($plan10071022, $sliceNumber): void {
        $plan = $plan10071022($sliceNumber);
        ...
        $t->same(6, $plan10071022($sliceNumber, 3)->currentSourceSummary()['current_source_row_count']);
    };
}
