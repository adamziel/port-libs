...
$plan542557 = static fn (?array $rows = null, ?array $samples = null): array => SQLitePlannerStat4ExpressionPartialCurrentSourceNextPlan::materializeStat4ExpressionPartialPreparedBridgeHandoff(
    $prepared542557(),
...
$tests = [
    ...
    'planner stat4 expression partial current source next542557 cursor appended' => static fn (TestRunner $t) => $t->same('PrepareStat4ExpressionPartialPreparedHandoffRange', $plan542557()['cursorProgram'][array_key_last($plan542557()['cursorProgram'])]['opcode']),
    'planner stat4 expression partial current source next542557 cursor mode' => static fn (TestRunner $t) => $t->same('prepared-handoff-range-current-source-stat4-expression-partial-prep', $plan542557()['cursorProgram'][array_key_last($plan542557()['cursorProgram'])]['mode']),
    ...
    'planner stat4 expression partial current source next542557 malformed needed column' => static function (TestRunner $t) use ($prepared542557, $current542557, $terms542557): void {
        $t->throws(InvalidArgumentException::class, static fn () => SQLitePlannerStat4ExpressionPartialCurrentSourceNextPlan::materializeStat4ExpressionPartialPreparedBridgeHandoff($prepared542557(), $current542557(), $terms542557(), ['option_name', ''], 6));
    },
];
