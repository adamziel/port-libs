...
$pageSize = 512;
$database = '/srv/wp-content/database/wp-next306.sqlite';
$usersDatabase = '/srv/wp-content/database/wp-next306-users.sqlite';
...
$before = [1 => $formatPage('next306 stale schema'), 2 => $page('next306 stale options'), 3 => $page('next306 stale plugins'), 4 => $page('next306 stale users')];
$recovered = [1 => $formatPage('next306 current schema'), 2 => $page('next306 current options'), 3 => $page('next306 current plugins'), 4 => $page('next306 current users')];
...
$base = [
    'source_id' => 'pager-reader-cache-current-source-next306',
    'epoch' => 306,
    'format_signature' => hash('sha256', implode('|', [512, 4, 2, 306, 0])),
    'publication_generation' => 306,
    'master_source_digest' => hash('sha256', 'master-next306'),
...
$plan = static function (?array $cache = null, ?array $reads = null) use ($database, $master, $masterBytes, $before, $pageSize, $recovered, $base, $tokens, $headers, $cacheEntry, $read): array {
    return SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::variantNext306($database, $master, $masterBytes, implode('', $before), $pageSize, $recovered, $cache ?? [
...
$cases = [
    'status' => [static fn (): mixed => $plan()['status'], 'pager-master-journal-reader-cache-current-source-next306'],
    ...
    'dependencies include next306' => [static fn (): mixed => in_array('sqlite-pager-master-journal-reader-cache-current-source-next306', $plan()['dependencies'], true), true],
    ...
    'default plan points at next306' => [static fn (): mixed => $plan()['status'], 'pager-master-journal-reader-cache-current-source-next306'],
];

foreach ($cases as $name => [$callback, $expected]) {
    $tests['pager master journal reader cache current source next306 ' . $name] = static function (TestRunner $t) use ($callback, $expected): void {
        $t->same($expected, $callback());
    };
}

$tests['pager master journal reader cache current source next306 missing token rejects'] = static function (TestRunner $t) use ($plan, $cacheEntry, $recovered): void {
