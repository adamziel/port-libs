...
$sourceId = 'next181-before-current-rollback-source';
...
$before = [
    1 => $page('next181 stale schema before current rollback journal'),
    2 => $page('next181 stale wp_options root before current rollback journal'),
    3 => $page('next181 stale active_plugins before current rollback journal'),
    4 => $page('next181 stale plugin settings before current rollback journal'),
    5 => $page('next181 unchanged comments before current rollback journal'),
    6 => $page('next181 stale transients before current rollback journal'),
    7 => $page('next181 stale cron before current rollback journal'),
    8 => $page('next181 stale optionmeta before current rollback journal'),
];
$recovered = [
    1 => $page('next181 recovered schema from current rollback journal'),
    2 => $page('next181 recovered wp_options root from current rollback journal'),
    3 => $page('next181 recovered active_plugins from current rollback journal'),
    4 => $page('next181 recovered plugin settings from current rollback journal'),
    6 => $page('next181 recovered transients from current rollback journal'),
    7 => $page('next181 recovered cron from current rollback journal'),
];
...
$nextWrites = [
    3 => $page('next181 rewritten active_plugins after rollback reader cache'),
    4 => $page('next181 rewritten plugin settings after rollback reader cache'),
];
...
$plan = static fn (
    ...
    ?string $source = null,
    int $epoch = 12,
): array => SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::planPendingMasterJournalMembershipFence(
    $path ?? $databasePath,
    $masterJournalPath ?? $masterPath,
    $master ?? $masterBytes,
    $pendingMaster ?? $pendingMasterBytes,
...
$cases = [
    'status' => [static fn (): mixed => $plan()['status'], 'pager-master-journal-reader-cache-current-source-next181'],
    'reason' => [static fn (): mixed => $plan()['reason'], 'current_master_journal_membership_rejects_pending_source_reader_cache'],
    ...
    'members' => [static fn (): mixed => $plan()['current_members'], [$databasePath . '-journal', '/srv/wp-content/database/wp-next181-meta.sqlite-journal']],
    'pending members' => [static fn (): mixed => $plan()['pending_members'], [$databasePath . '-journal', '/srv/wp-content/database/wp-next181-meta.sqlite-journal', '/srv/wp-content/database/wp-next181-pending.sqlite-journal']],
    'pending member added' => [static fn (): mixed => $plan()['pending_members_added'], ['/srv/wp-content/database/wp-next181-pending.sqlite-journal']],
    'pending member removed' => [static fn (): mixed => $plan()['pending_members_removed'], []],
    'master digest' => [static fn (): mixed => $plan()['current_master_journal_digest'], $masterDigest],
    'pending master digest' => [static fn (): mixed => $plan()['pending_master_journal_digest'], $pendingMasterDigest],
    ...
    'read unchanged cache miss prefix' => [static fn (): mixed => $plan()['next_reads'][4]['prefix'], 'next181 unchanged comments before current rollback journal'],
    'read recovered prefix' => [static fn (): mixed => $plan()['next_reads'][6]['prefix'], 'next181 recovered cron from current rollback journal'],
    ...
    'write active before' => [static fn (): mixed => $plan()['next_writes'][0]['before_prefix'], 'next181 recovered active_plugins from current rollback journal'],
    'write active after' => [static fn (): mixed => $plan()['next_writes'][0]['after_prefix'], 'next181 rewritten active_plugins after rollback reader cache'],
    'write settings before' => [static fn (): mixed => $plan()['next_writes'][1]['before_prefix'], 'next181 recovered plugin settings from current rollback journal'],
    ...
    'final prefix page four write' => [static fn (): mixed => $plan()['final_prefixes'][4], 'next181 rewritten plugin settings after rollback reader cache'],
    ...
    'dependency marker' => [static fn (): mixed => in_array('sqlite-pager-master-journal-reader-cache-current-source-next181', $plan()['dependencies'], true), true],
    'dependency pending source fence' => [static fn (): mixed => in_array('sqlite-pager-reader-cache-pending-master-source-fence-next181', $plan()['dependencies'], true), true],
    'non-overlap mentions pending' => [static fn (): mixed => str_contains($plan()['non_overlap'], 'pending master-journal'), true],
    'all cache valid no reopen' => [static fn (): mixed => $plan([1 => $cacheEntry('schema-retained', $recovered[1])], [1], [])['requires_reader_reopen'], false],
    'pending master digest invalidates matching image' => [static fn (): mixed => $plan([1 => $cacheEntry('schema-retained', $recovered[1], ['master_journal_digest' => $pendingMasterDigest])], [1], [])['reader_rows'][0]['reason'], 'reader_cache_pending_master_journal_source_rejected'],
    'pending master digest marks pending match' => [static fn (): mixed => $plan([1 => $cacheEntry('schema-retained', $recovered[1], ['master_journal_digest' => $pendingMasterDigest])], [1], [])['reader_rows'][0]['master_journal_digest_matches_pending'], true],
    'pending master digest misses cache' => [static fn (): mixed => $plan([1 => $cacheEntry('schema-retained', $recovered[1], ['master_journal_digest' => $pendingMasterDigest])], [1], [])['next_reads'][0]['cache_hit'], false],
    ...
    'duplicate master members collapsed' => [static fn (): mixed => $plan(null, [1], [], $masterBytes . $databasePath . "-journal\n")['current_members'], [$databasePath . '-journal', '/srv/wp-content/database/wp-next181-meta.sqlite-journal']],
];

foreach ($cases as $name => [$callback, $expected]) {
    $tests['pager master journal reader cache current source next181 ' . $name] = static function (TestRunner $t) use ($callback, $expected): void {
...
$throws = [
    ...
    'empty journal rejected' => static fn () => $plan(null, null, null, null, null, ''),
    'bad journal header rejected' => static fn () => $plan(null, null, null, null, null, 'bad'),
    'journal page size mismatch rejected' => static fn () => $plan(null, null, null, null, null, $journalBytes([1 => $recovered[1]], null, 0x170, 8, 512), null, 1024),
    ...
    'journal page outside rejected' => static fn () => $plan(null, null, null, null, null, $journalBytes([9 => $page('outside')])),
...
foreach ($throws as $name => $callback) {
    $tests['pager master journal reader cache current source next181 ' . $name] = static function (TestRunner $t) use ($callback): void {
