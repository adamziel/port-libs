...
$plan182 = static fn (
    ...
    int $nextCookie = 182,
): array => SQLiteUtf16NocaseLikeRtrimEscapeCurrentSourceNextPlan::keyValueRowKeyEscapeReplayPlan(
    $current ?? $current182,
...
$tests['utf16 nocase like rtrim escape current source nextOneEightTwo clean same bytes can continue'] = static function (TestRunner $t) use ($row182, $enc182): void {
    ...
    $result = SQLiteUtf16NocaseLikeRtrimEscapeCurrentSourceNextPlan::keyValueRowKeyEscapeReplayPlan(
        $rows,
        $rows,
...
$tests['utf16 nocase like rtrim escape current source nextOneEightTwo rejects unsupported encoding'] = static function (TestRunner $t) use ($current182, $nextOneEightTwo, $enc182, $token182): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteUtf16NocaseLikeRtrimEscapeCurrentSourceNextPlan::keyValueRowKeyEscapeReplayPlan(
        $current182,
        $nextOneEightTwo,
        $enc182('plugin!_!%!_cache%', 2),
        2,
