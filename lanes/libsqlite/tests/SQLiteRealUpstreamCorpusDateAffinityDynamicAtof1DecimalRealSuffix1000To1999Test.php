...
// recursive query covers decimal suffixes 0000..9999.  Accepted local files
// own 0000..0999, so this shard owns the next non-overlapping 1000..1999
// suffix window.
$firstSuffix = 1000;
$lastSuffix = 1999;
$cases = [];
...
$scriptFile = tempnam(sys_get_temp_dir(), 'libsqlite-atof1-decimal-1000-oracle-');
if ($scriptFile === false) {
...
$tests['real upstream corpus date affinity dynamic atof1 decimal REAL suffix 1000 1999 source truth'] =
    static function (TestRunner $t) use ($cases, $firstSuffix, $lastSuffix, $oracle, $sourceFile, $sourceText): void {
        ...
        $t->same(1000, $firstSuffix);
        $t->same(1999, $lastSuffix);
        $t->same(1000, $cases['suffix-1000']['suffix']);
        $t->same(1999, $cases['suffix-1999']['suffix']);
        $t->same('18.446744073709551000', $cases['suffix-1000']['value']);
        $t->same('18.446744073709551999', $cases['suffix-1999']['value']);
    };

foreach ($cases as $key => $case) {
    $tests['real upstream corpus date affinity dynamic atof1 decimal REAL suffix 1000 1999 ' . $key] =
...
$tests['real upstream corpus date affinity dynamic atof1 decimal REAL suffix 1000 1999 generic application rollup'] =
    static function (TestRunner $t) use ($cases): void {
        $rows = [
            ['key_name' => 'metric-low', 'decimal_metric' => $cases['suffix-1000']['value']],
            ['key_name' => 'metric-mid', 'decimal_metric' => $cases['suffix-1500']['value']],
            ['key_name' => 'metric-high', 'decimal_metric' => $cases['suffix-1999']['value']],
        ];
        $result = SQLiteSelectSql::execute(
            'SELECT key_name, '
...
$tests['real upstream corpus date affinity dynamic atof1 decimal REAL suffix 1000 1999 non overlap and dependency closure'] =
    static function (TestRunner $t): void {
        $t->same(
            'owns atof1.test atof-3.2 decimal REAL suffixes 1000..1999 only',
            'owns atof1.test atof-3.2 decimal REAL suffixes 1000..1999 only',
        );
        $t->same(
            'non-overlap: avoids accepted atof-3.2 decimal REAL/mantissa suffixes 0000..0999, accepted atof-3.1 integer-prefix suffixes 0592..1609, atof2 rounding, date4 rows, date/timediff matrices, and types storage-class batches',
            'non-overlap: avoids accepted atof-3.2 decimal REAL/mantissa suffixes 0000..0999, accepted atof-3.1 integer-prefix suffixes 0592..1609, atof2 rounding, date4 rows, date/timediff matrices, and types storage-class batches',
        );
        $t->same(
