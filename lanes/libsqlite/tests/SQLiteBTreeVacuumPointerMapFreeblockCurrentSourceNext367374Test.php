...
$makeFirstPage367374 = static function (int $pageCount): string {
...
$putPointerMapEntry367374 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database367374 = static function (int $sliceNumber) use ($makeFirstPage367374, $putPointerMapEntry367374): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage367374(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry367374($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan367374 = static function (int $sliceNumber, int $batchSize = 2) use ($database367374): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database367374($sliceNumber);
...
foreach ([367, 368, 369, 370, 371, 372, 373, 374] as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} keeps next367-374 freelist splice ready"] = static function (TestRunner $t) use ($plan367374, $sliceNumber): void {
        $plan = $plan367374($sliceNumber);
        ...
        $t->same(6, $plan367374($sliceNumber, 3)->freelistSummary()['freelist_row_count']);
    };
}
