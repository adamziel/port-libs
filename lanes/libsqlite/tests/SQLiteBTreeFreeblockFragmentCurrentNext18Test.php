...
$cases = [
    ...
    'reports header fragmented bytes' => static fn (array $fx): mixed => $fx[0]->freeblockCurrentNextFragmentReport($fx[1])['fragmented_free_bytes'],
    'reports current-next fragment status' => static fn (array $fx): mixed => $fx[0]->freeblockCurrentNextFragmentReport($fx[1])['status'],
    'reports current-next fragment bytes' => static fn (array $fx): mixed => $fx[0]->freeblockCurrentNextFragmentReport($fx[1])['current_next_fragment_bytes'],
    'reports no unaccounted fragments' => static fn (array $fx): mixed => $fx[0]->freeblockCurrentNextFragmentReport($fx[1])['unaccounted_fragment_bytes'],
    'reports one current-next fragment' => static fn (array $fx): mixed => count($fx[0]->freeblockCurrentNextFragmentReport($fx[1])['current_next_fragments']),
    'reports fragment current offset' => static fn (array $fx): mixed => $fx[0]->freeblockCurrentNextFragmentReport($fx[1])['current_next_fragments'][0]['current_offset'],
    'reports fragment current end offset' => static fn (array $fx): mixed => $fx[0]->freeblockCurrentNextFragmentReport($fx[1])['current_next_fragments'][0]['current_end_offset'],
    'reports fragment next offset' => static fn (array $fx): mixed => $fx[0]->freeblockCurrentNextFragmentReport($fx[1])['current_next_fragments'][0]['next_offset'],
    'reports fragment byte width' => static fn (array $fx): mixed => $fx[0]->freeblockCurrentNextFragmentReport($fx[1])['current_next_fragments'][0]['fragment_bytes'],
    'allows adjacent current-next freeblocks' => static fn (): mixed => count($fixture(0, 412)[0]->freeblocks($fixture(0, 412)[1])),
    'adjacent current-next report is ok' => static fn (): mixed => $fixture(0, 412)[0]->freeblockCurrentNextFragmentReport($fixture(0, 412)[1])['status'],
    'adjacent current-next has no fragment rows' => static fn (): mixed => count($fixture(0, 412)[0]->freeblockCurrentNextFragmentReport($fixture(0, 412)[1])['current_next_fragments']),
    'adjacent current-next freeblocks have zero fragment bytes' => static fn (): mixed => $fixture(0, 412)[0]->freeblockCurrentNextFragmentReport($fixture(0, 412)[1])['current_next_fragment_bytes'],
    'allows one-byte current-next fragment' => static fn (): mixed => $fixture(1, 413)[0]->freeblockCurrentNextFragmentReport($fixture(1, 413)[1])['current_next_fragment_bytes'],
    'one-byte current-next fragment row width' => static fn (): mixed => $fixture(1, 413)[0]->freeblockCurrentNextFragmentReport($fixture(1, 413)[1])['current_next_fragments'][0]['fragment_bytes'],
    'one-byte current-next fragment has no unaccounted bytes' => static fn (): mixed => $fixture(1, 413)[0]->freeblockCurrentNextFragmentReport($fixture(1, 413)[1])['unaccounted_fragment_bytes'],
    'allows two-byte current-next fragment' => static fn (): mixed => $fixture(2, 414)[0]->freeblockCurrentNextFragmentReport($fixture(2, 414)[1])['current_next_fragment_bytes'],
    'two-byte current-next fragment row width' => static fn (): mixed => $fixture(2, 414)[0]->freeblockCurrentNextFragmentReport($fixture(2, 414)[1])['current_next_fragments'][0]['fragment_bytes'],
    'two-byte current-next fragment has no unaccounted bytes' => static fn (): mixed => $fixture(2, 414)[0]->freeblockCurrentNextFragmentReport($fixture(2, 414)[1])['unaccounted_fragment_bytes'],
    'allows three-byte current-next fragment' => static fn (): mixed => $fixture(3, 415)[0]->freeblockCurrentNextFragmentReport($fixture(3, 415)[1])['current_next_fragment_bytes'],
    'three-byte current-next fragment row width' => static fn (): mixed => $fixture(3, 415)[0]->freeblockCurrentNextFragmentReport($fixture(3, 415)[1])['current_next_fragments'][0]['fragment_bytes'],
    'reports unaccounted fragment bytes separately' => static fn (): mixed => $fixture(5, 415)[0]->freeblockCurrentNextFragmentReport($fixture(5, 415)[1])['unaccounted_fragment_bytes'],
    'unaccounted fragment report remains ok' => static fn (): mixed => $fixture(5, 415)[0]->freeblockCurrentNextFragmentReport($fixture(5, 415)[1])['status'],
    'detects current-next fragments above header count' => static fn (): mixed => $fixture(2, 415)[0]->freeblockCurrentNextFragmentReport($fixture(2, 415)[1])['status'],
    'surfaces fragment overrun error' => static fn (): mixed => $fixture(2, 415)[0]->freeblockCurrentNextFragmentReport($fixture(2, 415)[1])['error'],
