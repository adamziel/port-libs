...
$foreignKey141 = ['parent_key' => 'record_id', 'child_key' => 'record_id', 'on_update' => 'cascade', 'deferred' => true];
$updates141 = [['match' => 10, 'set' => ['record_id' => 110, 'record_title' => 'Rekeyed parent']]];
$triggers141 = [
    [
...
$cases141 = [
    'savepoint name is carried into barrier' => [static fn (): mixed => $blocked141()['savepoint'], 'app_import_batch'],
    ...
    'yielded returning old ids remain visible' => [static fn (): mixed => array_column($blocked141()['yielded_returning_rows'], 'record_id'), [10, 20, 30]],
    'yielded returning new ids remain visible' => [static fn (): mixed => array_column($blocked141()['yielded_returning_rows'], 'new_record_id'), [110, 120, 130]],
    ...
    'invalidated rows carry savepoint name' => [static fn (): mixed => array_column($blocked141()['invalidated_returning_rows'], 'savepoint'), ['app_import_batch', 'app_import_batch', 'app_import_batch']],
    ...
    'invalidated row payload retains first new id' => [static fn (): mixed => $blocked141()['invalidated_returning_rows'][0]['row']['new_record_id'], 110],
    ...
    'attempt after savepoint restores parent ids' => [static fn (): mixed => array_column($blocked141()['attempt']['after_savepoint']['parent'], 'record_id'), [10, 20, 30]],
    'attempt after savepoint restores child ids' => [static fn (): mixed => array_column($blocked141()['attempt']['after_savepoint']['child'], 'record_id'), [10, 20, 30]],
    ...
    'release path keeps returning rows durable until caller rolls back transaction' => [static fn (): mixed => array_column($released141()['durable_returning_rows'], 'new_record_id'), [110, 120, 130]],
    ...
    'non recursive durable row survives clean commit' => [static fn (): mixed => array_column($nonRecursive141()['durable_returning_rows'], 'new_record_id'), [110]],
    ...
    'bad parent key throws' => [static fn (): mixed => SQLiteTriggerDeferredReturningSavepointCurrentSourceNextPlan::commitBarrierRetry($parents141, $children141, $updates141, ['parent_key' => 'bad-key', 'child_key' => 'record_id']), InvalidArgumentException::class],
    'missing child key throws from retry key collection' => [static function () use ($parents141, $updates141, $foreignKey141): mixed {
        return SQLiteTriggerDeferredReturningSavepointCurrentSourceNextPlan::commitBarrierRetry($parents141, [['detail_id' => 1]], $updates141, $foreignKey141);
