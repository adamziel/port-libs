...
$base = [
    'source_id' => 'pager-reader-cache-current-source-next526',
    'epoch' => 526,
    'format_signature' => hash('sha256', implode('|', [512, 4, 2, 526, 0])),
    'publication_generation' => 526,
    'master_source_digest' => hash('sha256', 'master-next526'),
...
$plan = static fn (array $cacheExtra = [], array $readExtra = []): array => SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::readerCacheStatementFence(526,
    $database,
    $master,
    $masterBytes,
    implode('', $before),
...
$tests['pager master journal reader cache current source next526 admits current VDBE branch fences'] = static function (TestRunner $t) use ($plan): void {
    $result = $plan();
    $t->same('pager-master-journal-reader-cache-current-source-next526', $result['status']);
    $t->same([], $result['invalidated_cache_page_numbers']);
    $t->same(['read-options' => true], $result['read_cache_hits']);
    $t->same('reader-cache-stmt-vdbe-vfilter-current-526', $result['current_reader_cache_stmt_vdbe_vfilter_token']);
};

$tests['pager master journal reader cache current source plan dispatches full width calls to next526'] = static function (TestRunner $t) use ($database, $master, $masterBytes, $before, $pageSize, $recovered, $cacheEntry, $read, $base, $tokens, $headers, $tokenFields): void {
...
$tests['pager master journal reader cache current source next526 invalidates stale vfilter cache'] = static function (TestRunner $t) use ($plan, $opCount): void {
    $result = $plan(['reader_cache_stmt_vdbe_vfilter_token' => 'stmt-vdbe-vfilter-old']);
    $t->same([1], $result['reader_cache_stmt_vdbe_vfilter_invalidated_cache_page_numbers']);
    $t->same([1], $result['invalidated_cache_page_numbers']);
    $t->same(1, $opCount($result, 'invalidate_reader_cache_reader_cache_stmt_vdbe_vfilter_current_source_next526'));
};

$tests['pager master journal reader cache current source next526 reopens stale vfilter read ticket'] = static function (TestRunner $t) use ($plan, $opCount): void {
    $result = $plan([], ['reader_cache_stmt_vdbe_vfilter_token' => 'stmt-vdbe-vfilter-old']);
    $t->same(['read-options'], $result['reopen_reader_ids']);
    $t->same(false, $result['next_reads'][0]['cache_hit']);
    $t->same('reader_ticket_reader_cache_stmt_vdbe_vfilter_predates_current_source', $result['next_reads'][0]['reader_cache_stmt_vdbe_vfilter_token_reason']);
    $t->same(1, $opCount($result, 'reopen_reader_for_reader_cache_stmt_vdbe_vfilter_current_source_next526'));
};

$tests['pager master journal reader cache current source next526 missing vfilter token rejects'] = static function (TestRunner $t) use ($plan): void {
    $t->throws(Throwable::class, static fn () => $plan(['reader_cache_stmt_vdbe_vfilter_token' => null]));
};
