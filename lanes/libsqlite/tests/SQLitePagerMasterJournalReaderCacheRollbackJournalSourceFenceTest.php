...
$pageSize = 512;
$database = '/srv/wp-content/database/wp-rollback-source-fence.sqlite';
$usersDatabase = '/srv/wp-content/database/wp-rollback-source-fence-users.sqlite';
$mainJournal = $database . '-journal';
$usersJournal = $usersDatabase . '-journal';
$master = $database . '-mj';
$sourceId = 'pager-reader-cache-rollback-source-rollback-source-fence';
...
$formatPage = static function (string $label) use ($pageSize): string {
    ...
    $page = substr_replace($page, pack('N', 267), 60, 4);
    $page = substr_replace($page, pack('N', 0x57503267), 68, 4);
...
$before = [
    1 => $formatPage('rollback-source-fence stale schema before rollback-source fence'),
    2 => $page('rollback-source-fence stale wp_options root before rollback-source fence'),
    3 => $page('rollback-source-fence stale active_plugins before rollback-source fence'),
    4 => $page('rollback-source-fence stale cron before rollback-source fence'),
    5 => $page('rollback-source-fence stale plugin cache before rollback-source fence'),
    6 => $page('rollback-source-fence stale autoload index before rollback-source fence'),
];
$recovered = [
    1 => $formatPage('rollback-source-fence current schema after rollback-source fence'),
    2 => $page('rollback-source-fence current wp_options root after rollback-source fence'),
    3 => $page('rollback-source-fence current active_plugins after rollback-source fence'),
    4 => $page('rollback-source-fence current cron after rollback-source fence'),
    5 => $page('rollback-source-fence current plugin cache after rollback-source fence'),
    6 => $page('rollback-source-fence current autoload index after rollback-source fence'),
];
$tokens = [
    $mainJournal => 'dev=8:ino=2671:size=4096:mtime=26701:generation=main-current',
    $usersJournal => 'dev=8:ino=2672:size=1024:mtime=26702:generation=users-current',
];
$headers = [
    $mainJournal => hash('sha256', 'main rollback header rollback-source-fence'),
    $usersJournal => hash('sha256', 'users rollback header rollback-source-fence'),
];
$formatSignature = hash('sha256', implode('|', [512, 4, 2, 267, 0x57503267]));
$masterDigest = hash('sha256', 'rollback-source-fence master source');
$masterToken = 'dev=8:ino=2670:size=96:mtime=26700:generation=master-current';
$databaseToken = 'dev=8:ino=2679:size=3072:mtime=26799:generation=database-current';
$cleanupToken = 'master-cleanup:deleted:mtime=26500:dirsync=ok';
$readerLeaseToken = 'reader-lease:shared-cache:epoch=267:opened-after-master-cleanup';
$pagerCacheSourceToken = 'pager-cache-source:epoch=267:master-journal-recovery=complete';
$readTransactionToken = 'read-transaction:epoch=267:schema=117:change-counter=267:master-current';
$schemaReparseToken = 'schema-reparse:epoch=267:schema-cookie=117:ddl=master-current';
$statementSchemaRootToken = 'statement-schema-root:epoch=267:root=1:cookie=117:sql=wp-options-current';
$currentSourceProvenanceToken = 'current-source:master-journal:epoch=267:members=2:database-token=2679:schema=117';
$oldCurrentSourceProvenanceToken = 'current-source:master-journal:epoch=263:members=2:database-token=2639:schema=116';
$currentPagerReaderCacheGenerationToken = 'pager-reader-cache-generation:epoch=267:master-current:reset=complete';
$oldPagerReaderCacheGenerationToken = 'pager-reader-cache-generation:epoch=263:before-master-reset';
$currentReaderSnapshotToken = 'reader-snapshot:epoch=267:source=master-current:lease=shared:pages=1,2,3,4,5,6';
$oldReaderSnapshotToken = 'reader-snapshot:epoch=263:source=before-master-current:lease=shared';
$currentRecoveryReceiptToken = 'master-journal-recovery-receipt:epoch=267:members=main,users:cleanup=deleted:dirsync=ok';
$oldRecoveryReceiptToken = 'master-journal-recovery-receipt:epoch=263:members=main,users:cleanup=pending';
$currentPagerSpillDrainToken = 'pager-spill-drain:epoch=267:dirty-pages=2,6:journal-sync=ok:exclusive-lock=held';
$currentRollbackJournalReaderSourceToken = 'pager-rollback-source:epoch=267:dirty-pages=2,6:journal-sync=ok:exclusive-lock=held';
$oldRollbackJournalReaderSourceToken = 'pager-rollback-source:epoch=263:dirty-pages=2,6:journal-sync=pending';
$base = [
    'source_id' => $sourceId,
    'epoch' => 267,
    'format_signature' => $formatSignature,
    'publication_generation' => 267,
    'master_source_digest' => $masterDigest,
    'recovery_sequence' => 267,
...
$plan = static fn (?array $readerCache = null, ?array $nextReads = null, ?string $rollbackSourceToken = null): array => SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::readerCacheRollbackJournalSourceFence('rollback-source-fence',
    $database,
    $master,
    $masterBytes,
    implode('', $before),
...
$cases = [
    'status' => [static fn (): mixed => $plan()['status'], 'pager-master-journal-reader-cache-current-source-rollback-source-fence'],
    ...
    'operation invalidates stale rollback-source cache' => [static fn (): mixed => $opCount($plan(), 'invalidate_reader_cache_rollback_journal_reader_source_current_source_rollback-source-fence'), 1],
    'operation reopens stale rollback-source reader' => [static fn (): mixed => $opCount($plan(), 'reopen_reader_for_rollback_journal_reader_source_current_source_rollback-source-fence'), 1],
    'dependency rollback-source-fence' => [static fn (): mixed => in_array('sqlite-pager-master-journal-reader-cache-current-source-rollback-source-fence', $plan()['dependencies'], true), true],
    ...
    'read stale rollback-source source' => [static fn (): mixed => $readRow('read-2')['source'], 'master-journal-reader-cache-rollback-source-fence-current-source-rollback-source-fence'],
    ...
    'changed current rollback-source invalidates admitted pages' => [static fn (): mixed => $plan(null, null, 'pager-rollback-source:epoch=265:dirty-pages=2,6:journal-sync=ok:exclusive-lock=held')['rollback_journal_reader_source_invalidated_cache_page_numbers'], [1, 2, 6]],
    'changed current rollback-source keeps inherited invalidation' => [static fn (): mixed => in_array(3, $plan(null, null, 'pager-rollback-source:epoch=265:dirty-pages=2,6:journal-sync=ok:exclusive-lock=held')['invalidated_cache_page_numbers'], true), true],
    'changed current rollback-source surfaced' => [static fn (): mixed => $plan(null, null, 'pager-rollback-source:epoch=265:dirty-pages=2,6:journal-sync=ok:exclusive-lock=held')['current_rollback_journal_reader_source_token'], 'pager-rollback-source:epoch=265:dirty-pages=2,6:journal-sync=ok:exclusive-lock=held'],
...
foreach ($cases as $name => [$callback, $expected]) {
    $tests['pager master journal reader cache current source rollback-source-fence ' . $name] = static function (TestRunner $t) use ($callback, $expected): void {
...
foreach ($throws as $name => $callback) {
    $tests['pager master journal reader cache current source rollback-source-fence ' . $name] = static function (TestRunner $t) use ($callback): void {
