...
): array => SQLitePragmaIndexListIntegrityRootpageCurrentSourceNext143::currentNextPage(
    $catalogFactory(),
    $currentDatabase,
...
$tests['pragma index list integrity rootpage current source next143 rejects negative offset'] = static function (TestRunner $t) use ($catalogFactory, $currentDatabase, $nextDatabase): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLitePragmaIndexListIntegrityRootpageCurrentSourceNext143::currentNextPage($catalogFactory(), $currentDatabase, $catalogFactory(), $nextDatabase, 'PRAGMA main.index_list(wp_options)', -1));
};
...
$tests['pragma index list integrity rootpage current source next143 rejects zero limit'] = static function (TestRunner $t) use ($catalogFactory, $currentDatabase, $nextDatabase): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLitePragmaIndexListIntegrityRootpageCurrentSourceNext143::currentNextPage($catalogFactory(), $currentDatabase, $catalogFactory(), $nextDatabase, 'PRAGMA main.index_list(wp_options)', 0, 0));
};
