...
$summary184 = static fn (): array => SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNextPlan::compareRecursiveLimitExhaustedYieldBoundary($sql184, $currentTables184, $nextTables184);
$tests = [];
...
$tests['compound select window recursive limit current source next184 rejects non exhausted recursive limit'] = static function (TestRunner $t) use ($currentTables184): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNextPlan::compareRecursiveLimitExhaustedYieldBoundary(
...
foreach (range(1, 52) as $case) {
    $tests['compound select window recursive limit current source next184 generated exhausted source tape ' . $case] = static function (TestRunner $t) use ($case): void {
        ...
        $plan = SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNextPlan::compareRecursiveLimitExhaustedYieldBoundary($generatedSql, $tables, $tables);
