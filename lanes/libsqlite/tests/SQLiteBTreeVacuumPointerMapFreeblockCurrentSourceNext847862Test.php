...
$makeFirstPage847862 = static function (int $pageCount): string {
...
$putPointerMapEntry847862 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database847862 = static function (int $sliceNumber) use ($makeFirstPage847862, $putPointerMapEntry847862): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage847862(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry847862($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan847862 = static function (int $sliceNumber, int $batchSize = 2) use ($database847862): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database847862($sliceNumber);
...
foreach ([847, 848, 849, 850, 851, 852, 853, 854, 855, 856, 857, 858, 859, 860, 861, 862] as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} extends next847-862 handoff receipts"] = static function (TestRunner $t) use ($plan847862, $sliceNumber): void {
        $plan = $plan847862($sliceNumber);
        ...
        $t->same(6, $plan847862($sliceNumber, 3)->currentSourceSummary()['current_source_row_count']);
    };
}
