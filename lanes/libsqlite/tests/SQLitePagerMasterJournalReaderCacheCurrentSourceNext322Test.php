...
$pageSize = 512;
$database = '/srv/wp-content/database/wp-next322.sqlite';
$usersDatabase = '/srv/wp-content/database/wp-next322-users.sqlite';
...
$before = [1 => $formatPage('next322 stale schema'), 2 => $page('next322 stale options'), 3 => $page('next322 stale plugins'), 4 => $page('next322 stale users')];
$recovered = [1 => $formatPage('next322 current schema'), 2 => $page('next322 current options'), 3 => $page('next322 current plugins'), 4 => $page('next322 current users')];
$tokens = [$mainJournal => 'member-main-current-322', $usersJournal => 'member-users-current-322'];
$headers = [$mainJournal => hash('sha256', 'main header next322'), $usersJournal => hash('sha256', 'users header next322')];
$base = [
    'source_id' => 'pager-reader-cache-current-source-next322',
    'epoch' => 322,
    'format_signature' => hash('sha256', implode('|', [512, 4, 2, 322, 0])),
    'publication_generation' => 322,
    'master_source_digest' => hash('sha256', 'master-next322'),
...
$plan = static function (?array $cache = null, ?array $reads = null) use ($database, $master, $masterBytes, $before, $pageSize, $recovered, $base, $tokens, $headers, $cacheEntry, $read): array {
    return SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::variantNext322($database, $master, $masterBytes, implode('', $before), $pageSize, $recovered, $cache ?? [
...
$cases = [
    'status' => [static fn (): mixed => $plan()['status'], 'pager-master-journal-reader-cache-current-source-next322'],
    ...
    'dependencies include next322' => [static fn (): mixed => in_array('sqlite-pager-master-journal-reader-cache-current-source-next322', $plan()['dependencies'], true), true],
    ...
    'default plan points at next322' => [static fn (): mixed => $plan()['status'], 'pager-master-journal-reader-cache-current-source-next322'],
];

foreach ($cases as $name => [$callback, $expected]) {
    $tests['pager master journal reader cache current source next322 ' . $name] = static function (TestRunner $t) use ($callback, $expected): void {
        $t->same($expected, $callback());
    };
}

$tests['pager master journal reader cache current source next322 missing token rejects'] = static function (TestRunner $t) use ($plan, $cacheEntry, $recovered): void {
