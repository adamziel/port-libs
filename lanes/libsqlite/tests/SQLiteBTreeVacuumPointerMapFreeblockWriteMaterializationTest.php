...
$plan217 = static function (int $batchSize = 2): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourcePlan {
    global $database217;

    $database = $database217();
    $deletedPage = SQLiteTableLeafPage::deleteCellByRowId($database->page(3), 2, secureDelete: true);

    return SQLiteBTreeVacuumPointerMapFreeblockCurrentSourcePlan::tableLeafWriteAdmissionAuditFromDeleteResult(
        $database,
        3,
        [
            'page' => $deletedPage,
...
$cases217 = [
    ...
    'current source token length' => static fn (): mixed => strlen($plan217()->writeSummary()['current_source_write_materialization_token']),
    ...
    'non overlap' => static fn (): mixed => str_contains($plan217()->writeSummary()['non_overlap'], 'does not repeat page-apply'),
...
$expected217 = [
    'action label' => 'btree-vacuum-pointermap-freeblock-write-materialization',
    ...
    'dependency closure' => 'no new support component needed; write-materialization reuses page-apply current-source apply rows, pointer-map apply pages, leaf freeblock receipts, and fenced-tail guards',
    'non overlap' => true,
...
foreach ($cases217 as $name => $callback) {
    $tests['btree vacuum pointermap freeblock current source write-materialization ' . $name] = static function (TestRunner $t) use ($callback, $expected217, $name): void {
        $t->same($expected217[$name], $callback());
    };
}

foreach (range(1, 70) as $index) {
    $tests['btree vacuum pointermap freeblock current source write-materialization write invariant ' . $index] = static function (TestRunner $t) use ($plan217): void {
        $plan = $plan217();
        ...
        $t->same('btree-vacuum-pointermap-freeblock-write-materialization-ready', $summary['status']);
