...
$pageSize = 512;
$database = '/srv/wp-content/database/wp-next686.sqlite';
...
$formatPage = static function (string $label) use ($pageSize): string {
    ...
    $page = substr_replace($page, pack('N', 686), 60, 4);
...
$variantFields = array_merge($tokenFields, $next639654Fields, $next655670Fields, $next671686Fields);
$before = [1 => $formatPage('stale schema'), 2 => $page('stale options')];
$recovered = [1 => $formatPage('current schema'), 2 => $page('current options')];
$tokens = [$journal => 'member-main-current-686'];
$headers = [$journal => hash('sha256', 'main header next686')];
$base = [
    'source_id' => 'pager-reader-cache-current-source-next686',
    'epoch' => 686,
    'format_signature' => hash('sha256', implode('|', [512, 4, 2, 686, 0])),
    'publication_generation' => 686,
    'master_source_digest' => hash('sha256', 'master-next686'),
    'recovery_sequence' => 686,
...
foreach ($variantFields as $field) {
    $base[$field] = str_replace('_', '-', preg_replace('/_token$/', '', $field)) . '-current-686';
}
...
$plan = static fn (array $cacheExtra = [], array $readExtra = []): array => SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::variantNext686(
    $database,
    $master,
    $masterBytes,
    implode('', $before),
...
$tests['pager master journal reader cache current source next686 admits current VDBE literal and arithmetic opcode fences'] = static function (TestRunner $t) use ($plan): void {
    $result = $plan();
    $t->same('pager-master-journal-reader-cache-current-source-next686', $result['status']);
    $t->same([], $result['invalidated_cache_page_numbers']);
    $t->same(['read-options' => true], $result['read_cache_hits']);
    $t->same('reader-cache-stmt-vdbe-null-branch-current-686', $result['current_reader_cache_stmt_vdbe_null_branch_token']);
    $t->same('reader-cache-stmt-vdbe-bit-and-handoff-current-686', $result['current_reader_cache_stmt_vdbe_bit_and_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-opcode-trace-branch-handoff-current-686', $result['current_reader_cache_stmt_vdbe_opcode_trace_branch_handoff_token']);
    $t->same(true, in_array('sqlite-pager-master-journal-reader-cache-current-source-next670', $result['dependencies'], true));
};

$tests['pager master journal reader cache current source plan dispatches full width calls to next686'] = static function (TestRunner $t) use ($database, $master, $masterBytes, $before, $pageSize, $recovered, $cacheEntry, $read, $base, $tokens, $headers, $variantFields): void {
...
$tests['pager master journal reader cache current source next686 invalidates stale opcode trace branch cache'] = static function (TestRunner $t) use ($plan, $opCount): void {
    $result = $plan(['reader_cache_stmt_vdbe_opcode_trace_branch_handoff_token' => 'stmt-vdbe-opcode-trace-branch-old']);
    $t->same([1], $result['reader_cache_stmt_vdbe_opcode_trace_branch_handoff_invalidated_cache_page_numbers']);
    $t->same([1], $result['invalidated_cache_page_numbers']);
    $t->same(1, $opCount($result, 'invalidate_reader_cache_reader_cache_stmt_vdbe_opcode_trace_branch_handoff_current_source_next686'));
};

$tests['pager master journal reader cache current source next686 reopens stale opcode trace branch read ticket'] = static function (TestRunner $t) use ($plan, $opCount): void {
    $result = $plan([], ['reader_cache_stmt_vdbe_opcode_trace_branch_handoff_token' => 'stmt-vdbe-opcode-trace-branch-old']);
    $t->same(['read-options'], $result['reopen_reader_ids']);
    $t->same(false, $result['next_reads'][0]['cache_hit']);
    $t->same('reader_ticket_reader_cache_stmt_vdbe_opcode_trace_branch_handoff_predates_current_source', $result['next_reads'][0]['reader_cache_stmt_vdbe_opcode_trace_branch_handoff_token_reason']);
    $t->same(1, $opCount($result, 'reopen_reader_for_reader_cache_stmt_vdbe_opcode_trace_branch_handoff_current_source_next686'));
};

$tests['pager master journal reader cache current source next686 missing opcode trace branch token rejects'] = static function (TestRunner $t) use ($plan): void {
    $t->throws(Throwable::class, static fn () => $plan(['reader_cache_stmt_vdbe_opcode_trace_branch_handoff_token' => null]));
};
