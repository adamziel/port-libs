...
$plan237 = static fn (array $options = []): array => SQLiteTriggerRecursiveViewUpsertCurrentSourceNextPlan::executeCurrentActionReceipt(
    ...
    $options + [
        ...
        'reset_generation' => 'app-current-reset-237',
        'post_reset_current_source_token' => 'app.current.source.postreset.237',
        'post_reset_cursor' => 'app.returning.postreset.cursor.237',
        ...
        'following_generation' => 'app-following-current-237',
...
$tokenMismatch237 = static fn (): array => $plan237(['auto_ack_current_source_upsert_actions_next237' => true, 'expected_current_source_upsert_action_token_next237' => 'app.current.source.upsert.action.stale.237']);
...
$cases237 = [
    ...
    'savepoint retained' => [static fn (): mixed => $released237()['savepoint'], 'app_recursive_view_237'],
    ...
    'action token retained' => [static fn (): mixed => $released237()['current_source_upsert_action_token_next237'], 'app.current.source.upsert.action.237'],
    'expected action token defaults' => [static fn (): mixed => $released237()['expected_current_source_upsert_action_token_next237'], 'app.current.source.upsert.action.237'],
    ...
    'default action names' => [static fn (): mixed => array_column($released237()['current_source_upsert_actions_next237'], 'name'), ['app_summary_child', 'template_child']],
    ...
    'current token stamped' => [static fn (): mixed => array_values(array_unique(array_column($released237()['current_source_rows_next237'], 'current_source_upsert_action_token_next237'))), ['app.current.source.upsert.action.237']],
    ...
    'visible payload names released' => [static fn (): mixed => array_column($released237()['visible_returning_payloads_next237'], 'name'), ['app_summary_child', 'template_child', 'landing_url', 'next_module']],
    'held payload names missing' => [static fn (): mixed => array_column($missing237()['held_next_returning_payloads_next237'], 'name'), ['landing_url', 'next_module']],
