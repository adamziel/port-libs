...
$plan108 = static fn (?array $prepared = null, ?array $states = null): array => SQLiteAttachTempWalSchemaTriggerPlan::triggerViewInvalidationCurrentSourceNext108(
    $current108(),
    $next108(),
...
$cases108 = [
    ...
    'retains view cache dependency' => ['dependencies.1', 'sqlite-attach-temp-wal-trigger-view-cache-current-source-next97'],
...
$tests['attach temp trigger view invalidation current source next108 rejects empty prepared list'] = static function (TestRunner $t) use ($current108, $next108): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteAttachTempWalSchemaTriggerPlan::triggerViewInvalidationCurrentSourceNext108($current108(), $next108(), []));
};
...
$tests['attach temp trigger view invalidation current source next108 rejects missing trigger'] = static function (TestRunner $t) use ($current108, $next108): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteAttachTempWalSchemaTriggerPlan::triggerViewInvalidationCurrentSourceNext108($current108(), $next108(), [['name' => 'missing']]));
};
