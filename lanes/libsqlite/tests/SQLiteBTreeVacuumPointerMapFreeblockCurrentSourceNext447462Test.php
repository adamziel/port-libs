...
$makeFirstPage447462 = static function (int $pageCount): string {
...
$putPointerMapEntry447462 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database447462 = static function (int $sliceNumber) use ($makeFirstPage447462, $putPointerMapEntry447462): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage447462(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry447462($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan447462 = static function (int $sliceNumber, int $batchSize = 2) use ($database447462): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database447462($sliceNumber);
...
foreach ([447, 448, 449, 450, 451, 452, 453, 454, 455, 456, 457, 458, 459, 460, 461, 462] as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} publishes next447-462 handoff receipts"] = static function (TestRunner $t) use ($plan447462, $sliceNumber): void {
        $plan = $plan447462($sliceNumber);
        ...
        $t->same(6, $plan447462($sliceNumber, 3)->currentSourceSummary()['current_source_row_count']);
    };
}
