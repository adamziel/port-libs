...
$pageSize = 512;
$database = '/srv/wp-content/database/wp-next798.sqlite';
...
$formatPage = static function (string $label) use ($pageSize): string {
    ...
    $page = substr_replace($page, pack('N', 798), 60, 4);
...
$tokenFields = [
    ...
    'reader_cache_stmt_vdbe_bit_not_token', 'reader_cache_stmt_vdbe_real_affinity_value_token', 'reader_cache_stmt_vdbe_cast_affinity_token', 'reader_cache_stmt_vdbe_permutation_affinity_token',
...
$next639654Fields = [
    ...
    'reader_cache_stmt_vdbe_bit_not_handoff_token', 'reader_cache_stmt_vdbe_real_affinity_value_handoff_token', 'reader_cache_stmt_vdbe_cast_affinity_handoff_token',
...
$next719734Fields = [
    'reader_cache_stmt_vdbe_add_imm_branch_handoff_token', 'reader_cache_stmt_vdbe_bit_not_branch_handoff_token', 'reader_cache_stmt_vdbe_real_affinity_value_branch_handoff_token',
...
$variantFields = array_merge($tokenFields, $next639654Fields, $next655670Fields, $next671686Fields, $next687702Fields, $next703718Fields, $next719734Fields, $next735766Fields, $next751766Fields, $next767782Fields, $next783798Fields);
$before = [1 => $formatPage('stale schema'), 2 => $page('stale options')];
$recovered = [1 => $formatPage('current schema'), 2 => $page('current options')];
$tokens = [$journal => 'member-main-current-798'];
$headers = [$journal => hash('sha256', 'main header next798')];
$base = [
    'source_id' => 'pager-reader-cache-current-source-next798',
    'epoch' => 798,
    'format_signature' => hash('sha256', implode('|', [512, 4, 2, 798, 0])),
    'publication_generation' => 798,
    'master_source_digest' => hash('sha256', 'master-next798'),
    'recovery_sequence' => 798,
...
foreach ($variantFields as $field) {
    $base[$field] = str_replace('_', '-', preg_replace('/_token$/', '', $field)) . '-current-798';
}
...
$plan = static fn (array $cacheExtra = [], array $readExtra = []): array => SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::currentSourceVdbeValueArithmeticBranchHandoff(
    $database,
    $master,
    $masterBytes,
    implode('', $before),
...
$tests['pager master journal reader cache current source next798 admits current VDBE literal and arithmetic opcode fences'] = static function (TestRunner $t) use ($plan): void {
    $result = $plan();
    $t->same('pager-master-journal-reader-cache-current-source-next798', $result['status']);
    $t->same([], $result['invalidated_cache_page_numbers']);
    $t->same(['read-options' => true], $result['read_cache_hits']);
    $t->same('reader-cache-stmt-vdbe-null-branch-current-798', $result['current_reader_cache_stmt_vdbe_null_branch_token']);
    $t->same('reader-cache-stmt-vdbe-opcode-trace-branch-handoff-current-798', $result['current_reader_cache_stmt_vdbe_opcode_trace_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-shift-right-branch-handoff-current-798', $result['current_reader_cache_stmt_vdbe_shift_right_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-gt-branch-handoff-current-798', $result['current_reader_cache_stmt_vdbe_gt_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-incr-vacuum-branch-handoff-current-798', $result['current_reader_cache_stmt_vdbe_incr_vacuum_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-init-branch-handoff-current-798', $result['current_reader_cache_stmt_vdbe_init_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-goto-branch-handoff-current-798', $result['current_reader_cache_stmt_vdbe_goto_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-null-row-branch-handoff-current-798', $result['current_reader_cache_stmt_vdbe_null_row_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-row-value-branch-handoff-current-798', $result['current_reader_cache_stmt_vdbe_row_value_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-real-affinity-value-branch-handoff-current-798', $result['current_reader_cache_stmt_vdbe_real_affinity_value_branch_handoff_token']);
...
$tests['pager master journal reader cache current source plan dispatches full width calls to next798'] = static function (TestRunner $t) use ($database, $master, $masterBytes, $before, $pageSize, $recovered, $cacheEntry, $read, $base, $tokens, $headers, $variantFields): void {
...
$tests['pager master journal reader cache current source next798 preserves earlier le branch invalidation before duplicate handoff'] = static function (TestRunner $t) use ($plan, $opCount): void {
    ...
    $t->same('reader_cache_stmt_vdbe_le_branch_handoff_must_match_current_stmt_vdbe_le_branch_handoff_state', $result['reader_rows'][0]['reader_cache_stmt_vdbe_le_branch_handoff_token_reason']);
    $t->same(0, $opCount($result, 'invalidate_reader_cache_reader_cache_stmt_vdbe_le_branch_handoff_current_source_next735'));
};

$tests['pager master journal reader cache current source next798 reopens stale real affinity branch handoff read ticket'] = static function (TestRunner $t) use ($plan, $opCount): void {
    $result = $plan([], ['reader_cache_stmt_vdbe_real_affinity_value_branch_handoff_token' => 'stmt-vdbe-real-affinity-value-branch-handoff-old']);
    $t->same(['read-options'], $result['reopen_reader_ids']);
    $t->same(false, $result['next_reads'][0]['cache_hit']);
    $t->same('reader_ticket_reader_cache_stmt_vdbe_real_affinity_value_branch_handoff_predates_current_source', $result['next_reads'][0]['reader_cache_stmt_vdbe_real_affinity_value_branch_handoff_token_reason']);
    $t->same(1, $opCount($result, 'reopen_reader_for_reader_cache_stmt_vdbe_real_affinity_value_branch_handoff_current_source_next798'));
};

$tests['pager master journal reader cache current source next798 missing real affinity branch handoff token rejects'] = static function (TestRunner $t) use ($plan): void {
    $t->throws(Throwable::class, static fn () => $plan(['reader_cache_stmt_vdbe_real_affinity_value_branch_handoff_token' => null]));
};
