...
$base = [
    'source_id' => 'pager-reader-cache-current-source-next782',
    'epoch' => 782,
    'format_signature' => hash('sha256', implode('|', [512, 4, 2, 782, 0])),
    'publication_generation' => 782,
    'master_source_digest' => hash('sha256', 'master-next782'),
...
$plan = static fn (array $cacheExtra = [], array $readExtra = []): array => SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::variantNext782(
    $database,
    $master,
    $masterBytes,
    implode('', $before),
...
$tests['pager master journal reader cache current source next782 admits current VDBE literal and arithmetic opcode fences'] = static function (TestRunner $t) use ($plan): void {
    $result = $plan();
...
$tests['pager master journal reader cache current source plan dispatches full width calls to next782'] = static function (TestRunner $t) use ($database, $master, $masterBytes, $before, $pageSize, $recovered, $cacheEntry, $read, $base, $tokens, $headers, $variantFields): void {
...
$tests['pager master journal reader cache current source next782 preserves earlier le branch invalidation before duplicate handoff'] = static function (TestRunner $t) use ($plan, $opCount): void {
...
$tests['pager master journal reader cache current source next782 reopens stale null row branch handoff read ticket'] = static function (TestRunner $t) use ($plan, $opCount): void {
...
$tests['pager master journal reader cache current source next782 missing null row branch handoff token rejects'] = static function (TestRunner $t) use ($plan): void {
