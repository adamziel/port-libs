...
use PortLibs\LibSqlite\SQLiteVfsCurrentSourceNext578593Plan;
...
$plan = static function () use ($readyCurrent): array {
    static $result = null;
    if ($result === null) {
        $result = SQLiteVfsCurrentSourceNext578593Plan::run([
...
return [
    ...
    'vfs current source next578-593 blocks dirty snapshot' => static fn (TestRunner $t) => $t->same(true, in_array('dirty-pages-present', SQLiteVfsCurrentSourceNext578593Plan::run(['snapshot(reader-ready-next593,shared-cache-next577)'], ['current' => $dirtyCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source next578-593 blocks old snapshot ack' => static fn (TestRunner $t) => $t->same(true, in_array('ack-not-latest-publish', SQLiteVfsCurrentSourceNext578593Plan::run(['snapshot(reader-ready-next593,shared-cache-next577)'], ['current' => $oldAckCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source next578-593 blocks publish without claim' => static fn (TestRunner $t) => $t->same(true, in_array('missing-reuse-claim', SQLiteVfsCurrentSourceNext578593Plan::run(['snapshot(reader-ready-next593,shared-cache-next577)', 'publish(reader-ready-next593,reader-reuse-next593,shared-cache-next593)'], ['current' => $readyCurrent])['events'][1]['blocked_reasons'], true)),
    'vfs current source next578-593 blocks stale claim' => static fn (TestRunner $t) => $t->same(true, in_array('stale-reuse-claim', SQLiteVfsCurrentSourceNext578593Plan::run(['publish(reader-ready-next593,reader-reuse-next593,shared-cache-next593)'], ['current' => $staleClaimCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source next578-593 accepts preclaimed publish' => static fn (TestRunner $t) => $t->same('published-reused-current-source', SQLiteVfsCurrentSourceNext578593Plan::run(['publish(reader-ready-next593,reader-reuse-next593,shared-cache-next593)'], ['current' => $claimedCurrent])['events'][0]['status']),
    'vfs current source next578-593 rejects empty operations' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteVfsCurrentSourceNext578593Plan::run([])),
    'vfs current source next578-593 rejects bad claim token' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteVfsCurrentSourceNext578593Plan::run([['op' => 'claim', 'snapshot' => 'reader-ready-next593', 'ack' => 'shared-cache-next577', 'claim' => 'bad claim']], ['current' => $readyCurrent])),
    'vfs current source next578-593 rejects unsupported operation' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteVfsCurrentSourceNext578593Plan::run(['republish(reader-ready-next593,shared-cache-next593)'], ['current' => $readyCurrent])),
