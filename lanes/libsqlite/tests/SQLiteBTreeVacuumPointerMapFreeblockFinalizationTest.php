...
$plan230 = static function (int $batchSize = 2): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourcePlan {
    global $database230;

    $database = $database230();
    $deletedPage = SQLiteTableLeafPage::deleteCellByRowId($database->page(3), 2, secureDelete: true);

    return SQLiteBTreeVacuumPointerMapFreeblockCurrentSourcePlan::tableLeafFinalizationFromDeleteResult(
        $database,
        3,
        [
            'page' => $deletedPage,
...
$cases230 = [
    ...
    'current source token length' => static fn (): mixed => strlen($plan230()->finalSummary()['current_source_finalization_token']),
    ...
    'non overlap' => static fn (): mixed => str_contains($plan230()->finalSummary()['non_overlap'], 'does not repeat publication'),
...
$expected230 = [
    'action label' => 'btree-vacuum-pointermap-freeblock-finalization',
    ...
    'dependency closure' => 'no new support component needed; finalization reuses publication seals, duplicate pointer-map rewrite receipts, leaf freeblock receipts, and fenced-tail guards',
    'non overlap' => true,
...
foreach ($cases230 as $name => $callback) {
    $tests['btree vacuum pointermap freeblock current source finalization ' . $name] = static function (TestRunner $t) use ($callback, $expected230, $name): void {
        $t->same($expected230[$name], $callback());
    };
}

foreach (range(1, 90) as $index) {
    $tests['btree vacuum pointermap freeblock current source finalization final invariant ' . $index] = static function (TestRunner $t) use ($plan230): void {
        $plan = $plan230();
        ...
        $t->same('btree-vacuum-pointermap-freeblock-finalization-ready', $summary['status']);
