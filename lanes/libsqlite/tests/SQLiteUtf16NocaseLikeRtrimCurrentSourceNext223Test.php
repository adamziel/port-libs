...
$row223 = static fn (int $id, string $name, int|string $encoding): array => [
    'setting_id' => $id,
    'key_name_bytes' => $enc223($name, $encoding),
...
$plan223 = static fn (
    ...
    ?array $cursor = null,
): array => SQLiteUtf16NocaseLikeRtrimCurrentSourceNextPlan::keyValueRowKeyDescYieldPagePlan(
    $current ?? $current223,
...
$cases223 = [
    ...
    'expression' => ['expression', 'rtrim(key_name) COLLATE NOCASE LIKE ? ESCAPE ? ORDER BY rtrim(key_name) COLLATE NOCASE DESC, rowid DESC LIMIT ? OFFSET ?'],
    ...
    'current source' => ['currentSource', 'main.app_settings@222'],
    'next source' => ['nextSource', 'main.app_settings@223'],
    ...
    'current token source' => ['currentPageToken.source', 'main.app_settings@222'],
