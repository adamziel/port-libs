...
$makeFirstPage383390 = static function (int $pageCount): string {
...
$putPointerMapEntry383390 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database383390 = static function (int $sliceNumber) use ($makeFirstPage383390, $putPointerMapEntry383390): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage383390(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry383390($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan383390 = static function (int $sliceNumber, int $batchSize = 2) use ($database383390): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database383390($sliceNumber);
...
foreach ([383, 384, 385, 386, 387, 388, 389, 390] as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} keeps next383-390 freelist splice ready"] = static function (TestRunner $t) use ($plan383390, $sliceNumber): void {
        $plan = $plan383390($sliceNumber);
        ...
        $t->same(6, $plan383390($sliceNumber, 3)->freelistSummary()['freelist_row_count']);
    };
}
