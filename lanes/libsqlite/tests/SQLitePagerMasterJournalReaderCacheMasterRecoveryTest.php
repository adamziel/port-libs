...
$pageSize = 512;
$databasePath = '/srv/wp-content/database/wp-master-recovery.sqlite';
$journalPath = $databasePath . '-journal';
$masterPath = '/srv/wp-content/database/wp-master-recovery.sqlite-mj';
$masterBytes = $journalPath . "\n/srv/wp-content/database/site-master-recovery.sqlite-journal\n";
$sourceId = 'master-reader-current-master-recovery';
$page = static fn (string $label): string => str_pad($label, $pageSize, '.', STR_PAD_RIGHT);

$before = [
    1 => $page('master recovery stale schema page before master recovery'),
    2 => $page('master recovery stale wp_options root before master recovery'),
    3 => $page('master recovery stale autoload index before master recovery'),
    4 => $page('master recovery unchanged comments before recovery'),
    5 => $page('master recovery stale plugin settings before master recovery'),
    6 => $page('master recovery stale pinned reader before master recovery'),
    7 => $page('master recovery stale dirty reader before master recovery'),
];
$databaseBytes = implode('', $before);
$recovered = [
    1 => $page('master recovery recovered schema page current source'),
    2 => $page('master recovery recovered wp_options root current source'),
    3 => $page('master recovery recovered autoload index current source'),
    5 => $page('master recovery recovered plugin settings current source'),
    6 => $page('master recovery recovered pinned reader current source'),
    7 => $page('master recovery recovered dirty reader current source'),
];
$cache = [
...
$cases = [
    'canonical status' => [static fn (): mixed => $plan()['canonical_status'], 'pager_master_journal_reader_cache_master_recovery'],
    'legacy status alias' => [static fn (): mixed => $plan()['status'], 'pager_master_journal_reader_cache_current_source_next155'],
    ...
    'master members' => [static fn (): mixed => $plan()['master_members'], [$journalPath, '/srv/wp-content/database/site-master-recovery.sqlite-journal']],
    ...
    'read two prefix recovered' => [static fn (): mixed => $plan()['next_reads'][1]['prefix'], 'master recovery recovered wp_options root current source'],
    'read three miss source' => [static fn (): mixed => $plan()['next_reads'][2]['cache_hit'], false],
    'read four unchanged miss prefix' => [static fn (): mixed => $plan()['next_reads'][3]['prefix'], 'master recovery unchanged comments before recovery'],
    'read five recovered miss prefix' => [static fn (): mixed => $plan()['next_reads'][4]['prefix'], 'master recovery recovered plugin settings current source'],
    'read six recovered miss prefix' => [static fn (): mixed => $plan()['next_reads'][5]['prefix'], 'master recovery recovered pinned reader current source'],
    'read seven recovered miss prefix' => [static fn (): mixed => $plan()['next_reads'][6]['prefix'], 'master recovery recovered dirty reader current source'],
    ...
    'dependency marker' => [static fn (): mixed => in_array('sqlite-pager-master-journal-reader-cache-master-recovery', $plan()['dependencies'], true), true],
    'legacy dependency alias' => [static fn (): mixed => in_array('sqlite-pager-master-journal-reader-cache-current-source-next155', $plan()['dependencies'], true), true],
    ...
    'duplicate master members collapsed' => [static fn (): mixed => $plan(null, null, [1], $masterBytes . $journalPath . "\n")['master_members'], [$journalPath, '/srv/wp-content/database/site-master-recovery.sqlite-journal']],
    'nul terminated master members accepted' => [static fn (): mixed => $plan(null, null, [1], $journalPath . "\0/srv/wp-content/database/site-master-recovery.sqlite-journal\0")['master_members'], [$journalPath, '/srv/wp-content/database/site-master-recovery.sqlite-journal']],
    'mixed newline and nul master members collapsed' => [static fn (): mixed => $plan(null, null, [1], $journalPath . "\0/srv/wp-content/database/site-master-recovery.sqlite-journal\n" . $journalPath . "\0")['master_members'], [$journalPath, '/srv/wp-content/database/site-master-recovery.sqlite-journal']],
...
foreach ($cases as $name => [$callback, $expected]) {
    $tests['pager master journal reader cache master recovery ' . $name] = static function (TestRunner $t) use ($callback, $expected): void {
...
foreach ($throws as $name => $callback) {
    $tests['pager master journal reader cache master recovery ' . $name] = static function (TestRunner $t) use ($callback): void {
