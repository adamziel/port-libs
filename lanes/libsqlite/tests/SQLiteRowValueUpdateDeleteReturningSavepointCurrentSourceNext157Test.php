...
use PortLibs\LibSqlite\SQLiteRowValueUpdateDeleteReturningSavepointCurrentSourceNext157Plan;
...
$plan = static fn (): array => SQLiteRowValueUpdateDeleteReturningSavepointCurrentSourceNext157Plan::execute(
    $tables,
    [$outerStageSql],
...
$cleanPlan = static fn (): array => SQLiteRowValueUpdateDeleteReturningSavepointCurrentSourceNext157Plan::execute(
    $tables,
    [$outerStageSql],
    [$innerUpdateSql],
    [$cleanAfterSql],
...
$cases = [
    ...
    'plan dependency marker next157' => [static fn (): mixed => in_array('sqlite-rowvalue-update-delete-returning-savepoint-current-source-next157', $plan()['dependencies'], true), true],
    ...
    'malformed empty outer rejected' => [static fn (): mixed => SQLiteRowValueUpdateDeleteReturningSavepointCurrentSourceNext157Plan::execute($tables, [], [$innerUpdateSql], [$afterRollbackSql], $unique), InvalidArgumentException::class],
    'malformed empty inner rejected' => [static fn (): mixed => SQLiteRowValueUpdateDeleteReturningSavepointCurrentSourceNext157Plan::execute($tables, [$outerStageSql], [], [$afterRollbackSql], $unique), InvalidArgumentException::class],
    'malformed empty after rejected' => [static fn (): mixed => SQLiteRowValueUpdateDeleteReturningSavepointCurrentSourceNext157Plan::execute($tables, [$outerStageSql], [$innerUpdateSql], [], $unique), InvalidArgumentException::class],
    'malformed empty unique rejected' => [static fn (): mixed => SQLiteRowValueUpdateDeleteReturningSavepointCurrentSourceNext157Plan::execute($tables, [$outerStageSql], [$innerUpdateSql], [$afterRollbackSql], []), InvalidArgumentException::class],
    'malformed same savepoint rejected' => [static fn (): mixed => SQLiteRowValueUpdateDeleteReturningSavepointCurrentSourceNext157Plan::execute($tables, [$outerStageSql], [$innerUpdateSql], [$afterRollbackSql], $unique, 'same_name', 'same_name'), InvalidArgumentException::class],
    'malformed bad savepoint rejected' => [static fn (): mixed => SQLiteRowValueUpdateDeleteReturningSavepointCurrentSourceNext157Plan::execute($tables, [$outerStageSql], [$innerUpdateSql], [$afterRollbackSql], $unique, 'bad-name'), InvalidArgumentException::class],
    'malformed row list rejected' => [static fn (): mixed => SQLiteRowValueUpdateDeleteReturningSavepointCurrentSourceNext157Plan::execute(['wp_options' => ['bad']], [$outerStageSql], [$innerUpdateSql], [$afterRollbackSql], $unique), InvalidArgumentException::class],
];

$tests = [];
foreach ($cases as $name => [$callback, $expected]) {
    $tests['rowvalue update delete returning savepoint current source next157 ' . $name] = static function (TestRunner $t) use ($callback, $expected): void {
