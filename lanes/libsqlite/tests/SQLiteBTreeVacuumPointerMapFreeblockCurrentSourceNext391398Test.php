...
$makeFirstPage391398 = static function (int $pageCount): string {
...
$putPointerMapEntry391398 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database391398 = static function (int $sliceNumber) use ($makeFirstPage391398, $putPointerMapEntry391398): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage391398(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry391398($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan391398 = static function (int $sliceNumber, int $batchSize = 2) use ($database391398): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database391398($sliceNumber);
...
foreach ([391, 392, 393, 394, 395, 396, 397, 398] as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} keeps next391-398 freelist splice ready"] = static function (TestRunner $t) use ($plan391398, $sliceNumber): void {
        $plan = $plan391398($sliceNumber);
        ...
        $t->same(6, $plan391398($sliceNumber, 3)->freelistSummary()['freelist_row_count']);
    };
}
