...
function libsqlite_release_progress48_evidence(): SQLiteUpstreamSuiteEvidence
{
    return SQLiteUpstreamSuiteEvidence::fromManifestPath(__DIR__ . '/../UPSTREAM_TEST_MANIFEST.json');
}

function libsqlite_release_progress48_output(int $assertions = 60, int $failures = 0): string
{
    return "Focused test run: 1 selected test files (root lock skipped)\n"
        . "1 test files, {$assertions} assertions, {$failures} failures\n";
}

function libsqlite_release_progress48_rows(int $case): array
{
    $rows = [
        [
...
$focusedPath48 = 'lanes/libsqlite/tests/SQLiteReleaseRunnerSuiteProgressMapCurrentNext48Test.php';
...
for ($i = 1; $i <= 44; $i++) {
    $tests['current next48 suite progress maps focused case ' . $i] = static function (TestRunner $t) use ($i, $currentHead48, $nextHead48, $focusedPath48, $nonOverlap48): void {
        $record = libsqlite_release_progress48_evidence()->releaseRunnerSuiteProgressMapCurrentNext48(
            libsqlite_release_progress48_rows($i),
            $currentHead48,
            $nextHead48,
            17373,
            $focusedPath48,
            libsqlite_release_progress48_output(60),
            $nonOverlap48
        );
...
$tests['current next48 suite progress reports open release blockers'] = static function (TestRunner $t) use ($currentHead48, $nextHead48, $focusedPath48, $nonOverlap48): void {
    $record = libsqlite_release_progress48_evidence()->releaseRunnerSuiteProgressMapCurrentNext48(
        libsqlite_release_progress48_rows(1),
        $currentHead48,
        $nextHead48,
        17373,
        $focusedPath48,
...
$tests['current next48 suite progress blocks regressed current rows'] = static function (TestRunner $t) use ($currentHead48, $nextHead48, $focusedPath48, $nonOverlap48): void {
    $record = libsqlite_release_progress48_evidence()->releaseRunnerSuiteProgressMapCurrentNext48(
        libsqlite_release_progress48_rows(11),
        $currentHead48,
        $nextHead48,
        17373,
        $focusedPath48,
...
$tests['current next48 suite progress rejects missing heads'] = static function (TestRunner $t) use ($focusedPath48, $nonOverlap48): void {
    $t->throws(
        ...
        static fn () => libsqlite_release_progress48_evidence()->releaseRunnerSuiteProgressMapCurrentNext48(
...
$tests['current next48 suite progress aggregates tiers deterministically'] = static function (TestRunner $t) use ($currentHead48, $nextHead48, $focusedPath48, $nonOverlap48): void {
    $record = libsqlite_release_progress48_evidence()->releaseRunnerSuiteProgressMapCurrentNext48(
        libsqlite_release_progress48_rows(5),
        $currentHead48,
        $nextHead48,
        17373,
        $focusedPath48,
