...
use PortLibs\LibSqlite\SQLiteVfsCurrentSourceNext498513Plan;
...
$plan = static function () use ($readyCurrent): array {
    static $result = null;
    if ($result === null) {
        $result = SQLiteVfsCurrentSourceNext498513Plan::run([
...
return [
    ...
    'vfs current source next498-513 blocks dirty snapshot' => static fn (TestRunner $t) => $t->same(true, in_array('dirty-pages-present', SQLiteVfsCurrentSourceNext498513Plan::run(['snapshot(reader-ready-next513,shared-cache-next497)'], ['current' => $dirtyCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source next498-513 blocks old snapshot ack' => static fn (TestRunner $t) => $t->same(true, in_array('ack-not-latest-publish', SQLiteVfsCurrentSourceNext498513Plan::run(['snapshot(reader-ready-next513,shared-cache-next497)'], ['current' => $oldAckCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source next498-513 blocks publish without claim' => static fn (TestRunner $t) => $t->same(true, in_array('missing-reuse-claim', SQLiteVfsCurrentSourceNext498513Plan::run(['snapshot(reader-ready-next513,shared-cache-next497)', 'publish(reader-ready-next513,reader-reuse-next513,shared-cache-next513)'], ['current' => $readyCurrent])['events'][1]['blocked_reasons'], true)),
    'vfs current source next498-513 blocks stale claim' => static fn (TestRunner $t) => $t->same(true, in_array('stale-reuse-claim', SQLiteVfsCurrentSourceNext498513Plan::run(['publish(reader-ready-next513,reader-reuse-next513,shared-cache-next513)'], ['current' => $staleClaimCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source next498-513 accepts preclaimed publish' => static fn (TestRunner $t) => $t->same('published-reused-current-source', SQLiteVfsCurrentSourceNext498513Plan::run(['publish(reader-ready-next513,reader-reuse-next513,shared-cache-next513)'], ['current' => $claimedCurrent])['events'][0]['status']),
    'vfs current source next498-513 rejects empty operations' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteVfsCurrentSourceNext498513Plan::run([])),
    'vfs current source next498-513 rejects bad claim token' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteVfsCurrentSourceNext498513Plan::run([['op' => 'claim', 'snapshot' => 'reader-ready-next513', 'ack' => 'shared-cache-next497', 'claim' => 'bad claim']], ['current' => $readyCurrent])),
    'vfs current source next498-513 rejects unsupported operation' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteVfsCurrentSourceNext498513Plan::run(['republish(reader-ready-next513,shared-cache-next497)'], ['current' => $readyCurrent])),
