...
$tclScriptFile = tempnam(sys_get_temp_dir(), 'libsqlite-atof1-random-4801-tcl-');
...
foreach (explode("\n", trim($tclOutput)) as $line) {
    ...
    $cases[$key] = [
        ...
        'ordinal' => $ordinal,
        'power' => (int) $powerText,
        'bound' => sqliteRealUpstreamAtof1RandomRoundtrip4801To6000NumericLiteral($boundText),
        'literal' => sqliteRealUpstreamAtof1RandomRoundtrip4801To6000NumericLiteral($literalText),
    ];
}
...
$oracleFile = tempnam(sys_get_temp_dir(), 'libsqlite-atof1-random-4801-oracle-');
if ($oracleFile === false) {
...
$tests['real upstream corpus date affinity dynamic atof1 random roundtrip 4801 6000 source truth'] =
    static function (TestRunner $t) use ($cases, $oracle, $ownedStart, $ownedEnd, $ownedCount, $sourceFile, $sourceText): void {
        ...
        $t->same(4801, $ownedStart);
        $t->same(6000, $ownedEnd);
        $t->same(1200, $ownedCount);
        $t->same(1200, count($cases));
        $t->same(1200, count($oracle));
        $t->same('atof1.test atof1-1.4801.1/.2', $cases['atof1-1.4801']['source']);
        $t->same('1.02524422398427114688779226980841e-01', $cases['atof1-1.4801']['literal']);
        $t->same('3', (string) $cases['atof1-1.4801']['power']);
        $t->same('atof1.test atof1-1.6000.1/.2', $cases['atof1-1.6000']['source']);
        $t->same('9.26396091974113970833517768795942e-26', $cases['atof1-1.6000']['literal']);
    };

foreach ($cases as $key => $case) {
    $tests['real upstream corpus date affinity dynamic atof1 random roundtrip 4801 6000 ' . $key] =
        static function (TestRunner $t) use ($case, $key, $oracle): void {
            ...
            $t->same(
                sqliteRealUpstreamAtof1RandomRoundtrip4801To6000DoubleHex($boundReal),
                sqliteRealUpstreamAtof1RandomRoundtrip4801To6000DoubleHex($literalReal),
                $key . ' literal text cast preserves Tcl bound double bits'
            );
            $t->same(
                sqliteRealUpstreamAtof1RandomRoundtrip4801To6000DoubleHex($boundReal),
                sqliteRealUpstreamAtof1RandomRoundtrip4801To6000DoubleHex($boundRoundTrip),
                $key . ' native quote CAST round-trip preserves bound REAL bits'
            );
            $t->same(
                sqliteRealUpstreamAtof1RandomRoundtrip4801To6000DoubleHex($literalReal),
                sqliteRealUpstreamAtof1RandomRoundtrip4801To6000DoubleHex($literalRoundTrip),
...
$tests['real upstream corpus date affinity dynamic atof1 random roundtrip 4801 6000 generic metric rollup'] =
    static function (TestRunner $t) use ($cases): void {
        $rows = [
            ['key_name' => 'metric.random.4801', 'literal_text' => $cases['atof1-1.4801']['literal']],
            ['key_name' => 'metric.random.5200', 'literal_text' => $cases['atof1-1.5200']['literal']],
            ['key_name' => 'metric.random.5600', 'literal_text' => $cases['atof1-1.5600']['literal']],
            ['key_name' => 'metric.random.6000', 'literal_text' => $cases['atof1-1.6000']['literal']],
        ];
        $result = SQLiteSelectSql::execute(
            'SELECT key_name, '
            . 'typeof(CAST(literal_text AS REAL)) AS storage_class, '
        ...
        $t->same(['metric.random.4801', 'metric.random.5200', 'metric.random.5600', 'metric.random.6000'], array_column($result, 'key_name'));
...
$tests['real upstream corpus date affinity dynamic atof1 random roundtrip 4801 6000 non overlap and dependency closure'] =
    static function (TestRunner $t): void {
        $t->same(
            'owns atof1.test atof1-1.4801..6000 text-to-REAL and quote round-trip rows only',
            'owns atof1.test atof1-1.4801..6000 text-to-REAL and quote round-trip rows only',
        );
        $t->same(
            'non-overlap: extends the accepted atof1-1.1..4800 random REAL shards and avoids accepted atof1-2 UTF16/blob rows, atof-3.1 integer-prefix suffixes, atof-3.2 decimal suffixes, atof-3.3 exponent rows, atof2 rounding, date4 rows, timediff matrices, affinity2, affinity3, and types storage-class batches',
            'non-overlap: extends the accepted atof1-1.1..4800 random REAL shards and avoids accepted atof1-2 UTF16/blob rows, atof-3.1 integer-prefix suffixes, atof-3.2 decimal suffixes, atof-3.3 exponent rows, atof2 rounding, date4 rows, timediff matrices, affinity2, affinity3, and types storage-class batches',
        );
        $t->same(
...
function sqliteRealUpstreamAtof1RandomRoundtrip4801To6000NumericLiteral(string $value): string
{
...
function sqliteRealUpstreamAtof1RandomRoundtrip4801To6000DoubleHex(float $value): string
{
