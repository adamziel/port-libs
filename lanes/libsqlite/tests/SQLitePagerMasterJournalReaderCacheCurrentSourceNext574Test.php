...
$pageSize = 512;
$database = '/srv/wp-content/database/wp-next574.sqlite';
...
$formatPage = static function (string $label) use ($pageSize): string {
    ...
    $page = substr_replace($page, pack('N', 574), 60, 4);
...
$tokenFields = [
    ...
    'reader_cache_stmt_vdbe_if_branch_token',
];
$before = [1 => $formatPage('stale schema'), 2 => $page('stale options')];
$recovered = [1 => $formatPage('current schema'), 2 => $page('current options')];
$tokens = [$journal => 'member-main-current-574'];
$headers = [$journal => hash('sha256', 'main header next574')];
$base = [
    'source_id' => 'pager-reader-cache-current-source-next574',
    'epoch' => 574,
    'format_signature' => hash('sha256', implode('|', [512, 4, 2, 574, 0])),
    'publication_generation' => 574,
    'master_source_digest' => hash('sha256', 'master-next574'),
    'recovery_sequence' => 574,
...
foreach ($tokenFields as $field) {
    $base[$field] = str_replace('_', '-', preg_replace('/_token$/', '', $field)) . '-current-574';
}
...
$tests['pager master journal reader cache current source next574 admits current VDBE opcode literal and arithmetic and branch fences'] = static function (TestRunner $t) use ($plan): void {
    $result = $plan();
    $t->same('pager-master-journal-reader-cache-current-source-next574', $result['status']);
...
$tests['pager master journal reader cache current source next574 invalidates stale if_branch cache'] = static function (TestRunner $t) use ($plan, $opCount): void {
    $result = $plan(['reader_cache_stmt_vdbe_if_branch_token' => 'stmt-vdbe-if-branch-old']);
    $t->same([1], $result['reader_cache_stmt_vdbe_if_branch_invalidated_cache_page_numbers']);
    $t->same([1], $result['invalidated_cache_page_numbers']);
    $t->same(1, $opCount($result, 'invalidate_reader_cache_reader_cache_stmt_vdbe_if_branch_current_source_next574'));
};

$tests['pager master journal reader cache current source next574 reopens stale if_branch read ticket'] = static function (TestRunner $t) use ($plan, $opCount): void {
    $result = $plan([], ['reader_cache_stmt_vdbe_if_branch_token' => 'stmt-vdbe-if-branch-old']);
    $t->same(['read-options'], $result['reopen_reader_ids']);
    $t->same(false, $result['next_reads'][0]['cache_hit']);
    $t->same('reader_ticket_reader_cache_stmt_vdbe_if_branch_predates_current_source', $result['next_reads'][0]['reader_cache_stmt_vdbe_if_branch_token_reason']);
    $t->same(1, $opCount($result, 'reopen_reader_for_reader_cache_stmt_vdbe_if_branch_current_source_next574'));
};

$tests['pager master journal reader cache current source next574 missing if_branch token rejects'] = static function (TestRunner $t) use ($plan): void {
    $t->throws(Throwable::class, static fn () => $plan(['reader_cache_stmt_vdbe_if_branch_token' => null]));
};
