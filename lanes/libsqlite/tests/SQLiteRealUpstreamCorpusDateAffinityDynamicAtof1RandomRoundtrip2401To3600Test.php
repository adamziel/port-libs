...
foreach (explode("\n", trim($tclOutput)) as $line) {
    ...
    $cases[$key] = [
        ...
        'ordinal' => $ordinal,
        'power' => (int) $powerText,
        'bound' => sqliteRealUpstreamAtof1RandomRoundtrip2401To3600NumericLiteral($boundText),
        'literal' => sqliteRealUpstreamAtof1RandomRoundtrip2401To3600NumericLiteral($literalText),
    ];
}
...
$tests['real upstream corpus date affinity dynamic atof1 random roundtrip 2401 3600 source truth'] =
    static function (TestRunner $t) use ($cases, $oracle, $ownedStart, $ownedEnd, $ownedCount, $sourceFile, $sourceText): void {
        ...
        $t->same(2401, $ownedStart);
        $t->same(3600, $ownedEnd);
        $t->same(1200, $ownedCount);
        $t->same(1200, count($cases));
        $t->same(1200, count($oracle));
        $t->same('atof1.test atof1-1.2401.1/.2', $cases['atof1-1.2401']['source']);
        $t->same('7.35284514822255665201047349077701e-07', $cases['atof1-1.2401']['literal']);
        $t->same('8', (string) $cases['atof1-1.2401']['power']);
        $t->same('atof1.test atof1-1.3600.1/.2', $cases['atof1-1.3600']['source']);
        $t->same('1.36854594175903343966638203710318e+03', $cases['atof1-1.3600']['literal']);
    };

foreach ($cases as $key => $case) {
    $tests['real upstream corpus date affinity dynamic atof1 random roundtrip 2401 3600 ' . $key] =
        static function (TestRunner $t) use ($case, $key, $oracle): void {
            ...
            $t->same(
                sqliteRealUpstreamAtof1RandomRoundtrip2401To3600DoubleHex($boundReal),
                sqliteRealUpstreamAtof1RandomRoundtrip2401To3600DoubleHex($literalReal),
                $key . ' literal text cast preserves Tcl bound double bits'
            );
            $t->same(
                sqliteRealUpstreamAtof1RandomRoundtrip2401To3600DoubleHex($boundReal),
                sqliteRealUpstreamAtof1RandomRoundtrip2401To3600DoubleHex($boundRoundTrip),
                $key . ' native quote CAST round-trip preserves bound REAL bits'
            );
            $t->same(
                sqliteRealUpstreamAtof1RandomRoundtrip2401To3600DoubleHex($literalReal),
                sqliteRealUpstreamAtof1RandomRoundtrip2401To3600DoubleHex($literalRoundTrip),
...
$tests['real upstream corpus date affinity dynamic atof1 random roundtrip 2401 3600 generic metric rollup'] =
    static function (TestRunner $t) use ($cases): void {
        $rows = [
            ['key_name' => 'metric.random.2401', 'literal_text' => $cases['atof1-1.2401']['literal']],
            ['key_name' => 'metric.random.2800', 'literal_text' => $cases['atof1-1.2800']['literal']],
            ['key_name' => 'metric.random.3200', 'literal_text' => $cases['atof1-1.3200']['literal']],
            ['key_name' => 'metric.random.3600', 'literal_text' => $cases['atof1-1.3600']['literal']],
        ];
        $result = SQLiteSelectSql::execute(
            'SELECT key_name, '
            . 'typeof(CAST(literal_text AS REAL)) AS storage_class, '
        ...
        $t->same(['metric.random.2401', 'metric.random.2800', 'metric.random.3200', 'metric.random.3600'], array_column($result, 'key_name'));
...
$tests['real upstream corpus date affinity dynamic atof1 random roundtrip 2401 3600 non overlap and dependency closure'] =
    static function (TestRunner $t): void {
        $t->same(
            'owns atof1.test atof1-1.2401..3600 text-to-REAL and quote round-trip rows only',
            'owns atof1.test atof1-1.2401..3600 text-to-REAL and quote round-trip rows only',
        );
        $t->same(
            'non-overlap: extends the accepted atof1-1.1..2400 random REAL shards and avoids accepted atof1-2 UTF16/blob rows, atof-3.1 integer-prefix suffixes, atof-3.2 decimal suffixes, atof-3.3 exponent rows, atof2 rounding, date4 rows, timediff matrices, affinity2, affinity3, and types storage-class batches',
            'non-overlap: extends the accepted atof1-1.1..2400 random REAL shards and avoids accepted atof1-2 UTF16/blob rows, atof-3.1 integer-prefix suffixes, atof-3.2 decimal suffixes, atof-3.3 exponent rows, atof2 rounding, date4 rows, timediff matrices, affinity2, affinity3, and types storage-class batches',
        );
        $t->same(
...
function sqliteRealUpstreamAtof1RandomRoundtrip2401To3600NumericLiteral(string $value): string
{
...
function sqliteRealUpstreamAtof1RandomRoundtrip2401To3600DoubleHex(float $value): string
{
