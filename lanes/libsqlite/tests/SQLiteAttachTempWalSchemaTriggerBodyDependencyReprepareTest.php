...
foreach ($pathCases104 as $name => [$path, $expected]) {
    $tests['attach temp wal schema trigger body dependency reprepare ' . $name] = static function (TestRunner $t) use ($plan104, $value104, $path, $expected): void {
...
foreach ($predicateCases104 as $name => $predicate) {
    $tests['attach temp wal schema trigger body dependency reprepare ' . $name] = static function (TestRunner $t) use ($predicate): void {
...
foreach ($errorCases104 as $name => $callback) {
    $tests['attach temp wal schema trigger body dependency reprepare ' . $name] = static function (TestRunner $t) use ($callback): void {
