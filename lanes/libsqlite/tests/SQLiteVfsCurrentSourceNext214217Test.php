...
return [
    ...
    'vfs current source next214-217 blocks dirty publication' => static fn (TestRunner $t) => $t->same(['dirty-pages-present'], SQLiteVfsCurrentSourceNextPlan::run(['publish-snapshot(reader-cache,ticket-next214)'], ['current' => $dirtyCurrent])['events'][0]['blocked_reasons']),
    'vfs current source next214-217 blocks stale publication' => static fn (TestRunner $t) => $t->same(true, in_array('data-version-changed', SQLiteVfsCurrentSourceNextPlan::run(['publish-snapshot(reader-cache,ticket-next214)'], ['current' => $staleCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source next214-217 blocks missing publication reuse' => static fn (TestRunner $t) => $t->same(['missing-publication'], SQLiteVfsCurrentSourceNextPlan::run(['reuse-publication(ticket-missing,reader-a)'], ['current' => $current])['events'][0]['blocked_reasons']),
    'vfs current source next214-217 blocks revoked publication reuse' => static fn (TestRunner $t) => $t->same(['publication-revoked'], SQLiteVfsCurrentSourceNextPlan::run(['revoke-publication(ticket-next214)', 'reuse-publication(ticket-next214,reader-a)'], ['current' => $publishedCurrent])['events'][1]['blocked_reasons']),
    'vfs current source next214-217 rejects empty operations' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteVfsCurrentSourceNextPlan::run([])),
    'vfs current source next214-217 rejects bad ticket token' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteVfsCurrentSourceNextPlan::run([['op' => 'publish_snapshot', 'snapshot' => 'reader-cache', 'ticket' => 'bad token']], ['current' => $current])),
    'vfs current source next214-217 rejects unsupported operation' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteVfsCurrentSourceNextPlan::run(['snapshot(reader-cache)'], ['current' => $current])),
