...
$plan252 = static fn (?array $ack = null, ?string $resume = null): array => SQLiteRowValueUpdateDeleteReturningWindowCurrentSourceNextPlan::executePublicationWindowFence(
    $tables252,
...
$cases252 = [
    ...
    'bad savepoint rejected by base' => [static fn (): mixed => SQLiteRowValueUpdateDeleteReturningWindowCurrentSourceNextPlan::executePublicationWindowFence($tables252, [$yieldUpdate252], [$attemptUpdate252], [$retryUpdate252], $unique252, 'bad-name'), InvalidArgumentException::class],
    'bad rowid rejected' => [static fn (): mixed => SQLiteRowValueUpdateDeleteReturningWindowCurrentSourceNextPlan::executePublicationWindowFence(['wp_options' => [['option_id' => ['bad'], 'blog_id' => 1, 'option_name' => 'home', 'autoload' => 'yes', 'status' => 'live', 'bytes' => 1, 'option_value' => 'x']]], ["UPDATE wp_options SET (status, option_value, bytes) = ('yield252', option_value, bytes) WHERE (blog_id, option_name) IN ((1, 'home')) RETURNING option_id, blog_id, option_name, status, bytes"], [$attemptUpdate252], [$retryUpdate252], $unique252, 'app_settings_rowvalue_window_current_next252', 'option_id'), InvalidArgumentException::class],
];
