...
use PortLibs\LibSqlite\SQLiteVfsCurrentSourceNext234237Plan;
...
$plan = static function () use ($readyCurrent): array {
    static $result = null;
    if ($result === null) {
        $result = SQLiteVfsCurrentSourceNext234237Plan::run([
...
return [
    ...
    'vfs current source next234-237 blocks dirty ack' => static fn (TestRunner $t) => $t->same(true, in_array('dirty-pages-present', SQLiteVfsCurrentSourceNext234237Plan::run(['ack(reader-ready,shared-cache-next229)'], ['current' => $dirtyCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source next234-237 blocks missing receipt ack' => static fn (TestRunner $t) => $t->same(true, in_array('missing-publish-receipt', SQLiteVfsCurrentSourceNext234237Plan::run(['ack(reader-ready,shared-cache-next229)'], ['current' => $missingReceiptCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source next234-237 blocks republish without ack' => static fn (TestRunner $t) => $t->same(true, in_array('missing-reuse-ack', SQLiteVfsCurrentSourceNext234237Plan::run(['republish(reader-ready,shared-cache-next237)'], ['current' => $readyCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source next234-237 blocks stale ack' => static fn (TestRunner $t) => $t->same(true, in_array('stale-reuse-ack', SQLiteVfsCurrentSourceNext234237Plan::run(['republish(reader-ready,shared-cache-next237)'], ['current' => $staleAckCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source next234-237 rejects empty operations' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteVfsCurrentSourceNext234237Plan::run([])),
    'vfs current source next234-237 rejects bad ack receipt' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteVfsCurrentSourceNext234237Plan::run([['op' => 'ack', 'snapshot' => 'reader-ready', 'receipt' => 'bad receipt']], ['current' => $readyCurrent])),
    'vfs current source next234-237 rejects unsupported operation' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteVfsCurrentSourceNext234237Plan::run(['publish(reader-ready,shared-cache-next237)'], ['current' => $readyCurrent])),
