...
use PortLibs\LibSqlite\SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext206Plan;
...
$currentTables206 = ['wp_options' => $currentOptions206];
$nextTables206 = ['wp_options' => $nextOptions206];
...
$summary206 = static fn (?array $cursor = null): array => SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext206Plan::compare($sql206, $currentTables206, $nextTables206, $cursor);
$tests = [];

$tests['compound select window recursive limit current source next206 status dependencies'] = static function (TestRunner $t) use ($summary206): void {
    $plan = $summary206();
    $t->same('compound-select-window-recursive-limit-current-source-next206-ready', $plan['status']);
    $t->same([
...
$tests['compound select window recursive limit current source next206 compound metadata'] = static function (TestRunner $t) use ($summary206): void {
    $compound = $summary206()['compound'];
...
$tests['compound select window recursive limit current source next206 current row boundary'] = static function (TestRunner $t) use ($summary206): void {
    $rows = $summary206()['currentRows'];
...
$tests['compound select window recursive limit current source next206 next row boundary'] = static function (TestRunner $t) use ($summary206): void {
    $rows = $summary206()['nextRows'];
...
$tests['compound select window recursive limit current source next206 recursive queue trace'] = static function (TestRunner $t) use ($summary206): void {
    $queue = $summary206()['recursiveQueue'];
    $t->same('q', $queue['name']);
...
$tests['compound select window recursive limit current source next206 window shape'] = static function (TestRunner $t) use ($summary206): void {
    $windows = $summary206()['windows'];
...
$tests['compound select window recursive limit current source next206 token fence'] = static function (TestRunner $t) use ($summary206): void {
    $first = $summary206();
    $second = $summary206($first['cursor']);
...
$tests['compound select window recursive limit current source next206 rejects stale cursor'] = static function (TestRunner $t) use ($summary206): void {
    $cursor = $summary206()['cursor'];
    $cursor['currentToken'] = str_repeat('0', 64);
    $t->throws(InvalidArgumentException::class, static fn () => $summary206($cursor));
};

$tests['compound select window recursive limit current source next206 limit trace'] = static function (TestRunner $t) use ($summary206): void {
    $trace = $summary206()['limitTrace'];
...
$tests['compound select window recursive limit current source next206 replan reasons'] = static function (TestRunner $t) use ($summary206): void {
    $plan = $summary206();
    $t->contains('avoids accepted next203', $plan['non_overlap']);
    $t->true(in_array('compound-lead-nth-value-current-source-next206', $plan['replanReasons'], true));
    $t->true(in_array('recursive-ordered-limit-before-intersect-windows-next206', $plan['replanReasons'], true));
    $t->true(in_array('intersect-after-window-output-next206', $plan['replanReasons'], true));
};

$tests['compound select window recursive limit current source next206 rejects missing nth value'] = static function (TestRunner $t) use ($currentTables206): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext206Plan::compare(
...
$tests['compound select window recursive limit current source next206 rejects unordered recursive limit'] = static function (TestRunner $t) use ($currentTables206): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext206Plan::compare(
...
foreach (range(1, 58) as $case) {
    $tests['compound select window recursive limit current source next206 generated lead intersect boundary ' . $case] = static function (TestRunner $t) use ($case): void {
        $finalLimit = 4 + ($case % 3);
        $tables = [
            'wp_options' => [
        ...
        $plan = SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext206Plan::compare($sql, $tables, $nextTables);
        $again = SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext206Plan::compare($sql, $tables, $nextTables, $plan['cursor']);
