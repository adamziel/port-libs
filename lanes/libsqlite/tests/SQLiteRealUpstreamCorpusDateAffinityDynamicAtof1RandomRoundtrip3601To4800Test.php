...
foreach (explode("\n", trim($tclOutput)) as $line) {
    ...
    $cases[$key] = [
        ...
        'ordinal' => $ordinal,
        'power' => (int) $powerText,
        'bound' => sqliteRealUpstreamAtof1RandomRoundtrip3601To4800NumericLiteral($boundText),
        'literal' => sqliteRealUpstreamAtof1RandomRoundtrip3601To4800NumericLiteral($literalText),
    ];
}
...
$tests['real upstream corpus date affinity dynamic atof1 random roundtrip 3601 4800 source truth'] =
    static function (TestRunner $t) use ($cases, $oracle, $ownedStart, $ownedEnd, $ownedCount, $sourceFile, $sourceText): void {
        ...
        $t->same(3601, $ownedStart);
        $t->same(4800, $ownedEnd);
        $t->same(1200, $ownedCount);
        $t->same(1200, count($cases));
        $t->same(1200, count($oracle));
        $t->same('atof1.test atof1-1.3601.1/.2', $cases['atof1-1.3601']['source']);
        $t->same('1.00000000000000000000000000000000e+00', $cases['atof1-1.3601']['literal']);
        $t->same('0', (string) $cases['atof1-1.3601']['power']);
        $t->same('atof1.test atof1-1.4800.1/.2', $cases['atof1-1.4800']['source']);
        $t->same('6.97813192888957800000000000000000e+15', $cases['atof1-1.4800']['literal']);
    };

foreach ($cases as $key => $case) {
    $tests['real upstream corpus date affinity dynamic atof1 random roundtrip 3601 4800 ' . $key] =
        static function (TestRunner $t) use ($case, $key, $oracle): void {
            ...
            $t->same(
                sqliteRealUpstreamAtof1RandomRoundtrip3601To4800DoubleHex($boundReal),
                sqliteRealUpstreamAtof1RandomRoundtrip3601To4800DoubleHex($literalReal),
                $key . ' literal text cast preserves Tcl bound double bits'
            );
            $t->same(
                sqliteRealUpstreamAtof1RandomRoundtrip3601To4800DoubleHex($boundReal),
                sqliteRealUpstreamAtof1RandomRoundtrip3601To4800DoubleHex($boundRoundTrip),
                $key . ' native quote CAST round-trip preserves bound REAL bits'
            );
            $t->same(
                sqliteRealUpstreamAtof1RandomRoundtrip3601To4800DoubleHex($literalReal),
                sqliteRealUpstreamAtof1RandomRoundtrip3601To4800DoubleHex($literalRoundTrip),
...
$tests['real upstream corpus date affinity dynamic atof1 random roundtrip 3601 4800 generic metric rollup'] =
    static function (TestRunner $t) use ($cases): void {
        $rows = [
            ['key_name' => 'metric.random.3601', 'literal_text' => $cases['atof1-1.3601']['literal']],
            ['key_name' => 'metric.random.4000', 'literal_text' => $cases['atof1-1.4000']['literal']],
            ['key_name' => 'metric.random.4400', 'literal_text' => $cases['atof1-1.4400']['literal']],
            ['key_name' => 'metric.random.4800', 'literal_text' => $cases['atof1-1.4800']['literal']],
        ];
        $result = SQLiteSelectSql::execute(
            'SELECT key_name, '
            . 'typeof(CAST(literal_text AS REAL)) AS storage_class, '
        ...
        $t->same(['metric.random.3601', 'metric.random.4000', 'metric.random.4400', 'metric.random.4800'], array_column($result, 'key_name'));
...
$tests['real upstream corpus date affinity dynamic atof1 random roundtrip 3601 4800 non overlap and dependency closure'] =
    static function (TestRunner $t): void {
        $t->same(
            'owns atof1.test atof1-1.3601..4800 text-to-REAL and quote round-trip rows only',
            'owns atof1.test atof1-1.3601..4800 text-to-REAL and quote round-trip rows only',
        );
        $t->same(
            'non-overlap: extends the accepted atof1-1.1..3600 random REAL shards and avoids accepted atof1-2 UTF16/blob rows, atof-3.1 integer-prefix suffixes, atof-3.2 decimal suffixes, atof-3.3 exponent rows, atof2 rounding, date4 rows, timediff matrices, affinity2, affinity3, and types storage-class batches',
            'non-overlap: extends the accepted atof1-1.1..3600 random REAL shards and avoids accepted atof1-2 UTF16/blob rows, atof-3.1 integer-prefix suffixes, atof-3.2 decimal suffixes, atof-3.3 exponent rows, atof2 rounding, date4 rows, timediff matrices, affinity2, affinity3, and types storage-class batches',
        );
        $t->same(
...
function sqliteRealUpstreamAtof1RandomRoundtrip3601To4800NumericLiteral(string $value): string
{
...
function sqliteRealUpstreamAtof1RandomRoundtrip3601To4800DoubleHex(float $value): string
{
