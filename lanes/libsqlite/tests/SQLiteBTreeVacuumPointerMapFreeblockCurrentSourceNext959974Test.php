...
$makeFirstPage959974 = static function (int $pageCount): string {
...
$putPointerMapEntry959974 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database959974 = static function (int $sliceNumber) use ($makeFirstPage959974, $putPointerMapEntry959974): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage959974(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry959974($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan959974 = static function (int $sliceNumber, int $batchSize = 2) use ($database959974): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database959974($sliceNumber);
...
foreach (range(959, 974) as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} extends next959-974 handoff receipts"] = static function (TestRunner $t) use ($plan959974, $sliceNumber): void {
        $plan = $plan959974($sliceNumber);
        ...
        $t->same(6, $plan959974($sliceNumber, 3)->currentSourceSummary()['current_source_row_count']);
    };
}
