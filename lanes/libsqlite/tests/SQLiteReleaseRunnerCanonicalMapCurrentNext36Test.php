...
function libsqlite_release_canonical36_evidence(): SQLiteUpstreamSuiteEvidence
{
    return SQLiteUpstreamSuiteEvidence::fromManifestPath(__DIR__ . '/../UPSTREAM_TEST_MANIFEST.json');
}

function libsqlite_release_canonical36_artifact(
    string $head,
    string $testset,
...
$tests = [
    'current next36 canonical map preserves current release baseline' => static function (TestRunner $t) use ($currentHead36, $nextHead36): void {
        $map = libsqlite_release_canonical36_evidence()->releaseRunnerUpstreamCanonicalMapCurrentNext36([
            'current-release' => libsqlite_release_canonical36_artifact($currentHead36, 'release', [], 12000),
    ...
    'current next36 canonical map suppresses launch when next release exists' => static function (TestRunner $t) use ($currentHead36, $nextHead36): void {
        $map = libsqlite_release_canonical36_evidence()->releaseRunnerUpstreamCanonicalMapCurrentNext36([
            'current-release' => libsqlite_release_canonical36_artifact($currentHead36, 'release', [], 12000),
            'next-release' => libsqlite_release_canonical36_artifact($nextHead36, 'release', [], 12100),
        ], $currentHead36, $nextHead36);
    ...
    'current next36 canonical map blocks stale release artifacts' => static function (TestRunner $t) use ($currentHead36, $nextHead36): void {
        $map = libsqlite_release_canonical36_evidence()->releaseRunnerUpstreamCanonicalMapCurrentNext36([
            'current-release' => libsqlite_release_canonical36_artifact($currentHead36, 'release', [], 12000),
            'stale-release' => libsqlite_release_canonical36_artifact('stale-head', 'release', [], 13000),
        ], $currentHead36, $nextHead36);
    ...
    'current next36 canonical map normalizes focused pattern order' => static function (TestRunner $t) use ($currentHead36, $nextHead36): void {
        $map = libsqlite_release_canonical36_evidence()->releaseRunnerUpstreamCanonicalMapCurrentNext36([
            'focused-a' => libsqlite_release_canonical36_artifact($currentHead36, 'veryquick', ['wal.test', 'pager.test'], 900),
            'focused-b' => libsqlite_release_canonical36_artifact($currentHead36, 'veryquick', ['pager.test', 'wal.test'], 901),
        ], $currentHead36, $nextHead36);
    ...
    'current next36 canonical map rejects missing head values' => static function (TestRunner $t) use ($currentHead36, $nextHead36): void {
        $t->throws(
            ...
            static fn () => libsqlite_release_canonical36_evidence()->releaseRunnerUpstreamCanonicalMapCurrentNext36([], '', $nextHead36)
        );
        $t->throws(
            InvalidArgumentException::class,
            static fn () => libsqlite_release_canonical36_evidence()->releaseRunnerUpstreamCanonicalMapCurrentNext36([], $currentHead36, '')
        );
    },
    'current next36 canonical map records failed artifact blockers' => static function (TestRunner $t) use ($currentHead36, $nextHead36): void {
        $map = libsqlite_release_canonical36_evidence()->releaseRunnerUpstreamCanonicalMapCurrentNext36([
            'current-release' => libsqlite_release_canonical36_artifact($currentHead36, 'release', [], 12000),
            'failed-release' => libsqlite_release_canonical36_artifact($currentHead36, 'all', [], 400, 1, 1),
        ], $currentHead36, $nextHead36);
    ...
    'current next36 canonical map records manifest mismatch blockers' => static function (TestRunner $t) use ($currentHead36, $nextHead36): void {
        $map = libsqlite_release_canonical36_evidence()->releaseRunnerUpstreamCanonicalMapCurrentNext36([
            'current-release' => libsqlite_release_canonical36_artifact($currentHead36, 'release', [], 12000),
            'wrong-manifest' => libsqlite_release_canonical36_artifact($currentHead36, 'release', [], 401, 0, 0, 'wrong-manifest'),
        ], $currentHead36, $nextHead36);
    ...
    'current next36 canonical map keeps focused evidence out of release-like count' => static function (TestRunner $t) use ($currentHead36, $nextHead36): void {
        $map = libsqlite_release_canonical36_evidence()->releaseRunnerUpstreamCanonicalMapCurrentNext36([
            'focused-current' => libsqlite_release_canonical36_artifact($currentHead36, 'release', ['json101.test'], 222),
        ], $currentHead36, $nextHead36);
    ...
    'current next36 canonical map normalizes comma pattern strings' => static function (TestRunner $t) use ($currentHead36, $nextHead36): void {
        $map = libsqlite_release_canonical36_evidence()->releaseRunnerUpstreamCanonicalMapCurrentNext36([
            'focused-current' => libsqlite_release_canonical36_artifact($currentHead36, 'veryquick', 'json102.test, json101.test', 333),
        ], $currentHead36, $nextHead36);
    ...
    'current next36 canonical map blocks empty artifact sets' => static function (TestRunner $t) use ($currentHead36, $nextHead36): void {
        $map = libsqlite_release_canonical36_evidence()->releaseRunnerUpstreamCanonicalMapCurrentNext36([], $currentHead36, $nextHead36);
...
for ($i = 1; $i <= 46; $i++) {
    $tests['current next36 canonical release sample ' . $i] = static function (TestRunner $t) use ($currentHead36, $nextHead36, $i): void {
        ...
        $records = [
            'current-' . $i => libsqlite_release_canonical36_artifact($currentHead36, $testset, [], 1000 + $i),
        ];

        if ($i % 5 === 0) {
            $records['focused-' . $i] = libsqlite_release_canonical36_artifact($currentHead36, 'veryquick', ['json' . (100 + $i) . '.test'], 200 + $i);
        }
        if ($i % 7 === 0) {
            $records['next-' . $i] = libsqlite_release_canonical36_artifact($nextHead36, $testset, [], 3000 + $i);
        }
        if ($i % 11 === 0) {
            $records['stale-' . $i] = libsqlite_release_canonical36_artifact('old-' . $i, $testset, [], 4000 + $i);
        }

        $map = libsqlite_release_canonical36_evidence()->releaseRunnerUpstreamCanonicalMapCurrentNext36($records, $currentHead36, $nextHead36);
