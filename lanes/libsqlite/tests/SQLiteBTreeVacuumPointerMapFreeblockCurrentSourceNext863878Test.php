...
$makeFirstPage863878 = static function (int $pageCount): string {
...
$putPointerMapEntry863878 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database863878 = static function (int $sliceNumber) use ($makeFirstPage863878, $putPointerMapEntry863878): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage863878(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry863878($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan863878 = static function (int $sliceNumber, int $batchSize = 2) use ($database863878): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourcePlan {
    $database = $database863878($sliceNumber);
...
foreach ([863, 864, 865, 866, 867, 868, 869, 870, 871, 872, 873, 874, 875, 876, 877, 878] as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} extends next863-878 handoff receipts"] = static function (TestRunner $t) use ($plan863878, $sliceNumber): void {
        $plan = $plan863878($sliceNumber);
        ...
        $t->same(6, $plan863878($sliceNumber, 3)->currentSourceSummary()['current_source_row_count']);
    };
}
