...
$apply = static function (string $root = null, bool $reserved = false, bool $requiresSuper = false, ?bool $superExists = null) use ($setup, $makeStack, $databasePath, $sourcePages, $nextBefore): array {
    ...
    return $writer->applyWalHotJournalStatementCurrentSourceNext117(
        $makeStack(),
...
$cases = [
    ...
    'recovery status' => [static fn (): mixed => $apply()['recovery']['status'], 'hot_journal_wal_statement_current_source_recovered_next91'],
    ...
    'dependency next117 apply' => [static fn (): mixed => in_array('sqlite-wal-hot-journal-statement-current-source-vfs-apply-next117', $apply()['dependencies'], true), true],
    'dependency atomic rollback' => [static fn (): mixed => in_array('vfs-atomic-rollback-on-write-failure', $apply()['dependencies'], true), true],
    'dependency existing statement current source' => [static fn (): mixed => in_array('sqlite-wal-hot-journal-statement-current-source-next91', $apply()['dependencies'], true), true],
...
$tests['pager wal statement hot rollback current source next117 skips blocked hot journal'] = static function (TestRunner $t) use ($setup, $apply, $databasePath, $local, $dirtyDatabase, $journalBytes, $walBytes): void {
    ...
    $t->same('hot_journal_wal_statement_current_source_skipped_next91', $result['recovery']['status']);
...
$throws = [
    ...
    'empty path rejected' => static fn () => (new SQLiteVfsFileWriter($setup()))->applyWalHotJournalStatementCurrentSourceNext117($makeStack(), 'sp', 'stmt', 'next', 5, $nextBefore, '', [1], $sourcePages),
    ...
    'read only rejected' => static fn () => (new SQLiteVfsFileWriter($setup(), true))->applyWalHotJournalStatementCurrentSourceNext117($makeStack(), 'plugin-batch-next117', 'insert-active-plugin-next117', 'retry-plugin-option-next117', 5, $nextBefore, $databasePath, [1, 2, 3, 4], $sourcePages, true),
    'bad current source rejected' => static fn () => (new SQLiteVfsFileWriter($setup()))->applyWalHotJournalStatementCurrentSourceNext117($makeStack(), 'plugin-batch-next117', 'insert-active-plugin-next117', 'retry-plugin-option-next117', 5, $nextBefore, $databasePath, [1, 2, 3, 4], [3 => $page('stale current source')], true),
    'empty current statement rejected' => static fn () => (new SQLiteVfsFileWriter($setup()))->applyWalHotJournalStatementCurrentSourceNext117($makeStack(), 'plugin-batch-next117', '', 'retry-plugin-option-next117', 5, $nextBefore, $databasePath, [1, 2, 3, 4], $sourcePages, true),
    'empty next statement rejected' => static fn () => (new SQLiteVfsFileWriter($setup()))->applyWalHotJournalStatementCurrentSourceNext117($makeStack(), 'plugin-batch-next117', 'insert-active-plugin-next117', '', 5, $nextBefore, $databasePath, [1, 2, 3, 4], $sourcePages, true),
    'zero next page rejected' => static fn () => (new SQLiteVfsFileWriter($setup()))->applyWalHotJournalStatementCurrentSourceNext117($makeStack(), 'plugin-batch-next117', 'insert-active-plugin-next117', 'retry-plugin-option-next117', 0, $nextBefore, $databasePath, [1, 2, 3, 4], $sourcePages, true),
];
