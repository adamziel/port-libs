...
$before = [
    1 => $page('next161 stale schema before current master journal'),
    2 => $page('next161 stale wp_options root before current master journal'),
    3 => $page('next161 stale active_plugins before current master journal'),
    4 => $page('next161 stale plugin settings before current master journal'),
    5 => $page('next161 comments page unchanged before current master journal'),
    6 => $page('next161 transient page before current master journal'),
];
$databaseBytes = implode('', $before);
$recovered = [
    1 => $page('next161 recovered schema from current master journal'),
    2 => $page('next161 recovered wp_options root from current master journal'),
    3 => $page('next161 recovered active_plugins from current master journal'),
    4 => $page('next161 recovered plugin settings from current master journal'),
    6 => $page('next161 recovered transient from current master journal'),
];
$sourceBefore = 'next161-before-current-master-source';
$recoveredId = static fn (): string => 'master-reader-cache:' . hash('sha256', $masterPath . '|' . $databasePath . '-journal|/srv/wp-content/database/current-plugin-next161.sqlite-journal');
$readerCache = static fn (): array => [
    1 => ['label' => 'schema-retained', 'image' => $recovered[1], 'source_id' => $sourceBefore, 'epoch' => 7, 'master_journal_digest' => $currentDigest],
    2 => ['label' => 'root-refreshable', 'image' => $before[2], 'source_id' => $sourceBefore, 'epoch' => 7, 'master_journal_digest' => $currentDigest],
    3 => ['label' => 'pinned-active-plugins', 'image' => $before[3], 'source_id' => $sourceBefore, 'epoch' => 7, 'pinned' => true, 'master_journal_digest' => $currentDigest],
    4 => ['label' => 'dirty-plugin-settings', 'image' => $before[4], 'source_id' => $sourceBefore, 'epoch' => 7, 'dirty' => true, 'master_journal_digest' => $currentDigest],
    5 => ['label' => 'wrong-source-comments', 'image' => $before[5], 'source_id' => 'old-reader-source', 'epoch' => 7, 'master_journal_digest' => $currentDigest],
    6 => ['label' => 'wrong-epoch-transient', 'image' => $recovered[6], 'source_id' => $sourceBefore, 'epoch' => 6, 'master_journal_digest' => $currentDigest],
];
$nextWrites = [
    3 => $page('next161 rewritten active_plugins after reader cache rebase'),
    4 => $page('next161 rewritten plugin settings after reader cache rebase'),
];
...
): array => SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::variantNext161(
    $path ?? $databasePath,
...
$cases = [
    'status' => [static fn (): mixed => $plan()['status'], 'pager-master-journal-reader-cache-current-source-next161'],
    ...
    'cached members' => [static fn (): mixed => $plan()['cached_members'], [$databasePath . '-journal', '/srv/wp-content/database/old-plugin-next161.sqlite-journal']],
    'current members' => [static fn (): mixed => $plan()['current_members'], [$databasePath . '-journal', '/srv/wp-content/database/current-plugin-next161.sqlite-journal']],
    'cached membership stale' => [static fn (): mixed => $plan()['cached_membership_stale'], true],
    'cached membership digest' => [static fn (): mixed => $plan()['cached_master_journal_digest'], $cachedDigest],
    'current membership digest' => [static fn (): mixed => $plan()['current_master_journal_digest'], $currentDigest],
    ...
    'row retained digest matches' => [static fn (): mixed => $plan()['reader_rows'][0]['master_journal_digest_matches_current'], true],
    ...
    'row refreshed prefix' => [static fn (): mixed => $plan()['reader_rows'][1]['current_prefix'], 'next161 recovered wp_options root from current master journal'],
    ...
    'read invalidated prefix' => [static fn (): mixed => $plan()['next_reads'][2]['prefix'], 'next161 recovered active_plugins from current master journal'],
    'read unchanged prefix' => [static fn (): mixed => $plan()['next_reads'][4]['prefix'], 'next161 comments page unchanged before current master journal'],
    ...
    'write active before' => [static fn (): mixed => $plan()['next_writes'][0]['before_prefix'], 'next161 recovered active_plugins from current master journal'],
    'write active after' => [static fn (): mixed => $plan()['next_writes'][0]['after_prefix'], 'next161 rewritten active_plugins after reader cache rebase'],
    'write settings before' => [static fn (): mixed => $plan()['next_writes'][1]['before_prefix'], 'next161 recovered plugin settings from current master journal'],
    ...
    'final prefix page four write' => [static fn (): mixed => $plan()['final_prefixes'][4], 'next161 rewritten plugin settings after reader cache rebase'],
    ...
    'dependency marker' => [static fn (): mixed => in_array('sqlite-pager-master-journal-reader-cache-current-source-next161', $plan()['dependencies'], true), true],
    'dependency digest fence' => [static fn (): mixed => in_array('sqlite-pager-reader-cache-master-journal-digest-fence-next161', $plan()['dependencies'], true), true],
    ...
    'duplicate current members collapsed' => [static fn (): mixed => $plan(null, null, [1], [], '__default__', $currentMaster . $databasePath . "-journal\n")['current_members'], [$databasePath . '-journal', '/srv/wp-content/database/current-plugin-next161.sqlite-journal']],
    'all cache valid no reopen' => [static fn (): mixed => $plan(null, [1 => ['image' => $recovered[1], 'source_id' => $sourceBefore, 'epoch' => 7, 'master_journal_digest' => $currentDigest]], [1], [])['requires_reader_reopen'], false],
    'stale digest invalidates matching image' => [static fn (): mixed => $plan(null, [1 => ['image' => $recovered[1], 'source_id' => $sourceBefore, 'epoch' => 7, 'master_journal_digest' => $cachedDigest]], [1], [])['invalidated_cache_page_numbers'], [1]],
    'stale digest reason' => [static fn (): mixed => $plan(null, [1 => ['image' => $recovered[1], 'source_id' => $sourceBefore, 'epoch' => 7, 'master_journal_digest' => $cachedDigest]], [1], [])['reader_rows'][0]['reason'], 'reader_cache_master_journal_digest_predates_current_membership'],
    'stale digest matching image misses cache' => [static fn (): mixed => $plan(null, [1 => ['image' => $recovered[1], 'source_id' => $sourceBefore, 'epoch' => 7, 'master_journal_digest' => $cachedDigest]], [1], [])['next_reads'][0]['cache_hit'], false],
];

foreach ($cases as $name => [$callback, $expected]) {
    $tests['pager master journal reader cache current source next161 ' . $name] = static function (TestRunner $t) use ($callback, $expected): void {
...
$throws = [
    ...
    'empty cache digest rejected' => static fn () => $plan(null, [1 => ['image' => $recovered[1], 'source_id' => $sourceBefore, 'epoch' => 7, 'master_journal_digest' => '']]),
...
foreach ($throws as $name => $callback) {
    $tests['pager master journal reader cache current source next161 ' . $name] = static function (TestRunner $t) use ($callback): void {
