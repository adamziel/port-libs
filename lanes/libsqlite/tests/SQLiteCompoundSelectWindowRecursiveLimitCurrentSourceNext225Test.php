...
use PortLibs\LibSqlite\SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext225Plan;
...
$summary225 = static fn (?array $cursor = null): array => SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext225Plan::compare($sql225, $currentTables225, $nextTables225, $cursor);
$tests = [];
...
$tests['compound select window recursive limit current source next225 rejects missing last value'] = static function (TestRunner $t) use ($currentTables225): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext225Plan::compare(
...
foreach (range(1, 58) as $case) {
    $tests['compound select window recursive limit current source next225 generated intersect except value boundary ' . $case] = static function (TestRunner $t) use ($case): void {
        ...
        $plan = SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext225Plan::compare($sql, $tables, $nextTables);
        $again = SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext225Plan::compare($sql, $tables, $nextTables, $plan['cursor']);
