...
$withTemp = static function (callable $callback) use ($pageSize, $page, $makeWalBytes): mixed {
    $root = sys_get_temp_dir() . '/libsqlite-savepoint-journal-filehandle-next99-' . bin2hex(random_bytes(4));
...
$run = static fn (): array => $withTemp(static function (string $root, string $databasePath, array $journals, string $nextJournal, int $pageSize) use ($makeStack): array {
    return (new SQLiteVfsFileWriter($root))->applySavepointRollbackAndBeginNextStatementFromCurrentSourceNext99(
        $makeStack(),
        'plugin-settings',
        $databasePath,
        $pageSize,
...
$cases = [
    ...
    'next99 dependency marker' => [static fn (): mixed => in_array('sqlite-savepoint-journal-filehandle-current-source-next99', $run()['dependencies'], true), true],
    ...
    'next statement source prefix' => [static fn (): mixed => $run()['next_statement']['source_prefix'], 'before plugin autoload next99'],
    ...
    'snapshot database restores page two' => [static fn (): mixed => str_contains($snapshot()['database'], 'before plugin autoload next99'), true],
    'snapshot database restores child page' => [static fn (): mixed => str_contains($snapshot()['database'], 'before plugin child next99'), true],
    'snapshot database removes dirty autoload' => [static fn (): mixed => str_contains($snapshot()['database'], 'plugin dirty autoload next99'), false],
    'snapshot wal retains two frames' => [static fn (): mixed => SQLiteWal::parse($snapshot()['wal'], null, true)->frameCount(), 2],
    'snapshot wal drops discarded retry frame' => [static fn (): mixed => str_contains($snapshot()['wal'], 'wal discarded retry commit next99'), false],
    'snapshot retry journal uses restored page image' => [static fn (): mixed => str_contains($snapshot()['retry_journal'], 'before plugin autoload next99'), true],
    'snapshot retry journal not dirty page image' => [static fn (): mixed => str_contains($snapshot()['retry_journal'], 'plugin dirty autoload next99'), false],
...
foreach ($cases as $name => [$callback, $expected]) {
    $tests['pager savepoint journal filehandle current source next99 ' . $name] = static function (TestRunner $t) use ($callback, $expected): void {
        $t->same($expected, $callback());
    };
}

$tests['pager savepoint journal filehandle current source next99 rejects missing next statement name'] = static function (TestRunner $t) use ($withTemp, $makeStack): void {
    $withTemp(static function (string $root, string $databasePath, array $journals, string $nextJournal, int $pageSize) use ($makeStack, $t): void {
        ...
        $t->throws(InvalidArgumentException::class, static fn (): mixed => $writer->applySavepointRollbackAndBeginNextStatementFromCurrentSourceNext99($makeStack(), 'plugin-settings', $databasePath, $pageSize, '', $nextJournal, 2, false, $journals));
    });
};

$tests['pager savepoint journal filehandle current source next99 rejects missing next journal path'] = static function (TestRunner $t) use ($withTemp, $makeStack): void {
    $withTemp(static function (string $root, string $databasePath, array $journals, string $nextJournal, int $pageSize) use ($makeStack, $t): void {
        $writer = new SQLiteVfsFileWriter($root);
        $t->throws(InvalidArgumentException::class, static fn (): mixed => $writer->applySavepointRollbackAndBeginNextStatementFromCurrentSourceNext99($makeStack(), 'plugin-settings', $databasePath, $pageSize, 'retry', '', 2, false, $journals));
    });
};

$tests['pager savepoint journal filehandle current source next99 rejects page outside current source'] = static function (TestRunner $t) use ($withTemp, $makeStack): void {
    $withTemp(static function (string $root, string $databasePath, array $journals, string $nextJournal, int $pageSize) use ($makeStack, $t): void {
        $writer = new SQLiteVfsFileWriter($root);
        $t->throws(InvalidArgumentException::class, static fn (): mixed => $writer->applySavepointRollbackAndBeginNextStatementFromCurrentSourceNext99($makeStack(), 'plugin-settings', $databasePath, $pageSize, 'retry', $nextJournal, 9, false, $journals));
    });
};
