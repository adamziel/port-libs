...
$basePages = [
    1 => $page('next163 clean schema page before crashed import'),
    2 => $page('next163 clean wp_options page before checkpoint'),
    3 => $page('next163 clean plugin page before savepoint'),
    4 => $page('next163 clean transient page before savepoint'),
];
$dirtyDatabaseBytes = $page('next163 dirty schema page from hot rollback journal')
    . $basePages[2]
    . $page('next163 dirty plugin page from rolled back savepoint')
    . $page('next163 dirty transient page from rolled back savepoint');
...
$makeWalBytes = static function () use ($pageSize, $page): string {
    ...
    foreach ([
        [1, 0, 'next163 wal schema retained for pinned reader'],
        [2, 4, 'next163 wal wp_options retained commit'],
        [3, 0, 'next163 wal plugin draft rolled back to savepoint'],
        [4, 4, 'next163 wal transient commit discarded by savepoint'],
...
$withFiles = static function (string $databaseBytes, string $journalInput, string $walInput, callable $callback) use ($databasePath): mixed {
    $root = sys_get_temp_dir() . '/port-libsqlite-hot-savepoint-checkpoint-next163-' . bin2hex(random_bytes(4));
    ...
    if (!is_dir($directory) && !mkdir($directory, 0777, true) && !is_dir($directory)) {
        throw new RuntimeException('Unable to create WAL hot journal savepoint checkpoint next163 test directory');
    }
...
$apply = static fn (): array => $withFiles(
    ...
    static fn (string $root, string $databaseLocal, string $journalLocal, string $walLocal): array => [
        'applied' => (new SQLiteVfsFileWriter($root))->applyHotJournalSavepointCheckpointPinnedReaderCurrentSourceNext163(
            $makeStack(),
...
$skipReserved = static fn (): array => $withFiles(
    ...
    $journalBytes,
    $walBytes,
    static fn (string $root): array => (new SQLiteVfsFileWriter($root))->applyHotJournalSavepointCheckpointPinnedReaderCurrentSourceNext163(
        $makeStack(),
        'plugin_batch_next163',
        $databasePath,
        [1],
...
$cases = [
    ...
    'pinned status' => [static fn (): mixed => $apply()['applied']['pinned_reader']['status'], 'wal-hot-journal-savepoint-checkpoint-pinned-reader-current-source-next163'],
    ...
    'database has retained schema wal page' => [static fn (): mixed => str_contains($apply()['database_bytes'], 'next163 wal schema retained for pinned reader'), true],
    'database restores clean plugin page' => [static fn (): mixed => str_contains($apply()['database_bytes'], 'next163 clean plugin page before savepoint'), true],
    'database excludes dirty schema page' => [static fn (): mixed => str_contains($apply()['database_bytes'], 'next163 dirty schema page from hot rollback journal'), false],
    'database excludes discarded plugin draft' => [static fn (): mixed => str_contains($apply()['database_bytes'], 'next163 wal plugin draft rolled back to savepoint'), false],
    'dependency next163 marker' => [static fn (): mixed => in_array('sqlite-wal-hot-journal-savepoint-checkpoint-pinned-reader-current-source-next163', $apply()['applied']['dependencies'], true), true],
    'dependency next160 marker' => [static fn (): mixed => in_array('sqlite-wal-hot-journal-savepoint-checkpoint-vfs-apply-next160', $apply()['applied']['dependencies'], true), true],
    ...
    'reserved lock pinned skipped' => [static fn (): mixed => $skipReserved()['pinned_reader']['status'], 'wal-hot-journal-savepoint-checkpoint-pinned-reader-skipped-next163'],
];

foreach ($cases as $name => [$callback, $expected]) {
    $tests['wal hot journal savepoint checkpoint current source next163 ' . $name] = static function (TestRunner $t) use ($callback, $expected): void {
        $t->same($expected, $callback());
    };
}

$tests['wal hot journal savepoint checkpoint current source next163 rejects empty path'] = static function (TestRunner $t): void {
    $t->throws(InvalidArgumentException::class, static fn (): mixed => (new SQLiteVfsFileWriter(sys_get_temp_dir()))->applyHotJournalSavepointCheckpointPinnedReaderCurrentSourceNext163(new SQLiteSavepointStack(), 'x', '', [1], 1));
};

$tests['wal hot journal savepoint checkpoint current source next163 rejects zero reader'] = static function (TestRunner $t) use ($databasePath): void {
    $t->throws(InvalidArgumentException::class, static fn (): mixed => (new SQLiteVfsFileWriter(sys_get_temp_dir()))->applyHotJournalSavepointCheckpointPinnedReaderCurrentSourceNext163(new SQLiteSavepointStack(), 'x', $databasePath, [1], 0));
};

$tests['wal hot journal savepoint checkpoint current source next163 rejects readonly writer'] = static function (TestRunner $t) use ($dirtyDatabaseBytes, $journalBytes, $walBytes, $withFiles, $databasePath, $makeStack): void {
    $withFiles($dirtyDatabaseBytes, $journalBytes, $walBytes, static function (string $root) use ($t, $databasePath, $makeStack): void {
        $t->throws(LogicException::class, static fn (): mixed => (new SQLiteVfsFileWriter($root, readOnly: true))->applyHotJournalSavepointCheckpointPinnedReaderCurrentSourceNext163($makeStack(), 'plugin_batch_next163', $databasePath, [1], 1));
    });
};

$tests['wal hot journal savepoint checkpoint current source next163 fixture wal is parseable'] = static function (TestRunner $t) use ($wal): void {
