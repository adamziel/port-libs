...
$makeFirstPage543558 = static function (int $pageCount): string {
...
$putPointerMapEntry543558 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database543558 = static function (int $sliceNumber) use ($makeFirstPage543558, $putPointerMapEntry543558): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage543558(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry543558($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan543558 = static function (int $sliceNumber, int $batchSize = 2) use ($database543558): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database543558($sliceNumber);
...
foreach ([543, 544, 545, 546, 547, 548, 549, 550, 551, 552, 553, 554, 555, 556, 557, 558] as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} publishes next543-558 handoff receipts"] = static function (TestRunner $t) use ($plan543558, $sliceNumber): void {
        $plan = $plan543558($sliceNumber);
        ...
        $t->same(6, $plan543558($sliceNumber, 3)->currentSourceSummary()['current_source_row_count']);
    };
}
