...
$plan223 = static function (int $batchSize = 2): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    global $database223;

    $database = $database223();
    $deletedPage = SQLiteTableLeafPage::deleteCellByRowId($database->page(3), 2, secureDelete: true);

    return SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan::tableLeafCheckpointAuditFromDeleteResult(
        $database,
        3,
        [
            'page' => $deletedPage,
...
$cases223 = [
    ...
    'current source token length' => static fn (): mixed => strlen($plan223()->sourceSummary()['current_source_checkpoint_token']),
    ...
    'non overlap' => static fn (): mixed => str_contains($plan223()->sourceSummary()['non_overlap'], 'does not repeat write-receipts'),
...
$expected223 = [
    'action label' => 'btree-vacuum-pointermap-freeblock-checkpoint-audit',
    ...
    'dependency closure' => 'no new support component needed; checkpoint-audit reuses write-receipts per-page write receipts and publishes a current-source source fence only',
    'non overlap' => true,
...
foreach ($cases223 as $name => $callback) {
    $tests['btree vacuum pointermap freeblock current source checkpoint-audit ' . $name] = static function (TestRunner $t) use ($callback, $expected223, $name): void {
        $t->same($expected223[$name], $callback());
    };
}

foreach (range(1, 90) as $index) {
    $tests['btree vacuum pointermap freeblock current source checkpoint-audit source invariant ' . $index] = static function (TestRunner $t) use ($plan223): void {
        $plan = $plan223();
        ...
        $t->same('btree-vacuum-pointermap-freeblock-checkpoint-audit-ready', $summary['status']);
