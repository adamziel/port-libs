...
$plan = static function () use ($readyCurrent): array {
    static $result = null;
    if ($result === null) {
        $result = SQLiteVfsCurrentSourceNext314321Plan::run([
...
return [
    ...
    'vfs current source next314-321 blocks dirty snapshot' => static fn (TestRunner $t) => $t->same(true, in_array('dirty-pages-present', SQLiteVfsCurrentSourceNext314321Plan::run(['snapshot(reader-ready-next321,shared-cache-next313)'], ['current' => $dirtyCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source next314-321 blocks old snapshot ack' => static fn (TestRunner $t) => $t->same(true, in_array('ack-not-latest-publish', SQLiteVfsCurrentSourceNext314321Plan::run(['snapshot(reader-ready-next321,shared-cache-next313)'], ['current' => $oldAckCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source next314-321 blocks publish without claim' => static fn (TestRunner $t) => $t->same(true, in_array('missing-reuse-claim', SQLiteVfsCurrentSourceNext314321Plan::run(['snapshot(reader-ready-next321,shared-cache-next313)', 'publish(reader-ready-next321,reader-reuse-next321,shared-cache-next321)'], ['current' => $readyCurrent])['events'][1]['blocked_reasons'], true)),
    'vfs current source next314-321 blocks stale claim' => static fn (TestRunner $t) => $t->same(true, in_array('stale-reuse-claim', SQLiteVfsCurrentSourceNext314321Plan::run(['publish(reader-ready-next321,reader-reuse-next321,shared-cache-next321)'], ['current' => $staleClaimCurrent])['events'][0]['blocked_reasons'], true)),
    'vfs current source next314-321 accepts preclaimed publish' => static fn (TestRunner $t) => $t->same('published-reused-current-source', SQLiteVfsCurrentSourceNext314321Plan::run(['publish(reader-ready-next321,reader-reuse-next321,shared-cache-next321)'], ['current' => $claimedCurrent])['events'][0]['status']),
    'vfs current source next314-321 rejects empty operations' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteVfsCurrentSourceNext314321Plan::run([])),
    'vfs current source next314-321 rejects bad claim token' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteVfsCurrentSourceNext314321Plan::run([['op' => 'claim', 'snapshot' => 'reader-ready-next321', 'ack' => 'shared-cache-next313', 'claim' => 'bad claim']], ['current' => $readyCurrent])),
    'vfs current source next314-321 rejects unsupported operation' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteVfsCurrentSourceNext314321Plan::run(['republish(reader-ready-next321,shared-cache-next313)'], ['current' => $readyCurrent])),
