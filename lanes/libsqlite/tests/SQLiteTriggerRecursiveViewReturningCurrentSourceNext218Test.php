...
$plan218 = static fn (array $options = []): array => SQLiteTriggerRecursiveViewReturningCurrentSourceNextPlan::executeCurrentSourceEpochFence(
    ...
    $options + [
        ...
        'reset_generation' => 'app-current-reset-218',
        'post_reset_current_source_token' => 'app.current.source.postreset.218',
        'post_reset_cursor' => 'app.returning.postreset.cursor.218',
        ...
        'following_generation' => 'app-following-current-218',
        ...
        'current_view_epoch_next218' => 'app.returning.view.epoch.cursor.218',
        'current_trigger_epoch_next218' => 'app.returning.trigger.epoch.cursor.218',
    ],
);
...
$custom218 = static fn (): array => $plan218([
    ...
    'current_source_epoch_next218' => 'app.current.source.epoch.custom.218',
    'current_view_epoch_next218' => 'app.returning.view.epoch.cursor.custom.218',
    'current_trigger_epoch_next218' => 'app.returning.trigger.epoch.cursor.custom.218',
]);
...
$cases218 = [
    ...
    'savepoint retained' => [static fn (): mixed => $released218()['savepoint'], 'app_recursive_view_218'],
    ...
    'epoch token retained' => [static fn (): mixed => $released218()['current_source_epoch_next218'], 'app.current.source.epoch.218'],
    'custom epoch token retained' => [static fn (): mixed => $custom218()['current_source_epoch_next218'], 'app.current.source.epoch.custom.218'],
    'view epoch cursor retained' => [static fn (): mixed => $released218()['current_view_epoch_next218'], 'app.returning.view.epoch.cursor.218'],
    'custom view epoch cursor retained' => [static fn (): mixed => $custom218()['current_view_epoch_next218'], 'app.returning.view.epoch.cursor.custom.218'],
    'trigger epoch cursor retained' => [static fn (): mixed => $released218()['current_trigger_epoch_next218'], 'app.returning.trigger.epoch.cursor.218'],
    'custom trigger epoch cursor retained' => [static fn (): mixed => $custom218()['current_trigger_epoch_next218'], 'app.returning.trigger.epoch.cursor.custom.218'],
    ...
    'current epoch token stamped' => [static fn (): mixed => array_values(array_unique(array_column($released218()['current_source_rows_next218'], 'current_source_epoch_next218'))), ['app.current.source.epoch.218']],
    'next epoch token stamped' => [static fn (): mixed => array_values(array_unique(array_column($released218()['attempted_next_source_rows_next218'], 'current_source_epoch_next218'))), ['app.current.source.epoch.218']],
    'current view cursor stamped' => [static fn (): mixed => array_values(array_unique(array_column($released218()['current_source_rows_next218'], 'current_view_epoch_next218'))), ['app.returning.view.epoch.cursor.218']],
    'next trigger cursor stamped' => [static fn (): mixed => array_values(array_unique(array_column($released218()['attempted_next_source_rows_next218'], 'current_trigger_epoch_next218'))), ['app.returning.trigger.epoch.cursor.218']],
    'visible payload names released' => [static fn (): mixed => array_column($released218()['visible_returning_payloads_next218'], 'name'), ['app_summary_child', 'template_child', 'landing_url', 'next_module']],
    'held payload names missing' => [static fn (): mixed => array_column($missing218()['held_next_returning_payloads_next218'], 'name'), ['landing_url', 'next_module']],
