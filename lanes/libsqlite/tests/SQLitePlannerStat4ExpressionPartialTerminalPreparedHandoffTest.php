...
$tests = [
    ...
    'planner stat4 expression partial current source terminalPreparedHandoff inherits penultimate prepared handoff' => static fn (TestRunner $t) => $t->same(true, $planTerminalPreparedHandoff()['selectedPlan']['penultimatePreparedHandoffPrepared']),
    ...
    'planner stat4 expression partial current source terminalPreparedHandoff prior signature threaded' => static fn (TestRunner $t) => $t->same($planTerminalPreparedHandoff()['stat4PenultimatePreparedHandoffPreparationFence']['handoffSignature'], $planTerminalPreparedHandoff()['selectedPlan']['terminalPreparedHandoffPriorHandoffSignature']),
    'planner stat4 expression partial current source terminalPreparedHandoff preserves penultimate fence' => static fn (TestRunner $t) => $t->same(range(942, 957), $planTerminalPreparedHandoff()['stat4PenultimatePreparedHandoffPreparationFence']['preparedSlices']),
    ...
    'planner stat4 expression partial current source terminalPreparedHandoff non overlap' => static fn (TestRunner $t) => $t->contains('penultimate prepared', $planTerminalPreparedHandoff()['non_overlap']),
