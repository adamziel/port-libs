...
use PortLibs\LibSqlite\SQLitePragmaIntegrityPartialIndexCurrentSourceNext126;
...
$page = static function (
    ...
    return SQLitePragmaIntegrityPartialIndexCurrentSourceNext126::page(
        $rows,
...
$tests['pragma integrity partial index current source next126 rejects empty index columns'] = static function (TestRunner $t) use ($rows, $validEntries, $predicate): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLitePragmaIntegrityPartialIndexCurrentSourceNext126::page($rows, $validEntries, $predicate, []));
};
...
$tests['pragma integrity partial index current source next126 rejects duplicate index entry keys'] = static function (TestRunner $t) use ($rows, $validEntries, $predicate, $indexColumns): void {
    ...
    $t->throws(InvalidArgumentException::class, static fn () => SQLitePragmaIntegrityPartialIndexCurrentSourceNext126::page($rows, $entries, $predicate, $indexColumns));
};
...
$tests['pragma integrity partial index current source next126 rejects unsupported pragma'] = static function (TestRunner $t) use ($rows, $validEntries, $predicate, $indexColumns): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLitePragmaIntegrityPartialIndexCurrentSourceNext126::page($rows, $validEntries, $predicate, $indexColumns, 0, 126, 'wp_options', 'wp_options_autoload_yes', 'PRAGMA table_info(wp_options)'));
};
