...
$plan = static fn (
    ...
    array $pages = [1, 2, 3, 4],
): array => SQLiteWalCheckpointHotJournalReaderCurrentSourceNextPlan::checkpointHotJournalReaderReleasedWalPlan(
    $databasePath,
    $dirtyDatabase,
    $journalBytes,
    $currentWal,
...
$throws = [
    'empty path rejected' => static fn () => SQLiteWalCheckpointHotJournalReaderCurrentSourceNextPlan::checkpointHotJournalReaderReleasedWalPlan('', $dirtyDatabase, $journalBytes, $currentWal, $walBytes, $hotDatabase, $walBytes, [1], 1),
    'empty database rejected' => static fn () => SQLiteWalCheckpointHotJournalReaderCurrentSourceNextPlan::checkpointHotJournalReaderReleasedWalPlan($databasePath, '', $journalBytes, $currentWal, $walBytes, $hotDatabase, $walBytes, [1], 1),
    'empty journal rejected' => static fn () => SQLiteWalCheckpointHotJournalReaderCurrentSourceNextPlan::checkpointHotJournalReaderReleasedWalPlan($databasePath, $dirtyDatabase, '', $currentWal, $walBytes, $hotDatabase, $walBytes, [1], 1),
    'empty wal rejected' => static fn () => SQLiteWalCheckpointHotJournalReaderCurrentSourceNextPlan::checkpointHotJournalReaderReleasedWalPlan($databasePath, $dirtyDatabase, $journalBytes, $currentWal, '', $hotDatabase, $walBytes, [1], 1),
    'empty reader database rejected' => static fn () => SQLiteWalCheckpointHotJournalReaderCurrentSourceNextPlan::checkpointHotJournalReaderReleasedWalPlan($databasePath, $dirtyDatabase, $journalBytes, $currentWal, $walBytes, '', $walBytes, [1], 1),
    'empty pages rejected' => static fn () => SQLiteWalCheckpointHotJournalReaderCurrentSourceNextPlan::checkpointHotJournalReaderReleasedWalPlan($databasePath, $dirtyDatabase, $journalBytes, $currentWal, $walBytes, $hotDatabase, $walBytes, [], 1),
    'stale current wal rejected' => static fn () => SQLiteWalCheckpointHotJournalReaderCurrentSourceNextPlan::checkpointHotJournalReaderReleasedWalPlan($databasePath, $dirtyDatabase, $journalBytes, $currentWal, substr_replace($walBytes, 'x', 80, 1), $hotDatabase, $walBytes, [1], 1),
    'unaligned database rejected' => static fn () => SQLiteWalCheckpointHotJournalReaderCurrentSourceNextPlan::checkpointHotJournalReaderReleasedWalPlan($databasePath, $dirtyDatabase . 'x', $journalBytes, $currentWal, $walBytes, $hotDatabase, $walBytes, [1], 1),
    'unaligned reader database rejected' => static fn () => SQLiteWalCheckpointHotJournalReaderCurrentSourceNextPlan::checkpointHotJournalReaderReleasedWalPlan($databasePath, $dirtyDatabase, $journalBytes, $currentWal, $walBytes, $hotDatabase . 'x', $walBytes, [1], 1),
    ...
    'reader past wal rejected' => static fn () => SQLiteWalCheckpointHotJournalReaderCurrentSourceNextPlan::checkpointHotJournalReaderReleasedWalPlan($databasePath, $dirtyDatabase, $journalBytes, $currentWal, $walBytes, $hotDatabase, $walBytes, [1], 4),
];
