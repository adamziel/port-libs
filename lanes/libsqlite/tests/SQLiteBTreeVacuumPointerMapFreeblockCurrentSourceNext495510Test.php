...
$makeFirstPage495510 = static function (int $pageCount): string {
...
$putPointerMapEntry495510 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database495510 = static function (int $sliceNumber) use ($makeFirstPage495510, $putPointerMapEntry495510): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage495510(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry495510($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan495510 = static function (int $sliceNumber, int $batchSize = 2) use ($database495510): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourcePlan {
    $database = $database495510($sliceNumber);
...
foreach ([495, 496, 497, 498, 499, 500, 501, 502, 503, 504, 505, 506, 507, 508, 509, 510] as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} publishes next495-510 handoff receipts"] = static function (TestRunner $t) use ($plan495510, $sliceNumber): void {
        $plan = $plan495510($sliceNumber);
        ...
        $t->same(6, $plan495510($sliceNumber, 3)->currentSourceSummary()['current_source_row_count']);
    };
}
