...
$current177 = [
    'option_id' => 177,
    'option_name' => 'wp_plugin_generated_path_rowid_cost_current_source_next177',
...
): array => SQLiteJsonTablePlan::currentSourceGeneratedPathRowidCostCurrentSourceNext177(
    'json_tree',
    $current ?? $current177,
    $next ?? $next177,
    'option_value',
...
$tests = [
    ...
    'bad function rejected' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteJsonTablePlan::currentSourceGeneratedPathRowidCostCurrentSourceNext177('json_blob', $current177, $next177, 'option_value', 'generated_path')),
    'bad json source column rejected' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteJsonTablePlan::currentSourceGeneratedPathRowidCostCurrentSourceNext177('json_tree', $current177, $next177, '', 'generated_path')),
    'dependency scenario has no new support component' => static fn (TestRunner $t) => $t->same('no-new-support-component', 'no-new-support-component'),
];

foreach ($tests as $name => $case) {
    $tests['json table generated path rowid cost current source next177 ' . $name] = $case;
    unset($tests[$name]);
}
