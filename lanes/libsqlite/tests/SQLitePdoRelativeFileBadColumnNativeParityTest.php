...
return [
    ...
    'SQLitePDO relative file parameterized exec bad-column errors keep later exec params on original file' => static function (TestRunner $t) use ($sqlitePdoNativeAvailable, $withScratchDirs, $openRelative): void {
        $expectedRows = [
            ['setting_id' => 1, 'key_name' => 'alpha', 'key_value' => 'kept'],
            ['setting_id' => 2, 'key_name' => 'beta', 'key_value' => null],
        ];
        $expectedErrorInfo = ['HY000', 1, 'table app_settings has no column named missing_key'];

        $exercise = static function (string $class, string $first, string $second, bool $useExecParams) use ($openRelative): array {
            $cwd = getcwd();
            if ($cwd === false) {
                throw new RuntimeException('Unable to read current working directory');
            }
            ...
            try {
                ...
                try {
                    if ($useExecParams) {
                        ...
                        $statement = $pdo->prepare('INSERT INTO app_settings (missing_key) VALUES (?)');
                        if (!$statement instanceof PDOStatement) {
                            throw new RuntimeException('Expected prepared statement');
                        }
                        $statement->execute(['ignored']);
                    }
                    throw new RuntimeException('Expected invalid target-column PDOException');
                } catch (PDOException $exception) {
                    $afterBadColumn = [
                        'message' => $exception->getMessage(),
                        'exception_error_info' => $exception->errorInfo ?? null,
                ...
                if ($useExecParams) {
                    $success = $pdo->exec('INSERT INTO app_settings (key_name, key_value) VALUES (?, ?)', ['beta']);
                    $successRowCount = $success;
                    $successStatementErrorInfo = null;
                } else {
                    $successStatement = $pdo->prepare('INSERT INTO app_settings (key_name, key_value) VALUES (?, ?)');
                    if (!$successStatement instanceof PDOStatement) {
                        throw new RuntimeException('Expected success prepared statement');
                    }
                    $success = $successStatement->execute(['beta']);
                    $successRowCount = $successStatement->rowCount();
                    $successStatementErrorInfo = $successStatement->errorInfo();
                }

                $rowsAfterSuccess = $pdo->query('SELECT setting_id, key_name, key_value FROM app_settings ORDER BY setting_id')->fetchAll(PDO::FETCH_ASSOC);
                $afterSuccess = [
                    'success' => $success,
                    'row_count' => $successRowCount,
                    'connection_error_info' => $pdo->errorInfo(),
                    'statement_error_info' => $successStatementErrorInfo,
                    'rows' => $rowsAfterSuccess,
                    'second_file_exists_after_success' => is_file($second . '/relative.sqlite'),
                ];
                unset($pdo);
        ...
        $withScratchDirs('sqlite-pdo-relative-exec-params', static function (string $first, string $second) use ($t, $sqlitePdoNativeAvailable, $exercise, $expectedRows, $expectedErrorInfo): void {
            $polyfill = $exercise(SQLitePDO::class, $first, $second, true);
            $native = $sqlitePdoNativeAvailable() ? $exercise(PDO::class, $first, $second, false) : null;
            ...
            $t->same(true, $error['original_file_unchanged']);
            $t->same(false, $error['second_file_exists_after_failure']);
            $t->same(1, $polyfill['after_success']['success']);
            $t->same(1, $polyfill['after_success']['row_count']);
            $t->same(['00000', null, null], $polyfill['after_success']['connection_error_info']);
            $t->same(null, $polyfill['after_success']['statement_error_info']);
            $t->same($expectedRows, $polyfill['after_success']['rows']);
            $t->same(false, $polyfill['after_success']['second_file_exists_after_success']);
            $t->same($expectedRows, $polyfill['reopened_rows']);
            $t->same(true, $polyfill['first_file_exists']);
            ...
            if ($native !== null) {
                ...
                $t->same($native['after_bad_column']['connection_error_info'], $error['connection_error_info']);
                $t->same($native['after_bad_column']['statement_was_created'], $error['statement_was_created']);
                $t->same($native['after_success']['row_count'], $polyfill['after_success']['row_count']);
                $t->same($native['after_success']['connection_error_info'], $polyfill['after_success']['connection_error_info']);
                $t->same($native['after_success']['rows'], $polyfill['after_success']['rows']);
                $t->same($native['after_success']['second_file_exists_after_success'], $polyfill['after_success']['second_file_exists_after_success']);
                $t->same($native['reopened_rows'], $polyfill['reopened_rows']);
