...
$makeFirstPage343350 = static function (int $pageCount): string {
...
$putPointerMapEntry343350 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database343350 = static function (int $sliceNumber) use ($makeFirstPage343350, $putPointerMapEntry343350): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage343350(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry343350($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan343350 = static function (int $sliceNumber, int $batchSize = 2) use ($database343350): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database343350($sliceNumber);
...
foreach ([343, 344, 345, 346, 347, 348, 349, 350] as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} keeps final next343-350 freelist splice isolated"] = static function (TestRunner $t) use ($plan343350, $sliceNumber): void {
        $plan = $plan343350($sliceNumber);
        ...
        $t->same(6, $plan343350($sliceNumber, 3)->freelistSummary()['freelist_row_count']);
    };
}
