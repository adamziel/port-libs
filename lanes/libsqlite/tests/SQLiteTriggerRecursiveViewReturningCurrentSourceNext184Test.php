...
$ackHeld184 = static fn (): array => $run184([
    'acknowledged_current_checkpoints' => [
        'app_recursive_view_checkpoint_184:app-current-returning-184:0',
        'app_recursive_view_checkpoint_184:app-current-returning-184:1',
    ],
]);
$exposed184 = static fn (): array => $run184(['admit_next_source' => true, 'auto_ack_current' => true]);
$missingAck184 = static fn (): array => $run184(['admit_next_source' => true, 'acknowledged_current_checkpoints' => ['app_recursive_view_checkpoint_184:app-current-returning-184:0']]);
$unexpectedAck184 = static fn (): array => $run184([
    'admit_next_source' => true,
    'acknowledged_current_checkpoints' => [
        'app_recursive_view_checkpoint_184:app-current-returning-184:0',
        'app_recursive_view_checkpoint_184:app-current-returning-184:1',
        'app_recursive_view_checkpoint_184:app-current-returning-184:9',
    ],
]);
$badToken184 = static fn (): array => $run184(['admit_next_source' => true, 'auto_ack_current' => true, 'expected_handoff_token' => 'app.returning.current.source.handoff.184.expected']);
...
$cases184 = [
    ...
    'handoff token retained' => [static fn (): mixed => $held184()['handoff_token'], 'app.returning.current.source.handoff.184'],
    ...
    'missing one ack' => [static fn (): mixed => $missingAck184()['missing_current_checkpoints'], ['app_recursive_view_checkpoint_184:app-current-returning-184:1']],
    'unexpected ack recorded' => [static fn (): mixed => $unexpectedAck184()['unexpected_current_checkpoints'], ['app_recursive_view_checkpoint_184:app-current-returning-184:9']],
    ...
    'current second ack token' => [static fn (): mixed => $exposed184()['current_checkpoint_acks'][1]['checkpoint_token'], 'app_recursive_view_checkpoint_184:app-current-returning-184:1'],
    ...
    'visible names held' => [static fn (): mixed => array_column($held184()['visible_returning_rows'], 'name'), ['base_url', 'current_module', 'base_url:child', 'current_module:child', 'base_url:child:child', 'current_module:child:child']],
    'held names held' => [static fn (): mixed => array_column($held184()['held_returning_rows'], 'name'), ['landing_url', 'next_module', 'landing_url:child', 'landing_url:child:child']],
    'visible names exposed' => [static fn (): mixed => array_column($exposed184()['visible_returning_rows'], 'name'), ['base_url', 'current_module', 'base_url:child', 'current_module:child', 'base_url:child:child', 'current_module:child:child', 'landing_url', 'next_module', 'landing_url:child', 'landing_url:child:child']],
    ...
    'handoff current generation' => [static fn (): mixed => $held184()['handoff_plan']['current_generation'], 'app-current-returning-184'],
    'handoff next generation' => [static fn (): mixed => $held184()['handoff_plan']['next_generation'], 'app-next-returning-184'],
    'handoff resume after token' => [static fn (): mixed => $held184()['handoff_plan']['resume_after_token'], 'app_recursive_view_returning_cursor_184:app-current-returning-184:5'],
    'handoff blocked token held' => [static fn (): mixed => $held184()['handoff_plan']['blocked_at_token'], 'app_recursive_view_returning_cursor_184:app-next-returning-184:6'],
    ...
    'non recursive visible names' => [static fn (): mixed => array_column($nonRecursive184()['visible_returning_rows'], 'name'), ['base_url', 'current_module', 'landing_url', 'next_module']],
