...
$pageSize = 512;
$database = '/srv/wp-content/database/wp-next510.sqlite';
...
$formatPage = static function (string $label) use ($pageSize): string {
    ...
    $page = substr_replace($page, pack('N', 510), 60, 4);
...
$tokens = [$journal => 'member-main-current-510'];
$headers = [$journal => hash('sha256', 'main header next510')];
$base = [
    'source_id' => 'pager-reader-cache-current-source-next510',
    'epoch' => 510,
    'format_signature' => hash('sha256', implode('|', [512, 4, 2, 510, 0])),
    'publication_generation' => 510,
    'master_source_digest' => hash('sha256', 'master-next510'),
    'recovery_sequence' => 510,
...
foreach ($tokenFields as $field) {
    $base[$field] = str_replace('_', '-', preg_replace('/_token$/', '', $field)) . '-current-510';
}
...
$plan = static fn (array $cacheExtra = [], array $readExtra = []): array => SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::readerCacheStatementFence(510,
    $database,
    $master,
    $masterBytes,
    implode('', $before),
...
$tests['pager master journal reader cache current source next510 admits current VDBE branch fences'] = static function (TestRunner $t) use ($plan): void {
    $result = $plan();
    $t->same('pager-master-journal-reader-cache-current-source-next510', $result['status']);
    $t->same([], $result['invalidated_cache_page_numbers']);
    $t->same(['read-options' => true], $result['read_cache_hits']);
    $t->same('reader-cache-stmt-vdbe-if-not-open-current-510', $result['current_reader_cache_stmt_vdbe_if_not_open_token']);
};

$tests['pager master journal reader cache current source plan dispatches full width calls to next510'] = static function (TestRunner $t) use ($database, $master, $masterBytes, $before, $pageSize, $recovered, $cacheEntry, $read, $base, $tokens, $headers, $tokenFields): void {
...
$tests['pager master journal reader cache current source next510 invalidates stale if_not_open cache'] = static function (TestRunner $t) use ($plan, $opCount): void {
    $result = $plan(['reader_cache_stmt_vdbe_if_not_open_token' => 'stmt-vdbe-if_not_open-old']);
    $t->same([1], $result['reader_cache_stmt_vdbe_if_not_open_invalidated_cache_page_numbers']);
    $t->same([1], $result['invalidated_cache_page_numbers']);
    $t->same(1, $opCount($result, 'invalidate_reader_cache_reader_cache_stmt_vdbe_if_not_open_current_source_next510'));
};

$tests['pager master journal reader cache current source next510 reopens stale if_not_open read ticket'] = static function (TestRunner $t) use ($plan, $opCount): void {
    $result = $plan([], ['reader_cache_stmt_vdbe_if_not_open_token' => 'stmt-vdbe-if_not_open-old']);
    $t->same(['read-options'], $result['reopen_reader_ids']);
    $t->same(false, $result['next_reads'][0]['cache_hit']);
    $t->same('reader_ticket_reader_cache_stmt_vdbe_if_not_open_predates_current_source', $result['next_reads'][0]['reader_cache_stmt_vdbe_if_not_open_token_reason']);
    $t->same(1, $opCount($result, 'reopen_reader_for_reader_cache_stmt_vdbe_if_not_open_current_source_next510'));
};

$tests['pager master journal reader cache current source next510 missing if_not_open token rejects'] = static function (TestRunner $t) use ($plan): void {
    $t->throws(Throwable::class, static fn () => $plan(['reader_cache_stmt_vdbe_if_not_open_token' => null]));
};
