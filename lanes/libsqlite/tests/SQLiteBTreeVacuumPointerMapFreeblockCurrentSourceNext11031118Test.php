...
$makeFirstPage11031118 = static function (int $pageCount): string {
...
$putPointerMapEntry11031118 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database11031118 = static function (int $sliceNumber) use ($makeFirstPage11031118, $putPointerMapEntry11031118): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage11031118(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry11031118($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan11031118 = static function (int $sliceNumber, int $batchSize = 2) use ($database11031118): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database11031118($sliceNumber);
...
foreach (range(1103, 1118) as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} continues next1103-1118 handoff receipts"] = static function (TestRunner $t) use ($plan11031118, $sliceNumber): void {
        $plan = $plan11031118($sliceNumber);
        ...
        $t->same(6, $plan11031118($sliceNumber, 3)->currentSourceSummary()['current_source_row_count']);
    };
}
