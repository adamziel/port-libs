...
$makeFirstPage511526 = static function (int $pageCount): string {
...
$putPointerMapEntry511526 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database511526 = static function (int $sliceNumber) use ($makeFirstPage511526, $putPointerMapEntry511526): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage511526(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry511526($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan511526 = static function (int $sliceNumber, int $batchSize = 2) use ($database511526): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database511526($sliceNumber);
...
foreach ([511, 512, 513, 514, 515, 516, 517, 518, 519, 520, 521, 522, 523, 524, 525, 526] as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} publishes next511-526 handoff receipts"] = static function (TestRunner $t) use ($plan511526, $sliceNumber): void {
        $plan = $plan511526($sliceNumber);
        ...
        $t->same(6, $plan511526($sliceNumber, 3)->currentSourceSummary()['current_source_row_count']);
    };
}
