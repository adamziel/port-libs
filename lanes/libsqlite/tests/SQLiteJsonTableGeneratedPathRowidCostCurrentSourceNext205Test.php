...
): array => SQLiteJsonTablePlan::currentSourceGeneratedPathRowidCostCurrentSourceNext205(
    'json_tree',
    $current ?? $current205,
    $next ?? $next205,
    'option_value',
...
$tests = [
    ...
    'current accepted rowids' => static fn (TestRunner $t) => $t->same([8], $plan205()['currentGeneratedPathRowidAliasOrder205']['acceptedRowids']),
    'current ordered rowids' => static fn (TestRunner $t) => $t->same([8], $plan205()['currentGeneratedPathRowidAliasOrder205']['orderedRowids']),
    'current first rowid' => static fn (TestRunner $t) => $t->same(8, $plan205()['currentGeneratedPathRowidAliasOrder205']['firstOrderedRowid']),
    'current last rowid' => static fn (TestRunner $t) => $t->same(8, $plan205()['currentGeneratedPathRowidAliasOrder205']['lastOrderedRowid']),
    'current estimated rows' => static fn (TestRunner $t) => $t->same(1, $plan205()['currentGeneratedPathRowidAliasOrder205']['estimatedRows']),
    'current estimated cost has reverse penalty' => static fn (TestRunner $t) => $t->same(8, $plan205()['currentGeneratedPathRowidAliasOrder205']['estimatedCost']),
    ...
    'current cost class reverse' => static fn (TestRunner $t) => $t->same('json-table-generated-path-rowid-order-reverse-next205', $plan205()['currentGeneratedPathRowidAliasOrder205']['costClass']),
    'current ordered tape rowid' => static fn (TestRunner $t) => $t->same(8, $plan205()['currentGeneratedPathRowidAliasOrder205']['orderedAliasTape'][0]['rowid']),
    'current ordered tape fullkey' => static fn (TestRunner $t) => $t->same('$.rules[2].slug', $plan205()['currentGeneratedPathRowidAliasOrder205']['orderedAliasTape'][0]['projectedColumns']['fullkey']),
    ...
    'descending range rowids preserved' => static fn (TestRunner $t) => $t->same([6, 5], $rangeDesc205()['currentGeneratedPathRowidAliasOrder205']['orderedRowids']),
    ...
    'descending range cost' => static fn (TestRunner $t) => $t->same(15, $rangeDesc205()['currentGeneratedPathRowidAliasOrder205']['estimatedCost']),
    'ascending range rowids sorted' => static fn (TestRunner $t) => $t->same([8, 9], $rangeAsc205()['currentGeneratedPathRowidAliasOrder205']['orderedRowids']),
    ...
    'ascending range cost class' => static fn (TestRunner $t) => $t->same('json-table-generated-path-rowid-order-forward-next205', $rangeAsc205()['currentGeneratedPathRowidAliasOrder205']['costClass']),
    'single alias asc without stable checkpoint uses sorter' => static fn (TestRunner $t) => $t->same('OP_JsonTableRowidAliasOrderSorterNext205', $pointAsc205()['currentGeneratedPathRowidAliasOrder205']['orderOpcode']),
    'single alias asc sorter cost class' => static fn (TestRunner $t) => $t->same('json-table-generated-path-rowid-order-sorter-next205', $pointAsc205()['currentGeneratedPathRowidAliasOrder205']['costClass']),
...
foreach ($tests as $name => $case) {
    $tests['json table generated path rowid cost current source next205 ' . $name] = $case;
    unset($tests[$name]);
}
