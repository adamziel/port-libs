...
$plan219 = static fn (
    ...
    int $nextCookie = 219,
): array => SQLiteUtf16NocaseLikeRtrimCurrentSourceNext219Plan::wordpressOptionNameSupplementaryWildcardPlan(
...
$tests['utf16 nocase like rtrim current source next219 stable supplementary wildcard still requires character split'] = static function (TestRunner $t) use ($row219, $emoji219): void {
    ...
    $result = SQLiteUtf16NocaseLikeRtrimCurrentSourceNext219Plan::wordpressOptionNameSupplementaryWildcardPlan(
        $rows,
        $rows,
        'plugin!_cache_',
        '!',
...
$tests['utf16 nocase like rtrim current source next219 rejects malformed row shape'] = static function (TestRunner $t) use ($enc219): void {
    ...
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteUtf16NocaseLikeRtrimCurrentSourceNext219Plan::wordpressOptionNameSupplementaryWildcardPlan($rows, $rows));
};
