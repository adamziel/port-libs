...
$summary224 = static fn (?array $cursor = null): array => SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext224Plan::compare($sql224, $currentTables224, $nextTables224, $cursor);
$tests = [];
...
$tests['compound select window recursive limit current source next224 rejects missing except'] = static function (TestRunner $t) use ($currentTables224): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext224Plan::compare(
...
foreach (range(1, 60) as $case) {
    $tests['compound select window recursive limit current source next224 generated intersect except boundary ' . $case] = static function (TestRunner $t) use ($case): void {
        ...
        $plan = SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext224Plan::compare($sql, $tables, $nextTables);
        $again = SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext224Plan::compare($sql, $tables, $nextTables, $plan['cursor']);
