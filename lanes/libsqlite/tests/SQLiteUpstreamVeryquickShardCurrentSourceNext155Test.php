...
function libsqlite_suite_next155_record(
    ...
    return libsqlite_suite_next155_evidence()->upstreamVeryquickShardCurrentSourceNext155(
        $rows,
        604,
        44622,
        $launcherBase,
        ...
        $nextHead,
        'lanes/libsqlite/tests/SQLiteUpstreamVeryquickShardCurrentSourceNext155Test.php',
...
foreach (range(1, 58) as $case) {
    $tests[sprintf('current source next155 admits veryquick shard case %02d', $case)] = static function (TestRunner $t) use ($case): void {
        ...
        $t->same('current-source-next155-veryquick-shard-advanced', $record['status']);
        ...
        $t->same(true, $record['counts_upstream_veryquick_shard_current_source_next155']);
...
$tests['current source next155 preserves already counted row without mapped inflation'] = static function (TestRunner $t): void {
    ...
    $t->same('current-source-next155-veryquick-shard-preserved', $record['status']);
