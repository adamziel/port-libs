...
$base = [
    'source_id' => 'pager-reader-cache-current-source-next750',
    'epoch' => 750,
...
$plan = static fn (array $cacheExtra = [], array $readExtra = []): array => SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::variantNext750(
    $database,
    $master,
    $masterBytes,
    implode('', $before),
...
$tests['pager master journal reader cache current source next750 admits current VDBE literal and arithmetic opcode fences'] = static function (TestRunner $t) use ($plan): void {
    $result = $plan();
    $t->same('pager-master-journal-reader-cache-current-source-next750', $result['status']);
