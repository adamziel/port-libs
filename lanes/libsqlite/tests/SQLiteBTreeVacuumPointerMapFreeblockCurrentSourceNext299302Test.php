...
$makeFirstPage299302 = static function (int $pageCount): string {
...
$putPointerMapEntry299302 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database299302 = static function (int $sliceNumber) use ($makeFirstPage299302, $putPointerMapEntry299302): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage299302(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry299302($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan299302 = static function (int $sliceNumber, int $batchSize = 2) use ($database299302): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourcePlan {
...
foreach ([299, 300, 301, 302] as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} keeps next299-302 splice receipts ordered"] = static function (TestRunner $t) use ($plan299302, $sliceNumber): void {
        $plan = $plan299302($sliceNumber);
        ...
        $t->same(6, $plan299302($sliceNumber, 3)->freelistSummary()['freelist_row_count']);
    };
}
