...
$cases = [
    'next257 tombstone publication candidate' => static function (TestRunner $t) use ($examplesDir): void {
        $result = require $examplesDir . '/wordpress-rowvalue-returning-window-current-source-next257.php';
    ...
    'next258 transition token admission candidate' => static function (TestRunner $t) use ($examplesDir): void {
        $result = require $examplesDir . '/wordpress-rowvalue-returning-window-current-source-next258.php';
    ...
    'next259 current row frame acknowledgement candidate' => static function (TestRunner $t) use ($examplesDir): void {
        ob_start();
        $argv = [];
        require $examplesDir . '/wordpress-rowvalue-update-delete-returning-window-current-source-next259.php';
    ...
    'next260 mixed boundary release candidate' => static function (TestRunner $t) use ($examplesDir): void {
        ob_start();
        $result = require $examplesDir . '/wordpress-rowvalue-returning-window-current-source-next260.php';
        $output = trim((string) ob_get_clean());

        $t->same(1, $result);
        $t->same('wordpress-rowvalue-returning-window-current-source-next260 self-test passed', $output);
    },
    'combined after-current handoff' => static function (TestRunner $t) use ($examplesDir): void {
        $result = require $examplesDir . '/wordpress-rowvalue-returning-window-current-source-next257-260-after-current.php';

        $t->same('rowvalue-update-delete-returning-window-current-source-next257-260-after-current', $result['status']);
        $t->same([
            ...
            'wordpress-rowvalue-returning-window-current-source-next260 self-test passed',
        ], $result['candidateStatuses']);
        $t->same([1, 6, 2, 6, 5, 6, 4, 3, 2], $result['next257PublicationRowids']);
        $t->same(8, $result['next259ReadyCount']);
        $t->same('wordpress-rowvalue-returning-window-current-source-next260 self-test passed', $result['next260SelfTestOutput']);
    },
];

$tests = [];
foreach ($cases as $name => $callback) {
    $tests['rowvalue update delete returning window current source next257-260 after current ' . $name] = $callback;
}
