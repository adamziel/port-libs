...
$tests['compound select window recursive limit next172 status dependencies'] = static function (TestRunner $t) use ($summary172): void {
    $plan = $summary172();
    $t->same('compound-select-window-recursive-limit-current-source-next172-ready', $plan['status']);
    $t->same([
...
$tests['compound select window recursive limit next172 compound metadata'] = static function (TestRunner $t) use ($summary172): void {
...
$tests['compound select window recursive limit next172 window metadata'] = static function (TestRunner $t) use ($summary172): void {
...
$tests['compound select window recursive limit next172 recursive trace'] = static function (TestRunner $t) use ($summary172): void {
...
$tests['compound select window recursive limit next172 current rows'] = static function (TestRunner $t) use ($summary172): void {
...
$tests['compound select window recursive limit next172 next rows'] = static function (TestRunner $t) use ($summary172): void {
...
$tests['compound select window recursive limit next172 prelimit rows expose final boundary'] = static function (TestRunner $t) use ($summary172): void {
...
$tests['compound select window recursive limit next172 source classes stay mixed'] = static function (TestRunner $t) use ($summary172): void {
    $classes = $summary172()['sourceClasses'];
    $t->same(['recursive' => 3, 'table' => 3], $classes['current']);
    $t->same(['recursive' => 3, 'table' => 3], $classes['next']);
};

$tests['compound select window recursive limit next172 boundary delta'] = static function (TestRunner $t) use ($summary172): void {
...
$tests['compound select window recursive limit next172 changed reasons'] = static function (TestRunner $t) use ($summary172): void {
    $plan = $summary172();
...
foreach (range(1, 52) as $case) {
    $tests['compound select window recursive limit next172 generated boundary ' . $case] = static function (TestRunner $t) use ($case): void {
