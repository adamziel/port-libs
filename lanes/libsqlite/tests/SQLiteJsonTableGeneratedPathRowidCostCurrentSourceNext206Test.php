...
): array => SQLiteJsonTablePlan::currentSourceGeneratedPathRowidCostCurrentSourceNext206(
    'json_tree',
    $current ?? $current206,
    $next ?? $next206,
    'option_value',
...
$tests = [
    ...
    'current accepted rowids from checkpoint' => static fn (TestRunner $t) => $t->same([8, 7], $plan206()['currentGeneratedPathRowidAliasOrder206']['acceptedRowids']),
    'current ordered rowids desc' => static fn (TestRunner $t) => $t->same([8, 7], $plan206()['currentGeneratedPathRowidAliasOrder206']['orderedRowids']),
    ...
    'current order tape first rowid' => static fn (TestRunner $t) => $t->same(8, $plan206()['currentGeneratedPathRowidAliasOrder206']['orderTape'][0]['rowid']),
    ...
    'current estimated rows' => static fn (TestRunner $t) => $t->same(2, $plan206()['currentGeneratedPathRowidAliasOrder206']['estimatedRows']),
    'current estimated cost inherited' => static fn (TestRunner $t) => $t->same(14, $plan206()['currentGeneratedPathRowidAliasOrder206']['estimatedCost']),
    ...
    'ascending alias rowids sorted asc' => static fn (TestRunner $t) => $t->same([8, 9], $ascending206()['currentGeneratedPathRowidAliasOrder206']['orderedRowids']),
    ...
    'bad function rejected' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteJsonTablePlan::currentSourceGeneratedPathRowidCostCurrentSourceNext206('json_bad', $current206, $current206, 'option_value', 'generated_path')),
    'dependency closure' => static fn (TestRunner $t) => $t->same('no-new-support-component', 'no-new-support-component'),
];

foreach ($tests as $name => $case) {
    $tests['json table generated path rowid cost current source next206 ' . $name] = $case;
    unset($tests[$name]);
}
