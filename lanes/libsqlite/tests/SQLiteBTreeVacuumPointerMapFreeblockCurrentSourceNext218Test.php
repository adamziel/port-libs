...
use PortLibs\LibSqlite\SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNext218Plan;
...
$database218 = static function () use ($makeFirstPage218, $putPointerMapEntry218): SQLiteDatabase {
    ...
    $pages[3] = SQLiteTableLeafPage::assemble([
        ...
        SQLiteTableLeafCell::encode(2, SQLiteRecord::encode([null, '_transient_next218', str_repeat('cache:', 42)])),
...
$plan218 = static function (int $batchSize = 2): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNext218Plan {
    global $database218;

    $database = $database218();
    $deletedPage = SQLiteTableLeafPage::deleteCellByRowId($database->page(3), 2, secureDelete: true);

    return SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNext218Plan::tableLeafFromDeleteResult(
        $database,
        3,
        [
            'page' => $deletedPage,
...
$cases218 = [
    ...
    'current source token length' => static fn (): mixed => strlen($plan218()->writeSummary()['current_source_next218_token']),
...
$expected218 = [
    'action label' => 'btree-vacuum-pointermap-freeblock-current-source-next218',
    'summary status' => 'btree-vacuum-pointermap-freeblock-current-source-next218-ready',
    'write row count' => 7,
    ...
    'dependency closure' => 'no new support component needed; next218 reuses next212 current-source apply rows and adds per-page write receipts only',
    'non overlap' => true,
...
foreach ($cases218 as $name => $callback) {
    $tests['btree vacuum pointermap freeblock current source next218 ' . $name] = static function (TestRunner $t) use ($callback, $expected218, $name): void {
        $t->same($expected218[$name], $callback());
    };
}

foreach (range(1, 80) as $index) {
    $tests['btree vacuum pointermap freeblock current source next218 write invariant ' . $index] = static function (TestRunner $t) use ($plan218): void {
        $plan = $plan218();
        ...
        $t->same('btree-vacuum-pointermap-freeblock-current-source-next218-ready', $summary['status']);
