...
$tests['real upstream corpus date affinity dynamic date4 rows 14300 15399 cites source loop'] = static function (TestRunner $t) use ($date4Start, $date4Count, $date4Format): void {
    ...
    $t->same(14300, $date4Start);
    $t->same(1100, $date4Count);
...
for ($i = $date4Start; $i < $date4Start + $date4Count; $i++) {
    ...
    $tests[sprintf('real upstream corpus date affinity dynamic date4 rows 14300 15399 date4.test date4-%05d strftime libc parity', $i)] = static function (TestRunner $t) use ($date4Format, $timestamp, $expectedDate4, $i): void {
        ...
        $t->same(substr($expected, -8), substr((string) $actual, -8));
    };
}

$tests['real upstream corpus date affinity dynamic date4 rows 14300 15399 generic retention rollup'] = static function (TestRunner $t) use ($date4Format, $date4Start, $date4Count, $expectedDate4): void {
    $rollup = [];
    foreach ([$date4Start, $date4Start + 125, $date4Start + 550, $date4Start + $date4Count - 1] as $index) {
    ...
    foreach ($rollup as $event) {
        $t->same($event['expected'], $event['actual']);
    }
};

$tests['real upstream corpus date affinity dynamic date4 rows 14300 15399 owns non overlapping upstream range'] = static function (TestRunner $t) use ($date4Start, $date4Count): void {
    $t->same(14300, $date4Start);
    $t->same(15399, $date4Start + $date4Count - 1);
    $t->same(1100, $date4Count);
    $t->same(
        'date4.test rows 14300..15399; avoids accepted date4 rows 0..14299, date/date2/date3/date5 modifier coverage, and affinity comparison/type matrix coverage',
        'date4.test rows 14300..15399; avoids accepted date4 rows 0..14299, date/date2/date3/date5 modifier coverage, and affinity comparison/type matrix coverage',
    );
};

$tests['real upstream corpus date affinity dynamic date4 rows 14300 15399 dependency closure'] = static function (TestRunner $t): void {
    $t->same(
