...
$tests['compound select window recursive limit current source next229 status dependencies'] = static function (TestRunner $t) use ($summary229): void {
    $plan = $summary229();
    $t->same('compound-select-window-recursive-limit-current-source-next229-ready', $plan['status']);
    $t->same([
...
$tests['compound select window recursive limit current source next229 compound metadata'] = static function (TestRunner $t) use ($summary229): void {
...
$tests['compound select window recursive limit current source next229 current rows'] = static function (TestRunner $t) use ($summary229): void {
    $rows = $summary229()['currentRows'];
    $t->same(['home', 'seed:2:3:4', 'rewrite_rules'], array_column($rows, 'label'));
    $t->same([2, 2, 3], array_column($rows, 'rn'));
};

$tests['compound select window recursive limit current source next229 next rows shift except page'] = static function (TestRunner $t) use ($summary229): void {
    $plan = $summary229();
...
$tests['compound select window recursive limit current source next229 recursive queue limit'] = static function (TestRunner $t) use ($summary229): void {
...
$tests['compound select window recursive limit current source next229 window metadata'] = static function (TestRunner $t) use ($summary229): void {
...
$tests['compound select window recursive limit current source next229 source window trace'] = static function (TestRunner $t) use ($summary229): void {
...
$tests['compound select window recursive limit current source next229 cursor token fence'] = static function (TestRunner $t) use ($summary229): void {
    $first = $summary229();
...
$tests['compound select window recursive limit current source next229 rejects stale cursor'] = static function (TestRunner $t) use ($summary229): void {
...
$tests['compound select window recursive limit current source next229 executor parity'] = static function (TestRunner $t) use ($sql229, $currentTables229, $summary229): void {
    $t->same(SQLiteSelectSql::execute($sql229, $currentTables229), $summary229()['currentRows']);
};

$tests['compound select window recursive limit current source next229 replan reasons'] = static function (TestRunner $t) use ($summary229): void {
    $plan = $summary229();
    $t->contains('does not repeat next224', $plan['non_overlap']);
    $t->true(in_array('compound-union-distinct-except-dense-rank-current-source-next229', $plan['replanReasons'], true));
    $t->true(in_array('recursive-limit-offset-before-union-except-window-next229', $plan['replanReasons'], true));
    $t->true(in_array('dense-rank-shift-changes-except-final-limit-page-next229', $plan['replanReasons'], true));
};
...
foreach (range(1, 60) as $case) {
    $tests['compound select window recursive limit current source next229 generated intersect except boundary ' . $case] = static function (TestRunner $t) use ($case): void {
        $tables = [
            'wp_options' => [
