...
$plan193 = static fn (array $options = []): array => SQLiteTriggerRecursiveViewReturningCurrentSourceNextPlan::executeSealedNextSourcePublication(
    ...
    $options + [
        ...
        'current_generation' => 'app-current-returning-193',
        'next_generation' => 'app-next-returning-193',
        'page_size' => 3,
        'admit_next_source' => true,
        'current_source_token' => 'app.current.source.193',
        'drain_ack_token' => 'app.returning.drain.193',
        'rollback_token' => 'app.rollback.current.193',
...
$handoffHeld193 = static fn (): array => $plan193(['expected_handoff_token' => 'app.recursive.view.returning.expected.193']);
$sequenceHeld193 = static fn (): array => $plan193(['expected_source_sequence_token' => 'seq-different193']);
$tokenHeld193 = static fn (): array => $plan193(['expected_next_source_token' => 'app.next.source.expected.193']);
$custom193 = static fn (): array => $plan193(['handoff_token' => 'app.recursive.view.returning.custom.193', 'expected_handoff_token' => 'app.recursive.view.returning.custom.193']);
...
$cases193 = [
    ...
    'savepoint retained' => [static fn (): mixed => $published193()['savepoint'], 'app_recursive_view_193'],
    ...
    'handoff token retained' => [static fn (): mixed => $published193()['handoff_token_next193'], 'app.recursive.view.returning.handoff.193'],
    'expected handoff token retained' => [static fn (): mixed => $published193()['expected_handoff_token_next193'], 'app.recursive.view.returning.handoff.193'],
    'custom handoff token retained' => [static fn (): mixed => $custom193()['handoff_token_next193'], 'app.recursive.view.returning.custom.193'],
    ...
    'published names' => [static fn (): mixed => array_column($published193()['published_next_source_payloads_next193'], 'name'), ['landing_url', 'next_module']],
    ...
    'handoff token stamped' => [static fn (): mixed => array_values(array_unique(array_column($published193()['published_next_source_rows_next193'], 'handoff_token_next193'))), ['app.recursive.view.returning.handoff.193']],
    ...
    'next189 cursor retained' => [static fn (): mixed => array_values(array_unique(array_column($published193()['published_next_source_rows_next193'], 'next_cursor_next189'))), ['app.returning.next.cursor.193']],
    'next189 token retained' => [static fn (): mixed => array_values(array_unique(array_column($published193()['published_next_source_rows_next193'], 'next_source_token_next189'))), ['app.next.source.193']],
    'next189 generation retained' => [static fn (): mixed => array_values(array_unique(array_column($published193()['published_next_source_rows_next193'], 'next_generation_next189'))), ['app-next-returning-193']],
