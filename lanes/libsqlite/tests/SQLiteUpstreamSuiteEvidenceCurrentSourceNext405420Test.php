...
function libsqlite_suite_next405420_evidence(): SQLiteUpstreamSuiteEvidence
{
    return SQLiteUpstreamSuiteEvidence::fromManifestPath(__DIR__ . '/../UPSTREAM_TEST_MANIFEST.json');
}

function libsqlite_suite_next405420_output(int $passLines = 44, int $assertions = 96, int $failures = 0): string
{
 ...
 */
function libsqlite_suite_next405420_rows(
    int $case = 1,
 ...
 */
function libsqlite_suite_next405420_record(
    array $rows,
    string $launcherBase = '8035502138608bf3cb7ee64d1b4686a5bb9f5ec6',
    string $integrationSource = '8035502138608bf3cb7ee64d1b4686a5bb9f5ec6',
    ?string $output = null,
    ...
    return libsqlite_suite_next405420_evidence()->upstreamVeryquickShardCurrentSourceNext405420(
        $rows,
        637,
        42276,
        $launcherBase,
        $integrationSource,
        'lanes/libsqlite/tests/SQLiteUpstreamSuiteEvidenceCurrentSourceNext405420Test.php',
        $output ?? libsqlite_suite_next405420_output(),
...
foreach (range(1, 16) as $case) {
    $tests[sprintf('current source next405-420 prepares suite evidence case %02d', $case)] = static function (TestRunner $t) use ($case): void {
        $record = libsqlite_suite_next405420_record(libsqlite_suite_next405420_rows($case));
...
$tests['current source next405-420 blocks missing slice'] = static function (TestRunner $t): void {
    $rows = libsqlite_suite_next405420_rows();
    unset($rows[1]);
    $rows = array_values($rows);

    $record = libsqlite_suite_next405420_record($rows);
...
$tests['current source next405-420 blocks runner errors and active broad runner'] = static function (TestRunner $t): void {
    ...
    $record = libsqlite_suite_next405420_record($rows, snapshot: "777 testfixture ../libsqlite/test/testrunner.tcl --jobs 4 all\n");
    $ids = implode(',', array_column($record['blockers'], 'id'));
...
$tests['current source next405-420 blocks focused pass mismatch'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next405420_record(
        libsqlite_suite_next405420_rows(),
        output: libsqlite_suite_next405420_output(assertions: 83),
        expected: 96
    );
...
$tests['current source next405-420 records next gate and dependency closure'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next405420_record(libsqlite_suite_next405420_rows(8));
