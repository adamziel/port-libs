...
use PortLibs\LibSqlite\SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext235Plan;
...
$summary235 = static fn (?array $cursor = null): array => SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext235Plan::compare($sql235, $currentTables235, $nextTables235, $cursor);
$tests = [];
...
foreach (range(1, 62) as $case) {
    $tests['compound select window recursive limit current source next235 generated promotion barrier ' . $case] = static function (TestRunner $t) use ($case): void {
        ...
        $plan = SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext235Plan::compare($sql, $tables, $nextTables);
        $cursor = $plan['cursor'];
        $cursor['acknowledgedCurrentAcksNext232'] = $cursor['requiredCurrentAcksNext232'];
        $cursor['acknowledgedPromotionAcksNext235'] = $cursor['requiredPromotionAcksNext235'];
        $again = SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext235Plan::compare($sql, $tables, $nextTables, $cursor);
