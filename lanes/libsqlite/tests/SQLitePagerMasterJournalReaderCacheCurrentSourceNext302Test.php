...
$pageSize = 512;
$database = '/srv/wp-content/database/wp-next302.sqlite';
$usersDatabase = '/srv/wp-content/database/wp-next302-users.sqlite';
...
$before = [1 => $formatPage('next302 stale schema'), 2 => $page('next302 stale options'), 3 => $page('next302 stale plugins'), 4 => $page('next302 stale users')];
$recovered = [1 => $formatPage('next302 current schema'), 2 => $page('next302 current options'), 3 => $page('next302 current plugins'), 4 => $page('next302 current users')];
$tokens = [$mainJournal => 'member-main-current-302', $usersJournal => 'member-users-current-302'];
$headers = [$mainJournal => hash('sha256', 'main header next302'), $usersJournal => hash('sha256', 'users header next302')];
$base = [
    'source_id' => 'pager-reader-cache-current-source-next302',
    'epoch' => 302,
    'format_signature' => hash('sha256', implode('|', [512, 4, 2, 302, 0])),
    'publication_generation' => 302,
    'master_source_digest' => hash('sha256', 'master-next302'),
...
$plan = static function (?array $cache = null, ?array $reads = null) use ($database, $master, $masterBytes, $before, $pageSize, $recovered, $base, $tokens, $headers, $cacheEntry, $read): array {
    return SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::variantNext302($database, $master, $masterBytes, implode('', $before), $pageSize, $recovered, $cache ?? [
...
$cases = [
    'status' => [static fn (): mixed => $plan()['status'], 'pager-master-journal-reader-cache-current-source-next302'],
    ...
    'dependencies include next302' => [static fn (): mixed => in_array('sqlite-pager-master-journal-reader-cache-current-source-next302', $plan()['dependencies'], true), true],
    ...
    'default plan points at next302' => [static fn (): mixed => $plan()['status'], 'pager-master-journal-reader-cache-current-source-next302'],
];

foreach ($cases as $name => [$callback, $expected]) {
    $tests['pager master journal reader cache current source next302 ' . $name] = static function (TestRunner $t) use ($callback, $expected): void {
        $t->same($expected, $callback());
    };
}

$tests['pager master journal reader cache current source next302 missing token rejects'] = static function (TestRunner $t) use ($plan, $cacheEntry, $recovered): void {
