...
$page = static fn (int $offset = 0, int $limit = 122, ?array $cursor = null, string $sql = 'PRAGMA foreign_key_check'): array => SQLitePragmaRootpagePointerMapForeignKeyCurrentSourceNext122::page($database, $schemas(), $catalog, $sql, $offset, $limit, $cursor);
$clean = static fn (): array => SQLitePragmaRootpagePointerMapForeignKeyCurrentSourceNext122::page($cleanDatabase, $schemas(), $catalog);
$missing = static fn (): array => SQLitePragmaRootpagePointerMapForeignKeyCurrentSourceNext122::page($cleanDatabase, $schemas(), $missingCatalog, "PRAGMA foreign_key_check('wp_options')");
...
$tests['pragma rootpage pointermap fk current source next122 accepts table valued pragma'] = static function (TestRunner $t) use ($database, $schemas, $catalog): void {
    $result = SQLitePragmaRootpagePointerMapForeignKeyCurrentSourceNext122::page($database, $schemas(), $catalog, "SELECT * FROM pragma_foreign_key_check('wp_term_taxonomy')");
...
$tests['pragma rootpage pointermap fk current source next122 source changes with database'] = static function (TestRunner $t) use ($database, $cleanDatabase, $schemas, $catalog): void {
    $dirty = SQLitePragmaRootpagePointerMapForeignKeyCurrentSourceNext122::page($database, $schemas(), $catalog);
    $clean = SQLitePragmaRootpagePointerMapForeignKeyCurrentSourceNext122::page($cleanDatabase, $schemas(), $catalog);
...
$tests['pragma rootpage pointermap fk current source next122 source changes with catalog'] = static function (TestRunner $t) use ($cleanDatabase, $schemas, $catalog, $missingCatalog): void {
    $full = SQLitePragmaRootpagePointerMapForeignKeyCurrentSourceNext122::page($cleanDatabase, $schemas(), $catalog);
    $missing = SQLitePragmaRootpagePointerMapForeignKeyCurrentSourceNext122::page($cleanDatabase, $schemas(), $missingCatalog, "PRAGMA foreign_key_check('wp_options')");
...
$tests['pragma rootpage pointermap fk current source next122 source changes with schemas'] = static function (TestRunner $t) use ($cleanDatabase, $schemas, $catalog): void {
    $three = SQLitePragmaRootpagePointerMapForeignKeyCurrentSourceNext122::page($cleanDatabase, $schemas(3), $catalog);
    $four = SQLitePragmaRootpagePointerMapForeignKeyCurrentSourceNext122::page($cleanDatabase, $schemas(4), $catalog);
...
$tests['pragma rootpage pointermap fk current source next122 rejects stale source cursor'] = static function (TestRunner $t) use ($page, $cleanDatabase, $schemas, $catalog): void {
    $first = $page(0, 2);
    $t->throws(InvalidArgumentException::class, static fn () => SQLitePragmaRootpagePointerMapForeignKeyCurrentSourceNext122::page($cleanDatabase, $schemas(), $catalog, 'PRAGMA foreign_key_check', 2, 2, ['source_id' => $first['source_id'], 'next_offset' => 2]));
};
...
$tests['pragma rootpage pointermap fk current source next122 rejects negative offset'] = static function (TestRunner $t) use ($database, $schemas, $catalog): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLitePragmaRootpagePointerMapForeignKeyCurrentSourceNext122::page($database, $schemas(), $catalog, 'PRAGMA foreign_key_check', -1, 122));
};
...
$tests['pragma rootpage pointermap fk current source next122 rejects zero limit'] = static function (TestRunner $t) use ($database, $schemas, $catalog): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLitePragmaRootpagePointerMapForeignKeyCurrentSourceNext122::page($database, $schemas(), $catalog, 'PRAGMA foreign_key_check', 0, 0));
};
