...
$makeFirstPage327330 = static function (int $pageCount): string {
...
$putPointerMapEntry327330 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database327330 = static function (int $sliceNumber) use ($makeFirstPage327330, $putPointerMapEntry327330): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage327330(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry327330($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan327330 = static function (int $sliceNumber, int $batchSize = 2) use ($database327330): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database327330($sliceNumber);
...
foreach ([327, 328, 329, 330] as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} keeps final next327-330 freelist splice isolated"] = static function (TestRunner $t) use ($plan327330, $sliceNumber): void {
        $plan = $plan327330($sliceNumber);
        ...
        $t->same(6, $plan327330($sliceNumber, 3)->freelistSummary()['freelist_row_count']);
    };
}
