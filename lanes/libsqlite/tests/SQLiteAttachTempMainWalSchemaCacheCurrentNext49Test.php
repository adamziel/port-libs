...
foreach ($pathCases as $name => [$path, $expected]) {
    $tests['attach temp main wal schema cache current next49 ' . $name] = static function (TestRunner $t) use ($plan, $value, $path, $expected): void {
        $t->same($expected, $value($plan(), $path));
    };
}

$tests['attach temp main wal schema cache current next49 qualified table paths are keyed by original SQL text'] = static function (TestRunner $t) use ($plan): void {
    $result = $plan();
...
foreach ($predicateCases as $name => $predicate) {
    $tests['attach temp main wal schema cache current next49 ' . $name] = static function (TestRunner $t) use ($predicate): void {
...
foreach ($errorCases as $name => $callback) {
    $tests['attach temp main wal schema cache current next49 ' . $name] = static function (TestRunner $t) use ($callback): void {
