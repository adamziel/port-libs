...
$summary230 = static fn (?array $cursor = null): array => SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNextPlan::compareUnionIntersectExceptWindowLimit($sql230, $currentTables230, $nextTables230, $cursor);
$tests = [];

$tests['compound select window recursive limit current source union-intersect-except-window-limit status dependencies'] = static function (TestRunner $t) use ($summary230): void {
    $plan = $summary230();
    $t->same('compound-select-window-recursive-limit-current-source-union-intersect-except-window-limit-ready', $plan['status']);
    $t->same([
        'sqlite-select-sql-recursive-queue-order-limit-union-intersect-except-window-limit',
        'sqlite-select-sql-avg-first-value-window-union-intersect-except-window-limit',
...
$tests['compound select window recursive limit current source union-intersect-except-window-limit compound metadata'] = static function (TestRunner $t) use ($summary230): void {
...
$tests['compound select window recursive limit current source union-intersect-except-window-limit avg window rows'] = static function (TestRunner $t) use ($summary230): void {
...
$tests['compound select window recursive limit current source union-intersect-except-window-limit next boundary'] = static function (TestRunner $t) use ($summary230): void {
    $plan = $summary230();
...
$tests['compound select window recursive limit current source union-intersect-except-window-limit recursive queue trace'] = static function (TestRunner $t) use ($summary230): void {
...
$tests['compound select window recursive limit current source union-intersect-except-window-limit window shape'] = static function (TestRunner $t) use ($summary230): void {
...
$tests['compound select window recursive limit current source union-intersect-except-window-limit token fence'] = static function (TestRunner $t) use ($summary230): void {
    $first = $summary230();
...
$tests['compound select window recursive limit current source union-intersect-except-window-limit rejects stale cursor'] = static function (TestRunner $t) use ($summary230): void {
...
$tests['compound select window recursive limit current source union-intersect-except-window-limit limit trace'] = static function (TestRunner $t) use ($summary230): void {
...
$tests['compound select window recursive limit current source union-intersect-except-window-limit replan reasons'] = static function (TestRunner $t) use ($summary230): void {
    $plan = $summary230();
    $t->contains('avoids accepted next226', $plan['non_overlap']);
    $t->true(in_array('compound-avg-first-value-union-distinct-current-source-union-intersect-except-window-limit', $plan['replanReasons'], true));
    $t->true(in_array('recursive-ordered-limit-before-avg-window-union-intersect-except-window-limit', $plan['replanReasons'], true));
    $t->true(in_array('intersect-except-after-window-output-union-intersect-except-window-limit', $plan['replanReasons'], true));
};

$tests['compound select window recursive limit current source union-intersect-except-window-limit rows match executor'] = static function (TestRunner $t) use ($sql230, $currentTables230, $summary230): void {
    $t->same(SQLiteSelectSql::execute($sql230, $currentTables230), $summary230()['currentRows']);
};

$tests['compound select window recursive limit current source union-intersect-except-window-limit rejects missing avg'] = static function (TestRunner $t) use ($currentTables230): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNextPlan::compareUnionIntersectExceptWindowLimit(
...
foreach (range(1, 50) as $case) {
    $tests['compound select window recursive limit current source union-intersect-except-window-limit generated avg first value boundary ' . $case] = static function (TestRunner $t) use ($case): void {
        $finalLimit = 4 + ($case % 3);
        $tables = [
            'wp_options' => [
        ...
        $plan = SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNextPlan::compareUnionIntersectExceptWindowLimit($sql, $tables, $nextTables);
        $again = SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNextPlan::compareUnionIntersectExceptWindowLimit($sql, $tables, $nextTables, $plan['cursor']);
