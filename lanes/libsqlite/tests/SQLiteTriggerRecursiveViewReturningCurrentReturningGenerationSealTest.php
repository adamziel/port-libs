...
$plan229 = static fn (array $options = []): array => SQLiteTriggerRecursiveViewReturningCurrentSourceNextPlan::executeCurrentReturningGenerationSeal(
    ...
    $options + [
        ...
        'reset_generation' => 'app-current-reset-229',
        'post_reset_current_source_token' => 'app.current.source.postreset.229',
        'post_reset_cursor' => 'app.returning.postreset.cursor.229',
...
$sourceHeld229 = static fn (): array => $plan229(['auto_ack_current_returning_generation_seals' => true, 'expected_current_returning_source_generation' => 'app.current.returning.source.generation.stale.229']);
...
$custom229 = static fn (): array => $plan229([
    ...
    'current_returning_source_generation' => 'app.current.returning.source.generation.custom.229',
...
$cases229 = [
    ...
    'savepoint retained' => [static fn (): mixed => $released229()['savepoint'], 'app_recursive_view_229'],
    'base source_seal released' => [static fn (): mixed => $released229()['base']['status_source_seal'], 'trigger-recursive-view-returning-current-source-source_seal-source-released'],
    ...
    'source generation retained' => [static fn (): mixed => $released229()['current_returning_source_generation'], 'app.current.returning.source.generation.229'],
    'custom source generation retained' => [static fn (): mixed => $custom229()['current_returning_source_generation'], 'app.current.returning.source.generation.custom.229'],
    'expected source generation defaults' => [static fn (): mixed => $released229()['expected_current_returning_source_generation'], 'app.current.returning.source.generation.229'],
    ...
    'current source generation stamped' => [static fn (): mixed => array_values(array_unique(array_column($released229()['current_source_rows_generation_seal'], 'current_returning_source_generation'))), ['app.current.returning.source.generation.229']],
    'next source generation stamped' => [static fn (): mixed => array_values(array_unique(array_column($released229()['attempted_next_source_rows_generation_seal'], 'current_returning_source_generation'))), ['app.current.returning.source.generation.229']],
    ...
    'visible payload names released' => [static fn (): mixed => array_column($released229()['visible_returning_payloads_generation_seal'], 'name'), ['app_summary_child', 'template_child', 'landing_url', 'next_module']],
    'held payload names missing' => [static fn (): mixed => array_column($missing229()['held_next_returning_payloads_generation_seal'], 'name'), ['landing_url', 'next_module']],
    ...
    'dependency includes source_seal' => [static fn (): mixed => in_array('sqlite-trigger-recursive-view-returning-current-source-source_seal', $released229()['dependencies_generation_seal'], true), true],
    'non overlap mentions source_seal' => [static fn (): mixed => str_contains($released229()['non_overlap_generation_seal'], 'source_seal source seals'), true],
