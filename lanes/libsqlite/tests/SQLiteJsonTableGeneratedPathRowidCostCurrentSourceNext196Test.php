...
): array => SQLiteJsonTablePlan::currentSourceGeneratedPathRowidCostCurrentSourceNext196(
    'json_tree',
    $current ?? $current196,
    $next ?? $next196,
    'option_value',
...
$tests = [
    ...
    'current checkpoint rowids' => static fn (TestRunner $t) => $t->same([8], $plan196()['currentGeneratedPathRowidXColumnCache196']['checkpointRowids']),
    'current accepted rowids' => static fn (TestRunner $t) => $t->same([8], $plan196()['currentGeneratedPathRowidXColumnCache196']['acceptedRowids']),
    ...
    'current estimated rows' => static fn (TestRunner $t) => $t->same(1, $plan196()['currentGeneratedPathRowidXColumnCache196']['estimatedRows']),
    'current estimated cost uses projection width' => static fn (TestRunner $t) => $t->same(6, $plan196()['currentGeneratedPathRowidXColumnCache196']['estimatedCost']),
    'current opcode point' => static fn (TestRunner $t) => $t->same('OP_JsonTableXColumnCachePointNext196', $plan196()['currentGeneratedPathRowidXColumnCache196']['xColumnOpcode']),
    'current cost point' => static fn (TestRunner $t) => $t->same('json-table-generated-path-rowid-xcolumn-cache-point-next196', $plan196()['currentGeneratedPathRowidXColumnCache196']['costClass']),
    ...
    'current tape rowid' => static fn (TestRunner $t) => $t->same(8, $plan196()['currentGeneratedPathRowidXColumnCache196']['xColumnTape'][0]['rowid']),
    ...
    'current tape value' => static fn (TestRunner $t) => $t->same('forms', $plan196()['currentGeneratedPathRowidXColumnCache196']['xColumnTape'][0]['columns']['value']),
    'current tape type' => static fn (TestRunner $t) => $t->same('text', $plan196()['currentGeneratedPathRowidXColumnCache196']['xColumnTape'][0]['columns']['type']),
    'current tape fullkey' => static fn (TestRunner $t) => $t->same('$.rules[2].slug', $plan196()['currentGeneratedPathRowidXColumnCache196']['xColumnTape'][0]['columns']['fullkey']),
    ...
    'range accepted rowids' => static fn (TestRunner $t) => $t->same([6, 5], $range196()['currentGeneratedPathRowidXColumnCache196']['acceptedRowids']),
    ...
    'range estimated cost' => static fn (TestRunner $t) => $t->same(12, $range196()['currentGeneratedPathRowidXColumnCache196']['estimatedCost']),
    ...
    'narrow projection cost' => static fn (TestRunner $t) => $t->same(3, $narrowProjection196()['currentGeneratedPathRowidXColumnCache196']['estimatedCost']),
    'narrow projection atom preserves scalar text' => static fn (TestRunner $t) => $t->same('forms', $narrowProjection196()['currentGeneratedPathRowidXColumnCache196']['xColumnTape'][0]['columns']['atom']),
    ...
    'generation-only change preserves checkpoint rowid' => static fn (TestRunner $t) => $t->same([8], $generationOnly196()['nextGeneratedPathRowidXColumnCache196']['checkpointRowids']),
...
foreach ($tests as $name => $case) {
    $tests['json table generated path rowid cost current source next196 ' . $name] = $case;
    unset($tests[$name]);
}
