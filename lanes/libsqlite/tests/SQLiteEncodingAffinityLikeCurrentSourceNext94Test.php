...
): array => SQLiteEncodingAffinityLikeCurrentSourceNextPlan::wordpressOptionValuePlan(
    $currentRows,
    $nextRows,
    'option_value',
    $pattern,
...
$tests['encoding affinity like current source next94 rejects unsupported operator'] = static function (TestRunner $t) use ($currentRows, $nextRows): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteEncodingAffinityLikeCurrentSourceNextPlan::wordpressOptionValuePlan($currentRows, $nextRows, 'option_value', 'plugin%', 'REGEXP'));
};
...
$tests['encoding affinity like current source next94 rejects glob escape'] = static function (TestRunner $t) use ($currentRows, $nextRows): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteEncodingAffinityLikeCurrentSourceNextPlan::wordpressOptionValuePlan($currentRows, $nextRows, 'option_value', 'plugin_*', 'GLOB', 'BINARY', '\\'));
};

$tests['encoding affinity like current source next94 rejects missing column'] = static function (TestRunner $t) use ($nextRows): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteEncodingAffinityLikeCurrentSourceNextPlan::wordpressOptionValuePlan([['option_id' => 1, 'option_name' => 'siteurl']], $nextRows, 'option_value', 's%'));
};
...
$tests['encoding affinity like current source next94 rejects malformed utf8 text before encoding'] = static function (TestRunner $t) use ($nextRows): void {
    ...
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteEncodingAffinityLikeCurrentSourceNextPlan::wordpressOptionValuePlan($current, $nextRows, 'option_value', 'plugin%'));
};
...
$tests['encoding affinity like current source next94 rejects unsupported encoding'] = static function (TestRunner $t) use ($currentRows, $nextRows): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteEncodingAffinityLikeCurrentSourceNextPlan::wordpressOptionValuePlan($currentRows, $nextRows, 'option_value', 'plugin%', 'LIKE', 'NOCASE', null, false, 4));
};
