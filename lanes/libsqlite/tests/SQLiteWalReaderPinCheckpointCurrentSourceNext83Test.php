...
$restart = static fn (): array => $wal->checkpointReaderPinRestartCurrentSourceNext83(
    $databaseBytes,
    $currentShm,
    $nextReaderShm,
    $currentReleasedShm,
...
$truncate = static fn (): array => $wal->checkpointReaderPinRestartCurrentSourceNext83(
    $databaseBytes,
    $currentShm,
    $nextReaderShm,
    $currentReleasedShm,
...
$tests['wal reader pin checkpoint current source next83 rejects empty page list'] = static function (TestRunner $t) use ($wal, $databaseBytes, $currentShm, $nextReaderShm, $currentReleasedShm, $allReleasedShm): void {
    $t->throws(InvalidArgumentException::class, static fn (): mixed => $wal->checkpointReaderPinRestartCurrentSourceNext83($databaseBytes, $currentShm, $nextReaderShm, $currentReleasedShm, $allReleasedShm, []));
};
...
$tests['wal reader pin checkpoint current source next83 rejects non integer page'] = static function (TestRunner $t) use ($wal, $databaseBytes, $currentShm, $nextReaderShm, $currentReleasedShm, $allReleasedShm): void {
    $t->throws(InvalidArgumentException::class, static fn (): mixed => $wal->checkpointReaderPinRestartCurrentSourceNext83($databaseBytes, $currentShm, $nextReaderShm, $currentReleasedShm, $allReleasedShm, ['2']));
};
...
$tests['wal reader pin checkpoint current source next83 rejects passive mode'] = static function (TestRunner $t) use ($wal, $databaseBytes, $currentShm, $nextReaderShm, $currentReleasedShm, $allReleasedShm): void {
    $t->throws(InvalidArgumentException::class, static fn (): mixed => $wal->checkpointReaderPinRestartCurrentSourceNext83($databaseBytes, $currentShm, $nextReaderShm, $currentReleasedShm, $allReleasedShm, [2], 'passive'));
};
