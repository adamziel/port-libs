...
$pageSize = 512;
$database = '/srv/wp-content/database/wp-next310.sqlite';
$usersDatabase = '/srv/wp-content/database/wp-next310-users.sqlite';
...
$formatPage = static function (string $label) use ($pageSize): string {
    ...
    $page = substr_replace($page, pack('N', 310), 60, 4);
...
$before = [1 => $formatPage('next310 stale schema'), 2 => $page('next310 stale options'), 3 => $page('next310 stale plugins'), 4 => $page('next310 stale users')];
$recovered = [1 => $formatPage('next310 current schema'), 2 => $page('next310 current options'), 3 => $page('next310 current plugins'), 4 => $page('next310 current users')];
$tokens = [$mainJournal => 'member-main-current-310', $usersJournal => 'member-users-current-310'];
$headers = [$mainJournal => hash('sha256', 'main header next310'), $usersJournal => hash('sha256', 'users header next310')];
$base = [
    'source_id' => 'pager-reader-cache-current-source-next310',
    'epoch' => 310,
    'format_signature' => hash('sha256', implode('|', [512, 4, 2, 310, 0])),
    'publication_generation' => 310,
    'master_source_digest' => hash('sha256', 'master-next310'),
    'recovery_sequence' => 310,
    ...
    'reader_cache_ignore_check_constraints_token' => 'ignore-check-constraints-current-310',
];
...
$plan = static function (?array $cache = null, ?array $reads = null) use ($database, $master, $masterBytes, $before, $pageSize, $recovered, $base, $tokens, $headers, $cacheEntry, $read): array {
    return SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::currentSourceReaderCacheIgnoreCheckConstraintsFence($database, $master, $masterBytes, implode('', $before), $pageSize, $recovered, $cache ?? [
...
$cases = [
    'status' => [static fn (): mixed => $plan()['status'], 'pager-master-journal-reader-cache-current-source-next310'],
    ...
    'dependencies include next310' => [static fn (): mixed => in_array('sqlite-pager-master-journal-reader-cache-current-source-next310', $plan()['dependencies'], true), true],
    ...
    'default plan points at next310' => [static fn (): mixed => $plan()['status'], 'pager-master-journal-reader-cache-current-source-next310'],
];

foreach ($cases as $name => [$callback, $expected]) {
    $tests['pager master journal reader cache current source next310 ' . $name] = static function (TestRunner $t) use ($callback, $expected): void {
        $t->same($expected, $callback());
    };
}

$tests['pager master journal reader cache current source next310 missing token rejects'] = static function (TestRunner $t) use ($plan, $cacheEntry, $recovered): void {
    $t->throws(Throwable::class, static fn () => $plan([1 => array_diff_key($cacheEntry('missing', $recovered[1]), ['reader_cache_ignore_check_constraints_token' => true])]));
};
