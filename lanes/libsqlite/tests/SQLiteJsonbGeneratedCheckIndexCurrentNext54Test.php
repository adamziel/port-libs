...
$updates54 = [
    ...
    ['rowid' => 104, 'mutations' => [
        ['function' => 'jsonb_set', 'path' => '$.plugin.slug', 'value' => ''],
    ]],
];
...
$decodeOption54 = static function (mixed $value): array {
...
$tests = [
    ...
    'jsonb generated check index current next54 extracts check constraints' => static function (TestRunner $t) use ($plan54): void {
        ...
        $t->same(['plugin_slug', 'plugin_enabled', 'plugin_rank'], array_column($checks, 'column'));
    ...
    'jsonb generated check index current next54 reports failed check details' => static function (TestRunner $t) use ($plan54, $valueAt54): void {
        ...
        $t->same('CHECK(plugin_rank BETWEEN 1 AND 99)', $valueAt54($plan, 'rejected_updates.0.checks.2.sql'));
        $t->same('', $valueAt54($plan, 'rejected_updates.1.checks.0.actual'));
        $t->same(false, $valueAt54($plan, 'rejected_updates.1.checks.0.ok'));
        $t->same('CHECK(plugin_slug <> \'\')', $valueAt54($plan, 'rejected_updates.1.checks.0.sql'));
    },
    'jsonb generated check index current next54 decodes accepted JSONB payloads' => static function (TestRunner $t) use ($plan54, $decodeOption54): void {
        ...
        $t->same('epsilon', $gamma['plugin']['slug']);
        $t->same(35, $gamma['plugin']['rank']);
        $t->same(20, $beta['plugin']['rank']);
        $t->same('beta', $beta['plugin']['slug']);
    },
    ...
    'jsonb generated check index current next54 updates partial enabled membership' => static function (TestRunner $t) use ($plan54): void {
        ...
        $t->same([101, 103, 104], array_column($plan['before_indexes']['idx_plugin_enabled_checked54']['current_entries'], 'rowid'));
        $t->same([103, 104], array_column($plan['after_indexes']['idx_plugin_enabled_checked54']['current_entries'], 'rowid'));
        $t->same('delete', $plan['index_actions'][0]['action']);
        $t->same('idx_plugin_enabled_checked54', $plan['index_actions'][0]['index']);
    ...
    'jsonb generated check index current next54 updates unique slug index image' => static function (TestRunner $t) use ($plan54): void {
        ...
        $t->same(['alpha', 'beta', 'delta', 'gamma'], array_column($plan['before_indexes']['idx_plugin_slug_checked54']['current_entries'], 'key'));
        $t->same(['alpha', 'beta', 'delta', 'epsilon'], array_column($plan['after_indexes']['idx_plugin_slug_checked54']['current_entries'], 'key'));
        $t->same([101, 102, 104, 103], array_column($plan['before_indexes']['idx_plugin_slug_checked54']['current_entries'], 'rowid'));
        $t->same([101, 102, 104, 103], array_column($plan['after_indexes']['idx_plugin_slug_checked54']['current_entries'], 'rowid'));
        $t->same('NOCASE', $plan['after_indexes']['idx_plugin_slug_checked54']['collation']);
        $t->true($plan['after_indexes']['idx_plugin_slug_checked54']['unique']);
    },
    'jsonb generated check index current next54 preserves descending rank order' => static function (TestRunner $t) use ($plan54): void {
        $plan = $plan54();

        $t->same([40, 30, 20, 10], array_column($plan['before_indexes']['idx_plugin_rank_checked54']['current_entries'], 'key'));
        $t->same([40, 35, 20, 15], array_column($plan['after_indexes']['idx_plugin_rank_checked54']['current_entries'], 'key'));
        $t->same([104, 103, 102, 101], array_column($plan['after_indexes']['idx_plugin_rank_checked54']['current_entries'], 'rowid'));
        $t->true($plan['after_indexes']['idx_plugin_rank_checked54']['descending']);
        $t->same('BINARY', $plan['after_indexes']['idx_plugin_rank_checked54']['collation']);
    },
