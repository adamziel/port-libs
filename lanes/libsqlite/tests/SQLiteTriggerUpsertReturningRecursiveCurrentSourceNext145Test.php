...
$triggers145 = [
    [
        'name' => 'wp_options_ai_recursive_child_145',
        'timing' => 'after',
        'event' => 'insert',
    ...
    [
        'name' => 'wp_options_au_recursive_child_145',
        'timing' => 'after',
        'event' => 'update',
        'action' => 'upsert-parent',
        'when' => ['new.depth', '<', 3],
...
$returning145 = [
    ...
    static fn (array $new, ?array $old, array $incoming, string $event, int $depth, ?string $trigger): mixed => $old['option_value'] ?? null,
...
$rolled145 = static fn (): array => $run145(['rollback_on_returning_key' => ['fresh_plugin_child']]);
...
$cases145 = [
    ...
    'released savepoint token' => [static fn (): mixed => $released145()['savepoint'], 'wp_import_recursive_145'],
    ...
    'released current returning names' => [static fn (): mixed => array_column(array_column($released145()['current_returning_rows'], 'returning'), 'option_name'), ['plugin_seed', 'plugin_seed_child', 'plugin_seed_child_child', 'fresh_plugin', 'fresh_plugin_child', 'fresh_plugin_child_child']],
    'released next returning names' => [static fn (): mixed => array_column(array_column($released145()['next_returning_rows'], 'returning'), 'option_name'), ['plugin_seed_child', 'plugin_seed_child_child', 'fresh_plugin', 'fresh_plugin_child', 'fresh_plugin_child_child']],
    'released combined returning count' => [static fn (): mixed => count($released145()['returning_rows']), 11],
    'released current rows include recursive fresh child' => [static fn (): mixed => in_array('fresh_plugin_child', array_column($released145()['current_attempt_rows'], 'option_name'), true), true],
    'released next rows update fresh plugin before recursive child' => [static fn (): mixed => $released145()['next_rows'][4]['option_value'], 'fresh-next'],
    ...
    'released summary new keys' => [static fn (): mixed => $released145()['recursive_summary']['attempted_current_new_keys'], ['plugin_seed_child', 'plugin_seed_child_child', 'fresh_plugin', 'fresh_plugin_child', 'fresh_plugin_child_child']],
    ...
    'rolled attempted current returning retained' => [static fn (): mixed => array_column(array_column($rolled145()['attempted_current_returning_rows'], 'returning'), 'option_name'), ['plugin_seed', 'plugin_seed_child', 'plugin_seed_child_child', 'fresh_plugin', 'fresh_plugin_child', 'fresh_plugin_child_child']],
    'rolled barrier key' => [static fn (): mixed => $rolled145()['rollback_barrier']['returning_key'], 'fresh_plugin_child'],
    ...
    'rolled next start rows are savepoint rows' => [static fn (): mixed => array_column($rolled145()['next_start_rows'], 'option_name'), ['siteurl', 'plugin_seed']],
    ...
    'rolled next returning names replay from savepoint' => [static fn (): mixed => array_column(array_column($rolled145()['next_returning_rows'], 'returning'), 'option_name'), ['plugin_seed_child', 'plugin_seed_child_child', 'fresh_plugin', 'fresh_plugin_child', 'fresh_plugin_child_child']],
    ...
    'ignore suppresses current conflict returning but inserts fresh chain' => [static fn (): mixed => array_column(array_column($ignore145()['current_returning_rows'], 'returning'), 'option_name'), ['fresh_plugin', 'fresh_plugin_child', 'fresh_plugin_child_child']],
    ...
    'recursive off current statement only' => [static fn (): mixed => array_column(array_column($recursiveOff145()['current_returning_rows'], 'returning'), 'option_name'), ['plugin_seed', 'fresh_plugin']],
    'recursive off next statement only' => [static fn (): mixed => array_column(array_column($recursiveOff145()['next_returning_rows'], 'returning'), 'option_name'), ['plugin_seed_child', 'fresh_plugin']],
