...
$plan893908 = static fn (array $events, ?array $statements = null, ?array $schemas = null): array => SQLiteAttachWalTempSchemaCacheCurrentSourceNext92Plan::currentSourceNext893908(
...
$tests['attach temp wal schema cache current source next893-908 extends next877-892 handoff'] = static function (TestRunner $t) use ($plan893908): void {
    ...
    $t->same('attach-wal-temp-schema-cache-current-source-next893-908', $result['operation']);
    $t->same('sqlite-attach-temp-wal-schema-cache-current-source-next893', $result['dependencies'][0]);
    $t->same('sqlite-attach-temp-wal-schema-cache-current-source-next908', $result['dependencies'][15]);
    $t->same('sqlite-attach-temp-wal-schema-cache-current-source-next877', $result['dependencies'][16]);
    $t->same('sqlite-attach-temp-wal-schema-cache-current-source-next892', $result['dependencies'][31]);
