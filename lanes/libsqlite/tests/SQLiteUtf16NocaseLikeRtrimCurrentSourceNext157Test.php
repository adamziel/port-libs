...
$plan = static fn (
    ...
    int $nextCookie = 157,
): array => SQLiteUtf16NocaseLikeRtrimCurrentSourceNextPlan::keyValueRowKeyPlan(
    $current ?? $currentRows,
    $next ?? $nextRows,
    $pattern,
    $escape,
...
$cases = [
    ...
    'current source' => ['currentSource', 'main.app_settings@156'],
    'next source' => ['nextSource', 'main.app_settings@157'],
...
$tests['utf16 nocase like rtrim current source nextOneFiveSeven stable byte order reusable'] = static function (TestRunner $t) use ($row): void {
    ...
    $result = SQLiteUtf16NocaseLikeRtrimCurrentSourceNextPlan::keyValueRowKeyPlan($rows, $rows, 'plugin\_%', '\\', 'stable', 'stable', 7, 7);
...
$tests['utf16 nocase like rtrim current source nextOneFiveSeven stable byte order change invalidates'] = static function (TestRunner $t) use ($row): void {
    ...
    $result = SQLiteUtf16NocaseLikeRtrimCurrentSourceNextPlan::keyValueRowKeyPlan($current, $next, 'plugin\_%', '\\', 'stable', 'stable', 7, 7);
...
$tests['utf16 nocase like rtrim current source nextOneFiveSeven rejects bad escape'] = static function (TestRunner $t) use ($currentRows): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteUtf16NocaseLikeRtrimCurrentSourceNextPlan::keyValueRowKeyPlan($currentRows, [], 'plugin%', 'xx'));
};
