...
$makeFirstPage315318 = static function (int $pageCount): string {
...
$putPointerMapEntry315318 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database315318 = static function (int $sliceNumber) use ($makeFirstPage315318, $putPointerMapEntry315318): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage315318(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry315318($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan315318 = static function (int $sliceNumber, int $batchSize = 2) use ($database315318): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database315318($sliceNumber);
...
foreach ([315, 316, 317, 318] as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} keeps final next315-318 freelist splice isolated"] = static function (TestRunner $t) use ($plan315318, $sliceNumber): void {
        $plan = $plan315318($sliceNumber);
        ...
        $t->same(6, $plan315318($sliceNumber, 3)->freelistSummary()['freelist_row_count']);
    };
}
