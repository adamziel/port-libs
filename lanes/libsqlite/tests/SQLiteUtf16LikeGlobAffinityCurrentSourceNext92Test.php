...
$cases = [
    ...
    'records source current' => ['autoload:%', 'LIKE', null, false, 'currentSource', 'main.app_settings'],
    'records source next' => ['autoload:%', 'LIKE', null, false, 'nextSource', 'main.app_settings'],
    ...
    'source change reason first' => ['autoload:%', 'LIKE', null, false, 'invalidationReasons.0', 'source-name', 'main.app_settings', 'temp.app_settings'],
    'source change records next source' => ['autoload:%', 'LIKE', null, false, 'nextSource', 'temp.app_settings', 'main.app_settings', 'temp.app_settings'],
...
$tests['utf16 like glob affinity current source nextNineTwo rejects missing option id'] = static function (TestRunner $t) use ($nextRows): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteUtf16LikeGlobAffinityCurrentSourceNextPlan::keyValueRowValuePlan([['option_value' => 'autoload:yes']], $nextRows, 'autoload:%'));
};
...
$tests['utf16 like glob affinity current source nextNineTwo rejects missing value'] = static function (TestRunner $t) use ($nextRows): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteUtf16LikeGlobAffinityCurrentSourceNextPlan::keyValueRowValuePlan([['option_id' => 1]], $nextRows, 'autoload:%'));
};
...
$tests['utf16 like glob affinity current source nextNineTwo rejects nonscalar value'] = static function (TestRunner $t) use ($nextRows): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteUtf16LikeGlobAffinityCurrentSourceNextPlan::keyValueRowValuePlan([['option_id' => 1, 'option_value' => ['autoload']]], $nextRows, 'autoload:%'));
};
