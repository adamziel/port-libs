...
$makeFirstPage11191134 = static function (int $pageCount): string {
...
$putPointerMapEntry11191134 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database11191134 = static function (int $sliceNumber) use ($makeFirstPage11191134, $putPointerMapEntry11191134): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage11191134(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry11191134($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan11191134 = static function (int $sliceNumber, int $batchSize = 2) use ($database11191134): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database11191134($sliceNumber);
...
foreach (range(1119, 1134) as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} continues next1119-1134 handoff receipts"] = static function (TestRunner $t) use ($plan11191134, $sliceNumber): void {
        $plan = $plan11191134($sliceNumber);
        ...
        $t->same(6, $plan11191134($sliceNumber, 3)->currentSourceSummary()['current_source_row_count']);
    };
}
