...
use PortLibs\LibSqlite\SQLiteBTreeDeleteOverflowCurrentNext80Plan;
...
$fixture = static function () use ($databaseFixture): array {
    ...
    $table = SQLiteBTreeDeleteOverflowCurrentNext80Plan::tableLeafCurrentNext($database, 3, $tableCurrentDelete, 3, [8], true);
    ...
    $index = SQLiteBTreeDeleteOverflowCurrentNext80Plan::indexLeafCurrentNext(
        $database,
        4,
        $indexCurrentDelete,
...
$cases = [
    ...
    'rejects missing table current rowid' => static fn (array $fx): mixed => $throwsMessage(static fn () => SQLiteBTreeDeleteOverflowCurrentNext80Plan::tableLeafCurrentNext($fx[0], 3, ['page' => $fx[0]->page(3), 'obsolete_overflow_page_numbers' => []], 3, [])),
    'rejects next missing rowid' => static fn (array $fx): mixed => $throwsMessage(static fn () => SQLiteBTreeDeleteOverflowCurrentNext80Plan::tableLeafCurrentNext($fx[0], 3, ['page' => SQLiteTableLeafPage::deleteCellByRowId($fx[0]->page(3), 2), 'rowid' => 2, 'obsolete_overflow_page_numbers' => []], 99, [])),
    'rejects missing index current record' => static fn (array $fx): mixed => $throwsMessage(static fn () => SQLiteBTreeDeleteOverflowCurrentNext80Plan::indexLeafCurrentNext($fx[0], 4, ['page' => $fx[0]->page(4), 'obsolete_overflow_page_numbers' => []], ['_transient_b', 3, str_repeat('j', 38)], [])),
    'rejects noninteger next overflow page' => static fn (array $fx): mixed => $throwsMessage(static fn () => SQLiteBTreeDeleteOverflowCurrentNext80Plan::tableLeafCurrentNext($fx[0], 3, ['page' => SQLiteTableLeafPage::deleteCellByRowId($fx[0]->page(3), 2), 'rowid' => 2, 'obsolete_overflow_page_numbers' => []], 3, ['bad'])),
];

$expected = [
    'table action' => 'btree-delete-overflow-materialization-current-next80',
    ...
    'index action' => 'btree-delete-overflow-materialization-current-next80',
...
foreach ($cases as $name => $case) {
    $tests['btree delete overflow materialization current next80 ' . $name] = static function (TestRunner $t) use ($fixture, $case, $expected, $name): void {
