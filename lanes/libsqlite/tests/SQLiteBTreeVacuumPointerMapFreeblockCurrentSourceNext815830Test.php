...
$makeFirstPage815830 = static function (int $pageCount): string {
...
$putPointerMapEntry815830 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database815830 = static function (int $sliceNumber) use ($makeFirstPage815830, $putPointerMapEntry815830): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage815830(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry815830($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan815830 = static function (int $sliceNumber, int $batchSize = 2) use ($database815830): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database815830($sliceNumber);
...
foreach ([815, 816, 817, 818, 819, 820, 821, 822, 823, 824, 825, 826, 827, 828, 829, 830] as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} extends next815-830 handoff receipts"] = static function (TestRunner $t) use ($plan815830, $sliceNumber): void {
        $plan = $plan815830($sliceNumber);
        ...
        $t->same(6, $plan815830($sliceNumber, 3)->currentSourceSummary()['current_source_row_count']);
    };
}
