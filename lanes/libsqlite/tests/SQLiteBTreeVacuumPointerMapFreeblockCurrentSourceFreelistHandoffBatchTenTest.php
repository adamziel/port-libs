...
$makeFirstPageFreelistHandoffBatchTen = static function (int $pageCount): string {
...
$putPointerMapEntryFreelistHandoffBatchTen = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$databaseFreelistHandoffBatchTen = static function (int $sliceNumber) use ($makeFirstPageFreelistHandoffBatchTen, $putPointerMapEntryFreelistHandoffBatchTen): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPageFreelistHandoffBatchTen(110);
    $pages[2] = str_repeat("\0", 512);
    $pages[3] = SQLiteTableLeafPage::assemble([
        SQLiteTableLeafCell::encode(1, SQLiteRecord::encode([null, 'home', 'https://example.test'])),
        SQLiteTableLeafCell::encode(2, SQLiteRecord::encode([null, "_transient_timeout_freelist_handoff_batch_ten_{$sliceNumber}", str_repeat('ttl:', 52)])),
    ...
    foreach ([
        ...
        $putPointerMapEntryFreelistHandoffBatchTen($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$planFreelistHandoffBatchTen = static function (int $sliceNumber, int $batchSize = 2) use ($databaseFreelistHandoffBatchTen): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $databaseFreelistHandoffBatchTen($sliceNumber);
...
foreach (range(0, 15) as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source freelist handoff batch ten case {$sliceNumber} receipts"] = static function (TestRunner $t) use ($planFreelistHandoffBatchTen, $sliceNumber): void {
        $plan = $planFreelistHandoffBatchTen($sliceNumber);
        ...
        $t->same(6, $planFreelistHandoffBatchTen($sliceNumber, 3)->currentSourceSummary()['current_source_row_count']);
    };
}
