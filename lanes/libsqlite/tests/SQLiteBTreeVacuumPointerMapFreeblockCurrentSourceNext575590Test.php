...
$makeFirstPage575590 = static function (int $pageCount): string {
...
$putPointerMapEntry575590 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database575590 = static function (int $sliceNumber) use ($makeFirstPage575590, $putPointerMapEntry575590): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage575590(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry575590($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan575590 = static function (int $sliceNumber, int $batchSize = 2) use ($database575590): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database575590($sliceNumber);
...
foreach ([575, 576, 577, 578, 579, 580, 581, 582, 583, 584, 585, 586, 587, 588, 589, 590] as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} publishes next575-590 handoff receipts"] = static function (TestRunner $t) use ($plan575590, $sliceNumber): void {
        $plan = $plan575590($sliceNumber);
        ...
        $t->same(6, $plan575590($sliceNumber, 3)->currentSourceSummary()['current_source_row_count']);
    };
}
