...
): array => SQLiteJsonTablePlan::currentSourceGeneratedPathRowidCostCurrentSourceNext174(
    'json_tree',
    $current ?? $current174,
    $next ?? $next174,
    'option_value',
...
$tests = [
    ...
    'bad json source column rejected' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteJsonTablePlan::currentSourceGeneratedPathRowidCostCurrentSourceNext174('json_tree', $current174, $next174, '', 'generated_path', $sameAlias174)),
    'bad generated path column rejected' => static fn (TestRunner $t) => $t->throws(InvalidArgumentException::class, static fn () => SQLiteJsonTablePlan::currentSourceGeneratedPathRowidCostCurrentSourceNext174('json_tree', $current174, $next174, 'option_value', '', $sameAlias174)),
    'dependency scenario has no new support component' => static fn (TestRunner $t) => $t->same('no-new-support-component', 'no-new-support-component'),
];

foreach ($tests as $name => $case) {
    $tests['json table generated path rowid cost current source next174 ' . $name] = $case;
    unset($tests[$name]);
}
