...
$base = [
    'source_id' => 'pager-reader-cache-current-source-next542',
    'epoch' => 542,
    'format_signature' => hash('sha256', implode('|', [512, 4, 2, 542, 0])),
    'publication_generation' => 542,
    'master_source_digest' => hash('sha256', 'master-next542'),
...
$plan = static fn (array $cacheExtra = [], array $readExtra = []): array => SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::readerCacheStatementFence(542,
    $database,
    $master,
    $masterBytes,
    implode('', $before),
...
$tests['pager master journal reader cache current source next542 admits current VDBE virtual-table and opcode fences'] = static function (TestRunner $t) use ($plan): void {
    $result = $plan();
    $t->same('pager-master-journal-reader-cache-current-source-next542', $result['status']);
    $t->same([], $result['invalidated_cache_page_numbers']);
    $t->same(['read-options' => true], $result['read_cache_hits']);
    $t->same('reader-cache-stmt-vdbe-must-be-int-current-542', $result['current_reader_cache_stmt_vdbe_must_be_int_token']);
};

$tests['pager master journal reader cache current source plan dispatches full width calls to next542'] = static function (TestRunner $t) use ($database, $master, $masterBytes, $before, $pageSize, $recovered, $cacheEntry, $read, $base, $tokens, $headers, $tokenFields): void {
...
$tests['pager master journal reader cache current source next542 invalidates stale vmustbeint cache'] = static function (TestRunner $t) use ($plan, $opCount): void {
    $result = $plan(['reader_cache_stmt_vdbe_must_be_int_token' => 'stmt-vdbe-must-be-int-old']);
    $t->same([1], $result['reader_cache_stmt_vdbe_must_be_int_invalidated_cache_page_numbers']);
    $t->same([1], $result['invalidated_cache_page_numbers']);
    $t->same(1, $opCount($result, 'invalidate_reader_cache_reader_cache_stmt_vdbe_must_be_int_current_source_next542'));
};

$tests['pager master journal reader cache current source next542 reopens stale vmustbeint read ticket'] = static function (TestRunner $t) use ($plan, $opCount): void {
    $result = $plan([], ['reader_cache_stmt_vdbe_must_be_int_token' => 'stmt-vdbe-must-be-int-old']);
    $t->same(['read-options'], $result['reopen_reader_ids']);
    $t->same(false, $result['next_reads'][0]['cache_hit']);
    $t->same('reader_ticket_reader_cache_stmt_vdbe_must_be_int_predates_current_source', $result['next_reads'][0]['reader_cache_stmt_vdbe_must_be_int_token_reason']);
    $t->same(1, $opCount($result, 'reopen_reader_for_reader_cache_stmt_vdbe_must_be_int_current_source_next542'));
};

$tests['pager master journal reader cache current source next542 missing vmustbeint token rejects'] = static function (TestRunner $t) use ($plan): void {
    $t->throws(Throwable::class, static fn () => $plan(['reader_cache_stmt_vdbe_must_be_int_token' => null]));
};
