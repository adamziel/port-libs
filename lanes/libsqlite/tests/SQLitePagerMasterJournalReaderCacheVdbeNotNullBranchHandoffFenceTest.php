...
$pageSize = 512;
$database = '/srv/wp-content/database/wp-next654.sqlite';
...
$formatPage = static function (string $label) use ($pageSize): string {
    ...
    $page = substr_replace($page, pack('N', 654), 60, 4);
...
$variantFields = array_merge($tokenFields, $next639654Fields);
$before = [1 => $formatPage('stale schema'), 2 => $page('stale options')];
$recovered = [1 => $formatPage('current schema'), 2 => $page('current options')];
$tokens = [$journal => 'member-main-current-654'];
$headers = [$journal => hash('sha256', 'main header next654')];
$base = [
    'source_id' => 'pager-reader-cache-current-source-next654',
    'epoch' => 654,
    'format_signature' => hash('sha256', implode('|', [512, 4, 2, 654, 0])),
    'publication_generation' => 654,
    'master_source_digest' => hash('sha256', 'master-next654'),
    'recovery_sequence' => 654,
...
foreach ($variantFields as $field) {
    $base[$field] = str_replace('_', '-', preg_replace('/_token$/', '', $field)) . '-current-654';
}
...
$plan = static fn (array $cacheExtra = [], array $readExtra = []): array => SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::currentSourceVdbeNotNullBranchHandoffFence(
    $database,
    $master,
    $masterBytes,
    implode('', $before),
...
$tests['pager master journal reader cache current source next654 admits current VDBE literal and arithmetic opcode fences'] = static function (TestRunner $t) use ($plan): void {
    $result = $plan();
    $t->same('pager-master-journal-reader-cache-current-source-next654', $result['status']);
    $t->same([], $result['invalidated_cache_page_numbers']);
    $t->same(['read-options' => true], $result['read_cache_hits']);
    $t->same('reader-cache-stmt-vdbe-null-branch-current-654', $result['current_reader_cache_stmt_vdbe_null_branch_token']);
    $t->same('reader-cache-stmt-vdbe-bit-and-handoff-current-654', $result['current_reader_cache_stmt_vdbe_bit_and_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-notnull-branch-handoff-current-654', $result['current_reader_cache_stmt_vdbe_notnull_branch_handoff_token']);
    $t->same(true, in_array('sqlite-pager-master-journal-reader-cache-current-source-next638', $result['dependencies'], true));
};

$tests['pager master journal reader cache current source plan dispatches full width calls to next654'] = static function (TestRunner $t) use ($database, $master, $masterBytes, $before, $pageSize, $recovered, $cacheEntry, $read, $base, $tokens, $headers, $variantFields): void {
...
$tests['pager master journal reader cache current source next654 invalidates stale notnull branch cache'] = static function (TestRunner $t) use ($plan, $opCount): void {
    $result = $plan(['reader_cache_stmt_vdbe_notnull_branch_handoff_token' => 'stmt-vdbe-notnull-branch-old']);
    $t->same([1], $result['reader_cache_stmt_vdbe_notnull_branch_handoff_invalidated_cache_page_numbers']);
    $t->same([1], $result['invalidated_cache_page_numbers']);
    $t->same(1, $opCount($result, 'invalidate_reader_cache_reader_cache_stmt_vdbe_notnull_branch_handoff_current_source_next654'));
};

$tests['pager master journal reader cache current source next654 reopens stale notnull branch read ticket'] = static function (TestRunner $t) use ($plan, $opCount): void {
    $result = $plan([], ['reader_cache_stmt_vdbe_notnull_branch_handoff_token' => 'stmt-vdbe-notnull-branch-old']);
    $t->same(['read-options'], $result['reopen_reader_ids']);
    $t->same(false, $result['next_reads'][0]['cache_hit']);
    $t->same('reader_ticket_reader_cache_stmt_vdbe_notnull_branch_handoff_predates_current_source', $result['next_reads'][0]['reader_cache_stmt_vdbe_notnull_branch_handoff_token_reason']);
    $t->same(1, $opCount($result, 'reopen_reader_for_reader_cache_stmt_vdbe_notnull_branch_handoff_current_source_next654'));
};

$tests['pager master journal reader cache current source next654 missing notnull branch token rejects'] = static function (TestRunner $t) use ($plan): void {
    $t->throws(Throwable::class, static fn () => $plan(['reader_cache_stmt_vdbe_notnull_branch_handoff_token' => null]));
};
