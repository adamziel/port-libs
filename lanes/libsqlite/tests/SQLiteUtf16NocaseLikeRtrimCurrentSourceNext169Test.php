...
$currentRows = [
    ...
    $row(4, 'module_cache_tab' . "\t", 'UTF-16BE'),
    $row(5, 'module_other', 'UTF-16LE'),
    $row(6, 'module_Éclair', 'UTF-16LE'),
];
$nextRows = [
    ...
    $row(3, 'module_cache_beta  ', 'UTF-16LE'),
    $row(4, 'module_cache_tab' . "\t", 'UTF-16BE'),
    $row(7, 'MODULE_CACHE_GAMMA', 'UTF-16LE'),
    $row(8, 'module_cache_early', 'UTF-8'),
...
$cases = [
    ...
    'next range upper' => ['nextRange.upperBound', 'module_cachf'],
    'current index usable' => ['currentIndexUsable', true],
    'next index usable' => ['nextIndexUsable', true],
    'last token key' => ['lastYielded.key', 'module_cache_alpha'],
    ...
    'yielded key row three' => ['yieldedKeys.3', 'module_cache_beta'],
    'yielded key row eight' => ['yieldedKeys.8', 'module_cache_early'],
    'yielded key row seven' => ['yieldedKeys.7', 'module_cache_gamma'],
    'deferred rowids' => ['deferredRowids', [4]],
    'deferred key tab preserved' => ['deferredKeys.4', "module_cache_tab\t"],
    'high water key' => ['highWaterToken.key', 'module_cache_gamma'],
...
$tests['utf16 nocase like rtrim current source nextOneSixNine source change restarts from range start'] = static function (TestRunner $t) use ($plan): void {
    ...
    $t->same(['key' => 'module_cache_alpha', 'rowid' => 2], $result['highWaterToken']);
};
...
$tests['utf16 nocase like rtrim current source nextOneSixNine page size one yields one high water token'] = static function (TestRunner $t) use ($plan): void {
    ...
    $t->same(['key' => 'module_cache_beta', 'rowid' => 3], $result['highWaterToken']);
...
$tests['utf16 nocase like rtrim current source nextOneSixNine large page drains cursor'] = static function (TestRunner $t) use ($plan): void {
    ...
    $t->same(['key' => "module_cache_tab\t", 'rowid' => 4], $result['highWaterToken']);
...
$tests['utf16 nocase like rtrim current source nextOneSixNine entered before token restarts before yielding'] = static function (TestRunner $t) use ($plan, $currentRows, $nextRows, $row): void {
    $next = array_merge($nextRows, [$row(10, 'module_cache_aaa', 'UTF-16LE')]);
...
$tests['utf16 nocase like rtrim current source nextOneSixNine retained moved across token restarts'] = static function (TestRunner $t) use ($plan, $currentRows, $row): void {
    $next = [
        $row(1, 'Module_Cache', 'UTF-16BE'),
        $row(2, 'module_cache_alpha', 'UTF-16BE'),
        $row(3, 'module_cache_aaa', 'UTF-16LE'),
        $row(4, 'module_cache_tab' . "\t", 'UTF-16BE'),
    ];
    $result = $plan($currentRows, $next);
...
$tests['utf16 nocase like rtrim current source nextOneSixNine unicode pattern keeps range but ascii nocase only'] = static function (TestRunner $t) use ($plan): void {
    $result = $plan(currentPattern: 'module\\_éclair%', nextPattern: 'module\\_éclair%');
    $t->same(true, $result['currentIndexUsable']);
    $t->same(true, $result['nextIndexUsable']);
    $t->same('module_éclair', $result['currentRange']['lowerInclusive']);
    $t->same([9], $result['yieldedRowids']);
    $t->same(['key' => 'module_éclair', 'rowid' => 9], $result['highWaterToken']);
};
...
$tests['utf16 nocase like rtrim current source nextOneSixNine rejects bad token shape'] = static function (TestRunner $t) use ($plan): void {
    $t->throws(InvalidArgumentException::class, static fn () => $plan(token: ['key' => 'module_cache']));
};
