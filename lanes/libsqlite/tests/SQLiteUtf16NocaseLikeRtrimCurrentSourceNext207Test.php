...
$plan207 = static fn (
    ...
    int $cookie = 206,
    int $nextCookie = 207,
): array => SQLiteUtf16NocaseLikeRtrimCurrentSourceNextPlan::wordpressOptionNameRtrimCollationRebindPlan(
    $current ?? $rows207,
    $next ?? $nextRows207,
    'plugin!_cache%',
    '!',
...
$tests['utf16 nocase like rtrim current source nextTwoZeroSeven stable rtrim cursor is reusable'] = static function (TestRunner $t) use ($row207): void {
    ...
    $result = SQLiteUtf16NocaseLikeRtrimCurrentSourceNextPlan::wordpressOptionNameRtrimCollationRebindPlan(
        $rows,
        $rows,
        'plugin!_cache%',
        '!',
