...
$makeFirstPageFreelistHandoffBatchNine = static function (int $pageCount): string {
...
$putPointerMapEntryFreelistHandoffBatchNine = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$databaseFreelistHandoffBatchNine = static function (int $sliceNumber) use ($makeFirstPageFreelistHandoffBatchNine, $putPointerMapEntryFreelistHandoffBatchNine): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPageFreelistHandoffBatchNine(110);
    $pages[2] = str_repeat("\0", 512);
    $pages[3] = SQLiteTableLeafPage::assemble([
        SQLiteTableLeafCell::encode(1, SQLiteRecord::encode([null, 'home', 'https://example.test'])),
        SQLiteTableLeafCell::encode(2, SQLiteRecord::encode([null, "_transient_timeout_freelist_handoff_batch_nine_{$sliceNumber}", str_repeat('ttl:', 52)])),
    ...
    foreach ([
        ...
        $putPointerMapEntryFreelistHandoffBatchNine($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$planFreelistHandoffBatchNine = static function (int $sliceNumber, int $batchSize = 2) use ($databaseFreelistHandoffBatchNine): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourcePlan {
    $database = $databaseFreelistHandoffBatchNine($sliceNumber);
    $deletedPage = SQLiteTableLeafPage::deleteCellByRowId($database->page(3), 2, secureDelete: true);
    $method = "tableLeafCurrentSourceFreelistHandoffFromDeleteResult";

    return SQLiteBTreeVacuumPointerMapFreeblockCurrentSourcePlan::{$method}(
        $database,
        3,
        [
            'page' => $deletedPage,
...
foreach (range(0, 15) as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source freelist handoff batch nine case {$sliceNumber} receipts"] = static function (TestRunner $t) use ($planFreelistHandoffBatchNine, $sliceNumber): void {
        $plan = $planFreelistHandoffBatchNine($sliceNumber);
        ...
        $t->same(6, $planFreelistHandoffBatchNine($sliceNumber, 3)->currentSourceSummary()['current_source_row_count']);
    };
}
