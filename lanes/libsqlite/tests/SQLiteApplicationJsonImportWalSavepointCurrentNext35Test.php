...
$plan = static fn (array $imports, array $settings = []) => SQLiteJsonImportWalSavepointPlan::plan(
    $currentRows(),
    $imports,
    $settings + ['database_path' => '/tmp/wp-json-import-current-next35.sqlite', 'page_size' => 1024],
);
...
$tests = [
    'plans released json option imports as WAL frames' => static function (TestRunner $t) use ($plan, $jsonRows): void {
        ...
        $t->same(['siteurl', 'active_plugins', 'theme_mods_old', 'plugin_settings'], $result['final_key_names']);
        $t->same(1, $result['wal']['frame_count']);
    },
    'updates current JSON setting rows on the original page' => static function (TestRunner $t) use ($plan, $jsonRows): void {
        $result = $plan([
            ['name' => 'active', 'json' => $jsonRows([
                ['setting_id' => 2, 'key_name' => 'active_plugins', 'key_value' => '["akismet/akismet.php"]', 'load_policy' => 'yes'],
            ]), 'path' => '$.rows'],
        ]);
    ...
    'rolls malformed JSON imports back to the current WAL frame' => static function (TestRunner $t) use ($plan): void {
        ...
        $t->same(['siteurl', 'active_plugins', 'theme_mods_old'], $result['final_key_names']);
    ...
    'preserves released rows when later JSON savepoint rolls back' => static function (TestRunner $t) use ($plan, $jsonRows): void {
        $result = $plan([
            ['name' => 'first', 'json' => $jsonRows([
                ['key_name' => 'plugin_one', 'key_value' => '{"enabled":true}'],
            ]), 'path' => '$.rows'],
        ...
        $t->same(['siteurl', 'active_plugins', 'theme_mods_old', 'plugin_one'], $result['released_key_names']);
    ...
    'keeps open savepoint rows visible but not released' => static function (TestRunner $t) use ($plan, $jsonRows): void {
        $result = $plan([
            ['name' => 'open_stage', 'json' => $jsonRows([
                ['key_name' => 'open_json_stage', 'key_value' => '{"staged":1}'],
            ]), 'path' => '$.rows', 'release' => false],
        ]);

        $t->same('open', $result['batches'][0]['status']);
        $t->same(['siteurl', 'active_plugins', 'theme_mods_old', 'open_json_stage'], $result['final_key_names']);
    ...
    'reports JSON subtype input rows through the same import planner' => static function (TestRunner $t) use ($currentRows): void {
        $subtype = new SQLiteJsonSubtypeValue('{"rows":[{"key_name":"subtype_settings","key_value":"{\"mode\":\"json\"}"}]}');
        $result = SQLiteJsonImportWalSavepointPlan::plan($currentRows(), [
    ...
    'aborts unique-name conflicts when replacement is disabled and requested' => static function (TestRunner $t) use ($plan, $jsonRows): void {
        $t->throws(LogicException::class, static fn () => $plan([
            ['name' => 'rename_conflict', 'json' => $jsonRows([
                ['setting_id' => 2, 'key_name' => 'siteurl', 'key_value' => 'https://duplicate.test'],
...
foreach (range(1, 18) as $batch) {
    $tests["imports generated plugin settings batch {$batch} with current WAL frame"] = static function (TestRunner $t) use ($plan, $jsonRows, $batch): void {
        ...
        $t->same($batch % 2 === 0 ? 'yes' : 'no', $result['final_rows'][3]['load_policy']);
        $t->same(1, $result['wal']['current_frame']);
    };
}
...
foreach ([64 => 2, 65 => 3, 128 => 3, 129 => 4, 192 => 4, 193 => 5, 256 => 5, 257 => 6] as $optionId => $pageNumber) {
    $tests["maps updated option {$optionId} to WAL page {$pageNumber}"] = static function (TestRunner $t) use ($currentRows, $jsonRows, $optionId, $pageNumber): void {
        $rows = $currentRows();
        $rows[] = ['setting_id' => $optionId, 'key_name' => 'generated_' . $optionId, 'key_value' => 'old', 'load_policy' => 'no'];
        $result = SQLiteJsonImportWalSavepointPlan::plan($rows, [
            ['name' => 'page_' . $optionId, 'json' => $jsonRows([
                ['setting_id' => $optionId, 'key_name' => 'generated_' . $optionId, 'key_value' => 'new'],
            ]), 'path' => '$.rows'],
...
foreach ([
    ...
    $tests["rolls back {$label} JSON import shape"] = static function (TestRunner $t) use ($plan, $path, $label): void {
        $json = match ($label) {
            ...
            'non json input' => 17,
            'empty row name' => '{"rows":[{"key_name":"","key_value":"x"}]}',
            'bad load_policy value' => '{"rows":[{"key_name":"bad_load_policy","load_policy":"maybe"}]}',
            default => '{"rows":[]}',
        };
...
foreach ([
    ...
    $tests["preserves {$mode} import settings while tracking WAL current frame"] = static function (TestRunner $t) use ($plan, $jsonRows, $mode, $settings): void {
        $result = $plan([
            ['name' => 'mode_' . $mode, 'json' => $jsonRows([
                ['key_name' => 'mode_' . $mode . '_settings', 'key_value' => '{"mode":"' . $mode . '"}'],
            ]), 'path' => '$.rows'],
        ], $settings);

        $t->same($mode, $result['journal_mode']);
        $t->same($settings['sync_mode'], $result['sync_mode']);
        $t->same(1, $result['wal']['current_frame']);
    };
}
