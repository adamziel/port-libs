...
function libsqlite_suite_next380_evidence(): SQLiteUpstreamSuiteEvidence
{
    return SQLiteUpstreamSuiteEvidence::fromManifestPath(__DIR__ . '/../UPSTREAM_TEST_MANIFEST.json');
}

function libsqlite_suite_next380_output(int $passLines = 96, int $assertions = 96, int $failures = 0): string
{
    $lines = ['Focused test run: 1 selected test files (root lock skipped)'];
    for ($i = 1; $i <= $passLines; $i++) {
        $lines[] = sprintf('PASS current source next380 veryquick shard admission case %02d', $i);
    }
 ...
 */
function libsqlite_suite_next380_rows(
    int $case = 1,
    ...
    string $nextHead = 'suite-upstream-veryquick-shard-current-source-next380'
): array {
    $script = sprintf('veryquick-current-source-next380-%02d.test', $case);

    return [
        [
            'unit' => 'suite-upstream-veryquick-shard-current-source-next380',
            'kind' => 'bounded-upstream-veryquick-shard-runner',
            'gap_id' => 'current-source-next380-veryquick-shard-gap',
            'gap_status' => 'removed',
            'removed_blocker' => 'next380 admits one focused veryquick shard row tied to launcher Base accepted HEAD 42ebf4f and current integration source 8a447f44 without duplicating accepted next155 through next339 suite evidence',
            ...
            'artifact_path' => 'lanes/libsqlite/notes/yield-suite-upstream-veryquick-shard-current-source-next380.md',
 ...
 */
function libsqlite_suite_next380_record(
    array $rows,
    string $launcherBase = '42ebf4f9ec69db260d2f3d077fd0ed0a509b8841',
    string $dashboardSource = '8a447f445e5d2fd32fc9fd463117f585d1416551',
    string $statusSource = '8a447f445e5d2fd32fc9fd463117f585d1416551',
    ...
    return libsqlite_suite_next380_evidence()->upstreamVeryquickShardCurrentSourceAdmission(
        ...
        $nextHead,
        'lanes/libsqlite/tests/SQLiteUpstreamVeryquickShardCurrentSourceNext380Test.php',
...
foreach (range(1, 80) as $case) {
    $tests[sprintf('current source next380 admits veryquick shard case %02d', $case)] = static function (TestRunner $t) use ($case): void {
        $record = libsqlite_suite_next380_record(libsqlite_suite_next380_rows($case));

        $t->same('current-source-next380-veryquick-shard-advanced', $record['status']);
        ...
        $t->same(['suite-upstream-veryquick-shard-current-source-next380'], $record['admitted_units']);
        $t->same(['batch225-current-source-anchor'], $record['preserved_units']);
        $t->contains(sprintf('veryquick-current-source-next380-%02d.test', $case), implode(',', $record['target_scripts']));
        $t->same(true, $record['counts_upstream_veryquick_shard_current_source_next380']);
...
$tests['current source next380 records authoritative launcher and source heads'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next380_record(libsqlite_suite_next380_rows(8));
    ...
    $t->same(['suite-upstream-veryquick-shard-current-source-next380'], $record['artifact_source_heads']);
};

$tests['current source next380 records target scripts and tier counts'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next380_record(libsqlite_suite_next380_rows(13));

    $t->same(145978, $record['tests_total_delta']);
    $t->same(['accepted-batch225-anchor.test', 'testrunner.test', 'veryquick-current-source-next380-13.test'], $record['target_scripts']);
...
$tests['current source next380 preserves already counted row without mapped inflation'] = static function (TestRunner $t): void {
    $rows = libsqlite_suite_next380_rows();
    $rows[0]['current_countable'] = true;
    $rows[0]['current_tests'] = 131297;

    $record = libsqlite_suite_next380_record($rows);

    $t->same('current-source-next380-veryquick-shard-preserved', $record['status']);
    $t->same(0, $record['mapped_delta']);
    $t->same(740, $record['next_mapped']);
    $t->same(['batch225-current-source-anchor', 'suite-upstream-veryquick-shard-current-source-next380'], $record['preserved_units']);
};

$tests['current source next380 blocks stale source provenance'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next380_record(
        libsqlite_suite_next380_rows(
...
$tests['current source next380 blocks unguarded and non local artifacts'] = static function (TestRunner $t): void {
    $rows = libsqlite_suite_next380_rows();
    $rows[0]['artifact_path'] = '/tmp/next380.md';
...
$tests['current source next380 blocks non zero runner artifacts'] = static function (TestRunner $t): void {
    $rows = libsqlite_suite_next380_rows();
    $rows[0]['exit'] = 1;
    $rows[0]['errors'] = 2;
...
$tests['current source next380 blocks missing removed blocker classification'] = static function (TestRunner $t): void {
    $rows = libsqlite_suite_next380_rows();
...
$tests['current source next380 blocks duplicate broad runner snapshot'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next380_record(
        libsqlite_suite_next380_rows(),
...
$tests['current source next380 blocks focused php admission mismatch'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next380_record(
        libsqlite_suite_next380_rows(),
        output: libsqlite_suite_next380_output(assertions: 83)
    );
...
$tests['current source next380 records exact focused php admission'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next380_record(libsqlite_suite_next380_rows());
...
$tests['current source next380 blocks missing focused runner summary'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next380_record(
        libsqlite_suite_next380_rows(),
...
$tests['current source next380 carries dependency closure and non overlap notes'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next380_record(libsqlite_suite_next380_rows());
...
$tests['current source next380 blocks missing next countability flag'] = static function (TestRunner $t): void {
    $rows = libsqlite_suite_next380_rows();
...
$tests['current source next380 keeps broad release parity unclaimed'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next380_record(libsqlite_suite_next380_rows(21));
...
$tests['current source next380 does not recount prior veryquick shards'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next380_record(libsqlite_suite_next380_rows(22));

    $t->same(false, $record['counts_upstream_veryquick_shard_current_source_next259']);
    $t->same(false, $record['counts_upstream_veryquick_shard_current_source_next202']);
};

$tests['current source next380 records focused shard dependency closure'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next380_record(libsqlite_suite_next380_rows(23));

    $t->contains('current-source next380 veryquick shard admission', $record['dependency_closure']);
