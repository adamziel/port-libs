...
$plan797812 = static fn (array $events, ?array $statements = null, ?array $schemas = null): array => SQLiteAttachWalTempSchemaCacheCurrentSourceNext92Plan::currentSourceNext797812(
...
$tests['attach temp wal schema cache current source next797-812 extends next781-796 handoff'] = static function (TestRunner $t) use ($plan797812): void {
    ...
    $t->same('attach-wal-temp-schema-cache-current-source-next797-812', $result['operation']);
    $t->same('sqlite-attach-temp-wal-schema-cache-current-source-next797', $result['dependencies'][0]);
    $t->same('sqlite-attach-temp-wal-schema-cache-current-source-next812', $result['dependencies'][15]);
    $t->same('sqlite-attach-temp-wal-schema-cache-current-source-next781', $result['dependencies'][16]);
    $t->same('sqlite-attach-temp-wal-schema-cache-current-source-next796', $result['dependencies'][31]);
