...
// own 0000..4999 and 6000..7999, while a pending ready shard owns 5000..5999,
// so this shard owns the next non-overlapping 8000..8999 suffix window.
$firstSuffix = 8000;
$lastSuffix = 8999;
$cases = [];
...
$scriptFile = tempnam(sys_get_temp_dir(), 'libsqlite-atof1-decimal-8000-oracle-');
if ($scriptFile === false) {
...
$tests['real upstream corpus date affinity dynamic atof1 decimal REAL suffix 8000 8999 source truth'] =
    static function (TestRunner $t) use ($cases, $firstSuffix, $lastSuffix, $oracle, $sourceFile, $sourceText): void {
        ...
        $t->same(8000, $firstSuffix);
        $t->same(8999, $lastSuffix);
        $t->same(8000, $cases['suffix-8000']['suffix']);
        $t->same(8999, $cases['suffix-8999']['suffix']);
        $t->same('18.446744073709558000', $cases['suffix-8000']['value']);
        $t->same('18.446744073709558999', $cases['suffix-8999']['value']);
    };

foreach ($cases as $key => $case) {
    $tests['real upstream corpus date affinity dynamic atof1 decimal REAL suffix 8000 8999 ' . $key] =
...
$tests['real upstream corpus date affinity dynamic atof1 decimal REAL suffix 8000 8999 generic application rollup'] =
    static function (TestRunner $t) use ($cases): void {
        $rows = [
            ['key_name' => 'metric-low', 'decimal_metric' => $cases['suffix-8000']['value']],
            ['key_name' => 'metric-mid', 'decimal_metric' => $cases['suffix-8500']['value']],
            ['key_name' => 'metric-high', 'decimal_metric' => $cases['suffix-8999']['value']],
        ];
        $result = SQLiteSelectSql::execute(
            'SELECT key_name, '
...
$tests['real upstream corpus date affinity dynamic atof1 decimal REAL suffix 8000 8999 non overlap and dependency closure'] =
    static function (TestRunner $t): void {
        $t->same(
            'owns atof1.test atof-3.2 decimal REAL suffixes 8000..8999 only',
            'owns atof1.test atof-3.2 decimal REAL suffixes 8000..8999 only',
        );
        $t->same(
            'non-overlap: avoids accepted atof-3.2 decimal REAL/mantissa suffixes 0000..4999 and 6000..7999, pending ready suffixes 5000..5999, accepted atof-3.1 integer-prefix suffixes 0592..1609, atof-3.3 exponent rows, atof2 rounding, date4 rows, date/timediff matrices, and types storage-class batches',
            'non-overlap: avoids accepted atof-3.2 decimal REAL/mantissa suffixes 0000..4999 and 6000..7999, pending ready suffixes 5000..5999, accepted atof-3.1 integer-prefix suffixes 0592..1609, atof-3.3 exponent rows, atof2 rounding, date4 rows, date/timediff matrices, and types storage-class batches',
        );
        $t->same(
