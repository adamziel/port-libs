...
$plan200 = static fn (
    ...
    int $nextCookie = 200,
): array => SQLiteUtf16NocaseLikeRtrimCurrentSourceNextPlan::keyValueRowKeyEscapeRebindPlan(
    $current ?? $rows200,
    $next ?? $nextRows200,
    $currentPattern,
    $currentEscape,
...
$cases200 = [
    ...
    'current source' => ['currentSource', 'main.app_settings@199'],
    'next source' => ['nextSource', 'main.app_settings@200'],
...
$tests['utf16 nocase like rtrim current source nextTwoZeroZero same escape stable cursor is reusable'] = static function (TestRunner $t) use ($row200): void {
    ...
    $result = SQLiteUtf16NocaseLikeRtrimCurrentSourceNextPlan::keyValueRowKeyEscapeRebindPlan(
        $rows,
        $rows,
        'plugin!_%',
        '!',
...
$tests['utf16 nocase like rtrim current source nextTwoZeroZero rejects invalid current escape length'] = static function (TestRunner $t) use ($rows200, $nextRows200): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteUtf16NocaseLikeRtrimCurrentSourceNextPlan::keyValueRowKeyEscapeRebindPlan($rows200, $nextRows200, 'plugin!_%', '!!'));
};
