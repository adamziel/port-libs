...
$summary239 = static fn (?array $cursor = null): array => SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext239Plan::compare($sql239, $currentTables239, $nextTables239, $cursor);
$tests = [];
...
foreach (range(1, 58) as $case) {
    $tests['compound select window recursive limit current source next239 generated resume fence ' . $case] = static function (TestRunner $t) use ($case): void {
        ...
        $plan = SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext239Plan::compare($sql, $tables, $nextTables);
        $cursor = $plan['cursor'];
        $cursor['acknowledgedCurrentAcksNext232'] = $cursor['requiredCurrentAcksNext232'];
        $cursor['acknowledgedPromotionAcksNext235'] = $cursor['requiredPromotionAcksNext235'];
        $cursor['acknowledgedResumeAcksNext239'] = $cursor['requiredResumeAcksNext239'];
        $again = SQLiteCompoundSelectWindowRecursiveLimitCurrentSourceNext239Plan::compare($sql, $tables, $nextTables, $cursor);
