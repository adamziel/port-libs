...
$pathCases64 = [
    'status ok' => ['status', 'ok'],
    'operation current next objects' => ['operation', 'attach-temp-main-wal-current-next-objects'],
    ...
    'dependency marker' => ['dependencies.0', 'sqlite-attach-wal-temp-current-next64'],
    'dependency wal ddl' => ['dependencies.1', 'sqlite-wal-ddl-object-resolution'],
];

foreach ($pathCases64 as $name => [$path, $expected]) {
    $tests['attach wal temp current next64 ' . $name] = static function (TestRunner $t) use ($plan64, $value64, $path, $expected): void {
...
foreach ($predicateCases64 as $name => $callback) {
    $tests['attach wal temp current next64 ' . $name] = static function (TestRunner $t) use ($callback): void {
        $t->true($callback());
    };
}
