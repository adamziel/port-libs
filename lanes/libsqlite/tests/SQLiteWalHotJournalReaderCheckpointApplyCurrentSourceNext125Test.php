...
$cleanPages = [
    1 => $page('next125 clean schema before interrupted import'),
    2 => $page('next125 clean wp_options before interrupted import'),
    3 => $page('next125 clean plugin settings before interrupted import'),
    4 => $page('next125 clean autoload index before interrupted import'),
];
$dirtyDatabase = $page('next125 dirty schema after interrupted import')
    . $page('next125 dirty wp_options after interrupted import')
    . $page('next125 dirty plugin settings after interrupted import')
    . $page('next125 dirty autoload index after interrupted import');
...
$walBytes = $makeWal([
    [2, 0, 'next125 wal draft option edit after hot recovery'],
    [2, 4, 'next125 wal committed siteurl after hot recovery'],
    [3, 0, 'next125 wal draft plugin settings reader tail'],
    [4, 4, 'next125 wal committed autoload cleanup reader tail'],
]);
...
$setup = static function (string $journalInput = null, string $walInput = null) use ($databasePath, $dirtyDatabase, $journalBytes, $walBytes): array {
    $root = sys_get_temp_dir() . '/port-libsqlite-wal-hot-reader-next125-' . bin2hex(random_bytes(4));
    ...
    if (!is_dir(dirname($localDatabase)) && !mkdir(dirname($localDatabase), 0777, true) && !is_dir(dirname($localDatabase))) {
        throw new RuntimeException('Unable to create next125 WAL hot-reader fixture directory');
    }
...
$cases = [
    ...
    'database has clean schema' => [static fn (): mixed => str_contains($restart()['database'], 'next125 clean schema before interrupted import'), true],
    'database has checkpointed siteurl' => [static fn (): mixed => str_contains($restart()['database'], 'next125 wal committed siteurl after hot recovery'), true],
    'database keeps clean plugin page while reader pins tail' => [static fn (): mixed => str_contains($restart()['database'], 'next125 clean plugin settings before interrupted import'), true],
    'database excludes dirty option page' => [static fn (): mixed => str_contains($restart()['database'], 'next125 dirty wp_options after interrupted import'), false],
    'database excludes reader tail plugin draft' => [static fn (): mixed => str_contains($restart()['database'], 'next125 wal draft plugin settings reader tail'), false],
    'wal preserved while reader pinned' => [static fn (): mixed => strlen($restart()['wal']), strlen($walBytes)],
    'wal keeps reader tail while pinned' => [static fn (): mixed => str_contains($restart()['wal'], 'next125 wal committed autoload cleanup reader tail'), true],
    'checkpoint status' => [static fn (): mixed => $restart()['applied']['checkpoint']['status'], 'wal-checkpoint-hot-journal-reader-current-source-next122'],
    ...
    'checkpoint row two reader label' => [static fn (): mixed => $restart()['applied']['checkpoint']['rows'][1]['reader_label'], 'next125 wal committed siteurl after hot recovery'],
    'checkpoint row three pinned label' => [static fn (): mixed => $restart()['applied']['checkpoint']['rows'][2]['pinned_next_label'], 'next125 wal draft plugin settings reader tail'],
    'checkpoint row three released label' => [static fn (): mixed => $restart()['applied']['checkpoint']['rows'][2]['released_next_label'], 'next125 wal draft plugin settings reader tail'],
    ...
    'dependency next125' => [static fn (): mixed => in_array('sqlite-wal-hot-journal-reader-checkpoint-vfs-apply-next125', $restart()['applied']['dependencies'], true), true],
    'dependency next122' => [static fn (): mixed => in_array('sqlite-wal-checkpoint-hot-journal-reader-current-source-next122', $restart()['applied']['dependencies'], true), true],
    ...
    'released database has autoload cleanup' => [static fn (): mixed => str_contains($released()['database'], 'next125 wal committed autoload cleanup reader tail'), true],
    ...
    'reserved skipped keeps dirty database' => [static fn (): mixed => str_contains($reserved()['database'], 'next125 dirty schema after interrupted import'), true],
...
foreach ($cases as $name => [$callback, $expected]) {
    $tests['wal hot journal reader checkpoint apply current source next125 ' . $name] = static function (TestRunner $t) use ($callback, $expected): void {
        $t->same($expected, $callback());
    };
}

$throws = [
    'empty path rejected' => static fn () => (new SQLiteVfsFileWriter(sys_get_temp_dir()))->applyWalCheckpointHotJournalReaderCurrentSourceNext125('', [1]),
    'missing database rejected' => static fn () => (new SQLiteVfsFileWriter(sys_get_temp_dir() . '/port-libsqlite-next125-missing-' . bin2hex(random_bytes(4))))->applyWalCheckpointHotJournalReaderCurrentSourceNext125($databasePath, [1]),
    'read only rejected' => static function () use ($setup, $databasePath): mixed {
        [$root] = $setup();
        return (new SQLiteVfsFileWriter($root, true))->applyWalCheckpointHotJournalReaderCurrentSourceNext125($databasePath, [1], 'restart', 1);
    },
    'immutable rejected' => static function () use ($setup, $databasePath): mixed {
        [$root] = $setup();
        return (new SQLiteVfsFileWriter($root, immutable: true))->applyWalCheckpointHotJournalReaderCurrentSourceNext125($databasePath, [1], 'restart', 1);
    },
    'bad mode rejected' => static function () use ($setup, $databasePath): mixed {
        [$root] = $setup();
        return (new SQLiteVfsFileWriter($root))->applyWalCheckpointHotJournalReaderCurrentSourceNext125($databasePath, [1], 'passive', 1);
    },
    'empty pages rejected' => static function () use ($setup, $databasePath): mixed {
        [$root] = $setup();
        return (new SQLiteVfsFileWriter($root))->applyWalCheckpointHotJournalReaderCurrentSourceNext125($databasePath, []);
    },
];

foreach ($throws as $name => $callback) {
    $tests['wal hot journal reader checkpoint apply current source next125 ' . $name] = static function (TestRunner $t) use ($callback): void {
