...
$makeFirstPage295298 = static function (int $pageCount): string {
...
$putPointerMapEntry295298 = static function (array &$pages, int $pageNumber, int $type, int $parentPageNumber): void {
...
$database295298 = static function (int $sliceNumber) use ($makeFirstPage295298, $putPointerMapEntry295298): SQLiteDatabase {
    $pages = array_fill(1, 110, str_repeat("\0", 512));
    $pages[1] = $makeFirstPage295298(110);
    ...
    foreach ([
        ...
        $putPointerMapEntry295298($pages, $pageNumber, $type, $parent);
    }

    return SQLiteDatabase::fromBytes(implode('', $pages));
};

$plan295298 = static function (int $sliceNumber, int $batchSize = 2) use ($database295298): SQLiteBTreeVacuumPointerMapFreeblockCurrentSourceNextPlan {
    $database = $database295298($sliceNumber);
...
foreach ([295, 296, 297, 298] as $sliceNumber) {
    $tests["btree vacuum pointermap freeblock current source next{$sliceNumber} keeps next295-298 splice receipts ordered"] = static function (TestRunner $t) use ($plan295298, $sliceNumber): void {
        $plan = $plan295298($sliceNumber);
        ...
        $t->same(6, $plan295298($sliceNumber, 3)->freelistSummary()['freelist_row_count']);
    };
}
