...
$cases = [
    ...
    'dependency records current source next slice' => ['dependencies.2', 'sqlite-utf16-nocase-current-source-next126'],
];

foreach ($cases as $name => [$path, $expected]) {
    $tests['utf16 nocase like current source next126 ' . $name] = static function (TestRunner $t) use ($plan, $valueAt, $path, $expected): void {
...
foreach ($stableCases as $name => [$path, $expected]) {
    $tests['utf16 nocase like current source next126 ' . $name] = static function (TestRunner $t) use ($plan, $valueAt, $stableRows, $path, $expected): void {
...
$tests['utf16 nocase like current source next126 rejects non utf16 text encoding'] = static function (TestRunner $t) use ($plan, $nextRows): void {
...
$tests['utf16 nocase like current source next126 rejects missing option bytes'] = static function (TestRunner $t) use ($plan, $nextRows): void {
...
$tests['utf16 nocase like current source next126 rejects non integer rowid'] = static function (TestRunner $t) use ($plan, $nextRows): void {
...
$tests['utf16 nocase like current source next126 rejects malformed utf16 bytes'] = static function (TestRunner $t) use ($plan, $nextRows): void {
...
$tests['utf16 nocase like current source next126 rejects non ascii nocase prefix range'] = static function (TestRunner $t) use ($plan): void {
...
$tests['utf16 nocase like current source next126 rejects case sensitive like on nocase index'] = static function (TestRunner $t) use ($plan): void {
    $result = $plan('plugin%', null, true);
    $t->same(false, $result['indexUsable']);
    $t->same('case_sensitive_like_requires_binary_index', $result['rejectedReason']);
};

$tests['utf16 nocase like current source next126 rejects bad escape length'] = static function (TestRunner $t) use ($plan): void {
    $t->throws(InvalidArgumentException::class, static fn () => $plan('plugin%', 'xx'));
};

$tests['utf16 nocase like current source next126 rejects utf8 database encoding'] = static function (TestRunner $t) use ($plan): void {
