...
$plan782797 = static fn (?array $rows = null, ?array $samples = null): array => SQLitePlannerStat4ExpressionPartialCurrentSourceNextPlan::materializePreparedHandoffRangeContinuation(
    $prepared782797(),
...
$tests = [
    ...
    'planner stat4 expression partial current source next782797 cursor appended' => static fn (TestRunner $t) => $t->same('PrepareStat4ExpressionPartialPreparedHandoffRange', $plan782797()['cursorProgram'][array_key_last($plan782797()['cursorProgram'])]['opcode']),
    'planner stat4 expression partial current source next782797 cursor mode' => static fn (TestRunner $t) => $t->same('prepared-handoff-range-current-source-stat4-expression-partial-prep', $plan782797()['cursorProgram'][array_key_last($plan782797()['cursorProgram'])]['mode']),
    'planner stat4 expression partial current source next782797 cursor omits dynamic legacy opcode' => static fn (TestRunner $t) => $t->same(false, array_key_exists('legacyOpcode', $plan782797()['cursorProgram'][array_key_last($plan782797()['cursorProgram'])])),
    'planner stat4 expression partial current source next782797 cursor omits dynamic legacy mode' => static fn (TestRunner $t) => $t->same(false, array_key_exists('legacyMode', $plan782797()['cursorProgram'][array_key_last($plan782797()['cursorProgram'])])),
    ...
    'planner stat4 expression partial current source next782797 malformed needed column' => static function (TestRunner $t) use ($prepared782797, $current782797, $terms782797): void {
        $t->throws(InvalidArgumentException::class, static fn () => SQLitePlannerStat4ExpressionPartialCurrentSourceNextPlan::materializePreparedHandoffRangeContinuation($prepared782797(), $current782797(), $terms782797(), ['option_name', ''], 6));
    },
];
