...
$row191 = static fn (int $id, string $name, int|string $encoding): array => [
    'setting_id' => $id,
    'key_name_bytes' => $enc191($name, $encoding),
...
$plan191 = static fn (
    ...
    int $nextCookie = 191,
): array => SQLiteUtf16NocaseLikeRtrimCurrentSourceNextPlan::keyValueRowKeyPreparedPatternRebindPlan(
...
$cases191 = [
    ...
    'expression' => ['expression', 'rtrim(key_name) COLLATE NOCASE LIKE ? ESCAPE ? /* prepared UTF-16 rebind */'],
    ...
    'current source' => ['currentSource', 'main.app_settings@190'],
    'next source' => ['nextSource', 'main.app_settings@191'],
...
$tests['utf16 nocase like rtrim current source nextOneNineOne byte order only pattern can keep residual semantics'] = static function (TestRunner $t) use ($row191, $enc191): void {
    ...
    $result = SQLiteUtf16NocaseLikeRtrimCurrentSourceNextPlan::keyValueRowKeyPreparedPatternRebindPlan(
        $rows,
        $rows,
...
$tests['utf16 nocase like rtrim current source nextOneNineOne malformed prepared pattern is rejected'] = static function (TestRunner $t) use ($currentRows191, $nextRows191, $nextPatternBytes191, $currentEscapeBytes191, $nextEscapeBytes191): void {
    $t->throws(InvalidArgumentException::class, static fn () => SQLiteUtf16NocaseLikeRtrimCurrentSourceNextPlan::keyValueRowKeyPreparedPatternRebindPlan(
        $currentRows191,
        $nextRows191,
        "\x00\xd8",
        'UTF-16LE',
