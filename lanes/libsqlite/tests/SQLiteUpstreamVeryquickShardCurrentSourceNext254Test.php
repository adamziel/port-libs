...
function libsqlite_suite_next254_evidence(): SQLiteUpstreamSuiteEvidence
{
    return SQLiteUpstreamSuiteEvidence::fromManifestPath(__DIR__ . '/../UPSTREAM_TEST_MANIFEST.json');
}

function libsqlite_suite_next254_output(int $passLines = 96, int $assertions = 96, int $failures = 0): string
{
    $lines = ['Focused test run: 1 selected test files (root lock skipped)'];
    for ($i = 1; $i <= $passLines; $i++) {
        $lines[] = sprintf('PASS current source next254 veryquick shard admission case %02d', $i);
    }
 ...
 */
function libsqlite_suite_next254_rows(
    int $case = 1,
    ...
    string $nextHead = 'suite-upstream-veryquick-shard-current-source-next254'
): array {
    $script = sprintf('veryquick-current-source-next254-%02d.test', $case);

    return [
        [
            'unit' => 'suite-upstream-veryquick-shard-current-source-next254',
            'kind' => 'bounded-upstream-veryquick-shard-runner',
            'gap_id' => 'current-source-next254-veryquick-shard-gap',
            'gap_status' => 'removed',
            'removed_blocker' => 'next254 admits one focused veryquick shard row tied to launcher Base accepted HEAD af40b132 and integration source 8a447f44 without duplicating accepted next155 through next250 suite evidence',
            ...
            'artifact_path' => 'lanes/libsqlite/notes/yield-suite-upstream-veryquick-shard-current-source-next254.md',
 ...
 */
function libsqlite_suite_next254_record(
    array $rows,
    string $launcherBase = 'af40b132cf3058811d656962ffce5fe8de5c9b2d',
    string $dashboardSource = '8a447f445e5d2fd32fc9fd463117f585d1416551',
    string $statusSource = '8a447f445e5d2fd32fc9fd463117f585d1416551',
    ...
    return libsqlite_suite_next254_evidence()->upstreamVeryquickShardCurrentSourceNext254(
        $rows,
        654,
        127481,
        $launcherBase,
...
foreach (range(1, 80) as $case) {
    $tests[sprintf('current source next254 admits veryquick shard case %02d', $case)] = static function (TestRunner $t) use ($case): void {
        $record = libsqlite_suite_next254_record(libsqlite_suite_next254_rows($case));

        $t->same('current-source-next254-veryquick-shard-advanced', $record['status']);
        ...
        $t->same(['suite-upstream-veryquick-shard-current-source-next254'], $record['admitted_units']);
        $t->same(['batch214-current-source-anchor'], $record['preserved_units']);
        $t->contains(sprintf('veryquick-current-source-next254-%02d.test', $case), implode(',', $record['target_scripts']));
        $t->same(true, $record['counts_upstream_veryquick_shard_current_source_next254']);
...
$tests['current source next254 records authoritative launcher and source heads'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next254_record(libsqlite_suite_next254_rows(8));
    ...
    $t->same(['suite-upstream-veryquick-shard-current-source-next254'], $record['artifact_source_heads']);
};

$tests['current source next254 records target scripts and tier counts'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next254_record(libsqlite_suite_next254_rows(13));

    $t->same(127494, $record['tests_total_delta']);
    $t->same(['accepted-batch214-anchor.test', 'testrunner.test', 'veryquick-current-source-next254-13.test'], $record['target_scripts']);
...
$tests['current source next254 preserves already counted row without mapped inflation'] = static function (TestRunner $t): void {
    $rows = libsqlite_suite_next254_rows();
    $rows[0]['current_countable'] = true;
    $rows[0]['current_tests'] = 127482;

    $record = libsqlite_suite_next254_record($rows);

    $t->same('current-source-next254-veryquick-shard-preserved', $record['status']);
    $t->same(0, $record['mapped_delta']);
    $t->same(654, $record['next_mapped']);
    $t->same(['batch214-current-source-anchor', 'suite-upstream-veryquick-shard-current-source-next254'], $record['preserved_units']);
};

$tests['current source next254 blocks stale source provenance'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next254_record(
        libsqlite_suite_next254_rows(
...
$tests['current source next254 blocks unguarded and non local artifacts'] = static function (TestRunner $t): void {
    $rows = libsqlite_suite_next254_rows();
    $rows[0]['artifact_path'] = '/tmp/next254.md';
...
$tests['current source next254 blocks non zero runner artifacts'] = static function (TestRunner $t): void {
    $rows = libsqlite_suite_next254_rows();
    $rows[0]['exit'] = 1;
    $rows[0]['errors'] = 2;
...
$tests['current source next254 blocks missing removed blocker classification'] = static function (TestRunner $t): void {
    $rows = libsqlite_suite_next254_rows();
...
$tests['current source next254 blocks duplicate broad runner snapshot'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next254_record(
        libsqlite_suite_next254_rows(),
...
$tests['current source next254 blocks focused php admission mismatch'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next254_record(
        libsqlite_suite_next254_rows(),
        output: libsqlite_suite_next254_output(assertions: 83)
    );
...
$tests['current source next254 records exact focused php admission'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next254_record(libsqlite_suite_next254_rows());
...
$tests['current source next254 blocks missing focused runner summary'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next254_record(
        libsqlite_suite_next254_rows(),
...
$tests['current source next254 carries dependency closure and non overlap notes'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next254_record(libsqlite_suite_next254_rows());
...
$tests['current source next254 blocks missing next countability flag'] = static function (TestRunner $t): void {
    $rows = libsqlite_suite_next254_rows();
...
$tests['current source next254 keeps broad release parity unclaimed'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next254_record(libsqlite_suite_next254_rows(21));
...
$tests['current source next254 does not recount prior veryquick shards'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next254_record(libsqlite_suite_next254_rows(22));

    $t->same(false, $record['counts_upstream_veryquick_shard_current_source_next237']);
    $t->same(false, $record['counts_upstream_veryquick_shard_current_source_next212']);
};

$tests['current source next254 records focused shard dependency closure'] = static function (TestRunner $t): void {
    $record = libsqlite_suite_next254_record(libsqlite_suite_next254_rows(23));

    $t->contains('current-source next254 veryquick shard admission', $record['dependency_closure']);
