...
$tests = [
    ...
    'planner stat4 expression partial current source next254269 cursor appended' => static fn (TestRunner $t) => $t->same('PrepareStat4ExpressionPartialPreparedHandoffRange', $plan254269()['cursorProgram'][array_key_last($plan254269()['cursorProgram'])]['opcode']),
    'planner stat4 expression partial current source next254269 cursor mode' => static fn (TestRunner $t) => $t->same('prepared-handoff-range-current-source-stat4-expression-partial-prep', $plan254269()['cursorProgram'][array_key_last($plan254269()['cursorProgram'])]['mode']),
