...
use PortLibs\LibSqlite\SQLitePagerMasterJournalReaderCacheCurrentSourceNext174Plan;
...
$masterBytes = $databasePath . "-journal\n/srv/wp-content/database/wp-next174-meta.sqlite-journal\n";
$canonicalMembers = ['/srv/wp-content/database/wp-next174-meta.sqlite-journal', $databasePath . '-journal'];
$masterDigest = hash('sha256', implode("\n", $canonicalMembers));
$reorderedMasterBytes = "/srv/wp-content/database/wp-next174-meta.sqlite-journal\n" . $databasePath . "-journal\n";
$sourceId = 'next174-before-current-rollback-source';
...
$before = [
    1 => $page('next174 stale schema before current rollback journal'),
    2 => $page('next174 stale wp_options root before current rollback journal'),
    3 => $page('next174 stale active_plugins before current rollback journal'),
    4 => $page('next174 stale plugin settings before current rollback journal'),
    5 => $page('next174 unchanged comments before current rollback journal'),
    6 => $page('next174 stale transients before current rollback journal'),
    7 => $page('next174 stale cron before current rollback journal'),
    8 => $page('next174 stale optionmeta before current rollback journal'),
];
$recovered = [
    1 => $page('next174 recovered schema from current rollback journal'),
    2 => $page('next174 recovered wp_options root from current rollback journal'),
    3 => $page('next174 recovered active_plugins from current rollback journal'),
    4 => $page('next174 recovered plugin settings from current rollback journal'),
    6 => $page('next174 recovered transients from current rollback journal'),
    7 => $page('next174 recovered cron from current rollback journal'),
];
...
$nextWrites = [
    3 => $page('next174 rewritten active_plugins after rollback reader cache'),
    4 => $page('next174 rewritten plugin settings after rollback reader cache'),
];
...
$plan = static fn (
    ...
    ?string $source = null,
    int $epoch = 12,
): array => SQLitePagerMasterJournalReaderCacheCurrentSourceNext174Plan::plan(
    $path ?? $databasePath,
...
$cases = [
    'status' => [static fn (): mixed => $plan()['status'], 'pager-master-journal-reader-cache-current-source-next174'],
    ...
    'members' => [static fn (): mixed => $plan()['current_members'], [$databasePath . '-journal', '/srv/wp-content/database/wp-next174-meta.sqlite-journal']],
    'canonical members sorted' => [static fn (): mixed => $plan()['canonical_members'], $canonicalMembers],
    ...
    'read unchanged cache miss prefix' => [static fn (): mixed => $plan()['next_reads'][4]['prefix'], 'next174 unchanged comments before current rollback journal'],
    'read recovered prefix' => [static fn (): mixed => $plan()['next_reads'][6]['prefix'], 'next174 recovered cron from current rollback journal'],
    ...
    'write active before' => [static fn (): mixed => $plan()['next_writes'][0]['before_prefix'], 'next174 recovered active_plugins from current rollback journal'],
    'write active after' => [static fn (): mixed => $plan()['next_writes'][0]['after_prefix'], 'next174 rewritten active_plugins after rollback reader cache'],
    'write settings before' => [static fn (): mixed => $plan()['next_writes'][1]['before_prefix'], 'next174 recovered plugin settings from current rollback journal'],
    ...
    'final prefix page four write' => [static fn (): mixed => $plan()['final_prefixes'][4], 'next174 rewritten plugin settings after rollback reader cache'],
    ...
    'dependency marker' => [static fn (): mixed => in_array('sqlite-pager-master-journal-reader-cache-current-source-next174', $plan()['dependencies'], true), true],
    'dependency journal source fence' => [static fn (): mixed => in_array('sqlite-pager-reader-cache-rollback-journal-source-fence-next174', $plan()['dependencies'], true), true],
    ...
    'duplicate master members collapsed' => [static fn (): mixed => $plan(null, [1], [], $masterBytes . $databasePath . "-journal\n")['current_members'], [$databasePath . '-journal', '/srv/wp-content/database/wp-next174-meta.sqlite-journal']],
    'reordered master members retains canonical cache' => [static fn (): mixed => $plan([1 => $cacheEntry('schema-retained', $recovered[1])], [1], [], $reorderedMasterBytes)['retained_cache_page_numbers'], [1]],
    'reordered master members keeps cache hit' => [static fn (): mixed => $plan([1 => $cacheEntry('schema-retained', $recovered[1])], [1], [], $reorderedMasterBytes)['next_reads'][0]['cache_hit'], true],
    'reordered master members keeps digest' => [static fn (): mixed => $plan([1 => $cacheEntry('schema-retained', $recovered[1])], [1], [], $reorderedMasterBytes)['current_master_journal_digest'], $masterDigest],
    'new master member still invalidates' => [static fn (): mixed => $plan([1 => $cacheEntry('schema-retained', $recovered[1])], [1], [], $masterBytes . "/srv/wp-content/database/wp-next174-new.sqlite-journal\n")['reader_rows'][0]['reason'], 'reader_cache_master_journal_digest_mismatch_next174'],
];

foreach ($cases as $name => [$callback, $expected]) {
    $tests['pager master journal reader cache current source next174 ' . $name] = static function (TestRunner $t) use ($callback, $expected): void {
...
foreach ($throws as $name => $callback) {
    $tests['pager master journal reader cache current source next174 ' . $name] = static function (TestRunner $t) use ($callback): void {
