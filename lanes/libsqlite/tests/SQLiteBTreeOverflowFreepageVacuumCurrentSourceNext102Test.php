...
$rows102 = static fn (SQLiteBTreeOverflowVacuumFreepagePlan $plan, int $count = 5, ?int $parent = 3): array => $plan->overflowFreepageVacuumCurrentSourceNext102($count, $parent);

$cases = [
    'summary embeds next102 rows' => static fn (SQLiteBTreeOverflowVacuumFreepagePlan $plan): mixed => array_column($plan->toArray()['overflow_freepage_vacuum_current_source_next102'], 'page_number'),
...
$expected = [
    'summary embeds next102 rows' => [7, 10, 9, 8, 6],
    ...
    'negative allocation is rejected' => 'SQLite overflow freepage vacuum current-source next102 allocation count cannot be negative',
    'bad parent is rejected' => 'SQLite overflow freepage vacuum current-source next102 parent page must be null or at page 2 or later',
...
foreach ($cases as $name => $callback) {
    $tests['btree overflow freepage vacuum current source next102 ' . $name] = static function (TestRunner $t) use ($fixture102, $callback, $expected, $name): void {
        $t->same($expected[$name], $callback($fixture102()));
    };
}

foreach (range(1, 35) as $index) {
    $tests['btree overflow freepage vacuum current source next102 invariant ' . $index] = static function (TestRunner $t) use ($fixture102, $rows102, $index): void {
