...
$base = [
    ...
    'master_source_digest' => hash('sha256', 'master-next734'),
...
$tests['pager master journal reader cache current source next734 admits current VDBE literal and arithmetic opcode fences'] = static function (TestRunner $t) use ($plan): void {
    ...
    $t->same('reader-cache-stmt-vdbe-opcode-trace-branch-handoff-current-734', $result['current_reader_cache_stmt_vdbe_opcode_trace_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-shift-right-branch-handoff-current-734', $result['current_reader_cache_stmt_vdbe_shift_right_branch_handoff_token']);
    $t->same('reader-cache-stmt-vdbe-gt-branch-handoff-current-734', $result['current_reader_cache_stmt_vdbe_gt_branch_handoff_token']);
    $t->same(true, in_array('sqlite-pager-master-journal-reader-cache-current-source-next718', $result['dependencies'], true));
};

$tests['pager master journal reader cache current source plan dispatches full width calls to next734'] = static function (TestRunner $t) use ($database, $master, $masterBytes, $before, $pageSize, $recovered, $cacheEntry, $read, $base, $tokens, $headers, $variantFields): void {
...
$tests['pager master journal reader cache current source next734 carries next719 stale add-imm branch handoff invalidation'] = static function (TestRunner $t) use ($plan, $opCount): void {
    $result = $plan(['reader_cache_stmt_vdbe_add_imm_branch_handoff_token' => 'stmt-vdbe-add-imm-branch-handoff-old']);
    $t->same([1], $result['reader_cache_stmt_vdbe_add_imm_branch_handoff_invalidated_cache_page_numbers']);
    $t->same([1], $result['invalidated_cache_page_numbers']);
    $t->same(1, $opCount($result, 'invalidate_reader_cache_reader_cache_stmt_vdbe_add_imm_branch_handoff_current_source_next719'));
};

$tests['pager master journal reader cache current source next734 reopens stale gt branch handoff read ticket'] = static function (TestRunner $t) use ($plan, $opCount): void {
    $result = $plan([], ['reader_cache_stmt_vdbe_gt_branch_handoff_token' => 'stmt-vdbe-gt-branch-handoff-old']);
    $t->same(['read-options'], $result['reopen_reader_ids']);
    $t->same(false, $result['next_reads'][0]['cache_hit']);
    $t->same('reader_ticket_reader_cache_stmt_vdbe_gt_branch_handoff_predates_current_source', $result['next_reads'][0]['reader_cache_stmt_vdbe_gt_branch_handoff_token_reason']);
    $t->same(1, $opCount($result, 'reopen_reader_for_reader_cache_stmt_vdbe_gt_branch_handoff_current_source_next734'));
};

$tests['pager master journal reader cache current source next734 missing gt branch handoff token rejects'] = static function (TestRunner $t) use ($plan): void {
    $t->throws(Throwable::class, static fn () => $plan(['reader_cache_stmt_vdbe_gt_branch_handoff_token' => null]));
};
