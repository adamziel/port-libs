...
$plan241 = static fn (array $options = []): array => SQLiteTriggerRecursiveViewUpsertCurrentSourceNextPlan::executeCurrentCloseReceipt(
    $rows241,
    $currentInput241,
    $nextInput241,
    $currentView241,
    ...
    $options + [
        ...
        'reset_generation' => 'app-current-reset-241',
        'post_reset_current_source_token' => 'app.current.source.postreset.241',
        'post_reset_cursor' => 'app.returning.postreset.cursor.241',
        ...
        'following_generation' => 'app-following-current-241',
...
$tokenHeld241 = static fn (): array => $plan241(['auto_ack_current_source_upsert_closes_next241' => true, 'expected_current_source_upsert_close_token_next241' => 'app.current.source.upsert.close.stale.241']);
...
$custom241 = static fn (): array => $plan241([
    ...
    'current_source_upsert_close_token_next241' => 'app.current.source.upsert.close.custom.241',
...
$cases241 = [
    ...
    'savepoint retained' => [static fn (): mixed => $released241()['savepoint'], 'app_recursive_view_241'],
    ...
    'close token retained' => [static fn (): mixed => $released241()['current_source_upsert_close_token_next241'], 'app.current.source.upsert.close.241'],
    'custom close token retained' => [static fn (): mixed => $custom241()['current_source_upsert_close_token_next241'], 'app.current.source.upsert.close.custom.241'],
    ...
    'visible payload names released' => [static fn (): mixed => array_column($released241()['visible_returning_payloads_next241'], 'name'), ['app_summary_child', 'template_child', 'landing_url', 'next_module']],
    'held payload names missing' => [static fn (): mixed => array_column($missing241()['held_next_returning_payloads_next241'], 'name'), ['landing_url', 'next_module']],
