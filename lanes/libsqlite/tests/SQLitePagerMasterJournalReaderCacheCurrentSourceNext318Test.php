...
$pageSize = 512;
$database = '/srv/wp-content/database/wp-next318.sqlite';
$usersDatabase = '/srv/wp-content/database/wp-next318-users.sqlite';
...
$before = [1 => $formatPage('next318 stale schema'), 2 => $page('next318 stale options'), 3 => $page('next318 stale plugins'), 4 => $page('next318 stale users')];
$recovered = [1 => $formatPage('next318 current schema'), 2 => $page('next318 current options'), 3 => $page('next318 current plugins'), 4 => $page('next318 current users')];
...
$base = [
    'source_id' => 'pager-reader-cache-current-source-next318',
    'epoch' => 318,
    'format_signature' => hash('sha256', implode('|', [512, 4, 2, 318, 0])),
    'publication_generation' => 318,
    'master_source_digest' => hash('sha256', 'master-next318'),
...
$plan = static function (?array $cache = null, ?array $reads = null) use ($database, $master, $masterBytes, $before, $pageSize, $recovered, $base, $tokens, $headers, $cacheEntry, $read): array {
    return SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::variantNext318($database, $master, $masterBytes, implode('', $before), $pageSize, $recovered, $cache ?? [
...
$cases = [
    'status' => [static fn (): mixed => $plan()['status'], 'pager-master-journal-reader-cache-current-source-next318'],
    ...
    'dependencies include next318' => [static fn (): mixed => in_array('sqlite-pager-master-journal-reader-cache-current-source-next318', $plan()['dependencies'], true), true],
    ...
    'default plan points at next318' => [static fn (): mixed => $plan()['status'], 'pager-master-journal-reader-cache-current-source-next318'],
];

foreach ($cases as $name => [$callback, $expected]) {
    $tests['pager master journal reader cache current source next318 ' . $name] = static function (TestRunner $t) use ($callback, $expected): void {
        $t->same($expected, $callback());
    };
}

$tests['pager master journal reader cache current source next318 missing token rejects'] = static function (TestRunner $t) use ($plan, $cacheEntry, $recovered): void {
