...
$tests['real upstream corpus date affinity dynamic real date4 cites non-overlapping upstream loop'] = static function (TestRunner $t) use ($date4Start, $date4Count): void {
...
for ($i = $date4Start; $i < $date4Start + $date4Count; $i++) {
    ...
    $tests[sprintf('real upstream corpus date affinity dynamic real date4.test date4-%05d strftime libc format parity', $i)] = static function (TestRunner $t) use ($date4Format, $timestamp, $expectedDate4, $i): void {
...
$tests['real upstream corpus date affinity dynamic real date4 application audit rollup'] = static function (TestRunner $t) use ($date4Format, $date4Start, $date4Count, $expectedDate4): void {
