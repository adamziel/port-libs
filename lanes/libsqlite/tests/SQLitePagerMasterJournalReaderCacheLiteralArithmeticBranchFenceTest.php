...
$pageSize = 512;
$database = '/srv/wp-content/database/wp-next718.sqlite';
...
$formatPage = static function (string $label) use ($pageSize): string {
    ...
    $page = substr_replace($page, pack('N', 718), 60, 4);
...
$variantFields = array_merge($tokenFields, $next639654Fields, $next655670Fields, $next671686Fields, $next687702Fields, $next703718Fields);
$before = [1 => $formatPage('stale schema'), 2 => $page('stale options')];
$recovered = [1 => $formatPage('current schema'), 2 => $page('current options')];
$tokens = [$journal => 'member-main-current-718'];
$headers = [$journal => hash('sha256', 'main header next718')];
$base = [
    'source_id' => 'pager-reader-cache-current-source-next718',
    'epoch' => 718,
    'format_signature' => hash('sha256', implode('|', [512, 4, 2, 718, 0])),
    'publication_generation' => 718,
    'master_source_digest' => hash('sha256', 'master-next718'),
    'recovery_sequence' => 718,
...
foreach ($variantFields as $field) {
    $base[$field] = str_replace('_', '-', preg_replace('/_token$/', '', $field)) . '-current-718';
}
...
$plan = static fn (array $cacheExtra = [], array $readExtra = []): array => SQLitePagerMasterJournalReaderCacheCurrentSourceNextPlan::currentSourceVdbeLiteralArithmeticBranchFence(
    $database,
    $master,
    $masterBytes,
    implode('', $before),
...
$tests['pager master journal reader cache current source next718 admits current VDBE literal and arithmetic opcode fences'] = static function (TestRunner $t) use ($plan): void {
    $result = $plan();
    $t->same('pager-master-journal-reader-cache-current-source-next718', $result['status']);
    $t->same([], $result['invalidated_cache_page_numbers']);
    $t->same(['read-options' => true], $result['read_cache_hits']);
    $t->same('reader-cache-stmt-vdbe-null-branch-current-718', $result['current_reader_cache_stmt_vdbe_null_branch_token']);
...
$tests['pager master journal reader cache current source next718 invalidates stale shift right branch handoff cache'] = static function (TestRunner $t) use ($plan, $opCount): void {
    $result = $plan(['reader_cache_stmt_vdbe_shift_right_branch_handoff_token' => 'stmt-vdbe-shift-right-branch-handoff-old']);
    $t->same([1], $result['reader_cache_stmt_vdbe_shift_right_branch_handoff_invalidated_cache_page_numbers']);
    $t->same([1], $result['invalidated_cache_page_numbers']);
    $t->same(1, $opCount($result, 'invalidate_reader_cache_reader_cache_stmt_vdbe_shift_right_branch_handoff_current_source_next718'));
};

$tests['pager master journal reader cache current source next718 reopens stale shift right branch handoff read ticket'] = static function (TestRunner $t) use ($plan, $opCount): void {
    $result = $plan([], ['reader_cache_stmt_vdbe_shift_right_branch_handoff_token' => 'stmt-vdbe-shift-right-branch-handoff-old']);
    $t->same(['read-options'], $result['reopen_reader_ids']);
    $t->same(false, $result['next_reads'][0]['cache_hit']);
    $t->same('reader_ticket_reader_cache_stmt_vdbe_shift_right_branch_handoff_predates_current_source', $result['next_reads'][0]['reader_cache_stmt_vdbe_shift_right_branch_handoff_token_reason']);
    $t->same(1, $opCount($result, 'reopen_reader_for_reader_cache_stmt_vdbe_shift_right_branch_handoff_current_source_next718'));
};

$tests['pager master journal reader cache current source next718 missing shift right branch handoff token rejects'] = static function (TestRunner $t) use ($plan): void {
    $t->throws(Throwable::class, static fn () => $plan(['reader_cache_stmt_vdbe_shift_right_branch_handoff_token' => null]));
};
