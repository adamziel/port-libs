...
For the JSON-LD title disambiguation, replace-font-tags, compact metadata, RTL direction, and data URL image slices, the upstream implementation and fixture inventory were inspected statically rather than rerunning the full upstream JavaScript suite again. The hydrated sparse checkout still exposes 130 fixture pages and 390 fixture files, and the lane now copies 56 Mozilla fixture pages, including `test-pages/telegraph`, `test-pages/nytimes-1`, `test-pages/nytimes-2`, `test-pages/nytimes-3`, `test-pages/nytimes-4`, `test-pages/guardian-1`, `test-pages/ars-1`, `test-pages/heise`, `test-pages/cnet-svg-classes`, `test-pages/medium-1`, `test-pages/medium-2`, `test-pages/medium-3`, `test-pages/data-url-image`, `test-pages/keep-images`, `test-pages/wordpress`, `test-pages/schema-org-context-object`, `test-pages/003-metadata-preferred`, `test-pages/004-metadata-space-separated-properties`, `test-pages/title-and-h1-discrepancy`, `test-pages/replace-font-tags`, and `test-pages/rtl-1` through `test-pages/rtl-4`.
...
- Readability-local PHP coverage: 106 behavior tests, 1029 assertions, 0 failures.
- Clean-worktree root `php tools/run-tests.php` with only readability lane changes: green with 183 test files, 17817 assertions, and 0 failures. The shared dirty checkout currently has unrelated gitoxide and quadrable failures outside this lane.
...
- Phase: cloned static inventory plus upstream npm runner evidence and Mozilla fixture/JSON-LD/video/lazy media/data-url-image/keep-images-medium-section-wrapper/ad wrapper/title-heading/title-h1-discrepancy/schema-org-context-object/wordpress-articleBody/out-of-band figure/post-process/classesToPreserve/leading-action-bar/heading-less-news-chrome/single-paragraph-wrapper/base-url-relative-uri/div-to-paragraph/js-link/clean-links-uri-trim-footer-parity-selected-root-nbsp/single-article/empty-paragraph/single-cell-table/table-style/links-in-tables/keep-tabular-data/data-table-marker/remove-aria-hidden/hidden-nodes/visibility-hidden/invisible-node/first-paragraph-excerpt/br-chain/scaffold-heading/basic-tag-cleaning/link-fieldset/script-style-social/title-separator/ordered-list/hash-link-density/body-byline/entity-unescape/replace-font-tags/rtl-direction/transparent-section-wrapper/readability-page-wrapper/comment-delimited-phrasing/block-boundary-spacing/medium-1-empty-heading-boundary/cnet-svg-sprite-dedupe/v8-blog-published-time-boundary/medium-2-syndication-footer/medium-3-hr-page-break/heise-single-article-promotion/ars-1-figure-credit-cleanup/guardian-1-media-caption-retention/nytimes-1-rich-caption-hidden-feedback-cleanup/nytimes-2-continuation-story-interrupter/nytimes-3-figure-itemid-lazyimage-related-card-cleanup/nytimes-4-debt-article-graphics-related-link-cleanup/nytimes-5-section-front-collection-card-pruning/telegraph-text-section-publisher-media-chrome-cleanup/figure-image-blocks/parse-options-custom-video-keepclasses-maxelems-charthreshold mappings.
- Native PHP lane tests: 106 passing, 0 failing, 1029 assertions.
- Latest readability-local verification: targeted `TestRunner` invocation for `ArticleExtractorTest.php` passes 106 tests, 1029 assertions, and 0 failures.
- Latest clean-worktree root verification: `php tools/run-tests.php` is green with 183 test files, 17817 assertions, and 0 failures when only readability lane changes are applied. The shared dirty checkout root run currently reports 183 test files, 17949 assertions, and 2 unrelated failures in `lanes/gitoxide/tests/ReferenceStoreTest.php` and `lanes/quadrable/tests/QuadbStoreTest.php`.
- Upstream runner verification: `npm test` passes 1984 Mozilla Mocha tests, 0 failures, including the 2026-05-22 WordPress articleBody microdata rerun in 41s. Targeted oracles pass `npm test -- --grep keep-images` with 15 checks in 825ms, `npm test -- --grep schema-org-context-object` with 17 checks in 759ms, `npm test -- --grep 'keep-tabular-data|replace-brs'` with 26 checks in 648ms, `npm test -- --grep clean-links` with 13 checks in 599ms, `npm test -- --grep medium-1` with 15 checks in 750ms, `npm test -- --grep cnet-svg-classes` with 15 checks in 639ms, `npm test -- --grep v8-blog` with 15 checks in 970ms, `npm test -- --grep lazy-image-1` with 17 checks in 780ms, `npm test -- --grep medium-2` with 15 checks in 157ms, `npm test -- --grep medium-3` with 17 checks in 1s, `npm test -- --grep heise` with 15 checks in 603ms, `npm test -- --grep ars-1` with 15 checks in 200ms, `npm test -- --grep guardian-1` with 17 checks in 1s, `npm test -- --grep nytimes-1` with 15 checks in 810ms, `npm test -- --grep nytimes-2` with 15 checks in 679ms, `npm test -- --grep nytimes-3` with 15 checks in 964ms, `npm test -- --grep nytimes-4` with 15 checks in 581ms, `npm test -- --grep nytimes-5` with 15 checks in 1s on the 2026-05-23 port-readability run, `npm test -- --grep telegraph` with 15 checks in 796ms, and `npm test -- --grep 'custom video regex|maxElemsToParse|keepClasses'` with 5 checks in 114ms; focused upstream Node oracle confirms `charThreshold` retry/nonempty and null outcomes.
- Blocker: no readability-local, upstream targeted, or clean-commit root blocker is active. The shared dirty checkout has unrelated gitoxide and quadrable failures outside readability scope.
- Current work: native NYT `nytimes-5` fixture mapping copies Mozilla source/expected/metadata, preserves metadata/readerable parity, keeps expected image-source and h2/h3 heading parity, retains 22 media figures and selected section-front summaries, removes NYT ad wrappers, prunes the secondary highlights rail, drops latest-stream cards, and prevents section-front tab navigation from entering WordPress block output. Telegraph, NYT article fixtures, Guardian, and parse-option handling remain green.

## Next Slice

Map another bounded collection/front-page fixture such as `bbc-1` or `wapo-1`, focusing on broad section-front pruning without overfitting NYT class names.
