...
- Native PHP lane tests: 148 passing, 0 failing, 1849 assertions.
- Latest readability-local verification: `php tools/run-tests.php lanes/readability/tests` passes 1 selected test file, 1849 assertions, and 0 failures.
...
- Current work: native standalone-video WordPress serialization maps retained upstream `iframe`/`object`/`embed` media from copied Mozilla fixtures and trusted custom-provider migration HTML into core HTML blocks, while preserving inline iframe paragraphs and existing paragraph/list/table/quote/media behavior.
