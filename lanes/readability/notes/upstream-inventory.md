...
For the JSON-LD title disambiguation and clean-links footer parity slices, the upstream implementation and fixture inventory were inspected statically rather than rerunning the full upstream JavaScript suite again. The hydrated sparse checkout still exposes 130 fixture pages and 390 fixture files, and the lane now copies 32 Mozilla fixture pages, including `test-pages/title-and-h1-discrepancy`.
...
- Native PHP lane tests: 63 passing, 0 failing, 552 assertions.
- Latest readability-local verification: direct `ArticleExtractorTest.php` run passes 63 tests, 552 assertions, and 0 failures.
- Latest required root verification: `php tools/run-tests.php` passes 125 test files, 11020 assertions, and 0 failures.
...
- Blocker: no readability-local or root execution blocker. Exact structural HTML parity is still incomplete for copied Medium lazy-image fixtures, including the readability-page root wrapper, non-title h1 removal, author/avatar wrapper nesting, and remaining wrapper/id differences.
- Current work: native extraction now removes compact trailing link/media footer bars after substantial article content, making copied `clean-links` hrefs, images, and paragraph count match upstream expected output while keeping popup JavaScript/event cleanup intact. It also keeps hidden/invisible/modal cleanup, fallback-image preservation, fallback excerpts, body byline extraction/removal, JSON-LD title disambiguation, title separator cleanup, duplicate heading cleanup, URI normalization, hash-only link-density weighting, metadata entity replacement, copied `title-and-h1-discrepancy`, `ol`, `001`, and `005-unescape-html-entities` fixtures, platform chrome cleanup, and WordPress block output passing.

## Next Slice

Map another compact Mozilla fixture such as `replace-font-tags`, or return to remaining Medium lazy-image structural HTML parity around readability-page wrappers and non-title h1 removal.
