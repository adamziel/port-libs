...
- Native PHP lane tests: 153 passing, 0 failing, 1901 assertions.
- Latest readability-local verification: `php tools/run-tests.php lanes/readability/tests/ArticleExtractorTest.php` passes 1 selected test file, 1901 assertions, and 0 failures.
...
- Current work: captioned retained media wrappers now serialize as WordPress HTML review blocks when a div/section directly contains or tightly nests one iframe/object/embed/video/audio payload with only short caption text. Inline paragraph embeds stay paragraph-scoped.
- Focused smoke: `php lanes/readability/examples/wordpress-captioned-embed-wrapper-import.php` reports `Captioned embed HTML blocks: 2`, `Embed wrapper retained: yes`, `Nested wrapper retained: yes`, `Inline embed paragraph retained: yes`, `Paragraph-wrapped embed wrapper: no`, and `Paragraph blocks: 3`.
