...
- Current work: native compact ordered-list and explicitly marked unordered-list WordPress serialization maps the copied Mozilla `ol` fixture plus `data-wp-block-list` migration markers to core list blocks while preserving existing long-list fixture review behavior.
...
Map `lifehacker-working` or another remaining Kinja/comment-heavy fixture with a targeted upstream oracle, or broaden unmarked ordinary list-block serialization only with deliberate updates to the affected long-list fixture expectations.
