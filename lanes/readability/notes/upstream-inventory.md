...
- Native PHP lane tests: 149 passing, 0 failing, 1854 assertions.
- Latest readability-local verification: `php tools/run-tests.php lanes/readability/tests` passes 1 selected test file, 1854 assertions, and 0 failures.
...
- Current work: native standalone-media WordPress serialization maps retained upstream `iframe`/`object`/`embed` media plus standalone `video`/`audio` elements into core HTML blocks, while preserving inline iframe paragraphs and existing paragraph/list/table/quote/media behavior.
