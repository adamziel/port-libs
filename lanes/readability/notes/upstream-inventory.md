...
- Native PHP lane tests: 150 passing, 0 failing, 1861 assertions.
- Latest readability-local verification: `php tools/run-tests.php lanes/readability/tests/ArticleExtractorTest.php` passes 1 selected test file, 1861 assertions, and 0 failures.
...
- Current work: native class-weight rearm retry maps the upstream-style threshold fallback where extraction can relax scoring flags after stricter attempts fail the character threshold, preventing a short high-weight teaser from suppressing a longer recoverable article body.
- Focused smoke: `php lanes/readability/examples/wordpress-class-weight-rearm-import.php` reports title `Class Weight Rearm Import`, recovered body `yes`, teaser retained `no`, and 2 paragraph blocks.
