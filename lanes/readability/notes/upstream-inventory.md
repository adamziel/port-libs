...
- Native PHP lane tests: 78 passing, 0 failing, 729 assertions.
- Latest readability-local verification: direct `ArticleExtractorTest.php` run passes 78 tests, 729 assertions, and 0 failures.
- Latest required root verification: `php tools/run-tests.php` passes with 149 test files, 13615 assertions, and 0 failures.
...
- Blocker: no readability-local, upstream targeted, or root harness blocker is active. Remaining exact-parity gaps are structural HTML parity for copied Medium keep-images/lazy-image fixtures and default/root wrapper strategy.
- Current work: native extraction now trims whitespace-only selected-root text nodes after cleanup, treating NBSP as boundary whitespace. This maps Mozilla `clean-links` exact normalized content text parity and adds a WordPress classic table-export scenario while preserving internal NBSP text. Existing coverage still includes `schema-org-context-object` comment-delimited contributor text cleanup, `keep-tabular-data`/`replace-brs` block-boundary spacing, `keep-images`, `data-url-image`, partial upstream class/id candidate weighting, strong `itemprop=articleBody` selection for WordPress BlogPosting pages, optional `div#readability-page-1.page` fixture output, copied `wordpress`, `rtl-1` through `rtl-4`, compact metadata fixtures, hidden/invisible/modal cleanup, fallback-image preservation, fallback excerpts, body byline extraction/removal, JSON-LD title disambiguation, title separator cleanup, duplicate heading cleanup, URI normalization, hash-only link-density weighting, metadata entity replacement, copied `replace-font-tags`, `title-and-h1-discrepancy`, `ol`, `001`, and `005-unescape-html-entities` fixtures, clean-links footer parity, platform chrome cleanup, and WordPress block output.

## Next Slice

Return to `keep-images`/`lazy-image-1` structural wrapper parity, especially Medium image/avatar wrapper shape and default/root wrapper strategy.
