...
- Blocker: none active. Focused readability and root aggregate verification are green.
- Current work: native Mozilla `lifehacker-post-comment-load` fixture mapping copies Mozilla source/expected/metadata, verifies Lifehacker/Kinja metadata/text/image/list parity, excludes Kinja comment/ad/follow/navigation chrome, and adds scoped WordPress list-block serialization for retained Kinja text-annotation editorial lists.
