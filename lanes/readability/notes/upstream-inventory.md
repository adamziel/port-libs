...
- Readability-local PHP coverage: 85 behavior tests, 784 assertions, 0 failures.
- Required root `php tools/run-tests.php`: passes with 162 test files, 14874 assertions, and 0 failures.
...
- Phase: cloned static inventory plus upstream npm runner evidence and Mozilla fixture/JSON-LD/video/lazy media/data-url-image/keep-images-medium-section-wrapper/ad wrapper/title-heading/title-h1-discrepancy/schema-org-context-object/wordpress-articleBody/out-of-band figure/post-process/leading-action-bar/heading-less-news-chrome/single-paragraph-wrapper/base-url-relative-uri/div-to-paragraph/js-link/clean-links-uri-trim-footer-parity-selected-root-nbsp/single-article/empty-paragraph/single-cell-table/table-style/links-in-tables/keep-tabular-data/data-table-marker/remove-aria-hidden/hidden-nodes/visibility-hidden/invisible-node/first-paragraph-excerpt/br-chain/scaffold-heading/basic-tag-cleaning/link-fieldset/script-style-social/title-separator/ordered-list/hash-link-density/body-byline/entity-unescape/replace-font-tags/rtl-direction/transparent-section-wrapper/readability-page-wrapper/comment-delimited-phrasing/block-boundary-spacing/medium-1-empty-heading-boundary/cnet-svg-sprite-dedupe/v8-blog-published-time-boundary mappings.
- Native PHP lane tests: 85 passing, 0 failing, 784 assertions.
- Latest readability-local verification: direct `ArticleExtractorTest.php` run passes 85 tests, 784 assertions, and 0 failures.
- Latest required root verification: `php tools/run-tests.php` passes with 162 test files, 14874 assertions, and 0 failures.
- Upstream runner verification: `npm test` passes 1984 Mozilla Mocha tests, 0 failures, including the 2026-05-22 WordPress articleBody microdata rerun in 41s. Targeted oracles pass `npm test -- --grep keep-images` with 15 checks in 825ms, `npm test -- --grep schema-org-context-object` with 17 checks in 759ms, `npm test -- --grep 'keep-tabular-data|replace-brs'` with 26 checks in 648ms, `npm test -- --grep clean-links` with 13 checks in 599ms, `npm test -- --grep medium-1` with 15 checks in 750ms, `npm test -- --grep cnet-svg-classes` with 15 checks in 639ms, and `npm test -- --grep v8-blog` with 15 checks in 970ms.
- Blocker: no readability-local, upstream targeted, or required root-test blocker is active.
- Current work: native metadata extraction now matches Mozilla's publishedTime boundary for the current slice: JSON-LD, `article:published_time`, and `parsely-pub-date` can populate the field, but generic visible `<time datetime>` and `pubdate` fallbacks do not. The copied Mozilla `v8-blog` fixture maps the null publishedTime boundary, and `wordpress-visible-time-metadata-boundary.php` shows visible template dates stay out of trusted post metadata.
