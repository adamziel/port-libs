...
- Native PHP lane tests: 77 passing, 0 failing, 722 assertions.
- Latest readability-local verification: direct `ArticleExtractorTest.php` run passes 77 tests, 722 assertions, and 0 failures.
- Latest required root verification: `php tools/run-tests.php` passes with 149 test files, 13500 assertions, and 0 failures.
- Upstream runner verification: `npm test` passes 1984 Mozilla Mocha tests, 0 failures, including the 2026-05-22 WordPress articleBody microdata rerun in 41s. Targeted oracles pass `npm test -- --grep keep-images` with 15 checks in 825ms, `npm test -- --grep schema-org-context-object` with 17 checks in 759ms, and `npm test -- --grep 'keep-tabular-data|replace-brs'` with 26 checks in 648ms.
- Blocker: no readability-local, upstream targeted, or root harness blocker is active. Remaining exact-parity gaps are clean-links leading NBSP/selected-root whitespace and structural HTML parity for copied Medium keep-images/lazy-image fixtures and default/root wrapper strategy.
- Current work: native extraction now inserts separator whitespace at text-bearing block/table/list boundaries after cleanup, mapping Mozilla `keep-tabular-data` and `replace-brs` exact normalized text parity and fixing the shared copied-fixture spacing family. Existing coverage still includes `schema-org-context-object` comment-delimited contributor text cleanup, `keep-images`, `data-url-image`, partial upstream class/id candidate weighting, strong `itemprop=articleBody` selection for WordPress BlogPosting pages, optional `div#readability-page-1.page` fixture output, copied `wordpress`, `rtl-1` through `rtl-4`, compact metadata fixtures, hidden/invisible/modal cleanup, fallback-image preservation, fallback excerpts, body byline extraction/removal, JSON-LD title disambiguation, title separator cleanup, duplicate heading cleanup, URI normalization, hash-only link-density weighting, metadata entity replacement, copied `replace-font-tags`, `title-and-h1-discrepancy`, `ol`, `001`, and `005-unescape-html-entities` fixtures, clean-links footer parity, platform chrome cleanup, and WordPress block output.

## Next Slice

Fix clean-links leading NBSP/selected-root whitespace next, then return to `keep-images`/`lazy-image-1` structural wrapper parity.
