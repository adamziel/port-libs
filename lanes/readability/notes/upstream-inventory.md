...
- Readability-local PHP coverage: 89 behavior tests, 822 assertions, 0 failures.
- Required root `php tools/run-tests.php`: passes with 164 test files, 15262 assertions, and 0 failures.
...
- Native PHP lane tests: 89 passing, 0 failing, 822 assertions.
- Latest readability-local verification: direct `ArticleExtractorTest.php` run passes 89 tests, 822 assertions, and 0 failures.
- Latest required root verification: `php tools/run-tests.php` passes with 164 test files, 15262 assertions, and 0 failures.
- Upstream runner verification: `npm test` passes 1984 Mozilla Mocha tests, 0 failures, including the 2026-05-22 WordPress articleBody microdata rerun in 41s. Targeted oracles pass `npm test -- --grep keep-images` with 15 checks in 825ms, `npm test -- --grep schema-org-context-object` with 17 checks in 759ms, `npm test -- --grep 'keep-tabular-data|replace-brs'` with 26 checks in 648ms, `npm test -- --grep clean-links` with 13 checks in 599ms, `npm test -- --grep medium-1` with 15 checks in 750ms, `npm test -- --grep cnet-svg-classes` with 15 checks in 639ms, `npm test -- --grep v8-blog` with 15 checks in 970ms, `npm test -- --grep lazy-image-1` with 17 checks in 780ms, `npm test -- --grep medium-2` with 15 checks in 157ms, and `npm test -- --grep medium-3` with 17 checks in 1s.
- Blocker: no readability-local, upstream targeted, or required root-test blocker is active.
- Current work: native hr-separated Medium page containers now unwrap into sibling article sections for optional oracle output and drop separator rules before WordPress block output. The copied Mozilla `medium-3` fixture maps the upstream boundary, and `wordpress-medium-page-break-cleanup.php` shows synthetic page rules are not serialized as paragraph blocks.
