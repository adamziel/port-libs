...
For the JSON-LD title disambiguation, clean-links footer parity, replace-font-tags, compact metadata, and RTL direction slices, the upstream implementation and fixture inventory were inspected statically rather than rerunning the full upstream JavaScript suite again. The hydrated sparse checkout still exposes 130 fixture pages and 390 fixture files, and the lane now copies 39 Mozilla fixture pages, including `test-pages/003-metadata-preferred`, `test-pages/004-metadata-space-separated-properties`, `test-pages/title-and-h1-discrepancy`, `test-pages/replace-font-tags`, and `test-pages/rtl-1` through `test-pages/rtl-4`.
...
- Phase: cloned static inventory plus upstream npm runner evidence and Mozilla fixture/JSON-LD/video/lazy media/ad wrapper/title-heading/title-h1-discrepancy/out-of-band figure/post-process/leading-action-bar/single-paragraph-wrapper/base-url-relative-uri/div-to-paragraph/js-link/clean-links-uri-trim-footer-parity/single-article/empty-paragraph/single-cell-table/table-style/links-in-tables/keep-tabular-data/data-table-marker/remove-aria-hidden/hidden-nodes/visibility-hidden/invisible-node/first-paragraph-excerpt/br-chain/scaffold-heading/basic-tag-cleaning/link-fieldset/script-style-social/title-separator/ordered-list/hash-link-density/body-byline/entity-unescape/replace-font-tags/rtl-direction mappings.
- Native PHP lane tests: 68 passing, 0 failing, 613 assertions.
- Latest readability-local verification: direct `ArticleExtractorTest.php` run passes 68 tests, 613 assertions, and 0 failures.
- Latest required root verification: `php tools/run-tests.php` rerun after this slice passed 130 test files, 11664 assertions, and 0 failures.
...
- Blocker: no readability-local or root execution blocker is active. Exact structural HTML parity is still incomplete for copied Medium lazy-image fixtures, including the readability-page root wrapper, non-title h1 removal, author/avatar wrapper nesting, and remaining wrapper/id differences.
- Current work: native extraction now maps copied `rtl-1` through `rtl-4` by deriving article direction metadata from selected content ancestors, while preserving upstream article-only `dir` null behavior. It also keeps compact metadata fixtures, hidden/invisible/modal cleanup, fallback-image preservation, fallback excerpts, body byline extraction/removal, JSON-LD title disambiguation, title separator cleanup, duplicate heading cleanup, URI normalization, hash-only link-density weighting, metadata entity replacement, copied `replace-font-tags`, `title-and-h1-discrepancy`, `ol`, `001`, and `005-unescape-html-entities` fixtures, clean-links footer parity, platform chrome cleanup, and WordPress block output passing.

## Next Slice

Return to remaining Medium lazy-image structural HTML parity around readability-page wrappers, non-title h1 removal, author/avatar wrapper nesting, and remaining wrapper/id differences.
