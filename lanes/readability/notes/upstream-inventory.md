...
## 2026-05-26 Isolated Slice: WordPress Base URL Canonicalization

- Behavior cluster: copied Mozilla `base-url`, `base-url-base-element`, and `base-url-base-element-relative` fixture coverage for document URL plus `<base href>` URL resolution.
- Status delta: extended the existing native fixture tests so WordPress block output must preserve the same resolved anchor and image URL sets as upstream expected HTML, including relative-base, root-relative, fragment, and absolute URL boundaries. Added `examples/wordpress-base-url-canonicalization.php` as a local smoke for migration previews/media references.
- Focused evidence: `php -l lanes/readability/tests/ArticleExtractorTest.php` passed; `php -l lanes/readability/examples/wordpress-base-url-canonicalization.php` passed; `php tools/run-tests.php lanes/readability/tests/ArticleExtractorTest.php` passed 1 selected test file / 1936 assertions / 0 failures; `php lanes/readability/examples/wordpress-base-url-canonicalization.php` printed resolved relative base link yes, root link yes, fragment link yes, and 5 resolved image references.
- Blocker: no focused readability blocker. Root harness not run - isolated micro-slice.
- Next task: map `lifehacker-working` or another remaining Kinja/comment-heavy fixture with a targeted upstream oracle when an upstream cache is available, or add focused WordPress evidence for another already-copied fixture with user-visible migration impact.
- Dependency closure: no new support component is needed; this slice reuses the existing DOM post-processing URL resolver, Mozilla fixture comparison helpers, and WordPress block serializer.
...
- Native PHP lane tests: 154 passing, 0 failing, 1936 assertions.
- Latest readability-local verification: `php tools/run-tests.php lanes/readability/tests/ArticleExtractorTest.php` passes 1 selected test file, 1936 assertions, and 0 failures.
...
- Current work: copied Mozilla base URL fixtures now prove WordPress block output keeps upstream-resolved anchor and image URLs for source URL, absolute base, and relative base boundaries.
- Focused smoke: `php lanes/readability/examples/wordpress-base-url-canonicalization.php` reports the base-relative link, root link, and fragment link resolved yes with 5 resolved image references.
- Dependency closure: no new support component is needed; this reuses the existing native DOM post-processing URL resolver, copied Mozilla fixture comparison helpers, and WordPress block serializer.
