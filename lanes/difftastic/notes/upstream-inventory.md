...
- The upstream Go keyword/builtin/type highlight boundary is inspected through the existing difftastic `tree_highlights` promotion boundary and Go language catalog support. The PHP lane maps Go keywords and builtin constants into the keyword display bucket, maps primitive/builtin type names into the type bucket, and deliberately leaves ordinary function, parameter, field, and struct identifiers normal unless a future upstream-query-backed slice promotes them.
