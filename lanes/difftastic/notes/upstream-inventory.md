...
```text
125 test files, 11107 assertions, 0 failures
```

An immediately preceding root attempt from the same dynamic-import slice passed the Difftastic section but ended red at `125 test files, 11097 assertions, 1 failures`; that aggregate failure did not reproduce on the same-snapshot rerun above.

The difftastic-focused test file remains green with 80 tests, 398 assertions, and 0 failures via a direct `TestRunner` invocation.
