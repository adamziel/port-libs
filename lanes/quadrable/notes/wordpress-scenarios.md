...
- `examples/wordpress-sync-fuzz-budget.php` runs deterministic upstream-shaped sync-fuzz trials and reports request/response budgets for a Playground snapshot preflight.
...
Run `SyncFuzzer::run(500)` as an explicit heavy parity check, then port persisted LMDB-style branch node-id equality for the upstream sync-fuzz loop.
