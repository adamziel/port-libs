...
After that, `make -r test` passed all upstream scenarios on 2026-05-22, and was reconfirmed after the tracked fork/branch node-id slice, after the bounded tracked diff node-id slice, and after the native memStore-range tracked-node slice. `-r` disables GNU Make built-in rules; plain `make test` now gets past the prior missing tooling, but this shell selects the built-in C++ compile rule for `check.o` and omits the repository `-Iinclude -Iexternal -Iexternal/hoytech-cpp -Iexternal/docopt.cpp` include flags, so it fails on `#include "quadrable.h"`. With built-ins disabled, the upstream Makefile's own pattern rule is used and the upstream test binary reports `All tests OK`.
...
- `NodeIdTest.php` maps focused `saves nodeId`, `fork`, `re-use leafs`, `memStore basic`, `memStore-only env`, and `Sync::diff` leaf node-id behavior for a native in-memory tracked tree: new leaf writes return non-zero node ids, duplicate unchanged puts return zero, changed puts return fresh ids, deletes return the removed leaf id, missing deletes return zero, saved branch head ids can be checked out to restore old and new trees, copy-on-write updates reuse unchanged branch child ids, reused leaves preserve their original leaf node ids across a fresh checkout, memStore-mode writes allocate from upstream's `firstMemStoreNodeId` range, detached overlays can add volatile records while the base tree remains unchanged, changed/added diffs report the target leaf node id, deleted diffs report the source leaf node id, bounded scan callbacks match final diff node ids, and WordPress snapshot compact rebuild/fork/diff/memStore overlay examples keep authenticated old and updated roots addressable by node id.

The native PHP hash primitive now uses a lane-local pure-PHP BLAKE2s-256 implementation matching upstream `Key::hash`, including multi-block values. Root/proof bytes for the mapped in-memory and proof surfaces are digest-compatible with upstream. The sync slice now has native scan-time diff callbacks and deterministic small sync-fuzz convergence, and the tracked tree covers leaf node-id output/reuse, saved branch-head checkout, copy-on-write unchanged branch reuse, tracked leaf diff node ids, bounded scan/final diff node-id equality, and memStore-range allocation for volatile detached writes. Full upstream-scale randomized sync fuzz parity, imported shadow node-id equality, and the named-head LMDB/MemStore guard from upstream `memStore forking from lmdb` remain unported.
