...
The native file-backed `QuadbStore` slice used a precise static upstream mapping instead of rerunning a new upstream binary command: `quadb.cpp` first reloads `detachedHead`, then `currHead`, and otherwise checks out `master`; `checkout <head>` persists `currHead`, bare `checkout` persists a detached node id at command commit, `fork <head>` copies the current node id into a named head and makes it current, and `import --int` / `export --int` use separator-delimited integer-key lines. The README command-line sections for heads, detached heads, `checkout`, `fork`, `import`, and `export` corroborate that shape. Native PHP maps that subset with a JSON-backed directory state file rather than shelling out to `quadb`.
