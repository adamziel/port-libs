...
After that, `make -r test` passed all upstream scenarios on 2026-05-22, and was reconfirmed after the tracked fork/branch node-id slice, after the bounded tracked diff node-id slice, after the native memStore-range tracked-node slice, and after the named-head memStore fork guard slice. `-r` disables GNU Make built-in rules; plain `make test` now gets past the prior missing tooling, but this shell selects the built-in C++ compile rule for `check.o` and omits the repository `-Iinclude -Iexternal -Iexternal/hoytech-cpp -Iexternal/docopt.cpp` include flags, so it fails on `#include "quadrable.h"`. With built-ins disabled, the upstream Makefile's own pattern rule is used and the upstream test binary reports `All tests OK`.
...
The native PHP hash primitive now uses a lane-local pure-PHP BLAKE2s-256 implementation matching upstream `Key::hash`, including multi-block values. Root/proof bytes for the mapped in-memory and proof surfaces are digest-compatible with upstream. The sync slice now has native scan-time diff callbacks and deterministic small sync-fuzz convergence, and the tracked tree covers leaf node-id output/reuse, saved branch-head checkout, named head checkout/persistence, copy-on-write unchanged branch reuse, tracked leaf diff node ids, bounded scan/final diff node-id equality, memStore-range allocation for volatile detached writes, and the named-head LMDB/MemStore guard from upstream `memStore forking from lmdb`. Full upstream-scale randomized sync fuzz parity and imported shadow node-id equality remain unported.
