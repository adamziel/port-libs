...
After that, `make -r test` passed all upstream scenarios on 2026-05-22, and was reconfirmed after the tracked fork/branch node-id slice, after the bounded tracked diff node-id slice, after the native memStore-range tracked-node slice, after the named-head memStore fork guard slice, after the tracked diff reconstruction slice, and after the imported sync shadow node-id slice. `-r` disables GNU Make built-in rules; plain `make test` now gets past the prior missing tooling, but this shell selects the built-in C++ compile rule for `check.o` and omits the repository `-Iinclude -Iexternal -Iexternal/hoytech-cpp -Iexternal/docopt.cpp` include flags, so it fails on `#include "quadrable.h"`. With built-ins disabled, the upstream Makefile's own pattern rule is used and the upstream test binary reports `All tests OK`.
...
- `SyncTest.php` maps the transport and in-memory proof-fragment boundary from upstream `sync fuzz`: `SyncRequest` encode/decode, response proof length-prefix encode/decode, bounded root and later witness proof fragments, witness-leaf expansion, imported shadow roots, imported proof-fragment leaf node ids in the memStore range, matching-hash witness skips during diff, scan-time diff callbacks, WordPress scan/final diff equivalence, deterministic small randomized multi-round sync convergence with scan/final node-id parity, and raw-key diff reconstruction. This is not full upstream sync fuzz parity because persisted LMDB branch node ids, memstore scan lists, mt19937 sizing, and the full randomized 500-trial node-id equivalence check remain unported.
