...
- `ProofTest.php` maps `proofRoundtrip`-style compact transport encode/decode, `FullKeys` proof transport with tracked string keys, imported partial-tree enumeration of proven string keys, `basic proof`, `no unnecessary empty witnesses`, `integer proofs`/`proof sizing`, `range proofs`, and partial-tree range sub-proof slices. Imported proofs build a partial tree that validates the expected root, returns authenticated values or proven absences, enumerates proven full-key leaves when `FullKeys` transport was used, exports narrower range proofs when enough witness material is present, and throws on unauthenticated witness subtrees.
