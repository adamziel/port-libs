...
$restoreDir = sys_get_temp_dir() . '/quadrable-wp-mixed-raw-merge-' . bin2hex(random_bytes(6));
...
try {
    ...
    $detachedValue = $oracleBytes($values['detachedValueHex']);
    ...
    $detachedBefore = $store->get($binaryKey);
    ...
    $detachedPostAfterMerge = $reopened->get('wp_posts:1');
    $detachedAfterMerge = $reopened->get($binaryKey);
    $reopened->checkout('binary-proof');
    $namedDelegated = $reopened->get($binaryKey);
    ...
    echo json_encode([
        'scenario' => 'restore mixed WordPress Quadrable raw LMDB backup and merge a delegated proof on the detached preview head',
        ...
        'reads' => [
            'detachedBeforeHex' => bin2hex($detachedBefore),
            'detachedAfterMergeHex' => bin2hex($detachedAfterMerge),
            'detachedPostAfterMerge' => $detachedPostAfterMerge,
            'namedDelegatedHex' => bin2hex($namedDelegated),
            'privateDelegatedHex' => bin2hex($privateDelegated),
            'masterBinaryHex' => bin2hex($masterBinary),
        ],
        'expectedReads' => [
            'detachedBeforeHex' => bin2hex($detachedValue),
            'detachedAfterMergeHex' => bin2hex($detachedMergedValue),
            'namedDelegatedHex' => bin2hex($delegatedValue),
