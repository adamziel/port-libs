...
- WordPress fixtures now cover `wp_posts` row-level migration changes, `wp_posts` row-level `dolt diff -r sql` INSERT/UPDATE/DELETE review queues, row-mode tabular `dolt diff` review queues for added/modified/removed `wp_posts` rows, multiline `post_content` review queues rendered through `--diff-mode=row|line|in-place|context`, `wp_posts` -> `wp_content_posts` table rename summaries, fixed-width `dolt diff --summary` CLI text and diff-type filtered summary/name-only queues for migration dashboards, a plugin table schema-drift projection, skinny post-review diffs, filtered publish-impacting review rows, aggregate migration-review diff stats, ignore-aware generated-table summaries, ambiguous scratch/cache ignore-rule conflict reporting, a `wp_postmeta` primary-key-change warning scenario, a status-review queue that hides generated cache tables without shelling out to Dolt, schema-object history for migration views/triggers/events, and stored-procedure history for import/review routines.
