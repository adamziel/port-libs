...
- Native `dolt_patch()` function-call parsing maps explicit from/to refs, `A..B` ranges, merge-base-backed `A...B` ranges, case-insensitive requested-table lookup, known unchanged-table empty results, table-not-found errors, and non-literal argument rejection.
