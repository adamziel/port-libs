...
- WordPress commit-log fan-in fixtures now expose default `dolt log --graph` text for multiple import branches converging into a reviewed migration head, so history UIs can render dense branch lanes without shelling out to Dolt.
