...
- Fresh focused upstream runner evidence reran `Test(PatchTableFunction|PatchTableFunctionPrepared)$` in `0.676s`, covering the binary hex patch assertions and same-ref `WORKING`/`STAGED` no-row assertions in the patch table-function suite.
