...
- `go/libraries/doltcore/sqle/dtables/log_table.go`: commit/log table row metadata shape was inspected as the next commit metadata boundary.
- `go/libraries/doltcore/sqle/dtablefunctions/dolt_log.go`: revision range, `--not`, parents, signature, refs, and table-filter behavior was inspected as the next native log slice boundary.
