...
- Native `dolt log` CLI rendering for `--oneline`, `--parents`, `--stat`, `--stat --oneline`, `--graph`, and `--graph --oneline`, including decorated ref parentheses, TTY-sensitive `--decorate=auto` boundaries, newline-flattened messages, modified-table bars, added/deleted table lines, skipped stats for merge commits, text graph branch paths, dense default graph fan-in lanes, and merge parent lines.
...
- `examples/wordpress-commit-log-fan-in-review.php` returns `dolt_log` rows plus default `dolt log --graph` text for the dense import branch fan-in, so a migration history UI can render multi-branch review lanes without shelling out to Dolt.
...
Map the remaining exact `dolt log --graph --oneline` dense fan-in spacing/ref-placement quirks from upstream `log.bats` and `log_graph.go`.
