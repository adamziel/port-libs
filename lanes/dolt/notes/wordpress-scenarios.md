...
- Native `dolt_preview_merge_conflicts_summary` and `dolt_preview_merge_conflicts` row projection for preflight merge review, including data/schema conflict counts, keyed row conflict previews, keyless duplicate/cardinality preview rows with `dolt_row_hash`, and schema-conflict error boundaries before an import merge mutates the working set.
...
Next best slice: extend preview merge conflict parity to schema-conflict descriptions from upstream schema merge cases.
