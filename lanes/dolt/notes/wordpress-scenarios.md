...
Next best slice: map another bounded Dolt CLI/system-table behavior, such as `dolt log -n` / `--limit` argument handling or a focused `dolt diff --summary` CLI text boundary, against upstream BATS evidence.
