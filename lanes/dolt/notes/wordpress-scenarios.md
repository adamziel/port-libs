...
- Native `dolt_log` and `dolt_commits` commit metadata projection, including computed commit order, selected-head ancestry, refs decoration, opt-in parents/signature columns, and `dolt_log()` revision-range filtering.
...
- `examples/wordpress-commit-log-review.php` returns `dolt_log` rows with parents and decorated refs, import-base and media-promotion revision ranges, plus `dolt_commits` rows, so a migration UI can audit which import branch produced each data checkpoint without shelling out to Dolt.
...
Add native `dolt_log --tables` filtering over per-commit changed table metadata.
