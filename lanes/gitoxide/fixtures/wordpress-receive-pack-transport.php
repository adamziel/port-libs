...
return [
    ...
    'sshNormalizedDnsHostTarget' => SshReceivePackTransport::parseRepositoryUrl('ssh://Deploy@Git.EXAMPLE.test:2222/var/www/wp-content.git'),
    'sshNormalizedDnsHostContext' => SshReceivePackTransport::connectorContext(
    ...
    'gitDaemonEmptyValueExtraServiceRequest' => GitDaemonReceivePackTransport::serviceRequestBytesForUrl('git://git.example.test/wp-content.git', ['server-option='], 2),
    ...
    'smartHttpTransportOptionsBoundary' => (static function () use ($packet, $flush, $advertisementBytes, $blob): array {
        ...
        return [
            ...
            'sslVersionMin' => $requests[0]['httpOptions']['sslVersion']['min'] ?? null,
            'sslVersionMax' => $requests[0]['httpOptions']['sslVersion']['max'] ?? null,
            'sslCryptoMethod' => $requests[0]['httpOptions']['sslVersion']['cryptoMethod'] ?? null,
    ...
    'unsafeSshDecodedQuestionHostRejected' => (static function (): bool {
        try {
            SshReceivePackTransport::parseRepositoryUrl('ssh://bad%3fhost.example.test/wp-content.git');
        } catch (InvalidArgumentException) {
            return true;
        }
    ...
    'unsafeSshMalformedBracketRejected' => (static function (): bool {
        try {
            SshReceivePackTransport::parseRepositoryUrl('[::1:wp-content.git');
        } catch (InvalidArgumentException) {
            return true;
        }
    ...
    'unsafeSshBracketSuffixRejected' => (static function (): bool {
        try {
            SshReceivePackTransport::parseRepositoryUrl('[::1]suffix:wp-content.git');
        } catch (InvalidArgumentException) {
            return true;
        }
