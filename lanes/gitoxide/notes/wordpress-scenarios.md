...
`examples/wordpress-commit-signature.php` parses a WordPress import/deploy commit body with author and committer actors, timezone offsets, `encoding`, a multiline signature header, extra-header position/count lookup, upstream-ordered commit token types, embedded release merge-tag header parsed as a native annotated tag object, commit summary/body, Signed-off-by/Co-authored-by/Acked-by/Reviewed-by/Tested-by trailers, and signature-stripped signed payload bytes. The parser now also rejects commit bytes that never reach Gitoxide's required header/message separator, preventing truncated deployment provenance from being treated as an empty-message commit. This models migration and deployment tooling that needs `git log`-style provenance, signed-commit metadata, structured commit token inspection, release-tag target/kind/tagger/message provenance, and importer/reviewer/tester attribution without invoking the Git binary.
...
Broaden annotated tag iterator error parity and raw-target preservation, broaden commit trailer edge cases from upstream fixtures, add broader directory rename conflict cases, broaden proxy credential persistence, or try a controlled crate-level upstream runner probe.
