...
`examples/wordpress-object-database.php` writes the deterministic WordPress pack fixture into a temporary `.git/objects/pack` directory, adds a loose draft object, links an alternate shared package object cache through `objects/info/alternates`, then reads every source through one object database. This models package managers, Playground snapshot tools, and shared-hosting deployment code traversing packed, loose, and shared cache repository content without invoking the Git binary.

## Next Task

Map object replacement refs or multi-pack-index semantics, then run a controlled gix-odb/gix-pack crate no-run probe if the VM remains clear.
