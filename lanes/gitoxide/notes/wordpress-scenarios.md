...
`examples/wordpress-receive-pack-transport.php` runs a deterministic receive-pack handshake, request write, and status response read over native PHP stream resources. The transport tests now also cover the git-daemon `git-receive-pack` service request envelope before delegating to the same stream flow, modeling the first concrete URL adapter boundary a shared-hosting deployment tool needs before SSH or HTTP adapters are introduced.
...
Add SSH/HTTP receive-pack adapters, broader directory rename cases, or broader rename heuristics.
