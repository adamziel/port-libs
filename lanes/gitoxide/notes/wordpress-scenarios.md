...
`examples/wordpress-commit-signature.php` parses a WordPress import/deploy commit body with author and committer actors, timezone offsets, `encoding`, a multiline signature header, commit summary/body, attribution trailers, and signature-stripped signed payload bytes. This models migration and deployment tooling that needs `git log`-style provenance, signed-commit metadata, and importer/reviewer attribution without invoking the Git binary.
