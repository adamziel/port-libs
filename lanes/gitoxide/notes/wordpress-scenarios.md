...
`examples/wordpress-reference-transaction.php` promotes a tenant-scoped production branch, deletes a reviewed plugin branch, and updates `HEAD` inside `refs/namespaces/site-a/` while returning store-relative ref names and symbolic targets. This models the bounded `git update-ref` behavior a multisite deployment tool needs to publish or prune review refs without invoking the Git binary. Packed-ref transaction rewrites and reflogs remain a later slice.
...
Map packed-ref transaction delete/update rewrites plus reflog behavior, or run another controlled `gix-object` message/tag integration subset.
