...
`examples/wordpress-receive-pack-transport.php` runs a deterministic receive-pack handshake, request write, and status response read over native PHP stream resources. The transport tests now also cover git-daemon `git-receive-pack` service requests and smart HTTP discovery/POST exchanges, modeling concrete URL adapter boundaries a shared-hosting deployment tool needs before SSH transport is introduced.
...
Add an SSH receive-pack adapter, broader directory rename cases, or broader rename heuristics.
