...
`examples/wordpress-smart-http-proxy-credentials.php` documents proxy credential-helper handling for WordPress deployment tools. It records ordinary helper lookup, username-only proxy URL helper context, final-status store/erase callbacks, proxy authorization kept out of origin headers, and redirect credential reuse without leaking proxy credentials to the Git origin.
