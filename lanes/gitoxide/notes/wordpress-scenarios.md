...
`examples/wordpress-commit-signature.php` parses a WordPress import/deploy commit body with author and committer actors, timezone offsets, `encoding`, a multiline signature header, extra-header position/count lookup, an embedded release merge-tag header parsed as a native annotated tag object, commit summary/body, Signed-off-by/Co-authored-by/Acked-by/Reviewed-by/Tested-by trailers, and signature-stripped signed payload bytes. This models migration and deployment tooling that needs `git log`-style provenance, signed-commit metadata, release-tag target/kind/tagger/message provenance, and importer/reviewer/tester attribution without invoking the Git binary.
