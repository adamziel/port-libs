...
`examples/wordpress-reference-transaction.php` promotes a tenant-scoped production branch, deletes a reviewed plugin branch, recovers an empty tenant `HEAD` directory left by an interrupted deploy, and updates `HEAD` inside `refs/namespaces/site-a/` while returning store-relative ref names and symbolic targets. This models the bounded `git update-ref` behavior a multisite deployment tool needs to publish or prune review refs and recover from empty directory blockers without invoking the Git binary.
