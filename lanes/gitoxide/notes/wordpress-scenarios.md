...
`examples/wordpress-commit-signature.php` parses a WordPress import/deploy commit body with author and committer actors, timezone offsets, `encoding`, and a multiline signature header. This models migration and deployment tooling that needs `git log`-style provenance and signed-commit metadata without invoking the Git binary.
...
Map `gix-object` commit message/trailer/signature extraction semantics, add broader directory rename conflict cases, broaden proxy credential persistence, or try a controlled crate-level upstream runner probe.
