...
`examples/wordpress-receive-pack-transport.php` runs a deterministic receive-pack handshake, request write, and status response read over native PHP stream resources. This models the transport boundary a shared-hosting deployment tool needs before concrete SSH, HTTP, or git-daemon adapters are introduced.
...
Add broader directory rename cases, broader rename heuristics, or concrete git-daemon/SSH/HTTP receive-pack URL adapters.
