...
- The next inventory slice should either add broader directory rename cases, add further rename heuristics, add HTTP redirect/proxy behavior, or materialize only the needed protocol/transport crate paths and try a controlled `cargo test -p gix-protocol --no-run --locked --offline` probe before any live runner attempt.
