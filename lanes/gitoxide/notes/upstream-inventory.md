...
- The next inventory slice should either add broader HTTP auth/session behavior, add broader directory rename cases or further rename heuristics, or materialize only the needed protocol/transport crate paths and try a controlled `cargo test -p gix-protocol --no-run --locked --offline` probe before any live runner attempt.
