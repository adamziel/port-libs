...
- The next inventory slice should either add HTTPS-through-SOCKS/TLS coverage, broader directory rename conflict cases, more rename heuristics, or materialize only the needed protocol/transport crate paths and try a controlled `cargo test -p gix-protocol --no-run --locked --offline` probe before any live runner attempt.
