...
- `gix-merge/tests/merge/tree/mod.rs` drives Git baseline cases through commit-level merge and index conflict comparison. The PHP slice now maps flat tree decisions plus a bounded recursive content-merge path that reads nested tree/blob objects, writes merged blob/tree objects, records full-path content conflicts, exposes ancestor/ours/theirs index stage views, writes blob conflicts into Git index v2 `DIRC` files, writes merged worktree files including marker blobs, classifies directory-file collisions before generic add-add conflicts, and detects exact same-object rename/delete plus rename/rename conflicts with ambiguity guards. Rename similarity, directory rename handling, tree-conflict index expansion, checkout deletions, and full commit merge orchestration remain unported.
...
- The next inventory slice should either add rename similarity/directory rename handling, add tree-conflict index expansion, add concrete receive-pack URL adapters, or materialize only the needed protocol/transport crate paths and try a controlled `cargo test -p gix-protocol --no-run --locked --offline` probe before any live runner attempt.
...
- `MergeWorktreeWriterTest.php` maps the bounded checkout side of recursive content conflicts: unmerged blob stages are written to a real Git index v2 `DIRC` file with stage bits and checksum, merged WordPress worktree files are written from tree objects, marker blobs are materialized for conflicted `theme.json`, unsafe checkout paths are rejected, and tree conflict stages are explicitly refused until tree conflicts can be expanded into file-level index entries.
