...
- `TestTSClass`: a focused computed-key extension is mapped behind `useDefineForClassFields=false`. Public computed instance/static field keys are evaluated once into class-scope temporaries before constructor/static assignment emission, and uninitialized computed fields preserve key side effects even though the field declaration is erased. This maps the simple computed field expectations from `TestTSClass`; the full `TestTSClassSideEffectOrder` interleaving matrix remains open.
