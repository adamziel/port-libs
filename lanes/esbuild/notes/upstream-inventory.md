...
- `TestTSClass`/`TestTSSuperCall`: selected constructor parameter property behavior is mapped for public/protected/private/readonly/override parameter properties, lowering them to ordinary parameters, `this.<name> = <name>` assignments, and matching class field declarations, with destructured parameter properties and missing access-modifier names rejected. The assign-semantics subset now omits the extra field declarations under `useDefineForClassFields=false`, inserts simple derived-constructor parameter-property assignments after a direct `super(...)` call, wraps multiple or conditional derived `super(...)` paths in an upstream-style `__super` helper, normalizes one-line derived constructors before injecting assign-semantics class-field assignments after `super(...)`, wraps initialized-field logical-assignment `super(...)` calls such as `y ||= super(1)`, and preserves upstream's dead `if (false) __super(...)` branch shape while assigning after the live direct `super(...)` call.
