...
- `TestTSUsing`: selected printer/error behavior is mapped for top-level explicit resource management declarations. Type annotations are erased from `using x: any = y` and multi-declarator `using x: any = y, z: any = _`, while `export using` is rejected at top level and inside namespaces.
