...
Focused update for the current slice: targeted reads of `internal/bundler_tests/bundler_lower_test.go` and `internal/bundler_tests/snapshots/snapshots_lower.txt` inspected upstream `TestLowerUsing` function-body and block-scope helper output. No broad `git grep` or full checkout hydration was used. The new native slice maps simple function-body cases where unsupported-target `using` and `await using` declarations are wrapped in a function-local stack, `try`/`catch`/`finally`, `__using`, and `__callDispose` cleanup, while nested block cases still get their own per-block cleanup scope.
