...
final class OdfReader
{
    ...
    public function readPackage(ZipPackage $package): array
    {
        ...
        $embeddedObjectPackages = is_array($packageProvenance['embeddedObjectPackages'] ?? null)
            ? $packageProvenance['embeddedObjectPackages']
            : [];
        ...
        if (($embeddedObjectPackages['count'] ?? 0) > 0) {
            $metadata['odfPackageEmbeddedObjects'] = $embeddedObjectPackages;
        }
    ...
    private function embeddedObjectPackageProvenance(
        ...
        foreach ($objectPackageRootParts as $rootPart => $rootItem) {
            ...
            foreach ($entriesByPart as $part => $entry) {
                ...
                $partSummary = $this->embeddedObjectContainedPartSummary($entry, $manifestByPart[$entry->name] ?? null);
     ...
     * @return array<string, mixed>
     */
    private function embeddedObjectContainedPartSummary(ZipPackageEntry $entry, ?array $manifestItem): array
    {
        $declared = $manifestItem !== null;

        return [
            'part' => $entry->name,
            'fullPath' => $declared ? ($manifestItem['fullPath'] ?? null) : null,
            'manifestIndex' => $declared ? ($manifestItem['manifestIndex'] ?? null) : null,
            'mediaType' => $declared ? (string) ($manifestItem['mediaType'] ?? '') : '',
            'mediaTypeBase' => $declared ? (string) ($manifestItem['mediaTypeBase'] ?? '') : '',
            'mediaTypeHasParameters' => $declared && ($manifestItem['mediaTypeHasParameters'] ?? false) === true,
            'mediaTypeParameterCount' => $declared ? (int) ($manifestItem['mediaTypeParameterCount'] ?? 0) : 0,
            'mediaTypeParameters' => $declared && is_array($manifestItem['mediaTypeParameters'] ?? null) ? $manifestItem['mediaTypeParameters'] : [],
            'mediaTypeParameterMap' => $declared && is_array($manifestItem['mediaTypeParameterMap'] ?? null) ? $manifestItem['mediaTypeParameterMap'] : [],
            'missingMediaType' => $declared && ($manifestItem['missingMediaType'] ?? false) === true,
            'encrypted' => $declared && ($manifestItem['encrypted'] ?? false) === true,
            'byteExposurePolicy' => $declared ? ($manifestItem['byteExposurePolicy'] ?? null) : null,
            'diagnostics' => $declared && is_array($manifestItem['diagnostics'] ?? null) ? $manifestItem['diagnostics'] : [],
            'byteLength' => $entry->uncompressedSize,
            'compressedByteLength' => $entry->compressedSize,
            'compressionMethod' => $entry->compressionMethod,
            'compressionMethodName' => self::compressionMethodName($entry->compressionMethod),
