...
final class OdfReader
{
    ...
    public function readPackage(ZipPackage $package): array
    {
        ...
        $packageSignatures = $this->packageSignatureMetadata($package, $manifest, $undeclaredEntries);
        ...
        $packageGalleries = $this->packageGalleryMetadata($package, $manifest, $undeclaredEntries);
        $packageForms = $this->packageFormMetadata($package, $manifest, $undeclaredEntries);
        $packageReports = $this->packageReportMetadata($package, $manifest, $undeclaredEntries);
        ...
        if ($packageSignatures['count'] > 0) {
            $metadata['odfPackageSignatures'] = $packageSignatures;
        }
        ...
        if ($packageVersions['count'] > 0) {
            $metadata['odfPackageVersions'] = $packageVersions;
        }
        if ($packageGalleries['count'] > 0) {
            $metadata['odfPackageGalleries'] = $packageGalleries;
        }
        if ($packageForms['count'] > 0) {
            $metadata['odfPackageForms'] = $packageForms;
        }
        if ($packageReports['count'] > 0) {
            $metadata['odfPackageReports'] = $packageReports;
        }
        if ($packageAttachments['count'] > 0) {
            $metadata['odfPackageAttachments'] = $packageAttachments;
        }
        if ($packageTemplates['count'] > 0) {
            $metadata['odfPackageTemplates'] = $packageTemplates;
        }
        if ($packageDictionaries['count'] > 0) {
            $metadata['odfPackageDictionaries'] = $packageDictionaries;
        }
        if ($packageEvents['count'] > 0) {
            $metadata['odfPackageEvents'] = $packageEvents;
        }
    ...
    private function readManifest(ZipPackage $package): array
    {
        ...
        foreach ($rawItems as $rawItem) {
            ...
            $embeddedObjectPackage = is_string($part) && $part !== ''
            ...
            $galleryPackagePart = is_string($part) && $this->isGalleryPackagePartName($part);
            $formPackagePart = is_string($part) && $this->isFormPackagePartName($part);
            $reportPackagePart = is_string($part) && $this->isReportPackagePartName($part);
            $attachmentPackagePart = is_string($part) && $this->isAttachmentPackagePartName($part);
            $templatePackagePart = is_string($part) && $this->isTemplatePackagePartName($part);
            $dictionaryPackagePart = is_string($part) && $this->isDictionaryPackagePartName($part);
            $eventPackagePart = is_string($part) && $this->isEventPackagePartName($part);
    ...
    private function corePackageHandoffPreflight(ZipPackage $package, array $manifest): array
    {
        ...
        foreach ([
            ...
            $summary[$listKey] = array_values(array_map(
                fn (array $entry): array => $this->corePackageHandoffEntryWithManifestState(['exists' => true] + $entry, $manifestByPart, $rootManifestEntry),
                $summary[$listKey]
            ));
        }
    ...
    private function packageProvenance(
        ...
        foreach ($manifest as $item) {
            ...
            $manifestFileEntryOrder[] = [
                ...
                'galleryPackagePart' => ($item['galleryPackagePart'] ?? false) === true,
                'formPackagePart' => ($item['formPackagePart'] ?? false) === true,
                'reportPackagePart' => ($item['reportPackagePart'] ?? false) === true,
                'attachmentPackagePart' => ($item['attachmentPackagePart'] ?? false) === true,
                'templatePackagePart' => ($item['templatePackagePart'] ?? false) === true,
                'dictionaryPackagePart' => ($item['dictionaryPackagePart'] ?? false) === true,
                ...
                'zipUnsupportedGeneralPurposeFlagBits' => $item['zipUnsupportedGeneralPurposeFlagBits'] ?? null,
                'zipGeneralPurposeFlagsSupported' => $item['zipGeneralPurposeFlagsSupported'] ?? null,
                'zipUsesUtf8Names' => $item['zipUsesUtf8Names'] ?? null,
                'zipUsesDataDescriptor' => $item['zipUsesDataDescriptor'] ?? null,
                'zipDeflateOptionFlags' => $item['zipDeflateOptionFlags'] ?? null,
                'zipDeflateOptionName' => $item['zipDeflateOptionName'] ?? null,
                'zipRequiresStrictFlagReview' => $item['zipRequiresStrictFlagReview'] ?? null,
        ...
        foreach ($package->entries() as $centralDirectoryIndex => $entry) {
            ...
            $parts[$entry->name] = [
                ...
                'reportPackagePart' => $this->isReportPackagePartName($entry->name),
                'attachmentPackagePart' => $this->isAttachmentPackagePartName($entry->name),
                'templatePackagePart' => $this->isTemplatePackagePartName($entry->name),
                'dictionaryPackagePart' => $this->isDictionaryPackagePartName($entry->name),
                'eventPackagePart' => $this->isEventPackagePartName($entry->name),
    ...
    private function packageIdentityProvenance(array $provenance): array
    {
        ...
        foreach ($provenance['parts'] ?? [] as $item) {
            ...
            $packageEntries[] = self::withoutEmpty([
                ...
                'versionMadeBy' => $item['versionMadeBy'] ?? null,
                'versionNeededToExtract' => $item['versionNeededToExtract'] ?? null,
                'creatorVersionMeetsNeeded' => $item['creatorVersionMeetsNeeded'] ?? null,
                'creatorVersionComparison' => $item['creatorVersionComparison'] ?? null,
                'creatorVersionDelta' => $item['creatorVersionDelta'] ?? null,
                'creatorHostIssues' => $item['creatorHostIssues'] ?? [],
                'externalAttributes' => $item['externalAttributes'] ?? null,
                'externalAttributesHex' => $item['externalAttributesHex'] ?? null,
                'hasExternalAttributes' => ($item['hasExternalAttributes'] ?? false) === true,
                'dosAttributes' => $item['dosAttributes'] ?? null,
                'dosAttributeNames' => $item['dosAttributeNames'] ?? [],
                'hasDosAttributes' => ($item['hasDosAttributes'] ?? false) === true,
                'hasDosReadOnlyAttribute' => ($item['hasDosReadOnlyAttribute'] ?? false) === true,
                'hasDosHiddenAttribute' => ($item['hasDosHiddenAttribute'] ?? false) === true,
                'hasDosSystemAttribute' => ($item['hasDosSystemAttribute'] ?? false) === true,
                'hasDosVolumeLabelAttribute' => ($item['hasDosVolumeLabelAttribute'] ?? false) === true,
                'hasDosDirectoryAttribute' => ($item['hasDosDirectoryAttribute'] ?? false) === true,
                'hasDosArchiveAttribute' => ($item['hasDosArchiveAttribute'] ?? false) === true,
                'internalFileAttributes' => $item['internalFileAttributes'] ?? null,
                'internalFileAttributesHex' => $item['internalFileAttributesHex'] ?? null,
                'internalAttributeNames' => $item['internalAttributeNames'] ?? [],
                'hasInternalFileAttributes' => ($item['hasInternalFileAttributes'] ?? false) === true,
                'hasTextInternalAttribute' => ($item['hasTextInternalAttribute'] ?? false) === true,
                'hasUnknownInternalAttributeBits' => ($item['hasUnknownInternalAttributeBits'] ?? false) === true,
                'unknownInternalAttributeBits' => $item['unknownInternalAttributeBits'] ?? null,
                'unixMode' => $item['unixMode'] ?? null,
                'unixModeOctal' => $item['unixModeOctal'] ?? null,
                'unixPermissions' => $item['unixPermissions'] ?? null,
                'unixPermissionsOctal' => $item['unixPermissionsOctal'] ?? null,
                'hasUnixMode' => ($item['hasUnixMode'] ?? false) === true,
                'unixFileType' => $item['unixFileType'] ?? null,
                'unixFileTypeName' => $item['unixFileTypeName'] ?? null,
                'isUnixExecutableFile' => ($item['isUnixExecutableFile'] ?? false) === true,
                'isGroupWritable' => ($item['isGroupWritable'] ?? false) === true,
                'isWorldWritable' => ($item['isWorldWritable'] ?? false) === true,
                'hasWritablePermissions' => ($item['hasWritablePermissions'] ?? false) === true,
                'platformMetadataPlatform' => $item['platformMetadataPlatform'] ?? null,
        ...
        $payload = [
            ...
            'zipStrictFlagReviewEntryCount' => $provenance['zipStrictFlagReviewEntryCount'] ?? 0,
            'zipUnsupportedGeneralPurposeFlagEntryCount' => $provenance['zipUnsupportedGeneralPurposeFlagEntryCount'] ?? 0,
            ...
            'unixModeEntryCount' => $provenance['unixModeEntryCount'] ?? 0,
            'executableFileCount' => $provenance['executableFileCount'] ?? 0,
            'writablePermissionEntryCount' => $provenance['writablePermissionEntryCount'] ?? 0,
            ...
            'packageFontPartCount' => $provenance['packageFontPartCount'] ?? 0,
            'rdfMetadataPartCount' => $provenance['rdfMetadataPartCount'] ?? 0,
            'objectReplacementPartCount' => $provenance['objectReplacementPartCount'] ?? 0,
            'layoutCachePartCount' => $provenance['layoutCachePartCount'] ?? 0,
            'metaInfSidecarPartCount' => $provenance['metaInfSidecarPartCount'] ?? 0,
            ...
            'galleryPackagePartCount' => $provenance['galleryPackagePartCount'] ?? 0,
            'formPackagePartCount' => $provenance['formPackagePartCount'] ?? 0,
            'reportPackagePartCount' => $provenance['reportPackagePartCount'] ?? 0,
            'attachmentPackagePartCount' => $provenance['attachmentPackagePartCount'] ?? 0,
            'templatePackagePartCount' => $provenance['templatePackagePartCount'] ?? 0,
            'dictionaryPackagePartCount' => $provenance['dictionaryPackagePartCount'] ?? 0,
            'eventPackagePartCount' => $provenance['eventPackagePartCount'] ?? 0,
     ...
     * @return array<string, mixed>|null
     */
    private function embeddedObjectPackageMembership(string $part, array $objectPackageRootParts): ?array
    {
        foreach ($objectPackageRootParts as $rootPart => $rootItem) {
            ...
            return [
                ...
                'mediaType' => (string) ($rootItem['mediaType'] ?? ''),
                'mediaTypeBase' => (string) ($rootItem['mediaTypeBase'] ?? ''),
     ...
     */
    private function zipEntryRawNameProvenance(ZipPackageEntry $entry): array
    {
        ...
        return [
            ...
            'hasRawNameProvenance' => !$rawNameMatchesDecodedName || $usesLegacyNameEncoding || $usesUnicodePathExtraField,
        ];
    }
    ...
    private function packagePartRoles(
        ...
        if ($this->isLayoutCachePackagePartName($entry->name)) {
            $roles[] = 'layout-cache';
        }
        ...
        if ($this->isVersionPackagePartName($entry->name)) {
            $roles[] = 'version-package';
        }
        if ($this->isGalleryPackagePartName($entry->name)) {
            $roles[] = 'gallery-package';
        }
        if ($this->isFormPackagePartName($entry->name)) {
            $roles[] = 'form-package';
        }
        if ($this->isReportPackagePartName($entry->name)) {
            $roles[] = 'report-package';
        }
        if ($this->isAttachmentPackagePartName($entry->name)) {
            $roles[] = 'attachment-package';
        }
        if ($this->isTemplatePackagePartName($entry->name)) {
            $roles[] = 'template-package';
        }
        if ($this->isDictionaryPackagePartName($entry->name)) {
            $roles[] = 'dictionary-package';
        }
        if ($this->isEventPackagePartName($entry->name)) {
            $roles[] = 'event-package';
        }
        if ($entry->isDirectory()) {
            $roles[] = 'zip-directory';
        }
        $embeddedObjectPackagePart = false;
        if (isset($objectPackageRootParts[$entry->name])) {
            $roles[] = 'embedded-object-root';
            $embeddedObjectPackagePart = true;
        } else {
        ...
        if (is_array($manifestItem)) {
            ...
            if ($this->isAttachmentPackagePartName($part)) {
                $roles[] = 'attachment-package';
            }
            if ($this->isTemplatePackagePartName($part)) {
                $roles[] = 'template-package';
            }
            if ($this->isDictionaryPackagePartName($part)) {
                $roles[] = 'dictionary-package';
            }
            if ($this->isEventPackagePartName($part)) {
                $roles[] = 'event-package';
            }
    ...
    private function readRdfMetadata(ZipPackage $package, array $manifest): array
    {
        ...
        foreach ($candidatesByPart as $part => $item) {
            ...
            $partMetadata = [
                ...
                'byteExposurePolicy' => $item['byteExposurePolicy'] ?? ($encrypted ? 'encrypted-resource-bytes-blocked' : 'rdf-metadata-bytes-blocked'),
                'reviewPolicy' => 'rdf-metadata-only',
                'tripleCount' => 0,
                'literalCount' => 0,
                'resourceCount' => 0,
                'subjectCount' => 0,
    ...
    private function isRdfManifestItem(array $item): bool
    {
        ...
        return $this->isRdfPackagePart($part, (string) ($item['mediaType'] ?? ''));
    }

    private function isRdfPackagePart(string $part, ?string $mediaType = null): bool
    {
        $base = $mediaType === null ? '' : self::mediaTypeReport($mediaType)['mediaTypeBase'];

        return $base === 'application/rdf+xml' || $this->isRdfPartName($part);
    }
    ...
    private function packageScriptMetadata(ZipPackage $package, array $manifest, array $undeclaredEntries): array
    {
        ...
        foreach ($undeclaredEntries as $entry) {
            ...
            $mediaType = self::scriptMediaTypeFromPart($part) ?? '';
            $mediaTypeReport = self::mediaTypeReport($mediaType);
        ...
        foreach ($candidatesByPart as $part => $item) {
            ...
            $mediaTypeReport = self::mediaTypeReport($mediaType);
            $pathInfo = self::scriptPackagePathInfo($part, $mediaTypeReport['mediaTypeBase']);
            $mediaTypeValid = $isDirectory
                ? $mediaTypeReport['mediaTypeBase'] === ''
                : self::scriptMediaTypeValid($part, $mediaTypeReport['mediaTypeBase']);
            $issues = [];
            if (!$exists) {
            ...
            if (is_string($pathInfo['scriptContainer'] ?? null)) {
                $containers[$pathInfo['scriptContainer']] = true;
            }
            if (is_string($pathInfo['scriptKind'] ?? null)) {
                $scriptKinds[$pathInfo['scriptKind']] = true;
            }

            $fullPath = (string) ($item['fullPath'] ?? $part);
            $partReference = $item['partReference'] ?? $part;
            $partSuffix = $item['partSuffix'] ?? null;
            $partQuery = $item['partQuery'] ?? null;
            $partFragment = $item['partFragment'] ?? null;
            $byteExposurePolicy = $item['byteExposurePolicy'] ?? ($isDirectory ? 'directory-entry-no-bytes' : 'script-package-bytes-blocked');
            ...
            $items[] = [
                ...
                'pathSuffix' => $partSuffix,
                'partSuffix' => $partSuffix,
                'pathQuery' => $partQuery,
                'partQuery' => $partQuery,
                'pathFragment' => $partFragment,
    ...
    private static function scriptMediaTypeFromPart(string $part): ?string
    {
        if (str_ends_with($part, '/')) {
            return '';
        }
    ...
    private static function scriptMediaTypeValid(string $part, string $mediaTypeBase): bool
    {
        if ($mediaTypeBase === '') {
            return false;
        }

        $expected = self::scriptMediaTypeFromPart($part);
        if ($expected === null) {
            return true;
        }
     ...
     */
    private static function scriptPackagePathInfo(string $part, string $mediaTypeBase): array
    {
        $trimmed = trim($part, '/');
        ...
        $module = $isDirectory ? null : ($extension === '' ? basename($part) : basename($part, '.' . $extension));
        $library = null;
    ...
    private function mediaReport(ZipPackage $package, array $manifest): array
    {
        ...
        foreach ($manifest as $item) {
            ...
            if (($item['rdfMetadataPart'] ?? false) === true) {
                continue;
            }
            ...
            if ($this->isFormPackagePartName($part)) {
                continue;
            }
            if ($this->isReportPackagePartName($part)) {
                continue;
            }
            if ($this->isAttachmentPackagePartName($part)) {
                continue;
     ...
     * @return array<string, mixed>
     */
    private function manifestMediaResourceRoleSummary(array $manifest): array
    {
        $items = [];
        $roleConflictItems = [];
        ...
        foreach ($manifest as $item) {
            ...
            $mediaTypeBase = (string) ($item['mediaTypeBase'] ?? self::mediaTypeReport($mediaType)['mediaTypeBase']);
            ...
            $resourceItem = self::withoutEmpty([
                'manifestIndex' => $item['manifestIndex'] ?? null,
                'fullPath' => $item['fullPath'] ?? null,
                'part' => $part,
                'mediaType' => $mediaType,
    ...
    private static function manifestPreferredViewModeSummary(array $manifest): array
    {
        ...
        foreach ($manifest as $item) {
            $mode = trim((string) ($item['preferredViewMode'] ?? ''));
            if ($mode === '') {
                continue;
            }
     ...
     * @return list<string>
     */
    private function mediaResourcePackagePrecedenceRoles(array $manifestItem): array
    {
        $part = (string) ($manifestItem['part'] ?? '');
        if ($part === '') {
            return [];
        ...
        $mediaType = (string) ($manifestItem['mediaType'] ?? '');
        $roles = [];
        if (($manifestItem['embeddedObjectPackagePart'] ?? false) === true) {
        ...
        if (($manifestItem['rdfMetadataPart'] ?? false) === true || $this->isRdfPackagePart($part, $mediaType)) {
            $roles[] = 'rdf-metadata';
        }
        if ($this->isObjectReplacementPackagePartName($part)) {
            $roles[] = 'object-replacement';
    ...
    private static function isManifestMediaResourceCandidate(string $part, string $mediaTypeBase): bool
    {
        return self::mediaResourceFamilyFromMediaTypeBase($mediaTypeBase) !== null
    ...
    private function packageConfigurationMetadata(ZipPackage $package, array $manifest, array $undeclaredEntries): array
    {
        ...
        foreach ($candidatesByPart as $part => $item) {
            ...
            if (($item['declaredSizeInvalid'] ?? false) === true) {
                $issues[] = 'odf-configuration-package-invalid-declared-size';
            }
            foreach ($issues as $issue) {
                $issueCodes[$issue] = true;
            }
     ...
     * @return array{count:int, readableCount:int, declaredCount:int, undeclaredCount:int, missingCount:int, encryptedCount:int, invalidMediaTypeCount:int, issueCount:int, issueCodes:list<string>, items:list<array<string, mixed>>}
     */
    private function packageSignatureMetadata(ZipPackage $package, array $manifest, array $undeclaredEntries): array
    {
        $candidatesByPart = [];
        foreach ($manifest as $item) {
            $part = $item['part'] ?? null;
    ...
    private function packageFontMetadata(ZipPackage $package, array $manifest, array $undeclaredEntries): array
    {
        ...
        foreach ($candidatesByPart as $part => $item) {
            ...
            $fontFormat = self::fontFormatProvenance(
                $part,
                $mediaTypeReport['mediaTypeBase'],
                $rawMediaType !== '' && ($item['fontFormatSourceHint'] ?? '') !== 'package-extension'
            );
            $issues = [];
            if (!$entry instanceof ZipPackageEntry) {
     ...
     */
    private static function fontFormatProvenance(string $part, string $mediaTypeBase, bool $hasManifestMediaType): array
    {
        $extension = strtolower(pathinfo($part, PATHINFO_EXTENSION));
        $extension = $extension === '' ? null : $extension;
     ...
     * @return array{count:int, readableCount:int, declaredCount:int, undeclaredCount:int, missingCount:int, encryptedCount:int, invalidMediaTypeCount:int, issueCount:int, issueCodes:list<string>, items:list<array<string, mixed>>}
     */
    private function packageObjectReplacementMetadata(ZipPackage $package, array $manifest, array $undeclaredEntries): array
    {
        $candidatesByPart = [];
        foreach ($manifest as $item) {
            $part = $item['part'] ?? null;
            if (!is_string($part) || $part === '' || !$this->isObjectReplacementPackagePartName($part)) {
                continue;
            }
        ...
        return [
            ...
            'invalidMediaTypeCount' => count(array_filter(
                $items,
                fn (array $item): bool => $item['mediaType'] !== null
                    && !$this->isObjectReplacementMediaType((string) $item['mediaType']),
            )),
            'issueCount' => count(array_filter($items, static fn (array $item): bool => $item['issues'] !== [])),
            'issueCodes' => array_keys($issueCodes),
            'kindCounts' => $kindCounts,
    ...
    private function packageLayoutCacheMetadata(ZipPackage $package, array $manifest, array $undeclaredEntries): array
    {
        ...
        foreach ($manifest as $item) {
            $part = $item['part'] ?? null;
            if (!is_string($part) || $part === '' || !$this->isLayoutCachePackagePartName($part)) {
                continue;
            }
     ...
     * @return array<string, mixed>
     */
    private function packageMetaInfSidecarMetadata(ZipPackage $package, array $manifest, array $undeclaredEntries): array
    {
        $candidatesByPart = [];
        foreach ($manifest as $item) {
            $part = $item['part'] ?? null;
            if (!is_string($part) || $part === '' || !$this->isMetaInfSidecarPackagePartName($part)) {
                continue;
            }
        ...
        foreach ($candidatesByPart as $part => $item) {
            ...
            $items[] = [
                ...
                'undeclared' => !$declared,
                'encrypted' => $encrypted,
                'valid' => $entry instanceof ZipPackageEntry && !$encrypted,
                'byteLength' => !$encrypted && $entry instanceof ZipPackageEntry ? $entry->uncompressedSize : null,
                'compressedByteLength' => $entry instanceof ZipPackageEntry ? $entry->compressedSize : null,
                'compressionMethod' => $entry instanceof ZipPackageEntry ? $entry->compressionMethod : null,
                'compressionMethodName' => $entry instanceof ZipPackageEntry ? self::compressionMethodName($entry->compressionMethod) : null,
                ...
                'declaredSizeMismatch' => ($item['declaredSizeMismatch'] ?? false) === true,
                'canExposeAsDocumentMedia' => false,
                'byteExposurePolicy' => $item['byteExposurePolicy'] ?? 'meta-inf-sidecar-package-bytes-blocked',
    ...
    private function packageDatabaseMetadata(ZipPackage $package, array $manifest, array $undeclaredEntries): array
    {
        ...
        foreach ($manifest as $item) {
            $part = $item['part'] ?? null;
            if (!is_string($part) || $part === '' || !$this->isDatabasePackagePartName($part)) {
                continue;
            }
        ...
        foreach ($undeclaredEntries as $entry) {
            ...
            $mediaType = $this->databaseMediaTypeFromPart($part) ?? '';
            $mediaTypeReport = self::mediaTypeReport($mediaType);
            $candidatesByPart[$part] = [
                'fullPath' => $part,
                'part' => $part,
        ...
        foreach ($candidatesByPart as $part => $item) {
            ...
            $mediaTypeValid = $isDirectory
                ? $mediaTypeReport['mediaTypeBase'] === ''
                : (!$missingMediaType && $this->isDatabaseMediaType($mediaType));
            $kind = $this->databasePackagePartKind($part, $mediaType, $isDirectory);
            $group = self::databasePackagePartGroup($part);
            $issues = [];
            if (!$exists) {
            ...
            $items[] = [
                ...
                'canExposeBytes' => false,
                'canExposeAsDocumentMedia' => false,
                'byteExposurePolicy' => $item['byteExposurePolicy'] ?? ($isDirectory ? 'directory-entry-no-bytes' : 'database-package-bytes-blocked'),
    ...
    private function databaseMediaTypeFromPart(string $part): ?string
    {
        if (str_ends_with($part, '/')) {
            return '';
        }
    ...
    private function isDatabaseMediaType(string $mediaType): bool
    {
        $base = self::mediaTypeReport($mediaType)['mediaTypeBase'];
    ...
    private function databasePackagePartKind(string $part, string $mediaType, bool $isDirectory): string
    {
        if ($isDirectory) {
    ...
    private static function databasePackagePartGroup(string $part): ?string
    {
        $segments = explode('/', trim($part, '/'));
        $group = strtolower($segments[1] ?? '');
     ...
     * @return array<string, mixed>
     */
    private function packageVersionMetadata(ZipPackage $package, array $manifest, array $undeclaredEntries): array
    {
        $candidatesByPart = [];
        foreach ($manifest as $item) {
            $part = $item['part'] ?? null;
            if (!is_string($part) || $part === '' || !$this->isVersionPackagePartName($part)) {
                continue;
            }
        ...
        foreach ($undeclaredEntries as $entry) {
            ...
            $mediaType = $this->versionMediaTypeFromPart($part) ?? '';
            $mediaTypeReport = self::mediaTypeReport($mediaType);
            $candidatesByPart[$part] = [
                'fullPath' => $part,
                'part' => $part,
        ...
        foreach ($candidatesByPart as $part => $item) {
            ...
            $mediaTypeValid = $isDirectory
                ? $mediaTypeReport['mediaTypeBase'] === ''
                : (!$missingMediaType && $this->isVersionPackageMediaType($mediaType));
            $kind = $this->versionPackagePartKind($part, $mediaType, $isDirectory);
            $versionId = self::versionPackagePartVersionId($part);
            $issues = [];
            if (!$exists) {
            ...
            $items[] = [
                ...
                'canExposeBytes' => false,
                'canExposeAsDocumentMedia' => false,
                'byteExposurePolicy' => $item['byteExposurePolicy'] ?? ($isDirectory ? 'directory-entry-no-bytes' : 'version-package-bytes-blocked'),
    ...
    private function versionMediaTypeFromPart(string $part): ?string
    {
        if (str_ends_with($part, '/')) {
            return '';
        }
    ...
    private function isVersionPackageMediaType(string $mediaType): bool
    {
        $base = self::mediaTypeReport($mediaType)['mediaTypeBase'];

        return self::isXmlMediaTypeBase($base)
            || $this->isEmbeddedObjectPackageMediaType($base)
    ...
    private function versionPackagePartKind(string $part, string $mediaType, bool $isDirectory): string
    {
        if ($isDirectory) {
        ...
        $base = self::mediaTypeReport($mediaType)['mediaTypeBase'];
        if ($this->isEmbeddedObjectPackageMediaType($base)) {
    ...
    private static function versionPackagePartVersionId(string $part): ?string
    {
        $segments = explode('/', trim($part, '/'));
    ...
    private function isDatabasePackagePartName(string $part): bool
    {
        return str_starts_with(strtolower(ltrim($part, '/')), 'database/');
    }
     ...
     * @return array<string, mixed>
     */
    private function packageGalleryMetadata(ZipPackage $package, array $manifest, array $undeclaredEntries): array
    {
        $candidatesByPart = [];
        foreach ($manifest as $item) {
            $part = $item['part'] ?? null;
            if (!is_string($part) || $part === '' || !$this->isGalleryPackagePartName($part)) {
                continue;
            }
        ...
        foreach ($undeclaredEntries as $entry) {
            ...
            $mediaType = $this->galleryMediaTypeFromPart($part) ?? '';
            $mediaTypeReport = self::mediaTypeReport($mediaType);
            $candidatesByPart[$part] = [
                'fullPath' => $part,
                'part' => $part,
                ...
                'declaredSize' => null,
                'declaredSizeMismatch' => false,
                'byteExposurePolicy' => str_ends_with($part, '/') ? 'directory-entry-no-bytes' : 'gallery-package-bytes-blocked',
            ];
        }
        ...
        foreach ($candidatesByPart as $part => $item) {
            ...
            $mediaTypeValid = $isDirectory
                ? $mediaTypeReport['mediaTypeBase'] === ''
                : (!$missingMediaType && $this->isGalleryMediaType($mediaType));
            $kind = $this->galleryPackagePartKind($part, $mediaType, $isDirectory);
            $group = self::galleryPackagePartGroup($part);
            $issues = [];
            if (!$exists) {
            ...
            $items[] = [
                ...
                'canExposeBytes' => false,
                'canExposeAsDocumentMedia' => false,
                'byteExposurePolicy' => $item['byteExposurePolicy'] ?? ($isDirectory ? 'directory-entry-no-bytes' : 'gallery-package-bytes-blocked'),
    ...
    private function isGalleryPackagePartName(string $part): bool
    {
        return str_starts_with(strtolower(ltrim($part, '/')), 'gallery/');
    }
     ...
     * @return array<string, mixed>
     */
    private function packageFormMetadata(ZipPackage $package, array $manifest, array $undeclaredEntries): array
    {
        $candidatesByPart = [];
        foreach ($manifest as $item) {
            $part = $item['part'] ?? null;
            if (!is_string($part) || $part === '' || !$this->isFormPackagePartName($part)) {
                continue;
            }
        ...
        foreach ($undeclaredEntries as $entry) {
            ...
            $mediaType = $this->formMediaTypeFromPart($part) ?? '';
            $mediaTypeReport = self::mediaTypeReport($mediaType);
            $candidatesByPart[$part] = [
                'fullPath' => $part,
                'part' => $part,
        ...
        foreach ($candidatesByPart as $part => $item) {
            ...
            $mediaTypeValid = $isDirectory
                ? $mediaTypeReport['mediaTypeBase'] === ''
                : (!$missingMediaType && $this->isFormMediaType($mediaType));
            $kind = $this->formPackagePartKind($part, $mediaType, $isDirectory);
            $group = self::formPackagePartGroup($part);
            $issues = [];
            if (!$exists) {
            ...
            $items[] = [
                ...
                'canExposeBytes' => false,
                'canExposeAsDocumentMedia' => false,
                'byteExposurePolicy' => $item['byteExposurePolicy'] ?? ($isDirectory ? 'directory-entry-no-bytes' : 'form-package-bytes-blocked'),
    ...
    private function isFormPackagePartName(string $part): bool
    {
        return str_starts_with(strtolower(ltrim($part, '/')), 'forms/');
    }

    private function isAttachmentPackagePartName(string $part): bool
    {
        return str_starts_with(strtolower(ltrim($part, '/')), 'attachments/');
    }

    private function galleryMediaTypeFromPart(string $part): ?string
    {
        if (str_ends_with($part, '/')) {
            return '';
        }
    ...
    private function formMediaTypeFromPart(string $part): ?string
    {
        if (str_ends_with($part, '/')) {
            return '';
        }
    ...
    private function isGalleryMediaType(string $mediaType): bool
    {
        $base = self::mediaTypeReport($mediaType)['mediaTypeBase'];
    ...
    private function isFormMediaType(string $mediaType): bool
    {
        $base = self::mediaTypeReport($mediaType)['mediaTypeBase'];
    ...
    private function galleryPackagePartKind(string $part, string $mediaType, bool $isDirectory): string
    {
        if ($isDirectory) {
    ...
    private function formPackagePartKind(string $part, string $mediaType, bool $isDirectory): string
    {
        if ($isDirectory) {
    ...
    private static function galleryPackagePartGroup(string $part): ?string
    {
        $segments = explode('/', trim($part, '/'));
        $group = strtolower($segments[1] ?? '');
    ...
    private static function formPackagePartGroup(string $part): ?string
    {
        $segments = explode('/', trim($part, '/'));
        $group = strtolower($segments[1] ?? '');
     ...
     * @return array<string, mixed>
     */
    private function packageReportMetadata(ZipPackage $package, array $manifest, array $undeclaredEntries): array
    {
        $candidatesByPart = [];
        foreach ($manifest as $item) {
            $part = $item['part'] ?? null;
            if (!is_string($part) || $part === '' || !$this->isReportPackagePartName($part)) {
                continue;
            }
        ...
        foreach ($undeclaredEntries as $entry) {
            ...
            $mediaType = $this->reportMediaTypeFromPart($part) ?? '';
            $mediaTypeReport = self::mediaTypeReport($mediaType);
            $candidatesByPart[$part] = [
                'fullPath' => $part,
                'part' => $part,
                ...
                'declaredSize' => null,
                'declaredSizeMismatch' => false,
                'byteExposurePolicy' => str_ends_with($part, '/') ? 'directory-entry-no-bytes' : 'report-package-bytes-blocked',
            ];
        }
        ...
        foreach ($candidatesByPart as $part => $item) {
            ...
            $mediaTypeValid = $isDirectory
                ? $mediaTypeReport['mediaTypeBase'] === ''
                : (!$missingMediaType && $this->isReportMediaType($mediaType));
            $kind = $this->reportPackagePartKind($part, $mediaType, $isDirectory);
            $group = self::reportPackagePartGroup($part);
            $issues = [];
            if (!$exists) {
            ...
            $items[] = [
                ...
                'canExposeBytes' => false,
                'canExposeAsDocumentMedia' => false,
                'byteExposurePolicy' => $item['byteExposurePolicy'] ?? ($isDirectory ? 'directory-entry-no-bytes' : 'report-package-bytes-blocked'),
    ...
    private function isReportPackagePartName(string $part): bool
    {
        return str_starts_with(strtolower(ltrim($part, '/')), 'reports/');
    }

    private function reportMediaTypeFromPart(string $part): ?string
    {
        if (str_ends_with($part, '/')) {
            return '';
        }
    ...
    private function isReportMediaType(string $mediaType): bool
    {
        $base = self::mediaTypeReport($mediaType)['mediaTypeBase'];
    ...
    private function reportPackagePartKind(string $part, string $mediaType, bool $isDirectory): string
    {
        if ($isDirectory) {
    ...
    private static function reportPackagePartGroup(string $part): ?string
    {
        $segments = explode('/', trim($part, '/'));
        $group = strtolower($segments[1] ?? '');
     ...
     * @return array<string, mixed>
     */
    private function packageAttachmentMetadata(ZipPackage $package, array $manifest, array $undeclaredEntries): array
    {
        $candidatesByPart = [];
        foreach ($manifest as $item) {
            $part = $item['part'] ?? null;
            if (!is_string($part) || $part === '' || !$this->isAttachmentPackagePartName($part)) {
                continue;
            }
        ...
        foreach ($undeclaredEntries as $entry) {
            ...
            $mediaType = $this->attachmentMediaTypeFromPart($part) ?? '';
            $mediaTypeReport = self::mediaTypeReport($mediaType);
            $candidatesByPart[$part] = [
                'fullPath' => $part,
                'part' => $part,
                ...
                'declaredSize' => null,
                'declaredSizeMismatch' => false,
                'byteExposurePolicy' => str_ends_with($part, '/') ? 'directory-entry-no-bytes' : 'attachment-package-bytes-blocked',
            ];
        }
        ...
        foreach ($candidatesByPart as $part => $item) {
            ...
            $mediaTypeValid = $isDirectory
                ? $mediaTypeReport['mediaTypeBase'] === ''
                : (!$missingMediaType && $this->isAttachmentMediaType($mediaType));
            $kind = $this->attachmentPackagePartKind($part, $mediaType, $isDirectory);
            $group = self::attachmentPackagePartGroup($part);
            $issues = [];
            if (!$exists) {
            ...
            $items[] = [
                ...
                'canExposeBytes' => false,
                'canExposeAsDocumentMedia' => false,
                'byteExposurePolicy' => $item['byteExposurePolicy'] ?? ($isDirectory ? 'directory-entry-no-bytes' : 'attachment-package-bytes-blocked'),
    ...
    private function attachmentMediaTypeFromPart(string $part): ?string
    {
        if (str_ends_with($part, '/')) {
            return '';
        }
    ...
    private function isAttachmentMediaType(string $mediaType): bool
    {
        $base = self::mediaTypeReport($mediaType)['mediaTypeBase'];
    ...
    private function attachmentPackagePartKind(string $part, string $mediaType, bool $isDirectory): string
    {
        if ($isDirectory) {
     ...
     * @return array<string, mixed>
     */
    private function packageTemplateMetadata(ZipPackage $package, array $manifest, array $undeclaredEntries): array
    {
        $candidatesByPart = [];
        foreach ($manifest as $item) {
            $part = $item['part'] ?? null;
            if (!is_string($part) || $part === '' || !$this->isTemplatePackagePartName($part)) {
                continue;
            }
        ...
        foreach ($undeclaredEntries as $entry) {
            ...
            $mediaType = $this->templateMediaTypeFromPart($part) ?? '';
            $mediaTypeReport = self::mediaTypeReport($mediaType);
            $candidatesByPart[$part] = [
                'fullPath' => $part,
                'part' => $part,
                ...
                'declaredSize' => null,
                'declaredSizeMismatch' => false,
                'byteExposurePolicy' => str_ends_with($part, '/') ? 'directory-entry-no-bytes' : 'template-package-bytes-blocked',
            ];
        }
        ...
        foreach ($candidatesByPart as $part => $item) {
            ...
            $mediaTypeValid = $isDirectory
                ? $mediaTypeReport['mediaTypeBase'] === ''
                : (!$missingMediaType && $this->isTemplateMediaType($mediaType));
            $kind = $this->templatePackagePartKind($part, $mediaType, $isDirectory);
            $group = self::templatePackagePartGroup($part);
            $issues = [];
            if (!$exists) {
            ...
            $items[] = [
                ...
                'canExposeBytes' => false,
                'canExposeAsDocumentMedia' => false,
                'byteExposurePolicy' => $item['byteExposurePolicy'] ?? ($isDirectory ? 'directory-entry-no-bytes' : 'template-package-bytes-blocked'),
    ...
    private function templateMediaTypeFromPart(string $part): ?string
    {
        if (str_ends_with($part, '/')) {
            return '';
        }
    ...
    private function isTemplateMediaType(string $mediaType): bool
    {
        $base = self::mediaTypeReport($mediaType)['mediaTypeBase'];
    ...
    private function templatePackagePartKind(string $part, string $mediaType, bool $isDirectory): string
    {
        if ($isDirectory) {
    ...
    private static function templatePackagePartGroup(string $part): ?string
    {
        $segments = explode('/', trim($part, '/'));
        $group = strtolower($segments[1] ?? '');
     ...
     * @return array<string, mixed>
     */
    private function packageDictionaryMetadata(ZipPackage $package, array $manifest, array $undeclaredEntries): array
    {
        $candidatesByPart = [];
        foreach ($manifest as $item) {
            $part = $item['part'] ?? null;
            if (!is_string($part) || $part === '' || !$this->isDictionaryPackagePartName($part)) {
                continue;
            }
        ...
        foreach ($undeclaredEntries as $entry) {
            ...
            $mediaType = $this->dictionaryMediaTypeFromPart($part) ?? '';
            $mediaTypeReport = self::mediaTypeReport($mediaType);
            $candidatesByPart[$part] = [
                'fullPath' => $part,
                'part' => $part,
                ...
                'declaredSize' => null,
                'declaredSizeMismatch' => false,
                'byteExposurePolicy' => str_ends_with($part, '/') ? 'directory-entry-no-bytes' : 'dictionary-package-bytes-blocked',
            ];
        }
        ...
        foreach ($candidatesByPart as $part => $item) {
            ...
            $mediaTypeValid = $isDirectory
                ? $mediaTypeReport['mediaTypeBase'] === ''
                : (!$missingMediaType && $this->isDictionaryMediaType($mediaType));
            $kind = $this->dictionaryPackagePartKind($part, $mediaType, $isDirectory);
            $group = self::dictionaryPackagePartGroup($part);
            $issues = [];
            if (!$exists) {
            ...
            $items[] = [
                ...
                'canExposeBytes' => false,
                'canExposeAsDocumentMedia' => false,
                'byteExposurePolicy' => $item['byteExposurePolicy'] ?? ($isDirectory ? 'directory-entry-no-bytes' : 'dictionary-package-bytes-blocked'),
    ...
    private function isDictionaryPackagePartName(string $part): bool
    {
        return str_starts_with(strtolower(ltrim($part, '/')), 'dictionaries/');
    }

    private function dictionaryMediaTypeFromPart(string $part): ?string
    {
        if (str_ends_with($part, '/')) {
            return '';
        }
    ...
    private function isDictionaryMediaType(string $mediaType): bool
    {
        $base = self::mediaTypeReport($mediaType)['mediaTypeBase'];
    ...
    private function dictionaryPackagePartKind(string $part, string $mediaType, bool $isDirectory): string
    {
        if ($isDirectory) {
    ...
    private static function dictionaryPackagePartGroup(string $part): ?string
    {
        $segments = explode('/', trim($part, '/'));
        $group = strtolower($segments[1] ?? '');
     ...
     * @return array<string, mixed>
     */
    private function packageEventMetadata(ZipPackage $package, array $manifest, array $undeclaredEntries): array
    {
        $candidatesByPart = [];
        foreach ($manifest as $item) {
            $part = $item['part'] ?? null;
            if (!is_string($part) || $part === '' || !$this->isEventPackagePartName($part)) {
                continue;
            }
        ...
        foreach ($undeclaredEntries as $entry) {
            ...
            $mediaType = $this->eventMediaTypeFromPart($part) ?? '';
            $mediaTypeReport = self::mediaTypeReport($mediaType);
            $candidatesByPart[$part] = [
                'fullPath' => $part,
                'part' => $part,
                ...
                'declaredSize' => null,
                'declaredSizeMismatch' => false,
                'byteExposurePolicy' => str_ends_with($part, '/') ? 'directory-entry-no-bytes' : 'event-package-bytes-blocked',
            ];
        }
        ...
        foreach ($candidatesByPart as $part => $item) {
            ...
            $mediaTypeValid = $isDirectory
                ? $mediaTypeReport['mediaTypeBase'] === ''
                : (!$missingMediaType && $this->isEventMediaType($mediaType));
            $kind = $this->eventPackagePartKind($part, $mediaType, $isDirectory);
            $group = self::eventPackagePartGroup($part);
            $issues = [];
            if (!$exists) {
            ...
            $items[] = [
                ...
                'canExposeBytes' => false,
                'canExposeAsDocumentMedia' => false,
                'byteExposurePolicy' => $item['byteExposurePolicy'] ?? ($isDirectory ? 'directory-entry-no-bytes' : 'event-package-bytes-blocked'),
    ...
    private function isEventPackagePartName(string $part): bool
    {
        return str_starts_with(strtolower(ltrim($part, '/')), 'events/');
    }

    private function eventMediaTypeFromPart(string $part): ?string
    {
        if (str_ends_with($part, '/')) {
            return '';
        }
    ...
    private function isEventMediaType(string $mediaType): bool
    {
        $base = self::mediaTypeReport($mediaType)['mediaTypeBase'];
    ...
    private function eventPackagePartKind(string $part, string $mediaType, bool $isDirectory): string
    {
        if ($isDirectory) {
    ...
    private static function eventPackagePartGroup(string $part): ?string
    {
        $segments = explode('/', trim($part, '/'));
        $group = strtolower($segments[1] ?? '');
    ...
    {
        return strtolower(ltrim($part, '/')) === 'layout-cache';
    }

    private function isMetaInfSidecarPackagePartName(string $part): bool
    {
        $normalized = strtolower(ltrim($part, '/'));
    ...
    private function isLayoutCacheMediaType(string $mediaType): bool
    {
