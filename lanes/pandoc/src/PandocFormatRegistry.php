...
final class PandocFormatRegistry
{
    ...
    private const PHP_INPUT_SUPPORT = [
        ...
        'pptx' => [
            ...
            'notes' => 'Bounded PPTX OpenXML package reader maps presentation slide order, slide-size metadata, slide-local title placeholders with upstream fallback slide titles, text boxes including grouped text boxes and explicit empty DrawingML text paragraphs, run-level/text-box/picture hyperlink relationships, DrawingML line breaks/tabs, Wingdings/explicit bullet groups, auto-numbered DrawingML list groups with start/style/delimiter metadata, nested DrawingML list levels, buNone list-item continuation paragraphs, speaker notesSlide relationships as notes Div blocks, simple tables with upstream-style rowless-table omission plus explicit empty cell text, chart graphic frames and missing graphic metadata as upstream-style generic graphic placeholders with chart metadata sidecars, verified image references including grouped pictures and upstream-compatible media-relative/direct targets with missing/linked-media/malformed-picture diagnostics, SmartArt hierarchy with placeholder/layout sidecars plus visible parse diagnostics or generic placeholders for incomplete SmartArt parts, slide comments, shape z-order/layout metadata, unsupported drawable-shape diagnostics, and internal rich-media references into the shared AST with checked-in current upstream pptx-reader/basic package/native content parity.',
        ],
        'ris' => [
            'status' => 'partial',
            'implementation' => BibliographyReader::class,
