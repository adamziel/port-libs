...
final class PandocFormatRegistry
{
    ...
    private const PHP_INPUT_SUPPORT = [
        ...
        'pptx' => [
            ...
            'notes' => 'Bounded PPTX OpenXML package reader maps presentation slide order, slide-size metadata, slide-local title placeholders with upstream fallback slide titles, text boxes including explicit empty DrawingML text paragraphs and upstream-style text extraction for break/tab markers, upstream-ignored run/text-box/picture hyperlink relationships, Wingdings/explicit bullet groups, upstream-plain auto-numbered and buNone DrawingML paragraphs, upstream-style adjacent same-level bullet grouping, review-only speaker notesSlide relationships, simple tables with upstream-style rowless-table omission, upstream-default visible spans, plus explicit empty cell text and table style sidecars, chart graphic frames and missing graphic metadata as upstream-style generic graphic placeholders with chart metadata sidecars, verified embedded image references with upstream-compatible media-relative/direct targets plus missing/linked-image/malformed-picture diagnostics, SmartArt hierarchy with placeholder/layout sidecars plus visible parse diagnostics or generic placeholders for incomplete SmartArt parts, review-only slide comments, shape z-order/layout metadata, upstream-skipped grouped-shape diagnostics, unsupported drawable-shape diagnostics, and review-only internal rich-media references with checked-in current upstream pptx-reader/basic package/native content parity.',
        ],
        'ris' => [
            'status' => 'partial',
            'implementation' => BibliographyReader::class,
