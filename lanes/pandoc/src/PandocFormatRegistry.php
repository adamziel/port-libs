...
final class PandocFormatRegistry
{
    ...
    private const PHP_INPUT_SUPPORT = [
        ...
        'pptx' => [
            ...
            'notes' => 'Bounded PPTX OpenXML package reader maps presentation slide order, slide-size metadata, title placeholders, text boxes including grouped text boxes, run-level/text-box/picture hyperlink relationships, DrawingML line breaks/tabs, Wingdings/explicit bullet groups, auto-numbered DrawingML list groups with start/style/delimiter metadata, nested DrawingML list levels, buNone list-item continuation paragraphs, speaker notesSlide relationships as notes Div blocks, simple tables with upstream-style rowless-table omission, chart graphic frames as upstream-style generic graphic placeholders with chart metadata sidecars, verified image references including grouped pictures with missing/linked-media/malformed-picture diagnostics, SmartArt hierarchy with placeholder/layout sidecars and visible parse diagnostics for broken SmartArt parts, slide comments, shape z-order/layout metadata, unsupported drawable-shape diagnostics, and internal rich-media references into the shared AST with checked-in current upstream pptx-reader/basic package/native content parity.',
        ],
        'ris' => [
            'status' => 'partial',
            'implementation' => BibliographyReader::class,
