...
final class PandocFormatRegistry
{
    ...
    private const PHP_OUTPUT_SUPPORT = [
        ...
        'pptx' => [
            ...
            'notes' => 'Bounded PPTX output emits a deterministic PresentationML OPC ZIP package with content types, root/presentation/slide relationships, core, extended, and custom document properties from Pandoc metadata, a slide master/layout/theme, table styles, title/content slides, standalone image-only and content-with-caption slide grouping with figure captions, single-column and two-column text/image content placeholders, shape-free blank slides for empty or NBSP-only headings/bodies, table-of-contents slides with internal slide links, slide-level-aware heading breaks, bullet and ordered list paragraphs preserving inline runs plus Pandoc style/delimiter autonumbering, ordered-list margins, and list continuation paragraphs, DrawingML tables, hyperlink relationships, local image media parts, Courier code inline/block runs, small-caps and adjacent-text inline run properties, background images from heading attributes, horizontal-rule slide splits, slide-level-0 first-heading title promotion, metadata title-slide author subtitles, speaker-note notesSlides/notesMaster parts from upstream-style notes Divs and metadata notes, public endnote slides from inline Note nodes, raw OpenXML inline/block passthrough, and upstream-style dropping of non-OpenXML raw inline plus empty-alt unresolved image content. Full upstream PowerPoint writer template/layout/media parity remains open.',
        ],
    ];
