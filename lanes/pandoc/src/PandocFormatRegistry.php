...
final class PandocFormatRegistry
{
    ...
    private const PHP_INPUT_SUPPORT = [
        ...
        'pptx' => [
            ...
            'notes' => 'Bounded PPTX OpenXML package reader maps upstream-style root officeDocument suffix discovery including missing root relationship parts, absent presentation relationships, missing root relationship IDs, ignored TargetMode, root-level and alternate presentation relationship sidecar paths with upstream ppt-prefixed slide targets, direct-child relationship element-name agnostic parsing with nested root/presentation/slide relationships ignored, malformed core root/presentation/slide relationship part failures and malformed presentation/slide XML part failures, unqualified relationship Type/Id/Target attribute lookup, first-match missing/empty Target failures, root-relative, dot-segment, and percent-encoded Target literal lookups, and missing presentation part entry failures, optional slide-less presentation lists, presentation slide order with direct-child p:sldId filtering and ignored p:sldId id attributes including missing slide r:id, first duplicate slide relationship IDs, missing slide relationship Id skips, root-relative/dot-segment/empty slide Target lookup failures, and missing slide part entry failures, direct-child slide-size lookup including missing/invalid numeric fallback and nested size fallback, visible shape matching, and slide shape-tree lookup scoped to the presentation/slide root p prefix binding including missing-cSld/missing-spTree fallback headers, DrawingML paragraph, paragraph-property, run-symbol, picture blip, graphic-frame, and table row/cell/text-body lookup scoped to the slide root a prefix binding with element-local fallback and upstream namespace-agnostic t descendant text extraction, prefix-scoped and element-local r-prefix relationship attribute lookup for slide IDs, embed-first picture blips over link alternatives, and SmartArt relIds without inherited shape-tree r-prefix fallback, unqualified picture name/description metadata lookup, missing picture blip-chain skips, permissive presentation/slide root names with namespace-scoped children, Haskell readMaybe-style decimal/hex/octal, parenthesized, Unicode-whitespace, and bounded Int bullet-level numeric attribute parsing, slide-size metadata, non-fatal invalid review-only sidecar targets and relationship parts, review-only core/extended/custom document properties, slide-local nvSpPr title and ctrTitle placeholders across immediate children with unqualified type matching, upstream fallback slide titles, and first-empty title placeholder selection, text boxes including explicit empty DrawingML text paragraphs, missing/paragraphless text-body skips, and upstream-style text extraction for break/tab markers and DrawingML field text, upstream-ignored run/text-box/picture hyperlink relationships, case-sensitive substring Wingdings and buChar-over-buNone explicit bullet groups, upstream-plain auto-numbered and buNone DrawingML paragraphs, upstream-style adjacent same-level bullet grouping including empty bullet list items, review-only speaker notesSlide relationships, simple tables with upstream-style rowless-table and missing-table-child omission, case-sensitive table-before-diagram URI precedence, and first-row native column specs, upstream-default visible spans, plus explicit missing/empty cell text and table style sidecars, chart graphic frames and missing/empty graphic metadata as upstream-style generic graphic placeholders with chart metadata sidecars, upstream-skipped graphic frames without graphic/graphicData, verified embedded image references with upstream-compatible media-relative/direct and untyped slide media relationship targets plus missing/unknown/non-embed/linked-image/malformed-picture diagnostics, review-only slide background image references, SmartArt direct-child local-name relIds discovery and data/layout lookup including partial, untyped, and empty-target relationship targets scoped to the SmartArt root dgm prefix binding, SmartArt layout uniqueId fallback including bare-name and empty-name semantics, unknown fallback, and all-descendant text extraction, SmartArt hierarchy including upstream present-empty model, missing modelId skips, absent connection-list empty hierarchies, sorted parent model IDs, connection-order children, parent-only paragraphs after empty child filtering, last duplicate modelId text, Unicode-whitespace-only node filtering, orphan/typed/malformed connection filtering, and connection IDs with placeholder/layout sidecars plus visible parse diagnostics for missing data/layout parts or invalid SmartArt data/layout XML and generic placeholders for incomplete SmartArt parts, review-only slide comments, shape z-order/layout metadata, upstream-skipped grouped-shape diagnostics, unsupported drawable-shape diagnostics, and review-only internal rich-media references with checked-in current upstream pptx-reader/basic package/native content parity.',
        ],
        'ris' => [
            'status' => 'partial',
            'implementation' => BibliographyReader::class,
