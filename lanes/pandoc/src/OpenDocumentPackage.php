...
final class OpenDocumentPackage
{
    ...
    private function packageInventory(): array
    {
        ...
        foreach ($this->package->entries() as $centralDirectoryIndex => $entry) {
            ...
            $item = [
                ...
                'packagePathByteLengthBucketMin' => $packagePathByteLengthBucket['packagePathByteLengthBucketMin'],
                'packagePathByteLengthBucketMax' => $packagePathByteLengthBucket['packagePathByteLengthBucketMax'],
                ...
                'packageThumbnailPart' => self::isThumbnailPackagePartName($entry->name),
        ...
        $packagePathByteLengths = self::packagePathByteLengthBucketInventory($parts, 'path');
    ...
    private static function packagePathSegmentCharacterSummary(array $parts): array
    {
        ...
        foreach ($parts as $path => $part) {
            ...
            foreach (array_keys($partFlags) as $flag) {
                $flagPartNames[$flag][] = $path;
            }

            $items[] = [
                'path' => $path,
                'part' => is_string($part['part'] ?? null) ? $part['part'] : $path,
                ...
                'manifestPath' => is_string($part['manifestPath'] ?? null) ? $part['manifestPath'] : null,
    ...
    private function packageIdentity(array $packageInventory, array $packageCoreParts): array
    {
        ...
        foreach ($packageInventory['parts'] ?? [] as $part) {
            ...
            $packageEntries[] = self::withoutEmptyValues([
                ...
                'rawNameMatchesDecodedName' => $part['rawNameMatchesDecodedName'] ?? null,
                ...
                'zipModifiedAt' => $part['zipModifiedAt'] ?? null,
                'zipTimestampSource' => $part['zipTimestampSource'] ?? null,
                ...
                'zipDosModifiedAt' => $part['zipDosModifiedAt'] ?? null,
                'zipExtendedModifiedAt' => $part['zipExtendedModifiedAt'] ?? null,
                'zipNtfsModifiedAt' => $part['zipNtfsModifiedAt'] ?? null,
                'zipCentralModifiedAt' => $part['zipCentralModifiedAt'] ?? null,
                'zipCentralTimestampSource' => $part['zipCentralTimestampSource'] ?? null,
                'zipLocalExtendedModifiedAt' => $part['zipLocalExtendedModifiedAt'] ?? null,
                'zipLocalNtfsModifiedAt' => $part['zipLocalNtfsModifiedAt'] ?? null,
                'zipLocalModifiedAt' => $part['zipLocalModifiedAt'] ?? null,
                'zipLocalTimestampSource' => $part['zipLocalTimestampSource'] ?? null,
                'zipTimestampIssues' => $part['zipTimestampIssues'] ?? [],
                ...
                'manifestMediaTypeParameterCount' => $part['manifestMediaTypeParameterCount'] ?? 0,
                'manifestMediaTypeParameters' => $part['manifestMediaTypeParameters'] ?? [],
                'manifestMediaTypeParameterMap' => $part['manifestMediaTypeParameterMap'] ?? [],
                ...
                'manifestAttributeCount' => $part['manifestAttributeCount'] ?? 0,
                'manifestAttributeNames' => $part['manifestAttributeNames'] ?? [],
        ...
        $payload = [
            ...
            'packageDirectoryDepthCounts' => $packageInventory['packageDirectoryDepthCounts'] ?? [],
            'maxPackageDirectoryDepth' => $packageInventory['maxPackageDirectoryDepth'] ?? 0,
            'deepestPackagePartNames' => $packageInventory['deepestPackagePartNames'] ?? [],
            'packageRoleSummaryCount' => $packageInventory['packageRoleSummaryCount'] ?? 0,
            'packageRoleSummaries' => $packageInventory['packageRoleSummaries'] ?? [],
            ...
            'zipTimestampEntryCount' => $packageInventory['zipTimestampEntryCount'] ?? 0,
            'zipDosTimestampEntryCount' => $packageInventory['zipDosTimestampEntryCount'] ?? 0,
            ...
            'zipExtraFieldEntryCount' => $packageInventory['zipExtraFieldEntryCount'] ?? 0,
            'zipDuplicateExtraFieldEntryCount' => $packageInventory['zipDuplicateExtraFieldEntryCount'] ?? 0,
            'zipMismatchedExtraFieldEntryCount' => $packageInventory['zipMismatchedExtraFieldEntryCount'] ?? 0,
            'zipMismatchedExtraFieldValueEntryCount' => $packageInventory['zipMismatchedExtraFieldValueEntryCount'] ?? 0,
            'zipCentralOnlyExtraFieldEntryCount' => $packageInventory['zipCentralOnlyExtraFieldEntryCount'] ?? 0,
            'zipLocalOnlyExtraFieldEntryCount' => $packageInventory['zipLocalOnlyExtraFieldEntryCount'] ?? 0,
            'zipExtraFieldIdCount' => $packageInventory['zipExtraFieldIdCount'] ?? 0,
            'zipCentralExtraFieldIdCount' => $packageInventory['zipCentralExtraFieldIdCount'] ?? 0,
            'zipLocalExtraFieldIdCount' => $packageInventory['zipLocalExtraFieldIdCount'] ?? 0,
            'zipSharedExtraFieldIdCount' => $packageInventory['zipSharedExtraFieldIdCount'] ?? 0,
            'zipCentralOnlyExtraFieldIdCount' => $packageInventory['zipCentralOnlyExtraFieldIdCount'] ?? 0,
            'zipLocalOnlyExtraFieldIdCount' => $packageInventory['zipLocalOnlyExtraFieldIdCount'] ?? 0,
            'zipExtraFieldIdUsage' => $packageInventory['zipExtraFieldIdUsage'] ?? [],
            'comments' => [
                'hasPackageComment' => ($comments['hasPackageComment'] ?? false) === true,
    ...
    /**
     * @return array{packagePathByteLengthBucket:string,packagePathByteLengthBucketMin:int,packagePathByteLengthBucketMax:?int}
     */
    private static function packagePathByteLengthBucket(int $byteLength): array
    {
        if ($byteLength <= 8) {
            return [
                ...
                'packagePathByteLengthBucketMin' => 0,
                'packagePathByteLengthBucketMax' => 8,
            ];
        }
        if ($byteLength <= 16) {
            return [
                'packagePathByteLengthBucket' => '9-to-16-bytes',
                'packagePathByteLengthBucketMin' => 9,
                'packagePathByteLengthBucketMax' => 16,
            ];
        }
        if ($byteLength <= 32) {
            return [
                'packagePathByteLengthBucket' => '17-to-32-bytes',
                'packagePathByteLengthBucketMin' => 17,
                'packagePathByteLengthBucketMax' => 32,
            ];
        }
        if ($byteLength <= 64) {
            return [
                'packagePathByteLengthBucket' => '33-to-64-bytes',
                'packagePathByteLengthBucketMin' => 33,
                'packagePathByteLengthBucketMax' => 64,
            ];
        }

        return [
            'packagePathByteLengthBucket' => 'over-64-bytes',
            'packagePathByteLengthBucketMin' => 65,
            'packagePathByteLengthBucketMax' => null,
        ];
    }
     ...
     *     packagePathByteLengthRoleCounts:array<string, int>,
     *     entryNamesByPackagePathByteLengthRole:array<string, list<string>>,
     *     packagePathByteLengthByteExposurePolicyCounts:array<string, int>,
     *     entryNamesByPackagePathByteLengthByteExposurePolicy:array<string, list<string>>,
     *     packagePathByteLengthBucketSummaries:list<array<string, mixed>>
     * }
     */
    private static function packagePathByteLengthBucketInventory(array $parts, string $nameKey): array
    {
        $bucketOrder = [
            'up-to-8-bytes',
            '9-to-16-bytes',
        ...
        foreach ($parts as $fallbackName => $part) {
            ...
            $bucketName = $bucket['packagePathByteLengthBucket'];
            $byteLength = is_int($part['byteLength'] ?? null) ? $part['byteLength'] : 0;
            $compressedByteLength = is_int($part['compressedByteLength'] ?? null) ? $part['compressedByteLength'] : 0;
            $roles = array_values(array_map('strval', is_array($part['roles'] ?? null) ? $part['roles'] : []));
            $byteExposurePolicy = is_string($part['byteExposurePolicy'] ?? null) ? $part['byteExposurePolicy'] : '';
            ...
            $entryNamesByBucket[$bucketName][] = $partName;

            if (!isset($summaries[$bucketName])) {
                $summaries[$bucketName] = [
                    'packagePathByteLengthBucket' => $bucketName,
                    'packagePathByteLengthBucketMin' => $bucket['packagePathByteLengthBucketMin'],
                    'packagePathByteLengthBucketMax' => $bucket['packagePathByteLengthBucketMax'],
                    'partCount' => 0,
                    'directoryEntryCount' => 0,
                    'manifestDeclaredPartCount' => 0,
                    'undeclaredPartCount' => 0,
            ...
            ++$summaries[$bucketName]['partCount'];
            $summaries[$bucketName]['byteLength'] += $byteLength;
            $summaries[$bucketName]['compressedByteLength'] += $compressedByteLength;
            $summaries[$bucketName]['packagePathByteLengthTotal'] += $pathByteLength;
            $summaries[$bucketName]['entryNames'][] = $partName;
            if (($part['isDirectory'] ?? false) === true) {
                ++$summaries[$bucketName]['directoryEntryCount'];
            }
            if (($part['declaredInManifest'] ?? false) === true) {
                ++$summaries[$bucketName]['manifestDeclaredPartCount'];
            }
            if (($part['undeclared'] ?? false) === true) {
                ++$summaries[$bucketName]['undeclaredPartCount'];
            }
            if (($part['encrypted'] ?? false) === true) {
                ++$summaries[$bucketName]['encryptedPartCount'];
            }
            if (($part['canExposeBytes'] ?? false) === true) {
                ++$summaries[$bucketName]['exposablePartCount'];
            } else {
                ++$summaries[$bucketName]['blockedPartCount'];
            }
            ...
            foreach ($roles as $role) {
                ...
                $roleCounts[$role] = ($roleCounts[$role] ?? 0) + 1;
                $entryNamesByRole[$role][] = $partName;
                $summaries[$bucketName]['roleCounts'][$role] =
                    ($summaries[$bucketName]['roleCounts'][$role] ?? 0) + 1;
            }
            if ($byteExposurePolicy !== '') {
                $byteExposurePolicyCounts[$byteExposurePolicy] = ($byteExposurePolicyCounts[$byteExposurePolicy] ?? 0) + 1;
                $entryNamesByByteExposurePolicy[$byteExposurePolicy][] = $partName;
                $summaries[$bucketName]['byteExposurePolicyCounts'][$byteExposurePolicy] =
                    ($summaries[$bucketName]['byteExposurePolicyCounts'][$byteExposurePolicy] ?? 0) + 1;
            }
            ...
            $longestPart = $summaries[$bucketName]['longestPart'];
            if (
                !is_array($longestPart)
                || $pathByteLength > (int) ($longestPart['packagePathByteLength'] ?? 0)
                || (
                    $pathByteLength === (int) ($longestPart['packagePathByteLength'] ?? 0)
                    && strcmp($partName, (string) ($longestPart['path'] ?? '')) < 0
                )
            ) {
                $summaries[$bucketName]['longestPackagePathByteLength'] = $pathByteLength;
                $summaries[$bucketName]['longestPart'] = $partSummary;
            }
        }

        foreach ($entryNamesByBucket as $bucketName => $names) {
            sort($names, SORT_STRING);
            $entryNamesByBucket[$bucketName] = $names;
        }
        foreach ($entryNamesByRole as $role => $names) {
            sort($names, SORT_STRING);
            $entryNamesByRole[$role] = $names;
        }
        foreach ($entryNamesByByteExposurePolicy as $policy => $names) {
            sort($names, SORT_STRING);
            $entryNamesByByteExposurePolicy[$policy] = $names;
        }
        ksort($roleCounts, SORT_STRING);
        ksort($entryNamesByRole, SORT_STRING);
        ksort($byteExposurePolicyCounts, SORT_STRING);
        ksort($entryNamesByByteExposurePolicy, SORT_STRING);

        $orderedBucketCounts = [];
        $orderedEntryNamesByBucket = [];
        $orderedSummaries = [];
        foreach ($bucketOrder as $bucketName) {
            if (!isset($bucketCounts[$bucketName])) {
                continue;
            }
            $orderedBucketCounts[$bucketName] = $bucketCounts[$bucketName];
            $orderedEntryNamesByBucket[$bucketName] = $entryNamesByBucket[$bucketName] ?? [];
            $summary = $summaries[$bucketName];
        ...
        return [
            'packagePathByteLengthBucketCount' => count($orderedBucketCounts),
            'packagePathByteLengthBuckets' => array_keys($orderedBucketCounts),
            'packagePathByteLengthBucketCounts' => $orderedBucketCounts,
            'entryNamesByPackagePathByteLengthBucket' => $orderedEntryNamesByBucket,
    ...
    private static function packagePartAreaDepthItemName(array $part): string
    {
        ...
        return is_string($partName) ? $partName : '';
    }
    ...
    private static function withPackageEntryMetadata(array $entries, ZipPackage $package): array
    {
        ...
        foreach ($entries as $entry) {
            ...
            $packageThumbnailPart = is_string($packagePath) && self::isThumbnailPackagePartName($packagePath);
    ...
    private static function packageMetaInfSidecarMetadata(ZipPackage $package, array $manifestEntries, array $undeclaredPackageEntries): array
    {
        ...
        foreach ($candidatesByPath as $packagePath => $entry) {
            ...
            if (($entry['declaredSizeInvalid'] ?? false) === true) {
                $issues[] = 'odf-meta-inf-sidecar-invalid-declared-size';
            }
            foreach ($issues as $issue) {
                $issueCodes[$issue] = true;
            }
    ...
    private static function packageGalleryMetadata(ZipPackage $package, array $manifestEntries, array $undeclaredPackageEntries): array
    {
        ...
        foreach ($candidatesByPath as $packagePath => $entry) {
            ...
            if (($entry['declaredSizeInvalid'] ?? false) === true) {
                $issues[] = 'odf-gallery-package-invalid-declared-size';
            }
            foreach ($issues as $issue) {
                $issueCodes[$issue] = true;
            }
    ...
    private static function packageTemplateMetadata(ZipPackage $package, array $manifestEntries, array $undeclaredPackageEntries): array
    {
        ...
        foreach ($candidatesByPath as $packagePath => $entry) {
            ...
            if (($entry['declaredSizeInvalid'] ?? false) === true) {
                $issues[] = 'odf-template-package-invalid-declared-size';
            }
            if ($missingMediaType) {
    ...
    private static function packageDictionaryMetadata(ZipPackage $package, array $manifestEntries, array $undeclaredPackageEntries): array
    {
        ...
        foreach ($candidatesByPath as $packagePath => $entry) {
            ...
            if (($entry['declaredSizeInvalid'] ?? false) === true) {
                $issues[] = 'odf-dictionary-package-invalid-declared-size';
            }
            foreach ($issues as $issue) {
                $issueCodes[$issue] = true;
            }
    ...
    private static function packageEventMetadata(ZipPackage $package, array $manifestEntries, array $undeclaredPackageEntries): array
    {
        ...
        foreach ($candidatesByPath as $packagePath => $entry) {
            ...
            if (($entry['declaredSizeInvalid'] ?? false) === true) {
                $issues[] = 'odf-event-package-invalid-declared-size';
            }
            foreach ($issues as $issue) {
                $issueCodes[$issue] = true;
            }
    ...
    private static function packageExtensionMetadata(ZipPackage $package, array $manifestEntries, array $undeclaredPackageEntries): array
    {
        ...
        foreach ($candidatesByPath as $packagePath => $entry) {
            ...
            if (($entry['declaredSizeInvalid'] ?? false) === true) {
                $issues[] = 'odf-extension-package-invalid-declared-size';
            }
            foreach ($issues as $issue) {
                $issueCodes[$issue] = true;
            }
    ...
    private static function packageDatabaseMetadata(ZipPackage $package, array $manifestEntries, array $undeclaredPackageEntries): array
    {
        ...
        foreach ($candidatesByPath as $packagePath => $entry) {
            ...
            if (($entry['declaredSizeInvalid'] ?? false) === true) {
                $issues[] = 'odf-database-package-invalid-declared-size';
            }
            foreach ($issues as $issue) {
                $issueCodes[$issue] = true;
            }
    ...
    private static function packageVersionMetadata(ZipPackage $package, array $manifestEntries, array $undeclaredPackageEntries): array
    {
        ...
        foreach ($candidatesByPath as $packagePath => $entry) {
            ...
            if (($entry['declaredSizeInvalid'] ?? false) === true) {
                $issues[] = 'odf-version-package-invalid-declared-size';
            }
            foreach ($issues as $issue) {
                $issueCodes[$issue] = true;
            }
    ...
    private static function packageConfigurationMetadata(ZipPackage $package, array $manifestEntries, array $undeclaredPackageEntries): array
    {
        ...
        foreach ($candidatesByPath as $packagePath => $entry) {
            ...
            if (($entry['declaredSizeInvalid'] ?? false) === true) {
                $issues[] = 'odf-configuration-invalid-declared-size';
            }
            foreach ($issues as $issue) {
                $issueCodes[$issue] = true;
            }
    ...
    private static function manifestReviewItem(array $entry): array
    {
        return [
            ...
            'signaturePackagePart' => ($entry['signaturePackagePart'] ?? false) === true,
            'configurationPackagePart' => ($entry['configurationPackagePart'] ?? false) === true,
            'packageThumbnailPart' => ($entry['packageThumbnailPart'] ?? false) === true,
            'fontPackagePart' => ($entry['fontPackagePart'] ?? false) === true,
            'rdfMetadataPart' => ($entry['rdfMetadataPart'] ?? false) === true,
            'layoutCachePackagePart' => ($entry['layoutCachePackagePart'] ?? false) === true,
            'metaInfSidecarPackagePart' => ($entry['metaInfSidecarPackagePart'] ?? false) === true,
