...
final class MediaBag
{
    ...
    public function fillDocument(AstNode $document, array $resources): array
    {
        ...
        $document = $this->mapResourceNodes(
            $document,
            function (AstNode $image) use ($resources, $resourcesByCanonicalSource, &$diagnostics): AstNode {
                ...
                if ($resource !== null) {
                    $this->insertResource($source, $resource);
    ...
    public function extractMedia(AstNode $document, string $destination): array
    {
        ...
        $document = $this->mapResourceNodes(
            $document,
            function (AstNode $image) use ($destination, $plannedPaths, &$diagnostics): AstNode {
                $source = (string) $image->attr('url', '');
     ...
     */
    private function insertResource(string $source, string|array $resource): void
    {
    ...
    private function mediaProvenanceAttributes(
        ...
        if (is_array($existingAttributes)) {
            foreach ($existingAttributes as $name => $value) {
                if (is_string($name) && (string) $value !== '') {
    ...
    /**
     * @return array<string, array{path:string, collision:string}>
     */
    private function plannedExtractionPaths(): array
    {
        ...
        foreach ($this->itemsForExtraction() as $item) {
            $path = $item['path'];
            $collision = 'none';
            $caseFoldedPath = strtolower($path);
            ...
            $paths[$item['canonicalSource']] = ['path' => $path, 'collision' => $collision];
            $used[$path] = $item['sha1'];
            $usedCaseFolded[strtolower($path)] = ['path' => $path, 'sha1' => $item['sha1']];
        }
    ...
    private static function disambiguateMediaPath(
        ...
        do {
            ...
            $candidateCaseFolded = strtolower($candidate);
    ...
    private static function pathOnlyRelativeSource(string $source): string
    {
        ...
        return $positions === [] ? $source : substr($source, 0, min($positions));
    }
    ...
    private function linkedMimeGroups(AstNode $node): array
    {
        ...
        foreach ($groups as $mimeType => $canonicalSources) {
            ...
            foreach ($sources as $canonicalSource) {
                $linkedMimeGroups[$canonicalSource] = ['group' => $group, 'size' => $size];
            }
        }
    ...
    private static function decodedRelativeSourceKey(string $source): ?string
    {
        ...
        if ($decoded === $source || str_contains($decoded, "\0") || !self::isSafeRelativeMediaPath($decoded)) {
            return null;
        }
    ...
    {
        return str_starts_with($source, 'data:') ? 'data-uri' : $source;
    }
}
