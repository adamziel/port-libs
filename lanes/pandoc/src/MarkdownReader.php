...
final class MarkdownReader
{
    ...
    private function tryReadDivBlock(array $lines, int &$index): ?AstNode
    {
        ...
        $attrs = $this->htmlDivOpeningAttributes($m[0][0]);
        $content = [];
        $depth = 1;
        $openingIndex = $index;
        $cursor = $index;
     ...
     * @return array<string, mixed>
     */
    private function htmlDivOpeningAttributes(string $openingTag): array
    {
        $body = XmlHtml5Dom::parseHtmlFragmentBody(trim($openingTag) . '</div>');
        if (!$body instanceof \DOMElement) {
            return [];
        }

        $div = $this->firstChildElement($body, 'div');
        if (!$div instanceof \DOMElement) {
            return [];
        }

        return $this->htmlElementPandocAttrs($div);
    }
