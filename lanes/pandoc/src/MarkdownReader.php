...
final class MarkdownReader
{
    ...
    private function tryReadRawHtmlBlock(array $lines, int &$index): ?AstNode
    {
        ...
        if (preg_match('/^ {0,3}<(abbr|address|animate|animateMotion|animateTransform|article|aside|audio|bdi|bdo|button|canvas|circle|cite|clipPath|data|datalist|defs|desc|details|dfn|dialog|ellipse|feDropShadow|feFlood|feGaussianBlur|feOffset|fieldset|figcaption|figure|filter|footer|foreignObject|form|g|header|hgroup|iframe|image|label|legend|line|linearGradient|main|map|marker|mark|mask|math|metadata|menu|meter|mpath|nav|object|optgroup|option|output|path|pattern|picture|polygon|polyline|progress|radialGradient|rect|rp|rt|ruby|script|search|section|set|slot|small|stop|style|select|summary|svg|switch|symbol|template|text|textarea|textPath|time|tspan|use|var|video|view)(?:\s+[^>]*)?>/i', $line, $m) === 1) {
    ...
    private function parseHtmlInlineNode(\DOMNode $node): array
    {
        ...
        if (in_array($name, ['abbr', 'address', 'animate', 'animatemotion', 'animatetransform', 'area', 'audio', 'base', 'bdi', 'bdo', 'button', 'canvas', 'circle', 'cite', 'clippath', 'data', 'datalist', 'defs', 'desc', 'details', 'dfn', 'dialog', 'ellipse', 'embed', 'fedropshadow', 'feflood', 'fegaussianblur', 'feoffset', 'fieldset', 'filter', 'foreignobject', 'form', 'g', 'hgroup', 'iframe', 'image', 'input', 'label', 'legend', 'line', 'lineargradient', 'link', 'map', 'marker', 'mark', 'mask', 'math', 'metadata', 'menu', 'meta', 'meter', 'mpath', 'object', 'optgroup', 'option', 'output', 'param', 'path', 'pattern', 'picture', 'polygon', 'polyline', 'progress', 'radialgradient', 'rect', 'rp', 'rt', 'ruby', 'search', 'select', 'set', 'slot', 'small', 'source', 'stop', 'summary', 'svg', 'switch', 'symbol', 'template', 'text', 'textarea', 'textpath', 'time', 'track', 'tspan', 'use', 'var', 'video', 'view', 'wbr'], true)) {
