...
final class PptxExecutableNativeAstComparisonHarness
{
    ...
    private const CLAIM = 'Compares local PHP PPTX reader output with native output generated by a local pandoc executable using the shared PPTX normalized AST policy; this is optional executable evidence and does not assert upstream Haskell/Tasty runner or writer parity.';
    ...
    public function run(string $pptxDirectory, array $options = []): array
    {
        ...
        return [
            ...
            'astParityStatus' => self::astParityStatus(count($parseFailures), $mismatchCount, $comparedPptxCount),
            ...
            'orderedRemainingGaps' => self::orderedRemainingGaps(true, true, $comparedPptxCount, count($parseFailures), $matchCount, $mismatchCount),
        ];
    }
    ...
    public function formatReport(array $report): string
    {
        ...
        $lines[] = sprintf(
            'pptx: total=%d compared=%d parsedBoth=%d parseFailures=%d',
    ...
    private static function astParityStatus(int $parseFailureCount, int $mismatchCount, int $comparedPptxCount): string
    {
    ...
    private static function orderedRemainingGaps(
        ...
        if (!$directoryPresent) {
            ...
            $sourceEvidence = "compared pptx={$comparedPptxCount}; parse failures={$parseFailureCount}; normalized AST matches={$matchCount}; normalized AST mismatches={$mismatchCount}";
        }
        ...
        return [
            [
                ...
                'status' => (!$directoryPresent || !$executablePresent)
                    ? 'not-evaluated'
                    : (($parseFailureCount === 0 && $mismatchCount === 0 && $comparedPptxCount > 0) ? 'covered-by-current-executable-evidence' : 'open'),
                'currentEvidence' => $sourceEvidence,
                'evidenceRequired' => 'Run local PHP PPTX reader and a pandoc executable against the same PPTX files, keeping parse failures and normalized AST mismatches at zero.',
            ],
            [
                'rank' => 2,
                'id' => 'upstream-haskell-pptx-reader-runner-results',
                'status' => 'open',
                'currentEvidence' => 'This harness invokes a pandoc executable only; it does not run the upstream Haskell/Tasty test target.',
