...
final class CitationCslProcessor
{
    ...
    private static function normalizeItem(array $item, int $index): array
    {
        ...
        return [
            ...
            'pmid' => self::firstStringField($item, ['PMID', 'pmid', 'pmid-id', 'pmid_id', 'pubmed', 'pubmedid', 'pubmed-id', 'pubmed_id']),
            'pmcid' => self::firstStringField($item, ['PMCID', 'pmcid', 'pmcid-id', 'pmcid_id', 'pmc', 'pmc-id', 'pmc_id']),
    ...
    private function renderVariableValue(array $item, string $variable, string $scope, ?AstNode $citation = null): string
    {
        ...
        return match ($normalized) {
            ...
            'isrn' => (string) ($item['isrn'] ?? ''),
            'iswc' => (string) ($item['iswc'] ?? ''),
            'pmid', 'pmid-id', 'pmid_id', 'pubmed', 'pubmedid', 'pubmed-id', 'pubmed_id' => (string) ($item['pmid'] ?? ''),
            'pmcid', 'pmcid-id', 'pmcid_id', 'pmc', 'pmc-id', 'pmc_id' => (string) ($item['pmcid'] ?? ''),
