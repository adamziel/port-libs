...
final class CitationCslProcessor
{
    ...
    private static function normalizeItem(array $item, int $index): array
    {
        ...
        return [
            ...
            'pmid' => self::firstStringField($item, ['PMID', 'pmid', 'PubMed', 'pubmed', 'pubmedId', 'pubmedID', 'pubmedid', 'pubmed-id', 'pmidId', 'pmidID', 'pmidid', 'pmid-id']),
            'pmcid' => self::firstStringField($item, ['PMCID', 'pmcid', 'PMC', 'pmc', 'pmcId', 'pmcID', 'pmcidId', 'pmcidID', 'pmcidid', 'pmc-id', 'pmcid-id']),
    ...
    private function renderVariableValue(array $item, string $variable, string $scope, ?AstNode $citation = null): string
    {
        ...
        return match ($normalized) {
            ...
            'isrn' => (string) ($item['isrn'] ?? ''),
            'iswc' => (string) ($item['iswc'] ?? ''),
            'pmid', 'pubmed', 'pubmedid', 'pubmed-id', 'pmidid', 'pmid-id' => (string) ($item['pmid'] ?? ''),
            'pmcid', 'pmc', 'pmcidid', 'pmcid-id', 'pmc-id' => (string) ($item['pmcid'] ?? ''),
