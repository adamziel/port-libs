...
final class CitationCslProcessor
{
    ...
    private static function normalizeItem(array $item, int $index): array
    {
        ...
        return [
            ...
            'pmid' => self::firstStringField($item, ['PMID', 'pmid', 'pubmed', 'pubmed-id', 'pubmedId', 'pubmedid']),
            'pmcid' => self::firstStringField($item, ['PMCID', 'pmcid', 'PMC', 'pmc', 'pmc-id', 'pmcId', 'pubmedcentral', 'pubmedCentral', 'pubmed-central-id']),
    ...
    private function renderVariableValue(array $item, string $variable, string $scope, ?AstNode $citation = null): string
    {
        ...
        return match ($normalized) {
            ...
            'isrn' => (string) ($item['isrn'] ?? ''),
            'iswc' => (string) ($item['iswc'] ?? ''),
            'pmid', 'pubmed', 'pubmed-id', 'pubmedid' => (string) ($item['pmid'] ?? ''),
            'pmcid', 'pmc', 'pmc-id', 'pubmedcentral', 'pubmed-central-id' => (string) ($item['pmcid'] ?? ''),
