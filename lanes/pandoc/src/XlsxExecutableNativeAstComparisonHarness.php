...
final class XlsxExecutableNativeAstComparisonHarness
{
    private const DEFAULT_MAX_EXAMPLES = 12;
    private const VERDICT = 'pandoc-executable-normalized-ast-comparison-not-full-xlsx-parity';
    private const CLAIM = 'Compares local PHP XLSX reader output, native output generated by a local pandoc executable, and paired upstream .native golden fixtures using the shared XLSX normalized AST policy; this is executable golden-fixture evidence and does not assert that the upstream Haskell/Tasty process itself was run.';
    ...
    public function run(string $xlsxDirectory, array $options = []): array
    {
        ...
        if (!is_dir($xlsxDirectory)) {
            return $this->skippedReport($xlsxDirectory, null, 'xlsx-directory-missing');
        }

        $pandoc = $this->resolvePandocBin($options['pandocBin'] ?? null);
        if ($pandoc === null) {
            return $this->skippedReport($xlsxDirectory, $options['pandocBin'] ?? null, 'pandoc-executable-missing');
        }

        $xlsxFiles = $this->filesByBasename($xlsxDirectory, 'xlsx');
        $nativeFiles = $this->filesByBasename($xlsxDirectory, 'native');
        $fixtureNames = array_keys($xlsxFiles);
        sort($fixtureNames, SORT_STRING);
        $totalXlsxCount = count($fixtureNames);
        if ($limit > 0) {
        ...
        $categoryCounts = [];
        $normalizer = new XlsxNativeAstComparisonHarness();

        foreach ($fixtureNames as $fixtureName) {
            $localResult = $this->readLocalXlsx($xlsxFiles[$fixtureName]);
            if ($localResult['ok']) {
                ++$localParsedCount;
            }

            $pandocResult = $this->readPandocNative($pandoc, $xlsxFiles[$fixtureName]);
            if ($pandocResult['ok']) {
                ++$pandocParsedCount;
            }
            ...
            if ($nativeFixtureResult['ok'] && $pandocResult['ok']) {
                ...
                if ($nativeFixtureAst === $pandocAst) {
                    ...
                    $difference = $this->firstDifference($nativeFixtureAst, $pandocAst) ?? 'unknown-upstream-native-fixture-difference';
                    $categories = $this->mismatchCategories($difference);
                    $this->addCategory($categoryCounts, 'upstream-native-fixture-drift', $fixtureName, $maxExamples);
                    foreach ($categories as $category) {
                        $this->addCategory($categoryCounts, $category, $fixtureName, $maxExamples);
                    }
        ...
        $comparedXlsxCount = count($fixtureNames);
        ...
        return [
            'schemaVersion' => 1,
            'tool' => 'pandoc-xlsx-executable-native-ast',
            'status' => 'completed',
            'skipped' => false,
            'reason' => null,
            'verdict' => self::VERDICT,
            'claim' => self::CLAIM,
            'evidenceKind' => 'xlsx-pandoc-executable-normalized-ast-comparison',
            'xlsxDirectory' => $xlsxDirectory,
            'pandocExecutable' => $pandoc,
            'pandocVersion' => $this->pandocVersion($pandoc),
            'normalizationPolicy' => XlsxNativeAstComparisonHarness::class . '::normalizedDocument',
            'totalXlsxCount' => $totalXlsxCount,
            'comparedXlsxCount' => $comparedXlsxCount,
            ...
            'normalizedAstMatchPercent' => self::percent($matchCount, $comparedXlsxCount),
            ...
            'pandocNativeFixtureMatchPercent' => self::percent($pandocNativeFixtureMatchCount, $comparedXlsxCount),
            'astParityStatus' => self::astParityStatus(count($parseFailures), $mismatchCount, $nativeFixtureMismatchCount, $comparedXlsxCount),
    ...
    {
        $lines = [
            'Pandoc XLSX executable/native AST comparison: ' . (string) ($report['status'] ?? 'unknown'),
            'Verdict: ' . (string) ($report['verdict'] ?? self::VERDICT),
            'Claim: ' . (string) ($report['claim'] ?? self::CLAIM),
            'xlsxDirectory=' . (string) ($report['xlsxDirectory'] ?? ''),
        ...
        $lines[] = sprintf(
            'xlsx: total=%d compared=%d parsedBoth=%d parseFailures=%d nativeFixturesParsed=%d',
            (int) ($report['totalXlsxCount'] ?? 0),
            (int) ($report['comparedXlsxCount'] ?? 0),
        ...
        $lines[] = sprintf(
            'upstreamNativeFixture: pandocCompared=%d pandocMatches=%d (%s) pandocMismatches=%d',
        ...
        if (is_array($nativeFixtureMismatches) && $nativeFixtureMismatches !== []) {
            $lines[] = 'upstreamNativeFixtureMismatchExamples:';
     ...
     * @param array<string, mixed> $report
     */
    public static function hasRequiredExecutableParity(array $report, int $requiredXlsxCount): bool
    {
        if ($requiredXlsxCount < 0) {
            throw new \InvalidArgumentException('Required XLSX count must not be negative');
        }

        return ($report['skipped'] ?? false) === false
            && ($report['status'] ?? null) === 'completed'
            && (int) ($report['totalXlsxCount'] ?? -1) === $requiredXlsxCount
            && (int) ($report['comparedXlsxCount'] ?? -1) === $requiredXlsxCount
            && (int) ($report['localParsedCount'] ?? -1) === $requiredXlsxCount
            && (int) ($report['pandocParsedCount'] ?? -1) === $requiredXlsxCount
            && (int) ($report['nativeFixtureParsedCount'] ?? -1) === $requiredXlsxCount
            && (int) ($report['bothParsedCount'] ?? -1) === $requiredXlsxCount
            && (int) ($report['parseFailureCount'] ?? -1) === 0
            && (int) ($report['normalizedAstMatchCount'] ?? -1) === $requiredXlsxCount
            && (int) ($report['normalizedAstMismatchCount'] ?? -1) === 0
            && (int) ($report['pandocNativeFixtureComparedCount'] ?? -1) === $requiredXlsxCount
            && (int) ($report['pandocNativeFixtureMatchCount'] ?? -1) === $requiredXlsxCount
     ...
     * @return array<string, mixed>
     */
    private function skippedReport(string $xlsxDirectory, ?string $pandocBin, string $reason): array
    {
        $directoryPresent = is_dir($xlsxDirectory);
        $executablePresent = $reason !== 'pandoc-executable-missing';
        $totalXlsxCount = $directoryPresent ? count($this->filesByBasename($xlsxDirectory, 'xlsx')) : 0;
        ...
        return [
            ...
            'astParityStatus' => $reason === 'pandoc-executable-missing'
                ...
                : 'not-evaluated-xlsx-directory-missing',
            'parseFailures' => [],
            'mismatchComparisons' => [],
     ...
     */
    private function readLocalXlsx(string $path): array
    {
        try {
     ...
     * @return array{ok: bool, document: ?AstNode, error: ?string}
     */
    private function readPandocNative(string $pandoc, string $xlsxPath): array
    {
    ...
    private static function astParityStatus(
        ...
        int $comparedXlsxCount
    ): string
    {
        if ($comparedXlsxCount === 0) {
            return 'not-evaluated-no-xlsx-files';
        }
    ...
    private static function orderedRemainingGaps(
        ...
        if (!$directoryPresent) {
            $sourceEvidence = 'XLSX directory absent; executable comparison did not run';
        } elseif (!$executablePresent) {
            $sourceEvidence = 'pandoc executable absent; executable comparison did not run';
        } else {
            $sourceEvidence = "compared xlsx={$comparedXlsxCount}; parse failures={$parseFailureCount}; local/executable normalized AST matches={$matchCount}; local/executable normalized AST mismatches={$mismatchCount}; executable/upstream-native matches={$nativeFixtureMatchCount}; executable/upstream-native mismatches={$nativeFixtureMismatchCount}";
        }
        ...
        $executableEvidenceCovered = $parseFailureCount === 0
            ...
            && $comparedXlsxCount > 0
            && $matchCount === $comparedXlsxCount
            && $nativeFixtureMatchCount === $comparedXlsxCount;

        return [
            [
                'rank' => 1,
                'id' => 'pandoc-executable-xlsx-native-ast-equality',
                ...
                'evidenceRequired' => 'Run local PHP XLSX reader, a pandoc executable, and paired upstream .native fixtures against the same XLSX files, keeping parse failures and normalized AST mismatches at zero.',
            ],
            [
                'rank' => 2,
                'id' => 'upstream-haskell-xlsx-reader-runner-results',
                'status' => 'open',
                'currentEvidence' => 'This harness invokes a pandoc executable and checks the same upstream native fixtures, but it does not run the upstream Haskell/Tasty process itself.',
                'evidenceRequired' => 'Record reproducible upstream Tests.Readers.Xlsx runner results when a Haskell runner is available.',
            ],
        ];
    }
