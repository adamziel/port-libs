...
if (($argv[1] ?? '') === '--self-test') {
    foreach ([
        'class="legacy-doc-field legacy-doc-equation-field legacy-doc-field-eq"',
        'data-legacy-doc-field="eq"',
        'data-legacy-doc-field-instruction="EQ \f(a,b) \* MERGEFORMAT"',
        'data-legacy-doc-equation-field-type="legacy-word-eq"',
        'data-legacy-doc-equation-field-policy="metadata-only-native-review"',
        'data-legacy-doc-equation-field-code="\f(a,b)"',
        'data-legacy-doc-field-format="MERGEFORMAT"',
        '>(a)/(b)</span>',
    ] as $needle) {
        if (!str_contains($blocks, $needle)) {
            throw new RuntimeException('Legacy DOC EQ-field smoke missing expected WordPress handoff: ' . $needle);
        }
    }

    foreach (['EQ', '\f(a,b)', 'MERGEFORMAT'] as $hidden) {
        if (str_contains(strip_tags($blocks), $hidden)) {
            throw new RuntimeException('Legacy DOC EQ-field smoke rendered hidden instruction text: ' . $hidden);
        }
    }

    echo "legacy doc EQ-field handoff self-test ok\n";
    return;
}
