...
if (($argv[1] ?? '') === '--self-test') {
    ...
    if (($result['importReport']['content']['fieldCount'] ?? 0) !== 21) {
        throw new RuntimeException('Expected ODT variable, input, dropdown, conditional, hidden, hidden paragraph, user, user-defined, typed expression, source metadata, page variable, chapter, filename, and statistic fields to be counted in the import report');
    }
