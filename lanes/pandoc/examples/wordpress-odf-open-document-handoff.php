...
if (($argv[1] ?? '') === '--self-test') {
    ...
    if (($result['importReport']['content']['fieldCount'] ?? 0) !== 18) {
        throw new RuntimeException('Expected ODT variable, input, dropdown, conditional, hidden, user, source metadata, page variable, chapter, filename, and statistic fields to be counted in the import report');
    }
    ...
    if (!str_contains($blocks, '<span class="odf-field odf-field-author-name" data-odf-field-type="author-name" data-odf-field-fixed="true">Migration Desk</span>')) {
        throw new RuntimeException('Expected ODT source author field to render in WordPress blocks');
    }
    if (!str_contains($blocks, '<span class="odf-field odf-field-creation-date" data-odf-field-type="creation-date" data-odf-field-date-value="2026-06-05">June 5, 2026</span>')) {
        throw new RuntimeException('Expected ODT source creation-date field to render in WordPress blocks');
    }
