...
Micro-slice: `pandoc-upstream-runner-deps-current-base-20260604T235951Z`.

Accepted base: `e03f7b2344215c65cfd549560b50dbdf696af406`.
...
- This isolated worktree is at accepted base
  `e03f7b2344215c65cfd549560b50dbdf696af406` with no pre-existing dirty lane
  changes before this audit.
- Current Pandoc lane counters are 2,276 upstream test/data/benchmark artifacts
  inspected, 899 mapped native PHP checks, and 431 focused PHP PASS lines with
  0 failures.
...
- The prior current-base dependency audit at
  ...
  raw-source dependency closure for `test-pandoc` and
...
`zip-archive`. The entry point also has command-golden emulation mode, so
upstream command parity requires the built test executable rather than only
...
marshalling support that are not represented by any current native PHP support
component.
...
This audit deliberately avoids the just-accepted DOCX/OpenXML altChunk,
EPUB3, ODT, archive compression, charset/Unicode, syntax-highlighting, and
other native support-library implementation slices. It makes no source changes
and claims no additional native mapping or PHP PASS growth.
...
- `php tools/run-tests.php lanes/pandoc/tests` passed: 17 test files, 4,210
