...
Micro-slice: `pandoc-upstream-runner-deps-current-base-20260603T093016Z`.
...
engine, or online service was executed as progress.
...
- The accepted Pandoc lane state records 2,276 upstream test/data/benchmark
  artifacts inspected, 699 focused Markdown/HTML/WordPress/package/CSL/YAML/
  OPC/doctemplate checks mapped, and 284 focused PHP PASS lines with 0
  failures.
- The current accepted source includes the YAML metadata placement, ZIP-backed
  OPC relationship graph, parameter-free doctemplate pipe, and ZIP integrity
  refinements accepted in the previous Pandoc dashboard publication.
...
from local source truth because the Cabal package/project files are absent
locally. Running Cabal would require hydrating or fetching the broad upstream
checkout and building/downloading the upstream dependency graph, which is out
of scope for this isolated audit.
...
- `php tools/run-tests.php lanes/pandoc/tests` passed for this audit handoff:
  5 test files, 2,641 assertions, 0 failures.
