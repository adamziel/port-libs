...
Micro-slice: `pandoc-upstream-runner-deps-current-base-20260604T070617Z`.
...
- The current accepted source includes native Markdown/HTML reader and writer
  coverage, YAML metadata placement, ZIP package primitives including
  extra-field timestamp handoff, OPC content types and relationship graph
  preflight, doctemplate rendering, bounded CSL citation processing, minimal
  DOCX body/core-property/style/numbering parsing, and bounded math/TeX
  conversion.
...
- The permitted main-repo scope outside `.tmux-team` and `.git` also contains
  no Pandoc Cabal package/project files, and this isolated worktree contains no
  such files.
- No hydrated `/home/claude/port-libs/.upstream-cache/pandoc` checkout is
...
local source truth because the upstream checkout and Cabal package/project files
are absent locally. Running Cabal would require hydrating or fetching the broad
upstream checkout and downloading/building the upstream dependency graph, which
is out of scope for this isolated audit.
