...
implementation after the related CSV/TSV parser, prefix, row-width, and repair
diagnostic slices. CSV/TSV imports preserve C0 control bytes in field text while
reporting bounded review metadata in the `delimitedText` packet.
...
- row-repair annotations connecting sampled controls to relaxed padded,
  unchanged, truncated, or unmapped rows;
...
- `php tools/run-tests.php lanes/pandoc/tests` was attempted and remains
  ...
  8,912 failures. The visible failures are in broad Markdown/raw-extension and
  MarkdownReader expectations, not the focused CSV/TSV reader coverage.
