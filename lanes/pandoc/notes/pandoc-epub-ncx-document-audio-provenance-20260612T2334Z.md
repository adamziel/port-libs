...
The EPUB3 closure check found `PANDOC_STATUS.md` absent in this checkout, so the shipping call uses `progress.md`, `lanes/pandoc/lane-status.json`, `lanes/pandoc/UPSTREAM_TEST_MANIFEST.json`, the accepted static upstream inventory, and local native PHP tests.
...
Expected local results for the final gate: focused EpubReaderTest passes 1 file, 4276 assertions, 0 failures; full `lanes/pandoc/tests` passes 44 files, 73855 assertions, 0 failures.
