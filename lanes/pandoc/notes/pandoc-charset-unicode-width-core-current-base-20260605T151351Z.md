...
Slice: `pandoc-charset-unicode-width-core-current-base-20260605T151351Z`

Base accepted HEAD: `5b7b0019c317588ee2abf0acee527fa17fea0987`

## Behavior Added

- Extended the bounded native East Asian wide table in `UnicodeText` for Kana
  Extended-B codepoints U+1AFF0..U+1AFFF.
- Verified U+1AFF0, U+1AFF5, and U+1AFFD through `displayWidth()`,
  `graphemes()`, `splitByDisplayBreakpoints()`, `splitAtDisplayWidth()`,
  `padDisplay()`, and `wrapByDisplayWidth()`.
- Extended the WordPress charset handoff smoke with a `Kana Extended-B` audit
  row so review tables expose the same two-column accounting used by Markdown
  table layout.
...
one missing East Asian wide block in the existing native PHP width table.
...
Red-first focused check after adding the Kana Extended-B test and before the
implementation:

- `php tools/run-tests.php lanes/pandoc/tests/UnicodeTextTest.php`
  - Result: `1 test files, 327 assertions, 1 failures`
  - Failure: the new Kana Extended-B case expected display width `6`, but
...
handoff planning, line-ending normalization, UTF-8/UTF-16/UTF-32 BOM handling,
...
supplementary/rare East Asian wide scripts and symbols already covered by the
16FE0/1B000/1F200 and Tangut/Khitan cases, BMP/geometric emoji symbols,
...
It only extends the charset/Unicode primitive with bounded Kana Extended-B East
