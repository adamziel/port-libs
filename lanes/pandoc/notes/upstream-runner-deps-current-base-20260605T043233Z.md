...
Micro-slice: `pandoc-upstream-runner-deps-current-base-20260605T043233Z`.
...
- This isolated worktree started clean at accepted base
  `5771f733e9e3256de06e48cb643fff27796d43dd`.
- Current Pandoc lane evidence remains 2,276 upstream test/data/benchmark
  artifacts inspected, 1,095 mapped native PHP checks in
  `UPSTREAM_TEST_MANIFEST.json`, and 621 lane PHP PASS cases with 0 recorded
...
The pinned `test-pandoc-lua-engine` runner remains a Tasty executable. Its
...
pulls local packages `.`, `pandoc-lua-engine`, `pandoc-server`, and
`pandoc-cli`, enables Pandoc data/http flags, and pins Git
source-repository dependencies for `doclayout`, `typst-symbols`, `typst-hs`,
`texmath`, and `citeproc`. A future runner plan therefore must resolve
Hackage packages plus those exact Git pins.
...
- Focused Pandoc lane directory:
  ...
  - `20 test files, 7,002 assertions, 0 failures`
- JSON validation:
