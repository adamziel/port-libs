...
Micro-slice: `pandoc-upstream-runner-deps-current-base-20260604T212918Z`.

Accepted base: `45692a207c18e5a850e689e675ab210e5115185b`.
...
- This isolated worktree is at accepted base
  `45692a207c18e5a850e689e675ab210e5115185b` with no pre-existing dirty Pandoc
  lane changes before this audit.
- This accepted base includes the prior upstream-runner dependency audit at
  `3e0384ae3` and subsequent native PHP support slices through the math/TeX
  source-annotation handoff at `45692a207`.
- The accepted Pandoc lane state records 2,276 upstream test/data/benchmark
  artifacts inspected, 832 focused Markdown/HTML/WordPress/package/CSL/YAML/
  OPC/doctemplate/DOCX/ODF/table-geometry/archive-compression/math/
  legacy-DOC-CFB/charset-Unicode checks mapped, and 375 focused PHP PASS lines
  with 0 failures.
- `/home/claude/port-libs/.upstream-cache` still has no Pandoc upstream
  ...
  `test-pandoc.hs`, or `test-pandoc-lua-engine.hs` there.
- This isolated worktree also has no Pandoc Cabal package, Cabal project, or
  upstream Haskell runner entrypoint files outside `.git` and `.tmux-team`.
...
compiler and Cabal executable are present, but the hydrated Pandoc checkout and
its `cabal.project`, `pandoc.cabal`, and
...
- `php tools/run-tests.php lanes/pandoc/tests` passed: 12 test files, 3,536
