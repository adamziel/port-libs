...
Micro-slice: `pandoc-upstream-runner-deps-current-base-20260604T074139Z`.
...
behavior slice. No Pandoc binary, Cabal build or solver, Haskell test binary,
...
- The current accepted source includes native ZIP/OPC/YAML/doctemplate/CSL,
  minimal DOCX body/core-property/style/numbering parsing, bounded TeX/MathML
  handoff, and shared ZIP package extra-field parsing/preservation.
- Local cache searches found no `pandoc.cabal`,
  `pandoc-lua-engine.cabal`, `cabal.project`, `cabal.project.freeze`,
  `stack.yaml`, or other `*.cabal` files under
  `/home/claude/port-libs/.upstream-cache`.
- A current-worktree search excluding `.git`, `.tmux-team`, and
  `lanes/pandoc` also found no Pandoc Cabal package/project files.
- `/home/claude/port-libs/.upstream-cache` exists, but its visible top-level
  hydrated project caches are for other lanes such as `gitoxide` and
  `libsqlite`; there is no local Pandoc checkout at manifest commit
