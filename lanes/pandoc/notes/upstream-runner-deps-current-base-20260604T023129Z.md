...
Micro-slice: `pandoc-upstream-runner-deps-current-base-20260604T023129Z`.
...
- The accepted Pandoc lane state records 2,276 upstream test/data/benchmark
  artifacts inspected, 737 focused Markdown/HTML/WordPress/package/CSL/YAML/
  OPC/doctemplate/DOCX checks mapped, and 301 focused PHP PASS lines with 0
  failures.
- The current accepted source includes native Markdown/HTML reader and writer
  coverage, YAML metadata placement, CSL handoff, doctemplate support, ZIP/OPC
  package primitives including central/local extra-field validation, minimal
  DOCX package/body/property/style/numbering parsing, and bounded math/TeX
  conversion.
- Local cache searches found no `pandoc.cabal`,
  `pandoc-lua-engine.cabal`, `cabal.project`, `cabal.project.freeze`, or
  `stack.yaml` under `/home/claude/port-libs/.upstream-cache`.
- A repo-local search outside `.git` and `.tmux-team` also found no Pandoc
  Cabal package/project files.
- No hydrated Pandoc upstream checkout was present under the local upstream
  cache search scope.
...
- `php tools/run-tests.php lanes/pandoc/tests` passed: 7 test files, 2,838
