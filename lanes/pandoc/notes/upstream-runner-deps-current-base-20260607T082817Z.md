...
Micro-slice: `pandoc-upstream-runner-deps-current-base-20260607T082817Z`.

Accepted base: `867192b58e262ad4f336e17b1620246d7ff6cad8`.
...
`UpstreamRunnerDependencyAudit` now records effective Cabal `data-files` fields
for the pinned upstream runner and benchmark closure:

- `test:test-pandoc`
- `test:test-pandoc-lua-engine`
- `benchmark:benchmark-pandoc`

The parser merges `data-files` through imported Cabal `common` stanzas and
blocks non-mutating Cabal planning when runner or benchmark stanzas add
unexpected runtime data globs. This closes a static dependency-closure gap
where a hydrated checkout could otherwise look ready while silently expanding
the runner or benchmark fixture/data set before a reviewed Cabal plan exists.
