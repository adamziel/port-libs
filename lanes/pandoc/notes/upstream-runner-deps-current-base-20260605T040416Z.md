...
Micro-slice: `pandoc-upstream-runner-deps-current-base-20260605T040416Z`.

Accepted base: `9bdcc4412b1e3929aacccdfe68ef298910f9c004`.
...
- This isolated worktree started clean at accepted base
  `9bdcc4412b1e3929aacccdfe68ef298910f9c004`.
- Current Pandoc lane evidence remains 2,276 upstream test/data/benchmark
  artifacts inspected and 1,079 mapped native PHP checks in
...
- `/home/claude/port-libs/.upstream-cache/pandoc` and
  ...
  `test-pandoc.hs`, or `test-pandoc-lua-engine.hs`.
...
- The dependency audit at
  ...
  raw-source closure source for `test-pandoc` and
  `test-pandoc-lua-engine` at upstream commit
  `0640c4c9859aa5a3ede082c190fcd5883c24ac83`.

## Verification Repair

The first current-base full lane verification exposed two stale expectations,
both in tests that were lagging accepted native behavior:

- `OdtReaderTest.php` expected the hero image HTML without
  `data-odt-width` / `data-odt-height`, while the current reader preserves ODT
  frame dimensions as safe reviewer metadata and the WordPress writer emits
  those attributes.
- `MarkdownReaderTest.php` expected the structured HTML table footer without
  preserved `id` attributes on the total cells, while the current table writer
  preserves those cell ids for review and anchor integrity.

The patch updates those expectations and adds direct ODT AST assertions for the
dimension metadata. It does not change production behavior, add a mapped
upstream unit, or change the native PHP support-library contract.
...
The test repairs are verification-only and preserve accepted current behavior:
Markdown table section cell ids remain rendered, and ODT frame image dimensions
remain emitted as safe `data-odt-*` reviewer attributes.
...
- Baseline full lane run before test repair:
  `php tools/run-tests.php lanes/pandoc/tests`
  - failed with ODT image-dimension metadata and Markdown table-footer cell id
    expectation mismatches on the accepted base.
- PHP syntax checks:
  - `php -l lanes/pandoc/tests/OdtReaderTest.php`: no syntax errors.
...
- Focused tests after repair:
  ...
  - `php tools/run-tests.php lanes/pandoc/tests/MarkdownReaderTest.php`
    - `1 test files, 2,568 assertions, 0 failures`
- Full focused Pandoc lane directory after repair:
  `php tools/run-tests.php lanes/pandoc/tests`
  - `19 test files, 6,764 assertions, 0 failures`
  - `rg -c '^PASS' /tmp/pandoc-lane-tests-040416.log` reported `600`.
- JSON validation:
