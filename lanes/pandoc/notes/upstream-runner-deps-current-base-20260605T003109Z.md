...
Micro-slice: `pandoc-upstream-runner-deps-current-base-20260605T003109Z`.

Accepted base: `875b50f8ab06c9077ed5d53273541e18ff997d7c`.
...
engine, MathJax, KaTeX, Typst, browser renderer, roff renderer, media player,
online conversion service, or other external converter was executed as
progress.
...
- This isolated worktree is at accepted base
  `875b50f8ab06c9077ed5d53273541e18ff997d7c` with no pre-existing dirty lane
  changes before this audit.
- Current Pandoc lane counters are 2,276 upstream test/data/benchmark artifacts
  inspected, 941 mapped native PHP checks, and 469 focused PHP PASS lines with
  0 failures.
...
`zip-archive`. Command-golden parity also needs the built test executable's
command emulation path, not only static source reads.
...
including the current DOCX/OpenXML, EPUB3, ODT/OpenDocument, archive
compression, charset/Unicode, syntax-highlighting, ZIP/OPC, YAML, CSL/BibTeX,
table-geometry, math/TeX, PDF handoff, and legacy DOC/CFB surfaces. It claims
no additional native mapping or PHP PASS growth.
...
- `php tools/run-tests.php lanes/pandoc/tests` passed: 19 test files, 4,677
