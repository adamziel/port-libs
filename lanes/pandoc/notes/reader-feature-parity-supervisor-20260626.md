...
- XLSX: merged cells, hyperlinks, rich text, date/number formats, cell style attributes including theme/tint color resolution, formula/error metadata, worksheet table/autofilter metadata, and worksheet drawing images have focused tests.
- PPTX: images with geometry/crop metadata, notes, lists, hyperlinks, merged table cells, layout/master/theme path metadata, theme table colors, side-specific table border metadata, and chart data plus legend/axis/grouping/workbook metadata have focused tests.
...
- Still open after this integration:
  ...
  - XLSX now preserves formula/error metadata, worksheet table/autofilter metadata, and theme/tint style colors but still lacks formula evaluation, pivots, charts, slicers, hidden-sheet policy controls, and full style/theme inheritance.
  - PPTX now preserves more image geometry/crop, table border metadata, and chart legend/axis/grouping/workbook metadata but still lacks SmartArt/diagram reconstruction, comments, full z-order/layout inheritance, and rich media extraction.
