...
- `php tools/run-tests.php lanes/pandoc/tests/ZipPackageTest.php`
  - Result: `1 test files, 3162 assertions, 0 failures`.
- `php tools/run-tests.php lanes/pandoc/tests`
  - Result: `44 test files, 63922 assertions, 0 failures`.

## Status Delta

- Added one focused ZIP package PASS case:
  `preflights zip central directory variable field issues before raw strict import`.
- `lane-status.json` `phpPass` moved from `3070` to `3071`.
