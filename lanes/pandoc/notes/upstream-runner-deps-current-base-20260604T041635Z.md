...
Micro-slice: `pandoc-upstream-runner-deps-current-base-20260604T041635Z`.
...
- The accepted Pandoc lane state records 2,276 upstream test/data/benchmark
  ...
  OPC/doctemplate/DOCX/math checks mapped, and 301 focused PHP PASS lines with
  0 failures.
- The current accepted source includes native ZIP/OPC/YAML/doctemplate/CSL
  ...
  shared AST, bounded math/TeX handoff, and shared ZIP package extra-field
...
- A repo-local search outside `.tmux-team` and `.git` also found no Pandoc
  Cabal package/project files.
- The local upstream cache contains hydrated checkouts for other lanes, but no
  Pandoc upstream checkout or Pandoc Cabal package files.
