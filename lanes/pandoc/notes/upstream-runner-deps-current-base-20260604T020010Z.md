...
Micro-slice: `pandoc-upstream-runner-deps-current-base-20260604T020010Z`.
...
- The current accepted source includes native Markdown/HTML reader and writer
  evidence, YAML metadata placement, CSL citation processing, doctemplates,
  ZIP/OPC package primitives with extra-field metadata, OPC relationship graph
  preflight, minimal DOCX/OpenXML parsing, and bounded math/TeX handoff.
- Local cache searches found no `pandoc.cabal`,
  `pandoc-lua-engine.cabal`, `cabal.project`, `cabal.project.freeze`, or
  hydrated Pandoc upstream checkout under
...
- `python3 -m json.tool lanes/pandoc/lane-status.json` passed after edit.
- `python3 -m json.tool lanes/pandoc/UPSTREAM_TEST_MANIFEST.json` passed
  after edit.
- `php tools/run-tests.php lanes/pandoc/tests` passed after edit: 7 test
  files, 2,838 assertions, 0 failures.
- `git diff --check -- lanes/pandoc` passed after edit.
