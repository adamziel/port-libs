...
Micro-slice: `pandoc-upstream-runner-deps-current-base-20260606T142111Z`.

Accepted base: `864fa416793ca581880ed9a5ef60aae3d908fe99`.
...
- This isolated worktree started clean at accepted base
  `864fa416793ca581880ed9a5ef60aae3d908fe99`.
...
- `ghc --numeric-version` returned `9.10.3`; `cabal --numeric-version`
  returned `3.12.1.0`.
- Static source truth for pinned upstream commit
  `0640c4c9859aa5a3ede082c190fcd5883c24ac83` records no runner or benchmark
  `autogen-modules` closure for `test:test-pandoc`,
  `test:test-pandoc-lua-engine`, or `benchmark:benchmark-pandoc`.

## Implemented Audit Tightening

`UpstreamRunnerDependencyAudit` now records effective Cabal `autogen-modules`
for:
...
The expected static closure remains empty for those components. A hydrated
checkout can no longer be marked ready for a non-mutating Cabal plan if the
runner or benchmark stanzas add generated-module dependencies such as
`Paths_pandoc`, `Test.Generated`, `Paths_pandoc_lua_engine`,
`Tests.Lua.Generated`, `Bench.Generated`, or `Paths_benchmark`.

The parser also merges `autogen-modules` through imported Cabal common
stanzas, matching the existing effective-field handling for dependency,
extension, mixin, and option closure.
...
Cabal project/package files, runner entry files, exact source-repository pins,
...
benchmark mixins, no runner or benchmark build-tool dependencies, no
unexpected runner or benchmark default-extensions, no unexpected runner or
benchmark other-extensions, no unexpected runner or benchmark `cpp-options`,
no unexpected runner or benchmark `autogen-modules`, supported `ghc`, and
...
closure, `cpp-options` closure, and `autogen-modules` closure before any Cabal
