...
Micro-slice: `pandoc-upstream-runner-deps-current-base-20260605T113242Z`.

Accepted base: `e8f31a5e61501cd7776038b0eb2cffdcddd721d4`.
...
- This isolated worktree started at accepted base
  `e8f31a5e61501cd7776038b0eb2cffdcddd721d4`.
- Current Pandoc lane evidence records 2,276 upstream test/data/benchmark
  artifacts inspected, 1,325 mapped native PHP checks in
  `UPSTREAM_TEST_MANIFEST.json`, and 867 lane PHP PASS cases with 0 recorded
  failures in `lane-status.json`.
- Searches under `/home/claude/port-libs/.upstream-cache` found no hydrated
  Pandoc checkout directory and no `pandoc.cabal`,
  `pandoc-lua-engine.cabal`, `cabal.project`, `cabal.project.freeze`,
  `test-pandoc.hs`, `test-pandoc-lua-engine.hs`, or `stack.yaml` files.
- A repo-local filename search outside `.git` and `.tmux-team` also found no
  Pandoc Cabal package or project files in this isolated worktree.
...
- The raw-source Cabal closure recorded by
  ...
  lane-local source truth for runner dependency shape at upstream commit
...
`tasty-golden`, `tasty-hunit`, `tasty-quickcheck`, `text`, `temporary`,
`time`, `xml`, and `zip-archive`. Its `--emulate` path calls
`convertWithOpts noEngine`, so command-golden parity requires the compiled
upstream test executable rather than static fixture reads alone.
...
`test:test-pandoc-lua-engine`, including how the `doclayout`,
`typst-symbols`, `typst-hs`, `texmath`, and `citeproc` Git source-repository
pins are resolved. Only after that plan is stable should a separate runner
...
- Focused Pandoc lane directory:
  ...
  - `20 test files, 10904 assertions, 0 failures`
