...
Slice: `pandoc-math-tex-conversion-core-current-base-20260605T080740Z`

Base accepted HEAD: `3bf14b9b638fe9c5e4ab82c6137cc41701a89b21`

## Behavior Added

- Extended `MathTexConverter` with bounded TeX `\middle` delimiter handoff.
- Converts `\middle` inside an active `\left...\right` fence pair to a MathML
  `<mo>` fence with `separator="true"`, preserving the separator intent for
  reviewer and accessibility tooling.
- Reuses the existing bounded delimiter reader for literal delimiters and
  commands such as `\vert`, `\langle`, and `\rangle`.
- Supports invisible `\left.` fences while still allowing a visible
  `\middle` separator before the matching `\right` fence.
- Rejects standalone `\middle`, `\middle` after a closed fence pair, and
  unsupported delimiter commands before exposing MathML.
...
- The accepted explicit-spacing math note left `\middle` fence validation as a
  follow-up after direct fractions, generalized/infix fractions, roots,
  scripts, fences, operators, matrices, cases, arrays, AMS rows, spacing, and
  explicit spacing dimensions. This slice owns only bounded `\middle`
  separator-fence handoff.
...
- Red check after adding focused coverage:
  `php tools/run-tests.php lanes/pandoc/tests/MathTexConverterTest.php`
  - Result: `1 test files, 217 assertions, 2 failures`.
  - Failure reason: `\middle` emitted literal `<mi>\middle</mi>` plus a plain
    delimiter, and malformed standalone `\middle` did not throw.
...
- Focused lane directory:
  ...
  - Result: `20 test files, 8953 assertions, 0 failures`.
...
- `phpPass`: `764` -> `765` by one newly passing focused math test case.
- `benchmarkDenominator.mapped`: `1223` -> `1224`.
...
- Does not repeat accepted Math/TeX direct fractions, style-aware fractions,
  ...
  conversion, `\substack`, AMS align/gather/split environment conversion,
  named spacing command conversion, or explicit `\hspace`/`\mspace` dimension
  parsing.
...
Keep `alignedat`, equation numbering/labels, optional macro arguments, Unicode
mathematical alphanumeric rewriting, MathML intent/accessibility annotations,
