...
Micro-slice: `pandoc-upstream-runner-deps-current-base-20260604T085326Z`.

Accepted base: `cb6cfe4542e0ad28600b36fba81b57c0842797c7`.
...
- The accepted Pandoc lane state records 2,276 upstream test/data/benchmark
  artifacts inspected, 746 focused Markdown/HTML/WordPress/package/CSL/YAML/
  OPC/doctemplate/DOCX/ODF/table-geometry checks mapped, and 306 focused PHP
  PASS lines with 0 failures.
- The current accepted source includes native Markdown/HTML readers,
  Markdown/LaTeX/WordPress writers, ZIP/OPC package primitives, doctemplates,
  YAML metadata, CSL citation handoff, minimal DOCX/OpenXML parsing, ODT
  package/content/style/meta parsing, table geometry, and bounded math/TeX
  conversion support.
...
- A worktree-local search found no Pandoc Cabal package/project files.
- A repo-local search outside `.git`, tmux worktrees, and tmux temporary
  handoff state also found no Pandoc Cabal package/project files.
...
upstream checkout and building/downloading the upstream dependency graph, which
...
- `php tools/run-tests.php lanes/pandoc/tests` passed: 8 test files, 2,897
