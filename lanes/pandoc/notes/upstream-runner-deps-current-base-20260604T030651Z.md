...
Micro-slice: `pandoc-upstream-runner-deps-current-base-20260604T030651Z`.
...
- The accepted Pandoc lane state records 2,276 upstream test/data/benchmark
  ...
  failures.
- The latest accepted Pandoc implementation source on this base is
  `91f83abc283610d07e211d3dd7d1eac536532d41`, adding bounded shared ZIP
  package extra-field support without external `zip`/`unzip` execution.
- Local cache inspection found no hydrated Pandoc upstream checkout under
  `/home/claude/port-libs/.upstream-cache`.
- Repo-local searches outside `.git` and `.tmux-team` found no `pandoc.cabal`,
