...
Micro-slice: `pandoc-upstream-runner-deps-current-base-20260606T230505Z`.

Accepted base: `ca789a5b6d3e33e7c4378e92f189c08d2e32e040`.
...
`UpstreamRunnerDependencyAudit` now records effective Cabal runtime option
fields for the pinned upstream runner and benchmark closure:

- `test:test-pandoc` `test-options`
- `test:test-pandoc-lua-engine` `test-options`
- `benchmark:benchmark-pandoc` `benchmark-options`
...
unexpected runtime options. This closes a static dependency-closure gap where a
hydrated checkout could otherwise look ready while silently filtering Tasty
tests, changing runner concurrency, or redirecting benchmark output before a
...
`autogen-modules` closure, `reexported-modules` closure, native/system
...
- Red-first:
  `php tools/run-tests.php lanes/pandoc/tests/UpstreamRunnerDependencyAuditTest.php`
  - `1 test files, 820 assertions, 1 failures`
  - Failure: a fixture with runner `test-options` and benchmark
    `benchmark-options` was still marked ready for non-mutating Cabal
    planning.
- Final focused:
  `php tools/run-tests.php lanes/pandoc/tests/UpstreamRunnerDependencyAuditTest.php`
