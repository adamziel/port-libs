...
Micro-slice: `pandoc-upstream-runner-deps-current-base-20260604T004524Z`.
...
- The accepted Pandoc lane state records 2,276 upstream test/data/benchmark
  ...
  failures.
- The latest accepted Pandoc implementation evidence is the shared ZIP package
  extra-field slice at lane commit `91f83abc283610d07e211d3dd7d1eac536532d41`.
- Local worktree searches found no `pandoc.cabal`,
  `pandoc-lua-engine.cabal`, `cabal.project`, `cabal.project.freeze`, or
  `stack.yaml`.
- Searches under `/home/claude/port-libs/.upstream-cache` found no Pandoc
  upstream checkout and no Pandoc Cabal package/project files. The available
  cache directories are for other lanes such as gitoxide, libsqlite, and
  lightningcss.
