...
return [
    ...
    'reports checked-in current upstream pptx static evidence gate' => static function (TestRunner $t): void {
        ...
        $t->same(68, $static['checkedInFixturePairCount']);
        ...
        $t->same(68, $nativeParity['requiredPairCount']);
        $t->same(68, $nativeParity['totalPairCount']);
        $t->same(68, $nativeParity['comparedPairCount']);
        $t->same(68, $nativeParity['pptxParsedCount']);
        $t->same(68, $nativeParity['nativeParsedCount']);
        $t->same(68, $nativeParity['bothParsedCount']);
        $t->same(0, $nativeParity['parseFailureCount']);
        $t->same(68, $nativeParity['normalizedAstMatchCount']);
        $t->same(0, $nativeParity['normalizedAstMismatchCount']);
        $t->same(68, count($nativeParity['fixtureComparisons']));
        ...
        $t->same(68, $executableParity['requiredPptxCount']);
        ...
        $t->same('cf22226ef24968c3b2562133c82fb4505afa49741e1452a1c2c49beccca4b84b', $executableParity['snapshotFile']['sha256']);
        $t->same(33666, $executableParity['snapshotFile']['bytes']);
        ...
        $t->same(68, $executableParity['totalPptxCount']);
        $t->same(68, $executableParity['comparedPptxCount']);
        ...
        $t->true(in_array('local PHP PPTX reader output matches all 68 checked-in current PPTX/native pairs by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        $t->true(in_array('checked-in executable native AST evidence shows pandoc 3.10, local PHP output, and paired .native fixtures match all 68 checked-in current PPTX fixtures by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        ...
        $t->contains('Static current evidence: valid-checked-in-current-pptx-reader-evidence comparisons=1 checkedInPairs=68', $text);
        $t->contains('Static native AST mapped parity: normalized-ast-equality-observed-not-runner-parity matches=68 mismatches=0 required=68', $text);
        $t->contains('Static executable native AST parity: normalized-ast-equality-observed-against-pandoc-executable matches=68 mismatches=0 required=68', $text);
        $t->contains('Runner status: not-run', $text);
        $t->contains('Runner plan: planned-not-run', $text);
    },
...
HS);
    ...
    'cli gates checked-in current pptx static evidence without upstream runner claim' => static function (TestRunner $t) use ($makeTempDir, $removeTree): void {
        ...
        $t->same(68, $summary['staticCurrentEvidence']['checkedInFixturePairCount']);
        $t->same(68, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(68, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(68, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['pandocNativeFixtureMatchCount']);
