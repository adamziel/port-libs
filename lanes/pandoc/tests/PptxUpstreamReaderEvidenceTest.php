...
return [
    ...
    'reports checked-in current upstream pptx static evidence gate' => static function (TestRunner $t): void {
        ...
        $t->same(87, $static['checkedInFixturePairCount']);
        ...
        $t->same(87, $nativeParity['requiredPairCount']);
        $t->same(87, $nativeParity['totalPairCount']);
        $t->same(87, $nativeParity['comparedPairCount']);
        $t->same(87, $nativeParity['pptxParsedCount']);
        $t->same(87, $nativeParity['nativeParsedCount']);
        $t->same(87, $nativeParity['bothParsedCount']);
        $t->same(0, $nativeParity['parseFailureCount']);
        $t->same(87, $nativeParity['normalizedAstMatchCount']);
        $t->same(0, $nativeParity['normalizedAstMismatchCount']);
        $t->same(87, count($nativeParity['fixtureComparisons']));
        ...
        $t->same(87, $executableParity['requiredPptxCount']);
        ...
        $t->same('e49193a113ccc3763d1ea2b91bad4fce6fd34324391d52e6bb70ed9de66d11e2', $executableParity['snapshotFile']['sha256']);
        $t->same(37839, $executableParity['snapshotFile']['bytes']);
        ...
        $t->same(87, $executableParity['totalPptxCount']);
        $t->same(87, $executableParity['comparedPptxCount']);
        $t->same(87, $executableParity['localParsedCount']);
        $t->same(87, $executableParity['pandocParsedCount']);
        $t->same(87, $executableParity['nativeFixtureParsedCount']);
        $t->same(87, $executableParity['bothParsedCount']);
        $t->same(0, $executableParity['parseFailureCount']);
        $t->same(87, $executableParity['normalizedAstMatchCount']);
        $t->same(0, $executableParity['normalizedAstMismatchCount']);
        $t->same(87, $executableParity['pandocNativeFixtureComparedCount']);
        $t->same(87, $executableParity['pandocNativeFixtureMatchCount']);
        ...
        $t->true(in_array('local PHP PPTX reader output matches all 87 checked-in current PPTX/native pairs by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        $t->true(in_array('checked-in executable native AST evidence shows pandoc 3.10, local PHP output, and paired .native fixtures match all 87 checked-in current PPTX fixtures by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        ...
        $t->contains('Static current evidence: valid-checked-in-current-pptx-reader-evidence comparisons=1 checkedInPairs=87', $text);
        $t->contains('Static native AST mapped parity: normalized-ast-equality-observed-not-runner-parity matches=87 mismatches=0 required=87', $text);
        $t->contains('Static executable native AST parity: normalized-ast-equality-observed-against-pandoc-executable matches=87 mismatches=0 required=87', $text);
        $t->contains('Runner status: not-run', $text);
        $t->contains('Runner plan: planned-not-run', $text);
    },
...
HS);
    ...
    'cli gates checked-in current pptx static evidence without upstream runner claim' => static function (TestRunner $t) use ($makeTempDir, $removeTree): void {
        ...
        $t->same(87, $summary['staticCurrentEvidence']['checkedInFixturePairCount']);
        $t->same(87, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(87, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(87, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['pandocNativeFixtureMatchCount']);
