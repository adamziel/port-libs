...
return [
    ...
    'reports checked-in current upstream pptx static evidence gate' => static function (TestRunner $t): void {
        ...
        $t->same(60, $static['checkedInFixturePairCount']);
        ...
        $t->same(60, $nativeParity['requiredPairCount']);
        $t->same(60, $nativeParity['totalPairCount']);
        $t->same(60, $nativeParity['comparedPairCount']);
        $t->same(60, $nativeParity['pptxParsedCount']);
        $t->same(60, $nativeParity['nativeParsedCount']);
        $t->same(60, $nativeParity['bothParsedCount']);
        $t->same(0, $nativeParity['parseFailureCount']);
        $t->same(60, $nativeParity['normalizedAstMatchCount']);
        $t->same(0, $nativeParity['normalizedAstMismatchCount']);
        $t->same(60, count($nativeParity['fixtureComparisons']));
        ...
        $t->same(60, $executableParity['requiredPptxCount']);
        ...
        $t->same('6471197baf72a766c7baf0e9e0c60c3ac5b68377b8591cb8fd5f8bb29676b7e9', $executableParity['snapshotFile']['sha256']);
        $t->same(29912, $executableParity['snapshotFile']['bytes']);
        ...
        $t->same(60, $executableParity['totalPptxCount']);
        $t->same(60, $executableParity['comparedPptxCount']);
        ...
        $t->true(in_array('local PHP PPTX reader output matches all 60 checked-in current PPTX/native pairs by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        $t->true(in_array('checked-in executable native AST evidence shows pandoc 3.10, local PHP output, and paired .native fixtures match all 60 checked-in current PPTX fixtures by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        ...
        $t->contains('Static current evidence: valid-checked-in-current-pptx-reader-evidence comparisons=1 checkedInPairs=60', $text);
        $t->contains('Static native AST mapped parity: normalized-ast-equality-observed-not-runner-parity matches=60 mismatches=0 required=60', $text);
        $t->contains('Static executable native AST parity: normalized-ast-equality-observed-against-pandoc-executable matches=60 mismatches=0 required=60', $text);
        $t->contains('Runner status: not-run', $text);
        $t->contains('Runner plan: planned-not-run', $text);
    },
...
HS);
    ...
    'cli gates checked-in current pptx static evidence without upstream runner claim' => static function (TestRunner $t) use ($makeTempDir, $removeTree): void {
        ...
        $t->same(60, $summary['staticCurrentEvidence']['checkedInFixturePairCount']);
        $t->same(60, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(60, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(60, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['pandocNativeFixtureMatchCount']);
