...
return [
    ...
    'reports checked-in current upstream pptx static evidence gate' => static function (TestRunner $t): void {
        ...
        $t->same(93, $static['checkedInFixturePairCount']);
        ...
        $t->same(93, $nativeParity['requiredPairCount']);
        $t->same(93, $nativeParity['totalPairCount']);
        $t->same(93, $nativeParity['comparedPairCount']);
        $t->same(93, $nativeParity['pptxParsedCount']);
        $t->same(93, $nativeParity['nativeParsedCount']);
        $t->same(93, $nativeParity['bothParsedCount']);
        $t->same(0, $nativeParity['parseFailureCount']);
        $t->same(93, $nativeParity['normalizedAstMatchCount']);
        $t->same(0, $nativeParity['normalizedAstMismatchCount']);
        $t->same(93, count($nativeParity['fixtureComparisons']));
        ...
        $t->same(93, $executableParity['requiredPptxCount']);
        ...
        $t->same('ed3d2f8a4c149776d41715fcac68fc60007f7c0c8255f099fa2852e13fca6c09', $executableParity['snapshotFile']['sha256']);
        $t->same(40421, $executableParity['snapshotFile']['bytes']);
        ...
        $t->same(93, $executableParity['totalPptxCount']);
        $t->same(93, $executableParity['comparedPptxCount']);
        $t->same(93, $executableParity['localParsedCount']);
        $t->same(93, $executableParity['pandocParsedCount']);
        $t->same(93, $executableParity['nativeFixtureParsedCount']);
        $t->same(93, $executableParity['bothParsedCount']);
        $t->same(0, $executableParity['parseFailureCount']);
        $t->same(93, $executableParity['normalizedAstMatchCount']);
        $t->same(0, $executableParity['normalizedAstMismatchCount']);
        $t->same(93, $executableParity['pandocNativeFixtureComparedCount']);
        $t->same(93, $executableParity['pandocNativeFixtureMatchCount']);
        ...
        $t->true(in_array('local PHP PPTX reader output matches all 93 checked-in current PPTX/native pairs by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        $t->true(in_array('checked-in executable native AST evidence shows pandoc 3.10, local PHP output, and paired .native fixtures match all 93 checked-in current PPTX fixtures by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        ...
        $t->contains('Static current evidence: valid-checked-in-current-pptx-reader-evidence comparisons=1 checkedInPairs=93', $text);
        $t->contains('Static native AST mapped parity: normalized-ast-equality-observed-not-runner-parity matches=93 mismatches=0 required=93', $text);
        $t->contains('Static executable native AST parity: normalized-ast-equality-observed-against-pandoc-executable matches=93 mismatches=0 required=93', $text);
        $t->contains('Runner status: not-run', $text);
        $t->contains('Runner plan: planned-not-run', $text);
    },
...
HS);
    ...
    'cli gates checked-in current pptx static evidence without upstream runner claim' => static function (TestRunner $t) use ($makeTempDir, $removeTree): void {
        ...
        $t->same(93, $summary['staticCurrentEvidence']['checkedInFixturePairCount']);
        $t->same(93, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(93, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(93, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['pandocNativeFixtureMatchCount']);
    ...
    'workflow gates checked-in pptx native and executable parity corpora' => static function (TestRunner $t): void {
        ...
        $t->contains('--require-executable-parity=93', $workflow);
