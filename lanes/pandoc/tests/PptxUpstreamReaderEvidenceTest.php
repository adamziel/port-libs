...
return [
    ...
    'reports checked-in current upstream pptx static evidence gate' => static function (TestRunner $t): void {
        ...
        $t->same(88, $static['checkedInFixturePairCount']);
        ...
        $t->same(88, $nativeParity['requiredPairCount']);
        $t->same(88, $nativeParity['totalPairCount']);
        $t->same(88, $nativeParity['comparedPairCount']);
        $t->same(88, $nativeParity['pptxParsedCount']);
        $t->same(88, $nativeParity['nativeParsedCount']);
        $t->same(88, $nativeParity['bothParsedCount']);
        $t->same(0, $nativeParity['parseFailureCount']);
        $t->same(88, $nativeParity['normalizedAstMatchCount']);
        $t->same(0, $nativeParity['normalizedAstMismatchCount']);
        $t->same(88, count($nativeParity['fixtureComparisons']));
        ...
        $t->same(88, $executableParity['requiredPptxCount']);
        ...
        $t->same('0904fb739fafb5a402b0a3ba1062a4ad572469b8f2414a483f8474c029ec6a2c', $executableParity['snapshotFile']['sha256']);
        $t->same(38294, $executableParity['snapshotFile']['bytes']);
        ...
        $t->same(88, $executableParity['totalPptxCount']);
        $t->same(88, $executableParity['comparedPptxCount']);
        $t->same(88, $executableParity['localParsedCount']);
        $t->same(88, $executableParity['pandocParsedCount']);
        $t->same(88, $executableParity['nativeFixtureParsedCount']);
        $t->same(88, $executableParity['bothParsedCount']);
        $t->same(0, $executableParity['parseFailureCount']);
        $t->same(88, $executableParity['normalizedAstMatchCount']);
        $t->same(0, $executableParity['normalizedAstMismatchCount']);
        $t->same(88, $executableParity['pandocNativeFixtureComparedCount']);
        $t->same(88, $executableParity['pandocNativeFixtureMatchCount']);
        ...
        $t->true(in_array('local PHP PPTX reader output matches all 88 checked-in current PPTX/native pairs by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        $t->true(in_array('checked-in executable native AST evidence shows pandoc 3.10, local PHP output, and paired .native fixtures match all 88 checked-in current PPTX fixtures by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        ...
        $t->contains('Static current evidence: valid-checked-in-current-pptx-reader-evidence comparisons=1 checkedInPairs=88', $text);
        $t->contains('Static native AST mapped parity: normalized-ast-equality-observed-not-runner-parity matches=88 mismatches=0 required=88', $text);
        $t->contains('Static executable native AST parity: normalized-ast-equality-observed-against-pandoc-executable matches=88 mismatches=0 required=88', $text);
        $t->contains('Runner status: not-run', $text);
        $t->contains('Runner plan: planned-not-run', $text);
    },
...
HS);
    ...
    'cli gates checked-in current pptx static evidence without upstream runner claim' => static function (TestRunner $t) use ($makeTempDir, $removeTree): void {
        ...
        $t->same(88, $summary['staticCurrentEvidence']['checkedInFixturePairCount']);
        $t->same(88, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(88, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(88, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['pandocNativeFixtureMatchCount']);
