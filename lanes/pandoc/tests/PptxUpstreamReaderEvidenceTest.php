...
return [
    ...
    'reports checked-in current upstream pptx static evidence gate' => static function (TestRunner $t): void {
        ...
        $t->same(52, $static['checkedInFixturePairCount']);
        ...
        $t->same(52, $nativeParity['requiredPairCount']);
        $t->same(52, $nativeParity['totalPairCount']);
        $t->same(52, $nativeParity['comparedPairCount']);
        $t->same(52, $nativeParity['pptxParsedCount']);
        $t->same(52, $nativeParity['nativeParsedCount']);
        $t->same(52, $nativeParity['bothParsedCount']);
        $t->same(0, $nativeParity['parseFailureCount']);
        $t->same(52, $nativeParity['normalizedAstMatchCount']);
        $t->same(0, $nativeParity['normalizedAstMismatchCount']);
        $t->same(52, count($nativeParity['fixtureComparisons']));
        ...
        $t->same(52, $executableParity['requiredPptxCount']);
        ...
        $t->same('6e812c3d34e08bf9ed1d0203d6884124a7af2f5137704449895dc3dfa9946a67', $executableParity['snapshotFile']['sha256']);
        $t->same(26227, $executableParity['snapshotFile']['bytes']);
        ...
        $t->same(52, $executableParity['totalPptxCount']);
        $t->same(52, $executableParity['comparedPptxCount']);
        ...
        $t->true(in_array('local PHP PPTX reader output matches all 52 checked-in current PPTX/native pairs by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        $t->true(in_array('checked-in executable native AST evidence shows pandoc 3.10, local PHP output, and paired .native fixtures match all 52 checked-in current PPTX fixtures by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        ...
        $t->contains('Static current evidence: valid-checked-in-current-pptx-reader-evidence comparisons=1 checkedInPairs=52', $text);
        $t->contains('Static native AST mapped parity: normalized-ast-equality-observed-not-runner-parity matches=52 mismatches=0 required=52', $text);
        $t->contains('Static executable native AST parity: normalized-ast-equality-observed-against-pandoc-executable matches=52 mismatches=0 required=52', $text);
        $t->contains('Runner status: not-run', $text);
        $t->contains('Runner plan: planned-not-run', $text);
    },
...
HS);
    ...
    'cli gates checked-in current pptx static evidence without upstream runner claim' => static function (TestRunner $t) use ($makeTempDir, $removeTree): void {
        ...
        $t->same(52, $summary['staticCurrentEvidence']['checkedInFixturePairCount']);
        $t->same(52, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(52, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(52, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['pandocNativeFixtureMatchCount']);
