...
return [
    ...
    'reports checked-in current upstream pptx static evidence gate' => static function (TestRunner $t): void {
        ...
        $t->same(49, $static['checkedInFixturePairCount']);
        ...
        $t->same(49, $nativeParity['requiredPairCount']);
        $t->same(49, $nativeParity['totalPairCount']);
        $t->same(49, $nativeParity['comparedPairCount']);
        $t->same(49, $nativeParity['pptxParsedCount']);
        $t->same(49, $nativeParity['nativeParsedCount']);
        $t->same(49, $nativeParity['bothParsedCount']);
        ...
        $t->same(49, $executableParity['requiredPptxCount']);
        ...
        $t->same('0ab3f8c0a957e85007ddd70b32ebb1870d435c657713db473f1c592a9a87c952', $executableParity['snapshotFile']['sha256']);
        $t->same(10107, $executableParity['snapshotFile']['bytes']);
        ...
        $t->same(49, $executableParity['totalPptxCount']);
        $t->same(49, $executableParity['comparedPptxCount']);
        ...
        $t->true(in_array('local PHP PPTX reader output matches all 49 checked-in current PPTX/native pairs by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        $t->true(in_array('checked-in executable native AST evidence shows pandoc 3.10, local PHP output, and paired .native fixtures match all 49 checked-in current PPTX fixtures by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        ...
        $t->contains('Static current evidence: valid-checked-in-current-pptx-reader-evidence comparisons=1 checkedInPairs=49', $text);
        $t->contains('Static native AST mapped parity: normalized-ast-equality-observed-not-runner-parity matches=49 mismatches=0 required=49', $text);
        $t->contains('Static executable native AST parity: normalized-ast-equality-observed-against-pandoc-executable matches=49 mismatches=0 required=49', $text);
        $t->contains('Runner status: not-run', $text);
        $t->contains('Runner plan: planned-not-run', $text);
    },
...
HS);
    ...
    'cli gates checked-in current pptx static evidence without upstream runner claim' => static function (TestRunner $t) use ($makeTempDir, $removeTree): void {
        ...
        $t->same(49, $summary['staticCurrentEvidence']['checkedInFixturePairCount']);
        $t->same(49, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(49, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(49, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['pandocNativeFixtureMatchCount']);
