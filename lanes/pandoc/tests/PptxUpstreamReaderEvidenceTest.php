...
return [
    ...
    'reports checked-in current upstream pptx static evidence gate' => static function (TestRunner $t): void {
        ...
        $t->same(48, $static['checkedInFixturePairCount']);
        ...
        $t->same(48, $nativeParity['requiredPairCount']);
        $t->same(48, $nativeParity['totalPairCount']);
        $t->same(48, $nativeParity['comparedPairCount']);
        $t->same(48, $nativeParity['pptxParsedCount']);
        $t->same(48, $nativeParity['nativeParsedCount']);
        $t->same(48, $nativeParity['bothParsedCount']);
        $t->same(0, $nativeParity['parseFailureCount']);
        $t->same(48, $nativeParity['normalizedAstMatchCount']);
        ...
        $t->same(48, $executableParity['requiredPptxCount']);
        ...
        $t->same('17f3e106864b138fbdba72ae5374f6ce0c90368da206b3afcb6e45b428604a27', $executableParity['snapshotFile']['sha256']);
        $t->same(9914, $executableParity['snapshotFile']['bytes']);
        ...
        $t->same(48, $executableParity['totalPptxCount']);
        $t->same(48, $executableParity['comparedPptxCount']);
        ...
        $t->true(in_array('local PHP PPTX reader output matches all 48 checked-in current PPTX/native pairs by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        $t->true(in_array('checked-in executable native AST evidence shows pandoc 3.10, local PHP output, and paired .native fixtures match all 48 checked-in current PPTX fixtures by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        ...
        $t->contains('Static current evidence: valid-checked-in-current-pptx-reader-evidence comparisons=1 checkedInPairs=48', $text);
        $t->contains('Static native AST mapped parity: normalized-ast-equality-observed-not-runner-parity matches=48 mismatches=0 required=48', $text);
        $t->contains('Static executable native AST parity: normalized-ast-equality-observed-against-pandoc-executable matches=48 mismatches=0 required=48', $text);
        $t->contains('Runner status: not-run', $text);
        $t->contains('Runner plan: planned-not-run', $text);
    },
...
HS);
    ...
    'cli gates checked-in current pptx static evidence without upstream runner claim' => static function (TestRunner $t) use ($makeTempDir, $removeTree): void {
        ...
        $t->same(48, $summary['staticCurrentEvidence']['checkedInFixturePairCount']);
        $t->same(48, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(48, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(48, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['pandocNativeFixtureMatchCount']);
