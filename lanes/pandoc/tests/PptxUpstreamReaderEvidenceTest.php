...
return [
    ...
    'reports checked-in current upstream pptx static evidence gate' => static function (TestRunner $t): void {
        ...
        $t->same(92, $static['checkedInFixturePairCount']);
        ...
        $t->same(92, $nativeParity['requiredPairCount']);
        $t->same(92, $nativeParity['totalPairCount']);
        $t->same(92, $nativeParity['comparedPairCount']);
        $t->same(92, $nativeParity['pptxParsedCount']);
        $t->same(92, $nativeParity['nativeParsedCount']);
        $t->same(92, $nativeParity['bothParsedCount']);
        $t->same(0, $nativeParity['parseFailureCount']);
        $t->same(92, $nativeParity['normalizedAstMatchCount']);
        $t->same(0, $nativeParity['normalizedAstMismatchCount']);
        $t->same(92, count($nativeParity['fixtureComparisons']));
        ...
        $t->same(92, $executableParity['requiredPptxCount']);
        ...
        $t->same('21a5349b4aa6149eed8dd269aee7ff951e0ac7dad7ae729b913069cadff9c955', $executableParity['snapshotFile']['sha256']);
        $t->same(39974, $executableParity['snapshotFile']['bytes']);
        ...
        $t->same(92, $executableParity['totalPptxCount']);
        $t->same(92, $executableParity['comparedPptxCount']);
        $t->same(92, $executableParity['localParsedCount']);
        $t->same(92, $executableParity['pandocParsedCount']);
        $t->same(92, $executableParity['nativeFixtureParsedCount']);
        $t->same(92, $executableParity['bothParsedCount']);
        $t->same(0, $executableParity['parseFailureCount']);
        $t->same(92, $executableParity['normalizedAstMatchCount']);
        $t->same(0, $executableParity['normalizedAstMismatchCount']);
        $t->same(92, $executableParity['pandocNativeFixtureComparedCount']);
        $t->same(92, $executableParity['pandocNativeFixtureMatchCount']);
        ...
        $t->true(in_array('local PHP PPTX reader output matches all 92 checked-in current PPTX/native pairs by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        $t->true(in_array('checked-in executable native AST evidence shows pandoc 3.10, local PHP output, and paired .native fixtures match all 92 checked-in current PPTX fixtures by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        ...
        $t->contains('Static current evidence: valid-checked-in-current-pptx-reader-evidence comparisons=1 checkedInPairs=92', $text);
        $t->contains('Static native AST mapped parity: normalized-ast-equality-observed-not-runner-parity matches=92 mismatches=0 required=92', $text);
        $t->contains('Static executable native AST parity: normalized-ast-equality-observed-against-pandoc-executable matches=92 mismatches=0 required=92', $text);
        $t->contains('Runner status: not-run', $text);
        $t->contains('Runner plan: planned-not-run', $text);
    },
...
HS);
    ...
    'cli gates checked-in current pptx static evidence without upstream runner claim' => static function (TestRunner $t) use ($makeTempDir, $removeTree): void {
        ...
        $t->same(92, $summary['staticCurrentEvidence']['checkedInFixturePairCount']);
        $t->same(92, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(92, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(92, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['pandocNativeFixtureMatchCount']);
    ...
    'workflow gates checked-in pptx native and executable parity corpora' => static function (TestRunner $t): void {
        ...
        $t->contains('--require-executable-parity=92', $workflow);
