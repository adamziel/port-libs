...
return [
    ...
    'reports checked-in current upstream pptx static evidence gate' => static function (TestRunner $t): void {
        ...
        $t->same(66, $static['checkedInFixturePairCount']);
        ...
        $t->same(66, $nativeParity['requiredPairCount']);
        $t->same(66, $nativeParity['totalPairCount']);
        $t->same(66, $nativeParity['comparedPairCount']);
        $t->same(66, $nativeParity['pptxParsedCount']);
        $t->same(66, $nativeParity['nativeParsedCount']);
        $t->same(66, $nativeParity['bothParsedCount']);
        $t->same(0, $nativeParity['parseFailureCount']);
        $t->same(66, $nativeParity['normalizedAstMatchCount']);
        $t->same(0, $nativeParity['normalizedAstMismatchCount']);
        $t->same(66, count($nativeParity['fixtureComparisons']));
        ...
        $t->same(66, $executableParity['requiredPptxCount']);
        ...
        $t->same('acbdca5b007404744c5f3d7b1f896f3cb04383060aaaeb084529f566d1fd16c6', $executableParity['snapshotFile']['sha256']);
        $t->same(32762, $executableParity['snapshotFile']['bytes']);
        ...
        $t->same(66, $executableParity['totalPptxCount']);
        $t->same(66, $executableParity['comparedPptxCount']);
        ...
        $t->true(in_array('local PHP PPTX reader output matches all 66 checked-in current PPTX/native pairs by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        $t->true(in_array('checked-in executable native AST evidence shows pandoc 3.10, local PHP output, and paired .native fixtures match all 66 checked-in current PPTX fixtures by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        ...
        $t->contains('Static current evidence: valid-checked-in-current-pptx-reader-evidence comparisons=1 checkedInPairs=66', $text);
        $t->contains('Static native AST mapped parity: normalized-ast-equality-observed-not-runner-parity matches=66 mismatches=0 required=66', $text);
        $t->contains('Static executable native AST parity: normalized-ast-equality-observed-against-pandoc-executable matches=66 mismatches=0 required=66', $text);
        $t->contains('Runner status: not-run', $text);
        $t->contains('Runner plan: planned-not-run', $text);
    },
...
HS);
    ...
    'cli gates checked-in current pptx static evidence without upstream runner claim' => static function (TestRunner $t) use ($makeTempDir, $removeTree): void {
        ...
        $t->same(66, $summary['staticCurrentEvidence']['checkedInFixturePairCount']);
        $t->same(66, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(66, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(66, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['pandocNativeFixtureMatchCount']);
