...
return [
    ...
    'reports checked-in current upstream pptx static evidence gate' => static function (TestRunner $t): void {
        ...
        $t->same(91, $static['checkedInFixturePairCount']);
        ...
        $t->same(91, $nativeParity['requiredPairCount']);
        $t->same(91, $nativeParity['totalPairCount']);
        $t->same(91, $nativeParity['comparedPairCount']);
        $t->same(91, $nativeParity['pptxParsedCount']);
        $t->same(91, $nativeParity['nativeParsedCount']);
        $t->same(91, $nativeParity['bothParsedCount']);
        $t->same(0, $nativeParity['parseFailureCount']);
        $t->same(91, $nativeParity['normalizedAstMatchCount']);
        $t->same(0, $nativeParity['normalizedAstMismatchCount']);
        $t->same(91, count($nativeParity['fixtureComparisons']));
        ...
        $t->same(91, $executableParity['requiredPptxCount']);
        ...
        $t->same('21f798da16d093631838bdaa58cf1185877e9a0081129d037c57d5708edf26b5', $executableParity['snapshotFile']['sha256']);
        $t->same(39571, $executableParity['snapshotFile']['bytes']);
        ...
        $t->same(91, $executableParity['totalPptxCount']);
        $t->same(91, $executableParity['comparedPptxCount']);
        $t->same(91, $executableParity['localParsedCount']);
        $t->same(91, $executableParity['pandocParsedCount']);
        $t->same(91, $executableParity['nativeFixtureParsedCount']);
        $t->same(91, $executableParity['bothParsedCount']);
        $t->same(0, $executableParity['parseFailureCount']);
        $t->same(91, $executableParity['normalizedAstMatchCount']);
        $t->same(0, $executableParity['normalizedAstMismatchCount']);
        $t->same(91, $executableParity['pandocNativeFixtureComparedCount']);
        $t->same(91, $executableParity['pandocNativeFixtureMatchCount']);
        ...
        $t->true(in_array('local PHP PPTX reader output matches all 91 checked-in current PPTX/native pairs by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        $t->true(in_array('checked-in executable native AST evidence shows pandoc 3.10, local PHP output, and paired .native fixtures match all 91 checked-in current PPTX fixtures by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        ...
        $t->contains('Static current evidence: valid-checked-in-current-pptx-reader-evidence comparisons=1 checkedInPairs=91', $text);
        $t->contains('Static native AST mapped parity: normalized-ast-equality-observed-not-runner-parity matches=91 mismatches=0 required=91', $text);
        $t->contains('Static executable native AST parity: normalized-ast-equality-observed-against-pandoc-executable matches=91 mismatches=0 required=91', $text);
        $t->contains('Runner status: not-run', $text);
        $t->contains('Runner plan: planned-not-run', $text);
    },
...
HS);
    ...
    'cli gates checked-in current pptx static evidence without upstream runner claim' => static function (TestRunner $t) use ($makeTempDir, $removeTree): void {
        ...
        $t->same(91, $summary['staticCurrentEvidence']['checkedInFixturePairCount']);
        $t->same(91, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(91, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(91, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['pandocNativeFixtureMatchCount']);
    ...
    'workflow gates checked-in pptx native and executable parity corpora' => static function (TestRunner $t): void {
        ...
        $t->contains('--require-executable-parity=91', $workflow);
