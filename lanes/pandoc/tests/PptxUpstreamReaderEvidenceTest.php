...
return [
    ...
    'reports checked-in current upstream pptx static evidence gate' => static function (TestRunner $t): void {
        ...
        $t->same(89, $static['checkedInFixturePairCount']);
        ...
        $t->same(89, $nativeParity['requiredPairCount']);
        $t->same(89, $nativeParity['totalPairCount']);
        $t->same(89, $nativeParity['comparedPairCount']);
        $t->same(89, $nativeParity['pptxParsedCount']);
        $t->same(89, $nativeParity['nativeParsedCount']);
        $t->same(89, $nativeParity['bothParsedCount']);
        $t->same(0, $nativeParity['parseFailureCount']);
        $t->same(89, $nativeParity['normalizedAstMatchCount']);
        $t->same(0, $nativeParity['normalizedAstMismatchCount']);
        $t->same(89, count($nativeParity['fixtureComparisons']));
        ...
        $t->same(89, $executableParity['requiredPptxCount']);
        ...
        $t->same('f54f5a4af3607136e2dfe4f4107a01b66a051af36ba384329bfe6b245e1ed4b7', $executableParity['snapshotFile']['sha256']);
        $t->same(38741, $executableParity['snapshotFile']['bytes']);
        ...
        $t->same(89, $executableParity['totalPptxCount']);
        $t->same(89, $executableParity['comparedPptxCount']);
        $t->same(89, $executableParity['localParsedCount']);
        $t->same(89, $executableParity['pandocParsedCount']);
        $t->same(89, $executableParity['nativeFixtureParsedCount']);
        $t->same(89, $executableParity['bothParsedCount']);
        $t->same(0, $executableParity['parseFailureCount']);
        $t->same(89, $executableParity['normalizedAstMatchCount']);
        $t->same(0, $executableParity['normalizedAstMismatchCount']);
        $t->same(89, $executableParity['pandocNativeFixtureComparedCount']);
        $t->same(89, $executableParity['pandocNativeFixtureMatchCount']);
        ...
        $t->true(in_array('local PHP PPTX reader output matches all 89 checked-in current PPTX/native pairs by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        $t->true(in_array('checked-in executable native AST evidence shows pandoc 3.10, local PHP output, and paired .native fixtures match all 89 checked-in current PPTX fixtures by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        ...
        $t->contains('Static current evidence: valid-checked-in-current-pptx-reader-evidence comparisons=1 checkedInPairs=89', $text);
        $t->contains('Static native AST mapped parity: normalized-ast-equality-observed-not-runner-parity matches=89 mismatches=0 required=89', $text);
        $t->contains('Static executable native AST parity: normalized-ast-equality-observed-against-pandoc-executable matches=89 mismatches=0 required=89', $text);
        $t->contains('Runner status: not-run', $text);
        $t->contains('Runner plan: planned-not-run', $text);
    },
...
HS);
    ...
    'cli gates checked-in current pptx static evidence without upstream runner claim' => static function (TestRunner $t) use ($makeTempDir, $removeTree): void {
        ...
        $t->same(89, $summary['staticCurrentEvidence']['checkedInFixturePairCount']);
        $t->same(89, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(89, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(89, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['pandocNativeFixtureMatchCount']);
    ...
    'workflow gates checked-in pptx native and executable parity corpora' => static function (TestRunner $t): void {
        ...
        $t->contains('--require-executable-parity=89', $workflow);
