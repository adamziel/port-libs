...
return [
    ...
    'reports checked-in current upstream pptx static evidence gate' => static function (TestRunner $t): void {
        ...
        $t->same(50, $static['checkedInFixturePairCount']);
        ...
        $t->same(50, $nativeParity['requiredPairCount']);
        $t->same(50, $nativeParity['totalPairCount']);
        $t->same(50, $nativeParity['comparedPairCount']);
        $t->same(50, $nativeParity['pptxParsedCount']);
        $t->same(50, $nativeParity['nativeParsedCount']);
        $t->same(50, $nativeParity['bothParsedCount']);
        $t->same(0, $nativeParity['parseFailureCount']);
        $t->same(50, $nativeParity['normalizedAstMatchCount']);
        $t->same(0, $nativeParity['normalizedAstMismatchCount']);
        $t->same(50, count($nativeParity['fixtureComparisons']));
        ...
        $t->same(50, $executableParity['requiredPptxCount']);
        ...
        $t->same('932a6cf4c5bd2c3e83387b69f69c90956c933f7039e199e442479446b995b0b3', $executableParity['snapshotFile']['sha256']);
        $t->same(25299, $executableParity['snapshotFile']['bytes']);
        ...
        $t->same(50, $executableParity['totalPptxCount']);
        $t->same(50, $executableParity['comparedPptxCount']);
        ...
        $t->true(in_array('local PHP PPTX reader output matches all 50 checked-in current PPTX/native pairs by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        $t->true(in_array('checked-in executable native AST evidence shows pandoc 3.10, local PHP output, and paired .native fixtures match all 50 checked-in current PPTX fixtures by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        ...
        $t->contains('Static current evidence: valid-checked-in-current-pptx-reader-evidence comparisons=1 checkedInPairs=50', $text);
        $t->contains('Static native AST mapped parity: normalized-ast-equality-observed-not-runner-parity matches=50 mismatches=0 required=50', $text);
        $t->contains('Static executable native AST parity: normalized-ast-equality-observed-against-pandoc-executable matches=50 mismatches=0 required=50', $text);
        $t->contains('Runner status: not-run', $text);
        $t->contains('Runner plan: planned-not-run', $text);
    },
...
HS);
    ...
    'cli gates checked-in current pptx static evidence without upstream runner claim' => static function (TestRunner $t) use ($makeTempDir, $removeTree): void {
        ...
        $t->same(50, $summary['staticCurrentEvidence']['checkedInFixturePairCount']);
        $t->same(50, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(50, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(50, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['pandocNativeFixtureMatchCount']);
