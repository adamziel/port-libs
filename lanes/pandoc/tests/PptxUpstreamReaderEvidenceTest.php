...
return [
    ...
    'reports checked-in current upstream pptx static evidence gate' => static function (TestRunner $t): void {
        ...
        $t->same(56, $static['checkedInFixturePairCount']);
        ...
        $t->same(56, $nativeParity['requiredPairCount']);
        $t->same(56, $nativeParity['totalPairCount']);
        $t->same(56, $nativeParity['comparedPairCount']);
        $t->same(56, $nativeParity['pptxParsedCount']);
        $t->same(56, $nativeParity['nativeParsedCount']);
        $t->same(56, $nativeParity['bothParsedCount']);
        $t->same(0, $nativeParity['parseFailureCount']);
        $t->same(56, $nativeParity['normalizedAstMatchCount']);
        $t->same(0, $nativeParity['normalizedAstMismatchCount']);
        $t->same(56, count($nativeParity['fixtureComparisons']));
        ...
        $t->same(56, $executableParity['requiredPptxCount']);
        ...
        $t->same('1696638f865f509d06cb5f45b5ee6df78a9fe99eecdc53f78ae9180ac94ba05c', $executableParity['snapshotFile']['sha256']);
        $t->same(28018, $executableParity['snapshotFile']['bytes']);
        ...
        $t->same(56, $executableParity['totalPptxCount']);
        $t->same(56, $executableParity['comparedPptxCount']);
        ...
        $t->true(in_array('local PHP PPTX reader output matches all 56 checked-in current PPTX/native pairs by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        $t->true(in_array('checked-in executable native AST evidence shows pandoc 3.10, local PHP output, and paired .native fixtures match all 56 checked-in current PPTX fixtures by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        ...
        $t->contains('Static current evidence: valid-checked-in-current-pptx-reader-evidence comparisons=1 checkedInPairs=56', $text);
        $t->contains('Static native AST mapped parity: normalized-ast-equality-observed-not-runner-parity matches=56 mismatches=0 required=56', $text);
        $t->contains('Static executable native AST parity: normalized-ast-equality-observed-against-pandoc-executable matches=56 mismatches=0 required=56', $text);
        $t->contains('Runner status: not-run', $text);
        $t->contains('Runner plan: planned-not-run', $text);
    },
...
HS);
    ...
    'cli gates checked-in current pptx static evidence without upstream runner claim' => static function (TestRunner $t) use ($makeTempDir, $removeTree): void {
        ...
        $t->same(56, $summary['staticCurrentEvidence']['checkedInFixturePairCount']);
        $t->same(56, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(56, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(56, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['pandocNativeFixtureMatchCount']);
