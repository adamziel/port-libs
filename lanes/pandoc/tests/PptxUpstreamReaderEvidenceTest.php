...
return [
    ...
    'reports checked-in current upstream pptx static evidence gate' => static function (TestRunner $t): void {
        ...
        $t->same(81, $static['checkedInFixturePairCount']);
        ...
        $t->same(81, $nativeParity['requiredPairCount']);
        $t->same(81, $nativeParity['totalPairCount']);
        $t->same(81, $nativeParity['comparedPairCount']);
        $t->same(81, $nativeParity['pptxParsedCount']);
        $t->same(81, $nativeParity['nativeParsedCount']);
        $t->same(81, $nativeParity['bothParsedCount']);
        $t->same(0, $nativeParity['parseFailureCount']);
        $t->same(81, $nativeParity['normalizedAstMatchCount']);
        $t->same(0, $nativeParity['normalizedAstMismatchCount']);
        $t->same(81, count($nativeParity['fixtureComparisons']));
        ...
        $t->same(81, $executableParity['requiredPptxCount']);
        ...
        $t->same('2bb340ac4c204eca93abdd0c23f9eb485f36b2eae6478c906a12d67dc8f1a5ec', $executableParity['snapshotFile']['sha256']);
        $t->same(35409, $executableParity['snapshotFile']['bytes']);
        ...
        $t->same(81, $executableParity['totalPptxCount']);
        $t->same(81, $executableParity['comparedPptxCount']);
        $t->same(81, $executableParity['localParsedCount']);
        $t->same(81, $executableParity['pandocParsedCount']);
        $t->same(81, $executableParity['nativeFixtureParsedCount']);
        $t->same(81, $executableParity['bothParsedCount']);
        $t->same(0, $executableParity['parseFailureCount']);
        $t->same(81, $executableParity['normalizedAstMatchCount']);
        $t->same(0, $executableParity['normalizedAstMismatchCount']);
        $t->same(81, $executableParity['pandocNativeFixtureComparedCount']);
        $t->same(81, $executableParity['pandocNativeFixtureMatchCount']);
        ...
        $t->true(in_array('local PHP PPTX reader output matches all 81 checked-in current PPTX/native pairs by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        $t->true(in_array('checked-in executable native AST evidence shows pandoc 3.10, local PHP output, and paired .native fixtures match all 81 checked-in current PPTX fixtures by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        ...
        $t->contains('Static current evidence: valid-checked-in-current-pptx-reader-evidence comparisons=1 checkedInPairs=81', $text);
        $t->contains('Static native AST mapped parity: normalized-ast-equality-observed-not-runner-parity matches=81 mismatches=0 required=81', $text);
        $t->contains('Static executable native AST parity: normalized-ast-equality-observed-against-pandoc-executable matches=81 mismatches=0 required=81', $text);
        $t->contains('Runner status: not-run', $text);
        $t->contains('Runner plan: planned-not-run', $text);
    },
...
HS);
    ...
    'cli gates checked-in current pptx static evidence without upstream runner claim' => static function (TestRunner $t) use ($makeTempDir, $removeTree): void {
        ...
        $t->same(81, $summary['staticCurrentEvidence']['checkedInFixturePairCount']);
        $t->same(81, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(81, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(81, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['pandocNativeFixtureMatchCount']);
