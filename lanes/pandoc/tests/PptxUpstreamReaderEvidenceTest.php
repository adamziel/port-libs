...
return [
    ...
    'reports checked-in current upstream pptx static evidence gate' => static function (TestRunner $t): void {
        ...
        $t->same(67, $static['checkedInFixturePairCount']);
        ...
        $t->same(67, $nativeParity['requiredPairCount']);
        $t->same(67, $nativeParity['totalPairCount']);
        $t->same(67, $nativeParity['comparedPairCount']);
        $t->same(67, $nativeParity['pptxParsedCount']);
        $t->same(67, $nativeParity['nativeParsedCount']);
        $t->same(67, $nativeParity['bothParsedCount']);
        $t->same(0, $nativeParity['parseFailureCount']);
        $t->same(67, $nativeParity['normalizedAstMatchCount']);
        $t->same(0, $nativeParity['normalizedAstMismatchCount']);
        $t->same(67, count($nativeParity['fixtureComparisons']));
        ...
        $t->same(67, $executableParity['requiredPptxCount']);
        ...
        $t->same('75f4160c1ff81842e5ce60b133f6722a0ac182d7af0d43f7aadac178cba8a53a', $executableParity['snapshotFile']['sha256']);
        $t->same(33232, $executableParity['snapshotFile']['bytes']);
        ...
        $t->same(67, $executableParity['totalPptxCount']);
        $t->same(67, $executableParity['comparedPptxCount']);
        ...
        $t->true(in_array('local PHP PPTX reader output matches all 67 checked-in current PPTX/native pairs by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        $t->true(in_array('checked-in executable native AST evidence shows pandoc 3.10, local PHP output, and paired .native fixtures match all 67 checked-in current PPTX fixtures by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        ...
        $t->contains('Static current evidence: valid-checked-in-current-pptx-reader-evidence comparisons=1 checkedInPairs=67', $text);
        $t->contains('Static native AST mapped parity: normalized-ast-equality-observed-not-runner-parity matches=67 mismatches=0 required=67', $text);
        $t->contains('Static executable native AST parity: normalized-ast-equality-observed-against-pandoc-executable matches=67 mismatches=0 required=67', $text);
        $t->contains('Runner status: not-run', $text);
        $t->contains('Runner plan: planned-not-run', $text);
    },
...
HS);
    ...
    'cli gates checked-in current pptx static evidence without upstream runner claim' => static function (TestRunner $t) use ($makeTempDir, $removeTree): void {
        ...
        $t->same(67, $summary['staticCurrentEvidence']['checkedInFixturePairCount']);
        $t->same(67, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(67, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(67, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['pandocNativeFixtureMatchCount']);
