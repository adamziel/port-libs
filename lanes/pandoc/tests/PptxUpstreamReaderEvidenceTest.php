...
return [
    ...
    'reports checked-in current upstream pptx static evidence gate' => static function (TestRunner $t): void {
        ...
        $t->same(61, $static['checkedInFixturePairCount']);
        ...
        $t->same(61, $nativeParity['requiredPairCount']);
        $t->same(61, $nativeParity['totalPairCount']);
        $t->same(61, $nativeParity['comparedPairCount']);
        $t->same(61, $nativeParity['pptxParsedCount']);
        $t->same(61, $nativeParity['nativeParsedCount']);
        $t->same(61, $nativeParity['bothParsedCount']);
        $t->same(0, $nativeParity['parseFailureCount']);
        $t->same(61, $nativeParity['normalizedAstMatchCount']);
        $t->same(0, $nativeParity['normalizedAstMismatchCount']);
        $t->same(61, count($nativeParity['fixtureComparisons']));
        ...
        $t->same(61, $executableParity['requiredPptxCount']);
        ...
        $t->same('6d5c0665fb0f1bbc211b9a6ca4e2138ee43d2bd0e3ccd1c08bedbaf09e43f707', $executableParity['snapshotFile']['sha256']);
        $t->same(30358, $executableParity['snapshotFile']['bytes']);
        ...
        $t->same(61, $executableParity['totalPptxCount']);
        $t->same(61, $executableParity['comparedPptxCount']);
        ...
        $t->true(in_array('local PHP PPTX reader output matches all 61 checked-in current PPTX/native pairs by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        $t->true(in_array('checked-in executable native AST evidence shows pandoc 3.10, local PHP output, and paired .native fixtures match all 61 checked-in current PPTX fixtures by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        ...
        $t->contains('Static current evidence: valid-checked-in-current-pptx-reader-evidence comparisons=1 checkedInPairs=61', $text);
        $t->contains('Static native AST mapped parity: normalized-ast-equality-observed-not-runner-parity matches=61 mismatches=0 required=61', $text);
        $t->contains('Static executable native AST parity: normalized-ast-equality-observed-against-pandoc-executable matches=61 mismatches=0 required=61', $text);
        $t->contains('Runner status: not-run', $text);
        $t->contains('Runner plan: planned-not-run', $text);
    },
...
HS);
    ...
    'cli gates checked-in current pptx static evidence without upstream runner claim' => static function (TestRunner $t) use ($makeTempDir, $removeTree): void {
        ...
        $t->same(61, $summary['staticCurrentEvidence']['checkedInFixturePairCount']);
        $t->same(61, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(61, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(61, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['pandocNativeFixtureMatchCount']);
