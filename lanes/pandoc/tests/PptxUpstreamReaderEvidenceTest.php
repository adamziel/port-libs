...
return [
    ...
    'reports checked-in current upstream pptx static evidence gate' => static function (TestRunner $t): void {
        ...
        $t->same(90, $static['checkedInFixturePairCount']);
        ...
        $t->same(90, $nativeParity['requiredPairCount']);
        $t->same(90, $nativeParity['totalPairCount']);
        $t->same(90, $nativeParity['comparedPairCount']);
        $t->same(90, $nativeParity['pptxParsedCount']);
        $t->same(90, $nativeParity['nativeParsedCount']);
        $t->same(90, $nativeParity['bothParsedCount']);
        $t->same(0, $nativeParity['parseFailureCount']);
        $t->same(90, $nativeParity['normalizedAstMatchCount']);
        $t->same(0, $nativeParity['normalizedAstMismatchCount']);
        $t->same(90, count($nativeParity['fixtureComparisons']));
        ...
        $t->same(90, $executableParity['requiredPptxCount']);
        ...
        $t->same('fa2ec1c7ae826f29461fbf79fa944c57a1b512069581e6864c8d879cfa242d0b', $executableParity['snapshotFile']['sha256']);
        $t->same(39176, $executableParity['snapshotFile']['bytes']);
        ...
        $t->same(90, $executableParity['totalPptxCount']);
        $t->same(90, $executableParity['comparedPptxCount']);
        $t->same(90, $executableParity['localParsedCount']);
        $t->same(90, $executableParity['pandocParsedCount']);
        $t->same(90, $executableParity['nativeFixtureParsedCount']);
        $t->same(90, $executableParity['bothParsedCount']);
        $t->same(0, $executableParity['parseFailureCount']);
        $t->same(90, $executableParity['normalizedAstMatchCount']);
        $t->same(0, $executableParity['normalizedAstMismatchCount']);
        $t->same(90, $executableParity['pandocNativeFixtureComparedCount']);
        $t->same(90, $executableParity['pandocNativeFixtureMatchCount']);
        ...
        $t->true(in_array('local PHP PPTX reader output matches all 90 checked-in current PPTX/native pairs by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        $t->true(in_array('checked-in executable native AST evidence shows pandoc 3.10, local PHP output, and paired .native fixtures match all 90 checked-in current PPTX fixtures by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        ...
        $t->contains('Static current evidence: valid-checked-in-current-pptx-reader-evidence comparisons=1 checkedInPairs=90', $text);
        $t->contains('Static native AST mapped parity: normalized-ast-equality-observed-not-runner-parity matches=90 mismatches=0 required=90', $text);
        $t->contains('Static executable native AST parity: normalized-ast-equality-observed-against-pandoc-executable matches=90 mismatches=0 required=90', $text);
        $t->contains('Runner status: not-run', $text);
        $t->contains('Runner plan: planned-not-run', $text);
    },
...
HS);
    ...
    'cli gates checked-in current pptx static evidence without upstream runner claim' => static function (TestRunner $t) use ($makeTempDir, $removeTree): void {
        ...
        $t->same(90, $summary['staticCurrentEvidence']['checkedInFixturePairCount']);
        $t->same(90, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(90, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(90, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['pandocNativeFixtureMatchCount']);
    ...
    'workflow gates checked-in pptx native and executable parity corpora' => static function (TestRunner $t): void {
        ...
        $t->contains('--require-executable-parity=90', $workflow);
