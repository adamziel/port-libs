...
return [
    ...
    'reports checked-in current upstream pptx static evidence gate' => static function (TestRunner $t): void {
        ...
        $t->same(58, $static['checkedInFixturePairCount']);
        ...
        $t->same(58, $nativeParity['requiredPairCount']);
        $t->same(58, $nativeParity['totalPairCount']);
        $t->same(58, $nativeParity['comparedPairCount']);
        $t->same(58, $nativeParity['pptxParsedCount']);
        $t->same(58, $nativeParity['nativeParsedCount']);
        $t->same(58, $nativeParity['bothParsedCount']);
        $t->same(0, $nativeParity['parseFailureCount']);
        $t->same(58, $nativeParity['normalizedAstMatchCount']);
        $t->same(0, $nativeParity['normalizedAstMismatchCount']);
        $t->same(58, count($nativeParity['fixtureComparisons']));
        ...
        $t->same(58, $executableParity['requiredPptxCount']);
        ...
        $t->same('336f8df897325675e6b8c4e61f83c6e286c2684276b59b4c7b8ee68ff989fa66', $executableParity['snapshotFile']['sha256']);
        $t->same(28964, $executableParity['snapshotFile']['bytes']);
        ...
        $t->same(58, $executableParity['totalPptxCount']);
        $t->same(58, $executableParity['comparedPptxCount']);
        ...
        $t->true(in_array('local PHP PPTX reader output matches all 58 checked-in current PPTX/native pairs by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        $t->true(in_array('checked-in executable native AST evidence shows pandoc 3.10, local PHP output, and paired .native fixtures match all 58 checked-in current PPTX fixtures by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        ...
        $t->contains('Static current evidence: valid-checked-in-current-pptx-reader-evidence comparisons=1 checkedInPairs=58', $text);
        $t->contains('Static native AST mapped parity: normalized-ast-equality-observed-not-runner-parity matches=58 mismatches=0 required=58', $text);
        $t->contains('Static executable native AST parity: normalized-ast-equality-observed-against-pandoc-executable matches=58 mismatches=0 required=58', $text);
        $t->contains('Runner status: not-run', $text);
        $t->contains('Runner plan: planned-not-run', $text);
    },
...
HS);
    ...
    'cli gates checked-in current pptx static evidence without upstream runner claim' => static function (TestRunner $t) use ($makeTempDir, $removeTree): void {
        ...
        $t->same(58, $summary['staticCurrentEvidence']['checkedInFixturePairCount']);
        $t->same(58, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(58, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(58, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['pandocNativeFixtureMatchCount']);
