...
return [
    ...
    'reports checked-in current upstream pptx static evidence gate' => static function (TestRunner $t): void {
        ...
        $t->same(47, $static['checkedInFixturePairCount']);
        ...
        $t->same(47, $nativeParity['requiredPairCount']);
        $t->same(47, $nativeParity['totalPairCount']);
        $t->same(47, $nativeParity['comparedPairCount']);
        $t->same(47, $nativeParity['pptxParsedCount']);
        $t->same(47, $nativeParity['nativeParsedCount']);
        $t->same(47, $nativeParity['bothParsedCount']);
        $t->same(0, $nativeParity['parseFailureCount']);
        $t->same(47, $nativeParity['normalizedAstMatchCount']);
        ...
        $t->same(47, $executableParity['requiredPptxCount']);
        ...
        $t->same('255b28c0ddc0a1782c629766a43f2722a4829dc04a7abaf6bef717ff3d376535', $executableParity['snapshotFile']['sha256']);
        $t->same(9743, $executableParity['snapshotFile']['bytes']);
        ...
        $t->same(47, $executableParity['totalPptxCount']);
        $t->same(47, $executableParity['comparedPptxCount']);
        ...
        $t->true(in_array('local PHP PPTX reader output matches all 47 checked-in current PPTX/native pairs by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        $t->true(in_array('checked-in executable native AST evidence shows pandoc 3.10, local PHP output, and paired .native fixtures match all 47 checked-in current PPTX fixtures by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        ...
        $t->contains('Static current evidence: valid-checked-in-current-pptx-reader-evidence comparisons=1 checkedInPairs=47', $text);
        $t->contains('Static native AST mapped parity: normalized-ast-equality-observed-not-runner-parity matches=47 mismatches=0 required=47', $text);
        $t->contains('Static executable native AST parity: normalized-ast-equality-observed-against-pandoc-executable matches=47 mismatches=0 required=47', $text);
        $t->contains('Runner status: not-run', $text);
        $t->contains('Runner plan: planned-not-run', $text);
    },
...
HS);
    ...
    'cli gates checked-in current pptx static evidence without upstream runner claim' => static function (TestRunner $t) use ($makeTempDir, $removeTree): void {
        ...
        $t->same(47, $summary['staticCurrentEvidence']['checkedInFixturePairCount']);
        $t->same(47, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(47, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(47, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['pandocNativeFixtureMatchCount']);
