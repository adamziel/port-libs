...
return [
    ...
    'reports checked-in current upstream pptx static evidence gate' => static function (TestRunner $t): void {
        ...
        $t->same(57, $static['checkedInFixturePairCount']);
        ...
        $t->same(57, $nativeParity['requiredPairCount']);
        $t->same(57, $nativeParity['totalPairCount']);
        $t->same(57, $nativeParity['comparedPairCount']);
        $t->same(57, $nativeParity['pptxParsedCount']);
        $t->same(57, $nativeParity['nativeParsedCount']);
        $t->same(57, $nativeParity['bothParsedCount']);
        $t->same(0, $nativeParity['parseFailureCount']);
        $t->same(57, $nativeParity['normalizedAstMatchCount']);
        $t->same(0, $nativeParity['normalizedAstMismatchCount']);
        $t->same(57, count($nativeParity['fixtureComparisons']));
        ...
        $t->same(57, $executableParity['requiredPptxCount']);
        ...
        $t->same('2dbbe54aa1ad3bb6d6d77f387735f8726dfbe8ace430e3b62afa8e3dd2aeaf00', $executableParity['snapshotFile']['sha256']);
        $t->same(28464, $executableParity['snapshotFile']['bytes']);
        ...
        $t->same(57, $executableParity['totalPptxCount']);
        $t->same(57, $executableParity['comparedPptxCount']);
        ...
        $t->true(in_array('local PHP PPTX reader output matches all 57 checked-in current PPTX/native pairs by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        $t->true(in_array('checked-in executable native AST evidence shows pandoc 3.10, local PHP output, and paired .native fixtures match all 57 checked-in current PPTX fixtures by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        ...
        $t->contains('Static current evidence: valid-checked-in-current-pptx-reader-evidence comparisons=1 checkedInPairs=57', $text);
        $t->contains('Static native AST mapped parity: normalized-ast-equality-observed-not-runner-parity matches=57 mismatches=0 required=57', $text);
        $t->contains('Static executable native AST parity: normalized-ast-equality-observed-against-pandoc-executable matches=57 mismatches=0 required=57', $text);
        $t->contains('Runner status: not-run', $text);
        $t->contains('Runner plan: planned-not-run', $text);
    },
...
HS);
    ...
    'cli gates checked-in current pptx static evidence without upstream runner claim' => static function (TestRunner $t) use ($makeTempDir, $removeTree): void {
        ...
        $t->same(57, $summary['staticCurrentEvidence']['checkedInFixturePairCount']);
        $t->same(57, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(57, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(57, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['pandocNativeFixtureMatchCount']);
