...
return [
    ...
    'reports checked-in current upstream pptx static evidence gate' => static function (TestRunner $t): void {
        ...
        $t->same(53, $static['checkedInFixturePairCount']);
        ...
        $t->same(53, $nativeParity['requiredPairCount']);
        $t->same(53, $nativeParity['totalPairCount']);
        $t->same(53, $nativeParity['comparedPairCount']);
        $t->same(53, $nativeParity['pptxParsedCount']);
        $t->same(53, $nativeParity['nativeParsedCount']);
        $t->same(53, $nativeParity['bothParsedCount']);
        $t->same(0, $nativeParity['parseFailureCount']);
        $t->same(53, $nativeParity['normalizedAstMatchCount']);
        $t->same(0, $nativeParity['normalizedAstMismatchCount']);
        $t->same(53, count($nativeParity['fixtureComparisons']));
        ...
        $t->same(53, $executableParity['requiredPptxCount']);
        ...
        $t->same('5bbff786b0c4c5d60d3b5183cea89f0a9df27b37d0afe7ea01b03ed1323a2d8d', $executableParity['snapshotFile']['sha256']);
        $t->same(26686, $executableParity['snapshotFile']['bytes']);
        ...
        $t->same(53, $executableParity['totalPptxCount']);
        $t->same(53, $executableParity['comparedPptxCount']);
        ...
        $t->true(in_array('local PHP PPTX reader output matches all 53 checked-in current PPTX/native pairs by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        $t->true(in_array('checked-in executable native AST evidence shows pandoc 3.10, local PHP output, and paired .native fixtures match all 53 checked-in current PPTX fixtures by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        ...
        $t->contains('Static current evidence: valid-checked-in-current-pptx-reader-evidence comparisons=1 checkedInPairs=53', $text);
        $t->contains('Static native AST mapped parity: normalized-ast-equality-observed-not-runner-parity matches=53 mismatches=0 required=53', $text);
        $t->contains('Static executable native AST parity: normalized-ast-equality-observed-against-pandoc-executable matches=53 mismatches=0 required=53', $text);
        $t->contains('Runner status: not-run', $text);
        $t->contains('Runner plan: planned-not-run', $text);
    },
...
HS);
    ...
    'cli gates checked-in current pptx static evidence without upstream runner claim' => static function (TestRunner $t) use ($makeTempDir, $removeTree): void {
        ...
        $t->same(53, $summary['staticCurrentEvidence']['checkedInFixturePairCount']);
        $t->same(53, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(53, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(53, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['pandocNativeFixtureMatchCount']);
