...
return [
    ...
    'reports checked-in current upstream pptx static evidence gate' => static function (TestRunner $t): void {
        ...
        $t->same(46, $static['checkedInFixturePairCount']);
        ...
        $t->same(46, $nativeParity['requiredPairCount']);
        $t->same(46, $nativeParity['totalPairCount']);
        $t->same(46, $nativeParity['comparedPairCount']);
        $t->same(46, $nativeParity['pptxParsedCount']);
        $t->same(46, $nativeParity['nativeParsedCount']);
        $t->same(46, $nativeParity['bothParsedCount']);
        $t->same(0, $nativeParity['parseFailureCount']);
        $t->same(46, $nativeParity['normalizedAstMatchCount']);
        ...
        $t->same(46, $executableParity['requiredPptxCount']);
        ...
        $t->same('3108ae83683406e24944d3b2d02287552a1d3c42e29888379c7cf5e9a0a53002', $executableParity['snapshotFile']['sha256']);
        $t->same(9546, $executableParity['snapshotFile']['bytes']);
        ...
        $t->same(46, $executableParity['totalPptxCount']);
        $t->same(46, $executableParity['comparedPptxCount']);
        ...
        $t->true(in_array('local PHP PPTX reader output matches all 46 checked-in current PPTX/native pairs by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        $t->true(in_array('checked-in executable native AST evidence shows pandoc 3.10, local PHP output, and paired .native fixtures match all 46 checked-in current PPTX fixtures by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        ...
        $t->contains('Static current evidence: valid-checked-in-current-pptx-reader-evidence comparisons=1 checkedInPairs=46', $text);
        $t->contains('Static native AST mapped parity: normalized-ast-equality-observed-not-runner-parity matches=46 mismatches=0 required=46', $text);
        $t->contains('Static executable native AST parity: normalized-ast-equality-observed-against-pandoc-executable matches=46 mismatches=0 required=46', $text);
        $t->contains('Runner status: not-run', $text);
        $t->contains('Runner plan: planned-not-run', $text);
    },
...
HS);
    ...
    'cli gates checked-in current pptx static evidence without upstream runner claim' => static function (TestRunner $t) use ($makeTempDir, $removeTree): void {
        ...
        $t->same(46, $summary['staticCurrentEvidence']['checkedInFixturePairCount']);
        $t->same(46, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(46, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(46, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['pandocNativeFixtureMatchCount']);
