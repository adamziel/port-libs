...
return [
    ...
    'reports checked-in current upstream pptx static evidence gate' => static function (TestRunner $t): void {
        ...
        $t->same(51, $static['checkedInFixturePairCount']);
        ...
        $t->same(51, $nativeParity['requiredPairCount']);
        $t->same(51, $nativeParity['totalPairCount']);
        $t->same(51, $nativeParity['comparedPairCount']);
        $t->same(51, $nativeParity['pptxParsedCount']);
        $t->same(51, $nativeParity['nativeParsedCount']);
        $t->same(51, $nativeParity['bothParsedCount']);
        $t->same(0, $nativeParity['parseFailureCount']);
        $t->same(51, $nativeParity['normalizedAstMatchCount']);
        $t->same(0, $nativeParity['normalizedAstMismatchCount']);
        $t->same(51, count($nativeParity['fixtureComparisons']));
        ...
        $t->same(51, $executableParity['requiredPptxCount']);
        ...
        $t->same('7c56905ff13ece0d313000fc55bbc857cb5c7b738551d180dbd09d2e28f8c0cd', $executableParity['snapshotFile']['sha256']);
        $t->same(25751, $executableParity['snapshotFile']['bytes']);
        ...
        $t->same(51, $executableParity['totalPptxCount']);
        $t->same(51, $executableParity['comparedPptxCount']);
        ...
        $t->true(in_array('local PHP PPTX reader output matches all 51 checked-in current PPTX/native pairs by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        $t->true(in_array('checked-in executable native AST evidence shows pandoc 3.10, local PHP output, and paired .native fixtures match all 51 checked-in current PPTX fixtures by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        ...
        $t->contains('Static current evidence: valid-checked-in-current-pptx-reader-evidence comparisons=1 checkedInPairs=51', $text);
        $t->contains('Static native AST mapped parity: normalized-ast-equality-observed-not-runner-parity matches=51 mismatches=0 required=51', $text);
        $t->contains('Static executable native AST parity: normalized-ast-equality-observed-against-pandoc-executable matches=51 mismatches=0 required=51', $text);
        $t->contains('Runner status: not-run', $text);
        $t->contains('Runner plan: planned-not-run', $text);
    },
...
HS);
    ...
    'cli gates checked-in current pptx static evidence without upstream runner claim' => static function (TestRunner $t) use ($makeTempDir, $removeTree): void {
        ...
        $t->same(51, $summary['staticCurrentEvidence']['checkedInFixturePairCount']);
        $t->same(51, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(51, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(51, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['pandocNativeFixtureMatchCount']);
