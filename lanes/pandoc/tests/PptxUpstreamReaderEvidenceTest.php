...
return [
    ...
    'reports checked-in current upstream pptx static evidence gate' => static function (TestRunner $t): void {
        ...
        $t->same(82, $static['checkedInFixturePairCount']);
        ...
        $t->same(82, $nativeParity['requiredPairCount']);
        $t->same(82, $nativeParity['totalPairCount']);
        $t->same(82, $nativeParity['comparedPairCount']);
        $t->same(82, $nativeParity['pptxParsedCount']);
        $t->same(82, $nativeParity['nativeParsedCount']);
        $t->same(82, $nativeParity['bothParsedCount']);
        $t->same(0, $nativeParity['parseFailureCount']);
        $t->same(82, $nativeParity['normalizedAstMatchCount']);
        $t->same(0, $nativeParity['normalizedAstMismatchCount']);
        $t->same(82, count($nativeParity['fixtureComparisons']));
        ...
        $t->same(82, $executableParity['requiredPptxCount']);
        ...
        $t->same('1a21160cdbb95a3a25519d50f41c7af8423bb25e5045a2a1824e6790c88bedf0', $executableParity['snapshotFile']['sha256']);
        $t->same(35836, $executableParity['snapshotFile']['bytes']);
        ...
        $t->same(82, $executableParity['totalPptxCount']);
        $t->same(82, $executableParity['comparedPptxCount']);
        $t->same(82, $executableParity['localParsedCount']);
        $t->same(82, $executableParity['pandocParsedCount']);
        $t->same(82, $executableParity['nativeFixtureParsedCount']);
        $t->same(82, $executableParity['bothParsedCount']);
        $t->same(0, $executableParity['parseFailureCount']);
        $t->same(82, $executableParity['normalizedAstMatchCount']);
        $t->same(0, $executableParity['normalizedAstMismatchCount']);
        $t->same(82, $executableParity['pandocNativeFixtureComparedCount']);
        $t->same(82, $executableParity['pandocNativeFixtureMatchCount']);
        ...
        $t->true(in_array('local PHP PPTX reader output matches all 82 checked-in current PPTX/native pairs by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        $t->true(in_array('checked-in executable native AST evidence shows pandoc 3.10, local PHP output, and paired .native fixtures match all 82 checked-in current PPTX fixtures by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        ...
        $t->contains('Static current evidence: valid-checked-in-current-pptx-reader-evidence comparisons=1 checkedInPairs=82', $text);
        $t->contains('Static native AST mapped parity: normalized-ast-equality-observed-not-runner-parity matches=82 mismatches=0 required=82', $text);
        $t->contains('Static executable native AST parity: normalized-ast-equality-observed-against-pandoc-executable matches=82 mismatches=0 required=82', $text);
        $t->contains('Runner status: not-run', $text);
        $t->contains('Runner plan: planned-not-run', $text);
    },
...
HS);
    ...
    'cli gates checked-in current pptx static evidence without upstream runner claim' => static function (TestRunner $t) use ($makeTempDir, $removeTree): void {
        ...
        $t->same(82, $summary['staticCurrentEvidence']['checkedInFixturePairCount']);
        $t->same(82, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(82, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(82, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['pandocNativeFixtureMatchCount']);
    ...
    'workflow gates checked-in pptx native and executable parity corpora' => static function (TestRunner $t): void {
        ...
        $t->contains('--require-executable-parity=', $workflow);
