...
return [
    ...
    'reports checked-in current upstream pptx static evidence gate' => static function (TestRunner $t): void {
        ...
        $t->same(106, $static['checkedInFixturePairCount']);
        ...
        $t->same(106, $nativeParity['requiredPairCount']);
        $t->same(106, $nativeParity['totalPairCount']);
        $t->same(106, $nativeParity['comparedPairCount']);
        $t->same(106, $nativeParity['pptxParsedCount']);
        $t->same(106, $nativeParity['nativeParsedCount']);
        $t->same(106, $nativeParity['bothParsedCount']);
        ...
        $t->same(106, $nativeParity['normalizedAstMatchCount']);
        $t->same(0, $nativeParity['normalizedAstMismatchCount']);
        $t->same(106, count($nativeParity['fixtureComparisons']));
        ...
        $t->same(106, $executableParity['requiredPptxCount']);
        ...
        $t->same('27d990e2fa865c1c094454755be1c40b6bde2387254543fd93c34a02a2246e8d', $executableParity['snapshotFile']['sha256']);
        $t->same(45746, $executableParity['snapshotFile']['bytes']);
        ...
        $t->same(106, $executableParity['totalPptxCount']);
        $t->same(106, $executableParity['comparedPptxCount']);
        $t->same(106, $executableParity['localParsedCount']);
        $t->same(106, $executableParity['pandocParsedCount']);
        $t->same(106, $executableParity['nativeFixtureParsedCount']);
        $t->same(106, $executableParity['bothParsedCount']);
        $t->same(0, $executableParity['parseFailureCount']);
        $t->same(106, $executableParity['normalizedAstMatchCount']);
        $t->same(0, $executableParity['normalizedAstMismatchCount']);
        $t->same(106, $executableParity['pandocNativeFixtureComparedCount']);
        $t->same(106, $executableParity['pandocNativeFixtureMatchCount']);
        ...
        $t->true(in_array('local PHP PPTX reader output matches all 106 checked-in current PPTX/native pairs by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        $t->true(in_array('checked-in executable native AST evidence shows pandoc 3.10, local PHP output, and paired .native fixtures match all 106 checked-in current PPTX fixtures by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        $t->true(in_array('additional PPTX fixture discovery outside the checked-in 106-pair corpus', $static['claimBoundaries']['doesNotAssert'], true));
        ...
        $t->contains('Static current evidence: valid-checked-in-current-pptx-reader-evidence comparisons=1 checkedInPairs=106', $text);
        $t->contains('Static native AST mapped parity: normalized-ast-equality-observed-not-runner-parity matches=106 mismatches=0 required=106', $text);
        $t->contains('Static executable native AST parity: normalized-ast-equality-observed-against-pandoc-executable matches=106 mismatches=0 required=106', $text);
...
HS);
    ...
    'cli gates checked-in current pptx static evidence without upstream runner claim' => static function (TestRunner $t) use ($makeTempDir, $removeTree): void {
        ...
        $command = escapeshellarg(PHP_BINARY)
            ...
            . ' --require-fixture-pair-count=106'
        ...
        $t->same(true, PptxUpstreamReaderEvidence::hasRequiredCheckedInFixturePairCount($decoded, 106));
        ...
        $t->same(106, $summary['staticCurrentEvidence']['checkedInFixturePairCount']);
        $t->same(106, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(106, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(106, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['pandocNativeFixtureMatchCount']);
        ...
        $wrongCountCommand = str_replace('--require-fixture-pair-count=106', '--require-fixture-pair-count=92', $command) . ' 2>/dev/null';
        $wrongCountOutput = [];
    ...
    'workflow gates checked-in pptx native and executable parity corpora' => static function (TestRunner $t): void {
        ...
        $t->contains('--require-executable-parity=106', $workflow);
