...
return [
    ...
    'reports checked-in current upstream pptx static evidence gate' => static function (TestRunner $t): void {
        ...
        $t->same(54, $static['checkedInFixturePairCount']);
        ...
        $t->same(54, $nativeParity['requiredPairCount']);
        $t->same(54, $nativeParity['totalPairCount']);
        $t->same(54, $nativeParity['comparedPairCount']);
        $t->same(54, $nativeParity['pptxParsedCount']);
        $t->same(54, $nativeParity['nativeParsedCount']);
        $t->same(54, $nativeParity['bothParsedCount']);
        $t->same(0, $nativeParity['parseFailureCount']);
        $t->same(54, $nativeParity['normalizedAstMatchCount']);
        $t->same(0, $nativeParity['normalizedAstMismatchCount']);
        $t->same(54, count($nativeParity['fixtureComparisons']));
        ...
        $t->same(54, $executableParity['requiredPptxCount']);
        ...
        $t->same('eb2c2184fabf52fdfec818e91619d1c4ca457ec0ab67a90cef9c65949828d514', $executableParity['snapshotFile']['sha256']);
        $t->same(27144, $executableParity['snapshotFile']['bytes']);
        ...
        $t->same(54, $executableParity['totalPptxCount']);
        $t->same(54, $executableParity['comparedPptxCount']);
        ...
        $t->true(in_array('local PHP PPTX reader output matches all 54 checked-in current PPTX/native pairs by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        $t->true(in_array('checked-in executable native AST evidence shows pandoc 3.10, local PHP output, and paired .native fixtures match all 54 checked-in current PPTX fixtures by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        ...
        $t->contains('Static current evidence: valid-checked-in-current-pptx-reader-evidence comparisons=1 checkedInPairs=54', $text);
        $t->contains('Static native AST mapped parity: normalized-ast-equality-observed-not-runner-parity matches=54 mismatches=0 required=54', $text);
        $t->contains('Static executable native AST parity: normalized-ast-equality-observed-against-pandoc-executable matches=54 mismatches=0 required=54', $text);
        $t->contains('Runner status: not-run', $text);
        $t->contains('Runner plan: planned-not-run', $text);
    },
...
HS);
    ...
    'cli gates checked-in current pptx static evidence without upstream runner claim' => static function (TestRunner $t) use ($makeTempDir, $removeTree): void {
        ...
        $t->same(54, $summary['staticCurrentEvidence']['checkedInFixturePairCount']);
        $t->same(54, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(54, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(54, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['pandocNativeFixtureMatchCount']);
