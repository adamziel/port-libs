...
return [
    ...
    'reports checked-in current upstream pptx static evidence gate' => static function (TestRunner $t): void {
        ...
        $t->same(69, $static['checkedInFixturePairCount']);
        ...
        $t->same(69, $nativeParity['requiredPairCount']);
        $t->same(69, $nativeParity['totalPairCount']);
        $t->same(69, $nativeParity['comparedPairCount']);
        $t->same(69, $nativeParity['pptxParsedCount']);
        $t->same(69, $nativeParity['nativeParsedCount']);
        $t->same(69, $nativeParity['bothParsedCount']);
        $t->same(0, $nativeParity['parseFailureCount']);
        $t->same(69, $nativeParity['normalizedAstMatchCount']);
        $t->same(0, $nativeParity['normalizedAstMismatchCount']);
        $t->same(69, count($nativeParity['fixtureComparisons']));
        ...
        $t->same(69, $executableParity['requiredPptxCount']);
        ...
        $t->same('a60c85b0ad08d4400d8f4bb6a3a9ce59512cd4689ddec082b404623908cb272b', $executableParity['snapshotFile']['sha256']);
        $t->same(30620, $executableParity['snapshotFile']['bytes']);
        ...
        $t->same(69, $executableParity['totalPptxCount']);
        $t->same(69, $executableParity['comparedPptxCount']);
        ...
        $t->true(in_array('local PHP PPTX reader output matches all 69 checked-in current PPTX/native pairs by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        $t->true(in_array('checked-in executable native AST evidence shows pandoc 3.10, local PHP output, and paired .native fixtures match all 69 checked-in current PPTX fixtures by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        ...
        $t->contains('Static current evidence: valid-checked-in-current-pptx-reader-evidence comparisons=1 checkedInPairs=69', $text);
        $t->contains('Static native AST mapped parity: normalized-ast-equality-observed-not-runner-parity matches=69 mismatches=0 required=69', $text);
        $t->contains('Static executable native AST parity: normalized-ast-equality-observed-against-pandoc-executable matches=69 mismatches=0 required=69', $text);
        $t->contains('Runner status: not-run', $text);
        $t->contains('Runner plan: planned-not-run', $text);
    },
...
HS);
    ...
    'cli gates checked-in current pptx static evidence without upstream runner claim' => static function (TestRunner $t) use ($makeTempDir, $removeTree): void {
        ...
        $t->same(69, $summary['staticCurrentEvidence']['checkedInFixturePairCount']);
        $t->same(69, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(69, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(69, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['pandocNativeFixtureMatchCount']);
