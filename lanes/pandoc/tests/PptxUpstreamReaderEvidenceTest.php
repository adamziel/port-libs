...
return [
    ...
    'reports checked-in current upstream pptx static evidence gate' => static function (TestRunner $t): void {
        ...
        $t->same(64, $static['checkedInFixturePairCount']);
        ...
        $t->same(64, $nativeParity['requiredPairCount']);
        $t->same(64, $nativeParity['totalPairCount']);
        $t->same(64, $nativeParity['comparedPairCount']);
        $t->same(64, $nativeParity['pptxParsedCount']);
        $t->same(64, $nativeParity['nativeParsedCount']);
        $t->same(64, $nativeParity['bothParsedCount']);
        $t->same(0, $nativeParity['parseFailureCount']);
        $t->same(64, $nativeParity['normalizedAstMatchCount']);
        $t->same(0, $nativeParity['normalizedAstMismatchCount']);
        $t->same(64, count($nativeParity['fixtureComparisons']));
        ...
        $t->same(64, $executableParity['requiredPptxCount']);
        ...
        $t->same('a82d0b66f15c69c7954bdb347b1ff220b21e0234e7b98f638cd053bf551087fc', $executableParity['snapshotFile']['sha256']);
        $t->same(31786, $executableParity['snapshotFile']['bytes']);
        ...
        $t->same(64, $executableParity['totalPptxCount']);
        $t->same(64, $executableParity['comparedPptxCount']);
        ...
        $t->true(in_array('local PHP PPTX reader output matches all 64 checked-in current PPTX/native pairs by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        $t->true(in_array('checked-in executable native AST evidence shows pandoc 3.10, local PHP output, and paired .native fixtures match all 64 checked-in current PPTX fixtures by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        ...
        $t->contains('Static current evidence: valid-checked-in-current-pptx-reader-evidence comparisons=1 checkedInPairs=64', $text);
        $t->contains('Static native AST mapped parity: normalized-ast-equality-observed-not-runner-parity matches=64 mismatches=0 required=64', $text);
        $t->contains('Static executable native AST parity: normalized-ast-equality-observed-against-pandoc-executable matches=64 mismatches=0 required=64', $text);
        $t->contains('Runner status: not-run', $text);
        $t->contains('Runner plan: planned-not-run', $text);
    },
...
HS);
    ...
    'cli gates checked-in current pptx static evidence without upstream runner claim' => static function (TestRunner $t) use ($makeTempDir, $removeTree): void {
        ...
        $t->same(64, $summary['staticCurrentEvidence']['checkedInFixturePairCount']);
        $t->same(64, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(64, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(64, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['pandocNativeFixtureMatchCount']);
