...
return [
    ...
    'reports checked-in current upstream pptx static evidence gate' => static function (TestRunner $t): void {
        ...
        $t->same(62, $static['checkedInFixturePairCount']);
        ...
        $t->same(62, $nativeParity['requiredPairCount']);
        $t->same(62, $nativeParity['totalPairCount']);
        $t->same(62, $nativeParity['comparedPairCount']);
        $t->same(62, $nativeParity['pptxParsedCount']);
        $t->same(62, $nativeParity['nativeParsedCount']);
        $t->same(62, $nativeParity['bothParsedCount']);
        $t->same(0, $nativeParity['parseFailureCount']);
        $t->same(62, $nativeParity['normalizedAstMatchCount']);
        $t->same(0, $nativeParity['normalizedAstMismatchCount']);
        $t->same(62, count($nativeParity['fixtureComparisons']));
        ...
        $t->same(62, $executableParity['requiredPptxCount']);
        ...
        $t->same('4f086593de4c86391a556c31183d86224b572458e11556501dfcffa5bdcc5132', $executableParity['snapshotFile']['sha256']);
        $t->same(30840, $executableParity['snapshotFile']['bytes']);
        ...
        $t->same(62, $executableParity['totalPptxCount']);
        $t->same(62, $executableParity['comparedPptxCount']);
        ...
        $t->true(in_array('local PHP PPTX reader output matches all 62 checked-in current PPTX/native pairs by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        $t->true(in_array('checked-in executable native AST evidence shows pandoc 3.10, local PHP output, and paired .native fixtures match all 62 checked-in current PPTX fixtures by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        ...
        $t->contains('Static current evidence: valid-checked-in-current-pptx-reader-evidence comparisons=1 checkedInPairs=62', $text);
        $t->contains('Static native AST mapped parity: normalized-ast-equality-observed-not-runner-parity matches=62 mismatches=0 required=62', $text);
        $t->contains('Static executable native AST parity: normalized-ast-equality-observed-against-pandoc-executable matches=62 mismatches=0 required=62', $text);
        $t->contains('Runner status: not-run', $text);
        $t->contains('Runner plan: planned-not-run', $text);
    },
...
HS);
    ...
    'cli gates checked-in current pptx static evidence without upstream runner claim' => static function (TestRunner $t) use ($makeTempDir, $removeTree): void {
        ...
        $t->same(62, $summary['staticCurrentEvidence']['checkedInFixturePairCount']);
        $t->same(62, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(62, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(62, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['pandocNativeFixtureMatchCount']);
