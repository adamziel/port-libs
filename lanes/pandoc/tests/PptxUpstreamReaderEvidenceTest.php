...
return [
    ...
    'reports checked-in current upstream pptx static evidence gate' => static function (TestRunner $t): void {
        ...
        $t->same(55, $static['checkedInFixturePairCount']);
        ...
        $t->same(55, $nativeParity['requiredPairCount']);
        $t->same(55, $nativeParity['totalPairCount']);
        $t->same(55, $nativeParity['comparedPairCount']);
        $t->same(55, $nativeParity['pptxParsedCount']);
        $t->same(55, $nativeParity['nativeParsedCount']);
        $t->same(55, $nativeParity['bothParsedCount']);
        $t->same(0, $nativeParity['parseFailureCount']);
        $t->same(55, $nativeParity['normalizedAstMatchCount']);
        $t->same(0, $nativeParity['normalizedAstMismatchCount']);
        $t->same(55, count($nativeParity['fixtureComparisons']));
        ...
        $t->same(55, $executableParity['requiredPptxCount']);
        ...
        $t->same('fd0c8d2053ba6dbda6850e22861bc51a8b8c951dabf04293b8fc73c835297569', $executableParity['snapshotFile']['sha256']);
        $t->same(27560, $executableParity['snapshotFile']['bytes']);
        ...
        $t->same(55, $executableParity['totalPptxCount']);
        $t->same(55, $executableParity['comparedPptxCount']);
        ...
        $t->true(in_array('local PHP PPTX reader output matches all 55 checked-in current PPTX/native pairs by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        $t->true(in_array('checked-in executable native AST evidence shows pandoc 3.10, local PHP output, and paired .native fixtures match all 55 checked-in current PPTX fixtures by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        ...
        $t->contains('Static current evidence: valid-checked-in-current-pptx-reader-evidence comparisons=1 checkedInPairs=55', $text);
        $t->contains('Static native AST mapped parity: normalized-ast-equality-observed-not-runner-parity matches=55 mismatches=0 required=55', $text);
        $t->contains('Static executable native AST parity: normalized-ast-equality-observed-against-pandoc-executable matches=55 mismatches=0 required=55', $text);
        $t->contains('Runner status: not-run', $text);
        $t->contains('Runner plan: planned-not-run', $text);
    },
...
HS);
    ...
    'cli gates checked-in current pptx static evidence without upstream runner claim' => static function (TestRunner $t) use ($makeTempDir, $removeTree): void {
        ...
        $t->same(55, $summary['staticCurrentEvidence']['checkedInFixturePairCount']);
        $t->same(55, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(55, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(55, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['pandocNativeFixtureMatchCount']);
