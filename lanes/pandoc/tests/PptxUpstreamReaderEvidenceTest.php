...
return [
    ...
    'reports checked-in current upstream pptx static evidence gate' => static function (TestRunner $t): void {
        ...
        $t->same(63, $static['checkedInFixturePairCount']);
        ...
        $t->same(63, $nativeParity['requiredPairCount']);
        $t->same(63, $nativeParity['totalPairCount']);
        $t->same(63, $nativeParity['comparedPairCount']);
        $t->same(63, $nativeParity['pptxParsedCount']);
        $t->same(63, $nativeParity['nativeParsedCount']);
        $t->same(63, $nativeParity['bothParsedCount']);
        $t->same(0, $nativeParity['parseFailureCount']);
        $t->same(63, $nativeParity['normalizedAstMatchCount']);
        $t->same(0, $nativeParity['normalizedAstMismatchCount']);
        $t->same(63, count($nativeParity['fixtureComparisons']));
        ...
        $t->same(63, $executableParity['requiredPptxCount']);
        ...
        $t->same('b666b7cbe631cabf00608d693f6c77a330292a4a4ff34f856e5c03840814fe98', $executableParity['snapshotFile']['sha256']);
        $t->same(31316, $executableParity['snapshotFile']['bytes']);
        ...
        $t->same(63, $executableParity['totalPptxCount']);
        $t->same(63, $executableParity['comparedPptxCount']);
        ...
        $t->true(in_array('local PHP PPTX reader output matches all 63 checked-in current PPTX/native pairs by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        $t->true(in_array('checked-in executable native AST evidence shows pandoc 3.10, local PHP output, and paired .native fixtures match all 63 checked-in current PPTX fixtures by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        ...
        $t->contains('Static current evidence: valid-checked-in-current-pptx-reader-evidence comparisons=1 checkedInPairs=63', $text);
        $t->contains('Static native AST mapped parity: normalized-ast-equality-observed-not-runner-parity matches=63 mismatches=0 required=63', $text);
        $t->contains('Static executable native AST parity: normalized-ast-equality-observed-against-pandoc-executable matches=63 mismatches=0 required=63', $text);
        $t->contains('Runner status: not-run', $text);
        $t->contains('Runner plan: planned-not-run', $text);
    },
...
HS);
    ...
    'cli gates checked-in current pptx static evidence without upstream runner claim' => static function (TestRunner $t) use ($makeTempDir, $removeTree): void {
        ...
        $t->same(63, $summary['staticCurrentEvidence']['checkedInFixturePairCount']);
        $t->same(63, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(63, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(63, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['pandocNativeFixtureMatchCount']);
