...
return [
    ...
    'reports checked-in current upstream pptx static evidence gate' => static function (TestRunner $t): void {
        ...
        $t->same(59, $static['checkedInFixturePairCount']);
        ...
        $t->same(59, $nativeParity['requiredPairCount']);
        $t->same(59, $nativeParity['totalPairCount']);
        $t->same(59, $nativeParity['comparedPairCount']);
        $t->same(59, $nativeParity['pptxParsedCount']);
        $t->same(59, $nativeParity['nativeParsedCount']);
        $t->same(59, $nativeParity['bothParsedCount']);
        $t->same(0, $nativeParity['parseFailureCount']);
        $t->same(59, $nativeParity['normalizedAstMatchCount']);
        $t->same(0, $nativeParity['normalizedAstMismatchCount']);
        $t->same(59, count($nativeParity['fixtureComparisons']));
        ...
        $t->same(59, $executableParity['requiredPptxCount']);
        ...
        $t->same('0d2509974f2219b67a7f3c0382a038beb6a7ea37761dcb2c94d1016e81511c3e', $executableParity['snapshotFile']['sha256']);
        $t->same(29479, $executableParity['snapshotFile']['bytes']);
        ...
        $t->same(59, $executableParity['totalPptxCount']);
        $t->same(59, $executableParity['comparedPptxCount']);
        ...
        $t->true(in_array('local PHP PPTX reader output matches all 59 checked-in current PPTX/native pairs by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        $t->true(in_array('checked-in executable native AST evidence shows pandoc 3.10, local PHP output, and paired .native fixtures match all 59 checked-in current PPTX fixtures by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        ...
        $t->contains('Static current evidence: valid-checked-in-current-pptx-reader-evidence comparisons=1 checkedInPairs=59', $text);
        $t->contains('Static native AST mapped parity: normalized-ast-equality-observed-not-runner-parity matches=59 mismatches=0 required=59', $text);
        $t->contains('Static executable native AST parity: normalized-ast-equality-observed-against-pandoc-executable matches=59 mismatches=0 required=59', $text);
        $t->contains('Runner status: not-run', $text);
        $t->contains('Runner plan: planned-not-run', $text);
    },
...
HS);
    ...
    'cli gates checked-in current pptx static evidence without upstream runner claim' => static function (TestRunner $t) use ($makeTempDir, $removeTree): void {
        ...
        $t->same(59, $summary['staticCurrentEvidence']['checkedInFixturePairCount']);
        $t->same(59, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(59, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(59, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['pandocNativeFixtureMatchCount']);
