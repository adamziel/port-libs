...
return [
    ...
    'reports checked-in current upstream pptx static evidence gate' => static function (TestRunner $t): void {
        ...
        $t->same(80, $static['checkedInFixturePairCount']);
        ...
        $t->same(80, $nativeParity['requiredPairCount']);
        $t->same(80, $nativeParity['totalPairCount']);
        $t->same(80, $nativeParity['comparedPairCount']);
        $t->same(80, $nativeParity['pptxParsedCount']);
        $t->same(80, $nativeParity['nativeParsedCount']);
        $t->same(80, $nativeParity['bothParsedCount']);
        $t->same(0, $nativeParity['parseFailureCount']);
        $t->same(80, $nativeParity['normalizedAstMatchCount']);
        $t->same(0, $nativeParity['normalizedAstMismatchCount']);
        $t->same(80, count($nativeParity['fixtureComparisons']));
        ...
        $t->same(80, $executableParity['requiredPptxCount']);
        ...
        $t->same('35f71df710b79cd449fba4323c84d1b9e926493b5c466c7f986516cb2bef8323', $executableParity['snapshotFile']['sha256']);
        $t->same(35014, $executableParity['snapshotFile']['bytes']);
        ...
        $t->same(80, $executableParity['totalPptxCount']);
        $t->same(80, $executableParity['comparedPptxCount']);
        $t->same(80, $executableParity['localParsedCount']);
        $t->same(80, $executableParity['pandocParsedCount']);
        $t->same(80, $executableParity['nativeFixtureParsedCount']);
        $t->same(80, $executableParity['bothParsedCount']);
        $t->same(0, $executableParity['parseFailureCount']);
        $t->same(80, $executableParity['normalizedAstMatchCount']);
        $t->same(0, $executableParity['normalizedAstMismatchCount']);
        $t->same(80, $executableParity['pandocNativeFixtureComparedCount']);
        $t->same(80, $executableParity['pandocNativeFixtureMatchCount']);
        ...
        $t->true(in_array('local PHP PPTX reader output matches all 80 checked-in current PPTX/native pairs by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        $t->true(in_array('checked-in executable native AST evidence shows pandoc 3.10, local PHP output, and paired .native fixtures match all 80 checked-in current PPTX fixtures by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        ...
        $t->contains('Static current evidence: valid-checked-in-current-pptx-reader-evidence comparisons=1 checkedInPairs=80', $text);
        $t->contains('Static native AST mapped parity: normalized-ast-equality-observed-not-runner-parity matches=80 mismatches=0 required=80', $text);
        $t->contains('Static executable native AST parity: normalized-ast-equality-observed-against-pandoc-executable matches=80 mismatches=0 required=80', $text);
        $t->contains('Runner status: not-run', $text);
        $t->contains('Runner plan: planned-not-run', $text);
    },
...
HS);
    ...
    'cli gates checked-in current pptx static evidence without upstream runner claim' => static function (TestRunner $t) use ($makeTempDir, $removeTree): void {
        ...
        $t->same(80, $summary['staticCurrentEvidence']['checkedInFixturePairCount']);
        $t->same(80, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(80, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(80, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['pandocNativeFixtureMatchCount']);
