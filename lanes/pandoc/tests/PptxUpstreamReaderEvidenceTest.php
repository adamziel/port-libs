...
return [
    ...
    'reports checked-in current upstream pptx static evidence gate' => static function (TestRunner $t): void {
        ...
        $t->same(86, $static['checkedInFixturePairCount']);
        ...
        $t->same(86, $nativeParity['requiredPairCount']);
        $t->same(86, $nativeParity['totalPairCount']);
        $t->same(86, $nativeParity['comparedPairCount']);
        $t->same(86, $nativeParity['pptxParsedCount']);
        $t->same(86, $nativeParity['nativeParsedCount']);
        $t->same(86, $nativeParity['bothParsedCount']);
        $t->same(0, $nativeParity['parseFailureCount']);
        $t->same(86, $nativeParity['normalizedAstMatchCount']);
        $t->same(0, $nativeParity['normalizedAstMismatchCount']);
        $t->same(86, count($nativeParity['fixtureComparisons']));
        ...
        $t->same(86, $executableParity['requiredPptxCount']);
        ...
        $t->same('d2abb993ce8b993860b521d71c13620465c24d6680ab6f3b4b9b616dfb7d5263', $executableParity['snapshotFile']['sha256']);
        $t->same(37452, $executableParity['snapshotFile']['bytes']);
        ...
        $t->same(86, $executableParity['totalPptxCount']);
        $t->same(86, $executableParity['comparedPptxCount']);
        $t->same(86, $executableParity['localParsedCount']);
        $t->same(86, $executableParity['pandocParsedCount']);
        $t->same(86, $executableParity['nativeFixtureParsedCount']);
        $t->same(86, $executableParity['bothParsedCount']);
        $t->same(0, $executableParity['parseFailureCount']);
        $t->same(86, $executableParity['normalizedAstMatchCount']);
        $t->same(0, $executableParity['normalizedAstMismatchCount']);
        $t->same(86, $executableParity['pandocNativeFixtureComparedCount']);
        $t->same(86, $executableParity['pandocNativeFixtureMatchCount']);
        ...
        $t->true(in_array('local PHP PPTX reader output matches all 86 checked-in current PPTX/native pairs by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        $t->true(in_array('checked-in executable native AST evidence shows pandoc 3.10, local PHP output, and paired .native fixtures match all 86 checked-in current PPTX fixtures by normalized AST shape', $static['claimBoundaries']['doesAssert'], true));
        ...
        $t->contains('Static current evidence: valid-checked-in-current-pptx-reader-evidence comparisons=1 checkedInPairs=86', $text);
        $t->contains('Static native AST mapped parity: normalized-ast-equality-observed-not-runner-parity matches=86 mismatches=0 required=86', $text);
        $t->contains('Static executable native AST parity: normalized-ast-equality-observed-against-pandoc-executable matches=86 mismatches=0 required=86', $text);
        $t->contains('Runner status: not-run', $text);
        $t->contains('Runner plan: planned-not-run', $text);
    },
...
HS);
    ...
    'cli gates checked-in current pptx static evidence without upstream runner claim' => static function (TestRunner $t) use ($makeTempDir, $removeTree): void {
        ...
        $t->same(86, $summary['staticCurrentEvidence']['checkedInFixturePairCount']);
        $t->same(86, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['nativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(86, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMatchCount']);
        $t->same(0, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['normalizedAstMismatchCount']);
        $t->same(86, $summary['staticCurrentEvidence']['executableNativeAstMappedParity']['pandocNativeFixtureMatchCount']);
