...
return [
    ...
    'matches checked-in current pptx reader fixture pairs through normalized ast harness' => static function (TestRunner $t): void {
        ...
        $t->same('completed', $report['status']);
        $t->same(17, $report['totalPairCount']);
        $t->same(17, $report['comparedPairCount']);
        $t->same(17, $report['pptxParsedCount']);
        $t->same(17, $report['nativeParsedCount']);
        $t->same(17, $report['bothParsedCount']);
        $t->same(0, $report['parseFailureCount']);
        $t->same(17, $report['normalizedAstMatchCount']);
        $t->same(0, $report['normalizedAstMismatchCount']);
        $t->same(true, PptxNativeAstComparisonHarness::hasRequiredMappedParity($report, 17));
    },
    'cli gates required mapped pptx parity from checked-in fixture selector' => static function (TestRunner $t): void {
        ...
        $t->same(true, PptxNativeAstComparisonHarness::hasRequiredMappedParity($decoded, 17));
    },
