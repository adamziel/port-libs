...
return [
    ...
    'matches checked-in current pptx reader fixture pairs through normalized ast harness' => static function (TestRunner $t): void {
        ...
        $t->same('completed', $report['status']);
        $t->same(15, $report['totalPairCount']);
        $t->same(15, $report['comparedPairCount']);
        $t->same(15, $report['pptxParsedCount']);
        $t->same(15, $report['nativeParsedCount']);
        $t->same(15, $report['bothParsedCount']);
        $t->same(0, $report['parseFailureCount']);
        $t->same(15, $report['normalizedAstMatchCount']);
        $t->same(0, $report['normalizedAstMismatchCount']);
        $t->same(true, PptxNativeAstComparisonHarness::hasRequiredMappedParity($report, 15));
    },
    'cli gates required mapped pptx parity from checked-in fixture selector' => static function (TestRunner $t): void {
        ...
        $t->same(true, PptxNativeAstComparisonHarness::hasRequiredMappedParity($decoded, 15));
    },
