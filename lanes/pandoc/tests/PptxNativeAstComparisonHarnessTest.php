...
return [
    ...
    'matches checked-in current pptx reader fixture pairs through normalized ast harness' => static function (TestRunner $t): void {
        ...
        $t->same(105, $report['pptxParsedCount']);
        ...
        $t->same(0, $report['normalizedAstMismatchCount']);
        $t->same(105, count($report['fixtureComparisons']));
        ...
        $t->same('selected-pptx-native-fixture-corpus-coverage', $report['orderedRemainingGaps'][2]['id']);
        $t->same('covered-by-current-selected-corpus-evidence', $report['orderedRemainingGaps'][2]['status']);
        $t->same(true, PptxNativeAstComparisonHarness::hasRequiredMappedParity($report, 105));
    },
    'cli gates required mapped pptx parity from checked-in fixture selector' => static function (TestRunner $t): void {
        ...
        $t->same(true, PptxNativeAstComparisonHarness::hasRequiredMappedParity($decoded, 105));
    },
