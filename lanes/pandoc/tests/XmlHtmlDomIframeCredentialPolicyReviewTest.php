...
return [
    'summarizes iframe credentialless and csp policy provenance for reviewer handoff' => static function (TestRunner $t): void {
        ...
        $t->same('iframe-credentialless-attribute-review', $frame['iframeCredentiallessReviewPolicy']);
        $t->same('', $frame['iframeCredentiallessRaw']);
        $t->same(true, $frame['iframeCredentialless']);
        $t->same('credentialless', $frame['iframeCredentialMode']);
        $t->same($policy, $frame['csp']);
        $t->same('iframe-csp-attribute-review', $frame['iframeCspReviewPolicy']);
        ...
        $t->same('meta-content-security-policy-review', $frame['contentSecurityPolicyReviewPolicy']);
        $t->same('iframe-csp', $frame['contentSecurityPolicyHttpEquiv']);
        ...
        $t->same(['invalid-iframe-csp-policy'], $bad['iframePolicyIssueCodes']);
