...
<office:document-content
  ...
  <office:body>
    <office:text>
      <text:p>Package path-depth role buckets.</text:p>
    </office:text>
  </office:body>
</office:document-content>
XML;
...
<office:document-meta
  ...
  <office:meta>
    <dc:title>Package path-depth role buckets</dc:title>
  </office:meta>
</office:document-meta>
XML;
...
], 'odt package path depth role buckets');

return [
    'summarizes ODT package path depths by package role and byte exposure' => static function (TestRunner $t) use ($buildPackage): void {
        ...
        foreach ([$compactInventory, $compactIdentity, $richProvenance, $richIdentity, $documentIdentity] as $handoff) {
            $t->same($expectedRoleCounts, $handoff['packagePathDepthRoleCounts']);
            $t->same($expectedPolicyCounts, $handoff['packagePathDepthByteExposurePolicyCounts']);
            $t->same(
                ['Basic/Standard/Review.xml', 'Configurations2/accelerator/current.xml'],
                $handoff['entryNamesByPackagePathDepthRole'][3]['manifest-declared']
            );
            $t->same(
                ['Configurations2/accelerator/current.xml'],
                $handoff['entryNamesByPackagePathDepthRole'][3]['configuration-package']
            );
            $t->same(
                ['Notes/private'],
                $handoff['entryNamesByPackagePathDepthByteExposurePolicy'][2]['undeclared-package-entry-no-bytes']
            );
            $t->same(
                ['layout-cache'],
                $handoff['entryNamesByPackagePathDepthByteExposurePolicy'][1]['encrypted-resource-bytes-blocked']
            );
        }

        $t->same($compactInventory['packagePathDepthRoleCounts'], $richProvenance['packagePathDepthRoleCounts']);
        $t->same($compactIdentity['entryNamesByPackagePathDepthRole'], $richIdentity['entryNamesByPackagePathDepthRole']);
        $t->same($richProvenance['packagePathDepthByteExposurePolicyCounts'], $documentProvenance['packagePathDepthByteExposurePolicyCounts']);
        $t->same($richIdentity['entryNamesByPackagePathDepthByteExposurePolicy'], $documentIdentity['entryNamesByPackagePathDepthByteExposurePolicy']);
    },
];
