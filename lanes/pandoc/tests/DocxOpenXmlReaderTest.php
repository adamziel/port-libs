...
XML;
    ...
    'summarizes docx package part name character provenance for review handoff' => static function (TestRunner $t): void {
        ...
        $inventory = $package['parts'];
        $byPartName = [];
        foreach ($summary['partNameCharacterReviewParts'] as $item) {
            $byPartName[$item['partName']] = $item;
        }

        $t->same(5, $summary['partNameCharacterReviewPartCount']);
        $t->same(3, $summary['partNameUppercasePartCount']);
        $t->same(1, $summary['partNameWhitespacePartCount']);
        $t->same(1, $summary['partNamePercentEncodedOctetPartCount']);
        $t->same(1, $summary['partNameNonAsciiPartCount']);
        $t->same([
            'non-ascii' => 1,
            'percent-encoded-octet' => 1,
            'uppercase' => 3,
            'whitespace' => 1,
        ], $summary['partNameCharacterFlagCounts']);
        $t->same(['[Content_Types].xml', 'docProps/core.xml', $mixedCaseWhitespacePart], $summary['partNameCharacterFlagPartNames']['uppercase']);
        $t->same([$mixedCaseWhitespacePart], $summary['partNameCharacterFlagPartNames']['whitespace']);
        $t->same([$percentEncodedPart], $summary['partNameCharacterFlagPartNames']['percent-encoded-octet']);
        $t->same([$nonAsciiPart], $summary['partNameCharacterFlagPartNames']['non-ascii']);

        $t->same(['uppercase'], $inventory['[Content_Types].xml']['partNameCharacterFlags']);
        $t->same(true, $inventory['[Content_Types].xml']['partNameHasUppercase']);
        $t->same(false, $inventory['[Content_Types].xml']['partNameHasWhitespace']);
        $t->same(['uppercase', 'whitespace'], $inventory[$mixedCaseWhitespacePart]['partNameCharacterFlags']);
        $t->same(true, $inventory[$mixedCaseWhitespacePart]['partNameHasUppercase']);
        $t->same(true, $inventory[$mixedCaseWhitespacePart]['partNameHasWhitespace']);
        $t->same(false, $inventory[$mixedCaseWhitespacePart]['partNameHasPercentEncodedOctet']);
        $t->same(false, $inventory[$mixedCaseWhitespacePart]['partNameHasNonAscii']);
        $t->same('png', $inventory[$mixedCaseWhitespacePart]['partExtension']);
        $t->same('image/png', $inventory[$mixedCaseWhitespacePart]['contentTypeBase']);
        $t->same('default', $inventory[$mixedCaseWhitespacePart]['contentTypeSource']);

        $t->same(['percent-encoded-octet'], $inventory[$percentEncodedPart]['partNameCharacterFlags']);
        $t->same(true, $inventory[$percentEncodedPart]['partNameHasPercentEncodedOctet']);
        $t->same(false, $inventory[$percentEncodedPart]['partNameHasWhitespace']);
        $t->same('word/data', $inventory[$percentEncodedPart]['directory']);
        $t->same('application/xml', $inventory[$percentEncodedPart]['contentTypeBase']);

        $t->same(['non-ascii'], $inventory[$nonAsciiPart]['partNameCharacterFlags']);
        $t->same(true, $inventory[$nonAsciiPart]['partNameHasNonAscii']);
        $t->same(false, $inventory[$nonAsciiPart]['partNameHasPercentEncodedOctet']);
        $t->same('word/data', $inventory[$nonAsciiPart]['directory']);
        $t->same('application/xml', $inventory[$nonAsciiPart]['contentTypeBase']);
        ...
        $t->same(['uppercase', 'whitespace'], $byPartName[$mixedCaseWhitespacePart]['flags']);
        $t->same('word/media', $byPartName[$mixedCaseWhitespacePart]['directory']);
        $t->same('Review Draft.PNG', $byPartName[$mixedCaseWhitespacePart]['baseName']);
        $t->same(3, $byPartName[$mixedCaseWhitespacePart]['pathSegmentCount']);
        $t->same('default', $byPartName[$mixedCaseWhitespacePart]['contentTypeSource']);
        $t->same(['package-part'], $byPartName[$mixedCaseWhitespacePart]['roles']);
        $t->same(['percent-encoded-octet'], $byPartName[$percentEncodedPart]['flags']);
        $t->same(['non-ascii'], $byPartName[$nonAsciiPart]['flags']);
    },
...
XML;
    ...
    'summarizes docx relationship source kind matrix for package review' => static function (TestRunner $t): void {
        $parts = docx_openxml_reader_fixture_parts();
        $parts['root-data.xml'] = '<rootData/>';
        $parts['word/source.xml'] = '<source/>';
        $parts['word/media/root-sidecar.png'] = 'root sidecar image bytes';
        $parts['word/media/content-types.png'] = 'content types source image bytes';
        $parts['word/media/source.png'] = 'source image bytes';
        $parts['word/media/missing-source.png'] = 'missing source image bytes';
        $parts['word/media/relationship-source.png'] = 'relationship source image bytes';
        $parts['word/media/invalid-source.png'] = 'invalid source image bytes';
        $parts['_rels/root-data.xml.rels'] = <<<'XML'
<?xml version="1.0" encoding="UTF-8"?>
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rRootSidecarImage" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="word/media/root-sidecar.png"/>
</Relationships>
XML;
        $parts['_rels/[Content_Types].xml.rels'] = <<<'XML'
<?xml version="1.0" encoding="UTF-8"?>
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rContentTypesImage" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="word/media/content-types.png"/>
</Relationships>
XML;
        $parts['word/_rels/source.xml.rels'] = <<<'XML'
<?xml version="1.0" encoding="UTF-8"?>
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rSourceImage" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="media/source.png"/>
</Relationships>
XML;
...
<?xml version="1.0" encoding="UTF-8"?>
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rRelationshipSourceImage" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="../media/relationship-source.png"/>
</Relationships>
XML;
        $parts['word/_rels/media/document.xml.rels'] = <<<'XML'
<?xml version="1.0" encoding="UTF-8"?>
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rInvalidSourceImage" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="word/media/invalid-source.png"/>
</Relationships>
XML;
        ...
        $root = $relationshipParts['_rels/.rels'];
        $documentRels = $relationshipParts['word/_rels/document.xml.rels'];
        $rootData = $relationshipParts['_rels/root-data.xml.rels'];
        $contentTypes = $relationshipParts['_rels/[Content_Types].xml.rels'];
        $source = $relationshipParts['word/_rels/source.xml.rels'];
        $missing = $relationshipParts['word/_rels/missing-source.xml.rels'];
        $relationshipSource = $relationshipParts['word/_rels/_rels/document.xml.rels.rels'];
        $invalid = $relationshipParts['word/_rels/media/document.xml.rels'];
        ...
        $t->same([
            ...
            'package-root' => 1,
            'relationship-part' => 1,
        ], $summary['relationshipSourceKindCounts']);
        $t->same(1, $summary['relationshipSourcePackageRootCount']);
        $t->same(3, $summary['relationshipSourcePackagePartCount']);
        $t->same(1, $summary['relationshipSourceContentTypesItemCount']);
        $t->same(1, $summary['relationshipSourceMissingCount']);
        $t->same(1, $summary['relationshipSourceRelationshipPartCount']);
        $t->same(1, $summary['relationshipSourceInvalidCount']);
        $t->same([
            'content-types-item' => ['_rels/[Content_Types].xml.rels'],
            'invalid-source' => ['word/_rels/media/document.xml.rels'],
            'missing-source' => ['word/_rels/missing-source.xml.rels'],
            'package-part' => ['_rels/root-data.xml.rels', 'word/_rels/document.xml.rels', 'word/_rels/source.xml.rels'],
            'package-root' => ['_rels/.rels'],
            'relationship-part' => ['word/_rels/_rels/document.xml.rels.rels'],
        ], $summary['relationshipPartsBySourceKind']);
        $t->same([
            '/' => 3,
            'word' => 3,
            'word/_rels' => 1,
        ], $summary['relationshipSourceDirectoryCounts']);
        $t->same([
            'application/vnd.openxmlformats-officedocument.wordprocessingml.document.main+xml' => 1,
            'application/vnd.openxmlformats-package.relationships+xml' => 1,
            'application/xml' => 3,
        ], $summary['relationshipSourceContentTypeCounts']);
        $t->same(['default' => 4, 'override' => 1], $summary['relationshipSourceContentTypeSourceCounts']);
        $t->same(2, $summary['relationshipPartMissingSourceCount']);

        $t->same('package-root', $root['relationshipSourceKind']);
        $t->same('/', $root['sourcePart']);
        $t->same(['package-root'], $root['sourceRoles']);
        $t->same('package-part', $documentRels['relationshipSourceKind']);
        $t->same('word/document.xml', $documentRels['sourcePart']);
        $t->same('application/vnd.openxmlformats-officedocument.wordprocessingml.document.main+xml', $documentRels['sourceContentTypeBase']);
        $t->true(in_array('office-document', $documentRels['sourceRoles'], true), 'document relationship source role missing');
        $t->same('package-part', $rootData['relationshipSourceKind']);
        $t->same('root-data.xml', $rootData['sourcePart']);
        $t->same('/', $rootData['sourceDirectory']);
        $t->same('application/xml', $rootData['sourceContentTypeBase']);
        $t->same(['package-part'], $rootData['sourceRoles']);
        $t->same('content-types-item', $contentTypes['relationshipSourceKind']);
        $t->same('[Content_Types].xml', $contentTypes['sourcePart']);
        $t->same(['content-types'], $contentTypes['sourceRoles']);
        $t->same('package-part', $source['relationshipSourceKind']);
        $t->same('word/source.xml', $source['sourcePart']);
        $t->same('missing-source', $missing['relationshipSourceKind']);
        $t->same(false, $missing['sourceExists']);
        $t->same([], $missing['sourceRoles']);
        ...
        $t->same(true, $relationshipSource['sourceIsRelationshipPart']);
        $t->true(in_array('office-document-relationships', $relationshipSource['sourceRoles'], true), 'relationship-part source role missing');
        $t->same('invalid-source', $invalid['relationshipSourceKind']);
        $t->same('', $invalid['sourcePart']);
        $t->same(false, $invalid['sourceExists']);
        $t->same('word/media/root-sidecar.png', $rootData['relationships']['rRootSidecarImage']['targetPart']);
        $t->same('content-types-item', $contentTypes['relationships']['rContentTypesImage']['relationshipSourceKind']);
    ...
    'summarizes docx relationship source directory buckets for package review' => static function (TestRunner $t): void {
        $parts = docx_openxml_reader_fixture_parts();
        $parts['word/source-a.xml'] = '<wordSourceA/>';
        $parts['word/source-b.xml'] = '<wordSourceB/>';
        $parts['customXml/source-a.xml'] = '<customSourceA/>';
        $parts['word/media/source-a.png'] = 'word source a image bytes';
        $parts['word/media/source-b.png'] = 'word source b image bytes';
        $parts['word/media/custom-source-a.png'] = 'custom source a image bytes';
        $parts['word/media/missing-source.png'] = 'missing source image bytes';
        $parts['word/media/relationship-source-directory.png'] = 'relationship source directory image bytes';
        $parts['word/media/invalid-source-directory.png'] = 'invalid source directory image bytes';
        $parts['word/_rels/source-a.xml.rels'] = <<<'XML'
<?xml version="1.0" encoding="UTF-8"?>
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rWordSourceAImage" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="media/source-a.png"/>
</Relationships>
XML;
        $parts['word/_rels/source-b.xml.rels'] = <<<'XML'
<?xml version="1.0" encoding="UTF-8"?>
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rWordSourceBImage" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="media/source-b.png"/>
</Relationships>
XML;
        $parts['customXml/_rels/source-a.xml.rels'] = <<<'XML'
<?xml version="1.0" encoding="UTF-8"?>
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rCustomSourceAImage" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="../word/media/custom-source-a.png"/>
</Relationships>
XML;
        $parts['word/_rels/missing-source.xml.rels'] = <<<'XML'
<?xml version="1.0" encoding="UTF-8"?>
...
<?xml version="1.0" encoding="UTF-8"?>
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rRelationshipSourceImage" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="../media/relationship-source-directory.png"/>
</Relationships>
XML;
        $parts['word/_rels/media/document.xml.rels'] = <<<'XML'
<?xml version="1.0" encoding="UTF-8"?>
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rInvalidSourceImage" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="word/media/invalid-source-directory.png"/>
</Relationships>
XML;
        ...
        $summary = $document->attr('docx')['packageProvenance']['summary'];
        $byDirectory = [];
        foreach ($summary['relationshipSourceDirectories'] as $directorySummary) {
            $byDirectory[$directorySummary['sourceDirectoryKey']] = $directorySummary;
        }

        $t->same(5, $summary['relationshipSourceDirectoryCount']);
        $t->same([
            '/' => 1,
            'customXml' => 1,
            'word' => 4,
        ...
        $t->same(['(invalid-source)', '/', 'customXml', 'word', 'word/_rels'], array_column($summary['relationshipSourceDirectories'], 'sourceDirectoryKey'));
        ...
        $t->same(['(none)' => 1], $root['sourcePartExtensionCounts']);
        $t->same(['_rels/.rels'], $root['relationshipParts']);

        $word = $byDirectory['word'];
        ...
        $t->same(
            strlen($parts['word/document.xml']) + strlen($parts['word/source-a.xml']) + strlen($parts['word/source-b.xml']),
            $word['existingSourceByteLength']
        );
        $t->same(['missing-source' => 1, 'package-part' => 3], $word['relationshipSourceKindCounts']);
        $t->same([
            'document.xml' => 1,
            'missing-source.xml' => 1,
            'source-a.xml' => 1,
            'source-b.xml' => 1,
        ], $word['sourceBaseNameCounts']);
        $t->same(['xml' => 4], $word['sourcePartExtensionCounts']);
        $t->same([
            '(missing)' => 1,
            'application/vnd.openxmlformats-officedocument.wordprocessingml.document.main+xml' => 1,
            'application/xml' => 2,
        ...
        $t->same(['word/document.xml', 'word/missing-source.xml', 'word/source-a.xml', 'word/source-b.xml'], $word['sourceParts']);
        $t->same([
            'word/_rels/document.xml.rels',
            'word/_rels/missing-source.xml.rels',
            'word/_rels/source-a.xml.rels',
            'word/_rels/source-b.xml.rels',
        ], $word['relationshipParts']);

        $customXml = $byDirectory['customXml'];
        $t->same(1, $customXml['sourceCount']);
        $t->same(strlen($parts['customXml/source-a.xml']), $customXml['existingSourceByteLength']);
        $t->same(['package-part' => 1], $customXml['relationshipSourceKindCounts']);
        $t->same(['source-a.xml' => 1], $customXml['sourceBaseNameCounts']);
        $t->same(['customXml/source-a.xml'], $customXml['sourceParts']);

        $relationshipSource = $byDirectory['word/_rels'];
        $t->same(1, $relationshipSource['sourceCount']);
        $t->same(['relationship-part' => 1], $relationshipSource['relationshipSourceKindCounts']);
        $t->same(['document.xml.rels' => 1], $relationshipSource['sourceBaseNameCounts']);
        $t->same(['rels' => 1], $relationshipSource['sourcePartExtensionCounts']);
        $t->same(['word/_rels/document.xml.rels'], $relationshipSource['sourceParts']);

        $invalid = $byDirectory['(invalid-source)'];
        $t->same(null, $invalid['sourceDirectory']);
        ...
        $t->same(['(none)' => 1], $invalid['sourcePartExtensionCounts']);
        $t->same(['word/_rels/media/document.xml.rels'], $invalid['relationshipParts']);
    },
    'summarizes docx relationship source content type buckets for package review' => static function (TestRunner $t): void {
        $parts = docx_openxml_reader_fixture_parts();
        $parts['[Content_Types].xml'] = str_replace(
            '</Types>',
            '  <Override PartName="/word/source-override.xml" ContentType="application/vnd.example.source+xml; profile=source-review"/>' . "\n" .
            '</Types>',
            $parts['[Content_Types].xml']
        );
        $parts['root-source.xml'] = '<rootSource/>';
        $parts['word/source-override.xml'] = '<overrideSource/>';
        $parts['customXml/source.xml'] = '<customSource/>';
        $parts['word/media/root-source-type.png'] = 'root source type image bytes';
        $parts['word/media/content-types-source-type.png'] = 'content types source image bytes';
        $parts['word/media/override-source-type.png'] = 'override source image bytes';
        $parts['word/media/custom-source-type.png'] = 'custom source image bytes';
        $parts['word/media/missing-source-type.png'] = 'missing source type image bytes';
        $parts['word/media/relationship-source-type.png'] = 'relationship source type image bytes';
        $parts['word/media/invalid-source-type.png'] = 'invalid source type image bytes';
        $parts['_rels/root-source.xml.rels'] = <<<'XML'
<?xml version="1.0" encoding="UTF-8"?>
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rRootSourceTypeImage" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="word/media/root-source-type.png"/>
</Relationships>
XML;
        $parts['_rels/[Content_Types].xml.rels'] = <<<'XML'
<?xml version="1.0" encoding="UTF-8"?>
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rContentTypesSourceTypeImage" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="word/media/content-types-source-type.png"/>
</Relationships>
XML;
        $parts['word/_rels/source-override.xml.rels'] = <<<'XML'
<?xml version="1.0" encoding="UTF-8"?>
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rOverrideSourceTypeImage" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="media/override-source-type.png"/>
</Relationships>
XML;
        $parts['customXml/_rels/source.xml.rels'] = <<<'XML'
<?xml version="1.0" encoding="UTF-8"?>
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rCustomSourceTypeImage" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="../word/media/custom-source-type.png"/>
</Relationships>
XML;
        $parts['word/_rels/missing-source-type.xml.rels'] = <<<'XML'
<?xml version="1.0" encoding="UTF-8"?>
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rMissingSourceTypeImage" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="media/missing-source-type.png"/>
</Relationships>
XML;
        $parts['word/_rels/_rels/document.xml.rels.rels'] = <<<'XML'
<?xml version="1.0" encoding="UTF-8"?>
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rRelationshipSourceTypeImage" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="../media/relationship-source-type.png"/>
</Relationships>
XML;
        $parts['word/_rels/media/document.xml.rels'] = <<<'XML'
<?xml version="1.0" encoding="UTF-8"?>
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rInvalidSourceTypeImage" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="word/media/invalid-source-type.png"/>
</Relationships>
XML;
        ...
        $summary = $document->attr('docx')['packageProvenance']['summary'];
        $byContentType = [];
        foreach ($summary['relationshipSourceContentTypes'] as $contentTypeSummary) {
            $byContentType[$contentTypeSummary['sourceContentTypeKey']] = $contentTypeSummary;
        }

        $t->same(9, $summary['relationshipSourceCount']);
        $t->same(5, $summary['relationshipSourceContentTypeBucketCount']);
        $t->same([
            '(missing)',
            'application/vnd.example.source+xml',
            'application/vnd.openxmlformats-officedocument.wordprocessingml.document.main+xml',
            'application/vnd.openxmlformats-package.relationships+xml',
            'application/xml',
        ], array_keys($byContentType));
        $t->same([
            'application/vnd.example.source+xml' => 1,
            'application/vnd.openxmlformats-officedocument.wordprocessingml.document.main+xml' => 1,
            'application/vnd.openxmlformats-package.relationships+xml' => 1,
            'application/xml' => 3,
        ], $summary['relationshipSourceContentTypeCounts']);
        ...
        $t->same(null, $missing['sourceContentTypeBase']);
        ...
        $t->same(4, $missing['relationshipRecordCount']);
        ...
        $t->same(['(missing)' => 3], $missing['sourceContentTypeSourceCounts']);
        ...
        $t->same(['/', 'word'], $missing['sourceDirectories']);
        $t->same(['/', 'word/missing-source-type.xml'], $missing['sourceParts']);
        $t->same([
            '_rels/.rels',
            'word/_rels/media/document.xml.rels',
            'word/_rels/missing-source-type.xml.rels',
        ], $missing['relationshipParts']);

        $xml = $byContentType['application/xml'];
        $t->same('application/xml', $xml['sourceContentTypeBase']);
        $t->same(['application/xml'], $xml['sourceContentTypes']);
        $t->same(3, $xml['sourceCount']);
        $t->same(3, $xml['existingSourceCount']);
        $t->same(0, $xml['nonExistingSourceCount']);
        $t->same(3, $xml['relationshipCount']);
        $t->same(strlen($parts['[Content_Types].xml']) + strlen($parts['customXml/source.xml']) + strlen($parts['root-source.xml']), $xml['existingSourceByteLength']);
        $t->same(['content-types-item' => 1, 'package-part' => 2], $xml['relationshipSourceKindCounts']);
        $t->same(['default' => 3], $xml['sourceContentTypeSourceCounts']);
        $t->same(['[Content_Types].xml' => 1, 'root-source.xml' => 1, 'source.xml' => 1], $xml['sourceBaseNameCounts']);
        $t->same(['xml' => 3], $xml['sourcePartExtensionCounts']);
        $t->same(['/', 'customXml'], $xml['sourceDirectories']);
        $t->same(['[Content_Types].xml', 'customXml/source.xml', 'root-source.xml'], $xml['sourceParts']);
        $t->same([
            '_rels/[Content_Types].xml.rels',
            '_rels/root-source.xml.rels',
            'customXml/_rels/source.xml.rels',
        ], $xml['relationshipParts']);

        $override = $byContentType['application/vnd.example.source+xml'];
        $t->same('application/vnd.example.source+xml', $override['sourceContentTypeBase']);
        $t->same(['application/vnd.example.source+xml; profile=source-review'], $override['sourceContentTypes']);
        $t->same(1, $override['sourceCount']);
        $t->same(1, $override['relationshipCount']);
        $t->same(['package-part' => 1], $override['relationshipSourceKindCounts']);
        $t->same(['override' => 1], $override['sourceContentTypeSourceCounts']);
        $t->same(['source-override.xml' => 1], $override['sourceBaseNameCounts']);
        $t->same(['xml' => 1], $override['sourcePartExtensionCounts']);
        $t->same(['word/source-override.xml'], $override['sourceParts']);

        $documentMain = $byContentType['application/vnd.openxmlformats-officedocument.wordprocessingml.document.main+xml'];
        $t->same(1, $documentMain['sourceCount']);
        $t->same(2, $documentMain['relationshipCount']);
        $t->same(['package-part' => 1], $documentMain['relationshipSourceKindCounts']);
        $t->same(['override' => 1], $documentMain['sourceContentTypeSourceCounts']);
        $t->same(['document.xml' => 1], $documentMain['sourceBaseNameCounts']);
        $t->same(['word/document.xml'], $documentMain['sourceParts']);

        $relationships = $byContentType['application/vnd.openxmlformats-package.relationships+xml'];
        $t->same(1, $relationships['sourceCount']);
        $t->same(1, $relationships['relationshipCount']);
        $t->same(['relationship-part' => 1], $relationships['relationshipSourceKindCounts']);
        $t->same(['default' => 1], $relationships['sourceContentTypeSourceCounts']);
        $t->same(['document.xml.rels' => 1], $relationships['sourceBaseNameCounts']);
    ...
    'summarizes docx relationship source filename buckets for package review' => static function (TestRunner $t): void {
        $parts = docx_openxml_reader_fixture_parts();
        $parts['root-source.xml'] = '<rootSource/>';
        $parts['word/source-a.xml'] = '<wordSource/>';
        $parts['customXml/source-a.xml'] = '<customSource/>';
        $parts['word/media/root-source.png'] = 'root source image bytes';
        $parts['word/media/source-a.png'] = 'word source image bytes';
        $parts['word/media/custom-source-a.png'] = 'custom source image bytes';
        $parts['word/media/missing-source-a.png'] = 'missing source image bytes';
        $parts['word/media/relationship-source-filename.png'] = 'relationship source filename image bytes';
        $parts['word/media/invalid-source-filename.png'] = 'invalid source filename image bytes';
        $parts['_rels/root-source.xml.rels'] = <<<'XML'
<?xml version="1.0" encoding="UTF-8"?>
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rRootSourceImage" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="word/media/root-source.png"/>
</Relationships>
XML;
        $parts['word/_rels/source-a.xml.rels'] = <<<'XML'
<?xml version="1.0" encoding="UTF-8"?>
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rWordSourceImage" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="media/source-a.png"/>
</Relationships>
XML;
        $parts['customXml/_rels/source-a.xml.rels'] = <<<'XML'
<?xml version="1.0" encoding="UTF-8"?>
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rCustomSourceImage" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="../word/media/custom-source-a.png"/>
</Relationships>
XML;
        $parts['word/_rels/missing-source-a.xml.rels'] = <<<'XML'
<?xml version="1.0" encoding="UTF-8"?>
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rMissingSourceImage" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="media/missing-source-a.png"/>
</Relationships>
XML;
        $parts['word/_rels/_rels/document.xml.rels.rels'] = <<<'XML'
<?xml version="1.0" encoding="UTF-8"?>
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rRelationshipSourceImage" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="../media/relationship-source-filename.png"/>
</Relationships>
XML;
        $parts['word/_rels/media/document.xml.rels'] = <<<'XML'
<?xml version="1.0" encoding="UTF-8"?>
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rInvalidSourceImage" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="word/media/invalid-source-filename.png"/>
</Relationships>
XML;
        ...
        $byExtension = [];
        foreach ($summary['relationshipSourcePartExtensions'] as $extensionSummary) {
            $byExtension[$extensionSummary['sourcePartExtensionKey']] = $extensionSummary;
        }

        $t->same(7, $summary['relationshipSourceBaseNameCount']);
        $t->same(1, $summary['duplicateRelationshipSourceBaseNameCount']);
        $t->same(['source-a.xml'], $summary['duplicateRelationshipSourceBaseNames']);
        $t->same([
            '(invalid-source)' => 1,
            '/' => 1,
            'document.xml' => 1,
            'document.xml.rels' => 1,
            'missing-source-a.xml' => 1,
            'root-source.xml' => 1,
            'source-a.xml' => 2,
        ], $summary['relationshipSourceBaseNameCounts']);
        $t->same(3, $summary['relationshipSourcePartExtensionCount']);
        $t->same(['(none)' => 2, 'rels' => 1, 'xml' => 5], $summary['relationshipSourcePartExtensionCounts']);

        $sourceA = $byBaseName['source-a.xml'];
        $t->same(2, $sourceA['sourceCount']);
        $t->same(2, $sourceA['existingSourceCount']);
        $t->same(0, $sourceA['nonExistingSourceCount']);
        $t->same(2, $sourceA['relationshipCount']);
        $t->same(2, $sourceA['relationshipRecordCount']);
        $t->same(strlen($parts['customXml/source-a.xml']) + strlen($parts['word/source-a.xml']), $sourceA['existingSourceByteLength']);
        $t->same(['package-part' => 2], $sourceA['relationshipSourceKindCounts']);
        $t->same(['xml' => 2], $sourceA['sourcePartExtensionCounts']);
        $t->same(['application/xml' => 2], $sourceA['sourceContentTypeBaseCounts']);
        $t->same(['default' => 2], $sourceA['sourceContentTypeSourceCounts']);
        $t->same(['customXml', 'word'], $sourceA['sourceDirectories']);
        $t->same(['customXml/source-a.xml', 'word/source-a.xml'], $sourceA['sourceParts']);
        $t->same(['customXml/_rels/source-a.xml.rels', 'word/_rels/source-a.xml.rels'], $sourceA['relationshipParts']);

        $missing = $byBaseName['missing-source-a.xml'];
        ...
        $t->same(['word/missing-source-a.xml'], $missing['sourceParts']);

        $relationshipPartSource = $byBaseName['document.xml.rels'];
        ...
        $t->same(['rels' => 1], $relationshipPartSource['sourcePartExtensionCounts']);
        $t->same(['word/_rels/document.xml.rels'], $relationshipPartSource['sourceParts']);

        $noneExtension = $byExtension['(none)'];
        $t->same(2, $noneExtension['sourceCount']);
        $t->same(1, $noneExtension['existingSourceCount']);
        $t->same(1, $noneExtension['nonExistingSourceCount']);
        $t->same(['invalid-source' => 1, 'package-root' => 1], $noneExtension['relationshipSourceKindCounts']);
        $t->same(['(invalid-source)' => 1, '/' => 1], $noneExtension['sourceBaseNameCounts']);

        $xmlExtension = $byExtension['xml'];
        $t->same(5, $xmlExtension['sourceCount']);
        $t->same(4, $xmlExtension['existingSourceCount']);
        $t->same(1, $xmlExtension['nonExistingSourceCount']);
        $t->same([
            'document.xml' => 1,
            'missing-source-a.xml' => 1,
            'root-source.xml' => 1,
    ...
    'summarizes docx relationship source directories for package review' => static function (TestRunner $t): void {
        ...
        $parts['[Content_Types].xml'] = str_replace(
            '</Types>',
            '  <Override PartName="/word/header/header1.xml" ContentType="' . $headerContentType . '"/>' . "\n" .
            '</Types>',
            $parts['[Content_Types].xml']
        );
        $parts['root-source.xml'] = '<rootSource/>';
        $parts['word/source-directory-a.xml'] = '<sourceA/>';
        $parts['word/source-directory-b.xml'] = '<sourceB/>';
        $parts['word/header/header1.xml'] = '<w:hdr xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"/>';
        $parts['customXml/source-directory.xml'] = '<customSource/>';
        $parts['word/media/root-dir.png'] = 'root directory source target';
        $parts['word/media/source-a-dir.png'] = 'source a directory target';
        $parts['word/media/source-b-dir.png'] = 'source b directory target';
        $parts['word/media/header-dir.png'] = 'header directory source target';
        $parts['word/media/custom-dir.png'] = 'custom directory source target';
        $parts['word/media/missing-dir.png'] = 'missing directory source target';
        $parts['word/media/relationship-dir.png'] = 'relationship directory source target';
        $parts['word/media/invalid-dir.png'] = 'invalid directory source target';
        $parts['_rels/root-source.xml.rels'] = <<<'XML'
<?xml version="1.0" encoding="UTF-8"?>
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rRootDirectorySource" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="word/media/root-dir.png"/>
</Relationships>
XML;
        $parts['word/_rels/source-directory-a.xml.rels'] = <<<'XML'
<?xml version="1.0" encoding="UTF-8"?>
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rWordDirectorySourceA" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="media/source-a-dir.png"/>
</Relationships>
XML;
        $parts['word/_rels/source-directory-b.xml.rels'] = <<<'XML'
<?xml version="1.0" encoding="UTF-8"?>
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rWordDirectorySourceB" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="media/source-b-dir.png"/>
</Relationships>
XML;
        $parts['word/header/_rels/header1.xml.rels'] = <<<'XML'
<?xml version="1.0" encoding="UTF-8"?>
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rHeaderDirectorySource" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="../media/header-dir.png"/>
</Relationships>
XML;
        $parts['customXml/_rels/source-directory.xml.rels'] = <<<'XML'
<?xml version="1.0" encoding="UTF-8"?>
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rCustomDirectorySource" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="../word/media/custom-dir.png"/>
</Relationships>
XML;
        $parts['word/_rels/missing-directory.xml.rels'] = <<<'XML'
<?xml version="1.0" encoding="UTF-8"?>
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rMissingDirectorySource" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="media/missing-dir.png"/>
</Relationships>
XML;
        $parts['word/_rels/_rels/document.xml.rels.rels'] = <<<'XML'
<?xml version="1.0" encoding="UTF-8"?>
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rRelationshipDirectorySource" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="../media/relationship-dir.png"/>
</Relationships>
XML;
        $parts['word/_rels/media/document.xml.rels'] = <<<'XML'
<?xml version="1.0" encoding="UTF-8"?>
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rInvalidDirectorySource" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="word/media/invalid-dir.png"/>
</Relationships>
XML;

        $summary = (new DocxOpenXmlReader())->readPackage($parts)->attr('docx')['packageProvenance']['summary'];
        $directories = [];
        foreach ($summary['relationshipSourceDirectories'] as $directory) {
            $directories[$directory['directoryKey']] = $directory;
        }
        ...
        $t->same(6, $summary['relationshipSourceDirectoryCount']);
        $t->same(['(invalid-source)', '/', 'customXml', 'word', 'word/_rels', 'word/header'], array_column($summary['relationshipSourceDirectories'], 'directoryKey'));

        $invalid = $directories['(invalid-source)'];
        $t->same(null, $invalid['directory']);
        $t->same(1, $invalid['sourceCount']);
        $t->same(0, $invalid['existingSourceCount']);
        $t->same(1, $invalid['nonExistingSourceCount']);
        ...
        $t->same(['(missing)' => 1], $invalid['sourceContentTypeSourceCounts']);
        ...
        $t->same(0, $root['nonExistingSourceCount']);
        $t->same(3, $root['relationshipCount']);
        $t->same(strlen($parts['root-source.xml']), $root['existingSourceByteLength']);
        $t->same(['package-part' => 1, 'package-root' => 1], $root['relationshipSourceKindCounts']);
        $t->same(['(missing)' => 1, 'application/xml' => 1], $root['sourceContentTypeBaseCounts']);
        $t->same(['(missing)' => 1, 'default' => 1], $root['sourceContentTypeSourceCounts']);
        $t->same(['package-part' => 1, 'package-root' => 1], $root['sourceRoleCounts']);
        $t->same(['/', 'root-source.xml'], $root['sourceParts']);
        $t->same(['_rels/.rels', '_rels/root-source.xml.rels'], $root['relationshipParts']);
        $t->same('root-source.xml', $root['largestExistingSourcePart']['sourcePart']);
        ...
        $t->same(1, $word['nonExistingSourceCount']);
        $t->same(5, $word['relationshipCount']);
        $t->same(strlen($parts['word/document.xml']) + strlen($parts['word/source-directory-a.xml']) + strlen($parts['word/source-directory-b.xml']), $word['existingSourceByteLength']);
        $t->same(['missing-source' => 1, 'package-part' => 3], $word['relationshipSourceKindCounts']);
        $t->same([
            '(missing)' => 1,
            'application/vnd.openxmlformats-officedocument.wordprocessingml.document.main+xml' => 1,
        ...
        $t->same(['(missing)' => 1, 'default' => 2, 'override' => 1], $word['sourceContentTypeSourceCounts']);
        $t->same(['office-document' => 1, 'package-part' => 2, 'root-relationship-target' => 1], $word['sourceRoleCounts']);
        $t->same(['word/document.xml', 'word/missing-directory.xml', 'word/source-directory-a.xml', 'word/source-directory-b.xml'], $word['sourceParts']);
        $t->same(['word/_rels/document.xml.rels', 'word/_rels/missing-directory.xml.rels', 'word/_rels/source-directory-a.xml.rels', 'word/_rels/source-directory-b.xml.rels'], $word['relationshipParts']);
        $t->same('word/document.xml', $word['largestExistingSourcePart']['sourcePart']);

        $custom = $directories['customXml'];
        $t->same(1, $custom['sourceCount']);
        $t->same(1, $custom['existingSourceCount']);
        $t->same(['package-part' => 1], $custom['relationshipSourceKindCounts']);
        $t->same(['application/xml' => 1], $custom['sourceContentTypeBaseCounts']);
        $t->same(['customXml/source-directory.xml'], $custom['sourceParts']);
        $t->same(['customXml/_rels/source-directory.xml.rels'], $custom['relationshipParts']);

        $relationshipPart = $directories['word/_rels'];
        ...
        $header = $directories['word/header'];
        $t->same(1, $header['sourceCount']);
        $t->same(1, $header['existingSourceCount']);
        $t->same([$headerContentType => 1], $header['sourceContentTypeBaseCounts']);
        $t->same(['override' => 1], $header['sourceContentTypeSourceCounts']);
...
XML;
    ...
    'summarizes docx relationship target content type buckets for review handoff' => static function (TestRunner $t): void {
        ...
        $parts['[Content_Types].xml'] = str_replace(
            '</Types>',
            '  <Override PartName="/word/comments.xml" ContentType="' . $commentsType . '; profile=review"/>' . "\n" .
            '</Types>',
            $parts['[Content_Types].xml']
        );
        $parts['word/_rels/document.xml.rels'] = str_replace(
            '</Relationships>',
            '  <Relationship Id="rCommentsTargetType" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/comments" Target="comments.xml"/>' . "\n" .
            '  <Relationship Id="rCustomXmlTargetType" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/customXml" Target="../customXml/target.xml"/>' . "\n" .
            '  <Relationship Id="rMissingTargetType" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/package" Target="raw/no-type.bin"/>' . "\n" .
            '  <Relationship Id="rExternalTargetType" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/hyperlink" Target="https://example.test/target-types" TargetMode="External"/>' . "\n" .
            '</Relationships>',
            $parts['word/_rels/document.xml.rels']
        );
        $parts['word/comments.xml'] = '<w:comments xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"/>';
        $parts['customXml/target.xml'] = '<target/>';
        $parts['word/raw/no-type.bin'] = 'missing content type target bytes';
        ...
        $t->same([
            ...
            'application/xml' => 1,
            'image/png' => 1,
        ], $summary['relationshipTargetContentTypeCounts']);
        $t->same(['default' => 2, 'missing' => 1, 'override' => 3], $summary['relationshipTargetContentTypeSourceCounts']);
        $t->same(1, $summary['relationshipTargetParameterizedContentTypeCount']);
        $t->same(6, count($summary['relationshipTargetContentTypes']));
...
XML;
        ...
        $t->same(3, $selected['relationshipSelectedCount']);
        $t->same(0, $selected['issueCount']);
        $t->same(18, $package['summary']['selectedXmlPartCount']);
...
XML;
        ...
        $selectedPeople = $package['selectedXmlParts']['byKind']['people'];
        ...
        $t->same(strlen($parts['word/people.xml']), $people['byteLength']);
        $t->same(sprintf('%08x', crc32($parts['word/people.xml'])), $people['crc32']);
        $t->same(hash('sha256', $parts['word/people.xml']), $people['sha256']);
        ...
        $t->same('relationship', $selectedPeople['selectionSource']);
        $t->same('rPeople', $selectedPeople['relationshipId']);
        $t->same($peopleRel, $selectedPeople['relationshipType']);
        $t->same('word/people.xml', $selectedPeople['partName']);
        $t->same('people.xml?presence=review#people', $selectedPeople['relationshipTarget']);
        $t->same('word/people.xml?presence=review#people', $selectedPeople['relationshipResolvedTarget']);
        $t->same('?presence=review#people', $selectedPeople['targetReferenceSuffix']);
        $t->same(strlen($parts['word/people.xml']), $selectedPeople['bytes']);
        $t->same(hash('sha256', $parts['word/people.xml']), $selectedPeople['sha256']);
        $t->same('w15:people', $selectedPeople['rootQualifiedName']);
        $t->same('w15', $selectedPeople['rootPrefix']);
        $t->same('http://schemas.microsoft.com/office/word/2012/wordml', $selectedPeople['rootNamespace']);
        $t->same('people', $selectedPeople['rootLocalName']);
        $t->same(true, $selectedPeople['validRoot']);
        $t->same(true, $selectedPeople['contentTypeMatchesExpected']);
        $t->same([], $selectedPeople['issues']);
