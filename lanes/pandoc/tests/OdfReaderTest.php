...
XML;
    ...
    'reports ODT embedded object package provenance without exposing payload bytes' => static function (TestRunner $t) use ($buildOdtPackage, $manifestXml): void {
        ...
        $t->same($provenance, $result['document']->attr('manifest')['packageProvenance']);
        $t->same($objects, $result['metadata']['odfPackageEmbeddedObjects']);
        ...
        $t->same($objects['containedMediaTypeSummary'], $provenance['embeddedObjectContainedMediaTypeSummary']);
        ...
        $t->same(4, $objects['containedMediaTypeSummary']['bucketCount']);
        $t->same(5, $objects['containedMediaTypeSummary']['partCount']);
        $t->same(4, $objects['containedMediaTypeSummary']['declaredPartCount']);
        $t->same(1, $objects['containedMediaTypeSummary']['undeclaredPartCount']);
        $t->same(1, $objects['containedMediaTypeSummary']['missingMediaTypePartCount']);
        $t->same(strlen($chartContent) + strlen($chartStyles) + strlen($chartPreview) + strlen($chartRdf) + strlen($oleBytes), $objects['containedMediaTypeSummary']['byteLength']);
        $t->same(['Object Chart/content.xml', 'Object Chart/styles.xml'], $objects['containedMediaTypeSummary']['byMediaTypeBase']['text/xml']['parts']);
        $t->same(['Object Chart/Pictures/preview.png'], $objects['containedMediaTypeSummary']['byMediaTypeBase']['image/png']['parts']);
        $t->same(['Object OLE/oleObject.bin'], $objects['containedMediaTypeSummary']['byMediaTypeBase']['application/vnd.openxmlformats-officedocument.oleobject']['parts']);
        $t->same(['Object Chart/manifest.rdf'], $objects['containedMediaTypeSummary']['byMediaTypeBase']['']['parts']);
        $t->same([
        ...
        $t->same([
            'odf-embedded-object-package-missing' => 1,
            'odf-embedded-object-package-missing-declared-part' => 1,
            'odf-embedded-object-package-undeclared-contained-part' => 1,
        ], $objects['issueCodeCounts']);
        ...
        $t->same(3, $chart['containedMediaTypeSummary']['bucketCount']);
        $t->same(4, $chart['containedMediaTypeSummary']['partCount']);
        $t->same(3, $chart['containedMediaTypeSummary']['declaredPartCount']);
        $t->same(1, $chart['containedMediaTypeSummary']['undeclaredPartCount']);
        $t->same(['Object Chart/content.xml', 'Object Chart/styles.xml'], $chart['containedMediaTypeSummary']['byMediaTypeBase']['text/xml']['parts']);
        $t->same(['Object Chart/Pictures/preview.png'], $chart['containedMediaTypeSummary']['byMediaTypeBase']['image/png']['parts']);
        $t->same(['Object Chart/manifest.rdf'], $chart['containedMediaTypeSummary']['byMediaTypeBase']['']['parts']);
        ...
        $t->same('Object%20Chart/Pictures/preview.png', $chartContainedByPart['Object Chart/Pictures/preview.png']['fullPath']);
        $t->same('image/png', $chartContainedByPart['Object Chart/Pictures/preview.png']['mediaTypeBase']);
        $t->same(true, $chartContainedByPart['Object Chart/Pictures/preview.png']['declaredInManifest']);
        $t->same(false, $chartContainedByPart['Object Chart/manifest.rdf']['declaredInManifest']);
        $t->same('', $chartContainedByPart['Object Chart/manifest.rdf']['mediaTypeBase']);
        ...
        $t->same(1, $ole['containedMediaTypeSummary']['bucketCount']);
        $t->same(['Object OLE/oleObject.bin'], $ole['containedMediaTypeSummary']['byMediaTypeBase']['application/vnd.openxmlformats-officedocument.oleobject']['parts']);
        $t->same('application/vnd.openxmlformats-officedocument.oleobject', $ole['containedParts'][0]['mediaTypeBase']);
        $t->same(0, $ole['issueCount']);
        ...
        $t->same(2, $missing['issueCount']);
        $t->same(0, $missing['containedMediaTypeSummary']['bucketCount']);
...
XML;
    ...
    'surfaces ODT ZIP package comments in rich package provenance' => static function (TestRunner $t) use ($manifestXml, $contentXml, $stylesXml, $metaXml): void {
        ...
        foreach ($packageManifest['entries'] as $packageManifestEntry) {
            $packageManifestEntriesByName[$packageManifestEntry['name']] = $packageManifestEntry;
        }
        $localHeaderMetadataEntriesByName = [];
        foreach ($localHeaderMetadata['entries'] as $entry) {
            $localHeaderMetadataEntriesByName[$entry['centralName']] = $entry;
        }
        $identity = $provenance['packageIdentity'];
        $content = $provenance['parts']['content.xml'];
        $hero = $provenance['parts']['Pictures/hero.png'];
        $contentManifestEntry = $packageManifestEntriesByName['content.xml'];
        $heroManifestEntry = $packageManifestEntriesByName['Pictures/hero.png'];
        $contentLocalHeaderMetadata = $localHeaderMetadataEntriesByName['content.xml'];
        $identityParts = [];
        ...
        $t->same($packageManifest['archiveSha256'], $provenance['archiveSha256']);
        $t->same($packageManifest['centralDirectorySha256'], $provenance['centralDirectorySha256']);
        $t->same($packageManifest['endOfCentralDirectorySha256'], $provenance['endOfCentralDirectorySha256']);
        $t->same($packageManifest['packageCommentSha256'], $provenance['packageCommentSha256']);
        $t->same($localHeaderMetadata['entryCount'], $provenance['localHeaderMetadataEntryCount']);
        $t->same($localHeaderMetadata['totalEntryCount'], $provenance['localHeaderMetadataTotalEntryCount']);
        $t->same($localHeaderMetadata['centralDirectoryOffset'], $provenance['localHeaderMetadataCentralDirectoryOffset']);
        $t->same($localHeaderMetadata['centralDirectorySize'], $provenance['localHeaderMetadataCentralDirectoryBytes']);
        $t->same(true, $provenance['localHeaderMetadataIsSupportedByBoundedReader']);
        $t->same([], $provenance['localHeaderMetadataIssueCodes']);
        $t->same(0, $provenance['localHeaderMetadataMismatchEntryCount']);
        $t->same([], $provenance['localHeaderMetadataMismatchedEntries']);
        $t->same(strlen('odt package review'), $provenance['packageCommentBytes']);
        $t->same(false, $provenance['hasCentralDirectorySignature']);
        ...
        $t->same($packageManifest['archiveSha256'], $identity['archiveSha256']);
        $t->same($packageManifest['centralDirectorySha256'], $identity['centralDirectorySha256']);
        $t->same($packageManifest['endOfCentralDirectorySha256'], $identity['endOfCentralDirectorySha256']);
        $t->same($packageManifest['packageCommentSha256'], $identity['packageCommentSha256']);
        $t->same($provenance['localHeaderMetadataEntryCount'], $identity['localHeaderMetadataEntryCount']);
        $t->same($provenance['localHeaderMetadataIsSupportedByBoundedReader'], $identity['localHeaderMetadataIsSupportedByBoundedReader']);
        $t->same($provenance['localHeaderMetadataIssueCodes'], $identity['localHeaderMetadataIssueCodes']);
        $t->same($provenance['localHeaderMetadataMismatchEntryCount'], $identity['localHeaderMetadataMismatchEntryCount']);
        $t->same($provenance['localHeaderMetadataMismatchedEntries'], $identity['localHeaderMetadataMismatchedEntries']);
        $t->same(strlen('odt package review'), $identity['packageCommentBytes']);
        $t->same($provenance, $documentProvenance);
        ...
        $t->same($contentManifestEntry, $content['zipPackageManifestEntry']);
        $t->same($contentManifestEntry['directoryRoot'], $content['zipPackageManifestDirectoryRoot']);
        $t->same($contentManifestEntry['pathSegments'], $content['zipPackageManifestPathSegments']);
        $t->same($contentManifestEntry['pathSegmentCount'], $content['zipPackageManifestPathSegmentCount']);
        $t->same($contentManifestEntry['directoryDepth'], $content['zipPackageManifestDirectoryDepth']);
        $t->same($contentManifestEntry['centralDirectoryIndex'], $content['zipPackageManifestCentralDirectoryIndex']);
        $t->same($contentManifestEntry['localHeaderOrder'], $content['zipPackageManifestLocalHeaderOrder']);
        $t->same($contentManifestEntry['localHeaderOffset'], $content['zipLocalHeaderOffset']);
        $t->same($contentManifestEntry['localHeaderLength'], $content['zipLocalHeaderBytes']);
        $t->same($contentManifestEntry['localHeaderSha256'], $content['zipLocalHeaderSha256']);
        $t->same($contentManifestEntry['localHeaderFixedHeaderBytes'], $content['zipLocalHeaderFixedHeaderBytes']);
        $t->same($contentManifestEntry['localHeaderVariableFieldOffset'], $content['zipLocalHeaderVariableFieldOffset']);
        $t->same($contentManifestEntry['localHeaderVariableFieldBytes'], $content['zipLocalHeaderVariableFieldBytes']);
        $t->same($contentManifestEntry['localHeaderVariableFieldSha256'], $content['zipLocalHeaderVariableFieldSha256']);
        $t->same($contentManifestEntry['localRecordOffset'], $content['zipLocalRecordOffset']);
        $t->same($contentManifestEntry['localRecordBytes'], $content['zipLocalRecordBytes']);
        $t->same($contentManifestEntry['localRecordEnd'], $content['zipLocalRecordEnd']);
        $t->same($contentManifestEntry['localRecordSha256'], $content['zipLocalRecordSha256']);
        $t->same($contentManifestEntry['compressedDataOffset'], $content['zipCompressedDataOffset']);
        $t->same($contentManifestEntry['compressedDataEnd'], $content['zipCompressedDataEnd']);
        $t->same($contentManifestEntry['compressedDataSha256'], $content['zipCompressedDataSha256']);
        $t->same($contentManifestEntry['usesDataDescriptor'], $content['zipUsesDataDescriptor']);
        $t->same($contentManifestEntry['dataDescriptorBytes'], $content['zipDataDescriptorBytes']);
        $t->same($contentManifestEntry['centralDirectoryRecordOffset'], $content['zipCentralDirectoryRecordOffset']);
        $t->same($contentManifestEntry['centralDirectoryRecordBytes'], $content['zipCentralDirectoryRecordBytes']);
        $t->same($contentManifestEntry['centralDirectoryRecordSha256'], $content['zipCentralDirectoryRecordSha256']);
        $t->same($contentManifestEntry['centralDirectoryVariableFieldBytes'], $content['zipCentralDirectoryVariableFieldBytes']);
        $t->same($contentManifestEntry['centralDirectoryReviewFieldBytes'], $content['zipCentralDirectoryReviewFieldBytes']);
        $t->same(true, $content['zipLocalHeaderMetadataMatchesCentralDirectory']);
        $t->same([], $content['zipLocalHeaderMetadataIssues']);
        $t->same($contentLocalHeaderMetadata['centralVersionNeededToExtract'], $content['zipCentralVersionNeededToExtract']);
        $t->same($contentLocalHeaderMetadata['localVersionNeededToExtract'], $content['zipLocalVersionNeededToExtract']);
        $t->same($contentLocalHeaderMetadata['centralGeneralPurposeFlags'], $content['zipCentralGeneralPurposeFlags']);
        $t->same($contentLocalHeaderMetadata['localGeneralPurposeFlags'], $content['zipLocalGeneralPurposeFlags']);
        $t->same($contentLocalHeaderMetadata['centralCompressionMethod'], $content['zipCentralCompressionMethod']);
        $t->same($contentLocalHeaderMetadata['localCompressionMethod'], $content['zipLocalCompressionMethod']);
        $t->same($contentLocalHeaderMetadata['centralCrc32'], $content['zipCentralCrc32']);
        $t->same($contentLocalHeaderMetadata['localCrc32'], $content['zipLocalCrc32']);
        $t->same($contentLocalHeaderMetadata['centralCompressedSize'], $content['zipCentralCompressedSize']);
        $t->same($contentLocalHeaderMetadata['localCompressedSize'], $content['zipLocalCompressedSize']);
        $t->same($contentLocalHeaderMetadata['centralUncompressedSize'], $content['zipCentralUncompressedSize']);
        $t->same($contentLocalHeaderMetadata['localUncompressedSize'], $content['zipLocalUncompressedSize']);
        $t->same($contentLocalHeaderMetadata['usesDataDescriptor'], $content['zipLocalHeaderUsesDataDescriptor']);
        $t->same($contentLocalHeaderMetadata['hasZeroLocalHeaderPlaceholders'], $content['zipLocalHeaderHasZeroDataDescriptorPlaceholders']);
        $t->same('media review', $hero['zipEntryComment']);
        $t->same(true, $hero['zipEntryHasComment']);
        $t->same($heroManifestEntry, $hero['zipPackageManifestEntry']);
        $t->same($heroManifestEntry['localRecordSha256'], $hero['zipLocalRecordSha256']);
        $t->same($heroManifestEntry['compressedDataSha256'], $hero['zipCompressedDataSha256']);
        $t->same($heroManifestEntry['centralDirectoryRecordSha256'], $hero['zipCentralDirectoryRecordSha256']);
        $t->same($contentManifestEntry, $identityParts['content.xml']['zipPackageManifestEntry']);
        $t->same($contentManifestEntry['localRecordSha256'], $identityParts['content.xml']['zipLocalRecordSha256']);
        $t->same($contentManifestEntry['compressedDataSha256'], $identityParts['content.xml']['zipCompressedDataSha256']);
        $t->same($contentManifestEntry['centralDirectoryRecordSha256'], $identityParts['content.xml']['zipCentralDirectoryRecordSha256']);
        $t->same($contentManifestEntry['localHeaderVariableFieldSha256'], $identityParts['content.xml']['zipLocalHeaderVariableFieldSha256']);
        $t->same($content['zipLocalHeaderMetadataMatchesCentralDirectory'], $identityParts['content.xml']['zipLocalHeaderMetadataMatchesCentralDirectory']);
        $t->same($content['zipCentralVersionNeededToExtract'], $identityParts['content.xml']['zipCentralVersionNeededToExtract']);
        $t->same($content['zipLocalVersionNeededToExtract'], $identityParts['content.xml']['zipLocalVersionNeededToExtract']);
        $t->same($content['zipCentralGeneralPurposeFlags'], $identityParts['content.xml']['zipCentralGeneralPurposeFlags']);
        $t->same($content['zipLocalGeneralPurposeFlags'], $identityParts['content.xml']['zipLocalGeneralPurposeFlags']);
        $t->same($heroManifestEntry, $identityParts['Pictures/hero.png']['zipPackageManifestEntry']);
        $t->same($heroManifestEntry['localRecordSha256'], $identityParts['Pictures/hero.png']['zipLocalRecordSha256']);
        $t->same($heroManifestEntry['compressedDataSha256'], $identityParts['Pictures/hero.png']['zipCompressedDataSha256']);
        $t->same($heroManifestEntry['centralDirectoryRecordSha256'], $identityParts['Pictures/hero.png']['zipCentralDirectoryRecordSha256']);
    ...
    'surfaces ODT ZIP extra fields in rich package provenance' => static function (TestRunner $t) use ($buildOdtPackage, $manifestXml): void {
        ...
        $t->same(['0xcafe'], $provenance['extraFieldIdHexes']);
        $t->same(['0xcafe'], $provenance['centralExtraFieldIdHexes']);
        $t->same(['0xcafe'], $provenance['localExtraFieldIdHexes']);
        $t->same(['0xcafe'], $provenance['sharedExtraFieldIdHexes']);
        $t->same([], $provenance['centralOnlyExtraFieldIdHexes']);
        $t->same([], $provenance['localOnlyExtraFieldIdHexes']);
        ...
        $t->same(['0xcafe'], $provenance['packageIdentity']['extraFieldIdHexes']);
        $t->same(['0xcafe'], $provenance['packageIdentity']['sharedExtraFieldIdHexes']);
    },
