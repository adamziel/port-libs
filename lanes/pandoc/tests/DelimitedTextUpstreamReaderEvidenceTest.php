...
return [
    'reports skipped delimited text reader evidence when upstream root is absent' => static function (TestRunner $t): void {
        ...
        $t->contains('Generated CSV native parity: 7/7 status=generated-csv-native-parity-observed-not-upstream-fixture', $text);
        $t->contains('Generated TSV native parity: 6/6 status=generated-tsv-native-parity-observed-not-upstream-fixture', $text);
    },
    'reports checked-in current csv command fixture static evidence' => static function (TestRunner $t): void {
        ...
        $t->same(7, $evidence['generatedCsvNativeStaticEvidence']['sampleCount']);
        $t->same(14, $evidence['generatedCsvNativeStaticEvidence']['checkedInFixtureCount']);
        ...
        $t->same(6, $evidence['generatedTsvNativeStaticEvidence']['sampleCount']);
        $t->same(12, $evidence['generatedTsvNativeStaticEvidence']['checkedInFixtureCount']);
    ...
    'executes generated csv native parity evidence' => static function (TestRunner $t): void {
        ...
        $t->same(array_fill(0, 7, 'valid-generated-csv-native-sample-static-binding'), array_column($evidence['samples'], 'staticFixtureBindingStatus'));
    ...
    'executes generated tsv native parity evidence' => static function (TestRunner $t): void {
        ...
        $t->same(0, $evidence['staticFixtureBindingInvalidCount']);
        $t->same(array_fill(0, 6, 'valid-generated-tsv-native-sample-static-binding'), array_column($evidence['samples'], 'staticFixtureBindingStatus'));
    ...
    'validates hydrated upstream delimited text reader fixture evidence' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeDelimitedTextEvidenceTree): void {
        ...
        try {
            ...
            $t->same(7, $report['generatedCsvNativeParityEvidence']['generatedNativeMatchCount']);
            $t->same('generated-csv-native-parity-observed-not-upstream-fixture', $report['generatedCsvNativeParityEvidence']['parityStatus']);
            $t->same(6, $report['generatedTsvNativeParityEvidence']['generatedNativeMatchCount']);
    ...
    'cli reports generated tsv native parity without changing tsv denominator' => static function (TestRunner $t): void {
        ...
        $t->same(7, $decoded['generatedCsvNativeParity']['sampleCount']);
        $t->same(7, $decoded['generatedCsvNativeParity']['generatedNativeMatchCount']);
        ...
        $t->same(6, $decoded['tsv']['generatedNativeParitySampleCount']);
        $t->same(6, $decoded['generatedTsvNativeParity']['generatedNativeMatchCount']);
    ...
    'cli gates generated csv native parity against explicit repo root' => static function (TestRunner $t) use ($makeTempDir, $removeTree): void {
        ...
        try {
            ...
            $t->same(7, $decoded['generatedCsvNativeParity']['parseFailureCount']);
            $t->same(0, $decoded['generatedCsvNativeParity']['staticFixtureBindingValidCount']);
            $t->same(7, $decoded['generatedCsvNativeParity']['staticFixtureBindingInvalidCount']);
            $t->same('blocked-by-generated-csv-native-fixture-validation', $decoded['generatedCsvNativeParity']['parityStatus']);
            $t->same(array_fill(0, 7, 'invalid-generated-csv-native-sample-static-binding'), array_column($decoded['generatedCsvNativeParity']['samples'], 'staticFixtureBindingStatus'));
    ...
    'cli gates generated tsv native parity against explicit repo root' => static function (TestRunner $t) use ($makeTempDir, $removeTree): void {
        ...
        try {
            ...
            $t->same(1, $exitCode);
            $t->same(6, $decoded['generatedTsvNativeParity']['sampleCount']);
            $t->same(0, $decoded['generatedTsvNativeParity']['comparedSampleCount']);
            $t->same(6, $decoded['generatedTsvNativeParity']['parseFailureCount']);
            $t->same(0, $decoded['generatedTsvNativeParity']['staticFixtureBindingValidCount']);
            $t->same(6, $decoded['generatedTsvNativeParity']['staticFixtureBindingInvalidCount']);
            $t->same('blocked-by-generated-tsv-native-fixture-validation', $decoded['generatedTsvNativeParity']['parityStatus']);
            $t->same(array_fill(0, 6, 'invalid-generated-tsv-native-sample-static-binding'), array_column($decoded['generatedTsvNativeParity']['samples'], 'staticFixtureBindingStatus'));
