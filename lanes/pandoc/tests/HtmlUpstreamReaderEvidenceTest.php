...
return [
    'reports skipped html reader evidence when upstream root is absent' => static function (TestRunner $t): void {
        ...
        $t->same(true, HtmlUpstreamReaderEvidence::hasRequiredSelectedFixtureCount($report, 102));
        $t->same(true, HtmlUpstreamReaderEvidence::hasRequiredStaticCurrentEvidence($report));
        $t->same(true, HtmlUpstreamReaderEvidence::hasRequiredNativeMappedParity($report, 102));
        ...
        $t->contains('Static current evidence: valid-checked-in-current-html-reader-evidence checkedInFixtures=102 nativePairs=102', $text);
        $t->contains('Native AST mapped parity: 102/102', $text);
        $t->contains('Native AST fixture inventory: html=102 native=102 paired=102 unpairedHtml=0 unpairedNative=0', $text);
    ...
    'reports checked-in current html fixture static evidence' => static function (TestRunner $t): void {
        ...
        $t->same(102, $evidence['readerDenominator']['selectedFixtureCount']);
        ...
        $t->same(102, $evidence['readerDenominator']['nativeMappedPairCount']);
        $t->same(102, $evidence['checkedInFixtureCount']);
        $t->same(102, $evidence['checkedInNativePairCount']);
    ...
    'rejects hydrated upstream html reader source evidence without pinned git head' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeHtmlEvidenceTree): void {
        ...
        try {
            ...
            $t->same(102, $report['denominator']['selectedFixtureCount']);
    ...
    'cli gates checked-in current html fixture evidence without hydrated upstream cache' => static function (TestRunner $t): void {
        ...
        $command = escapeshellarg(PHP_BINARY)
            ...
            . ' --checked-in-fixtures'
            . ' --json'
            . ' --require-selected-fixture-count=102'
            . ' --require-static-current-evidence'
            . ' --require-native-mapped-parity=102'
        ...
        $t->same(102, $decoded['staticCurrentEvidence']['readerDenominator']['selectedFixtureCount']);
        $t->same('valid-checked-in-current-html-reader-evidence', $decoded['staticCurrentEvidence']['validation']['status']);
        $t->same(102, $decoded['staticCurrentEvidence']['checkedInNativePairCount']);
        $t->same(102, $decoded['nativeAstEvidence']['normalizedAstMatchCount']);
        ...
        $failingCommand = str_replace('--require-selected-fixture-count=102', '--require-selected-fixture-count=85', $command) . ' 2>/dev/null';
        $failingOutput = [];
        $failingExitCode = 0;
    ...
    'workflow gate is either current or awaiting integrator count update' => static function (TestRunner $t): void {
        $repoRoot = dirname(__DIR__, 3);
        ...
        $workflowAtCurrentCount = str_contains($workflow, '--require-selected-fixture-count=102')
            && str_contains($workflow, '--require-native-mapped-parity=102')
            && str_contains($workflow, '--require-mapped-parity=102');
        $workflowAwaitingIntegratorUpdate = str_contains($workflow, '--require-selected-fixture-count=101')
            && str_contains($workflow, '--require-native-mapped-parity=101')
            && str_contains($workflow, '--require-mapped-parity=101');
