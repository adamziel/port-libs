...
return [
    'reports skipped html reader evidence when upstream root is absent' => static function (TestRunner $t): void {
        ...
        $t->same(true, HtmlUpstreamReaderEvidence::hasRequiredSelectedFixtureCount($report, 70));
        $t->same(true, HtmlUpstreamReaderEvidence::hasRequiredStaticCurrentEvidence($report));
        $t->same(true, HtmlUpstreamReaderEvidence::hasRequiredNativeMappedParity($report, 70));
        ...
        $t->contains('Static current evidence: valid-checked-in-current-html-reader-evidence checkedInFixtures=70 nativePairs=70', $text);
        $t->contains('Native AST mapped parity: 70/70', $text);
        $t->contains('Native AST fixture inventory: html=70 native=70 paired=70 unpairedHtml=0 unpairedNative=0', $text);
    ...
    'reports checked-in current html fixture static evidence' => static function (TestRunner $t): void {
        ...
        $t->same(70, $evidence['readerDenominator']['selectedFixtureCount']);
        ...
        $t->same(70, $evidence['readerDenominator']['nativeMappedPairCount']);
        $t->same(70, $evidence['checkedInFixtureCount']);
        $t->same(70, $evidence['checkedInNativePairCount']);
    ...
    'rejects hydrated upstream html reader source evidence without pinned git head' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeHtmlEvidenceTree): void {
        ...
        try {
            ...
            $t->same(70, $report['denominator']['selectedFixtureCount']);
    ...
    'cli gates checked-in current html fixture evidence without hydrated upstream cache' => static function (TestRunner $t): void {
        ...
        $command = escapeshellarg(PHP_BINARY)
            ...
            . ' --checked-in-fixtures'
            . ' --json'
            . ' --require-selected-fixture-count=70'
            . ' --require-static-current-evidence'
            . ' --require-native-mapped-parity=70'
        ...
        $t->same(70, $decoded['staticCurrentEvidence']['readerDenominator']['selectedFixtureCount']);
        $t->same('valid-checked-in-current-html-reader-evidence', $decoded['staticCurrentEvidence']['validation']['status']);
        $t->same(70, $decoded['staticCurrentEvidence']['checkedInNativePairCount']);
        $t->same(70, $decoded['nativeAstEvidence']['normalizedAstMatchCount']);
        ...
        $failingCommand = str_replace('--require-selected-fixture-count=70', '--require-selected-fixture-count=71', $command) . ' 2>/dev/null';
        $failingOutput = [];
        $failingExitCode = 0;
