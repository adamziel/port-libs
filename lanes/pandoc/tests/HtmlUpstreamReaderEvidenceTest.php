...
return [
    'reports skipped html reader evidence when upstream root is absent' => static function (TestRunner $t): void {
        ...
        $t->same(true, HtmlUpstreamReaderEvidence::hasRequiredSelectedFixtureCount($report, 97));
        $t->same(true, HtmlUpstreamReaderEvidence::hasRequiredStaticCurrentEvidence($report));
        $t->same(true, HtmlUpstreamReaderEvidence::hasRequiredNativeMappedParity($report, 97));
        ...
        $t->contains('Static current evidence: valid-checked-in-current-html-reader-evidence checkedInFixtures=97 nativePairs=97', $text);
        $t->contains('Native AST mapped parity: 97/97', $text);
        $t->contains('Native AST fixture inventory: html=97 native=97 paired=97 unpairedHtml=0 unpairedNative=0', $text);
    ...
    'reports checked-in current html fixture static evidence' => static function (TestRunner $t): void {
        ...
        $t->same(97, $evidence['readerDenominator']['selectedFixtureCount']);
        ...
        $t->same(97, $evidence['readerDenominator']['nativeMappedPairCount']);
        $t->same(97, $evidence['checkedInFixtureCount']);
        $t->same(97, $evidence['checkedInNativePairCount']);
    ...
    'rejects hydrated upstream html reader source evidence without pinned git head' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeHtmlEvidenceTree): void {
        ...
        try {
            ...
            $t->same(97, $report['denominator']['selectedFixtureCount']);
    ...
    'cli gates checked-in current html fixture evidence without hydrated upstream cache' => static function (TestRunner $t): void {
        ...
        $command = escapeshellarg(PHP_BINARY)
            ...
            . ' --checked-in-fixtures'
            . ' --json'
            . ' --require-selected-fixture-count=97'
            . ' --require-static-current-evidence'
            . ' --require-native-mapped-parity=97'
        ...
        $t->same(97, $decoded['staticCurrentEvidence']['readerDenominator']['selectedFixtureCount']);
        $t->same('valid-checked-in-current-html-reader-evidence', $decoded['staticCurrentEvidence']['validation']['status']);
        $t->same(97, $decoded['staticCurrentEvidence']['checkedInNativePairCount']);
        $t->same(97, $decoded['nativeAstEvidence']['normalizedAstMatchCount']);
        ...
        $failingCommand = str_replace('--require-selected-fixture-count=97', '--require-selected-fixture-count=85', $command) . ' 2>/dev/null';
        $failingOutput = [];
        $failingExitCode = 0;
    ...
    'workflow gate is either current or awaiting integrator count update' => static function (TestRunner $t): void {
        ...
        $workflowAtCurrentCount = str_contains($workflow, '--require-selected-fixture-count=97')
            && str_contains($workflow, '--require-native-mapped-parity=97')
            && str_contains($workflow, '--require-mapped-parity=97');
        $workflowAwaitingIntegratorUpdate = str_contains($workflow, '--require-selected-fixture-count=96')
            && str_contains($workflow, '--require-native-mapped-parity=96')
            && str_contains($workflow, '--require-mapped-parity=96');
