...
return [
    'reports skipped html reader evidence when upstream root is absent' => static function (TestRunner $t): void {
        ...
        $t->same(true, HtmlUpstreamReaderEvidence::hasRequiredSelectedFixtureCount($report, 88));
        $t->same(true, HtmlUpstreamReaderEvidence::hasRequiredStaticCurrentEvidence($report));
        $t->same(true, HtmlUpstreamReaderEvidence::hasRequiredNativeMappedParity($report, 88));
        ...
        $t->contains('Static current evidence: valid-checked-in-current-html-reader-evidence checkedInFixtures=88 nativePairs=88', $text);
        $t->contains('Native AST mapped parity: 88/88', $text);
        $t->contains('Native AST fixture inventory: html=88 native=88 paired=88 unpairedHtml=0 unpairedNative=0', $text);
    ...
    'reports checked-in current html fixture static evidence' => static function (TestRunner $t): void {
        ...
        $t->same(88, $evidence['readerDenominator']['nativeMappedPairCount']);
        $t->same(88, $evidence['checkedInFixtureCount']);
        $t->same(88, $evidence['checkedInNativePairCount']);
    ...
    'cli gates checked-in current html fixture evidence without hydrated upstream cache' => static function (TestRunner $t): void {
        ...
        $t->same(88, $decoded['staticCurrentEvidence']['checkedInNativePairCount']);
        ...
        $failingCommand = str_replace('--require-selected-fixture-count=88', '--require-selected-fixture-count=85', $command) . ' 2>/dev/null';
        $failingOutput = [];
        $failingExitCode = 0;
