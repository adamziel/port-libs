...
return [
    'reads EPUB3 container OPF metadata manifest spine and XHTML assets' => static function (TestRunner $t) use ($buildEpubPackage): void {
        ...
        $spineDocuments = $result['spineDocuments'];
        $t->same(2, $spineDocuments['itemCount']);
        $t->same(2, $spineDocuments['parsedItemCount']);
        $t->same(1, $spineDocuments['linearItemCount']);
        $t->same(1, $spineDocuments['linearParsedItemCount']);
        $t->same(0, $spineDocuments['diagnosticCount']);
        $t->same('/OEBPS/text/chapter1.xhtml', $spineDocuments['items'][0]['contentPart']);
        $t->same('Imported packet', $spineDocuments['items'][0]['title']);
        $t->same(true, $spineDocuments['items'][0]['bodyPresent']);
        $t->same('raw_html', $spineDocuments['items'][0]['handoffNodeType']);
        $t->same('/OEBPS/text/chapter2.xhtml', $spineDocuments['items'][1]['contentPart']);
        $t->same('Review appendix', $spineDocuments['items'][1]['title']);
        $t->same(false, $spineDocuments['items'][1]['linear']);
        $t->same($spineDocuments, $result['importReport']['spineDocuments']);
        $t->same($spineDocuments, $result['importReport']['spine']['documents']);
        ...
        $t->same($spineDocuments, $document->attr('spineDocuments'));
        $t->same('raw_html', $document->children[0]->type);
        $t->same('/OEBPS/text/chapter1.xhtml', $document->children[0]->attr('part'));
        $t->same($spineDocuments['items'][0], $document->children[0]->attr('spineDocument'));
