...
XML;
        ...
        $pageDiagnostic = $report['diagnostics'][2];
        ...
        $t->same(2, $report['missingEntryLabelCount']);
        $t->same(4, $result['nav']['documentDiagnosticCount']);
        $t->same($report, $result['importReport']['nav']['documentDiagnostics']);
    },
    'reports EPUB nav entry label diagnostics for non primary review sections' => static function (TestRunner $t) use ($buildEpubPackage): void {
        $navWithMissingEntryLabel = <<<'XML'
<html xmlns="http://www.w3.org/1999/xhtml" xmlns:epub="http://www.idpf.org/2007/ops">
  <body>
    <nav id="main-toc" epub:type="toc">
      <h1>Contents</h1>
    ...
    <nav id="review-links" epub:type="loa">
      <h2>Review links</h2>
      <ol>
        <li><a href="text/chapter2.xhtml#media"></a></li>
      </ol>
    </nav>
  </body>
</html>
XML;
        ...
        $result = (new EpubReader())->readPackage($buildEpubPackage(
            overrideNavXhtml: $navWithMissingEntryLabel,
        ));
        ...
        $t->same('/OEBPS/nav.xhtml', $report['part']);
        $t->same(2, $report['sectionCount']);
        $t->same(1, $report['primarySectionCount']);
        $t->same(1, $report['tocSectionCount']);
        $t->same(0, $report['pageListSectionCount']);
        $t->same(0, $report['missingHeadingSectionCount']);
        $t->same(0, $report['missingOrderedListSectionCount']);
        $t->same(1, $report['missingEntryLabelCount']);
        $t->same(2, $report['diagnosticCount']);

        $diagnostic = $report['diagnostics'][0];
        $t->same('missing-nav-entry-label', $diagnostic['type']);
        $t->same('review-links', $diagnostic['sectionId']);
        $t->same(['loa'], $diagnostic['sectionTypes']);
        $t->same(0, $diagnostic['entryIndex']);
        $t->same('text/chapter2.xhtml#media', $diagnostic['href']);
        $t->same('/OEBPS/text/chapter2.xhtml#media', $diagnostic['target']);
        $t->same(0, $diagnostic['depth']);
        $t->same(2, $result['nav']['documentDiagnosticCount']);
        $t->same($report, $result['importReport']['nav']['documentDiagnostics']);
    },
    'reports EPUB nav item diagnostics for package review' => static function (TestRunner $t) use ($buildEpubPackage): void {
...
XML;
        ...
        $result = (new EpubReader())->readPackage($buildEpubPackage(
            overrideNavXhtml: $navWithItemDiagnostics,
        ));
...
XML;
    ...
    'parses OPF metadata link records without treating linked records as undeclared assets' => static function (TestRunner $t) use ($buildEpubPackage, $opfXml): void {
        ...
        $t->same(strlen(gzdeflate($reviewRecordBytes)), $links[0]['compressedByteLength']);
        $t->same(8, $links[0]['compressionMethod']);
        $t->same('deflated', $links[0]['compressionMethodName']);
        $t->same(true, $links[0]['compressionSupported']);
        $t->same(sprintf('%08x', (int) sprintf('%u', crc32($reviewRecordBytes))), $links[0]['crc32']);
        ...
        $t->same(null, $links[1]['compressedByteLength']);
        $t->same(null, $links[1]['compressionMethod']);
        $t->same(null, $links[1]['compressionMethodName']);
        $t->same(null, $links[1]['compressionSupported']);
        $t->same(null, $links[1]['crc32']);
    ...
    'preserves OPF metadata link media type parameter provenance' => static function (TestRunner $t) use ($buildEpubPackage, $opfXml): void {
        $reviewRecordBytes = '{"@context":"https://schema.org","name":"Parameterized EPUB review record"}';
        $manifestedRecordBytes = '{"@context":"https://schema.org","name":"Manifest-declared EPUB review record"}';
        $badRecordBytes = '{"@context":"https://schema.org","name":"Bad media type record"}';
        $opfWithLinkMediaTypes = str_replace(
            '<item id="toc" href="toc.ncx" media-type="application/x-dtbncx+xml"/>',
            '<item id="publication-record" href="meta/publication-record.json" media-type="application/ld+json; profile=manifested"/>'
            . '<item id="toc" href="toc.ncx" media-type="application/x-dtbncx+xml"/>',
            $opfXml
        );
        $opfWithLinkMediaTypes = str_replace(
            '</metadata>',
            '<link id="review-record" rel="record alternate" href="meta/review-record.json" media-type="application/ld+json; profile=review; charset=UTF-8" properties="schema-org reviewer"/>'
            . '<link id="manifested-record" rel="record" href="meta/publication-record.json"/>'
            . '<link id="bad-record" rel="record" href="meta/bad-record.json" media-type="application/x-review; profile"/>'
            . '</metadata>',
        ...
        $result = (new EpubReader())->readPackage($buildEpubPackage(
            ...
            [
                ['name' => 'OEBPS/meta/review-record.json', 'data' => $reviewRecordBytes],
                ['name' => 'OEBPS/meta/publication-record.json', 'data' => $manifestedRecordBytes],
                ['name' => 'OEBPS/meta/bad-record.json', 'data' => $badRecordBytes],
            ]
        ));

        $links = $result['metadata']['links'];
        $t->same('application/ld+json; profile=review; charset=UTF-8', $links[0]['declaredMediaType']);
        $t->same('application/ld+json; profile=review; charset=UTF-8', $links[0]['effectiveMediaType']);
        $t->same('declared', $links[0]['mediaTypeSource']);
        $t->same('application/ld+json; profile=review; charset=utf-8', $links[0]['normalizedMediaType']);
        $t->same('application/ld+json', $links[0]['baseMediaType']);
        $t->same(['profile' => 'review', 'charset' => 'UTF-8'], $links[0]['mediaTypeParameters']);
        ...
        $t->same(null, $links[1]['declaredMediaType']);
        $t->same('application/ld+json; profile=manifested', $links[1]['effectiveMediaType']);
        $t->same('manifest', $links[1]['mediaTypeSource']);
        $t->same('application/ld+json; profile=manifested', $links[1]['normalizedMediaType']);
        $t->same(['profile' => 'manifested'], $links[1]['mediaTypeParameters']);
        $t->same('publication-record', $links[1]['manifestId']);

        $t->same('application/x-review; profile', $links[2]['declaredMediaType']);
        $t->same('application/x-review', $links[2]['baseMediaType']);
        $t->same(false, $links[2]['mediaTypeSyntaxValid']);
        $t->same([], $links[2]['mediaTypeParameters']);
        $t->same('invalid-metadata-link-media-type-parameter', $links[2]['mediaTypeDiagnostics'][0]['type']);
        $t->same('profile', $links[2]['mediaTypeDiagnostics'][0]['parameter']);
        $t->same('bad-record', $links[2]['mediaTypeDiagnostics'][0]['id']);

        $mediaTypes = $result['metadata']['linkMediaTypes'];
        $t->same(true, $mediaTypes['present']);
        $t->same(3, $mediaTypes['linkCount']);
        $t->same(2, $mediaTypes['declaredLinkCount']);
        $t->same(1, $mediaTypes['manifestInheritedLinkCount']);
        $t->same(2, $mediaTypes['parameterizedLinkCount']);
        $t->same(3, $mediaTypes['parameterCount']);
        $t->same(['charset', 'profile'], $mediaTypes['parameterNames']);
        $t->same(1, $mediaTypes['invalidMediaTypeCount']);
        $t->same(['application/ld+json' => 2, 'application/x-review' => 1], $mediaTypes['baseMediaTypes']);
        $t->same('invalid-metadata-link-media-type-parameter', $mediaTypes['diagnostics'][0]['type']);
        $t->same($links[0]['mediaTypeParameters'], $mediaTypes['items'][0]['mediaTypeParameters']);
        $t->same($links[1]['mediaTypeParameters'], $mediaTypes['items'][1]['mediaTypeParameters']);
        $t->same($links[2]['mediaTypeDiagnostics'], $mediaTypes['items'][2]['diagnostics']);
        ...
        $t->same('manifest', $targetReport['publicationItems'][1]['mediaTypeSource']);
        $t->same($links[2]['mediaTypeDiagnostics'], $targetReport['publicationItems'][2]['mediaTypeDiagnostics']);
        $t->same($mediaTypes, $result['importReport']['metadata']['linkMediaTypes']);
        $t->same($mediaTypes, $result['document']->attr('metadata')['linkMediaTypes']);
    },
    ...
    'reports OPF metadata link target policy for publication resources' => static function (TestRunner $t) use ($buildEpubPackage, $opfXml): void {
        ...
        $t->same(strlen($reviewRecordBytes), $publication[0]['byteLength']);
        $t->same(strlen(gzdeflate($reviewRecordBytes)), $publication[0]['compressedByteLength']);
        $t->same(8, $publication[0]['compressionMethod']);
        $t->same('deflated', $publication[0]['compressionMethodName']);
        $t->same(true, $publication[0]['compressionSupported']);
        $t->same(sprintf('%08x', (int) sprintf('%u', crc32($reviewRecordBytes))), $publication[0]['crc32']);
        ...
        $t->same(strlen($manifestRecordBytes), $publication[1]['byteLength']);
        $t->same(strlen(gzdeflate($manifestRecordBytes)), $publication[1]['compressedByteLength']);
        $t->same(8, $publication[1]['compressionMethod']);
        $t->same('deflated', $publication[1]['compressionMethodName']);
        $t->same(true, $publication[1]['compressionSupported']);
        $t->same(sprintf('%08x', (int) sprintf('%u', crc32($manifestRecordBytes))), $publication[1]['crc32']);
...
XML;
        ...
        $semanticBlock = $result['document']->children[2];
        $semanticNotes = $report['semanticNotes'];
        ...
        $t->same(1, $report['semanticNoteCount']);
        $t->same(2, $report['semanticNoterefCount']);
        $t->same(1, $report['semanticMatchedNoterefCount']);
        $t->same(1, $report['semanticUnmatchedNoterefCount']);
        $t->same(['missing-noteref-note-target'], array_column($report['semanticNoteDiagnostics'], 'type'));

        $t->same(true, $semanticNotes['present']);
        $t->same(1, $semanticNotes['noteCount']);
        $t->same(1, $semanticNotes['footnoteCount']);
        $t->same(0, $semanticNotes['endnoteCount']);
        $t->same(1, $semanticNotes['addressableNoteCount']);
        $t->same(2, $semanticNotes['noterefCount']);
        $t->same(1, $semanticNotes['matchedNoterefCount']);
        $t->same(1, $semanticNotes['unmatchedNoterefCount']);
        $t->same(1, $semanticNotes['missingTargetNoterefCount']);
        $t->same(0, $semanticNotes['nonNoteTargetNoterefCount']);
        $t->same(1, $semanticNotes['diagnosticCount']);
        $t->same('/OEBPS/text/semantic.xhtml#fn-1', $semanticNotes['notes'][0]['target']);
        $t->same('Footnote source text.', $semanticNotes['notesByTarget']['/OEBPS/text/semantic.xhtml#fn-1'][0]['text']);
        $t->same('fn-1', $semanticNotes['noterefs'][0]['noteId']);
        $t->same('footnote', $semanticNotes['noterefs'][0]['noteType']);
        $t->same('Footnote source text.', $semanticNotes['noterefs'][0]['noteText']);
        $t->same(true, $semanticNotes['noterefs'][0]['matched']);
        $t->same(false, $semanticNotes['noterefs'][1]['matched']);
        $t->same('missing-note', $semanticNotes['noterefs'][1]['fragment']);
        $t->same('missing-noteref-note-target', $semanticNotes['noterefs'][1]['diagnostics'][0]['type']);
        $t->same('missing-noteref-note-target', $semanticNotes['diagnostics'][0]['type']);
