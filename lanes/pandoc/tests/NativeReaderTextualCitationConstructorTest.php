...
NATIVE;
        ...
        $t->same('citation', $cluster->type);
        $t->same('[see @smith1899, p. 7; @doe1901]', $cluster->attr('text'));
        $t->same(['smith1899', 'doe1901'], array_map(static fn (array $citation): string => $citation['id'], $cluster->attr('citations')));
        $t->same('see', $cluster->attr('citations')[0]['prefix'][0]->attr('text'));
        $t->same('p. 7', $cluster->attr('citations')[0]['suffix'][0]->attr('text') . ' ' . $cluster->attr('citations')[0]['suffix'][2]->attr('text'));
        $t->same('author_in_text', $cluster->attr('citations')[1]['mode']);
        $t->same(3, $cluster->attr('citations')[0]['citationNoteNum']);
        $t->same(1901, $cluster->attr('citations')[1]['citationHash']);
