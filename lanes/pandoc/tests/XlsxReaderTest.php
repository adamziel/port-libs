...
XML);
    ...
    'matches pinned upstream xlsx reader basic fixture semantics' => static function (TestRunner $t) use ($buildXlsxPackage): void {
        ...
        $mainTable = $document->children[3];
        $secondaryTable = $document->children[5];
        ...
        $t->same(true, $review['sheetNavigationEmitted'] ?? null);
        ...
        $t->same('sheets', $document->children[0]->attr('id'));
        $t->same('Sheets', $document->children[0]->attr('text'));
        $t->same('bullet_list', $document->children[1]->type);
        $t->same('list_item', $document->children[1]->children[0]->type);
        $t->same('#sheet-1', $document->children[1]->children[0]->children[0]->children[0]->attr('url'));
        $t->same('Main', $document->children[1]->children[0]->children[0]->children[0]->children[0]->attr('text'));
        $t->same('#sheet-2', $document->children[1]->children[1]->children[0]->children[0]->attr('url'));
        $t->same('heading', $document->children[2]->type);
        $t->same('sheet-1', $document->children[2]->attr('id'));
        $t->same('Main', $document->children[2]->attr('text'));
        $t->same('heading', $document->children[4]->type);
        $t->same('sheet-2', $document->children[4]->attr('id'));
        $t->same('Secondary', $document->children[4]->attr('text'));
        ...
        $t->contains('Header 2 ( "sheets" , [  ] , [  ] ) [ Str "Sheets" ]', $native);
        $t->contains('BulletList [ [ Plain [ Link ( "" , [  ] , [  ] ) [ Str "Main" ] ( "#sheet-1" , "" ) ] ]', $native);
        ...
        $t->contains('<h2 id="sheets">Sheets</h2>', $blocks);
        $t->contains('<!-- wp:list -->', $blocks);
        $t->contains('<a href="#sheet-1">Main</a>', $blocks);
        $t->contains('<a href="#sheet-2">Secondary</a>', $blocks);
