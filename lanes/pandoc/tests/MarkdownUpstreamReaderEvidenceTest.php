...
return [
    'reports skipped markdown reader evidence when upstream root is absent' => static function (TestRunner $t): void {
        ...
        $t->same(128, $report['staticCurrentEvidence']['nativeExpectationEvidence']['presentFixtureCount']);
        ...
        $t->same(128, $report['nativeAstEvidence']['totalPairCount']);
        $t->same(128, $report['nativeAstEvidence']['normalizedAstMatchCount']);
        $t->same(0, $report['nativeAstEvidence']['unpairedMarkdownFixtureCount']);
        $t->same(true, MarkdownUpstreamReaderEvidence::hasRequiredSelectedFixtureCount($report, 128));
        $t->same(true, MarkdownUpstreamReaderEvidence::hasRequiredStaticCurrentEvidence($report));
        $t->same(true, MarkdownUpstreamReaderEvidence::hasRequiredNativeMappedParity($report, 128));
        ...
        $t->contains('Selected checked-in fixtures: 128', $text);
        $t->contains('Static current evidence: valid-checked-in-current-markdown-reader-evidence checkedInFixtures=128 nativeExpectations=128 nativeManifest=valid-checked-in-current-markdown-native-expectation-evidence', $text);
        $t->contains('Native AST mapped parity: 128/128 status=normalized-ast-equality-observed-not-runner-parity', $text);
    ...
    'reports checked-in current markdown fixture static evidence' => static function (TestRunner $t): void {
        ...
        $t->same(128, $evidence['nativeExpectationEvidence']['expectedFixtureCount']);
        $t->same(128, $evidence['nativeExpectationEvidence']['fixtureCount']);
        $t->same(128, $evidence['nativeExpectationEvidence']['presentFixtureCount']);
    ...
    'cli gates checked-in current markdown fixture evidence through checked-in fixtures mode' => static function (TestRunner $t): void {
        ...
        $failingCommand = str_replace('--require-selected-fixture-count=128', '--require-selected-fixture-count=129', $command) . ' 2>/dev/null';
        $failingOutput = [];
        $failingExitCode = 0;
        ...
        $failingNativeCommand = str_replace('--require-native-mapped-parity=128', '--require-native-mapped-parity=129', $command) . ' 2>/dev/null';
        $failingNativeOutput = [];
    ...
    'generic reader runner artifact tool writes and validates markdown result artifact' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeMarkdownEvidenceTree, $writeRunnerTranscripts): void {
        ...
        try {
            ...
            $t->same(128, $decoded['resultArtifact']['payload']['testCount']);
            $t->same(128, $decoded['resultArtifact']['payload']['passedCount']);
            $t->same(0, $decoded['resultArtifact']['payload']['failedCount']);
            $t->same(128, count($decoded['expectedTestNames']));
            ...
            $t->same(128, $payload['testCount']);
            $t->same(128, $payload['passedCount']);
    ...
    'workflow gates current markdown fixture denominator' => static function (TestRunner $t): void {
        ...
        $t->contains('--require-selected-fixture-count=128', $runnerWorkflow);
        $t->contains('--require-native-mapped-parity=128', $runnerWorkflow);
