...
return [
    'reports skipped markdown reader evidence when upstream root is absent' => static function (TestRunner $t): void {
        ...
        $t->same(45, $report['nativeAstEvidence']['totalPairCount']);
        $t->same(45, $report['nativeAstEvidence']['normalizedAstMatchCount']);
        $t->same(2, $report['nativeAstEvidence']['unpairedMarkdownFixtureCount']);
        ...
        $t->same(true, MarkdownUpstreamReaderEvidence::hasRequiredNativeMappedParity($report, 45));
        ...
        $t->contains('Native AST mapped parity: 45/45 status=normalized-ast-equality-observed-not-runner-parity', $text);
    },
    ...
    'cli gates checked-in current markdown fixture evidence through checked-in fixtures mode' => static function (TestRunner $t): void {
        ...
        $failingNativeCommand = str_replace('--require-native-mapped-parity=45', '--require-native-mapped-parity=46', $command) . ' 2>/dev/null';
        $failingNativeOutput = [];
