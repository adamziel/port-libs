...
return [
    'reports skipped markdown reader evidence when upstream root is absent' => static function (TestRunner $t): void {
        ...
        $t->same(134, $report['staticCurrentEvidence']['nativeExpectationEvidence']['presentFixtureCount']);
        ...
        $t->same(134, $report['nativeAstEvidence']['totalPairCount']);
        $t->same(134, $report['nativeAstEvidence']['normalizedAstMatchCount']);
        $t->same(0, $report['nativeAstEvidence']['unpairedMarkdownFixtureCount']);
        $t->same(true, MarkdownUpstreamReaderEvidence::hasRequiredSelectedFixtureCount($report, 134));
        $t->same(true, MarkdownUpstreamReaderEvidence::hasRequiredStaticCurrentEvidence($report));
        $t->same(true, MarkdownUpstreamReaderEvidence::hasRequiredNativeMappedParity($report, 134));
        ...
        $t->contains('Selected checked-in fixtures: 134', $text);
        $t->contains('Static current evidence: valid-checked-in-current-markdown-reader-evidence checkedInFixtures=134 nativeExpectations=134 nativeManifest=valid-checked-in-current-markdown-native-expectation-evidence', $text);
        $t->contains('Native AST mapped parity: 134/134 status=normalized-ast-equality-observed-not-runner-parity', $text);
    ...
    'reports checked-in current markdown fixture static evidence' => static function (TestRunner $t): void {
        ...
        $t->same(134, $evidence['readerDenominator']['selectedFixtureCount']);
        ...
        $t->same(134, $evidence['checkedInFixtureCount']);
        $t->same('static-checked-in-current-markdown-native-expectation-evidence', $evidence['nativeExpectationEvidence']['kind']);
        $t->same(134, $evidence['nativeExpectationEvidence']['expectedFixtureCount']);
        $t->same(134, $evidence['nativeExpectationEvidence']['fixtureCount']);
        $t->same(134, $evidence['nativeExpectationEvidence']['presentFixtureCount']);
    ...
    'validates hydrated upstream markdown reader source evidence' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeMarkdownEvidenceTree): void {
        ...
        try {
            ...
            $t->same(134, $report['denominator']['selectedFixtureCount']);
    ...
    'cli gates checked-in current markdown fixture evidence through checked-in fixtures mode' => static function (TestRunner $t): void {
        ...
        $command = escapeshellarg(PHP_BINARY)
            ...
            . ' --checked-in-fixtures'
            . ' --json'
            . ' --require-selected-fixture-count=134'
            . ' --require-static-current-evidence'
            . ' --require-native-mapped-parity=134'
        ...
        $t->same(134, $decoded['staticCurrentEvidence']['readerDenominator']['selectedFixtureCount']);
        ...
        $t->same(134, $decoded['nativeAstEvidence']['normalizedAstMatchCount']);
        ...
        $failingCommand = str_replace('--require-selected-fixture-count=134', '--require-selected-fixture-count=135', $command) . ' 2>/dev/null';
        $failingOutput = [];
        $failingExitCode = 0;
        ...
        $failingNativeCommand = str_replace('--require-native-mapped-parity=134', '--require-native-mapped-parity=135', $command) . ' 2>/dev/null';
        $failingNativeOutput = [];
    ...
    'generic reader runner artifact tool writes and validates markdown result artifact' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeMarkdownEvidenceTree, $writeRunnerTranscripts): void {
        ...
        try {
            ...
            $t->same(134, $decoded['resultArtifact']['payload']['testCount']);
            $t->same(134, $decoded['resultArtifact']['payload']['passedCount']);
            $t->same(0, $decoded['resultArtifact']['payload']['failedCount']);
            $t->same(134, count($decoded['expectedTestNames']));
            ...
            $t->same(134, $payload['testCount']);
            $t->same(134, $payload['passedCount']);
    ...
    'workflow gates current markdown fixture denominator' => static function (TestRunner $t): void {
        ...
        $t->contains('--require-selected-fixture-count=134', $workflow);
        $t->contains('--require-native-mapped-parity=134', $workflow);
        $t->contains('--require-mapped-parity=134', $workflow);
        $t->contains('--require-selected-fixture-count=134', $runnerWorkflow);
        $t->contains('--require-native-mapped-parity=134', $runnerWorkflow);
