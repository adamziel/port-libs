...
return [
    'reports skipped markdown reader evidence when upstream root is absent' => static function (TestRunner $t): void {
        ...
        $t->same(true, MarkdownUpstreamReaderEvidence::hasRequiredSelectedFixtureCount($report, 14));
        ...
        $t->contains('Selected checked-in fixtures: 14', $text);
        $t->contains('Static current evidence: valid-checked-in-current-markdown-reader-evidence checkedInFixtures=14', $text);
    },
    ...
    'reports checked-in current markdown fixture static evidence' => static function (TestRunner $t): void {
        ...
        $t->same(14, $evidence['readerDenominator']['selectedFixtureCount']);
        ...
        $t->same(14, $evidence['checkedInFixtureCount']);
        ...
        $t->same('upstream-markdown-inline-code-list-markers.md', $evidence['checkedInFixtures'][13]['name']);
        $t->same('66bbbbf31d775879a2df0a391f044b95241598d42a48a583148224664c50fcef', $evidence['checkedInFixtures'][13]['checkedInFile']['sha256']);
        $t->same(132, $evidence['checkedInFixtures'][13]['checkedInFile']['bytes']);
        $t->true(in_array('lanes/pandoc/tests/MarkdownReaderInlineCodeListMarkerCompletionTest.php', $evidence['checkedInFixtures'][13]['coverageTests'], true));
        $t->true(in_array('lanes/pandoc/tests/MarkdownReaderInlineCodeListMarkerCompletionTest.php', $evidence['checkedInFixtures'][13]['localTestReferences'], true));
    ...
    'validates hydrated upstream markdown reader source evidence' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeMarkdownEvidenceTree): void {
        ...
        try {
            ...
            $t->same(14, $report['denominator']['selectedFixtureCount']);
    ...
    'cli gates checked-in current markdown fixture evidence through checked-in fixtures mode' => static function (TestRunner $t): void {
        ...
        $command = escapeshellarg(PHP_BINARY)
            ...
            . ' --checked-in-fixtures'
            . ' --json'
            . ' --require-selected-fixture-count=14'
        ...
        $t->same(14, $decoded['staticCurrentEvidence']['readerDenominator']['selectedFixtureCount']);
        ...
        $failingCommand = str_replace('--require-selected-fixture-count=14', '--require-selected-fixture-count=15', $command) . ' 2>/dev/null';
        $failingOutput = [];
        $failingExitCode = 0;
