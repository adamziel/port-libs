...
return [
    'reports skipped markdown reader evidence when upstream root is absent' => static function (TestRunner $t): void {
        ...
        $t->same(44, $report['nativeAstEvidence']['totalPairCount']);
        $t->same(44, $report['nativeAstEvidence']['normalizedAstMatchCount']);
        $t->same(3, $report['nativeAstEvidence']['unpairedMarkdownFixtureCount']);
        ...
        $t->same(true, MarkdownUpstreamReaderEvidence::hasRequiredNativeMappedParity($report, 44));
        ...
        $t->contains('Native AST mapped parity: 44/44 status=normalized-ast-equality-observed-not-runner-parity', $text);
    },
    ...
    'cli gates checked-in current markdown fixture evidence through checked-in fixtures mode' => static function (TestRunner $t): void {
        ...
        $failingNativeCommand = str_replace('--require-native-mapped-parity=44', '--require-native-mapped-parity=45', $command) . ' 2>/dev/null';
        $failingNativeOutput = [];
