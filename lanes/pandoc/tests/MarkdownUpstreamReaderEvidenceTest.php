...
return [
    'reports skipped markdown reader evidence when upstream root is absent' => static function (TestRunner $t): void {
        ...
        $t->same(true, MarkdownUpstreamReaderEvidence::hasRequiredSelectedFixtureCount($report, 7));
        ...
        $t->contains('Selected checked-in fixtures: 7', $text);
        $t->contains('Static current evidence: valid-checked-in-current-markdown-reader-evidence checkedInFixtures=7', $text);
    },
    ...
    'reports checked-in current markdown fixture static evidence' => static function (TestRunner $t): void {
        ...
        $t->same(7, $evidence['readerDenominator']['selectedFixtureCount']);
        ...
        $t->same(7, $evidence['checkedInFixtureCount']);
    ...
    'validates hydrated upstream markdown reader source evidence' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeMarkdownEvidenceTree): void {
        ...
        try {
            ...
            $t->same(7, $report['denominator']['selectedFixtureCount']);
    ...
    'cli gates checked-in current markdown fixture evidence without hydrated upstream cache' => static function (TestRunner $t): void {
        ...
        $command = escapeshellarg(PHP_BINARY)
            ...
            . ' --json'
            . ' --require-selected-fixture-count=7'
        ...
        $t->same(7, $decoded['staticCurrentEvidence']['readerDenominator']['selectedFixtureCount']);
        ...
        $failingCommand = str_replace('--require-selected-fixture-count=7', '--require-selected-fixture-count=8', $command) . ' 2>/dev/null';
        $failingOutput = [];
        $failingExitCode = 0;
