...
return [
    'reports skipped markdown reader evidence when upstream root is absent' => static function (TestRunner $t): void {
        ...
        $t->same(true, MarkdownUpstreamReaderEvidence::hasRequiredSelectedFixtureCount($report, 16));
        ...
        $t->contains('Selected checked-in fixtures: 16', $text);
        $t->contains('Static current evidence: valid-checked-in-current-markdown-reader-evidence checkedInFixtures=16', $text);
    },
    ...
    'reports checked-in current markdown fixture static evidence' => static function (TestRunner $t): void {
        ...
        $t->same(16, $evidence['readerDenominator']['selectedFixtureCount']);
        ...
        $t->same(16, $evidence['checkedInFixtureCount']);
    ...
    'validates hydrated upstream markdown reader source evidence' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeMarkdownEvidenceTree): void {
        ...
        try {
            ...
            $t->same(16, $report['denominator']['selectedFixtureCount']);
    ...
    'cli gates checked-in current markdown fixture evidence through checked-in fixtures mode' => static function (TestRunner $t): void {
        ...
        $command = escapeshellarg(PHP_BINARY)
            ...
            . ' --checked-in-fixtures'
            . ' --json'
            . ' --require-selected-fixture-count=16'
        ...
        $t->same(16, $decoded['staticCurrentEvidence']['readerDenominator']['selectedFixtureCount']);
        ...
        $failingCommand = str_replace('--require-selected-fixture-count=16', '--require-selected-fixture-count=17', $command) . ' 2>/dev/null';
        $failingOutput = [];
        $failingExitCode = 0;
