...
return [
    'reports skipped markdown reader evidence when upstream root is absent' => static function (TestRunner $t): void {
        ...
        $t->same(true, MarkdownUpstreamReaderEvidence::hasRequiredSelectedFixtureCount($report, 9));
        ...
        $t->contains('Selected checked-in fixtures: 9', $text);
        $t->contains('Static current evidence: valid-checked-in-current-markdown-reader-evidence checkedInFixtures=9', $text);
    },
    ...
    'reports checked-in current markdown fixture static evidence' => static function (TestRunner $t): void {
        ...
        $t->same(9, $evidence['readerDenominator']['selectedFixtureCount']);
        ...
        $t->same(9, $evidence['checkedInFixtureCount']);
        ...
        $t->same('upstream-command-empty-paragraphs.md', $evidence['checkedInFixtures'][8]['name']);
        $t->same('3cec1e6ab0a690ebe90035bd1a71453c755b6ff198283b8651c8161c20983314', $evidence['checkedInFixtures'][8]['checkedInFile']['sha256']);
        $t->same(1800, $evidence['checkedInFixtures'][8]['checkedInFile']['bytes']);
        $t->true(in_array('lanes/pandoc/tests/MarkdownReaderTest.php', $evidence['checkedInFixtures'][8]['localTestReferences'], true));
    ...
    'validates hydrated upstream markdown reader source evidence' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeMarkdownEvidenceTree): void {
        ...
        try {
            ...
            $t->same(9, $report['denominator']['selectedFixtureCount']);
    ...
    'cli gates checked-in current markdown fixture evidence through checked-in fixtures mode' => static function (TestRunner $t): void {
        ...
        $command = escapeshellarg(PHP_BINARY)
            ...
            . ' --checked-in-fixtures'
            . ' --json'
            . ' --require-selected-fixture-count=9'
        ...
        $t->same(9, $decoded['staticCurrentEvidence']['readerDenominator']['selectedFixtureCount']);
        ...
        $failingCommand = str_replace('--require-selected-fixture-count=9', '--require-selected-fixture-count=10', $command) . ' 2>/dev/null';
        $failingOutput = [];
        $failingExitCode = 0;
