...
return [
    'reports skipped markdown reader evidence when upstream root is absent' => static function (TestRunner $t): void {
        ...
        $t->same(53, $report['staticCurrentEvidence']['nativeExpectationEvidence']['presentFixtureCount']);
        ...
        $t->same(53, $report['nativeAstEvidence']['totalPairCount']);
        $t->same(53, $report['nativeAstEvidence']['normalizedAstMatchCount']);
        $t->same(0, $report['nativeAstEvidence']['unpairedMarkdownFixtureCount']);
        $t->same(true, MarkdownUpstreamReaderEvidence::hasRequiredSelectedFixtureCount($report, 53));
        $t->same(true, MarkdownUpstreamReaderEvidence::hasRequiredStaticCurrentEvidence($report));
        $t->same(true, MarkdownUpstreamReaderEvidence::hasRequiredNativeMappedParity($report, 53));
        ...
        $t->contains('Selected checked-in fixtures: 53', $text);
        $t->contains('Static current evidence: valid-checked-in-current-markdown-reader-evidence checkedInFixtures=53 nativeExpectations=53 nativeManifest=valid-checked-in-current-markdown-native-expectation-evidence', $text);
        $t->contains('Native AST mapped parity: 53/53 status=normalized-ast-equality-observed-not-runner-parity', $text);
    ...
    'reports checked-in current markdown fixture static evidence' => static function (TestRunner $t): void {
        ...
        $t->same(53, $evidence['nativeExpectationEvidence']['expectedFixtureCount']);
        $t->same(53, $evidence['nativeExpectationEvidence']['fixtureCount']);
        $t->same(53, $evidence['nativeExpectationEvidence']['presentFixtureCount']);
        ...
        $t->same('upstream-markdown-definition-list-nested-list.native', $evidence['nativeExpectationEvidence']['checkedInNativeFixtures'][14]['name']);
        $t->same('c22963e813336f323a81771346bf208a06d7cb1b0863aa6ec7073b80fed459e7', $evidence['nativeExpectationEvidence']['checkedInNativeFixtures'][14]['sha256']);
        $t->same(110, $evidence['nativeExpectationEvidence']['checkedInNativeFixtures'][14]['bytes']);
        $t->same('upstream-markdown-definition-list-tight-bodies.native', $evidence['nativeExpectationEvidence']['checkedInNativeFixtures'][15]['name']);
        $t->same('79070528c9fff595351668c7a84bdba031bbbfbfe94ca511a58923559172c36b', $evidence['nativeExpectationEvidence']['checkedInNativeFixtures'][15]['sha256']);
        $t->same(303, $evidence['nativeExpectationEvidence']['checkedInNativeFixtures'][15]['bytes']);
        $t->same('upstream-markdown-inline-note-citations.native', $evidence['nativeExpectationEvidence']['checkedInNativeFixtures'][32]['name']);
        $t->same('04ec5abd9d97dad4c60a9502fcda48ab6d596d22d8773742dfaa787593513ad0', $evidence['nativeExpectationEvidence']['checkedInNativeFixtures'][32]['sha256']);
        $t->same(1115, $evidence['nativeExpectationEvidence']['checkedInNativeFixtures'][32]['bytes']);
        $t->same('upstream-markdown-lhs-inverse-bird-html.native', $evidence['nativeExpectationEvidence']['checkedInNativeFixtures'][33]['name']);
        $t->same('ded30bb9ec48394e6dd42c95ab504456b4f85ff37c1f1a68076bf603260c25d8', $evidence['nativeExpectationEvidence']['checkedInNativeFixtures'][33]['sha256']);
        $t->same(129, $evidence['nativeExpectationEvidence']['checkedInNativeFixtures'][33]['bytes']);
        $t->same('upstream-markdown-strict-compact-heading.native', $evidence['nativeExpectationEvidence']['checkedInNativeFixtures'][46]['name']);
        $t->same('dca4b0a1370fb86f4ea2ab8f19d1254313558ae57954679d1b3ca1fbaa2c8044', $evidence['nativeExpectationEvidence']['checkedInNativeFixtures'][46]['sha256']);
        $t->same(43, $evidence['nativeExpectationEvidence']['checkedInNativeFixtures'][46]['bytes']);
        $t->same('upstream-markdown-task-list.native', $evidence['nativeExpectationEvidence']['checkedInNativeFixtures'][49]['name']);
        $t->same('80ead08088210467f4ce4ab35800a2be9e47b76a5c63fb92c3692ce3b49728df', $evidence['nativeExpectationEvidence']['checkedInNativeFixtures'][49]['sha256']);
        $t->same(846, $evidence['nativeExpectationEvidence']['checkedInNativeFixtures'][49]['bytes']);
        $t->same('upstream-markdown-unbalanced-brackets.native', $evidence['nativeExpectationEvidence']['checkedInNativeFixtures'][50]['name']);
        $t->same('9179740322c07d3717a32cb023a504dafaf164a6f39e2b1cc2fd1f3d11b7e5d8', $evidence['nativeExpectationEvidence']['checkedInNativeFixtures'][50]['sha256']);
        $t->same(34, $evidence['nativeExpectationEvidence']['checkedInNativeFixtures'][50]['bytes']);
        $t->same('upstream-markdown-yaml-metadata.native', $evidence['nativeExpectationEvidence']['checkedInNativeFixtures'][51]['name']);
        $t->same('7ef515f951844ff049c5f6a4c98f42094843aa311061f59eb20d2be6bca6d7b2', $evidence['nativeExpectationEvidence']['checkedInNativeFixtures'][51]['sha256']);
        $t->same(75, $evidence['nativeExpectationEvidence']['checkedInNativeFixtures'][51]['bytes']);
        $t->same('upstream-markdown-z-commonmark-x-grid-table-default.native', $evidence['nativeExpectationEvidence']['checkedInNativeFixtures'][52]['name']);
        $t->same('566596459e8d3e3ab0ae5842fecc54764cad72c34c81a104ca9df16451e5143d', $evidence['nativeExpectationEvidence']['checkedInNativeFixtures'][52]['sha256']);
        $t->same(389, $evidence['nativeExpectationEvidence']['checkedInNativeFixtures'][52]['bytes']);
        ...
        $t->true(in_array('the fifty-three selected checked-in Markdown native expectation snapshots match the expected deterministic manifest hash', $evidence['claimBoundaries']['doesAssert'], true));
    ...
    'cli gates checked-in current markdown fixture evidence through checked-in fixtures mode' => static function (TestRunner $t): void {
        ...
        $failingNativeCommand = str_replace('--require-native-mapped-parity=53', '--require-native-mapped-parity=54', $command) . ' 2>/dev/null';
        $failingNativeOutput = [];
