...
return [
    'reports skipped markdown reader evidence when upstream root is absent' => static function (TestRunner $t): void {
        ...
        $t->same(137, $report['staticCurrentEvidence']['nativeExpectationEvidence']['presentFixtureCount']);
        ...
        $t->same(137, $report['nativeAstEvidence']['totalPairCount']);
        $t->same(137, $report['nativeAstEvidence']['normalizedAstMatchCount']);
        $t->same(0, $report['nativeAstEvidence']['unpairedMarkdownFixtureCount']);
        $t->same(true, MarkdownUpstreamReaderEvidence::hasRequiredSelectedFixtureCount($report, 137));
        $t->same(true, MarkdownUpstreamReaderEvidence::hasRequiredStaticCurrentEvidence($report));
        $t->same(true, MarkdownUpstreamReaderEvidence::hasRequiredNativeMappedParity($report, 137));
    ...
    'reports checked-in current markdown fixture static evidence' => static function (TestRunner $t): void {
        ...
        $t->same(137, $evidence['readerDenominator']['selectedFixtureCount']);
        ...
        $t->same(137, $evidence['checkedInFixtureCount']);
        $t->same('static-checked-in-current-markdown-native-expectation-evidence', $evidence['nativeExpectationEvidence']['kind']);
        $t->same(137, $evidence['nativeExpectationEvidence']['expectedFixtureCount']);
        $t->same(137, $evidence['nativeExpectationEvidence']['fixtureCount']);
        $t->same(137, $evidence['nativeExpectationEvidence']['presentFixtureCount']);
        ...
        $checkedInFixturesByName = [];
        foreach ($evidence['checkedInFixtures'] as $checkedInFixture) {
            $checkedInFixturesByName[$checkedInFixture['name']] = $checkedInFixture;
        }
        $t->same(
        ...
        $t->same(
            '6084124d7e6aca8de969d3a580d65f5ba7c23fab8ffbc6a49736d0385abde671',
            $nativeFixturesByName['upstream-markdown-citation-simple-baseline.native']['sha256'] ?? null
        );
        $t->same(
            304,
            $nativeFixturesByName['upstream-markdown-citation-simple-baseline.native']['bytes'] ?? null
        );
        $t->same(
            '74a943dce8555f6469529658ff5638464a7d727c640e5bb775abac5f488e25c3',
            $nativeFixturesByName['upstream-markdown-zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz-gfm-definition-list-profile.native']['sha256'] ?? null
        );
        $t->same(
            292,
            $nativeFixturesByName['upstream-markdown-zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz-gfm-definition-list-profile.native']['bytes'] ?? null
        );
        $t->same(
            '35d54d8906f82622900ca3d1852607307e4a003fc095ab8b8fa150c1c3f72c9c',
            $nativeFixturesByName['upstream-markdown-definition-list-laziness.native']['sha256'] ?? null
        );
        $t->same(
            159,
            $nativeFixturesByName['upstream-markdown-definition-list-laziness.native']['bytes'] ?? null
        );
        $t->same(
            '860081e856874831ac302b2355a26c8d971d15789afbf36e41cc4ee54f12fab0',
            $nativeFixturesByName['upstream-markdown-gfm-math-fence-profile.native']['sha256'] ?? null
        );
        $t->same(
            112,
            $nativeFixturesByName['upstream-markdown-gfm-math-fence-profile.native']['bytes'] ?? null
        );
        $t->same(
            '948d631affcf3df7a41e2e23dab3b4424d38ecbb75075a278b2c93e01762e45e',
            $nativeFixturesByName['upstream-markdown-zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz-markdown-github-yaml-profile.native']['sha256'] ?? null
        );
        $t->same(
            159,
            $nativeFixturesByName['upstream-markdown-zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz-markdown-github-yaml-profile.native']['bytes'] ?? null
        );
        $t->same(
            '479bbae933a8d15b634b511619dd1ec259b9e99b79b89c6ce5f51d31a8143ac5',
            $nativeFixturesByName['upstream-markdown-z-old-dashes-profile.native']['sha256'] ?? null
        );
        $t->same(
            233,
            $nativeFixturesByName['upstream-markdown-z-old-dashes-profile.native']['bytes'] ?? null
        );
        $t->same(
            'e7b96caa6520efc1dc375cb49177e6691fd538ac792db336e2be77c9db3ae480',
            $nativeFixturesByName['upstream-markdown-z-fancy-list-parenthesized-profile.native']['sha256'] ?? null
        );
        $t->same(
            320,
            $nativeFixturesByName['upstream-markdown-z-fancy-list-parenthesized-profile.native']['bytes'] ?? null
        );
        $t->same(
            '5dbdcb89d0aeb633b4c759fd4f43be3b27fb6ca1411639a62628a543dcfccaad',
            $nativeFixturesByName['upstream-markdown-two-space-line-break.native']['sha256'] ?? null
        );
        $t->same(
            50,
            $nativeFixturesByName['upstream-markdown-two-space-line-break.native']['bytes'] ?? null
        );
        $oldDashesFixture = $checkedInFixturesByName['upstream-markdown-z-old-dashes-profile.md'] ?? null;
        $t->true(is_array($oldDashesFixture));
        $oldDashesFixture ??= ['checkedInFile' => [], 'coverageTests' => [], 'localTestReferences' => []];
        $t->same('markdown+old_dashes+smart paired dash-run profile', $oldDashesFixture['formatProfile'] ?? null);
        $t->same('f1ff8794375eca5989866b585159bbca22cf09c6e904520a727bd351206d9298', $oldDashesFixture['checkedInFile']['sha256'] ?? null);
        $t->same(46, $oldDashesFixture['checkedInFile']['bytes'] ?? null);
        $t->true(in_array('lanes/pandoc/tests/MarkdownReaderOldDashesProfileFixtureCompletionTest.php', $oldDashesFixture['coverageTests'], true));
        $t->true(in_array('lanes/pandoc/tests/MarkdownNativeAstComparisonHarnessTest.php', $oldDashesFixture['coverageTests'], true));
        $t->true(in_array('lanes/pandoc/tests/MarkdownReaderOldDashesProfileFixtureCompletionTest.php', $oldDashesFixture['localTestReferences'], true));
        $parenthesizedFancyFixture = $checkedInFixturesByName['upstream-markdown-z-fancy-list-parenthesized-profile.md'] ?? null;
        $t->true(is_array($parenthesizedFancyFixture));
        $parenthesizedFancyFixture ??= ['checkedInFile' => [], 'coverageTests' => [], 'localTestReferences' => []];
        $t->same('Pandoc 3.10 markdown+fancy_lists parenthesized Roman and alpha marker profile probe', $parenthesizedFancyFixture['sourceReference'] ?? null);
        $t->same('markdown+fancy_lists lower-roman and lower-alpha two-parenthesis ordered markers', $parenthesizedFancyFixture['formatProfile'] ?? null);
        $t->same('e1fbc26de79ed2924d7bb178af704d714682b46f98f7c775717a66ce53e72c09', $parenthesizedFancyFixture['checkedInFile']['sha256'] ?? null);
        $t->same(58, $parenthesizedFancyFixture['checkedInFile']['bytes'] ?? null);
        $t->true(in_array('lanes/pandoc/tests/MarkdownReaderFancyListFixtureCompletionTest.php', $parenthesizedFancyFixture['coverageTests'], true));
        $t->true(in_array('lanes/pandoc/tests/MarkdownNativeAstComparisonHarnessTest.php', $parenthesizedFancyFixture['coverageTests'], true));
        $t->true(in_array('lanes/pandoc/tests/MarkdownReaderFancyListFixtureCompletionTest.php', $parenthesizedFancyFixture['localTestReferences'], true));
        $t->true(in_array('lanes/pandoc/tests/MarkdownNativeAstComparisonHarnessTest.php', $parenthesizedFancyFixture['localTestReferences'], true));
        $twoSpaceLineBreakFixture = $checkedInFixturesByName['upstream-markdown-two-space-line-break.md'] ?? null;
        $t->true(is_array($twoSpaceLineBreakFixture));
        $twoSpaceLineBreakFixture ??= ['checkedInFile' => [], 'coverageTests' => [], 'localTestReferences' => []];
        $t->same('Pandoc 3.10 markdown trailing two-space hard line break probe', $twoSpaceLineBreakFixture['sourceReference'] ?? null);
        $t->same('markdown default trailing two-space line break as LineBreak', $twoSpaceLineBreakFixture['formatProfile'] ?? null);
        $t->same('421f07b92161d9e26a8fd2e5fd8a3dc9edd66b12f6dfbbde13ab1d9e2e41c8cd', $twoSpaceLineBreakFixture['checkedInFile']['sha256'] ?? null);
        $t->same(13, $twoSpaceLineBreakFixture['checkedInFile']['bytes'] ?? null);
        $t->true(in_array('lanes/pandoc/tests/MarkdownReaderTwoSpaceLineBreakFixtureCompletionTest.php', $twoSpaceLineBreakFixture['coverageTests'], true));
        $t->true(in_array('lanes/pandoc/tests/MarkdownNativeAstComparisonHarnessTest.php', $twoSpaceLineBreakFixture['coverageTests'], true));
        $t->true(in_array('lanes/pandoc/tests/MarkdownReaderTwoSpaceLineBreakFixtureCompletionTest.php', $twoSpaceLineBreakFixture['localTestReferences'], true));
        $t->true(in_array('lanes/pandoc/tests/MarkdownNativeAstComparisonHarnessTest.php', $twoSpaceLineBreakFixture['localTestReferences'], true));
        ...
        $bareUriPortPathFixture = null;
        foreach ($evidence['checkedInFixtures'] as $checkedInFixture) {
            if (($checkedInFixture['name'] ?? null) === 'upstream-markdown-bare-uri-port-path-boundaries.md') {
                $bareUriPortPathFixture = $checkedInFixture;
                break;
            }
        }
        $t->true(is_array($bareUriPortPathFixture));
        $bareUriPortPathFixture ??= ['checkedInFile' => [], 'coverageTests' => [], 'localTestReferences' => []];
        $t->same('upstream-markdown-bare-uri-port-path-boundaries.md', $bareUriPortPathFixture['name']);
        $t->same('2785c81b2106589d4e877f6ddd13157c9035fc2a8c6b1e757073e555bc85a852', $bareUriPortPathFixture['checkedInFile']['sha256']);
        $t->same(82, $bareUriPortPathFixture['checkedInFile']['bytes']);
        $t->same('markdown+autolink_bare_uris port and plus path boundaries', $bareUriPortPathFixture['formatProfile']);
        $t->true(in_array('lanes/pandoc/tests/MarkdownReaderBareUriQueryBoundaryFixtureCompletionTest.php', $bareUriPortPathFixture['coverageTests'], true));
        $t->true(in_array('lanes/pandoc/tests/MarkdownNativeAstComparisonHarnessTest.php', $bareUriPortPathFixture['coverageTests'], true));
        $t->true(in_array('lanes/pandoc/tests/MarkdownReaderBareUriQueryBoundaryFixtureCompletionTest.php', $bareUriPortPathFixture['localTestReferences'], true));
        ...
        $bareUriUnicodePathFixture = null;
        foreach ($evidence['checkedInFixtures'] as $checkedInFixture) {
            if (($checkedInFixture['name'] ?? null) === 'upstream-markdown-bare-uri-unicode-path.md') {
                $bareUriUnicodePathFixture = $checkedInFixture;
                break;
            }
        }
        $t->true(is_array($bareUriUnicodePathFixture));
        $bareUriUnicodePathFixture ??= ['checkedInFile' => [], 'coverageTests' => [], 'localTestReferences' => []];
        $t->same('upstream-markdown-bare-uri-unicode-path.md', $bareUriUnicodePathFixture['name']);
        $t->same('3d28e9f4f4dbc355b43c1d205778926a1480acad2e7b10290ed7391c8bb15369', $bareUriUnicodePathFixture['checkedInFile']['sha256']);
        $t->same(51, $bareUriUnicodePathFixture['checkedInFile']['bytes']);
        $t->same('markdown+autolink_bare_uris Unicode path and trailing comma boundary', $bareUriUnicodePathFixture['formatProfile']);
        $t->true(in_array('lanes/pandoc/tests/MarkdownReaderBareUriSchemeBoundaryFixtureCompletionTest.php', $bareUriUnicodePathFixture['coverageTests'], true));
        $t->true(in_array('lanes/pandoc/tests/MarkdownNativeAstComparisonHarnessTest.php', $bareUriUnicodePathFixture['coverageTests'], true));
        $t->true(in_array('lanes/pandoc/tests/MarkdownReaderBareUriSchemeBoundaryFixtureCompletionTest.php', $bareUriUnicodePathFixture['localTestReferences'], true));
        $t->true(in_array('lanes/pandoc/tests/MarkdownNativeAstComparisonHarnessTest.php', $bareUriUnicodePathFixture['localTestReferences'], true));
        ...
        $citationSimpleFixture = null;
        foreach ($evidence['checkedInFixtures'] as $checkedInFixture) {
            if (($checkedInFixture['name'] ?? null) === 'upstream-markdown-citation-simple-baseline.md') {
                $citationSimpleFixture = $checkedInFixture;
                break;
            }
        }
        $t->true(is_array($citationSimpleFixture));
        $citationSimpleFixture ??= ['checkedInFile' => [], 'coverageTests' => [], 'localTestReferences' => []];
        $t->same('upstream-markdown-citation-simple-baseline.md', $citationSimpleFixture['name']);
        $t->same('Tests.Readers.Markdown citations/simple', $citationSimpleFixture['sourceReference']);
        $t->same('markdown+citations AuthorInText baseline citation', $citationSimpleFixture['formatProfile']);
        $t->same('bc4f311dfc4cec47a0db605bae407fa3fee60201e3b25eaa36654299959d027b', $citationSimpleFixture['checkedInFile']['sha256']);
        $t->same(7, $citationSimpleFixture['checkedInFile']['bytes']);
        $t->true(in_array('lanes/pandoc/tests/MarkdownNativeAstComparisonHarnessTest.php', $citationSimpleFixture['coverageTests'], true));
        $t->true(in_array('lanes/pandoc/tests/MarkdownNativeAstComparisonHarnessTest.php', $citationSimpleFixture['localTestReferences'], true));
        $gfmDefinitionListProfileFixture = null;
        foreach ($evidence['checkedInFixtures'] as $checkedInFixture) {
            if (($checkedInFixture['name'] ?? null) === 'upstream-markdown-zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz-gfm-definition-list-profile.md') {
                $gfmDefinitionListProfileFixture = $checkedInFixture;
                break;
            }
        }
        $t->true(is_array($gfmDefinitionListProfileFixture));
        $gfmDefinitionListProfileFixture ??= ['checkedInFile' => [], 'coverageTests' => [], 'localTestReferences' => []];
        $t->same('upstream-markdown-zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz-gfm-definition-list-profile.md', $gfmDefinitionListProfileFixture['name']);
        $t->same('Pandoc 3.10 gfm+definition_lists definition-list profile probe', $gfmDefinitionListProfileFixture['sourceReference']);
        $t->same('gfm+definition_lists definition-list profile', $gfmDefinitionListProfileFixture['formatProfile']);
        $t->same('15f9f02ecd616b4b847d7dacf103790e57bc41a850121c98abface8f3130a756', $gfmDefinitionListProfileFixture['checkedInFile']['sha256']);
        $t->same(33, $gfmDefinitionListProfileFixture['checkedInFile']['bytes']);
        $t->true(in_array('lanes/pandoc/tests/MarkdownReaderGfmDefinitionListProfileFixtureCompletionTest.php', $gfmDefinitionListProfileFixture['coverageTests'], true));
        $t->true(in_array('lanes/pandoc/tests/MarkdownNativeAstComparisonHarnessTest.php', $gfmDefinitionListProfileFixture['coverageTests'], true));
        $t->true(in_array('lanes/pandoc/tests/MarkdownReaderGfmDefinitionListProfileFixtureCompletionTest.php', $gfmDefinitionListProfileFixture['localTestReferences'], true));
        $t->true(in_array('lanes/pandoc/tests/MarkdownNativeAstComparisonHarnessTest.php', $gfmDefinitionListProfileFixture['localTestReferences'], true));
        $tableAttributesDisabledFixture = null;
        $gfmMathFenceFixture = null;
        $githubYamlProfileFixture = null;
        foreach ($evidence['checkedInFixtures'] as $checkedInFixture) {
            if (($checkedInFixture['name'] ?? null) === 'upstream-markdown-table-attributes-disabled-profile.md') {
                $tableAttributesDisabledFixture = $checkedInFixture;
            }
            if (($checkedInFixture['name'] ?? null) === 'upstream-markdown-gfm-math-fence-profile.md') {
                $gfmMathFenceFixture = $checkedInFixture;
            }
            if (($checkedInFixture['name'] ?? null) === 'upstream-markdown-zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz-markdown-github-yaml-profile.md') {
                $githubYamlProfileFixture = $checkedInFixture;
            }
        }
        $t->true(is_array($tableAttributesDisabledFixture));
        $tableAttributesDisabledFixture ??= ['checkedInFile' => [], 'coverageTests' => [], 'localTestReferences' => []];
        $t->same('upstream-markdown-table-attributes-disabled-profile.md', $tableAttributesDisabledFixture['name']);
        $t->same('Pandoc 3.10 markdown-table_attributes table caption attribute literal profile', $tableAttributesDisabledFixture['sourceReference']);
        $t->same('markdown-table_attributes table caption attributes remain literal', $tableAttributesDisabledFixture['formatProfile']);
        $t->same('a7bcfe06e2566f9297ee294e5496fadb2760658d6ed58c53e069cdc2097bc06f', $tableAttributesDisabledFixture['checkedInFile']['sha256']);
        $t->same(127, $tableAttributesDisabledFixture['checkedInFile']['bytes']);
        $t->true(in_array('lanes/pandoc/tests/MarkdownReaderTableAttributeProfileFixtureCompletionTest.php', $tableAttributesDisabledFixture['coverageTests'], true));
        $t->true(in_array('lanes/pandoc/tests/MarkdownNativeAstComparisonHarnessTest.php', $tableAttributesDisabledFixture['coverageTests'], true));
        $t->true(in_array('lanes/pandoc/tests/MarkdownReaderTableAttributeProfileFixtureCompletionTest.php', $tableAttributesDisabledFixture['localTestReferences'], true));
        $t->true(in_array('lanes/pandoc/tests/MarkdownNativeAstComparisonHarnessTest.php', $tableAttributesDisabledFixture['localTestReferences'], true));
        $t->true(is_array($gfmMathFenceFixture));
        $gfmMathFenceFixture ??= ['checkedInFile' => [], 'coverageTests' => [], 'localTestReferences' => []];
        $t->same('upstream-markdown-gfm-math-fence-profile.md', $gfmMathFenceFixture['name']);
        $t->same('Pandoc 3.10 gfm tex_math_gfm fenced display math profile probe', $gfmMathFenceFixture['sourceReference']);
        $t->same('gfm tex_math_gfm fenced code block converted to display math', $gfmMathFenceFixture['formatProfile']);
        $t->same('4cee04122c802974cf1e28f16cde8cd8cf09c0f0a35be9e8b1f2672d9625394e', $gfmMathFenceFixture['checkedInFile']['sha256']);
        $t->same(35, $gfmMathFenceFixture['checkedInFile']['bytes']);
        $t->true(in_array('lanes/pandoc/tests/MarkdownReaderGfmMathFenceProfileCompletionTest.php', $gfmMathFenceFixture['coverageTests'], true));
        $t->true(in_array('lanes/pandoc/tests/MarkdownNativeAstComparisonHarnessTest.php', $gfmMathFenceFixture['coverageTests'], true));
        $t->true(in_array('lanes/pandoc/tests/MarkdownReaderGfmMathFenceProfileCompletionTest.php', $gfmMathFenceFixture['localTestReferences'], true));
        $t->true(in_array('lanes/pandoc/tests/MarkdownNativeAstComparisonHarnessTest.php', $gfmMathFenceFixture['localTestReferences'], true));
        $t->true(is_array($githubYamlProfileFixture));
        $githubYamlProfileFixture ??= ['checkedInFile' => [], 'coverageTests' => [], 'localTestReferences' => []];
        $t->same('upstream-markdown-zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz-markdown-github-yaml-profile.md', $githubYamlProfileFixture['name']);
        $t->same('Pandoc 3.10 markdown_github YAML-looking front matter profile probe', $githubYamlProfileFixture['sourceReference']);
        $t->same('markdown_github keeps YAML-looking front matter as HorizontalRule and Setext heading body', $githubYamlProfileFixture['formatProfile']);
        $t->same('7afb355ffac7bad52a1f21c8ab041f9870a90d6a71106d6e60b3e08f47ab71bc', $githubYamlProfileFixture['checkedInFile']['sha256']);
        $t->same(33, $githubYamlProfileFixture['checkedInFile']['bytes']);
        $t->true(in_array('lanes/pandoc/tests/MarkdownReaderGithubYamlProfileCompletionTest.php', $githubYamlProfileFixture['coverageTests'], true));
        $t->true(in_array('lanes/pandoc/tests/MarkdownNativeAstComparisonHarnessTest.php', $githubYamlProfileFixture['coverageTests'], true));
        $t->true(in_array('lanes/pandoc/tests/MarkdownReaderGithubYamlProfileCompletionTest.php', $githubYamlProfileFixture['localTestReferences'], true));
        $t->true(in_array('lanes/pandoc/tests/MarkdownNativeAstComparisonHarnessTest.php', $githubYamlProfileFixture['localTestReferences'], true));
    ...
    'validates hydrated upstream markdown reader source evidence' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeMarkdownEvidenceTree): void {
        ...
        try {
            ...
            $t->same(137, $report['denominator']['selectedFixtureCount']);
    ...
    'cli gates checked-in current markdown fixture evidence through checked-in fixtures mode' => static function (TestRunner $t): void {
        ...
        $command = escapeshellarg(PHP_BINARY)
            ...
            . ' --checked-in-fixtures'
            . ' --json'
            . ' --require-selected-fixture-count=137'
            . ' --require-static-current-evidence'
            . ' --require-native-mapped-parity=137'
        ...
        $t->same(137, $decoded['staticCurrentEvidence']['readerDenominator']['selectedFixtureCount']);
        ...
        $t->same(137, $decoded['nativeAstEvidence']['normalizedAstMatchCount']);
        ...
        $failingNativeCommand = str_replace('--require-native-mapped-parity=137', '--require-native-mapped-parity=138', $command) . ' 2>/dev/null';
        $failingNativeOutput = [];
    ...
    'generic reader runner artifact tool writes and validates markdown result artifact' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeMarkdownEvidenceTree, $writeRunnerTranscripts): void {
        ...
        try {
            ...
            $t->same(137, $decoded['resultArtifact']['payload']['testCount']);
            $t->same(137, $decoded['resultArtifact']['payload']['passedCount']);
            $t->same(0, $decoded['resultArtifact']['payload']['failedCount']);
            $t->same(137, count($decoded['expectedTestNames']));
            ...
            $t->same(137, $payload['testCount']);
            $t->same(137, $payload['passedCount']);
    ...
    'workflow gates current markdown fixture denominator' => static function (TestRunner $t): void {
        ...
        $t->contains('--require-selected-fixture-count=137', $workflow);
        $t->contains('--require-native-mapped-parity=137', $workflow);
        $t->contains('--require-mapped-parity=137', $workflow);
        $t->contains('--require-selected-fixture-count=137', $runnerWorkflow);
        $t->contains('--require-native-mapped-parity=137', $runnerWorkflow);
        $t->contains('lanes/pandoc/tests/MarkdownReaderGithubYamlProfileCompletionTest.php', $workflow);
        $t->contains('lanes/pandoc/tests/MarkdownReaderGfmDefinitionListProfileFixtureCompletionTest.php', $workflow);
        $t->contains('lanes/pandoc/tests/MarkdownReaderDefinitionListFixtureCompletionTest.php', $workflow);
