...
return [
    'reports skipped markdown reader evidence when upstream root is absent' => static function (TestRunner $t): void {
        ...
        $t->same(123, $report['staticCurrentEvidence']['nativeExpectationEvidence']['presentFixtureCount']);
        ...
        $t->same(123, $report['nativeAstEvidence']['totalPairCount']);
        $t->same(123, $report['nativeAstEvidence']['normalizedAstMatchCount']);
        $t->same(0, $report['nativeAstEvidence']['unpairedMarkdownFixtureCount']);
        $t->same(true, MarkdownUpstreamReaderEvidence::hasRequiredSelectedFixtureCount($report, 123));
        $t->same(true, MarkdownUpstreamReaderEvidence::hasRequiredStaticCurrentEvidence($report));
        $t->same(true, MarkdownUpstreamReaderEvidence::hasRequiredNativeMappedParity($report, 123));
        ...
        $t->contains('Selected checked-in fixtures: 123', $text);
        $t->contains('Static current evidence: valid-checked-in-current-markdown-reader-evidence checkedInFixtures=123 nativeExpectations=123 nativeManifest=valid-checked-in-current-markdown-native-expectation-evidence', $text);
        $t->contains('Native AST mapped parity: 123/123 status=normalized-ast-equality-observed-not-runner-parity', $text);
    ...
    'reports checked-in current markdown fixture static evidence' => static function (TestRunner $t): void {
        ...
        $t->same(123, $evidence['nativeExpectationEvidence']['expectedFixtureCount']);
        $t->same(123, $evidence['nativeExpectationEvidence']['fixtureCount']);
        $t->same(123, $evidence['nativeExpectationEvidence']['presentFixtureCount']);
    ...
    'cli gates checked-in current markdown fixture evidence through checked-in fixtures mode' => static function (TestRunner $t): void {
        ...
        $failingCommand = str_replace('--require-selected-fixture-count=123', '--require-selected-fixture-count=124', $command) . ' 2>/dev/null';
        $failingOutput = [];
        $failingExitCode = 0;
        ...
        $failingNativeCommand = str_replace('--require-native-mapped-parity=123', '--require-native-mapped-parity=124', $command) . ' 2>/dev/null';
        $failingNativeOutput = [];
    ...
    'generic reader runner artifact tool writes and validates markdown result artifact' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeMarkdownEvidenceTree, $writeRunnerTranscripts): void {
        ...
        try {
            ...
            $t->same(123, $decoded['resultArtifact']['payload']['testCount']);
            $t->same(123, $decoded['resultArtifact']['payload']['passedCount']);
            $t->same(0, $decoded['resultArtifact']['payload']['failedCount']);
            $t->same(123, count($decoded['expectedTestNames']));
            ...
            $t->same(123, $payload['testCount']);
            $t->same(123, $payload['passedCount']);
    ...
    'workflow gates current markdown fixture denominator' => static function (TestRunner $t): void {
        ...
        $t->contains('--require-selected-fixture-count=123', $runnerWorkflow);
        $t->contains('--require-native-mapped-parity=123', $runnerWorkflow);
