...
return [
    'reports skipped markdown reader evidence when upstream root is absent' => static function (TestRunner $t): void {
        ...
        $t->same(true, MarkdownUpstreamReaderEvidence::hasRequiredSelectedFixtureCount($report, 8));
        ...
        $t->contains('Selected checked-in fixtures: 8', $text);
        $t->contains('Static current evidence: valid-checked-in-current-markdown-reader-evidence checkedInFixtures=8', $text);
    },
    ...
    'reports checked-in current markdown fixture static evidence' => static function (TestRunner $t): void {
        ...
        $t->same(8, $evidence['readerDenominator']['selectedFixtureCount']);
        ...
        $t->same(8, $evidence['checkedInFixtureCount']);
        ...
        $t->same('upstream-command-empty-paragraphs.md', $evidence['checkedInFixtures'][7]['name']);
        $t->same('3cec1e6ab0a690ebe90035bd1a71453c755b6ff198283b8651c8161c20983314', $evidence['checkedInFixtures'][7]['checkedInFile']['sha256']);
        $t->same(1800, $evidence['checkedInFixtures'][7]['checkedInFile']['bytes']);
        $t->true(in_array('lanes/pandoc/tests/MarkdownReaderTest.php', $evidence['checkedInFixtures'][7]['localTestReferences'], true));
    ...
    'validates hydrated upstream markdown reader source evidence' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeMarkdownEvidenceTree): void {
        ...
        try {
            ...
            $t->same(8, $report['denominator']['selectedFixtureCount']);
