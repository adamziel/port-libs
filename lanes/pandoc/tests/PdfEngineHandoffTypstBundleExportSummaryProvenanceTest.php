...
$document = static function (): AstNode {
    $parsed = (new MarkdownReader())->read('Typst bundle export summary packet keeps bundle output controls reviewable.');
...
return [
    'records mapped typst bundle export summary provenance case count' => static function (TestRunner $t): void {
        ...
        $t->same(39, $manifest['typstBundleExportSummaryProvenanceAssertions'] ?? null);
        $t->same(1, $manifest['benchmarkDenominator']['breakdown']['mappedTypstBundleExportSummaryProvenanceCases'] ?? null);
        $t->same(39, $manifest['benchmarkDenominator']['breakdown']['typstBundleExportSummaryProvenanceAssertions'] ?? null);
        $t->same(1, $manifest['benchmarkDenominator']['inventory']['mappedTypstBundleExportSummaryProvenanceCases'] ?? null);
        $t->same(39, $manifest['benchmarkDenominator']['inventory']['typstBundleExportSummaryProvenanceAssertions'] ?? null);
        $t->same(1, $manifest['inventory']['mappedTypstBundleExportSummaryProvenanceCases'] ?? null);
        $t->same(39, $manifest['inventory']['typstBundleExportSummaryProvenanceAssertions'] ?? null);
    },

    'summarizes typst bundle export provenance without executing' => static function (TestRunner $t) use ($document): void {
        ...
        $pdfBytes = "%PDF-1.7\n% fake Typst bundle export summary packet\n%%EOF\n";
        ...
        ], $missingFeaturePlan['typstBoundarySummary']['bundleExportIssues']);
        $t->same('review', $summary['reviewStatus']);
        $t->same(1, $summary['bundleExportControlCount']);
        $t->same(true, $summary['bundleExportPresent']);
        ...
        $t->contains('typst-boundary-summary-bundle-features:2', implode(',', $plan['diagnostics']));
        ...
        $t->same($plan['typstBoundaryProvenance']['bundleExport'], $result['artifactProvenanceReview']['typstBoundaryProvenance']['bundleExport']);
        $t->same($plan['typstBoundaryProvenance']['bundleExport'], $sequence['finalTypstBoundaryProvenance']['bundleExport']);
