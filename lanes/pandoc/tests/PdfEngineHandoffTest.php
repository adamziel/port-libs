...
return [
    ...
    'plans typst package cache path alias boundary provenance without executing' => static function (TestRunner $t) use ($document): void {
        ...
        $plan = $handoff->plan($document(), [
            'engine' => 'typst',
            'outputPath' => 'build/cache-path.pdf',
            'source' => '= Typst Package Cache Path Packet',
            'engineOptions' => [
                '--package-cache-path=cache/typst',
                '--package-cache-path',
                'https://cache.example.invalid/typst',
            ],
        ]);
        $pdfBytes = "%PDF-1.7\n% fake Typst package cache path packet\n%%EOF\n";
        $expected = [
            'reviewStatus' => 'review',
            'root' => null,
            'fontPaths' => [],
    ...
    'plans typst system font boundary provenance without executing' => static function (TestRunner $t) use ($document): void {
        ...
        $plan = $handoff->plan($document(), [
            'engine' => 'typst',
            'outputPath' => 'build/system-font-boundary.pdf',
            'source' => '= Typst System Font Boundary Packet',
            'engineOptions' => [
                '--ignore-system-fonts',
                '--font-path=review-fonts',
            ],
        ]);
        ...
        $result = $handoff->fakeRun($plan, [
            'files' => [
                'build/system-font-boundary.pdf' => $pdfBytes,
            ],
        ]);
        $sequence = $handoff->fakeRunSequence($plan, [[
            'files' => [
        ...
        $t->contains('typst-ignore-system-fonts:1', implode(',', $plan['diagnostics']));
        $t->same(true, $result['ok']);
        $t->same($expected, $result['typstBoundaryProvenance']);
        $t->same($expected, $result['artifactProvenanceReview']['typstBoundaryProvenance']);
        $t->same('ok', $result['artifactProvenanceReview']['reviewStatus']);
    ...
    'plans typst embedded font boundary provenance without executing' => static function (TestRunner $t) use ($document): void {
        ...
        $plan = $handoff->plan($document(), [
            'engine' => 'typst',
            'outputPath' => 'build/embedded-font-boundary.pdf',
            'source' => '= Typst Embedded Font Boundary Packet',
            'engineOptions' => [
                '--ignore-embedded-fonts',
            ],
        ]);
        $pdfBytes = "%PDF-1.7\n% fake Typst embedded font boundary packet\n%%EOF\n";
        $expected = [
            'reviewStatus' => 'ok',
            'root' => null,
            'fontPaths' => [],
        ...
        $result = $handoff->fakeRun($plan, [
            'files' => [
                'build/embedded-font-boundary.pdf' => $pdfBytes,
            ],
        ]);
        $sequence = $handoff->fakeRunSequence($plan, [[
            'files' => [
    ...
    'maps typst shadowed environment controls into boundary matrix without executing' => static function (TestRunner $t) use ($document): void {
        $handoff = new PdfEngineHandoff();
        $environmentTimestamp = '1700000000';
        $selectedTimestamp = '1700000100';
        $plan = $handoff->plan($document(), [
            'engine' => 'typst',
            'outputPath' => 'build/environment-shadow-matrix.pdf',
            'source' => '= Typst Environment Shadow Matrix Packet',
            'engineOptions' => [
        ...
        $pdfBytes = "%PDF-1.7\n% fake Typst environment shadow matrix packet\n%%EOF\n";

        $result = $handoff->fakeRun($plan, [
            'files' => [
                'build/environment-shadow-matrix.pdf' => $pdfBytes,
            ],
        ]);
        $sequence = $handoff->fakeRunSequence($plan, [[
            'files' => [
        ...
        $t->same(8, $plan['typstBoundaryMatrix']['issueCount']);
        $t->same(2, $cases['environment-shadows']['observed']);
        $t->same(2, $cases['environment-shadows']['details']['environmentVariableCount']);
        $t->same(2, $cases['environment-shadows']['details']['shadowedCount']);
        $t->same(['SOURCE_DATE_EPOCH', 'TYPST_FEATURES'], $cases['environment-shadows']['details']['shadowedVariables']);
        $t->same(3, $cases['feature-gates']['observed']);
        $t->same(2, $cases['feature-gates']['details']['featureCount']);
        $t->same(1, $cases['feature-gates']['details']['environmentFeatureCount']);
        $t->same(['html', 'packages'], $cases['feature-gates']['details']['features']);
        $t->same(2, $cases['creation-timestamp']['observed']);
        ...
        $t->contains('environment-shadows:creation-timestamp-environment-shadowed', implode(',', $plan['typstBoundaryMatrix']['issues']));
        $t->contains('environment-shadows:features-environment-shadowed', implode(',', $plan['typstBoundaryMatrix']['issues']));
        $t->contains('feature-gates:features-invalid-token-boundary:unsafe feature', implode(',', $plan['typstBoundaryMatrix']['issues']));
    ...
    'preserves typst font path environment uri provenance without splitting scheme boundary' => static function (TestRunner $t) use ($document): void {
        ...
        $plan = $handoff->plan($document(), [
            'engine' => 'typst',
            'outputPath' => 'build/font-env-uri.pdf',
            'source' => '= Typst Font Environment URI Packet',
            'engineEnvironment' => [
                'TYPST_FONT_PATHS' => $fontUri,
            ],
        ]);
        $pdfBytes = "%PDF-1.7\n% fake Typst font environment URI packet\n%%EOF\n";
        $expected = [
            'reviewStatus' => 'review',
            'root' => null,
            'fontPaths' => [
            ...
            'fontPathPolicy' => [
                ...
                'safeFontPathCount' => 0,
                'unsafeFontPathCount' => 1,
                'relativeFontPathCount' => 0,
                'workspaceFontPathCount' => 0,
                'absoluteFontPathCount' => 0,
                'uriFontPathCount' => 1,
                'invalidFontPathCount' => 0,
                'issues' => ['font-path-external-boundary'],
            ],
        ...
        $result = $handoff->fakeRun($plan, [
            'files' => [
                'build/font-env-uri.pdf' => $pdfBytes,
            ],
        ]);
        $sequence = $handoff->fakeRunSequence($plan, [[
            'files' => [
        ...
        $t->same($expected, $plan['typstBoundaryProvenance']);
        $t->same(1, $plan['typstBoundarySummary']['pathEntryCount']);
        $t->same(1, $plan['typstBoundarySummary']['uriPathEntryCount']);
        $t->same(1, $plan['typstBoundarySummary']['unsafePathEntryCount']);
        $t->contains('typst-boundary-provenance:review', implode(',', $plan['diagnostics']));
        $t->contains('typst-boundary-environment:1', implode(',', $plan['diagnostics']));
        $t->contains('typst-font-paths:1', implode(',', $plan['diagnostics']));
    ...
    'maps typst font access environment flags into boundary matrix without executing' => static function (TestRunner $t) use ($document): void {
        $handoff = new PdfEngineHandoff();
        $plan = $handoff->plan($document(), [
            'engine' => 'typst',
            'outputPath' => 'build/font-access-env-matrix.pdf',
            'source' => '= Typst Font Access Environment Matrix Packet',
            'engineEnvironment' => [
                'TYPST_IGNORE_SYSTEM_FONTS' => 'maybe',
                'TYPST_IGNORE_EMBEDDED_FONTS' => 'sometimes',
            ],
        ]);
        $pdfBytes = "%PDF-1.7\n% fake Typst font access environment matrix packet\n%%EOF\n";

        $result = $handoff->fakeRun($plan, [
            'files' => [
                'build/font-access-env-matrix.pdf' => $pdfBytes,
            ],
        ]);
        $sequence = $handoff->fakeRunSequence($plan, [[
            'files' => [
        ...
        $t->same(['TYPST_IGNORE_SYSTEM_FONTS', 'TYPST_IGNORE_EMBEDDED_FONTS'], $plan['typstBoundaryProvenance']['environmentVariables']);
        ...
        $t->same(2, $cases['environment-shadows']['observed']);
        $t->same(2, $cases['environment-shadows']['details']['environmentVariableCount']);
        $t->same(0, $cases['environment-shadows']['details']['shadowedCount']);
        $t->same(0, $cases['font-access-controls']['observed']);
        $t->same(false, $cases['font-access-controls']['details']['systemFontAccessDisabled']);
        $t->same(false, $cases['font-access-controls']['details']['embeddedFontAccessDisabled']);
        $t->same([
            'ignore-embedded-fonts-environment-invalid-boundary',
            'ignore-system-fonts-environment-invalid-boundary',
        ], $cases['font-access-controls']['issues']);
        $t->same([
            'font-access-controls:ignore-embedded-fonts-environment-invalid-boundary',
            'font-access-controls:ignore-system-fonts-environment-invalid-boundary',
        ], $plan['typstBoundaryMatrix']['issues']);
    ...
    'plans typst open output side effect provenance without executing' => static function (TestRunner $t) use ($document): void {
        ...
        $plan = $handoff->plan($document(), [
            'engine' => 'typst',
            'outputPath' => 'build/open-output-boundary.pdf',
            'source' => '= Typst Open Output Boundary Packet',
            'engineOptions' => [
                '--open',
                '--open',
            ],
        ]);
        $pdfBytes = "%PDF-1.7\n% fake Typst open output boundary packet\n%%EOF\n";
        $expected = [
            'reviewStatus' => 'review',
            'root' => null,
            'fontPaths' => [],
        ...
        $result = $handoff->fakeRun($plan, [
            'files' => [
                'build/open-output-boundary.pdf' => $pdfBytes,
            ],
        ]);
        $sequence = $handoff->fakeRunSequence($plan, [[
            'files' => [
    ...
    'plans typst open output viewer provenance without executing' => static function (TestRunner $t) use ($document): void {
        ...
        $plan = $handoff->plan($document(), [
            'engine' => 'typst',
            'outputPath' => 'build/open-output-viewer.pdf',
        ...
        $expected = [
            ...
            'openOutput' => [
                ...
                'issues' => ['open-output-side-effect-boundary'],
                'viewer' => [
                    'raw' => 'tools/review-viewer',
                    'viewer' => 'tools/review-viewer',
        ...
        $result = $handoff->fakeRun($plan, [
            'files' => [
                'build/open-output-viewer.pdf' => $pdfBytes,
            ],
        ]);
        $sequence = $handoff->fakeRunSequence($plan, [[
            'files' => [
    ...
    'plans typst pdf standard boundary provenance without executing' => static function (TestRunner $t) use ($document): void {
        ...
        $plan = $handoff->plan($document(), [
            'engine' => 'typst',
            'outputPath' => 'build/pdf-standard-boundary.pdf',
            'source' => '= Typst PDF Standard Boundary Packet',
            'engineOptions' => [
        ...
        $pdfBytes = "%PDF-1.7\n% fake Typst PDF standard boundary packet\n%%EOF\n";
        $expected = [
            'reviewStatus' => 'review',
            'root' => null,
            'fontPaths' => [],
            ...
            'overrides' => [
                [
                    'option' => 'pdfStandard',
                    'count' => 2,
                    'values' => ['pdf/ua-1', 'a-2b,UA-1'],
                    'selected' => 'a-2b,UA-1',
                    'issue' => 'pdf-standard-boundary-overridden',
                ],
            ],
            'pdfStandardHistory' => [
                [
                    ...
                    'standardCount' => 0,
                    'safe' => false,
                    'issues' => ['pdf-standard-invalid-boundary'],
                ],
                [
                    'raw' => 'a-2b,UA-1',
                    'value' => 'a-2b,ua-1',
        ...
        $result = $handoff->fakeRun($plan, [
            'files' => [
                'build/pdf-standard-boundary.pdf' => $pdfBytes,
            ],
        ]);
        $sequence = $handoff->fakeRunSequence($plan, [[
            'files' => [
    ...
    'plans typst pdf ppi boundary provenance without executing' => static function (TestRunner $t) use ($document): void {
        ...
        $plan = $handoff->plan($document(), [
            'engine' => 'typst',
            'outputPath' => 'build/pdf-ppi-boundary.pdf',
            'source' => '= Typst PDF PPI Boundary Packet',
            'engineOptions' => [
                '--pages=2-4',
                '--ppi=0',
                '--ppi',
        ...
        $pdfBytes = "%PDF-1.7\n% fake Typst PDF ppi boundary packet\n%%EOF\n";
        $expected = [
            'reviewStatus' => 'review',
            'root' => null,
            'fontPaths' => [],
        ...
        $result = $handoff->fakeRun($plan, [
            'files' => [
                'build/pdf-ppi-boundary.pdf' => $pdfBytes,
            ],
        ]);
        $sequence = $handoff->fakeRunSequence($plan, [[
            'files' => [
        ...
        $t->same($expected, $plan['typstBoundaryProvenance']);
        $t->contains('typst-boundary-provenance:review', implode(',', $plan['diagnostics']));
        $t->contains('typst-pdf-pages:2-4', implode(',', $plan['diagnostics']));
    ...
    'plans typst excessive pdf ppi boundary provenance without executing' => static function (TestRunner $t) use ($document): void {
        ...
        $plan = $handoff->plan($document(), [
            'engine' => 'typst',
            'outputPath' => 'build/pdf-ppi-excessive-boundary.pdf',
            'source' => '= Typst Excessive PDF PPI Boundary Packet',
            'engineOptions' => [
                '--ppi=12000',
            ],
        ]);
        $pdfBytes = "%PDF-1.7\n% fake Typst excessive PDF ppi boundary packet\n%%EOF\n";
        $expected = [
            'reviewStatus' => 'review',
            'root' => null,
            'fontPaths' => [],
        ...
        $result = $handoff->fakeRun($plan, [
            'files' => [
                'build/pdf-ppi-excessive-boundary.pdf' => $pdfBytes,
            ],
        ]);
        $sequence = $handoff->fakeRunSequence($plan, [[
            'files' => [
    ...
    'maps typst feature environment shadow details into boundary matrix without executing' => static function (TestRunner $t) use ($document): void {
        $handoff = new PdfEngineHandoff();
        $plan = $handoff->plan($document(), [
            'engine' => 'typst',
            'outputPath' => 'build/feature-env-shadow-matrix-details.pdf',
            'source' => '= Typst Feature Environment Shadow Matrix Detail Packet',
            'engineOptions' => [
                '--features=html,unsafe feature,',
                '--features',
                'html,a11y',
        ...
        $pdfBytes = "%PDF-1.7\n% fake Typst feature environment shadow matrix detail packet\n%%EOF\n";

        $result = $handoff->fakeRun($plan, [
            'files' => [
                'build/feature-env-shadow-matrix-details.pdf' => $pdfBytes,
            ],
        ]);
        $sequence = $handoff->fakeRunSequence($plan, [[
            'files' => [
        ...
        $t->same(1, $cases['environment-shadows']['details']['environmentVariableCount']);
        $t->same(['TYPST_FEATURES'], $cases['environment-shadows']['details']['shadowedVariables']);
        $t->same(4, $cases['feature-gates']['observed']);
        $t->same('review', $cases['feature-gates']['reviewStatus']);
        $t->same(2, $cases['feature-gates']['details']['featureCount']);
        $t->same(2, $cases['feature-gates']['details']['environmentFeatureCount']);
        $t->same(['html', 'a11y'], $cases['feature-gates']['details']['features']);
        $t->same(['legacy-html', 'packages'], $cases['feature-gates']['details']['environmentFeatures']);
        $t->same('TYPST_FEATURES', $cases['feature-gates']['details']['environmentVariable']);
        $t->same(true, $cases['feature-gates']['details']['environmentShadowed']);
        $t->same('html,a11y', $cases['feature-gates']['details']['selected']);
        ...
        $t->same([
            ...
            'features-environment-shadowed',
            'features-invalid-token-boundary:unsafe feature',
        ], $cases['feature-gates']['issues']);
        $t->same('pdf', $cases['output-format']['details']['inferredOutputFormat']);
        $t->contains('boundary-overrides:features-boundary-overridden', implode(',', $matrix['issues']));
        $t->contains('feature-gates:features-boundary-overridden', implode(',', $matrix['issues']));
        $t->contains('feature-gates:features-environment-shadowed', implode(',', $matrix['issues']));
        $t->contains('typst-boundary-matrix-cases:4', implode(',', $plan['diagnostics']));
        $t->contains('typst-boundary-matrix-review-cases:3', implode(',', $plan['diagnostics']));
    ...
    'maps typst execution jobs history into boundary matrix without executing engines' => static function (TestRunner $t) use ($document): void {
        ...
        $plan = $handoff->plan($document(), [
            'engine' => 'typst',
            'outputPath' => 'build/jobs-matrix.pdf',
            'source' => '= Typst Jobs Matrix Packet',
            'engineOptions' => [
                '--jobs=0',
                '--jobs',
                '12',
        ...
        $pdfBytes = "%PDF-1.7\n% fake Typst jobs matrix packet\n%%EOF\n";

        $result = $handoff->fakeRun($plan, [
            'files' => [
                'build/jobs-matrix.pdf' => $pdfBytes,
            ],
        ]);
        $sequence = $handoff->fakeRunSequence($plan, [[
            'files' => [
        ...
        $t->same(['boundary-overrides', 'output-format', 'execution-policy'], array_column($plan['typstBoundaryMatrix']['cases'], 'case'));
        $t->same('review', $plan['typstBoundaryMatrix']['reviewStatus']);
        $t->same(3, $plan['typstBoundaryMatrix']['caseCount']);
        $t->same(2, $plan['typstBoundaryMatrix']['reviewCaseCount']);
        $t->same(4, $plan['typstBoundaryMatrix']['issueCount']);
        $t->same(1, $planCases['boundary-overrides']['observed']);
        $t->same(['jobs'], $planCases['boundary-overrides']['details']['overriddenOptions']);
        $t->same(3, $planCases['execution-policy']['observed']);
        $t->same('invalid', $planCases['execution-policy']['details']['mode']);
        $t->same(null, $planCases['execution-policy']['details']['jobCount']);
        $t->same('many', $planCases['execution-policy']['details']['raw']);
        $t->same(3, $planCases['execution-policy']['details']['historyEntryCount']);
        $t->same(1, $planCases['execution-policy']['details']['overrideCount']);
        $t->same(2, $planCases['execution-policy']['details']['invalidJobCount']);
        $t->same(1, $planCases['execution-policy']['details']['fixedJobCount']);
        $t->same(0, $planCases['execution-policy']['details']['autoJobCount']);
        $t->contains('boundary-overrides:jobs-boundary-overridden', implode(',', $plan['typstBoundaryMatrix']['issues']));
        $t->contains('execution-policy:jobs-boundary-overridden', implode(',', $plan['typstBoundaryMatrix']['issues']));
        $t->contains('execution-policy:jobs-invalid-boundary', implode(',', $plan['typstBoundaryMatrix']['issues']));
        $t->contains('execution-policy:jobs-nonpositive-boundary', implode(',', $plan['typstBoundaryMatrix']['issues']));
        $t->contains('typst-boundary-matrix-cases:3', implode(',', $plan['diagnostics']));
        $t->contains('typst-boundary-matrix-review-cases:2', implode(',', $plan['diagnostics']));
        $t->contains('typst-boundary-matrix-issues:4', implode(',', $plan['diagnostics']));
        $t->same(true, $result['ok']);
        $t->same($planCases['boundary-overrides'], $resultCases['boundary-overrides']);
        $t->same($planCases['execution-policy'], $resultCases['execution-policy']);
        $t->same($result['typstBoundaryMatrix'], $result['artifactProvenanceReview']['typstBoundaryMatrix']);
        $t->same($result['typstBoundaryMatrix'], $sequence['finalTypstBoundaryMatrix']);
    },
    ...
    'plans typst timings sidecar boundary provenance without executing' => static function (TestRunner $t) use ($document): void {
        ...
        $plan = $handoff->plan($document(), [
            ...
            'source' => '= Typst Timings Boundary Packet',
            'engineOptions' => ['--timings=build/timings-boundary.json'],
        ]);
        $pdfBytes = "%PDF-1.7\n% fake Typst timings sidecar packet\n%%EOF\n";
        $timingsBytes = '{"traceEvents":[{"name":"parse","args":{"file":"build/timings-boundary.typ","line":1}}]}';
        $expected = [
            'reviewStatus' => 'ok',
            'root' => null,
            'fontPaths' => [],
            ...
            'issues' => [],
            'timingsOutput' => [
                'raw' => 'build/timings-boundary.json',
                'path' => 'build/timings-boundary.json',
                'kind' => 'relative',
                'safe' => true,
                'issues' => [],
            ],
        ...
        $result = $handoff->fakeRun($plan, [
            'files' => [
                ...
                'build/timings-boundary.json' => $timingsBytes,
            ],
        ]);
        $sequence = $handoff->fakeRunSequence($plan, [[
            'files' => [
        ...
        $t->same(['build/timings-boundary.json'], $plan['expectedEngineArtifacts']);
        $t->same($expected, $plan['typstBoundaryProvenance']);
        $t->contains('typst-boundary-provenance:ok', implode(',', $plan['diagnostics']));
        $t->contains('typst-timings-output:build/timings-boundary.json', implode(',', $plan['diagnostics']));
        $t->contains('pdf-engine-artifacts:1', implode(',', $plan['diagnostics']));
        $t->same(true, $result['ok']);
        $t->same($expected, $result['typstBoundaryProvenance']);
        $t->same($expected, $result['artifactProvenanceReview']['typstBoundaryProvenance']);
        $t->same(['build/timings-boundary.json'], $result['artifactProvenanceReview']['expectedEngineArtifacts']);
        $t->same(hash('sha256', $timingsBytes), $result['producedArtifactsSha256']['build/timings-boundary.json']);
        $t->same('ok', $result['artifactProvenanceReview']['reviewStatus']);
        $t->same($expected, $sequence['finalTypstBoundaryProvenance']);

        $external = $handoff->plan($document(), [
            'engine' => 'typst',
            'outputPath' => 'build/timings-external.pdf',
            'source' => '= Typst External Timings Boundary Packet',
            'engineOptions' => [
        ...
        $externalExpected = [
            ...
            'inputVariables' => [],
            'issues' => [
                'timings-output-external-boundary',
                'timings-output-boundary-overridden',
            ],
            'timingsOutput' => [
                'raw' => 'https://trace.example.invalid/timings.json',
                'path' => 'https://trace.example.invalid/timings.json',
                'kind' => 'uri',
                'safe' => false,
                'issues' => ['timings-output-external-boundary'],
            ],
            'overrides' => [
                [
                    'option' => 'timingsOutput',
                    'count' => 2,
                    'values' => ['build/local-timings.json', 'https://trace.example.invalid/timings.json'],
                    'selected' => 'https://trace.example.invalid/timings.json',
                    'issue' => 'timings-output-boundary-overridden',
                ],
            ],
            'timingsOutputHistory' => [
                [
                    'raw' => 'build/local-timings.json',
                    'path' => 'build/local-timings.json',
                    'kind' => 'relative',
                    'safe' => true,
                    'issues' => [],
                ],
        ...
        $externalResult = $handoff->fakeRun($external, [
            'files' => [
                'build/timings-external.pdf' => $pdfBytes,
            ],
        ]);

        $t->same([], $external['expectedEngineArtifacts']);
        $t->same($externalExpected, $external['typstBoundaryProvenance']);
        $t->contains('typst-timings-output:https://trace.example.invalid/timings.json', implode(',', $external['diagnostics']));
        $t->contains('typst-boundary-overrides:1', implode(',', $external['diagnostics']));
        $t->contains('typst-boundary-issues:2', implode(',', $external['diagnostics']));
    ...
    'plans typst short timings sidecar boundary provenance without executing' => static function (TestRunner $t) use ($document): void {
        ...
        $plan = $handoff->plan($document(), [
            'engine' => 'typst',
            'outputPath' => 'build/short-timings-boundary.pdf',
            'source' => '= Typst Short Timings Boundary Packet',
            'engineOptions' => ['-tbuild/old-timings.json', '-t', 'build/short-timings.json'],
        ]);
        $pdfBytes = "%PDF-1.7\n% fake Typst short timings sidecar packet\n%%EOF\n";
        $timingsBytes = '{"traceEvents":[{"name":"layout","args":{"file":"build/short-timings-boundary.typ","line":1}}]}';
        $expected = [
            'reviewStatus' => 'review',
            'root' => null,
            'fontPaths' => [],
            ...
            'packageCache' => null,
            'inputVariables' => [],
            'issues' => ['timings-output-boundary-overridden'],
            'timingsOutput' => [
            ...
            'overrides' => [
                [
                    'option' => 'timingsOutput',
                    'count' => 2,
                    'values' => ['build/old-timings.json', 'build/short-timings.json'],
                    'selected' => 'build/short-timings.json',
                    'issue' => 'timings-output-boundary-overridden',
                ],
            ],
        ];
        ...
        $result = $handoff->fakeRun($plan, [
            'files' => [
                'build/short-timings-boundary.pdf' => $pdfBytes,
                'build/short-timings.json' => $timingsBytes,
            ],
        ]);
        $sequence = $handoff->fakeRunSequence($plan, [[
            'files' => [
        ...
        $t->same(['build/short-timings.json'], $plan['expectedEngineArtifacts']);
        $t->same($expected, $plan['typstBoundaryProvenance']);
        $t->contains('typst-boundary-provenance:review', implode(',', $plan['diagnostics']));
        $t->contains('typst-timings-output:build/short-timings.json', implode(',', $plan['diagnostics']));
        $t->contains('typst-boundary-overrides:1', implode(',', $plan['diagnostics']));
        $t->contains('typst-boundary-issues:1', implode(',', $plan['diagnostics']));
        $t->contains('pdf-engine-artifacts:1', implode(',', $plan['diagnostics']));
        $t->same(true, $result['ok']);
        $t->same($expected, $result['typstBoundaryProvenance']);
        $t->same($expected, $result['artifactProvenanceReview']['typstBoundaryProvenance']);
        $t->same(['build/short-timings.json'], $result['artifactProvenanceReview']['expectedEngineArtifacts']);
        $t->same(hash('sha256', $timingsBytes), $result['producedArtifactsSha256']['build/short-timings.json']);
        $t->same('review', $result['artifactProvenanceReview']['reviewStatus']);
        $t->contains('typst-boundary-provenance:review', implode(',', $result['artifactProvenanceReview']['issues']));
        $t->same($expected, $sequence['finalTypstBoundaryProvenance']);
    },
    ...
    'maps typst sidecar output histories into boundary matrix without executing' => static function (TestRunner $t) use ($document): void {
        $handoff = new PdfEngineHandoff();
        $plan = $handoff->plan($document(), [
            'engine' => 'typst',
            'outputPath' => 'build/sidecar-output-matrix.pdf',
            'source' => '= Typst Sidecar Output Matrix Packet',
            'engineOptions' => [
                '--deps=build/local-deps.d',
                '--deps',
                'https://deps.example.invalid/remote.d',
                '--deps-format=custom format',
                '--deps-format',
                'json',
                '--timings=build/local-timings.json',
        ...
        $pdfBytes = "%PDF-1.7\n% fake Typst sidecar output matrix packet\n%%EOF\n";
        $expectedCase = [
            'case' => 'sidecar-outputs',
            'reviewStatus' => 'review',
            'observed' => 2,
            'details' => [
                'dependencyOutputPresent' => true,
        ...
        $result = $handoff->fakeRun($plan, [
            'files' => [
                'build/sidecar-output-matrix.pdf' => $pdfBytes,
            ],
        ]);
        $sequence = $handoff->fakeRunSequence($plan, [[
            'files' => [
        ...
        $t->same($expectedCase, $planCases['sidecar-outputs']);
        $t->same('review', $plan['typstBoundaryMatrix']['reviewStatus']);
        $t->same(3, $plan['typstBoundaryMatrix']['caseCount']);
        $t->same(2, $plan['typstBoundaryMatrix']['reviewCaseCount']);
        $t->contains('sidecar-outputs:dependency-format-invalid-boundary', implode(',', $plan['typstBoundaryMatrix']['issues']));
        $t->contains('typst-boundary-matrix-review-cases:2', implode(',', $plan['diagnostics']));
        $t->same([], $plan['expectedEngineArtifacts']);
        $t->same(true, $result['ok']);
        $t->same($expectedCase, $resultCases['sidecar-outputs']);
        $t->same($result['typstBoundaryMatrix'], $result['artifactProvenanceReview']['typstBoundaryMatrix']);
        $t->same($result['typstBoundaryMatrix'], $sequence['finalTypstBoundaryMatrix']);
    },

    'plans typst dependency stdout boundary provenance without executing' => static function (TestRunner $t) use ($document): void {
        $handoff = new PdfEngineHandoff();
        $plan = $handoff->plan($document(), [
            'engine' => 'typst',
            'outputPath' => 'build/deps-stdout-boundary.pdf',
            'source' => '= Typst Dependency Stdout Boundary Packet',
            'engineOptions' => [
                '--deps=-',
                '--deps-format=json',
            ],
        ]);
        $pdfBytes = "%PDF-1.7\n% fake Typst dependency stdout boundary packet\n%%EOF\n";
        $expected = [
            'reviewStatus' => 'review',
            'root' => null,
            'fontPaths' => [],
        ...
        $result = $handoff->fakeRun($plan, [
            'files' => [
                'build/deps-stdout-boundary.pdf' => $pdfBytes,
            ],
        ]);
        $sequence = $handoff->fakeRunSequence($plan, [[
            'files' => [
    ...
    'maps typst creation timestamp history into boundary matrix without executing engines' => static function (TestRunner $t) use ($document): void {
        $handoff = new PdfEngineHandoff();
        $environmentTimestamp = '1700000000';
        $selectedTimestamp = '1700000100';
        $plan = $handoff->plan($document(), [
            'engine' => 'typst',
            'outputPath' => 'build/timestamp-matrix.pdf',
            'source' => '= Typst Timestamp Matrix Packet',
            'engineOptions' => [
        ...
        $pdfBytes = "%PDF-1.7\n% fake Typst timestamp matrix packet\n%%EOF\n";

        $result = $handoff->fakeRun($plan, [
            'files' => [
                'build/timestamp-matrix.pdf' => $pdfBytes,
            ],
        ]);
        $sequence = $handoff->fakeRunSequence($plan, [[
            'files' => [
        ...
        $t->same(['environment-shadows', 'boundary-overrides', 'output-format', 'creation-timestamp'], array_column($plan['typstBoundaryMatrix']['cases'], 'case'));
        $t->same('review', $plan['typstBoundaryMatrix']['reviewStatus']);
        $t->same(4, $plan['typstBoundaryMatrix']['caseCount']);
        $t->same(3, $plan['typstBoundaryMatrix']['reviewCaseCount']);
        $details = $planCases['creation-timestamp']['details'];
        $t->same('review', $planCases['creation-timestamp']['reviewStatus']);
        $t->same(3, $planCases['creation-timestamp']['observed']);
        ...
        $t->same($selectedTimestamp, $details['environmentSelected']);
        ...
        $t->contains('typst-boundary-matrix-cases:4', implode(',', $plan['diagnostics']));
        $t->same(true, $result['ok']);
        $t->same($planCases['creation-timestamp'], $resultCases['creation-timestamp']);
        $t->same($result['typstBoundaryMatrix'], $result['artifactProvenanceReview']['typstBoundaryMatrix']);
        $t->same($result['typstBoundaryMatrix'], $sequence['finalTypstBoundaryMatrix']);
    },
    ...
    'plans unsafe typst boundary override histories without executing' => static function (TestRunner $t) use ($document): void {
        ...
        $plan = $handoff->plan($document(), [
            'engine' => 'typst',
            'sourcePath' => 'workspace/build/override-boundary.typ',
            'outputPath' => 'build/override-boundary.pdf',
            'source' => '= Typst Override Boundary Packet',
            'engineOptions' => [
                '--root=/srv/shared',
                '--root',
                'workspace',
                '--package-path=https://packages.example.invalid/typst',
                '--package-path=vendor/typst-packages',
                '--package-cache=/tmp/typst-cache',
                '--package-cache=cache/typst',
                '--creation-timestamp=not-a-time',
                '--creation-timestamp',
                $timestamp,
        ...
        $pdfBytes = "%PDF-1.7\n% fake Typst override boundary packet\n%%EOF\n";
        $expected = [
            'reviewStatus' => 'review',
            'root' => ['raw' => 'workspace', 'path' => 'workspace', 'kind' => 'relative', 'safe' => true, 'issues' => []],
            'fontPaths' => [],
            ...
            'overrides' => [
                ...
                [
                    'option' => 'packagePath',
                    'count' => 2,
                    'values' => ['https://packages.example.invalid/typst', 'vendor/typst-packages'],
                    'selected' => 'vendor/typst-packages',
                    'issue' => 'package-path-boundary-overridden',
                ],
                [
                    'option' => 'packageCache',
                    'count' => 2,
                    'values' => ['/tmp/typst-cache', 'cache/typst'],
                    'selected' => 'cache/typst',
                    'issue' => 'package-cache-boundary-overridden',
                ],
                [
                    'option' => 'creationTimestamp',
                    'count' => 2,
                    'values' => ['not-a-time', $timestamp],
                    'selected' => $timestamp,
                    'issue' => 'creation-timestamp-boundary-overridden',
                ],
            ],
            'rootHistory' => [
                ['raw' => '/srv/shared', 'path' => '/srv/shared', 'kind' => 'absolute', 'safe' => false, 'issues' => ['root-external-boundary']],
                ['raw' => 'workspace', 'path' => 'workspace', 'kind' => 'relative', 'safe' => true, 'issues' => []],
            ],
            'packagePathHistory' => [
                ['raw' => 'https://packages.example.invalid/typst', 'path' => 'https://packages.example.invalid/typst', 'kind' => 'uri', 'safe' => false, 'issues' => ['package-path-external-boundary']],
                ['raw' => 'vendor/typst-packages', 'path' => 'vendor/typst-packages', 'kind' => 'relative', 'safe' => true, 'issues' => []],
            ],
            'packageCacheHistory' => [
                ['raw' => '/tmp/typst-cache', 'path' => '/tmp/typst-cache', 'kind' => 'absolute', 'safe' => false, 'issues' => ['package-cache-external-boundary']],
                ['raw' => 'cache/typst', 'path' => 'cache/typst', 'kind' => 'relative', 'safe' => true, 'issues' => []],
            ],
    ...
    'fake runner preserves typst dependency sidecar provenance without executing' => static function (TestRunner $t) use ($document): void {
        ...
        $expectedPackagePolicy = [
            ...
            'packageCoordinates' => ['preview/cetz:0.3.2'],
            'packageNames' => ['preview/cetz'],
            'sidecarFileCount' => 1,
            'sidecarFiles' => ['build/review.d'],
            'sidecarPackageInputCounts' => [
                [
                    'artifact' => 'build/review.d',
                    'packageInputCount' => 1,
                    'packageInputs' => ['typst-package:@preview/cetz:0.3.2'],
                ],
            ],
            'metadataOnly' => true,
    ...
    'fake runner reports typst package dependency conflict policy from sidecars' => static function (TestRunner $t) use ($document): void {
        ...
        $plan = $handoff->plan($document(), [
            'engine' => 'typst',
            'outputPath' => 'build/typst-package-policy.pdf',
            'source' => $source,
            'engineOptions' => ['--deps=build/typst-package-policy.d'],
        ]);
        $pdfBytes = "%PDF-1.7\n% fake Typst package policy packet\n%%EOF\n";
        $depfile = implode("\n", [
            'build/typst-package-policy.pdf: build/typst-package-policy.typ \\',
            '  @preview/cetz:0.3.2/src/lib.typ \\',
            '  @preview/cetz:0.4.0/src/lib.typ \\',
            '  @typst/symbols:0.1.0',
            '',
        ]);
        $expectedDependencies = [
        ...
        $expectedPolicy = [
            ...
            'sidecarFileCount' => 1,
            'sidecarFiles' => ['build/typst-package-policy.d'],
            'sidecarPackageInputCounts' => [
                [
                    'artifact' => 'build/typst-package-policy.d',
                    'packageInputCount' => 3,
                    'packageInputs' => [
                        'typst-package:@preview/cetz:0.3.2/src/lib.typ',
                        'typst-package:@preview/cetz:0.4.0/src/lib.typ',
                        'typst-package:@typst/symbols:0.1.0',
                    ],
                ],
            ],
            ...
            'issues' => [
                'typst-package-dependencies:3',
                'package-version-conflict:preview/cetz',
            ],
        ];

        $result = $handoff->fakeRun($plan, [
            'files' => [
                'build/typst-package-policy.d' => $depfile,
                'build/typst-package-policy.pdf' => $pdfBytes,
            ],
        ]);
        $sequence = $handoff->fakeRunSequence($plan, [
            [
    ...
    'fake runner reviews typst output format boundary provenance' => static function (TestRunner $t) use ($document): void {
        ...
        $okPlan = $handoff->plan($document(), [
            'engine' => 'typst',
            'outputPath' => 'build/format-ok.pdf',
            'source' => '= Typst Format OK Packet',
            'engineOptions' => ['--format', 'pdf'],
        ]);
        $plan = $handoff->plan($document(), [
            'engine' => 'typst',
            'outputPath' => 'build/format-boundary.pdf',
    ...
    'preserves typst output format in boundary provenance without executing' => static function (TestRunner $t) use ($document): void {
        $handoff = new PdfEngineHandoff();
        $plan = $handoff->plan($document(), [
            'engine' => 'typst',
            'outputPath' => 'build/format-boundary-provenance.pdf',
            'source' => '= Typst Format Boundary Provenance Packet',
            'engineOptions' => ['--format=svg'],
        ]);
        $pdfBytes = "%PDF-1.7\n% fake Typst format boundary provenance packet\n%%EOF\n";
        $expected = [
            'reviewStatus' => 'review',
            'root' => null,
            'fontPaths' => [],
        ...
        $result = $handoff->fakeRun($plan, [
            'files' => [
                'build/format-boundary-provenance.pdf' => $pdfBytes,
            ],
        ]);
        $sequence = $handoff->fakeRunSequence($plan, [[
            'files' => [
        ...
        $t->contains('typst-output-format-boundary-policy:review', implode(',', $plan['diagnostics']));
    ...
    'fake runner preserves typst output format history boundary provenance' => static function (TestRunner $t) use ($document): void {
        ...
        $t->contains('typst-output-format-history:3', implode(',', $plan['diagnostics']));
        $t->contains('typst-output-format-overrides:1', implode(',', $plan['diagnostics']));
        $t->contains('typst-output-format-issues:4', implode(',', $plan['diagnostics']));
        $t->same(true, $result['ok']);
        $t->same($expectedPolicy, $result['typstOutputFormatPolicy']);
        $t->same($expectedPolicy, $result['artifactProvenanceReview']['typstOutputFormatPolicy']);
        $t->same('review', $result['artifactProvenanceReview']['reviewStatus']);
    ...
    'fake runner preserves typst short output format alias provenance' => static function (TestRunner $t) use ($document): void {
        $handoff = new PdfEngineHandoff();
        $plan = $handoff->plan($document(), [
            'engine' => 'typst',
            'outputPath' => 'build/format-short-alias.pdf',
            'source' => '= Typst Short Format Alias Packet',
            'engineOptions' => [
                '-f',
                'svg',
                '--format',
        ...
        $pdfBytes = "%PDF-1.7\n% fake Typst short format alias packet\n%%EOF\n";
        $expectedPolicy = [
            'reviewStatus' => 'review',
            'declaredOutputFile' => 'build/format-short-alias.pdf',
            'inferredOutputFormat' => 'pdf',
            'explicitFormat' => 'pdf',
            'formatOptions' => ['svg', 'pdf'],
            'issues' => [
        ...
        $result = $handoff->fakeRun($plan, [
            'files' => [
                'build/format-short-alias.pdf' => $pdfBytes,
            ],
        ]);
        $sequence = $handoff->fakeRunSequence($plan, [[
            'files' => [
        ...
        $t->contains('typst-output-format-policy:review', implode(',', $plan['diagnostics']));
        $t->contains('typst-output-format-explicit:pdf', implode(',', $plan['diagnostics']));
        $t->contains('typst-output-format-history:2', implode(',', $plan['diagnostics']));
        $t->contains('typst-output-format-overrides:1', implode(',', $plan['diagnostics']));
        $t->contains('typst-output-format-issues:3', implode(',', $plan['diagnostics']));
        $t->same(true, $result['ok']);
        $t->same($expectedPolicy, $result['typstOutputFormatPolicy']);
        $t->same($expectedPolicy, $result['artifactProvenanceReview']['typstOutputFormatPolicy']);
        $t->same('review', $result['artifactProvenanceReview']['reviewStatus']);
    ...
    'fake runner preserves typst attached short output format provenance' => static function (TestRunner $t) use ($document): void {
        $handoff = new PdfEngineHandoff();
        $plan = $handoff->plan($document(), [
            'engine' => 'typst',
            'outputPath' => 'build/format-attached-short.pdf',
            'source' => '= Typst Attached Short Format Packet',
            'engineOptions' => [
                '-fsvg',
                '-f=pdf',
            ],
        ]);
        $pdfBytes = "%PDF-1.7\n% fake Typst attached short format packet\n%%EOF\n";
        $expectedPolicy = [
            'reviewStatus' => 'review',
            'declaredOutputFile' => 'build/format-attached-short.pdf',
            'inferredOutputFormat' => 'pdf',
            'explicitFormat' => 'pdf',
            'formatOptions' => ['svg', 'pdf'],
            'issues' => [
        ...
        $result = $handoff->fakeRun($plan, [
            'files' => [
                'build/format-attached-short.pdf' => $pdfBytes,
            ],
        ]);
        $sequence = $handoff->fakeRunSequence($plan, [[
            'files' => [
        ...
        $t->same(['typst', 'compile', '-fsvg', '-f=pdf', 'build/format-attached-short.typ', 'build/format-attached-short.pdf'], $plan['argv']);
        $t->same($expectedPolicy, $plan['typstOutputFormatPolicy']);
        $t->contains('typst-output-format-policy:review', implode(',', $plan['diagnostics']));
        $t->contains('typst-output-format-explicit:pdf', implode(',', $plan['diagnostics']));
        $t->contains('typst-output-format-history:2', implode(',', $plan['diagnostics']));
    ...
    'fake runner reviews typst root read boundary provenance' => static function (TestRunner $t) use ($document): void {
        ...
        $t->same(true, $plan['typstBoundaryProvenance']['root']['safe']);
        $t->contains('typst-root-boundary:project', implode(',', $plan['diagnostics']));
        $t->same(false, $result['ok']);
        $t->same('engine-boundary-violation', $result['reason']);
        $t->same(['project/assets/chart.svg', 'project/main.typ', 'shared/chart.svg'], $result['engineInputFiles']);
        $t->same('project', $result['engineBoundaryRoot']);
        $t->same(['shared/chart.svg'], $result['engineBoundaryViolations']);
        $t->same($expectedPolicy, $result['typstReadBoundaryPolicy']);
        $t->same('project', $result['artifactProvenanceReview']['engineBoundaryRoot']);
        $t->same(['shared/chart.svg'], $result['artifactProvenanceReview']['engineBoundaryViolations']);
        $t->same($expectedPolicy, $result['artifactProvenanceReview']['typstReadBoundaryPolicy']);
        $t->same('failed', $result['artifactProvenanceReview']['reviewStatus']);
        $t->contains('engine-boundary-violations:1', implode(',', $result['artifactProvenanceReview']['issues']));
        $t->contains('engine-boundary-violation:shared/chart.svg', implode(',', $result['diagnostics']));
        $t->contains('typst-root-boundary-policy:review', implode(',', $result['diagnostics']));
        $t->contains('typst-root-boundary-inputs:3', implode(',', $result['diagnostics']));
        $t->contains('typst-root-boundary-outside-inputs:1', implode(',', $result['diagnostics']));
        $t->contains('typst-root-boundary-policy:review', implode(',', $result['artifactProvenanceReview']['issues']));
        $t->same(['shared/chart.svg'], $sequence['finalEngineBoundaryViolations']);
...
MARKDOWN);
    ...
    'fake runner surfaces bounded pdf uri active actions from produced bytes' => static function (TestRunner $t) use ($document): void {
        $handoff = new PdfEngineHandoff();
        $plan = $handoff->plan($document(), ['engine' => 'xelatex', 'outputPath' => 'packets/uri-active-actions.pdf']);
        $pdfBytes = implode("\n", [
            '%PDF-1.7',
            '1 0 obj',
            '<< /Type /Catalog /Pages 2 0 R /OpenAction << /S /URI /URI (https://example.test/review/open) >> >>',
            'endobj',
            '2 0 obj',
            '<< /Type /Pages /Count 1 /Kids [3 0 R] >>',
            'endobj',
            '3 0 obj',
            '<< /Type /Page /Parent 2 0 R /AA << /O << /S /URI /URI (mailto:review@example.test) >> >> /Annots [8 0 R] >>',
            'endobj',
            '8 0 obj',
            '<< /Type /Annot /Subtype /Link /A << /S /URI /URI (https://example.test/review/annotation) >> >>',
            'endobj',
            'trailer',
            '<< /Root 1 0 R >>',
            '%%EOF',
        ...
        $result = $handoff->fakeRun($plan, [
            'files' => [
                'packets/uri-active-actions.pdf' => $pdfBytes,
            ],
        ]);
        $sequence = $handoff->fakeRunSequence($plan, [
            [
        ...
        $expectedActiveActions = [
            [
                'source' => 'annotation:8 0 R.A',
                'type' => 'URI',
                'target' => 'https://example.test/review/annotation',
                'scriptBytes' => null,
                'scriptSha256' => null,
            ],
            [
                'source' => 'catalog.OpenAction',
                'type' => 'URI',
                'target' => 'https://example.test/review/open',
                'scriptBytes' => null,
                'scriptSha256' => null,
            ],
            [
                'source' => 'page:3 0 R.AA.O',
                'type' => 'URI',
                'target' => 'mailto:review@example.test',
                'scriptBytes' => null,
                'scriptSha256' => null,
            ],
        ];
        ...
        $expectedPageActions = [
            [
                ...
                'source' => 'page:3 0 R.AA.O',
                'actionType' => 'URI',
                'actionTarget' => 'mailto:review@example.test',
                'scriptBytes' => null,
                'scriptSha256' => null,
            ],
        ];
        ...
        $t->same(true, $result['ok']);
        $t->same(['type' => 'URI', 'target' => 'https://example.test/review/open'], $result['pdfOpenAction']);
        $t->same($expectedActiveActions, $result['pdfActiveActions']);
        $t->same(['URI' => 3], $result['pdfActiveActionTypes']);
