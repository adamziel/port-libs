...
return [
    ...
    'checked-in html fixtures match native ast shape' => static function (TestRunner $t) use ($fixtureRoot): void {
        ...
        $t->same(66, $report['htmlFixtureCount']);
        $t->same(66, $report['nativeFixtureCount']);
        $t->same(66, $report['pairedFixtureCount']);
        ...
        $t->same(66, $report['totalPairCount']);
        $t->same(66, $report['comparedPairCount']);
        $t->same(66, $report['htmlParsedCount']);
        $t->same(66, $report['nativeParsedCount']);
        $t->same(66, $report['bothParsedCount']);
        $t->same(0, $report['parseFailureCount']);
        $t->same(66, $report['normalizedAstMatchCount']);
        ...
        $t->same(true, HtmlNativeAstComparisonHarness::hasRequiredMappedParity($report, 66));
        ...
        $t->same('e4c6d2e982cdf08fcff43d5a1433575dabed228978cdd89b1e88255e3523b0aa', $signature['sha256'] ?? null);
        $t->same('e4c6d2e982cdf08fcff43d5a1433575dabed228978cdd89b1e88255e3523b0aa', $signature['expectedSha256'] ?? null);
        $t->same(66, $signature['htmlFixtureCount'] ?? null);
        $t->same(66, $signature['nativeFixtureCount'] ?? null);
        $t->same(66, $signature['pairedFixtureCount'] ?? null);
        $t->same(132, $signature['recordCount'] ?? null);
        ...
        $t->same('HTML fixtures=66; native fixtures=66; same-basename pairs=66; HTML fixtures without native pairs=0', $report['orderedRemainingGaps'][1]['currentEvidence']);
        $t->same('The current checked-in gate covers 66 paired fixture(s) out of 66 HTML fixture(s).', $report['orderedRemainingGaps'][3]['currentEvidence']);
        $t->contains('fixtureInventory: html=66 native=66 paired=66 unpairedHtml=0 unpairedNative=0', $text);
        $t->contains('fixtureInventorySignature: status=valid-checked-in-html-fixture-inventory matchesExpected=yes sha256=e4c6d2e982cdf08fcff43d5a1433575dabed228978cdd89b1e88255e3523b0aa', $text);
        $t->contains('pairs: total=66 compared=66 parsedBoth=66 parseFailures=0', $text);
        $t->contains('normalizedAst: matches=66 (100.00%) mismatches=0', $text);
        ...
        $t->same(0, $exitCode);
        $t->same(66, $decoded['htmlFixtureCount']);
        ...
        $t->same('e4c6d2e982cdf08fcff43d5a1433575dabed228978cdd89b1e88255e3523b0aa', $decoded['checkedInFixtureInventorySignature']['sha256'] ?? null);
