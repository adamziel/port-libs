...
return [
    ...
    'checked-in html fixtures match native ast shape' => static function (TestRunner $t) use ($fixtureRoot): void {
        ...
        $t->same(38, $report['nativeFixtureCount']);
        $t->same(38, $report['pairedFixtureCount']);
        $t->same(10, $report['unpairedHtmlFixtureCount']);
        ...
        $t->same(38, $report['totalPairCount']);
        $t->same(38, $report['comparedPairCount']);
        $t->same(38, $report['htmlParsedCount']);
        $t->same(38, $report['nativeParsedCount']);
        $t->same(38, $report['bothParsedCount']);
        $t->same(0, $report['parseFailureCount']);
        $t->same(38, $report['normalizedAstMatchCount']);
        ...
        $t->same(true, HtmlNativeAstComparisonHarness::hasRequiredMappedParity($report, 38));
        ...
        $t->same('HTML fixtures=48; native fixtures=38; same-basename pairs=38; HTML fixtures without native pairs=10', $report['orderedRemainingGaps'][1]['currentEvidence']);
        $t->same('The current checked-in gate covers 38 paired fixture(s) out of 48 HTML fixture(s).', $report['orderedRemainingGaps'][3]['currentEvidence']);
        $t->contains('fixtureInventory: html=48 native=38 paired=38 unpairedHtml=10 unpairedNative=0', $text);
        $t->contains('pairs: total=38 compared=38 parsedBoth=38 parseFailures=0', $text);
        $t->contains('normalizedAst: matches=38 (100.00%) mismatches=0', $text);
