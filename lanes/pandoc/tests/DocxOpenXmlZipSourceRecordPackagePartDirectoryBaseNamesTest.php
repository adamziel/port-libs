...
return [
    ...
    'summarizes DOCX ZIP source records by package part directory base names' => static function (TestRunner $t): void {
        ...
        $t->same(6, $summary['partZipSourceRecordPackagePartDirectoryBaseNameCount']);
        $t->same([
            '/' => 1,
            'Media' => 1,
            '_rels' => 2,
        ...
        $t->same(5, $summary['partZipSourceRecordDuplicatePackagePartCaseFoldDirectoryBaseNamePartCount']);
        ...
        $t->same(['word/Media' => 1], $upperMedia['directoryCounts']);
        ...
        $t->same(['Media' => 1, 'media' => 2], $caseFoldMedia['directoryBaseNameCounts']);
        $t->same([
            'customXml/media' => 1,
            'word/Media' => 1,
            'word/media' => 1,
...
function docx_zip_source_record_directory_base_name_sums(
    ...
    foreach ($inventory as $part) {
        ...
        $key = (string) ($part[$groupField] ?? '');
        $sums[$key] = ($sums[$key] ?? 0) + (int) ($part[$sumField] ?? 0);
    }
...
function docx_zip_source_record_directory_base_name_sum_for_name(
    ...
    $sum = 0;
    foreach ($inventory as $part) {
        if (($part['zipEntryPresent'] ?? false) !== true || (string) ($part[$groupField] ?? '') !== $groupName) {
            continue;
        }

        $sum += (int) ($part[$sumField] ?? 0);
    }
