...
<style xmlns="http://purl.org/net/xbiblio/csl" version="1.0" class="in-text" default-locale="en-US">
  <info>
    <title>Bounded Given Add Names Year Disambiguation Review</title>
    <id>https://example.test/styles/bounded-given-add-names-year-disambiguation-review</id>
...
XML);
        ...
        $t->same(true, $summary['citationOptions']['disambiguateAddNames'] ?? null);
        $t->same(true, $summary['citationOptions']['disambiguateAddYearSuffix'] ?? null);
        $t->same('(Ada Smith, Doe, et al. 2026a,b; Alan Smith et al. 2026; Ada Smith, Ng, et al. 2026)', $processor->renderCitationCluster([
            new AstNode('citation', ['id' => 'smith-ada-doe-a', 'text' => '[@smith-ada-doe-a]']),
            new AstNode('citation', ['id' => 'smith-ada-doe-b', 'text' => '[@smith-ada-doe-b]']),
            new AstNode('citation', ['id' => 'smith-alan-doe', 'text' => '[@smith-alan-doe]']),
            new AstNode('citation', ['id' => 'smith-ada-ng', 'text' => '[@smith-ada-ng]']),
        ]));
        $t->same('Smith, A.; Doe, J.; Roe, P. 2026a. Ada Doe First Packet. https://example.test/ada-doe-a.', $processor->renderBibliographyEntry('smith-ada-doe-a'));
        $t->same('Smith, A.; Doe, J.; Roe, P. 2026b. Ada Doe Second Packet. https://example.test/ada-doe-b.', $processor->renderBibliographyEntry('smith-ada-doe-b'));
        $t->same('Smith, A.; Doe, J.; Roe, P. 2026. Alan Doe Packet. https://example.test/alan-doe.', $processor->renderBibliographyEntry('smith-alan-doe'));
        $t->same('Smith, A.; Ng, N.; Rao, R. 2026. Ada Ng Packet. https://example.test/ada-ng.', $processor->renderBibliographyEntry('smith-ada-ng'));

        $document = (new MarkdownReader())->read('Review cites [@smith-ada-doe-a; @smith-ada-doe-b; @smith-alan-doe; @smith-ada-ng] before publishing imported source notes.');
        $processed = $processor->appendBibliography($document, 'Works Cited');
        $citationNodes = [];
        $collectCitations = static function (AstNode $node) use (&$collectCitations, &$citationNodes): void {
            if ($node->type === 'citation') {
        ...
        $collectCitations($processed);
        $t->same('full', $citationNodes['smith-ada-doe-a']->attr('cslGivenNameDisambiguation'));
        $t->same('full', $citationNodes['smith-ada-doe-b']->attr('cslGivenNameDisambiguation'));
        $t->same('full', $citationNodes['smith-alan-doe']->attr('cslGivenNameDisambiguation'));
        $t->same('full', $citationNodes['smith-ada-ng']->attr('cslGivenNameDisambiguation'));
        $t->same(2, $citationNodes['smith-ada-doe-a']->attr('cslDisambiguateNameCount'));
        $t->same(2, $citationNodes['smith-ada-doe-b']->attr('cslDisambiguateNameCount'));
        $t->same(null, $citationNodes['smith-alan-doe']->attr('cslDisambiguateNameCount'));
        $t->same(2, $citationNodes['smith-ada-ng']->attr('cslDisambiguateNameCount'));
        $t->same('a', $citationNodes['smith-ada-doe-a']->attr('cslYearSuffix'));
        $t->same('b', $citationNodes['smith-ada-doe-b']->attr('cslYearSuffix'));
        $t->same('', $citationNodes['smith-alan-doe']->attr('cslYearSuffix'));
        $t->same('', $citationNodes['smith-ada-ng']->attr('cslYearSuffix'));
...
XML
        ...
        $t->same('(de la Cruz uncertain 2026; Ng accessed? 2026-06-04; Archive Team uncertain 2024; Review Desk 2023)', $processor->renderCitationCluster([
...
<style xmlns="http://purl.org/net/xbiblio/csl" version="1.0" class="in-text" default-locale="en-US">
  <info>
    <title>Bounded Approximate Date Conditional Review Style</title>
    <id>https://example.test/styles/bounded-approximate-date-conditional-review</id>
...
XML);
        ...
        $summary = $processor->cslStyleSummary();
        $t->same('Bounded Approximate Date Conditional Review Style', $summary['title'] ?? null);
        $t->same(['issued'], $summary['citationRendering'][0]['branches'][0]['isUncertainDate'] ?? null);
        $t->same(['accessed'], $summary['citationRendering'][0]['branches'][1]['isUncertainDate'] ?? null);

        $t->same('(Smith c. 2026; Ng accessed c. 2026-06-06; Review Desk 2024)', $processor->renderCitationCluster([
            new AstNode('citation', ['id' => 'circa-issued', 'text' => '[@circa-issued]']),
            new AstNode('citation', ['id' => 'circa-accessed', 'text' => '[@circa-accessed]']),
            new AstNode('citation', ['id' => 'stable-source', 'text' => '[@stable-source]']),
        ]));
        $t->same('Approximate Issued Packet :: c. 2026', $processor->renderBibliographyEntry('circa-issued'));
        $t->same('Approximate Access Packet :: Date markers: accessed circa (2026-06-06~)', $processor->renderBibliographyEntry('circa-accessed'));
        $t->same('Stable Source Packet :: stable date', $processor->renderBibliographyEntry('stable-source'));
...
XML
        ...
        $t->same('Title Only Packet 2025.', $processor->renderBibliographyEntry('title-source'));
        ...
        $t->same([
            ...
            ['display' => 'right-inline', 'text' => '2025.'],
...
<style xmlns="http://purl.org/net/xbiblio/csl" version="1.0" class="in-text">
  <info>
    <title>Bounded Substitute Suppression Review</title>
    <id>https://example.test/styles/bounded-substitute-suppression-review</id>
...
XML);
        ...
        $summary = $processor->cslStyleSummary();
        $t->same('Bounded Substitute Suppression Review', $summary['title'] ?? null);
        $t->same('title', $summary['citationRendering'][0]['children'][0]['substitute'][0]['variable'] ?? null);
        $t->same('title', $summary['bibliographyRendering'][0]['substitute'][0]['variable'] ?? null);

        $t->same('(Diaz | Named Source Packet | 2026; Title Only Packet | 2025)', $processor->renderCitationCluster([
...
<style xmlns="http://purl.org/net/xbiblio/csl" version="1.0" class="in-text">
  <info>
    <title>Bounded Choose Multiple Values Review</title>
    <id>https://example.test/styles/bounded-choose-multiple-values-review</id>
...
XML);
        ...
        $summary = $processor->cslStyleSummary();
        $t->same('all', $summary['macros']['type-route'][0]['branches'][0]['match'] ?? null);
        $t->same(['book', 'article-journal'], $summary['macros']['type-route'][0]['branches'][0]['types'] ?? null);
        $t->same('any', $summary['macros']['type-route'][0]['branches'][1]['match'] ?? null);
        $t->same('all', $summary['macros']['locator-route'][0]['branches'][0]['match'] ?? null);
        $t->same(['page', 'section'], $summary['macros']['locator-route'][0]['branches'][0]['locators'] ?? null);

        $t->same('(type-any | locator-any 7 | Smith; type-any | locator-any 2 | Doe; type-none | locator-none 1 | Archive Desk)', $processor->renderCitationCluster([
            new AstNode('citation', ['id' => 'book-packet', 'text' => '[@book-packet]', 'locator' => 'p. 7']),
            new AstNode('citation', ['id' => 'article-packet', 'text' => '[@article-packet]', 'locator' => 'sec. 2']),
            new AstNode('citation', ['id' => 'web-packet', 'text' => '[@web-packet]', 'locator' => 'vol. 1']),
        ]));
        $t->same('type-any | Book Source Packet', $processor->renderBibliographyEntry('book-packet'));
        $t->same('type-any | Article Source Packet', $processor->renderBibliographyEntry('article-packet'));
        $t->same('type-none | Web Source Packet', $processor->renderBibliographyEntry('web-packet'));
