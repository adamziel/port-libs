...
return [
    ...
    'records csv uneven row width diagnostics without changing padded table output' => static function (TestRunner $t): void {
        $document = (new DelimitedTextReader())->readCsv(implode("\n", [
            'id,title,published',
            '42,"Legacy, ""quoted"" title",true',
        ...
        $table = $document->children[0];
        $packet = $table->attr('delimitedText');
        $summary = $packet['rowWidthSummary'] ?? [];
        ...
        $t->same([3, 3, 4, 2], $summary['rowWidths'] ?? null);
        $t->same([0, 1, 2, 3], $summary['sourceRowIndexes'] ?? null);
        $t->same([
            ['width' => 2, 'count' => 1],
            ['width' => 3, 'count' => 2],
            ['width' => 4, 'count' => 1],
        ], $summary['widthCounts'] ?? null);
        $t->same(false, $summary['strict']['consistent'] ?? null);
        $t->same(2, $summary['strict']['mismatchCount'] ?? null);
        $t->same('source-row-2', $summary['strict']['mismatches'][0]['rowLabel'] ?? null);
        $t->same(1, $summary['strict']['mismatches'][0]['extraFields'] ?? null);
        $t->same(1, $summary['strict']['mismatches'][1]['missingFields'] ?? null);
        $t->same(3, $summary['relaxed']['paddedRowCount'] ?? null);
        $t->same(2, $summary['header']['mismatchCount'] ?? null);
        $t->same([
    ...
    'records tsv blank rows trailing empty fields and relaxed width summaries' => static function (TestRunner $t): void {
        $document = (new DelimitedTextReader())->readTsv(implode("\n", [
            "key\tvalue\t",
            '',
            "alpha\t10\t",
        ...
        $t->same([3, 3, 2, 3], $summary['rowWidths'] ?? null);
        $t->same([0, 2, 3, 4], $summary['sourceRowIndexes'] ?? null);
        $t->same([0, 2, 4], $summary['trailingEmptyFieldRows'] ?? null);
        $t->same(false, $summary['strict']['consistent'] ?? null);
        $t->same(1, $summary['strict']['mismatchCount'] ?? null);
        $t->same(3, $summary['strict']['mismatches'][0]['sourceRow'] ?? null);
        $t->same(1, $summary['strict']['mismatches'][0]['missingFields'] ?? null);
        $t->same(3, $summary['relaxed']['columnCount'] ?? null);
        $t->same(1, $summary['relaxed']['paddedRowCount'] ?? null);
        $t->same('source-row-3', $summary['relaxed']['paddedRows'][0]['rowLabel'] ?? null);
        $t->same(1, $summary['header']['mismatchCount'] ?? null);
        $t->same([
