...
return [
    ...
    'checked-in markdown fixtures match native ast shape for paired seed corpus' => static function (TestRunner $t) use ($fixtureRoot): void {
        ...
        $t->same(115, $report['markdownFixtureCount']);
        ...
        $t->same(115, $report['markdownParsedCount']);
        ...
        $t->same(true, MarkdownNativeAstComparisonHarness::hasRequiredMappedParity($report, 115));
        $t->same(false, MarkdownNativeAstComparisonHarness::hasRequiredMappedParity($report, 116));
        ...
        $t->same('Markdown fixtures=115; native fixtures=115; same-basename pairs=115; Markdown fixtures without native pairs=0', $report['orderedRemainingGaps'][1]['currentEvidence']);
        $t->same('The current checked-in gate covers 115 paired fixture(s) out of 115 selected Markdown fixture(s).', $report['orderedRemainingGaps'][3]['currentEvidence']);
        $t->contains('fixtureInventory: markdown=115 native=115 paired=115 unpairedMarkdown=0 unpairedNative=0', $text);
        $t->contains('pairs: total=115 compared=115 parsedBoth=115 parseFailures=0', $text);
        ...
        $t->same(0, $exitCode);
        $t->same(115, $decoded['markdownFixtureCount']);
