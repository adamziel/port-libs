...
return [
    ...
    'checked-in markdown fixtures match native ast shape for paired seed corpus' => static function (TestRunner $t) use ($fixtureRoot): void {
        ...
        $t->same(true, MarkdownNativeAstComparisonHarness::hasRequiredMappedParity($report, 45));
        ...
        $t->same('Markdown fixtures=47; native fixtures=45; same-basename pairs=45; Markdown fixtures without native pairs=2', $report['orderedRemainingGaps'][1]['currentEvidence']);
        $t->same('The current checked-in gate covers 45 paired fixture(s) out of 47 selected Markdown fixture(s).', $report['orderedRemainingGaps'][3]['currentEvidence']);
        $t->contains('fixtureInventory: markdown=47 native=45 paired=45 unpairedMarkdown=2 unpairedNative=0', $text);
        ...
        $t->same(2, $decoded['unpairedMarkdownFixtureCount']);
