...
return [
    ...
    'checked-in markdown fixtures match native ast shape for paired seed corpus' => static function (TestRunner $t) use ($fixtureRoot): void {
        ...
        $t->same(48, $report['markdownFixtureCount']);
        ...
        $t->same(48, $report['markdownParsedCount']);
        ...
        $t->same(true, MarkdownNativeAstComparisonHarness::hasRequiredMappedParity($report, 48));
        $t->same(false, MarkdownNativeAstComparisonHarness::hasRequiredMappedParity($report, 49));
        ...
        $t->same('Markdown fixtures=48; native fixtures=48; same-basename pairs=48; Markdown fixtures without native pairs=0', $report['orderedRemainingGaps'][1]['currentEvidence']);
        $t->same('The current checked-in gate covers 48 paired fixture(s) out of 48 selected Markdown fixture(s).', $report['orderedRemainingGaps'][3]['currentEvidence']);
        $t->contains('fixtureInventory: markdown=48 native=48 paired=48 unpairedMarkdown=0 unpairedNative=0', $text);
        ...
        $t->same(0, $exitCode);
        $t->same(48, $decoded['markdownFixtureCount']);
