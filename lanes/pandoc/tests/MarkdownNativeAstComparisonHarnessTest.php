...
return [
    ...
    'checked-in markdown fixtures match native ast shape for paired seed corpus' => static function (TestRunner $t) use ($fixtureRoot): void {
        ...
        $t->same(true, MarkdownNativeAstComparisonHarness::hasRequiredMappedParity($report, 41));
        ...
        $t->same('Markdown fixtures=47; native fixtures=41; same-basename pairs=41; Markdown fixtures without native pairs=6', $report['orderedRemainingGaps'][1]['currentEvidence']);
        $t->same('The current checked-in gate covers 41 paired fixture(s) out of 47 selected Markdown fixture(s).', $report['orderedRemainingGaps'][3]['currentEvidence']);
        $t->contains('fixtureInventory: markdown=47 native=41 paired=41 unpairedMarkdown=6 unpairedNative=0', $text);
        ...
        $t->same(6, $decoded['unpairedMarkdownFixtureCount']);
