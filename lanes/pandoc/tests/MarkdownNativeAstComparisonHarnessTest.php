...
return [
    ...
    'checked-in markdown fixtures match native ast shape for paired seed corpus' => static function (TestRunner $t) use ($fixtureRoot): void {
        ...
        $t->same(78, $report['markdownFixtureCount']);
        ...
        $t->same(78, $report['markdownParsedCount']);
        ...
        $t->same(true, MarkdownNativeAstComparisonHarness::hasRequiredMappedParity($report, 78));
        $t->same(false, MarkdownNativeAstComparisonHarness::hasRequiredMappedParity($report, 79));
        ...
        $t->same('Markdown fixtures=78; native fixtures=78; same-basename pairs=78; Markdown fixtures without native pairs=0', $report['orderedRemainingGaps'][1]['currentEvidence']);
        $t->same('The current checked-in gate covers 78 paired fixture(s) out of 78 selected Markdown fixture(s).', $report['orderedRemainingGaps'][3]['currentEvidence']);
        $t->contains('fixtureInventory: markdown=78 native=78 paired=78 unpairedMarkdown=0 unpairedNative=0', $text);
        ...
        $t->same(0, $exitCode);
        $t->same(78, $decoded['markdownFixtureCount']);
