...
return [
    ...
    'checked-in markdown fixtures match native ast shape for paired seed corpus' => static function (TestRunner $t) use ($fixtureRoot): void {
        ...
        $paired = [
            ...
            'upstream-markdown-two-space-line-break',
        ];
        ...
        $t->true(
            is_file($root . '/upstream-markdown-two-space-line-break.md'),
            'upstream-markdown-two-space-line-break.md must be checked in'
        );
        $t->true(
            is_file($root . '/upstream-markdown-two-space-line-break.native'),
            'upstream-markdown-two-space-line-break.native must be checked in'
        );
        ...
        $t->same(137, $report['markdownFixtureCount']);
        $t->same(137, $report['nativeFixtureCount']);
        $t->same(137, $report['pairedFixtureCount']);
        $t->same(0, $report['unpairedMarkdownFixtureCount']);
        $t->same(0, $report['unpairedNativeFixtureCount']);
        $t->same(137, $report['totalPairCount']);
        $t->same(137, $report['comparedPairCount']);
        $t->same(137, $report['markdownParsedCount']);
        $t->same(137, $report['nativeParsedCount']);
        $t->same(137, $report['bothParsedCount']);
        ...
        $t->contains('normalizedAst: matches=137 (100.00%) mismatches=0', $text);
        ...
        $t->same(0, $exitCode);
        $t->same(137, $decoded['markdownFixtureCount']);
