...
return [
    ...
    'checked-in markdown fixtures match native ast shape for paired seed corpus' => static function (TestRunner $t) use ($fixtureRoot): void {
        ...
        $t->same(83, $report['markdownFixtureCount']);
        ...
        $t->same(83, $report['markdownParsedCount']);
        ...
        $t->same(true, MarkdownNativeAstComparisonHarness::hasRequiredMappedParity($report, 83));
        $t->same(false, MarkdownNativeAstComparisonHarness::hasRequiredMappedParity($report, 84));
        ...
        $t->same('Markdown fixtures=83; native fixtures=83; same-basename pairs=83; Markdown fixtures without native pairs=0', $report['orderedRemainingGaps'][1]['currentEvidence']);
        $t->same('The current checked-in gate covers 83 paired fixture(s) out of 83 selected Markdown fixture(s).', $report['orderedRemainingGaps'][3]['currentEvidence']);
        $t->contains('fixtureInventory: markdown=83 native=83 paired=83 unpairedMarkdown=0 unpairedNative=0', $text);
        ...
        $t->same(0, $exitCode);
        $t->same(83, $decoded['markdownFixtureCount']);
