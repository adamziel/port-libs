...
return [
    'validates checked-in real pandoc pptx executable evidence snapshot' => static function (TestRunner $t): void {
        ...
        $t->same(86, $snapshot['totalPptxCount']);
        $t->same(86, $snapshot['comparedPptxCount']);
        $t->same(86, $snapshot['localParsedCount']);
        $t->same(86, $snapshot['pandocParsedCount']);
        $t->same(86, $snapshot['nativeFixtureParsedCount']);
        $t->same(86, $snapshot['bothParsedCount']);
        $t->same(86, $snapshot['normalizedAstMatchCount']);
        $t->same(0, $snapshot['normalizedAstMismatchCount']);
        $t->same(86, $snapshot['pandocNativeFixtureComparedCount']);
        $t->same(86, $snapshot['pandocNativeFixtureMatchCount']);
        ...
        $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($snapshot, 86));
        ...
        $t->contains('--require-executable-parity=86', implode(' ', $snapshot['sourceCommand']));
    ...
    'cli selects checked-in fixtures for executable pptx parity with fake pandoc' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeFixtureAwareFakePandoc): void {
        ...
        try {
            ...
            $t->same(86, $decoded['comparedPptxCount']);
            $t->same(86, $decoded['localParsedCount']);
            $t->same(86, $decoded['pandocParsedCount']);
            $t->same(86, $decoded['nativeFixtureParsedCount']);
            $t->same(86, $decoded['bothParsedCount']);
            $t->same(86, $decoded['normalizedAstMatchCount']);
            $t->same(0, $decoded['normalizedAstMismatchCount']);
            $t->same(86, $decoded['pandocNativeFixtureComparedCount']);
            $t->same(86, $decoded['pandocNativeFixtureMatchCount']);
            $t->same(0, $decoded['pandocNativeFixtureMismatchCount']);
            $t->same(86, count($decoded['fixtureComparisons']));
            $t->same([], array_values(array_filter(
                $decoded['fixtureComparisons'],
                static fn (array $row): bool => ($row['status'] ?? null) !== 'matched'
            )));
            $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($decoded, 86));
