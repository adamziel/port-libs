...
return [
    'validates checked-in real pandoc pptx executable evidence snapshot' => static function (TestRunner $t): void {
        ...
        $t->same(['basic', 'body-before-title', 'break-tab-field', 'bullets', 'bunone-wingdings', 'center-title-placeholder', 'chart-placeholder', 'comments-ignored', 'connector-skip', 'content-part-skip', 'embedded-image', 'empty-paragraph-textbox', 'generated-table', 'grouped-shapes', 'hex-list-level', 'hidden-slide', 'hyperlink-text', 'inline-formatting', 'list-continuation', 'minimal', 'missing-relationship-skip', 'multi-paragraph-textbox', 'multiple-paragraph-properties', 'nested-list', 'no-title-fallback', 'numbered-list', 'paragraphless-textbox', 'shape-order', 'slide-placeholders', 'smartart-hierarchy', 'speaker-notes', 'table-span-review', 'two-slides', 'wingdings-typeface-case'], $snapshot['fixtureStems']);
        $t->same(34, $snapshot['totalPptxCount']);
        $t->same(34, $snapshot['comparedPptxCount']);
        $t->same(34, $snapshot['localParsedCount']);
        $t->same(34, $snapshot['pandocParsedCount']);
        $t->same(34, $snapshot['nativeFixtureParsedCount']);
        $t->same(34, $snapshot['bothParsedCount']);
        $t->same(34, $snapshot['normalizedAstMatchCount']);
        $t->same(0, $snapshot['normalizedAstMismatchCount']);
        $t->same(34, $snapshot['pandocNativeFixtureComparedCount']);
        $t->same(34, $snapshot['pandocNativeFixtureMatchCount']);
        ...
        $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($snapshot, 34));
        $t->contains('--require-executable-parity=34', implode(' ', $snapshot['sourceCommand']));
        ...
        $t->true(in_array('that generated body-before-title, break-tab-field, bullets, bunone-wingdings, center-title-placeholder, chart-placeholder, comments-ignored, connector-skip, content-part-skip, embedded-image, empty-paragraph-textbox, generated-table, grouped-shapes, hex-list-level, hidden-slide, hyperlink-text, inline-formatting, list-continuation, minimal, missing-relationship-skip, multi-paragraph-textbox, multiple-paragraph-properties, nested-list, no-title-fallback, numbered-list, paragraphless-textbox, shape-order, slide-placeholders, smartart-hierarchy, speaker-notes, table-span-review, two-slides, wingdings-typeface-case fixtures are upstream Tests.Readers.Pptx fixtures', $snapshot['claimBoundaries']['doesNotAssert'], true));
    ...
    'cli selects checked-in fixtures for executable pptx parity with fake pandoc' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeFixtureAwareFakePandoc): void {
        ...
        try {
            ...
            $t->same(34, $decoded['comparedPptxCount']);
            $t->same(34, $decoded['localParsedCount']);
            $t->same(34, $decoded['pandocParsedCount']);
            $t->same(34, $decoded['nativeFixtureParsedCount']);
            $t->same(34, $decoded['bothParsedCount']);
            $t->same(34, $decoded['normalizedAstMatchCount']);
            $t->same(0, $decoded['normalizedAstMismatchCount']);
            $t->same(34, $decoded['pandocNativeFixtureComparedCount']);
            $t->same(34, $decoded['pandocNativeFixtureMatchCount']);
            $t->same(0, $decoded['pandocNativeFixtureMismatchCount']);
            $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($decoded, 34));
