...
return [
    'validates checked-in real pandoc pptx executable evidence snapshot' => static function (TestRunner $t): void {
        ...
        $t->same(75, $snapshot['totalPptxCount']);
        $t->same(75, $snapshot['comparedPptxCount']);
        $t->same(75, $snapshot['localParsedCount']);
        $t->same(75, $snapshot['pandocParsedCount']);
        $t->same(75, $snapshot['nativeFixtureParsedCount']);
        $t->same(75, $snapshot['bothParsedCount']);
        $t->same(75, $snapshot['normalizedAstMatchCount']);
        $t->same(0, $snapshot['normalizedAstMismatchCount']);
        $t->same(75, $snapshot['pandocNativeFixtureComparedCount']);
        $t->same(75, $snapshot['pandocNativeFixtureMatchCount']);
        ...
        $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($snapshot, 75));
        ...
        $t->contains('--require-executable-parity=75', implode(' ', $snapshot['sourceCommand']));
    ...
    'cli selects checked-in fixtures for executable pptx parity with fake pandoc' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeFixtureAwareFakePandoc): void {
        ...
        try {
            ...
            $t->same(75, $decoded['comparedPptxCount']);
            $t->same(75, $decoded['localParsedCount']);
            $t->same(75, $decoded['pandocParsedCount']);
            $t->same(75, $decoded['nativeFixtureParsedCount']);
            $t->same(75, $decoded['bothParsedCount']);
            $t->same(75, $decoded['normalizedAstMatchCount']);
            $t->same(0, $decoded['normalizedAstMismatchCount']);
            $t->same(75, $decoded['pandocNativeFixtureComparedCount']);
            $t->same(75, $decoded['pandocNativeFixtureMatchCount']);
            $t->same(0, $decoded['pandocNativeFixtureMismatchCount']);
            $t->same(75, count($decoded['fixtureComparisons']));
            $t->same([], array_values(array_filter(
                $decoded['fixtureComparisons'],
                static fn (array $row): bool => ($row['status'] ?? null) !== 'matched'
            )));
            $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($decoded, 75));
