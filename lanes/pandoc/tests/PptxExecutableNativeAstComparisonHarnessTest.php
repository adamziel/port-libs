...
return [
    'validates checked-in real pandoc pptx executable evidence snapshot' => static function (TestRunner $t): void {
        ...
        $t->same(62, $snapshot['totalPptxCount']);
        $t->same(62, $snapshot['comparedPptxCount']);
        $t->same(62, $snapshot['localParsedCount']);
        $t->same(62, $snapshot['pandocParsedCount']);
        $t->same(62, $snapshot['nativeFixtureParsedCount']);
        $t->same(62, $snapshot['bothParsedCount']);
        $t->same(62, $snapshot['normalizedAstMatchCount']);
        $t->same(0, $snapshot['normalizedAstMismatchCount']);
        $t->same(62, $snapshot['pandocNativeFixtureComparedCount']);
        $t->same(62, $snapshot['pandocNativeFixtureMatchCount']);
        ...
        $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($snapshot, 62));
        ...
        $t->contains('--require-executable-parity=62', implode(' ', $snapshot['sourceCommand']));
    ...
    'cli selects checked-in fixtures for executable pptx parity with fake pandoc' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeFixtureAwareFakePandoc): void {
        ...
        try {
            ...
            $t->same(62, $decoded['comparedPptxCount']);
            $t->same(62, $decoded['localParsedCount']);
            $t->same(62, $decoded['pandocParsedCount']);
            $t->same(62, $decoded['nativeFixtureParsedCount']);
            $t->same(62, $decoded['bothParsedCount']);
            $t->same(62, $decoded['normalizedAstMatchCount']);
            $t->same(0, $decoded['normalizedAstMismatchCount']);
            $t->same(62, $decoded['pandocNativeFixtureComparedCount']);
            $t->same(62, $decoded['pandocNativeFixtureMatchCount']);
            $t->same(0, $decoded['pandocNativeFixtureMismatchCount']);
            $t->same(62, count($decoded['fixtureComparisons']));
            $t->same([], array_values(array_filter(
                $decoded['fixtureComparisons'],
                static fn (array $row): bool => ($row['status'] ?? null) !== 'matched'
            )));
            $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($decoded, 62));
