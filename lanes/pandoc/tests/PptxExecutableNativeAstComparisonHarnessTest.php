...
return [
    'validates checked-in real pandoc pptx executable evidence snapshot' => static function (TestRunner $t): void {
        ...
        $t->same(84, $snapshot['totalPptxCount']);
        $t->same(84, $snapshot['comparedPptxCount']);
        $t->same(84, $snapshot['localParsedCount']);
        $t->same(84, $snapshot['pandocParsedCount']);
        $t->same(84, $snapshot['nativeFixtureParsedCount']);
        $t->same(84, $snapshot['bothParsedCount']);
        $t->same(84, $snapshot['normalizedAstMatchCount']);
        $t->same(0, $snapshot['normalizedAstMismatchCount']);
        $t->same(84, $snapshot['pandocNativeFixtureComparedCount']);
        $t->same(84, $snapshot['pandocNativeFixtureMatchCount']);
        ...
        $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($snapshot, 84));
        ...
        $t->contains('--require-executable-parity=84', implode(' ', $snapshot['sourceCommand']));
    ...
    'cli selects checked-in fixtures for executable pptx parity with fake pandoc' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeFixtureAwareFakePandoc): void {
        ...
        try {
            ...
            $t->same(84, $decoded['comparedPptxCount']);
            $t->same(84, $decoded['localParsedCount']);
            $t->same(84, $decoded['pandocParsedCount']);
            $t->same(84, $decoded['nativeFixtureParsedCount']);
            $t->same(84, $decoded['bothParsedCount']);
            $t->same(84, $decoded['normalizedAstMatchCount']);
            $t->same(0, $decoded['normalizedAstMismatchCount']);
            $t->same(84, $decoded['pandocNativeFixtureComparedCount']);
            $t->same(84, $decoded['pandocNativeFixtureMatchCount']);
            $t->same(0, $decoded['pandocNativeFixtureMismatchCount']);
            $t->same(84, count($decoded['fixtureComparisons']));
            $t->same([], array_values(array_filter(
                $decoded['fixtureComparisons'],
                static fn (array $row): bool => ($row['status'] ?? null) !== 'matched'
            )));
            $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($decoded, 84));
