...
return [
    'validates checked-in real pandoc pptx executable evidence snapshot' => static function (TestRunner $t): void {
        ...
        $t->same(89, $snapshot['totalPptxCount']);
        $t->same(89, $snapshot['comparedPptxCount']);
        $t->same(89, $snapshot['localParsedCount']);
        $t->same(89, $snapshot['pandocParsedCount']);
        $t->same(89, $snapshot['nativeFixtureParsedCount']);
        $t->same(89, $snapshot['bothParsedCount']);
        $t->same(89, $snapshot['normalizedAstMatchCount']);
        $t->same(0, $snapshot['normalizedAstMismatchCount']);
        $t->same(89, $snapshot['pandocNativeFixtureComparedCount']);
        $t->same(89, $snapshot['pandocNativeFixtureMatchCount']);
        ...
        $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($snapshot, 89));
        ...
        $t->contains('--require-executable-parity=89', implode(' ', $snapshot['sourceCommand']));
    ...
    'cli selects checked-in fixtures for executable pptx parity with fake pandoc' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeFixtureAwareFakePandoc): void {
        ...
        try {
            ...
            $t->same(89, $decoded['comparedPptxCount']);
            $t->same(89, $decoded['localParsedCount']);
            $t->same(89, $decoded['pandocParsedCount']);
            $t->same(89, $decoded['nativeFixtureParsedCount']);
            $t->same(89, $decoded['bothParsedCount']);
            $t->same(89, $decoded['normalizedAstMatchCount']);
            $t->same(0, $decoded['normalizedAstMismatchCount']);
            $t->same(89, $decoded['pandocNativeFixtureComparedCount']);
            $t->same(89, $decoded['pandocNativeFixtureMatchCount']);
            $t->same(0, $decoded['pandocNativeFixtureMismatchCount']);
            $t->same(89, count($decoded['fixtureComparisons']));
            $t->same([], array_values(array_filter(
                $decoded['fixtureComparisons'],
                static fn (array $row): bool => ($row['status'] ?? null) !== 'matched'
            )));
            $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($decoded, 89));
