...
return [
    'validates checked-in real pandoc pptx executable evidence snapshot' => static function (TestRunner $t): void {
        ...
        $t->same(96, $snapshot['totalPptxCount']);
        $t->same(96, $snapshot['comparedPptxCount']);
        $t->same(96, $snapshot['localParsedCount']);
        $t->same(96, $snapshot['pandocParsedCount']);
        $t->same(96, $snapshot['nativeFixtureParsedCount']);
        $t->same(96, $snapshot['bothParsedCount']);
        $t->same(96, $snapshot['normalizedAstMatchCount']);
        $t->same(0, $snapshot['normalizedAstMismatchCount']);
        $t->same(96, $snapshot['pandocNativeFixtureComparedCount']);
        $t->same(96, $snapshot['pandocNativeFixtureMatchCount']);
        ...
        $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($snapshot, 96));
        ...
        $t->contains('--require-executable-parity=96', implode(' ', $snapshot['sourceCommand']));
    ...
    'cli selects checked-in fixtures for executable pptx parity with fake pandoc' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeFixtureAwareFakePandoc): void {
        ...
        try {
            ...
            $t->same(96, $decoded['comparedPptxCount']);
            $t->same(96, $decoded['localParsedCount']);
            $t->same(96, $decoded['pandocParsedCount']);
            $t->same(96, $decoded['nativeFixtureParsedCount']);
            $t->same(96, $decoded['bothParsedCount']);
            $t->same(96, $decoded['normalizedAstMatchCount']);
            $t->same(0, $decoded['normalizedAstMismatchCount']);
            $t->same(96, $decoded['pandocNativeFixtureComparedCount']);
            $t->same(96, $decoded['pandocNativeFixtureMatchCount']);
            $t->same(0, $decoded['pandocNativeFixtureMismatchCount']);
            $t->same(96, count($decoded['fixtureComparisons']));
            $t->same([], array_values(array_filter(
                $decoded['fixtureComparisons'],
                static fn (array $row): bool => ($row['status'] ?? null) !== 'matched'
            )));
            $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($decoded, 96));
