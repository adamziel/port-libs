...
return [
    'validates checked-in real pandoc pptx executable evidence snapshot' => static function (TestRunner $t): void {
        ...
        $t->same(66, $snapshot['totalPptxCount']);
        $t->same(66, $snapshot['comparedPptxCount']);
        $t->same(66, $snapshot['localParsedCount']);
        $t->same(66, $snapshot['pandocParsedCount']);
        $t->same(66, $snapshot['nativeFixtureParsedCount']);
        $t->same(66, $snapshot['bothParsedCount']);
        $t->same(66, $snapshot['normalizedAstMatchCount']);
        $t->same(0, $snapshot['normalizedAstMismatchCount']);
        $t->same(66, $snapshot['pandocNativeFixtureComparedCount']);
        $t->same(66, $snapshot['pandocNativeFixtureMatchCount']);
        ...
        $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($snapshot, 66));
        ...
        $t->contains('--require-executable-parity=66', implode(' ', $snapshot['sourceCommand']));
    ...
    'cli selects checked-in fixtures for executable pptx parity with fake pandoc' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeFixtureAwareFakePandoc): void {
        ...
        try {
            ...
            $t->same(66, $decoded['comparedPptxCount']);
            $t->same(66, $decoded['localParsedCount']);
            $t->same(66, $decoded['pandocParsedCount']);
            $t->same(66, $decoded['nativeFixtureParsedCount']);
            $t->same(66, $decoded['bothParsedCount']);
            $t->same(66, $decoded['normalizedAstMatchCount']);
            $t->same(0, $decoded['normalizedAstMismatchCount']);
            $t->same(66, $decoded['pandocNativeFixtureComparedCount']);
            $t->same(66, $decoded['pandocNativeFixtureMatchCount']);
            $t->same(0, $decoded['pandocNativeFixtureMismatchCount']);
            $t->same(66, count($decoded['fixtureComparisons']));
            $t->same([], array_values(array_filter(
                $decoded['fixtureComparisons'],
                static fn (array $row): bool => ($row['status'] ?? null) !== 'matched'
            )));
            $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($decoded, 66));
