...
return [
    'validates checked-in real pandoc pptx executable evidence snapshot' => static function (TestRunner $t): void {
        ...
        $t->same(100, $snapshot['totalPptxCount']);
        $t->same(100, $snapshot['comparedPptxCount']);
        $t->same(100, $snapshot['localParsedCount']);
        $t->same(100, $snapshot['pandocParsedCount']);
        $t->same(100, $snapshot['nativeFixtureParsedCount']);
        $t->same(100, $snapshot['bothParsedCount']);
        $t->same(100, $snapshot['normalizedAstMatchCount']);
        $t->same(0, $snapshot['normalizedAstMismatchCount']);
        $t->same(100, $snapshot['pandocNativeFixtureComparedCount']);
        $t->same(100, $snapshot['pandocNativeFixtureMatchCount']);
        ...
        $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($snapshot, 100));
        ...
        $t->contains('--require-executable-parity=100', implode(' ', $snapshot['sourceCommand']));
    ...
    'cli selects checked-in fixtures for executable pptx parity with fake pandoc' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeFixtureAwareFakePandoc): void {
        ...
        try {
            ...
            $t->same(100, $decoded['comparedPptxCount']);
            $t->same(100, $decoded['localParsedCount']);
            $t->same(100, $decoded['pandocParsedCount']);
            $t->same(100, $decoded['nativeFixtureParsedCount']);
            $t->same(100, $decoded['bothParsedCount']);
            $t->same(100, $decoded['normalizedAstMatchCount']);
            $t->same(0, $decoded['normalizedAstMismatchCount']);
            $t->same(100, $decoded['pandocNativeFixtureComparedCount']);
            $t->same(100, $decoded['pandocNativeFixtureMatchCount']);
            $t->same(0, $decoded['pandocNativeFixtureMismatchCount']);
            $t->same(100, count($decoded['fixtureComparisons']));
            $t->same([], array_values(array_filter(
                $decoded['fixtureComparisons'],
                static fn (array $row): bool => ($row['status'] ?? null) !== 'matched'
            )));
            $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($decoded, 100));
