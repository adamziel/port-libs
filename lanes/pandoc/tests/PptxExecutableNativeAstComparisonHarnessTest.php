...
return [
    'validates checked-in real pandoc pptx executable evidence snapshot' => static function (TestRunner $t): void {
        ...
        $t->same(98, $snapshot['totalPptxCount']);
        $t->same(98, $snapshot['comparedPptxCount']);
        $t->same(98, $snapshot['localParsedCount']);
        $t->same(98, $snapshot['pandocParsedCount']);
        $t->same(98, $snapshot['nativeFixtureParsedCount']);
        $t->same(98, $snapshot['bothParsedCount']);
        $t->same(98, $snapshot['normalizedAstMatchCount']);
        $t->same(0, $snapshot['normalizedAstMismatchCount']);
        $t->same(98, $snapshot['pandocNativeFixtureComparedCount']);
        $t->same(98, $snapshot['pandocNativeFixtureMatchCount']);
        ...
        $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($snapshot, 98));
        ...
        $t->contains('--require-executable-parity=98', implode(' ', $snapshot['sourceCommand']));
    ...
    'cli selects checked-in fixtures for executable pptx parity with fake pandoc' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeFixtureAwareFakePandoc): void {
        ...
        try {
            ...
            $t->same(98, $decoded['comparedPptxCount']);
            $t->same(98, $decoded['localParsedCount']);
            $t->same(98, $decoded['pandocParsedCount']);
            $t->same(98, $decoded['nativeFixtureParsedCount']);
            $t->same(98, $decoded['bothParsedCount']);
            $t->same(98, $decoded['normalizedAstMatchCount']);
            $t->same(0, $decoded['normalizedAstMismatchCount']);
            $t->same(98, $decoded['pandocNativeFixtureComparedCount']);
            $t->same(98, $decoded['pandocNativeFixtureMatchCount']);
            $t->same(0, $decoded['pandocNativeFixtureMismatchCount']);
            $t->same(98, count($decoded['fixtureComparisons']));
            $t->same([], array_values(array_filter(
                $decoded['fixtureComparisons'],
                static fn (array $row): bool => ($row['status'] ?? null) !== 'matched'
            )));
            $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($decoded, 98));
