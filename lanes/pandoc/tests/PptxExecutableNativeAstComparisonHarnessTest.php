...
return [
    'validates checked-in real pandoc pptx executable evidence snapshot' => static function (TestRunner $t): void {
        ...
        $t->same(['alternate-content-skip', 'background-image-skip', 'basic', 'body-before-title', 'break-tab-field', 'bullets', 'bunone-wingdings', 'case-sensitive-placeholder-type', 'cdata-entity-text', 'center-title-placeholder', 'chart-placeholder', 'comments-ignored', 'connector-skip', 'connector-text-skip', 'content-part-skip', 'diagram-missing-rels', 'diagram-no-relids', 'direct-drawing-paragraphs', 'document-properties', 'dot-presentation-target', 'dot-slide-target', 'duplicate-relationship-id', 'duplicate-slide-reference', 'embed-and-link-image', 'embedded-image', 'empty-bullet-paragraph', 'empty-header-table', 'empty-paragraph-textbox', 'end-paragraph-symbol', 'external-mode-slide-target', 'first-run-property-symbol', 'first-text-body', 'first-title-placeholder', 'generated-table', 'graphic-no-uri', 'grouped-shape-media-review', 'grouped-shapes', 'hex-list-level', 'hidden-shape-metadata', 'hidden-slide', 'hyperlink-text', 'ignored-slide-id-attributes', 'inline-formatting', 'linked-image-skip', 'list-continuation', 'media-relative-image-target', 'minimal', 'missing-relationship-skip', 'missing-slide-relationship-type', 'multi-paragraph-table-cell', 'multi-paragraph-textbox', 'multiple-paragraph-properties', 'namespace-agnostic-drawing-text', 'namespace-scoped-table', 'nested-list', 'no-slides', 'no-title-fallback', 'numbered-list', 'octal-list-level', 'overflow-bullet-level', 'pandoc-generated-image-alt-title', 'paragraph-property-descendant-text', 'paragraphless-textbox', 'parenthesized-bullet-level', 'percent-encoded-target', 'picture-shape-hyperlink', 'qualified-bullet-level', 'qualified-picture-metadata', 'rel-prefix-image-skip', 'repeated-slash-slide-target', 'rich-media-skip', 'rooted-slide-target', 'shape-order', 'signed-bullet-level', 'slide-layout-placeholder-no-inherit', 'slide-placeholders', 'smartart-hierarchy', 'smartart-title-fallback', 'speaker-notes', 'subtitle-placeholder', 'table-span-review', 'table-styles-relationship', 'textbox-without-nonvisual-properties', 'transition-animation-metadata', 'two-slides', 'unicode-drawing-text', 'unknown-graphic-uri', 'whitespace-drawing-text', 'wingdings-typeface-case', 'wrong-typed-slide-relationship'], $snapshot['fixtureStems']);
        $t->same(90, $snapshot['totalPptxCount']);
        $t->same(90, $snapshot['comparedPptxCount']);
        $t->same(90, $snapshot['localParsedCount']);
        $t->same(90, $snapshot['pandocParsedCount']);
        $t->same(90, $snapshot['nativeFixtureParsedCount']);
        $t->same(90, $snapshot['bothParsedCount']);
        $t->same(90, $snapshot['normalizedAstMatchCount']);
        $t->same(0, $snapshot['normalizedAstMismatchCount']);
        $t->same(90, $snapshot['pandocNativeFixtureComparedCount']);
        $t->same(90, $snapshot['pandocNativeFixtureMatchCount']);
        ...
        $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($snapshot, 90));
        ...
        $t->contains('--require-executable-parity=90', implode(' ', $snapshot['sourceCommand']));
    ...
    'cli selects checked-in fixtures for executable pptx parity with fake pandoc' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeFixtureAwareFakePandoc): void {
        ...
        try {
            ...
            $t->same(90, $decoded['comparedPptxCount']);
            $t->same(90, $decoded['localParsedCount']);
            $t->same(90, $decoded['pandocParsedCount']);
            $t->same(90, $decoded['nativeFixtureParsedCount']);
            $t->same(90, $decoded['bothParsedCount']);
            $t->same(90, $decoded['normalizedAstMatchCount']);
            $t->same(0, $decoded['normalizedAstMismatchCount']);
            $t->same(90, $decoded['pandocNativeFixtureComparedCount']);
            $t->same(90, $decoded['pandocNativeFixtureMatchCount']);
            $t->same(0, $decoded['pandocNativeFixtureMismatchCount']);
            $t->same(90, count($decoded['fixtureComparisons']));
            $t->same([], array_values(array_filter(
                $decoded['fixtureComparisons'],
                static fn (array $row): bool => ($row['status'] ?? null) !== 'matched'
            )));
            $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($decoded, 90));
