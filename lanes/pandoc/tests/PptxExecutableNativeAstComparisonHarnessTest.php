...
return [
    'validates checked-in real pandoc pptx executable evidence snapshot' => static function (TestRunner $t): void {
        ...
        $t->same(99, $snapshot['totalPptxCount']);
        $t->same(99, $snapshot['comparedPptxCount']);
        $t->same(99, $snapshot['localParsedCount']);
        $t->same(99, $snapshot['pandocParsedCount']);
        $t->same(99, $snapshot['nativeFixtureParsedCount']);
        $t->same(99, $snapshot['bothParsedCount']);
        $t->same(99, $snapshot['normalizedAstMatchCount']);
        $t->same(0, $snapshot['normalizedAstMismatchCount']);
        $t->same(99, $snapshot['pandocNativeFixtureComparedCount']);
        $t->same(99, $snapshot['pandocNativeFixtureMatchCount']);
        ...
        $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($snapshot, 99));
        ...
        $t->contains('--require-executable-parity=99', implode(' ', $snapshot['sourceCommand']));
    ...
    'cli selects checked-in fixtures for executable pptx parity with fake pandoc' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeFixtureAwareFakePandoc): void {
        ...
        try {
            ...
            $t->same(99, $decoded['comparedPptxCount']);
            $t->same(99, $decoded['localParsedCount']);
            $t->same(99, $decoded['pandocParsedCount']);
            $t->same(99, $decoded['nativeFixtureParsedCount']);
            $t->same(99, $decoded['bothParsedCount']);
            $t->same(99, $decoded['normalizedAstMatchCount']);
            $t->same(0, $decoded['normalizedAstMismatchCount']);
            $t->same(99, $decoded['pandocNativeFixtureComparedCount']);
            $t->same(99, $decoded['pandocNativeFixtureMatchCount']);
            $t->same(0, $decoded['pandocNativeFixtureMismatchCount']);
            $t->same(99, count($decoded['fixtureComparisons']));
            $t->same([], array_values(array_filter(
                $decoded['fixtureComparisons'],
                static fn (array $row): bool => ($row['status'] ?? null) !== 'matched'
            )));
            $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($decoded, 99));
