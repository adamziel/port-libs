...
return [
    'validates checked-in real pandoc pptx executable evidence snapshot' => static function (TestRunner $t): void {
        ...
        $t->same(64, $snapshot['totalPptxCount']);
        $t->same(64, $snapshot['comparedPptxCount']);
        $t->same(64, $snapshot['localParsedCount']);
        $t->same(64, $snapshot['pandocParsedCount']);
        $t->same(64, $snapshot['nativeFixtureParsedCount']);
        $t->same(64, $snapshot['bothParsedCount']);
        $t->same(64, $snapshot['normalizedAstMatchCount']);
        $t->same(0, $snapshot['normalizedAstMismatchCount']);
        $t->same(64, $snapshot['pandocNativeFixtureComparedCount']);
        $t->same(64, $snapshot['pandocNativeFixtureMatchCount']);
        ...
        $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($snapshot, 64));
        ...
        $t->contains('--require-executable-parity=64', implode(' ', $snapshot['sourceCommand']));
    ...
    'cli selects checked-in fixtures for executable pptx parity with fake pandoc' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeFixtureAwareFakePandoc): void {
        ...
        try {
            ...
            $t->same(64, $decoded['comparedPptxCount']);
            $t->same(64, $decoded['localParsedCount']);
            $t->same(64, $decoded['pandocParsedCount']);
            $t->same(64, $decoded['nativeFixtureParsedCount']);
            $t->same(64, $decoded['bothParsedCount']);
            $t->same(64, $decoded['normalizedAstMatchCount']);
            $t->same(0, $decoded['normalizedAstMismatchCount']);
            $t->same(64, $decoded['pandocNativeFixtureComparedCount']);
            $t->same(64, $decoded['pandocNativeFixtureMatchCount']);
            $t->same(0, $decoded['pandocNativeFixtureMismatchCount']);
            $t->same(64, count($decoded['fixtureComparisons']));
            $t->same([], array_values(array_filter(
                $decoded['fixtureComparisons'],
                static fn (array $row): bool => ($row['status'] ?? null) !== 'matched'
            )));
            $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($decoded, 64));
