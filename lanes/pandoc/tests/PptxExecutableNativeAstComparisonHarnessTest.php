...
return [
    'validates checked-in real pandoc pptx executable evidence snapshot' => static function (TestRunner $t): void {
        ...
        $t->same(58, $snapshot['totalPptxCount']);
        $t->same(58, $snapshot['comparedPptxCount']);
        $t->same(58, $snapshot['localParsedCount']);
        $t->same(58, $snapshot['pandocParsedCount']);
        $t->same(58, $snapshot['nativeFixtureParsedCount']);
        $t->same(58, $snapshot['bothParsedCount']);
        $t->same(58, $snapshot['normalizedAstMatchCount']);
        $t->same(0, $snapshot['normalizedAstMismatchCount']);
        $t->same(58, $snapshot['pandocNativeFixtureComparedCount']);
        $t->same(58, $snapshot['pandocNativeFixtureMatchCount']);
        ...
        $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($snapshot, 58));
        ...
        $t->contains('--require-executable-parity=58', implode(' ', $snapshot['sourceCommand']));
    ...
    'cli selects checked-in fixtures for executable pptx parity with fake pandoc' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeFixtureAwareFakePandoc): void {
        ...
        try {
            ...
            $t->same(58, $decoded['comparedPptxCount']);
            $t->same(58, $decoded['localParsedCount']);
            $t->same(58, $decoded['pandocParsedCount']);
            $t->same(58, $decoded['nativeFixtureParsedCount']);
            $t->same(58, $decoded['bothParsedCount']);
            $t->same(58, $decoded['normalizedAstMatchCount']);
            $t->same(0, $decoded['normalizedAstMismatchCount']);
            $t->same(58, $decoded['pandocNativeFixtureComparedCount']);
            $t->same(58, $decoded['pandocNativeFixtureMatchCount']);
            $t->same(0, $decoded['pandocNativeFixtureMismatchCount']);
            $t->same(58, count($decoded['fixtureComparisons']));
            $t->same([], array_values(array_filter(
                $decoded['fixtureComparisons'],
                static fn (array $row): bool => ($row['status'] ?? null) !== 'matched'
            )));
            $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($decoded, 58));
