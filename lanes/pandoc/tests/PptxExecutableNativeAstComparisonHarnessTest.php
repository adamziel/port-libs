...
return [
    'validates checked-in real pandoc pptx executable evidence snapshot' => static function (TestRunner $t): void {
        ...
        $t->same(72, $snapshot['totalPptxCount']);
        $t->same(72, $snapshot['comparedPptxCount']);
        $t->same(72, $snapshot['localParsedCount']);
        $t->same(72, $snapshot['pandocParsedCount']);
        $t->same(72, $snapshot['nativeFixtureParsedCount']);
        $t->same(72, $snapshot['bothParsedCount']);
        $t->same(72, $snapshot['normalizedAstMatchCount']);
        $t->same(0, $snapshot['normalizedAstMismatchCount']);
        $t->same(72, $snapshot['pandocNativeFixtureComparedCount']);
        $t->same(72, $snapshot['pandocNativeFixtureMatchCount']);
        ...
        $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($snapshot, 72));
        ...
        $t->contains('--require-executable-parity=72', implode(' ', $snapshot['sourceCommand']));
    ...
    'cli selects checked-in fixtures for executable pptx parity with fake pandoc' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeFixtureAwareFakePandoc): void {
        ...
        try {
            ...
            $t->same(72, $decoded['comparedPptxCount']);
            $t->same(72, $decoded['localParsedCount']);
            $t->same(72, $decoded['pandocParsedCount']);
            $t->same(72, $decoded['nativeFixtureParsedCount']);
            $t->same(72, $decoded['bothParsedCount']);
            $t->same(72, $decoded['normalizedAstMatchCount']);
            $t->same(0, $decoded['normalizedAstMismatchCount']);
            $t->same(72, $decoded['pandocNativeFixtureComparedCount']);
            $t->same(72, $decoded['pandocNativeFixtureMatchCount']);
            $t->same(0, $decoded['pandocNativeFixtureMismatchCount']);
            $t->same(72, count($decoded['fixtureComparisons']));
            $t->same([], array_values(array_filter(
                $decoded['fixtureComparisons'],
                static fn (array $row): bool => ($row['status'] ?? null) !== 'matched'
            )));
            $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($decoded, 72));
