...
return [
    'validates checked-in real pandoc pptx executable evidence snapshot' => static function (TestRunner $t): void {
        ...
        $t->same(57, $snapshot['totalPptxCount']);
        $t->same(57, $snapshot['comparedPptxCount']);
        $t->same(57, $snapshot['localParsedCount']);
        $t->same(57, $snapshot['pandocParsedCount']);
        $t->same(57, $snapshot['nativeFixtureParsedCount']);
        $t->same(57, $snapshot['bothParsedCount']);
        $t->same(57, $snapshot['normalizedAstMatchCount']);
        $t->same(0, $snapshot['normalizedAstMismatchCount']);
        $t->same(57, $snapshot['pandocNativeFixtureComparedCount']);
        $t->same(57, $snapshot['pandocNativeFixtureMatchCount']);
        ...
        $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($snapshot, 57));
        ...
        $t->contains('--require-executable-parity=57', implode(' ', $snapshot['sourceCommand']));
    ...
    'cli selects checked-in fixtures for executable pptx parity with fake pandoc' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeFixtureAwareFakePandoc): void {
        ...
        try {
            ...
            $t->same(57, $decoded['comparedPptxCount']);
            $t->same(57, $decoded['localParsedCount']);
            $t->same(57, $decoded['pandocParsedCount']);
            $t->same(57, $decoded['nativeFixtureParsedCount']);
            $t->same(57, $decoded['bothParsedCount']);
            $t->same(57, $decoded['normalizedAstMatchCount']);
            $t->same(0, $decoded['normalizedAstMismatchCount']);
            $t->same(57, $decoded['pandocNativeFixtureComparedCount']);
            $t->same(57, $decoded['pandocNativeFixtureMatchCount']);
            $t->same(0, $decoded['pandocNativeFixtureMismatchCount']);
            $t->same(57, count($decoded['fixtureComparisons']));
            $t->same([], array_values(array_filter(
                $decoded['fixtureComparisons'],
                static fn (array $row): bool => ($row['status'] ?? null) !== 'matched'
            )));
            $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($decoded, 57));
