...
return [
    'validates checked-in real pandoc pptx executable evidence snapshot' => static function (TestRunner $t): void {
        ...
        $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($snapshot, 49));
        ...
        $t->contains('--require-executable-parity=49', implode(' ', $snapshot['sourceCommand']));
    ...
    'cli selects checked-in fixtures for executable pptx parity with fake pandoc' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeFixtureAwareFakePandoc): void {
        ...
        try {
            ...
            $t->same(49, $decoded['comparedPptxCount']);
            $t->same(49, $decoded['localParsedCount']);
            $t->same(49, $decoded['pandocParsedCount']);
            $t->same(49, $decoded['nativeFixtureParsedCount']);
            $t->same(49, $decoded['bothParsedCount']);
