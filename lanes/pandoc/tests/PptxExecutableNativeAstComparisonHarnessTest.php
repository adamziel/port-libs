...
return [
    'validates checked-in real pandoc pptx executable evidence snapshot' => static function (TestRunner $t): void {
        ...
        $t->same(67, $snapshot['totalPptxCount']);
        $t->same(67, $snapshot['comparedPptxCount']);
        $t->same(67, $snapshot['localParsedCount']);
        $t->same(67, $snapshot['pandocParsedCount']);
        $t->same(67, $snapshot['nativeFixtureParsedCount']);
        $t->same(67, $snapshot['bothParsedCount']);
        $t->same(67, $snapshot['normalizedAstMatchCount']);
        $t->same(0, $snapshot['normalizedAstMismatchCount']);
        $t->same(67, $snapshot['pandocNativeFixtureComparedCount']);
        $t->same(67, $snapshot['pandocNativeFixtureMatchCount']);
        ...
        $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($snapshot, 67));
        ...
        $t->contains('--require-executable-parity=67', implode(' ', $snapshot['sourceCommand']));
    ...
    'cli selects checked-in fixtures for executable pptx parity with fake pandoc' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeFixtureAwareFakePandoc): void {
        ...
        try {
            ...
            $t->same(67, $decoded['comparedPptxCount']);
            $t->same(67, $decoded['localParsedCount']);
            $t->same(67, $decoded['pandocParsedCount']);
            $t->same(67, $decoded['nativeFixtureParsedCount']);
            $t->same(67, $decoded['bothParsedCount']);
            $t->same(67, $decoded['normalizedAstMatchCount']);
            $t->same(0, $decoded['normalizedAstMismatchCount']);
            $t->same(67, $decoded['pandocNativeFixtureComparedCount']);
            $t->same(67, $decoded['pandocNativeFixtureMatchCount']);
            $t->same(0, $decoded['pandocNativeFixtureMismatchCount']);
            $t->same(67, count($decoded['fixtureComparisons']));
            $t->same([], array_values(array_filter(
                $decoded['fixtureComparisons'],
                static fn (array $row): bool => ($row['status'] ?? null) !== 'matched'
            )));
            $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($decoded, 67));
