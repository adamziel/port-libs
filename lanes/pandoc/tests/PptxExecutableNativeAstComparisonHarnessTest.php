...
return [
    'validates checked-in real pandoc pptx executable evidence snapshot' => static function (TestRunner $t): void {
        ...
        $t->same(46, $snapshot['totalPptxCount']);
        $t->same(46, $snapshot['comparedPptxCount']);
        $t->same(46, $snapshot['localParsedCount']);
        $t->same(46, $snapshot['pandocParsedCount']);
        $t->same(46, $snapshot['nativeFixtureParsedCount']);
        $t->same(46, $snapshot['bothParsedCount']);
        $t->same(46, $snapshot['normalizedAstMatchCount']);
        $t->same(0, $snapshot['normalizedAstMismatchCount']);
        $t->same(46, $snapshot['pandocNativeFixtureComparedCount']);
        $t->same(46, $snapshot['pandocNativeFixtureMatchCount']);
        ...
        $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($snapshot, 46));
        ...
        $t->contains('--require-executable-parity=46', implode(' ', $snapshot['sourceCommand']));
    ...
    'cli selects checked-in fixtures for executable pptx parity with fake pandoc' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeFixtureAwareFakePandoc): void {
        ...
        try {
            ...
            $t->same(46, $decoded['comparedPptxCount']);
            $t->same(46, $decoded['localParsedCount']);
            $t->same(46, $decoded['pandocParsedCount']);
            $t->same(46, $decoded['nativeFixtureParsedCount']);
            $t->same(46, $decoded['bothParsedCount']);
            $t->same(46, $decoded['normalizedAstMatchCount']);
            $t->same(0, $decoded['normalizedAstMismatchCount']);
            $t->same(46, $decoded['pandocNativeFixtureComparedCount']);
            $t->same(46, $decoded['pandocNativeFixtureMatchCount']);
            $t->same(0, $decoded['pandocNativeFixtureMismatchCount']);
            $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($decoded, 46));
