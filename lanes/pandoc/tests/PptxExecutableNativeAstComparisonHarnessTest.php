...
return [
    'validates checked-in real pandoc pptx executable evidence snapshot' => static function (TestRunner $t): void {
        ...
        $t->same(82, $snapshot['totalPptxCount']);
        $t->same(82, $snapshot['comparedPptxCount']);
        $t->same(82, $snapshot['localParsedCount']);
        $t->same(82, $snapshot['pandocParsedCount']);
        $t->same(82, $snapshot['nativeFixtureParsedCount']);
        $t->same(82, $snapshot['bothParsedCount']);
        $t->same(82, $snapshot['normalizedAstMatchCount']);
        $t->same(0, $snapshot['normalizedAstMismatchCount']);
        $t->same(82, $snapshot['pandocNativeFixtureComparedCount']);
        $t->same(82, $snapshot['pandocNativeFixtureMatchCount']);
        ...
        $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($snapshot, 82));
        ...
        $t->contains('--require-executable-parity=82', implode(' ', $snapshot['sourceCommand']));
    ...
    'cli selects checked-in fixtures for executable pptx parity with fake pandoc' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeFixtureAwareFakePandoc): void {
        ...
        try {
            ...
            $t->same(82, $decoded['comparedPptxCount']);
            $t->same(82, $decoded['localParsedCount']);
            $t->same(82, $decoded['pandocParsedCount']);
            $t->same(82, $decoded['nativeFixtureParsedCount']);
            $t->same(82, $decoded['bothParsedCount']);
            $t->same(82, $decoded['normalizedAstMatchCount']);
            $t->same(0, $decoded['normalizedAstMismatchCount']);
            $t->same(82, $decoded['pandocNativeFixtureComparedCount']);
            $t->same(82, $decoded['pandocNativeFixtureMatchCount']);
            $t->same(0, $decoded['pandocNativeFixtureMismatchCount']);
            $t->same(82, count($decoded['fixtureComparisons']));
            $t->same([], array_values(array_filter(
                $decoded['fixtureComparisons'],
                static fn (array $row): bool => ($row['status'] ?? null) !== 'matched'
            )));
            $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($decoded, 82));
