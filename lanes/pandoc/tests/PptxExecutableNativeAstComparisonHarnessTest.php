...
return [
    'validates checked-in real pandoc pptx executable evidence snapshot' => static function (TestRunner $t): void {
        ...
        $t->same(70, $snapshot['totalPptxCount']);
        $t->same(70, $snapshot['comparedPptxCount']);
        $t->same(70, $snapshot['localParsedCount']);
        $t->same(70, $snapshot['pandocParsedCount']);
        $t->same(70, $snapshot['nativeFixtureParsedCount']);
        $t->same(70, $snapshot['bothParsedCount']);
        $t->same(70, $snapshot['normalizedAstMatchCount']);
        $t->same(0, $snapshot['normalizedAstMismatchCount']);
        $t->same(70, $snapshot['pandocNativeFixtureComparedCount']);
        $t->same(70, $snapshot['pandocNativeFixtureMatchCount']);
        ...
        $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($snapshot, 70));
        ...
        $t->contains('--require-executable-parity=70', implode(' ', $snapshot['sourceCommand']));
    ...
    'cli selects checked-in fixtures for executable pptx parity with fake pandoc' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeFixtureAwareFakePandoc): void {
        ...
        try {
            ...
            $t->same(70, $decoded['comparedPptxCount']);
            $t->same(70, $decoded['localParsedCount']);
            $t->same(70, $decoded['pandocParsedCount']);
            $t->same(70, $decoded['nativeFixtureParsedCount']);
            $t->same(70, $decoded['bothParsedCount']);
            $t->same(70, $decoded['normalizedAstMatchCount']);
            $t->same(0, $decoded['normalizedAstMismatchCount']);
            $t->same(70, $decoded['pandocNativeFixtureComparedCount']);
            $t->same(70, $decoded['pandocNativeFixtureMatchCount']);
            $t->same(0, $decoded['pandocNativeFixtureMismatchCount']);
            $t->same(70, count($decoded['fixtureComparisons']));
            $t->same([], array_values(array_filter(
                $decoded['fixtureComparisons'],
                static fn (array $row): bool => ($row['status'] ?? null) !== 'matched'
            )));
            $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($decoded, 70));
