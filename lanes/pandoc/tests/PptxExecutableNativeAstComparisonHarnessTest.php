...
return [
    'validates checked-in real pandoc pptx executable evidence snapshot' => static function (TestRunner $t): void {
        ...
        $t->same(77, $snapshot['totalPptxCount']);
        $t->same(77, $snapshot['comparedPptxCount']);
        $t->same(77, $snapshot['localParsedCount']);
        $t->same(77, $snapshot['pandocParsedCount']);
        $t->same(77, $snapshot['nativeFixtureParsedCount']);
        $t->same(77, $snapshot['bothParsedCount']);
        $t->same(77, $snapshot['normalizedAstMatchCount']);
        $t->same(0, $snapshot['normalizedAstMismatchCount']);
        $t->same(77, $snapshot['pandocNativeFixtureComparedCount']);
        $t->same(77, $snapshot['pandocNativeFixtureMatchCount']);
        ...
        $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($snapshot, 77));
        ...
        $t->contains('--require-executable-parity=77', implode(' ', $snapshot['sourceCommand']));
    ...
    'cli selects checked-in fixtures for executable pptx parity with fake pandoc' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeFixtureAwareFakePandoc): void {
        ...
        try {
            ...
            $t->same(77, $decoded['comparedPptxCount']);
            $t->same(77, $decoded['localParsedCount']);
            $t->same(77, $decoded['pandocParsedCount']);
            $t->same(77, $decoded['nativeFixtureParsedCount']);
            $t->same(77, $decoded['bothParsedCount']);
            $t->same(77, $decoded['normalizedAstMatchCount']);
            $t->same(0, $decoded['normalizedAstMismatchCount']);
            $t->same(77, $decoded['pandocNativeFixtureComparedCount']);
            $t->same(77, $decoded['pandocNativeFixtureMatchCount']);
            $t->same(0, $decoded['pandocNativeFixtureMismatchCount']);
            $t->same(77, count($decoded['fixtureComparisons']));
            $t->same([], array_values(array_filter(
                $decoded['fixtureComparisons'],
                static fn (array $row): bool => ($row['status'] ?? null) !== 'matched'
            )));
            $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($decoded, 77));
