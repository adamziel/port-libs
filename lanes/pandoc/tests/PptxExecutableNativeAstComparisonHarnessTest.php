...
return [
    'validates checked-in real pandoc pptx executable evidence snapshot' => static function (TestRunner $t): void {
        ...
        $t->same(['basic', 'break-tab-field', 'bullets', 'embedded-image', 'hyperlink-text', 'minimal', 'numbered-list', 'speaker-notes', 'two-slides'], $snapshot['fixtureStems']);
        $t->same(9, $snapshot['totalPptxCount']);
        $t->same(9, $snapshot['comparedPptxCount']);
        $t->same(9, $snapshot['localParsedCount']);
        $t->same(9, $snapshot['pandocParsedCount']);
        $t->same(9, $snapshot['nativeFixtureParsedCount']);
        $t->same(9, $snapshot['bothParsedCount']);
        $t->same(9, $snapshot['normalizedAstMatchCount']);
        $t->same(0, $snapshot['normalizedAstMismatchCount']);
        $t->same(9, $snapshot['pandocNativeFixtureComparedCount']);
        $t->same(9, $snapshot['pandocNativeFixtureMatchCount']);
        ...
        $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($snapshot, 9));
        $t->contains('--require-executable-parity=9', implode(' ', $snapshot['sourceCommand']));
        ...
        $t->true(in_array('that generated break-tab-field, bullets, embedded-image, hyperlink-text, minimal, numbered-list, speaker-notes, or two-slides fixtures are upstream Tests.Readers.Pptx fixtures', $snapshot['claimBoundaries']['doesNotAssert'], true));
    ...
    'cli selects checked-in fixtures for executable pptx parity with fake pandoc' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeFixtureAwareFakePandoc): void {
        ...
        try {
            ...
            $t->same(9, $decoded['comparedPptxCount']);
            $t->same(9, $decoded['localParsedCount']);
            $t->same(9, $decoded['pandocParsedCount']);
            $t->same(9, $decoded['nativeFixtureParsedCount']);
            $t->same(9, $decoded['bothParsedCount']);
            $t->same(9, $decoded['normalizedAstMatchCount']);
            $t->same(0, $decoded['normalizedAstMismatchCount']);
            $t->same(9, $decoded['pandocNativeFixtureComparedCount']);
            $t->same(9, $decoded['pandocNativeFixtureMatchCount']);
            $t->same(0, $decoded['pandocNativeFixtureMismatchCount']);
            $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($decoded, 9));
