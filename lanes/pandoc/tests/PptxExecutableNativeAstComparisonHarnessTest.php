...
return [
    'validates checked-in real pandoc pptx executable evidence snapshot' => static function (TestRunner $t): void {
        ...
        $t->same(63, $snapshot['totalPptxCount']);
        $t->same(63, $snapshot['comparedPptxCount']);
        $t->same(63, $snapshot['localParsedCount']);
        $t->same(63, $snapshot['pandocParsedCount']);
        $t->same(63, $snapshot['nativeFixtureParsedCount']);
        $t->same(63, $snapshot['bothParsedCount']);
        $t->same(63, $snapshot['normalizedAstMatchCount']);
        $t->same(0, $snapshot['normalizedAstMismatchCount']);
        $t->same(63, $snapshot['pandocNativeFixtureComparedCount']);
        $t->same(63, $snapshot['pandocNativeFixtureMatchCount']);
        ...
        $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($snapshot, 63));
        ...
        $t->contains('--require-executable-parity=63', implode(' ', $snapshot['sourceCommand']));
    ...
    'cli selects checked-in fixtures for executable pptx parity with fake pandoc' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeFixtureAwareFakePandoc): void {
        ...
        try {
            ...
            $t->same(63, $decoded['comparedPptxCount']);
            $t->same(63, $decoded['localParsedCount']);
            $t->same(63, $decoded['pandocParsedCount']);
            $t->same(63, $decoded['nativeFixtureParsedCount']);
            $t->same(63, $decoded['bothParsedCount']);
            $t->same(63, $decoded['normalizedAstMatchCount']);
            $t->same(0, $decoded['normalizedAstMismatchCount']);
            $t->same(63, $decoded['pandocNativeFixtureComparedCount']);
            $t->same(63, $decoded['pandocNativeFixtureMatchCount']);
            $t->same(0, $decoded['pandocNativeFixtureMismatchCount']);
            $t->same(63, count($decoded['fixtureComparisons']));
            $t->same([], array_values(array_filter(
                $decoded['fixtureComparisons'],
                static fn (array $row): bool => ($row['status'] ?? null) !== 'matched'
            )));
            $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($decoded, 63));
