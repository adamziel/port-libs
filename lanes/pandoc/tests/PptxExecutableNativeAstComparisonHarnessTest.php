...
return [
    'validates checked-in real pandoc pptx executable evidence snapshot' => static function (TestRunner $t): void {
        ...
        $t->same(68, $snapshot['totalPptxCount']);
        $t->same(68, $snapshot['comparedPptxCount']);
        $t->same(68, $snapshot['localParsedCount']);
        $t->same(68, $snapshot['pandocParsedCount']);
        $t->same(68, $snapshot['nativeFixtureParsedCount']);
        $t->same(68, $snapshot['bothParsedCount']);
        $t->same(68, $snapshot['normalizedAstMatchCount']);
        $t->same(0, $snapshot['normalizedAstMismatchCount']);
        $t->same(68, $snapshot['pandocNativeFixtureComparedCount']);
        $t->same(68, $snapshot['pandocNativeFixtureMatchCount']);
        ...
        $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($snapshot, 68));
        ...
        $t->contains('--require-executable-parity=68', implode(' ', $snapshot['sourceCommand']));
    ...
    'cli selects checked-in fixtures for executable pptx parity with fake pandoc' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeFixtureAwareFakePandoc): void {
        ...
        try {
            ...
            $t->same(68, $decoded['comparedPptxCount']);
            $t->same(68, $decoded['localParsedCount']);
            $t->same(68, $decoded['pandocParsedCount']);
            $t->same(68, $decoded['nativeFixtureParsedCount']);
            $t->same(68, $decoded['bothParsedCount']);
            $t->same(68, $decoded['normalizedAstMatchCount']);
            $t->same(0, $decoded['normalizedAstMismatchCount']);
            $t->same(68, $decoded['pandocNativeFixtureComparedCount']);
            $t->same(68, $decoded['pandocNativeFixtureMatchCount']);
            $t->same(0, $decoded['pandocNativeFixtureMismatchCount']);
            $t->same(68, count($decoded['fixtureComparisons']));
            $t->same([], array_values(array_filter(
                $decoded['fixtureComparisons'],
                static fn (array $row): bool => ($row['status'] ?? null) !== 'matched'
            )));
            $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($decoded, 68));
