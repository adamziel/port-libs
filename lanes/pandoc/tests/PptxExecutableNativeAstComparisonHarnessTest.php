...
return [
    'validates checked-in real pandoc pptx executable evidence snapshot' => static function (TestRunner $t): void {
        ...
        $t->same(25, $snapshot['totalPptxCount']);
        $t->same(25, $snapshot['comparedPptxCount']);
        $t->same(25, $snapshot['localParsedCount']);
        $t->same(25, $snapshot['pandocParsedCount']);
        $t->same(25, $snapshot['nativeFixtureParsedCount']);
        $t->same(25, $snapshot['bothParsedCount']);
        $t->same(25, $snapshot['normalizedAstMatchCount']);
        $t->same(0, $snapshot['normalizedAstMismatchCount']);
        $t->same(25, $snapshot['pandocNativeFixtureComparedCount']);
        $t->same(25, $snapshot['pandocNativeFixtureMatchCount']);
        ...
        $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($snapshot, 25));
        $t->contains('--require-executable-parity=25', implode(' ', $snapshot['sourceCommand']));
    ...
    'cli selects checked-in fixtures for executable pptx parity with fake pandoc' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeFixtureAwareFakePandoc): void {
        ...
        try {
            ...
            $t->same(25, $decoded['comparedPptxCount']);
            $t->same(25, $decoded['localParsedCount']);
            $t->same(25, $decoded['pandocParsedCount']);
            $t->same(25, $decoded['nativeFixtureParsedCount']);
            $t->same(25, $decoded['bothParsedCount']);
            $t->same(25, $decoded['normalizedAstMatchCount']);
            $t->same(0, $decoded['normalizedAstMismatchCount']);
            $t->same(25, $decoded['pandocNativeFixtureComparedCount']);
            $t->same(25, $decoded['pandocNativeFixtureMatchCount']);
            $t->same(0, $decoded['pandocNativeFixtureMismatchCount']);
            $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($decoded, 25));
