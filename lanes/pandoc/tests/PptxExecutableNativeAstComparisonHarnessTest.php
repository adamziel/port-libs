...
return [
    'validates checked-in real pandoc pptx executable evidence snapshot' => static function (TestRunner $t): void {
        ...
        $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($snapshot, 48));
        ...
        $t->contains('--require-executable-parity=48', implode(' ', $snapshot['sourceCommand']));
    ...
    'cli selects checked-in fixtures for executable pptx parity with fake pandoc' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeFixtureAwareFakePandoc): void {
        ...
        try {
            ...
            $t->same(48, $decoded['comparedPptxCount']);
            $t->same(48, $decoded['localParsedCount']);
            $t->same(48, $decoded['pandocParsedCount']);
            $t->same(48, $decoded['nativeFixtureParsedCount']);
            $t->same(48, $decoded['bothParsedCount']);
            $t->same(48, $decoded['normalizedAstMatchCount']);
            $t->same(0, $decoded['normalizedAstMismatchCount']);
            $t->same(48, $decoded['pandocNativeFixtureComparedCount']);
            $t->same(48, $decoded['pandocNativeFixtureMatchCount']);
            $t->same(0, $decoded['pandocNativeFixtureMismatchCount']);
            $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($decoded, 48));
