...
return [
    'validates checked-in real pandoc pptx executable evidence snapshot' => static function (TestRunner $t): void {
        ...
        $t->same(103, $snapshot['totalPptxCount']);
        $t->same(103, $snapshot['comparedPptxCount']);
        $t->same(103, $snapshot['localParsedCount']);
        $t->same(103, $snapshot['pandocParsedCount']);
        $t->same(103, $snapshot['nativeFixtureParsedCount']);
        $t->same(103, $snapshot['bothParsedCount']);
        $t->same(103, $snapshot['normalizedAstMatchCount']);
        $t->same(0, $snapshot['normalizedAstMismatchCount']);
        $t->same(103, $snapshot['pandocNativeFixtureComparedCount']);
        $t->same(103, $snapshot['pandocNativeFixtureMatchCount']);
        ...
        $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($snapshot, 103));
        ...
        $t->contains('--require-executable-parity=103', implode(' ', $snapshot['sourceCommand']));
    ...
    'cli selects checked-in fixtures for executable pptx parity with fake pandoc' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeFixtureAwareFakePandoc): void {
        ...
        try {
            ...
            $t->same(103, $decoded['comparedPptxCount']);
            $t->same(103, $decoded['localParsedCount']);
            $t->same(103, $decoded['pandocParsedCount']);
            $t->same(103, $decoded['nativeFixtureParsedCount']);
            $t->same(103, $decoded['bothParsedCount']);
            $t->same(103, $decoded['normalizedAstMatchCount']);
            $t->same(0, $decoded['normalizedAstMismatchCount']);
            $t->same(103, $decoded['pandocNativeFixtureComparedCount']);
            $t->same(103, $decoded['pandocNativeFixtureMatchCount']);
            $t->same(0, $decoded['pandocNativeFixtureMismatchCount']);
            $t->same(103, count($decoded['fixtureComparisons']));
            $t->same([], array_values(array_filter(
                $decoded['fixtureComparisons'],
                static fn (array $row): bool => ($row['status'] ?? null) !== 'matched'
            )));
            $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($decoded, 103));
