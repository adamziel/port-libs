...
return [
    'validates checked-in real pandoc pptx executable evidence snapshot' => static function (TestRunner $t): void {
        ...
        $t->same(91, $snapshot['totalPptxCount']);
        $t->same(91, $snapshot['comparedPptxCount']);
        $t->same(91, $snapshot['localParsedCount']);
        $t->same(91, $snapshot['pandocParsedCount']);
        $t->same(91, $snapshot['nativeFixtureParsedCount']);
        $t->same(91, $snapshot['bothParsedCount']);
        $t->same(91, $snapshot['normalizedAstMatchCount']);
        $t->same(0, $snapshot['normalizedAstMismatchCount']);
        $t->same(91, $snapshot['pandocNativeFixtureComparedCount']);
        $t->same(91, $snapshot['pandocNativeFixtureMatchCount']);
        ...
        $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($snapshot, 91));
        ...
        $t->contains('--require-executable-parity=91', implode(' ', $snapshot['sourceCommand']));
    ...
    'cli selects checked-in fixtures for executable pptx parity with fake pandoc' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeFixtureAwareFakePandoc): void {
        ...
        try {
            ...
            $t->same(91, $decoded['comparedPptxCount']);
            $t->same(91, $decoded['localParsedCount']);
            $t->same(91, $decoded['pandocParsedCount']);
            $t->same(91, $decoded['nativeFixtureParsedCount']);
            $t->same(91, $decoded['bothParsedCount']);
            $t->same(91, $decoded['normalizedAstMatchCount']);
            $t->same(0, $decoded['normalizedAstMismatchCount']);
            $t->same(91, $decoded['pandocNativeFixtureComparedCount']);
            $t->same(91, $decoded['pandocNativeFixtureMatchCount']);
            $t->same(0, $decoded['pandocNativeFixtureMismatchCount']);
            $t->same(91, count($decoded['fixtureComparisons']));
            $t->same([], array_values(array_filter(
                $decoded['fixtureComparisons'],
                static fn (array $row): bool => ($row['status'] ?? null) !== 'matched'
            )));
            $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($decoded, 91));
