...
return [
    'validates checked-in real pandoc pptx executable evidence snapshot' => static function (TestRunner $t): void {
        ...
        $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($snapshot, 41));
        $t->contains('--require-executable-parity=41', implode(' ', $snapshot['sourceCommand']));
    ...
    'cli selects checked-in fixtures for executable pptx parity with fake pandoc' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeFixtureAwareFakePandoc): void {
        ...
        try {
            ...
            $t->same(41, $decoded['comparedPptxCount']);
            $t->same(41, $decoded['localParsedCount']);
            $t->same(41, $decoded['pandocParsedCount']);
            $t->same(41, $decoded['nativeFixtureParsedCount']);
            $t->same(41, $decoded['bothParsedCount']);
            $t->same(41, $decoded['normalizedAstMatchCount']);
            $t->same(0, $decoded['normalizedAstMismatchCount']);
            $t->same(41, $decoded['pandocNativeFixtureComparedCount']);
            $t->same(41, $decoded['pandocNativeFixtureMatchCount']);
            $t->same(0, $decoded['pandocNativeFixtureMismatchCount']);
            $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($decoded, 41));
