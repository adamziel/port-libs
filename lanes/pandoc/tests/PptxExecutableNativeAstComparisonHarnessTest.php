...
return [
    'validates checked-in real pandoc pptx executable evidence snapshot' => static function (TestRunner $t): void {
        ...
        $t->same(88, $snapshot['totalPptxCount']);
        $t->same(88, $snapshot['comparedPptxCount']);
        $t->same(88, $snapshot['localParsedCount']);
        $t->same(88, $snapshot['pandocParsedCount']);
        $t->same(88, $snapshot['nativeFixtureParsedCount']);
        $t->same(88, $snapshot['bothParsedCount']);
        $t->same(88, $snapshot['normalizedAstMatchCount']);
        $t->same(0, $snapshot['normalizedAstMismatchCount']);
        $t->same(88, $snapshot['pandocNativeFixtureComparedCount']);
        $t->same(88, $snapshot['pandocNativeFixtureMatchCount']);
        ...
        $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($snapshot, 88));
        ...
        $t->contains('--require-executable-parity=88', implode(' ', $snapshot['sourceCommand']));
    ...
    'cli selects checked-in fixtures for executable pptx parity with fake pandoc' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeFixtureAwareFakePandoc): void {
        ...
        try {
            ...
            $t->same(88, $decoded['comparedPptxCount']);
            $t->same(88, $decoded['localParsedCount']);
            $t->same(88, $decoded['pandocParsedCount']);
            $t->same(88, $decoded['nativeFixtureParsedCount']);
            $t->same(88, $decoded['bothParsedCount']);
            $t->same(88, $decoded['normalizedAstMatchCount']);
            $t->same(0, $decoded['normalizedAstMismatchCount']);
            $t->same(88, $decoded['pandocNativeFixtureComparedCount']);
            $t->same(88, $decoded['pandocNativeFixtureMatchCount']);
            $t->same(0, $decoded['pandocNativeFixtureMismatchCount']);
            $t->same(88, count($decoded['fixtureComparisons']));
            $t->same([], array_values(array_filter(
                $decoded['fixtureComparisons'],
                static fn (array $row): bool => ($row['status'] ?? null) !== 'matched'
            )));
            $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($decoded, 88));
