...
return [
    'validates checked-in real pandoc pptx executable evidence snapshot' => static function (TestRunner $t): void {
        ...
        $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($snapshot, 74));
        ...
        $t->contains('--require-executable-parity=74', implode(' ', $snapshot['sourceCommand']));
    ...
    'cli selects checked-in fixtures for executable pptx parity with fake pandoc' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeFixtureAwareFakePandoc): void {
        ...
        try {
            ...
            $t->same(74, $decoded['comparedPptxCount']);
            $t->same(74, $decoded['localParsedCount']);
            $t->same(74, $decoded['pandocParsedCount']);
            $t->same(74, $decoded['nativeFixtureParsedCount']);
            $t->same(74, $decoded['bothParsedCount']);
            $t->same(74, $decoded['normalizedAstMatchCount']);
            $t->same(0, $decoded['normalizedAstMismatchCount']);
            $t->same(74, $decoded['pandocNativeFixtureComparedCount']);
            $t->same(74, $decoded['pandocNativeFixtureMatchCount']);
            $t->same(0, $decoded['pandocNativeFixtureMismatchCount']);
            $t->same(74, count($decoded['fixtureComparisons']));
            $t->same([], array_values(array_filter(
                $decoded['fixtureComparisons'],
                static fn (array $row): bool => ($row['status'] ?? null) !== 'matched'
            )));
            $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($decoded, 74));
