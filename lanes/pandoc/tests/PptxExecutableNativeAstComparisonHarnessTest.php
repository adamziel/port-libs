...
return [
    'validates checked-in real pandoc pptx executable evidence snapshot' => static function (TestRunner $t): void {
        ...
        $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($snapshot, 40));
        $t->contains('--require-executable-parity=40', implode(' ', $snapshot['sourceCommand']));
    ...
    'cli selects checked-in fixtures for executable pptx parity with fake pandoc' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeFixtureAwareFakePandoc): void {
        ...
        try {
            ...
            $t->same(40, $decoded['comparedPptxCount']);
            $t->same(40, $decoded['localParsedCount']);
            $t->same(40, $decoded['pandocParsedCount']);
            $t->same(40, $decoded['nativeFixtureParsedCount']);
            $t->same(40, $decoded['bothParsedCount']);
            $t->same(40, $decoded['normalizedAstMatchCount']);
            $t->same(0, $decoded['normalizedAstMismatchCount']);
            $t->same(40, $decoded['pandocNativeFixtureComparedCount']);
            $t->same(40, $decoded['pandocNativeFixtureMatchCount']);
            $t->same(0, $decoded['pandocNativeFixtureMismatchCount']);
            $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($decoded, 40));
