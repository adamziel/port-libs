...
return [
    'validates checked-in real pandoc pptx executable evidence snapshot' => static function (TestRunner $t): void {
        ...
        $t->same(95, $snapshot['totalPptxCount']);
        $t->same(95, $snapshot['comparedPptxCount']);
        $t->same(95, $snapshot['localParsedCount']);
        $t->same(95, $snapshot['pandocParsedCount']);
        $t->same(95, $snapshot['nativeFixtureParsedCount']);
        $t->same(95, $snapshot['bothParsedCount']);
        $t->same(95, $snapshot['normalizedAstMatchCount']);
        $t->same(0, $snapshot['normalizedAstMismatchCount']);
        $t->same(95, $snapshot['pandocNativeFixtureComparedCount']);
        $t->same(95, $snapshot['pandocNativeFixtureMatchCount']);
        ...
        $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($snapshot, 95));
        ...
        $t->contains('--require-executable-parity=95', implode(' ', $snapshot['sourceCommand']));
    ...
    'cli selects checked-in fixtures for executable pptx parity with fake pandoc' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeFixtureAwareFakePandoc): void {
        ...
        try {
            ...
            $t->same(95, $decoded['comparedPptxCount']);
            $t->same(95, $decoded['localParsedCount']);
            $t->same(95, $decoded['pandocParsedCount']);
            $t->same(95, $decoded['nativeFixtureParsedCount']);
            $t->same(95, $decoded['bothParsedCount']);
            $t->same(95, $decoded['normalizedAstMatchCount']);
            $t->same(0, $decoded['normalizedAstMismatchCount']);
            $t->same(95, $decoded['pandocNativeFixtureComparedCount']);
            $t->same(95, $decoded['pandocNativeFixtureMatchCount']);
            $t->same(0, $decoded['pandocNativeFixtureMismatchCount']);
            $t->same(95, count($decoded['fixtureComparisons']));
            $t->same([], array_values(array_filter(
                $decoded['fixtureComparisons'],
                static fn (array $row): bool => ($row['status'] ?? null) !== 'matched'
            )));
            $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($decoded, 95));
