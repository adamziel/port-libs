...
return [
    'validates checked-in real pandoc pptx executable evidence snapshot' => static function (TestRunner $t): void {
        ...
        $t->same(92, $snapshot['totalPptxCount']);
        $t->same(92, $snapshot['comparedPptxCount']);
        $t->same(92, $snapshot['localParsedCount']);
        $t->same(92, $snapshot['pandocParsedCount']);
        $t->same(92, $snapshot['nativeFixtureParsedCount']);
        $t->same(92, $snapshot['bothParsedCount']);
        $t->same(92, $snapshot['normalizedAstMatchCount']);
        $t->same(0, $snapshot['normalizedAstMismatchCount']);
        $t->same(92, $snapshot['pandocNativeFixtureComparedCount']);
        $t->same(92, $snapshot['pandocNativeFixtureMatchCount']);
        ...
        $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($snapshot, 92));
        ...
        $t->contains('--require-executable-parity=92', implode(' ', $snapshot['sourceCommand']));
    ...
    'cli selects checked-in fixtures for executable pptx parity with fake pandoc' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeFixtureAwareFakePandoc): void {
        ...
        try {
            ...
            $t->same(92, $decoded['comparedPptxCount']);
            $t->same(92, $decoded['localParsedCount']);
            $t->same(92, $decoded['pandocParsedCount']);
            $t->same(92, $decoded['nativeFixtureParsedCount']);
            $t->same(92, $decoded['bothParsedCount']);
            $t->same(92, $decoded['normalizedAstMatchCount']);
            $t->same(0, $decoded['normalizedAstMismatchCount']);
            $t->same(92, $decoded['pandocNativeFixtureComparedCount']);
            $t->same(92, $decoded['pandocNativeFixtureMatchCount']);
            $t->same(0, $decoded['pandocNativeFixtureMismatchCount']);
            $t->same(92, count($decoded['fixtureComparisons']));
            $t->same([], array_values(array_filter(
                $decoded['fixtureComparisons'],
                static fn (array $row): bool => ($row['status'] ?? null) !== 'matched'
            )));
            $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($decoded, 92));
