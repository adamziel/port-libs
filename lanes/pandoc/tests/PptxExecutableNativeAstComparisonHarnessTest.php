...
return [
    'validates checked-in real pandoc pptx executable evidence snapshot' => static function (TestRunner $t): void {
        ...
        $t->same(['basic', 'body-before-title', 'break-tab-field', 'bullets', 'bunone-wingdings', 'center-title-placeholder', 'chart-placeholder', 'comments-ignored', 'connector-skip', 'content-part-skip', 'direct-drawing-paragraphs', 'embedded-image', 'empty-paragraph-textbox', 'generated-table', 'grouped-shapes', 'hex-list-level', 'hidden-slide', 'hyperlink-text', 'inline-formatting', 'linked-image-skip', 'list-continuation', 'minimal', 'missing-relationship-skip', 'multi-paragraph-textbox', 'multiple-paragraph-properties', 'nested-list', 'no-title-fallback', 'numbered-list', 'paragraphless-textbox', 'percent-encoded-target', 'shape-order', 'slide-placeholders', 'smartart-hierarchy', 'speaker-notes', 'table-span-review', 'two-slides', 'wingdings-typeface-case'], $snapshot['fixtureStems']);
        $t->same(37, $snapshot['totalPptxCount']);
        $t->same(37, $snapshot['comparedPptxCount']);
        $t->same(37, $snapshot['localParsedCount']);
        $t->same(37, $snapshot['pandocParsedCount']);
        $t->same(37, $snapshot['nativeFixtureParsedCount']);
        $t->same(37, $snapshot['bothParsedCount']);
        $t->same(37, $snapshot['normalizedAstMatchCount']);
        $t->same(0, $snapshot['normalizedAstMismatchCount']);
        $t->same(37, $snapshot['pandocNativeFixtureComparedCount']);
        $t->same(37, $snapshot['pandocNativeFixtureMatchCount']);
        ...
        $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($snapshot, 37));
        $t->contains('--require-executable-parity=37', implode(' ', $snapshot['sourceCommand']));
        ...
        $t->true(in_array('that generated body-before-title, break-tab-field, bullets, bunone-wingdings, center-title-placeholder, chart-placeholder, comments-ignored, connector-skip, content-part-skip, direct-drawing-paragraphs, embedded-image, empty-paragraph-textbox, generated-table, grouped-shapes, hex-list-level, hidden-slide, hyperlink-text, inline-formatting, linked-image-skip, list-continuation, minimal, missing-relationship-skip, multi-paragraph-textbox, multiple-paragraph-properties, nested-list, no-title-fallback, numbered-list, paragraphless-textbox, percent-encoded-target, shape-order, slide-placeholders, smartart-hierarchy, speaker-notes, table-span-review, two-slides, wingdings-typeface-case fixtures are upstream Tests.Readers.Pptx fixtures', $snapshot['claimBoundaries']['doesNotAssert'], true));
    ...
    'cli selects checked-in fixtures for executable pptx parity with fake pandoc' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeFixtureAwareFakePandoc): void {
        ...
        try {
            ...
            $t->same(37, $decoded['comparedPptxCount']);
            $t->same(37, $decoded['localParsedCount']);
            $t->same(37, $decoded['pandocParsedCount']);
            $t->same(37, $decoded['nativeFixtureParsedCount']);
            $t->same(37, $decoded['bothParsedCount']);
            $t->same(37, $decoded['normalizedAstMatchCount']);
            $t->same(0, $decoded['normalizedAstMismatchCount']);
            $t->same(37, $decoded['pandocNativeFixtureComparedCount']);
            $t->same(37, $decoded['pandocNativeFixtureMatchCount']);
            $t->same(0, $decoded['pandocNativeFixtureMismatchCount']);
            $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($decoded, 37));
