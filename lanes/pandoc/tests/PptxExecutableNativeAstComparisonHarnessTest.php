...
return [
    'validates checked-in real pandoc pptx executable evidence snapshot' => static function (TestRunner $t): void {
        ...
        $t->same(36, $snapshot['totalPptxCount']);
        $t->same(36, $snapshot['comparedPptxCount']);
        $t->same(36, $snapshot['localParsedCount']);
        $t->same(36, $snapshot['pandocParsedCount']);
        $t->same(36, $snapshot['nativeFixtureParsedCount']);
        $t->same(36, $snapshot['bothParsedCount']);
        $t->same(36, $snapshot['normalizedAstMatchCount']);
        $t->same(0, $snapshot['normalizedAstMismatchCount']);
        $t->same(36, $snapshot['pandocNativeFixtureComparedCount']);
        $t->same(36, $snapshot['pandocNativeFixtureMatchCount']);
        ...
        $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($snapshot, 36));
        $t->contains('--require-executable-parity=36', implode(' ', $snapshot['sourceCommand']));
    ...
    'cli selects checked-in fixtures for executable pptx parity with fake pandoc' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeFixtureAwareFakePandoc): void {
        ...
        try {
            ...
            $t->same(36, $decoded['comparedPptxCount']);
            $t->same(36, $decoded['localParsedCount']);
            $t->same(36, $decoded['pandocParsedCount']);
            $t->same(36, $decoded['nativeFixtureParsedCount']);
            $t->same(36, $decoded['bothParsedCount']);
            $t->same(36, $decoded['normalizedAstMatchCount']);
            $t->same(0, $decoded['normalizedAstMismatchCount']);
            $t->same(36, $decoded['pandocNativeFixtureComparedCount']);
            $t->same(36, $decoded['pandocNativeFixtureMatchCount']);
            $t->same(0, $decoded['pandocNativeFixtureMismatchCount']);
            $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($decoded, 36));
