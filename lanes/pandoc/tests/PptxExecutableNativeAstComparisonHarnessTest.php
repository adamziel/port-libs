...
return [
    'validates checked-in real pandoc pptx executable evidence snapshot' => static function (TestRunner $t): void {
        ...
        $t->same(55, $snapshot['totalPptxCount']);
        $t->same(55, $snapshot['comparedPptxCount']);
        $t->same(55, $snapshot['localParsedCount']);
        $t->same(55, $snapshot['pandocParsedCount']);
        $t->same(55, $snapshot['nativeFixtureParsedCount']);
        $t->same(55, $snapshot['bothParsedCount']);
        $t->same(55, $snapshot['normalizedAstMatchCount']);
        $t->same(0, $snapshot['normalizedAstMismatchCount']);
        $t->same(55, $snapshot['pandocNativeFixtureComparedCount']);
        $t->same(55, $snapshot['pandocNativeFixtureMatchCount']);
        ...
        $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($snapshot, 55));
        ...
        $t->contains('--require-executable-parity=55', implode(' ', $snapshot['sourceCommand']));
    ...
    'cli selects checked-in fixtures for executable pptx parity with fake pandoc' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeFixtureAwareFakePandoc): void {
        ...
        try {
            ...
            $t->same(55, $decoded['comparedPptxCount']);
            $t->same(55, $decoded['localParsedCount']);
            $t->same(55, $decoded['pandocParsedCount']);
            $t->same(55, $decoded['nativeFixtureParsedCount']);
            $t->same(55, $decoded['bothParsedCount']);
            $t->same(55, $decoded['normalizedAstMatchCount']);
            $t->same(0, $decoded['normalizedAstMismatchCount']);
            $t->same(55, $decoded['pandocNativeFixtureComparedCount']);
            $t->same(55, $decoded['pandocNativeFixtureMatchCount']);
            $t->same(0, $decoded['pandocNativeFixtureMismatchCount']);
            $t->same(55, count($decoded['fixtureComparisons']));
            $t->same([], array_values(array_filter(
                $decoded['fixtureComparisons'],
                static fn (array $row): bool => ($row['status'] ?? null) !== 'matched'
            )));
            $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($decoded, 55));
