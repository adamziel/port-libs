...
return [
    'validates checked-in real pandoc pptx executable evidence snapshot' => static function (TestRunner $t): void {
        ...
        $t->same(35, $snapshot['totalPptxCount']);
        $t->same(35, $snapshot['comparedPptxCount']);
        $t->same(35, $snapshot['localParsedCount']);
        $t->same(35, $snapshot['pandocParsedCount']);
        $t->same(35, $snapshot['nativeFixtureParsedCount']);
        $t->same(35, $snapshot['bothParsedCount']);
        $t->same(35, $snapshot['normalizedAstMatchCount']);
        $t->same(0, $snapshot['normalizedAstMismatchCount']);
        $t->same(35, $snapshot['pandocNativeFixtureComparedCount']);
        $t->same(35, $snapshot['pandocNativeFixtureMatchCount']);
        ...
        $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($snapshot, 35));
        $t->contains('--require-executable-parity=35', implode(' ', $snapshot['sourceCommand']));
    ...
    'cli selects checked-in fixtures for executable pptx parity with fake pandoc' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeFixtureAwareFakePandoc): void {
        ...
        try {
            ...
            $t->same(35, $decoded['comparedPptxCount']);
            $t->same(35, $decoded['localParsedCount']);
            $t->same(35, $decoded['pandocParsedCount']);
            $t->same(35, $decoded['nativeFixtureParsedCount']);
            $t->same(35, $decoded['bothParsedCount']);
            $t->same(35, $decoded['normalizedAstMatchCount']);
            $t->same(0, $decoded['normalizedAstMismatchCount']);
            $t->same(35, $decoded['pandocNativeFixtureComparedCount']);
            $t->same(35, $decoded['pandocNativeFixtureMatchCount']);
            $t->same(0, $decoded['pandocNativeFixtureMismatchCount']);
            $t->same(true, PptxExecutableNativeAstComparisonHarness::hasRequiredExecutableParity($decoded, 35));
