...
return [
    'reports skipped epub reader evidence when upstream root is absent' => static function (TestRunner $t) use ($makeTempDir, $removeTree): void {
        ...
        try {
            ...
            $t->contains('Native/package parity: package=0/39 nativeAst=0/39 status=not-evaluated-source-directory-unavailable', $text);
            $t->contains('Executable/native parity: localPandoc=0/39 checkedNative=0/39 status=not-evaluated-not-requested version=not-evaluated', $text);
    ...
    'validates checked-in current epub reader evidence through fixture base' => static function (TestRunner $t) use ($repoRoot, $checkedInFixtureRoot, $expectedCurrentReaderStaticSignatureSha256, $expectedReferencedFixtureIdentity): void {
        ...
        $t->same(39, $nativePackageParity['requiredEpubCount']);
        $t->same(39, $nativePackageParity['requiredPairCount']);
        $t->same(39, $nativePackageParity['totalEpubCount']);
        $t->same(39, $nativePackageParity['comparedEpubCount']);
        $t->same(39, $nativePackageParity['packageParsedCount']);
        $t->same(39, $nativePackageParity['readerParsedCount']);
        ...
        $t->same(39, $nativePackageParity['totalPairCount']);
        $t->same(39, $nativePackageParity['comparedPairCount']);
        $t->same(39, $nativePackageParity['bothParsedCount']);
        $t->same(0, $nativePackageParity['astParseFailureCount']);
        $t->same(0, $nativePackageParity['nativeParseFailureCount']);
        $t->same(39, $nativePackageParity['normalizedAstMatchCount']);
        ...
        $t->contains('Native/package parity: package=39/39 nativeAst=39/39 status=normalized-ast-equality-observed-not-runner-parity', $text);
        $t->same('not-evaluated', $report['executableNativeAstParity']['status']);
        $t->same('not-requested', $report['executableNativeAstParity']['reason']);
        $t->same(false, EpubUpstreamReaderEvidence::hasRequiredExecutableNativeAstParity($report));
        $t->same(true, EpubUpstreamReaderEvidence::hasNoValidationIssues($report));
    ...
    'validates checked-in current epub executable native ast parity with supplied pandoc' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $repoRoot, $checkedInFixtureRoot, $writeFakePandoc): void {
        ...
        try {
            ...
            $t->same(39, $executableParity['requiredEpubCount']);
            $t->same(39, $executableParity['totalEpubCount']);
            $t->same(39, $executableParity['comparedEpubCount']);
            $t->same(39, $executableParity['localParsedCount']);
            $t->same(39, $executableParity['pandocParsedCount']);
            $t->same(39, $executableParity['nativeFixtureParsedCount']);
            $t->same(39, $executableParity['bothParsedCount']);
            $t->same(0, $executableParity['parseFailureCount']);
            $t->same(39, $executableParity['normalizedAstMatchCount']);
            $t->same(0, $executableParity['normalizedAstMismatchCount']);
            $t->same(39, $executableParity['pandocNativeFixtureComparedCount']);
            $t->same(39, $executableParity['pandocNativeFixtureMatchCount']);
            $t->same(0, $executableParity['pandocNativeFixtureMismatchCount']);
            $t->same(39, $executableParity['pandocNativeFixtureByteComparedCount']);
            $t->same(39, $executableParity['pandocNativeFixtureByteMatchCount']);
            ...
            $t->contains('Executable/native parity: localPandoc=39/39 checkedNative=39/39 status=normalized-ast-equality-observed-against-pandoc-executable version=pandoc fake 3.10', $text);
...
HS);
    ...
    'cli gates checked-in current epub reader evidence through checked-in fixtures mode' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $repoRoot, $checkedInFixtureRoot, $expectedCurrentReaderStaticSignatureSha256, $expectedReferencedFixtureIdentity, $writeFakePandoc): void {
        ...
        try {
            ...
            $t->same(39, $decoded['nativeAstPackageParity']['normalizedAstMatchCount']);
            ...
            $t->same(39, $decoded['executableNativeAstParity']['normalizedAstMatchCount']);
            $t->same(39, $decoded['executableNativeAstParity']['pandocNativeFixtureMatchCount']);
