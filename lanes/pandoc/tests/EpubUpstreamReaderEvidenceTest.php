...
return [
    'reports skipped epub reader evidence when upstream root is absent' => static function (TestRunner $t) use ($makeTempDir, $removeTree): void {
        ...
        try {
            ...
            $t->contains('Native/package parity: package=0/72 nativeAst=0/72 status=not-evaluated-source-directory-unavailable', $text);
            $t->contains('Executable/native parity: localPandoc=0/72 checkedNative=0/72 status=not-evaluated-not-requested version=not-evaluated', $text);
    ...
    'validates checked-in current epub reader evidence through fixture base' => static function (TestRunner $t) use ($repoRoot, $checkedInFixtureRoot, $expectedCurrentReaderStaticSignatureSha256, $expectedReferencedFixtureIdentity): void {
        ...
        $t->same(72, $nativePackageParity['requiredEpubCount']);
        $t->same(72, $nativePackageParity['requiredPairCount']);
        $t->same(72, $nativePackageParity['totalEpubCount']);
        $t->same(72, $nativePackageParity['comparedEpubCount']);
        $t->same(72, $nativePackageParity['packageParsedCount']);
        $t->same(72, $nativePackageParity['readerParsedCount']);
        ...
        $t->same(72, $nativePackageParity['totalPairCount']);
        $t->same(72, $nativePackageParity['comparedPairCount']);
        $t->same(72, $nativePackageParity['bothParsedCount']);
        $t->same(0, $nativePackageParity['astParseFailureCount']);
        $t->same(0, $nativePackageParity['nativeParseFailureCount']);
        $t->same(72, $nativePackageParity['normalizedAstMatchCount']);
        ...
        $t->contains('Native/package parity: package=72/72 nativeAst=72/72 status=normalized-ast-equality-observed-not-runner-parity', $text);
        $t->same('not-evaluated', $report['executableNativeAstParity']['status']);
        $t->same('not-requested', $report['executableNativeAstParity']['reason']);
        $t->same(false, EpubUpstreamReaderEvidence::hasRequiredExecutableNativeAstParity($report));
        $t->same(true, EpubUpstreamReaderEvidence::hasNoValidationIssues($report));
    ...
    'validates checked-in current epub executable native ast parity with supplied pandoc' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $repoRoot, $checkedInFixtureRoot, $writeFakePandoc): void {
        ...
        try {
            ...
            $t->same(72, $executableParity['requiredEpubCount']);
            $t->same(72, $executableParity['totalEpubCount']);
            $t->same(72, $executableParity['comparedEpubCount']);
            $t->same(72, $executableParity['localParsedCount']);
            $t->same(72, $executableParity['pandocParsedCount']);
            $t->same(72, $executableParity['nativeFixtureParsedCount']);
            $t->same(72, $executableParity['bothParsedCount']);
            $t->same(0, $executableParity['parseFailureCount']);
            $t->same(72, $executableParity['normalizedAstMatchCount']);
            $t->same(0, $executableParity['normalizedAstMismatchCount']);
            $t->same(72, $executableParity['pandocNativeFixtureComparedCount']);
            $t->same(72, $executableParity['pandocNativeFixtureMatchCount']);
            $t->same(0, $executableParity['pandocNativeFixtureMismatchCount']);
            $t->same(72, $executableParity['pandocNativeFixtureByteComparedCount']);
            $t->same(72, $executableParity['pandocNativeFixtureByteMatchCount']);
            ...
            $t->contains('Executable/native parity: localPandoc=72/72 checkedNative=72/72 status=normalized-ast-equality-observed-against-pandoc-executable version=pandoc fake 3.10', $text);
...
HS);
    ...
    'cli gates checked-in current epub reader evidence through checked-in fixtures mode' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $repoRoot, $checkedInFixtureRoot, $expectedCurrentReaderStaticSignatureSha256, $expectedReferencedFixtureIdentity, $writeFakePandoc): void {
        ...
        try {
            ...
            $t->same(72, $decoded['nativeAstPackageParity']['normalizedAstMatchCount']);
            ...
            $t->same(72, $decoded['executableNativeAstParity']['normalizedAstMatchCount']);
            $t->same(72, $decoded['executableNativeAstParity']['pandocNativeFixtureMatchCount']);
