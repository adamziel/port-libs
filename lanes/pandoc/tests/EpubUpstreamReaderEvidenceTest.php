...
return [
    'reports skipped epub reader evidence when upstream root is absent' => static function (TestRunner $t) use ($makeTempDir, $removeTree): void {
        ...
        try {
            ...
            $t->contains('Native/package parity: package=0/46 nativeAst=0/46 status=not-evaluated-source-directory-unavailable', $text);
            $t->contains('Executable/native parity: localPandoc=0/46 checkedNative=0/46 status=not-evaluated-not-requested version=not-evaluated', $text);
    ...
    'validates checked-in current epub reader evidence through fixture base' => static function (TestRunner $t) use ($repoRoot, $checkedInFixtureRoot, $expectedCurrentReaderStaticSignatureSha256, $expectedReferencedFixtureIdentity): void {
        ...
        $t->same(46, $nativePackageParity['requiredEpubCount']);
        $t->same(46, $nativePackageParity['requiredPairCount']);
        $t->same(46, $nativePackageParity['totalEpubCount']);
        $t->same(46, $nativePackageParity['comparedEpubCount']);
        $t->same(46, $nativePackageParity['packageParsedCount']);
        $t->same(46, $nativePackageParity['readerParsedCount']);
        ...
        $t->same(46, $nativePackageParity['totalPairCount']);
        $t->same(46, $nativePackageParity['comparedPairCount']);
        $t->same(46, $nativePackageParity['bothParsedCount']);
        $t->same(0, $nativePackageParity['astParseFailureCount']);
        $t->same(0, $nativePackageParity['nativeParseFailureCount']);
        $t->same(46, $nativePackageParity['normalizedAstMatchCount']);
        ...
        $t->contains('Native/package parity: package=46/46 nativeAst=46/46 status=normalized-ast-equality-observed-not-runner-parity', $text);
        $t->same('not-evaluated', $report['executableNativeAstParity']['status']);
        $t->same('not-requested', $report['executableNativeAstParity']['reason']);
        $t->same(false, EpubUpstreamReaderEvidence::hasRequiredExecutableNativeAstParity($report));
        $t->same(true, EpubUpstreamReaderEvidence::hasNoValidationIssues($report));
    ...
    'validates checked-in current epub executable native ast parity with supplied pandoc' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $repoRoot, $checkedInFixtureRoot, $writeFakePandoc): void {
        ...
        try {
            ...
            $t->same(46, $executableParity['requiredEpubCount']);
            $t->same(46, $executableParity['totalEpubCount']);
            $t->same(46, $executableParity['comparedEpubCount']);
            $t->same(46, $executableParity['localParsedCount']);
            $t->same(46, $executableParity['pandocParsedCount']);
            $t->same(46, $executableParity['nativeFixtureParsedCount']);
            $t->same(46, $executableParity['bothParsedCount']);
            $t->same(0, $executableParity['parseFailureCount']);
            $t->same(46, $executableParity['normalizedAstMatchCount']);
            $t->same(0, $executableParity['normalizedAstMismatchCount']);
            $t->same(46, $executableParity['pandocNativeFixtureComparedCount']);
            $t->same(46, $executableParity['pandocNativeFixtureMatchCount']);
            $t->same(0, $executableParity['pandocNativeFixtureMismatchCount']);
            $t->same(46, $executableParity['pandocNativeFixtureByteComparedCount']);
            $t->same(46, $executableParity['pandocNativeFixtureByteMatchCount']);
            ...
            $t->contains('Executable/native parity: localPandoc=46/46 checkedNative=46/46 status=normalized-ast-equality-observed-against-pandoc-executable version=pandoc fake 3.10', $text);
...
HS);
    ...
    'cli gates checked-in current epub reader evidence through checked-in fixtures mode' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $repoRoot, $checkedInFixtureRoot, $expectedCurrentReaderStaticSignatureSha256, $expectedReferencedFixtureIdentity, $writeFakePandoc): void {
        ...
        try {
            ...
            $t->same(46, $decoded['nativeAstPackageParity']['normalizedAstMatchCount']);
            ...
            $t->same(46, $decoded['executableNativeAstParity']['normalizedAstMatchCount']);
            $t->same(46, $decoded['executableNativeAstParity']['pandocNativeFixtureMatchCount']);
