...
return [
    'reports skipped epub reader evidence when upstream root is absent' => static function (TestRunner $t) use ($makeTempDir, $removeTree): void {
        ...
        try {
            ...
            $t->contains('Native/package parity: package=0/45 nativeAst=0/45 status=not-evaluated-source-directory-unavailable', $text);
            $t->contains('Executable/native parity: localPandoc=0/45 checkedNative=0/45 status=not-evaluated-not-requested version=not-evaluated', $text);
    ...
    'validates checked-in current epub reader evidence through fixture base' => static function (TestRunner $t) use ($repoRoot, $checkedInFixtureRoot, $expectedCurrentReaderStaticSignatureSha256, $expectedReferencedFixtureIdentity): void {
        ...
        $t->same(45, $nativePackageParity['requiredEpubCount']);
        $t->same(45, $nativePackageParity['requiredPairCount']);
        $t->same(45, $nativePackageParity['totalEpubCount']);
        $t->same(45, $nativePackageParity['comparedEpubCount']);
        $t->same(45, $nativePackageParity['packageParsedCount']);
        $t->same(45, $nativePackageParity['readerParsedCount']);
        ...
        $t->same(45, $nativePackageParity['totalPairCount']);
        $t->same(45, $nativePackageParity['comparedPairCount']);
        $t->same(45, $nativePackageParity['bothParsedCount']);
        $t->same(0, $nativePackageParity['astParseFailureCount']);
        $t->same(0, $nativePackageParity['nativeParseFailureCount']);
        $t->same(45, $nativePackageParity['normalizedAstMatchCount']);
        ...
        $t->contains('Native/package parity: package=45/45 nativeAst=45/45 status=normalized-ast-equality-observed-not-runner-parity', $text);
        $t->same('not-evaluated', $report['executableNativeAstParity']['status']);
        $t->same('not-requested', $report['executableNativeAstParity']['reason']);
        $t->same(false, EpubUpstreamReaderEvidence::hasRequiredExecutableNativeAstParity($report));
        $t->same(true, EpubUpstreamReaderEvidence::hasNoValidationIssues($report));
    ...
    'validates checked-in current epub executable native ast parity with supplied pandoc' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $repoRoot, $checkedInFixtureRoot, $writeFakePandoc): void {
        ...
        try {
            ...
            $t->same(45, $executableParity['requiredEpubCount']);
            $t->same(45, $executableParity['totalEpubCount']);
            $t->same(45, $executableParity['comparedEpubCount']);
            ...
            $t->same(45, $executableParity['pandocNativeFixtureByteComparedCount']);
            $t->same(45, $executableParity['pandocNativeFixtureByteMatchCount']);
            ...
            $t->contains('Executable/native parity: localPandoc=45/45 checkedNative=45/45 status=normalized-ast-equality-observed-against-pandoc-executable version=pandoc fake 3.10', $text);
...
HS);
    ...
    'cli gates checked-in current epub reader evidence through checked-in fixtures mode' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $repoRoot, $checkedInFixtureRoot, $expectedCurrentReaderStaticSignatureSha256, $expectedReferencedFixtureIdentity, $writeFakePandoc): void {
        ...
        try {
            ...
            $t->same(45, $decoded['nativeAstPackageParity']['normalizedAstMatchCount']);
            ...
            $t->same(45, $decoded['executableNativeAstParity']['normalizedAstMatchCount']);
            $t->same(45, $decoded['executableNativeAstParity']['pandocNativeFixtureMatchCount']);
