...
$writeRunnerTranscripts = static function (string $root, array $paths, string $label = 'epub', array $testNames = []) use ($writeFile): array {
    $records = [];
...
return [
    'reports skipped epub reader evidence when upstream root is absent' => static function (TestRunner $t) use ($makeTempDir, $removeTree): void {
        ...
        try {
            ...
            $t->contains('Native/package parity: package=0/74 nativeAst=0/74 status=not-evaluated-source-directory-unavailable', $text);
            $t->contains('Executable/native parity: localPandoc=0/74 checkedNative=0/74 status=not-evaluated-not-requested version=not-evaluated', $text);
    ...
    'validates checked-in current epub reader evidence through fixture base' => static function (TestRunner $t) use ($repoRoot, $checkedInFixtureRoot, $expectedCurrentReaderStaticSignatureSha256, $expectedReferencedFixtureIdentity): void {
        ...
        $t->same([
            ...
            'epub/inline-abbr-subscript-superscript.epub',
            'epub/language-french-metadata.epub',
            'epub/main-section-spine.epub',
            'epub/manifest-fallback-chain.epub',
            'epub/manifest-fallback-style.epub',
            'epub/manifest-href-encoding.epub',
            'epub/mathml-spine.epub',
            'epub/measurement-inline-spine.epub',
            'epub/media-manifest-mix.epub',
            'epub/media-overlay-invalid-clips.epub',
            'epub/media-overlay-package.epub',
            'epub/metadata-link-page-list-image.epub',
            'epub/metadata-link-vocab-diagnostics.epub',
            ...
            'epub/raw-media-source-spine.epub',
            ...
            'epub/xhtml-address-spine.epub',
            'epub/xhtml-del-edit-mark-spine.epub',
            'epub/xhtml-details-summary-spine.epub',
            'epub/xhtml-kbd-samp-var-spine.epub',
        ...
        $t->same(74, $nativePackageParity['requiredEpubCount']);
        $t->same(74, $nativePackageParity['requiredPairCount']);
        $t->same(74, $nativePackageParity['totalEpubCount']);
        $t->same(74, $nativePackageParity['comparedEpubCount']);
        $t->same(74, $nativePackageParity['packageParsedCount']);
        $t->same(74, $nativePackageParity['readerParsedCount']);
        ...
        $t->same(74, $nativePackageParity['totalPairCount']);
        $t->same(74, $nativePackageParity['comparedPairCount']);
        $t->same(74, $nativePackageParity['bothParsedCount']);
        $t->same(0, $nativePackageParity['astParseFailureCount']);
        $t->same(0, $nativePackageParity['nativeParseFailureCount']);
        $t->same(74, $nativePackageParity['normalizedAstMatchCount']);
        ...
        $t->contains('Native/package parity: package=74/74 nativeAst=74/74 status=normalized-ast-equality-observed-not-runner-parity', $text);
        $t->same('not-evaluated', $report['executableNativeAstParity']['status']);
        $t->same('not-requested', $report['executableNativeAstParity']['reason']);
        $t->same(false, EpubUpstreamReaderEvidence::hasRequiredExecutableNativeAstParity($report));
        $t->same(true, EpubUpstreamReaderEvidence::hasNoValidationIssues($report));
    ...
    'validates checked-in current epub executable native ast parity with supplied pandoc' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $repoRoot, $checkedInFixtureRoot, $writeFakePandoc): void {
        ...
        try {
            ...
            $t->same(74, $executableParity['requiredEpubCount']);
            $t->same(74, $executableParity['totalEpubCount']);
            $t->same(74, $executableParity['comparedEpubCount']);
            $t->same(74, $executableParity['localParsedCount']);
            $t->same(74, $executableParity['pandocParsedCount']);
            $t->same(74, $executableParity['nativeFixtureParsedCount']);
            $t->same(74, $executableParity['bothParsedCount']);
            $t->same(0, $executableParity['parseFailureCount']);
            $t->same(74, $executableParity['normalizedAstMatchCount']);
            $t->same(0, $executableParity['normalizedAstMismatchCount']);
            $t->same(74, $executableParity['pandocNativeFixtureComparedCount']);
            $t->same(74, $executableParity['pandocNativeFixtureMatchCount']);
            $t->same(0, $executableParity['pandocNativeFixtureMismatchCount']);
            $t->same(74, $executableParity['pandocNativeFixtureByteComparedCount']);
            $t->same(74, $executableParity['pandocNativeFixtureByteMatchCount']);
            ...
            $t->contains('Executable/native parity: localPandoc=74/74 checkedNative=74/74 status=normalized-ast-equality-observed-against-pandoc-executable version=pandoc fake 3.10', $text);
...
HS);
    ...
    'cli gates checked-in current epub reader evidence through checked-in fixtures mode' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $repoRoot, $checkedInFixtureRoot, $expectedCurrentReaderStaticSignatureSha256, $expectedReferencedFixtureIdentity, $writeFakePandoc): void {
        ...
        try {
            ...
            $t->same(74, $decoded['nativeAstPackageParity']['normalizedAstMatchCount']);
            ...
            $t->same(74, $decoded['executableNativeAstParity']['normalizedAstMatchCount']);
            $t->same(74, $decoded['executableNativeAstParity']['pandocNativeFixtureMatchCount']);
    ...
    'generic reader runner artifact tool writes and validates epub result artifact' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $writeRunnerTranscripts, $repoRoot): void {
        ...
        try {
            ...
            $writeRunnerTranscripts($root, $runnerPlan['requiredTranscripts'], 'epub', $testNames);
