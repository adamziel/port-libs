...
return [
    'reports skipped epub reader evidence when upstream root is absent' => static function (TestRunner $t) use ($makeTempDir, $removeTree): void {
        ...
        try {
            ...
            $t->contains('Native/package parity: package=0/50 nativeAst=0/50 status=not-evaluated-source-directory-unavailable', $text);
            $t->contains('Executable/native parity: localPandoc=0/50 checkedNative=0/50 status=not-evaluated-not-requested version=not-evaluated', $text);
    ...
    'validates checked-in current epub reader evidence through fixture base' => static function (TestRunner $t) use ($repoRoot, $checkedInFixtureRoot, $expectedCurrentReaderStaticSignatureSha256, $expectedReferencedFixtureIdentity): void {
        ...
        $t->same(50, $nativePackageParity['requiredEpubCount']);
        $t->same(50, $nativePackageParity['requiredPairCount']);
        $t->same(50, $nativePackageParity['totalEpubCount']);
        $t->same(50, $nativePackageParity['comparedEpubCount']);
        $t->same(50, $nativePackageParity['packageParsedCount']);
        $t->same(50, $nativePackageParity['readerParsedCount']);
        ...
        $t->same(50, $nativePackageParity['totalPairCount']);
        $t->same(50, $nativePackageParity['comparedPairCount']);
        $t->same(50, $nativePackageParity['bothParsedCount']);
        $t->same(0, $nativePackageParity['astParseFailureCount']);
        $t->same(0, $nativePackageParity['nativeParseFailureCount']);
        $t->same(50, $nativePackageParity['normalizedAstMatchCount']);
        ...
        $t->contains('Native/package parity: package=50/50 nativeAst=50/50 status=normalized-ast-equality-observed-not-runner-parity', $text);
        $t->same('not-evaluated', $report['executableNativeAstParity']['status']);
        $t->same('not-requested', $report['executableNativeAstParity']['reason']);
        $t->same(false, EpubUpstreamReaderEvidence::hasRequiredExecutableNativeAstParity($report));
        $t->same(true, EpubUpstreamReaderEvidence::hasNoValidationIssues($report));
    ...
    'validates checked-in current epub executable native ast parity with supplied pandoc' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $repoRoot, $checkedInFixtureRoot, $writeFakePandoc): void {
        ...
        try {
            ...
            $t->same(50, $executableParity['requiredEpubCount']);
            $t->same(50, $executableParity['totalEpubCount']);
            $t->same(50, $executableParity['comparedEpubCount']);
            $t->same(50, $executableParity['localParsedCount']);
            $t->same(50, $executableParity['pandocParsedCount']);
            $t->same(50, $executableParity['nativeFixtureParsedCount']);
            $t->same(50, $executableParity['bothParsedCount']);
            $t->same(0, $executableParity['parseFailureCount']);
            $t->same(50, $executableParity['normalizedAstMatchCount']);
            $t->same(0, $executableParity['normalizedAstMismatchCount']);
            $t->same(50, $executableParity['pandocNativeFixtureComparedCount']);
            $t->same(50, $executableParity['pandocNativeFixtureMatchCount']);
            $t->same(0, $executableParity['pandocNativeFixtureMismatchCount']);
            $t->same(50, $executableParity['pandocNativeFixtureByteComparedCount']);
            $t->same(50, $executableParity['pandocNativeFixtureByteMatchCount']);
            ...
            $t->contains('Executable/native parity: localPandoc=50/50 checkedNative=50/50 status=normalized-ast-equality-observed-against-pandoc-executable version=pandoc fake 3.10', $text);
...
HS);
    ...
    'cli gates checked-in current epub reader evidence through checked-in fixtures mode' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $repoRoot, $checkedInFixtureRoot, $expectedCurrentReaderStaticSignatureSha256, $expectedReferencedFixtureIdentity, $writeFakePandoc): void {
        ...
        try {
            ...
            $t->same(50, $decoded['nativeAstPackageParity']['normalizedAstMatchCount']);
            ...
            $t->same(50, $decoded['executableNativeAstParity']['normalizedAstMatchCount']);
            $t->same(50, $decoded['executableNativeAstParity']['pandocNativeFixtureMatchCount']);
