...
return [
    'reports skipped epub reader evidence when upstream root is absent' => static function (TestRunner $t) use ($makeTempDir, $removeTree): void {
        ...
        try {
            ...
            $t->contains('Native/package parity: package=0/34 nativeAst=0/34 status=not-evaluated-source-directory-unavailable', $text);
            $t->contains('Executable/native parity: localPandoc=0/34 checkedNative=0/34 status=not-evaluated-not-requested version=not-evaluated', $text);
    ...
    'validates checked-in current epub reader evidence through fixture base' => static function (TestRunner $t) use ($repoRoot, $checkedInFixtureRoot, $expectedCurrentReaderStaticSignatureSha256, $expectedReferencedFixtureIdentity): void {
        ...
        $t->same(34, $nativePackageParity['requiredEpubCount']);
        $t->same(34, $nativePackageParity['requiredPairCount']);
        $t->same(34, $nativePackageParity['totalEpubCount']);
        $t->same(34, $nativePackageParity['comparedEpubCount']);
        $t->same(34, $nativePackageParity['packageParsedCount']);
        $t->same(34, $nativePackageParity['readerParsedCount']);
        ...
        $t->same(34, $nativePackageParity['totalPairCount']);
        $t->same(34, $nativePackageParity['comparedPairCount']);
        $t->same(34, $nativePackageParity['bothParsedCount']);
        $t->same(0, $nativePackageParity['astParseFailureCount']);
        $t->same(0, $nativePackageParity['nativeParseFailureCount']);
        $t->same(34, $nativePackageParity['normalizedAstMatchCount']);
        ...
        $t->contains('Native/package parity: package=34/34 nativeAst=34/34 status=normalized-ast-equality-observed-not-runner-parity', $text);
        $t->same('not-evaluated', $report['executableNativeAstParity']['status']);
        $t->same('not-requested', $report['executableNativeAstParity']['reason']);
        $t->same(false, EpubUpstreamReaderEvidence::hasRequiredExecutableNativeAstParity($report));
        $t->same(true, EpubUpstreamReaderEvidence::hasNoValidationIssues($report));
    ...
    'validates checked-in current epub executable native ast parity with supplied pandoc' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $repoRoot, $checkedInFixtureRoot, $writeFakePandoc): void {
        ...
        try {
            ...
            $t->same(34, $executableParity['requiredEpubCount']);
            $t->same(34, $executableParity['totalEpubCount']);
            $t->same(34, $executableParity['comparedEpubCount']);
            $t->same(34, $executableParity['localParsedCount']);
            $t->same(34, $executableParity['pandocParsedCount']);
            $t->same(34, $executableParity['nativeFixtureParsedCount']);
            $t->same(34, $executableParity['bothParsedCount']);
            $t->same(0, $executableParity['parseFailureCount']);
            $t->same(34, $executableParity['normalizedAstMatchCount']);
            $t->same(0, $executableParity['normalizedAstMismatchCount']);
            $t->same(34, $executableParity['pandocNativeFixtureComparedCount']);
            $t->same(34, $executableParity['pandocNativeFixtureMatchCount']);
            $t->same(0, $executableParity['pandocNativeFixtureMismatchCount']);
            $t->same(34, $executableParity['pandocNativeFixtureByteComparedCount']);
            $t->same(34, $executableParity['pandocNativeFixtureByteMatchCount']);
            ...
            $t->contains('Executable/native parity: localPandoc=34/34 checkedNative=34/34 status=normalized-ast-equality-observed-against-pandoc-executable version=pandoc fake 3.10', $text);
...
HS);
    ...
    'cli gates checked-in current epub reader evidence through checked-in fixtures mode' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $repoRoot, $checkedInFixtureRoot, $expectedCurrentReaderStaticSignatureSha256, $expectedReferencedFixtureIdentity, $writeFakePandoc): void {
        ...
        try {
            ...
            $t->same(34, $decoded['nativeAstPackageParity']['normalizedAstMatchCount']);
            ...
            $t->same(34, $decoded['executableNativeAstParity']['normalizedAstMatchCount']);
            $t->same(34, $decoded['executableNativeAstParity']['pandocNativeFixtureMatchCount']);
