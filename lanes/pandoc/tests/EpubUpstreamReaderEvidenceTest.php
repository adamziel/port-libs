...
return [
    'reports skipped epub reader evidence when upstream root is absent' => static function (TestRunner $t) use ($makeTempDir, $removeTree): void {
        ...
        try {
            ...
            $t->contains('Native/package parity: package=0/35 nativeAst=0/35 status=not-evaluated-source-directory-unavailable', $text);
            $t->contains('Executable/native parity: localPandoc=0/35 checkedNative=0/35 status=not-evaluated-not-requested version=not-evaluated', $text);
    ...
    'validates checked-in current epub reader evidence through fixture base' => static function (TestRunner $t) use ($repoRoot, $checkedInFixtureRoot, $expectedCurrentReaderStaticSignatureSha256, $expectedReferencedFixtureIdentity): void {
        ...
        $t->same(35, $nativePackageParity['requiredEpubCount']);
        $t->same(35, $nativePackageParity['requiredPairCount']);
        $t->same(35, $nativePackageParity['totalEpubCount']);
        $t->same(35, $nativePackageParity['comparedEpubCount']);
        $t->same(35, $nativePackageParity['packageParsedCount']);
        $t->same(35, $nativePackageParity['readerParsedCount']);
        ...
        $t->same(35, $nativePackageParity['totalPairCount']);
        $t->same(35, $nativePackageParity['comparedPairCount']);
        $t->same(35, $nativePackageParity['bothParsedCount']);
        $t->same(0, $nativePackageParity['astParseFailureCount']);
        $t->same(0, $nativePackageParity['nativeParseFailureCount']);
        $t->same(35, $nativePackageParity['normalizedAstMatchCount']);
        ...
        $t->contains('Native/package parity: package=35/35 nativeAst=35/35 status=normalized-ast-equality-observed-not-runner-parity', $text);
        $t->same('not-evaluated', $report['executableNativeAstParity']['status']);
        $t->same('not-requested', $report['executableNativeAstParity']['reason']);
        $t->same(false, EpubUpstreamReaderEvidence::hasRequiredExecutableNativeAstParity($report));
        $t->same(true, EpubUpstreamReaderEvidence::hasNoValidationIssues($report));
    ...
    'validates checked-in current epub executable native ast parity with supplied pandoc' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $repoRoot, $checkedInFixtureRoot, $writeFakePandoc): void {
        ...
        try {
            ...
            $t->same(35, $executableParity['requiredEpubCount']);
            $t->same(35, $executableParity['totalEpubCount']);
            $t->same(35, $executableParity['comparedEpubCount']);
            $t->same(35, $executableParity['localParsedCount']);
            $t->same(35, $executableParity['pandocParsedCount']);
            $t->same(35, $executableParity['nativeFixtureParsedCount']);
            $t->same(35, $executableParity['bothParsedCount']);
            $t->same(0, $executableParity['parseFailureCount']);
            $t->same(35, $executableParity['normalizedAstMatchCount']);
            $t->same(0, $executableParity['normalizedAstMismatchCount']);
            $t->same(35, $executableParity['pandocNativeFixtureComparedCount']);
            $t->same(35, $executableParity['pandocNativeFixtureMatchCount']);
            $t->same(0, $executableParity['pandocNativeFixtureMismatchCount']);
            $t->same(35, $executableParity['pandocNativeFixtureByteComparedCount']);
            $t->same(35, $executableParity['pandocNativeFixtureByteMatchCount']);
            ...
            $t->contains('Executable/native parity: localPandoc=35/35 checkedNative=35/35 status=normalized-ast-equality-observed-against-pandoc-executable version=pandoc fake 3.10', $text);
...
HS);
    ...
    'cli gates checked-in current epub reader evidence through checked-in fixtures mode' => static function (TestRunner $t) use ($makeTempDir, $removeTree, $repoRoot, $checkedInFixtureRoot, $expectedCurrentReaderStaticSignatureSha256, $expectedReferencedFixtureIdentity, $writeFakePandoc): void {
        ...
        try {
            ...
            $t->same(35, $decoded['nativeAstPackageParity']['normalizedAstMatchCount']);
            ...
            $t->same(35, $decoded['executableNativeAstParity']['normalizedAstMatchCount']);
            $t->same(35, $decoded['executableNativeAstParity']['pandocNativeFixtureMatchCount']);
