...
return [
    'reads docx package body metadata notes headers footers and review spans into shared ast' => static function (TestRunner $t): void {
        ...
        $zip->addFromString('word/styles.xml', '<?xml version="1.0"?><w:styles xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:style w:type="paragraph" w:styleId="Heading1"><w:name w:val="heading 1"/><w:pPr><w:outlineLvl w:val="0"/></w:pPr></w:style><w:style w:type="paragraph" w:styleId="BodyText"><w:name w:val="Body Text"/></w:style><w:style w:type="character" w:styleId="StrongStyle"><w:name w:val="Strong Style"/><w:rPr><w:b/></w:rPr></w:style><w:style w:type="character" w:styleId="EmphasisStyle"><w:name w:val="Emphasis Style"/><w:basedOn w:val="StrongStyle"/><w:rPr><w:i/></w:rPr></w:style></w:styles>');
        ...
        $t->contains('<p data-docx-style-id="BodyText" data-docx-style-name="Body Text" data-docx-style-type="paragraph" class="docx-style-bodytext docx-paragraph-style">', $blocks);
    ...
    'preserves docx comment ranges moves table merges styles and image metadata' => static function (TestRunner $t): void {
        ...
        $zip->addFromString('word/media/chart.png', $image);
        ...
        $imageNode = $document->children[3]->children[0];
        ...
        $t->same('Chart alt', $imageNode->attr('alt'));
        $t->same('Chart title', $imageNode->attr('title'));
        $t->same('2in', $imageNode->attr('width'));
        $t->same('1in', $imageNode->attr('height'));
        $t->same('Chart 1', $imageNode->attr('attributes')['data-docx-image-name']);
        $t->same('9', $imageNode->attr('attributes')['data-docx-image-id']);
