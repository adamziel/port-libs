...
<html xmlns="http://www.w3.org/1999/xhtml" xmlns:epub="http://www.idpf.org/2007/ops">
  <body>
    ...
    <nav epub:type="page-list">
      <ol>
        <li><a href="java&#x20;script:alert(1)">Script action</a></li>
        <li><a href="../../outside.xhtml#outside">Package escape</a></li>
        <li><a href="chapter.xhtml#page-2">Page 2</a></li>
      </ol>
    </nav>
  </body>
</html>
XML);
            ...
            $t->same('EPUB/nav.xhtml', $report['part']);
            $t->same(3, $report['sectionCount']);
            $t->same(['toc' => 1, 'landmarks' => 1, 'page-list' => 1], $report['sectionTypeCounts']);
            $t->same(8, $report['entryCount']);
            $t->same(8, $report['targetedItemCount']);
            $t->same(2, $report['localTargetCount']);
            $t->same(2, $report['safeLocalTargetCount']);
            $t->same(6, $report['externalTargetCount']);
            $t->same(2, $report['unsafeTargetCount']);
            $t->same(8, $report['diagnosticCount']);
            $t->same(['toc', 'landmarks', 'page-list'], array_column($report['sections'], 'type'));
            $t->same([3, 2, 3], array_column($report['sections'], 'entryCount'));
            $t->same([2, 2, 2], array_column($report['sections'], 'externalTargetCount'));
            $t->same([0, 1, 1], array_column($report['sections'], 'unsafeTargetCount'));
            ...
            $t->same(['external-nav-href-target'], array_column($entriesByLabel['HTTP resource']['diagnostics'], 'type'));
            $t->same('https', $entriesByLabel['HTTPS resource']['hrefScheme']);
            $t->same(['external-nav-href-target'], array_column($entriesByLabel['HTTPS resource']['diagnostics'], 'type'));
            ...
            $t->same(['external-nav-href-target', 'unsafe-nav-href-target'], array_column($entriesByLabel['Data payload']['diagnostics'], 'type'));
            ...
            $t->same(['external-nav-href-target', 'unsafe-nav-href-target'], array_column($entriesByLabel['Script action']['diagnostics'], 'type'));
