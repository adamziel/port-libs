...
return [
    'keeps DOCX parity evidence bounded to defensible coverage' => static function (TestRunner $t) use ($repoRoot, $readJson, $readText, $countTrackedFiles): void {
        ...
        $generalCacheStart = strpos($workflow, '- name: Restore pinned Pandoc DOCX upstream cache');
        $targetedRunnerStart = strpos($workflow, '- name: Hydrate pinned Pandoc DOCX runner checkout');
        $targetedRunnerEnd = strpos($workflow, '- name: Set up Haskell for targeted DOCX runner');
        $t->true($generalCacheStart !== false, 'DOCX CI workflow must restore and hydrate the general upstream cache');
        $t->true($targetedRunnerStart !== false, 'DOCX CI workflow must hydrate the targeted runner checkout');
        $t->true($targetedRunnerEnd !== false, 'DOCX CI workflow must keep the targeted runner setup step after checkout hydration');
        $generalCacheBlock = substr($workflow, (int) $generalCacheStart, (int) $targetedRunnerStart - (int) $generalCacheStart);
        $targetedRunnerBlock = substr($workflow, (int) $targetedRunnerStart, (int) $targetedRunnerEnd - (int) $targetedRunnerStart);
        ...
        $t->contains('uses: actions/cache@v4', $generalCacheBlock);
        $t->contains('key: pandoc-docx-upstream-sparse-v3-${{ runner.os }}-612e143fbe6d735b612c4800d21e61b7d44e4dca', $generalCacheBlock);
        $t->contains('git -C "$target" sparse-checkout set \\', $generalCacheBlock);
        $t->contains('/cabal.project \\', $generalCacheBlock);
        $t->contains('/pandoc.cabal \\', $generalCacheBlock);
        $t->contains('/test/test-pandoc.hs \\', $generalCacheBlock);
        $t->contains('/test/lalune.jpg \\', $generalCacheBlock);
        $t->contains('/test/Tests/Readers/Docx.hs \\', $generalCacheBlock);
        $t->contains('/test/Tests/Writers/Docx.hs \\', $generalCacheBlock);
        $t->contains('/test/docx \\', $generalCacheBlock);
        $t->contains('/data/docx', $generalCacheBlock);
        $t->true(!str_contains($generalCacheBlock, 'data/default.docx'), 'DOCX CI sparse cache must use the current data/docx template directory, not legacy data/default.docx');
        $t->contains('git -C "$target" fetch --depth=1 --filter=blob:none origin 612e143fbe6d735b612c4800d21e61b7d44e4dca', $generalCacheBlock);
        $t->contains('test "$(git -C "$target" rev-parse HEAD)" = "612e143fbe6d735b612c4800d21e61b7d44e4dca"', $generalCacheBlock);
        $t->contains('test -f "$target/test/lalune.jpg"', $generalCacheBlock);
        $t->contains('test -d "$target/test/docx"', $generalCacheBlock);
        $t->contains('test -d "$target/data/docx"', $generalCacheBlock);
        $t->contains('test -f "$target/data/docx/[Content_Types].xml"', $generalCacheBlock);
        ...
        $t->same(false, is_file($repoRoot . DIRECTORY_SEPARATOR . 'PANDOC_STATUS.md'), 'PANDOC_STATUS.md must stay deleted; DOCX parity evidence lives in JSON status files');
        $t->contains('test:test-pandoc', (string) ($runnerPlan['runnerTarget'] ?? ''));
        $t->contains('--write-selected-inventory .port-libs/pandoc-runner/artifacts/docx-targeted-run/selected-test-inventory.json', (string) ($runnerPlan['selectedTestInventoryArtifact']['command'] ?? ''));
        $t->contains('--write-result-artifact .port-libs/pandoc-runner/artifacts/docx-targeted-run/result.json', (string) ($runnerPlan['manualWorkflow']['resultWriterCommand'] ?? ''));
        $t->contains('--validate-result-artifacts --artifact-root .port-libs/pandoc-runner/artifacts/docx-targeted-run --log-root .port-libs/pandoc-runner/logs', (string) ($runnerPlan['resultArtifactGate']['command'] ?? ''));
        $t->contains('run_upstream_docx_tasty', $workflow);
        $t->contains('placeholder runner transcripts remain a blocking hard evidence gap', (string) ($runnerPlan['resultArtifactGate']['checks'][5] ?? ''));
        $t->same('PortLibs\\Pandoc\\DocxWriter', $manifestAudit['docxWriterImplementation']['expectedClass'] ?? null);
        $t->same(38, $manifestAudit['writerGoldenPackageGeneration']['generatedPackageCount'] ?? null);
        $t->same(38, $manifestAudit['writerGoldenPackageComparison']['comparedPackageCount'] ?? null);
        $t->same(38, $manifestAudit['writerGoldenPackageComparison']['matchedPackageCount'] ?? null);
        $t->contains('UPSTREAM_DOCX_HASKELL_INVENTORY.json', (string) ($manifestAudit['haskellInventory']['reportPath'] ?? ''));
        $t->contains('36 reader uncovered cases and 7 writer direct HUnit uncovered cases', (string) ($manifestAudit['haskellInventory']['claim'] ?? ''));
        $t->contains('DocxWriterTest.php', (string) ($manifestAudit['focusedCiEvidenceWiring']['commands']['writerCoreTest'] ?? ''));
        $t->contains('--require-generated-stable-matches 38', (string) ($manifestAudit['focusedCiEvidenceWiring']['commands']['writerGoldenAudit'] ?? ''));
        $t->contains('not an upstream Haskell/Cabal/Tasty runner result', (string) ($manifestAudit['focusedCiEvidenceWiring']['claim'] ?? ''));
        $t->same(2, $manifestAudit['localCurrentUpstreamDocxPackageFixtureFiles'] ?? null);
        $t->same(0, $manifestAudit['localPinnedDocxPackageFixtureFiles'] ?? null);
        $t->same('partial', $manifestAudit['docxWriterImplementation']['outputRegistryStatus'] ?? null);
        $t->same('matched-stable-package-semantics', $manifestAudit['writerGoldenPackageComparison']['status'] ?? null);
        ...
        $jsonEvidence = json_encode([$manifestAudit, $statusAudit], JSON_THROW_ON_ERROR);
        $t->true(!str_contains($jsonEvidence, $staleZeroMatch), 'DOCX JSON status evidence must not retain stale writer-golden zero-match evidence');
        $t->true(!str_contains($jsonEvidence, $staleMismatchCount), 'DOCX JSON status evidence must not retain stale writer-golden mismatch evidence');
        $t->contains('UPSTREAM_DOCX_CACHE_MANIFEST.json', $jsonEvidence);
        $t->true((int) ($manifestAudit['localPinnedDocxPackageFixtureFiles'] ?? -1) === 0, 'checked-in pinned DOCX package corpus gap must remain explicit');
